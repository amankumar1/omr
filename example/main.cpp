...
int testMain(int argc, char **argv, char **envp) {
  ...
  while (true) {
    omrobjectptr_t obj = (omrobjectptr_t)OMR_GC_AllocateNoGC(
        omrVMThread, OMR_EXAMPLE_ALLOCATION_CATEGORY, allocSize,
        allocatedFlags);
    if (NULL != obj) {
  ...
  uintptr_t allocationTotalBytes = allocationStats->tlhBytesAllocated() +
  ...
  omrobjectptr_t obj = (omrobjectptr_t)OMR_GC_Allocate(
      omrVMThread, OMR_EXAMPLE_ALLOCATION_CATEGORY, allocSize, allocatedFlags);
