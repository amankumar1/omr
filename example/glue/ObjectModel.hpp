 ...
 * objects and are used in GC_ObjectModel::initializeAllocation() to determine
 * how to
...
/**
 * Define structure of object slot that is to be used to represent an object's
 * metadata. In this slot, one byte
 * must be reserved to hold flags and object age (4 bits age, 4 bits flags). The
 * remaining bytes in this slot may
 * be used by the client language for other purposes and will not be altered by
 * OMR.
 */
#define OMR_OBJECT_METADATA_SLOT_OFFSET                                        \
  0 /* fomrobject_t offset from object header address to metadata slot */
...
#define OMR_OBJECT_FLAGS(object)                                               \
  (*(OMR_OBJECT_METADATA_SLOT_EA(object)) & OMR_OBJECT_METADATA_FLAGS_MASK)
#define OMR_OBJECT_SIZE(object)                                                \
  (*(OMR_OBJECT_METADATA_SLOT_EA(object)) >> OMR_OBJECT_METADATA_SIZE_SHIFT)

#define OMR_OBJECT_METADATA_REMEMBERED_BITS OMR_OBJECT_METADATA_AGE_MASK
#define OMR_OBJECT_METADATA_REMEMBERED_BITS_TO_SET                             \
  0x10 /* OBJECT_HEADER_LOWEST_REMEMBERED */
#define OMR_OBJECT_METADATA_REMEMBERED_BITS_SHIFT OMR_OBJECT_METADATA_AGE_SHIFT

#define STATE_NOT_REMEMBERED 0
#define STATE_REMEMBERED                                                       \
  (OMR_OBJECT_METADATA_REMEMBERED_BITS_TO_SET &                                \
   OMR_OBJECT_METADATA_REMEMBERED_BITS)
...
class GC_ObjectModel {
   ...
   */
private:
  uintptr_t _objectAlignmentInBytes; /**< Cached copy of object alignment for
                                        getting object alignment for adjusting
                                        for alignment */
  uintptr_t _objectAlignmentShift;   /**< Cached copy of object alignment shift,
...
public:
   ...
   */
  MMINLINE void setFlags(omrobjectptr_t objectPtr, uintptr_t bitsToClear,
                         uintptr_t bitsToSet) {
    fomrobject_t *flagsPtr =
        (fomrobject_t *)OMR_OBJECT_METADATA_SLOT_EA(objectPtr);
  ...
  MMINLINE bool isDeadObject(void *objectPtr) {
    return J9_GC_OBJ_HEAP_HOLE ==
           (OMR_OBJECT_FLAGS(objectPtr) & J9_GC_OBJ_HEAP_HOLE);
  }
  ...
  MMINLINE bool isSingleSlotDeadObject(omrobjectptr_t objectPtr) {
    return J9_GC_SINGLE_SLOT_HOLE ==
           (OMR_OBJECT_FLAGS(objectPtr) & J9_GC_OBJ_HEAP_HOLE_MASK);
  }
  ...
  getConsumedSizeInSlotsWithHeader(omrobjectptr_t objectPtr) {
    return MM_Bits::convertBytesToSlots(
        getConsumedSizeInBytesWithHeader(objectPtr));
  }
  ...
  MMINLINE uintptr_t getSizeInBytesWithHeader(omrobjectptr_t objectPtr) {
    return OMR_OBJECT_SIZE(objectPtr);
  }

  MMINLINE void setObjectSize(omrobjectptr_t objectPtr, uintptr_t size,
                              bool preserveAgeAndFlags = true) {
    fomrobject_t *header = OMR_OBJECT_METADATA_SLOT_EA(objectPtr);
    fomrobject_t ageAndFlags =
        preserveAgeAndFlags ? OMR_OBJECT_FLAGS(objectPtr) : 0;
    *header =
        ((fomrobject_t)size << OMR_OBJECT_METADATA_SIZE_SHIFT) | ageAndFlags;
  }
   ...
   * from reference graph (eg, heap object bound to associated class)
   * @param vmThread Pointer to requesting thread
   * @param objectPtr Pointer to an object
   * @return true if object holds indirect references to heap objects that may
   * be in new space
   */
  MMINLINE bool hasIndirectObjectReferents(void *vmThread,
   ...
   */
  MMINLINE uint32_t getRealAge(omrobjectptr_t objectPtr) {
   ...
   * @param objectPtr Pointer to an object
   */
  MMINLINE void setObjectCurrentlyReferenced(omrobjectptr_t objectPtr) {
   ...
   * @param objectPtr Pointer to an object
   * @return true if OMR_TENURED_STACK_OBJECT_CURRENTLY_REFERENCED bit has been
   * set this call
   */
  MMINLINE bool atomicSetObjectCurrentlyReferenced(omrobjectptr_t objectPtr) {
    bool result = true;

    volatile fomrobject_t *flagsPtr =
        (fomrobject_t *)OMR_OBJECT_METADATA_SLOT_EA(objectPtr);
  ...
  atomicSetObjectCurrentlyReferencedAndRemembered(omrobjectptr_t objectPtr) {
    ...
    if (result && ((oldFlags & OMR_OBJECT_METADATA_REMEMBERED_BITS) >=
                   STATE_REMEMBERED)) {
      result = false;
    }
   ...
   * @return true, if Remembered bit has been set this call
   */
  MMINLINE bool atomicSetRemembered(omrobjectptr_t objectPtr) {
    bool result = true;
    ...
    do {
      ...
      newFlags =
          (oldFlags & ~OMR_OBJECT_METADATA_REMEMBERED_BITS) | STATE_REMEMBERED;
    }
#if defined(OMR_INTERP_COMPRESSED_OBJECT_HEADER)
    while (oldFlags != MM_AtomicOperations::lockCompareExchangeU32(
                           flagsPtr, oldFlags, newFlags));
   ...
   *
   * This method will assert if the object is not indexable or has been marked
   * as forwarded.
   *
   * @param[in] forwardedHeader pointer to the MM_ForwardedHeader instance
   * encapsulating the object
  ...
  /**
   * Returns TRUE if an unforwarded object has been moved after being hashed,
   * FALSE otherwise.
   *
   * This method will assert if the object has been marked as forwarded.
   *
   ...
   * encapsulating the object
   * @return TRUE if an object has been moved after being hashed, FALSE
   * otherwise
   */
  MMINLINE bool hasBeenMoved(MM_ForwardedHeader *forwardedHeader) {
    return false;
  }
   ...
   */
  MMINLINE bool hasBeenHashed(MM_ForwardedHeader *forwardedHeader) {
    return false;
  }
   ...
   */
  MMINLINE bool isHasBeenHashedBitSet(MM_ForwardedHeader *forwardedHeader) {
    return false;
  }
   ...
   * otherwise
   */
  MMINLINE bool hasBeenRecentlyMoved(MM_ForwardedHeader *forwardedHeader) {
    return false;
  }

  /**
   * Returns TRUE if an unforwarded object's has been hashed bit is set, FALSE
   * otherwise.
   *
   * This method will assert if the object has been marked as forwarded.
   *
   ...
   */
  MMINLINE bool isObjectJustHasBeenMoved(MM_ForwardedHeader *forwardedHeader) {
    return false;
  }
   ...
   * @return TRUE if an object's has been hashed bit set, FALSE otherwise
   */
  MMINLINE bool isObjectJustHasBeenMoved(omrobjectptr_t objectPtr) {
    return false;
  }
   ...
   *
   * This method will assert if the object has been marked as forwarded. The
   * calculated object size
   * includes expansion slot for hash code if the object has been hashed or
   * moved.
   *
   * @param[in] forwardedHeader pointer to the MM_ForwardedHeader instance
   * encapsulating the object
   ...
   */
  MMINLINE void
  calculateObjectDetailsForCopy(MM_ForwardedHeader *forwardedHeader,
    ...
    *objectCopySizeInBytes =
        forwardedHeader->getPreservedSlot() >> OMR_OBJECT_METADATA_SIZE_SHIFT;
   ...
   * any damaged
   * caused by installing the forwarding pointer into the original prior to the
   * copy. This will
   * install the correct (i.e. unforwarded) class pointer, update the
   * hashed/moved flags and
   * install the hash code if the object has been hashed but not previously
   * moved.
   *
   * @param[in] forwardedHeader pointer to the MM_ForwardedHeader instance
   * encapsulating the object
  ...
  MMINLINE void fixupForwardedObject(MM_ForwardedHeader *forwardedHeader,
                                     ...
                                     uintptr_t objectAge) {
    /* Copy the preserved fields from the forwarded header into the destination
     * object */
   ...
   * Function exists because we can only determine it is way after ObjectModel
   * is init
   */
  MMINLINE void setObjectAlignmentInBytes(uintptr_t objectAlignmentInBytes) {
   ...
   * is init
   */
  MMINLINE void setObjectAlignmentShift(uintptr_t objectAlignmentShift) {
