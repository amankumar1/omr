 ...
 * omr_ras_sampleStackTraceStart()
 * with the frame's method key. For each successive stack frame, call
 * omr_ras_sampleStackTraceContinue()
 * with the frame's method key.
 *
 * The method key must be the same as the key value that was used to insert the
 * method in the method dictionary
 * by omr_ras_insertMethodDictionary(). The context represents a
 * language-specific data structure which
 ...
 * currently running
 * thread's stack. If a JIT is implemented, then JITted code may also need to
 * call this function
 * periodically.
 *
 * In this example, a backoff counter is used to control the sampling frequency
 * and restrict the
 * overhead of callstack sampling. The context parameter represents a
 ...
 * methods that
 * are sampled (see omr_ras_sampleStackTraceStart(),
 * omr_ras_sampleStackTraceContinue()).
 * The method parameter represents a language-specific data structure which
 * contains the
