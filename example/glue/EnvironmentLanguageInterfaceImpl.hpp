...
class MM_EnvironmentLanguageInterfaceImpl
    : public MM_EnvironmentLanguageInterface {
private:
protected:
public:
private:
...
public:
  ...
  virtual void acquireExclusiveVMAccess() {
    if (0 == _omrThread->exclusiveCount) {
      ...
      /* unconditionally acquire exclusive VM access by locking the VM thread
       * list mutex */
  ...
  virtual bool tryAcquireExclusiveVMAccess() {
    if (0 == _omrThread->exclusiveCount) {
      ...
      /* tell the rest of the world that a thread is going for exclusive VM<
       * access */
      uintptr_t vmExclusiveAccessCount =
          MM_AtomicOperations::add(&exampleVM->_vmExclusiveAccessCount, 1);

      /* try to acquire exclusive VM access by locking the VM thread list mutex
       */
      if ((1 != vmExclusiveAccessCount) ||
          (0 != omrthread_rwmutex_try_enter_write(exampleVM->_vmAccessMutex))) {
