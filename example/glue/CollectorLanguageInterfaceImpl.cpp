...
/* This enum extends ConcurrentStatus with values > CONCURRENT_ROOT_TRACING.
 * Values from this
 * and from ConcurrentStatus are treated as uintptr_t values everywhere except
 * when used as
 ...
 * call
 * MM_CollectorLanguageInterfaceImpl::concurrentGC_collectRoots(...,
 * concurrentStatus, ...)
 * only once with each client-defined status value. The thread that receives the
 * call
...
enum {
  CONCURRENT_ROOT_TRACING1 =
      ((uintptr_t)((uintptr_t)CONCURRENT_ROOT_TRACING + 1))
};
...
void MM_CollectorLanguageInterfaceImpl::flushNonAllocationCaches(
    MM_EnvironmentBase *env) {
#if defined(OMR_GC_MODRON_SCAVENGER)
  MM_EnvironmentStandard *envStd = MM_EnvironmentStandard::getEnvironment(env);
  MM_SublistFragment::flush(
      (J9VMGC_SublistFragment *)&envStd->_scavengerRememberedSet);
#endif /* defined(OMR_GC_MODRON_STANDARD) */
}

OMR_VMThread *MM_CollectorLanguageInterfaceImpl::attachVMThread(
...
void MM_CollectorLanguageInterfaceImpl::detachVMThread(
...
void MM_CollectorLanguageInterfaceImpl::markingScheme_masterSetupForGC(
    MM_EnvironmentBase *env) {}
...
void MM_CollectorLanguageInterfaceImpl::markingScheme_completeMarking(
    MM_EnvironmentBase *env) {}

void MM_CollectorLanguageInterfaceImpl::markingScheme_markLiveObjectsComplete(
    MM_EnvironmentBase *env) {}

void MM_CollectorLanguageInterfaceImpl::markingScheme_masterSetupForWalk(
    MM_EnvironmentBase *env) {}

void MM_CollectorLanguageInterfaceImpl::markingScheme_masterCleanupAfterGC(
    MM_EnvironmentBase *env) {
  OMR_VM_Example *omrVM = (OMR_VM_Example *)env->getOmrVM()->_language_vm;
  J9HashTableState state;
...
uintptr_t MM_CollectorLanguageInterfaceImpl::markingScheme_scanObject(
...
#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
uintptr_t MM_CollectorLanguageInterfaceImpl::markingScheme_scanObjectWithSize(
    MM_EnvironmentBase *env, omrobjectptr_t objectPtr,
...
void MM_CollectorLanguageInterfaceImpl::parallelDispatcher_handleMasterThread(
    OMR_VMThread *omrVMThread) {
  /* Do nothing for now.  only required for SRT */
}

#if defined(OMR_GC_MODRON_SCAVENGER)
void MM_CollectorLanguageInterfaceImpl::scavenger_reportObjectEvents(
    MM_EnvironmentBase *env) {
  /* Do nothing for now */
}
...
bool MM_CollectorLanguageInterfaceImpl::
   ...
   * object references that are live
   * but not reachable by traversing the reference graph from the root set or
   * remembered set. Otherwise this
...
bool MM_CollectorLanguageInterfaceImpl::scavenger_scavengeIndirectObjectSlots(
   ...
   * that case, this method should
   * call MM_Scavenger::copyObjectSlot(..) for each such indirect object
   * reference, ORing the boolean result
   * from each call to MM_Scavenger::copyObjectSlot(..) into a single boolean
...
void MM_CollectorLanguageInterfaceImpl::scavenger_backOutIndirectObjectSlots(
   ...
   * by traversing the reference graph from the root set or remembered set. In
   * that case, this method should
   * call MM_Scavenger::backOutFixSlotWithoutCompression(..) for each
   * uncompressed slot holding a reference to
...
void MM_CollectorLanguageInterfaceImpl::scavenger_reverseForwardedObject(
  ...
  if (forwardedHeader->isForwardedPointer()) {
    ...
    _extensions->objectModel.setObjectSize(
        originalObject,
        _extensions->objectModel.getSizeInBytesWithHeader(forwardedObject));
    _extensions->objectModel.setFlags(originalObject,
                                      OMR_OBJECT_METADATA_FLAGS_MASK,
                                      OMR_OBJECT_FLAGS(forwardedObject));

#if defined(OMR_INTERP_COMPRESSED_OBJECT_HEADER)
    /* Restore destroyed overlapped slot in the original object. This slot might
     * need to be reversed
     * as well or it may be already reversed - such fixup will be completed at
     * in a later pass.
     */
...
void MM_CollectorLanguageInterfaceImpl::scavenger_fixupDestroyedSlot(
   ...
   * stored in a compressed slot. in that
   * case the other half of the full (omrobjectptr_t sized) slot may hold a
   * compressed object reference that
  ...
  if ((0 != forwardedHeader->getPreservedOverlap()) &&
    ...
    if (0 == ((uintptr_t)survivingCopyAddress &
      ...
      if (subSpaceNew->isObjectInNewSpace(survivingCopyAddress, topOfObject) ||
        ...
        MM_ForwardedHeader reverseForwardedHeader(
            survivingCopyAddress, OMR_OBJECT_METADATA_SLOT_OFFSET);
...
MM_CollectorLanguageInterfaceImpl::concurrentGC_createSafepointCallback(
    MM_EnvironmentBase *env) {
  MM_EnvironmentStandard *envStd = MM_EnvironmentStandard::getEnvironment(env);
  return MM_ConcurrentSafepointCallback::newInstance(envStd);
}

uintptr_t MM_CollectorLanguageInterfaceImpl::concurrentGC_getNextTracingMode(
...
uintptr_t MM_CollectorLanguageInterfaceImpl::concurrentGC_collectRoots(
    MM_EnvironmentStandard *env, uintptr_t concurrentStatus,
...
omrobjectptr_t
MM_CollectorLanguageInterfaceImpl::heapWalker_heapWalkerObjectSlotDo(
