...
class MM_MarkVLHGCStatsCore : public MM_Base {
  /* data members */
private:
  uint64_t _scanTime; /**< The amount of time spent scanning by the owning
                         thread (or globally) during marking, in hi-res timer
                         resolution */
...
public:
  ...
  uintptr_t
      _bytesScanned; /**< The number of bytes scanned by the owning thread (or
                        globally) during marking */
...
public:
   ...
   * timer resolution.
   * For the global stats structure, this is the sum of time spent by all
   * threads.
