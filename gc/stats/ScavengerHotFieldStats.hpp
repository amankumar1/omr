...
#define ABS_UINTPTR_DISTANCE(a, b)                                             \
  ((uintptr_t)((a) < (b) ? (b) - (a) : (a) - (b)))

/**
 * Storage for hot field statistics relevant to a scavenging (semi-space
 * copying) collector.
 * @ingroup GC_Stats
 */
...
public:
  enum ScavengerObjectConnectorType {
    ...
    SizeScavengerObjectConnectorType /* how large an array to allocate to index
                                        connection types */
  };
  ...
  /* the following fields are stored to eliminate overhead measured at 3% of
   * scavenge times
   * if the parameters are passed by argument */
  omrobjectptr_t _objectPtr; /**< current object being scanned, parent of
                                current object being copied */
private:
  GC_ObjectModel
      *_objectModel; /**< pointer to object model (from GC extensions) */

  uint8_t _hotness; /**< hotness of current slot being scanned */

  /**
   * count of connections between parent and child objects
   * categorized by hotness and by connector type (nursery-nursery,
   * nursery-tenured, tenured-tenured)
   */
  uintptr_t _connectionCount[SizeScavengerHotness]
                            [SizeScavengerObjectConnectorType];
 ...
 */
  ...
  uint64_t _interObjectDistance[SizeScavengerHotness]
                               [SizeScavengerObjectConnectorType];
#else
  double _interObjectDistance[SizeScavengerHotness]
                             [SizeScavengerObjectConnectorType];
#endif
  /**
   * histogram of logarithmic distance between parent and child objects
   * categorized by hotness and by connector type
   * histogram bin boundaries are powers of 2, hence uintptr_t*8 bins are
   * required.
   */
  uintptr_t _connectionHistgm[sizeof(uintptr_t) * 8][SizeScavengerHotness]
                             [SizeScavengerObjectConnectorType];

public:
  /**
   * When no information is known about the hotness of a field, then the default
   * is hot.
   * This clears to this default value
   */
  MMINLINE void clearHotnessOfField() { _hotness = Hot; }
  ...
  void mergeStats(MM_ScavengerHotFieldStats *statsToMerge) {
    for (uintptr_t i = 0; i < SizeScavengerHotness; i++) {
      for (uintptr_t j = 0; j < SizeScavengerObjectConnectorType; j++) {
        ...
        for (uintptr_t k = 0; k < sizeof(uintptr_t) * 8; k++) {
          _connectionHistgm[k][i][j] +=
              statsToMerge->_connectionHistgm[k][i][j];
        }
      }
    }
  }
  ...
  /**
   * Determines the hotness of a reference slot from the
   * instanceHotFieldDescription
  ...
  void setHotnessOfField(fomrobject_t *slotPtr, uintptr_t hotFieldsDescriptor) {
    ...
    if ((NULL == _objectPtr) || _objectModel->isIndexable(_objectPtr)) {
      /* did not have a parent or cannot calculate hotness here, can only assume
       * it is hot */
      clearHotnessOfField();
    } else {
      /* get the slot index into the hot bit vector */
      fomrobject_t *startOfObjectAfterHeader = (fomrobject_t *)(_objectPtr + 1);
      /* NOTE: pointer math means that slotIndex is calculated in terms of
       * sizeof(fomrobject_t) */
      uintptr_t slotIndex = (uintptr_t)(slotPtr - startOfObjectAfterHeader);

      /* x >> y is undefined if y >= #bits in word, but we know it should be
       * zero! This feature of right shift caught me out! */
  ...
  /**
   * Update hot statistics for this connection. Assumes that
   * initializeConnection has been
   ...
   */
  void updateStats(bool isParentInNewSpace, bool isChildInNewSpace,
                   omrobjectptr_t childPtr) {
    if (NULL == _objectPtr) {
    ...
    uintptr_t distance =
        ABS_UINTPTR_DISTANCE((uintptr_t)_objectPtr, (uintptr_t)childPtr);
    uintptr_t bin = 0;
    if (0 != distance) {
