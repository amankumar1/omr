...
bool MM_FreeEntrySizeClassStats::initialize(MM_EnvironmentBase *env,
  ...
  if (0 != _maxSizeClasses) {
    _count = (uintptr_t *)env->getForge()->allocate(
        sizeof(uintptr_t) * _maxSizeClasses, MM_AllocationCategory::FIXED,
        OMR_GET_CALLSITE());
    ...
    if (0 != _maxFrequentAllocateSizes) {
      ...
      if (veryLargeObjectThreshold > env->getExtensions()->memoryMax) {
        ...
        _veryLargeEntryPool = (FrequentAllocation *)env->getForge()->allocate(
            sizeof(FrequentAllocation) * count, MM_AllocationCategory::FIXED,
            OMR_GET_CALLSITE());
...
MM_LargeObjectAllocateStats *MM_LargeObjectAllocateStats::newInstance(
  ...
  largeObjectAllocateStats =
      (MM_LargeObjectAllocateStats *)env->getForge()->allocate(
          sizeof(MM_LargeObjectAllocateStats), MM_AllocationCategory::FIXED,
          OMR_GET_CALLSITE());
...
bool MM_LargeObjectAllocateStats::initialize(
  ...
  if (NULL == (_spaceSavingSizes =
                   spaceSavingNew(_portLibrary, _maxAllocateSizes * 2))) {
    return false;
  }
   ...
   * way earlier;
   * but at the point where most of extension fields are initialized,
   * maxHeapSize is not known yet
   */
...
MM_LargeObjectAllocateStats::estimateFragmentation(MM_EnvironmentBase *env) {
   ...
   * probablistic size decay of very large free entries).
   * stop up until we cannot satisfy allocate or remaining free memory is less
   * than 1% of initial free memory.
  ...
  for (; (0 == unsatisfiedAlloc) &&
    ...
    for (uintptr_t i = 0;
      ...
      if (objectPercent >= ignoreObjectPercentThreshold) {
         ...
         * total TLH allocates (10% of other large object allocations)
         * in this stride we will simulate allocation of 30 / 10 = 3% for the
         * large object and  30/(30+10) x 60 / 10 = 4.5%
         * (tlhAllocBytesPerObjectPerObjectStride) for TLHs.
         * TLHs allocations will be further divided into several strides
         * (allocTLHStridesInteger), where each TLH allocation
...
uintptr_t MM_LargeObjectAllocateStats::simulateAllocateObjects(
  ...
  while ((allocBytes > 0) &&
    ...
    if (0 !=
        ext->freeEntrySizeClassStatsSimulated.getFrequentAllocCount(
            sizeClassIndex)) {
      /* for each size class, find frequent allocation sizes */
      MM_FreeEntrySizeClassStats::FrequentAllocation *curr =
          ext->freeEntrySizeClassStatsSimulated
...
void MM_LargeObjectAllocateStats::averageForSpaceSaving(
   ...
   * will carry 9/10 weight (new weight is 1/10)
   * if bytesAllocatedThisRound = 9 x _averageBytesAllocated, the historic
   * average will will carry 1/2 weight (new weight is 1/2)
   * as bytesAllocatedThisRound / _averageBytesAllocated ratio approaches
   * infinity, the historic average weight will approach 0 (new weight
   * approaches 1)
   * if _averageBytesAllocated = 9 x bytesAllocatedThisRound, the historic
   * average weight will carry 99/100 weigth (new weight is 1/100)
   * as bytesAllocatedThisRound / _averageBytesAllocated approaches 0, the
   * historic average weight will approach 1 (new weight approaches 0)
   */
...
uintptr_t MM_LargeObjectAllocateStats::getSizeClassIndex(uintptr_t size) {
   ...
   * for a few RT Linux kernels. If one of this assertions is triggered call for
   * IT Support to check/update
