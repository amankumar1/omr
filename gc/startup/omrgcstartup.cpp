...
static omr_error_t heapCreationHelper(OMR_VM *omrVM,
   ...
   * thread
   * is done by calling out to MM_Configuration, which itself uses the
   * environment
   * to obtain pointers to MM_GCExtensionsBase and MM_Forge, etc. While this
   * circular
   * dependency could be eliminated, it would require changes to many
   * interfaces. */
  ...
  configurationLanguageInterface =
      startupManager->createConfigurationLanguageInterface(&envBase);
  ...
  extensions->configuration = startupManager->createConfiguration(
      &envBase, configurationLanguageInterface);
