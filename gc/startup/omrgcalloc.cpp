...
omrobjectptr_t OMR_GC_Allocate(OMR_VMThread *omrVMThread,
  ...
  uintptr_t adjustedFlags =
      allocateFlags & ~(uintptr_t)OMR_GC_ALLOCATE_OBJECT_NO_GC;

  omrobjectptr_t objectPtr = NULL;
  omr_error_t retry = OMR_ERROR_RETRY;
  while ((NULL == objectPtr) && (retry == OMR_ERROR_RETRY)) {
    MM_AllocateInitialization withGc(env, allocationCategory, adjustedSize,
                                     adjustedFlags);
    if (withGc.initializeAllocateDescription(env)) {
      objectPtr = withGc.allocateAndInitializeObject(omrVMThread);
      if ((NULL == objectPtr) &&
          (NULL == env->getExtensions()->getGlobalCollector())) {
...
omrobjectptr_t OMR_GC_AllocateNoGC(OMR_VMThread *omrVMThread,
  ...
  omrobjectptr_t objectPtr = NULL;
  MM_AllocateInitialization noGc(env, allocationCategory, adjustedSize,
                                 adjustedFlags);
