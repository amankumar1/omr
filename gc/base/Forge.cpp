 ...
 * pointer to the allocated memory, or NULL if the request could
 ...
 * be performed
 */
void *MM_Forge::allocate(uintptr_t bytesRequested,
                         MM_AllocationCategory::Enum category,
                         const char *callsite) {
  MM_AlignedMemoryHeader *memoryPointer;

  memoryPointer = (MM_AlignedMemoryHeader *)_portLibrary->mem_allocate_memory(
      _portLibrary, bytesRequested + sizeof(MM_AlignedMemoryHeader), callsite,
      OMRMEM_CATEGORY_MM);
 ...
 * function should not be called to deallocate memory that has
 * not been allocated by either the allocate or reallocate functions.  This
...
void MM_Forge::free(void *memoryPointer) {
  ...
  MM_AlignedMemoryHeader *alignedHeader =
      (MM_AlignedMemoryHeader *)memoryPointer;
  alignedHeader -= 1;

  omrthread_monitor_enter(_mutex);
  _statistics[alignedHeader->header.category].allocated -=
      alignedHeader->header.allocatedBytes;
  omrthread_monitor_exit(_mutex);
...
/**
 * Returns the current memory usage statistics for the garbage collector.  Each
 * entry in the array corresponds to a memory usage category type.
 * To locate memory usage statistics for a particular category, use the
 * enumeration value as the array index (e.g. stats[REFERENCES]).
 *
 * @return an array of memory usage statistics indexed using the
 * MM_AllocationCategory enumeration
 */
MM_MemoryStatistics *MM_Forge::getCurrentStatistics() { return _statistics; }
