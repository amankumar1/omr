...
void *MM_MemorySubSpaceGeneric::allocateObject(
   ...
   * (_allocateAtSafePointOnly is false),
   * so we most often proceed with allocate.
   * If we are notified that concurrent mark is complete (
   * _allocateAtSafePointOnly is true, and typically the stack frame is not
   * built yet,
   * we should not proceed with allocate, but return NULL and force JIT to call
   * us again with the frame built
   * Next time when the frame is built (shouldCollectOnFailure is true, while
   * _allocateAtSafePointOnly is still true) we proceed with allocate (otherwise
   * risk OOM),
 ...
 * another
 * during collection, may require that the destination area expand during the
 * collection
 * cycle.  An example of this would be the tenure area in a 2-generational
 * system, if the
...
MM_MemorySubSpaceGeneric *MM_MemorySubSpaceGeneric::newInstance(
  ...
  memorySubSpace = (MM_MemorySubSpaceGeneric *)env->getForge()->allocate(
      sizeof(MM_MemorySubSpaceGeneric), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
 ...
 * removed from the current memory space's
