 ...
 * memory.  It will not
 * consider deferred work that may be done to increase current free memory
 * stores.
 ...
 * high address.
 * This routines shoud find any available free entries that can be removed that
 * are within the given range,
 * ending at a particular range, all while satisfying the described allocation.
 * The allocDescription is passed
 * so that if the allocation type or size class has an effect on what can/cannot
 * be contracted, it can be factored
