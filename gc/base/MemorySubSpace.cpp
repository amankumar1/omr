...
/**
 * Get the sum of all free memory currently available for allocation in the
 * subspace and its children of the specified type.
 * This call will return an accurate count of the current size of all free
 * memory of the specified type.  It will not
 * consider defered work that may be done to increase current free memory
 * stores.
 *
 ...
 * subspace and its children of the specified type.
 * This call will return an estimated count of the current size of all free
 * memory of the specified type.  Although this
 * estimate may be accurate, it will consider defered work that may be done to
 ...
 * made available to the subspace
 * as free memory.
 ...
 * removed from the current memory spaces
...
bool MM_MemorySubSpace::heapAddRange(MM_EnvironmentBase *env,
  ...
  if (_parent) {
    result =
        result &&
        _parent->heapAddRange(env, subspace, size, lowAddress, highAddress);
  } else if (_memorySpace) {
    result = result &&
             _memorySpace->heapAddRange(env, subspace, size, lowAddress,
                                        highAddress);
  }
  return result;
}
...
bool MM_MemorySubSpace::heapRemoveRange(MM_EnvironmentBase *env,
  ...
  if (_parent) {
    result =
        result &&
        _parent->heapRemoveRange(env, subspace, size, lowAddress, highAddress,
                                 lowValidAddress, highValidAddress);
  } else if (_memorySpace) {
    result = result &&
             _memorySpace->heapRemoveRange(env, subspace, size, lowAddress,
                                           highAddress, lowValidAddress,
                                           highValidAddress);
  }
  return result;
 ...
 * resize. Latter these variables are read
 ...
 * other subspaces to fill minimum
 * quotas, etc.  The call assumes that the contract size is at least small
 * enough to fit into the locally available
 ...
 * (expanding) other subspaces to fill minimum
 * quotas, etc.
 * The generic implementation consists of limiting the contraction to set
 * limits.  Override to allow sibling of the
 ...
 * remaining quota with the expand request.  The expand request size can
...
uintptr_t MM_MemorySubSpace::counterBalanceContractWithExpand(
  ...
  if (NULL != _physicalSubArena) {
    ...
    if (maximumExpandSize < adjustedExpandSize) {
       ...
       * expand.  Adjust the contraction size to account
       * for the lack of an expand, and return the value.  The expandSizeDelta
       * can be increased through rounding to be
 ...
 * triggered when the current size adjustment
 * takes place.
 ...
 * based on size adjustments made by the
 * receiver.
 */
 ...
 * in failure cases after a counter
 * balance has been decided upon (the resulting size change is unacceptable by
 * the receiver).
 */
