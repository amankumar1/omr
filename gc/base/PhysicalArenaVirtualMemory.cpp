...
MM_PhysicalArenaVirtualMemory::newInstance(MM_EnvironmentBase *env,
  ...
  arena = (MM_PhysicalArenaVirtualMemory *)env->getForge()->allocate(
      sizeof(MM_PhysicalArenaVirtualMemory), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (arena) {
 ...
 * receiver.
 * This reserves the address space within the receiver for the subarena, and
 * connects the subarena to the list
 * of those associated to the receiver (in address order).  Commit the subarena
 * memory.
 *
 ...
 * address is.
 * This routine does not account for any internal sub arenas, but rather what
 * the theoretical maximum expansion is.
 ...
 * given address is.
 * This routine does not account for any internal sub arenas, but rather what
 * the theoretical maximum contraction is.
