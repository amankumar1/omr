...
MM_MemoryPoolBumpPointer::newInstance(MM_EnvironmentBase *env,
  ...
  MM_MemoryPoolBumpPointer *memoryPool =
      (MM_MemoryPoolBumpPointer *)env->getForge()->allocate(
          sizeof(MM_MemoryPoolBumpPointer), MM_AllocationCategory::FIXED,
          OMR_GET_CALLSITE());
  if (NULL != memoryPool) {
    memoryPool =
...
MMINLINE bool MM_MemoryPoolBumpPointer::internalAllocateTLH(
  ...
  if (_minimumFreeEntrySize <= spaceRemaining) {
    ...
    if (newFreeSpace < _minimumFreeEntrySize) {
       ...
       * the requested size.  If we were going to discard
...
MMINLINE void
MM_MemoryPoolBumpPointer::internalRecycleHeapChunk(void *addrBase,
                                                   void *addrTop) {
