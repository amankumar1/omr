...
MM_TLHAllocationInterface::newInstance(MM_EnvironmentBase *env) {
  ...
  allocationInterface = (MM_TLHAllocationInterface *)env->getForge()->allocate(
      sizeof(MM_TLHAllocationInterface), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
 ...
 * such that the existing cache is
 * no longer valid.  An example of this is a change of memory space.  Perform
 * the necessary flushing and
 * re-initialization of the cache details such that new allocations will occur
 ...
 * For this implementation we simply redirect back to the supplied memory pool
 * and if successful, populate the localized TLH and supplied
...
void MM_TLHAllocationInterface::reconnectCache(MM_EnvironmentBase *env) {
  uintptr_t vmState;

  if (!_owningEnv->_envLanguageInterface->isInlineTLHAllocateEnabled()) {
    /* Only need to reset heapAlloc and realHeapAlloc if there is an existing
     * TLH to reset */
    _owningEnv->_envLanguageInterface->enableInlineTLHAllocate();
  }
