...
protected:
  ...
  uint32_t _next; /**< tagged pointer to the next free list entry, or a tagged
                     pointer to NULL */
...
private:
   ...
   * functions
   * due to a bug in an old version of GCC used on the HardHat Linux platforms.
   * These
   * functions can't be in-lined if they're not declared before the point
   * they're used.
   */
private:
  /**
   ...
   */
  MMINLINE uintptr_t getNextImpl() {
#if defined(SPLIT_NEXT_POINTER)
   ...
   */
  MMINLINE void setNextImpl(uintptr_t value) {
#if defined(SPLIT_NEXT_POINTER)
...
public:
  ...
  MMINLINE MM_HeapLinkedFreeHeader *getNext() {
    ...
    return (MM_HeapLinkedFreeHeader *)((getNextImpl()) &
  ...
  MMINLINE void updateNext(MM_HeapLinkedFreeHeader *freeEntryPtr) {
    setNextImpl(((uintptr_t)freeEntryPtr) |
                (getNextImpl() & (uintptr_t)J9_GC_OBJ_HEAP_HOLE_MASK));
  }
   ...
   */
  MMINLINE static void fillWithSingleSlotHoles(void *addrBase,
