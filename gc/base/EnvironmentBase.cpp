...
bool MM_EnvironmentBase::initialize(MM_GCExtensionsBase *extensions) {
  ...
  if (extensions->isStandardGC()) {
     ...
     * veryLargeEntryPool for thread (to reduce footprint),
     * but if the threshold is bigger than maxHeap size, we would pass orignal
     * threshold to indicate no veryLargeEntryPool needed
     */
...
bool MM_EnvironmentBase::tryAcquireExclusiveVMAccess() {
...
bool MM_EnvironmentBase::acquireExclusiveVMAccessForGC(
...
bool MM_EnvironmentBase::tryAcquireExclusiveVMAccessForGC(
    MM_Collector *collector) {
  MM_GCExtensionsBase *extensions = getExtensions();
  uintptr_t collectorAccessCount = collector->getExclusiveAccessCount();
  ...
  while (_omrVMThread != extensions->gcExclusiveAccessThreadId) {
    if (NULL == extensions->gcExclusiveAccessThreadId) {
      /* there is a chance the thread can win the race to acquiring exclusive
       * for GC */
      omrthread_monitor_enter(extensions->gcExclusiveAccessMutex);
      if (NULL == extensions->gcExclusiveAccessThreadId) {
        /* thread is the winner and will request the GC */
    ...
    if (_omrVMThread != extensions->gcExclusiveAccessThreadId) {
      /* thread was not the winner for requesting a GC - allow the GC to proceed
      ...
      /* there is a chance the GC will already have executed at this point or
       * other threads will re-win and re-execute.  loop until the
      ...
      /* May have been beaten to a GC, but perhaps not the one we wanted.  Check
       * and if in fact the collection we intended has been
...
bool MM_EnvironmentBase::tryAcquireExclusiveForConcurrentKickoff(
    MM_ConcurrentGCStats *stats) {
  MM_GCExtensionsBase *extensions = MM_GCExtensionsBase::getExtensions(_omrVM);
...
void MM_EnvironmentBase::releaseExclusiveForConcurrentKickoff() {
  MM_GCExtensionsBase *extensions = MM_GCExtensionsBase::getExtensions(_omrVM);
