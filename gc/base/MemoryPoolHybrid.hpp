...
private:
   ...
   * ignored,
   * If the range is smaller than last free Entry, create last freeEntry from
   * the remainder.
   *
   * @param[in]      env
   * @param[in]	   freelist
...
public:
   ...
   * of memoryPool(high address),
   * in order to simplify the code, the below four methods (addFreeEntries,
   * removeFreeEntriesWithinRange, expandWithRange, contractWithRange) only
   * handle expand/contract/redistribute at the end of the memory pool.
   */
