 ...
 * collector asks a subspace to expand
 * while trying to allocate memory for objects.  An example is the scavenger,
 * which will attempt to expand the
 * tenure area if it fails to allocate tenure space.  This call is expected to
 * be made under the allocation lock
 * of a particular sub space - if there is no lock, or multiple sub spaces could
 * expand, some form of atomicity
...
void MM_Collector::recordExcessiveStatsForGCEnd(MM_EnvironmentBase *env) {
   ...
   * believes we are actually
   * doing sufficient freeing work to not decide there is excessive GC'ing going
   * on.
   */
  ...
  if (stats->endGCTimeStamp > stats->lastEndGlobalGCTimeStamp) {
     ...
     * weight.  Given that in a generational system, you are likely to
     * experience
...
void MM_Collector::preCollect(MM_EnvironmentBase *env,
   ...
   * a result
   * of a local gc.  We use extensions->didGlobalGC to track if a global gc
   * occured.
   * If this is the outter most garbage collect invocation then set the global
   * gc flag to	false.
   */
  if (!_isRecursiveGC) {
     ...
     * invocation
     * Only time implicit GCs -- system GCs are accounted for "user" time in
     * GC/total
   ...
   * us to
   * trigger excessiveGC checks from a local collection that has recursed into a
   * global gc .
   */
  if (_globalCollector) {
...
void MM_Collector::postCollect(MM_EnvironmentBase *env,
  ...
  if (!_isRecursiveGC) {
     ...
     * GCs:
     * ulg*ulululg* (g=global; l=local; u=user; we sum the gc times between the
     * *s and compare
     * against the total time between the *s.)
     * We need to clear the accumulated gc times and track the end of this gc if
     * a global collection
     * occured, in order to adhere to the interval above.
     * This needs to be done here to reset the stats when a system gc has
     * occured.
     */
 ...
 * resetting/releasing said
 * work such that a resumption would be the equivalent of a fresh start.  All
 * hidden references to heap objects
