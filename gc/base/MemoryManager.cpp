...
MM_MemoryManager *MM_MemoryManager::newInstance(MM_EnvironmentBase *env) {
  MM_MemoryManager *memoryManager =
      (MM_MemoryManager *)env->getForge()->allocate(
          sizeof(MM_MemoryManager), MM_AllocationCategory::FIXED,
          OMR_GET_CALLSITE());
...
/* define the scan direction when reserving the GC heap in the range of (4G,
 * 32G) */
#if defined(S390) || defined(J9ZOS390)
    /* s390 benefits from smaller shift values so allocate direction is bottom
     * up */
    options |= OMRPORT_VMEM_ALLOC_DIR_BOTTOM_UP;
     ...
     * however
     * in case when preferred address is specified we will try to allocate heap
     * first
...
#define TWO_GB_ADDRESS ((void *)((uintptr_t)2 * 1024 * 1024 * 1024))
    ...
    if (requestedTopAddress <= ceiling) {
      ...
      if (allocationTopDown &&
        ...
        instance = MM_VirtualMemory::newInstance(
            env, heapAlignment, allocateSize, pageSize, pageFlags, tailPadding,
            preferredAddress, maxAddress, mode, options, memoryCategory);
      } else {
...
#define THIRTY_TWO_GB_ADDRESS ((uintptr_t)32 * 1024 * 1024 * 1024)
          if (requestedTopAddress <= (void *)THIRTY_TWO_GB_ADDRESS) {
            /*
             * If requested object heap size is in range 28G-32G its allocation
             * with 3-bit shift might compromise amount of low memory below 4G
             ...
             *  - allocation direction is top-down, otherwise it does not make
             * sense
             */
       ...
       * release memory
       * for backward compatibility this check should be done for compressedrefs
       * platforms only
       */
    ...
    /*
     * Code above might be used for non-compressedrefs platforms but need a few
     * adjustments on it for this:
...
bool MM_MemoryManager::createVirtualMemoryForMetadata(MM_EnvironmentBase *env,
  ...
  if (NULL == handle->getVirtualMemory()) {
    ...
    if (isMetadataAllocatedInVirtualMemory(env)) {
       ...
       * allocated in virtual memory
       * Segmentation is enabled for AIX-64 only, so physical page size is used
       * as a segment size for other platforms
       * To get advantage of preallocation this memory should be allocated in
       * large pages
       */
