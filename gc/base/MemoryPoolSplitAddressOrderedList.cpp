...
skipSearch:
  ...
  if (recycleHeapChunk(
          env, recycleEntry, ((uint8_t *)recycleEntry) + recycleEntrySize,
          previousFreeEntry, currentFreeEntry->getNext(), curFreeList)) {
    if (!skipReserved &&
  ...
  if (cause == forSweep) {
    ...
    if (0 < _sweepPoolState->_largestFreeEntry) {
      ...
      if (NULL == _sweepPoolState->_previousLargestFreeEntry) {
        ...
        largestFreeEntry =
            _sweepPoolState->_previousLargestFreeEntry->getNext();
      }
    ...
    for (chunk = sectioningIterator.nextChunk(); NULL != chunk;
      ...
      if (currentFreeListSize >= freeListSplitSize) {
         ...
         * which is set during sweep
         * if previousLargestFreeEntry == splitCandidatePreviousEntry, it means
         * largestFreeEntry == splitCandidateEntry and the largestFreeEntry is
         * in the next free list
         * if previousLargestFreeEntry < splitCandidatePreviousEntry, it means
  ...
  for (curFreeListIndex = 0; curFreeListIndex < _heapFreeListCount;
    ...
    if (canCoalesce) {
      ...
      if (previousFreeEntry &&
         ...
         * the entry does not change) or
         *    2) if its size is larger than size of reservedFreeEntry, but only
         * if the entry has not been identified yet than
      ...
      if (nextFreeEntry && (highAddress == (void *)nextFreeEntry)) {
         ...
         * only if the entry has not been identified yet
         * update the size of reservedFreeEntry
...
void *MM_MemoryPoolSplitAddressOrderedList::contractWithRange(
  ...
  Assert_GC_true_with_message2(
      env, reservedFreeEntryConsistencyCheck(), "contractWithRange "
                                                "_previousReservedFreeEntry=%p,"
                                                " _reservedFreeEntrySize=%zu\n",
...
void MM_MemoryPoolSplitAddressOrderedList::addFreeEntries(
  ...
  if (previousFreeEntry == NULL) {
    ...
    if ((uint8_t *)previousFreeEntry->afterEnd() == (uint8_t *)freeListHead) {
      ...
      Assert_MM_true((NULL == freeListHead->getNext()) ||
...
bool MM_MemoryPoolSplitAddressOrderedList::removeFreeEntriesWithinRange(
  ...
  Assert_GC_true_with_message2(
      env, reservedFreeEntryConsistencyCheck(), "removeFreeEntriesWithinRange "
                                                "_previousReservedFreeEntry=%p,"
                                                " _reservedFreeEntrySize=%zu\n",
      _previousReservedFreeEntry, _reservedFreeEntrySize);
  return true;
