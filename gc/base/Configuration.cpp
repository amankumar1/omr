...
void MM_Configuration::tearDown(MM_EnvironmentBase *env) {
   ...
   * cache data
   * Do it last, because some collectors allocate/free resources based on number
   * of NUMA nodes.
  */
...
/**
 * Initialize and return the appropriate object allocation interface for the
 * given Environment.
 * Given the allocation scheme and particular environment, select and return the
 * appropriate object allocation interface.
 * @return new subtype instance of MM_ObjectAllocationInterface, or NULL on
 * failure.
 */
...
bool MM_Configuration::initializeEnvironment(MM_EnvironmentBase *env) {
  env->_envLanguageInterface =
      _configurationLanguageInterface->createEnvironmentLanguageInterface(env);
...
bool MM_Configuration::initializeRunTimeObjectAlignmentAndCRShift(
  ...
  if (heapTop <= (maxAddressValue << shift)) {
    ...
    /* impossible geometry:  use an assert for now but just return false once we
     * are done testing the shifting */
    Assert_MM_unreachable();
    return false;
  }
#if !defined(S390) && !defined(J9ZOS390)
  /* s390 benefits from smaller shift values but other platforms don't so just
   * force the shift to 3 if it was not 0 to save
   * on testing resources.
   * (still honour the "canChangeShift" flag, though, since we want our testing
...
#define MINIMUM_OBJECT_ALIGNMENT_SHIFT 0x3
  uintptr_t objectAlignmentInBytes =
      OMR_MAX((uintptr_t)1 << omrVM->_compressedPointersShift,
              MINIMUM_OBJECT_ALIGNMENT);
  uintptr_t objectAlignmenShift =
      OMR_MAX(omrVM->_compressedPointersShift, MINIMUM_OBJECT_ALIGNMENT_SHIFT);
...
bool MM_Configuration::initializeSizeClasses(MM_EnvironmentBase *env) {
  /* Note: if memory is allocated here (not at present) a teardown method must
   * be implemented to free it */
  bool result = _configurationLanguageInterface->initializeSizeClasses(env);
  return result;
}

void MM_Configuration::initializeWriteBarrierType(MM_EnvironmentBase *env) {
  _configurationLanguageInterface->initializeWriteBarrierType(
      env, internalGetWriteBarrierType(env));
}

void MM_Configuration::initializeAllocationType(MM_EnvironmentBase *env) {
  _configurationLanguageInterface->initializeAllocationType(
      env, internalGetAllocationType(env));
}
...
void MM_Configuration::initializeGCThreadCount(MM_EnvironmentBase *env) {
  ...
  if (!extensions->gcThreadCountForced) {
    ...
    if (_configurationLanguageInterface->getMaxGCThreadCount() <
        extensions->gcThreadCount) {
      extensions->gcThreadCount =
          _configurationLanguageInterface->getMaxGCThreadCount();
    }
  }
}
