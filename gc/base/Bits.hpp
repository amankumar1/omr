...
class MM_Bits {
private:
/** The number of bits to shift by when converting to/from bytes/slots and
 * bytes/ObjectFieldslots */
#if defined(OMR_ENV_DATA64)
  enum {
 ...
 */
#if defined(WIN32)
/* Implicit return in eax, not seen by compiler.  Disable compile warning C4035:
 * no return value */
...
#pragma warning(default : 4035) /* re-enable warning */
   ...
   * at the highest
   * significant bit.
   * Use a binary search style divide and conquer algorithm.  The check
   * constantly splits the group
   * in half, and reduces the working set to either the upper or lower half of
   * the remaining value,
