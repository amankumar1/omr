 ...
 * to satisfy
 * alignment requirements. Use getMaximumSize() to get the actual allocation
 * size.
 */
...
MM_HeapVirtualMemory::newInstance(MM_EnvironmentBase *env,
  ...
  heap = (MM_HeapVirtualMemory *)env->getForge()->allocate(
      sizeof(MM_HeapVirtualMemory), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (heap) {
...
bool MM_HeapVirtualMemory::initialize(MM_EnvironmentBase *env, uintptr_t size) {
  ...
  /* 390 JIT codegen requires a uintptr_t-worth of padding at end of heap -- see
   * CMVC 95853 */
 ...
 * ignore extra full page padding if page size is too large (hard coded here for
 * 1G or larger)
 */
#define ONE_GB ((uintptr_t)1 * 1024 * 1024 * 1024)
      if (extensions->requestedPageSize < ONE_GB)
#endif /* defined(OMR_GC_COMPRESSED_POINTERS) && !(defined(J9ZOS390) ||        \
          defined(S390)) */
      {
  ...
  if (!created &&
      !memoryManager->createVirtualMemoryForHeap(
          env, &_vmemHandle, effectiveHeapAlignment, size, padding,
 ...
 * the heap can ever occupy.  This value includes any memory that may never be
 * used by the heap (e.g.,
 ...
 * connects the arena to the list
...
bool MM_HeapVirtualMemory::initializeHeapRegionManager(
   ...
   * region manager (which was just
   * initialized by super) that we want to enable this range of regions for
   * later use.
   */
  MM_MemoryManager *memoryManager =
      MM_GCExtensionsBase::getExtensions(_omrVM)->memoryManager;
