...
MM_MarkingScheme *MM_MarkingScheme::newInstance(MM_EnvironmentBase *env) {
  ...
  markingScheme = (MM_MarkingScheme *)env->getForge()->allocate(
      sizeof(MM_MarkingScheme), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (markingScheme) {
 ...
 * heap.
 * This should be called in place of masterSetupForGC if no actual garbage
 * collection
 * is to take place.
 *
...
void MM_MarkingScheme::masterSetupForWalk(MM_EnvironmentBase *env) {
  ...
  _extensions->collectorLanguageInterface->markingScheme_masterSetupForWalk(
      env);
}

void MM_MarkingScheme::masterCleanupAfterGC(MM_EnvironmentBase *env) {
  _extensions->collectorLanguageInterface->markingScheme_masterCleanupAfterGC(
      env);
}
...
void MM_MarkingScheme::markLiveObjectsComplete(MM_EnvironmentBase *env) {
  _extensions->collectorLanguageInterface
      ->markingScheme_markLiveObjectsComplete(env);
}
