...
protected:
  /**
   * Initialize internal structures of the memory forge.  An instance of
   * MM_Forge must be initialized before
   ...
   * false.  A instance of MM_Forge
   * 		   should not be used before it has been successfully
   * initialized.
   */
   ...
   * there should be no calls to the
...
public:
   ...
   * or NULL if the request could not be performed.  This function is a wrapper
   * of omrmem_allocate_memory.
   *
   ...
   * which should be found using
   * 						 the OMR_GET_CALLSITE() macro
   * @return a pointer to the allocated memory, or NULL if the request could not
   * be performed
   */
  void *allocate(uintptr_t bytesRequested, MM_AllocationCategory::Enum category,
                 const char *callsite);

  /**
   * Deallocate memory that has been allocated by the garbage collector.  This
   * function should not be called
   * to deallocate memory that has not been allocated by either the allocate or
   * reallocate functions.  This
  ...
  /**
   * Returns the current memory usage statistics for the garbage collector.
   * Each entry in the array corresponds
   * to a memory usage category type.  To locate memory usage statistics for a
   * particular category, use the
   * enumeration value as the array index (e.g. stats[REFERENCES]).
   *
   * @return an array of memory usage statistics indexed using the CategoryType
   * enumeration
   */
