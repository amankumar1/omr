 ...
 * yielded less then optimal results),
...
uintptr_t MM_MemorySubSpaceUniSpace::performContract(
   ...
   * contract
   * by this much as we may not have this much storage free at end of heap in
   * first place
  */
 ...
 * whether a contract based on
 * percentage of GC time to total time is required.  If there is room to
 * contract, the value is derived from,
...
uintptr_t MM_MemorySubSpaceUniSpace::calculateTargetContractSize(
   ...
   * allocSize is greater than the total free memory,
   * the currentFree value is a large positive number (negative unsigned number
   * calculated above).
   */
...
uintptr_t MM_MemorySubSpaceUniSpace::calculateCollectorExpandSize(
   ...
   * size
   * todo: This is a bit of hack. Should really ask MP its minimum expansion
   * size.
   */
