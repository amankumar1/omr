...
MM_MemoryPoolHybrid::newInstance(MM_EnvironmentBase *env,
  ...
  memoryPool = (MM_MemoryPoolHybrid *)env->getForge()->allocate(
      sizeof(MM_MemoryPoolHybrid), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (memoryPool) {
...
void MM_MemoryPoolHybrid::expandWithRange(MM_EnvironmentBase *env,
   ...
   * free entry,
   * if the newly created entry is bigger than VeryLargeObjectThreshold, move or
   * append the entry into reserved freelist.
...
* coalesce it with regular freelist,
   * if the newly created entry or the rest of free entries is bigger than
* ReservedFreeEntryThreshold,
