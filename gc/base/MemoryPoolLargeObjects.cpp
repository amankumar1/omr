 ...
 *
 */
void reportGlobalGCComplete(J9HookInterface **hook, uintptr_t eventNum,
                            void *eventData, void *userData) {
...
MM_MemoryPoolLargeObjects *MM_MemoryPoolLargeObjects::newInstance(
  ...
  memoryPool = (MM_MemoryPoolLargeObjects *)env->getForge()->allocate(
      sizeof(MM_MemoryPoolLargeObjects), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (NULL != memoryPool) {
...
bool MM_MemoryPoolLargeObjects::initialize(MM_EnvironmentBase *env) {
  ...
  /* ..and register hook for global GC end to check if a SOA/LOA rebalance is
   * required. */
  (*mmHooks)->J9HookRegister(mmHooks, J9HOOK_MM_OMR_GLOBAL_GC_END,
                             reportGlobalGCComplete, (void *)this);
  ...
  _largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(
      ...
      _extensions->tlhMaximumSize + minimumFreeEntrySize,
...
void MM_MemoryPoolLargeObjects::tearDown(MM_EnvironmentBase *env) {
  ...
  (*mmPrivateHooks)
      ->J9HookUnregister(mmPrivateHooks,
                         J9HOOK_MM_PRIVATE_GLOBAL_GC_INCREMENT_START,
                         reportGlobalGCIncrementStart, (void *)this);
  (*mmHooks)->J9HookUnregister(mmHooks, J9HOOK_MM_OMR_GLOBAL_GC_END,
                               reportGlobalGCComplete, (void *)this);
 ...
 */
void MM_MemoryPoolLargeObjects::postCollect(MM_EnvironmentBase *env) {
  bool debug = _extensions->debugLOAResize;

  _soaBytesAfterLastGC =
      _memoryPoolSmallObjects->getApproximateFreeMemorySize();

  uintptr_t minimumSOAFreeRequired =
      (_soaSize / _extensions->heapFreeMinimumRatioDivisor) *
      _extensions->heapFreeMinimumRatioMultiplier;

  if ((_soaBytesAfterLastGC < minimumSOAFreeRequired) &&
    ...
    uintptr_t loaMinimumSize = MM_Math::roundToFloor(
    ...
    /* Recalculate minimum size required to account for memory transfer.  This
     * is solving the following equation for MinimumRequired:
     *
     ...
     */
    uintptr_t minimumLOAContractRequired =
        _extensions->heapFreeMinimumRatioMultiplier * _soaSize;
    minimumLOAContractRequired -=
        _extensions->heapFreeMinimumRatioDivisor * _soaBytesAfterLastGC;
    minimumLOAContractRequired /= _extensions->heapFreeMinimumRatioDivisor -
                                  _extensions->heapFreeMinimumRatioMultiplier;
    ...
    newLOABase =
        (void *)((uint8_t *)_currentLOABase + minimumLOAContractRequired);

    newLOABase = (void *)MM_Math::roundToCeiling(_extensions->heapAlignment,
    ...
    if (debug) {
      OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
      omrtty_printf("LOA Rebalanced to meet minimum SOA requirements: LOA "
                    "ratio has decreased from %.3f --> %.3f\n",
                    oldLOARatio, _currentLOARatio);
    }
 ...
 * redistribute free memory between
...
MM_MemoryPoolLargeObjects::calculateTargetLOARatio(MM_EnvironmentBase *env,
  ...
  if (allocSize >= _extensions->largeObjectMinimumSize) {
    /* If the allocation size is 5 times greater than current LOA size..expand
     * LOA */
    ...
    */
    ...
    if (_extensions->globalGCStats.gcCount >= JVM_INITIALIZATION_COLLECTIONS) {
      if (_currentLOARatio > _extensions->largeObjectAreaInitialRatio) {
        ...
        /* Current size of LOA equal to or less than intial size so contract
         * more slowly */
  ...
  if (debug && newLOARatio != _currentLOARatio) {
    omrtty_printf(
        "LOA Calculate target ratio: ratio has %s from  %.3f --> %.3f\n",
...
double MM_MemoryPoolLargeObjects::resetTargetLOARatio(MM_EnvironmentBase *env) {
  ...
  if (debug) {
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
    omrtty_printf("LOA Reset target ratio: ratio reset from %.3f to minimum "
                  "size of %.3f\n",
                  _currentLOARatio, _extensions->largeObjectAreaMinimumRatio);
  }
...
void MM_MemoryPoolLargeObjects::resetLOASize(MM_EnvironmentBase *env,
  ...
  if (_currentLOARatio != newLOARatio) {
    ...
    /* Calculate LOA size based on new loa ratio */
    newLOASize =
        MM_Math::roundToFloor(_extensions->heapAlignment,
                              (uintptr_t)(oldAreaSize * _currentLOARatio));
 ...
 * receiver.
 * This call will return an accurate count of the current size of all free
 * memory.  It will not
 * consider defered work that may be done to increase current free memory
 * stores.
 ...
 * receiver.
 * This call will return an estimated count of the current size of all free
 * memory.  Although this
 * estimate may be accurate, it will consider potential defered work that may be
 * done to increase current
 * free memory stores.
 ...
 * regions.
 * This involves fixing up any free list information that may change as a result
 * of an
 * address change.
 *
...
void MM_MemoryPoolLargeObjects::expandWithRange(MM_EnvironmentBase *env,
  ...
  assume0(MM_Math::roundToCeiling(_extensions->heapAlignment, expandSize) ==
          expandSize);
  ...
  if (0 == _currentOldAreaSize) {
    ...
    if (debug) {
      omrtty_printf("LOA Initial Allocation: heapSize %zu, Initial LOA ratio "
                    "is %.3f --> LOA base is %p initial LOA size %zu\n",
    ...
    if (0 == _currentLOARatio) {
      ...
      if (debug) {
        omrtty_printf("LOA resized on heap expansion: heapSize %zu,  LOA ratio "
                      "is %.3f --> LOA base is now %p LOA size %zu\n",
                      oldAreaSize, _currentLOARatio, _currentLOABase, _loaSize);
      }
    }
...
void *MM_MemoryPoolLargeObjects::contractWithRange(MM_EnvironmentBase *env,
  ...
  if (0 == _currentLOARatio) {
    ...
    if (debug) {
      omrtty_printf("LOA resized on heap contraction: heapSize %zu,  LOA ratio "
                    "is %.3f --> LOA base is now %p LOA size %zu\n",
                    oldAreaSize, _currentLOARatio, _currentLOABase, _loaSize);
    }
  }
...
void MM_MemoryPoolLargeObjects::redistributeFreeMemory(
  ...
  assume0(MM_Math::roundToFloor(_extensions->heapAlignment, _soaSize) ==
          _soaSize);
