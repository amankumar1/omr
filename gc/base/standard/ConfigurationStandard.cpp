...
bool MM_ConfigurationStandard::initialize(MM_EnvironmentBase *env) {
  ...
  if (result) {
    ...
    extensions->payAllocationTax =
        extensions->payAllocationTax || extensions->concurrentMark;
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */
#if defined(OMR_GC_CONCURRENT_SWEEP)
    extensions->payAllocationTax =
        extensions->payAllocationTax || extensions->concurrentSweep;
...
MM_ConfigurationStandard::createEnvironmentPool(MM_EnvironmentBase *env) {
  ...
  uintptr_t numberElements =
      _configurationLanguageInterface->getEnvPoolNumElements();
  uintptr_t poolFlags = _configurationLanguageInterface->getEnvPoolFlags();

  return pool_new(sizeof(MM_EnvironmentStandard), numberElements,
                  sizeof(uint64_t), poolFlags, OMR_GET_CALLSITE(),
...
uintptr_t MM_ConfigurationStandard::internalGetDefaultRegionSize(
    MM_EnvironmentBase *env) {
  return STANDARD_REGION_SIZE_BYTES;
}

uintptr_t MM_ConfigurationStandard::internalGetDefaultArrayletLeafSize(
    MM_EnvironmentBase *env) {
