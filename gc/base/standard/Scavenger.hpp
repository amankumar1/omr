...
private:
  ...
  void *_evacuateSpaceBase, *_evacuateSpaceTop; /**< cached base and top heap
                                                   pointers within evacuate
                                                   subspace */
  void *_survivorSpaceBase, *_survivorSpaceTop; /**< cached base and top heap
  ...
  MM_CycleState
      _cycleState; /**< Embedded cycle state to be used as the master cycle
                      state for GC activity */
  ...
  volatile uintptr_t _cachedEntryCount; /**< non-empty scanCacheList count (not
                                           the total count of caches in the
                                           lists) */
  ...
  volatile uintptr_t
      _waitingCount; /**< count of threads waiting  on scan cache queues
                        ...
                        _freeCacheMonitor */
  uintptr_t _cacheLineAlignment; /**< The number of bytes per cache line which
                                    is used to determine which boundaries in
                                    memory represent the beginning of a cache
                                    line */
#if !defined(OMR_GC_CONCURRENT_SCAVENGER)
  volatile bool
      _rescanThreadsForRememberedObjects; /**< Indicates that thread-referenced
                                             ...
                                             must be rescanned */
#endif
  bool _backOutFlag; /**< set to true if a thread is unable to copy an object
                        due to lack of free space in both Survivor and Tenure */
  uintptr_t _backOutDoneIndex; /**< snapshot of _doneIndex, when backOut was
                                  detected */
...
private:
   ...
   * getNextScanCache.
   * This removes the requirement of a synchronization point after calls to
   * completeScan when
  ...
  MMINLINE MM_CopyScanCacheStandard *
  getSurvivorCopyCache(MM_EnvironmentStandard *env);
  MMINLINE MM_CopyScanCacheStandard *
  getDeferredCopyCache(MM_EnvironmentStandard *env);
...
public:
  ...
  /* mutator thread */
  void mutatorFinalReleaseCopyCaches(MM_EnvironmentBase *env,
                                     MM_EnvironmentBase *threadEnvironment);
  ...
  virtual void forceConcurrentFinish() { _forceConcurrentTermination = true; }
   ...
   * copy() for client language
   * runtime. Slot holding reference will be updated with new address for
   * referent on return.
   ...
   * referenced
   * objects were tenured during the scavenge. It is called by the RootScanner
   * on each thread
   * or stack slot.
   *
   * @param env[in] the current thread
  ...
  MM_Scavenger(MM_EnvironmentBase *env, MM_CollectorLanguageInterface *cli,
      ...
      : MM_Collector(cli),
        ...
        _waitingCount(0), _cacheLineAlignment(0)
#if !defined(OMR_GC_CONCURRENT_SCAVENGER)
        ,
        _rescanThreadsForRememberedObjects(false)
#endif
        ,
        _backOutFlag(false), _backOutDoneIndex(0), _heapBase(NULL),
        _heapTop(NULL), _regionManager(regionManager)
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
        ,
