...
private:
  ...
  typedef enum { PHASE1 = 1, PHASE2 } KickoffPhase;
...
private:
  ...
  bool getInitRange(MM_EnvironmentStandard *env, void **from, void **to,
                    InitType *type, bool *concurrentCollectable);
  bool allInitRangesProcessed() {
    return (_nextInitRange == _numInitRanges ? true : false);
  };
  uintptr_t calculateInitSize(MM_EnvironmentStandard *env,
                              uintptr_t allocationSize);
  uintptr_t calculateTraceSize(MM_EnvironmentStandard *env,
  ...
  void kickoffCardCleaning(MM_EnvironmentStandard *env,
  ...
  bool timeToKickoffConcurrent(MM_EnvironmentStandard *env,
                               MM_AllocateDescription *allocDescription);
  uintptr_t doConcurrentInitialization(MM_EnvironmentStandard *env,
                                       uintptr_t initToDo);
  uintptr_t doConcurrentTrace(MM_EnvironmentStandard *env,
                              MM_AllocateDescription *allocDescription,
  ...
  uintptr_t potentialFreeSpace(MM_EnvironmentStandard *env,
                               MM_AllocateDescription *allocDescription);
  static void hookObjectRemovedFromRememberedSet(J9HookInterface **hook,
                                                 uintptr_t eventNum,
                                                 void *eventData,
                                                 void *userData);
  ...
  void reportConcurrentFinalCardCleaningEnd(MM_EnvironmentStandard *env,
                                            uint64_t duration);
  void reportConcurrentCollectionStart(MM_EnvironmentStandard *env);
  void reportConcurrentCollectionEnd(MM_EnvironmentStandard *env,
                                     uint64_t duration);

  void reportConcurrentBackgroundThreadActivated(MM_EnvironmentStandard *env);
  void reportConcurrentBackgroundThreadFinished(MM_EnvironmentStandard *env,
                                                uintptr_t traceTotal);

  void reportConcurrentCompleteTracingStart(MM_EnvironmentStandard *env);
  void reportConcurrentCompleteTracingEnd(MM_EnvironmentStandard *env,
                                          uint64_t duration);

  void reportConcurrentRememberedSetScanStart(MM_EnvironmentStandard *env);
  void reportConcurrentRememberedSetScanEnd(MM_EnvironmentStandard *env,
                                            uint64_t duration);
...
#if defined(OMR_GC_CONCURRENT_SWEEP)
  void concurrentSweep(MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace,
                       MM_AllocateDescription *allocDescription);
...
protected:
  ...
  void concurrentMark(MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace,
                      MM_AllocateDescription *allocDescription);
...
public:
  virtual uintptr_t getVMStateID() {
    return J9VMSTATE_GC_COLLECTOR_CONCURRENTGC;
  };
  ...
  static void
  signalThreadsToDirtyCardsAsyncEventHandler(OMR_VMThread *omrVMThread,
                                             void *userData);
  ...
  virtual void payAllocationTax(MM_EnvironmentBase *env,
                                ...
                                MM_AllocateDescription *allocDescription);
  bool concurrentFinalCollection(MM_EnvironmentStandard *env,
                                 MM_MemorySubSpace *subspace);
  ...
  void objectRemovedFromRememberedSet(MM_EnvironmentStandard *env,
  ...
  */
  MMINLINE bool
  isExclusiveAccessRequestWaitingSparseSample(MM_EnvironmentStandard *env,
                                              uintptr_t value) {
  ...
  MM_ConcurrentGC(MM_EnvironmentBase *env, MM_CollectorLanguageInterface *cli)
      : MM_ParallelGlobalGC(env, cli), _cardTable(NULL), _heapBase(NULL),
        _heapAlloc(NULL), _rebuildInitWork(false), _retuneAfterHeapResize(false)
#if defined(OMR_GC_LARGE_OBJECT_AREA)
        ,
        ...
        _globalCollectionInProgress(false), _initializeMarkMap(false),
        _initWorkMonitor(NULL), _initWorkCompleteMonitor(NULL),
        _concurrentTuningMonitor(NULL), _initRanges(NULL), _numInitRanges(0),
        _numPhysicalInitRanges(0), _nextInitRange(0), _initializers(0),
        _initSetupDone(false), _bytesToCleanPass1(0), _bytesToCleanPass2(0),
        _bytesToTracePass1(0), _bytesToTracePass2(0),
        ...
        _concurrentCycleState(), _callback(NULL),
