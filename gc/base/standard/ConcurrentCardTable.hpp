 ...
 * and top card where base card
 * is first card in range and top card is card immediately AFTER the end of the
 * range.
 * @ingroup GC_Modron_Standard
 */
...
private:
  ...
  bool allocateCardTableEntriesForHeapRange(MM_EnvironmentStandard *env,
                                            MM_MemorySubSpace *subspace,
                                            uintptr_t size, void *lowAddress,
                                            void *highAddress,
                                            bool clearNewCards);
  bool freeCardTableEntriesForHeapRange(MM_EnvironmentStandard *env,
                                        uintptr_t size, void *lowAddress,
                                        void *highAddress,
                                        void *lowValidAddress,
                                        void *highValidAddress);
  bool allocateTLHMarkMapEntriesForHeapRange(MM_EnvironmentStandard *env,
                                             MM_MemorySubSpace *subspace,
                                             uintptr_t size, void *lowAddress,
                                             void *highAddress);
  bool freeTLHMarkMapEntriesForHeapRange(MM_EnvironmentStandard *env,
                                         uintptr_t size, void *lowAddress,
                                         void *highAddress,
                                         void *lowValidAddress,
                                         void *highValidAddress);

  void setTLHMarkBits(MM_EnvironmentStandard *env, uintptr_t slotIndex,
                      uintptr_t slotBits);
  void clearTLHMarkBits(MM_EnvironmentStandard *env, uintptr_t slotIndex,
                        uintptr_t slotBits);
  static uintptr_t calculateTLHMarkMapSize(MM_EnvironmentStandard *env,
  ...
  void processTLHMarkBits(MM_EnvironmentStandard *env,
                          MM_MemorySubSpace *subspace, void *tlhBase,
                          void *tlhTop, BitMapAction action);

protected:
  bool initialize(MM_EnvironmentBase *env, MM_Heap *heap);

  bool cleanSingleCard(MM_EnvironmentStandard *env, Card *card,
                       uintptr_t bytesToClean, uintptr_t *totalBytesCleaned);
  Card *getNextDirtyCard(MM_EnvironmentStandard *env, Card cardMask,
  ...
  virtual bool getExclusiveCardTableAccess(MM_EnvironmentStandard *env,
...
public:
   ...
   */
  bool heapAddRange(MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace,
   ...
   */
  bool heapRemoveRange(MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace,
                       uintptr_t size, void *lowAddress, void *highAddress,
   ...
   */
  uintptr_t cardBytesForHeapRange(MM_EnvironmentStandard *env, void *heapBase,
                                  void *heapTop);
  /**
   ...
   */
  void dirtyCardsInRange(MM_EnvironmentStandard *env, void *heapBase,
                         void *heapTop);
  /**
   * Called to clear the cards backing the heap regions which are in active but
   * not concurrently collectable subspaces.
   * The purpose of this call is to clear the cards backing active nursery heap
   * ranges, under the gencon policy.
   ...
   * otherwise
   */
  bool isObjectInDirtyCardNoCheck(MM_EnvironmentStandard *env,
   ...
   */
  virtual bool isObjectInUncleanedDirtyCard(MM_EnvironmentStandard *env,
                                            omrobjectptr_t object);
