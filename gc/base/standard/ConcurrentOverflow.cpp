...
MM_ConcurrentOverflow::newInstance(MM_EnvironmentBase *env,
  ...
  overflow = (MM_ConcurrentOverflow *)env->getForge()->allocate(
      sizeof(MM_ConcurrentOverflow), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (NULL != overflow) {
...
void MM_ConcurrentOverflow::overflowItemInternal(
  ...
  if ((PACKET_ARRAY_SPLIT_TAG != ((UDATA)item & PACKET_ARRAY_SPLIT_TAG)) &&
    ...
    _extensions->collectorLanguageInterface->workPacketOverflow_overflowItem(
        env, objectPtr);
  }
}
...
void MM_ConcurrentOverflow::clearCardsForNewSpace(MM_EnvironmentStandard *env,
   ...
   * already done so,
   * then we need to clear cards for new space so we can resolve overflow by
   * dirtying cards
  */
  if (_extensions->scavengerEnabled &&
      collector->isGlobalCollectionInProgress()) {
    /*
