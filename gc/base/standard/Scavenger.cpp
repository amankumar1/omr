 ...
 * library but this will suffice for
...
MM_Scavenger *MM_Scavenger::newInstance(MM_EnvironmentStandard *env,
  ...
  scavenger = (MM_Scavenger *)env->getForge()->allocate(
      sizeof(MM_Scavenger), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
  if (scavenger) {
...
bool MM_Scavenger::initialize(MM_EnvironmentBase *env) {
  ...
  (*mmOmrHooks)
      ->J9HookRegister(mmOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_START,
                       hookGlobalCollectionStart, (void *)this);
  (*mmOmrHooks)
      ->J9HookRegister(mmOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_END,
   ...
   * tenure, scan, large, possibly deferred)
   * So long as (N * _cachesPerThread) cache entries exist,the head of the scan
   * list
   * will contain a valid entry. We set the appropriate number of caches per
   * thread here */
...
void MM_Scavenger::masterSetupForGC(MM_EnvironmentStandard *env) {
  ...
  GC_MemorySubSpaceRegionIterator allocateRegionIterator(
      _evacuateMemorySubSpace);
  ...
  GC_MemorySubSpaceRegionIterator survivorRegionIterator(
      _survivorMemorySubSpace);
...
void MM_Scavenger::workerSetupForGC(MM_EnvironmentStandard *env) {
  ...
  env->_scavengerRememberedSet.fragmentSize =
      (uintptr_t)J9_SCV_REMSET_FRAGMENT_SIZE;
...
void MM_Scavenger::reportGCEnd(MM_EnvironmentStandard *env) {
  ...
  Trc_OMRMM_LocalGCEnd(
      env->getOmrVMThread(), _extensions->scavengerStats._rememberedSetOverflow,
      _extensions->scavengerStats._causedRememberedSetOverflow,
      _extensions->scavengerStats._scanCacheOverflow,
      _extensions->scavengerStats._failedFlipCount,
      _extensions->scavengerStats._failedFlipBytes,
 ...
 */
void MM_Scavenger::masterClearHotFieldStats() {
 ...
 */
void MM_Scavenger::masterReportHotFieldStats() {
  if (_extensions->scavengerTraceHotFields) {
 ...
 */
void MM_Scavenger::clearHotFieldStats(MM_EnvironmentStandard *env) {
  if (_extensions->scavengerTraceHotFields) {
...
/**
 * Merges hot field statistics for worker into master, if tracing hot fields is
 * enabled.
 */
void MM_Scavenger::mergeHotFieldStats(MM_EnvironmentStandard *env) {
  if (_extensions->scavengerTraceHotFields) {
 ...
 */
void MM_Scavenger::clearGCStats(MM_EnvironmentStandard *env) {
 ...
 */
void MM_Scavenger::mergeGCStats(MM_EnvironmentStandard *env) {
  ...
  if (canCalcGCStats(env)) {
    ...
    if (scavengerGCStats->_gcCount > 1) {
      ...
      scavengerGCStats->_avgTenureBytes = (uintptr_t)MM_Math::weightedAverage(
          ...
          (float)scavengerGCStats->_tenureAggregateBytes,
          TENURE_BYTES_HISTORY_WEIGHT);
#if defined(OMR_GC_LARGE_OBJECT_AREA)
      scavengerGCStats->_avgTenureSOABytes =
          (uintptr_t)MM_Math::weightedAverage(
              (float)scavengerGCStats->_avgTenureSOABytes,
              (float)(scavengerGCStats->_tenureAggregateBytes -
      ...
      scavengerGCStats->_avgTenureBytes =
          scavengerGCStats->_tenureAggregateBytes;
#if defined(OMR_GC_LARGE_OBJECT_AREA)
      scavengerGCStats->_avgTenureSOABytes =
          scavengerGCStats->_tenureAggregateBytes -
          scavengerGCStats->_tenureLOABytes;
...
MM_Scavenger::calculateOptimumCopyScanCacheSize(MM_EnvironmentStandard *env) {
  /* scale down maximal scan cache size using wait/copy/scan factor and round up
   * to nearest tlh size */
  uintptr_t scaleSize =
      (uintptr_t)(_extensions->copyScanRatio.getScalingFactor(env) *
                  _extensions->scavengerScanCacheMaximumSize);
  uintptr_t cacheSize =
      MM_Math::roundToCeiling(_extensions->tlhMinimumSize, scaleSize);
  ...
  env->_scavengerStats.countCopyCacheSize(
      cacheSize, _extensions->scavengerScanCacheMaximumSize);
...
MM_Scavenger::reserveMemoryForAllocateInSemiSpace(
  ...
  if ((NULL != env->_survivorCopyScanCache) &&
      ...
      if (((uintptr_t)env->_survivorTLHRemainderTop -
        ...
        allocateResult = (NULL !=
                          _survivorMemorySubSpace->collectorAllocateTLH(
                              env, this, &allocDescription, scanCacheSize,
                              addrBase, addrTop));
    ...
    if (allocateResult) {
       ...
       * condition between checking and setting
       * the minimum.  This means that this value may not actually be the lowest
       * value, or may increase.
       */
  ...
  if (NULL != objectPtr) {
    if (isObjectInEvacuateMemory(objectPtr)) {
      ...
      MM_ForwardedHeader forwardHeader(objectPtr,
                                       OMR_OBJECT_METADATA_SLOT_OFFSET);
...
// it's valid for this atomic write to fail. it means another thread raced us
// with either fixing the slot (with same value, though),
...
omrobjectptr_t MM_Scavenger::copy(MM_EnvironmentStandard *env,
  ...
  _extensions->objectModel.calculateObjectDetailsForCopy(
      forwardedHeader, &objectCopySizeInBytes, &objectReserveSizeInBytes,
      &hotFieldsDescriptor);
...
/* Move straight to tenuring on the object */
    ...
    if (_extensions->scavengerAlignHotFields &&
       ...
       * always assumes maximum padding,
       * but free entry created by sweep in tenure could be less than that
       * (since some of unused padding can overlap with next copied object)).
       * we limit this optimization for arrays up to the size of 2 cache lines,
       * beyond which the benefits of the optimization are believed to be
       * non-existant */
...
/* now correct for the hot field alignment */
  ...
  if (0 != hotFieldsAlignment) {
     ...
     * said we might need.  This is done by
     * removing the excess reserve since we already accounted for the
     * hotFieldPadSize by bumping the destination pointer
     * and now we need to revert to the amount needed for the object allocation
     * and its array alignment so the rest of
     * the method continues to function without needing to know about this extra
...
#if !defined(OMR_GC_CONCURRENT_SCAVENGER)
  /* Try to swap the forwarding pointer to the destination copy array into the
   * source object */
...
/* Copy and adjust the age value */
     ...
     * has been copied
     * (since mutator may access the object as soon as forwarding pointer is
     * installed) */
    omrobjectptr_t originalDestinationObjectPtr = destinationObjectPtr;
    destinationObjectPtr =
        forwardedHeader->setForwardedObject(destinationObjectPtr);
...
/* Succeeded in forwarding the object */
       ...
       * allocations. If this space was reserved for an individual object,
       * we might have created a TLH remainder from previous cache just before
       * reserving this space. This space eventaully can create another
       * remainder.
       * At that point, ideally (to recycle as much memory as possibly) we could
       * enqueue this remainder, but as a simple solution we will now abandon
       * the current remainder (we assert across the code, there is at most one
       * at a give point of time).
       * If we see large amount of discards even with low discard threshold, we
       * may reconsider enqueueing discarded TLHs.
       */
      if (copyCache->flags & OMR_SCAVENGER_CACHE_TYPE_TENURESPACE) {
  ...
  uintptr_t MM_Scavenger::getArraySplitAmount(MM_EnvironmentStandard * env,
    ...
    if (!backOutFlagRaised()) {
       ...
       * amount is proportional to array size.
       * the less busy we are, the smaller the split amount, while obeying
       * specified minimum and maximum.
       * but for single-threaded backout, do not split arrays.
       */
      scvArraySplitAmount = sizeInElements / (_dispatcher->activeThreadCount() +
                                              2 * _waitingCount);
      scvArraySplitAmount =
    ...
    if (endIndex < maxIndex) {
      ...
      if (NULL != splitCache) {
        ...
        omrarrayptr_t arrayPtr =
            (omrarrayptr_t)objectScanner->getParentObject();
        void *arrayTop =
  ...
  /**
   * GC thread calls this to update its slots copied/scanned counts after
   * scanning a range of objects. This may trigger
   ...
   * @param[in] slotsCopied number of slots copied on thread from
   * MM_CopyScanCache since last call to this method
   */
  MMINLINE void MM_Scavenger::updateCopyScanCounts(
      MM_EnvironmentBase * env, uint64_t slotsScanned, uint64_t slotsCopied) {
  ...
  MMINLINE bool MM_Scavenger::scavengeObjectSlots(
    ...
    if ((NULL == scanCache) || !scanCache->isSplitArray()) {
      ...
      objectScanner = _cli->scavenger_getObjectScanner(
    ...
    if (NULL != scanCache) {
      Assert_MM_true(
          objectScanner->isIndexableObject() ==
          (scanCache->isSplitArray() && (0 < scanCache->_arraySplitIndex)));
    ...
    while (NULL != (slotObject = objectScanner->getNextSlot())) {
      ...
      if (NULL != *copyCache) {
        if (NULL != hotFieldStats) {
          hotFieldStats->setHotnessOfField(
              slotObject->readAddressFromSlot(),
              objectScanner->getHotFieldsDescriptor());
          hotFieldStats->updateStats(isParentInNewSpace, isSlotObjectInNewSpace,
   ...
   * as soon as there is a copy cache that is preferred to the current scan
   * cache. This is returned
   * in nextScanCache.
   *
   ...
   */
  MMINLINE void MM_Scavenger::incrementalScavengeObjectSlots(
    ...
    if (!scanCache->_hasPartiallyScannedObject) {
      if (!scanCache->isSplitArray()) {
        /* try to get a new scanner instance from the cli */
        objectScanner = _cli->scavenger_getObjectScanner(
            env, objectPtr, (void *)scanCache->getObjectScanner(),
            GC_ObjectScanner::scanHeap);
        if ((NULL == objectScanner) || objectScanner->isLeafObject()) {
          /* Object scanner will be NULL if object not scannable by cli (eg,
           * empty pointer array, primitive array) */
          if (NULL != objectScanner) {
            /* Otherwise this is a leaf object -- contains no reference slots */
            env->_scavengerStats._leafObjectCount += 1;
          }
      ...
      if (objectScanner->isIndexableObject()) {
        ...
        if (!splitIndexableObjectScanner(
                env, objectScanner, scanCache->_arraySplitIndex,
    ...
    MM_ScavengerHotFieldStats *hotFieldStats = NULL;
    if (!objectScanner->isIndexableObject() &&
        _extensions->scavengerTraceHotFields) {
      /* maintain hotness of fields copied from this object */
      hotFieldStats = getHotFieldStats(env);
      hotFieldStats->_objectPtr = objectPtr;
      hotFieldStats->clearHotnessOfField();
    ...
    *nextScanCache = NULL;
    ...
    if (scanCache->_shouldBeRemembered) {
      if (NULL != scanCache->_arraySplitRememberedSlot) {
        ...
        Assert_MM_true(
            objectPtr ==
            (omrobjectptr_t)((uintptr_t) *
                                 (scanCache->_arraySplitRememberedSlot) &
                             ~(uintptr_t)DEFERRED_RS_REMOVE_FLAG));
        /* Set the remembered set slot to the object pointer in case it was
         * still marked for removal. */
  ...
  MM_CopyScanCacheStandard *MM_Scavenger::getNextScanCache(
    ...
    if (NULL != (cache = getDeferredCopyCache(env))) {
      /* deferred copy caches are used to merge memory-contiguous caches that
       * got chopped up due to large objects not fitting and resuing remainder.
       * we want to delay scanning them as much as possible (up to the size of
       * the original cache size being chopped up),
    ...
    while (!doneFlag && !backOutFlagRaised()) {
      while (_cachedEntryCount > 0) {
        ...
        if (NULL != cache) {
          /* Check if there are threads waiting that should be notified because
           * of pending entries */
        ...
        if ((env->_currentTask->getThreadCount() == _waitingCount) &&
          ...
          while ((0 == _cachedEntryCount) && (doneIndex == _doneIndex) &&
            ...
            if (doneIndex != _doneIndex) {
              ...
              env->_scavengerStats.addToWorkStallTime(waitStartTime,
                                                      waitEndTime);
            }
#endif /* J9MODRON_TGC_PARALLEL_STATISTICS */
          }
  ...
  /**
   * Scans all the objects to scan in the env->_scanCache, remembering objects
   * as required,
  ...
  void MM_Scavenger::completeScanCache(MM_EnvironmentStandard * env) {
    ...
    MM_CopyScanCacheStandard *scanCache =
        (MM_CopyScanCacheStandard *)env->_scanCache;
  ...
  /**
  * Scans the objects to scan in the env->_scanCache, remembering objects as
  * required.
  * Slots are scanned until there is an opportunity to alias the scan cache to a
  * copy cache. When
  * this happens, scanning is interrupted and the present scan cache is either
  * pushed onto the scan
  * list or "deferred" in thread-local storage. Deferring is done to reduce
  * contention due to the
  * increased need to change scan cache. If the cache is scanned completely
  * without interruption
  * the cache is flushed at the end.
  */
  void MM_Scavenger::incrementalScanCacheBySlot(MM_EnvironmentStandard * env) {
    ...
    while (isWorkAvailableInCache(scanCache)) {
      ...
      while ((objectPtr = heapChunkIterator.nextObjectNoAdvance()) != NULL) {
        ...
        if (scanCache->_hasPartiallyScannedObject) {
          ...
          /* Only save scan cache if it is not a copy cache, and then don't add
           * to scanlist - this
           * can cause contention, just defer to later time on same thread
           * if deferred cache is occupied, then queue current scan cache on
           * scan list
           */
          scanCache->flags &= ~OMR_SCAVENGER_CACHE_TYPE_SCAN;
  ...
  bool MM_Scavenger::completeScan(MM_EnvironmentStandard * env) {
    ...
    while ((env->_scanCache = getNextScanCache(env)) != NULL) {
      ...
      omrtty_printf("{SCAV: Completing scan (%p) %p-%p-%p-%p}\n",
                    env->_scanCache, env->_scanCache->cacheBase,
                    env->_scanCache->cacheAlloc, env->_scanCache->scanCurrent,
                    env->_scanCache->cacheTop);
#endif /* OMR_SCAVENGER_TRACE */
    ...
    /* A slow  thread can see backOutFlag raised by a fast thread aborting in
     * the next scan cycle.
     * By checking that thread local doneIndex of the current scan cycle matches
     * the doneIndex from scan cycle that raised the flag,
     * we ensure consistent behavior (return of backOutRaised flag) of all
     * threads within this scan cycle, so that all threads proceed
     * consistently to the next step (being just another scan cycle, or backout
     * procedure).
     */
    bool backOutRaisedThisScanCycle =
        backOutFlagRaised() && (doneIndex == _backOutDoneIndex);
  ...
  void MM_Scavenger::workThreadGarbageCollect(MM_EnvironmentStandard * env) {
     ...
     * between beginning of scavenge and this point,
     * otherwise it is hang situation when one group of threads might wait for
     * more work in complete scan and another group of threads
  ...
  void MM_Scavenger::addToRememberedSetFragment(MM_EnvironmentStandard * env,
    ...
    *rememberedSetEntry = (uintptr_t)objectPtr;
    ...
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
    omrtty_printf("{SCAV: Add to remembered set %p; env count = %lld; ext "
                  "count = %lld}\n",
  ...
  void MM_Scavenger::rememberObject(MM_EnvironmentStandard * env,
                                    omrobjectptr_t objectPtr) {
    /* Try to set the REMEMBERED bit in the flags field (if it hasn't already
     * been set) */
  ...
  bool MM_Scavenger::processRememberedThreadReference(
      MM_EnvironmentStandard * env, omrobjectptr_t objectPtr) {
    Assert_MM_true(NULL != objectPtr);
    Assert_MM_true(!isObjectInNewSpace(objectPtr));
    Assert_MM_true(_extensions->objectModel.isRemembered(objectPtr));
  ...
  bool MM_Scavenger::shouldRememberObject(MM_EnvironmentStandard * env,
    ...
    GC_ObjectScannerState objectScannerState;
    GC_ObjectScanner *objectScanner = _cli->scavenger_getObjectScanner(
        env, objectPtr, &objectScannerState,
  ...
  MMINLINE bool MM_Scavenger::scavengeRememberedObject(
      MM_EnvironmentStandard * env, omrobjectptr_t objectPtr) {
    bool shouldBeBemembered = scavengeObjectSlots(
        env, NULL, objectPtr, GC_ObjectScanner::scanRoots, NULL);
    if (_extensions->objectModel.hasIndirectObjectReferents(
            env->getLanguageVMThread(), objectPtr)) {
      shouldBeBemembered |=
          _cli->scavenger_scavengeIndirectObjectSlots(env, objectPtr);
    }
    return shouldBeBemembered;
  }

  void MM_Scavenger::scavengeRememberedSetOverflow(MM_EnvironmentStandard *
                                                   env) {
    /* Reset the local remembered set fragment */
    env->_scavengerRememberedSet.fragmentCurrent = NULL;
    env->_scavengerRememberedSet.fragmentTop = NULL;
    ...
    if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env,
       ...
       * Objects that no longer need remembering will be pruned at the end of
       * the scavenge.
       */
        ...
        while ((objectPtr = objectIterator.nextObject()) != NULL) {
          if (_extensions->objectModel.isRemembered(objectPtr)) {
            ...
            if (shouldBeRemembered) {
              ...
              /* Inform interested parties that an object has been removed from
               * the remembered set */
              TRIGGER_J9HOOK_MM_PRIVATE_OBJECT_REMOVED_FROM_REMEMBERED_SET(
                  _extensions->privateHookInterface, env->getOmrVMThread(),
                  objectPtr);
            }
          }
        ...
        while ((slotPtr = (omrobjectptr_t *)remSetSlotIterator.nextSlot()) !=
          ...
          if (NULL == objectPtr) {
            ...
            if (!shouldRememberObject(env, objectPtr)) {
               ...
               * todo: consider doing double remembering, if remembered during
               * CS cycle, to avoid the rescan of the object
               */
              *slotPtr = objectPtr;
            }
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
...
/* Retain remembered object */
  ...
  void MM_Scavenger::copyAndForwardThreadSlot(
      MM_EnvironmentStandard * env, omrobjectptr_t * objectPtrIndirect) {
    ...
    if (NULL != objectPtr) {
      if (isObjectInEvacuateMemory(objectPtr)) {
        ...
        if (_extensions->isOld(objectPtr)) {
          if (_extensions->objectModel.atomicSetObjectCurrentlyReferenced(
                  objectPtr)) {
  ...
  void MM_Scavenger::rescanThreadSlot(MM_EnvironmentStandard * env,
    ...
    if (NULL != objectPtr) {
      if (isObjectInEvacuateMemory(objectPtr)) {
        /* the slot is still pointing at evacuate memory. This means that it
         * must have been left unforwarded
  ...
  /*
   * definition of the following method is earlier in the file to ensure
   * inlining.
   *
   ...
   */
  MMINLINE bool MM_Scavenger::isWorkAvailableInCache(MM_CopyScanCacheStandard *
                                                     cache) {
...
/* Check the free list */
    ...
    if (NULL == cache) {
      ...
      env->_scavengerStats._scanCacheAllocationDurationDuringSavenger +=
          duration;
    }
  ...
  MM_CopyScanCacheStandard *MM_Scavenger::createCacheInHeap(
    ...
    /* Recheck the free list - avoids a potential timing hole here after
     * releasing the lock previously.
...
#if defined(OMR_SCAVENGER_TRACE)
        omrtty_printf("{SCAV: Temporary chunk of scan cache headers allocated "
                      "in Survivor (%p)}\n",
                      cache);
#endif /* OMR_SCAVENGER_TRACE */
      }
...
/* temporary chunk of scanCaches is created in Tenure */
  ...
  MM_CopyScanCacheStandard *MM_Scavenger::releaseLocalCopyCache(
    ...
    if (NULL != cache) {
      ...
      if (cache->isCurrentlyBeingScanned()) {
        /* Since it is being scanned, cannot reuse and should not add to scan
         * list */
        ...
        if (NULL != env->_deferredCopyCache) {
          ...
          if ((env->_deferredCopyCache->cacheAlloc == cache->scanCurrent) &&
            ...
            /* append current copy cache to the deferred one. yet, decide
             * whether to keep deferring it or push it for scanning */
            Assert_MM_true((cache->flags & ~OMR_SCAVENGER_CACHE_TYPE_HEAP) ==
                           (env->_deferredCopyCache->flags &
                            ~OMR_SCAVENGER_CACHE_TYPE_HEAP));
            Assert_MM_false(cache->flags &
                            OMR_SCAVENGER_CACHE_TYPE_SPLIT_ARRAY);
            if (remainderCreated) {
              /* keep deferring the joint copy cache, there might be more
               * appends to come */
          ...
          /* No deferred cache exists. Decide what to do with current one
           * (defer, push for scanning, or just ignore) */
          if (cache->isScanWorkAvailable()) {
  ...
  bool MM_Scavenger::clearCache(MM_EnvironmentStandard * env,
    ...
    /* Broadcast details of that portion of memory within which objects have
     * been allocated */
  ...
  void MM_Scavenger::abandonTLHRemainders(MM_EnvironmentStandard * env) {
    abandonSurvivorTLHRemainder(env);
    abandonTenureTLHRemainder(env);
  }
  ...
  MMINLINE uintptr_t MM_Scavenger::copyCacheDistanceMetric(
      MM_CopyScanCacheStandard * copyCache) {
    /* (best estimate) distance from first reference slot to prospective
     * referent copy location */
    return ((uintptr_t)copyCache->cacheAlloc -
            ((uintptr_t)copyCache->scanCurrent + J9_GC_MINIMUM_OBJECT_SIZE));
  }
  ...
  MMINLINE MM_CopyScanCacheStandard *MM_Scavenger::aliasToCopyCache(
      ...
      MM_CopyScanCacheStandard * scanCache,
      MM_CopyScanCacheStandard * copyCache) {
    /* Only alias a copy cache IF there are 0 threads waiting.  If the current
     * thread is the only producer and
     * it aliases a copy cache then it will be the only thread able to consume.
     * This will alleviate the stalling issues
     * described in VMDESIGN 1359.
     */
    if (0 == _waitingCount) {
      /* was the object received by an unaliased copy cache or aliased scan
       * cache? */
      if (scanCache != copyCache) {
        /* unaliased copy cache received the object; alias and switch to it if
         * possible */
        if ((0 == (scanCache->flags & OMR_SCAVENGER_CACHE_TYPE_COPY)) ||
            (copyCacheDistanceMetric(copyCache) <
        ...
        /* scan cache is aliased and received the object so identify the other
         * copy cache */
  ...
  MMINLINE MM_CopyScanCacheStandard *MM_Scavenger::getDeferredCopyCache(
      MM_EnvironmentStandard * env) {
    return env->_deferredCopyCache;
  }

  MMINLINE MM_CopyScanCacheStandard *MM_Scavenger::getSurvivorCopyCache(
      MM_EnvironmentStandard * env) {
    MM_CopyScanCacheStandard *cache = NULL;
  ...
  /**
   * Determine whether a scavenge that has been started did complete
   * successfully.
   * @return true if the scavenge completed successfully, false otherwise.
   */
  bool MM_Scavenger::scavengeCompletedSuccessfully(MM_EnvironmentStandard *
                                                   env) {
    return !backOutFlagRaised();
  }
   ...
   */
  void MM_Scavenger::setBackOutFlag(MM_EnvironmentBase * env, bool value) {
    ...
    if (!(_backOutFlag && value)) {
      ...
      _backOutFlag = value;
      /* Might be an overkill, but ensure that other CPUs see correct
       * _backOutDoneIndex, by the time they see _backOutFlag is set */
      ...
      TRIGGER_J9HOOK_MM_PRIVATE_SCAVENGER_BACK_OUT(
          _extensions->privateHookInterface, env->getOmrVM(), value);
    }
  }

  bool MM_Scavenger::backOutFixSlotWithoutCompression(volatile omrobjectptr_t *
                                                      slotPtr) {
    omrobjectptr_t objectPtr = *slotPtr;
  ...
  void MM_Scavenger::backOutObjectScan(MM_EnvironmentStandard * env,
    ...
    GC_ObjectScannerState objectScannerState;
    GC_ObjectScanner *objectScanner = _cli->scavenger_getObjectScanner(
        env, objectPtr, (void *)&objectScannerState,
        GC_ObjectScanner::scanRoots);
    if (NULL != objectScanner) {
#if defined(OMR_SCAVENGER_TRACE_BACKOUT)
      OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  ...
  void MM_Scavenger::backoutFixupAndReverseForwardPointersInSurvivor(
      ...
      if (isObjectInEvacuateMemory(
        ...
        while ((objectPtr = evacuateHeapIterator.nextObjectNoAdvance()) !=
          ...
          if (header.isForwardedPointer()) {
            ...
            /* A reverse forwarded object is a hole whose 'next' pointer
             * actually points at the original object.
             * This keeps tenure space walkable once the reverse forwarded
            ...
            omrtty_printf(
                ...
                objectPtr, *objectPtr, forwardedObject, freeHeader->getNext(),
                freeHeader->getSize());
    ...
    while (NULL != (rootRegion = evacuateRegionIterator1.nextRegion())) {
      if (isObjectInEvacuateMemory(
         ...
         * detect if this object needs its destroyed slot fixed up (but
         * the above loop must complete before we have the information with
         * which to fixup the destroyed slot).  Fixing up a slot in dark
         * matter could crash, though, since the slot could point to contracted
         * memory or could point to corrupted data updated in a previous
         * backout.  The simple work-around for this problem is to check if the
         * slot points at a readable part of the heap (specifically,
         * tenure or survivor - the only locations which would require us to fix
         * up the slot) and only read and fixup the slot in those cases.
         * This means that we could still corrupt the slot but we will never
         * crash during fixup and nobody else should be trusting slots found
         * in dead objects.
         */
        GC_ObjectHeapIteratorAddressOrderedList evacuateHeapIterator(
            _extensions, rootRegion, false);
 ...
 * 2) Walk the evacuate space, fixing up objects and installing reverse forward
          ...
          if (isObjectInEvacuateMemory(
            ...
            while ((objectPtr = evacuateHeapIterator.nextObjectNoAdvance()) !=
              ...
              if (NULL != fwdObjectPtr) {
                ...
                /* Move to the next object - the heap iterator is incapable of
                 * dealing with
                 * tagged class pointers.
                 */
                /* If this scavenge was the first move of a hashed object, the
                 * sizes don't match.
                 * This approach won't work if the flags of the original (i.e.
                 * evacuate space)
                 * object are destroyed.
                 */
                if (_extensions->objectModel.isObjectJustHasBeenMoved(
                        fwdObjectPtr)) {
                  uintptr_t size =
                      _extensions->objectModel.getSizeInBytesWithHeader(
                          fwdObjectPtr);
...
/* 3) Walk the remembered set, updating list pointers as well as remembered
 * object ptrs */
#if defined(OMR_SCAVENGER_TRACE_BACKOUT)
         ...
         * tenured copy that is remembered)
        ...
        while ((puddle = remSetIterator.nextList()) != NULL) {
          ...
          while ((slotPtr = (omrobjectptr_t *)remSetSlotIterator.nextSlot()) !=
            ...
            *slotPtr = (omrobjectptr_t)((uintptr_t)*slotPtr &
            ...
            if (objectPtr) {
              if (MM_ForwardedHeader(objectPtr, OMR_OBJECT_METADATA_SLOT_OFFSET)
  ...
  void MM_Scavenger::masterThreadGarbageCollect(
    ...
    MM_EnvironmentStandard *env =
        MM_EnvironmentStandard::getEnvironment(envBase);
    Trc_MM_Scavenger_masterThreadGarbageCollect_Entry(
        env->getLanguageVMThread());
...
/* Flip the memory space allocate profile */
        ...
        _activeSubSpace->flip(
            env,
            MM_MemorySubSpaceSemiSpace::restore_allocation_and_set_survivor);
        ...
        if (_extensions->scvTenureStrategyAdaptive) {
          ...
          uintptr_t newSpaceConsumedSize =
              newSpaceTotalSize -
              _activeSubSpace->getActualActiveFreeMemorySize();
        ...
        /* Build free list in survivor profile - the scavenge was unsuccessful,
         * so rebuild the free list */
    ...
    Trc_MM_Scavenger_masterThreadGarbageCollect_Exit(
        env->getLanguageVMThread());
  }

  void MM_Scavenger::processLargeAllocateStatsBeforeGC(MM_EnvironmentBase *
                                                       env) {
  ...
  void MM_Scavenger::processLargeAllocateStatsAfterGC(MM_EnvironmentBase *
                                                      env) {
    MM_MemorySpace *defaultMemorySpace =
        _extensions->heap->getDefaultMemorySpace();
    MM_MemorySubSpace *tenureMemorySubspace =
    ...
    if (LOCALGC_ESTIMATE_FRAGMENTATION ==
        (_extensions->estimateFragmentation & LOCALGC_ESTIMATE_FRAGMENTATION)) {
  ...
  void MM_Scavenger::reportGCCycleFinalIncrementEnding(MM_EnvironmentStandard *
                                                       env) {
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  ...
  bool MM_Scavenger::heapRemoveRange(
      ...
      void *lowAddress, void *highAddress, void *validLowAddress,
   ...
   * allocation fails, and a decision on whether
   ...
   * The query is made only on collectorAllocate() type requests when the
   * allocation fails, and a decision
    ...
    if (!env->_cycleState->_gcCode.isExplicitGC()) {
      if (excessive_gc_normal != _extensions->excessiveGCLevel) {
        ...
        env->_cycleState->_gcCode =
            MM_GCCode(J9MMCONSTANT_IMPLICIT_GC_EXCESSIVE);
      }
    }
  ...
  bool MM_Scavenger::internalGarbageCollect(
    ...
    MM_ScavengerStats *scavengerGCStats = &_extensions->scavengerStats;
    MM_MemorySubSpace *tenureMemorySubSpace =
        ((MM_MemorySubSpaceSemiSpace *)subSpace)->getTenureMemorySubSpace();

    /* First, if the previous scavenge had a failed tenure of a size greater
     * than the threshold,
    ...
    if (failedTenureThresholdReached()) {
      ...
      uint32_t aggressivePercolate =
          _extensions->heap->getPercolateStats()
                      ->getScavengesSincePercolate() <= 1
    ...
    /*
     * Second, if the previous scavenge failed to expand tenure, ask parent MSS
     * to try a collect.
     */
    if (expandFailed()) {
    ...
    /* If the tenure MSS is not expandable and/or  there is insufficent space
     * left to tenure
     * the average number of bytes tenured by a scavenge then percolate the
     * collect to avoid
     ...
     * scavenge. */
    // TODO Probably should rename this -Xgc option as it may not always result
    // in a ggc
    // in futre, e.g if we implement multiple generations.
    if (_extensions->maxScavengeBeforeGlobal) {
      if (_countSinceForcingGlobalGC++ >=
        ...
        /* Percolate the collect to parent MSS */
        bool result =
            percolateGarbageCollect(env, subSpace, NULL, MAX_SCAVENGES,
                                    J9MMCONSTANT_IMPLICIT_GC_PERCOLATE);
     ...
     * Allow the CollectorLanguageInterface to advise if percolation should
     * occur.
     */
    ...
    bool shouldPercolate =
        _cli->scavenger_internalGarbageCollect_shouldPercolateGarbageCollect(
    ...
    if (isRememberedSetInOverflowState() &&
        _extensions->scavengerRsoScanUnsafe) {
      /* NOTE: No need to set that the collect was unsuccessful - we will
       * actually execute
  ...
  bool MM_Scavenger::percolateGarbageCollect(
      MM_EnvironmentBase * env, MM_MemorySubSpace * subSpace,
      MM_AllocateDescription * allocDescription,
      PercolateReason percolateReason, uint32_t gcCode) {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    ...
    /* save the cycle state since we are about to call back into the collector
     * to start a new global cycle */
  ...
  /**
   * Re-size all structures which are dependent on the current size of the heap.
   * No new memory has been added to a heap reconfiguration.  This call
   * typically is the result
   * of having segment range changes (memory redistributed between segments) or
   * the meaning of
   * memory changed.
   *
   */
  ...
  void MM_Scavenger::globalCollectionStart(MM_EnvironmentBase * env) {
    /* Hold on to allocation stats that are useful but cleared on global
     * collects. */
    MM_ScavengerStats *scavengerStats = &_extensions->scavengerStats;
    MM_HeapStats heapStatsSemiSpace;
    MM_HeapStats heapStatsTenureSpace;
  ...
  void MM_Scavenger::reportGCCycleEnd(MM_EnvironmentStandard * env) {
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_GC_POST_CYCLE_END(
        ...
        env->_cycleState->_type, extensions->globalGCStats.workPacketStats
                                     .getSTWWorkStackOverflowOccured(),
        extensions->globalGCStats.workPacketStats
            .getSTWWorkStackOverflowCount(),
        extensions->globalGCStats.workPacketStats
  ...
  uintptr_t MM_Scavenger::calculateTiltRatio() {
    ...
    uintptr_t tmp =
        _extensions->heap->getActiveMemorySize(MEMORY_TYPE_NEW) / 100;
  ...
  uintptr_t MM_Scavenger::calculateTenureMask() {
    ...
    if (_extensions->scvTenureStrategyAdaptive) {
      newMask |= calculateTenureMaskUsingFixed(
          _extensions->scvTenureAdaptiveTenureAge);
    }
  ...
  uintptr_t MM_Scavenger::calculateTenureMaskUsingLookback(
    ...
    /* Second, we calculate the standard deviation of the initial generation
     * size over history. */
    ...
    for (uintptr_t age = 0; age <= OBJECT_HEADER_AGE_MAX + 1; ++age) {
       ...
       * now).
       * Also skip the last row in the history (there's no previous to compare
       * it to)
       */
       ...
       * If the generation would occupy at least half of survivor space, only
       * look back one collection.
       * If the generation would occupy at least a quarter of survivor space,
       * look back two collections.
       * If the generation would occupy at least an eighth of survivor space,
       * look back three collections.
       * . . .
       */
      const uintptr_t maximumLookback = SCAVENGER_FLIP_HISTORY_SIZE - 1;
      uintptr_t requiredLookback = 1;
      uintptr_t minimumBytesForRequiredLookback =
      ...
      if (shouldTenureThisAge) {
        /* Objects in this age are historically still dying at a rate of less
         * than 1%. Tenure them. */
  ...
  uintptr_t MM_Scavenger::calculateTenureMaskUsingHistory(
    ...
    for (uintptr_t age = 0; age < OBJECT_HEADER_AGE_MAX; ++age) {
      ...
      /* Skip the first row in the history (it's the current scavenge, and is
       * all zero right now).
       * Also skip the last row in the history (there's no previous to compare
       * it to).
       */
  ...
  bool MM_Scavenger::scavengeRoots(MM_EnvironmentBase * env) {
    ...
    MM_ConcurrentScavengeTask scavengeTask(
        ...
        U_64_MAX, NULL, env->_cycleState);
    _dispatcher->run(env, &scavengeTask);

    return false;
  }

  bool MM_Scavenger::scavengeScan(MM_EnvironmentBase * env,
                                  int64_t timeThreshold) {
  ...
  void MM_Scavenger::mutatorFinalReleaseCopyCaches(MM_EnvironmentBase * envBase,
                                                   MM_EnvironmentBase *
                                                       threadEnvironmentBase) {
    MM_EnvironmentStandard *env =
        MM_EnvironmentStandard::getEnvironment(envBase);
    MM_EnvironmentStandard *threadEnvironment =
        MM_EnvironmentStandard::getEnvironment(threadEnvironmentBase);

    if (MUTATOR_THREAD == threadEnvironment->getThreadType()) {
      /* in case of scavenge complete phase, master thread will act on behalf
       * (use its own environment) of mutator threads
       * in case of thread teardown, caller ensures that own environment is used
       */

      // todo: try to simplify. perhaps even we can use threadEnvironment
      // directly?
      ...
      if (envBase != threadEnvironmentBase) {
        ...
        env->_tenureTLHRemainderBase =
            threadEnvironment->_tenureTLHRemainderBase;
        env->_tenureTLHRemainderTop = threadEnvironment->_tenureTLHRemainderTop;
        env->_loaAllocation = threadEnvironment->_loaAllocation;
        env->_survivorTLHRemainderBase =
            threadEnvironment->_survivorTLHRemainderBase;
        env->_survivorTLHRemainderTop =
            threadEnvironment->_survivorTLHRemainderTop;
      ...
      if (threadEnvironment->_survivorCopyScanCache) {
        Assert_MM_true(threadEnvironment->_survivorCopyScanCache->flags &
                       OMR_SCAVENGER_CACHE_TYPE_COPY);
        threadEnvironment->_survivorCopyScanCache->flags &=
            ~OMR_SCAVENGER_CACHE_TYPE_COPY;
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
        env->_scavengerStats._releaseScanListCount += 1;
#endif /* J9MODRON_TGC_PARALLEL_STATISTICS */
        clearCache(env, threadEnvironment->_survivorCopyScanCache);
        addCacheEntryToScanListAndNotify(
            env, threadEnvironment->_survivorCopyScanCache);
        threadEnvironment->_survivorCopyScanCache = NULL;
      }
      if (threadEnvironment->_deferredCopyCache) {
        Assert_MM_true(threadEnvironment->_deferredCopyCache->flags &
                       OMR_SCAVENGER_CACHE_TYPE_CLEARED);
        Assert_MM_true(threadEnvironment->_deferredCopyCache->flags &
                       OMR_SCAVENGER_CACHE_TYPE_COPY);
        threadEnvironment->_deferredCopyCache->flags &=
            ~OMR_SCAVENGER_CACHE_TYPE_COPY;
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
        env->_scavengerStats._releaseScanListCount += 1;
#endif /* J9MODRON_TGC_PARALLEL_STATISTICS */
        addCacheEntryToScanListAndNotify(env,
                                         threadEnvironment->_deferredCopyCache);
        threadEnvironment->_deferredCopyCache = NULL;
      }
      if (threadEnvironment->_tenureCopyScanCache) {
        Assert_MM_true(threadEnvironment->_tenureCopyScanCache->flags &
                       OMR_SCAVENGER_CACHE_TYPE_COPY);
        threadEnvironment->_tenureCopyScanCache->flags &=
            ~OMR_SCAVENGER_CACHE_TYPE_COPY;
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
        env->_scavengerStats._releaseScanListCount += 1;
#endif /* J9MODRON_TGC_PARALLEL_STATISTICS */
        ...
        addCacheEntryToScanListAndNotify(
            env, threadEnvironment->_tenureCopyScanCache);
  ...
  bool MM_Scavenger::scavengeIncremental(MM_EnvironmentBase * env,
                                         int64_t scavengeIncrementEndTime) {
    bool result = false;
    bool timeout = false;
  ...
  void MM_Scavenger::workThreadScan(MM_EnvironmentStandard * env) {
    ...
    // we probably have to clear all things for master since it'll be doing
    // final release/clear on behalf of mutator threads
  ...
  uintptr_t MM_Scavenger::scavengeConcurrent(MM_EnvironmentBase * env,
                                             UDATA totalBytesToScavenge,
                                             volatile bool *forceExit) {
    Assert_MM_true(concurrent_state_scan == _concurrentState);

    MM_ConcurrentScavengeTask scavengeTask(
        env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_SCAN,
        totalBytesToScavenge, forceExit, env->_cycleState);
    _dispatcher->run(env, &scavengeTask);

    uintptr_t bytesScanned = scavengeTask.getBytesScanned();
    /* we can't assert the work queue is empty. some mutator threads could have
     * just flushed their copy caches, after the task terminated */
    _concurrentState = concurrent_state_complete;
  ...
  uintptr_t MM_Scavenger::masterThreadConcurrentCollect(MM_EnvironmentBase *
                                                        env) {
    /* note that we can't check isConcurrentWorkAvailable at this point since
     * another thread could have set _forceConcurrentTermination since the
    ...
    /* We pass a pointer to _forceConcurrentTermination so that we can cause the
     * concurrent to terminate early by setting the
     * flag to true if we want to interrupt it so that the master thread returns
     * to the control mutex in order to receive a
     * new GC request.
     */
    uintptr_t bytesConcurrentlyScanned =
        scavengeConcurrent(env, 100000, &_forceConcurrentTermination);
  ...
  void MM_Scavenger::triggerConcurrentScavengerTransition(
     ...
     * will be done indirectly,
     * first time a thread acquires VM access after exclusive VM access is
     * released, through a VM access hook. */
    GC_OMRVMThreadListIterator threadIterator(_extensions->getOmrVM());
    OMR_VMThread *walkThread = NULL;
