...
class MM_ConcurrentCardTableForWC : public MM_ConcurrentCardTable {
  ...
  uintptr_t countCardsInRange(MM_EnvironmentStandard *env, Card *rangeStart,
                              Card *rangeEnd);
...
public:
  ...
  void prepareCardTableChunk(MM_EnvironmentStandard *env, Card *chunkStart,
   ...
   * CARD_DIRTY
   * as we know such cards are sure to be cleaned in future either after a STW
   * card table
   * prepare phase or during FCC.
   */
  MMINLINE virtual bool
  isObjectInUncleanedDirtyCard(MM_EnvironmentStandard *env,
