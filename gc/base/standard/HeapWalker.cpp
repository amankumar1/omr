...
static void heapWalkerObjectSlotDo(OMR_VMThread *omrVMThread,
   ...
   * everywhere.
   */
  omrobjectptr_t clazzObject =
      MM_GCExtensionsBase::getExtensions(omrVM)
          ->collectorLanguageInterface->heapWalker_heapWalkerObjectSlotDo(
              object);
...
MM_HeapWalker *MM_HeapWalker::newInstance(MM_EnvironmentBase *env) {
  ...
  heapWalker = (MM_HeapWalker *)env->getForge()->allocate(
      sizeof(MM_HeapWalker), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
  if (heapWalker) {
...
void MM_HeapWalker::allObjectSlotsDo(MM_EnvironmentBase *env,
   ...
   * walk
   * new spaces, so we must also go through all slots of objects in the
   * remembered set.
   */
