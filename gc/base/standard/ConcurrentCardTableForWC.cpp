...
MM_ConcurrentCardTableForWC::newInstance(MM_EnvironmentBase *env, MM_Heap *heap,
  ...
  cardTable = (MM_ConcurrentCardTableForWC *)env->getForge()->allocate(
      sizeof(MM_ConcurrentCardTableForWC), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (NULL != cardTable) {
 ...
 *
 */
void MM_ConcurrentCardTableForWC::prepareCardTable(
    MM_EnvironmentStandard *env) {
 ...
 * could
 * be reversed so a card is dirtied before the associated update is visible. As
 * we need
 * to be sure that we see all the updated references when we rescan the cards
 * marked
 * objects we need to ensure the card table and objects are synchronized. As we
 * don't want
 * to have to issue a store barrier after every reference update (far too
 * expensive) we
 * instead stop all threads to ensure all its updates are flushed. We then walk
 * the card table
 * and flag any dirty cards (i.e set them to CARD_CLEAN_SAFE) to show that all
 * updates to its
 * associated objects are visible. We then restart all threads and start
 * concurrent card cleaning.
 * If a card is subsequently redirtied it is set again to CARD_DIRTY by the
 * write barrier.
 *
 * Thus during the concurrent phase we can clean all unclean cards (CARD_DIRTY
 * or CARD_CLEAN_SAFE)
 * but any marked as CARD_DIRTY at final card cleaning time must be re-cleaned
 * to ensure we don't
...
MMINLINE bool MM_ConcurrentCardTableForWC::getExclusiveCardTableAccess(
    MM_EnvironmentStandard *env, CardCleanPhase currentPhase,
    bool threadAtSafePoint) {
  /* Because the WC CardTable requires exclusive access to prepare the cards for
   * cleaning, we cannot gain
   * exclusive access to the card table if the thread is not at a safe point.
   * Request async call back
  ...
  while (!acquired) {
     ...
     * us to it and moved things along
     * anyway,or we may have collected and be in a different concurrent cycle.
     * If we are not in the same concurrent
     * collection cycle or the _cardCleanPhase is not as it was when we
     * requested exclusive access then nothing to do
     * anymore. Otherwise if we got beat to exclusive access request it again
     * and re-check, if we got it prepare the
     * card table.
     */
...
uintptr_t MM_ConcurrentCardTableForWC::countCardsInRange(
    MM_EnvironmentStandard *env, Card *chunkStart, Card *chunkEnd) {
 ...
 * needs preparing
 * for card cleaning; be it concurrent or final card cleaning. As the chunk may
 * contain
 * holes (non-contiguous heap) we use the _cleaningRanges array to determine
 * which cards within
 * the chunk actually need processing, ie a chunk can span part of one or more
 * cleaning ranges.
 * The "action" passed to this function defines what we do to each unclean card
 * within the chunk.
 * If action is MARK_DIRTY_CARD_SAFE we modify all cards with value CARD_DIRTY
 * to CARD_CLEAN_SAFE;
 * for action MARK_SAFE_CARD_DIRTY we reset any cards marked as CARD_CLEAN_SAFE
 * to CARD_DIRTY.
 *
 ...
 * MARK_DIRTY_CARD_SAFE or
 * MARK_SAFE_CARD_DIRTY.
 */
void MM_ConcurrentCardTableForWC::prepareCardTableChunk(
    MM_EnvironmentStandard *env, Card *chunkStart, Card *chunkEnd,
    CardAction action) {
    ...
    while (prepareSizeRemaining > 0) {
      ...
      if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
        ...
        for (Card *currentCard = firstCard; currentCard < endCard;
           ...
           * table
           * found. This is based on the premise that the card table will be
           * mostly
           * empty and scanning an uintptr_t at a time will reduce the time
           * taken to
           * scan the card table.
           */
 ...
 * ensure
 * we managed to process them all, i.e. did we complete concurrent card cleaning
 * pass. If
 * not we need to reset any prepared cards (i.e those set to CARD_CLEAN_SAFE)
 * which were
 * not cleaned back to card_DIRTY so that they are rescanned during final card
 * cleaning.
 */
