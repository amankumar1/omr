...
MM_ParallelSweepScheme::newInstance(MM_EnvironmentBase *env) {
  ...
  sweepScheme = (MM_ParallelSweepScheme *)env->getForge()->allocate(
      sizeof(MM_ParallelSweepScheme), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (sweepScheme) {
 ...
 * sweep the area to include
 * inner free list information as well as surrounding details (leading/trailing,
 * projection, etc).  The routine
 * is self contained to run on an individual chunk, and does not rely on
 * surrounding chunks having been processed.
...
bool MM_ParallelSweepScheme::sweepChunk(MM_EnvironmentBase *env,
  ...
  if (darkMatterSamples == 0) {
    ...
    if (projectedDarkMatter >= (chunkSize - freeSpace)) {
       ...
       * the candidates.
       * It usually only happens for very small samples, and will cause us to
       * estimate more dark
       * matter than is possible. In this case, don't apply the projection. Just
       * record what we
 ...
 * associated memory subspace.
 * This requires that all previous chunks in the memory subspace have been
 * swept.
 ...
 * work towards completing the sweep as it wants.  In this case, a full sweep of
 * all memory spaces will be performed.
 *
