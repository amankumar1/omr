...
MM_CompactScheme::newInstance(MM_EnvironmentBase *env,
  ...
  compactScheme = (MM_CompactScheme *)env->getForge()->allocate(
      sizeof(MM_CompactScheme), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (compactScheme) {
...
void MM_CompactScheme::masterSetupForGC(MM_EnvironmentStandard *env) {
  ...
  _extensions->collectorLanguageInterface
      ->compactScheme_languageMasterSetupForGC(env);
}
...
void MM_CompactScheme::compact(MM_EnvironmentBase *envBase,
  ...
  if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
     ...
     * the initialization doesn't need to be
     * done at a synchronize point?
     */
    masterSetupForGC(env);
#if defined(DEBUG)
    _extensions->collectorLanguageInterface->compactScheme_verifyHeap(env,
                                                                      _markMap);
#endif /* DEBUG */
   ...
   * avoid potentially having
   *    multiple holes created per segment, thereby fragmenting the space. This
   * will result in
   *    singlethreaded compaction per segment, and so should only be done in
   * extreme OOM situations.
   *  o no slave GC threads
   *  o the J9HOOK_MM_OMR_OBJECT_RENAME hook has registered users. JVMPI does
   * not support events being issued
   * 	  in parallel so we force single sub area compact to ensure all events
   * issued under master GC thread.
   */
  if (aggressive || 1 == env->_currentTask->getThreadCount() ||
      J9_EVENT_IS_HOOKED(_extensions->omrHookInterface,
  ...
  _extensions->collectorLanguageInterface->compactScheme_workerCleanupAfterGC(
      env);
...
omrobjectptr_t MM_CompactScheme::doCompact(
  ...
  for (objectPtr = markedObjectIterator.nextObject(); objectPtr != 0;
    ...
    if (deadObject == objectPtr) {
       ...
       * more objects
       * as this is not needed and it will cause corruption if objects grow
       * after this point.
       */
    ...
    TRIGGER_J9HOOK_MM_OMR_OBJECT_RENAME(env->getExtensions()->omrHookInterface,
                                        env->getOmrVMThread(), objectPtr,
                                        deadObject);
 ...
 * any areas which
 * weren't compacted. In Tarok, regions are entirely compacted or entirely fixed
 * up. There is
