...
MM_ParallelGlobalGC::newInstance(MM_EnvironmentBase *env,
  ...
  globalGC = (MM_ParallelGlobalGC *)env->getForge()->allocate(
      sizeof(MM_ParallelGlobalGC), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (globalGC) {
...
bool MM_ParallelGlobalGC::initialize(MM_EnvironmentBase *env) {
  ...
  if (!_cli->parallelGlobalGC_createHeapWalker(env, this,
  ...
  (*mmPrivateHooks)
      ->J9HookRegister(mmPrivateHooks,
                       ...
                       globalGCHookAFCycleStart, NULL);
  (*mmPrivateHooks)
      ->J9HookRegister(mmPrivateHooks,
                       J9HOOK_MM_PRIVATE_ALLOCATION_FAILURE_CYCLE_END,
                       globalGCHookAFCycleEnd, NULL);

#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
  (*mmPrivateHooks)
      ->J9HookRegister(mmPrivateHooks,
                       J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START,
                       globalGCHookCCStart, NULL);
  (*mmPrivateHooks)
      ->J9HookRegister(mmPrivateHooks,
                       J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END,
                       globalGCHookCCEnd, NULL);
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */

  (*mmPrivateHooks)
      ->J9HookRegister(mmPrivateHooks, J9HOOK_MM_PRIVATE_SYSTEM_GC_START,
                       globalGCHookSysStart, NULL);
  (*mmPrivateHooks)
      ->J9HookRegister(mmPrivateHooks, J9HOOK_MM_PRIVATE_SYSTEM_GC_END,
                       globalGCHookSysEnd, NULL);
 ...
 ****************************************
 */
void MM_ParallelGlobalGC::setupBeforeGC(MM_EnvironmentBase *env) {
...
bool MM_ParallelGlobalGC::shouldCompactThisCycle(
   ...
   * point doing
   * a compaction, we just need to expand. However, if we are fully expanded but
   * low
   * on free memory there is little more we can do to avoid OOM at this point
   * other than
...
compactionReqd:
  compactPreventedReason =
      _extensions->collectorLanguageInterface
          ->parallelGlobalGC_checkIfCompactionShouldBePrevented(env);
  if (COMPACT_PREVENTED_NONE != compactPreventedReason) {
    goto nocompact;
  }
...
void MM_ParallelGlobalGC::masterThreadReportObjectEvents(
...
void MM_ParallelGlobalGC::masterThreadRestartAllocationCaches(
  ...
  while ((walkThread = vmThreadListIterator.nextOMRVMThread()) != NULL) {
     ...
     * uses this as the reset point for all threads scanned state. This should
     * really be moved
...
void MM_ParallelGlobalGC::processLargeAllocateStatsAfterSweep(
  ...
  if (GLOBALGC_ESTIMATE_FRAGMENTATION ==
      (_extensions->estimateFragmentation & GLOBALGC_ESTIMATE_FRAGMENTATION)) {
...
bool MM_ParallelGlobalGC::heapAddRange(MM_EnvironmentBase *env,
  ...
  result = _cli->parallelGlobalGC_heapAddRange(env, subspace, size, lowAddress,
                                               highAddress);
  if (0 == result) {
...
bool MM_ParallelGlobalGC::heapRemoveRange(MM_EnvironmentBase *env,
  ...
  bool result = _markingScheme->heapRemoveRange(env, subspace, size, lowAddress,
                                                ...
                                                highValidAddress);
  result = result &&
           _sweepScheme->heapRemoveRange(env, subspace, size, lowAddress,
                                         highAddress, lowValidAddress,
                                         highValidAddress);

  result = result &&
           _cli->parallelGlobalGC_heapRemoveRange(
 ...
 * compact will obviously result
 * in a large increase in time in GC and could result in an
 * unexpected/undesirable ratio EXPAND on next GC
 */
#if defined(OMR_GC_MODRON_COMPACTION)
...
void MM_ParallelGlobalGC::reportGCCycleEnd(MM_EnvironmentBase *env) {
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_GC_POST_CYCLE_END(
      ...
      _extensions->getHeap()->initializeCommonGCData(env, &commonData),
      env->_cycleState->_type, _extensions->globalGCStats.workPacketStats
...
void MM_ParallelGlobalGC::reportGCEnd(MM_EnvironmentBase *env) {
  ...
  Trc_MM_GlobalGCEnd(
      env->getLanguageVMThread(), _extensions->globalGCStats.workPacketStats
                                      .getSTWWorkStackOverflowOccured(),
      _extensions->globalGCStats.workPacketStats.getSTWWorkStackOverflowCount(),
      approximateTotalActiveFreeMemorySize, totalActiveMemorySizeTotal);

  Trc_OMRMM_GlobalGCEnd(
      env->getOmrVMThread(), _extensions->globalGCStats.workPacketStats
                                 .getSTWWorkStackOverflowOccured(),
      _extensions->globalGCStats.workPacketStats.getSTWWorkStackOverflowCount(),
      approximateTotalActiveFreeMemorySize, totalActiveMemorySizeTotal);
