 ...
 * heap and expected
 * configuration (generations, regions, etc) and determines the approximate
 * maximum number of chunks
 * that will be required for a sweep at any given time.  It is safe to
 * underestimate the number of chunks,
 * as the sweep sectioning mechnanism will compensate, but the the expectation
 * is that by having all
 * chunk memory allocated in one go will keep the data localized and fragment
 * system memory less.
...
uintptr_t MM_SweepHeapSectioningSegmented::estimateTotalChunkCount(
   ...
   * the real numbers.
   * Assume that if the scavenger is enabled, each of the semispaces will need
   * an extra chunk */
 ...
 * represent the current heap.
 * The chunk calculation is done on a per segment basis (no segment can
 * represent memory from more than 1 chunk),
 * and partial sized chunks (ie: less than the chunk size) are reserved for any
 * remaining space at the end of a
 * segment.
 ...
 * walk the chunk
 * list reassigning each chunk to an appropriate range of memory.  This will
 * clear each chunk
 * structure and then assign its basic values that connect it to a range of
 * memory (base/top,
 * pool, segment, etc).
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    if (region->isCommitted()) {
      ...
      while (heapChunkBase < regionHighAddress) {
         ...
         * pools.  If it does,
         * the current chunk can only be attributed to one, so we limit the
         * upper range of the chunk
         * to the first pool and will continue the assignment at the upper
         * address range.
         */
...
MM_SweepHeapSectioningSegmented::newInstance(MM_EnvironmentBase *env) {
  MM_SweepHeapSectioningSegmented *sweepHeapSectioning =
      (MM_SweepHeapSectioningSegmented *)env->getForge()->allocate(
          sizeof(MM_SweepHeapSectioningSegmented), MM_AllocationCategory::FIXED,
          OMR_GET_CALLSITE());
