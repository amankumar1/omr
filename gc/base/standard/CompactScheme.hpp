...
protected:
  ...
  MM_HeapRegionManager *_rootManager; /**< The root region manager which holds
                                         the MM_HeapRegionDescriptor instances
                                         which manage the properties of the
                                         regions */
...
public:
   ...
   * Mark Map
   * In Compressed Mark Map one bit represents twice more space then one bit of
   * regular Mark Map
   * So, sizeof_page should be double of number of bytes represented by one
   * uintptr_t in Mark Map
   */
...
*/
   ...
   * object
   * Each bit in Compressed Mark Map represents twice more heap bytes then
   * regular mark map
   * Each page represents number of bytes in heap might be covered by one
   * Compressed Mark Map slot (uintptr_t)
   */
   ...
   * action. This allows
   * a worker thread to claim responsibility for performing the specified action
   * on the specified
   * area.
   *
   * @param env[in] the current thread
...
public:
  ...
  MM_CompactScheme(MM_EnvironmentBase *env, MM_MarkingScheme *markingScheme)
      : MM_BaseVirtual(), _omrVM(env->getOmrVM()),
        ...
        _subAreaTableSize(0), _subAreaTable(NULL) {
