...
protected:
  ...
  MM_MarkMap
      *_currentMarkMap; /**< The MarkMap which the ParallelGlobalGC gave to us
                           to use for this cycle */
  uint8_t *_currentSweepBits; /*< The base address of the raw bits used by the
                                 _currentMarkMap (sweep knows about this in
                                 order to perform some optimized types of map
                                 walking) */
  ...
  omrthread_monitor_t
      _mutexSweepPoolState; /**< Monitor to protect memory pool operations for
                               sweepPoolState*/
...
public:
   ...
   * Called by the ParallelGlobalGC to configure sweep and could
  ...
  */
   ...
   * at heapSlotFreeCurrent.
   * Leading dark matter (before the first marked object) is not included.
   * Trailing dark matter (after the last marked object) is included, even if it
