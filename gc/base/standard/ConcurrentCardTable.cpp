 ...
 */
uintptr_t
MM_ConcurrentCardTable::calculateTLHMarkMapSize(MM_EnvironmentStandard *env,
...
bool MM_ConcurrentCardTable::allocateCardTableEntriesForHeapRange(
    MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace, uintptr_t size,
...
bool MM_ConcurrentCardTable::freeCardTableEntriesForHeapRange(
    MM_EnvironmentStandard *env, uintptr_t size, void *lowAddress,
...
bool MM_ConcurrentCardTable::allocateTLHMarkMapEntriesForHeapRange(
  ...
  if (_extensions->isFvtestForceConcurrentTLHMarkMapCommitFailure()) {
    ...
    if (NULL != _tlhMarkBits) {
       ...
       * the highTLHMarkMap address of another range).
       * as we dont want to commit a slot we have previously commited andn more
       * importantly we dont want to clear and existing
       * slot. */
       ...
       * calculated.
       * However as we always commit multiples of uintptr_t we need to check if
       * the the above
       * shift has rounded down the number of uintptr_t slots we need to commit.
       * If it has
       * we need to commit an extra uintptr_t worth otherwise will will end up
       * trying to set
...
bool MM_ConcurrentCardTable::freeTLHMarkMapEntriesForHeapRange(
  ...
  if (_extensions->isFvtestForceConcurrentTLHMarkMapDecommitFailure()) {
    ...
    if (NULL != _tlhMarkBits) {
       ...
       * calculated.
       * However as we always decommit multiples of uintptr_t we need to check
       * if the the above
       * shift has rounded down the number of uintptr_t slots we need to commit.
       * If it has
       * we need to commit an extra uintptr_t worth otherwise will will end up
 ...
 */
bool MM_ConcurrentCardTable::heapAddRange(MM_EnvironmentStandard *env,
 ...
 *
 */
bool MM_ConcurrentCardTable::heapRemoveRange(MM_EnvironmentStandard *env,
                                             MM_MemorySubSpace *subspace,
                                             uintptr_t size, void *lowAddress,
                                             void *highAddress,
 ...
 *
 */
void MM_ConcurrentCardTable::heapReconfigured(MM_EnvironmentStandard *env) {}
...
MM_ConcurrentCardTable::newInstance(MM_EnvironmentBase *env, MM_Heap *heap,
  ...
  MM_ConcurrentCardTable *cardTable =
      (MM_ConcurrentCardTable *)env->getForge()->allocate(
          sizeof(MM_ConcurrentCardTable), MM_AllocationCategory::FIXED,
          OMR_GET_CALLSITE());
  if (NULL != cardTable) {
 ...
 */
bool MM_ConcurrentCardTable::initialize(MM_EnvironmentBase *envModron,
                                        MM_Heap *heap) {
  MM_EnvironmentStandard *env =
 ...
 */
...
#endif /* OMR_GC_MODRON_SCAVENGER */
    {
       ...
       * bits */
      (*mmPrivateHooks)
          ->J9HookRegister(mmPrivateHooks, J9HOOK_MM_PRIVATE_CACHE_CLEARED,
                           tlhCleared, (void *)this);
      (*mmPrivateHooks)
          ->J9HookRegister(mmPrivateHooks, J9HOOK_MM_PRIVATE_CACHE_REFRESHED,
                           tlhRefreshed, (void *)this);
    }
 ...
 */
uintptr_t
MM_ConcurrentCardTable::cardBytesForHeapRange(MM_EnvironmentStandard *env,
 ...
 * non-concurrently
 * collectable subspaces, e.g. allocate subspaces, clear the corresponding cards
 * in card
 * table.
 *
 */
void MM_ConcurrentCardTable::clearNonConcurrentCards(
    MM_EnvironmentStandard *env) {
 ...
 */
void MM_ConcurrentCardTable::dirtyCardsInRange(MM_EnvironmentStandard *env,
                                               void *heapBase, void *heapTop) {
 ...
 *
 */
void MM_ConcurrentCardTable::initializeCardCleaning(
    MM_EnvironmentStandard *env) {
 ...
 *
 */
void MM_ConcurrentCardTable::prepareCardsForCleaning(
    MM_EnvironmentStandard *env) {
  void *firstFree;
  MM_MemorySubSpace *subspace;
  ...
  case PHASE1_PREPARING:
     ...
     * start of free list */
    _lastCardInPhase =
        (firstFree > 0) ? heapAddrToCardAddr(env, firstFree) : _lastCard;
 ...
 */
MMINLINE bool
MM_ConcurrentCardTable::getExclusiveCardTableAccess(MM_EnvironmentStandard *env,
 ...
 */
bool MM_ConcurrentCardTable::cleanCards(MM_EnvironmentStandard *env,
  ...
  *sizeDone = 0;
  ...
  MM_ConcurrentGCStats *stats = _extensions->collectorLanguageInterface
                                    ->concurrentGC_getConcurrentStats();
  while (cleanedSoFar < sizeToDo && currentCleaningPhase == _cardCleanPhase) {
     ...
     *overflow has
     * occurred then we are done as all live objects in the card will be
     *processed later. This
     * is true as we know the object will have been pushed to a work packet when
     *it was marked
     ...
     *not hold as to
     * relieve work stack overflow we empty packets by dirtying cards for their
     *referenced
     * objects. Therefore we cannot be sure tracing into all active TLH's will
     *be deferred.
     */
   ...
   * call
   * to getNextDirtyCard so we may count the occasional card in wrong phase but
   * the
 ...
 */
bool MM_ConcurrentCardTable::cleanSingleCard(MM_EnvironmentStandard *env,
   ...
   * pay at least as big as
   * one cards worth. If last object is large pointer array  we may only
   * partially retrace it but it
  ...
  while (NULL != (objectPtr = markedObjectIterator.nextObject())) {
     ...
     * If the object is in an active TLH and provided no concurrent work stack
     *overflow has
     * occurred then we are done as we know this object, and any others in the
     *same
     * card,  will be processed later. This is true as we know the object will
     *have been
     ...
     *don't need to
     * re-dirty the card as we know we will revisit it again later, and all
     *cards for an
     ...
     * relieve work stack overflow we empty packets by dirtying cards for their
     *referenced
     * objects. Therefore we cannot be sure all tracing into this particular TLH
     *will be
     * deferred.
    */
    ...
    sizeDone += _markingScheme->scanObjectWithSize(
        env, objectPtr, MM_CollectorLanguageInterface::SCAN_REASON_DIRTY_CARD,
        (sizeToDo - sizeDone));
  }
 ...
 *
 */
bool MM_ConcurrentCardTable::finalCleanCards(MM_EnvironmentStandard *env,
  ...
  for (; (nextDirtyCard = getNextDirtyCard(env, _finalCardCleanMask, false)) !=
    ...
    while (NULL != (objectPtr = markedObjectIterator.nextObject())) {
      objects += 1;
      traceCount += _markingScheme->scanObjectWithSize(
          env, objectPtr, MM_CollectorLanguageInterface::SCAN_REASON_DIRTY_CARD,
          SCAN_MAX);
    }
 ...
 *
 */
void MM_ConcurrentCardTable::processTLHMarkBits(MM_EnvironmentStandard *env,
                                                MM_MemorySubSpace *subspace,
  ...
  if (base < top) {
     ...
     */
    if ((CLEAR == action) &&
        !(_extensions->collectorLanguageInterface
              ->concurrentGC_getConcurrentStats()
    ...
    if (SET == action) {
       ...
       * thread
       * can possibly attempt to trace an object in a active TLH and find the
       * TLH bits
       * still off.
       */
      MM_AtomicOperations::storeSync();
 ...
 */
bool MM_ConcurrentCardTable::isCardTableEmpty(MM_EnvironmentStandard *env) {
  bool empty = true;
 ...
 */
bool MM_ConcurrentCardTable::isTLHMarkBitsEmpty(MM_EnvironmentStandard *env) {
  bool empty = true;
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    ...
    if (region->getSubSpace()->isConcurrentCollectable()) {
       ...
       * don't want to miss
       * last few bits. We always round up the high address when we commit and
       * clear so this should be
       * OK.
       */
 ...
 * otherwise
 */
bool MM_ConcurrentCardTable::isObjectInActiveTLH(MM_EnvironmentStandard *env,
 ...
 */
bool MM_ConcurrentCardTable::isCardInActiveTLH(MM_EnvironmentStandard *env,
                                               Card *card) {
  uintptr_t markIndex, markBit;
  bool active;
 ...
 * otherwise
 */
bool MM_ConcurrentCardTable::isObjectInDirtyCardNoCheck(
    MM_EnvironmentStandard *env, omrobjectptr_t object) {
  bool dirty;
 ...
 * otherwise
 */
bool MM_ConcurrentCardTable::isObjectInDirtyCard(MM_EnvironmentStandard *env,
                                                 omrobjectptr_t object) {
  bool dirty = false;
 ...
 *
 */
void MM_ConcurrentCardTable::determineCleaningRanges(
    MM_EnvironmentStandard *env) {
  bool initDone = false;
  ...
  while (!initDone) {
    ...
    if (numRanges > _maxCleaningRanges) {
      ...
      _cleaningRanges = (CleaningRange *)env->getForge()->allocate(
          sizeRequired, MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
 ...
 *
 */
void MM_ConcurrentCardTable::resetCleaningRanges(MM_EnvironmentStandard *env) {
 ...
 */
Card *MM_ConcurrentCardTable::getNextDirtyCard(MM_EnvironmentStandard *env,
                                               Card cardMask,
  ...
  while (NULL != firstCard) {
    ...
    for (currentCard = firstCard; currentCard < lastCardToClean;
       ...
       * uintptr_t
       * at a time until we find a slot which is non-zero or the end of card
       * table
     ...
     * scan ptr
      ...
      if (firstCard != (Card *)currentRange->nextCard) {
         ...
         * fail
         * then someone beat us to it so re-sync with race winner and start
         * again
         */
    ...
    if (currentCard < lastCardToClean) {
       ...
       * need
       * to restart scan. First though make sure no thread is waiting for
       * exclusive access.
       */
      if (concurrentCardClean && env->isExclusiveAccessRequestWaiting()) {
        return (Card *)EXCLUSIVE_VMACCESS_REQUESTED;
       ...
       * to clean
       * cards in next phase we start at last card in this phase. If we fail
       * then
 ...
 *
 */
MMINLINE void MM_ConcurrentCardTable::setTLHMarkBits(
    MM_EnvironmentStandard *env, uintptr_t slotIndex, uintptr_t slotBits) {
 ...
 *
 */
MMINLINE void MM_ConcurrentCardTable::clearTLHMarkBits(
    MM_EnvironmentStandard *env, uintptr_t slotIndex, uintptr_t slotBits) {
  volatile uintptr_t *tlhMarkMapSlot = (uintptr_t *)&(_tlhMarkBits[slotIndex]);
  uintptr_t oldValue, newValue;
 ...
 *
 */
bool MM_ConcurrentCardTable::cardHasMarkedObjects(MM_EnvironmentStandard *env,
                                                  Card *card) {
  bool hasMarked;
