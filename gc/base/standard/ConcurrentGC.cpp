...
void J9ConcurrentWriteBarrierStore(OMR_VMThread *vmThread,
  ...
  MM_EnvironmentStandard *env =
      MM_EnvironmentStandard::getEnvironment(vmThread);
...
int J9THREAD_PROC con_helper_thread_proc(void *info) {
  ...
  omrsig_handler_fn signalHandler =
      conHelperThreadInfo->collector->_cli
          ->concurrentGC_getProtectedSignalHandler(&signalHandlerArg);
...
uintptr_t con_helper_thread_proc2(OMRPortLibrary *portLib, void *info) {
  ...
  OMR_VMThread *omrThread = collector->_cli->attachVMThread(
      omrVM, "Concurrent Mark Helper",
      MM_CollectorLanguageInterfaceImpl::ATTACH_GC_HELPER_THREAD);
...
void MM_ConcurrentGC::reportConcurrentKickoff(MM_EnvironmentStandard *env) {
...
void MM_ConcurrentGC::reportConcurrentAborted(MM_EnvironmentStandard *env,
...
void MM_ConcurrentGC::reportConcurrentHalted(MM_EnvironmentStandard *env) {
  ...
  Trc_MM_ConcurrentHaltedState(
      ...
      _cli->concurrentGC_isConcurrentScanningComplete(env) ? "complete"
                                                           : "incomplete",
...
void MM_ConcurrentGC::reportConcurrentFinalCardCleaningEnd(
    MM_EnvironmentStandard *env, uint64_t duration) {
  MM_ConcurrentCardTable *cardTable = (MM_ConcurrentCardTable *)_cardTable;
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ConcurrentGC::reportConcurrentCollectionStart(
    MM_EnvironmentStandard *env) {
  MM_ConcurrentCardTable *cardTable = (MM_ConcurrentCardTable *)_cardTable;
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ConcurrentGC::reportConcurrentCollectionEnd(MM_EnvironmentStandard *env,
                                                    uint64_t duration) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ConcurrentGC::reportConcurrentBackgroundThreadFinished(
    MM_EnvironmentStandard *env, uintptr_t traceTotal) {
 ...
 * This is a wrapper into the non-static
 * MM_ConcurrentGC::objectRemovedFromRememberedSet
 */
void MM_ConcurrentGC::hookObjectRemovedFromRememberedSet(J9HookInterface **hook,
                                                         uintptr_t eventNum,
                                                         void *eventData,
                                                         void *userData) {
  MM_ObjectRemovedFromRememberedSetEvent *event =
      (MM_ObjectRemovedFromRememberedSetEvent *)eventData;
  MM_EnvironmentStandard *env =
      MM_EnvironmentStandard::getEnvironment(event->currentThread);

  ((MM_ConcurrentGC *)userData)
      ->objectRemovedFromRememberedSet(
          env, static_cast<omrobjectptr_t>(event->objectPtr));
}
 ...
 * starts.
 * This is a wrapper into the non-static
 * MM_ConcurrentGC::recordCardCleanPass2Start
 */
void MM_ConcurrentGC::hookCardCleanPass2Start(J9HookInterface **hook,
                                              uintptr_t eventNum,
 ...
 *
 */
void MM_ConcurrentGC::signalThreadsToDirtyCardsAsyncEventHandler(
    OMR_VMThread *omrVMThread, void *userData) {
  MM_ConcurrentGC *collector = (MM_ConcurrentGC *)userData;
  MM_EnvironmentStandard *env =
      MM_EnvironmentStandard::getEnvironment(omrVMThread);
...
MM_ConcurrentGC::newInstance(MM_EnvironmentBase *env,
  ...
  MM_ConcurrentGC *concurrentGC = (MM_ConcurrentGC *)env->getForge()->allocate(
      sizeof(MM_ConcurrentGC), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
 ...
 * all
 * monitors required by concurrent. Allocate and initialize the concurrent
 * helper thread
 * table.
 *
...
bool MM_ConcurrentGC::initialize(MM_EnvironmentBase *env) {
  ...
  if (_extensions->optimizeConcurrentWB) {
    _callback =
        env->getExtensions()
            ->collectorLanguageInterface->concurrentGC_createSafepointCallback(
                env);
    if (NULL == _callback) {
      goto error_no_memory;
    }
    _callback->registerCallback(env, signalThreadsToDirtyCardsAsyncEventHandler,
                                this);
  }
  ...
  if (_conHelperThreads > 0) {
    ...
    _conHelpersTable = (omrthread_t *)env->getForge()->allocate(
        _conHelperThreads * sizeof(omrthread_t), MM_AllocationCategory::FIXED,
        OMR_GET_CALLSITE());
    if (!_conHelpersTable) {
      goto error_no_memory;
    }
  ...
  if (_extensions->debugConcurrentMark) {
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
    omrtty_printf("Initial tuning statistics: Card Cleaning Factors "
                  "Pass1=\"%.3f\" Pass2=\"%.3f\" (Maximum: Pass1=\"%.3f\" "
                  "Pass2=\"%.3f\")\n",
  ...
  if (_extensions->largeObjectArea) {
    if (_extensions->concurrentMetering == MM_GCExtensionsBase::METER_DYNAMIC) {
      ...
      _meteringHistory = (MeteringHistory *)env->getForge()->allocate(
          historySize, MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
  ...
  (*mmPrivateHooks)
      ->J9HookRegister(mmPrivateHooks,
                       J9HOOK_MM_PRIVATE_CARD_CLEANING_PASS_2_START,
                       hookCardCleanPass2Start, (void *)this);
  ...
  (*mmPrivateHooks)
      ->J9HookRegister(mmPrivateHooks,
                       ...
                       hookObjectRemovedFromRememberedSet, this);
#endif /* OMR_GC_MODRON_SCAVENGER */
...
bool MM_ConcurrentGC::createCardTable(MM_EnvironmentBase *env) {
  bool result = false;
 ...
 */
void MM_ConcurrentGC::determineInitWork(MM_EnvironmentStandard *env) {
  bool initDone = false;
  uintptr_t initWork;
  ...
  while (!initDone) {
    ...
    if (_numInitRanges > _numPhysicalInitRanges) {
      ...
      _initRanges = (InitWorkItem *)env->getForge()->allocate(
          sizeof(InitWorkItem) * _numInitRanges, MM_AllocationCategory::FIXED,
          OMR_GET_CALLSITE());
      if (NULL == _initRanges) {
        initDone = true;
      ...
      /* Add init ranges for all card table ranges for concurrently collectable
       * segments */
 ...
 */
void MM_ConcurrentGC::resetInitRangesForSTW(MM_EnvironmentStandard *env) {
  for (uint32_t i = 0; i < _numInitRanges; i++) {
 ...
 */
bool MM_ConcurrentGC::getInitRange(MM_EnvironmentStandard *env, void **from,
 ...
 */
bool MM_ConcurrentGC::tracingRateDropped(MM_EnvironmentStandard *env) {
  // Always return false for now until we understand how to fix this code to
  // ensure
  // we do not prematurely report that the rate has dropped and so start card
  // cleaning
  // too early.
  return false;
 ...
 * with
 * minimum priority so they do not compete with applciation threads for use of
 * procesors;
 * they are only intended to use up any spare cycles not used by application
 * threads.
 *
 ...
 *
 */
void MM_ConcurrentGC::resumeConHelperThreads(MM_EnvironmentStandard *env) {
  if (_conHelpersStarted > 0) {
    omrthread_monitor_enter(_conHelpersActivationMonitor);
    if (!env->isExclusiveAccessRequestWaiting()) {
 ...
 * cycle.
 */
void MM_ConcurrentGC::tuneToHeap(MM_EnvironmentStandard *env) {
  ...
  if (0 == _stats->getKickoffThreshold() || _retuneAfterHeapResize) {
    ...
    if ((NULL == env->_cycleState) ||
       ...
       * PLUS
       * the tracing we did to complete processing of any work packets that
       * remained at
       * the start of the collection PLUS tracing done during final card
       * cleaning.
       ...
       * best we can do
       * is estimate based on current heap size, live object factor, leaf object
       * factor etc.
       */
      totalBytesToTrace = (uintptr_t)(heapSize * _tenureLiveObjectFactor *
                                      _tenureNonLeafObjectFactor);
   ...
   * during the
   * initialization and marking phases we need to allow for that when
   * calculating
   * the KO point.
   */
  kickoffThreshold =
   ...
   * storage otherwise we will more than likley get an AF whilst last few
   * allocates
   * are paying finishing off the last bit of tracing. So we create a buffer
   * zone
 ...
 */
void MM_ConcurrentGC::adjustTraceTarget(MM_EnvironmentStandard *env) {
    ...
    */
    ...
    default:
      Assert_GC_true_with_message(
          env, (CONCURRENT_ROOT_TRACING <= executionModeAtGC) &&
 ...
 * current
 * allocation request. We dont need to worry about work being available at the
 * right time
 * as we do when tracing. Initialization work will either be available or it
 * will be all
 * allocated; one exhausted no more will arrive at a later time as is the case
 * with tracing.
 * We boost the target trace rate during initialization in order to minimize the
 * time to
 ...
 */
uintptr_t MM_ConcurrentGC::calculateInitSize(MM_EnvironmentStandard *env,
 ...
 */
uintptr_t
MM_ConcurrentGC::calculateTraceSize(MM_EnvironmentStandard *env,
   ...
   * more work
   * than we predicted calculate required trace rate for this allocate request
   * to keep us on
   * track.
   */
  if ((remainingFree > 0) && (workCompleteSoFar < traceTarget)) {
     ...
     * work we predicted and we are
     * still not done. We run a high risk of hitting AF before we do complete so
     * trace at the max from
 ...
 */
bool MM_ConcurrentGC::periodicalTuningNeeded(MM_EnvironmentStandard *env,
                                             uintptr_t freeSize) {
...
MMINLINE uintptr_t MM_ConcurrentGC::potentialFreeSpace(
    MM_EnvironmentStandard *env, MM_AllocateDescription *allocDescription) {
  ...
  if (LOA == _meteringType) {
    ...
    nurseryPromotion = scavengerStats->_avgTenureSOABytes == 0
                           ? 1
                           : scavengerStats->_avgTenureSOABytes;
  ...
  nurseryPromotion = scavengerStats->_avgTenureBytes == 0
                         ? 1
                         : scavengerStats->_avgTenureBytes;
  currentOldFree = oldSubspace->getApproximateActiveFreeMemorySize();
#endif
  ...
  /* KO concurrent one scavenge early to reduce chances of tenure space being
   * exhausted
   * before we complete concurrent marking by a scavenge tenuring significantly
   * more than
   ...
   * before
   * we will fill the tenure area. On 32 bit platforms this can be greater than
   * 4G
   * hence the 64 bit maths. We assume that the heap will not be big enough on
   * 64 bit
   ...
   * Therefore
   * if the amount of potential free space is greater than can be expressed in
   * 32
 ...
 * @param  freeSize the current amount of free space in the old area
 */
void MM_ConcurrentGC::periodicalTuning(MM_EnvironmentStandard *env,
                                       uintptr_t freeSize) {
 ...
 *
 */
void MM_ConcurrentGC::kickoffCardCleaning(MM_EnvironmentStandard *env,
 ...
 * given size.  See if there is work
 * that can be done to increase the free stores of the pool so that the request
 * can be met.
...
void MM_ConcurrentGC::payAllocationTax(
  ...
  if (CONCURRENT_OFF == _stats->getExecutionMode()) {
    if (!timeToKickoffConcurrent(envStandard, allocDescription)) {
      ...
      if (_extensions->concurrentSweep) {
        concurrentSweep(MM_EnvironmentStandard::getEnvironment(envStandard),
                        baseSubSpace, allocDescription);
      }
#endif /* OMR_GC_CONCURRENT_SWEEP */
      return;
    }
 ...
 * @note This is a potential GC point.
 */
void MM_ConcurrentGC::concurrentMark(MM_EnvironmentStandard *env,
...
/* .. we must not mark anything if WB not yet active */
  ...
  while (!taxPaid) {
    ...
    case CONCURRENT_EXHAUSTED:
       ...
       * can trigger
       * the final collection here provided we are at a safe point. If not then
       * GC will be
    ...
    case CONCURRENT_ROOT_TRACING:
      nextExecutionMode =
          _cli->concurrentGC_getNextTracingMode(CONCURRENT_ROOT_TRACING);
      Assert_GC_true_with_message(
          env, (CONCURRENT_ROOT_TRACING < nextExecutionMode) ||
    ...
    default:
       ...
       * CONCURRENT_ROOT_TRACING */
      Assert_GC_true_with_message(
          env, (CONCURRENT_ROOT_TRACING < executionMode) &&
      ...
      if (_stats->switchExecutionMode(executionMode, nextExecutionMode)) {
        Assert_GC_true_with_message2(
            env, (CONCURRENT_ROOT_TRACING < nextExecutionMode) &&
                     (CONCURRENT_TRACE_ONLY >= nextExecutionMode),
            "MM_ConcurrentStats::_executionMode = %zu; "
            "_cli->concurrentGC_getNextTracingMode(MM_ConcurrentStats::_"
            "executionMode) = %zu\n",
            executionMode, nextExecutionMode);
        /* Collect some roots */
        bool collectedRoots = false;
        _cli->concurrentGC_collectRoots(env, executionMode, &collectedRoots,
                                        &taxPaid);
        if (collectedRoots) {
...
void MM_ConcurrentGC::signalThreadsToDirtyCards(MM_EnvironmentStandard *env) {
...
bool MM_ConcurrentGC::forceKickoff(MM_EnvironmentBase *env,
  ...
  if (_extensions->concurrentKickoffEnabled &&
      _cli->concurrentGC_forceConcurrentKickoff(env, gcCode,
                                                &_languageKickoffReason)) {
...
void MM_ConcurrentGC::concurrentSweep(
    MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace,
    MM_AllocateDescription *allocDescription) {
 ...
 */
void MM_ConcurrentGC::completeConcurrentSweep(MM_EnvironmentStandard *env) {
 ...
 */
uintptr_t
MM_ConcurrentGC::doConcurrentInitialization(MM_EnvironmentStandard *env,
                                            uintptr_t initToDo) {
...
/* Determine how much "taxable" free space remains to be allocated. */
  ...
  if (_cli->concurrentGC_startConcurrentScanning(
   ...
   * thread
   * requests exclusive VM access to do a gc, or another thread has swithed to
   * exhausted
   */
    ...
    if (!env->isExclusiveAccessRequestWaiting() && sizeTraced < sizeToTrace) {
      ...
      if (CONCURRENT_CLEAN_TRACE == _stats->getExecutionMode()) {
        if (!((MM_ConcurrentCardTable *)_cardTable)->isCardCleaningComplete()) {
           ...
           * trace. If we have not scanned
           * this thread yet (and this should be a very rare event) then break
           * out of tracing loop and return.
           * Hopefully this will allow the thread to get to and async message
           * check and so call us back to
          ...
          if (!env->isThreadScanned()) {
            ...
            if (_markingScheme->getWorkPackets()->tracingExhausted()) {
               ...
               * threads.
               * If we get here we have finished all card cleaning and cannot
               * get an input packet so we
               * must be near completing the tracing. However, the lower
               * priority concurrent helpers could be suspended and
               * hanging onto packets and so preventing us from getting to
               * CONCURENT_EXHAUSTED. This will delay
               * the final collection; maybe until an AF which we don't want. We
               * need to tell the concurrent helpers
               * to release any packets they have so we can get them processed.
               * This is easily achieved by suspending them.
               * Anyway, now is also a good time to tell the concurrent helpers
               * to park themselves for the imminent global collect.
               */
 ...
 */
bool MM_ConcurrentGC::concurrentFinalCollection(MM_EnvironmentStandard *env,
                                                MM_MemorySubSpace *subSpace) {
 ...
 */
uintptr_t MM_ConcurrentGC::localMark(MM_EnvironmentStandard *env,
  ...
  while (NULL != (objectPtr = (omrobjectptr_t)env->_workStack.popNoWait(env))) {
    ...
    if ((uintptr_t)objectPtr & PACKET_ARRAY_SPLIT_TAG) {
      ...
      _extensions->collectorLanguageInterface->concurrentGC_processItem(
          env, objectPtr);
      ...
      */
      ...
      sizeTraced += _markingScheme->scanObjectWithSize(
          env, objectPtr, MM_CollectorLanguageInterface::SCAN_REASON_PACKET,
 ...
 */
MMINLINE bool MM_ConcurrentGC::cleanCards(MM_EnvironmentStandard *env,
 ...
 * concurrent
 * helper threads should have parked themselves. Before the final collection can
 * take
 * place we need to ensure that all objects in dirty cards are retraced and the
 * remembered
 * set is processed.
 *
...
void MM_ConcurrentGC::internalPreCollect(
  ...
  Trc_MM_ConcurrentGC_internalPreCollect_Entry(
      envStandard->getLanguageVMThread(), subSpace);
   ...
   * sure the
   * heap is walkable (the sweep could have connected part of an entry that
   * spans many chunks,
  ...
  /* Set flag to show global collector is active; some operations need to know
   * if they
   ...
   * find
   * we got far enough through the concurrent mark cycle then we will reset this
   * flag
   */
  ...
  if (_extensions->isRememberedSetInOverflowState() ||
     ...
     * up the cycle state */
    MM_ParallelGlobalGC::internalPreCollect(envStandard, subSpace,
                                            allocDescription, gcCode);
    ...
    if (CONCURRENT_FINAL_COLLECTION > executionModeAtGC) {
      ...
      if (!_markingScheme->getWorkPackets()->tracingExhausted()) {
         ...
         * In the event of work stack overflow we will just dirty cards which
         * will get processed during
         * final card cleaning.
         */
        MM_ConcurrentCompleteTracingTask completeTracingTask(
            envStandard, _dispatcher, this, envStandard->_cycleState);
        _dispatcher->run(envStandard, &completeTracingTask);

        reportConcurrentCompleteTracingEnd(envStandard,
    ...
    if (_extensions->scavengerEnabled) {
       ...
       * NEW areas of heap */
      MM_ConcurrentClearNewMarkBitsTask clearNewMarkBitsTask(envStandard,
                                                             _dispatcher, this);
      ...
      reportConcurrentRememberedSetScanEnd(envStandard,
                                           omrtime_hires_clock() - startTime);
    }
#endif /* OMR_GC_MODRON_SCAVENGER */
    ...
    do {
      ...
      MM_ConcurrentFinalCleanCardsTask cleanCardsTask(
          envStandard, _dispatcher, this, envStandard->_cycleState);
      ((MM_ConcurrentCardTable *)_cardTable)
          ->initializeFinalCardCleaning(envStandard);
    ...
    reportConcurrentFinalCardCleaningEnd(envStandard,
                                         omrtime_hires_clock() - startTime);

    assume(_cardTable->isCardTableEmpty(envStandard),
           "internalPreCollect: card cleaning has failed to clean all cards");
  ...
  Trc_MM_ConcurrentGC_internalPreCollect_Exit(
      envStandard->getLanguageVMThread(), subSpace);
}
...
bool MM_ConcurrentGC::internalGarbageCollect(
  ...
  masterThreadGarbageCollect(
      envStandard, static_cast<MM_AllocateDescription *>(allocDescription),
      _initializeMarkMap, false);
...
void MM_ConcurrentGC::internalPostCollect(MM_EnvironmentBase *env,
  ...
  Trc_MM_ConcurrentGC_internalPostCollect_Entry(
      envStandard->getLanguageVMThread(), subSpace);
  ...
  Trc_MM_ConcurrentGC_internalPostCollect_Exit(
      envStandard->getLanguageVMThread(), subSpace);
}
 ...
 * This
 * implies that all concurrent tracers will have stopped including any
 * concurrent
...
void MM_ConcurrentGC::abortCollection(MM_EnvironmentBase *env,
   ...
   * allocation
   * we will then restart a concurrent mark cycle. We probably won't finish
   * before
...
bool MM_ConcurrentGC::heapAddRange(MM_EnvironmentBase *env,
  ...
  Trc_MM_ConcurrentGC_heapAddRange_Entry(envStandard->getLanguageVMThread(),
                                         subspace, size, lowAddress,
                                         highAddress);
  ...
  bool result = MM_ParallelGlobalGC::heapAddRange(envStandard, subspace, size,
  ...
  if (result) {
    ...
    if (CONCURRENT_OFF < _stats->getExecutionMode()) {
       ...
       */
      if (subspace->isConcurrentCollectable()) {
        _markingScheme->setMarkBitsInRange(envStandard, lowAddress, highAddress,
                                           true);
        clearCards = true;
      } else {
        _markingScheme->setMarkBitsInRange(envStandard, lowAddress, highAddress,
    ...
    result = ((MM_ConcurrentCardTable *)_cardTable)
                 ->heapAddRange(envStandard, subspace, size, lowAddress,
                                highAddress, clearCards);
    if (!result) {
      /* Expansion of Concurrent Card Table has failed
       * ParallelGlobalGC expansion must be reversed
       */
      MM_ParallelGlobalGC::heapRemoveRange(envStandard, subspace, size,
                                           lowAddress, highAddress, NULL, NULL);
    }
  }
  ...
  if (!_globalCollectionInProgress) {
    ...
    if (_stats->getExecutionMode() < CONCURRENT_INIT_COMPLETE) {
      ...
      /* Heap expand is during a concurrent cycle..we need to adjust the trace
      * target so
...
bool MM_ConcurrentGC::heapRemoveRange(MM_EnvironmentBase *env,
  ...
  Trc_MM_ConcurrentGC_heapRemoveRange_Entry(
      envStandard->getLanguageVMThread(), subspace, size, lowAddress,
      highAddress, lowValidAddress, highValidAddress);
  ...
  bool result = MM_ParallelGlobalGC::heapRemoveRange(
      envStandard, subspace, size, lowAddress, highAddress, lowValidAddress,
      highValidAddress);
  ...
  result =
      result &&
      ((MM_ConcurrentCardTable *)_cardTable)
          ->heapRemoveRange(envStandard, subspace, size, lowAddress,
                            highAddress, lowValidAddress, highValidAddress);
  ...
  if (!_globalCollectionInProgress) {
    ...
    if (_stats->getExecutionMode() < CONCURRENT_INIT_COMPLETE) {
      ...
      /* Heap contract is during a concurrent cycle..we need to adjust the trace
       * target so
       * that the trace rate is adjusted correctly on  subsequent allocates.
       */
      adjustTraceTarget(envStandard);
...
void MM_ConcurrentGC::heapReconfigured(MM_EnvironmentBase *env) {
  ...
  /* The heap has been reconfigured; most likely a change in scavenger tilt
   * ratio
 ...
 */
void MM_ConcurrentGC::clearNewMarkBits(MM_EnvironmentStandard *env) {
  void *from, *to;
  InitType type;
  bool concurrentCollectable;
 ...
 *
 */
void MM_ConcurrentGC::completeTracing(MM_EnvironmentStandard *env) {
  omrobjectptr_t objectPtr;
  uintptr_t bytesTraced = 0;
  env->_workStack.reset(env, _markingScheme->getWorkPackets());

  while (NULL != (objectPtr = (omrobjectptr_t)env->_workStack.popNoWait(env))) {
    bytesTraced += _markingScheme->scanObjectWithSize(
        env, objectPtr, MM_CollectorLanguageInterface::SCAN_REASON_PACKET,
        SCAN_MAX);
  }
 ...
 */
void MM_ConcurrentGC::finalCleanCards(MM_EnvironmentStandard *env) {
  omrobjectptr_t objectPtr;
  bool moreCards = true;
  bool moreRefs = true;
  uintptr_t totalTraced = 0;
    ...
    while (objectPtr) {
      ...
      if (objectPtr) {
        ...
        if ((uintptr_t)objectPtr & PACKET_ARRAY_SPLIT_TAG) {
          ...
          if (!dirty) {
            totalTraced += _markingScheme->scanObjectWithSize(
                env, objectPtr,
                MM_CollectorLanguageInterface::SCAN_REASON_PACKET, SCAN_MAX);
          } else {
            _extensions->collectorLanguageInterface->concurrentGC_processItem(
                env, objectPtr);
          }
 ...
 * cards.
 * A marked object which is not in a dirty card needs rescanning now for any
 * references
 ...
 */
void MM_ConcurrentGC::scanRememberedSet(MM_EnvironmentStandard *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  ...
  while ((puddle = rememberedSetIterator.nextList()) != NULL) {
    if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
      ...
      while (
         ...
         * object
         * unless its card is dirty in which case we leave it for later
         * processing
         * by finalCleanCards()
         */
        if ((objectPtr >= _heapBase) && (objectPtr < _heapAlloc) &&
          ...
          if (_extensions->dirtCardDuringRSScan) {
             ...
             * may not have dirtied
             * cards for some objects in the remembered set. Therefore we may
             * discover references
             * to both nursery and tenure objects while scanning remembered
             * objects.
             */

            bytesTraced += _markingScheme->scanObjectWithSize(
                env, objectPtr,
                MM_CollectorLanguageInterface::SCAN_REASON_REMEMBERED_SET_SCAN,
                SCAN_MAX);
            ...
            if (env->_workStack.getPushCount() >= maxPushes) {
               ...
               * remaining
               * workpackets at this point. This will make RS processing appear
               * more
              ...
              while (NULL !=
                ...
                bytesTraced += _markingScheme->scanObjectWithSize(
                    env, objectPtr,
                    MM_CollectorLanguageInterface::SCAN_REASON_PACKET,
                    SCAN_MAX);
              }
              env->_workStack.clearPushCount();
            }
 ...
 * invoke the concurrent write barrier we need to rescan the object, as slots
 * which
 * previously contained new area references could now contain references into
 * old area.
 *
 * @param objectPtr  Reference to the object removed from the remembered set
 */
void MM_ConcurrentGC::objectRemovedFromRememberedSet(
...
void MM_ConcurrentGC::recordCardCleanPass2Start(MM_EnvironmentStandard *env) {
  _pass2Started = true;
...
void MM_ConcurrentGC::flushLocalBuffers(MM_EnvironmentStandard *env) {
 ...
 * @see MM_ParallelGlobalGC::collectorShutdown()
 */
void MM_ConcurrentGC::updateMeteringHistoryBeforeGC(
    MM_EnvironmentStandard *env) {
 ...
 * @see MM_ParallelGlobalGC::collectorShutdown()
 */
void MM_ConcurrentGC::updateMeteringHistoryAfterGC(
    MM_EnvironmentStandard *env) {
  /* If LOA disabled or a system GC then nothing to do */
  if (!_extensions->largeObjectArea ||
      env->_cycleState->_gcCode.isExplicitGC()) {
