...
protected:
  MM_EnvironmentLanguageInterface(MM_EnvironmentBase *env)
      : MM_BaseVirtual(), _omrThread(env->getOmrVMThread()),
        _omrVM(env->getOmrVM()), _portLibrary(env->getPortLibrary()),
        _env(env) {
...
public:
  ...
  /**
   * Acquire exclusive VM access. This must lock the VM thread list mutex
   * (OMR_VM::_vmThreadListMutex).
   *
   * This method may be called by a thread that already holds exclusive VM
   * access. In that case, the
   * OMR_VMThread::exclusiveCount counter is incremented (without reacquiring
   * lock on VM thread list
   * mutex).
   */
  ...
  /**
   * This method may be required if a concurrent GC strategy is employed. It
   * will
   * be called by the concurrent GC if it is unable to immediately obtain
   * exclusive
   * VM access (because a stop-the-world GC is in progress). If the concurrent
   * GC
   * holds resources that must be temporarily relinquished while waiting for
   * exclusive VM access, this is the place to release them. They can be
   * recovered
   * in exclusiveAccessForGCObtainedAfterBeatenByOtherThread(), which will be
   * called
   ...
   * This method may be required if a concurrent GC strategy is employed. It
   * will be
   * called when the stop-the-world GC completes and releases exclusive VM
   * access if
   * the thread previously ran
   * exclusiveAccessForGCObtainedAfterBeatenByOtherThread().
   * Any resources temporarily released in that method can be recovered here
   * before
   ...
   * collaborating thread
   * (i.e. main-to-master or master-to-main). This may involve nothing more than
   * transferring OMR_VMThread::exclusiveCount from the owning thread to the
   * another
   * thread that thereby assumes exclusive access. Implement if this kind of
   * collaboration
   * is required.
   *
