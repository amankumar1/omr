...
protected:
  ...
  volatile uintptr_t _largestFreeEntry; /**< largest free entry found at the end
                                           of a global GC cycle (no meaning
                                           during mutator execution) */

  uintptr_t _approximateFreeMemorySize; /**< The approximate number of bytes
                                           free that could be made part of the
                                           free list */
...
public:
   ...
   * objects, all indexable objects outside of arraylet
   * VMs, and arraylet spines (including their leaves, if inline) take this
   * path.
   */
  ...
  MM_MemoryPool(MM_EnvironmentBase *env, uintptr_t minimumFreeEntrySize)
      : MM_BaseVirtual(), _next(NULL), _previous(NULL), _children(NULL),
        _poolName("Unknown"), _parent(NULL), _memorySubSpace(NULL),
  ...
  MM_MemoryPool(MM_EnvironmentBase *env, uintptr_t minimumFreeEntrySize,
                const char *name)
      : MM_BaseVirtual(), _next(NULL), _previous(NULL), _children(NULL),
        _poolName(name), _parent(NULL), _memorySubSpace(NULL),
        _minimumFreeEntrySize(minimumFreeEntrySize), _freeMemorySize(0),
        _freeEntryCount(0), _largestFreeEntry(0), _approximateFreeMemorySize(0),
        _allocCount(0), _allocBytes(0), _lastFreeBytes(0),
        _allocDiscardedBytes(0), _allocSearchCount(0),
