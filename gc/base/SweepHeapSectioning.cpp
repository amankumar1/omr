    ...
    if (useVmem) {
      ...
      if (0 != _size) {
        _array = (MM_ParallelSweepChunk *)env->getForge()->allocate(
            _size * sizeof(MM_ParallelSweepChunk), MM_AllocationCategory::FIXED,
            OMR_GET_CALLSITE());
        result = (NULL != _array);
      } else {
        result = true;
...
MM_ParallelSweepChunkArray::newInstance(MM_EnvironmentBase *env, uintptr_t size,
  ...
  array = (MM_ParallelSweepChunkArray *)env->getForge()->allocate(
      sizeof(MM_ParallelSweepChunkArray), MM_AllocationCategory::OTHER,
      OMR_GET_CALLSITE());
  if (NULL != array) {
 ...
 * Walk all arrays in the receiver reserving the requested number of chunks.
 * Any arrays in excess will have their
 * used count set to 0.
...
bool MM_SweepHeapSectioning::initArrays(uintptr_t chunkCount) {
   ...
   * incorrect (may skip arrays if they
   * are currently empty
   */
