...
void *MM_MemorySubSpaceSemiSpace::allocationRequestFailed(
  ...
  if (!env->tryAcquireExclusiveVMAccessForGC(_collector)) {
    ...
    if (!env->acquireExclusiveVMAccessForGC(_collector)) {
      ...
      if (NULL != addr) {
         ...
         * perform a GC (without actually performing the GC).  Raise
   ...
   * because the subspaces will swap, if the collection is
   * successful, but not if we backout so we don't know which subspace will
   * allocate until after the collect - our own allocation
   * routines will call the correct child subspace.
   *
   * we might not be the only SemiSpace around (think NUMA). if so, we want to
   * do scavenge on whole new space.
   * OMRTODO we might be short-cutting here. more formal approach is to pass
   * allocationRequestFailed to parent and he decides what to do
   */
   ...
   *are we not calling the parent?
   *	Calling of parent for TLH is wrong because objects allocated into it on
   *the in-line path won't have the OLD bit set
   *  CMVC 144061
   */
...
void MM_MemorySubSpaceSemiSpace::payAllocationTax(
...
void *MM_MemorySubSpaceSemiSpace::allocateTLH(
   ...
   * the parent but we shouldn't satisfy TLH allocates
   * in this space (since that would allow us to fill old space before
   * collecting new space) so just return NULL.
...
uintptr_t MM_MemorySubSpaceSemiSpace::getActualActiveFreeMemorySize(
  ...
  if (includeMemoryType & MEMORY_TYPE_NEW) {
    return _memorySubSpaceAllocate->getActualActiveFreeMemorySize() +
           getActualActiveFreeSurvivorMemorySize(includeMemoryType);
  } else {
    return 0;
  }
}
...
uintptr_t MM_MemorySubSpaceSemiSpace::getApproximateActiveFreeMemorySize(
  ...
  if (includeMemoryType & MEMORY_TYPE_NEW) {
    return _memorySubSpaceAllocate->getApproximateActiveFreeMemorySize() +
           getApproximateActiveFreeSurvivorMemorySize(includeMemoryType);
  } else {
    return 0;
  }
}
...
MM_MemorySubSpaceSemiSpace::getApproximateActiveFreeSurvivorMemorySize(
    uintptr_t includeMemoryType) {
  /* If we ask directly MemorySubSpaceSurvivor, we'll get as answer: everything
   * is free. While it is true literally,
...
MM_MemorySubSpaceSemiSpace *MM_MemorySubSpaceSemiSpace::newInstance(
  ...
  memorySubSpace = (MM_MemorySubSpaceSemiSpace *)env->getForge()->allocate(
      sizeof(MM_MemorySubSpaceSemiSpace), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (memorySubSpace) {
...
bool MM_MemorySubSpaceSemiSpace::initialize(MM_EnvironmentBase *env) {
  ...
  _largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(
      ...
      _extensions->tlhMaximumSize + _extensions->minimumFreeEntrySize,
...
void MM_MemorySubSpaceSemiSpace::masterSetupForGC(MM_EnvironmentBase *envBase) {
...
void MM_MemorySubSpaceSemiSpace::rebuildFreeListForEvacuate(
    MM_EnvironmentBase *env) {
  _memorySubSpaceEvacuate->rebuildFreeList(env);
}

void MM_MemorySubSpaceSemiSpace::rebuildFreeListForBackout(
    MM_EnvironmentBase *env) {
...
void MM_MemorySubSpaceSemiSpace::checkSubSpaceMemoryPostCollectTilt(
  ...
  if (extensions->tiltedScavenge) {
    ...
    /* OMRTODO consider maintaining _flipBytes separately for each NUMA
     * SemiSpace.
     * Currently, _flipBytes is cumulative for all SemiSpaces. To compensate,
     * current size is obtained
     * for all of SemiSpaces too (from top level New SubSpace). This will result
     * in blended tilt ratio,
...
void MM_MemorySubSpaceSemiSpace::checkSubSpaceMemoryPostCollectResize(
  ...
  if (extensions->dynamicNewSpaceSizing) {
    ...
    if (doDynamicNewSpaceSizing) {
       ...
       * determine what weight to assign when
