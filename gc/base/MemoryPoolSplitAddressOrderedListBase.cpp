...
bool J9ModronHeapFreeList::initialize(MM_EnvironmentBase *env) {
  if (!_lock.initialize(env, &env->getExtensions()->lnrlOptions,
                        "J9ModronHeapFreeList:_lock")) {
    return false;
  }
  _freeList = NULL;
  _timesLocked = 0;
...
void J9ModronHeapFreeList::tearDown() { _lock.tearDown(); }

/****************************************
 * Hint Functionality
 ****************************************
 */
void J9ModronHeapFreeList::clearHints() {
...
bool MM_MemoryPoolSplitAddressOrderedListBase::initialize(
  ...
  _currentThreadFreeList = (uintptr_t *)extensions->getForge()->allocate(
      sizeof(uintptr_t) * _heapFreeListCount, MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  ...
  _heapFreeLists = (J9ModronHeapFreeList *)extensions->getForge()->allocate(
      sizeof(J9ModronHeapFreeList) * _heapFreeListCountExtended,
      MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
  if (NULL == _heapFreeLists) {
    return false;
  } else {
    memset(_heapFreeLists, 0,
           sizeof(J9ModronHeapFreeList) * _heapFreeListCountExtended);
  ...
  _largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(
      ...
      _extensions->tlhMaximumSize + _minimumFreeEntrySize,
   ...
   * newInstance instead of just initialize
   ...
   * _largeObjectCollectorAllocateStatsForFreeList yet.
   * Tenure SubSpace for Gencon will set
   * _largeObjectCollectorAllocateStatsForFreeList to
   * _largeObjectCollectorAllocateStatsForFreeList (we append collector stats to
   * mutator stats)
   * Tenure SubSpace for Flat will leave
   * _largeObjectCollectorAllocateStatsForFreeList at NULL (no interest in
   * collector stats)
   * We do not even need _largeObjectCollectorAllocateStats here (as we do in
   * plain MPAOL) - we'll just merge whatever
...
/* TODO 108399 Determine whether this is still necessary (OMR_SCAVENGER_DEBUG is
 * defined in Scavenger.cpp and is not accessible here) */
...
bool MM_MemoryPoolSplitAddressOrderedListBase::printFreeListValidity(
  ...
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
    ...
    while (NULL != currentFreeEntry) {
      listIsSane =
          listIsSane && ((NULL == currentFreeEntry->getNext()) ||
 ...
 * the next free starting address but
 * ONLY if the free list index has changed from the current free address to the
 * next free address.
 *
