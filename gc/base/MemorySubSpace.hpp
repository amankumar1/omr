...
public:
   ...
   * allocationRequestFailed.  This is used to determine
   * what call-out function to use to restart the allocation after we have dealt
   * with the failure
   */
  enum AllocationType {
   ...
   * allow the receiver to recycle the given region
   ...
   * "allocationType"
   * variable is consulted to determine which allocation routine is to be
   * called, after the collection, but before giving up exclusive to ensure that
   ...
   * attempt stages.  It decodes
   * the allocationType to determine which actual allocation call to issue
   * (Object, Leaf, or TLH).
   *
   * @param[in] env The current thread
   ...
   * a succesful allocation.  However, upon
   * acquiring exclusive access, the allocation was satisfied WITHOUT having to
   * perform a GC.  This routine is called
