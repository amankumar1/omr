 ...
 * methods but must call the base methods before proceeding with
 * language-specific
 ...
 * to the appropriate subclass to complete object initialization for the
 * category.
 */
...
protected:
  ...
  const uintptr_t _requestedSizeInBytes; /**< minimum number of bytes to
                                            allocate for object header+instance
                                            data */
  bool _isAllocatable; /**< this is set if the allocation should not proceed */

  MM_AllocateDescription _allocateDescription; /**< mutable allocation
                                                  descriptor holds actual
                                                  allocation terms */
...
public:
  ...
  /**
   * Vet the allocation description and set the _isAllocatable flag to false if
   * not viable.
   *
   * Subclasses that override this method must call the corresponding method on
   * their super
   * class until this base class method is called, and return false if the super
   * class or
   ...
   */
  MMINLINE bool initializeAllocateDescription(MM_EnvironmentBase *env) {
    /* _isAllocatable reset in initializer is carried over from the now-defunct
     * "NoGC"
     * code path and may be unused - more investigation required. If cached
     * allocations
     * are enabled, it is assumed that instrumentable allocations are off
     * (otherwise, a
     * JIT resolve frame is required to report the allocation event).
     */
    if (isAllocatable() && !isGCAllowed() &&
  ...
  allocateAndInitializeObject(OMR_VMThread *omrVMThread) {
    ...
    if (NULL != heapBytes) {
      ...
      fomrobject_t *headerPtr =
          OMR_OBJECT_METADATA_SLOT_EA((omrobjectptr_t)heapBytes);
      *headerPtr = (fomrobject_t)(_allocateDescription.getObjectFlags()
                                  << OMR_OBJECT_METADATA_FLAGS_SHIFT);

      /* let the language object model complete header and object initialization
       */
      objectPtr = env->getExtensions()->objectModel.initializeAllocation(
          env, heapBytes, this);
   ...
   * to be allocated
   * @param[in] objectAllocationFlags union of the following
   * OMR_GC_ALLOCATE_OBJECT_* bits
   *
   * OMR_GC_ALLOCATE_OBJECT_TENURED 0x2		(allocate from old space)
   * OMR_GC_ALLOCATE_OBJECT_NON_ZERO_TLH 0x10 (suppress clearing allocated
   * memory if possible)
   * OMR_GC_ALLOCATE_OBJECT_NO_GC 0x20		(fail if allocation would force
   * garbage collection)
   *
