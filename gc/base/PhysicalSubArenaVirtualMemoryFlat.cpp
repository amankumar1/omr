...
MM_PhysicalSubArenaVirtualMemoryFlat::expand(MM_EnvironmentBase *env,
   ...
   * if not, try and contract the neighbour.
   * No matter what the result, adjust the expand size to be as much as our
   * neighbour will allow.
   */
 ...
 * the expand size will in fact fit and can
...
MM_PhysicalSubArenaVirtualMemoryFlat::expandNoCheck(MM_EnvironmentBase *env,
  ...
  if (_highAddress != highExpandAddress) {
    ...
    _subSpace->expanded(env, this, expandSize, lowExpandAddress,
                        highExpandAddress, true);
    _subSpace->heapReconfigured(env);
  }
