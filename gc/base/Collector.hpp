...
class MM_Collector : public MM_BaseVirtual {
   ...
   */
private:
  uintptr_t _exclusiveAccessCount; /**< The number of times exclusive access
                                      requests have been made to use the
                                      receiver */

protected:
  MM_CollectorLanguageInterface
      *_cli; /**< Language specific interface used to extend the Modron GC
                framework */
  ...
  uint64_t
      _masterThreadCpuTimeStart; /**< slot to store the master CPU time at the
...
protected:
   ...
   * storage to
   * request that this collect should be more aggressive, ie clear any soft
   * references.
   ...
   * such as the aggressiveness of collection, excessive GC calculation,
...
public:
   ...
   * dependent on this to be updated.
   * This call could be triggered by memory ranges being added to or removed
   * from the heap or memory being
   ...
   * exclusive access count of the receiver.
   * This is most often used to determine if a GC completed while a thread was
   * blocked requesting exclusive.
  ...
  */
  ...
  MM_Collector(MM_CollectorLanguageInterface *cli)
      : MM_BaseVirtual(), _exclusiveAccessCount(0), _cli(cli),
        _bytesRequested(0), _globalCollector(false), _gcCompleted(false),
        _isRecursiveGC(false), _collectorExpandedSize(0),
