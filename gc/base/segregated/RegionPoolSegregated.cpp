...
MM_RegionPoolSegregated::newInstance(MM_EnvironmentBase *env,
  ...
  regionPool = (MM_RegionPoolSegregated *)env->getForge()->allocate(
      sizeof(MM_RegionPoolSegregated), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (NULL != regionPool) {
    regionPool =
    ...
    for (int32_t i = 0; i < NUM_DEFRAG_BUCKETS; i++) {
      ...
      _smallAvailableRegions[szClass][i] =
          (MM_LockingHeapRegionQueue *)env->getForge()->allocate(
              splitAvailableListsSize, MM_AllocationCategory::FIXED,
              OMR_GET_CALLSITE());
...
MM_RegionPoolSegregated::allocateFromRegionPool(MM_EnvironmentBase *env,
  ...
  if (region != NULL) {
     ...
     * allocated, it will know how to
     * account for bytes lost to internal fragmentation and will account for all
