...
MM_MemoryPoolSegregated *MM_MemoryPoolSegregated::newInstance(
  ...
  MM_MemoryPoolSegregated *memoryPool =
      (MM_MemoryPoolSegregated *)env->getForge()->allocate(
          sizeof(MM_MemoryPoolSegregated), MM_AllocationCategory::FIXED,
          OMR_GET_CALLSITE());
  if (memoryPool) {
      ...
      if (0 < bytesRemaining) {
        ...
        if (arraylet == NULL) {
          ...
          for (uintptr_t j = 0; j < i; j++) {
             ...
             * cycle, so we need to correct for
             * their un-allocation
             */
         ...
         * to NULL.
         * This allows us to calculate an address for the after-last element
         * without reading
