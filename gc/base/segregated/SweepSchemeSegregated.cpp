...
MM_SweepSchemeSegregated::newInstance(MM_EnvironmentBase *env,
  ...
  instance = (MM_SweepSchemeSegregated *)env->getForge()->allocate(
      sizeof(MM_SweepSchemeSegregated), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (NULL != instance) {
...
void MM_SweepSchemeSegregated::unmarkRegion(
   ...
   * is set for the ref array copy optimization.
   * To not miss the scan bit of an object that's in the last cell of a region,
   * we pretend the last object starts
   * at 16 bytes past its header, which is where we would set the scan bit on
   * the mark map.
   * Note we need to ensure that cell size is at least >16 bytes, otherwise 16
   * bytes past J9Object* would go off the end of page, hence the if check
   * below.
   * And since there is no scan bit for object that's 16 bytes, this should be
   * safe.
   */
...
void MM_SweepSchemeSegregated::incrementalSweepSmall(MM_EnvironmentBase *env) {
   ...
   * right away
   * so that application have somewhere to allocate from and minimize the usage
   * of
   * non deterministic sweeps.
   * Any marked objects are unmarked.
   * If a region holds a marked object, then the region is kept active;
   * if a region contains no marked objects, then it can be returned to a free
   * list.
   */
