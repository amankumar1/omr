...
MM_WorkPackets *MM_WorkPackets::newInstance(MM_EnvironmentBase *env) {
  ...
  workPackets = (MM_WorkPackets *)env->getForge()->allocate(
      sizeof(MM_WorkPackets), MM_AllocationCategory::WORK_PACKETS,
      OMR_GET_CALLSITE());
  if (workPackets) {
...
bool MM_WorkPackets::initialize(MM_EnvironmentBase *env) {
   ...
   * progress at once.
   * This means that we need to have at least 2 packets per thread (one input
   * and one output.)
...
bool MM_WorkPackets::initWorkPacketsBlock(MM_EnvironmentBase *env) {
  ...
  if (NULL == (_packetsStart[_packetsBlocksTop] =
                   (MM_Packet *)env->getForge()->allocate(
                       blockSize, MM_AllocationCategory::WORK_PACKETS,
                       OMR_GET_CALLSITE()))) {
    return false;
  }
...
MM_WorkPackets::getPacketByAdddingWorkPacketBlock(MM_EnvironmentBase *env) {
   ...
   * thread may have allocated
   * another WorkPacket block while this thread was attempting to take control
   * of the allocatingPackets field
   */
  packet = getPacket(env, &_emptyPacketList);
  if (NULL == packet) {
    /* Since the current thread now has control of the allocatingPackets field
     * and there are no packets
