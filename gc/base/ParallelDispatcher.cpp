...
uintptr_t dispatcher_thread_proc2(OMRPortLibrary *portLib, void *info) {
  ...
  omrVMThread = cli->attachVMThread(
      omrVM, "GC Slave",
      MM_CollectorLanguageInterfaceImpl::ATTACH_GC_DISPATCHER_THREAD);
  if (NULL == omrVMThread) {
    goto startup_failed;
  }
...
MM_ParallelDispatcher::newInstance(MM_EnvironmentBase *env,
  ...
  dispatcher = (MM_ParallelDispatcher *)env->getForge()->allocate(
      sizeof(MM_ParallelDispatcher), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (dispatcher) {
    new (dispatcher)
...
bool MM_ParallelDispatcher::initialize(MM_EnvironmentBase *env) {
  ...
  _threadTable = (omrthread_t *)forge->allocate(
      _threadCountMaximum * sizeof(omrthread_t), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (!_threadTable) {
    goto error_no_memory;
  }
  ...
  _statusTable = (uintptr_t *)forge->allocate(
      _threadCountMaximum * sizeof(uintptr_t *), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (!_statusTable) {
    goto error_no_memory;
  }
  ...
  _taskTable = (MM_Task **)forge->allocate(
      _threadCountMaximum * sizeof(MM_Task *), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (!_taskTable) {
    goto error_no_memory;
  }
 ...
 */
void MM_ParallelDispatcher::recomputeActiveThreadCount(
...
uintptr_t MM_ParallelDispatcher::adjustThreadCount(uintptr_t maxThreadCount) {
  ...
  if (!_extensions->gcThreadCountForced) {
     ...
     * many
     * can lead to unacceptable pause times due to insufficient parallelism.
     * Additionally,
