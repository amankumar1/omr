...
MM_GCExtensionsBase *MM_GCExtensionsBase::newInstance(MM_EnvironmentBase *env) {
  ...
  /* Avoid using MM_Forge to allocate memory for the extension, since MM_Forge
...
void MM_GCExtensionsBase::kill(MM_EnvironmentBase *env) {
  /* Avoid using MM_Forge to free memory for the extension, since MM_Forge was
   * not used to allocate the memory */
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  tearDown(env);
...
#error Default GC policy cannot be determined
  ...
  /* Set Xmx (heap default).  For most platforms the heap default is a fraction
   * of
   ...
   *
   * Linux, AIX, and z/OS:  half of OMR_MIN(physical memory, RLIMIT_AS) with a
   * min of
   * 16 MiB and a max of 512 MiB.
   * -note that RLIMIT_AS is as extracted from getrlimit and represents the
   * resouce
  ...
  if (OMRPORT_LIMIT_LIMITED ==
      omrsysinfo_get_limit(OMRPORT_RESOURCE_ADDRESS_SPACE, &memoryLimit)) {
    ...
    /* if there is no memory limit being imposed on us, we will use physical
     * memory as our max */
...
#define J9_PHYSICAL_MEMORY_DEFAULT (16 * 1024 * 1024)
  ...
  maxSizeDefaultMemorySpace =
      MM_Math::roundToFloor(heapAlignment, (uintptr_t)memoryToRequest);
