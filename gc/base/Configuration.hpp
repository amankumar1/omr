...
enum MM_AlignmentType { mm_heapAlignment = 1, mm_regionAlignment };
...
public:
   ...
   * been allocated.
   * The configuration may use this opportunity to configure values related to
   * the heap address.
   *
...
protected:
  ...
  virtual MM_ObjectAllocationInterface *
  createObjectAllocationInterface(MM_EnvironmentBase *env);
   ...
   */
  virtual bool verifyRegionSize(MM_EnvironmentBase *env,
                                uintptr_t regionSize) = 0;
  /**
   ...
   */
  virtual uintptr_t
  internalGetDefaultArrayletLeafSize(MM_EnvironmentBase *env) = 0;
  /**
   * Each configuration is responsible for providing the barrier type it is
   * using
   *
  ...
  /**
   * Each configuration is responsible for providing the allocation type it is
   * using
   *
   * @return allocationType - The allocation type being used for this
   * configuration
   */
   ...
   *
   * OMRTODO move this out of configuration as we should not have to "configure"
   * NUMA.  The only reason this is here is
   * because ConfigurationIncrementalGenerational.hpp will disable physical NUMA
   * if it would create too many ACs and ideal AC calculation
...
private:
  ...
  bool initializeRunTimeObjectAlignmentAndCRShift(MM_EnvironmentBase *env,
                                                  MM_Heap *heap);
  MMINLINE uintptr_t calculatePowerOfTwoShift(MM_EnvironmentBase *env,
                                              uintptr_t value) {
