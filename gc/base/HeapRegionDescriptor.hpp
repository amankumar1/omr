 ...
 * potentially many different properties
 * (e.g., object size class, node affinity, etc).  The heap region descriptor is
 * found in the heap region table
...
public:
  enum RegionType {
    ...
    ADDRESS_ORDERED_IDLE = 6,   /**< Represents regions that have an AOL
                                   MemoryPool but are not actively being used
                                   (that is, they can be made into other kinds of
                                   regions without penalty) */
    ADDRESS_ORDERED_MARKED = 7, /**< Represents regions that have an AOL
                                   MemoryPool and an accurate mark set in
                                   extensions->previousMarkMap */
    BUMP_ALLOCATED =
  ...
  uintptr_t
      _regionsInSpan; /**< most likely a temporary piece of data which, along
                         with _nextInSet, builds a freelist (public for
                         assertions) */
...
private:
  ...
  MM_HeapRegionDescriptor *_previousRegion; /**< previous region descriptor in
                                               the list (used by
                                               HeapRegionManager) */
  MM_HeapRegionDescriptor
      *_nextRegion; /**< next region descriptor in the list (used by
                       HeapRegionManager) */
  ...
  MM_HeapRegionDescriptor *_nextInSet; /**< most likely a temporary link which
                                          is used for passing around ad-hoc
...
public:
  ...
  void *getHighAddress() const {
     ...
     * backing since, when a spanning
     * region is being released, a root backing becomes temporarily out of sync
     * with a descriptor built on it
     */
    return result;
  }
   ...
   * on any other region type.
   * This can be extended to other region types which have a "marked" variant as
   * they are needed.
   */
