...
private:
  ...
  MM_AllocationContext
      *_commonAllocationContext; /**< Common Allocation Context shared by all
                                    threads */
...
public:
  ...
  MM_ObjectAllocationInterface *_objectAllocationInterface; /**< Per-thread
                                                               interface that
                                                               guides object
                                                               allocation
                                                               decisions */
  ...
  bool
      _isInNoGCAllocationCall; /**< NOTE:  this is a "best-efforts" flag, only,
                                  for use by an assertion in the collector.  If
                                  this is true, the owning thread is attempting
                                  to perform a NoGC allocation.  If it is false,
                                  nothing can be implied about the owning
                                  thread's state. */
  ...
  MM_FreeEntrySizeClassStats _freeEntrySizeClassStats; /**< GC thread local
  ...
  uintptr_t
      _oolTraceAllocationBytes; /**< Tracks the bytes allocated since the last
                                   ool object trace */
  ...
  MM_CardCleaningStats _cardCleaningStats; /**< Per thread stats to track the
                                              performance of the card cleaning
                                              */
  ...
  MM_ScavengerHotFieldStats
      _hotFieldStats; /**< hot field statistics for this GC thread */
#endif /* defined(OMR_GC_MODRON_SCAVENGER) || defined(OMR_GC_VLHGC) */
#if defined(OMR_GC_MODRON_SCAVENGER)
  bool _concurrentScavengerInProgress; /**< thread local flag/state, if
                                          concurrent scavenger is in progress */
...
public:
  ...
  /**
   * Try and acquire exclusive access if no other thread is already requesting
   * it.
   * Make an attempt at acquiring exclusive access if the current thread does
   * not already have it.  The
   * attempt will abort if another thread is already going for exclusive, which
   * means this
   * call can return without exclusive access being held.  As well, this call
   * will block for any other
   ...
   * attempt will abort if another thread is already going for exclusive, which
   * means this
   * call can return without exclusive access being held, it will also abort if
   * another another thread
   * may have beat us to it and prepared the threads or even collected. As well,
   * this call will block for
   ...
   * access request, and alert other threads that it has completed its
   ...
   * If the thread is beaten to acquiring exclusive access by another thread
   * then the caller will not acquire exclusive access.
   * @param collector gc intended to be used for collection.
   * @return boolean indicating whether the thread now holds exclusive access
   * (ie: it won the race).
   *
   * @note this call should be considered a safe-point as the thread may release
   * VM access to allow the winning thread to acquire exclusivity.
  ...
  MM_EnvironmentBase(OMR_VMThread *omrVMThread)
      : MM_BaseVirtual(), _slaveID(0), _environmentId(0),
        _omrVM(omrVMThread->_vm), _omrVMThread(omrVMThread),
...
#endif /* OMR_GC_SEGREGATED_HEAP */
        ,
        _envLanguageInterface(NULL), _objectAllocationInterface(NULL),
        _workStack(), _threadType(MUTATOR_THREAD), _cycleState(NULL),
        ...
        _activeValidator(NULL), _lastSyncPointReached(NULL)
#if defined(OMR_GC_SEGREGATED_HEAP)
        ,
  ...
  MM_EnvironmentBase(OMR_VM *omrVM)
      : MM_BaseVirtual(), _slaveID(0), _environmentId(0), _omrVM(omrVM),
        _omrVMThread(NULL), _portLibrary(omrVM->_runtime->_portLibrary),
        _workUnitIndex(0), _workUnitToHandle(0), _threadScanned(false),
        _allocationContext(NULL), _commonAllocationContext(NULL),
        _allocationFailureReported(false)
#if defined(OMR_GC_SEGREGATED_HEAP)
