...
MM_MemoryPoolAddressOrderedList *MM_MemoryPoolAddressOrderedList::newInstance(
  ...
  memoryPool = (MM_MemoryPoolAddressOrderedList *)env->getForge()->allocate(
      sizeof(MM_MemoryPoolAddressOrderedList), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (memoryPool) {
...
bool MM_MemoryPoolAddressOrderedList::initialize(MM_EnvironmentBase *env) {
   ...
   * _largeObjectCollectorAllocateStats yet.
   * Tenure SubSpace for Gencon will set _largeObjectCollectorAllocateStats to
   * _largeObjectAllocateStats (we append collector stats to mutator stats)
   * SemiSpace will leave _largeObjectCollectorAllocateStats at NULL (no
   * interest in Collector stats)
 ...
 * free entry.  If found, reset
 * the pointer to be the free entry.  This is used when free entries are added
 * to the middle of a free list;
...
retry:
  ...
  if (recycleHeapChunk(recycleEntry,
                       ...
                       previousFreeEntry, currentFreeEntry->getNext())) {
 ...
 * defined in Scqavenger.cpp and is not accessible here) */
...
void MM_MemoryPoolAddressOrderedList::expandWithRange(MM_EnvironmentBase *env,
  ...
  if (canCoalesce) {
    ...
    if (nextFreeEntry && (highAddress == (void *)nextFreeEntry)) {
      ...
      assume0((NULL == nextFreeEntry->getNext()) ||
              (newFreeEntry < nextFreeEntry->getNext()));
...
void MM_MemoryPoolAddressOrderedList::addFreeEntries(
  ...
  if (previousFreeEntry == NULL) {
    ...
    if ((uint8_t *)freeListTail->afterEnd() == (uint8_t *)_heapFreeList) {
      ...
      assume0((NULL == _heapFreeList->getNext()) ||
              (freeListTail < _heapFreeList->getNext()));
    ...
    assume0((NULL == previousFreeEntry->getNext()) ||
            (freeListTail < previousFreeEntry->getNext()));
    ...
    if ((uint8_t *)previousFreeEntry->afterEnd() == (uint8_t *)freeListHead) {
      ...
      assume0((NULL == freeListHead->getNext()) ||
              (previousFreeEntry < freeListHead->getNext()));
