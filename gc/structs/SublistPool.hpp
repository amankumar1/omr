 ...
 * uintptr_t-sized elements.
 * An MM_SublistPool is a pool of memory which consists of a linked list of zero
 * or
 * more <i>puddles</i> (instances of MM_SublistPuddle). A thread can reserve a
 * block
 * of memory from the list (an instance of MM_SublistFragment), and then operate
 * without
...
public:
   ...
   * #startProcessingSublist() was called.
   * Return returnedPuddle to the list of puddles. It should be a puddle
  ...
  MM_SublistPool()
      : _list(NULL), _allocPuddle(NULL), _mutex(NULL), _growSize(0),
        ...
        _allocCategory(MM_AllocationCategory::OTHER), _previousList(NULL) {}
