 ...
 * characters in the
 * filename will be replaced with %seq (unless the # is already preceded by an
 * odd number
...
bool MM_VerboseWriterFileLogging::initializeFilename(MM_EnvironmentBase *env,
  ...
  if (_mode == rotating_files) {
    ...
    _filename = (char *)extensions->getForge()->allocate(
        nameLen, MM_AllocationCategory::DIAGNOSTIC, OMR_GET_CALLSITE());
    if (NULL == _filename) {
      return false;
    }
    ...
    *write = '\0';
    ...
    _filename = (char *)extensions->getForge()->allocate(
        strlen(filename) + 1, MM_AllocationCategory::DIAGNOSTIC,
        OMR_GET_CALLSITE());
    if (NULL == _filename) {
      return false;
    }
...
char *MM_VerboseWriterFileLogging::expandFilename(MM_EnvironmentBase *env,
  ...
  char *filenameToOpen = (char *)extensions->getForge()->allocate(
      len, MM_AllocationCategory::DIAGNOSTIC, OMR_GET_CALLSITE());
