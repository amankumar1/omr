...
CFileWriter::writeRegistrationFunctionsOnStream(FILE *fd, J9TDFFile *tdf,
  ...
  if (NULL != tdf->header.submodules) {
    ...
    while (NULL != registerSubmodule) {
      fprintf(fd, "\n\tif (rc == %s) {\n", ok);
      fprintf(fd, "\t\textern int32_t register%sWithTrace(%s%s, UtModuleInfo* "
                  "containerModule);\n\n",
...
RCType CFileWriter::writeComponentDataForNonTraceEnabledBuildsOnStream(
  ...
  fprintf(fd, "UtModuleInfo %s_UtModuleInfo = {(char*) \"%s\", %u, 0, 0, NULL, "
...
RCType CFileWriter::writeModuleInfo(FILE *fd, J9TDFFile *tdf,
  ...
  RCType rc = RC_FAILED;
  fprintf(fd, "UtModuleInfo %s_UtModuleInfo = {(char*) \"%s\", %u, %u, 0, "
              "%s_UtActive , NULL, NULL, &%s_UtTraceVersionInfo, (char*) "
              "\"%s\", %s_UtLevels, &%s_group0, NULL, NULL, 0, %s};",
          tdf->header.executable, tdf->header.executable,
