...
static RCType help(int argc, char *argv[]) {
  for (int i = 0; i < argc; i++) {
    if (0 == strcmp(argv[i], "-help") || 0 == strcmp(argv[i], "--help") ||
      ...
      printf("%s [-threshold num] [-w2cd] [-generateCfiles] "
             "[-treatWarningAsError] [-root rootDir] [-force]\n",
             argv[0]);
      printf(
...
RCType TraceGen::generate(J9TDFOptions *options, const char *currentTDFFile) {
   ...
   * files.
   * To handle relative paths omrfile_realpath is used to resolve canonicalized
   * absolute pathname.
   */
...
J9TDFGroup *TraceGen::calculateGroups(J9TDFFile *tdf,
                                      unsigned int *groupCount) {
