...
char *Port::omrfile_realpath(const char *utf8Path) {
   ...
   * MSDN docs for GetFullPathNameW state
   * that it should not be used in multi-threaded applications, but the docs are
   * fairly clear that this is because the conversion from a
   * relative path to a fully qualified path depends on the current working
   * directory (CWD), which is a process global that may change.
   *
   * However, we can't simply limit the use of GetFullPathNameW to non-absolute
   * paths, because all occurrences of "." and "..",
   * need to be removed. This includes the occurrences that don't simply prepend
   * the path, in which case the CWD is not relevant and
   * where we assume the call to GetFullPathNameW is thread-safe.
   *
   */
