...
DDR_RC getOptions(OMRPortLibrary *portLibrary, int argc, char *argv[],
                  const char **macroFile, const char **supersetFile,
                  const char **blobFile, const char **overrideFile,
                  vector<string> *debugFiles);
DDR_RC readFileList(OMRPortLibrary *portLibrary, const char *debugFileList,
                    vector<string> *debugFiles);
...
int main(int argc, char *argv[]) {
  ...
  rc = getOptions(&portLibrary, argc, argv, &macroFile, &supersetFile,
                  &blobFile, &overrideFile, &debugFiles);
...
DDR_RC
getOptions(OMRPortLibrary *portLibrary, int argc, char *argv[],
           const char **macroFile, const char **supersetFile,
           const char **blobFile, const char **overrideFile,
           vector<string> *debugFiles) {
  DDR_RC rc = DDR_RC_OK;
  ...
  if (showHelp) {
    printf("USAGE\n"
           ...
           "		Specify input list of macros. See macro tool for "
           "format. Default is macroList in current directory.\n"
           "	-s FILE, --superset FILE\n"
           "		Output superset file. Default is superset.out in "
           "current directory.\n"
           "	-b FILE, --blob FILE\n"
           "		Output binary blob file. Default is blob.dat in current "
           "directory.\n"
           "	-o FILE, --overrides FILE\n"
           "		Optional. File containing a list of files which contain "
           "type and field overrides for specific fields.\n"
           "		Overrides are of the format "
           "'typeoverride.STRUCT_NAME.FIELD_NAME=TypeToChangeTo' or\n"
...
DDR_RC
readFileList(OMRPortLibrary *portLibrary, const char *debugFileList,
             vector<string> *debugFiles) {
