...
private:
  ...
  DDR_RC addBlobStruct(string name, string superName, uint32_t constCount,
                       uint32_t fieldCount, uint32_t size);
...
public:
  ...
  DDR_RC dispatchBuildBlob(NamespaceUDT *type, bool addFieldsOnly,
                           string prefix);
