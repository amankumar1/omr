...
JavaBlobGenerator::genBinaryBlob(OMRPortLibrary *portLibrary, Symbol_IR *ir,
  ...
  if (DDR_RC_OK == rc) {
    /* open blob file */
    fd = omrfile_open(blobFile, EsOpenCreate | EsOpenWrite | EsOpenAppend |
                                    EsOpenTruncate,
                      0644);
    _buildInfo.fd = fd;
    if (-1 == fd) {
  ...
  if (DDR_RC_OK == rc) {
    ...
    for (unsigned int s_ix = 0; s_ix < _buildInfo.header.structureCount;
         s_ix++) {
    ...
    if ((unsigned long)amountWritten != _buildInfo.header.structDataSize) {
      ERRMSG("Written: %d fields, %d constants, %d structs.\nCounted: %d "
             "fields, %d constants, %d structures.\n",
             f_ix, c_ix,
             (int)(_buildInfo.curBlobStruct - _buildInfo.blobStructs),
...
pair<string, unsigned long long> JavaBlobGenerator::cLimits[] = {
    make_pair("CHAR_MAX", CHAR_MAX),   make_pair("CHAR_MIN", CHAR_MIN),
    make_pair("INT_MAX", INT_MAX),     make_pair("INT_MIN", INT_MIN),
    make_pair("LONG_MAX", LONG_MAX),   make_pair("LONG_MIN", LONG_MIN),
    make_pair("SCHAR_MAX", SCHAR_MAX), make_pair("SCHAR_MIN", SCHAR_MIN),
    make_pair("SHRT_MAX", SHRT_MAX),   make_pair("SHRT_MIN", SHRT_MIN),
    make_pair("UCHAR_MAX", UCHAR_MAX), make_pair("UINT_MAX", UINT_MAX),
...
JavaBlobGenerator::enumerateCLimits() {
  ...
  size_t structDataSize = sizeof(BlobField) * fieldCount +
                          sizeof(BlobConstant) * constCount +
                          sizeof(BlobStruct);
...
JavaBlobGenerator::addCLimits() {
  ...
  if (DDR_RC_OK == rc) {
    rc = addBlobStruct("CLimits", "", (uint32_t)constCount,
...
JavaBlobGenerator::countStructsAndStrings(Symbol_IR *const ir) {
  ...
  for (vector<Type *>::iterator v = (ir->_types).begin();
       v != (ir->_types).end(); ++v) {
...
DDR_RC
JavaBlobGenerator::buildBlobData(OMRPortLibrary *portLibrary,
                                 Symbol_IR *const ir) {
  DDR_RC rc = DDR_RC_OK;
  /* allocate hashtable */
  ...
  _buildInfo.blobStructs = (BlobStruct *)malloc(
      _buildInfo.header.structureCount * sizeof(BlobStruct));
  _buildInfo.curBlobStruct = _buildInfo.blobStructs;
  _buildInfo.blobFields =
      (BlobField *)malloc(_buildInfo.fieldCount * sizeof(BlobField));
  _buildInfo.curBlobField = _buildInfo.blobFields;
  _buildInfo.blobConsts =
      (BlobConstant *)malloc(_buildInfo.constCount * sizeof(BlobConstant));
...
DDR_RC
JavaBlobGenerator::dispatchBuildBlob(EnumUDT *e, bool addFieldsOnly,
                                     string prefix) {
  DDR_RC rc = DDR_RC_OK;
  /* Do not add anonymous inner types as their own type. */
  if ((!e->isAnonymousType() || addFieldsOnly) &&
      (e->_enumMembers.size() > 0)) {
    /* Format class name */
...
DDR_RC
JavaBlobGenerator::dispatchBuildBlob(UnionUDT *u, bool addFieldsOnly,
                                     string prefix) {
  DDR_RC rc = DDR_RC_OK;
  /* Do not add anonymous inner types as their own type. */
  if ((!u->isAnonymousType() || addFieldsOnly) &&
      (u->_fieldMembers.size() > 0)) {
    /* Format class name */
    ...
    if (DDR_RC_OK == rc) {
      if (addFieldsOnly) {
        ...
        rc = addBlobStruct(nameFormatted, "", constCount, fieldCount,
                           (uint32_t)u->_sizeOf);
      }
    }
  }
  if ((DDR_RC_OK == rc) && !addFieldsOnly) {
    for (vector<UDT *>::iterator v = u->_subUDTs.begin();
         v != u->_subUDTs.end(); ++v) {
      rc = (*v)->buildBlob(this, false, "");
      if (DDR_RC_OK != rc) {
        break;
...
DDR_RC
JavaBlobGenerator::dispatchBuildBlob(ClassUDT *cu, bool addFieldsOnly,
                                     string prefix) {
  DDR_RC rc = DDR_RC_OK;
  /* Do not add anonymous inner types as their own type. */
  if ((!cu->isAnonymousType() || addFieldsOnly) &&
      (cu->_fieldMembers.size() > 0)) {
    /* Format class name */
    ...
    if (DDR_RC_OK == rc) {
      if (addFieldsOnly) {
        ...
        rc = addBlobStruct(nameFormatted, superName, constCount, fieldCount,
                           (uint32_t)cu->_sizeOf);
      }
    }
  }
  if ((DDR_RC_OK == rc) && !addFieldsOnly) {
    for (vector<UDT *>::iterator v = cu->_subUDTs.begin();
         v != cu->_subUDTs.end(); ++v) {
      rc = (*v)->buildBlob(this, false, "");
      if (DDR_RC_OK != rc) {
        break;
...
DDR_RC
JavaBlobGenerator::dispatchBuildBlob(NamespaceUDT *ns, bool addFieldsOnly,
                                     string prefix) {
  DDR_RC rc = DDR_RC_OK;

  uint32_t constCount = 0;
  for (vector<Macro>::iterator m = (ns->_macros).begin();
       m != (ns->_macros).end(); ++m) {
    long long value = 0;
    if (DDR_RC_OK == m->getNumeric(&value)) {
      rc = addBlobConst(m->_name, value, &constCount);
  ...
  if ((DDR_RC_OK == rc) && !addFieldsOnly) {
    for (vector<UDT *>::iterator v = ns->_subUDTs.begin();
         v != ns->_subUDTs.end(); ++v) {
      rc = (*v)->buildBlob(this, false, "");
      if (DDR_RC_OK != rc) {
        break;
...
DDR_RC
JavaBlobGenerator::addBlobField(Field *f, uint32_t *fieldCount,
                                uint32_t *constCount) {
...
DDR_RC
JavaBlobGenerator::addBlobField(Field *f, uint32_t *fieldCount,
                                uint32_t *constCount, string prefix) {
  DDR_RC rc = DDR_RC_OK;
  ...
  if (NULL == type) {
    ...
    /* Anonymous classes, structs, and unions add their fields directly to the
     * outer type. */
...
DDR_RC
JavaBlobGenerator::addBlobConst(string name, long long value,
                                uint32_t *constCount) {
  DDR_RC rc = DDR_RC_OK;
  uint32_t offset = UINT32_MAX;
...
DDR_RC
JavaBlobGenerator::addBlobStruct(string name, string superName,
  ...
  *fieldType = "";
  ...
  if (DDR_RC_OK == rc) {
    if (BASE == st) {
      ...
      if (NULL != td) {
        /* If typedef is void*, or otherwise known as a function pointer, return
         * name as void*. */
        if ((NULL != td->_type) && ("void" == td->_type->_name) &&
            (1 == td->_modifiers._pointerCount)) {
          *fieldType += "void*";
        } else {
...
DDR_RC
JavaBlobGenerator::dispatchEnumerateType(NamespaceUDT *type,
                                         bool addFieldsOnly) {
  DDR_RC rc = DDR_RC_OK;
  size_t constCount = 0;
   ...
   * subUDTs twice. */
  if (!addFieldsOnly) {
    for (vector<UDT *>::iterator v = (type->_subUDTs).begin();
         v != (type->_subUDTs).end(); ++v) {
      rc = (*v)->enumerateType(this, false);
      if (DDR_RC_OK != rc) {
        break;
      }
...
JavaBlobGenerator::dispatchEnumerateType(ClassUDT *type, bool addFieldsOnly) {
  ...
  if ((DDR_RC_OK == rc) && (!type->isAnonymousType() || addFieldsOnly) &&
      (type->_fieldMembers.size() > 0)) {
    for (vector<Field *>::iterator v = type->_fieldMembers.begin();
         v != type->_fieldMembers.end(); ++v) {
      if (!(*v)->_isStatic) {
        /* Anonymous type members are added to the struct and not counted as a
         * field themselves. */
...
DDR_RC
JavaBlobGenerator::dispatchBuildBlob(Type *type, bool addFieldsOnly,
                                     string prefix) {
  /* No op: base types are not added to the blob at this time. */
  return DDR_RC_OK;
}
...
DDR_RC
JavaBlobGenerator::dispatchBuildBlob(TypedefUDT *type, bool addFieldsOnly,
                                     string prefix) {
  /* No op: base types are not added to the blob at this time. */
  return DDR_RC_OK;
}
