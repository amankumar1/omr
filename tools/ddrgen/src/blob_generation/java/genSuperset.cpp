...
string JavaSupersetGenerator::getUDTname(Type *type) {
...
void JavaSupersetGenerator::initBaseTypedefSet() {
  ...
  for (unordered_map<string, string>::iterator it = _baseTypedefReplace.begin();
       it != _baseTypedefReplace.end(); ++it) {
    _baseTypedefIgnore.insert(it->first);
    _baseTypedefIgnore.insert(it->second);
  }
  for (unordered_map<string, string>::iterator it = _baseTypedefMap.begin();
       it != _baseTypedefMap.end(); ++it) {
    _baseTypedefIgnore.insert(it->first);
    _baseTypedefIgnore.insert(it->second);
  }
}
...
JavaSupersetGenerator::getTypeName(Field *f, string *typeName) {
  ...
  if (NULL != f->_fieldType) {
    ...
    if (DDR_RC_OK == rc) {
      ...
      while ((NULL != td) &&
             (_baseTypedefIgnore.find(name) == _baseTypedefIgnore.end())) {
        type = td->_type;
        name = td->_type->_name;
 ...
 */
DDR_RC
JavaSupersetGenerator::getFieldType(Field *f, string *assembledTypeName,
  ...
  if (DDR_RC_OK == rc) {
    ...
    /* For typedef types, follow the typedef recursively and add pointer/array
     * notation. */
...
JavaSupersetGenerator::printFieldMember(Field *field, string prefix = "") {
  ...
  if (NULL == field->_fieldType) {
    ...
    if (field->_fieldType->isAnonymousType()) {
      field->_fieldType->printToSuperset(this, true,
                                         prefix + field->_name + ".");
    } else {
      string nameFormatted =
          prefix + (field->_name == "class" ? "klass" : field->_name);
      string lineToPrint =
          "F|" + replace(nameFormatted, ".", "$") + "|" + nameFormatted + "|";
...
void JavaSupersetGenerator::printConstantMember(string name) {
  OMRPORT_ACCESS_FROM_OMRPORT(_portLibrary);
  string lineToPrint = "C|" + name + "\n";
  omrfile_write(_file, lineToPrint.c_str(), lineToPrint.length());
}

string JavaSupersetGenerator::replace(string str, string subStr,
                                      string newStr) {
  size_t i = 0;
  while (true) {
    i = str.find(subStr, i);
...
DDR_RC
JavaSupersetGenerator::dispatchPrintToSuperset(Type *type, bool addFieldsOnly,
                                               string prefix) {
...
DDR_RC
JavaSupersetGenerator::dispatchPrintToSuperset(TypedefUDT *type,
                                               bool addFieldsOnly,
                                               string prefix) {
...
DDR_RC
JavaSupersetGenerator::dispatchPrintToSuperset(ClassUDT *type,
                                               bool addFieldsOnly,
                                               string prefix) {
  OMRPORT_ACCESS_FROM_OMRPORT(_portLibrary);
  DDR_RC rc = DDR_RC_OK;
  ...
  if ((!type->isAnonymousType() || addFieldsOnly) &&
    ...
    if (!addFieldsOnly) {
      ...
      string lineToPrint =
          "S|" + nameFormatted + "|" + nameFormatted + "Pointer|";
    ...
    for (vector<EnumMember *>::iterator m = type->_enumMembers.begin();
         m != type->_enumMembers.end(); ++m) {
      printConstantMember((*m)->_name);
    }

    for (vector<Field *>::iterator m = type->_fieldMembers.begin();
         m != type->_fieldMembers.end(); ++m) {
    ...
    if (DDR_RC_OK == rc) {
      for (vector<Macro>::iterator m = type->_macros.begin();
           m != type->_macros.end(); ++m) {
...
DDR_RC
JavaSupersetGenerator::dispatchPrintToSuperset(UnionUDT *type,
                                               bool addFieldsOnly,
                                               string prefix) {
  OMRPORT_ACCESS_FROM_OMRPORT(_portLibrary);
  DDR_RC rc = DDR_RC_OK;
...
DDR_RC
JavaSupersetGenerator::dispatchPrintToSuperset(EnumUDT *type,
                                               bool addFieldsOnly,
                                               string prefix) {
  OMRPORT_ACCESS_FROM_OMRPORT(_portLibrary);
  DDR_RC rc = DDR_RC_OK;
...
DDR_RC
JavaSupersetGenerator::dispatchPrintToSuperset(NamespaceUDT *type,
                                               bool addFieldsOnly,
                                               string prefix) {
  OMRPORT_ACCESS_FROM_OMRPORT(_portLibrary);
  DDR_RC rc = DDR_RC_OK;
...
JavaSupersetGenerator::printSuperset(OMRPortLibrary *portLibrary, Symbol_IR *ir,
  ...
  _file = omrfile_open(supersetFile, EsOpenCreate | EsOpenWrite | EsOpenAppend |
                                         EsOpenTruncate,
                       0644);
