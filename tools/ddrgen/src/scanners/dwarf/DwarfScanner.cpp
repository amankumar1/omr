  ...
  for (Dwarf_Signed i = 0; i < _fileNameCount; i += 1) {
    ...
    if (0 == strncmp(_fileNamesTable[i], "../", 3)) {
      ...
      fileNamesTableConcat[i] =
          (char *)malloc(sizeof(char) * (strlen(compDir) + strlen(path) + 2));
...
DwarfScanner::blackListedDie(Dwarf_Die die, bool *dieBlackListed) {
  ...
  *dieBlackListed = true;
  ...
  if (hasDeclFile) {
    ...
    /* If the decl_file matches an entry in the file table not beginning with
     * "/usr/",
    ...
    if ((NULL == strstr(_fileNamesTable[declFile - 1], "<built-in>")) &&
        ...
        (0 != strncmp(_fileNamesTable[declFile - 1], "/Applications/Xcode.app/",
                      24))) {
...
DwarfScanner::getName(Dwarf_Die die, string *name) {
  ...
  if (DW_DLV_ERROR == dwarfRC) {
    ...
    if (hasAttr) {
      /* If it has a specification attribute, check the specification for a name
       * recursively. */
      Dwarf_Attribute attr = NULL;
      ...
      /* If the Die has no specification attribute either, check for
       * linkage_name. */
      if (DW_DLV_ERROR ==
          dwarf_hasattr(die, DW_AT_linkage_name, &hasAttr, &err)) {
      ...
      if (hasAttr) {
        /* Die's with a linkage_name and no name can get the name from the
         * sibling die. */
  ...
  if (DDR_RC_OK == getBitField(typeDie, bitField)) {
    ...
    do {
      ...
      if (DW_TAG_const_type == tag) {
         ...
         * name.
         * Pointers of other types will override this name with the base type
         * name.
         */
        rc = DDR_RC_OK;
        if (NULL != typeName) {
 ...
 */
DDR_RC
DwarfScanner::addType(Dwarf_Die die, Dwarf_Half tag, bool ignoreFilter,
                      bool isSubUDT, Type **type) {
  DDR_RC rc = DDR_RC_ERROR;
  Type *newType = NULL;
  int typeNum = -1;
  ...
  if (DDR_RC_OK == rc) {
    if (2 == typeNum) {
      ...
      if (isSubUDT) {
        ...
        if ((NULL != udt) && (NULL == udt->_outerUDT)) {
          ...
          if ((newType->_name.length() > 9) &&
              ("Constants" ==
               newType->_name.substr(newType->_name.length() - 9, 9))) {
            newType->_name = "";
      ...
      if ((DW_TAG_class_type == tag) || (DW_TAG_structure_type == tag) ||
          (DW_TAG_union_type == tag) || (DW_TAG_namespace == tag) ||
          (DW_TAG_enumeration_type == tag) || (DW_TAG_typedef == tag)) {
        /* If the type was added as a stub with an outer type, such as is the
         * case
         * for types defined within namespaces, do not add it to the main list
         * of types.
         */
        isSubUDT =
 ...
 * time
 * called for each type, this function will create a Type object and add it to
 * the map.
 * On subsequent calls for a given type, the existing Type object in the map
 * will be
 * returned.
 *
 * The return code indicates if the returned type already had a full
 * declaration, the
 * type had been found before but only as a stub without children, the type is a
 * stub
 * and had not been found before, or the type is a full declaration and had not
 * been
 * found before. The caller is responsible for adding the Type's
 * properties/children
 * and adding it to the IR based on this return code.
 *
 * To keep track of duplicate Types, this function maintains two hash maps: one
 * for
 * stub types and one for full declarations. Stub types only have a name, while
 * full
 * declarations have a file name, line number, and possibly children/properties.
 * A
 * type can be found first as a stub and later as a full declaration, as a full
 * declaration and later as a stub, as only a stub, or only as a full
 * declaration.
 */
DDR_RC
DwarfScanner::getType(Dwarf_Die die, Dwarf_Half tag, Type **const newType,
                      bool isSubUDT, int *typeNum) {
  /* Set typeNum to -1 for error.
   * Set typeNum to 0 for full types which have not been found before--add to
   * types list and add children.
   * Set typeNum to 1 for full types which have been found before as a stub--do
   * not add to types list but add children.
   * Set typeNum to 2 for full types which already exist--do not add to types
   * list or add children.
   * Set typeNum to 3 for stub types which have not been found before--add to
   * types list but do not add children.
   */
  DDR_RC rc = DDR_RC_OK;
  *typeNum = -1;
  ...
  if (DDR_RC_OK != rc) {
    ...
    string fileName = "";
    TypeKey key;
    /* createTypeKey() succeeds if the Die has a declaration file and line
     * number.
     * Use these as a map key to check for duplicates.
     */
    if (DDR_RC_OK ==
        createTypeKey(die, dieName, &key, &lineNumber, &fileName)) {
      if (dieName.empty() && !isSubUDT && (DW_TAG_enumeration_type == tag)) {
        /* Anonymous enums with no outer type are added to a constants structure
         * named by file name. */
        size_t lastSlash = fileName.find_last_of("/");
        size_t lastDot = fileName.find_last_of(".");
        dieName = fileName.substr(lastSlash + 1, lastDot - lastSlash - 1) +
                  "Constants";
        dieName[0] = toupper(dieName[0]);
      }
      /* If the Type is not in the map yet, add it. */
      if (_typeMap.find(key) == _typeMap.end()) {
        /* If the Type is not in the stub map either, or as a stub which has
         * already
         * been promoted to the map for types with declarations, create a new
         * Type for this Die. */
        if ((_typeStubMap.find(stubKey) == _typeStubMap.end()) ||
            ((DW_TAG_base_type != tag) &&
             (0 != ((UDT *)_typeStubMap[stubKey])->_lineNumber))) {
          if (DDR_RC_OK == createType(die, tag, dieName, lineNumber, newType)) {
            _typeMap[key] = *newType;

            /* Anonymous types cannot also be a stub elsewhere and as such do
             * not need
          ...
          *newType = _typeStubMap[stubKey];
          ...
          /* Add the line number for the stub type as well, now that it is
           * found. */
          if (DDR_RC_OK == rc) {
            UDT *udt = dynamic_cast<UDT *>(*newType);
            if (NULL != udt) {
      ...
      /* Die's without a declaration file and line number are remembered by die
      ...
      if (_typeStubMap.find(stubKey) == _typeStubMap.end()) {
        ...
        /* The Type already exists as a stub and the declaration has not yet
         * been found. */
        *newType = _typeStubMap[stubKey];
        *typeNum = 2;
      }
 ...
 */
DDR_RC
DwarfScanner::createType(Dwarf_Die die, Dwarf_Half tag, string dieName,
                         unsigned int lineNumber, Type **const newType) {
  DDR_RC rc = DDR_RC_OK;
  size_t typeSize = 0;
...
DwarfScanner::dispatchScanChildInfo(TypedefUDT *newTypedef, void *data) {
  ...
  DDR_RC rc = getTypeInfo(die, &typeDie, &typedefName, &newTypedef->_modifiers,
                          &newTypedef->_sizeOf, NULL);
  ...
  if (DW_DLV_ERROR == dwRc) {
    ...
    do {
      ...
      if (DW_TAG_subprogram == childTag) {
        ...
        UDT *innerUDT = NULL;
        if (DDR_RC_OK != addType(childDie, childTag, true, (NULL != newClass),
                                 (Type **)&innerUDT)) {
          rc = DDR_RC_ERROR;
          break;
    ...
    /* If the class is not a stub and contained an anonymous enum not used as a
     * field within the class, add
     * the enum members to the class instead. */
    ClassType *ct = dynamic_cast<ClassType *>(newClass);
    if ((NULL != ct) && (0 != newClass->_lineNumber)) {
      if (NULL != ct) {
        for (vector<UDT *>::iterator it = ct->_subUDTs.begin();
             it != ct->_subUDTs.end();) {
          EnumUDT *enumUDT = dynamic_cast<EnumUDT *>(*it);
          if ((NULL != enumUDT) && (*it)->isAnonymousType()) {
            bool usedAsField = false;
            for (vector<Field *>::iterator fit = ct->_fieldMembers.begin();
                 fit != ct->_fieldMembers.end(); fit += 1) {
            ...
            if (!usedAsField) {
              ...
              ct->_enumMembers.insert(ct->_enumMembers.end(),
                                      eu->_enumMembers.begin(),
                                      eu->_enumMembers.end());
...
DDR_RC
DwarfScanner::addClassField(Dwarf_Die die, ClassType *const newClass,
                            string fieldName) {
  DDR_RC rc = DDR_RC_ERROR;
  string typeName;
  ...
  if (DDR_RC_OK == getTypeInfo(die, &baseDie, &typeName, &newField->_modifiers,
                               &typeSize, &newField->_bitField)) {
    ...
    Type *fieldType = NULL;
    if ((DDR_RC_OK != addType(baseDie, tag, true, false, &fieldType)) ||
        (NULL == fieldType)) {
      goto AddUDTFieldDone;
    }
...
DwarfScanner::getSuperUDT(Dwarf_Die die, ClassUDT *const udt) {
  ...
  if (DDR_RC_OK == getTypeTag(die, &superTypeDie, &tag)) {
    ...
    /* Get the super udt. */
    if ((DDR_RC_OK ==
         addType(superTypeDie, tag, true, false, (Type **)&superUDT)) &&
        (NULL != superUDT)) {
      rc = DDR_RC_OK;
...
DDR_RC
DwarfScanner::startScan(OMRPortLibrary *portLibrary, Symbol_IR *const ir,
...
DDR_RC
DwarfScanner::scanFile(OMRPortLibrary *portLibrary, Symbol_IR *const ir,
                       const char *filepath) {
  ...
  if (DDR_RC_OK == rc) {
    ...
    if (DW_DLV_OK != res) {
      if (DW_DLV_ERROR == res) {
        ERRMSG("Failed to Initialize libDwarf! DW_DLV_ERROR: res: "
               "%s\nExiting...\n",
               dwarf_errmsg(error));
      } else if (DW_DLV_NO_ENTRY == res) {
        ERRMSG("Failed to Initialize libDwarf! DW_DLV_NO_ENTRY: res: "
               "%s\nExiting...\n",
               dwarf_errmsg(error));
      } else {
        ERRMSG("Failed to Initialize libDwarf! Unknown ERROR: res: "
               "%s\nExiting...\n",
               dwarf_errmsg(error));
      }
      if (NULL != error) {
...
DwarfScanner::~DwarfScanner() {
  for (unordered_map<TypeKey, Type *, KeyHasher>::iterator it =
           _typeMap.begin();
       it != _typeMap.end(); it++) {
...
DDR_RC
DwarfScanner::createTypeKey(Dwarf_Die die, string typeName, TypeKey *key,
  ...
  /* If the Die has a file name and line number declaration, use it to check for
   * duplicate types. */
  if (hasAttr) {
    Dwarf_Attribute attr = NULL;
    if (DW_DLV_ERROR == dwarf_attr(die, DW_AT_decl_file, &attr, &err)) {
    ...
    /* Copy the filename so it is still accessible after libdwarf frees and
     * allocates
     * a new file names table with each CU.
     */
    key->fileName = (char *)malloc(sizeof(char) *
                                   (strlen(_fileNamesTable[declFile - 1]) + 1));
...
bool TypeKey::operator==(const TypeKey &other) const {
  return (0 == strcmp(fileName, other.fileName)) &&
         (typeName == other.typeName) && (lineNumber == other.lineNumber);
}

size_t KeyHasher::operator()(const TypeKey &key) const {
  return ((hash<string>()(key.fileName) << 5) ^
          (hash<string>()(key.typeName) << 3) ^ (hash<int>()(key.lineNumber)));
}
