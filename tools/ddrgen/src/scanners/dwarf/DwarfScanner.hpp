...
public:
  ...
  DDR_RC startScan(OMRPortLibrary *portLibrary, Symbol_IR *const ir,
...
private:
  ...
  DDR_RC addType(Dwarf_Die die, Dwarf_Half tag, bool ignoreFilter,
                 bool isSubUDT, Type **type);
  DDR_RC getType(Dwarf_Die die, Dwarf_Half tag, Type **const newUDT,
                 bool isSubUDT, int *typeNum);
  DDR_RC createType(Dwarf_Die die, Dwarf_Half tag, string dieName,
                    unsigned int lineNumber, Type **const newUDT);
  DDR_RC addEnumMember(Dwarf_Die die, EnumUDT *const udt);
  DDR_RC addClassField(Dwarf_Die die, ClassType *const newClass,
                       string fieldName);
  ...
  DDR_RC createTypeKey(Dwarf_Die die, string typeName, TypeKey *key,
