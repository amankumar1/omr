...
Modifiers::Modifiers()
    : _modifierFlags(NO_MOD), _offset(0), _pointerCount(0), _referenceCount(0) {
}

Modifiers::~Modifiers() {}

const string Modifiers::MODIFIER_NAMES[] = {"const", "volatile", "unaligned",
                                            "restrict", "shared"};

string Modifiers::getModifierNames() {
...
string Modifiers::getPointerType() {
  string s = string("");
...
void Modifiers::addArrayDimension(int length) {
  _arrayLengths.push_back(length);
}

bool Modifiers::isArray() { return _arrayLengths.size() > 0; }

size_t Modifiers::getArrayLength(unsigned int i) {
...
size_t Modifiers::getArrayDimensions() { return _arrayLengths.size(); }

size_t Modifiers::getSize(size_t typeSize) {
  size_t sizeOf = 0;
  if (_pointerCount > 0) {
    sizeOf = sizeof(void *);
  } else {
  ...
  if (isArray()) {
    ...
    for (vector<size_t>::iterator it = _arrayLengths.begin();
         it != _arrayLengths.end(); it += 1) {
      arrayElements += *it;
    }
...
bool Modifiers::operator==(Modifiers const &type) const {
