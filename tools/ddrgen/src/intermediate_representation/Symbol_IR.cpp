...
DDR_RC
Symbol_IR::applyOverrideList(OMRPortLibrary *portLibrary,
  ...
  if (0 > fd) {
    ...
    if (-1 != offset) {
      ...
      /* Read each line as a file name, then open that file to get type
       * overrides. */
...
DDR_RC
Symbol_IR::applyOverrides(OMRPortLibrary *portLibrary,
  ...
  if (0 > fd) {
    ...
    if (-1 != offset) {
      ...
      /* Read each line, expecting format of
       * "[typeoverride/fieldoverride].[StructureName].[fieldName]=[overrideType]"
       */
      if (0 < omrfile_read(fd, buff, offset)) {
        ...
        while (NULL != nextLine) {
          ...
          /* Remove any pointer or array specifiers in the line and use those
           * the field has. */
          ...
          if ((1 == dotCount) && (1 == equalCount)) {
            /* Get the structure name, field name, and override name. Add to
             * list to process. */
  ...
  for (vector<FieldOverride>::iterator it = overrideList.begin();
       it != overrideList.end(); it += 1) {
    ...
    if (typeNames.find(type.structName) != typeNames.end()) {
      ...
      for (vector<Type *>::iterator it2 = typesWithName->begin();
           it2 != typesWithName->end(); it2 += 1) {
        ClassType *ct = dynamic_cast<ClassType *>(*it2);
        /* Iterate the fields of structures with matching names. */
        if (NULL != ct) {
          for (vector<Field *>::iterator it3 = ct->_fieldMembers.begin();
               it3 != ct->_fieldMembers.end(); it3 += 1) {
            /* Once a matching structure and field name are found, apply the
             * override. */
        ...
        if (!type.isTypeOverride) {
          if (NULL != ns) {
            for (vector<Macro>::iterator it3 = ns->_macros.begin();
                 it3 != ns->_macros.end(); it3 += 1) {
              if (it3->_name == type.fieldName) {
                it3->_name = type.overrideName;
              }
            }
...
/* Compute the field offsets for all types in the IR from the sizes of the
 * fields. */
DDR_RC
Symbol_IR::computeOffsets() {
  DDR_RC rc = DDR_RC_OK;
  /* For each Type in the ir, compute the field offsets from the size of each
   * field. */
  for (size_t i = 0; i < _types.size(); i++) {
...
Symbol_IR::removeDuplicates() {
  ...
  for (vector<Type *>::iterator it = _types.begin(); it != _types.end();) {
    ...
    if (NULL == duplicateType) {
       ...
       */
      for (vector<Type *>::iterator it2 = _types.begin(); it2 != _types.end();
           it2 += 1) {
