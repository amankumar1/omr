 ...
 * string.
 *                              Currently this is assumed to be in the OMR
 * layer.
 *
 ...
 * reach a non-extensible class.
 *    After exiting the extensiblePhase, iterate through the rest of the parent
 * classes, checking if they are extensible.
 *       If you find an extensible class, you've found an extensible class that
 * is in a hierarchy like so: (parent -> child)
 *
...
public:
  explicit ExtensibleClassCheckingVisitor(
      ...
      : Extensible(extensible), Context(Context), currentPass(1) {}
    ...
    if (passNumber[decl] != 0) {
      if (Extensible.isExtensible(decl)) {
        ...
        if (pass != passNumber[decl]) {
          std::string diagnostic("Extensible classes heirarchy was incorrect, "
                                 "multiple extensible classes deriving from "
    ...
    for (CXXRecordDecl::base_class_const_iterator BI = decl->bases_begin(),
      ...
      if (extensiblePhase) {
        ...
        if (!Extensible.isExtensible(decl)) {
          ...
          std::string diagnostic("Extensible classes heirarchy was incorrect, "
                                 "multiple extensible class chains in "
                                 "heirarchy.");
          DiagnosticsEngine &diagEngine = Context->getDiagnostics();
          unsigned diagID =
              diagEngine.getCustomDiagID(DiagnosticsEngine::Error, "%0");
...
public:
  ...
  void printRelations() {
    ...
    for (std::map<CXXRecordDecl *, CXXRecordDecl *>::iterator
      ...
      llvm::errs() << " => " << (concrete ? concrete->getQualifiedNameAsString()
                                          : "<no concrete found>");
      llvm::errs() << "\n";
    }
  }
  ...
  CXXRecordDecl *mostDerivedType(CXXRecordDecl *decl) {
    std::map<CXXRecordDecl *, CXXRecordDecl *>::iterator itr =
      ...
      if (extensible) { // Extnsible type .
        ...
        for (CXXRecordDecl::base_class_iterator BI = Type->bases_begin(),
          ...
          if (base_class &&
            ...
            // no concrete class in the middle.
            // Issue diagnostic.
  ...
  CXXRecordDecl *getAssociatedConcreteType(CXXRecordDecl *decl) {
    ...
    if (isExtensible(decl)) {
      ...
      trace("==isExtensible: "
            ...
            << " concrete: " << (concrete ? concrete->getQualifiedNameAsString()
                                          : "no concrete found"));
      while (concrete) {
        if (loopCount++ > 50) {
...
private:
   ...
   * reducing complexity.
   */
  bool inSameClassString(CXXRecordDecl *queryType, CXXRecordDecl *derivedType,
                         int level = 0) {
    ...
    for (CXXRecordDecl::base_class_iterator BI = derivedType->bases_begin(),
                                            BE = derivedType->bases_end();
         BI != BE; ++BI) {
      CXXRecordDecl *base_class = BI->getType()->getAsCXXRecordDecl();
      if (base_class) {
...
public:
  ...
  bool VisitCXXMemberCallExpr(CXXMemberCallExpr *call) {
    // TODO: Check that this member call expression is inside of an extensible
    // class!
    ...
    if (receiver->isImplicitCXXThis()) {
      ...
      if (!isSelfCall(call)) {
        // Ignore member function calls that specifically call a base class
        // member function
        MemberExpr *memberFunc;
        // hasQualifier checks for a nested name specifier, e.g. the
        // 'IBM::Foo::' part of 'IBM::Foo::baz()'
      ...
      if (thisExpr) {
        ...
        if (thisConcrete) {
          ...
          if (targetClass == NULL || targetClass != thisConcrete) {
            DiagnosticsEngine &diagEngine = Context->getDiagnostics();
            unsigned diagID = diagEngine.getCustomDiagID(
                DiagnosticsEngine::Error, "Static casts of this in extensible "
                                          "classes need to be the most derived "
                                          "type. The preferred way of handling "
                                          "this is actually a call to self()");
            DiagnosticBuilder builder =
