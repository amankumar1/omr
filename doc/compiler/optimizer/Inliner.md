...
**TR_CallTarget** directly corresponds to one of the possible implementations of a particular **TR_CallSite**. As such, it typically includes the exact `OMR::ResolvedMethod` and `OMR::ResolvedMethodSymbol`. It also includes a `TR_VirtualGuardSelection` object that describes testing conditions Inliner needs to generate to make sure that it is valid ("validitity" here could mean different things depending on a type of **TR_CallSite**) to execute the inlined body for the implementation it has chosen to inline. 
