...
static uintptr_t comparatorToEqualFn(void *leftKey, void *rightKey,
   ...
   * avl_insert() has the search key
   * as the second node argument. The hash table implementation on the other
   * hand has the search key as
 ...
 * too many entries hash to the same slot.
 * \ingroup     hash_table
 *
 *
 ...
 * designed to be resilient to hash collisions.
 * Specifically, if there are more than listToTreeThreshold collisions in a
 * single bucket, the backing linked list used
 ...
 * into AVL trees.
 * Note also that the trees are only transformed back into normal lists during a
 * hash table grow (assuming they are less than or equal
 ...
 *  a backing pool. When the hashTable grows beyond a certain size, it will
 *expand
...
void hashTableRehash(J9HashTable *table) {
  ...
  if (J9HASH_TABLE_COLLISION_RESILIENT ==
     ...
     * listNodes might become treeNodes (and vice-versa) and
     * it's impossible to predict the required pool capacities.  One might
     * expect this is a just a special case of collisionResilientHashTableGrow,
     * however the problem is much harder as you cannot simply rebuild the
     * original hash table on failure as the entries' hashcodes may
...
static void rebuildFromPools(J9HashTable *table, uint32_t newSize,
   ...
   * and clear all trees. ListToTree() will increment
...
void *hashTableStartDo(J9HashTable *table, J9HashTableState *handle) {
  ...
  if (NULL == table->listNodePool) {
     ...
     * trees, but currently that API is not supported, so we just
