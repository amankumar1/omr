...
void OMRZeroMemory(void *ptr, uintptr_t length) {
   ...
   * smaller then total requested size
   * This is correct because sizes smaller then 2 * Data Cache Block size are
   * served already
   */
  ...
  if (length >= 256 && sse2Supported == 1) {
    ...
    /* because length >= 256 to start, there will always be at least 128 bytes
     * left here */
