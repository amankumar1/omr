...
static J9CONST_TABLE J9HookInterface hookFunctionTable = {
    J9HookDispatch,  J9HookDisable,         J9HookReserve,
    J9HookRegister,  J9HookUnregister,      J9HookShutdownInterface,
    J9HookIsEnabled, J9HookAllocateAgentID, J9HookDeallocateAgentID,
};
 ...
 * disabled
 * before the listeners are informed. Any attempts to add listeners to a
 * TAG_ONCE event
 ...
 * action is taken,
 * and 0 is returned (indicating success). Registrations are NOT counted. Even
 * if a listener
 ...
 * argument must be
 * a uintptr_t agent ID. If the bit is not set, the listener will be added to
 * the J9HOOK_AGENTID_DEFAULT
 * agent. The use of an agent ID allows more control over the order in which
 * events are reported.
 * Listeners with lower agent IDs will always receive events before listeners
 * with higher agent IDs.
 * The special J9HOOK_AGENT_FIRST and J9HOOK_AGENT_LAST IDs may be used to
 * register
...
static intptr_t J9HookRegister(struct J9HookInterface **hookInterface,
  ...
  if (HOOK_FLAGS(commonInterface, eventNum) & J9HOOK_FLAG_DISABLED) {
     ...
     * requested agent.
     * (It is a legitimate position if all records before this position have the
     * same or lower agent IDs (tested previously)
 ...
 * modify the
 * hook interface in any way. Note that there's no atomic protection of any kind
 * -- just
 * because the event wasn't disabled when you called this function doesn't mean
 * that it
