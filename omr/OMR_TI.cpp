...
omr_error_t omrtiRegisterRecordSubscriber(OMR_VMThread *vmThread,
  ...
  if (NULL == vmThread) {
     ...
     * guarantee that
     * the serverf pointer is consistent. Another thread could be in the process
     * of deleting the
     * trace engine.
     *
     * This situation could occur if the trace engine were started & shutdown,
     * and the current
     * thread was bound to the OMR_VM after trace shutdown, so it has a valid
     * OMR_VMThread, but
...
static int64_t computeTimeInterval(const int64_t endNS, const int64_t startNS) {
   ...
   * indicate the timer has gone backwards.
   * In this case, the return value will be negative when interpreted as an
   * int64_t.
   */
