...
omr_error_t OMR_MethodDictionary::getEntries(
  ...
  if (0 == omrthread_monitor_enter(_lock)) {
     ...
     * fit into
     * the available nameBuffer. At this point, flip to RETRY mode: Stop
     * collecting names,
     * and only count the remaining space required.
     */

    for (size_t i = 0; i < methodArrayCount; i++) {
      OMR_MethodDictionaryEntry searchEntryHdr; /* NOTE This is only an entry
                                                   header, and can't hold any
                                                   propertyValues */
