...
void omr_vm_postForkChild(OMR_VM *omrVM) {
   ...
   * corrupted
   * by forking, such as the mutex protecting the threadName. The memory for
   * those
   * sync objects can't be reused, so they must not be freed. Destroying a
   * vmThread
   * normally frees them. We can destroy vmThreads normally here because we have
   * already run omrthread_lib_postForkChild() to remove the references to these
   * sync
   * objects.
   *
