...
omr_error_t omr_ras_initMemCategories(OMRPortLibrary *portLibrary) {
   ...
   * in the table.
   * The port library will supplement this set with internal port library
   * categories.
   * Space must be reserved for the thread library to insert its four
   * categories.
   *
   * This table should not be a global variable because it is modified by
   * omrthread_lib_control(),
   * and the modifications should not be visible across multiple calls to this
   * function.
   */
