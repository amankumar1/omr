...
omr_error_t OMR_Initialize(void *languageVM, OMR_VM **vmSlot) {
   ...
   * attached) is cheaper and less fragile
   * than non-recursive. If performing a sequence of function calls that are
   * likely to attach & detach internally,
   ...
   * except SIGXFSZ.
   * Handling other signals in the port library will interfere with
   * language-specific signal handlers.
   */
  if (0 !=
...
omr_error_t OMR_Shutdown_VM(OMR_VM *omrVM, OMR_VMThread *omrVMThread) {
  ...
  if (NULL != omrVMThread) {
    ...
    fprintf(stderr, "No OMR VMThread so skipping shutdown phases that required "
                    "an attached thread, rc=%d.\n",
            rc);
    fflush(stderr);
  }
  ...
  if (NULL != omrVM) {
    ...
    fprintf(stderr, "No OMR VM so skipping shutdown phases that required an "
                    "initialized VM, rc=%d.\n",
            rc);
    fflush(stderr);
  }
