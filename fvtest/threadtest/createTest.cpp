 ...
 * out of scope
 * while the child thread is running, and that the parent waits for the child
 * thread to
...
static uintptr_t canCreateThread(const create_attr_t *expected,
  ...
  if (J9THREAD_SUCCESS == rc) {
     ...
     * thread.
     * NOTE: It is not guaranteed that tid 0 is invalid in all pthread
     * implementations.
     */
    if (0 == tid) {
...
/* We used to check that a child thread inherits its parent's affinity. This is
 * Linux behaviour, but not
 * AIX or Windows behaviour. We don't rely on, specify or document this
 * behaviour, so don't test it.
 */
#if 0
    ...
    while (numaNode > 0) {
      ...
      if (result == J9THREAD_NUMA_ERR_NO_CPUS_FOR_NODE) {
        PRINTF("Tried to set thread numa affinity to node %zu, but no CPUs "
 ...
 * the setAffinity API without crashing.
 * Note that this should be safe on all platforms (either succeeds or fails but
 * we ignore the return code since
 * any answer is correct, so long as we don't crash - without reading the
 * affinity, we don't know of any more
 * interesting tests).
 */
...
TEST_F(ThreadCreateTest, NumaSetAffinitySuspended) {
  ...
  if (numaMaxNode > 0) {
    ...
    if (expectedAffinityBeforeStart != data.expectedAffinity) {
      PRINTF("Suspended thread's deferred affinity is not what it should be. "
