...
/* Test using a signal handler which installs a handler and raises a signal. */
  ...
  EXPECT_TRUE(handlerCalls == 4) << "Expected 4 handler calls, got "
                                 << handlerCalls << ".";
}
  ...
  for (int i = 0; (i < NUM_TEST_CONDITIONS) && (OMR_ERROR_NONE == rc); i += 1) {
    ...
    if (OMR_ERROR_NONE != runTest(existingPrimary, existingSecondary,
      ...
      printf("Test %d:%d failed at %s:%d. Conditions: %sexisting primary, "
             "%sexisting secondary, flags = %ld, signum is %d, handler is "
             "0x%p, %salt signal stack.\n",
#else  /* (defined(S390) && defined(OMR_ENV_DATA64)) */
      printf("Test %d:%d failed at %s:%d. Conditions: %sexisting primary, "
             "%sexisting secondary, flags = %d, signum is %d, handler is 0x%p, "
             "%salt signal stack.\n",
             ...
             act.sa_flags,
#endif /* defined(WIN32) */
             signum, handlerOptions[handlerIndex], onStack ? "" : "no ");
    }
  }
  return rc;
}
