...
omr_error_t OMRAgent_OnLoad(OMR_TI const *ti, OMR_VM *vm, char const *options,
   ...
   * omrsysinfo_get_CPU_utilization(), which the two APIs rely on, is
   * problematic on z/OS. (details see JAZZ103 53507)
   */
  if (OMR_ERROR_NONE == rc) {
...
static omr_error_t testProcessCpuLoad(OMR_VMThread *vmThread, OMR_TI const *ti,
  ...
  if ((processCpuLoad < 0.0) || (processCpuLoad > 1.0)) {
    omrtty_printf("%s:%d callIndex: %d: GetProcessCpuLoad() returned an "
                  "invalid value, processCpuLoad = %lf ! ProcessCpuLoad should "
                  "be in [0, 1].\n",
...
static omr_error_t testSystemCpuLoad(OMR_VMThread *vmThread, OMR_TI const *ti,
  ...
  if ((systemCpuLoad < 0.0) || (systemCpuLoad > 1.0)) {
    omrtty_printf("%s:%d callIndex: %d: GetSystemCpuLoad() returned an invalid "
                  "value, systemCpuLoad = %lf ! systemCpuLoad should be in [0, "
                  "1].\n",
 ...
 *Value
 *	1 		multiple 	sufficient 		True
 *valid 		valid 		invalid 	valid
 *OMR_ERROR_NONE
 *	2 		multiple 	sufficient 		True
 *valid 		valid 		valid 		valid
 *OMR_ERROR_NONE
 *	3 		multiple 	sufficient 		True
 *valid 		valid 		valid 		doNotExist
 *OMR_ERROR_NONE
 *	4 		multiple 	sufficient 		True
 *valid 		valid 		invalid 	doNotExist
 *OMR_ERROR_RETRY
 *	5 		multiple 	sufficient 		True
 *valid 		valid 		doNotExist 	doNotExist
 *OMR_ERROR_RETRY
 *	6 		multiple 	sufficient 		True
 *valid 		valid 		invalid 	invalid
 *OMR_ERROR_RETRY
 *	7 		multiple 	sufficient 		True
 *valid 		invalid 	valid 		valid
 *OMR_ERROR_NONE
 *	8 		single 		sufficient 		True
 *valid 		valid 		valid 		valid
 *OMR_ERROR_NONE
 ...
 *call 13
 *	(call 8 to 10 valid, single CPU)   (no privilege) (unsupported) (NULL
 *thread)
 *
 *	In this way,
 ...
 *	case 7 is covered by call 2 to 4,	case 8 is covered by call 8 to
 *10,	case 9 is covered by call 11,
 ...
 *sequence:
 *	callIndex		expected return value			covered
 *caseID
 *	1				OMR_ERROR_RETRY
 *5
 *	2				OMR_ERROR_RETRY
 ...
 * get_process_times.
 * Its run time is deterministic and it does the same amount of work each time
 * it is invoked.
 ...
 * Runtime is deterministic and it does the same amount of work each time it is
 * invoked.
 * This implementation is copied from thrbasetest/processtimetest.c
 */
