...
omr_error_t processTraceRecord(PerThreadWrapBuffer *wrapBuffer,
  ...
  if (recordSize > subscriptionID->dataLength) {
    fprintf(stderr, "ERROR: trace buffer (%d bytes) doesn't contain last entry "
                    "(%d bytes)\n",
  ...
  if (-1 == record->nextEntry) {
     ...
     * newest tracepoint) in the buffer.
     ...
     * there was no wrapped tracepoint.
...
static TracePointIteratorState nextTracePoint(PerThreadWrapBuffer *wrapBuffer,
   ...
   * exists.
   * 'offset + 1 - tpLength' points to start of current tracepoint's data, which
   * must be at least 'record->firstEntry'
