...
#define notImplemented(A)                                                      \
  TR_ASSERT(0, "This function is not defined for TestCompiler::FrontEnd %s",   \
            (A))
...
void FrontEnd::encodeStackMap(TR_GCStackMap *map, uint8_t *location,
...
bool FrontEnd::mapsAreIdentical(TR_GCStackMap *mapCursor,
  ...
  if (nextMapCursor && nextMapCursor != stackAtlas->getParameterMap() &&
      ...
      !memcmp(mapCursor->getMapBits(), nextMapCursor->getMapBits(),
              mapCursor->getMapSizeInBytes()) &&
#ifdef TR_HOST_S390
      (mapCursor->getHighWordRegisterMap() ==
       nextMapCursor->getHighWordRegisterMap()) &&
#endif
      (comp->getOption(TR_DisableShrinkWrapping) ||
       (mapCursor->getRegisterSaveDescription() ==
        nextMapCursor->getRegisterSaveDescription()))) {
    return true;
  } else {
    return false;
  }
...
uint8_t *FrontEnd::createStackAtlas(bool encodeFourByteOffsets,
  ...
  if (bytesPerStackMap > USHRT_MAX ||
      ...
      stackAtlas->getParmBaseOffset() < SHRT_MIN ||
      stackAtlas->getParmBaseOffset() > SHRT_MAX ||
      stackAtlas->getLocalBaseOffset() < SHRT_MIN ||
      stackAtlas->getLocalBaseOffset() > SHRT_MAX) {
  ...
  while (mapCursor != NULL) {
    ...
    if (!mapsAreIdentical(mapCursor, nextMapCursor, stackAtlas, comp)) {
      ...
      encodeStackMap(mapCursor, cursorInEncodedAtlas, encodeFourByteOffsets,
                     bytesPerStackMap, comp);
    }
...
uint32_t FrontEnd::calculateSizeOfStackAtlas(bool encodeFourByteOffsets,
                                             uint32_t numberOfSlotsMapped,
                                             uint32_t bytesPerStackMap,
                                             TR::Compilation *comp) {
  TR::CodeGenerator *cg = comp->cg();
