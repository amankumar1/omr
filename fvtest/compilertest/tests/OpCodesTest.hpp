...
public:
  ...
  uint8_t *compileOpCodeMethod(int32_t opCodeArgsNum, TR::ILOpCodes opCode,
                               char *resolvedMethodName, TR::DataType *argTypes,
                               TR::DataType returnType, int32_t &returnCode,
                               uint16_t numArgs = 0, ...);

  uint8_t *compileDirectCallOpCodeMethod(
...
protected:
  ...
  template <typename T> static int32_t comparel(T a, T b) {
    return std::isnan(a) ? -1 : std::isnan(b) ? -1 : a > b ? 1 : a == b ? 0
                                                                        : -1;
  }
  template <typename T> static int32_t compareg(T a, T b) {
    return std::isnan(a) ? 1 : std::isnan(b) ? 1 : a > b ? 1 : a == b ? 0 : -1;
  }
