...
static uintptr_t sigHandlerFunction(struct OMRPortLibrary *portLibrary,
  ...
  outputComment(OMRPORTLIB, "\t%s\t:sigHandlerFunction invoked (type = 0x%x)\n",
...
static uintptr_t maskProtectedFunction(struct OMRPortLibrary *portLibrary,
  ...
  outputComment(OMRPORTLIB, "\t%s\t:maskProtectedFunction is ready for test.\n",
                info->testName);
  ...
  outputComment(OMRPORTLIB, "\t%s\t:maskProtectedFunction exit normally.\n",
                info->testName);
...
static uintptr_t unmaskProtectedFunction(struct OMRPortLibrary *portLibrary,
  ...
  outputComment(OMRPORTLIB, "\t%s\t:unmaskProtectedFunction exit normally.\n",
                info->testName);
  return 8096;
}
...
static int sigMaskThread(void *arg) {
  ...
  outputComment(OMRPORTLIB, "\t%s\t:thread is ready for testing.\n", testName);
  ...
  outputComment(OMRPORTLIB, "\t%s\t:starting pthread_sigmask test...\n",
                testName);
  ...
  outputComment(OMRPORTLIB, "\t%s\t:pthread_sigmask test has been done. send "
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    outputComment(OMRPORTLIB, "\t%s\t:protected function finished.\n",
                  testName);
...
TEST(PortSignalExtendedTests, sig_ext_test1) {
   ...
   * This thread's mask will be inherited. Mask SIGILL for all threads for mask
   * testing.
   */
  ...
  if ((NULL != mainThread) && (NULL != maskThread) && (NULL != unmaskThread)) {
    outputComment(OMRPORTLIB, "\t%s\t:created test threads.\n", testName);
    ...
    outputComment(OMRPORTLIB, "\t%s\t:test threads are READY.\n", testName);

    /* test pthread_sigmask */
    /* ask maskThread to mask SIGBUS signal */
    flags = SIGBUS;
    outputComment(OMRPORTLIB,
                  "\t%s\t:configure mask thread to mask SIGBUS signal.\n",
                  testName);
    ...
    outputComment(OMRPORTLIB, "\t%s\t:testing pthread_sigmask...\n", testName);
    ...
    outputComment(OMRPORTLIB, "\t%s\t:operation pthread_sigmask has been done. "
                              "checking pthread_sigmask result...\n",
                  testName);
    ...
    if (0 != (memcmp_ret = memcmp(&currentMask, &mask, sizeof(currentMask)))) {
      ...
      outputComment(OMRPORTLIB,
                    "\t%s\t:main thread signal mask was not affected.\n",
                    testName);
    }
    ...
    if (!sigismember(&maskThread_mask, SIGILL) ||
      ...
      outputErrorMessage(PORTTEST_ERROR_ARGS, "created threads did not inherit "
                                              "mask.(maskThreadInfo=0x%X, "
    ...
    if (!sigismember(&maskThread_mask, SIGBUS) ||
      ...
      outputErrorMessage(PORTTEST_ERROR_ARGS, "pthread_sigmask did not "
                                              "work.(maskThreadInfo=0x%X, "
                                              "unmaskThreadInfo=0x%X)\n",
                         *((uint32_t *)&maskThread_mask),
                         *((uint32_t *)&unmaskThread_mask));
      goto exit;
    } else {
      outputComment(OMRPORTLIB, "\t%s\t:pthread_sigmask operation in each "
                                "child thread did not interfere each other.\n",
                    testName);
    }
     ...
     * function unmaskProtectedFunction().
     * unmaskProtectedFunction() shall be terminated by SIGSEGV generated in its
     * body, and the unmask thread
     * will send report to this main thread.
     */
    flags = OMRPORT_SIG_FLAG_MAY_RETURN |
            OMRPORT_SIG_FLAG_SIGSEGV; /* SIGSEGV shall be received */
    outputComment(OMRPORTLIB, "\t%s\t:configure unmask thread to prepare for "
                              "unmasked signal SIGSEGV test.\n",
                  testName);
    ...
    if (!waitForEvent(testName, &unmaskThreadInfo, FINISHED, NULL, 0)) {
      ...
      outputComment(OMRPORTLIB, "\t%s\t:unmasked thread finished. checking "
                                "unmask thread's signal status...\n",
                    testName);

      if (unmaskThreadInfo.bulletinBoard.status == SIGNALED) {
        outputComment(OMRPORTLIB,
                      "\t%s\t:unmasked thread received signal as expected.\n",
                      testName);
      } else {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "unmasked thread did not "
                                                "received signal as "
     ...
     */
    outputComment(OMRPORTLIB, "\t%s\t:testing pthread_kill...\n", testName);
    flags = OMRPORT_SIG_FLAG_MAY_RETURN |
    ...
    outputComment(OMRPORTLIB, "\t%s\t:mask thread is ready to receive signal. "
                              "sending pthread_kill...\n",
                  testName);
    ...
    outputComment(OMRPORTLIB, "\t%s\t:notify mask thread to exit.\n", testName);
    sendEvent(&maskThreadInfo, EXIT, NULL, 0);
    if (!waitForEvent(testName, &maskThreadInfo, FINISHED, NULL, 0)) {
      goto exit;
    } else {
      outputComment(OMRPORTLIB, "\t%s\t:masked thread finished. checking mask "
    ...
    outputComment(OMRPORTLIB,
                  "\t%s\t:pthread_sigmask works on each individual thread.\n",
                  testName);
  }

  outputComment(OMRPORTLIB, "\t%s\t:destroying unmaskMonitor...\n", testName);
  omrthread_monitor_destroy(unmaskMonitor);

  outputComment(OMRPORTLIB, "\t%s\t:destroying maskMonitor...\n", testName);
  omrthread_monitor_destroy(maskMonitor);

  goto cleanup;

exit:
  outputComment(OMRPORTLIB, "\t%s\t:test stopped with errors\n", testName);
  FAIL();
