...
TEST_F(PortFileTest2, file_test4) {
  ...
  if (0 == omrsysinfo_get_executable_name("java.properties", &knownFile)) {
    ...
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrsysinfo_get_executable_name("
                                            "\"java.properties\", &knownFile) "
                                            "failed\n");
  }
 ...
 * append and
 * writes FGHIJ to it, fileseeks to start of file and reads 10 chars and
...
TEST_F(PortFileTest2, file_test8) {
  ...
  if (0 != prevFilePtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "File pointer location is wrong "
                                            "after opening file. Expected = 0, "
                                            "Got = %lld",
                       prevFilePtr);
  }
  ...
  if (prevFilePtr + expectedReturnValue != filePtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "File pointer location is wrong "
                                            "after writing into file. Expected "
                                            "= %lld, Got = %lld",
  ...
  if (5 != prevFilePtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "File pointer location is wrong "
                                            "after opening the file. Expected "
                                            "= 5, Got = %lld",
                       prevFilePtr);
  }
#else
  ...
  if (prevFilePtr + expectedReturnValue != filePtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "File pointer location is wrong "
                                            "after reading from file. Expected "
                                            "= %lld, Got = %lld",
                       prevFilePtr + expectedReturnValue, filePtr);
  }
 ...
 *
 ...
 returned -1 expected a valid handle\n");
        ...
        if (fileCount != expectedFileCount)	{
                outputErrorMessage(PORTTEST_ERROR_ARGS, "file count incorrrect,
 ...
 incorrect, found %d expected %d\n", dirCount, expectedDirCount);
        ...
        if (count != expectedCount) {
                outputErrorMessage(PORTTEST_ERROR_ARGS, "count incorrect, found
 %d expected %d\n", count, expectedCount);
                goto exit;
        }
...
intptr_t omrfile_create_file(OMRPortLibrary *portLibrary, const char *filename,
  ...
  if (0 == openMode) {
    ...
    if (-1 == returnFd) {
      lastErrorMessage = (char *)omrerror_last_error_message();
      lastErrorNumber = omrerror_last_error_number();
      outputErrorMessage(PORTTEST_ERROR_ARGS, "Open of file %s in mode %d "
                                              "failed: lastErrorNumber=%d, "
                                              "lastErrorMessage=%s\n",
...
TEST_F(PortFileTest2, file_test18) {
  ...
  if (0 != lockRc) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Locking of bytes %lld for %lld "
                                            "failed: lastErrorNumber=%d, "
                                            "lastErrorMessage=%s\n",
  ...
  if (0 != unlockRc) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Unlocking of bytes %lld for %lld "
                                            "failed: lastErrorNumber=%d, "
                                            "lastErrorMessage=%s\n",
                       offset, length, lastErrorNumber, lastErrorMessage);
    goto exit;
...
int omrfile_test25_child_1(OMRPortLibrary *portLibrary) {
  ...
  if (0 != unlockRc) {
    ...
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(
        PORTTEST_ERROR_ARGS, "Unlocking of bytes %lld for %lld failed: "
                             "lastErrorNumber=%d, lastErrorMessage=%s\n",
  ...
  outputComment(OMRPORTLIB,
                "Child 1: Released write lock on bytes %lld for %lld\n",
                (offset + 2), length);
...
#define nModes 22
   ...
   * is on a locally-mounted filesystem
   * On OSX, when chmod() returns EPERM, it is called again with bits S_ISGID
   * and S_ISUID ignored and returns success.
   */
...
TEST_F(PortFileTest2, file_test30) {
  ...
  if (-1 == invalidFileFd) {
    ...
    if (lastErrorNumber == OMRPORT_ERROR_FILE_EXIST) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrerror_last_error_number(\"%"
                                              "s\") returned %i "
                                              "(OMRPORT_ERROR_FILE_EXIST), "
                                              "anything but would be fine %i\n",
...
TEST_F(PortFileTest2, file_test31) {
  ...
  if (myGid != fileGid) {
    outputComment(OMRPORTLIB, "unexpected group ID.  Possibly setgid bit set. "
 ...
 * amount of free disk space again.
 * It is necessary for the amount of free disk space to changes after the file
 * is created.
 * It is necessary for the total amount of disk space to be constant after the
 * file is created.
 *
...
#define J9FILETEST_FILESIZE 1000 * 512
  ...
  if (buf.isRemote) {
    outputComment(OMRPORTLIB, "WARNING: Test omrfile_test34 cannot be run on a "
                              "remote file system.\nSkipping test.\n");
...
TEST_F(PortFileTest2, file_test36) {
  ...
  if (omrfileFD != nativeFD) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrfileFD != nativeFD, they "
                                            "should be the same. omrfileFD: "
                                            "%x, nativeFD: %x\n",
                       omrfileFD, nativeFD);
  }
  ...
  if (omrfileFD == OMRPORT_INVALID_FD) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "j9fileFD == J9PORT_INVALID_FD, "
...
TEST_F(PortFileTest2, file_test37) {
  ...
  if (rc >= 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Negative test failure: omrfile_fstat(-1, ..) on "
                       "invalid file descriptor is expected to fail but it "
                       "returned success with rc=%d (ignore the following "
                       "LastErrorNumber and LastErrorMessage)\n",
                       rc);
  }
...
TEST_F(PortFileTest2, file_test38) {
  ...
  if (-1 != fd) {
    ...
    if (0 != rc) {
      ...
      if (myGid != fileGid) {
        outputComment(OMRPORTLIB, "Unexpected group ID. Possibly setgid bit "
                                  "set. Comparing to group of the current "
                                  "directory\n");
        rc = omrfile_stat(".", 0, &stat);
        if (0 != rc) {
          outputErrorMessage(
...
TEST_F(PortFileTest2, file_test39) {
   ...
   */
  outputComment(OMRPORTLIB,
                "omrfile_fstat() Test a user only read-writeable file\n");
  omrfile_unlink(filename);
   ...
   */
  outputComment(OMRPORTLIB,
                "omrfile_fstat() Test a user only read-only file\n");
   ...
   */
  outputComment(OMRPORTLIB,
                "omrfile_fstat() Test a user only write-only file\n");
...
TEST_F(PortFileTest2, file_test40) {
  ...
  if (prevFilePtr != filePtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "%s() Changed the location of file "
                                            "pointer. Expected : %lld. Got : "
                                            "%lld \n",
