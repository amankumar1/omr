 ...
 * that was actually
 * found using the Port API.  It then tries to match these, in a OS specific
 * manner.  For example,
 * on Unices (and z/OS), executables names could be ./exec or full-path to the
 * executable; on
 * windows we do not require ./ as long as the executable is in the current
 * directory, and so on.
 *
...
/* On Windows, disregard comparing the extension, should this be dropped on the
 * command
 ...
 */
   ...
   * reduce
   * the number of characters to compare (against the executable name found by
   * API).
   */
...
#define J9SYSINFO_TEST3_OSNAME_LENGTH 1024
  ...
  if (NULL == strstr(osName, "Windows")) {
     ...
     * need to update  omrsysinfo_get_OS_type
...
#undef SI_DEBUG
  ...
  if (rc < 0) {
    ...
    envSize = rc;
#if defined(SI_DEBUG)
    omrtty_printf("Should have a buffer of size %x bytes, using one of size %x "
                  "instead, which will result in a truncated environment\n",
...
TEST(PortSysinfoTest, sysinfo_test_sysinfo_set_limit_ADDRESS_SPACE) {
  ...
  if (geteuid()) {
    /* we should be able to set the hard limit to the current value as an
       unprivileged user
       When the hard limit is set to unlimited (-1) a regular user can change it
       to
  ...
  if (rc == -1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "restoring the original soft limit "
                                            "failed omrsysinfo_set_limit "
                                            "returns -1.\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
...
TEST(PortSysinfoTest, sysinfo_test_sysinfo_set_limit_CORE_FLAGS) {
  ...
  if (geteuid()) {
    ...
    if (OMRPORT_ERROR_FILE_NOPERMISSION == lastErrorNumber) {
      outputComment(OMRPORTLIB, "omrsysinfo_set_limit CORE_FLAGS failed as "
                                "expected because we aren't root.\n");
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrsysinfo_set_limit CORE_FLAGS "
                                              "did not fail with the proper "
                                              "error.\n");
      reportTestExit(OMRPORTLIB, testName);
      return;
    }
...
TEST(PortSysinfoTest, sysinfo_test_sysinfo_get_limit_FILE_DESCRIPTORS) {
  ...
  if (OMRPORT_LIMIT_UNLIMITED ==
    ...
    if (RLIM_INFINITY == curLimit) {
      outputComment(OMRPORTLIB, "omrsysinfo_get_limit(nofiles): soft "
                                "limit=RLIM_INFINITY (unlimited).\n");
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrsysinfo_get_limit(nofiles): "
                                              "soft limit (unlimited), bad "
                                              "maximum reported %lld.\n",
                         ((int64_t)curLimit));
      reportTestExit(OMRPORTLIB, testName);
      return;
    }
    ...
    if ((((int64_t)curLimit) > 0) && (((int64_t)curLimit) <= INT64_MAX)) {
      outputComment(OMRPORTLIB,
                    "omrsysinfo_get_limit(nofiles) soft limit: %lld.\n",
                    ((int64_t)curLimit));
    } else {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
  ...
  if (OMRPORT_LIMIT_UNLIMITED == rc) {
    ...
    if (RLIM_INFINITY == maxLimit) {
      outputComment(OMRPORTLIB, "omrsysinfo_get_limit(nofiles): hard limit = "
                                "RLIM_INFINITY (unlimited).\n");
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrsysinfo_get_limit(nofiles): "
                                              "hard limit (unlimited), bad "
                                              "maximum reported %lld.\n",
                         ((int64_t)curLimit));
      reportTestExit(OMRPORTLIB, testName);
      return;
    ...
    if ((((int64_t)maxLimit) > 0) && (((int64_t)maxLimit) <= INT64_MAX)) {
      outputComment(OMRPORTLIB,
                    "omrsysinfo_get_limit(nofiles) hard limit: %lld.\n",
                    ((int64_t)maxLimit));
    } else {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
  ...
  if (curLimit > maxLimit) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrsysinfo_get_limit(nofiles): "
                                            "current limit exceeds the hard "
                                            "limit.\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
 ...
 * zOS (neither
 * 31-bit not 64-bit) yet, so we exclude these tests from running on zOS. When
 * the zOS
 * implementation is available, we must remove these guards so that they are
 * built and
 ...
 * indeed
 * able to retrieve memory statistics on all supported platforms and also,
 * perform
...
TEST(PortSysinfoTest, sysinfo_testMemoryInfo) {
  ...
  if (0 != rc) {
    ...
    if ((OMRPORT_MEMINFO_NOT_AVAILABLE == memInfo.totalPhysical) ||
         ...
         * OMRPORT_MEMINFO_NOT_AVAILABLE is
         * surely a failure!
         */
        || (OMRPORT_MEMINFO_NOT_AVAILABLE == memInfo.buffered)
 ...
 * the
 * records delivered by the port library routine
...
TEST(PortSysinfoTest, sysinfo_testProcessorInfo) {
   ...
   * drive up the
   * user and kernel utilization.  Use the result of the call to cpuBurner to
   * ensure it isn't optimized out.
  ...
  for (cntr = 1; cntr < currInfo.totalProcessorCount + 1; cntr++) {
     ...
     * in various
     * modes, or else, signal an error. Add platform-specific exceptions
     * (undefined parameter).
     */
   ...
   * This ensures a
   * monotonically increasing nature of processor times. Note: We cannot do this
   * for each individual
   * processor since there may be architectures such as AIXPPC where times don't
   * change for certain
  ...
  if (0 < (deltaTotalBusyTime + deltaTotalIdleTime)) {
    outputComment(OMRPORTLIB,
                  "Processor times in monotonically increasing order.\n");
  } else {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Unexpected change in processor "
                                            "time "
 ...
 * OMRPORT_CPU_ONLINE.
 * We obtain the online processor count using other (indirect) method - calling
 * the other
 * port library API omrsysinfo_get_processor_info() and cross-check against
 * this.
 */
TEST(PortSysinfoTest, sysinfo_testOnlineProcessorCount2) {
   ...
   * from
   * which we may then ascertain the number of processors online. This will help
   * us
  ...
  if (0 != rc) {
     ...
     * that the online
     * processor count received is valid (that is, it does not fail) and that
     * this indeed
     * matches the online processor count as per the processor usage retrieval
     * API.
     */
 ...
 * Validate the number of available (configured) logical CPUs by cross-checking
 * with what is
...
TEST(PortSysinfoTest, sysinfo_testTotalProcessorCount) {
  ...
  /* Call omrsysinfo_get_processor_info() to retrieve a set of processor records
   * from
   * which we may then ascertain the total number of processors configured. We
   * then
  ...
  if (0 != rc) {
     ...
     * the correct total
     * processor count by checking against what omrsysinfo_get_processor_info()
     * returned.
     */
...
TEST(PortSysinfoTest, sysinfo_test_get_CPU_utilization) {
   ...
   * The call to cpuBurner probably won't be optimized out, but use the result
   * to make absolutely sure that it isn't.
   */
  omrthread_sleep(3000 + cpuBurner(OMRPORTLIB, "a"));
 ...
 * on other platforms)
 * Expected result: Successfully create non-ascii directory, and create a file
 * there.
 */
  ...
  if (rc <= 0) {
    ...
    if (NULL == buffer) {
      ...
      if (0 != rc) {
        ...
        if (!strcmp(modifiedTmpDir, buffer)) {
          outputErrorMessage(PORTTEST_ERROR_ARGS, "expected omrsysinfo_get_tmp "
                                                  "to ignore TMPDIR. TMPDIR: "
                                                  "%s, returned: %s\n",
                             modifiedTmpDir, buffer);
        }
      }
 ...
 * ammount of bites and try again.
 * Expected result size of buffer required
 */
...
TEST(PortSysinfoTest, sysinfo_test_get_groups) {
  ...
  if (-1 != rc) {
    ...
    for (i = 0; i < rc; i++) {
      ...
      if (NULL == grent) {
        if (0 == error) {
          outputComment(OMRPORTLIB, "\tthis group id is not found in group "
                                    "database (not an error as per getgrgid "
                                    "documentation)\n");
        } else {
          outputErrorMessage(
              PORTTEST_ERROR_ARGS,
...
TEST(PortSysinfoTest, sysinfo_test_get_open_file_count) {
  ...
  if (((int64_t)openCount) > ((int64_t)curLimit)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrsysinfo_get_open_file_count() "
                                            "failed: reports more files opened "
                                            "than allowed!\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
