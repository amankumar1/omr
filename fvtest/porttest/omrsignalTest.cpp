 ...
 * undefined.
 * Otherwise the register must be expectedKind, or it may be undefined if
 * optional is TRUE.
 */
...
void validateGPInfo(struct OMRPortLibrary *portLibrary, uint32_t gpType,
  ...
  if (infoKind != OMRPORT_SIG_VALUE_ADDRESS) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "OMRPORT_SIG_SIGNAL_HANDLER type "
                                            "is not OMRPORT_SIG_VALUE_ADDRESS. "
                                            "It is %u\n",
                       infoKind);
...
currentSigNumHandlerFunction(struct OMRPortLibrary *portLibrary,
  ...
  if (currentSigNum < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "currentSigNumHandlerFunction -- "
                                            "omrsig_get_current_signal call "
                                            "failed\n");
...
TEST(PortSigTest, sig_test0) {
  ...
  if (!omrsig_can_protect(OMRPORT_SIG_FLAG_MAY_RETURN)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "portLibrary->can_protect() must "
                                            "always support 0 | "
  ...
  if (!omrsig_can_protect(OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION)) {
     ...
     * under a configuration that does not support it.
     * See implementation of omrsig_can_proctect() and omrsig_startup()
     */
    omrtty_printf("OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION is not supported if "
                  "the default settings for TRAP(ON,SPIE) have been changed. "
                  "The should both be set to 1\n");
#endif
  }
...
TEST(PortSigTest, sig_test1) {
  ...
  if (!omrsig_can_protect(0)) {
    ...
    if (result != 8096) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "portLibrary->sig_protect -- "
                                              "protected function did not "
                                              "execute correctly\n");
    }
  }
 ...
 *Function B
 * against a SEGV with handler HB. Function B raises a sigsegv. HB should handle
 *it, returning
 * OMRPORT_SIG_EXCEPTION_RETURN, control ends up after the call to
 *omrsig_protect in B.
 ...
 * Test protecting Function A against a SEGV with handler HA. A in turn protects
 *Function B against
 * nothing with handler HB. Function B raises a sigsegv. HB won't get invoked
 *because it was not protecting against anything
 ...
 * Test protecting Function A against a SEGV with handler HA. A in turn protects
 * Function B
 * against a SEGV with handler HB. Function B raises a sigsegv. HB gets invoked,
 * returns OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH
 * so that HA gets invoked, which then returns control to A (and then
 * omrsig_test5).
 *
 * 		->SEGV
 * 		|	 |
...
TEST(PortSigTest, sig_test8) {
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    if (protectResult != OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "portLibrary->sig_protect -- "
                                              "expected "
                                              "OMRPORT_SIG_EXCEPTION_CONTINUE_"
                                              "SEARCH in protectResult\n");
    }
#else
    if (protectResult != OMRPORT_SIG_EXCEPTION_OCCURRED) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "portLibrary->sig_protect -- "
...
TEST(PortSigTest, sig_test_async_unix_handler) {
  ...
  for (index = 0; index < sizeof(testSignalMap) / sizeof(testSignalMap[0]);
    ...
    if (1 != handlerInfo.controlFlag) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "%s handler was NOT invoked when "
                                              "%s was injected by another "
                                              "process\n",
                         testSignalMap[index].portLibSignalString,
                         testSignalMap[index].unixSignalString);
    }
     ...
     * thread.
     * pltest runs with 2 threads only, this thread and the asyncSignalReporter
     * thread.
     * We will mask the signal on this thread, inject it via kill, and verify
     * that it does
     * not get received.
     */
    ...
    if (0 != handlerInfo.controlFlag) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "%s handler was unexpectedly "
                                              "invoked when %s was injected by "
                                              "another process\n",
                         testSignalMap[index].portLibSignalString,
                         testSignalMap[index].unixSignalString);
    }
