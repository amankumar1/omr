...
TEST(PortSysinfoTest, sysinfo_numcpus_test0) {
  ...
  if (0 == numberPhysicalCPUs) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "\tomrsysinfo_get_number_CPUs_by_"
                                            "type(OMRPORT_CPU_PHYSICAL) "
...
TEST(PortSysinfoTest, sysinfo_numcpus_test1) {
  ...
  if (0 == boundCPUs) {
    ...
    if (numberBoundCPUs < numberPhysicalCPUs) {
      outputComment(OMRPORTLIB, "The test was configured to run without CPU "
                                "restrictions (e.g. taskset), but CPU "
                                "restrictions were detected.\n");
      outputComment(
          OMRPORTLIB,
  ...
  if ((0 == boundTest) && (numberPhysicalCPUs == numberBoundCPUs)) {
    ...
    outputErrorMessage(PORTTEST_ERROR_ARGS, "\nomrsysinfo_get_number_CPUs_by_"
                                            "type(OMRPORT_CPU_BOUND) returned "
                                            "unexpected value.\n");
  }
...
TEST(PortSysinfoTest, sysinfo_numcpus_test2) {
  ...
  if (0 == numberOnlineCPUs) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "\tomrsysinfo_get_number_CPUs_by_"
                                            "type(OMRPORT_CPU_ONLINE) returned "
                                            "invalid value 0.\n");
  }
...
TEST(PortSysinfoTest, sysinfo_numcpus_test3) {
  ...
  outputComment(OMRPORTLIB, "\nGet number of online and physical CPUs. "
                            "Validate online <= physical.\n");
...
TEST(PortSysinfoTest, sysinfo_numcpus_test4) {
  ...
  if (result > 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "\tomrsysinfo_get_number_CPUs_by_"
                                            "type returned non-zero value when "
                                            "given invalid type.\n");
  }
 ...
 * @param[in] type The type of CPU number to query (physical, bound, entitled,
 * target).
 *
 ...
 * online
 * processor count using other (indirect) method - calling the other port
 * library API
 ...
 * number of
 * available (configured) logical CPUs by cross-checking with what is obtained
 * from
