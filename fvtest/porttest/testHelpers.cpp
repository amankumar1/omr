 ...
 * exit
 * of a testcase @ref reportTestExit will indicate if the test passed or failed
 * based
 * on this variable.
 */
 ...
 *
 * Duplicated function from UTH.  This function determines what level of tracing
 * is
 * available, a command line option of UTH.  The levels supported are
 * application dependent.
 *
 * Application programs thus decide how much tracing they want, it is possible
 * to display
 ...
 */
static int engine_getTraceLevel(void) { return 1; }
 ...
 * file systems
 * are examples of these areas.  In addition even if an area is present not all
 * functionality
 ...
 *
 * @internal @note For UTH environment only, displaying the string is controlled
 * by the verbose
 * level expressed by @ref engine_getTraceLevel.  Outside the UTH environment
 * the messages
...
void dumpTestFailuresToConsole(struct OMRPortLibrary *portLibrary) {
  ...
  for (i = 0; i < numTestFailures; i++) {
    ...
    omrtty_printf("\t%s line %4zi: %s\n", testFailures[i].fileName,
                  testFailures[i].lineNumber, testFailures[i].errorMessage);
