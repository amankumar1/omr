 ...
 * This information is required for us to walk the heap and check its integrity
 * after an allocation or free.
 */
struct J9Heap {
 ...
 * fill the memory with 1's.
 * It then enters a loop that calls omrheap_create on each of the first 16
 * byte-aligned locations with a heap size of 1MB.
 * After each omrheap_create, we perform a series of heap_alloc and heap_free
 * from size 0 to the maximum possible.
 * And finally, we check the bytes after the heap memory to see if we have
    ...
    for (i = 0; i < 16; i++) {
      ...
      do {
        ...
        if (subAllocPtr) {
          ...
          /*The size returned may have been rounded-up to the nearest U64, and
           * may have an extra 2 slots added in some circumstances*/
          if (querySize < subAllocSize ||
              querySize > (subAllocSize + 3 * sizeof(uint64_t))) {
            outputErrorMessage(PORTTEST_ERROR_ARGS, "omrheap_query_size "
                                                    "returned the wrong size. "
                                                    "Expected %zu, got %zu\n",
 ...
 * a fairly large heap.
 * We go into a loop of heap_allocate and heap_free, the size of alloc is based
 * on the random value (i.e. [0, allocSizeBoundary]).
 * We store the returned address, request size, etc in a sub-allocated memory
 * identifier, which is stored in a J9Pool.
 * When we do a heap_free, we randomly pick an element from the pool and free
 * the memory based on the values stored in the identifier.
 * We keep a separate counter for number of heap_allocate and heap_free calls,
 * and we terminate the loop when heap_free count reaches a certain value e.g.
 * 500,000.
 *
 ...
 * and again allocate the largest possible size.
 * If this allocation is no longer successful, it means we have leaked memory on
 * the heap during our loop execution.
 *
...
int omrheap_test2(struct OMRPortLibrary *portLibrary, int randomSeed) {
  ...
  outputComment(OMRPORTLIB, "Random seed value: %d. Add -srand:[seed] to the "
                            "command line to reproduce this test manually.\n",
                randomSeed);
  srand(randomSeed);
  ...
  while (freeSerialNumber <= serialNumberTop) {
    ...
    if (0 == operation) {
      ...
      while ((subAllocMem = omrheap_allocate(heapBase, allocSize)) == NULL) {
        /*outputComment(OMRPORTLIB, "**Failed omrheap_allocate,
         * sizeRequested=%d\n", allocSize);*/
      ...
      if (0 == reallocSerialNumber % outputInterval) {
        outputComment(OMRPORTLIB, "Realloc: index=%zu, size=%zu, result=%p, "
...
TEST(PortHeapTest, heap_grow_test) {
   ...
   *of uint64_t which is 8 bytes.
   * So expected heapSize is 1024/8 = 128 slots
   * Initially when heap is created, 2*sizeof(uintptr_t) of the slots are used
   *for heap header.
   * So 1 slot is used for header on 32 bit platforms, 2 slots are used on 64
   *bit platforms.
   * For each block of memory, either allocated or free, there is a head and
   *tail slots.
   * If the value in tail is negative, then it means that the block is
   *allocated.
   * If it is free, then it means that the block is free.
   * Absolute value of the integer in head and tail show how many slots there
   *are BETWEEN head and tail slots.
   * So when omrheap created initially, firstFreeBlock always has the value 2(on
   *64bit) or 1 (on 32bit) and heapSize should be 128 slots in this test heap.
   * And firstFreeBlock should point to the first slot after header
   * which should have value 128 - sizeof(omrheap)/sizeof(uint64_t) - 2(head and
   *tail of the slot) = 124 (on 64bit) or 125 (on 32bit)
   *
   *
   * |--------|-----|--------------------------------------------------------------------------------------------------------
   * |heapSize|first|    |    |
  ...
  if (128 != heapBase->heapSize) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "heap_create failed to set the "
                                            "heap->heapSize correctly. "
                                            "Expected :  128, Found : %zu \n",
                       heapBase->heapSize);
    goto exit;
  }
  ...
  if (128 != heapBase->heapSize) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "heap_allocate corrupted "
                                            "heapBase->heapSize. Expected :  "
                                            "128, Found : %zu \n",
                       heapBase->heapSize);
    goto exit;
  }
  ...
  if (initialFirstFreeBlockValue + 10 != heapBase->firstFreeBlock) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "heap_allocate failed to set the "
                                            "heap->firstFreeBlock correctly. "
                                            "Expected :  %zu, Found : %zu \n",
  ...
  */
   ...
   * firstFreeBlock points to the head of last slots and the value of head
   * changes as we grow heap. So do tail.
   *
   */
...
TEST(PortHeapTest, heap_test_pmr_28277_999_760) {
  ...
  alloc1 = omrheap_allocate(
      ...
      16); /* 16/8 = 2   Alloc 2 slots. Slot Size = sizeof(uint64_t) */
            ...
            * |
            * |   |
            * |		|Block  | Slot	|  Size   |-2|Alloc0|-2|-2|Alloc1|-2|86|   FREE
  ...
  omrheap_allocate(
      ...
      16); /* 16/8 = 2   Alloc 2 slots. Slot Size = sizeof(uint64_t) */
            ...
            * |
            * |		|Block  | Slot	|  Size
            * |-2|Alloc0|-2|-2|Alloc1|-2|-2|Alloc2|-2|82|	FREE
            * FREE
            * FREE			FREE			FREE
            * |82
            * |
  ...
  omrheap_allocate(
      ...
      16); /* 16/8 = 2   Alloc 2 slots. Slot Size = sizeof(uint64_t) */
            ...
            * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|----------------------|--|--|------|--|--|------|--|--|-------------------|---|
            * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15
            * 16 17	  19 20 21	               		29 30 31     33 34 35     37
            * 38
            * 39					99	100
            *
            */
   ...
   * 2|-2|Alloc2|-2|-2|Alloc3|-2|-2|Alloc4_1
   * |-2|-2|Alloc5|-2|-2|Alloc6|-2|60|		FREE		|60	|
   * |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |
   * |2Slots|
   * |  |2Slots|  |  |8Slots        	    |  |  |2Slots|  |  |2Slots|  |  |
   ...
   * |   |
   * |		|Block  | Slot	|  Size   |-2|Alloc0|-2| 2|FREE  |
   * 2|-2|Alloc2|-2|-2|Alloc3|-2|8 |FREE              	|8 |-2|Alloc
  ...
  alloc4_1 = omrheap_allocate(
      ...
      24); /* 24/8 = 3   Alloc 3 slots. Slot Size = sizeof(uint64_t) */
            ...
            * 2|-2|Alloc
            * |-2|-2|Alloc |-2|-3|Alloc4_1|-3|3 |FREE    |3 |-2|Alloc |-2|-2|Alloc
            * |-2|60|	FREE			|60	|
            * |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |
            * |2Slots|
            ...
            * ---------------|---|
            * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15
            * 16 17	  19 20 21	     24 25 26        29 30 31     33 34 35   	37
            * 38
            * 39 				99	100
            *
            */
  ...
  alloc4_2 = omrheap_allocate(
      ...
      24); /* 24/8 = 3   Alloc 3 slots. Slot Size = sizeof(uint64_t) */
            ...
            * |		|Block  | Slot	|  Size   |-2|Alloc	|-2| 2|FREE  |
            * 2|-2|Alloc
            * |-2|-2|Alloc |-2|-3|Alloc4_1|-3|-3|Alloc4_2|-3|-2|Alloc |-2|-2|Alloc
            * |-2|60|	FREE			|60	|
            * |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |
            * |2Slots|
            * |  |2Slots|  |  | 3Slots |  |  |3Slots  |  |  |2Slots|  |  |2Slots|  |  |
   ...
   * |		|Block  | Slot	|  Size   |-2|Alloc	|-2| 2|FREE  |
   * 2|-2|Alloc
   * |-2|-2|Alloc |-2| 3|FREE	  | 3|-3|Alloc4_2|-3|-2|Alloc |-2|-2|Alloc
   * |-2|60|	FREE			|60	|
   * |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |
   * |2Slots|
   * |  |2Slots|  |  | 3Slots |  |  |3Slots  |  |  |2Slots|  |  |2Slots|  |  |
   ...
   * |   |
   * |		|Block  | Slot	|  Size   |-2|Alloc0|-2| 2|FREE  |
   * 2|-2|Alloc2|-2|-2|Alloc3|-2|-8|Alloc4_1            	|-8|-2|Alloc
   * |-2|-2|Alloc |-2|60|	FREE			|60	|
   * |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |
   * |2Slots|
   * |  |2Slots|  |  |8slots            	|  |  |2Slots|  |  |2Slots|  | |
 ...
 * J9Heap.
 * It tries to call heap_allocate with the size used to create the heap, and
 * gradually decreases the size until an heap_allocate succeeds.
...
static void verifySubAllocMem(struct OMRPortLibrary *portLibrary,
  ...
  if (blockSizeStart >= 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Sub-allocated memory is not part "
                                            "of an occupied block: 0x%p, top "
                                            "size: %lld, bottom size: %lld\n",
                       subAllocMem, blockSizeStart, blockSizeEnd);
    return;
  }
  blockSizeEnd = blockStart[-blockSizeStart];
  if (blockSizeStart != blockSizeEnd) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Top and bottom padding of the "
                                            "Sub-allocated memory do not "
                                            "match: 0x%p\n",
                       subAllocMem);
    return;
  }
  if (0 == allocSize) {
    if (!(1 <= -blockSizeStart && -blockSizeStart <= 3)) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "The allocated block seems to be "
                                              "overly large (3 or more free "
                                              "slots): 0x%p, request size: "
                                              "%zu, block size: %lld\n",
 ...
 * the heap.
 * We check for memory location from memAllocStart to
 * memAllocStart+heapStartOffset, which immediate precedes the heap.
...
static void verifyHeapOutofRegionWrite(struct OMRPortLibrary *portLibrary,
  ...
  for (i = 0; i <= 500; i++) {
    if (0xff != *(heapEnd + i)) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "Address %p following the heap "
...
static void walkHeap(struct OMRPortLibrary *portLibrary, J9Heap *heapBase,
  ...
  while (((uintptr_t)blockTopPaddingCursor) < ((uintptr_t)lastSlot)) {
    ...
    if (0 == topBlockSize) {
      ...
      if (0 == firstFreeBlock) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "\nfirstFreeBlock indicates "
                                                "heap is full but found empty "
                                                "block @ 0x%p\n",
                           blockTopPaddingCursor);
        return;
      }
      if (firstFreeUnmatched == TRUE) {
        if (firstFreeBlock !=
            (((uintptr_t)blockTopPaddingCursor - (uintptr_t)basePtr) /
             sizeof(uint64_t))) {
          outputErrorMessage(PORTTEST_ERROR_ARGS, "\nfirstFreeBlock and the "
                                                  "actual fist free block "
                                                  "don't match @ 0x%p\n",
                             blockTopPaddingCursor);
          return;
        }
  ...
  for (i = 0; i < 10; i += 1) {
    ...
    if (rc) {
      ...
      if ((omrtime_current_time_millis() - timeStart) >=
        ...
        outputComment(OMRPORTLIB, "We've run for too long, ending the test. "
