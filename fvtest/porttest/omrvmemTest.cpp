 ...
 *
 * Exercise the API for port library virtual memory management operations.
 * These functions
...
#if defined(LINUX) || defined(AIXPPC) || defined(WIN32) ||                     \
    defined(J9ZOS390) || defined(OSX)
#define ENABLE_RESERVE_MEMORY_EX_TESTS
#endif /* defined(LINUX) || defined(AIXPPC) || defined(WIN32) ||               \
          defined(J9ZOS390) || defined(OSX) */
 ...
 * below 2G bar, FALSE otherwise.
 * On z/OS, 1M fixed and 2G fixed pages can be used only to allocate above 2G
 * bar.
...
/* reserve and commit */
    ...
    if (memPtr == NULL) {
      ...
      if (OMRPORT_ERROR_VMEM_INSUFFICENT_RESOURCES == lastErrorNumber) {
        ...
        outputComment(OMRPORTLIB, "\t\tREBOOT THE MACHINE to free up resources "
      ...
      outputComment(OMRPORTLIB, "\treserved and committed 0x%zx bytes with "
                                "page size 0x%zx at address 0x%zx\n",
      ...
      if (finalBlocks <= initialBlocks) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "vmem reserve didn't increment "
                                                "category block as expected. "
                                                "Final blocks=%zu, initial "
                                                "blocks=%zu, page size=%zu.\n",
                           finalBlocks, initialBlocks, pageSizes[i]);
      }

      if (finalBytes < (initialBytes + pageSizes[i])) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "vmem reserve didn't increment "
                                                "category bytes as expected. "
                                                "Initial bytes=%zu, final "
                                                "bytes=%zu, page size=%zu.\n",
    ...
    if (rc != 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrvmem_free_memory returned %i "
                                              "when trying to free 0x%zx bytes "
                                              "at 0x%zx\n",
...

    {
      ...
      if (finalBlocks != initialBlocks) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "vmem free didn't decrement "
                                                "category block as expected. "
                                                "Final blocks=%zu, initial "
                                                "blocks=%zu, page size=%zu.\n",
                           finalBlocks, initialBlocks, pageSizes[i]);
...
int omrvmem_bench_write_and_decommit_memory(struct OMRPortLibrary *portLibrary,
  ...
  if (memPtr == NULL) {
    ...
    if (OMRPORT_ERROR_VMEM_INSUFFICENT_RESOURCES == lastErrorNumber) {
      omrtty_printf(
          "\tPortable error OMRPORT_ERROR_VMEM_INSUFFICENT_RESOURCES...\n");
      omrtty_printf("\t\tREBOOT THE MACHINE to free up resources AND TRY THE "
                    "TEST AGAIN\n");
    }
    goto exit;
  } else {
  ...
  omrtty_printf("%s pageSize 0x%zx, byteAmount 0x%zx, millis for %u "
...
int omrvmem_bench_reserve_write_decommit_and_free_memory(
  ...
  for (i = 0; numIterations != i; i++) {
    ...
    if (memPtr == NULL) {
      ...
      lastErrorNumber = omrerror_last_error_number();
      outputErrorMessage(
          PORTTEST_ERROR_ARGS, "In iteration %d, unable to reserve and commit "
                               "0x%zx bytes with page size 0x%zx.\n"
                               "\tlastErrorNumber=%d, lastErrorMessage=%s\n ",
  ...
  omrtty_printf("%s pageSize 0x%zx, byteAmount 0x%zx, millis for %u "
                "iterations: [reserve/write_to_all/decommit/free] test: %lli\n",
                disclaim ? "disclaim" : "nodisclaim", pageSize, byteAmount,
                numIterations, deltaMillis);
...
TEST(PortVmemTest, vmem_testReserveMemoryEx) {
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    ...
    outputComment(OMRPORTLIB, "***Did not find enough memory available on "
                              "system, not running test "
                              "omrvmem_testReserveMemoryEx\n");
  }
}
...
#define NUM_SEGMENTS 3
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    for (j = 0; j < NUM_SEGMENTS; j++) {
      ...
      outputComment(
          OMRPORTLIB, "\tPage Size: 0x%zx Range: [0x%zx,0x%zx] "
                      "topDown: %s quickSearch: %s strictAddress: %s "
                      "strictPageSize: %s use2To32G: %s\n",
      ...
      if (memPtr[j] == NULL) {
        if (strictPageSize) {
          outputComment(OMRPORTLIB, "\t! Unable to reserve and commit 0x%zx "
          ...
          if (OMRPORT_ERROR_VMEM_INSUFFICENT_RESOURCES == lastErrorNumber) {
            outputComment(OMRPORTLIB, "\tPortable error "
                                      "OMRPORT_ERROR_VMEM_INSUFFICENT_"
                                      "RESOURCES...\n");
            outputComment(OMRPORTLIB, "\t\tREBOOT THE MACHINE to free up "
        ...
        outputErrorMessage(PORTTEST_ERROR_ARGS, "\t use2To32G flag is set and "
                                                "returned pointer [0x%zx] is "
                                                "outside of range.\n",
                           memPtr[j]);
#endif /* OMR_ENV_DATA64 */
      } else {
        outputComment(
            OMRPORTLIB, "\t reserved and committed 0x%zx bytes with page size "
      ...
      if (NULL != memPtr[j]) {
        ...
        if (vmemID[j].pageFlags != omrvmem_get_page_flags(&(vmemID[j]))) {
          outputErrorMessage(
              PORTTEST_ERROR_ARGS, "vmemID[j].pageFlags (0x%zx)!= "
        ...
        if (finalBlocks <= initialBlocks) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "vmem reserve didn't increment category block as "
                             "expected. Final blocks=%zu, initial blocks=%zu, "
                             "page size=%zu.\n",
                             finalBlocks, initialBlocks, pageSizes[i]);
        }
        ...
        if (finalBytes < (initialBytes + pageSizes[i])) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "vmem reserve didn't increment category bytes as "
                             "expected. Final bytes=%zu, initial bytes=%zu, "
                             "page size=%zu.\n",
                             finalBytes, initialBytes, pageSizes[i]);
        }
      }
    ...
    for (j = 0; j < NUM_SEGMENTS; j++) {
      if (NULL != memPtr[j]) {
        ...
        if (rc != 0) {
          outputErrorMessage(PORTTEST_ERROR_ARGS, "omrvmem_free_memory "
                                                  "returned %i when trying to "
        ...
        if (finalBlocks >= initialBlocks) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "vmem free didn't decrement category blocks as "
                             "expected. Final blocks=%zu, initial blocks=%zu, "
                             "page size=%zu.\n",
                             finalBlocks, initialBlocks, pageSizes[i]);
        }
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    for (j = 0; j < NUM_SEGMENTS; j++) {
      ...
      if (NULL != memPtr[j]) {
        ...
        if (rc != 0) {
          isMemoryAvailable = FALSE;
          outputComment(OMRPORTLIB, "\t**omrvmem_free_memory returned %i when "
                                    "trying to free 0x%zx bytes at 0x%zx\n",
                        rc, pageSizes[i], memPtr);
        }
      }
    }
...
TEST(PortVmemTest, vmem_testReserveMemoryEx_zOSLargePageBelowBar) {
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    /* did we get any memory? */
    if (NULL == memPtr) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "unable to reserve and commit "
                                              "0x%zx bytes with page size "
                                              "0x%zx, page type 0x%zx.\n",
    ...
    if (rc != 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrvmem_free_memory returned "
                                              "0x%zx when trying to free 0x%zx "
                                              "bytes\n",
    ...
    if (NULL == memPtr) {
      ...
      if (FALSE == isPageSizeSupportedBelowBar(pageSizes[i], pageFlags[i])) {
        if (FOUR_KB != vmemID.pageSize) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "\t didn't expect to allocate memory below 2GB "
                             "using page size 0x%zx and page flags 0x%x with "
                             "strict address\n",
 ...
 * set on z/OS.
 * In this case, since STRICT_ADDRESS flag is not set, USE2TO32G flag is given
 * priority over
 * the requested address range. Therefore, memory is expected to be allocated
 * between 2G-32G.
 */
...
TEST(PortVmemTest,
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
     ...
     * requested address is below 2G with STRICT_ADDRESS set
     * The two conditions are incompatible and should return NULL.
     */
    if (memPtr != NULL) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Did not expect to allocate memory using page size "
                         "0x%zx, page type 0x%zx in < 2GB range with "
                         "OMRPORT_VMEM_ZOS_USE2TO32G_AREA and "
      ...
      outputComment(OMRPORTLIB, "\t Expected to fail to allocate large pages "
                                "in < 2GB range with "
                                "OMRPORT_VMEM_ZOS_USE2TO32G_AREA and "
...
TEST(PortVmemTest, vmem_test_reserveExecutableMemory) {
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (NULL == memPtr) {
      outputComment(OMRPORTLIB, "! Could not find 0x%zx bytes available with "
                                "page size 0x%zx and specified mode\n",
                    params.byteAmount, pageSizes[i]);
    } else {
      intptr_t rc;
      void (*function)();
      ...
      *((unsigned int *)memPtr) = (unsigned int)0x4e800020; /* blr instruction
                                                               (equivalent to
                                                               RET on x86)*/
       ...
       * memory below 2G bar
       * can be used as executable pages. Other page sizes do not support
       * executable pages.
       */
      if (FALSE == isPageSizeSupportedBelowBar(pageSizes[i], pageFlags[i])) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
...
TEST(PortVmemTest, vmem_test_numa) {
  ...
  if ((detailReturnCode != 0) || (0 == totalNumaNodeCount)) {
    ...
    if (nodeArray == NULL) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrvmem_free_memory returned "
                                              "NULL when trying to allocate "
                                              "J9MemoryNodeDetail array for "
                                              "NUMA test (FAIL)\n");
      goto exit;
    }
    ...
    if ((detailReturnCode != rc) || (totalNumaNodeCount != originalNodeCount)) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS, "omrvmem_numa_get_node_details returned "
                               "different results on second call (return %zd "
                               "versus %zd, count %zu versus %zu)\n",
    ...
    if (0 == nodesWithMemoryCount) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "Found zero nodes with memory "
                                              "even after NUMA was reported as "
                                              "supported (FAIL)\n");
      goto exit;
    }
    /* reserve and commit memory for each page size */
      ...
      if (memPtr != NULL) {
        ...
        for (nodeIndex = 0; nodeIndex < totalNumaNodeCount; nodeIndex++) {
          if (policyForMemoryNode == nodeArray[nodeIndex].memoryPolicy) {
            ...
            if (rc != 0) {
              outputErrorMessage(PORTTEST_ERROR_ARGS,
                                 "omrvmem_numa_set_affinity returned 0x%zx "
                                 "when trying to set affinity for 0x%zx bytes "
                                 "at 0x%p to node %zu\n",
...
#define PRINT_FIND_VALID_PAGE_SIZE_INPUT(mode, pageSize, pageFlags)            \
  outputComment(OMRPORTLIB, "Input > mode: %zu, requestedPageSize: 0x%zx, "    \
...
void verifyFindValidPageSizeOutput(
  ...
  outputComment(OMRPORTLIB, "Expected > expectedPageSize: 0x%zx, "
                            "expectedPageFlags: 0x%zx, isSizeSupported: "
                            "0x%zx\n\n",
...
#define MODE_NOT_USED 0
   ...
   * Only difference is in case of -Xlp<size>, omrvmem_find_valid_page_size() is
   * not called
...
#define SAMPLE_BLOCK_SIZE 4
  ...
  if (NULL == address) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Failed to allocate block of "
                                            "memory to determine page size of "
                                            "data segment");
    goto _exit;
  }
...
#if !defined(OMR_ENV_DATA64)
  outputComment(OMRPORTLIB, "\nCase %d: -Xlp:codecache:pagesize=<not in 16G, "
                            "16M, 64K, 4K> with "
                            "TR_ppcCodeCacheConsolidationEnabled set\n",
                caseIndex++);
  /* No port library API to set the environment variable. Use setenv() */
...
/* Test -Xlp:objectheap options */
  ...
  outputComment(OMRPORTLIB, "\nCase %d: -Xlp2G\n", caseIndex++);
  if (0 == defaultLargePageSize) {
    outputComment(OMRPORTLIB, "Skip this test as the configuration does not "
                              "support default large page size\n");
  } else {
    mode = 0;
    requestedPageSize = TWO_GB;
    requestedPageFlags = OMRPORT_VMEM_PAGE_FLAG_NOT_USED;
 ...
 * tests effects of disclaiming memory
...
int32_t omrvmem_disclaimPerfTests(struct OMRPortLibrary *portLibrary,
  ...
  if (TEST_PASS == rc) {
    outputComment(OMRPORTLIB, "\npageSize: 0x%zx, byteAmount 0x%zx, memTotal "
                              "0x%zx, numIterations %i, disclaim %i\n",
...
TEST(PortVmemTest, vmem_testOverlappingSegments) {
  ...
  outputComment(OMRPORTLIB, "Segment           Start             End           "
                            "    Size              Keep cycle\n");
  ...
  for (i = 0; i < CYCLES; i++) {
    ...
    outputComment(OMRPORTLIB, "%.16d  %.16X  %.16X  %.16X  %d\n", i,
                  ...
                  (uintptr_t)vmemID[i].address + vmemID[i].size, vmemID[i].size,
                  keepCycles[i]);
