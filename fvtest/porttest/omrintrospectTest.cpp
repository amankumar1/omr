...
TEST(PortIntrospectTest, introspect_test_set_signal_offset) {
  ...
  if (0 == status) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrintrospect_set_suspend_signal_"
                                            "offset with negative offset "
  ...
  outputComment(OMRPORTLIB, "verify that "
                            "omrintrospect_set_suspend_signal_offset returns "
                            "failure on unsupported platforms\n");
  status = omrintrospect_set_suspend_signal_offset(0);
  outputComment(OMRPORTLIB,
                "omrintrospect_set_suspend_signal_offset returns %d \n",
                status);
  if (0 == status) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
