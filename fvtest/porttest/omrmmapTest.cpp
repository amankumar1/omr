 ...
 * to allocate
 *       memory and read in the file.  This is the equivalent of copy-on-write
 * mmapping
 *
 */
#include <stdlib.h>
...
typedef struct simpleHandlerInfo { const char *testName; } simpleHandlerInfo;
...
static uintptr_t simpleHandlerFunction(struct OMRPortLibrary *portLibrary,
  ...
  outputComment(OMRPORTLIB,
                " A crash occured, signal handler invoked (type = 0x%x)\n",
                gpType);
...
#define J9MMAP_TEST1_BUFFER_SIZE 100
  ...
  if (-1 == fileLength) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Getting length of file %s failed: "
                                            "lastErrorNumber=%d, "
                                            "lastErrorMessage=%s\n",
                       filename, lastErrorNumber, lastErrorMessage);
    goto exit;
  ...
  if (-1 == fd) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Open of file %s for mapping "
  ...
  if ((NULL == mmapHandle) || (NULL == mmapHandle->pointer)) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Mmap_map_file of file %s failed: "
                                            "lastErrorNumber=%d, "
                                            "lastErrorMessage=%s\n",
                       filename, lastErrorNumber, lastErrorMessage);
    goto exit;
  ...
  if (-1 == rc) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Close of file %s after mapping "
                                            "failed: lastErrorNumber=%d, "
                                            "lastErrorMessage=%s\n",
                       filename, lastErrorNumber, lastErrorMessage);
  }
...
#define J9MMAP_TEST2_BUFFER_SIZE 100
  ...
  if (!(mmapCapabilities & OMRPORT_MMAP_CAPABILITY_WRITE)) {
    outputComment(OMRPORTLIB, "OMRPORT_MMAP_CAPABILITY_WRITE not supported on "
  ...
  if (-1 == rc) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Read of file %s after update "
                                            "failed: lastErrorNumber=%d, "
                                            "lastErrorMessage=%s\n",
                       filename, lastErrorNumber, lastErrorMessage);
    goto exit;
  ...
  if (-1 == rc) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Close of file %s after read "
                                            "failed: lastErrorNumber=%d, "
                                            "lastErrorMessage=%s\n",
                       filename, lastErrorNumber, lastErrorMessage);
  }
...
#define J9MMAP_TEST3_BUFFER_SIZE 100
  ...
  if (!(mmapCapabilities & OMRPORT_MMAP_CAPABILITY_COPYONWRITE)) {
    outputComment(OMRPORTLIB, "OMRPORT_MMAP_CAPABILITY_COPYONWRITE not "
                              "supported on this platform, bypassing test\n");
    goto exit;
  }
...
#define J9MMAP_TEST4_BUFFER_SIZE 100
  ...
  if (!(mmapCapabilities & OMRPORT_MMAP_CAPABILITY_WRITE) ||
    ...
    outputComment(OMRPORTLIB, "OMRPORT_MMAP_CAPABILITY_WRITE or _MSYNC not "
                              "supported on this platform, bypassing test\n");
    goto exit;
  }
...
* 	that @ref omrmmap::omrmmap_get_region_granularity
* "omrmmap_get_region_granularity()" does not return 0
*/
TEST_F(PortMmapTest, mmap_test6) {
  ...
  if (mmapCapabilities & OMRPORT_MMAP_CAPABILITY_PROTECT) {
    if (omrmmap_get_region_granularity(mapAddr) == 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "Mmap claims to be capable of "
                                              "PROTECT, however, region "
                                              "granularity is 0\n");
      goto exit;
    }
    rc =
    ...
    if ((rc & OMRPORT_PAGE_PROTECT_NOT_SUPPORTED) != 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Mmap is claims to be capable of PROTECT, however, "
                         "omrmmap_protect() returns "
    ...
    if ((rc & OMRPORT_PAGE_PROTECT_NOT_SUPPORTED) == 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Mmap is not capable of PROTECT, however, "
                         "omrmmap_protect() does not return "
                         "OMRPORT_PAGE_PROTECT_NOT_SUPPORTED\n");
      goto exit;
    }
  }
...
#define J9MMAP_TEST1_BUFFER_SIZE 100
  ...
  if ((mmapCapabilities & OMRPORT_MMAP_CAPABILITY_PROTECT) == 0) {
    outputComment(OMRPORTLIB, "OMRPORT_MMAP_CAPABILITY_PROTECT not supported "
                              "on this platform, bypassing test\n");
    goto exit;
  }

  if ((mmapCapabilities & OMRPORT_MMAP_CAPABILITY_READ) == 0) {
    outputComment(OMRPORTLIB, "OMRPORT_MMAP_CAPABILITY_READ not supported on "
                              "this platform, bypassing test\n");
    goto exit;
  }
...
TEST_F(PortMmapTest, mmap_test9) {
  ...
  if (!omrsig_can_protect(signalHandlerFlags)) {
    outputComment(OMRPORTLIB, "Signal handling framework not available, can't "
                              "test readonly functionality without crashing, "
                              "bypassing test");
    goto exit;
  } else {
 ...
 * first and
 * then write to the second page (should be ok) and the last byte of the first
 * page (should fail).
 */
TEST_F(PortMmapTest, mmap_test10) {
  ...
  outputComment(OMRPORTLIB,
                "Written out the buffer %d times to fill two pages\n",
                i / bufferSize);
...
TEST_F(PortMmapTest, mmap_test11) {
  ...
  outputComment(OMRPORTLIB,
                "Written out the buffer %d times to fill three pages\n",
                i / bufferSize);
...
#define J9MMAP_TEST1_BUFFER_SIZE 100
  ...
  if (finalBlocks <= initialBlocks) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Block count did not increase as "
                                            "expected. initialBlocks=%zu, "
                                            "finalBlocks=%zu\n",
                       initialBlocks, finalBlocks);
  }

  if (finalBytes < (initialBytes + fileLength)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Byte count did not increase as "
                                            "expected. initialBytes=%zu, "
                                            "finalBytes=%zu, fileLength=%lld\n",
  ...
  if (finalBlocks != initialBlocks) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Block count did not decrease as "
                                            "expected. initialBlocks=%zu, "
                                            "finalBlocks=%zu\n",
                       initialBlocks, finalBlocks);
  }
  ...
  if (finalBytes != initialBytes) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Byte count did not decrease as "
                                            "expected. initialBytes=%zu, "
                                            "finalBytes=%zu\n",
