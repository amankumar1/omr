...
OMRProcessHandle launchChildProcess(OMRPortLibrary *portLibrary,
  ...
  if (retVal != 0) {
    ...
    if (NULL == errMsg) {
      omrtty_printf("%s: launchChildProcess: omrsysinfo_get_executable_name "
                    "failed!\n\tportableErrno = %d\n",
  ...
  */
  ...
  if (0 != retVal) {
    ...
    const char *errMsg = omrerror_last_error_message();
    if (NULL == errMsg) {
      omrtty_printf("%s: launchChildProcess: Failed to start process '%s "
                    "%s'\n\tportableErrno = %d\n",
...
intptr_t waitForTestProcess(OMRPortLibrary *portLibrary,
  ...
  if (0 != j9process_waitfor(OMRPORTLIB, processHandle)) {
    ...
    const char *errMsg = omrerror_last_error_message();
    if (NULL == errMsg) {
      omrtty_printf("waitForTestProcess: j9process_waitfor() "
                    "failed\n\tportableErrno = %d\n",
                    portableErrno);
    } else {
      omrtty_printf("waitForTestProcess: j9process_waitfor() "
  ...
  if (0 != j9process_close(OMRPORTLIB, &processHandle, 0)) {
    ...
    const char *errMsg = omrerror_last_error_message();
    if (NULL == errMsg) {
      omrtty_printf("waitForTestProcess: j9process_close() "
                    "failed\n\tportableErrno = %d\n",
                    portableErrno);
    } else {
      omrtty_printf("waitForTestProcess: j9process_close() "
...
static intptr_t getUnicodeCmdline(struct OMRPortLibrary *portLibrary,
   ...
   * Note (see "XXX references in the code)
           * Our CDev scanner/parser does not handle '"' correctly. A workaround
   * is to close
...
intptr_t j9process_create(OMRPortLibrary *portLibrary, const char *command[],
   ...
   * execv[p|e]()
   * We could do this in the child, but if mem_allocate_memory fails, it's
   * easier to diagnose
  ...
  if (grdpid == 0) {
     ...
     * successfully,
     * a single block read/write pipe call is used, if process creation failed,
     * errorNumber
     * with sizeof(errno) will be returned, otherwise, read will fail due to
     * pipe closure.
     */
    if (-1 != rc) {
