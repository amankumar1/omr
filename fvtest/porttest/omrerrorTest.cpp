 ...
 * are recorded in the per thread
 * buffers without an error message.
 * Verify the @ref omrerror_last_error_number "omrerror_last_error_number()"
 * returns
 ...
 * Verify the @ref omrerror_last_error_message "omrerror_last_error_message()"
 * returns
 * the correct message.
 */
TEST(PortErrorTest, error_test2) {
  ...
  if (0 != strlen(message)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrerror_last_error_message() "
                                            "returned message of length %d "
                                            "expected %d\n",
                       strlen(message), 1);
  }
