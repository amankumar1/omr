 ...
 * exported
 * by the port library and are not in the actual port library table, however
 * there
 * are a few.
 *
 ...
 * provide
 * a size for the port library to create.  The size of any port library version
 * can
 ...
 * with
 * default values prior to starting the port library allows applications to
 * override
 * functionality.  For example an application may with to control all memory
 * management,
...
TEST(PortTest, port_test2) {
  ...
  if (eyeCatcher != bigBuffer[0]) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrport_create_library() trampled "
                                            "memory expected \"%c\" found "
                                            "\"%c\"\n",
...
TEST(PortTest, port_test5) {
   ...
   * port library is
   * not updated as part of omrport_startup_library(), so we are now pointing at
   * dead memory.
   *
