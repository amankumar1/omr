...
TEST(PortMemTest, mem_test1) {
   ...
   * This ensures that one can free any pointer returned from
 ...
 * library
 * table.  Need to verify that these functions have been correctly implemented.
 * These
 * functios are called by the port library management routines in @ref j9port.c
 * "j9port.c".
 * The API for these functions clearly state they must be implemented.
 *
 * Verify @ref omrmem.c::omrmem_allocate_portLibrary
 * "omrmem_allocate_portLibrary()" allocates
 * memory correctly.
 *
 * @note This function does not need to be implemented in terms of
 ...
 * "omrmem_allocate_memory()"
 * returns a memory location that is below the 4 gig boundary, and as well that
 * the entire region
 ...
 * omrheap suallocation (because they cannot be satisfied by
 *    suballocating a omrheap due to heap management overhead), but rather given
...
/* Use a more exhausive size array for 64 bit platforms to exercise the omrheap
 * API.
 * Retain the old sizes for regular 32 bit VMs, including ME platforms with very
...
static OMRMemCategory dummyCategoryOne = {
    "Dummy One", DUMMY_CATEGORY_ONE, 0, 0, 0, 0, 4, childrenOfDummyCategoryOne};

static OMRMemCategory dummyCategoryTwo = {
    "Dummy Two", DUMMY_CATEGORY_TWO, 0, 0, 0, 0, 0, NULL};

static OMRMemCategory dummyCategoryThree = {
    "Dummy Three", DUMMY_CATEGORY_THREE, 0, 0, 0, 0, 0, NULL};
...
static uintptr_t categoryWalkFunction(uint32_t categoryCode,
  ...
  case DUMMY_CATEGORY_THREE:
    ...
    if (DUMMY_CATEGORY_ONE != parentCategoryCode) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "DUMMY_CATEGORY_THREE's parent "
                                              "should be DUMMY_CATEGORY_ONE, "
                                              "not %u\n",
                         parentCategoryCode);
      state->otherError = TRUE;
      return J9MEM_CATEGORIES_STOP_ITERATING;
...
TEST(PortMemTest, mem_test8_categories) {
  ...
  if (categoriesState.otherError) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Some other error hit while "
                                            "walking categories (see messages "
                                            "above).\n");
    goto end;
  }
   ...
   * counters are incremented */
  {
    ...
    if (finalBlocks != expectedBlocks) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "Unexpected number of blocks "
                                              "after allocate. Expected %zd, "
                                              "got %zd.\n",
...
/* n.b. we're reliant on previous tests having initialized the 32bithelpers. */
#if defined(OMR_ENV_DATA64)
  {
    ...
    if (finalUnused32BitSlabBlocks != initialUnused32BitSlabBlocks) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS, "Number of unused 32bit slab blocks should have "
    ...
    if (finalUnused32BitSlabBlocks != initialUnused32BitSlabBlocks) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS, "Unexpected number of unused 32 bit slab blocks "
                               "after free. Expected %zu, now %zu\n",
          initialUnused32BitSlabBlocks, finalUnused32BitSlabBlocks);
      goto end;
    }
    if (finalUnused32BitSlabBytes != initialUnused32BitSlabBytes) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS, "Unexpected number of unused 32 bit slab bytes "
                               "after free. Expected %zu, now %zu\n",
