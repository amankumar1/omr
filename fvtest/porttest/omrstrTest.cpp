...
static void validate_omrstr_vprintf(struct OMRPortLibrary *portLibrary,
  ...
  if (strlen(buffer) != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrstr_vprintf(\"%s\") with "
                                            "\"%s\" returned buffer with "
                                            "length %d, expected %d\n",
 ...
 * "omrstr_vprintf()".
 * Tests include basic printing of characters, strings, numbers and Unicode
 * characters.
 */
...
TEST(PortStrTest, str_test3) {
  ...
  omrtty_printf("\t This test could fail if you abut the international "
  ...
  test_omrstr_ftime(OMRPORTLIB, testName, buf, J9STR_BUFFER_SIZE,
                    "%Y %m %b %d XX:%M:%S", timeMillis, expected);
 ...
 * various tokens.
 * Tests include simple token tests, too short a dest buffer and token
 * precedence
 */
...
#define TEST_OVERFLOW_LEN 8
  ...
  if (NULL == tokens) {
     ...
     * that the user may be relying on) */
    {
       ...
       * that the time is substituted properly .
       * Do this by making sure that %<token> is not present in the output
       * buffer*/
      if (rc > TEST_BUF_LEN) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
 ...
 * omrtime_current_time_millis() followed
...
#define TEST_OVERFLOW_LEN 8
  ...
  tokens = omrstr_create_tokens(timeMillis);
  (void)omrstr_subst_tokens(buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens);
  omrtty_printf("\tExpecting local time (in Ottawa, Eastern Standard Time): "
                "2006/02/14 16:30:06 \n",
                timeMillis);
  omrtty_printf(
      "\t                                       ... converted to: %s\n", buf);
  ...
  tokens = omrstr_create_tokens(timeMillis);
  (void)omrstr_subst_tokens(buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens);
  omrtty_printf("\tExpecting local time (in Ottawa, Eastern Daylight Time): "
                "2006/06/14 17:30:06 \n",
                timeMillis);
  omrtty_printf(
      "\t                                       ... converted to: %s\n", buf);
...
static int printToBuffer(struct OMRPortLibrary *portLibrary, BOOLEAN precision,
  ...
  if (stringLength >= 0) {
    if (precision) {
      ...
      omrtty_printf("\n\tFinish Testing without precision specifier: String in "
                    "buffer is: \"%s\", length = %d\n",
                    buffer, stringLength);
    }
 ...
 * fstring().
 * It actually calls invoke atoe_vsnprintf() to run the tests through
 * sub-functions.
 */
