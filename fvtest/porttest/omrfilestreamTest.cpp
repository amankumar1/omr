 ...
 * the number of bytes
 * written to the buffer on success (0 bytes is success), or negative error code
 * on failure.
 */
 ...
 * and modes are working
 * correctly.  This will create the file, open and close the file twice, and
 ...
 * omrfilestream::omrfilestream_write "omrfilestream_write()"
 * append operations.
 *
...
TEST(PortFileStreamTest, omrfilestream_test_file_owner_and_group_attributes) {
  ...
  if (NULL == file) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrfilestream_open failed to "
                                            "create file %s: "
 ...
 * omrfilestream_open() will not open a filestream for reading.  This test makes
 * sure that
...
TEST(PortFileStreamTest, omrfilestream_test_fdopen_and_fileno) {
  ...
  if (NULL == file) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrfilestream_fdopen failed to "
                                            "create file %s: "
                                            "error_last_error_number %i\n",
                       filename, omrerror_last_error_number());
...
/*
 * Get a filestream from the file descriptor, write to the filestream, read from
 * the file descriptor
 */
test3:
  /* open a file descriptor */
  ...
  if (NULL == file) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrfilestream_fd open failed to "
                                            "open fd for: %s: "
                                            "error_last_error_number %i\n",
                       filename, omrerror_last_error_number());
    omrfile_close(fileDescriptor);
 ...
 * buffering methods. With
 * a different stream to the same file, it will ensure that the proper file
 * buffering works
 * correctly.
 *
 * @param[in] portLibrary The port library under test
...
TEST(PortFileStreamTest, omrfilestream_test_buffering) {
   ...
   * Reading from the
   * stream should return nothing. After flushing writes to the file, read the
   * text.
   */
 ...
 * entire buffer,
 * make sure the strings are the same.  Note that there is no guarantee that
 * line buffering
 * will actually line buffer.  The only guarantee is that all text has been
 * written to the file
...
/*
 * Attempt to use no buffering.  Ensure that flushing wont write more characters
 * to the file.
 */
test4:
  writeFile = omrfilestream_open(
 ...
 * enough to avoid all
 * file writes using full buffering. Reading from the stream should return
 * nothing. Flush the
 ...
 * when the toCode is J9STR_CODE_PLATFORM_RAW.  The test then reads in the the
 * files as binary and
 ...
 * encoding.
 * There is no way to read back the characters and verify that they are correct,
 * as not all
...
TEST(PortFileStreamTest, omrfilestream_test_invalid_filestream_access) {
  ...
  if (NULL != noAccessStream) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrfilestream_open() returned "
                                            "non-null, expected invalid file "
                                            "handle\n");
...
writeTextInvalidFDEnd:
  ...
  if (rc != -1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrfilestream_fileno() returned "
                                            "%d, expected invalid file "
                                            "descriptor -1\n",
                       rc);
  }
 ...
 * streams are stdin,
 * stdout, and stderr.  Make sure that they will not error when opened and
 * written to.
 *
 * @param[in] portLibrary The port library under test
 *
...
TEST(PortFileStreamTest, omrfilestream_test_std_streams) {
   ...
   * as that will close
   * the underlying file descriptors.  In the future the smart thing to do would
   * be to DUP the
