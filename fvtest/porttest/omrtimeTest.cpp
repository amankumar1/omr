...
TEST(PortTimeTest, time_test1) {
  ...
  if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
    ...
    if (timeCheck == timeStart) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrtime_current_time_millis did "
                                              "not change after a half-second "
                                              "forced delay\n");
    }
    if (mtimeStart == mtimeCheck) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrtime_msec_clock did not "
                                              "change after a half-second "
                                              "forced delay\n");
    }
    if (utimeStart == utimeCheck) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrtime_usec_clock did not "
                                              "change after a half-second "
                                              "forced delay\n");
    }
    if (ntimeStart == ntimeCheck) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrtime_nano_time did not "
                                              "change after a half-second "
                                              "forced delay\n");
    }
    ...
    if (success) {
      if (nClocktimeStart == nClocktimeCheck) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "omrtime_current_time_nanos "
                                                "did not change after a "
    ...
    if (0 != omrtime_hires_frequency()) {
      ...
      if (hiresTimeCheck == hiresTimeStart) {
        ...
        if (hiresTimeCheck == hiresTimeStart) {
          outputErrorMessage(PORTTEST_ERROR_ARGS, "omrtime_hires_clock has not "
                                                  "advanced after a "
                                                  "half-second forced delay\n");
        }
      } else {
    ...
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrthread_attach failed in "
                                            "omrtime_test1 so testing is not "
                                            "possible\n");
  }
...
TEST(PortTimeTest, time_test3) {
  ...
  if (1 == omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE)) {
    ...
    outputComment(OMRPORTLIB, "%10s %10s %10s %10s %12s %12s\n", "millires",
                  "millis", "msec", "usec", "hires msec ", "hires usec");
    for (i = 0; i < J9TIME_REPEAT_TEST; i++) {
      ...
      outputComment(OMRPORTLIB, "%10d %10d %10d %10d %12d %12d %12d\n",
                    ...
                    (int32_t)utimeDelta, (int32_t)ntimeDelta,
     ...
     * time,
     * there is no reason why the time values need to be monotonically
     * increasing */
    outputComment(OMRPORTLIB, "Test is invalid since the host machine reports "
                              "more than one CPU (time may differ across CPUs "
                              "- makes test results useless - re-enable if we "
...
/* Disabled because
                 ...
                 * - the output is confusing and does not help to diagnose
 * errors
 ...
 * omrtime_hires_clock implementation
                 */
TEST(DISABLED_PortTimeTest, time_test4) {
  ...
  for (i = 0; i < NUMBER_OF_ITERATIONS; i++) {
    ...
    if (0 != omrthread_sleep(SLEEP_TIME)) {
      outputComment(OMRPORTLIB, "WARNING: Skipping check due to "
    ...
    if (delta > EPSILON) {
      ...
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrtime_current_time_millis() "
...
TEST(PortTimeTest, time_nano_time_direction) {
   ...
   */
  {
    ...
    if (QueryPerformanceCounter(&i)) {
      outputComment(OMRPORTLIB, "WARNING: Test is invalid since the host "
                                "machine uses QueryPerformanceCounter() (time "
                                "may differ across CPUs - makes test results "
                                "useless - re-enable if we develop thread "
                                "affinity support)\n");
      reportTestExit(OMRPORTLIB, testName);
    }
  }
...
static int J9THREAD_PROC nanoTimeDirectionTest(void *arg) {
  ...
  for (i = 0; i < numLoops; i++) {
    ...
    if (finish <= start) {
      omrtty_printf("\tTime did not go forward after omrthread_sleep, "
 ...
 * resolution by
 * effectively multiplying the interval by (requiredRes /
 * omrtime_hires_frequency()).
 *
...
TEST(PortTimeTest, time_test_hires_delta_rounding) {
   ...
   * which
   * has a big fractional part that would be lost if naive integer math were
   * used.
   *
   * If we did lose the fractional part, then we would get:
   * delta = ticksPerSec / (ticksPerSec / requiredRes) = ticksPerSec / 1 =
   * ticksPerSec
   * error = abs(ticksPerSec - requiredRes) / requiredRes = 0.9 ... which fails
   * the test.
   */
  ...
  outputComment(OMRPORTLIB,
                "Case 1. expected: %llu    actual: %llu    error: %lf\n",
  ...
  outputComment(OMRPORTLIB,
                "Case 2. expected: %llu    actual: %llu    error: %lf\n",
                requiredRes, delta, error);
  ...
  outputComment(OMRPORTLIB,
                "Case 3. expected: %llu    actual: %llu    error: %lf\n",
                requiredRes, delta, error);
