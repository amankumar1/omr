...
void GCConfigTest::TearDown() {
  ...
  ASSERT_EQ(OMR_ERROR_NONE, rc)
      << "TearDown(): OMR_GC_ShutdownCollector failed, rc=" << rc;

  /* Detach from VM */
  rc = OMR_Thread_Free(exampleVM->_omrVMThread);
  ASSERT_EQ(OMR_ERROR_NONE, rc) << "TearDown(): OMR_Thread_Free failed, rc="
                                << rc;

  /* Shut down heap */
  rc = OMR_GC_ShutdownHeap(exampleVM->_omrVM);
  ASSERT_EQ(OMR_ERROR_NONE, rc) << "TearDown(): OMR_GC_ShutdownHeap failed, rc="
                                << rc;
...
OMRGCObjectType GCConfigTest::parseObjectType(pugi::xml_node node) {
  ...
  if (0 == strcmp(typeStr, "root")) {
    if (0 == strcmp(parentStr, "allocation")) {
      ...
      gcTestEnv->log(LEVEL_ERROR, "%s:%d Invalid XML input: root object %s can "
                                  "not be child of other object!\n",
    ...
    if ((0 == strcmp(parentTypeStr, "root")) ||
      ...
      gcTestEnv->log(LEVEL_ERROR, "%s:%d Invalid XML input: normal object %s "
                                  "has to be child of root or normal object!\n",
                     __FILE__, __LINE__, namePrefixStr);
    }
    ...
    if (0 == strcmp(parentStr, "allocation")) {
      ...
      gcTestEnv->log(LEVEL_ERROR, "%s:%d Invalid XML input: garbage object %s "
                                  "has to be child of root, normal or "
                                  "allocation node!\n",
                     __FILE__, __LINE__, namePrefixStr);
    }
  } else {
    gcTestEnv->log(LEVEL_ERROR, "%s:%d Invalid XML input: object %s type "
                                "should be root, normal or garbage .\n",
                   __FILE__, __LINE__, namePrefixStr);
  }
...
ObjectEntry *GCConfigTest::allocateHelper(const char *objName, uintptr_t size) {
  ...
  objEntry.objPtr = OMR_GC_AllocateNoGC(
      exampleVM->_omrVMThread, OMR_EXAMPLE_ALLOCATION_CATEGORY, size, 0);

  if (NULL == objEntry.objPtr) {
    gcTestEnv->log("No free memory to allocate %s of size 0x%llx, GC start.\n",
                   objName, size);
    objEntry.objPtr = OMR_GC_Allocate(exampleVM->_omrVMThread,
                                      OMR_EXAMPLE_ALLOCATION_CATEGORY, size, 0);
  }
  ...
  if (NULL != objEntry.objPtr) {
    ...
    gcTestEnv->log(LEVEL_ERROR, "%s:%d No free memory after a GC. Failed to "
                                "allocate object %s of size 0x%llx.\n",
...
int32_t GCConfigTest::insertGarbage() {
  ...
  gcTestEnv->log(LEVEL_VERBOSE, "Inserting garbage %s of 0x%llx (%f%% of "
                                "previous object/tree size 0x%llx) ...\n",
...
int32_t GCConfigTest::attachChildEntry(ObjectEntry *parentEntry,
  ...
  if ((uint32_t)parentEntry->numOfRef < slotCount) {
    ...
    cli->writeBarrierStore(exampleVM->_omrVMThread, parentEntry->objPtr,
                           childSlot, childEntry->objPtr);
    gcTestEnv->log(
        LEVEL_VERBOSE,
        "\tadd child %s(%p[0x%llx]) to parent %s(%p[0x%llx]) slot %p[%llx].\n",
        childEntry->name, childEntry->objPtr, *(childEntry->objPtr),
        parentEntry->name, parentEntry->objPtr, *(parentEntry->objPtr),
        childSlot, (uintptr_t)*childSlot);
    parentEntry->numOfRef += 1;
  } else {
    gcTestEnv->log(LEVEL_ERROR, "%s:%d Invalid XML input: numOfFields %d "
                                "defined for %s(%p[0x%llx]) is not enough to "
                                "hold child reference for %s(%p[0x%llx]).\n",
                   __FILE__, __LINE__, parentEntry->numOfRef, parentEntry->name,
                   parentEntry->objPtr, *(parentEntry->objPtr),
                   childEntry->name, childEntry->objPtr, *(childEntry->objPtr));
    rc = 1;
  }
  return rc;
...
int32_t GCConfigTest::removeObjectFromRootTable(const char *name) {
  ...
  gcTestEnv->log(LEVEL_VERBOSE,
                 ...
                 rootEntry->rootPtr, *(rootEntry->rootPtr));
...
int32_t GCConfigTest::removeObjectFromObjectTable(const char *name) {
  ...
  gcTestEnv->log(LEVEL_VERBOSE,
                 ...
                 foundEntry->objPtr, *(foundEntry->objPtr));
  ...
  while (currentSlot < endSlot) {
    ...
    if (objEntry->objPtr == slotObject.readReferenceFromSlot()) {
      gcTestEnv->log(
          ...
          name, objEntry->objPtr, *(objEntry->objPtr), parentEntry->name,
          parentEntry->objPtr, *(parentEntry->objPtr),
...
int32_t GCConfigTest::allocationWalker(pugi::xml_node node) {
  ...
  if ((0 == strcmp(namePrefixStr, "")) || (0 == strcmp(typeStr, ""))) {
    rt = 1;
    gcTestEnv->log(LEVEL_ERROR, "%s:%d Invalid XML input: please specify "
                                "namePrefix and type for object %s.\n",
                   __FILE__, __LINE__, namePrefixStr);
    goto done;
  }
...
int32_t GCConfigTest::verifyVerboseGC(pugi::xpath_node_set verboseGCs) {
  ...
  do {
    ...
    for (pugi::xpath_node_set::const_iterator it = verboseGCs.begin();
      ...
      if (!resultQuery) {
        rt = 1;
        gcTestEnv->log(LEVEL_ERROR, "%s:%d Invalid xquery string \"%s\" "
      ...
      if (!nodeQuery) {
        rt = 1;
        gcTestEnv->log(LEVEL_ERROR, "%s:%d Invalid xpathNodes string \"%s\" "
                                    "specified in configuration file.\n",
...
int32_t GCConfigTest::parseGarbagePolicy(pugi::xml_node node) {
  ...
  if (!node.empty()) {
    ...
    if ((0 != strcmp(gp.frequency, "perObject")) &&
      ...
      rt = 1;
      gcTestEnv->log(LEVEL_ERROR, "%s:%d Invalid XML input: the valid value of "
                                  "attribute \"frequency\" is \"perObject\" or "
                                  "\"perRootStruct\".\n",
                     __FILE__, __LINE__);
      goto done;
    }
    ...
    if ((0 != strcmp(gp.structure, "node")) &&
      ...
      rt = 1;
      gcTestEnv->log(LEVEL_ERROR, "%s:%d Invalid XML input: the valid value of "
                                  "attribute \"structure\" is \"node\" or "
                                  "\"tree\".\n",
                     __FILE__, __LINE__);
      goto done;
    }
  ...
  for (; node; node = node.next_sibling()) {
    if (0 == strcmp(node.name(), "systemCollect")) {
      ...
      if (OMR_ERROR_NONE != rt) {
        gcTestEnv->log(LEVEL_ERROR, "%s:%d Failed to perform "
                                    "OMR_GC_SystemCollect with error code "
                                    "%d.\n",
