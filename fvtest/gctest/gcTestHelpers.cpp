...
void GCTestEnvironment::initParams() {
  ...
  for (int i = 1; i < _argc; i++) {
    if (strncmp(_argv[i], "-configListFile=", strlen("-configListFile=")) ==
        0) {
      ...
      while (readLine == omrfile_read_text(fileDescriptor, readLine, 2048)) {
        ...
        if (NULL != newLine) {
          ...
          *newLine = '\0';
          ...
          FAIL() << "The file provided in option -configListFile= must contain "
                    "a list of configuration files with extension "
                 << extension;
        }
  ...
  if (params.empty()) {
    FAIL() << "Please specify a file containing a list of configuration files "
              "using option -configListFile=";
  }
}
...
void GCTestEnvironment::GCTestSetUp() {
  ...
  ASSERT_EQ(OMR_ERROR_NONE, rc) << "GCTestSetUp(): OMR_Initialize failed, rc="
                                << rc;
