...
static intptr_t testInsertionComparator(J9AVLTree *tree,
                                        J9AVLTestNode *insertNode,
                                        J9AVLTestNode *walkNode);
static uintptr_t get_node_string(OMRPortLibrary *portlib, J9AVLTree *tree,
                                 J9AVLTestNode *walk, char *buffer,
                                 uintptr_t bufSize);
static void avlReset(J9AVLTree *tree);
static uintptr_t get_datum_string(OMRPortLibrary *portlib, J9AVLTestNode *walk,
...
int32_t verifyAVLTree(OMRPortLibrary *portLib, char *testListFile,
  ...
  line = 0;
  while (NULL !=
         (result = omrfile_read_text(handle, buf, AVL_TEST_MAX_LINELEN))) {
    line++;
    ...
    for (;;) {
      ...
      if (action == 0) {
        ...
        if ((!strncmp(buffer, success, successEnd - success)) &&
            (buffer[successEnd - success] == 0)) {
...
static void avl_get_string(OMRPortLibrary *portlib, J9AVLTree *tree,
  ...
  uintptr_t len = get_node_string(
      portlib, tree, (J9AVLTestNode *)tree->rootNode, buffer, bufSize);
  if (len >= bufSize) {
    buffer[bufSize - 1] = 0;
  } else {
    buffer[len] = 0;
...
static uintptr_t get_node_string(OMRPortLibrary *portlib, J9AVLTree *tree,
  ...
  len += get_node_string(portlib, tree,
                         (J9AVLTestNode *)AVL_SRP_GETNODE(walk->leftChild),
                         &buffer[len], bufSize - len);
  len += get_node_string(portlib, tree,
                         (J9AVLTestNode *)AVL_SRP_GETNODE(walk->rightChild),
                         &buffer[len], bufSize - len);
