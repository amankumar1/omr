 ...
 * reset the OMR_VM,
 * including cleaning up the _vmThreadList of threads that do not exist after a
 * fork.
 *
 * @param[in] vm The OMR vm.
 */
 ...
 * initialization
 * that requires the new thread to be currently executing, such as setting TLS
 * values.
 *
 ...
 * For z/OS, no token is necessary because the VM directory is found by
 * searching
 * the LIBPATH.
 *
 ...
 * method
 * inserted into the method dictionary. See
 ...
 * identifying
 * the method properties common to all methods to be inserted into the method
 * dictionary
 * for profiling, such as method name, file name, line number, class name, or
 * other properties.
 *
