 ...
 * translate this OS specific error code to a
 * portable error code.  All portable error codes are negative numbers.  Not
 * every module of the port library will have
 * error codes dedicated to it's own use, some will use the generic portable
 * error code values.
 *
 * Errors reported by the OS may be recorded by calling the port library
 * functions @ref omrerror.c::omrerror_set_last_error
 * "omrerror_set_last_error()"
 * or @ref omrerror.c::omrerror_set_last_error_with_message
 * "omrerror_set_last_error_with_message()".  The mapping of the OS specific
 * error
 ...
 * Where possible it is recommended that all
