...
typedef enum J9CudaError {
   ...
   * (J9CUDA_DEVICE_ATTRIBUTE_KERNEL_EXEC_TIMEOUT).
   * The device cannot be used until omrcuda_deviceReset() is called. All
   * existing
   ...
   * require
   * the runtime to reserve large amounts of device memory that cannot be used
   * for
   * user allocations.
   */
   ...
   * to the device runtime. Keep in mind that raising the limit of pending
   * device
...
typedef struct J9CudaJitOptions {
   ...
   * be imposed. If set to a non-zero value, this field will be updated to
   * reflect
