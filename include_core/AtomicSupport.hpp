 ...
 * complete before any instructions following
 ...
 * prefetched instructions, causing subsequent
 * instructions to be fetched (or refetched) and executed in the context
 * established by previous instructions.
 */
 ...
 * before any new instructions can be executed
 * on the processor that executed the function. This allows you to synchronize
 * between multiple processors with minimal
 * performance impact, as __lwsync does not wait for confirmation from each
 * processor.
 */
 ...
 * swap.
 * Benchmarking has proven this to be a negative on Power and no benchmarking
 * has been
 * performed on ARM.
 */
...
public:
   ...
   * load caused
   * by an instruction following the storeSync may be performed before a store
   * caused by
   * an instruction that precedes the storeSync, and the ordering does not apply
   * to accesses
...
*/
   ...
   * executed)
   * instructions and refetch the next following instructions. It is used to
   * ensure that
   * no loads following entry into a critical section can access data (because
   * of aggressive
...
*/
   ...
   * <b>address</b>.  If it is
   * equal to <b>oldValue</b> then update this memory location with
   * <b>newValue</b>
 ...
 */
    ...
    return J9CAS8Helper(address, ((uint32_t *)&oldValue)[1],
                        ...
                        ((uint32_t *)&newValue)[0]);
#elif defined(                                                                 \
    __GNUC__)           /* defined(OMR_ARCH_POWER) && !defined(OMR_ENV_DATA64) */
    /* Assume GCC >= 4.2 */
    return __sync_val_compare_and_swap(address, oldValue, newValue);
#elif defined(_MSC_VER) /* defined(__GNUC__) */
...
#error "lockCompareExchangeU64(): unsupported platform!"
  ...
  VMINLINE static double addDouble(volatile double *address, double addend) {
    ...
    while (lockCompareExchangeU64(localAddr, *(uint64_t *)&oldValue,
                                  *(uint64_t *)&newValue) !=
           *(uint64_t *)&oldValue) {
      oldValue = *address;
  ...
  /**
   * Subtracts a number from the value at a specific memory location asn an
   * atomic operation.
   * Subtracts the value <b>value</b> from the value stored at memory location
   * pointed
   * to by <b>address</b>.
   *
   * @param address The memory location to be updated
  ...
  /**
   * Return true approximately one time in frequency, which must be a power of
   * 2.
