...
MM_ConfigurationLanguageInterfaceImpl::newInstance(MM_EnvironmentBase *env) {
  ...
  configurationLanguageInterface =
      (MM_ConfigurationLanguageInterfaceImpl *)env->getForge()->allocate(
          sizeof(MM_ConfigurationLanguageInterfaceImpl),
          MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
  if (NULL != configurationLanguageInterface) {
    new (configurationLanguageInterface)
        MM_ConfigurationLanguageInterfaceImpl();
 ...
 */
bool MM_ConfigurationLanguageInterfaceImpl::initialize(
 ...
 */
bool MM_ConfigurationLanguageInterfaceImpl::initializeEnvironment(
    MM_EnvironmentBase *env) {
  return true;
}
...
MM_ConfigurationLanguageInterfaceImpl::createObjectAllocationInterface(
    MM_EnvironmentBase *env) {
  return MM_TLHAllocationInterface::newInstance(env);
}

MM_EnvironmentLanguageInterface *
MM_ConfigurationLanguageInterfaceImpl::createEnvironmentLanguageInterface(
    MM_EnvironmentBase *env) {
  return MM_EnvironmentLanguageInterfaceImpl::newInstance(env);
}

bool MM_ConfigurationLanguageInterfaceImpl::initializeArrayletLeafSize(
    MM_EnvironmentBase *env) {
  return true;
}
...
MM_GlobalCollector *
MM_ConfigurationLanguageInterfaceImpl::createGlobalCollector(
    MM_EnvironmentBase *env) {
  return MM_ParallelGlobalGC::newInstance(
      env, MM_GCExtensionsBase::getExtensions(env->getOmrVM())
               ->collectorLanguageInterface);
}

void MM_ConfigurationLanguageInterfaceImpl::initializeSaltData(
    MM_EnvironmentBase *env, uintptr_t index, uint32_t startValue) {}

uintptr_t MM_ConfigurationLanguageInterfaceImpl::internalGetWriteBarrierType(
    MM_EnvironmentBase *env) {
  return 0;
}

uintptr_t MM_ConfigurationLanguageInterfaceImpl::internalGetAllocationType(
    MM_EnvironmentBase *env) {
  return 0;
}
