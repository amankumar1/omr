...
public:
  ...
  /**
   * This method must be implemented to initialize the object header for a new
   * allocation
   * of heap memory. The MM_AllocateInitialization instance provided allows
   * access to the
   * MM_AllocateDescription instance used to allocate the heap memory and
   * language-specific
...
#error GC_ObjectModel::initializeAllocation() must be implemented.
  ...
  MMINLINE bool isSingleSlotDeadObject(omrobjectptr_t objectPtr) {
    return J9_GC_SINGLE_SLOT_HOLE ==
           (*((uintptr_t *)objectPtr) & J9_GC_OBJ_HEAP_HOLE_MASK);
  }
...
#error provide an implementation of how big objects are including any header data associated with each object
  ...
  /**
   * Before objects are moved during compaction is there any language specific
   * updates
  ...
  /**
   * After objects are moved during compaction is there any language specific
   * updates
   * @param vmThread - the thread performing the work
   * @param objectPtr Pointer to an object
   */
   ...
   */
  MMINLINE bool isRemembered(omrobjectptr_t objectPtr) { return false; }
