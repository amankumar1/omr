...
static void sa_flags_to_sv_flags(const long unsigned int *sa_flags,
                                 int *sv_flags);
static void sv_flags_to_sa_flags(const int *sv_flags,
 ...
 * if it had been evoked by the OS
...
int omrsig_handler(int sig, void *siginfo, void *uc) {
  ...
  if (validSignalNum(sig, false)) {
    ...
    if (handlerIsFunction(&handlerSlot.secondaryAction)) {
       ...
       * unless it is included in sa_mask. Otherwise, sig will always be added
       * to the process' signal mask on
       ...
       * SIG_DFL and the SA_SIGINFO flag
       * will be cleared on entry to the signal handler. Does not work for
       * SIGILL or SIGTRAP, silently. Also
      ...
      if (0 == ec) {
         ...
         * this case, sa_sigaction should be
         * used instead of sa_handler.
         * NOTE FOR NOW: null, DFL, and IGN check the sa_handler when
         * registering it, not here
         */
        if (flags & SA_SIGINFO) {
...
int omrsig_signalOS_internal(int signum, const struct sigaction *act,
  ...
  if (NULL == signalOS) {
    signalOS =
        (SIGNAL)GetProcAddress(GetModuleHandle(TEXT("MSVCR100.dll")), "signal");
  }
  if (NULL == signalOS) {
    rc = -1;
  } else {
 ...
 * to reinstall the secondary handler before entering the primary handler
 * function.
 */
#if defined(POSIX_SIGNAL)
      if (primary) {
       ...
       * pthread_sigmask() or
       * sigprocmask() within a signal handler will cause the program to sig
       * kill itself.
       */
...
#if defined(AIXPPC)
  /* On AIX, sigvec does not respect SV_RESETHAND and always applies
   * SV_INTERRUPT. */
...
#endif /* defined(OSX) */
{
  ...
  if ((NULL == vec) && (NULL == ovec)) {
    ...
    /* If a previous argument is given, convert the struct sigaction to struct
     * sigvec. */
    if (NULL != ovec) {
