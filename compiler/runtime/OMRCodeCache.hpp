...
// Base class capturing VM dependency on the _warmCodeAlloc and _coldCodeAlloc
// pointers at the
// beginning of a code cache object. Do not change this base class without fully
// appreciating this
// dependency.
//
class CodeCacheBase {
...
public:
  ...
  void adjustTrampolineReservation(TR_OpaqueMethodBlock *method, void *cp,
                                   int32_t cpIndex);
  void resolveHashEntry(CodeCacheHashEntry *entry,
...
public:
  ...
  // monitors we may hold (not at the same time!) when we write to this.
  // We can either be holding the code cache monitor *OR* the manager's code
  // cache list monitor.
