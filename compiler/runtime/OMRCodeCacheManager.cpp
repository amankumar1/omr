...
#define REACHEABLE_RANGE_KB (2048 * 1024)
  ...
  config._needsMethodTrampolines =
      !(config.trampolineCodeSize() == 0 || config.maxNumberOfCodeCaches() == 1
        ...
        || (!TR::Options::getCmdLineOptions()->getOption(
                TR_EnableMethodTrampolineReservation) &&
  ...
  for (int32_t i = 0; i < cachesCreatedOnInit; i++) {
    ...
    //
    codeCache =
        TR::CodeCache::allocate(self(), config.codeCacheKB() << 10, -2); // MCT
  }
...
void OMR::CodeCacheManager::lateInitialization() {
  TR::CodeCacheConfig &config = self()->codeCacheConfig();
  ...
  for (TR::CodeCache *codeCache = self()->getFirstCodeCache(); codeCache;
    ...
    config.mccCallbacks().createHelperTrampolines(
        (uint8_t *)codeCache->getHelperBase(), config.numRuntimeHelpers());
  }
}
...
void OMR::CodeCacheManager::destroy() {
  ...
  if (_elfHeader) {
    ...
    static const int maxElfFilenameSize =
        15 + sizeof(jvmPid) * 3; // "/tmp/perf-%ld.jit"
    ...
    if (numCharsWritten > 0 && numCharsWritten < maxElfFilenameSize) {
      ...
      TR::CodeCacheMemorySegment *repositorySegment =
          _codeCacheRepositorySegment;
      fwrite(repositorySegment->segmentBase(), sizeof(uint8_t),
             repositorySegment->segmentTop() - repositorySegment->segmentBase(),
             elfFile);
...
TR::CodeCache *OMR::CodeCacheManager::allocateCodeCacheObject(
    TR::CodeCacheMemorySegment *codeCacheSegment, size_t codeCacheSizeAllocated,
...
// A compThreadID of -1 means unknown. This ID will be written into the code
// cache
// The ID of the thread that last reserved the cache will remain written after
// the
...
TR::CodeCache *OMR::CodeCacheManager::reserveCodeCache(
  ...
  *numReserved = numCachesAlreadyReserved;
  ...
  // Stats: count how many caches were not selected because they were already
  // reserved
  ...
  if (self()->canAddNewCodeCache()) {
    TR::CodeCacheConfig &config = self()->codeCacheConfig();
    codeCache = TR::CodeCache::allocate(self(), config.codeCacheKB() << 10,
                                        compThreadID);
  } else {
...
// with
// the address of the cold section. The sizes may increase due to alignment and
// headers
//
...
bool OMR::CodeCacheManager::almostOutOfCodeCache() {
  ...
  if (self()->canAddNewCodeCache())
...

    {
      ...
      for (TR::CodeCache *codeCache = self()->getFirstCodeCache(); codeCache;
           codeCache = codeCache->next()) {
        if (codeCache->getFreeContiguousSpace() >=
            config.lowCodeCacheThreshold()) {
          foundSpace = true;
          break;
        }
      }
    ...
    if (!foundSpace) {
      _lowCodeCacheSpaceThresholdReached =
          true; // Flag can be checked under debugger
...
void OMR::CodeCacheManager::printMccStats() {
...
void OMR::CodeCacheManager::printRemainingSpaceInCodeCaches() {
  CacheListCriticalSection scanCacheList(self());
  for (TR::CodeCache *codeCache = self()->getFirstCodeCache(); codeCache;
       codeCache = codeCache->next()) {
    fprintf(stderr, "cache %p has %u bytes empty\n", codeCache,
            codeCache->getFreeContiguousSpace());
...
void OMR::CodeCacheManager::printOccupancyStats() {
  CacheListCriticalSection scanCacheList(self());
  for (TR::CodeCache *codeCache = self()->getFirstCodeCache(); codeCache;
       codeCache = codeCache->next()) {
...
//
OMR::CodeCacheTrampolineCode *
OMR::CodeCacheManager::findHelperTrampoline(void *callingPC,
                                            int32_t helperIndex) {
...
//
void OMR::CodeCacheManager::onClassRedefinition(
    TR_OpaqueMethodBlock *oldMethod, TR_OpaqueMethodBlock *newMethod) {
  TR::CodeCacheConfig &config = self()->codeCacheConfig();
  if (!config.needsMethodTrampolines())
    return;
...
//
void OMR::CodeCacheManager::onFSDDecompile() {
  TR::CodeCacheConfig &config = self()->codeCacheConfig();
  if (!config.needsMethodTrampolines())
    return;
...
void OMR::CodeCacheManager::addFaintCacheBlock(FaintCacheBlock *&faintBlockList,
...
#if DEBUG
void OMR::CodeCacheManager::dumpCodeCaches() {
  CacheListCriticalSection scanCacheList(self());
  for (TR::CodeCache *codeCache = self()->getFirstCodeCache(); codeCache;
       codeCache = codeCache->next())
...
void OMR::CodeCacheManager::addFreeBlock(void *metaData, uint8_t *startPC) {
...
// Caller should expect that block may sometimes not be added.
void OMR::CodeCacheManager::freeFaintCacheBlock(FaintCacheBlock *block,
                                                uint8_t *startPC) {
  TR::CodeCache *owningCodeCache = self()->findCodeCacheFromPC(startPC);
...
//        allocationRetries, numCachesVisited, numCachesAlreadyReserved);
  ...
  codeCache = TR::CodeCache::allocate(
      self(), segmentSize,
      compThreadID); // the allocated cache is in reserved state // MCT
  if (!codeCache) {
    self()->setCodeCacheFull();
    return NULL;
  }
...
#ifdef MCT_DEBUG
  fprintf(stderr, "cache %p reset reservation in allocateCodeMemory after new "
                  "code cache allocation. New cache=%p needsToBeContiguous=%d "
                  "allocationRetries=%d\n",
...
TR::CodeCacheMemorySegment *OMR::CodeCacheManager::getNewCacheMemorySegment(
...
OMR::CodeCacheManager::allocateCodeCacheRepository(size_t repositorySize) {
  ...
  if (_codeCacheRepositorySegment) {
    ...
    *((TR::CodeCache **)start) = self()->getRepositoryCodeCacheAddress();
    ...
    if (config.verboseCodeCache()) {
      TR_VerboseLog::writeLineLocked(
          TR_Vlog_CODECACHE, "allocateCodeCacheRepository: size=%u heapBase=%p "
                             "heapAlloc=%p heapTop=%p",
...
void OMR::CodeCacheManager::registerCompiledMethod(const char *sig,
                                                   ...
                                                   uint32_t codeSize) {
#if (HOST_OS == OMR_LINUX)
  CodeCacheSymbol *newSymbol = static_cast<CodeCacheSymbol *>(
      self()->getMemory(sizeof(CodeCacheSymbol)));
...
void OMR::CodeCacheManager::increaseFreeSpaceInCodeCacheRepository(
  ...
  // (coldCodeAlloc-warmCodeAlloc))
  // For the repository segment we will keep warmCodeAlloc at 0 and set
  // coldCodeAlloc to
...
OMR::CodeCacheManager::carveCodeCacheSpaceFromRepository(
  ...
  // and codeCachePadKB.
  // If not defined, codeCachePadKB is 0, so getting the maximum will give
  // segmentSize.
  ...
  // scope for repository monitor critical section
  {
    ...
    // beginning
    // To keep caches nicely aligned pretend we want sizeof(TR::CodeCache *)
    // less
  ...
  if (config.verboseCodeCache()) {
    if (start)
      TR_VerboseLog::writeLineLocked(
          TR_Vlog_CODECACHE, "carved size=%u range: " POINTER_PRINTF_FORMAT
                             "-" POINTER_PRINTF_FORMAT,
...
void OMR::CodeCacheManager::reservationInterfaceCache(
    void *callSite, TR_OpaqueMethodBlock *method) {
  TR::CodeCacheConfig &config = self()->codeCacheConfig();
  if (!config.needsMethodTrampolines())
    return;
...
uint32_t OMR::CodeCacheManager::_numELFSymbols =
    0; // does not include UNDEF symbol: embedded in ELfCodeCacheTrailer
uint32_t OMR::CodeCacheManager::_totalELFSymbolNamesLength =
    1; // pre-count 0 for the UNDEF symbol name

void OMR::CodeCacheManager::initializeELFHeader() {
  // fprintf(stderr,"segmentAlloc: %p, base %p, diff %d\n",
  // _codeCacheRepositorySegment->segmentAlloc(),
  // _codeCacheRepositorySegment->segmentBase(),
  // _codeCacheRepositorySegment->segmentAlloc() -
  // _codeCacheRepositorySegment->segmentBase());
  ELFCodeCacheHeader *hdr = static_cast<ELFCodeCacheHeader *>(
      self()->getMemory(sizeof(ELFCodeCacheHeader)));
  ...
  uint32_t cacheSize = _codeCacheRepositorySegment->segmentTop() -
                       _codeCacheRepositorySegment->segmentBase();
...
void OMR::CodeCacheManager::initializeELFTrailer() {
  _elfTrailerSize =
      sizeof(ELFCodeCacheTrailer) +
      _numELFSymbols * sizeof(ELFSymbol) + // NOTE: ELFCodeCacheTrailer includes
                                           // 1 ELFSymbol: UNDEF
      _totalELFSymbolNamesLength;
  ELFCodeCacheTrailer *trlr =
      static_cast<ELFCodeCacheTrailer *>(self()->getMemory(_elfTrailerSize));

  uint32_t trailerStartOffset =
      sizeof(ELFCodeCacheHeader) + _elfHeader->phdr.p_filesz;
  uint32_t symbolsStartOffset =
      trailerStartOffset + offsetof(ELFCodeCacheTrailer, symbols);
  uint32_t symbolNamesStartOffset =
      symbolsStartOffset + (_numELFSymbols + 1) * sizeof(ELFSymbol);
  ...
  trlr->textSection.sh_addr =
      (ELFAddress)_codeCacheRepositorySegment->segmentBase();
  trlr->textSection.sh_offset = sizeof(ELFCodeCacheHeader);
  trlr->textSection.sh_size = _codeCacheRepositorySegment->segmentTop() -
                              _codeCacheRepositorySegment->segmentBase();
  ...
  trlr->shstrtabSection.sh_name =
      trlr->shstrtabSectionName - trlr->zeroSectionName;
  trlr->shstrtabSection.sh_type = SHT_STRTAB;
  trlr->shstrtabSection.sh_flags = 0;
  trlr->shstrtabSection.sh_addr = 0;
  trlr->shstrtabSection.sh_offset =
      trailerStartOffset + offsetof(ELFCodeCacheTrailer, zeroSectionName);
  trlr->shstrtabSection.sh_size =
      sizeof(trlr->zeroSectionName) + sizeof(trlr->shstrtabSectionName) +
      sizeof(trlr->textSectionName) + sizeof(trlr->dynsymSectionName) +
  ...
  // now walk list of compiled code symbols building up the symbol names and
  // filling in array of ELFSymbol structures
  ELFSymbol *elfSymbols = trlr->symbols + 0;
  char *elfSymbolNames = (char *)(elfSymbols + (_numELFSymbols + 1));

  // first symbol is UNDEF symbol: all zeros, even name is zero-terminated empty
  // string
  elfSymbolNames[0] = 0;
  ...
  while (sym) {
    ...
    // elfSymbols), sym->_nameLength, sym->_name);
...
void OMR::CodeCacheManager::initializeELFHeaderForPlatform(
    ELFCodeCacheHeader *hdr) {
...
#else
  TR_ASSERT(
      0, "unrecognized architecture: cannot initialize code cache elf header");
#endif
  ...
  TR_ASSERT(
      0,
      "unrecognized operating system: cannot initialize code cache elf header");
#endif

  hdr->hdr.e_ident[EI_ABIVERSION] = 0;

  hdr->hdr.e_ident[EI_DATA] =
      TR::Compiler->host.cpu.isLittleEndian() ? ELFDATA2LSB : ELFDATA2MSB;
}
