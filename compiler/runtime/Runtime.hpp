...
public:
  void *getAddress(TR_RuntimeHelper h) {
    return h < TR_numRuntimeHelpers ? _helpers[h]
                                    : (void *)(uintptr_t)0xDEADB00F;
  }
...
inline void *runtimeHelperValue(TR_RuntimeHelper h) {
  return runtimeHelpers.getAddress(h);
}
...
#define OFFSET_SAMPLING_PRESERVED_FROM_STARTPC (-8)
#define START_PC_TO_METHOD_INFO_ADDRESS                                        \
  -8 // offset from startpc to jitted body info
...
// recombined after LO_VALUE is sign-extended
// (e.g. when LO_VALUE is used with an instruction that takes a signed 16-bit
// operand).
// In this case we have to adjust HI_VALUE now so that the original value will
// be obtained once the two are recombined.
 ...
 * get converted to actual relocation types later on
 * (purposely all capital to differentiate from actual relo types and larger
 * than actual relocation type so we catch any incorrect
...
// within reach of a RIL instruction.
// In RIL instruction, the relative address is specified in number of half
// words.
...
// J9
typedef enum { needsFullSizeRuntimeAssumption = 1 } TR_HCRAssumptionFlags;
