...
bool CodeMetaDataManager::initializeCodeMetaDataManager() {
  ...
  if (_codeMetaDataManager) {
    ...
    // TR::Monitor *monitor =
    // TR::Monitor::create("JIT-CodeMetaDataManagerMonitor");
    // if (monitor)
    {
...
bool CodeMetaDataManager::containsMetaData(TR::MethodMetaDataPOD *metaData) {
  // OMR::CriticalSection searchingMetaData(_monitor);
  return (metaData && metaData == self()->findMetaDataForPC(metaData->startPC));
}

bool CodeMetaDataManager::removeMetaData(TR::MethodMetaDataPOD *metaData) {
  TR_ASSERT(metaData, "metaData must not be null");
...
// protected
bool CodeMetaDataManager::removeRange(TR::MethodMetaDataPOD *metaData,
                                      uintptr_t startPC, uintptr_t endPC) {
  bool removeSuccess = false;
  self()->updateCache(metaData->startPC);
  if (_cachedHashTable) {
...
void CodeMetaDataManager::updateCache(uintptr_t currentPC) {
  ...
  if (currentPC != _cachedPC) {
    ...
    TR_ASSERT(_cachedHashTable, "Either we lost a code cache or we attempted "
                                "to find a hash table for a non-code cache "
                                "startPC");
  }
}
...
TR::MethodMetaDataPOD **CodeMetaDataManager::insertMetaDataArrayInHash(
  ...
  if (LOW_BIT_SET(array)) {
    ...
    // we will
    // always be allocating a new chain from the current allocate of the method
    // store.
    // We'll need 2 entries (one for the new entry and one for the existing
    // tagged entry
    ...
    // there.
    // It is safe to check *newElement even when the method store is full
    // because
    ...
    if (*newElement == NULL) {
       ...
       * avoid twizzling
       * bits, copy the existing chain end forward one slot, and place the new
       * entry in the
       * old end slot.  A write barrier must be issued before storing the new
       * element, both
       * to ensure the metadata is fully visible before it can be seen in the
       * array, and to
      ...
      *(newElement - 1) = dataToInsert;
       ...
       * be copied
       * and extended in free space.  There may be enough free space in the
       * current
       * method store.  Once space is found, copy the chain into it with the new
       * entry at
       * the beginning (to avoid twizzling tag bits).  There's no need for a
       * write barrier
       * here since the new chain is not visible to anyone yet, and the caller
       * of this
...
uintptr_t CodeMetaDataManager::removeMetaDataRangeFromHash(
    TR::MetaDataHashTable *table, TR::MethodMetaDataPOD *dataToRemove,
    uintptr_t startPC, uintptr_t endPC) {
  TR::MethodMetaDataPOD **index;
  TR::MethodMetaDataPOD **endIndex;
  TR::MethodMetaDataPOD *temp;
...
TR::MethodMetaDataPOD **CodeMetaDataManager::removeMetaDataArrayFromHash(
    TR::MethodMetaDataPOD **array, TR::MethodMetaDataPOD *dataToRemove) {
  TR::MethodMetaDataPOD **index;
  uintptr_t count = 0;
  uintptr_t size;
