...
struct ELFCodeCacheTrailer {
  ...
  // start of a variable sized region: an ELFSymbol structure per symbol + total
  // size of elf symbol names
  ELFSymbol symbols[1];
};
...
public:
  ...
  CodeCacheManager(TR_FrontEnd *fe) : _fe(fe) { _initialized = false; }
  ...
  // structures
  // In future these facilities should probably be implemented via cs2
  // allocators
  ...
  getNewCacheMemorySegment(size_t segmentSize, size_t &codeCacheSizeAllocated);
  ...
  void addFaintCacheBlock(FaintCacheBlock *&blockList, FaintCacheBlock *block,
  ...
  CodeCacheTrampolineCode *findMethodTrampoline(TR_OpaqueMethodBlock *method,
                                                void *callingPC);
  CodeCacheTrampolineCode *findHelperTrampoline(void *callingPC,
                                                int32_t helperIndex);
  ...
  void onClassRedefinition(TR_OpaqueMethodBlock *oldMethod,
