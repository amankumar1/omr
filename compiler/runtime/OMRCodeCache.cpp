...
//
bool OMR::CodeCache::resizeCodeMemory(void *memoryBlock, size_t newSize) {
  if (newSize == 0)
  ...
  memoryBlock = (uint8_t *)memoryBlock -
  ...
  TR_ASSERT(cacheHeader->_eyeCatcher[0] == config.warmEyeCatcher()[0],
            "Missing eyecatcher during resizeCodeMemory");
  ...
  if (config.verboseReclamation()) {
    ...
    TR_VerboseLog::writeLineLocked(
        TR_Vlog_CODECACHE, "--resizeCodeMemory-- CC=%p cacheHeader=%p "
                           "oldSize=%u newSize=%d shrinkage=%u",
        this, cacheHeader, oldSize, newSize, shrinkage);
  }
         ...
         // portion
  {
    if (shrinkage >= MIN_SIZE_BLOCK) {
      ...
      if (self()->addFreeBlock2((uint8_t *)memoryBlock + newSize,
...
void OMR::CodeCache::query(TR::CodeCache *cc) {
  mcc_printf("======================\n");
  ...
  TR::CodeCacheConfig &config =
      TR::CodeCacheManager::instance()->codeCacheConfig();
...
bool OMR::CodeCache::initialize(TR::CodeCacheManager *manager,
  ...
  // codeCacheSegment->heapBase), which is equal to segmentSize
  // If codeCachePadKB is set, this will make the system believe that we
  // allocated segmentSize bytes,
...
//
OMR::CodeCacheTrampolineCode *OMR::CodeCache::reserveTrampoline() {
  TR::CodeCacheConfig &config = _manager->codeCacheConfig();
  ...
  if (_trampolineReservationMark <
      _trampolineBase + config.trampolineCodeSize()) {
...
//
void OMR::CodeCache::unreserveTrampoline() {
...
int32_t OMR::CodeCache::reserveResolvedTrampoline(TR_OpaqueMethodBlock *method,
  ...
  // scope for cache critical section
  {
    ...
    if (!entry) {
      // reserve a new trampoline since we got no active reservation for given
      // method */
      CodeCacheTrampolineCode *trampoline = self()->reserveTrampoline();
      if (trampoline) {
        // add hashtable entry
        if (!self()->addResolvedMethod(method))
          retValue = CodeCacheErrorCode::ERRORCODE_FATALERROR; // couldn't
                                                               // allocate
                                                               // memory from VM
      } else // no space in this code cache; must allocate a new one
      {
        ...
        if (config.verboseCodeCache()) {
          TR_VerboseLog::writeLineLocked(
              TR_Vlog_CODECACHE,
              "CodeCache %p marked as full in reserveResolvedTrampoline", this);
        }
      }
    }
  }
...
// Returns 0 on success or a negative error code on failure
//---------------------------------------------------------------------------
int32_t OMR::CodeCache::reserveUnresolvedTrampoline(void *cp, int32_t cpIndex) {
  int32_t retValue = CodeCacheErrorCode::ERRORCODE_SUCCESS; // assume success
  ...
  // scope for cache critical section
  {
    ...
    CodeCacheHashEntry *entry =
        _unresolvedMethodHT->findUnresolvedMethod(cp, cpIndex);
    if (!entry) {
      // don't have any reservation for this particular name/classLoader, make
      // one
      CodeCacheTrampolineCode *trampoline = self()->reserveTrampoline();
      if (trampoline) {
      ...
      } else // no space in this code cache; must allocate a new one
      {
        ...
        if (config.verboseCodeCache()) {
          TR_VerboseLog::writeLineLocked(
              TR_Vlog_CODECACHE,
              "CodeCache %p marked as full in reserveUnresolvedTrampoline",
              this);
        }
      }
    }
...
//
void OMR::CodeCache::adjustTrampolineReservation(TR_OpaqueMethodBlock *method,
                                                 void *cp, int32_t cpIndex) {
  ...
  // scope to update reservation
  {
    ...
    // seems suspicious to have this assertion disabled....why is it ok to fall
    // off without a reservation?
  ...
  if (_flags & CODECACHE_FULL_SYNC_REQUIRED) {
    for (uint32_t entryIdx = 0; entryIdx < _resolvedMethodHT->_size;
      ...
      for (CodeCacheHashEntry *entry = _resolvedMethodHT->_buckets[entryIdx];
        ...
        void *newPC = (void *)_manager->fe()->getMethodStartPC(
...
void OMR::CodeCache::resolveHashEntry(CodeCacheHashEntry *entry,
                                      TR_OpaqueMethodBlock *method) {
  // extract the entry from the unresolved hash table
  if (!_unresolvedMethodHT->remove(entry)) {
    // suspicious: should any asserts actually happen? why is this commented
    // out?
...
void OMR::CodeCache::onClassRedefinition(TR_OpaqueMethodBlock *oldMethod,
...
void OMR::CodeCache::onFSDDecompile() {
  ...
  // In theory, we can recycle everything in codeCaches at this point. For
  // minimum changes, we will only
...
void OMR::CodeCache::patchCallPoint(TR_OpaqueMethodBlock *method,
  ...
  // scope to patch trampoline
  {
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(
            TR_VerbosePatching)) {
      TR_VerboseLog::writeLineLocked(
          TR_Vlog_PATCH, "Patching callsite=0x%p using "
                         "j9method=0x%p,resolvedTramp=0x%p,methodRunAddress=0x%"
                         "p,newStartPC=0x%p,extraArg=0x%p",
...
//
bool OMR::CodeCache::addUnresolvedMethod(void *constPool,
  ...
  entry->_key =
      _unresolvedMethodHT->hashUnresolvedMethod(constPool, constPoolIndex);
...
bool OMR::CodeCache::addFreeBlock2WithCallSite(uint8_t *start, uint8_t *end,
  ...
  if (end <= (start + sizeof(CodeCacheFreeCacheBlock))) {
    if (config.verboseReclamation()) {
      TR_FrontEnd *fe = _manager->fe();
      TR_VerboseLog::writeLineLocked(
          TR_Vlog_FAILURE, "addFreeBlock2[%s.%d]: failed to add free block. "
                           "start = 0x%016x end = 0x%016x alignment = 0x%04x "
  ...
  if (_freeBlockList) {
    ...
    } else if (curr->_next && ((uint8_t *)curr->_next - end <
                               sizeof(CodeCacheFreeCacheBlock)) &&
  ...
  memset((void *)paintStart, 0xcc, ((CodeCacheFreeCacheBlock *)start)->_size -
                                       sizeof(CodeCacheFreeCacheBlock));
#endif
...
uint8_t *OMR::CodeCache::findFreeBlock(size_t size, bool isCold,
  ...
  for (currLink = _freeBlockList, prevLink = NULL; currLink;
    ...
    if (!biggestLink) {
      ...
      if (currLink->_size >= biggestLink->_size) // >= is important; do not use
                                                 // > only as secondBiggestLink
                                                 // may be wrong
      {
  ...
  if (bestFitLink) {
    ...
    if (config.verboseReclamation()) {
      TR_FrontEnd *fe = _manager->fe();
      TR_VerboseLog::writeLineLocked(
          TR_Vlog_CODECACHE, "--ccr- findFreeBlock: CodeCache=%p size=%u "
                             "isCold=%d bestFitLink=%p bestFitLink->size=%u "
                             "leftBlock=%p",
    ...
    // scope for cache walk
    {
      ...
      for (CodeCacheFreeCacheBlock *currLink = _freeBlockList; currLink;
        ...
        if (currLink->_size > cacheSize) {
          fprintf(stderr, "checkForErrors cache %p: Error: Size of the free "
                          "block %u is bigger than the size of the cache %u\n",
        ...
        if ((uint8_t *)currLink < _segment->segmentBase() + sizeof(void *) ||
          ...
          fprintf(stderr, "checkForErrors cache %p: Error: curLink %p is "
                          "outside cache boundaries\n",
                  this, currLink);
          doCrash = true;
        }
        ...
        if (endBlock < _segment->segmentBase() + sizeof(void *) ||
          ...
          fprintf(stderr, "checkForErrors cache %p: Error: End of block %p "
                          "residing at %p is outside cache boundaries\n",
        ...
        if (currLink->_next) {
          if ((uint8_t *)currLink->_next == endBlock) {
            ...
            if (!((uint8_t *)currLink < _warmCodeAlloc &&
              ...
              fprintf(stderr, "checkForErrors cache %p: Error: missed freed "
                              "block coalescing opportunity. Next block (%p) "
                              "is adjacent to current one %p-%p\n",
                      this, currLink->_next, currLink, endBlock);
              doCrash = true;
            }
          } else {
            if ((uint8_t *)currLink->_next <= endBlock) {
              fprintf(stderr, "checkForErrors cache %p: Error: next block (%p) "
                              "should come after end of current one %p-%p\n",
                      this, currLink->_next, currLink, endBlock);
              doCrash = true;
              ;
            }
            ...
            if (endBlock != _warmCodeAlloc) {
              ...
              if (*eyeCatcherPosition !=
                ...
                fprintf(stderr, "checkForErrors cache %p: Error: block coming "
                                "after this free one (%p-%p) does not have the "
                                "eye catcher but %u\n",
      ...
      if (_sizeOfLargestFreeWarmBlock != maxFreeWarmSize) {
        fprintf(stderr, "checkForErrors cache %p: Error: "
                        "_sizeOfLargestFreeWarmBlock(%d) != "
                        "maxFreeWarmSize(%d)\n",
      ...
      while (start < this->_trampolineBase) {
        ...
        } else // used seg
        {
          ...
          if (*eyeCatcherPosition !=
            ...
            fprintf(stderr, "checkForErrors cache %p: block %p is not in the "
                            "free list and does not have eye-catcher; "
                            "prevBlock=%p\n",
...
void OMR::CodeCache::setupSegment(TR::CodeCacheMemorySegment *codeCacheSegment,
                                  size_t codeCacheSizeAllocated,
                                  CodeCacheHashEntrySlab *hashEntrySlab) {
  // heapSize can be calculated as (codeCache->helperTop -
  // codeCacheSegment->heapBase), which is equal to segmentSize
...
uint8_t *OMR::CodeCache::allocateCodeMemory(size_t warmCodeSize,
  ...
  if (!warmIsFreeBlock) {
    ...
    // pointer to CodeCacheMethodHeader
    ...
    // what we wanted
    // Change the warmSize to the higher size so that the size of the block is
  ...
  if (size) {
    if (_trampolineReservationMark >= _trampolineBase + size) {
      ...
      if (config.verboseCodeCache()) {
        TR_VerboseLog::writeLineLocked(
            TR_Vlog_CODECACHE,
            "CodeCache %p marked as full in reserveNTrampoline", this);
      }
    }
  }
...
// A positive value means that a compilation thread is requesting the
// reservation
...
TR::CodeCache *OMR::CodeCache::allocate(TR::CodeCacheManager *manager,
  ...
  size_t codeCacheSizeAllocated;
  TR::CodeCacheMemorySegment *codeCacheSegment =
      manager->getNewCacheMemorySegment(segmentSize, codeCacheSizeAllocated);

  if (codeCacheSegment) {
    // allocate hash entry slab now, instead of after codeCache allocation as
    // its less expensive and easier to
    // fully unroll
    CodeCacheHashEntrySlab *hashEntrySlab = CodeCacheHashEntrySlab::allocate(
        manager, config.codeCacheHashEntryAllocatorSlabSize());
    if (hashEntrySlab) {
      uint8_t *start = NULL;
      TR::CodeCache *codeCache = manager->allocateCodeCacheObject(
          codeCacheSegment, codeCacheSizeAllocated, hashEntrySlab);
