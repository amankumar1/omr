...
// TR and OMR
// FECommon: is a repository of code and data that is common to all OMR
// frontends but not all TR frontends
// FEBase<D>: is a repository of code and data that is specific to a single
// frontend, but described as an abstract templat
//            based on the type D
...
public:
  FEBase()
      : FECommon(), _config(), _codeCacheManager(this),
  ...
  virtual uint8_t *allocateCodeMemory(TR::Compilation *comp,
                                      uint32_t warmCodeSize,
                                      uint32_t coldCodeSize, uint8_t **coldCode,
                                      bool isMethodHeaderNeeded);
  virtual void resizeCodeMemory(TR::Compilation *comp, uint8_t *bufferStart,
                                uint32_t numBytes);
  virtual uint8_t *allocateRelocationData(TR::Compilation *comp, uint32_t size);

  virtual intptrj_t indexedTrampolineLookup(int32_t helperIndex,
                                            void *callSite);
