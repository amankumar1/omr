...
public:
  explicit SegmentAllocator(::TR::RawAllocator allocator)
    ...
    TR_ASSERT(((pageSize() & (pageSize() - 1)) == 0),
              "Page size is not a power of 2, %llu",
  ...
  friend bool operator==(const SegmentAllocator &left,
                         const SegmentAllocator &right) {
  ...
  TR::MemorySegment allocate(size_t const segmentSize) {
    ...
    return TR::MemorySegment(_rawAllocator.allocate(alignedSize), alignedSize);
  }
  ...
  /**
   * @brief pageSize determines the appropriate page size for which to align
   * segment alloctions
