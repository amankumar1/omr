...
void TR::FECommon::outOfMemory(TR::Compilation *comp, const char *reason) {
...
static const int S390MachineTypes[] = {
    TR_FREEWAY,       TR_Z800,        TR_MIRAGE,       TR_MIRAGE2,
    TR_TREX,          TR_Z890,        TR_GOLDEN_EAGLE, TR_DANU_GA2,
    TR_Z9BC,          TR_Z10,         TR_Z10BC,        TR_ZG,
    TR_ZGMR,          TR_ZEC12,       TR_ZEC12MR,      TR_ZG_RESERVE,
    TR_ZEC12_RESERVE, TR_Z13,         TR_Z13BC,        TR_ZH,
    TR_DATAPOWER,     TR_ZH_RESERVE1, TR_ZH_RESERVE2};

static const int S390UnsupportedMachineTypes[] = {TR_G5, TR_MULTIPRISE7000};

static TR_S390MachineType portLib_get390zLinuxMachineType() {
  char line[80];
    ...
    while (fgets(line, LINE_SIZE, fp) > 0) {
      ...
      if (len > PROC_HEADER_SIZE &&
        ...
        if (len == PROC_LINE_SIZE) {
          ...
          // Scan list of unsupported machines - We do not initialize the JIT
          // for such hardware.
          for (i = 0; i < sizeof(S390UnsupportedMachineTypes) / sizeof(int);
               ++i) {
...
#define notImplemented(A)                                                      \
  TR_ASSERT(0, "This function is not defined for FEBase %s", (A))
...
TR::OptionTable OMR::Options::_feOptions[] = {
    ...
    {"limitfile=", "D<filename>\tfilter methods as defined in filename.  "
                   "Use limitfile=(filename,firstLine,lastLine) to limit lines "
                   "considered from firstLine to lastLine",
    ...
    {"maxInlinedCalls=", "O<nnn>\tmaximum number of calls to be inlined",
     ...
     offsetof(TR::Options, _maxInlinedCalls), 0, "F%d"},
    {"verbose", "L\twrite compiled method names to vlog file or stdout in "
                "limitfile format",
    ...
    {"vlog=", "L<filename>\twrite verbose output to filename",
     TR::Options::setString, offsetof(TR::JitConfig, options.vLogFileName), 0,
     "P%s"},
    {0}};
...
// s390.  Also, use a better
// name for this
void setDllSlip(char *CodeStart, char *CodeEnd, char *dllName,
                TR::Compilation *comp) {
  notImplemented("setDllSlip");
}

// X86 specific functions
extern "C" bool jitTestOSForSSESupport(void) {
  return false;
} // TODO there are some signal handling peculiarities on x86-32 linux

// runtime assumptions
#ifdef J9_PROJECT_SPECIFIC
// FIXME:
#include "runtime/RuntimeAssumptions.hpp"
void TR_PatchNOPedGuardSite::compensate(TR_FrontEnd *fe, bool isSMP,
                                        uint8_t *location,
                                        uint8_t *destination) {
  notImplemented("TR_PatchNOPedGuardSite::compensate");
}
void TR_PersistentClassInfo::removeASubClass(TR_PersistentClassInfo *) {
  notImplemented("TR_PersistentClassInfo::removeASubClass");
}
bool isOrderedPair(uint8_t recordType) {
  notImplemented("isOrderedPair");
  return false;
}
void TR_RuntimeAssumption::addToRAT(TR_PersistentMemory *persistentMemory,
                                    TR_RuntimeAssumptionKind kind,
                                    TR_FrontEnd *fe,
                                    TR_RuntimeAssumption **sentinel) {
  notImplemented("addToRAT");
}
void TR_RuntimeAssumption::dumpInfo(char *subclassName) {
  notImplemented("dumpInfo");
}
void TR_PatchJNICallSite::compensate(TR_FrontEnd *, bool, void *) {
  notImplemented("TR_PatchJNICallSite::compensate");
}
void TR_PreXRecompile::compensate(TR_FrontEnd *, bool, void *) {
  notImplemented("TR_PreXRecompile::compensate");
}
...
TR_PatchNOPedGuardSiteOnClassPreInitialize::make(TR_FrontEnd *fe,
  ...
  notImplemented("TR_PatchNOPedGuardSiteOnClassPreInitialize::allocate");
  return 0;
}
#endif
...
/* Hack - to satisfy the runtime linker we need a definition of the following
   functions.
   Should never be called.  This code can be removed when we permit the JIT to
   be linked
