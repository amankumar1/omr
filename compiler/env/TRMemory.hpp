...
class TR_MemoryBase {
protected:
  TR_MemoryBase(void *jitConfig) : _jitConfig(jitConfig) {}
 ...
 * them,
 * including in array form.  Array forms of placement new are irredeemably
 * broken, needing an
 * implementation defined prefix size for which there is no portable way to
 * discover.
...
namespace TR {
typedef CS2::stat_allocator<
    CS2::heap_allocator<65536, 12, TRCS2MemoryAllocator>>
...
template <typename T, typename Allocator> class Allocatable {
public:
  static void *operator new(size_t size, Allocator a) {
    return a.allocate(size);
  }
  static void operator delete(void *ptr, size_t size) {
    ((T *)ptr)->allocator().deallocate(ptr, size);
...
public:
  ...
  static Metric _currentMemUsage; // current statistics across all alloc types
                                  // and memory, persistent, stack, heap and CS2
                                  // allocators
  static uint8_t
...
private:
  Metric
      _reading; // meter reading while metering, uninitialised on construction
};
