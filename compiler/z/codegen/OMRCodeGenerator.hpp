...
namespace TR {
class S390LabelTableSnippet;
}
namespace TR {
class S390LookupSwitchSnippet;
}
class TR_S390OutOfLineCodeSection;
class TR_S390PrivateLinkage;
class TR_S390ScratchRegisterManager;
namespace TR {
class S390SortJumpTrampSnippet;
}
namespace TR {
class S390TargetAddressSnippet;
}
namespace TR {
...
#define NEEDS_TRAMPOLINE(target, rip, cg)                                      \
  (cg->alwaysUseTrampolines() || !CHECK_32BIT_TRAMPOLINE_RANGE(target, rip))
...
#define TR_MAX_FROM_TO_TABLE_STRING_SIZE                                       \
  1 // i.e. only TR one byte to one byte translations supported
...
#define TR_MAX_NUM_TRANSLATE_QUADS_BY_LOOP                                     \
  1 // note: for isInspectConvertingOp operations the length of the from/to
    // strings is the # of 'all' replaces
...
#define TR_MAX_REPLACE_ALL_LOOP_PERF                                           \
  150 // search > 1 byte (otherwise it is table lookup)
...
public:
  ...
  bool supportsArch(TR_S390ProcessorArchitectures arch) {
    TR_ASSERT(arch >= TR_UnknownArchitecture && arch <= TR_LatestArchitecture,
  ...
  void disableArch(TR_S390ProcessorArchitectures arch) {
    ...
    _processorArchitecture =
        _processorArchitecture < arch
            ? _processorArchitecture
            : (TR_S390ProcessorArchitectures)((uint32_t)arch - 1);
  }
  ...
  void enableArch(TR_S390ProcessorArchitectures arch) {
    TR_ASSERT(arch >= TR_UnknownArchitecture && arch <= TR_LatestArchitecture,
              "Invalid Processor Architecture.");
    _processorArchitecture =
        _processorArchitecture > arch ? _processorArchitecture : arch;
  }
...
private:
  ...
  TR_S390ProcessorInfo()
      : _processorArchitecture(TR_UnknownArchitecture), _crossCompile(false) {
#ifndef TR_HOST_S390
    _crossCompile = true;
#endif
    initialize();
...
public:
  ...
  void setNodeAddressOfCachedStaticTree(TR::Node *n) {
    _nodeAddressOfCachedStatic = n;
  }
  TR::Node *getNodeAddressOfCachedStatic() {
    return _nodeAddressOfCachedStatic;
  }

  bool supportsNamedVirtualRegisters(); // no virt, default
  TR::SparseBitVector &getBucketPlusIndexRegisters() {
    return _bucketPlusIndexRegisters;
  }
  ...
  uintptr_t getOutgoingArgLevelDuringTreeEvaluation() {
    return _outgoingArgLevelDuringTreeEvaluation;
  }
  void changeRegisterKind(TR::Register *temp, TR_RegisterKinds rk);

  TR::SymbolReference *_ARSaveAreaForTM;
  void setARSaveAreaForTM(TR::SymbolReference *symRef) {
    _ARSaveAreaForTM = symRef;
  }
  ...
  void setLabelHashTable(TR_HashTab *notPrintLabelHashTab) {
    _notPrintLabelHashTab = notPrintLabelHashTab;
  }
  ...
  TR::RegisterIterator *setARegisterIterator(TR::RegisterIterator *iter) {
    return _aRegisterIterator = iter;
  }

  TR::RegisterIterator *getHPRegisterIterator() { return _hpRegisterIterator; }
  TR::RegisterIterator *setHPRegisterIterator(TR::RegisterIterator *iter) {
    return _hpRegisterIterator = iter;
  }
  ...
  bool codegenSupportsLoadlessBNDCheck() {
    return _processorInfo.supportsArch(TR_S390ProcessorInfo::TR_zEC12);
  }
  ...
  TR::RegisterPair *allocateArGprPair(TR::Register *lowRegister,
                                      TR::Register *highRegister);
  void splitBaseRegisterPairsForRRMemoryInstructions(
  ...
  TR::Register *copyRestrictedVirtual(TR::Register *virtReg, TR::Node *node,
                                      TR::Instruction **preced = NULL);
  ...
  int64_t setAvailableHPRSpillMask(int64_t i) {
    return _availableHPRSpillMask = i;
  }
  int64_t maskAvailableHPRSpillMask(int64_t i) {
    return _availableHPRSpillMask &= i;
  }
  int64_t getAvailableHPRSpillMask() { return _availableHPRSpillMask; }
  int32_t getPreprologueOffset() { return _preprologueOffset; }
  int32_t setPreprologueOffset(int32_t offset) {
    return _preprologueOffset = offset;
  }

  // Flags to VETO all other policies wrt locked regs
  //
  bool getLitPoolRegisterIsFree() {
    return _cgFlags.testAny(S390CG_litPoolRegisterIsFree);
  }
  void setLitPoolRegisterIsFree(bool val) {
    _cgFlags.set(S390CG_litPoolRegisterIsFree, val);
  }
  ...
  bool getExtCodeBaseRegisterIsFree() {
    return _cgFlags.testAny(S390CG_extCodeBaseRegisterIsFree);
  }
  void setExtCodeBaseRegisterIsFree(bool val) {
    return _cgFlags.set(S390CG_extCodeBaseRegisterIsFree, val);
  }
   ...
   * transactional lock elision.  On Z, we require 25-30 cycles between
   * transactions, or else the latter transaction will
   * be aborted (with significant penalty).  The 45 is an estimate based on CPI
   * of 1.5-2, and average of 1 instruction per node.
   */
  ...
  bool specializedEpilogues() {
    return _cgFlags.testAny(S390CG_specializedEpilogues);
  }
  void setSpecializedEpilogues(bool b) {
    _cgFlags.set(S390CG_specializedEpilogues, b);
  }
  ...
  void setFCondMoveBranchOpCond(TR::InstOpCode::S390BranchCondition b) {
    fCondMoveBranchOpCond = (getMaskForBranchCondition(b) >> 4) & 0xF;
  }

  uint8_t getRCondMoveBranchOpCond() { return 0xF - fCondMoveBranchOpCond; }

  void markBlockThatModifiesRegister(TR::RealRegister::RegNum reg,
                                     int32_t blockNum);
  ...
  bool processInstruction(TR::Instruction *instr,
                          ...
                          int32_t &isFence, bool traceIt); // virt
  ...
  void insertPad(TR::Node *theNode, TR::Instruction *insertionPoint,
                 int32_t padSize, bool);
  ...
  TR_BitVector *
  setKilledRestrictedRegisters(TR_BitVector *killRestrictedRegsBV) {
  ...
  TR_BitVector *
  setCurrentlyRestrictedRegisters(TR_BitVector *currentRestrictedRegsBV) {
  ...
  int32_t
  setEstimatedOffsetForConstantDataSnippets(int32_t targetAddressSnippetSize,
                                            bool isWarm = 0);
  int32_t
  ...
  findOrCreateConstant(TR::Node *, void *c, uint16_t size, bool isWarm = 0);
  ...
  bool getDoingInstructionSelection() {
    return _cgFlags.testAny(S390CG_doingInstructionSelection);
  }
  void setDoingInstructionSelection(bool b) {
    _cgFlags.set(S390CG_doingInstructionSelection, b);
  }
  ...
  TR::S390LookupSwitchSnippet *CreateLookupSwitchSnippet(TR::Node *,
                                                         TR::Snippet *s);
  TR::S390TargetAddressSnippet *CreateTargetAddressSnippet(TR::Node *,
                                                           TR::Snippet *s);
  TR::S390TargetAddressSnippet *CreateTargetAddressSnippet(TR::Node *,
                                                           TR::LabelSymbol *s);
  TR::S390TargetAddressSnippet *CreateTargetAddressSnippet(TR::Node *,
                                                           TR::Symbol *s);
  TR::S390TargetAddressSnippet *findOrCreateTargetAddressSnippet(TR::Node *,
                                                                 uintptrj_t s);
  ...
  void setSupportsHighWordFacility(bool val) {
    _cgFlags.set(S390CG_supportsHighWordFacility, val);
  }
  bool supportsHighWordFacility() {
    return _cgFlags.testAny(S390CG_supportsHighWordFacility);
  }
  ...
  bool disableCommoningOfVolatiles() {
    return false;
  ...
  TR::RegisterDependencyConditions *
  addVMThreadPreCondition(TR::RegisterDependencyConditions *deps,
                          TR::Register *reg);
  TR::RegisterDependencyConditions *
  addVMThreadPostCondition(TR::RegisterDependencyConditions *deps,
                           TR::Register *reg);
  TR::RegisterDependencyConditions *
  addVMThreadDependencies(TR::RegisterDependencyConditions *deps,
                          TR::Register *reg);
  virtual void setVMThreadRequired(
      bool v); // override TR::CodeGenerator::setVMThreadRequired
   ...
   * array translate takes awhile to initialize - for very short translations,
   * the original
  ...
  bool parseEditParm(uint8_t *parm, int32_t length, bool &leadingSigs,
                     bool &trailingStr, uint8_t &fill, int32_t &numDigits,
                     int32_t &numSigs, int32_t &sigOff, int32_t &lastDigit,
...
#define TR_DEFAULT_DATA_SNIPPET_EXPONENT 4
  ...
  } // 1 << 4 = 16 byte max size for each constantDataSnippet
...
public:
  TR_S390Peephole(TR::Compilation *comp, TR::CodeGenerator *cg)
      : _fe(comp->fe()), _outFile(comp->getOutFile()),
        _cursor(comp->getFirstInstruction()), _cg(cg) {}
