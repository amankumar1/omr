...
#define PRINT_ME(string, node, cg)                                             \
  TR::Delimiter evalDelimiter(cg->comp(), cg->comp()->getOption(TR_TraceCG),   \
                              "EVAL", string)
#else
...
TR::InstOpCode::S390BranchCondition intToBrCond(const int32_t brCondInt) {
  ...
  default:
    TR_ASSERT(false, "Bad condition mask integer.\n");
...
int32_t brCondToInt(const TR::InstOpCode::S390BranchCondition brCond) {
  ...
  case TR::InstOpCode::COND_MASK15:
    return 15;
  default:
    TR_ASSERT(false, "Bad condition mask.\n");
    return -1;
  }
}

TR::InstOpCode::S390BranchCondition
getOppositeBranchCondition(TR::InstOpCode::S390BranchCondition brCond) {
  return intToBrCond((~brCondToInt(brCond)) & 0xf);
}

void orInPlace(TR::InstOpCode::S390BranchCondition &brCond,
               const TR::InstOpCode::S390BranchCondition other) {
...
generateLoad32BitConstant(TR::CodeGenerator *cg, TR::Node *node, int32_t value,
  ...
  if (node->getOpCode().hasSymbolReference())
    sym = node->getSymbol();
  bool needRegPair =
      cg->evaluateNodeInRegPair(node) ||
      !(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit());
  bool load64bit =
      !needRegPair && (node->getType().isInt64() || node->force64BitLoad());
...
TR::Instruction *genLoadLongConstant(TR::CodeGenerator *cg, TR::Node *node,
  ...
  if (comp->compileRelocatableCode() && sym && (sym->isCountForRecompile())) {
    ...
    // immediate.
    cursor = generateRILInstruction(cg, TR::InstOpCode::LGFI, node,
                                    targetRegister, (int32_t)value, cursor);
    ...
    cursor = generateRILInstruction(cg, TR::InstOpCode::LLILF, node,
                                    targetRegister, value, cursor);
  }
  ...
  // cannot safely generate LARL for longs on 31-bit
           ...
           !(comp->compileRelocatableCode())) {
    cursor = generateRILInstruction(cg, TR::InstOpCode::LARL, node,
                                    targetRegister, value, cursor);
  } else {
...
TR::Instruction *genLoadAddressConstant(TR::CodeGenerator *cg, TR::Node *node,
  ...
  if (node->isClassUnloadingConst()) {
    ...
    TR::Instruction *unloadableConstInstr = generateRILInstruction(
        cg, TR::InstOpCode::LARL, node, targetRegister, value);
...
static TR::Register *generateLoad32BitConstant(TR::CodeGenerator *cg,
  ...
  case TR::Int64: {
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
      ...
      generateLoad32BitConstant(cg, constExpr, constExpr->getLongIntLow(),
                                lowRegister, false);
      generateLoad32BitConstant(cg, constExpr, constExpr->getLongIntHigh(),
                                highRegister, false);
    }
    break;
  }
...
TR::Instruction *multiply31Reduction(TR::CodeGenerator *cg, TR::Node *node,
  ...
  // shift/subtract
  // sequence can be done. also - optimize for the case where it is (2**i -1) or
  // (2**i + 1)
  // since this can be done more efficiently (the shift degrades into a no-op
  // since it is a shift
  // of 0
...
TR::Instruction *generateS390ImmOp(TR::CodeGenerator *cg,
    ...
    if (cg->mayImmedInstructionCauseOverFlow(node)) {
      if (cg->canUseGoldenEagleImmediateInstruction(value)) {
        ...
        if (sourceRegister == targetRegister) {
          ...
          cursor =
              generateRRInstruction(cg, TR::InstOpCode::ALR, node,
  ...
  case TR::InstOpCode::AG:
    ...
    if (cg->mayImmedInstructionCauseOverFlow(node)) {
      if (cg->canUseGoldenEagleImmediateInstruction(value) && value > 0) {
        // LL: If Golden Eagle - can use Add Logical Long Immediate with max
        // 32-bit value, cannot use negative values because ALGFI does not sign
        // extend its immediate value.
      ...
      if (value >= MIN_IMMEDIATE_VAL && value <= MAX_IMMEDIATE_VAL) {
        if (sourceRegister == targetRegister) {
          ...
          if (cg->getCurrentRegisterPressure() <
            ...
            cursor = generateRIInstruction(cg, TR::InstOpCode::LGHI, node,
                                           tempReg, value, preced);
            cursor = generateRRInstruction(cg, TR::InstOpCode::ALGR, node,
                                           targetRegister, tempReg, cursor);
            if (cond)
              cond->addPostConditionIfNotAlreadyInserted(
                  tempReg, TR::RealRegister::AssignAny);
          ...
          cursor = generateRIInstruction(cg, TR::InstOpCode::LGHI, node,
                                         sourceRegister, value, preced);
          cursor =
              generateRRInstruction(cg, TR::InstOpCode::ALGR, node,
                                    targetRegister, sourceRegister, cursor);
        }
        return cursor;
      } else {
  ...
  case TR::InstOpCode::MSG:
    ...
    } else if (!disableMSFI && value >= GE_MIN_IMMEDIATE_VAL &&
...
TR::Instruction *generateS390ImmOp(TR::CodeGenerator *cg,
  ...
  TR::Instruction *cursor = NULL;
  TR::InstOpCode::Mnemonic immOp = TR::InstOpCode::BAD;
  bool is64BitRegister =
      targetRegister->getKind() == TR_GPR64 || cg->use64BitRegsOn32Bit();
  ...
  if (memOp != TR::InstOpCode::N && memOp != TR::InstOpCode::O &&
    ...
    if (((TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) &&
         !isUnsigned && value == (int32_t)value) ||
        ((TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) &&
  ...
  // constant
  // in 32bit target.  Currently we only handle long constant for 32bit target
  // for
  // bitwise instructions.
  // NOTE: You can only get in here if bitwiseOpNeedsLiteralFromPool() return
  // false.
    ...
    if (!(value & hhMask)) { // value = 0x****000000000000
      ...
      if (!(value & highMask)) {
        ...
        if (TR::Compiler->target.is64Bit() || is64BitRegister) {
          ...
          generateRILInstruction(cg, TR::InstOpCode::OILF, node,
                                 targetRegister->getLowOrder(), l_value);
          cursor =
              generateRILInstruction(cg, TR::InstOpCode::OILF, node,
                                     targetRegister->getHighOrder(), h_value);
        }
        return cursor;
      }
    }
    ...
    if ((value & hhMask) == hhMask) {
      ...
      if ((value & highMask) == highMask) { // value = 0x********FFFFFFFF
        ...
        if (TR::Compiler->target.is64Bit() || is64BitRegister) {
          ...
          generateRILInstruction(cg, TR::InstOpCode::NILF, node,
                                 targetRegister->getLowOrder(), l_value);
          cursor =
              generateRILInstruction(cg, TR::InstOpCode::NILF, node,
                                     targetRegister->getHighOrder(), h_value);
  ...
  case TR::InstOpCode::XG:
    if (!(value & highMask)) {
      ...
      if (TR::Compiler->target.is64Bit() || is64BitRegister) {
        ...
        generateRILInstruction(cg, TR::InstOpCode::XILF, node,
                               targetRegister->getLowOrder(), l_value);
        cursor =
            generateRILInstruction(cg, TR::InstOpCode::XILF, node,
                                   targetRegister->getHighOrder(), h_value);
  ...
  if (targetRegister != sourceRegister) {
    if (is64BitRegister)
      cursor = generateRRInstruction(cg, TR::InstOpCode::LGR, node,
                                     targetRegister, sourceRegister);
    else
      cursor = generateRRInstruction(
          cg, TR::InstOpCode::getLoadRegOpCodeFromNode(cg, node), node,
 ...
 * instruction(s) and return that instruction.
 * retBranchOpCond (reference parameter) returns the appropriate fBranchOpCond
 * or rBranchOpCond depending on direction of compare instruction that is
 * generated
 */
TR::Instruction *generateS390PackedCompareAndBranchOps(
    TR::Node *node, TR::CodeGenerator *cg,
  ...
  if (cg->traceBCDCodeGen())
    traceMsg(comp, "pdcompare node %p attempt to gen CLC with firstReg %s "
                   "(symRef #%d) and secondReg %s (symRef #%d)\n",
  ...
  if (signsAndDataAreValid) {
    ...
    if (knownSigns || cleanSigns) {
      if (cg->traceBCDCodeGen()) {
        traceMsg(
            comp, "\t+validateDecimalSignAndData=%s (firstReg (validData=%s "
        ...
        if (knownSigns)
          traceMsg(comp, "\t+firstReg and secondReg have matching known or "
                         "assumed sign codes (both are 0x%x)\n",
                   secondReg->getKnownOrAssumedSignCode());
        if (cleanSigns)
          traceMsg(
              comp,
      ...
      if (knownSignsArePositive || fBranchOpCond == TR::InstOpCode::COND_BERC ||
          fBranchOpCond == TR::InstOpCode::COND_BNERC) {
        if (cg->traceBCDCodeGen())
          traceMsg(comp, "\t+branchCond %s is allowed (knownSignsArePositive = "
                         "%s) so check sizes\n",
        ...
        if (firstReg->getSize() == secondReg->getSize()) {
          ...
          if (cg->traceBCDCodeGen())
            traceMsg(comp, "\t+regSizes match (firstRegSize = secondRegSize = "
                           "%d) so do gen CLC\n",
                     firstReg->getSize());
        } else {
          if (firstReg->getSize() < secondReg->getSize()) {
            if (cg->traceBCDCodeGen())
              traceMsg(comp, "\t+firstRegSize < secondRegSize (%d < %d) so "
                             "check upper bytes on firstReg\n",
                       firstReg->getSize(), secondReg->getSize());
            if (firstReg->getLiveSymbolSize() >= secondReg->getSize()) {
              if (cg->traceBCDCodeGen())
                traceMsg(comp, "\t+firstReg->liveSymSize() >= "
                               "secondReg->getSize() (%d >= %d)\n",
                         firstReg->getLiveSymbolSize(), secondReg->getSize());
              if (firstReg->getBytesToClear(firstReg->getSize(),
                                            secondReg->getSize()) == 0) {
                if (cg->traceBCDCodeGen())
                  traceMsg(comp, "\t+upper bytes (byte %d to %d) are clear so "
                                 "do gen CLC with clcSize = "
                                 "secondReg->getSize() = %d\n",
                           firstReg->getSize(), secondReg->getSize(),
                           secondReg->getSize());
                useCLC = true;
                ...
                traceMsg(comp, "\t-upper bytes (byte %d to %d) are not clear "
                               "so do not gen CLC\n",
                         firstReg->getSize(), secondReg->getSize());
              }
            } else if (cg->traceBCDCodeGen()) {
              traceMsg(comp, "\t-firstReg->liveSymSize() < "
                             "secondReg->getSize() (%d < %d) so do not gen "
                             "CLC\n",
                       firstReg->getLiveSymbolSize(), secondReg->getSize());
            }
          } else // firstReg->getSize() > secondReg->getSize()
          {
            if (cg->traceBCDCodeGen())
              traceMsg(comp, "\t+firstRegSize > secondRegSize (%d > %d) so "
                             "check upper bytes on secondReg\n",
                       firstReg->getSize(), secondReg->getSize());
            if (secondReg->getLiveSymbolSize() >= firstReg->getSize()) {
              if (cg->traceBCDCodeGen())
                traceMsg(comp, "\t+secondReg->liveSymSize() >= "
                               "firstReg->getSize() (%d >= %d)\n",
              ...
              traceMsg(comp, "\t-secondReg->liveSymSize() < "
                             "firstReg->getSize() (%d < %d) so do not gen "
                             "CLC\n",
                       secondReg->getLiveSymbolSize(), firstReg->getSize());
            }
          }
        ...
        traceMsg(comp, "\t-branchCond %s is not allowed (knownSignsArePositive "
                       "= %s) so do not gen CLC\n",
                 node->getOpCode().getName(),
                 knownSignsArePositive ? "yes" : "no");
      }
    } else if (cg->traceBCDCodeGen()) {
      traceMsg(comp, "\t-firstReg (clean=%s known=0x%x) and secondReg "
                     "(clean=%s known=0x%x) signs are not compatible so do not "
                     "gen CLC\n",
    ...
    traceMsg(comp, "\t-firstReg (validData=%s validSign=%s) and secondReg "
                   "(validData=%s validSign=%s) signs/data are not valid so do "
                   "not gen CLC\n",
...
static bool tryGenerateSIComparisons(TR::Node *node, TR::Node *constNode,
  ...
  if (!operand->isSingleRefUnevaluated() ||
      (operand->getOpCode().hasSymbolReference() &&
       operand->getSymbolReference()->getSymbol()->isRegisterSymbol()))
    return 0;
    ...
    if (isUnsignedCmp && 0 != (value >> 8) || // only the bottom byte
        !isUnsignedCmp &&
            (operand == otherNode &&
             ...
             operand != otherNode && 0 != (value >> 7))) // guaranteed to be
                                                         // positive byte if
                                                         // interpreted as
                                                         // logical or signed
    {
...
static TR::Instruction *tryGenerateCLCForComparison(TR::Node *node,
  ...
  // loaded (and masked) in a temp slot.
  // If this is the case, we won't use CLC on compressed refs, because the temp
  // slot is 8 bytes whereas VFT Symbol
  // is 4 bytes and CLC needs a slightly modified displacement to point to the
  // correct bytes.
  //
  // ============================================================
  ...
  //  \\  344 JBaload0
  // 0x000003ffdf6ca49a 0000269e [0x000003ff9e54a730] e3 60 c0 00 00 16
  // LLGF    GPR6, Shadow[<vft-symbol>] 0(GPR12)
  // 0x000003ffdf6ca4a0 000026a4 [0x000003ff9e54a800] a5 67 ff 00
  // NILL    GPR6,0xff00
  // 0x000003ffdf6ca4a4 000026a8 [0x000003ff9e54a9f0] e3 60 50 78 00 24
  // STG     GPR6, Auto[<temp slot 57>] 120(GPR5)
  // \\ gnu/testlet/java/text/DateFormat/Test.test(Lgnu/testlet/TestHarness;)V
  //  \\  354 JBinvokevirtual 52
  //  gnu/testlet/java/text/DateFormat/Test.equals(Ljava/lang/Object;)Z
  //  \\      19 JBifacmpeq 24
  // 0x000003ffdf6cb13a 0000333e [0x000003ff9e44be48] d5 02 50 78 70 00
  // CLC      Auto[<temp slot 57>] 120(3,GPR5), Shadow[<vft-symbol>] 0(GPR7)
  // 0x000003ffdf6cb140 00003344 [0x000003ff9e44c190] ASSOCREGS
  // 0x000003ffdf6cb140 00003344 [0x000003ff9e44bf88] a7 64 12 fc
  // BRC     MASK3(0x6), Label L0175, labelTargetAddr=0x000003FFDF6CD738
  ...
  if (numOfBytesToCompare == 4 &&
      cg->getCurrentRegisterPressure() < CODEGEN_REGPRESSURE_THRESHOLD) {
    return 0;
  }

  // TODO: We can still use CLC if only one of operands is a VFT symbol when we
  // are using
  // compressed refs. But we have to adjust/increase the displacement of the
  // operand that is a temp
  ...
  // (flags).
  // Instead, we ignore that byte and do not use it in our comparison. Since the
  // operands
  // must be the same size (or we would have returned earlier), it does not
  // matter form
  // which operand we get the size.
  // We use OR operator in case we have already loaded (and masked) one of the
  // operands
  // in a temp slot.
 ...
 * this
 * however given the time budget, this wasn't possible. There is a fair amount
 * of
 * intelligence in the binary commutative analyser that we dont' try to
 * duplicate
 ...
 * be superior
 * if they can be used. Only use the CH, CGH, CGF, CLGF instructions when we
 * know (looking through
 * conversions) that one operand is bigger than the other and we know that it is
 * going to
 * be in a register. The other operand must be a single reference, unevaluated
 * memory reference
 * of a narrower kind.
 * If these conditions don't match we are going to fall back to the binary
 * commutative analyser
...
static TR::Instruction *tryGenerateConversionRXComparison(TR::Node *node,
  ...
  if (!isUnsignedCmp) {
    ...
    case TR::ifacmpgt:
      isUnsignedCmp = true;
      //            traceMsg(TR::comp(), "Setting isUnsignedCmp to true for
      //            address compare\n");
      break;
    default:
      break;
  ...
  if (memNode->getOpCode().isConversion()) {
    ...
    if (memNode->getSize() == 2 &&       // only short memref
      ...
      // that a CH will be generated
      // as there are still more bailout conditions to be checked
      // otherwise an su2i will be changed to an s2i and then a CR could be used
      // and this would be incorrect
      if (regNode->isZeroExtension() &&
          regNode->getFirstChild()->getSize() == 2 &&
          regNode->isSingleRefUnevaluated() &&
          performTransformation(comp, "O^O Convert zero extension node [%p] to "
                                      "sign extension so that we can use "
                                      "CH/CGH\n",
                                regNode)) {
...
TR::Instruction *generateS390CompareAndBranchOpsHelper(
  ...
  if (cg->profiledPointersRequireRelocation() && node->isProfiledGuard() &&
     ...
     * fix is to
     * create a relocation record on the immediate in the CLGFI. Proper fix also
     * needs to
     * take out the forced evaluation of second child in
     * generateS390CompareBranch (hint:
  ...
  if ((secondChild->getOpCode().isLoadConst() ||
    ...
    //
             ...
             !(nonConstNode->getOpCode().hasSymbolReference() &&
               nonConstNode->getSymbolReference()
                   ->getSymbol()
                   ->isRegisterSymbol())) {
      // case 1.3
      ...
      bool useLTG = (TR::Compiler->target.is64Bit() &&
      ...
      } else if (!nonConstNode->couldIgnoreExtend() &&
      ...
      bool mustExtend = !useLTG && !nonConstNode->couldIgnoreExtend() &&
                        nonConstNode->force64BitLoad();

      if (mustExtend) {
        if (isUnsignedCmp) {
          generateRXYInstruction(cg, TR::InstOpCode::LLGF, nonConstNode,
          ...
          if (!nonConstNode->isUnsignedLoad()) {
            generateRXYInstruction(cg, TR::InstOpCode::LTGF, nonConstNode,
                                   testRegister,
      ...
      // Can replace L/CHI instructions with LT (31bit) or LTG (64bit)
        ...
        generateRXYInstruction(cg, (useLTG) ? TR::InstOpCode::LTG
                                            : TR::InstOpCode::LT,
        ...
        if (cg->use64BitRegsOn32Bit() && (constType == TR::Int64)) {
          ...
          if (!nonConstNode->couldIgnoreExtend() &&
              nonConstNode->force64BitLoad())
            load = nonConstNode->isUnsignedLoad() ? TR::InstOpCode::LLGF
                                                  : TR::InstOpCode::LGF;
      ...
      case TR::Address: {
        ...
        if (testRegister->getKind() == TR_GPR64 || cg->use64BitRegsOn32Bit())
          ...
          compareOpCode =
              isUnsignedCmp
                  ? TR::InstOpCode::getCmpLogicalOpCodeFromNode(nonConstNode)
                  : TR::InstOpCode::getCmpOpCodeFromNode(nonConstNode);
        break;
      }
      case TR::Int32:
      ...
      if (loadCCForArraycmp) {
        ...
        if ((constType == TR::Int64 || constType == TR::Address) &&
            !(testRegister->getKind() == TR_GPR64 ||
              cg->use64BitRegsOn32Bit()) &&
            constNode->getRegister())
          generateRRInstruction(cg, isUnsignedCmp ? TR::InstOpCode::CLGR
                                                  : TR::InstOpCode::CGR,
  ...
  //
           ...
           (firstChild->getFirstChild()->getOpCodeValue() == TR::buloadi ||
            ...
            (firstChild->getFirstChild()->getOpCode().hasSymbolReference() &&
           ...
           (secondChild->getFirstChild()->getOpCodeValue() == TR::buloadi ||
            ...
            (secondChild->getFirstChild()->getOpCode().hasSymbolReference() &&
  ...
  //
    ...
    case TR::Address: // FIXME: impossible to get here with TR::Address
      TR_ASSERT(
          (childType == TR::Address || childType == TR::Int64) &&
              (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()),
          "generateS390CompareOps: 64bit cg only when Long type\n");

      if ((childType == TR::Int64) && cg->use64BitRegsOn32Bit()) {
        TR_ASSERT(dataType == TR::Int64,
                  "First child and second child data types do not match.");
        ...
        if (isUnsignedCmp) {
          // If the data types are not the same size (can happen in compressed
          // refs), must use the widening compare opcode
          TR::InstOpCode::Mnemonic regToRegOpCode =
              (dataType == TR::Int64)
                  ? TR::InstOpCode::getCmpLogicalRegOpCode()
                  : TR::InstOpCode::getCmpLogicalRegWidenOpCode();
          TR::InstOpCode::Mnemonic memToRegOpCode =
              (dataType == TR::Int64)
                  ? TR::InstOpCode::getCmpLogicalOpCode()
                  : TR::InstOpCode::getCmpLogicalWidenOpCode();

          temp.genericAnalyser(node, regToRegOpCode, memToRegOpCode,
                               TR::InstOpCode::getLoadRegOpCode(), true,
                               branchTarget, fBranchOpCond, rBranchOpCond);
        } else {
          TR_ASSERT(dataType == TR::Int64,
                    "First child and second child data types do not match.");
          temp.genericAnalyser(node, TR::InstOpCode::getCmpRegOpCode(),
  ...
  if (cg->hasCCInfo() && cg->ccInstruction()->getOpCode().setsCarryFlag()) {
    TR_ASSERT((returnCond == TR::InstOpCode::COND_BERC ||
               returnCond == TR::InstOpCode::COND_BNERC),
...
TR::Register *generateS390CompareBool(
  ...
  if (isUnorderedOK) {
    ...
    cursor = generateS390BranchInstruction(cg, branchOp, branchOpCond, node,
                                           doneCmp);
  } else {
    cursor =
        generateS390CompareOps(node, cg, fBranchOpCond, rBranchOpCond, doneCmp);
    // generateS390CompareOps may generate separate compare and branch
    // instruction.
    // In this case the cursor points to the branch instruction, and the compare
    // is
    ...
    if (cursor->isCompare()) {
      ...
      // We expect children to return a register (getRegister()) or be constant
      // node (CIJ,CGIJ).
  ...
  // targetRegister
  // to the label we generate at the very end of BCDCHK. The reason is that
  // between the point of
  // TR::InstOpCode::CP and the return point of OOL path, targetRegister is
  // used. And we are potentially
  ...
  // DONE
  cursor =
      generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, doneCmp);
...
TR::Instruction *genCompareAndBranchInstructionIfPossible(
  ...
  // replacing compare and branch with
  // explicit compare followed by branch
  // As warm cache can contain cold blocks
  // what we really need is a way to know if the two blocks are in same
  // warm/cold cache
...
TR::InstOpCode::S390BranchCondition useBranchOnZeroConditionMaskIfPossible(
    TR::Node *node, TR::InstOpCode::S390BranchCondition brCond) {
...
TR::InstOpCode::S390BranchCondition generateTestUnderMaskIfPossible(
  ...
  if (memRefNode &&
      memRefNode->getSymbolReference()->getSymbol()->isRegisterSymbol())
    memRefNode = NULL;
  ...
  if (nonConstNode != NULL && nonConstNode->getOpCode().isAnd() &&
     ...
     * single byte. If so, we can use
     * in-memory TM + BRC sequence to do that. Below, we're trying to find the
     * byte to test
  ...
  if (constNode &&
    ...
    newBranchOpCond =
        useBranchOnZeroConditionMaskIfPossible(constNode, newBranchOpCond);
  }
  // Case: Pattern Matching for Byte Testing
    ...
    */
    ...
    newBranchOpCond =
        useBranchOnZeroConditionMaskIfPossible(constNode, fBranchOpCond);
  } else if (constNode && (node->getOpCodeValue() == TR::iflcmpeq ||
                           node->getOpCodeValue() == TR::iflucmpeq ||
                           node->getOpCodeValue() == TR::iflcmpne ||
                           node->getOpCodeValue() == TR::iflucmpne) &&
    ...
    cg->decReferenceCount(firstChild);
    cg->decReferenceCount(secondChild);
  } else if (constNode && (node->getOpCodeValue() == TR::ificmpeq ||
                           node->getOpCodeValue() == TR::ifiucmpeq ||
                           node->getOpCodeValue() == TR::ificmpne ||
                           node->getOpCodeValue() == TR::ifiucmpne) &&
             constNode->getInt() == 0 &&
...
static TR::Block *checkForCandidateBlockForConditionalLoadAndStores(
  ...
  TR::list<TR::CFGEdge *> &fallthroughSuccessors =
      fallthroughBlock->getSuccessors();
  TR::list<TR::CFGEdge *> &fallthroughExceptionSuccessors =
      fallthroughBlock->getExceptionSuccessors();

  TR::list<TR::CFGEdge *> &targetSuccessors = targetBlock->getSuccessors();
  TR::list<TR::CFGEdge *> &targetExceptionSuccessors =
...
bool isBranchOnCountAddSub(TR::CodeGenerator *cg, TR::Node *node) {
  ...
  if (regLoad->getOpCodeValue() != TR::iRegLoad &&
      !(regLoad->getOpCode().hasSymbolReference() &&
        regLoad->getSymbolReference()->getSymbol()->isRegisterSymbol()))
    return false;
...
TR::Register *generateS390CompareBranch(
  ...
  bool isCmpGT =
      node->getOpCodeValue() == TR::ificmpgt ||
  ...
  // (add or sub) hasn't been evaluated yet
  // (eg. nothing else references it), we can proceed. The tree should look like
  // this:
  // ificmpgt
  //   => iadd
  //     iRegLoad GRX
 ...
 * 64 ----- LGR     ----- LGR   ----- LG    -----   LG
 */
static const TR::InstOpCode::Mnemonic loadInstrs
    [2 /*Form*/][4 /*numberOfBits*/][2 /*isSigned*/]
    [2 /*numberOfExtendBits*/] = {
        /* ----- Logical -------   ----- Signed -------*/
        /*   32          64          32          64    */
        /*RegReg*/ {/* 8*/ {{TR::InstOpCode::LLCR, TR::InstOpCode::LLGCR},
                            {TR::InstOpCode::LBR, TR::InstOpCode::LGBR}},
                    /*16*/ {{TR::InstOpCode::LLHR, TR::InstOpCode::LLGHR},
                            {TR::InstOpCode::LHR, TR::InstOpCode::LGHR}},
                    /*32*/ {{TR::InstOpCode::LR, TR::InstOpCode::LLGFR},
                            {TR::InstOpCode::LR, TR::InstOpCode::LGFR}},
                    /*64*/ {{TR::InstOpCode::BAD, TR::InstOpCode::LGR},
                            {TR::InstOpCode::BAD, TR::InstOpCode::LGR}}},
        /*MemReg*/ {/* 8*/ {{TR::InstOpCode::LLC, TR::InstOpCode::LLGC},
                            {TR::InstOpCode::LB, TR::InstOpCode::LGB}},
                    /*16*/ {{TR::InstOpCode::LLH, TR::InstOpCode::LLGH},
                            {TR::InstOpCode::LH, TR::InstOpCode::LGH}},
                    /*32*/ {{TR::InstOpCode::L, TR::InstOpCode::LLGF},
                            {TR::InstOpCode::L, TR::InstOpCode::LGF}},
                    /*64*/ {{TR::InstOpCode::BAD, TR::InstOpCode::LG},
...
/**
 * Numbers as per "z/Architecture Instructions Sorted by Machine"
 * AR-99999-00-POK
 */
...
TR::Register *genericLoad(TR::Node *node, TR::CodeGenerator *cg,
  ...
  if (node->getType().isAddress() ||
      node->getType().isAggregate()) // o- and a-type and PLX-Fixed24/Fixed8
                                     // always unsigned
    nodeSigned = false;

  if (numberOfBits > 32 || numberOfBits == 31 || node->force64BitLoad())
    return genericLoadHelper<numberOfBits, 64, MemReg>(
        node, cg, tempMR, srcRegister, nodeSigned, node->couldIgnoreExtend());
  else
...
/**
 * This monster function handles all loads on zos and there are many, many
 * variations. If we had a guarantee that we are executing
 * on GoldenEagle or better, this function would had been just couple of lines
 * to pick the right instruction of the loadInstr
 * array(!!!):
 *
 *    TR::InstOpCode::Mnemonic load =
 * loadInstrs[form][numberOfBytesLog2][isSourceSigned][numberOfExtendBits/32-1];
 *    if (form == RegReg)
 *       generateRRInstruction(cg, load, node, targetRegister, srcRegister);
 *    else //if (form == MemReg)
 *       generateRXInstruction(cg, load, node, targetRegister, tempMR);
 *
 * Unfortunately, life isn't that easy... The rest of this function is to handle
 * cases when these instructions are not available
 * (i.e. insufficient arch, or must use register pairs)
 * Fallback path tries to pick the best instructions out of the reduced set. The
 * best fallback alternative is ICM (or IC) for load
 * from memory. Sign/zero extension, in the fall back case is done using two
 * shifts. If ICM was used, a single shift could be
 * used to sign/zero extend, by telling ICM to load into the top bits.
 * Furthermore, in the fallback case, sign/zero extension
 ...
 * to small (i.e. s2b)
 * In general, those wrappers check for the flags set on nodes.
 * genericLoadHelper is agnostic to the settings of the flags.
 *
 * PS: This is a template to help the compiler eliminate dead code, as there is
 * much of it for particular combinations of template
 * parameters.
 *
 ...
 * Register-to-Register load (extensible)
 *                            Should/could skip manual extension using two
 * shifts.
...
TR::Register *genericLoadHelper(TR::Node *node, TR::CodeGenerator *cg,
  ...
  bool couldIgnoreExtend =
      form == MemReg && couldIgnoreExtend_OR_canClobberSrcReg;
  ...
  const bool useRegPairs =
      (numberOfExtendBits == 64 && TR::Compiler->target.is32Bit() &&
       !cg->use64BitRegsOn32Bit()) ||
      cg->evaluateNodeInRegPair(node);
  ...
  // (0xF << (4 - numberOfBits / 8)) & 0xF   numberOfBits/8 is 1|2|3, will pass
  // 0b1000|0b1100=12|0b1110=14
  // (0xF << (8 - numberOfBits / 8)) & 0xF   numberOfBits/8 is 5|6|7, will pass
  // 0b1000|0b1100=12|0b1110=14
  // (0x1 << (numberOfBits / 8))     - 1     numberOfBits/8 is 1|2|3, will pass
  // 0b0001|0b0011=3|0b0111=7
  // (0x1 << (numberOfBits / 8 - 4)) - 1     numberOfBits/8 is 5|6|7, will pass
  // 0b0001|0b0011=3|0b0111=7
  ...
  // Function pointer for checking if the particular instruction is available on
  // the hardware we run on.
  TR_S390ProcessorInfo::TR_S390ProcessorArchitectures ReqArch =
      reqArchForLoad[form][numberOfBytesLog2][isSourceSigned]
                    [numberOfExtendBits / 32 - 1];

  if (cg->comp()->getOption(TR_TraceCG))
    traceMsg(cg, "Calling genericLoadHelper with %c%d(%d)>%d %s %s\n",
             isSourceSigned ? '-' : '0', numberOfBits, numberOfBytesLog2,
             numberOfExtendBits, form == MemReg ? "MEM" : "REG",
             couldIgnoreExtend ? "could" : "");

  // If register cannot be clobered, allocate a new one
  // Also allocate a new one if, for whichever reason, incorrect register has
  // been provided
  // Also handle the case for Register Pairs, when the srcReg is 32bit (i.e. put
  // the src register in the top half, to save one instruction).
  //
  // !useRegPairs && srcRegister->getKind() == TR_GPR &&
  // cg->use64BitRegsOn32Bit() && TR::Compiler->target.is32Bit() check is for
  // the mixed mode case where we have a 32 bit reg
  // on input and must allocate a new 64 bit register (otherwise later
  // spills/fills may not preserve the top 32 bits (i.e. LR/L will be used
  // instead of LGR/LG)
  if (form == RegReg &&
      (!canClobberSrcReg || (useRegPairs && numberOfBytes <= 4) ||
       (numberOfExtendBits == 64 &&
        ((!useRegPairs && srcRegister->getKind() == TR_GPR &&
          cg->use64BitRegsOn32Bit()) ||
         (useRegPairs && targetRegister->getRegisterPair() == NULL))))) {
    bool c1 = (useRegPairs && numberOfBytes <= 4);
    bool c2 = numberOfExtendBits == 64 && !useRegPairs &&
              srcRegister->getKind() == TR_GPR;
    bool c3 = numberOfExtendBits == 64 && useRegPairs &&
              targetRegister->getRegisterPair() == NULL;
    ...
    else if (numberOfExtendBits == 64) // useRegPairs
    {
      if (!canClobberSrcReg)
        ...
        targetRegister = cg->allocateConsecutiveRegisterPair(
            cg->allocateRegister(), srcRegister);
      }
      // else canClobberChildsReg and both high and low are used. No need to
      // alloc new registers
    } else
      targetRegister = cg->allocateRegister();
             ...
             (srcRegister == NULL ||
              (numberOfExtendBits == 64 &&
               ((!useRegPairs && srcRegister->getKind() == TR_GPR) ||
                (useRegPairs && srcRegister->getRegisterPair() == NULL))))) {
  ...
  if (useRegPairs && numberOfBytes != 8) {
    ...
    flogrDeps->addPostCondition(targetRegister->getHighOrder(),
                                TR::RealRegister::LegalEvenOfPair);
    flogrDeps->addPostCondition(targetRegister->getLowOrder(),
  ...
  // here.....................................................................
  //-------------------------------------------------------------------------------------------
  if ((numberOfBits & (numberOfBits - 1)) ==
          0 && // Clever way to check if numberOfBits is a power-of-two, i.e. 8,
               // 16, 32, 64
      !useRegPairs &&
      cg->getS390ProcessorInfo()->supportsArch(
          ReqArch)) // Are we running on awesome hardware and OS?
  {
    CASE(1);
    // Use a single load to load and extend in one shot
    // Ideally, should fall through to here for most cases,
    // However, have to deal with old architectures, strange data types and
    // register pairs
    TR::InstOpCode::Mnemonic load =
    ...
    TR_ASSERT(isSourceSigned == false && numberOfExtendBits == 64,
              "Not Implemented: not expecting to see addresses sign extended, "
              "only zero extended.");
    if (form == RegReg)
      generateRRInstruction(cg, TR::InstOpCode::LLGTR, node, targetRegister,
                            srcRegister);
    else // form == MemReg
      generateRXInstruction(cg, TR::InstOpCode::LLGT, node, targetRegister,
                            tempMR);
  } else if (form == MemReg) {
    // Generate dumb instructions, should work everywhere
    TR_ASSERT((numberOfBytes != 4 && numberOfBytes != 8) || useRegPairs,
              "This case should have been already handled");
    CASE(2);

    if (numberOfBytes == 8)
      generateRSInstruction(cg, TR::InstOpCode::LM, node, targetRegister,
                            tempMR);
    else if (numberOfBytes == 4) {
      CASE(3);
      if (couldIgnoreExtend)
        generateRXInstruction(cg, TR::InstOpCode::L, node,
                              targetRegister->getLowOrder(), tempMR);
      else {
        CASE(4);
        generateRXInstruction(cg, TR::InstOpCode::L, node,
                              targetRegister->getHighOrder(), tempMR);
        cursor = generateRSInstruction(
            cg, isSourceSigned ? TR::InstOpCode::SRDA : TR::InstOpCode::SRDL,
            node, targetRegister, 32);
        cursor->setDependencyConditions(flogrDeps);
      }
    } else // numberOfBytes == 1|2|3|5|6|7
    {
      CASE(5);
      tempMR->separateIndexRegister(node, cg, true,
                                    NULL); // Make it so that we _can_ use ICM

      if (useRegPairs) {
        if (numberOfBytes > 4) {
          CASE(6);
          generateRSInstruction(cg, TR::InstOpCode::ICM, node,
                                targetRegister->getHighOrder(), icmMask,
                                tempMR);
          if (!couldIgnoreExtend)
            generateRSInstruction(
                cg, isSourceSigned ? TR::InstOpCode::SRA : TR::InstOpCode::SRL,
                node, targetRegister->getHighOrder(), 64 - numberOfBits);

          TR::MemoryReference *lowMR =
              generateS390MemoryReference(*tempMR, numberOfBytes - 4, cg);
          generateRXInstruction(cg, TR::InstOpCode::L, node,
                                targetRegister->getLowOrder(), lowMR);
          lowMR->stopUsingMemRefRegister(cg);
        } else if (couldIgnoreExtend) {
          CASE(7);
          generateRSInstruction(cg, TR::InstOpCode::ICM, node,
                                targetRegister->getLowOrder(), icmMask, tempMR);
        } else {
          CASE(8);
          generateRSInstruction(cg, TR::InstOpCode::ICM, node,
                                targetRegister->getHighOrder(), icmMask,
          ...
          cursor = generateRSInstruction(
              cg, isSourceSigned ? TR::InstOpCode::SRDA : TR::InstOpCode::SRDL,
              node, targetRegister, numberOfBits);
          cursor->setDependencyConditions(flogrDeps);
        }
      } else {
        CASE(9);
        TR::InstOpCode::Mnemonic icm =
            ((couldIgnoreExtend && numberOfBytes < 4) ||
             numberOfExtendBits == 32)
                ? TR::InstOpCode::ICM
                : TR::InstOpCode::ICMH;
        ...
        if (!couldIgnoreExtend) {
          ...
          if (numberOfExtendBits == 32)
            generateRSInstruction(
                cg, isSourceSigned ? TR::InstOpCode::SRA : TR::InstOpCode::SRL,
                node, targetRegister, numberOfExtendBits - numberOfBits);
          else
            generateRSInstruction(cg, isSourceSigned ? TR::InstOpCode::SRAG
                                                     : TR::InstOpCode::SRLG,
                                  node, targetRegister, targetRegister,
                                  numberOfExtendBits - numberOfBits);
        }
        ...
        {
          CASE(11);
          targetRegister->setKind(TR_GPR64); // Just to triple check that GRA
                                             // knows this is a 64 bit register,
                                             // not two separate halves
          TR::MemoryReference *lowMR =
              generateS390MemoryReference(*tempMR, numberOfBits / 8 - 4, cg);
          generateRXInstruction(cg, TR::InstOpCode::L, node, targetRegister,
                                lowMR);
          lowMR->stopUsingMemRefRegister(cg);
        }
      }
  ...
  } else // form == RegReg
  {
    ...
    if (useRegPairs) {
      CASE(13);
      TR_ASSERT(targetRegister->getHighOrder() != NULL,
                "No input provided to genericLoadHelper");
      rightShift = isSourceSigned ? TR::InstOpCode::SRDA : TR::InstOpCode::SRDL;
      if (numberOfBytes > 4)
        generateRSInstruction(cg, TR::InstOpCode::SLDA, node, targetRegister,
                              leftShiftBits);
      else if (leftShiftBits - 32 == 0 && !canClobberSrcReg)
        generateRRInstruction(cg, TR::InstOpCode::LR, node,
                              targetRegister->getHighOrder(), srcRegister);
      else if (leftShiftBits - 32 != 0)
        generateRSInstruction(cg, TR::InstOpCode::SLLK, node,
                              targetRegister->getHighOrder(), srcRegister,
                              leftShiftBits - 32);
      else if (leftShiftBits - 32 != 0 &&
               targetRegister->getHighOrder() == srcRegister)
        generateRSInstruction(cg, TR::InstOpCode::SLL, node,
                              targetRegister->getHighOrder(),
                              leftShiftBits - 32);
      else if (leftShiftBits - 32 != 0) {
        generateRRInstruction(cg, TR::InstOpCode::LR, node,
      ...
      cursor = generateRSInstruction(cg, rightShift, node, targetRegister,
                                     leftShiftBits);
      cursor->setDependencyConditions(flogrDeps);
    } else {
      CASE(14);
      if (numberOfBits == 24 && numberOfExtendBits == 32 && !isSourceSigned &&
          canClobberSrcReg) {
        generateRIInstruction(cg, TR::InstOpCode::NILH, node, targetRegister,
                              (int16_t)0x00FF);
      } else {
        if (numberOfExtendBits == 64) {
          generateRSInstruction(cg, TR::InstOpCode::SLLG, node, targetRegister,
                                srcRegister, leftShiftBits);
          generateRSInstruction(cg, isSourceSigned ? TR::InstOpCode::SRAG
                                                   : TR::InstOpCode::SRLG,
                                node, targetRegister, targetRegister,
                                numberOfExtendBits - numberOfBits);
        } else {
          generateRSInstruction(cg, TR::InstOpCode::SLLK, node, targetRegister,
                                srcRegister, leftShiftBits);
          generateRSInstruction(
              cg, isSourceSigned ? TR::InstOpCode::SRA : TR::InstOpCode::SRL,
              node, targetRegister, numberOfExtendBits - numberOfBits);
...
void OMR::Z::TreeEvaluator::evaluateRegLoads(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  using namespace CS2;
 ...
 * the conversions that narrow data.
 * Narrowing conversions are done by either zero- or sign-extending; which of
 * the two, depends only on the signnedness of the
 * destination type. Note that on 32 bit AMODE, narrowing will always be into a
 * 32bit register. (i.e. there is no
 * case that narrows into a 64bit register yet, unless this function were to
 * support o-types -- it does not)
 *
 * It is possible for this evaluator to be skipped, when the
 * isUnneededConversion() flag is set.
 * The flags are set elsewhere (i.e. CodeGenPrep.cpp). Any patterns to be
 * detected, should be detected elsewhere.
 ...
 * all the conversions that grow/extend data.
 * Extension conversions are done by either zero- or sign-extending; which of
 * the two, depends only on the signnedness of the
 * source type.
 *
 * It is possible for this evaluator to be skipped, when the
...
OMR::Z::TreeEvaluator::extendCastEvaluator(TR::Node *node,
  ...
  if (numberOfExtendBits == 32 &&
      targetRegister->getRegisterPair() !=
  ...
  if (cg->supportsHighWordFacility() &&
      !comp->getOption(TR_DisableHighWordRA) && (numberOfExtendBits == 64))
    canClobberSrc = false;
  ...
  if (!node->isUnneededConversion() && !childRegister->alreadySignExtended()) {
    targetRegister = genericLoadHelper<srcSize, numberOfExtendBits, RegReg>(
        ...
        canClobberSrc);
  } else if (numberOfExtendBits == 64 && TR::Compiler->target.is32Bit() &&
             cg->use64BitRegsOn32Bit() &&
  ...
  // Might need to fixup register kind in pass-through case
  {
    cg->changeRegisterKind(targetRegister, TR_GPR64);
             ...
             !cg->use64BitRegsOn32Bit() &&
             targetRegister->getRegisterPair() == NULL)
    TR_ASSERT(0, "Incorrect flags set on node %p; register pairs need always "
                 "be extended\n",
              node);
 ...
 * to- or from-). Address conversions behave
 * exactly like the code from extendCastEvaluator() or narrowCastEvaluator().
 * However, those templates need to know
 * the precision at compile time, meanwhile AMODE is unlike other types, for
 * which bit-size is not constant.
 *
 * To avoid creating __YET__ another wrapper, depending on the direction of the
 * conversion (to- or from- an address), the
...
OMR::Z::TreeEvaluator::addressCastEvaluator(TR::Node *node,
  ...
  } else if (dirToAddr && otherSize > addrSize) // i.e. (otherSize)2a
  {
    ...
    switch (addrSize) {
    case 56:
      return extendCastEvaluator<false, 56, 64>(node, cg);
      break;
    case 48:
      return extendCastEvaluator<false, 48, 64>(node, cg);
      break;
    case 40:
      return extendCastEvaluator<false, 40, 64>(node, cg);
      break;
    case 32:
      return narrowCastEvaluator<false, 32>(node, cg);
      break;
    case 24:
      return narrowCastEvaluator<false, 24>(node, cg);
      break;
    case 16:
      return narrowCastEvaluator<false, 16>(node, cg);
      break;
    case 8:
      return narrowCastEvaluator<false, 8>(node, cg);
      break;
    default:
  ...
  } else // if (!dirToAddr && addrSize < otherSize) //i.e. a2(otherSize)
  {
    ...
    // NOTE 31, on purpose!
    case 32:
      return extendCastEvaluator<false, 31, 64>(node, cg);
      break;
    case 24:
      return extendCastEvaluator < false, 24,
             (otherSize > 32) ? 64 : 32 > (node, cg);
      break;
    case 16:
      return extendCastEvaluator < false, 16,
             (otherSize > 32) ? 64 : 32 > (node, cg);
      break;
    case 8:
      return extendCastEvaluator < false, 8,
             (otherSize > 32) ? 64 : 32 > (node, cg);
      break;
    default:
      TR_ASSERT(0, "Invalid Address Precision (%d)\n", addrSize);
...
// all callable variations in this compilation units,
  ...
  if (isReversed) {
    ...
    if (node->force64BitLoad())
      tempReg = genericLoadHelper<16, 64, RegReg>(
          node, cg, NULL, tempReg, !node->isUnsignedLoad(), true);
    else
...
bool relativeLongLoadHelper(TR::CodeGenerator *cg, TR::Node *node,
  ...
  if (symbol->isStatic() && !symRef->isUnresolved() &&
    ...
    TR_ASSERT(!node->getType().isInt64() || TR::Compiler->target.is64Bit() ||
                  cg->use64BitRegsOn32Bit(),
              "Helper cannot handle 64bit loads on 32-bit platforms.");
    // TR::InstOpCode::Mnemonic op =
    // (node->getType().isInt32())?TR::InstOpCode::LRL :
    ...
    if ((!disableFORCELRL ||
      ...
      // traceMsg(cg->comp(), "node->useSignExtensionMode = %d
      // node->isUnsignedLoad = %d\n",node->useSignExtensionMode(),
      // node->isUnsignedLoad());
      if (node->useSignExtensionMode()) {
...
TR::Register *iloadHelper(TR::Node *node, TR::CodeGenerator *cg,
  ...
  if (isReversed) {
    ...
    if (node->force64BitLoad()) {
      if (node->isUnsignedLoad())
        generateRRInstruction(cg, TR::InstOpCode::LLGFR, node, tempReg,
                              tempReg);
      else
 ...
 */
TR::Register *lloadHelper(TR::Node *node, TR::CodeGenerator *cg,
                          TR::MemoryReference *highMR, bool isReversed) {
  TR::Compilation *comp = cg->comp();
  // TR_ASSERTC( !isReversed,comp, "need to write logic for reverse load
  // still");
  TR_ASSERT(TR::Compiler->target.is32Bit() || cg->evaluateNodeInRegPair(node),
            "should call 64-bit version: lloadHelper64()!\n");
  ...
  if (node->getOpCode().hasSymbolReference() &&
      ...
      !node->getSymbolReference()->isUnresolved() && // Cannot split LM's into
                                                     // two L's - unresolved
                                                     // patching cannot handle
                                                     // two memrefs.
    ...
    if (isReversed) {
      generateRXInstruction(cg, TR::InstOpCode::LRV, node,
                            longRegister->getHighOrder(), lowMR);
      generateRXInstruction(cg, TR::InstOpCode::LRV, node,
                            longRegister->getLowOrder(), highMR);
    } else {
      generateRXInstruction(cg, TR::InstOpCode::L, node,
                            longRegister->getHighOrder(), highMR);
      generateRXInstruction(cg, TR::InstOpCode::L, node,
                            longRegister->getLowOrder(), lowMR);
    }
  } else {
    if (highMR == NULL) {
    ...
    if (node->getOpCode().hasSymbolReference()) {
      if (node->getSymbolReference()->getSymbol()->getSize() == 5)
        longRegister =
            (TR::RegisterPair *)genericLoad<40>(node, cg, highMR, longRegister);
      else if (node->getSymbolReference()->getSymbol()->getSize() == 6)
        longRegister =
            (TR::RegisterPair *)genericLoad<48>(node, cg, highMR, longRegister);
      else if (node->getSymbolReference()->getSymbol()->getSize() == 7)
        longRegister =
            (TR::RegisterPair *)genericLoad<56>(node, cg, highMR, longRegister);
      else {
        if (isReversed) {
          TR::MemoryReference *lowMR =
              generateS390MemoryReference(*highMR, 4, cg);
          generateRXInstruction(cg, TR::InstOpCode::LRV, node,
                                longRegister->getHighOrder(), lowMR);
          generateRXInstruction(cg, TR::InstOpCode::LRV, node,
                                longRegister->getLowOrder(), highMR);
        } else
          longRegister = (TR::RegisterPair *)genericLoad<64>(node, cg, highMR,
                                                             longRegister);
      }
    } else if (isReversed) {
      TR::MemoryReference *lowMR = generateS390MemoryReference(*highMR, 4, cg);
      ...
      longRegister =
          (TR::RegisterPair *)genericLoad<64>(node, cg, highMR, longRegister);
  }
  // IvanB
...
TR::Register *lloadHelper64(TR::Node *node, TR::CodeGenerator *cg,
  ...
  TR_ASSERT(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(),
            "should call 32-bit version: lloadHelper()");
...
TR::Register *aloadHelper(TR::Node *node, TR::CodeGenerator *cg,
  ...
  TR::Register
      *tempReg; // msf - ia32 does not check for not internal pointer...
  ...
  // conservatively. The logic should be modified
  // to skip evaluation of unneeded aloadi nodes which do not contain any symbol
  // reference.
  ...
  if ((node->getOpCodeValue() == TR::aload) &&
    ...
    generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), node, tempReg,
                          cg->getS390Linkage()->getStaticBaseRealRegister());
    node->setRegister(tempReg);
    return tempReg;
  }

  if ((node->getOpCodeValue() == TR::aload) &&
      cg->isAddressOfPrivateStaticSymRefWithLockedReg(
    ...
    generateRRInstruction(
        cg, TR::InstOpCode::getLoadRegOpCode(), node, tempReg,
        cg->getS390Linkage()->getPrivateStaticBaseRealRegister());
    node->setRegister(tempReg);
    return tempReg;
  }
  ...
  if (isPICCandidate &&
    ...
    TR::Instruction *unloadableConstInstr =
        generateRILInstruction(cg, TR::InstOpCode::LARL, node, tempReg, value);
    TR_OpaqueClassBlock *unloadableClass = NULL;
    ...
    if (cg->isCompressedClassPointerOfObjectHeader(node) && !dynLitPoolLoad) {
      ...
      if (node->isLoadAndTest()) {
        ...
        if (cg->getConditionalMovesEvaluationMode()) {
          generateRSInstruction(
              cg, TR::InstOpCode::getLoadOpCode() == TR::InstOpCode::L
                      ? TR::InstOpCode::LOC
                      : TR::InstOpCode::LOCG,
...
bool relativeLongStoreHelper(TR::CodeGenerator *cg, TR::Node *node,
  ...
  if (!disableSTRL && symbol->isStatic() && !symRef->isUnresolved() &&
      !cg->comp()->compileRelocatableCode() &&
      !node->getOpCode().isIndirect() &&
      !cg->getConditionalMovesEvaluationMode()) {
    uintptrj_t staticAddress =
    ...
    TR_ASSERT(!node->getType().isInt64() || TR::Compiler->target.is64Bit() ||
              ...
              "Helper cannot handle 64bit loads on 32-bit platforms.");
    TR::InstOpCode::Mnemonic op =
        (node->getType().isInt64() && cg->use64BitRegsOn32Bit())
            ? TR::InstOpCode::STGRL
            : (node->getType().isInt32())
                  ? TR::InstOpCode::STRL
                  : TR::InstOpCode::getStoreRelativeLongOpCode();
 ...
 * values, if it can
 * find an immediate in the 16 bit range, and the correct hardware prerequisites
 * are met.
 * Otherwise it returns a value to indicate another method must be used to move
 * one value from
...
void traverseRegisterStoreTree(TR::Compilation *comp, TR::Node *node,
                               int32_t regNum, int32_t &loadCount,
                               bool &doCopy) {
  int32_t childNum = 0;
  TR::Node *child = NULL;
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
    ...
    if (child->getOpCode().isMul() || child->getOpCode().isDiv() ||
...
void storeToRegStarNodeHelper(TR::Node *node, TR::Node *valueChild,
                              TR::CodeGenerator *cg) {}

void storeToStaticBaseNodeHelper(TR::Node *node, TR::Node *valueChild,
                                 TR::CodeGenerator *cg,
                                 OMR::RealRegister *staticRegister) {
  TR::Register *srcRegister = cg->evaluate(valueChild);
  // TR_Register *srcRegister = cg->gprClobberEvaluate(valueChild, false);
  TR::Register *tempReg;

  TR_ASSERT((srcRegister->getAssignedRegister() == NULL ||
             srcRegister->getAssignedRegister() == staticRegister),
            " static base is assigned with a different register than the "
            "locked static register");
  ...
  if (valueChild->getReferenceCount() == 1) {
    ...
    if (node->getSymbol()->getSize() == 8) {
      tempReg = cg->allocate64bitRegister();
      generateRRInstruction(cg, TR::InstOpCode::LGR, node, tempReg,
                            srcRegister);
    } else {
      tempReg = cg->allocateRegister();
 ...
 * counts of the storeNode subtree.
 *     If this transformation fails the state of the storeNode subtree remains
 * unchanged.
 */
    ...
    if (!storeNode->getOpCode().isReverseLoadOrStore() &&
      ...
      if (valueNode->getOpCode().isLoadVar() &&
        ...
        if (valueNode->getOpCode().isIndirect() ||
            !valueNode->getSymbolReference()->getSymbol()->isRegisterSymbol()) {
        ...
        if (valueNode->getOpCode().isIntegralLoadVar() &&
          ...
          if (valueNode->getSize() > storeNode->getSize()) {
            if (valueNode->getOpCode().isIndirect() ||
              ...
              // Force the memory references to not use an index register
              // because MVC is an SS instruction
              TR::MemoryReference *targetMemRef =
                  generateS390MemoryReference(storeNode, cg, false);
              TR::MemoryReference *sourceMemRef =
                  generateS390MemoryReference(valueNode, cg, false);
  ...
  if (directToMemoryAddHelper(cg, node, valueChild, TR::InstOpCode::ASI)) {
    ...
    if (sourceRegister != NULL) {
      if (isReversed) {
        ...
        generateRSInstruction(cg, TR::InstOpCode::STAM, node, sourceRegister,
 ...
 */
TR::MemoryReference *lstoreHelper(TR::Node *node, TR::CodeGenerator *cg,
                                  bool isReversed) {
  TR_ASSERT(TR::Compiler->target.is32Bit(),
            "must call 64bit version: lstoreHelper64(...)");
  ...
  TR::MemoryReference *highMR =
      generateS390MemoryReference(*lowMR, lowMR->getDisp() + 4, cg);

  if (valueChild->getOpCodeValue() == TR::lconst &&
    ...
    TR::Register *highRegister = cg->allocateRegister();
    TR::Register *lowRegister = cg->allocateRegister();
    TR::RegisterPair *valueReg =
        cg->allocateConsecutiveRegisterPair(lowRegister, highRegister);
    ...
    if (isReversed) {
      ...
      generateRXInstruction(cg, TR::InstOpCode::STRV, node, highRegister,
                            highMR);
    } else {
...
TR::MemoryReference *lstoreHelper64(TR::Node *node, TR::CodeGenerator *cg,
                                    bool isReversed) {
  TR_ASSERT(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(),
            "must call 32bit version -- lstoreHelper(...)");
  TR::Node *valueChild;
  TR::Node *addrChild = NULL;
    ...
    if (valueReg != NULL) {
      if (isReversed) {
        ...
        if (regPair && valueReg->getKind() != TR_GPR64) {
          generateRSInstruction(cg, TR::InstOpCode::STM, node, valueReg,
                                longMR);
        } else if (node->getSymbolReference()->getSymbol()->getSize() == 5) {
          TR::MemoryReference *lowMR =
              generateS390MemoryReference(*longMR, 1, cg);
          generateRSYInstruction(cg, TR::InstOpCode::STCMH, node, valueReg,
                                 (uint32_t)0x1, longMR);
          generateRSInstruction(cg, TR::InstOpCode::STCM, node, valueReg,
                                (uint32_t)0xF, lowMR);
        } else if (node->getSymbolReference()->getSymbol()->getSize() == 6) {
          TR::MemoryReference *lowMR =
              generateS390MemoryReference(*longMR, 2, cg);
          generateRSYInstruction(cg, TR::InstOpCode::STCMH, node, valueReg,
                                 (uint32_t)0x3, longMR);
          generateRSInstruction(cg, TR::InstOpCode::STCM, node, valueReg,
                                (uint32_t)0xF, lowMR);
          ...
          TR::MemoryReference *lowMR =
              generateS390MemoryReference(*longMR, 3, cg);
          generateRSYInstruction(cg, TR::InstOpCode::STCMH, node, valueReg,
                                 (uint32_t)0x7, longMR);
          generateRSInstruction(cg, TR::InstOpCode::STCM, node, valueReg,
                                (uint32_t)0xF, lowMR);
...
TR::MemoryReference *astoreHelper(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (cg->isAddressOfStaticSymRefWithLockedReg(node->getSymbolReference())) {
    storeToStaticBaseNodeHelper(
        node, valueChild, cg,
        cg->getS390Linkage()->getStaticBaseRealRegister());
  } else if (cg->isAddressOfPrivateStaticSymRefWithLockedReg(
                 node->getSymbolReference())) {
    storeToStaticBaseNodeHelper(
        node, valueChild, cg,
        cg->getS390Linkage()->getPrivateStaticBaseRealRegister());
  ...
  if (!relativeLongStoreHelper(cg, node, valueChild)) {
    ...
    // directly among memory
             ...
             !(cg->isAddressOfStaticSymRefWithLockedReg(
                 valueChild->getSymbolReference())) &&
             !(cg->isAddressOfPrivateStaticSymRefWithLockedReg(
                 valueChild->getSymbolReference())) &&
    ...
    TR_ASSERT(!(sourceRegister && sourceRegister->isArGprPair()),
              "not expecting AR reg pair");
    ...
    if (storeOp == TR::InstOpCode::STCM)
      ...
      if (cg->getConditionalMovesEvaluationMode())
        generateRSInstruction(
            cg, (storeOp == TR::InstOpCode::STG) ? TR::InstOpCode::STOCG
                                                 : TR::InstOpCode::STOC,
 ...
 */
bool OMR::Z::TreeEvaluator::genNullTestForCompressedPointers(
    TR::Node *node, TR::CodeGenerator *cg, TR::Register *targetRegister,
    ...
    if (!isNonZero) {
      ...
      if (addOrSubNode->getFirstChild()->getOpCode().isShift() &&
        ...
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::getCmpOpCode(), node, r, (signed char)0,
            TR::InstOpCode::COND_BERC, skipAdd);
      } else {
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::getCmpOpCode(), node, targetRegister,
            (signed char)0, TR::InstOpCode::COND_BERC, skipAdd);
      }

      // determine where internal control flow begins by looking for the first
      // branch
      ...
      while (next != NULL && !next->isBranchOp())
        next = next->getNext();
      TR_ASSERT(next != NULL, "Could not find branch instruction where "
                              "internal control flow begins");
 ...
 * Cause assert
 */
TR::Register *OMR::Z::TreeEvaluator::badILOpEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
...
TR::Register *OMR::Z::TreeEvaluator::aiaddEvaluator(TR::Node *node,
  ...
  // this is partial extract from enforceDisplacementLimit in order to remove
  // the need for the final LA in some cases
  if (aiaddMR->getOffset() >= MAXLONGDISP &&
      aiaddMR->getOffset() <= TR::getMaxSigned<TR::Int32>() &&
      TR::Compiler->target.is32Bit()) {
    skipFinalLA = true;
    TR::MemoryReference *tempMR = generateS390MemoryReference(
        aiaddMR->getBaseRegister(), aiaddMR->getIndexRegister(), 0, cg);
    tempMR->setSymbolReference(aiaddMR->getSymbolReference());
    generateRXInstruction(cg, TR::InstOpCode::LA, node, targetRegister, tempMR);
    generateS390ImmOp(cg, TR::InstOpCode::getAddOpCode(), node, targetRegister,
                      targetRegister, (int32_t)aiaddMR->getOffset());
  } else {
  ...
  if (!skipFinalLA) {
    TR::Instruction *laInst = generateRXInstruction(
        cg, TR::InstOpCode::LA, node, targetRegister, aiaddMR);
    TR::Register *baseReg = aiaddMR->getBaseRegister();
    if (aiaddMR->getOffset() == 0 && aiaddMR->getIndexRegister() == NULL &&
        baseReg &&
        cg->getBucketPlusIndexRegisters().ValueAt(baseReg->getIndex())) {
      aiaddMR->setBucketBaseRegMemRef();
      if (cg->traceBCDCodeGen())
        traceMsg(comp, "\tfound baseReg %s (idx=%d) for node %s (%p) in "
                       "BucketPlusIndexRegisters setBucketBaseRegMemRef on LA "
                       "inst %p memRef\n",
                 cg->getDebug()->getName(baseReg), baseReg->getIndex(),
                 node->getOpCode().getName(), node, laInst);
    }
  }
...
TR::Register *OMR::Z::TreeEvaluator::bstoreEvaluator(TR::Node *node,
  ...
  //     bconst
           ...
           performTransformation(comp, "O^O for bstore [%p], emitting single "
...
TR::Register *OMR::Z::TreeEvaluator::z990PopCountHelper(TR::Node *node,
  ...
  generateRIInstruction(cg,
                        TR::Compiler->target.is64Bit() ? TR::InstOpCode::LGHI
                                                       : TR::InstOpCode::LHI,
                        node, outReg, 0);
  generateRIInstruction(cg,
  ...
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_JE, node, returnLabel);
  cursor->setStartInternalControlFlow();
  ...
  generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, returnLabel,
                               regDeps);
...
extern TR::Register *inlineNumberOfTrailingZeros(TR::Node *node,
  ...
  if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit() &&
      argReg->getRegisterPair()) {
    // reconstruct a GPR 64 from reg pair
    TR::Register *tempReg = cg->allocate64bitRegister();
    generateRSInstruction(cg, TR::InstOpCode::SLLG, node, tempReg,
                          argReg->getHighOrder(), 32);
    generateRRInstruction(cg, TR::InstOpCode::LR, node, tempReg,
                          argReg->getLowOrder());
...
/**
 * Used for
 * com.ibm.dataaccess.ByteArrayUtils.inlineTrailingZerosQuadWordAtATime_
 */
extern TR::Register *inlineTrailingZerosQuadWordAtATime(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
#define iComment(str)                                                          \
  if (debug)                                                                   \
  ...
  */
  ...
  TR::LabelSymbol *LabelProcessNext8Bytes =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  ...
  const int valueContentOffset =
      TR::Compiler->om.contiguousArrayHeaderSizeInBytes();

  TR::RegisterDependencyConditions *regDeps = new (cg->trHeapMemory())
      TR::RegisterDependencyConditions(0, numPostDeps, cg);
  ...
  if (cg->supportsHighWordFacility() &&
      !comp->getOption(TR_DisableHighWordRA) && is64) {
  ...
  cursor = generateRILInstruction(cg, TR::InstOpCode::NIHF, node, rOffset,
                                  (uintptrj_t)0x0);
  cursor = generateRXInstruction(
      cg, TR::InstOpCode::getLoadAddressOpCode(), node, rInputByteArray,
      generateS390MemoryReference(rInputByteArray, valueContentOffset, cg));
  iComment("get to the actual content of the byte array");
  cursor = generateRSInstruction(
      cg, TR::InstOpCode::getShiftRightLogicalSingleOpCode(), node,
      rBranchCounter, rOffset, 3);
  iComment("divide by 8 to use BCTRG");

  /* ===================== Core loop : double-word i.e 8 bytes at a time
   * processing =====================*/

  // Can use CLGHSI and BRXLE for this. It will get rid of the LG/L + CIJ combo.
  // We also won't need LAY anymore since BRXLE will take care of it.
  /*
  loop:
     CLGHSI
     BRXLE
  */
  // LabelProcessNext8Bytes: we don't actually count trailing zeros in a
  // register since we can do some math on rOffset to find out where mismatch
  // occurs later on
  cursor = generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                        LabelProcessNext8Bytes);
  iComment("====== core loop ======");
  cursor->setStartInternalControlFlow();

  cursor = generateRXInstruction(
      cg, TR::InstOpCode::getLoadOpCode(), node, rInput,
      generateS390MemoryReference(rInputByteArray, rOffset, 0, cg));
  iComment("next double-word");
  cursor = generateRIEInstruction(
      cg, is64 ? TR::InstOpCode::CGIJ : TR::InstOpCode::CIJ, node, rInput,
      (int8_t)0x0, LabelDone, TR::InstOpCode::COND_BNE);
  iComment("byte is non-zero?");
  cursor = generateRXInstruction(
      cg, TR::InstOpCode::getLoadOpCode(), node, rInput,
      generateS390MemoryReference(rInputByteArray, rOffset, 8, cg));
  iComment("next double-word");
  cursor = generateRIEInstruction(
      cg, is64 ? TR::InstOpCode::CGIJ : TR::InstOpCode::CIJ, node, rInput,
      (int8_t)0x0, LabelDone, TR::InstOpCode::COND_BNE);
  iComment("byte is non-zero?");
  cursor = generateRXInstruction(cg, TR::InstOpCode::LAY, node, rOffset,
                                 generateS390MemoryReference(rOffset, -16, cg));
  cursor = generateS390BranchInstruction(
      cg, is64 ? TR::InstOpCode::BRCTG : TR::InstOpCode::BRCT, node,
      rBranchCounter, LabelProcessNext8Bytes);

  // doneLabel : we get here in 3 cases: i. if no trailing non-zeros were found
  // ii. if trailing non-zero was found in core-loop iii. if trailing non-zero
  // was found in residue processing
  cursor =
  ...
  cursor = generateRXInstruction(
      cg, TR::InstOpCode::LAY, node, rInputByteArray,
      generateS390MemoryReference(rInputByteArray, -valueContentOffset, cg));
  iComment("reset the byte array adress to original location");
...
extern TR::Register *inlineHighestOneBit(TR::Node *node, TR::CodeGenerator *cg,
  ...
  if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit() && isLong) {
    ...
    cursor = generateRSInstruction(cg, TR::InstOpCode::SLLG, node,
                                   srcReg->getHighOrder(),
                                   srcReg->getHighOrder(), 32);

    cursor =
        generateRRInstruction(cg, TR::InstOpCode::LR, node,
                              srcReg->getHighOrder(), srcReg->getLowOrder());
  ...
  if (!isLong) {
    cursor = generateRILInstruction(cg, TR::InstOpCode::NIHF, node, srcReg,
                                    (uintptrj_t)0x00000000);
  }

  // generate the FLOGR
  if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit() && isLong)
    cursor = generateRRInstruction(cg, TR::InstOpCode::FLOGR, node, pairReg,
                                   srcReg->getHighOrder());
  else
    cursor =
  ...
  if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit() && isLong)
    cursor = generateRRInstruction(cg, TR::InstOpCode::XGR, node, retReg,
                                   srcReg->getHighOrder());
  else
    cursor =
        generateRRInstruction(cg, TR::InstOpCode::XGR, node, retReg, srcReg);
  ...
  if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit() && isLong) {
    ...
    TR::Register *retPairReg =
        cg->allocateConsecutiveRegisterPair(oddRetReg, evenRetReg); // low, high

    // now utilize only lower 32-bits of each reg
    cursor = generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                   retPairReg->getLowOrder(), retReg);

    cursor = generateRSInstruction(cg, TR::InstOpCode::SRLG, node, retReg,
                                   retReg, 32);

    cursor = generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                   retPairReg->getHighOrder(), retReg);
    ...
    if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit() &&
        isLong) {
      flogrDeps = new (cg->trHeapMemory())
          TR::RegisterDependencyConditions(0, 3 + 7, cg);
      flogrDeps->addPostCondition(srcReg, TR::RealRegister::EvenOddPair);
      flogrDeps->addPostCondition(srcReg->getHighOrder(),
                                  TR::RealRegister::LegalEvenOfPair);
      flogrDeps->addPostCondition(srcReg->getLowOrder(),
                                  TR::RealRegister::LegalOddOfPair);
    } else {
      flogrDeps = new (cg->trHeapMemory())
          TR::RegisterDependencyConditions(0, 1 + 7, cg);
    ...
    */
    ...
    if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit() &&
        isLong) {
      flogrDeps = new (cg->trHeapMemory())
          TR::RegisterDependencyConditions(0, 3 + 4, cg);
      flogrDeps->addPostCondition(srcReg, TR::RealRegister::EvenOddPair);
      flogrDeps->addPostCondition(srcReg->getHighOrder(),
                                  TR::RealRegister::LegalEvenOfPair);
      flogrDeps->addPostCondition(srcReg->getLowOrder(),
      ...
      flogrDeps = new (cg->trHeapMemory())
          TR::RegisterDependencyConditions(0, 1 + 4, cg);
      flogrDeps->addPostCondition(srcReg, TR::RealRegister::AssignAny);
    }
...
inlineNumberOfLeadingZeros(TR::Node *node, TR::CodeGenerator *cg, bool isLong) {
  ...
  if (!isLong) {
    ...
    cg->stopUsingRegister(argReg);
    argReg = tempReg;
  } else if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit() &&
             argReg->getRegisterPair()) {
    // reconstruct a GPR 64 from reg pair
    TR::Register *tempReg = cg->allocate64bitRegister();
    generateRSInstruction(cg, TR::InstOpCode::SLLG, node, tempReg,
...
static TR::Register *readKMCOffset(TR::Node *child, TR::Node *node,
                                   TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit()) {
...
readKMCPair(TR::Node *node, TR::Node *baseArr, TR::Register *offset,
            TR::Register *length, TR::CodeGenerator *cg,
            TR::RegisterDependencyConditions *conditions) {
  TR::Register *arr = cg->gprClobberEvaluate(baseArr);
  TR::RegisterPair *arrRegisterPair =
      cg->allocateConsecutiveRegisterPair(length, arr);
  ...
  TR::MemoryReference *dataOffset = generateS390MemoryReference(
      arr, offset, TR::Compiler->om.contiguousArrayHeaderSizeInBytes(), cg);
...
TR::Register *inlineZosKMC(TR::InstOpCode::Mnemonic opcode, TR::Node *node,
                           TR::CodeGenerator *cg, NumCryptArrays numArrays) {
  TR_ASSERT(node->getNumChildren() == 5 + numArrays,
            "Call to TR::com_ibm_jit_crypto_JITFullHardwareCrypt_z_km* expects "
            "8 children.");
  ...
  conditions = new (cg->trHeapMemory())
      TR::RegisterDependencyConditions(0, 5 + numArrays, cg);
  redoLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  doneLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg);

  /*
   * zos_km_native   (in, inputLength, inputOffset, out, outputOffset,
   * _IV_And_key, modeCode);
   * zos_kmctr_native(in, inputLength, inputOffset, out, outputOffset, ctr,
   * ctrOffset, _IV_And_key, modeCode);
   */
  ...
  parmOffset = generateS390MemoryReference(
      parm, TR::Compiler->om.contiguousArrayHeaderSizeInBytes(), cg);
  generateRXInstruction(cg, TR::InstOpCode::LA, node, parm, parmOffset);

  switch (
      numArrays) { // notice how break statements are _intentionally_ left out
  case THREE_ARRAYS_CRYPT_CTR:
    ctrOffsetReg = readKMCOffset(node->getChild(6), node, cg);
    ctrRegisterPair = readKMCPair(node, node->getChild(5), ctrOffsetReg, dummy2,
                                  cg, conditions);
  case TWO_ARRAYS_CRYPT:
    outOffsetReg = readKMCOffset(node->getChild(4), node, cg);
    outRegisterPair = readKMCPair(node, node->getChild(3), outOffsetReg, dummy1,
                                  cg, conditions);
  case ONE_ARRAY_SHA:
    inOffsetReg = readKMCOffset(node->getChild(2), node, cg);
    inRegisterPair =
        readKMCPair(node, node->getChild(0), inOffsetReg, len, cg, conditions);
  }

  iCursor = generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                         redoLabel, conditions);

  if (numArrays == THREE_ARRAYS_CRYPT_CTR)
    iCursor = generateRRFInstruction(cg, opcode, node, outRegisterPair,
                                     inRegisterPair, ctrRegisterPair, iCursor);
  else if (numArrays == TWO_ARRAYS_CRYPT)
    iCursor = generateRREInstruction(cg, opcode, node, outRegisterPair,
                                     inRegisterPair, conditions, iCursor);
  else // ONE_ARRAY_SHA
    iCursor = generateRREInstruction(cg, opcode, node, inRegisterPair,
                                     inRegisterPair, conditions, iCursor);

  iCursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                          TR::InstOpCode::COND_MASK1, node,
                                          redoLabel, conditions, iCursor);
...
void inline128Multiply(TR::Node *node, TR::CodeGenerator *cg,
  ...
  // not to have internal control flow, RA can do its job properly, so mostly
  // try not to reuse registers
...
TR::Register *inline256Multiply(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  TR::Register *rZ10 = cg->allocate64bitRegister(); // unique name to avoid
                                                    // const conflict in J9 VM
...
TR::Register *inlineSIMDP256KaratsubaMultiply(TR::Node *node,
   ...
   * - [highZ2,lowZ2]
   *                = Asum_high*Bsum_high    // 1 bit with correct shift (2
   * vector shift)
...
#define p256digit(x)                                                           \
  ...
  TR::MemoryReference *partialOperands[10][8] = {
      ...
      /*S3*/ {NULL, NULL, NULL, p256digit(12), p256digit(13), p256digit(14),
              p256digit(15), NULL},
      /*S2*/ {NULL, NULL, NULL, p256digit(11), p256digit(12), p256digit(13),
              p256digit(14), p256digit(15)},
      /*S2*/ {NULL, NULL, NULL, p256digit(11), p256digit(12), p256digit(13),
              p256digit(14), p256digit(15)},
      /*S1*/ {p256digit(0), p256digit(1), p256digit(2), p256digit(3),
              p256digit(4), p256digit(5), p256digit(6), p256digit(7)},
      /*S4*/ {p256digit(8), p256digit(9), p256digit(10), NULL, NULL, NULL,
              p256digit(14), p256digit(15)},
      /*S5*/ {p256digit(9), p256digit(10), p256digit(11), p256digit(13),
              p256digit(14), p256digit(15), p256digit(13), p256digit(8)},
      // Subtract the following
      /*S6*/ {p256digit(11), p256digit(12), p256digit(13), NULL, NULL, NULL,
              p256digit(8), p256digit(10)},
      /*S7*/ {p256digit(12), p256digit(13), p256digit(14), p256digit(15), NULL,
              NULL, p256digit(9), p256digit(11)},
      /*S8*/ {p256digit(13), p256digit(14), p256digit(15), p256digit(8),
              p256digit(9), p256digit(10), NULL, p256digit(12)},
      /*S9*/ {p256digit(14), p256digit(15), NULL, p256digit(9), p256digit(10),
...
void generateLongDoubleStore(TR::Node *node, TR::CodeGenerator *cg,
                             TR::Register *reg, TR::Register *addressReg) {
  ...
  generateRSInstruction(cg, TR::InstOpCode::STD, node, reg->getHighOrder(),
                        lowRef);
  lowRef->stopUsingMemRefRegister(cg);
  generateRSInstruction(cg, TR::InstOpCode::STD, node, reg->getLowOrder(),
                        highRef);
...
TR::Register *inlineLongReverseBytes(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    ...
    generateRREInstruction(cg, TR::InstOpCode::LRVR, node,
                           trgReg->getLowOrder(), srcReg->getHighOrder());
    generateRREInstruction(cg, TR::InstOpCode::LRVR, node,
                           trgReg->getHighOrder(), srcReg->getLowOrder());
  }
  node->setRegister(trgReg);
  return trgReg;
}

extern TR::Register *forwardArraycopyEvaluator(TR::Node *node,
                                               TR::CodeGenerator *cg) {
  // edTODO
  //   Options to disable
  ...
  /*
  This version of arraycopy only works in simple cases i.e arraycopy has 3
  children:
  source
  dest
  length (in bytes)
  ...
  */
  ...
  TR_ASSERT(node->getNumChildren() == 3,
            "vectorized arraycopy can only have 3 children");
...
#define iComment(str)                                                          \
  ...
  TR::Register *rSrc = cg->evaluate(node->getChild(0)); // The actual char array
                                                        // storing the contents
                                                        // of j.l.S.value of
                                                        // input String
  TR::Register *rDst = cg->evaluate(node->getChild(1)); // The actual char array
                                                        // storing the contents
                                                        // of j.l.S.value of
                                                        // output String
  TR::Register *rLen =
  ...
  TR::Register *rIdx = cg->allocateRegister(); // Used to iterate over input
                                               ...
                                               // overlap)
  TR::Register *rResidue =
      cg->allocateRegister(); // rResidue after alignment of length to 16
  TR::Register *rLoopCounter = cg->allocateRegister();
  TR::Register *rResult =
      cg->allocateCollectedReferenceRegister(); // To return the reg containing
                                                // reference to output String

  TR::Register *vBuf =
      cg->allocateRegister(TR_VRF); // Temp buffer used to store output of
                                    // upper/lower case conversion

  TR_Debug *debug = cg->getDebug();
  TR::Compilation *comp = cg->comp();

  const int sizeOfVector = 16; // in Bytes. Should really make this dynamic (i.e
                               // read from some machine() query since this can
                               // increase in future)
  ...
  int32_t indexScale =
      usesCompressedrefs ? 2 : TR::Compiler->target.is64Bit() ? 3 : 2;
  ...
  TR::LabelSymbol *processNext16Bytes =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *doneLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg);

  processNext16Bytes->setStartInternalControlFlow(); // ICF ends at the BRCT

  generateRRInstruction(cg, TR::InstOpCode::getXORRegOpCode(), node, rIdx,
                        rIdx);

  /* ===================== Step 2: Length calculations (rResidue and 16
   * alignment) =====================*/

  // The core loop works in multiples of 16 so we need to align the length to
  // 16, and also load the rResidue so we can operate on them separately
  ...
  iComment("Calc rResidue (can eq length if length < 16)");
  cursor = generateRSInstruction(cg, TR::InstOpCode::SRLG, node, rLoopCounter,
                                 rLen, 4);
  iComment("reduce len to multiple of 16 (loop counter)");
  cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                         TR::InstOpCode::COND_BZ, node,
                                         processNext16Bytes);
  iComment("Len perfect multiple of 16, goto main loop (rare)"); // go to main
                                                                 // loop if no
                                                                 // rResidue
                                                                 // from result
                                                                 // of NI 0xF
  ...
  // Process residue
  cursor =
      generateVRSbInstruction(cg, TR::InstOpCode::VLL, node, vBuf, rResidue,
                              generateS390MemoryReference(rSrc, rIdx, 0, cg));
  cursor = generateVRSbInstruction(
      cg, TR::InstOpCode::VSTL, node, vBuf, rResidue,
      generateS390MemoryReference(rDst, rIdx, 0, cg),
      0); // Intentionally use srcIndex since we only want to increment 1 reg

  // If len < 16 then we're done else continue to main loop that does 16 byte at
  // a time
  // eds : add getCompareImmAndBranchOpCode(cg) for CIJ, CGIJ.
  cursor = generateRIEInstruction(cg, TR::InstOpCode::CIJ, node, rLen,
                                  (int8_t)sizeOfVector, doneLabel,
                                  TR::InstOpCode::COND_BNH);
  iComment("len <= 16 ? we're done..") cursor =
      generateRRInstruction(cg, TR::InstOpCode::AR, node, rIdx, rResidue);
  iComment("Add # rResidue code-points to index");
  ...
  cursor = generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                        processNext16Bytes);
  iComment("Core loop - works in chunks of 16 bytes");

  cursor =
      generateVRXInstruction(cg, TR::InstOpCode::VL, node, vBuf,
                             generateS390MemoryReference(rSrc, rIdx, 0, cg));
  cursor = generateVRXInstruction(
      cg, TR::InstOpCode::VST, node, vBuf,
      generateS390MemoryReference(rDst, rIdx, 0, cg), 0, cursor);
  cursor = generateRXInstruction(
      cg, TR::InstOpCode::getLoadAddressOpCode(), node, rIdx,
      generateS390MemoryReference(rIdx, sizeOfVector, cg), cursor);
  iComment("Src/dst index += 16");
  cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRCT, node,
                                         rLoopCounter, processNext16Bytes);
  iComment("branch if numLoopIterations-- == 0");
  cursor->setDependencyConditions(regDeps);
 ...
 * compareTarget is NULL and needResultReg is false.
 * Notice that
...
TR::Register *OMR::Z::TreeEvaluator::arraycmpHelper(
  ...
  bool isIfxcmpBrCondContainEqual =
      getMaskForBranchCondition(ifxcmpBrCond) & 0x80;
  ...
  if ((isConstLength && (length == 0)) || (source1Node == source2Node)) {
    ...
    if (length <= (earlyCLCEnabled ? earlyCLCThreshold : TR_MAX_CLC_SIZE) &&
      ...
      if (length > (earlyCLCEnabled ? earlyCLCThreshold
                                    : TR_MAX_CLC_SIZE)) // NOTE: similar logic
                                                        // below when actually
                                                        // adding to regDeps
      {
      ...
      if (length > (earlyCLCEnabled ? earlyCLCThreshold : TR_MAX_CLC_SIZE)) {
        ...
        if (baseSource1Ref->getBaseRegister())
          regDeps->addPostConditionIfNotAlreadyInserted(
              baseSource1Ref->getBaseRegister()->getGPRofArGprPair(),
              TR::RealRegister::AssignAny);
        ...
        if (baseSource2Ref->getBaseRegister() &&
            (baseSource2Ref->getBaseRegister() !=
             baseSource1Ref->getBaseRegister()))
          regDeps->addPostConditionIfNotAlreadyInserted(
              baseSource2Ref->getBaseRegister()->getGPRofArGprPair(),
              TR::RealRegister::AssignAny);
      }
    }

    TR::LabelSymbol *condLabel =
        isCondLabelNeeded ? TR::LabelSymbol::create(cg->trHeapMemory(), cg)
                          : NULL;

    TR::LabelSymbol *endLabel =
        isEndLabelNeeded ? TR::LabelSymbol::create(cg->trHeapMemory(), cg)
                         : NULL;
      ...
      if (needResultReg) {
        if (return102) {
          ...
          generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                        ...
                                        endLabel);
          cursor = generateRIInstruction(cg, TR::Compiler->target.is64Bit()
                                                 ? TR::InstOpCode::LGHI
                                                 : TR::InstOpCode::LHI,
                                         node, retValReg, 1);
        } else {
    ...
    if (maxLenIn256) {
      ...
      lengthReg =
          evaluateLengthMinusOneForMemoryCmp(lengthNode, cg, true, lenMinusOne);
    ...
    if (maxLenIn256) {
      ...
      if (!lenMinusOne) {
        generateRIInstruction(cg, (needs64BitOpCode) ? TR::InstOpCode::AGHI
                                                     : TR::InstOpCode::AHI,
                              node, lengthReg, -1);
      }
      ...
      if (lengthCanBeZero) {
        if (lenMinusOne)
          generateRRInstruction(cg, (needs64BitOpCode) ? TR::InstOpCode::LTGR
                                                       : TR::InstOpCode::LTR,
      ...
      if (isWideChar) {
        ...
        // including isWideChar via CLCLE/CLCLU
        // Now it is used only for WideChar as CLC loop is prefered on other
        // general cases
        ...
        generateRRInstruction(cg, TR::Compiler->target.is64Bit()
                                      ? TR::InstOpCode::LTGR
                                      : TR::InstOpCode::LTR,
                              node, lengthReg, lengthReg);
        ...
        if (lengthCanBeZero) {
          ...
          if (isFoldedIf) {
            if (isIfxcmpBrCondContainEqual)
              // generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
              // TR::InstOpCode::COND_BLRC, node, compareTarget);
              setStartInternalControlFlow(
                  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                                TR::InstOpCode::COND_BLRC, node,
                                                compareTarget),
        ...
        TR::LabelSymbol *topOfLoop =
            TR::LabelSymbol::create(cg->trHeapMemory(), cg);
        TR::LabelSymbol *bottomOfLoop =
            TR::LabelSymbol::create(cg->trHeapMemory(), cg);
        TR::LabelSymbol *outOfCompare =
            TR::LabelSymbol::create(cg->trHeapMemory(), cg);
        ...
        generateRXInstruction(cg, TR::InstOpCode::LA, node,
                              source1Reg->getGPRofArGprPair(),
                              generateS390MemoryReference(
                                  source1Reg->getGPRofArGprPair(), 256, cg));
        generateRXInstruction(cg, TR::InstOpCode::LA, node,
                              source2Reg->getGPRofArGprPair(),
                              generateS390MemoryReference(
                                  source2Reg->getGPRofArGprPair(), 256, cg));
    ...
    if (isFoldedIf) {
      ...
      if (needResultReg) {
        if (return102) {
          ...
          generateRIInstruction(cg, TR::Compiler->target.is64Bit()
                                        ? TR::InstOpCode::LGHI
                                        : TR::InstOpCode::LHI,
                                node, retValReg, 1);
        } else {
...
TR::Register *OMR::Z::TreeEvaluator::memcmpEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
...
TR::Register *OMR::Z::TreeEvaluator::ixfrsEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  ...
  if (isConstValue && isConstSign) {
    ...
    generateRRInstruction(cg,
                          TR::Compiler->target.is64Bit() ? TR::InstOpCode::LGR
                                                         : TR::InstOpCode::LR,
                          node, valueReg, signReg);
    if (TR::Compiler->target.is64Bit())
      generateS390ImmOp(cg, TR::InstOpCode::XG, node, valueReg, valueReg,
                        (int64_t)(abs(value)));
    else
      generateS390ImmOp(cg, TR::InstOpCode::X, node, valueReg, valueReg,
                        (int32_t)(abs(value)));
    generateRRInstruction(cg,
    ...
    generateRSInstruction(cg, TR::InstOpCode::SRA, node, signReg, 31);
    generateRRInstruction(cg,
                          TR::Compiler->target.is64Bit() ? TR::InstOpCode::XGR
                                                         : TR::InstOpCode::XR,
                          node, valueReg, signReg);
    generateRRInstruction(cg,
                          TR::Compiler->target.is64Bit() ? TR::InstOpCode::SLGR
...
TR::Register *OMR::Z::TreeEvaluator::lxfrsEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Node *valueNode = node->getChild(0);
  TR::Node *signNode = node->getChild(1);
  ...
  if (isConstValue && isConstSign) {
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      TR::Register *valuePair =
          cg->allocateConsecutiveRegisterPair(valueLowReg, valueHighReg);
      generateLoad32BitConstant(cg, node, (uint32_t)(unsignedResult >> 32),
                                valueHighReg, true);
      generateLoad32BitConstant(
          cg, node, (uint32_t)(unsignedResult & 0xFFFFFFFF), valueLowReg, true);
    ...
    if (TR::Compiler->target.is64Bit()) {
      if (sign < 0)
        generateRRInstruction(cg, TR::InstOpCode::LNGR, node, valueReg,
                              valueReg);
      else
        generateRRInstruction(cg, TR::InstOpCode::LPGR, node, valueReg,
                              valueReg);
      ...
      generateRRInstruction(cg, TR::InstOpCode::LR, node, valueHighCopyReg,
                            valueHighReg);
      generateRSInstruction(cg, TR::InstOpCode::SRA, node, valueHighCopyReg,
                            31);

      generateRRInstruction(cg, TR::InstOpCode::XR, node, valueHighReg,
                            valueHighCopyReg);
      generateRRInstruction(cg, TR::InstOpCode::XR, node, valueLowReg,
                            valueHighCopyReg);
      generateRRInstruction(cg, TR::InstOpCode::SLR, node, valueLowReg,
                            valueHighCopyReg);
      generateRRInstruction(cg, TR::InstOpCode::SLBR, node, valueHighReg,
                            valueHighCopyReg);

      if (sign < 0) {
        generateS390ImmOp(cg, TR::InstOpCode::X, node, valueHighReg,
                          valueHighReg, (int32_t)0xFFFFFFFF);
        generateS390ImmOp(cg, TR::InstOpCode::X, node, valueLowReg, valueLowReg,
                          (int32_t)0xFFFFFFFF);
        ...
        generateRRInstruction(cg, TR::InstOpCode::ALR, node, valueLowReg,
                              oneTemp);
        generateRRInstruction(cg, TR::InstOpCode::ALCR, node, valueHighReg,
                              zeroTemp);
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      generateRSInstruction(cg, TR::InstOpCode::SRAG, node, signReg, signReg,
                            63);
      generateRRInstruction(cg, TR::InstOpCode::LGR, node, valueReg, signReg);
      generateS390ImmOp(cg, TR::InstOpCode::XG, node, valueReg, valueReg,
                        (int64_t)(value < 0 ? -value : value));
      ...
      TR::Register *valuePair =
          cg->allocateConsecutiveRegisterPair(valueLowReg, valueHighReg);
      generateLoad32BitConstant(cg, valueNode, valueNode->getLongIntLow(),
                                valueLowReg, true);
      generateLoad32BitConstant(cg, valueNode, valueNode->getLongIntHigh(),
                                valueHighReg, true);

      TR::RegisterDependencyConditions *regDeps =
          new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 3, cg);

      TR::Register *signPair =
          cg->allocateConsecutiveRegisterPair(signLowReg, signHighReg);
      ...
      TR::Instruction *cursor =
          generateRSInstruction(cg, TR::InstOpCode::SRDA, node, signPair,
                                63); // SRDA operates on a register pair
      cursor->setDependencyConditions(regDeps);
...
TR::Register *OMR::Z::TreeEvaluator::fxfrsEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Node *valueNode = node->getChild(0);
  TR::Node *signNode = node->getChild(1);
  ...
  if (isConstValue && isConstSign) {
    ...
    generateRegLitRefInstruction(cg, TR::InstOpCode::LE, node, resultReg,
                                 absVal);
    ...
    TR::Register *resultReg = cg->allocateRegister(TR_FPR);
    if (sign < 0.0f)
      generateRRInstruction(cg, TR::InstOpCode::LNER, node, resultReg,
                            valueFprReg);
    else
      generateRRInstruction(cg, TR::InstOpCode::LPER, node, resultReg,
                            valueFprReg);
    ...
    TR::MemoryReference *tempSlotStoreMemRef =
        generateS390MemoryReference(node, tempSlotSymRef, cg);
    TR::MemoryReference *tempSlotLoadMemRef =
        generateS390MemoryReference(node, tempSlotSymRef, cg);

    // store and load to transfer
    generateRXInstruction(cg, TR::InstOpCode::STE, node, signFprReg,
                          tempSlotStoreMemRef);
    generateRXInstruction(cg,
                          TR::Compiler->target.is64Bit() ? TR::InstOpCode::LLGF
                                                         : TR::InstOpCode::L,
                          node, signReg, tempSlotLoadMemRef);
    ...
    generateRRInstruction(cg,
                          TR::Compiler->target.is64Bit() ? TR::InstOpCode::LGR
                                                         : TR::InstOpCode::LR,
                          node, signCopyReg, signReg);

    // manipulate the bits to remove the sign
    TR::Register *complementReg = cg->allocateRegister();
    if (TR::Compiler->target.is64Bit())
      generateRRInstruction(cg, TR::InstOpCode::LLGTR, node, signCopyReg,
                            signCopyReg);
    else
      generateS390ImmOp(cg, TR::InstOpCode::N, node, signCopyReg, signCopyReg,
                        (int32_t)(0x7FFFFFFF));
    generateRRInstruction(cg,
                          TR::Compiler->target.is64Bit() ? TR::InstOpCode::LCGR
                                                         : TR::InstOpCode::LCR,
                          node, complementReg, signCopyReg);
    generateRRInstruction(cg,
                          TR::Compiler->target.is64Bit() ? TR::InstOpCode::OGR
                                                         : TR::InstOpCode::OR,
                          node, signCopyReg, complementReg);
    generateRRInstruction(cg,
                          TR::Compiler->target.is64Bit() ? TR::InstOpCode::NGR
                                                         : TR::InstOpCode::NR,
                          node, signReg, signCopyReg);
    if (TR::Compiler->target.is64Bit())
      generateS390ImmOp(cg, TR::InstOpCode::NG, node, signReg, signReg,
                        (int64_t)(0x0000000080000000LL));
    else
      generateS390ImmOp(cg, TR::InstOpCode::N, node, signReg, signReg,
                        (int32_t)(0x80000000));
    ...
    generateRXInstruction(cg, TR::InstOpCode::ST, node, signReg,
                          tempSlotStoreMemRef);
    generateRXInstruction(cg, TR::InstOpCode::LE, node, resultReg,
                          tempSlotLoadMemRef);
    ...
    // store and load to transfer
    generateRXInstruction(cg, TR::InstOpCode::STE, node, valueFprReg,
                          tempSlotStoreMemRef);
    generateRXInstruction(cg,
                          TR::Compiler->target.is64Bit() ? TR::InstOpCode::LLGF
                                                         : TR::InstOpCode::L,
                          node, valueReg, tempSlotLoadMemRef);
    ...
    if (TR::Compiler->target.is64Bit())
      generateRRInstruction(cg, TR::InstOpCode::LLGTR, node, valueReg,
                            valueReg);
    else
      generateS390ImmOp(cg, TR::InstOpCode::N, node, valueReg, valueReg,
                        (int32_t)(0x7FFFFFFF));
    generateRRInstruction(cg,
                          TR::Compiler->target.is64Bit() ? TR::InstOpCode::OGR
                                                         : TR::InstOpCode::OR,
                          node, signReg, valueReg);
...
OMR::Z::TreeEvaluator::directCallEvaluator(TR::Node *node,
  ...
  if (!cg->inlineDirectCall(node, resultReg)) {
    ...
    if (cg->supportsHighWordFacility() &&
        !cg->comp()->getOption(TR_DisableHighWordRA) &&
        TR::Compiler->target.is64Bit()) {
...
bool OMR::Z::TreeEvaluator::isCandidateForBIFEvaluation(TR::Node *node) {
  return false;
}
  ...
  if (node->getFirstChild()->getReferenceCount() == 1) {
    ...
    case TR::aiadd: {
      if (comp->getOptions()->getOption(TR_TraceCG)) {
        traceMsg(comp, " found %s [%p] with ref count 1 under treetop, "
                       "avoiding evaluation into register.\n",
    ...
    if (storageReference->isNodeBased()) {
      ...
      if (storageReference->getNode()->getOpCode().isIndirect() ||
        ...
        if (storageReference->getNodeReferenceCount() == 0 &&
          ...
          if (storageReference->getNodeReferenceCount() == 0) {
            if (cg->traceBCDCodeGen())
              traceMsg(comp, "storageReference->nodeRefCount == 0 so dec addr "
                             "child %p refCount %d->%d\n",
...
/**
 * exceptionRangeFenceEvaluator - SymbolReference is the aliasing effect,
 * initialiser is where the
 *   code address gets put when binary is generated
 */
TR::Register *
OMR::Z::TreeEvaluator::exceptionRangeFenceEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
 ...
 * a loadaddr -but- the parameter is not a complete object yet. SO, it is
 * important
...
TR::Register *OMR::Z::TreeEvaluator::loadaddrEvaluator(TR::Node *node,
  ...
  if (symRef->isUnresolved()) {
    ...
    uds = new (cg->trHeapMemory())
        TR::UnresolvedDataSnippet(cg, node, symRef, false, false);
    cg->addSnippet(uds);

    // generate branch to the unresolved data snippet
    cursor = generateRegUnresolvedSym(cg, TR::InstOpCode::getLoadOpCode(), node,
                                      targetRegister, symRef, uds);
    ...
    if (node->getSymbol()->isStatic()) {
      ...
      if (comp->getOption(TR_EnableHCR) && node->getSymbol()->isMethod() &&
        ...
        cursor = genLoadAddressConstantInSnippet(
            cg, node, (uintptrj_t)node->getSymbol()
                          ->getStaticSymbol()
                          ->getStaticAddress(),
            targetRegister, NULL, NULL, NULL, true);
      else {
        // Generate Static Address into a register
        TR::StaticSymbol *sym = node->getSymbol()->getStaticSymbol();
        if (sym && sym->isStartPC()) {
          cursor =
              generateRILInstruction(cg, TR::InstOpCode::LARL, node,
                                     targetRegister, node->getSymbolReference(),
          ...
          cursor =
              genLoadAddressConstant(cg, node, (uintptrj_t)node->getSymbol()
                                                   ->getStaticSymbol()
                                                   ->getStaticAddress(),
                                     targetRegister);
        }
...
OMR::Z::TreeEvaluator::passThroughEvaluator(TR::Node *node,
  ...
  if (!targetRegister->getRegisterPair() && node->getReferenceCount() > 1 &&
    ...
    if (cg->supportsHighWordFacility() &&
      ...
      if (cg->machine()->getHPRFromGlobalRegisterNumber(
              node->getGlobalRegisterNumber()) != NULL) {
...
TR::Register *OMR::Z::TreeEvaluator::BBStartEvaluator(TR::Node *node,
  ...
  if (!block->isSuperCold() && comp->getFirstColdInstruction() != NULL) {
        ...
        if (sym != NULL) {
          ...
          if ((TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit() ||
              ...
              !(TR::Compiler->target.isLinux() &&
                TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit() &&
                type.isAggregate() && sym->getSize() > 4 &&
                sym->getSize() <= 8)) {
            sym->setAllocatedIndex(cg->getGlobalRegister(
                child->getChild(i)->getGlobalRegisterNumber()));
          } else {
            sym->setAllocatedHigh(cg->getGlobalRegister(
                child->getChild(i)->getHighGlobalRegisterNumber()));
            sym->setAllocatedLow(cg->getGlobalRegister(
  ...
  // TODO remove this dependency once we can get the same dependency on the
  // TR::InstOpCode::PROC instruction
...
TR::Register *OMR::Z::TreeEvaluator::BBEndEvaluator(TR::Node *node,
  ...
  if (!nextTT || !nextTT->getNode()->getBlock()->isExtensionOfPreviousBlock()) {
    if (cg->enableRegisterAssociations() &&
        comp->getAppendInstruction()->getOpCodeValue() !=
...
OMR::Z::TreeEvaluator::arraytranslateAndTestEvaluator(TR::Node *node,
  ...
  if (!node->isArrayTRT()) {
    ...
    cursor = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BORC, node,
  ...
  } else // Use the new loopReducer - CISC Transfomer.
  {
    ...
    TR::LabelSymbol *boundCheckFailureLabel =
        TR::LabelSymbol::create(cg->trHeapMemory());
    ...
    // minimum
    // length we need to search by taking the smaller of the two values - array
    // length
    ...
    if (isAdditionalLen) {
      ...
      cursor = generateS390BranchInstruction(
          cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNHRC, node, labelSkip);
      cursor->setStartInternalControlFlow();
      generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), node,
                            minReg, lenReg);
      TR::RegisterDependencyConditions *deps =
    ...
    if (isUsingTRT) {
      ...
      TR::LabelSymbol *labelEntryElementChar =
          TR::LabelSymbol::create(cg->trHeapMemory());
      ...
      // will
      // go to helper table.  CLR checks if current index + 256 (indexReg) <
      // length (minReg)
      cursor = generateS390CompareAndBranchInstruction(
          cg, TR::InstOpCode::CLR, node, indexReg, minReg,
          TR::InstOpCode::COND_BHRC, labelHelper);
      cursor->setStartInternalControlFlow();
      ...
      generateS390CompareAndBranchInstruction(
          cg, TR::InstOpCode::CLR, node, indexReg, minReg,
          TR::InstOpCode::COND_BNHRC, label256Top);
      ...
      //
      generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, labelHelper,
                                   loopDeps);
      ...
      if (isAdditionalLen) {
        ...
        generateS390CompareAndBranchInstruction(
            ...
            TR::InstOpCode::COND_BLRC, labelEnd);

        instr = generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::CLR, node, lenReg, alenReg,
            TR::InstOpCode::COND_BNERC, boundCheckFailureLabel);
        cg->stopUsingRegister(lenReg);
        generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                      TR::InstOpCode::COND_BRC, node, labelEnd);
      } else {
        instr = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BZRC, node,
      ...
      //    CC 3:  CPU defined limit (at least 256 characters searched).  R2 is
      //    updated to point to next character.
      ...
      generateRRInstruction(cg, (useSRSTU) ? TR::InstOpCode::SRSTU
                                           : TR::InstOpCode::SRST,
                            node, tmpReg, ptrReg);
      generateS390BranchInstruction(
          cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BORC, node,
      ...
      if (isAdditionalLen) {
        generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BMRC, node,
        ...
        instr = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BPRC, node,
      ...
      // positive
      // if we match the top byte of a 2-byte array.  i.e. delimiter = 0xAA,
      // SRST
      // will match the character 0xFFFF 00AA.  Solution is to load the 2-byte
      // value
...
OMR::Z::TreeEvaluator::arraytranslateEvaluator(TR::Node *node,
    ...
    if (!isLengthConstant) {
      if (TR::Compiler->target.is64Bit()) {
        if (!(inputLengthNode->getOpCode().isLong()))
          ...
          generateRSInstruction(cg, TR::InstOpCode::SLLG, node, inputLenReg,
                                inputLenReg, 1); // outLen is already 64-bit..
                                                 // use it instead for better
                                                 // dependencies.
      } else {
  ...
  if (node->getTermCharNodeIsHint() && (opCode == TR::InstOpCode::TRTO)) {
    generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node,
        afterLoop); // skip after loop if all chars processed
    if (node->getSourceCellIsTermChar()) {
      ...
      generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                    TR::InstOpCode::COND_BNERC, node,
                                    afterLoop); // skip after loop if the source
                                                // cell is not equal to stopChar
  ...
  TR::Instruction *cursor =
      generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, afterLoop);
...
static TR::Register *largearraysetEvaluator(TR::Node *node,
                                            TR::CodeGenerator *cg) {
...
TR::Register *OMR::Z::TreeEvaluator::arraysetEvaluator(TR::Node *node,
  ...
  // MemInitConstLenMacroOp), but only if the constant isn't
  // already in a register and the value doesn't need to remain in a register
  ...
  if (mvcCopy) {
    ...
    generateRRInstruction(cg,
                          TR::Compiler->target.is64Bit() ? TR::InstOpCode::XGR
                                                         : TR::InstOpCode::XR,
                          node, sourceLenReg, sourceLenReg);
    ...
    cursor = generateRSInstruction(cg, TR::InstOpCode::MVCLU, node,
                                   targetPairReg, sourcePairReg, paddingRef);
    cursor->setDependencyConditions(regDeps);
    ...
    if (varLength) {
      TR::LabelSymbol *topOfLoop1 =
          TR::LabelSymbol::create(cg->trHeapMemory(), cg);
      TR::LabelSymbol *endOfLoop1 =
          TR::LabelSymbol::create(cg->trHeapMemory(), cg);
      ...
      */
      ...
      generateS390BranchInstruction(
          cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node, endOfLoop1);
      ...
      if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit())
        ...
        generateRRInstruction(cg, TR::InstOpCode::LR, node,
                              tmpConstExprRegister, constExprRegister);
...
TR::Register *OMR::Z::TreeEvaluator::arraycopyEvaluator(TR::Node *node,
  ...
  // Only a simple memmove or memcpy type of arraycopy is done as a node
  // operation
  // for 390 (this is different than other platforms which support other forms
  // of
  // arraycopy).
  ...
  //    3) The byte count
  //
  // Also, note that 390 codegen -requires- that isForwardArrayCopy() really
  // means
  // that a forward array copy is safe and that if it is -not- set it really
  // means
  // that a backward array copy is safe - the common code should have generated
  // the
  // correct tests around the arraycopy.
  //
  // Also, note that 390 codegen does not need to check for a particular
  // frequency
  // on the length - again - the common code will have generated a test for a
  // particular length, if one is quite common, and there will be 2 versions of
  // the
  ...
  if (false && isSimpleCopy && TR::isJ9() &&
      !comp->getOption(TR_DisableSIMDArrayCopy)) {
  ...
  // The fast-path code that follows currently only supports memcpy() of
  // primitive
  // array types.
  // Reference arraycopy could be handled, or it could be done as a 'java'
  // private
  ...
  if (byteLenNode->getOpCode().isLoadConst()) {
    ...
    byteLenReg =
        cg->evaluateLengthMinusOneForMemoryOps(byteLenNode, true, lenMinusOne);
    evaluateChildren = true;
  }
  ...
  if (evaluateChildren) {
    ...
    if (!cg->canClobberNodesRegister(byteSrcNode)) {
      ...
      generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), node,
                            copyByteSrcReg->getGPRofArGprPair(), byteSrcReg);

      // Kill the copy rather than the original to allow better
      // rematerialisation
    ...
    if (!cg->canClobberNodesRegister(byteDstNode)) {
      ...
      generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), node,
                            copyByteDstReg->getGPRofArGprPair(), byteDstReg);
  ...
  if (!isSimpleCopy && !node->isNoArrayStoreCheckArrayCopy()) {
    ...
    if (!cg->canClobberNodesRegister(byteDstNode)) {
      if (byteSrcObjReg->containsInternalPointer()) {
        ...
        copybyteSrcObjReg->setPinningArrayPointer(
            byteSrcObjReg->getPinningArrayPointer());
      ...
      generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), node,
                            copybyteSrcObjReg, byteSrcObjReg);
    ...
    if (!cg->canClobberNodesRegister(byteSrcObjNode)) {
      if (byteDstObjReg->containsInternalPointer()) {
        ...
        copybyteDstObjReg->setPinningArrayPointer(
            byteDstObjReg->getPinningArrayPointer());
      ...
      generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), node,
                            copybyteDstObjReg, byteDstObjReg);
...
#ifdef J9_PROJECT_SPECIFIC
      TR::TreeEvaluator::genArrayCopyWithArrayStoreCHK(
          node, byteSrcObjReg, byteDstObjReg, byteSrcReg, byteDstReg,
          tempLenReg, cg);
#endif
  ...
  static char *disableAtomic = "ON"; // feGetEnv("TR_ArrayCopyDisableAtomic");
                                     // //always disable runtime check path for
                                     // arraycopy

  // If it's a referenced arraycopy (i.e. !isSimpleCopy), we have to ensure the
  // elements are copied atomically - do not use MVC
  // Everything needs to be atomically copied... disabling this path for non 8
  // bit arrays
  if (isSimpleCopy &&
      ((bool)disableAtomic || node->getArrayCopyElementType() == TR::Int8) &&
      node->isForwardArrayCopy()) {
    if (byteLenReg == NULL) {
      MemCpyConstLenMacroOp op(node, byteDstNode, byteSrcNode, cg,
                               constantByteLength);
      op.generate(byteDstReg, byteSrcReg);
    } else {
      MemCpyVarLenMacroOp op(node, byteDstNode, byteSrcNode, cg, byteLenReg,
                             byteLenNode, lenMinusOne);
      op.setUseEXForRemainder(true);
      op.generate(byteDstReg, byteSrcReg);
    }
  } else {
    ...
    if (byteLenReg == NULL) {
      ...
      if (byteLenNode->getDataType() == TR::Int64)
        genLoadLongConstant(cg, byteLenNode, constantByteLength, byteLenReg,
                            NULL, NULL);
      else
        generateLoad32BitConstant(cg, byteLenNode, constantByteLength,
                                  byteLenReg, true);
      cg->stopUsingRegister(byteLenReg);
      isConst = true;
    }
    ...
    if ((bool)disableAtomic) {
      ...
      //
      // We may end up in this block for backwards array copies of variable
      // length. The problem is that
      // MemToMemTypedMacroOp class hierarchy does not handle the use of
      // evaluateLengthMinusOneForMemoryOps
      // function because arraycopy deals with bytes rather than Object counts.
      // In the case where lenMinusOne
      // variable is set we must restore it back before performing the array
      // copy to ensure we do not exclude
      // copying of the last byte in the array.

      if (lenMinusOne)
        generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(),
                              byteLenNode, byteLenReg, 1);

      // Helper that can do backward copy. !node->isForwardArrayCopy() =
      // backward copy
      MemCpyVarLenTypedMacroOp vtOp(node, byteDstNode, byteSrcNode, cg,
                                    node->getArrayCopyElementType(), byteLenReg,
                                    byteLenNode, node->isForwardArrayCopy());
      ...
      if (generateRuntimeMVCOverlap) {
        // use strideRegister to hold difference, since it doesn't get used
        // before the real routine. stride = dstAddr - srcAddr
        cursor = generateRRRInstruction(
            cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::SGRK
                                               : TR::InstOpCode::SRK,
            byteSrcNode, strideRegister, byteDstReg, byteSrcReg);

        // mvc if stride <= 0   (src >= dst)
        cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                               TR::InstOpCode::COND_BNHRC,
                                               byteSrcNode, mvcRoutine);
        // mvc if length <= stride (no overlap)
        cursor = generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::getCmpRegOpCodeFromNode(byteLenNode),
            byteSrcNode, byteLenReg, strideRegister, TR::InstOpCode::COND_BNHRC,
            mvcRoutine, false, false);
      }

      static char *unroll = feGetEnv("TR_ArrayCopyDontUnroll");
      MemCpyAtomicMacroOp vtOp(
          node, byteDstNode, byteSrcNode, cg, node->getArrayCopyElementType(),
          byteLenReg, byteLenNode, node->isForwardArrayCopy(), (bool)!unroll,
          isConst ? constantByteLength : -1);

      // set applyDepLocally to false if generating runtime check
      // create dependencies out here
      cursor = vtOp.generate(byteDstReg, byteSrcReg, strideRegister,
                             !generateRuntimeMVCOverlap);

      if (generateRuntimeMVCOverlap) {
        TR::LabelSymbol *endOfCopy =
            TR::LabelSymbol::create(cg->trHeapMemory(), cg);
        cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                               TR::InstOpCode::COND_BRC,
                                               byteSrcNode, endOfCopy);

        cursor = generateS390LabelInstruction(cg, TR::InstOpCode::LABEL,
                                              byteSrcNode, mvcRoutine);

        if (byteLenReg == NULL) {
          MemCpyConstLenMacroOp op(node, byteDstNode, byteSrcNode, cg,
                                   constantByteLength, strideRegister);
          ...
          MemCpyVarLenMacroOp op(node, byteDstNode, byteSrcNode, cg, byteLenReg,
                                 byteLenNode, lengthMinusOneTmp, itersReg,
                                 raReg);
          op.setUseEXForRemainder(true);
        ...
        TR::RegisterDependencyConditions *oldDep =
            oldS390cursor->getDependencyConditions();

        // Going through existing post conditions on old dependency to decrement
        // total use by one,
        // it will increase by one when added to the vtOp's dependency group
        for (int32_t i = 0; i < oldDep->getNumPostConditions(); i++) {
          // if it exists
          if (oldDep->getPostConditions()->getRegisterDependency(i) &&
              oldDep->getPostConditions()
  ...
  if (!isSimpleCopy) {
    TR::TreeEvaluator::genWrtbarForArrayCopy(node, cg->evaluate(byteSrcObjNode),
                                             cg->evaluate(byteDstObjNode),
                                             byteSrcNode->isNonNull(), cg);
  }
#endif
  ...
  // At this point, copyByte{Src,Dst,Len}Reg == byte{Src,Dst,Len}Reg,
  // respectively.
  //
  if (copyByteSrcReg) {
    cg->stopUsingRegister(byteSrcReg);
...
TR::Register *OMR::Z::TreeEvaluator::aRegLoadEvaluator(TR::Node *node,
  ...
  if (globalReg == NULL) {
    if (node->getOpCode().hasSymbolReference() &&
        node->getSymbolReference()->getSymbol()->isRegisterSymbol()) {
      TR::AutomaticSymbol *regSym =
          node->getSymbolReference()->getSymbol()->castToRegisterSymbol();
...
TR::Register *OMR::Z::TreeEvaluator::iRegLoadEvaluator(TR::Node *node,
  ...
  if (globalReg == NULL) {
    ...
    } else if (cg->getExtendedToInt64GlobalRegisters().ValueAt(
                   node->getGlobalRegisterNumber())) {
      // getExtendedToInt64GlobalRegisters is set by TR_LoadExtensions and it
      // means a larger larger virtual register must be used here
      // so any instructions generated by local RA are the correct size to
      // preserve the upper bits (e.g. use LGR vs LR)
      globalReg = cg->allocate64bitRegister();
    } else {
      globalReg = cg->allocateRegister();
    }
...
TR::Register *OMR::Z::TreeEvaluator::iRegStoreEvaluator(TR::Node *node,
  ...
  if (needsLGFR && child->getOpCode().isLoadVar() &&
      (child->getRegister() == NULL) && child->getType().isInt32()) {
    child->setForce64BitLoad(true);
  ...
  if (useHPR && child->getOpCode().isLoadConst()) {
    ...
    if (globalReg == NULL) {
      globalReg = cg->allocateRegister();
      generateRILInstruction(cg, TR::InstOpCode::IIHF, child, globalReg,
                             (getIntegralValue(child)));
      child->setRegister(globalReg);
    }
    globalReg->setAssignToHPR(true);
  } else if (useHPR && child->getOpCode().isLoadVar() &&
             child->getType().isInt32()) {
    TR_ASSERT(!needsLGFR, " GlobalHPR should not need signExt! \n");
    needsLGFR = false;
    globalReg = child->getRegister();
    if (globalReg == NULL) {
      globalReg = cg->allocateRegister();
      generateRXYInstruction(cg, TR::InstOpCode::LFH, child, globalReg,
                             generateS390MemoryReference(child, cg));
      child->setRegister(globalReg);
    }
    globalReg->setAssignToHPR(true);
  } else {
  ...
  // LGB generated for  TR::bloadi or LLGH generated for TR::cloadi sign extend
  // implicitly
  child_sign_extended =
      (((child->getOpCodeValue() == TR::b2i) &&
        (child->getFirstChild()->getOpCodeValue() == TR::bloadi)) ||
       ((child->getOpCodeValue() == TR::su2i) &&
        (child->getFirstChild()->getOpCodeValue() == TR::cloadi)) ||
       ((child->getOpCodeValue() == TR::l2i) &&
        (child->getFirstChild()->getOpCodeValue() == TR::i2l)));
...
OMR::Z::TreeEvaluator::long2StringEvaluator(TR::Node *node,
  ...
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node, labelDigit1);
  cursor->setStartInternalControlFlow();
...
TR::Register *OMR::Z::TreeEvaluator::bitOpMemEvaluator(TR::Node *node,
  ...
  TR::InstOpCode::Mnemonic SI_opcode =
      isXor ? TR::InstOpCode::XI : isAnd ? TR::InstOpCode::NI
  ...
  if (useSIFormat) {
    ...
    TR::InstOpCode::Mnemonic opcode =
        isXor ? TR::InstOpCode::XC : isAnd ? TR::InstOpCode::NC
...
TR::Register *OMR::Z::TreeEvaluator::getstackEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Register *spReg =
      ((cg->getS390Linkage()))->getNormalStackPointerRealRegister();
  TR::Register *trgReg = cg->allocateRegister();
...
TR::Register *OMR::Z::TreeEvaluator::deallocaEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *srcReg = cg->evaluate(firstChild);
...
/**
 * This evaluator builds a call to a function found in a library such us our
 * XLOPT library.
 * The function name is selected here based on the opcode
 */
TR::Register *
OMR::Z::TreeEvaluator::libxloptFuncEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
...
TR::Register *OMR::Z::TreeEvaluator::aletEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::composeEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR_ASSERT(0, "not expected to be here after TR::alet folding");
  return NULL;
}
...
TR::Register *OMR::Z::TreeEvaluator::PrefetchEvaluator(TR::Node *node,
  ...
  if (memAccessMode) {
    ...
    generateRXYbInstruction(cg, TR::InstOpCode::PFD, node, memAccessMode,
                            memRef);
  } else {
    cg->recursivelyDecReferenceCount(firstChild);
    cg->recursivelyDecReferenceCount(secondChild);
...
OMR::Z::TreeEvaluator::loadAutoOffsetEvaluator(TR::Node *node,
  ...
  TR::MemoryReference *mr = generateS390MemoryReference(
      cg->machine()->getS390RealRegister(TR::RealRegister::GPR0), 0, cg);
...
getButestBranchCondition(TR::ILOpCodes opCode, int32_t cmpValue) {
  ...
  TR::InstOpCode::S390BranchCondition TMtoTMLLBranchMaskTable[6][4] = {
      ...
      /* TR::ifiucmpne */ {TR::InstOpCode::COND_MASK7,
                           TR::InstOpCode::COND_MASK9,
                           TR::InstOpCode::COND_MASK15,
                           TR::InstOpCode::COND_MASK14},
      /* TR::ifiucmpgt */ {TR::InstOpCode::COND_MASK7,
                           TR::InstOpCode::COND_MASK1,
                           TR::InstOpCode::COND_MASK1,
                           TR::InstOpCode::COND_MASK0},
      /* TR::ifiucmpge */ {TR::InstOpCode::COND_MASK15,
                           TR::InstOpCode::COND_MASK7,
                           TR::InstOpCode::COND_MASK1,
                           TR::InstOpCode::COND_MASK1},
      /* TR::ifiucmplt */ {TR::InstOpCode::COND_MASK0,
                           TR::InstOpCode::COND_MASK8,
                           TR::InstOpCode::COND_MASK14,
                           TR::InstOpCode::COND_MASK14},
      /* TR::ifiucmple */ {TR::InstOpCode::COND_MASK8,
                           TR::InstOpCode::COND_MASK14,
                           TR::InstOpCode::COND_MASK14,
                           TR::InstOpCode::COND_MASK15}};
...
void arraycmpWithPadHelper::setStartInternalControlFlow(
    TR::Instruction *cursor) {
  if (!isStartInternalControlFlowSet) {
    cursor->setStartInternalControlFlow();
    isStartInternalControlFlowSet = true;
...
TR::Register *arraycmpWithPadHelper::generateCLCLE() {
  ...
  TR::Instruction *cursor = NULL;
  cursor = generateRSInstruction(
      cg, isUTF16 ? TR::InstOpCode::CLCLU : TR::InstOpCode::CLCLE, node,
      source1PairReg, source2PairReg, paddingRef);
...
void arraycmpWithPadHelper::generateVarCLCSetup() {
  ...
  if (isEqual) {
    ...
    TR::LabelSymbol *doneAssignLabel =
        TR::LabelSymbol::create(cg->trHeapMemory(), cg);
    generateS390CompareAndBranchInstruction(
        cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::CGR
                                           : TR::InstOpCode::CR,
        node, source2LenReg, source1LenReg, TR::InstOpCode::COND_BHRC,
        doneAssignLabel);
...
void arraycmpWithPadHelper::generateConstCLCMainLoop() {
  ...
  for (int32_t i = 0, offset = 0; i < loopCount; ++i, offset += 256) {
    ...
    if (!isAddr1Const) {
      ...
      TR_ASSERT(offset + addr1Const < 4096, "arraycmpWithPadHelper: memRef "
                                            "offset too large, we will "
                                            "generate wrong code\n");
    ...
    if (!isAddr2Const) {
      ...
      TR_ASSERT(offset + addr2Const < 4096, "arraycmpWithPadHelper: memRef "
                                            "offset too large, we will "
                                            "generate wrong code\n");
    ...
    if (!(isPerfectMultiple && (i == loopCount - 1) && isEqual)) {
      setStartInternalControlFlow(generateS390BranchInstruction(
...
void arraycmpWithPadHelper::generateVarCLCMainLoop() {
  TR::LabelSymbol *loopStartLabel =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *loopTopLabel =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  ...
  generateRXInstruction(
      cg, TR::InstOpCode::LA, node, source1Reg->getGPRofArGprPair(),
      generateS390MemoryReference(source1Reg->getGPRofArGprPair(), 256, cg));
  generateRXInstruction(
      cg, TR::InstOpCode::LA, node, source2Reg->getGPRofArGprPair(),
      generateS390MemoryReference(source2Reg->getGPRofArGprPair(), 256, cg));

  generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, loopStartLabel);

  generateS390BranchInstruction(cg, TR::Compiler->target.is64Bit()
                                        ? TR::InstOpCode::BRCTG
                                        : TR::InstOpCode::BRCT,
...
void arraycmpWithPadHelper::generateVarCLCRemainder() {
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    generateS390CompareAndBranchInstruction(
        ...
        TR::InstOpCode::COND_BERC, isFoldedIf ? unequalLabel : cmpDoneLabel,
        false, false);
  } else {
...
void arraycmpWithPadHelper::setupConstCLCPadding() {
  ...
  if ((source1Len != 0) && (source2Len != 0)) {
    setStartInternalControlFlow(generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, brCond, node,
        skipUnequal ? cmpDoneLabel : unequalLabel, branchDeps));
  }
...
TR::MemoryReference *arraycmpWithPadHelper::convertToShortDispMemRef(
  ...
  // etc.
  // Early loading addr to the explicit reg can thus avoid the LA generated from
  // the later "generateSS1Instruction(cg, TR::InstOpCode::CLC,...)"
...
void arraycmpWithPadHelper::generateConstCLCSpacePaddingLoop() {
  ...
  if (remain) {
    ...
    // which leads an additional BRC following CLI. i.e.,
    // CLI
...
void arraycmpWithPadHelper::generateConstCLCPaddingLoop() {
  ...
  TR::LabelSymbol *paddingUnequalLabel =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  doneLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  if (isUTF16)
    paddingLen /= 2;
...
void arraycmpWithPadHelper::generateVarCLCPaddingLoop() {
  ...
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                ...
                                doneLabel); // if we finish the loop, then
                                            // everything is equal and we're
                                            // done

  generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                               paddingUnequalLabel);
  generateRRInstruction(cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::LGR
                                                           : TR::InstOpCode::LR,
...
void arraycmpWithPadHelper::generateCLCLitPoolPadding() {
  ...
  if (constPadding) {
    ...
    paddingLitPoolOffset =
        cg->fe()->findOrCreateLiteral(comp, paddingValue, paddingByteLength);
...
void arraycmpWithPadHelper::generateCLCFoldedIfResult() {
  ...
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC, finalBrCond, node,
                                cmpTargetLabel, branchDeps);
  TR::Instruction *cursor = generateS390LabelInstruction(
      cg, TR::InstOpCode::LABEL, node, cmpDoneLabel);
  cursor->setDependencyConditions(regDeps);
 ...
 * created as part of the memRef evaluation,
 * specifically when fixup for long displacement occurs.
 * This is a common occurrance, but in this situation it's problematic because
 * it occurs inside Internal Control Flow, and
 * the result is that registers used are not properly assigned afterwards.
 * Furthermore, since these register uses occur
 * 'down' in the instruction creation process it is not possible to add them to
 * the reg deps at that point.
 *
 * The solution is to perform the long displacement fixup before trying to
 * create instructions.  This function
 * can (or should?) be called whenever a memory reference is created inside
...
TR::Register *arraycmpWithPadHelper::generate() {
  ...
  // so if
  // we're unsure if there's padding (due to variable lengths), or we're not
  // sure what
...
TR::Register *
OMR::Z::TreeEvaluator::integerLowestOneBit(TR::Node *node,
                                           TR::CodeGenerator *cg) {
...
TR::Register *OMR::Z::TreeEvaluator::longLowestOneBit(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR_ASSERT(0, "not implemented!");
  return NULL;
}
 ...
 * FoldIf:      depends on whether nodes returning CC are uncommoned (moved out
 * of treetop) via TR::CodeGenerator::uncommonIfBifs()
 *              To add, include check in uncommonIfBifs() and isIfFoldable().
 * FoldNoIPM:   set by calling through treetopEvaluator(). Include a check
 * there.
 * FoldLookUp:  set by calling through lookupEvaluator(). Include a check there.
 * For icall BIF, returnsConditionCodeOrBool flag should be set by including the
 * BIF in idIsBIFReturningConditionCode() during ILGen
 */
TR::Register *OMR::Z::TreeEvaluator::getConditionCodeOrFold(
  ...
  switch (foldType) {
  case TR_FoldNone:
    return getConditionCodeOrFoldIf(node, cg, false, NULL);
  case TR_FoldIf:
    return getConditionCodeOrFoldIf(node, cg, true, parent);
  case TR_FoldLookup:
    return foldLookup(cg, parent);
  case TR_FoldNoIPM:
    return NULL;
  default:
...
TR::Register *OMR::Z::TreeEvaluator::ifxcmpoEvaluator(TR::Node *node,
  ...
  if ((opCode == TR::ificmno) || (opCode == TR::ificmnno) ||
      (opCode == TR::iflcmno) || (opCode == TR::iflcmnno))
    generateRRInstruction(cg, nodeIs64Bit ? TR::InstOpCode::AGR
                                          : TR::InstOpCode::AR,
                          node, tmp, rs2);
  else
    generateRRInstruction(cg, nodeIs64Bit ? TR::InstOpCode::SGR
                                          : TR::InstOpCode::SR,
                          node, tmp, rs2);
...
TR::Register *OMR::Z::TreeEvaluator::arraytranslateDecodeSIMDEvaluator(
  ...
  TR::LabelSymbol *processMultiple16Bytes =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processMultiple16BytesEnd =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);

  processMultiple16Bytes->setStartInternalControlFlow();
  processMultiple16BytesEnd->setEndInternalControlFlow();

  TR::LabelSymbol *processUnder16Bytes =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processUnder16BytesEnd =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);

  processUnder16Bytes->setStartInternalControlFlow();
  processUnder16BytesEnd->setEndInternalControlFlow();

  TR::LabelSymbol *processSaturated =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processSaturatedEnd =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processUnSaturatedInput1 =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processUnSaturatedInput2 =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  ...
  generateS390CompareAndBranchInstruction(
      ...
      TR::InstOpCode::COND_BLRC, processMultiple16Bytes, false, false);
  ...
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_CC0, node, processEnd);
  ...
  generateS390CompareAndBranchInstruction(
      ...
      TR::InstOpCode::COND_CC0, processEnd, false, false);
  ...
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_MASK15, node, processEnd);
...
TR::Register *OMR::Z::TreeEvaluator::arraytranslateEncodeSIMDEvaluator(
  ...
  // Create the necessary labels
  TR::LabelSymbol *processMultiple16Chars =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processMultiple16CharsEnd =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);

  processMultiple16Chars->setStartInternalControlFlow();
  processMultiple16CharsEnd->setEndInternalControlFlow();

  TR::LabelSymbol *processUnder16Chars =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processUnder16CharsEnd =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);

  TR::LabelSymbol *processUnder8Chars =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processUnder8CharsEnd =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  ...
  TR::LabelSymbol *processSaturatedInput1 =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processSaturatedInput2 =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processUnSaturated =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processEnd = TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  ...
  generateS390CompareAndBranchInstruction(
      cg, TR::InstOpCode::getCmpRegOpCode(), node, translated, inputLen16,
      TR::InstOpCode::COND_BLRC, processMultiple16Chars, false, false);
...
TR::Register *inlineStringHashCode(TR::Node *node, TR::CodeGenerator *cg,
  ...
  // be processes by loop
  // VLLEZ instruction will load word(compressed String) or double word
  // (decompressed String), elementSize is used for that
  ...
  generateS390CompareAndBranchInstruction(
      ...
      stringSize, TR::InstOpCode::COND_MASK4, labelSerial, false, false);
  ...
  generateS390CompareAndBranchInstruction(
      cg, TR::InstOpCode::getCmpLogicalRegOpCode(), node, registerIndex,
      registerEnd, TR::InstOpCode::COND_MASK10, labelEnd, false, false);
...
/** \brief Inline evaluates a call to java/lang/String.hashCodeImpl([CII)I
  *
  * This function uses an implementation of a 4 time unrolled Duff's Device loop
 * to implement the Java String hashcode
  * algorithm with a switch table and a do while loop. This implementation
 * contains only 2 branches (one for the switch
  * and one for the backwards loop branch). Empirically this implementation is
 * measured to be faster than a naive for
  * loop implementation at low optimization levels, however in production
 * benchmark environments this implementation
  * seems to perform 10% slower than the naive for loop due to the JIT doing a
 * great job unrolling the loop and using
  * multiple registers in the unrolled version to avoid stalls.
  *
  * Because of these shortcomings this implementation is checked in for
 * reference and is not being used in production.
  */
TR::Register *inlineStringHashCodeUnrolled(TR::Node *node,
                                           TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  ...
  // registerCount *= 2
  generateRSInstruction(cg, TR::InstOpCode::getShiftLeftLogicalSingleOpCode(),
                        node, countRegister, countRegister, 1);

  // registerTemp = registerCount
  generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), node,
                        tempRegister, countRegister);

  // registerTemp = registerTemp % 8
  generateRILInstruction(cg, TR::InstOpCode::NILF, node, tempRegister,
                         0x00000007);

  // registerIndex = registerTemp
  generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), node,
                        indexRegister, tempRegister);

  // registerIndex -= 8
  generateRILInstruction(cg, TR::InstOpCode::getSubtractLogicalImmOpCode(),
                         node, indexRegister, 8);

  // registerTemp *= 2 or 4 depending on the address size
  generateRSInstruction(cg, TR::InstOpCode::getShiftLeftLogicalSingleOpCode(),
                        node, tempRegister, tempRegister,
                        TR::Compiler->target.is64Bit() ? 2 : 1);

  // registerHash = current PSW address
  generateRILInstruction(cg, TR::InstOpCode::LARL, node, hashRegister,
                         switchTableLabel);

  // registerTemp += registerHash
  generateRRInstruction(cg, TR::InstOpCode::getAddRegOpCode(), node,
                        tempRegister, hashRegister);
  ...
  generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(), node, tempRegister,
                        generateS390MemoryReference(tempRegister, 0, cg));

  static_cast<TR::S390RegInstruction *>(
      generateS390RegInstruction(cg, TR::InstOpCode::BCR, node, tempRegister))
      ->setBranchCondition(TR::InstOpCode::COND_MASK15);

  generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                               switchTableLabel);

  // Generate the data constants with the case label addresses
  generateS390LabelInstruction(cg, TR::InstOpCode::DC, node,
                               adjustIndexForCase0Label);
  ...
  generateRXInstruction(cg, TR::InstOpCode::getLoadAddressOpCode(), node,
                        indexRegister,
                        generateS390MemoryReference(indexRegister, 8, cg));
  ...
  // registerTemp = registerHash << 5
  generateRSInstruction(cg, TR::InstOpCode::SLLK, node, tempRegister,
                        hashRegister, 5);

  // registerHash = registerTemp - registerHash
  generateRRRInstruction(cg, TR::InstOpCode::getSubtractThreeRegOpCode(), node,
                         hashRegister, tempRegister, hashRegister);

  // registerHash += char at registerIndex
  generateRXInstruction(cg, TR::InstOpCode::AH, node, hashRegister,
                        generateS390MemoryReference(
                            valueRegister, indexRegister,
                            TR::Compiler->om.contiguousArrayHeaderSizeInBytes(),
                            cg));
  ...
  // registerHash += char at registerIndex
  generateRXInstruction(
      cg, TR::InstOpCode::AH, node, hashRegister,
      generateS390MemoryReference(
          valueRegister, indexRegister,
          TR::Compiler->om.contiguousArrayHeaderSizeInBytes() + 2, cg));
  ...
  generateRXInstruction(
      ...
      generateS390MemoryReference(
          valueRegister, indexRegister,
          TR::Compiler->om.contiguousArrayHeaderSizeInBytes() + 4, cg));
  ...
  generateRXInstruction(
      ...
      generateS390MemoryReference(
          valueRegister, indexRegister,
          TR::Compiler->om.contiguousArrayHeaderSizeInBytes() + 6, cg));
  ...
  generateS390CompareAndBranchInstruction(
      cg, TR::InstOpCode::getCmpLogicalRegOpCode(), node, indexRegister,
      countRegister, TR::InstOpCode::COND_MASK4, case0Label, false, false);
  ...
  // Set up the proper register dependencies
  dependencies->addPostConditionIfNotAlreadyInserted(
      valueRegister, TR::RealRegister::AssignAny);
  dependencies->addPostConditionIfNotAlreadyInserted(
      indexRegister, TR::RealRegister::AssignAny);
  dependencies->addPostConditionIfNotAlreadyInserted(
      countRegister, TR::RealRegister::AssignAny);

  dependencies->addPostConditionIfNotAlreadyInserted(
      hashRegister, TR::RealRegister::AssignAny);
  dependencies->addPostConditionIfNotAlreadyInserted(
      tempRegister, TR::RealRegister::AssignAny);
...
TR::Register *inlineUTF16BEEncodeSIMD(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  // Create the necessary labels
  TR::LabelSymbol *process8Chars =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *process8CharsEnd =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  ...
  TR::LabelSymbol *processSurrogate =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processSurrogateEnd =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  ...
  generateS390CompareAndBranchInstruction(
      cg, TR::InstOpCode::getCmpRegOpCode(), node, translated, inputLen16,
      TR::InstOpCode::COND_BLRC, process8Chars, false, false);
...
TR::Register *inlineUTF16BEEncode(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  // Create the necessary labels
  TR::LabelSymbol *processChar4 =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processChar4End =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processChar1 =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processChar1End =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *processChar1Copy =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  ...
  generateRXYInstruction(cg, TR::InstOpCode::LG, node, temp1,
                         generateS390MemoryReference(input, translated, 0, cg));
  ...
  generateS390CompareAndBranchInstruction(
      ...
      TR::InstOpCode::COND_BNLRC, processChar1End, false, false);

  // Load an input character from memory
  generateRXYInstruction(cg, TR::InstOpCode::LLH, node, temp1,
                         generateS390MemoryReference(input, translated, 0, cg));
  ...
  TR::Instruction *cursor = generateS390LabelInstruction(
      cg, TR::InstOpCode::LABEL, node, processChar1End);
...
TR::Register *OMR::Z::TreeEvaluator::arraycmpSIMDHelper(
  ...
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_MASK3, node, topOfLoop);
  ...
  if (isFoldedIf) {
    if (isIfxcmpBrCondContainEqual)
      ...
      generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                    TR::InstOpCode::COND_BRC, node, afterLoop);
    ...
    if (isArrayCmp && node->isArrayCmpLen())
      ...
      generateRIInstruction(cg, TR::InstOpCode::getLoadHalfWordImmOpCode(),
                            node, resultReg,
                            0); // Return zero to indicate equal
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BRC, node, afterLoop);
  }
  ...
  if (isFoldedIf) {
    ...
    if (isArrayCmp && node->isArrayCmpLen()) {
      ...
      // last loop
      // vectorOutputReg contains the 0-based index of the first non-matching
      // element (index is its position in the vector)
      generateRRInstruction(cg, TR::InstOpCode::getSubstractRegOpCode(), node,
                            resultReg, lastByteIndexReg);
...
TR::Register *OMR::Z::TreeEvaluator::vnegEvaluator(TR::Node *node,
  ...
  case TR::VectorInt32:
  case TR::VectorInt64:
    return inlineVectorUnaryOp(node, cg, TR::InstOpCode::VLC);
  case TR::VectorDouble:
    return inlineVectorUnaryOp(node, cg, TR::InstOpCode::VFPSO);
  default:
 ...
 * If negateOp is provided, generates a fused multiply sub from the tree (A -
 * B*C), where addNode is the + node
 * and mulNode the * subtree.
 * Negates one operand of the multiply and generates a fused multiply add to
 * perform (A - B*C)
 */
bool generateFusedMultiplyAddIfPossible(TR::CodeGenerator *cg,
  ...
  if (addNode->getType().isFloatingPoint() && !comp->getOption(TR_FloatMAF) &&
      !mulNode->isFPStrictCompliant())
    return false;
...
TR::Register *OMR::Z::TreeEvaluator::vaddEvaluator(TR::Node *node,
  ...
  if (node->getDataType() == TR::VectorDouble &&
    ...
    if (TR::comp()->getOption(TR_TraceCG))
      traceMsg(TR::comp(), "Successfully changed vadd with vmul child to fused "
    ...
    case TR::VectorInt32:
    case TR::VectorInt64:
      return inlineVectorBinaryOp(node, cg, TR::InstOpCode::VA);
    case TR::VectorDouble:
      return inlineVectorBinaryOp(node, cg, TR::InstOpCode::VFA);
    default:
      TR_ASSERT(false, "unrecognized vector type %s\n",
                node->getDataType().toString());
      return NULL;
...
TR::Register *OMR::Z::TreeEvaluator::vsubEvaluator(TR::Node *node,
  ...
  if (node->getDataType() == TR::VectorDouble &&
    ...
    if (TR::comp()->getOption(TR_TraceCG))
      traceMsg(TR::comp(), "Successfully changed vsub with vmul child to fused "
    ...
    case TR::VectorInt32:
    case TR::VectorInt64:
      return inlineVectorBinaryOp(node, cg, TR::InstOpCode::VS);
    case TR::VectorDouble:
      return inlineVectorBinaryOp(node, cg, TR::InstOpCode::VFS);
    default:
      TR_ASSERT(false, "unrecognized vector type %s\n",
                node->getDataType().toString());
      return NULL;
...
TR::Register *OMR::Z::TreeEvaluator::vmulEvaluator(TR::Node *node,
  ...
  case TR::VectorInt16:
  case TR::VectorInt32:
    return inlineVectorBinaryOp(node, cg, TR::InstOpCode::VML);
  case TR::VectorInt64: {
  ...
  case TR::VectorDouble:
    return inlineVectorBinaryOp(node, cg, TR::InstOpCode::VFM);
  default:
    TR_ASSERT(false, "unrecognized vector type %s\n",
              node->getDataType().toString());
    return NULL;
...
TR::Register *OMR::Z::TreeEvaluator::vDivOrRemHelper(TR::Node *node,
  ...
  if (dividend == divisor) {
    ...
    for (int i = 0; i < (16 / getVectorElementSize(node)); i++) {
      ...
      if (!is64Bit)
        generateRSInstruction(cg, isUnsigned ? TR::InstOpCode::SRDL
                                             : TR::InstOpCode::SRDA,
                              node, dividendGPR, 32);
...
TR::Register *OMR::Z::TreeEvaluator::vdivEvaluator(TR::Node *node,
  ...
  case TR::VectorInt32:
  case TR::VectorInt64:
    return vDivOrRemHelper(node, cg, true);
  case TR::VectorDouble:
    return inlineVectorBinaryOp(node, cg, TR::InstOpCode::VFD);
  default:
    TR_ASSERT(false, "unrecognized vector type %s\n",
              node->getDataType().toString());
    return NULL;
...
TR::Register *OMR::Z::TreeEvaluator::vcmpeqEvaluator(TR::Node *node,
  ...
  case TR::VectorInt32:
  case TR::VectorInt64:
    return inlineVectorBinaryOp(node, cg, TR::InstOpCode::VCEQ);
  case TR::VectorDouble:
    return inlineVectorBinaryOp(node, cg, TR::InstOpCode::VFCE);
  default:
    TR_ASSERT(false, "unrecognized vector type %s\n",
              node->getDataType().toString());
    return NULL;
...
TR::Register *OMR::Z::TreeEvaluator::vcmpneEvaluator(TR::Node *node,
  ...
  case TR::VectorInt32:
  case TR::VectorInt64:
    targetReg = inlineVectorBinaryOp(node, cg, TR::InstOpCode::VCEQ);
    break;
  case TR::VectorDouble:
    targetReg = inlineVectorBinaryOp(node, cg, TR::InstOpCode::VFCE);
    break;
  default:
    TR_ASSERT(false, "unrecognized vector type %s\n",
              node->getDataType().toString());
...
TR::Register *OMR::Z::TreeEvaluator::vcmpgtEvaluator(TR::Node *node,
  ...
  case TR::VectorInt32:
  case TR::VectorInt64:
    return inlineVectorBinaryOp(node, cg, op);
  case TR::VectorDouble:
    return inlineVectorBinaryOp(node, cg, TR::InstOpCode::VFCH);
  default:
    TR_ASSERT(false, "unrecognized vector type %s\n",
              node->getDataType().toString());
    return NULL;
...
TR::Register *OMR::Z::TreeEvaluator::vsplatsEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR_ASSERT(!firstChild->getOpCode().isFloat(),
            "Float splat to vector float not handled!\n");
  TR::Register *returnReg = cg->allocateRegister(TR_VRF);
  uint8_t ESMask = getVectorElementSizeMask(firstChild->getSize());
  bool inRegister =
      !firstChild->isSingleRefUnevaluated() ||
      (firstChild->getOpCode().hasSymbolReference() &&
       firstChild->getSymbolReference()->getSymbol()->isRegisterSymbol());
                 ...
                 2) // Just load from mem and skip evaluation if low refcount
  {
    ...
    if (firstChild->getOpCode().isDouble()) {
      ...
      mr = generateS390MemoryReference(cg->findOrCreateConstant(node, &v, 8),
                                       cg, 0, node);
    } else {
      switch (firstChild->getSize()) {
      case 1: {
        uint8_t v = firstChild->getIntegerNodeValue<uint8_t>();
        mr = generateS390MemoryReference(cg->findOrCreateConstant(node, &v, 1),
                                         cg, 0, node);
        break;
      }
      case 2: {
        uint16_t v = firstChild->getIntegerNodeValue<uint16_t>();
        mr = generateS390MemoryReference(cg->findOrCreateConstant(node, &v, 2),
                                         cg, 0, node);
        break;
      }
      case 4: {
        uint32_t v = firstChild->getIntegerNodeValue<uint32_t>();
        mr = generateS390MemoryReference(cg->findOrCreateConstant(node, &v, 4),
                                         cg, 0, node);
        break;
      }
      case 8: {
...
// TreeEvaluator.cpp, suppress implicit instantiation
// using these extern definitions. Unfortunately, Visual Studio likes doing
// things differently, so for VS, this chunk
