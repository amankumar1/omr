  ...
  if (self()->getKind() == TR::Snippet::IsUnresolvedCall) {
    ...
    *(int16_t *)cursor = 0xC0E5;
    ...
    if (NEEDS_TRAMPOLINE(destAddr, cursor, cg)) {
      ...
      // trampoline.
      destAddr = cg->fe()->indexedTrampolineLookup(
          glueRef->getReferenceNumber(), (void *)cursor);
      self()->setUsedTrampoline(true);
    }
#endif

    TR_ASSERT(CHECK_32BIT_TRAMPOLINE_RANGE(destAddr, cursor),
...
uint8_t *OMR::Z::Snippet::generateLoadVMThreadInstruction(TR::CodeGenerator *cg,
  ...
  if (cg->comp()->getOption(TR_Enable390FreeVMThreadReg)) {
    TR_BackingStore *vmThreadBackingStore =
        cg->getVMThreadRegister()->getBackingStorage();
    if (vmThreadBackingStore) {
      intptrj_t symbolOffset =
          vmThreadBackingStore->getSymbolReference()->getSymbol()->getOffset();
