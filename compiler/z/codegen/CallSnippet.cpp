...
uint8_t *TR::S390CallSnippet::loadArgumentItem(TR::InstOpCode::Mnemonic op,
                                               uint8_t *buffer,
                                               TR::RealRegister *reg,
                                               int32_t offset) {
  TR::RealRegister *stackPtr = cg()->getStackPointerRealRegister();
  ...
  stackPtr->setBaseRegisterField(toS390Cursor(buffer));
  TR_ASSERT((offset & 0xfffff000) == 0,
            "CallSnippet: Unexpected displacement for loadArgumentItem");
  *toS390Cursor(buffer) |= offset & 0x00000fff;
  return buffer + opCode.getInstructionLength();
}
...
uint8_t *TR::S390CallSnippet::S390flushArgumentsToStack(uint8_t *buffer,
  ...
  bool rightToLeft = linkage->getRightToLeft() &&
                     ...
                     callNode->getSymbolReference() !=
                         cg->symRefTab()->element(TR_induceOSRAtCurrentPC);
  if (rightToLeft) {
