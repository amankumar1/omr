...
public:
  ...
  TR::Register *searchForRegister(TR::Register *vr, uint8_t flag,
    ...
    for (int32_t i = 0; i < numberOfRegisters; i++) {
      if (_dependencies[i].getRegister(cg) == vr &&
          (_dependencies[i].getFlags() & flag))
        return _dependencies[i].getRegister(cg);
      if (vr->isArGprPair() &&
          _dependencies[i].getRegister(cg)->isArGprPair() &&
          vr->getARofArGprPair() ==
              _dependencies[i].getRegister(cg)->getARofArGprPair() &&
          vr->getGPRofArGprPair() ==
              _dependencies[i].getRegister(cg)->getGPRofArGprPair())
        return _dependencies[i].getRegister(cg);
    }
    return NULL;
  }
...
public:
  ...
  searchPostConditionRegisterPos(TR::Register *vr,
                                 uint8_t flag = RefsAndDefsDependentRegister) {
    return _postConditions == NULL ? -1 : _postConditions->searchForRegisterPos(
                                              vr, flag, _addCursorForPost, _cg);
  }
  ...
  // road
  // We only add in the post condition if the desired virt reg is not already
  // spoken for
