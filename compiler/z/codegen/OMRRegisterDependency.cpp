...
OMR::Z::RegisterDependencyConditions::RegisterDependencyConditions(
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    ...
    if (cg->supportsHighWordFacility() &&
        ...
        cg->machine()->getHPRFromGlobalRegisterNumber(
            child->getGlobalRegisterNumber())) {
      if (reg->is64BitReg() && !reg->containsCollectedReference()) {
        ...
        iCursor = generateRRInstruction(cg, TR::InstOpCode::LR, node, copyHPR,
                                        reg, iCursor);
        reg = copyHPR;
      }
...
void OMR::Z::RegisterDependencyExt::
operator=(const TR::RegisterDependency &other) {
  ...
  if (foundLow || foundHigh) {
    ...
    if (cg->supportsHighWordFacility() &&
      ...
      if (cg->machine()->getHPRFromGlobalRegisterNumber(
              child->getGlobalRegisterNumber()) != NULL) {
    ...
    if (highReg) {
      if (kind == TR_GPR) {
        ...
        TR::InstOpCode::Mnemonic loadOpHigh =
            TR::InstOpCode::getLoadRegOpCode();
        if (cg->supportsHighWordFacility() &&
            !comp->getOption(TR_DisableHighWordRA) && kind == TR_GPR) {
          loadOpHigh = TR::InstOpCode::LR;
        }
        iCursor = generateRRInstruction(cg, loadOpHigh, node, highCopyReg,
                                        highReg, iCursor);
        highReg = highCopyReg;
      } else // kind == FPR
      {
...
OMR::Z::RegisterDependencyConditions::clone(TR::CodeGenerator *cg,
  ...
  for (i = _numPreConditions - 1; i >= 0; --i) {
    ...
    other->getPreConditions()->setDependencyInfo(
        i, dep->getRegister(_cg), dep->getRealRegister(), dep->getFlags());
  }
  ...
  if (instr->getOpCodeValue() != TR::InstOpCode::ASSOCREGS) {
    ...
    if (_preConditions != NULL) {
      for (int32_t i = oldPreCursor; i < _addCursorForPre; i++) {
        instr->useRegister(
            _preConditions->getRegisterDependency(i)->getRegister(_cg), true);
      }
    }
    ...
    if (_postConditions != NULL) {
      for (int32_t j = oldPostCursor; j < _addCursorForPost; j++) {
        instr->useRegister(
            _postConditions->getRegisterDependency(j)->getRegister(_cg), true);
      }
    }
  }
}

uint32_t TR_S390RegisterDependencyGroup::genBitVectOfAssignableGPRs(
    TR::Instruction *currentInstruction, uint32_t numberOfRegisters,
    TR::CodeGenerator *cg) {
  int32_t i;
...
uint32_t TR_S390RegisterDependencyGroup::checkDependencyGroup(
    TR::Instruction *currentInstruction, uint32_t numberOfRegisters,
    TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  int32_t i, j;
  ...
  uint32_t availRegMap =
      genBitVectOfAssignableGPRs(currentInstruction, numberOfRegisters, cg);
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    if (cg->supportsHighWordFacility() &&
      ...
      if (_dependencies[i].getRegister(cg) &&
        ...
        // on 64-bit conservatively assume rEP and rRA are going to clobber
        // highword
        if (TR::Compiler->target.is64Bit() &&
            !(currentInstruction->isLabel() &&
              toS390LabelInstruction(currentInstruction)
                  ->getLabelSymbol()
                  ->isStartOfColdInstructionStream()) &&
            (_dependencies[i].getRealRegister() ==
                 cg->getReturnAddressRegister() ||
             _dependencies[i].getRealRegister() ==
                 cg->getEntryPointRegister())) {
        ...
        if (TR::RealRegister::isHPR(_dependencies[i].getRealRegister())) {
          ...
          cg->maskAvailableHPRSpillMask(~(TR::RealRegister::getBitMask(
              _dependencies[i].getRealRegister())));
          // traceMsg (comp, "\n HPR: i = %d, _availableHPRSpillMask = 0x%x\n",
          // i, cg->getAvailableHPRSpillMask());
        } else if (_dependencies[i].getRegister(cg)->is64BitReg()) {
          cg->maskAvailableHPRSpillMask(
              ~(TR::RealRegister::getBitMask(_dependencies[i].getRealRegister())
                << 16));
          // traceMsg (comp, "\n 64: i = %d, _availableHPRSpillMask = 0x%x, reg=
          // %d, mask = 0x%x\n", i, cg->getAvailableHPRSpillMask(),
          //  _dependencies[i].getRealRegister(),
          //  (TR::RealRegister::getBitMask(_dependencies[i].getRealRegister())
          //  << 16));
  ...
  if (numReqPairs > numAvailPairs) {
    if (cg->getDebug()) {
      ...
      cg->getDebug()->printRegisterDependencies(comp->getOutFile(), this,
                                                numberOfRegisters);
    }
    TR_ASSERT(0, "checkDependencyGroup::Insufficient available pairs to "
                 "satisfy all reg deps.\n");
    traceMsg(comp, "checkDependencyGroup::Insufficient available pairs to "
                   "satisfy all reg deps, abort compilation\n");
    ...
    while (j < numberOfRegisters) {
      if (virtReg == _dependencies[j].getRegister(cg) &&
        ...
        if (cg->getDebug())
          cg->getDebug()->printRegisterDependencies(comp->getOutFile(), this,
                                                    numberOfRegisters);
        TR_ASSERT(0, "checkDependencyGroup::Virtual dup found in register "
                     "dependencies\n");
      }

      if (TR::RealRegister::isRealReg(realReg) &&
          realReg == _dependencies[j].getRealRegister()) {
        if (cg->getDebug())
          cg->getDebug()->printRegisterDependencies(comp->getOutFile(), this,
                                                    numberOfRegisters);
        TR_ASSERT(0, "checkDependencyGroup::Real reg dup found in register "
                     "dependencies\n");
      }
...
void TR_S390RegisterDependencyGroup::assignRegisters(
  ...
  int32_t i, j;
  bool changed;
  uint32_t availRegMask =
      checkDependencyGroup(currentInstruction, numberOfRegisters, cg);
  ...
  if (!comp->getOption(TR_DisableOOL)) {
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      if (dependentRegNum == TR::RealRegister::SpilledReg &&
          !_dependencies[i].getRegister(cg)->getRealRegister()) {
        TR_ASSERT(virtReg->getBackingStorage(), "should have a backing store "
                                                "if dependentRegNum == "
                                                "spillRegIndex()\n");
        ...
        if (virtReg->getAssignedRealRegister() || spilledHPR) {
          ...
          if (cg->supportsHighWordFacility() &&
              !comp->getOption(TR_DisableHighWordRA) && rk != TR_FPR &&
              rk != TR_VRF) {
            opCode =
                virtReg->is64BitReg() ? TR::InstOpCode::LG : TR::InstOpCode::L;

            if (assignedReg->isHighWordRegister()) {
              // virtReg was spilled to an HPR and now we need it spilled to
              // stack
              opCode = TR::InstOpCode::LFH;
              if (virtReg->is64BitReg()) {
                TR_ASSERT(virtReg->containsCollectedReference(),
                          "virtReg is assigned to HPR but is not a spilled "
                          "compress pointer");
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    if (virtReg->getAssignedRealRegister() != NULL) {
      ...
      if (_dependencies[i].getRealRegister() == TR::RealRegister::NoReg) {
        ...
        // A specific real reg is required. Get real reg assigned to this
        // virtreg
        //
        dependentRegNum = toRealRegister(virtReg->getAssignedRealRegister())
                              ->getRegisterNumber();

        // Traverse dependency list, block real reg if already assigned to a
        // different virt
        //
  ...
  // register dependency
  // list.  The following pieces of code are responsible for replacing these
  // stubs with actual
  // real register values, using register allocation routines in the
  // S390Machine, and hence,
  ...
  // for the low
  // and high order regs in the list of deps.  We update the low and high order
  // pair deps with
  // the real reg assigned by to the pair, and then let the these deps be
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    TR::RegisterPair *virtRegPair =
        _dependencies[i].getRegister(cg)->getRegisterPair();
    ...
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      if (dependentRegNum != TR::RealRegister::NoReg &&
        ...
        machine->coerceRegisterAssignment(currentInstruction, virtReg,
                                          dependentRegNum, DEPSREG);
        if (comp->getOption(TR_EnableTrueRegisterModel) &&
            _dependencies[i].getDefsRegister() &&
            virtReg->isPendingSpillOnDef()) {
          TR::RealRegister *realReg =
              machine->getS390RealRegister(dependentRegNum);
          if (cg->insideInternalControlFlow())
            machine->reverseSpillState(
                cg->getInstructionAtEndInternalControlFlow(), virtReg,
                toRealRegister(realReg));
          else
            machine->reverseSpillState(currentInstruction, virtReg,
                                       toRealRegister(realReg));
        }
        virtReg->block();
        changed = true;
...
// Coerce the assignment
          ...
          if (assignedRegister != NULL &&
            ...
            for (int32_t lcount = 0; lcount < numberOfRegisters; lcount++) {
              ...
              if (aRealNum != TR::RealRegister::NoReg &&
                ...
                if (comp->getOption(TR_EnableTrueRegisterModel) &&
                    _dependencies[lcount].getDefsRegister() &&
                    depReg->isPendingSpillOnDef()) {
                  TR::RealRegister *realReg =
                      machine->getS390RealRegister(aRealNum);
                  if (cg->insideInternalControlFlow())
                    machine->reverseSpillState(
                        cg->getInstructionAtEndInternalControlFlow(), depReg,
                        toRealRegister(realReg));
                  else
                    machine->reverseSpillState(currentInstruction, depReg,
                                               toRealRegister(realReg));
                }
                break;
              }
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    if (_dependencies[i].getRealRegister() == TR::RealRegister::AssignAny) {
      ...
      if (virtReg->isUsedInMemRef()) {
        ...
        // Assigning an AR-GPR pair will assign the GPR and create a new AR
        // dependency.
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    if (dependentRegister->getRegisterPair() == NULL &&
        (dependentRegister->getAssignedRegister() ||
         (comp->getOption(TR_EnableTrueRegisterModel) &&
          dependentRegister->isLive())) &&
      ...
      if ((dependentRegister->getFutureUseCount() == 0) ||
          (comp->getOption(TR_EnableTrueRegisterModel) &&
           !currentInstruction->isLabel() &&
           getRegisterDependency(i)->getDefsRegister() &&
           !getRegisterDependency(i)->getRefsRegister() &&
           !currentInstruction->defsRegister(dependentRegister))) {
        // check if need to free HW
        if (assignedRealRegister != NULL && cg->supportsHighWordFacility() &&
            !comp->getOption(TR_DisableHighWordRA)) {
          if (dependentRegister->is64BitReg() &&
              dependentRegister->getKind() != TR_FPR &&
              dependentRegister->getKind() != TR_VRF) {
            toRealRegister(assignedRealRegister)
                ->getHighWordRegister()
                ->setState(TR::RealRegister::Unlatched);
            cg->traceRegFreed(
                dependentRegister,
                toRealRegister(assignedRealRegister)->getHighWordRegister());
          }
        }
      ...
      if (highAssignedRegister &&
        ...
        if (cg->supportsHighWordFacility() &&
            !comp->getOption(TR_DisableHighWordRA)) {
          if (dependentRegisterHigh->is64BitReg() &&
              dependentRegisterHigh->getKind() != TR_FPR &&
              dependentRegisterHigh->getKind() != TR_VRF) {
            toRealRegister(assignedRegister)
                ->getHighWordRegister()
                ->setState(TR::RealRegister::Unlatched);
            cg->traceRegFreed(
                dependentRegisterHigh,
      ...
      if (lowAssignedRegister &&
        ...
        if (cg->supportsHighWordFacility() &&
            !comp->getOption(TR_DisableHighWordRA)) {
          if (dependentRegisterLow->is64BitReg() &&
              dependentRegisterLow->getKind() != TR_FPR &&
              dependentRegisterLow->getKind() != TR_VRF) {
            toRealRegister(assignedRegister)
                ->getHighWordRegister()
                ->setState(TR::RealRegister::Unlatched);
            cg->traceRegFreed(
                dependentRegisterLow,
                toRealRegister(assignedRegister)->getHighWordRegister());
          }
        }
      }
  ...
  if (!comp->getOption(TR_DisableOOL)) {
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      if (_dependencies[i].getRegister(cg)->getRealRegister()) {
        TR::RealRegister *highWordReg =
            toRealRegister(_dependencies[i].getRegister(cg));
        dependentRegNum = _dependencies[i].getRealRegister();
        if (dependentRegNum == TR::RealRegister::SpilledReg) {
          virtReg = highWordReg->getAssignedRegister();
          // printf ("\nOOL HPR Spill in %s", comp->signature());fflush(stdout);
          traceMsg(comp, "\nOOL HPR Spill: %s",
                   cg->getDebug()->getName(highWordReg));
