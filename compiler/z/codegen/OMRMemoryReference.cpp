 ...
 * eventually be removed after applying bumps for left or right alignment
 * So a premature LAY or LA/AHI fixup is not done for these negative offsets a
 * flag HasTemporaryNegativeOffset is set on the memRef to prevent this fixup
 * However, if there is already a real negative offset then the fixup is
 * required for this part of the negative offset
 * To deal with this first split off the real negative offset by calling
 * enforceSSFormatLimits and then set the HasTemporaryNegativeOffset flag and
 * add in the new negative offset
 */
void OMR::Z::MemoryReference::addToTemporaryNegativeOffset(
  ...
  if (offset < 0) {
    if (cg->traceBCDCodeGen())
      traceMsg(comp, "\taddToTemporaryNegativeOffset : %s (%p) new offset %d, "
                     "existing mr offset %d (existing mr hasTempNegOffset = "
                     "%s)\n",
    ...
    {
      if (cg->traceBCDCodeGen())
        traceMsg(comp, "\t\texisting mr->offset %d < 0 so call "
                       "enforceSSFormatLimits to clear this up before setting "
                       "HasTemporaryNegativeOffset\n",
                 self()->getOffset());
      self()->enforceSSFormatLimits(node, cg, NULL); // call SSFormatLimits to
                                                     // also take this chance to
                                                     // fold in an index
                                                     // register if needed
    }
 ...
 * incrementing the
 * refCounts for all children in the address tree. So if folding is not done
 * then poorer quality
 * code (due to more clobberEvals from the higher refCounts) will be generated.
 * In some simple cases where folding is very likely then forceFolding is set
 * (the higher refCounts do not
 * matter as the evaluators are not actually going to be called)
 * Note that setting forceFolding does not require that folding will be done it
 * only allows it to be possible
 */
bool OMR::Z::MemoryReference::setForceFoldingIfAdvantageous(
  ...
  if (addressChild->getRegister() != NULL) {
    ...
    //    iconst
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s "
                     "(%p) has register %s\n",
  ...
  if (!addressChild->getOpCode().isAdd()) {
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s "
                     "(%p) is not an add type - most likely cannot fold\n",
               addressChild->getOpCode().getName(), addressChild);
    }
    return false;
  }
  ...
  if (!addressChild->getSecondChild()->getOpCode().isLoadConst()) {
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s "
                     "(%p) second child is not a load const - cannot fold\n",
               addressChild->getOpCode().getName(), addressChild);
    }
    return false;
  }
  ...
  if (addressChild->getSecondChild()->getRegister() != NULL) {
    ...
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s "
                     "(%p) const child %s (%p) has register %s, cannot fold.\n",
               addressChild->getOpCode().getName(), addressChild,
  ...
  if (addressChild->getReferenceCount() != 1) {
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s "
                     "(%p) has ref count %d, cannot fold, need value in "
                     "register.\n",
               addressChild->getOpCode().getName(), addressChild,
  ...
  if (haveToEvalConvIntoRegister) {
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s "
                     "(%p) first child has conv. with ref count > 1 and no "
  ...
  // reasons
  // only allow folding if conversions are deemed 'unneeded' by LoadExtension
  // step

  if (!allConvNodesAreUnneeded) {
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s "
                     "(%p) first child has conv. which are needed, folding "
                     "after evaluation.\n",
               addressChild->getOpCode().getName(), addressChild);
    }
    cg->evaluate(addressChild->getFirstChild());
  ...
  if (foundRegister) {
    ...
    if (cg->traceBCDCodeGen()) {
      ...
      traceMsg(comp, "\t\taddressChild %s (%p) node %s (%p) is "
                     "evaluatedReg+const (%s + %lld) so %s=true\n",
               addressChild->getOpCode().getName(), addressChild,
  ...
  if (!eventualNonConversionIsSuitableForFolding) {
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s "
                     "(%p) eventual non conversion %s (%p) is not suitable for "
                     "folding.\n",
               addressChild->getOpCode().getName(), addressChild,
  ...
  if (!eventualNonConversion->getOpCode().isIndirect()) {
    ...
    if (cg->traceBCDCodeGen()) {
      traceMsg(
          comp, " inside setForceFoldingIfAdvantageous, eventualNonConversion "
  ...
  } else // if (eventualNonConversion->getReferenceCount() == 1)
  {
    ...
    if (cg->traceBCDCodeGen()) {
      traceMsg(
          comp, " inside setForceFoldingIfAdvantageous, eventualNonConversion "
                "%s (%p) has no register and refCount==1 and is an "
                "iaload+const so eval(%s - %p) and setForceFolding=true\n",
          eventualNonConversion->getOpCode().getName(), eventualNonConversion,
 ...
 * recursivelyIncrementReferenceCount), so evaluation (and therefore
 * the reference count decrementing that evaluators do) will miss nodes that are
 * part of the incremented nodes list and that are under a node that
 * was assigned a register after memory reference population. This function will
 * descend the tree and look for this case, only removing nodes if a
 * node with a register was seen. It is important to only decend down nodes that
 * are part of the incremented nodes list, because that is what the
...
OMR::Z::MemoryReference::MemoryReference(TR::Node *rootLoadOrStore,
    ...
    : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
      ...
      _originalSymbolReference(rootLoadOrStore->getOpCode().hasSymbolReference()
                                   ? rootLoadOrStore->getSymbolReference()
                                   : NULL),
      _listingSymbolReference(NULL), _offset(0), _displacement(0), _flags(0),
  ...
  if (rootLoadOrStore->getOpCode().isIndirect() ||
    ...
    // indexRegister may setup in populateMemoryReference to hold the address
    // for the load/store
    // since indexRegister is invalid in a RS instruction (LM/STM for long), we
    // need to insert an intermediate
    // LA instruction to get the address using index and base register before
    // doing LM/STM
    // we'll never generate LM to load/store long int on 64bit platform
    // storageReference cases may contain symbol sizes from 1->64 and in any
    // case this condition is now enforced when
    // the subsequent generate*Instruction routine is called with this memRef
    if ((rootLoadOrStore->getType().isInt64() ||
         (symbol && symbol->getSize() == 8)) &&
        !(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) &&
        !canUseRX && (storageReference == NULL)) {
      self()->separateIndexRegister(rootLoadOrStore, cg, false,
                                    NULL); // enforce4KDisplacementLimit=false
    }
  } else {
    // symbol is static
    if (symbol && symbol->isStatic()) {
...
OMR::Z::MemoryReference::MemoryReference(TR::Node *addressChild,
    ...
    : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
      ...
      _offset(0), _displacement(0), _flags(0), _storageReference(NULL),
      _fixedSizeForAlignment(0), _leftMostByte(0), _name(NULL),
      _incrementedNodesList(cg->comp()->trMemory()) {
  bool done = false;
  if (addressChild->getRegister() == NULL) {
    if (addressChild->getReferenceCount() == 1 &&
...
OMR::Z::MemoryReference::MemoryReference(TR::Node *node,
    ...
    : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _targetSnippet(NULL), _targetSnippetInstruction(NULL),
      _displacement(0), _flags(0), _storageReference(storageReference),
      _fixedSizeForAlignment(0), _leftMostByte(0), _name(NULL),
      _incrementedNodesList(cg->comp()->trMemory()) {
...
OMR::Z::MemoryReference::MemoryReference(TR::Snippet *s, TR::Register *indx,
    ...
    : _baseNode(NULL), _baseRegister(NULL), _indexRegister(indx),
      _indexNode(NULL), _targetSnippetInstruction(NULL), _displacement(0),
...
OMR::Z::MemoryReference::MemoryReference(int32_t disp, TR::CodeGenerator *cg,
    ...
    : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _targetSnippet(NULL), _targetSnippetInstruction(NULL),
      _displacement(0), _flags(0), _storageReference(NULL),
      _fixedSizeForAlignment(0), _leftMostByte(0), _name(NULL),
      _incrementedNodesList(cg->comp()->trMemory()) {
  TR::Compilation *comp = cg->comp();
  {
...
OMR::Z::MemoryReference::MemoryReference(TR::Register *br, int32_t disp,
    ...
    : _baseRegister(br), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _targetSnippet(NULL), _targetSnippetInstruction(NULL),
      _displacement(0), _offset(disp), _flags(0), _storageReference(NULL),
...
OMR::Z::MemoryReference::MemoryReference(TR::Register *br, int32_t disp,
    ...
    : _baseRegister(br), _symbolReference(symRef), _baseNode(NULL),
      ...
      _targetSnippetInstruction(NULL), _displacement(0), _offset(disp),
...
OMR::Z::MemoryReference::MemoryReference(TR::Register *br, TR::Register *ir,
    ...
    : _baseRegister(br), _baseNode(NULL), _indexRegister(ir), _indexNode(NULL),
      _targetSnippet(NULL), _targetSnippetInstruction(NULL), _displacement(0),
      _offset(disp), _flags(0), _storageReference(NULL),
      _fixedSizeForAlignment(0), _leftMostByte(0), _name(NULL),
      _incrementedNodesList(cg->comp()->trMemory()) {
  _symbolReference =
...
OMR::Z::MemoryReference::getTargetAddressSnippet() {
  return self()->isTargetAddressSnippet()
             ? (TR::S390TargetAddressSnippet *)_targetSnippet
             : NULL;
}

TR::S390TargetAddressSnippet *OMR::Z::MemoryReference::setTargetAddressSnippet(
    TR::S390TargetAddressSnippet *s) {
...
TR::S390LookupSwitchSnippet *OMR::Z::MemoryReference::getLookupSwitchSnippet() {
  return self()->isLookupSwitchSnippet()
             ? (TR::S390LookupSwitchSnippet *)_targetSnippet
             : NULL;
}

TR::S390LookupSwitchSnippet *OMR::Z::MemoryReference::setLookupSwitchSnippet(
    TR::S390LookupSwitchSnippet *s) {
  ...
  for (uint32_t i = 0; i < nodeArray.size(); i++) {
    ...
    if ((nodeArray[i]->getReferenceCount() == 1) &&
      ...
      if (comp->getOption(TR_TraceCG)) {
        traceMsg(
            comp, " ArtificiallyInflateReferenceCountWhenNecessary: bug "
                  "potential with node %p (reg %p %s%s): ref count after %s is "
                  "1.\n",
      ...
      if (incrementedNodesList) {
        ...
        // somewhere else - look for the reference count
        // changing like so:
        //
        // 1 -> N -> 1
        //
        ...
        if (listNode == nodeArray[i]) {
          ...
          if (comp->getOption(TR_TraceCG)) {
            traceMsg(comp, " ArtificiallyInflateReferenceCountWhenNecessary: "
                           "skip inflating node %p (reg %p %s%s) because it "
                           "exists in incrementedNodesList.\n",
                     nodeArray[i], nodeArray[i]->getRegister(),
                     nodeArray[i]->getRegister()
...
void OMR::Z::MemoryReference::populateShiftLeftTree(TR::Node *subTree,
  ...
  if (((subTree->getOpCodeValue() == TR::ishl) &&
    ...
    if (firstRegister == NULL && firstChild->getOpCode().hasSymbolReference() &&
...
bool OMR::Z::MemoryReference::tryBaseIndexDispl(TR::CodeGenerator *cg,
  ...
  if (topAdd->getRegister() == NULL &&
      topAdd->getOpCode().hasSymbolReference() &&
      topAdd->getSymbolReference()->getSymbol()->isRegisterSymbol())
    cg->evaluate(topAdd);
  if (topAdd->getRegister() != NULL)
    return false;
  if (integerChild->getOpCode().isLoadConst())
  ...
  if (breg == NULL && base->getOpCode().hasSymbolReference() &&
      base->getSymbolReference()->getSymbol()->isRegisterSymbol())
    breg = cg->evaluate(base);
  if (breg == NULL && base->getReferenceCount() <= 1)
    return false;
  ireg = index->getRegister();
  if (ireg == NULL && index->getOpCode().hasSymbolReference() &&
      index->getSymbolReference()->getSymbol()->isRegisterSymbol())
...
bool OMR::Z::MemoryReference::ZeroBasePtr_EvaluateSubtree(
  ...
  case TR::cconst: // load unicode constant (16-bit unsigned)
  {
    ...
    // a null pointer error, but
    // if the base of a memory reference is explicitly set with "based(0)", then
    // the intention is to base
    // off of the contents of R0, which could be a constant 0. This avoids
    // flagging the instruction as
...
void OMR::Z::MemoryReference::populateMemoryReference(TR::Node *subTree,
  ...
  if (self()->doEvaluate(subTree, cg)) {
    ...
    // because it is not the doEvaluate branch
    // and there is the possibility that the mem ref will not be loaded into a
    // register.
    cg->AddFoldedMemRefToStack(self());
  ...
  if (noopNode && noopNode->getRegister() == NULL) {
    if (subTree->getRegister() && subTree->getRegister()->getRegisterPair() &&
        !subTree->getRegister()->getRegisterPair()->isArGprPair())
...
TR::Register *OMR::Z::MemoryReference::swapBaseRegister(TR::Register *br,
  ...
  if (base && base->isArGprPair()) {
    TR::RegisterPair *regpair =
        cg->allocateArGprPair(base->getARofArGprPair(), br);
...
TR::Instruction *OMR::Z::MemoryReference::handleLargeOffset(
  ...
  if (_offset > MINLONGDISP && _offset < MAXLONGDISP) {
    ...
    if (preced)
      preced = generateRXYInstruction(cg, op, node, tempTargetRegister,
                                      interimMemoryReference, preced);
    else
      generateRXYInstruction(cg, op, node, tempTargetRegister,
                             interimMemoryReference);
  } else {
    op = TR::InstOpCode::LA;
...
bool OMR::Z::MemoryReference::alignmentBumpMayRequire4KFixup(
  ...
  // e.g. _offset=4088 and the node is 10 byte in size so the max possible
  // alignment is 10-1 = 9 giving a max possible offset of 4088+9 = 4097 (>=
  // MAXDISP of 4096)
  //
  if (_offset < MAXDISP && // skip if going to fixup naturally anyway
    ...
    int32_t maxAlignmentBump = self()->getFixedSizeForAlignment() -
                               1; // alignment is getFixedSizeForAlignment() -
                                  // (length or leftMostByte) and each of these
                                  // must be at least 1
    if (_offset + maxAlignmentBump >= MAXDISP) {
      if (cg->traceBCDCodeGen())
        traceMsg(
            cg->comp(), "\tz^z : node %s (%p) : _offset %d + maxAlignmentBump "
                        "%d >= MAXDISP %d : (%d >= %d) -- force 4K fixup\n",
            node ? node->getOpCode().getName() : "NULL", node, _offset,
            maxAlignmentBump, MAXDISP, _offset + maxAlignmentBump, MAXDISP);
      return true;
    }
  }
  return false;
 ...
 * The forceFixup flag will skip the displacement check and always use a
 * register. It is useful
 * for cases where the memref will be reused with an additional offset that will
 * put it above
 * the limit, but there is a reason to do the fixup early (such as an internal
 * control flow
 * region).
 */
TR::Instruction *OMR::Z::MemoryReference::enforce4KDisplacementLimit(
  ...
  TR_ASSERT(!forcePLXFixup, "This logic is only used for "
                            "markAndAdjustForLongDisplacementIfNeeded. You "
...
void OMR::Z::MemoryReference::assignRegisters(
  ...
  if (_indexRegister && _indexRegister->isArGprPair() &&
      _baseRegister == NULL) {
    self()->setBaseRegister(_indexRegister, cg);
    _indexRegister = NULL;
  }
  ...
  if (_baseRegister != NULL && _baseRegister->getRealRegister() == NULL) {
    ...
    // We postpone bookkeeping until after _indexRegister is assigned to avoid
    // assigning
    ...
    if (_baseRegister != NULL && _baseRegister->getRealRegister() == NULL)
      assignedBaseRegister =
          (TR::RealRegister *)machine->assignBestRegisterSingle(
    ...
    if (virtualBaseAR != NULL) {
      currentInstruction->addARDependencyCondition(virtualBaseAR,
                                                   assignedBaseRegister);
    }
    if (_indexRegister != NULL) {
  ...
  if (_baseRegister != NULL && _baseRegister->getRealRegister() == NULL) {
    ...
    if (_baseRegister->decFutureUseCount() == 0) {
      ...
      if (cg->supportsHighWordFacility() &&
        ...
        toRealRegister(assignedBaseRegister)
            ->getHighWordRegister()
...
/**
 * Generate binary encoding for the second memory reference operand in an
 * instruction
 *
 ...
 */
uint8_t *
OMR::Z::MemoryReference::generateBinaryEncoding2(uint8_t *cursor,
                                                 TR::CodeGenerator *cg) {
...
int32_t OMR::Z::MemoryReference::getRightAlignmentBump(TR::Instruction *instr,
  ...
  if (length == -1) {
    TR_ASSERT(false, "only SS and bcdLoadaddr instructions should be right "
                     "aligned (inst %p)\n",
              instr);
    needsBump = false;
  }
...
int32_t OMR::Z::MemoryReference::getLeftAlignmentBump(TR::Instruction *instr,
  ...
  TR_ASSERT(self()->getTotalSizeForAlignment() > 0, "getTotalSizeForAlignment()"
                                                    " %d not initialized in "
...
int32_t OMR::Z::MemoryReference::calcDisplacement(uint8_t *cursor,
  ...
  if (self()->rightAlignMemRef()) {
    TR_ASSERT(!self()->leftAlignMemRef(), "A memory reference should not be "
                                          "marked as both right and left "
                                          "aligned\n");
    if (cg->traceBCDCodeGen())
      traceMsg(
          comp, "instr %p : right aligning memRef with symRef #%d (%s, isTemp "
                "%s) _offset %d, totalSize %d : bump = ",
  ...
  if (self()->leftAlignMemRef()) {
    TR_ASSERT(!self()->rightAlignMemRef(), "A memory reference should not be "
                                           "marked as both left and right "
                                           "aligned\n");
    if (cg->traceBCDCodeGen())
      traceMsg(
          comp, "instr %p : left aligning memRef with symRef #%d (%s, isTemp "
                "%s), _offset %d : bump = totalSize - leftMostByte = %d - %d "
                "= ",
          instr, self()->getSymbolReference()->getReferenceNumber(),
          cg->getDebug()->getName(self()->getSymbolReference()->getSymbol()),
          self()->getSymbolReference()->isTempVariableSizeSymRef() ? "yes"
  ...
  if (self()->getSymbolReference()->isTempVariableSizeSymRef()) {
    if (cg->traceBCDCodeGen())
      traceMsg(
          comp, "instr %p : bumping tempVariableSizeSymRef memRef with symRef "
                "#%d (%s), _offset %d : bump = symSize - totalSize = %d - %d "
                "= ",
          instr, self()->getSymbolReference()->getReferenceNumber(),
          cg->getDebug()->getName(self()->getSymbolReference()->getSymbol()),
  ...
  if (snippet != NULL) {
    {
      ...
      if (cursor != NULL) {
        cg->addRelocation(new (cg->trHeapMemory())
                              TR_12BitLabelRelativeRelocation(
...
int32_t OMR::Z::MemoryReference::generateBinaryEncoding(
  ...
  // Most cases we will know at inst selection phase that the disp is big.  But
  // it is still possible
  // for us to hit bin phase and realize the DISP is large (eg when the stack is
  // large).
  if (disp < 0 || disp >= MAXDISP) {
    ...
    TR_ASSERT(scratchReg != NULL, "OMR::Z::MemoryReference::"
                                  "generateBinaryEncoding -- A scratch reg "
                                  "should always be found.");

    // If the instr is already of RSY/RXY/SIY form && its disp is within the
    // range, no fixup is needed.
    if ((instr->hasLongDisplacementSupport() ||
         instr->getOpCode().getInstructionFormat(instr->getOpCodeValue()) ==
             RSY_FORMAT ||
         instr->getOpCode().getInstructionFormat(instr->getOpCodeValue()) ==
             RXY_FORMAT ||
         instr->getOpCode().getInstructionFormat(instr->getOpCodeValue()) ==
      ...
      currInstr = generateImmToRegister(cg, node, scratchReg, (intptr_t)(disp),
                                        prevInstr);
      currInstr->setEstimatedBinaryLength(
          currInstr->getOpCode().getInstructionLength());
      ...
      if (TR::Compiler->target.is64Bit()) {
        ...
        *(int32_t *)cursor =
            boi(0xa7090000 | (disp & 0xFFFF)); // LGHI Rscrtch, disp&0xFFFF
        scratchReg->setRegisterField((uint32_t *)cursor);
      } else {
        if (!comp->getOption(TR_DisableLongDispStackSlot) && spillNeeded) {
          *(int32_t *)cursor =
              boi(0x50005000 | (offsetToLongDispSlot &
                                0xFFF)); // ST Rscrtch,offToLongDispSlot(,GPR5)
          scratchReg->setRegisterField((uint32_t *)cursor);
          cursor += 4;
          nbytes += 4;
        }
        ...
        *(int32_t *)cursor =
            boi(0xa7080000 | (disp & 0xFFFF)); // LHI Rscrtch, disp&0xFFFF
        scratchReg->setRegisterField((uint32_t *)cursor);
      }
      cursor += 4;
      nbytes += 4;
      ...
      TR_ASSERT(high <= MAX_IMMEDIATE_VAL && high >= MIN_IMMEDIATE_VAL,
                "OMR::Z::MemoryReference::generateBinaryEncoding -- Immediate "
                "value out of range.");
      ...
      if (TR::Compiler->target.is64Bit()) {
        ...
        *(int32_t *)cursor = boi(
            0xa7090000 | (high & 0xFFFF)); //  LGHI Rscrtch, (disp>>12)&0xFFFF
        scratchReg->setRegisterField((uint32_t *)cursor);
        cursor += 4;
        nbytes += 4;
        ...
        *(int16_t *)cursor = bos(0x000d);
        ...
        if (low) {
          *(int32_t *)cursor =
              boi(0x41000000 | low); //  LA Rscrtch, disp&0xFFF(,Rscrtch)
          scratchReg->setRegisterField((uint32_t *)cursor);
        ...
        *(int32_t *)cursor = boi(
            0xa7080000 | (high & 0xFFFF)); //  LHI Rscrtch, (disp>>12)&0xFFFF
        scratchReg->setRegisterField((uint32_t *)cursor);
        cursor += 4;
        nbytes += 4;
    ...
    {
      TR_ASSERT(0, "OMR::Z::MemoryReference::generateBinaryEncoding -- 0x%x "
                   "displacement not handled\n",
                disp);
    }
    ...
    if (largeDisp) {
      if (instr->getKind() == TR::Instruction::IsRS ||
        ...
        TR_ASSERT(base != NULL, "OMR::Z::MemoryReference::"
                                "generateBinaryEncoding --  Expected non-NULL "
                                "base reg for RS/S/SI type inst.");
        ...
        if (useNodesForLongFixup) {
          ...
          cg->setBinaryBufferCursor(cursor =
                                        currInstr->generateBinaryEncoding());
          prevInstr = currInstr;
        } else {
  ...
  if (instr->getKind() == TR::Instruction::IsRXY ||
    ...
    *nextWCursor |= boi((disp & 0x000FF000) << 12);
    ...
    TR_ASSERT((disp >= 0 && disp < MAXDISP),
              "memory reference: displacement %d out of range", disp);
  ...
  if (!comp->getOption(TR_DisableLongDispStackSlot)) {
    ...
    TR_ASSERT(offsetToLongDispSlot < MAXDISP,
              "OMR::Z::MemoryReference::generateBinaryEncoding -- "
              "offsetToLongDispSlot %d >= MAXDISP\n",
              offsetToLongDispSlot);

    // We don't allow stack slot to be used for branch insts, as gcmaps would be
    // wrong on the gcpoint.
    if (largeDisp) {
      TR_ASSERT(instr->getOpCode().isBranchOp() == false,
                "OMR::Z::MemoryReference::generateBinaryEncoding -- We don't "
                "handle long disp for branch insts %p using ExtCodeBase stack "
                "slot.\n",
                instr);
    }
    ...
    if (!largeDisp && self()->getCheckForLongDispSlot() &&
      ...
      TR_ASSERT(0, "OMR::Z::MemoryReference::generateBinaryEncoding -- BAD use "
                   "of ExtCodeBase SLOT %d on the stack %x node is %x.\n",
  ...
  } else if (largeDisp) // && no long disp slot
  {
    TR_ASSERT(cg->isExtCodeBaseFreeForAssignment() == false,
              "OMR::Z::MemoryReference::generateBinaryEncoding -- Ext Code "
              "Base was wrongly released\n");
  }
...
int32_t OMR::Z::MemoryReference::generateBinaryEncodingTouchUpForLongDisp(
  ...
  if (comp->getOption(TR_DisableLongDispStackSlot)) {
    ...
    TR_ASSERT(!self()->isMemRefMustNotSpill(),
              "OMR::Z::MemoryReference::"
              "generateBinaryEncodingTouchUpForLongDisp -- This "
              "memoryReference must not spill");
    if (!is2ndSSMemRef) {
  ...
  TR_ASSERT(scratchReg != NULL, "OMR::Z::MemoryReference::"
                                "generateBinaryEncodingTouchUpForLongDisp -- A "
                                "scratch reg should always be found.");

  // Restore the scratch register
  if (!comp->getOption(TR_DisableLongDispStackSlot) && largeDisp &&
      spillNeeded) {
    if (useNodesForLongFixup) {
      ...
      // May need to copy some attributes of instr into currInstr e.g. lastWarm
      // etc.
      ...
      *(int32_t *)cursor =
          boi(0x58005000 | (offsetToLongDispSlot &
                            0xFFF)); // L  Rscrtch,offToLongDispSlot(,GPR5)
      scratchReg->setRegisterField((uint32_t *)cursor);
      cursor += 4;
      nbytes += 4;
    }
...
bool OMR::Z::MemoryReference::doEvaluate(TR::Node *subTree,
  ...
  if (self()->forceFirstTimeFolding()) {
    if (cg->traceBCDCodeGen())
      traceMsg(
          comp, "\tforceFirstTimeFolding=true for subTree %s (%p) refCount %d, "
 ...
 * within this file
 * calls from outside must go through the node based wrapper routine so the
 * correct symRef is
 * pulled off the node
 */
...
TR::MemoryReference *reuseS390MemoryReference(TR::MemoryReference *baseMR,
  ...
  // new memory reference based on current offset
  // and return New MR. This will prevent original Memory reference being
  // modified even if it hasn't been used before.
...
generateS390AlignedMemoryReference(TR::MemoryReference &baseMR, TR::Node *node,
  ...
  // for negative offsets that will
