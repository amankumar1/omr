...
bool OMR::Z::Register::isArGprPair() {
  if (self()->getRegisterPair() && self()->getLowOrder()->getKind() == TR_AR &&
      (self()->getHighOrder()->getKind() == TR_GPR ||
       self()->getHighOrder()->getKind() == TR_GPR64))
    return true;
...
TR::Register *OMR::Z::Register::getARofArGprPair() {
  return self()->getLowOrder();
}

TR::Register *OMR::Z::Register::getGPRofArGprPair() {
  if (self()->isArGprPair())
    return self()->getHighOrder();
  else
    return self();
}
...
bool OMR::Z::Register::assignToHPR() {
  return (!(self()->is64BitReg()) && _flags.testAny(AssignToHPR));
}

bool OMR::Z::Register::assignToGPR() {
  return (!(self()->is64BitReg()) && !_flags.testAny(AssignToHPR));
}

bool OMR::Z::Register::isHighWordUpgradable() {
  return (!_flags.testAny(IsNotHighWordUpgradable) &&
          !self()->isUsedInMemRef() && !self()->containsCollectedReference() &&
          !self()->containsInternalPointer() && !self()->getRegisterPair() &&
          (self()->getKind() == TR_GPR) && !self()->is64BitReg());
}
