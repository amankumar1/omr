...
uint32_t OMR::Z::InstOpCode::isAdmin() {
  return (_mnemonic == DIRECTIVE || _mnemonic == RET ||
          _mnemonic == ASSOCREGS || _mnemonic == DEPEND || _mnemonic == FENCE ||
          _mnemonic == SCHEDFENCE || _mnemonic == PROC || _mnemonic == DC ||
          _mnemonic == DC2 || _mnemonic == LOCK || _mnemonic == UNLOCK ||
          _mnemonic == ASM || _mnemonic == DS || _mnemonic == DCB);
}

uint32_t OMR::Z::InstOpCode::isHighWordInstruction() {
  return (_mnemonic == AHHHR || _mnemonic == AHHLR || _mnemonic == AIH ||
          _mnemonic == ALHHHR || _mnemonic == ALHHLR || _mnemonic == ALSIH ||
          _mnemonic == ALSIHN || _mnemonic == BRCTH || _mnemonic == CHF ||
          _mnemonic == CHHR || _mnemonic == CHLR || _mnemonic == CIH ||
          _mnemonic == CLHF || _mnemonic == CLHHR || _mnemonic == CLHLR ||
          _mnemonic == CLIH || _mnemonic == LBH || _mnemonic == LHH ||
          _mnemonic == LFH || _mnemonic == LFHAT || _mnemonic == LLCH ||
          _mnemonic == LLHH || _mnemonic == RISBHG || _mnemonic == RISBLG ||
          _mnemonic == STCH || _mnemonic == STHH || _mnemonic == STFH ||
          _mnemonic == SHHHR || _mnemonic == SHHLR || _mnemonic == SLHHHR ||
          _mnemonic == SLHHLR);
}
...
uint64_t OMR::Z::InstOpCode::isOperandLW(uint32_t i) {
  uint64_t mask = ((i == 1) ? S390OpProp_TargetLW : 0) |
                  ((i == 2) ? S390OpProp_SrcLW : 0) |
                  ((i == 3) ? S390OpProp_Src2LW : 0);
  return properties[_mnemonic] & mask;
}
...
#define GET_OPCODE_FROM_NODE(opKind, opcode64, opcode32)                       \
  TR::InstOpCode::Mnemonic OMR::Z::InstOpCode::get##opKind##OpCodeFromNode(    \
      TR::Node *node) {                                                        \
    if (TR::Compiler->target.is64Bit() &&                                      \
        (node->getType().isInt64() || node->getType().isAddress())) {          \
      return opcode64;                                                         \
    }                                                                          \
    return opcode32;                                                           \
  }
...
GET_OPCODE_FROM_NODE(CmpHalfWordImmToMem, TR::InstOpCode::CGHSI,
                     TR::InstOpCode::CHSI)
...
GET_OPCODE_FROM_NODE(SubtractThreeReg, TR::InstOpCode::SGRK,
                     TR::InstOpCode::SRK)

// getSubtractLogicalImmOpCodeFromNode
GET_OPCODE_FROM_NODE(SubtractLogicalImm, TR::InstOpCode::SLGFI,
                     TR::InstOpCode::SLFI)
...
GET_OPCODE_FROM_NODE(MultiplySingleReg, TR::InstOpCode::MSGR,
                     TR::InstOpCode::MSR)
...
GET_OPCODE_FROM_NODE(CmpLogicalTrap, TR::InstOpCode::CLGRT,
                     TR::InstOpCode::CLRT)

// getCmpLogicalWidenTrapOpCodeFromNode
GET_OPCODE_FROM_NODE(CmpLogicalWidenTrap, TR::InstOpCode::CLGFRT,
                     TR::InstOpCode::CLRT)

// getCmpLogicalImmTrapOpCodeFromNode
GET_OPCODE_FROM_NODE(CmpLogicalImmTrap, TR::InstOpCode::CLGIT,
                     TR::InstOpCode::CLFIT)
...
GET_OPCODE_FROM_NODE(ShiftLeftLogicalSingle, TR::InstOpCode::SLLG,
                     TR::InstOpCode::SLL)
...
GET_OPCODE_FROM_NODE(CmpLogicalRegWiden, TR::InstOpCode::CLGFR,
                     TR::InstOpCode::CLR)
...
TR::InstOpCode::Mnemonic OMR::Z::InstOpCode::getCmpWidenTrapOpCode() {
  return TR::Compiler->target.is64Bit() ? TR::InstOpCode::CGFRT
                                        : TR::InstOpCode::CRT;
}
...
TR::InstOpCode::Mnemonic OMR::Z::InstOpCode::getCmpLogicalWidenTrapOpCode() {
  return TR::Compiler->target.is64Bit() ? TR::InstOpCode::CLGFRT
                                        : TR::InstOpCode::CLRT;
}
