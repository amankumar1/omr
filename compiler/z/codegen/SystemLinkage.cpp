...
TR::SystemLinkage *TR_S390SystemLinkage::self() {
  return static_cast<TR::SystemLinkage *>(this);
}

void TR_S390SystemLinkage::initS390RealRegisterLinkage() {
...
// May lock this if GOT needed
  ...
  // Similar code in TR_S390Machine::initializeGlobalRegisterTable() for GRA
  // tables

  for (icount = TR::RealRegister::FirstGPR; icount < TR::RealRegister::LastGPR;
       ++icount) {
    ...
    if (cg()->machine()->isRestrictedReg(regReal->getRegisterNumber())) {
      ...
      if (cg()->supportsHighWordFacility() &&
          !comp()->getOption(TR_DisableHighWordRA) &&
          TR::Compiler->target.is64Bit()) {
        TR::RealRegister *tempHigh =
            toRealRegister(regReal)->getHighWordRegister();
        tempHigh->setState(TR::RealRegister::Locked);
        tempHigh->setAssignedRegister(tempHigh);
        tempHigh->setHasBeenAssignedInMethod(true);
      }
  ...
  for (icount = TR::RealRegister::FirstGPR;
    ...
    cg()->machine()
        ->getS390RealRegister((TR::RealRegister::RegNum)icount)
        ->setWeight(weight);
  }
}

// Non-Java use
void TR_S390SystemLinkage::initParamOffset(
...
TR_S390SystemLinkage::createAutoMarkerSymbol(TR_S390AutoMarkers markerType) {
  ...
  TR::AutomaticSymbol *sym =
      TR::AutomaticSymbol::createMarker(trHeapMemory(), name);
  TR::SymbolReference *symRef =
      new (trHeapMemory()) TR::SymbolReference(comp()->getSymRefTab(), sym);

  TR_Array<TR::SymbolReference *> *symbols = _autoMarkerSymbols;
  if (symbols == NULL) {
    symbols = new (trHeapMemory()) TR_Array<TR::SymbolReference *>(
        trMemory(), TR_AutoMarker_NumAutoMarkers, true);
 ...
 */
TR::Instruction *
TR_S390SystemLinkage::getputFPRs(TR::InstOpCode::Mnemonic opcode,
  ...
  beginSaveOffset = getFPRSaveAreaEndOffset(); // end offset is really begin
                                               // offset -- see mapStack()
  ...
  for (i = firstSaved; i <= lastSaved; ++i) {
    if (FPRSaveMask & (1 << (i))) {
      ...
      cursor = generateRXInstruction(
          cg(), opcode, node,
          getS390RealRegister(REGNUM(i + TR::RealRegister::FirstFPR)), rsa,
          cursor);
      j++;
    }
  }
 ...
 */
TR::Instruction *
TR_S390SystemLinkage::getputARs(TR::InstOpCode::Mnemonic opcode,
                                TR::Instruction *cursor, TR::Node *node,
                                TR::RealRegister *spReg) {
  int16_t ARSaveMask;
  int32_t offset, firstSaved, lastSaved, beginSaveOffset;
  ...
  beginSaveOffset = getARSaveAreaEndOffset(); // end offset is really begin
                                              // offset -- see mapStack()
  ...
  for (i = firstSaved; i <= lastSaved; ++i) {
    if (ARSaveMask & (1 << (i))) {
      ...
      cursor = generateRSInstruction(
          cg(), opcode, node,
          getS390RealRegister(REGNUM(i + TR::RealRegister::FirstAR)),
          getS390RealRegister(REGNUM(i + TR::RealRegister::FirstAR)), rsa,
          cursor);
      j++;
    }
  }
...
TR::Instruction *
TR_S390SystemLinkage::getputHPRs(TR::InstOpCode::Mnemonic opcode,
                                 TR::Instruction *cursor, TR::Node *node,
                                 TR::RealRegister *spReg) {
  int16_t HPRSaveMask;
  int32_t offset, firstSaved, lastSaved, beginSaveOffset;
  ...
  beginSaveOffset = getHPRSaveAreaEndOffset(); // following suit, see other
                                               // getput functions, map stack
  ...
  for (i = firstSaved; i <= lastSaved; ++i) {
    ...
    if (previousWasOne && (i == lastSaved || !(HPRSaveMask & (1 << i)))) {
      ...
      cursor = generateRSInstruction(
          cg(), opcode, node,
          getS390RealRegister(REGNUM(k + TR::RealRegister::FirstGPR)),
          getS390RealRegister(REGNUM(i - shift + TR::RealRegister::FirstGPR)),
          rsa, cursor);
    }
 ...
 */
TR::Instruction *TR_S390SystemLinkage::addImmediateToRealRegister(
 ...
 */
uint16_t TR_S390SystemLinkage::flipBitsRegisterSaveMask(uint16_t mask) {
...
TR_S390zOSSystemLinkage::TR_S390zOSSystemLinkage(TR::CodeGenerator *codeGen)
  ...
  setDebugHooksRegister(TR::Compiler->target.is64Bit()
 ...
 */
uint32_t TR_S390zOSSystemLinkage::calculateInterfaceMappingFlags(
  ...
  while ((parmCursor != NULL) && !done) {
    ...
    done = updateFloatParmDescriptorFlags(
        &parmDescriptorFields, funcSymbol, parmCount, argSize, dataType,
        &floatParmNum, &lastFloatParmAreaOffset, &parmAreaOffset);
 ...
 */
uint32_t TR_S390zOSSystemLinkage::calculateReturnValueAdjustFlag(
    TR::DataType dataType, int32_t aggregateLength) {
 ...
 * determined.
 */
bool TR_S390zOSSystemLinkage::updateFloatParmDescriptorFlags(
    uint32_t *parmDescriptorFields, TR::Symbol *funcSymbol, int32_t parmCount,
    int32_t argSize, TR::DataType dataType, int32_t *floatParmNum,
    uint32_t *lastFloatParmAreaOffset, uint32_t *parmAreaOffset) {
  ...
  if (numFPRsNeeded != 0) {
    ...
    (*parmDescriptorFields) |=
        shiftFloatParmDescriptorFlag(val, (*floatParmNum));
    ...
    if (numFPRsNeeded == 2) {
      (*parmDescriptorFields) |= shiftFloatParmDescriptorFlag(
          0x33, (*floatParmNum)); // 0x33 == bits 110001
      (*floatParmNum)++;
      if ((*floatParmNum) >= getNumFloatArgumentRegisters()) {
        return true; // done
      }
...
TR::Register *TR_S390zOSSystemLinkage::loadEnvironmentIntoRegister(
    TR::Node *callNode, TR::RegisterDependencyConditions *deps,
    bool loadIntoEnvironmentRegister) {
  // Important note: this should only be called in pre-prologue phase for
  // routines we know are
  // not leaf routines.

  TR::Register *envReg;
  if (loadIntoEnvironmentRegister) {
    TR::Register *systemEnvironmentRegister = deps->searchPostConditionRegister(
        getEnvironmentPointerRegister()); // gpr 5
 ...
 * WCode use
 */
int32_t TR_S390zOSSystemLinkage::getOutgoingParameterBlockSize() {
  //
...
////////////////////////////////////////////////////////////////////////////////
TR_S390zLinuxSystemLinkage::TR_S390zLinuxSystemLinkage(
  ...
  setRegisterFlag(TR::RealRegister::GPR14,
                  Preserved); // Setting the return address register as
                              // preserved so that way we do not always have to
                              // emit stores in prologues / epilogues
  // This possibly assumes that we don't shuffle the return address register
  // around elsewhere
  setRegisterFlag(TR::RealRegister::GPR15, Preserved);
  ...
  if (!comp()->getCurrentMethod()->isJ9()) {
    ...
    // GPR9
    // during param evaluation and will move them to GPR5 and GPR6 just before
    // going
    // into native code. This is needed for correct evaluation of parms as GPR5
    // is
 ...
 */
void TR_S390SystemLinkage::generateInstructionsForCall(
    TR::Node *callNode, TR::RegisterDependencyConditions *deps,
 ...
 */
TR::Register *TR_S390SystemLinkage::callNativeFunction(
    TR::Node *callNode, TR::RegisterDependencyConditions *deps,
    intptrj_t targetAddress, TR::Register *methodAddressReg,
    TR::Register *javaLitOffsetReg, TR::LabelSymbol *returnFromJNICallLabel,
    TR::S390JNICallDataSnippet *jniCallDataSnippet, bool isJNIGCPoint) {
 ...
 * Front-end customization of callNativeFunction
 */
void TR_S390zOSSystemLinkage::generateInstructionsForCall(
    TR::Node *callNode, TR::RegisterDependencyConditions *deps,
    intptrj_t targetAddress, TR::Register *methodAddressReg,
    TR::Register *javaLitOffsetReg, TR::LabelSymbol *returnFromJNICallLabel,
    TR::S390JNICallDataSnippet *jniCallDataSnippet, bool isJNIGCPoint) {
  ...
  TR_ASSERT(0,
            "Python/Ruby/Test should have their own front-end customization.");
#endif
}
 ...
 * linkage return reg
 */
TR::Register *TR_S390zOSSystemLinkage::callNativeFunction(
    TR::Node *callNode, TR::RegisterDependencyConditions *deps,
    intptrj_t targetAddress, TR::Register *methodAddressReg,
    TR::Register *javaLitOffsetReg, TR::LabelSymbol *returnFromJNICallLabel,
    TR::S390JNICallDataSnippet *jniCallDataSnippet, bool isJNIGCPoint) {
 ...
 * Front-end customization of callNativeFunction
 */
void TR_S390zLinuxSystemLinkage::generateInstructionsForCall(
    TR::Node *callNode, TR::RegisterDependencyConditions *deps,
    intptrj_t targetAddress, TR::Register *methodAddressReg,
    TR::Register *javaLitOffsetReg, TR::LabelSymbol *returnFromJNICallLabel,
    TR::S390JNICallDataSnippet *jniCallDataSnippet, bool isJNIGCPoint) {
 ...
 */
TR::Register *TR_S390zLinuxSystemLinkage::callNativeFunction(
    TR::Node *callNode, TR::RegisterDependencyConditions *deps,
    intptrj_t targetAddress, TR::Register *methodAddressReg,
    TR::Register *javaLitOffsetReg, TR::LabelSymbol *returnFromJNICallLabel,
    TR::S390JNICallDataSnippet *jniCallDataSnippet, bool isJNIGCPoint) {
  ...
  case TR::lucalli: {
    if (cg()->use64BitRegsOn32Bit()) {
      ...
      generateRSInstruction(cg(), TR::InstOpCode::SLLG, callNode, highReg,
                            highReg, 32);
      cursor = generateRRInstruction(cg(), TR::InstOpCode::LR, callNode,
                                     highReg, lowReg);
 ...
 */
void TR_S390zLinuxSystemLinkage::initParamOffset(
    TR::ResolvedMethodSymbol *method, int32_t stackIndex,
    List<TR::ParameterSymbol> *parameterList) {
  ListIterator<TR::ParameterSymbol> parameterIterator(
      &method->getParameterList());
  ...
  while (parmCursor != NULL) {
    ...
    if (argRegNum == TR::RealRegister::NoReg) {
      ...
      {
        TR_ASSERT(getParmOffsetInLocalArea() >= 0,
                  "ParmOffsetInLocalArea is negative");
    ...
    if (TR::Compiler->target.is64Bit() && parmCursor->getType().isAddress() &&
      ...
      // the start of the
      // 8-byte parm slot. Since ptr31 parms are passed as 64-bit values, the
      // prologue code has the
      // spill the full 8-bytes into the parm slot. However, the rest of the
      // code will load/store
...
void TR_S390SystemLinkage::mapStack(TR::ResolvedMethodSymbol *method) {
  { mapStack(method, 0); }
}
...
void TR_S390SystemLinkage::mapStack(TR::ResolvedMethodSymbol *method,
  ...
  setStrictestAutoSymbolAlignment(TR::Compiler->target.is64Bit() ? 8 : 8,
                                  true); // initial setting

  setStackSizeCheckNeeded(true);

  // process incomming parameters
  if (!isParmBlockRegister() &&
      !isZLinuxLinkageType()) // ! OS (Type 1 linkage) && ! zLinux Linkage
  {
  ...
  if (true) {
    ...
    for (i = TR::RealRegister::FirstFPR; i <= TR::RealRegister::LastFPR; ++i) {
      if (getPreserved(REGNUM(i)) &&
          ((getS390RealRegister(REGNUM(i)))->getHasBeenAssignedInMethod())) {
...
#define DELTA_ALIGN(x, align)                                                  \
  ...
  if (isParmBlockRegister()) // OS Type 1 linkage
  {
    ...
    stackIndex -=
        (getIncomingParmAreaBeginOffset() - getIncomingParmAreaEndOffset());
...
void TR_S390SystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol *p,
                                              uint32_t &stackIndex) {
  size_t align = 1;
  size_t size = p->getSize();
  ...
  setStrictestAutoSymbolAlignment(
      align); // API affected if (align > current strictest)
}
...
TR_S390SystemLinkage::saveGPRsInPrologue2(TR::Instruction *cursor) {
  ...
  for (i = lastReg = firstReg = TR::RealRegister::FirstGPR;
       i <= TR::RealRegister::LastGPR - 2;
       ++i) // special case for r14, the return value
  {
  ...
  // Original Strategy --- use 1 store multiple operation that goes from 1st to
  // last preserved reg.  We will preserve regs in between even if they aren't
  // assigned.
  ...
  if (lastReg - firstReg == 0)
    cursor = generateRXInstruction(
        cg(), TR::InstOpCode::getStoreOpCode(), firstNode,
        getS390RealRegister(REGNUM(firstReg)), retAddrMemRef, cursor);
  else if (lastReg > firstReg)
    cursor = generateRSInstruction(
        cg(), TR::InstOpCode::getStoreMultipleOpCode(), firstNode,
        getS390RealRegister(REGNUM(firstReg)),
        getS390RealRegister(REGNUM(lastReg)), retAddrMemRef, cursor);

  // r14 is a special case.  It is the return value register.  Since we will
  // have to save/restore it often, we will try to reduce the range above by
  // just emiting its own save/restore
  ...
  if (getPreserved(REGNUM(i)) &&
    ...
    cursor = generateRXInstruction(cg(), TR::InstOpCode::getStoreOpCode(),
                                   firstNode, getS390RealRegister(REGNUM(i)),
                                   retAddrMemRef, cursor);
  }

  // r15 (stack pointer) might need to be put in to the register mask as well.
  // Other code will deal with how we save/restore it.
...
TR::Linkage::FrameType TR_S390zLinuxSystemLinkage::checkLeafRoutine(
    int32_t stackFrameSize, TR::Instruction **callInstruction) {
  ...
  int32_t zeroArgsAndLocalsStackFrameSize =
      (((getOffsetToFirstParm() + 7) >> 3) << 3);

  if (comp()->getOption(TR_TraceCG))
    traceMsg(comp(), "stackFrameSize = %d  default 0 arg and 0 local stack "
                     "frame size = %d\n",
  ...
  if (stackFrameSize != zeroArgsAndLocalsStackFrameSize) {
    if (comp()->getOption(TR_TraceCG))
      traceMsg(
          comp(),
          "LeafRoutine: stackFrameSize != zeroArgsAndLocalStackFrameSize\n");
    return ft;
  }
  ...
  if (spReg->isUsedInMemRef() || getARSaveMask() ||
      getHPRSaveMask()) // don't need to check gprmask or hpr mask as these are
                        // saved in caller stack.
  {
    if (comp()->getOption(TR_TraceCG))
      traceMsg(comp(), "LeafRoutine: stack pointer isUsedInMemRef = %d  or "
                       "arsavemask = %d  or hprsavemask = %d is set to 1\n",
...
#define ZLINUX_SYSTEMLINK_LEAF_INSTRUCTION_CHECK_THRESHOLD 150
  ...
  bool hasCalls =
      findPossibleCallInstruction(cursor, numToCheck, callInstruction, regs,
  ...
  if (!hasCalls && cursor == NULL) {
    ...
    if (callInstruction && *callInstruction &&
        checkPreservedRegisterUsage(regs, TR::RealRegister::NumRegisters)) {
      ft = noStackForwardingFrame;
    } else if (callInstruction && *callInstruction) // register usage doesn't
                                                    // allow a framewithout a
                                                    // stack.
    {
      ft = standardFrame;
    } else if (checkPreservedRegisterUsage(regs,
                                           TR::RealRegister::NumRegisters)) {
      ft = noStackLeafFrame;
    } else {
      ft = StackLeafFrame;
...
bool OMR::Z::Linkage::checkPreservedRegisterUsage(bool *regs,
                                                  int32_t regsSize) {
  int32_t i;
  bool retval = true;

  if (self()->comp()->getOption(TR_TraceCG))
    traceMsg(self()->comp(), "LeafRoutine: Checking Register Usage.  Preserved "
                             "Registers that are assigned:");

  for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR - 2;
       ++i) // special case for r14, the return value
  {
    // TR_ASSERT(regs[i] ==
    // (getS390RealRegister(REGNUM(i)))->getHasBeenAssignedInMethod(), "Mismatch
    // between register assigned information!!\n");

    // traceMsg(comp(), "regs[%d] = %d getHasBeenAssignedInMethod =
    // %d\n",i,regs[i],(getS390RealRegister(REGNUM(i)))->getHasBeenAssignedInMethod());

    if (self()->getPreserved(REGNUM(i)) &&
        (self()->getS390RealRegister(REGNUM(i)))
  ...
  // check r14 usage -- don't check if it has been assigned because it could be
  // used in a call. the regs array passed in will take this in to account.
  if (regs[TR::RealRegister::GPR14]) {
    if (self()->comp()->getOption(TR_TraceCG))
      traceMsg(self()->comp(), "\t%d (r14)",
               TR::RealRegister::GPR14 - TR::RealRegister::FirstGPR);
...
void TR_S390SystemLinkage::createPrologue(TR::Instruction *cursor) {
  ...
  setStackFrameSize(((getOffsetToFirstParm() + argSize + localSize + 7) >> 3)
                    << 3);
  int32_t stackFrameSize = getStackFrameSize();

  if (comp()->getOption(TR_TraceCG))
    traceMsg(comp(), "initial stackFrameSize: stack size %d = offset to first "
                     "parm %d + arg size %d + localSize %d, rounded up to 8\n",
  ...
  //      "TR_S390SystemLinkage::createPrologue -- Frame size (0x%x) greater
  //      than 0x7FFF\n",stackFrameSize);
  ...
  if ((ft == noStackLeafFrame || ft == StackLeafFrame ||
       ft == noStackForwardingFrame) &&
      performTransformation(comp(),
                            "O^O Generating method as leaf of type %d\n", ft)) {
    setFrameType(ft);
  ...
  if (!(getFrameType() == noStackForwardingFrame ||
        getFrameType() == noStackLeafFrame)) {
  ...
  if (cg()->isLiteralPoolOnDemandOn() != true && firstSnippet != NULL)
    cursor = new (trHeapMemory()) TR::S390RILInstruction(
        TR::InstOpCode::LARL, firstNode, lpReg, firstSnippet, cursor, cg());
#endif

  if (!(getFrameType() == noStackForwardingFrame ||
        getFrameType() == noStackLeafFrame ||
        getFrameType() == StackLeafFrame)) {
    // Check for large stack
    bool largeStack = (stackFrameSize < MIN_IMMEDIATE_VAL ||
                       stackFrameSize > MAX_IMMEDIATE_VAL);

    TR::RealRegister *tempReg = getS390RealRegister(TR::RealRegister::GPR0);
    TR::RealRegister *backChainReg =
        getS390RealRegister(TR::RealRegister::GPR1);

    if (!largeStack) {
      // Adjust stack pointer with LA (reduce AGI delay)
      cursor = generateRXYInstruction(
          cg(), TR::InstOpCode::LAY, firstNode, spReg,
      ...
      if (largeStack) {
        cursor = generateS390ImmToRegister(
            cg(), firstNode, tempReg, (intptr_t)(stackFrameSize * -1), cursor);
        cursor = generateRRInstruction(cg(), TR::InstOpCode::getAddRegOpCode(),
                                       firstNode, spReg, tempReg, cursor);
      } else {
        cursor = generateRIInstruction(
            cg(), TR::InstOpCode::getAddHalfWordImmOpCode(), firstNode, spReg,
            (stackFrameSize) * -1, cursor);
      }
    }
  } else {
    if (comp()->getOption(TR_TraceCG))
      traceMsg(comp(),
               "Not emitting stack save because method is leaf Routine\n");
  }
  ...
  for (i = firstSaved; i <= lastSaved; ++i) {
    if (FPRSaveMask & (1 << (i))) {
      ...
      cursor = generateRXInstruction(
          cg(), TR::InstOpCode::STD, firstNode,
          getS390RealRegister(REGNUM(i + TR::RealRegister::FirstFPR)),
          retAddrMemRef, cursor);
    }
  }
  ...
  for (i = firstSaved; i <= lastSaved; ++i) {
    if (ARSaveMask & (1 << (i))) {
      ...
      cursor = generateRSInstruction(
          cg(), TR::InstOpCode::STAM, firstNode,
          getS390RealRegister(REGNUM(i + TR::RealRegister::FirstAR)),
          getS390RealRegister(REGNUM(i + TR::RealRegister::FirstAR)),
          retAddrMemRef, cursor);
    }
  ...
  if (_notifiedOfalloca) {
    cursor = generateRRInstruction(
        cg(), TR::InstOpCode::getLoadRegOpCode(), firstNode,
...
TR_S390SystemLinkage::restoreGPRsInEpilogue2(TR::Instruction *cursor) {
  ...
  for (i = lastReg = firstReg = TR::RealRegister::FirstGPR;
       i <= TR::RealRegister::LastGPR - 2;
       ++i) // special casing r14 (ret value)
  {
    traceMsg(comp(), "Considering Register %d:\n",
             i - TR::RealRegister::FirstGPR);
    if (getPreserved(REGNUM(i))) {
  ...
  // Original Strategy --- use 1 load multiple operation that goes from 1st to
  // last preserved reg.  We will preserve regs in between even if they aren't
  // assigned.
  ...
  if (lastReg - firstReg == 0)
    cursor = generateRXInstruction(
        cg(), TR::InstOpCode::getLoadOpCode(), nextNode,
        getS390RealRegister(REGNUM(firstReg)), retAddrMemRef, cursor);
  else if (lastReg > firstReg)
    cursor = generateRSInstruction(
        cg(), TR::InstOpCode::getLoadMultipleOpCode(), nextNode,
        getS390RealRegister(REGNUM(firstReg)),
        getS390RealRegister(REGNUM(lastReg)), retAddrMemRef, cursor);
  ...
  if (getPreserved(REGNUM(i)) &&
    ...
    cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(),
                                   nextNode, getS390RealRegister(REGNUM(i)),
                                   retAddrMemRef, cursor);
  }
...
TR_S390SystemLinkage::restoreGPRsInEpilogue(TR::Instruction *cursor) {
  ...
  bool specializedEpilogues =
      cg()->specializedEpilogues() && TR::Compiler->target.isLinux();
    ...
    if (getPreserved(REGNUM(i))) {
      ...
      if ((getS390RealRegister(REGNUM(i)))->getHasBeenAssignedInMethod()) {
        if ((!specializedEpilogues) ||
            restoreRegister(getS390RealRegister(REGNUM(i))->getRegisterNumber(),
                            blockNumber)) {
          if (comp()->getOption(TR_TraceCG))
            traceMsg(comp(), "\tand Assigned. ");
          findStartReg = false;
...
void TR_S390SystemLinkage::createEpilogue(TR::Instruction *cursor) {
  ...
  cursor = generateS390LabelInstruction(cg(), TR::InstOpCode::LABEL, nextNode,
                                        epilogBeginLabel, cursor);

  if (_notifiedOfalloca) {
    cursor = generateRRInstruction(
        cg(), TR::InstOpCode::getLoadRegOpCode(), nextNode, spReg,
        getS390RealRegister(getAlternateStackPointerRegister()), cursor);
  }
  ...
  cursor = generateS390RegInstruction(
      cg(), TR::InstOpCode::BCR, nextNode,
...
void TR_S390SystemLinkage::notifyHasalloca() {
  ...
  TR::RealRegister *alternateStackReg =
      getS390RealRegister(getAlternateStackPointerRegister());
  ...
  if (cg()->supportsHighWordFacility() &&
      ...
      TR::Compiler->target.is64Bit()) {
    TR::RealRegister *tempHigh =
        toRealRegister(alternateStackReg)->getHighWordRegister();
    tempHigh->setState(TR::RealRegister::Locked);
    tempHigh->setAssignedRegister(tempHigh);
    tempHigh->setHasBeenAssignedInMethod(true);
  }
  ...
  int32_t globalAltStackReg =
      cg()->machine()->findGlobalRegisterIndex(alternateSP);
  if (globalAltStackReg != -1) {
    cg()->machine()->lockGlobalRegister(globalAltStackReg);
  }
}

int32_t TR_S390zLinuxSystemLinkage::getRegisterSaveOffset(
  ...
  case TR::RealRegister::GPR1:
    TR_ASSERT(false, "ERROR: TR_zLinuxSystemLinkage::getRegisterSaveOffset "
                     "called for volatile reg: %d",
              srcReg);
    return -1;
...
int32_t TR_S390zOSSystemLinkage::getRegisterSaveOffset(
    TR::RealRegister::RegNum srcReg) {
  int32_t offset;
  if ((srcReg >= TR::RealRegister::GPR4) &&
    ...
    int32_t offset =
        2048 +
        (srcReg - TR::RealRegister::GPR4) * cg()->machine()->getGPRSize();
    return offset;
  } else {
    TR_ASSERT(false, "ERROR: TR_S390zOSSystemLinkage::getRegisterSaveOffset "
                     "called for volatile reg: %d\n",
              srcReg);
    return -1;
  }
...
/**
 * Find first instruction, within a window of instructions, that may be a
 * possible call banch instruction
 ...
 *
 * @note
 *  1) Some linkages may have different instructions used for branching to
 * callee
 ...
 *  2) A "true" return does not mean the instruction found is a call, it may be
 * a call.
 *     If necessary, The caller of this routine can do further verication base
 * on
 *     the returned instruction.
 */
void OMR::Z::Linkage::setUsedRegisters(TR::Instruction *instruction, bool *regs,
                                       int32_t regsSize) {
  if (!regs || regsSize == 0)
    return;

  TR::list<TR::Register *> usedRegs(
    ...
    for (auto cursor = usedRegs.begin(); cursor != usedRegs.end(); ++cursor) {
      if ((*cursor) && (*cursor)->getRealRegister()) {
        TR::RealRegister::RegNum regNum =
            toRealRegister((*cursor))->getRegisterNumber();

        TR_ASSERT(regNum < regsSize,
                  "Trying to set regNum %d to true when max regsSize = %d\n",
                  regNum, regsSize);

        if (self()->comp()->getOption(TR_TraceCG))
          traceMsg(self()->comp(), "Instruction %p uses reg %d\n", instruction,
                   regNum - TR::RealRegister::FirstGPR);
        regs[regNum] = true;
      }
    }
  }
 ...
 *        On Output: Instruction found or instruction just outside window (or
 * NULL if no more -- this is the successful indicator that this may be a leaf
 * routine)
 ...
 * @return Either true for no call found or a single call that can be changed to
 * a jump
 */
bool OMR::Z::Linkage::findPossibleCallInstruction(
    TR::Instruction *&cursor, int32_t &numToCheck,
    TR::Instruction **callInstruction, bool *regs, int32_t regsSize)

{
  ...
  if (result && canReplaceCallWithJump && *callInstruction && n < numToCheck) {
    ...
    if (opCode == TR::InstOpCode::BASR) // don't know which instruction to use
                                        // for a virtual dispatch (yet).  In
                                        // theory, BCR.
    {
      canReplaceCallWithJump = false;
...
bool OMR::Z::Linkage::canDataTypeBePassedByReference(TR::DataType type) {
  return false;
}

bool TR_S390zLinuxSystemLinkage::canDataTypeBePassedByReference(
    TR::DataType type) {
...
bool OMR::Z::Linkage::isSymbolPassedByReference(TR::Symbol *sym) {
  return false;
}
...
bool TR_S390zLinuxSystemLinkage::isSymbolPassedByReference(TR::Symbol *sym) {
  ...
  if (sym->getDataType() != TR::Aggregate) // any non-aggregate symbols that
                                           // have a data type that can be
                                           // passed by reference *will* be
                                           // passed by reference
    return true;
  ...
  TR_ASSERT(
      0,
      "should not reach here due to folding of getDataTypeOfAggregateMember()");

  TR::DataType aggrMemberDataType = TR::NoType;
  bool isVectorAggregate = (TR::DataType(aggrMemberDataType).isVector() &&
                            cg()->getSupportsVectorRegisters());

  if ((symSize > gprSize) && !(gprSize == 4 && symSize == 8) &&
      !isVectorAggregate)
    return true;
...
bool TR::Z::ZOSBaseSystemLinkage::isAggregateReturnedInIntRegistersAndMemory(
    int32_t aggregateLenth) {
  return isFastLinkLinkageType() &&
         (aggregateLenth >
          (getNumIntegerArgumentRegisters() * cg()->machine()->getGPRSize()));
}

bool TR_S390zOSSystemLinkage::isAggregateReturnedInIntRegisters(
    int32_t aggregateLenth) {
  return aggregateLenth <=
         (getNumIntegerArgumentRegisters() * cg()->machine()->getGPRSize());
}
...
#define XPLINK_FLOAT_PARM_UNPROTYPED_CALL 0x08
  ...
  if (!isPrototyped) {
    parmDescriptorFields = shiftFloatParmDescriptorFlag(
        XPLINK_FLOAT_PARM_UNPROTYPED_CALL,
        0); // FPR0 field has special value for unprototped call
    callDescValue |= parmDescriptorFields;
    return callDescValue;
  }
