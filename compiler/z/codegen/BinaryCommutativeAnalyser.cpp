...
void TR_S390BinaryCommutativeAnalyser::remapInputs(
  ...
  if (!cg()->useClobberEvaluate()) {
    ...
    if (firstChild == secondChild && firstChild->getReferenceCount() == 2 &&
      ...
      // the unclobbered f1 value
      // isRegisterClobberable(firstRegister,1) returns false in this latter
      // case as firstRegister is attached to more than one unique node
      setClob1();
      setClob2();
    }
  }
...
TR::Register *TR_S390BinaryCommutativeAnalyser::allocateAddSubRegister(
  ...
  if (src1Reg->containsInternalPointer() ||
    ...
    if ((node->getType().isInt64() || src1Reg->getKind() == TR_GPR64) &&
...
void TR_S390BinaryCommutativeAnalyser::genericAnalyser(
  ...
  //     Shadow[java/lang/ThreadGroup.childrenGroups
  ...
  if (getOpReg1Reg2()) {
      ...
      if (targetLabel != NULL) {
        ...
        if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) &&
            cg()->isActiveCompareCC(regToRegOpCode, firstRegister,
                                    secondRegister) &&
            performTransformation(comp, "O^O BinaryCommunicativeAnalyser case "
                                        "3 RR Compare [%s\t %s, %s]: reuse CC "
                                        "from ccInst [%p].",
          ...
          nodeReg = firstRegister; // CCInfo already exists; don't generate
                   ...
                   performTransformation(
                       comp, "O^O BinaryCommunicativeAnalyser case 4 RR "
                             "Compare [%s\t %s, %s]: reuse CC from ccInst "
                             "[%p].",
                       debugObj->getOpCodeName(&ccInst->getOpCode()),
                       debugObj->getName(firstRegister),
                       debugObj->getName(secondRegister), ccInst)) {
    ...
    if (targetLabel != NULL) {
      ...
      if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) &&
          cg()->isActiveCompareCC(regToRegOpCode, firstRegister,
                                  secondRegister) &&
          performTransformation(comp, "O^O BinaryCommunicativeAnalyser case 5 "
                                      "RR Compare [%s\t %s, %s]: reuse CC from "
                                      "ccInst [%p].",
                                debugObj->getOpCodeName(&ccInst->getOpCode()),
                                debugObj->getName(firstRegister),
                                debugObj->getName(secondRegister), ccInst)) {
        nodeReg = firstRegister; // CCInfo already exists; don't generate
                 ...
                 cg()->isActiveCompareCC(regToRegOpCode, secondRegister,
                                         firstRegister) &&
                 performTransformation(
                     comp, "O^O BinaryCommunicativeAnalyser case 6 RR Compare "
    ...
    } else if (secondRegister->getKind() != TR_FPR &&
               secondRegister->getKind() != TR_VRF) {
    ...
    if (getOpReg3Mem2()) {
      if (firstRegister->getKind() == TR_GPR64) {
        nodeReg = cg()->allocate64bitRegister();
      } else if (firstRegister->getKind() != TR_FPR &&
                 firstRegister->getKind() != TR_VRF) {
        nodeReg = allocateAddSubRegister(root, firstRegister);
      } else {
        nodeReg = cg()->allocateRegister(TR_FPR);
      }
    ...
    TR::MemoryReference *tempMR =
        generateS390MemoryReference(firstChild, cg(), true);
    // floating-point arithmatics don't have RXY format instructions, so no long
    // displacement
...
void TR_S390BinaryCommutativeAnalyser::genericLongAnalyser(
    TR::Node *root, TR::InstOpCode::Mnemonic lowRegToRegOpCode,
  ...
  if (root->getOpCodeValue() == TR::land || root->getOpCodeValue() == TR::lor ||
  ...
  if (getOpReg1Reg2()) {
    ...
    TR::RegisterPair *tempReg =
        cg()->allocateConsecutiveRegisterPair(oneLow, oneHigh);

    if (needsRegPairDep) {
      dependencies = new (cg()->trHeapMemory())
          TR::RegisterDependencyConditions(0, 3, cg());
      dependencies->addPostCondition(tempReg, TR::RealRegister::EvenOddPair);
      dependencies->addPostCondition(oneHigh,
                                     TR::RealRegister::LegalEvenOfPair);
      dependencies->addPostCondition(oneLow, TR::RealRegister::LegalOddOfPair);
    }

    cursor =
        generateRRInstruction(cg(), lowRegToRegOpCode, root, oneLow, twoLow);

    if (firstHighZero) {
      if (secondHighZero || root->getOpCodeValue() == TR::land) {
        cursor = generateRRInstruction(cg(), TR::InstOpCode::XR, root, oneHigh,
                                       oneHigh);
      } else {
        cursor =
            generateRRInstruction(cg(), copyOpCode, root, oneHigh, twoHigh);
      }
      ...
      cursor = generateRRInstruction(cg(), highRegToRegOpCode, root, oneHigh,
                                     twoHigh);
    }
    ...
    TR::RegisterPair *tempReg =
        cg()->allocateConsecutiveRegisterPair(twoLow, twoHigh);

    if (needsRegPairDep) {
      dependencies = new (cg()->trHeapMemory())
          TR::RegisterDependencyConditions(0, 3, cg());
      dependencies->addPostCondition(tempReg, TR::RealRegister::EvenOddPair);
      dependencies->addPostCondition(twoHigh,
                                     TR::RealRegister::LegalEvenOfPair);
      dependencies->addPostCondition(twoLow, TR::RealRegister::LegalOddOfPair);
    }

    cursor =
        generateRRInstruction(cg(), lowRegToRegOpCode, root, twoLow, oneLow);

    if (secondHighZero) {
      if (firstHighZero || root->getOpCodeValue() == TR::land) {
        cursor = generateRRInstruction(cg(), TR::InstOpCode::XR, root, twoHigh,
                                       twoHigh);
      } else {
        cursor =
            generateRRInstruction(cg(), copyOpCode, root, twoHigh, oneHigh);
      }
      ...
      cursor = generateRRInstruction(cg(), highRegToRegOpCode, root, twoHigh,
                                     oneHigh);
    }
    ...
    if (firstHighZero) {
      if (secondHighZero || root->getOpCodeValue() == TR::land) {
        generateRRInstruction(cg(), TR::InstOpCode::XR, root, highRegister,
                              highRegister);
      } else {
      ...
      generateRRInstruction(cg(), copyOpCode, root, highRegister, oneHigh);
      generateRRInstruction(cg(), highRegToRegOpCode, root, highRegister,
                            twoHigh);
    }

    cursor = generateRRInstruction(cg(), lowRegToRegOpCode, root, lowRegister,
                                   twoLow);
  ...
  } else // RX instruction
  {
    ...
    if (getOpReg1Mem2() || getOpReg3Mem2()) {
      if (!secondHighZero &&
          secondChild->getSymbolReference()->isUnresolved()) {
         ...
         *
         ...
         [0x65cd0bd0] (  0)      lushr
         [0x65cd0b60] (  0)        ilload #496[0x6b699868]+8  final
        Shadow[java/util/UUID.mostSigBits J]
                      (  0)          ==>aRegLoad at [0x789ae4e8] (in
        &GPR_0x78e6ae4c)
         [0x65cd0b98] (  1)        iconst 32
         [0x65cd0af0] (  0)      lor (in GPR_0x78e6b5f8:GPR_0x78e6b5bc)
        ...
        For trees with one of the children having lushr by 32 bits and in
        highWordZero when useSecondHighOrder or useFirstHighOrder are set
        and that child is a mem ref, we need to use high order
         */
        highMR =
            generateS390MemoryReference(secondChild, cg(), true /*canUseRX*/);
        if (!secondHighZero ||
            (secondChild->getType().isInt64() && !useSecondHighOrder)) {
      ...
      if (getOpReg3Mem2()) {
        if (firstHighZero) {
          ...
          generateRRInstruction(cg(), copyOpCode, root, targetReg,
                                firstRegister);
        } else {
          createdClobberPair = true;
          TR::Register *lowRegister =
              allocateAddSubRegister(root, firstRegister->getLowOrder());
          TR::Register *highRegister =
              allocateAddSubRegister(root, firstRegister->getHighOrder());
          targetReg =
              cg()->allocateConsecutiveRegisterPair(lowRegister, highRegister);
          cursor = generateRRInstruction(cg(), copyOpCode, root,
                                         targetReg->getLowOrder(),
      ...
      if (!firstHighZero && firstChild->getSymbolReference()->isUnresolved()) {
         ...
         */
        highMR =
            generateS390MemoryReference(firstChild, cg(), true /*canUseRX*/);
        if (!firstHighZero ||
            (firstChild->getType().isInt64() && !useFirstHighOrder)) {
          lowMR = generateS390MemoryReference(*highMR, 4, cg());
        } else {
          lowMR = highMR;
        }
      ...
      if (getOpReg3Mem1()) {
        if (secondHighZero) {
          ...
          createdClobberPair = true;
          TR::Register *lowRegister =
              allocateAddSubRegister(root, secondRegister->getLowOrder());
          TR::Register *highRegister =
              allocateAddSubRegister(root, secondRegister->getHighOrder());
          targetReg =
              cg()->allocateConsecutiveRegisterPair(lowRegister, highRegister);
          cursor = generateRRInstruction(cg(), copyOpCode, root,
                                         targetReg->getLowOrder(),
    ...
    if (needsRegPairDep) {
      dependencies->addPostCondition(tempReg, TR::RealRegister::EvenOddPair);
      dependencies->addPostCondition(targetHigh,
                                     TR::RealRegister::LegalEvenOfPair);
      dependencies->addPostCondition(targetLow,
                                     TR::RealRegister::LegalOddOfPair);
    }
    ...
    if (secondOperandIsInRegister) {
      cursor = generateRRInstruction(cg(), lowRegToRegOpCode, root, targetLow,
                                     longRegister->getLowOrder());
    } else {
      cursor = generateRXInstruction(cg(), lowMemToRegOpCode, root, targetLow,
                                     lowMR);
    }

    if (memHighZero) {
      if (root->getOpCodeValue() == TR::land || targetHighZero) {
        cursor = generateRRInstruction(cg(), TR::InstOpCode::XR, root,
      ...
      if (root->getOpCodeValue() == TR::land) {
        ...
        if (secondOperandIsInRegister) {
          cursor = generateRRInstruction(cg(), copyOpCode, root, targetHigh,
                                         longRegister->getHighOrder());
        } else {
          cursor = generateRXInstruction(cg(), TR::InstOpCode::L, root,
                                         targetHigh, highMR);
        }
      }
    } else {
      if (secondOperandIsInRegister) {
        cursor =
            generateRRInstruction(cg(), highRegToRegOpCode, root, targetHigh,
                                  longRegister->getHighOrder());
      } else {
        cursor = generateRXInstruction(cg(), highMemToRegOpCode, root,
                                       targetHigh, highMR);
      }
    }
  ...
  if (needsRegPairDep) {
    generateS390PseudoInstruction(cg(), TR::InstOpCode::DEPEND, root,
                                  dependencies);
  }
...
void TR_S390BinaryCommutativeAnalyser::integerAddAnalyser(
  ...
  // alternative add instructions
  // which do not produce a carry. The flag would be used to prevent such
  // optimizations when the carry is needed.
  ...
  if (getOpReg1Reg2()) {
    ...
    if (regToRegOpCode == TR::InstOpCode::AR) {
      cursor = generateRRRInstruction(cg(), TR::InstOpCode::ARK, root, tempReg,
                                      secondRegister, firstRegister);
      done = true;
    } else if (regToRegOpCode == TR::InstOpCode::ALR) {
      cursor = generateRRRInstruction(cg(), TR::InstOpCode::ALRK, root, tempReg,
                                      secondRegister, firstRegister);
      done = true;
    } else if (regToRegOpCode == TR::InstOpCode::AGR) {
      cursor = generateRRRInstruction(cg(), TR::InstOpCode::AGRK, root, tempReg,
                                      secondRegister, firstRegister);
      done = true;
    ...
    if (!done) {
      if (firstRegister->getKind() == TR_GPR64 ||
          ((firstChild->getType().isInt64() ||
            TR::Compiler->target.is64Bit()) &&
           cg()->use64BitRegsOn32Bit())) {
        cursor = generateRRInstruction(cg(), TR::InstOpCode::LGR, root, tempReg,
                                       firstRegister);
      } else {
        if (cg()->supportsHighWordFacility() &&
            !comp->getOption(TR_DisableHighWordRA)) {
          cursor = generateRRInstruction(
              cg(), TR::InstOpCode::getLoadRegOpCodeFromNode(cg(), root), root,
              tempReg, firstRegister);
        } else {
          cursor =
              generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(),
                                    root, tempReg, firstRegister);
        }
      }
      if (debugObj) {
        debugObj->addInstructionComment(toS390RRInstruction(cursor),
...
void TR_S390BinaryCommutativeAnalyser::longAddAnalyser(
    TR::Node *root, TR::InstOpCode::Mnemonic copyOpCode) {
  // *this    swipeable for debugging purposes
  TR_ASSERT(TR::Compiler->target.is32Bit(),
            " should call integerAddAnalyser() for 64Bit code-gen!");
  TR::Node *firstChild;
  TR::Node *secondChild;
  TR::Register *highSum = NULL;
  TR::LabelSymbol *doneLAdd =
  ...
  bool useFirstHighOrder = false;
  bool useSecondHighOrder = false;
  bool zArchTrexSupported =
      performTransformation(comp, "O^O Use AL/ALC for long add.");

  // Can generate better code for long adds when one or more children have a
  // high order zero word
  // can avoid the evaluation when we don't need the result of such nodes for
  // another parent.
  //
  ...
  if (getOpReg1Reg2()) {
    ...
    if ((ENABLE_ZARCH_FOR_32 && zArchTrexSupported) || setsOrReadsCC) {
      ...
      // instructions and hence code has been re-enabled for handling luaddc and
      // luadd under computeCC.
      //
      generateRRInstruction(cg(), regToRegOpCode, root, oneLow, twoLow);
      if (firstHighZero) {
        if (secondHighZero) {
          generateRRInstruction(cg(), TR::InstOpCode::XR, root, oneHigh,
                                oneHigh);
        } else {
          generateRRInstruction(cg(), TR::InstOpCode::LR, root, oneHigh,
                                twoHigh);
        }
      } else {
        if (secondHighZero) {
          //            generateRIInstruction(cg(), TR::InstOpCode::AHI, root,
          //            oneHigh, 0);
        } else {
          generateRRInstruction(cg(), TR::InstOpCode::ALCR, root, oneHigh,
                                twoHigh);
        }
      }
    } else {
      if (firstHighZero) {
        ...
        if (secondHighZero) {
          ...
          generateRRInstruction(cg(), TR::InstOpCode::AR, root, oneHigh,
                                twoHigh);
        }
      }
    }
    ...
    if (!((ENABLE_ZARCH_FOR_32 && zArchTrexSupported) || setsOrReadsCC)) {
      cursor = generateRRInstruction(cg(), TR::InstOpCode::ALR, root, oneLow,
                                     twoLow);
    }
    ...
    if ((ENABLE_ZARCH_FOR_32 && zArchTrexSupported) || setsOrReadsCC) {
      ...
      if (firstHighZero) {
        if (secondHighZero) {
          generateRRInstruction(cg(), TR::InstOpCode::XR, root, twoHigh,
                                twoHigh);
        }
      } else {
        if (secondHighZero) {
          generateRRInstruction(cg(), TR::InstOpCode::LR, root, twoHigh,
                                oneHigh);
        } else {
          generateRRInstruction(cg(), TR::InstOpCode::ALCR, root, twoHigh,
                                oneHigh);
        }
      }
    } else {
      ...
      if (firstHighZero) {
        ...
        if (secondHighZero) {
          ...
          generateRRInstruction(cg(), TR::InstOpCode::AR, root, twoHigh,
                                oneHigh);
        }
      }
    }
    ...
    if (!((ENABLE_ZARCH_FOR_32 && zArchTrexSupported) || setsOrReadsCC)) {
      cursor = generateRRInstruction(cg(), TR::InstOpCode::ALR, root, twoLow,
                                     oneLow);
    }
    ...
    if ((ENABLE_ZARCH_FOR_32 && zArchTrexSupported) || setsOrReadsCC) {
      generateRRInstruction(cg(), TR::InstOpCode::LR, root, lowRegister,
                            oneLow);
      generateRRInstruction(cg(), regToRegOpCode, root, lowRegister, twoLow);

      if (!firstHighZero) {
        generateRRInstruction(cg(), TR::InstOpCode::LR, root, highRegister,
                              oneHigh);
      } else {
        generateRRInstruction(cg(), TR::InstOpCode::XR, root, highRegister,
                              highRegister);
      ...
      if (secondHighZero) {
        ...
        generateRRInstruction(cg(), TR::InstOpCode::ALCR, root, highRegister,
                              twoHigh);
      }
    } else {
      generateRRInstruction(cg(), TR::InstOpCode::LR, root, lowRegister,
      ...
      if (secondHighZero) {
        ...
        generateRRInstruction(cg(), TR::InstOpCode::AR, root, highRegister,
                              twoHigh);
      }

      cursor = generateRRInstruction(cg(), TR::InstOpCode::ALR, root,
                                     lowRegister, twoLow);
  ...
  } else // RX instruction
  {
    ...
    if (targetHighZero) {
      targetLow = targetReg;
      targetHigh = cg()->allocateRegister();
      generateRRInstruction(cg(), TR::InstOpCode::XR, root, targetHigh,
                            targetHigh);
    } else {
      targetLow = targetReg->getLowOrder();
      targetHigh = targetReg->getHighOrder();
    ...
    if ((ENABLE_ZARCH_FOR_32 && zArchTrexSupported) || setsOrReadsCC) {
      if (secondOperandIsInRegister) {
        generateRRInstruction(cg(), regToRegOpCode, root, targetLow,
                              longRegister->getLowOrder());
      } else {
      ...
      if (secondOperandIsInRegister) {
        cursor =
            generateRRInstruction(cg(), TR::InstOpCode::ALCR, root, targetHigh,
                                  longRegister->getHighOrder());
      } else {
        cursor = generateRXInstruction(cg(), TR::InstOpCode::ALC, root,
                                       targetHigh, highMR);
      }
    } else {
      if (!memHighZero) {
        if (secondOperandIsInRegister) {
          generateRRInstruction(cg(), TR::InstOpCode::AR, root, targetHigh,
                                longRegister->getHighOrder());
        } else {
          generateRXInstruction(cg(), TR::InstOpCode::A, root, targetHigh,
                                highMR);
        }
      }
      ...
      if (secondOperandIsInRegister) {
        cursor = generateRRInstruction(cg(), TR::InstOpCode::ALR, root,
                                       targetLow, longRegister->getLowOrder());
      } else {
        cursor = generateRXInstruction(cg(), TR::InstOpCode::AL, root,
                                       targetLow, lowMR);
      }
    }
  ...
  if (!((ENABLE_ZARCH_FOR_32 && zArchTrexSupported) || setsOrReadsCC)) {
    ...
    generateS390BranchInstruction(cg(), TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BNCRC, root, doneLAdd);

    // Increment MS int due to overflow in LS int
    generateRIInstruction(cg(), TR::InstOpCode::AHI, root, highSum, 1);

    generateS390LabelInstruction(cg(), TR::InstOpCode::LABEL, root, doneLAdd,
                                 dependencies);
  }
