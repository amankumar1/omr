...
OMR::Z::Instruction::Instruction(TR::CodeGenerator *cg,
    ...
    : OMR::Instruction(cg, op, n), CTOR_INITIALIZER_LIST {
  self()->initialize();
}
...
OMR::Z::Instruction::Instruction(TR::CodeGenerator *cg,
                                 ...
                                 TR::InstOpCode::Mnemonic op, TR::Node *n)
    : OMR::Instruction(cg, precedingInstruction, op, n), CTOR_INITIALIZER_LIST {
...
// OMR).
// Therefore, we will reuse the field for the binary buffer pointer to store the
// binary
...
void OMR::Z::Instruction::setCCInfo() {
  ...
  if (_opcode.setsCC()) {
    ...
    if (!self()->cg()->comp()->getOption(TR_EnableEBBCCInfo)) {
      ...
      // remove redudent compare ops;
      // note this will not intruduce mismatching for compare immediate ops with
      // CC from compare Ops,
      // because compare immed ops looks for setsSignedFlag or setsZeroFlags -
      // compare ops does not sets these flags
      {
        // we have detailed information on the CC setting
        self()->cg()->setHasCCInfo(true);
        self()->cg()->setHasCCOverflow(_opcode.setsOverflowFlag() &&
...
bool OMR::Z::Instruction::matchesAnyRegister(TR::Register *reg,
  ...
  bool enableHighWordRA =
      ...
      (reg->getKind() != TR_FPR) && (instReg->getKind() != TR_FPR) &&
      (reg->getKind() != TR_VRF) && (instReg->getKind() != TR_VRF);
...
void OMR::Z::Instruction::useRegister(TR::Register *reg, bool isDummy) {
  ...
  if (regPair) {
    ...
    // be considered dummy.
    // S390RegisterDependency also calls useRegister, in this case we do not
    // want to reset the dummy status of these regs
    //
...
bool OMR::Z::Instruction::startOfLiveRange(TR::Register *reg) {
  ...
  if ((self()->getOpCodeValue() == TR::InstOpCode::SRDA ||
       self()->getOpCodeValue() == TR::InstOpCode::SRDL) &&
      toS390RSInstruction(self())->getSourceImmediate() >= 32) {
    // Clobbering the low order half by shifing completely over it so not really
    // using it
    TR::RegisterPair *rp = _targetReg[0]->getRegisterPair();
    if (reg == rp->getLowOrder())
      return true;
  }
  if ((self()->getOpCodeValue() == TR::InstOpCode::SLDA ||
       self()->getOpCodeValue() == TR::InstOpCode::SLDL) &&
      toS390RSInstruction(self())->getSourceImmediate() >= 32) {
    // Clobbering the high order half by shifing completely over it so not
    // really using it
    TR::RegisterPair *rp = _targetReg[0]->getRegisterPair();
    if (reg == rp->getHighOrder())
      return true;
  }
  if (self()->getOpCodeValue() == TR::InstOpCode::RISBG ||
      self()->getOpCodeValue() == TR::InstOpCode::RISBGN ||
      self()->getOpCodeValue() == TR::InstOpCode::RISBHG ||
      self()->getOpCodeValue() == TR::InstOpCode::RISBLG) {
    uint8_t endBit =
        ((TR::S390RIEInstruction *)self())->getSourceImmediate8Two();
  ...
  if (_targetReg && !(self()->getOpCode().usesTarget())) {
    for (i = 0; i < _targetRegSize; ++i) {
      if (_targetReg[i]->usesRegister(reg) && !self()->usesOnlyRegister(reg)) {
        ...
        if (self()->getOpCodeValue() == TR::InstOpCode::LTR &&
            self()->getRegisterOperand(1) == self()->getRegisterOperand(2))
          return false;
        if (_targetReg[i]->getKind() == TR_GPR64 &&
            self()->getOpCode().is32bit() && TR::Compiler->target.is32Bit())
          return false;
        return true;
      }
    }
...
bool OMR::Z::Instruction::getRegisters(TR::list<TR::Register *> &regs) {
  ...
  for (i = 0; i < _sourceMemSize; ++i) {
    ...
    if (baseReg)
      regs.push_back(baseReg);
    if (indexReg)
      regs.push_back(indexReg);
  }
...
bool OMR::Z::Instruction::getUsedRegisters(TR::list<TR::Register *> &usedRegs) {
  ...
  if (self()->getOpCodeValue() == TR::InstOpCode::BCR && _sourceRegSize == 1 &&
      _sourceReg[0]->getRealRegister() ==
          machine->getS390RealRegister(TR::RealRegister::GPR0))
    return false;
    ...
    for (i = 0; i < _targetRegSize; ++i) {
      ...
      if (rp) {
        if ((self()->getOpCodeValue() == TR::InstOpCode::SRDA ||
             self()->getOpCodeValue() == TR::InstOpCode::SRDL) &&
            toS390RSInstruction(self())->getSourceImmediate() >= 32)
          ; // Clobbering the low order half by shifing completely over it so
            // not really using it
        else
          usedRegs.push_back(rp->getLowOrder());
        if ((self()->getOpCodeValue() == TR::InstOpCode::SLDA ||
             self()->getOpCodeValue() == TR::InstOpCode::SLDL) &&
            toS390RSInstruction(self())->getSourceImmediate() >= 32)
          ; // Clobbering the high order half by shifing completely over it so
            // not really using it
        else
          usedRegs.push_back(rp->getHighOrder());
      } else {
        if (i == 0 && (self()->getOpCodeValue() == TR::InstOpCode::RISBG ||
                       self()->getOpCodeValue() == TR::InstOpCode::RISBGN ||
                       self()->getOpCodeValue() == TR::InstOpCode::RISBHG ||
                       self()->getOpCodeValue() == TR::InstOpCode::RISBLG)) {
          uint8_t endBit =
              ((TR::S390RIEInstruction *)self())->getSourceImmediate8Two();
          if ((endBit & 128) != 0)
            continue; // Zeroing out unused bits therefore the first operand is
                      // not used
          else
            usedRegs.push_back(r);
        } else
  ...
  for (i = 0; i < _sourceMemSize; ++i) {
    ...
    indexReg = _sourceMem[i]->getIndexRegister();
    if (baseReg)
      usedRegs.push_back(baseReg);
    if (indexReg)
      usedRegs.push_back(indexReg);
  }
  ...
  // Also check the register dependency conditions. They record implied register
  // uses
  TR::RegisterDependencyConditions *conds = self()->getDependencyConditions();
  if (conds) {
    TR_S390RegisterDependencyGroup *preConds = conds->getPreConditions();
...
bool OMR::Z::Instruction::getDefinedRegisters(
    TR::list<TR::Register *> &defedRegs) {
  TR::Compilation *comp = self()->cg()->comp();
  OMR::Z::Machine *machine = self()->cg()->machine();
  bool afterRA = self()->cg()->afterRA();
  ...
  if (!((self()->getOpCodeValue() == TR::InstOpCode::LTR ||
         self()->getOpCodeValue() == TR::InstOpCode::LTGR) &&
        self()->getRegisterOperand(1) == self()->getRegisterOperand(2))) {
    TR::Register **_targetReg = self()->targetRegBase();
    for (i = 0; i < _targetRegSize; ++i) {
      TR::Register *r = _targetReg[i];
  ...
  TR::RegisterDependencyConditions *conds = self()->getDependencyConditions();
  if (conds) {
    // For function calls pre-conditions are uses and post-conditions are
    // results
    TR_S390RegisterDependencyGroup *preConds = conds->getPreConditions();
    n = conds->getNumPreConditions();
    for (i = 0; i < n; i++) {
...
bool OMR::Z::Instruction::getKilledRegisters(
    TR::list<TR::Register *> &killedRegs) {
  // *this    swipeable for debugging purposes
  int32_t i, n;
  TR::Compilation *comp = self()->cg()->comp();
  ...
  // Post dependency conditions will have placeholder regs which indicate which
  ...
  if (conds) {
    ...
    for (i = 0; i < n; i++) {
      if (postConds->getRegisterDependency(i)->getDefsRegister()) {
        ...
        TR::Register *r = dep->getRegister(self()->cg());
        if (r && r->isPlaceholderReg())
          killedRegs.push_back(
              machine->getS390RealRegister(dep->getRealRegister()));
      }
    }
  }

  // A function call via address in a register uses register r1 to hold the
  // target address but this register is also killed.
...
void OMR::Z::Instruction::assignRegisters(TR_RegisterKinds kindToBeAssigned) {
  ...
  if (self()->getOpCodeValue() != TR::InstOpCode::ASSOCREGS) {
    ...
    // if (kindToBeAssigned == TR_GPR)
    {
      ...
      // machine and by associating the virtual
      ...
      for (int32_t j = 0; j < last; ++j) {
        TR::Register *virtReg =
            depGroup->getRegisterDependency(j)->getRegister(self()->cg());
      ...
      if (self()->cg()->supportsHighWordFacility() &&
          !comp->getOption(TR_DisableHighWordRA)) {
        for (int32_t j = 0;
             j < TR::RealRegister::LastHPR - TR::RealRegister::FirstHPR; ++j) {
          TR::Register *virtReg =
              depGroup->getRegisterDependency(j + last)->getRegister(
                  self()->cg());
          machine->setVirtualAssociatedWithReal(
              (TR::RealRegister::RegNum)(j + TR::RealRegister::FirstHPR),
              virtReg);
        }
      }
  ...
  // isOutOfLineEX flag set
  // we must register assign the instruction object hanging off of the memory
  // reference's constantInstructionSnippet
  TR::Instruction *outOfLineEXInstr = self()->getOutOfLineEXInstr();
  if (outOfLineEXInstr) {
    TR::Instruction *savePrev = outOfLineEXInstr->getPrev();

    outOfLineEXInstr->setPrev(self()->getPrev()); // Temporarily set Prev()
                                                  // instruction of snippet to
                                                  // Prev() of EX just in case
                                                  // we insert LR_move on
                                                  // assignRegisters()
    ...
    // If the value is set here, the snippet can be handled in an identical
    // fashion to a normal constantdataSnippet
  }
  ...
  if (self()->getOpCodeValue() == TR::InstOpCode::TBEGIN ||
    ...
    for (int32_t i = TR::RealRegister::GPR0; i != TR::RealRegister::GPR15 + 1;
         i++) {
      if (0 !=
          self()->cg()->getS390Linkage()->getPreserved(
              (TR::RealRegister::RegNum)i)) {
...
bool OMR::Z::Instruction::isHPRUpgradable(uint16_t operandNumber) {
  if (_opcode.is64bit())
    return false;
  ...
  if (self()->cg()->supportsHighWordFacility() &&
    ...
    if (_opcode.is64bit()) {
      if (self()->getOpCodeValue() == TR::InstOpCode::RISBG ||
          self()->getOpCodeValue() == TR::InstOpCode::RISBGN) {
        uint8_t startBit =
            ((TR::S390RIEInstruction *)self())->getSourceImmediate8One();
        uint8_t endBit =
            ((TR::S390RIEInstruction *)self())->getSourceImmediate8Two();
        uint8_t rotateAmnt =
            ((TR::S390RIEInstruction *)self())->getSourceImmediate8();
        if ((startBit + rotateAmnt) < 32 && (endBit - rotateAmnt) > 63) {
    ...
    if (reg->getRegisterPair()) {
      ...
      //        cg()->getDebug()->getName(reg->getLowOrder()),
      //        cg()->getDebug()->getName(reg->getHighOrder()), self());
    }
  }
...
uint32_t OMR::Z::Instruction::useTargetRegister(TR::Register *reg) {
  ...
  if (rp) {
    ...
    redefined |= (lowReg->getStartOfRange() != NULL) ||
                 (highReg->getStartOfRange() != NULL);
...
OMR::Z::Instruction::opCodeCanBeAdjustedTo(TR::InstOpCode::Mnemonic op) {
  switch (op) {
...
void OMR::Z::Instruction::attemptOpAdjustmentForLongDisplacement() {
  TR::InstOpCode::Mnemonic n_op =
      self()->opCodeCanBeAdjustedTo(self()->getOpCodeValue());
  if (n_op != TR::InstOpCode::BAD)
    self()->setOpCodeValue(n_op);

  if (self()->getOpCode().getInstructionFormat(self()->getOpCodeValue()) ==
      RXY_FORMAT)
    self()->setKind(IsRXY);
  else if (self()->getOpCode().getInstructionFormat(self()->getOpCodeValue()) ==
           RSY_FORMAT)
    self()->setKind(IsRSY);
  else if (self()->getOpCode().getInstructionFormat(self()->getOpCodeValue()) ==
           SIY_FORMAT) // LL: Add SIY
    self()->setKind(IsSIY);
  else if (self()->cg()->getDebug())
    TR_ASSERT(0, "only RX, RS and SI instructions can be safely mapped to long "
                 "displacement: opCode: %s",
              self()->cg()->getDebug()->getOpCodeName(&self()->getOpCode()));
  else
    TR_ASSERT(0, "only RX, RS and SI instructions can be safely mapped to long "
                 "displacement");
}
...
OMR::Z::Instruction::useSourceMemoryReference(TR::MemoryReference *memRef) {
  ...
  if (!self()->cg()->afterRA()) {
    ...
    if (self()->hasLongDisplacementSupport() &&
        ...
        (disp < 0 || disp >= MAXDISP) &&
        (disp >= MINLONGDISP || disp <= MAXLONGDISP)) {
...
OMR::Z::Instruction::assignRegisterNoDependencies(TR::Register *reg) {
  ...
  if (reg->getRealRegister() != NULL) {
    ...
    if (!skipBookkeeping && realReg->getState() != TR::RealRegister::Locked &&
      ...
      if (enableHighWordRA && virtReg->getKind() != TR_FPR &&
          virtReg->is64BitReg() && virtReg->getKind() != TR_VRF) {
        toRealRegister(realReg)->getHighWordRegister()->setAssignedRegister(
            NULL);
        toRealRegister(realReg)->getHighWordRegister()->setState(
            TR::RealRegister::Free);
        self()->cg()->traceRegFreed(
            virtReg, toRealRegister(realReg)->getHighWordRegister());
      }
  ...
  if ((reg->getRegisterPair() &&
    ...
    if (realRegHigh->getState() != TR::RealRegister::Locked && BOOKKEEPING &&
      ...
      if (enableHighWordRA && virtRegHigh->getKind() != TR_FPR &&
          virtRegHigh->is64BitReg() && virtRegHigh->getKind() != TR_VRF) {
        toRealRegister(realRegHigh)
            ->getHighWordRegister()
        ...
        self()->cg()->traceRegFreed(
            virtRegHigh, toRealRegister(realRegHigh)->getHighWordRegister());
      }
    ...
    if (realRegLow->getState() != TR::RealRegister::Locked && BOOKKEEPING &&
      ...
      if (enableHighWordRA && virtRegLow->getKind() != TR_FPR &&
          virtRegLow->is64BitReg() && virtRegLow->getKind() != TR_VRF) {
        toRealRegister(realRegLow)
            ->getHighWordRegister()
            ->setAssignedRegister(NULL);
        toRealRegister(realRegLow)
            ->getHighWordRegister()
            ->setState(TR::RealRegister::Free);
        self()->cg()->traceRegFreed(
            virtRegLow, toRealRegister(realRegLow)->getHighWordRegister());
      }
...
void OMR::Z::Instruction::assignOrderedRegisters(
  ...
  // but care
  // must be taken to avoid calling assignRegisterNoDependencies on the same
  // index twice.
  //
  // For example, a GPR:AR register pair seems to be replaced with one real
  // register. If
  // using a two pass method that only checks for 'if reg[i]->getRegisterPair()'
  // on the
  ...
  for (i = 0; i < _targetRegSize; ++i) {
    ...
    if (self()->cg()->supportsHighWordFacility() &&
        !comp->getOption(TR_DisableHighWordRA)) {
      // don't need to block HPR here beacuse no Highword instruction uses
      // register pair
      // but make sure we do not spill to the targetReg's HPR (even if it became
      // free) while assigning sourceReg
      if (_targetReg[i]->getRegisterPair() &&
          _targetReg[i]->getRegisterPair()->getHighOrder()->getKind() !=
              TR_FPR &&
          _targetReg[i]->getRegisterPair()->getHighOrder()->getKind() !=
              TR_VRF) {
        uint32_t availHPRMask = self()->cg()->getAvailableHPRSpillMask();
        availHPRMask &= ~(TR::RealRegister::getBitMask(
            toRealRegister(_targetReg[i]->getRegisterPair()->getHighOrder())
                ->getHighWordRegister()
                ->getRegisterNumber()));
        availHPRMask &= ~(TR::RealRegister::getBitMask(
            toRealRegister(_targetReg[i]->getRegisterPair()->getLowOrder())
                ->getHighWordRegister()
                ->getRegisterNumber()));
  ...
  if (_targetReg) {
    for (i = 0; i < _targetRegSize; ++i) {
      ...
      /*
        When we are mixing 32 and 64 bit registers, we have to ensure that 64
        bit instructions use 64 bit registers.
        If this assertion is triggered, it is probably because an evaluator
        called allocateRegister() when it should
        have called allocate64bitRegister().
       */
      if ((_opcode.is64bit() || _opcode.is32to64bit()) &&
          self()->cg()->use64BitRegsOn32Bit() &&
          TR::Compiler->target.is32Bit() &&
          !_targetReg[i]->getRealRegister()) // skip check if the target reg is
                                             // already a real register.
      {
        TR_ASSERT(_targetReg[i]->getKind() != TR_GPR,
                  "Using GPR for 64 bit instruction %s [%s] in 32 bit mode",
      ...
      if (self()->cg()->supportsHighWordFacility() &&
        ...
        if (_opcode.is64bit() || _opcode.is32to64bit() ||
          ...
          if (self()->getOpCodeValue() == TR::InstOpCode::RISBG ||
            ...
            uint8_t endBit =
                ((TR::S390RIEInstruction *)self())->getSourceImmediate8Two();
            if (endBit &
                0x80) // if the zero bit is set, target reg will be 64bit
            {
        ...
        if ((self()->getOpCodeValue() == TR::InstOpCode::RISBLG ||
             self()->getOpCodeValue() == TR::InstOpCode::RISBHG) &&
            ((TR::S390RIEInstruction *)self())
                    ->getExtendedHighWordOpCode()
                    .getOpCodeValue() != TR::InstOpCode::BAD) {
          if (((TR::S390RIEInstruction *)self())
                  ->getExtendedHighWordOpCode()
                  .isOperandHW(registerOperandNum))
      ...
      if (self()->cg()->supportsHighWordFacility() &&
          ...
          _targetReg[i]->getKind() != TR_FPR &&
          _targetReg[i]->getKind() != TR_VRF) {
        if (toRealRegister(_targetReg[i])->getState() ==
                TR::RealRegister::Free &&
            targetRegIs64Bit) {
          blockTargetHighword = true;
          toRealRegister(_targetReg[i])
              ->getHighWordRegister()
              ->setState(TR::RealRegister::Blocked);
          TR_ASSERT(i == 0, "more than 1 HPR target regs need to be blocked?");
        }

        // make sure we do not spill to the targetReg's HPR later (even if it's
        // free)
        uint32_t availHPRMask = self()->cg()->getAvailableHPRSpillMask();
        availHPRMask &=
            ~(TR::RealRegister::getBitMask(toRealRegister(_targetReg[i])
                                               ->getHighWordRegister()
                                               ->getRegisterNumber()));
        self()->cg()->setAvailableHPRSpillMask(availHPRMask);
      }
  ...
  // reblock them right after (required for srcReg assignment)."
  // "We only unblock those target registers that are not alive anymore. Block
  // them again after pre conds are done."
      ...
      if (firstNonPairSourceRegister) {
        ...
        if ((self()->getOpCodeValue() == TR::InstOpCode::RISBLG ||
            ...
            ((TR::S390RIEInstruction *)self())
                    ->getExtendedHighWordOpCode()
                    .getOpCodeValue() != TR::InstOpCode::BAD) {
          firstNonPairSourceRegister->setAssignToHPR(
              ((TR::S390RIEInstruction *)self())
                  ->getExtendedHighWordOpCode()
                  .isOperandHW(registerOperandNum));
        } else
          firstNonPairSourceRegister->setAssignToHPR(
              _opcode.isOperandHW(registerOperandNum));
      }

      if (secondNonPairSourceRegister) {
        secondNonPairSourceRegister->setAssignToHPR(
            _opcode.isOperandHW(registerOperandNum + 1));
      }
    }
  ...
  /* Unblock registers that were Assigned and then Blocked in deeper stack
   * frames of RA (src/target is HPR/GPR). */
  TR::RealRegister *reg = NULL;
  while (reg = self()->cg()->machine()->getNextRegFromUpgradedBlockedList()) {
    self()->cg()->traceRegisterAssignment(
        "Restoring blocked register %R (0x%p) to previous state", reg, reg);
    reg->unblock();
  }
}
...
void OMR::Z::Instruction::assignRegistersAndDependencies(
  ...
  if (self()->getOpCode().isBranchOp() && self()->getKind() == IsRIE &&
    ...
    TR_ASSERT(oi, "Could not find S390OutOfLineCodeSection stream from label.  "
                  "instr=%p, label=%p\n",
  ...
  if (self()->getDependencyConditions()) {
    ...
    // if they hold vector data
    // that part of the data would not be preserved. This code looks at the data
    // stored in non-volatile FPRs
      ...
      for (int32_t i = TR::RealRegister::FirstFPR;
        ...
        if (linkage->getPreserved(REGNUM(i))) {
          TR::RealRegister *targetRegister =
              machine->getS390RealRegister(REGNUM(i));
...
void OMR::Z::Instruction::blockHPR(TR::Register *reg) {
  TR::Compilation *comp = self()->cg()->comp();
  if (reg->getKind() != TR_FPR && reg->getKind() != TR_VRF &&
    ...
    if (reg->is64BitReg() && reg->getAssignedRegister() != NULL) {
      TR::RealRegister *assignedReg =
          reg->getAssignedRegister()->getRealRegister();
      if (assignedReg != NULL) {
        if (toRealRegister(assignedReg)->getHighWordRegister()->getState() ==
            TR::RealRegister::Assigned) {
          toRealRegister(assignedReg)
              ->getHighWordRegister()
              ->setState(TR::RealRegister::Blocked);
        }
      }
...
void OMR::Z::Instruction::unblockHPR(TR::Register *reg) {
  TR::Compilation *comp = self()->cg()->comp();
  if (reg->getKind() != TR_FPR && reg->getKind() != TR_VRF &&
      self()->cg()->supportsHighWordFacility() &&
      !comp->getOption(TR_DisableHighWordRA)) {
    if (reg->is64BitReg() && reg->getAssignedRegister() != NULL) {
      ...
      if (assignedReg != NULL) {
        if (toRealRegister(assignedReg)->getHighWordRegister()->getState() ==
          ...
          toRealRegister(assignedReg)
              ->getHighWordRegister()
              ->setState(TR::RealRegister::Assigned, reg->isPlaceholderReg());
        }
      }
    }
  }
...
void OMR::Z::Instruction::setUseDefRegisters(bool updateDependencies) {
  ...
  // should unnecessary as each instruction
      ...
      if (regPair != NULL) {
        ...
        if ((self()->getOpCodeValue() == TR::InstOpCode::CPSDR) && (i == 1)) {
          ...
          (*_useRegs)[indexSource++] = _sourceReg[i];
                   ...
                   self()->getOpCodeValue() != TR::InstOpCode::LAM &&
  ...
  if (self()->getOpCodeValue() == TR::InstOpCode::LM ||
    ...
    loadOrStoreMultiple = 0; // load
           ...
           self()->getOpCodeValue() == TR::InstOpCode::STAM ||
  ...
  if (loadOrStoreMultiple >= 0) {
    ...
    if (lastReg->getRealRegister() && firstReg->getRealRegister()) {
      ...
      bool isAR = ((self()->getOpCodeValue() == TR::InstOpCode::LAM) ||
                   (self()->getOpCodeValue() == TR::InstOpCode::STAM));
      ...
      if (numRegs > 0) {
        ...
        {
          TR::RealRegister *reg = self()->cg()->machine()->getS390RealRegister(
...
void OMR::Z::Instruction::addARDependencyCondition(TR::Register *virtAR,
                                                   TR::Register *assignedGPR) {
  bool assignPreCondition = false;
  bool newCondition = false;

  // AR register for stack pointer doesn't need to be set
  for (uint32_t x = 1;
       x <= self()->cg()->getS390Linkage()->getNumStackPointerRegisters();
       x++) {
    if (REGINDEX((toRealRegister(assignedGPR->getRealRegister()))
                     ->getRegisterNumber()) ==
        REGINDEX(self()
                     ->cg()
                     ->getS390Linkage()
                     ->getStackPointerRealRegister(x)
                     ->getRegisterNumber())) {
  ...
  if (self()->getDependencyConditions()) {
    if (self()->getDependencyConditions()->searchPreConditionRegister(
            assignedGPR->getAssignedRegister())) {
      // add another pre condition to current list
      TR::RegisterDependencyConditions *tmp = new (self()->cg()->trHeapMemory())
          TR::RegisterDependencyConditions(self()->getDependencyConditions(), 1,
                                           0, self()->cg());
      ...
      TR::RegisterDependencyConditions *tmp = new (self()->cg()->trHeapMemory())
          TR::RegisterDependencyConditions(self()->getDependencyConditions(), 0,
                                           1, self()->cg());
      self()->resetDependencyConditions(tmp);
    }
  } else {
    ...
    TR::RegisterDependencyConditions *tmp = new (self()->cg()->trHeapMemory())
        TR::RegisterDependencyConditions(0, 1, self()->cg());
    self()->resetDependencyConditions(tmp);
  }
  if (assignPreCondition) {
    self()->cg()->traceRegisterAssignment(
        "PRE dependancy for instruction %p on virtual reg %s -> %s to be "
        "added\n",
        self(), self()->cg()->getDebug()->getName(virtAR),
        self()->cg()->getDebug()->getName(assignedGPR));
    newCondition =
        self()->getDependencyConditions()->addPreConditionIfNotAlreadyInserted(
            virtAR,
            REGNUM(TR::RealRegister::FirstAR - TR::RealRegister::FirstGPR +
                   toRealRegister(assignedGPR)->getRegisterNumber()));
  } else {
    self()->cg()->traceRegisterAssignment(
        "POST dependancy for instruction %p on virtual reg %s -> %s to be "
        "added\n",
        self(), self()->cg()->getDebug()->getName(virtAR),
        self()->cg()->getDebug()->getName(assignedGPR));
    newCondition =
        self()->getDependencyConditions()->addPostConditionIfNotAlreadyInserted(
            virtAR,
            REGNUM(TR::RealRegister::FirstAR - TR::RealRegister::FirstGPR +
                   toRealRegister(assignedGPR)->getRegisterNumber()));
  }
  ...
  if (newCondition) {
    self()->cg()->traceRegisterAssignment(
        "dependancy for instruction %p on virtual reg %s -> %s added\n", self(),
        self()->cg()->getDebug()->getName(virtAR),
        self()->cg()->getDebug()->getName(assignedGPR));
  } else {

    self()->cg()->traceRegisterAssignment(
        "duplicate dependancy for instruction %p on virtual reg %s ->  %s not "
        "added\n",
        self(), self()->cg()->getDebug()->getName(virtAR),
        self()->cg()->getDebug()->getName(assignedGPR));
    virtAR->decTotalUseCount();
...
bool OMR::Z::Instruction::isAsmGen() {
...
bool OMR::Z::Instruction::isTailCall() {
...
void OMR::Z::Instruction::setJITEntry() {
  TR_ASSERT(self()->getOpCodeValue() == TR::InstOpCode::LABEL,
            "JIT entry point should be a branch instruction\n");
  _flags.set(JITEntry);
}

void OMR::Z::Instruction::setJITExit() {
  TR_ASSERT(
      self()->getOpCode().isBranchOp() || self()->isExceptBranchOp() ||
          self()->throwsImplicitException(),
