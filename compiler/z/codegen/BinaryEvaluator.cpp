...
#define PRINT_ME(string, node, cg)                                             \
  TR::Delimiter evalDelimiter(TR::comp(), TR::comp()->getOption(TR_TraceCG),   \
                              "EVAL", string)
#else
...
TR::RegisterPair *lnegFor32Bit(TR::Node *node, TR::CodeGenerator *cg,
  ...
  // that are
  // handled by the caller.   If the deps are null, we must handle deps for
  // internal control
  // flow ourselves here.
  //
  if (!dep) {
  ...
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node, doneLNeg);
  ...
  generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, doneLNeg,
                               localDeps);
...
inline TR::RegisterPair *laddConst(TR::Node *node, TR::CodeGenerator *cg,
  ...
  if (ENABLE_ZARCH_FOR_32 &&
    ...
    if (l_value != 0) {
      ...
      // MS(l_value) int.
      cursor = generateS390BranchInstruction(
          cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNCRC, node, doneLAdd);
      cursor->setStartInternalControlFlow();
    ...
    cursor =
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, doneLAdd);
    if (dependencies) {
 ...
 */
TR::Register *laddHelper(TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is32Bit(),
            "Should call laddEvaluator64(...) for 64Bit CodeGen!");
  ...
  if (NEED_CC(node) || (node->getOpCodeValue() == TR::luaddc)) {
    ...
    TR_S390BinaryCommutativeAnalyser(cg).longAddAnalyser(node,
                                                         TR::InstOpCode::LR);
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    targetRegisterPair = laddConst(node, cg, targetRegisterPair,
                                   secondChild->getLongInt(), NULL);
...
TR::Register *laddHelper64(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (NEED_CC(node) || (node->getOpCodeValue() == TR::luaddc)) {
    ...
    // use logical add instead of add
    // we assume that the analyser will use the opcodes provided and does not
  ...
  bool hasCompressedPointers =
      TR::TreeEvaluator::genNullTestForCompressedPointers(
          node, cg, targetRegister, skipAdd);
    ...
    if (isCompressionSequence) {
      if (skipAdd) {
        if (targetRegister && (targetRegister == firstChild->getRegister())) {
          ...
          generateRRInstruction(cg, TR::InstOpCode::LGR, node,
                                firstRegisterCopied, firstChild->getRegister());
        }

        generateRRInstruction(cg, TR::InstOpCode::LGR, node, targetRegister,
                              secondRegister);
      } else {
  ...
  if (hasCompressedPointers && ((secondChild->getOpCodeValue() != TR::lconst) ||
    ...
    if (oldSequence) {
      if (olderSequence) {
        ...
        generateRRInstruction(cg, TR::InstOpCode::AGR, node, targetRegister,
                              secondRegister);
    } else {
      if (firstRegisterCopied == NULL) {
        ...
        TR::MemoryReference *tempMR =
            generateS390MemoryReference(memRefChild, cg);
        generateRXInstruction(cg, TR::InstOpCode::L, node, targetRegister,
                              tempMR);
      } else {
        generateRRInstruction(cg, TR::InstOpCode::LR, node, targetRegister,
                              firstRegisterCopied);
      }
    }
    ...
    if (skipAdd) {
      ...
      if (true || oldSequence) {
        conditions->addPostCondition(secondRegister,
                                     TR::RealRegister::AssignAny);
...
TR::Register *generic32BitAddEvaluator(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    bool useLA = node->isNonNegative() && !cg->use64BitRegsOn32Bit() &&
                 ...
                 (TR::Compiler->target.is64Bit() || node->isNonNegative());
    ...
    if (!findPreviousAGIDef(comp->getAppendInstruction(), delay, childTargetReg,
                            comp)) {
      useLA = false;
    }
    ...
    if (useAHIK) {
      ...
      if (value < 0 || value > MAXDISP) {
        generateRXYInstruction(cg, TR::InstOpCode::LAY, node, targetRegister,
                               memRef);
      } else {
        generateRXInstruction(cg, TR::InstOpCode::LA, node, targetRegister,
                              memRef);
 ...
 * and handles all subtract types shorter than an int
...
inline TR::Register *generic32BitSubEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    switch (secondChild->getDataType()) {
    case TR::Address:
      TR_ASSERT(TR::Compiler->target.is32Bit(), "generic32BitSubEvaluator(): "
    ...
    if (firstChild->getRegister() && !cg->canClobberNodesRegister(firstChild) &&
        ((-value) >= MIN_IMMEDIATE_VAL && (-value) <= MAX_IMMEDIATE_VAL)) {
  ...
  } else // The second child of the sub node isn't a loadConstant type.
  {
    ...
    TR::DataType type = node->getDataType();
    temp.intBinaryAnalyser(node, TR::InstOpCode::SR, type == TR::Int16
                                                         ? TR::InstOpCode::SH
                                                         : TR::InstOpCode::S);
    targetRegister = node->getRegister();
  }
 ...
 */
inline TR::Register *lDivRemGenericEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg,
                                             bool isDivision) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild == firstChild) {
    if (isDivision) {
      ...
      generateLoad32BitConstant(cg, node, 0, dividendPair->getHighOrder(),
                                true);
    } else // remainder
    {
  ...
  if (handleConstDenominator) {
    ...
    if (denominator == -1 &&
      ...
      {
        generateLoad32BitConstant(cg, node, 0, dividendPair->getLowOrder(),
                                  true);
        generateLoad32BitConstant(cg, node, 0, dividendPair->getHighOrder(),
                                  true);
      } else {
        ...
        dep->addPostCondition(dividendPair->getHighOrder(),
                              TR::RealRegister::LegalEvenOfPair);
        dep->addPostCondition(dividendPair->getLowOrder(),
                              TR::RealRegister::LegalOddOfPair);

        TR::LabelSymbol *endDiv =
        ...
        TR::LabelSymbol *doDiv =
            TR::LabelSymbol::create(cg->trHeapMemory(), cg);
        generateS390ImmOp(cg, TR::InstOpCode::C, node,
                          dividendPair->getHighOrder(),
                          dividendPair->getHighOrder(), (int32_t)0x80000000);
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, node, doDiv);
        cursor->setStartInternalControlFlow();
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::C, node, dividendPair->getLowOrder(),
            (signed char)0, TR::InstOpCode::COND_BERC, endDiv);
        ...
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, endDiv,
                                     dep);
      }
      node->setRegister(dividendPair);
      cg->decReferenceCount(firstChild);
    ...
    // case A/1 Q=A, R=0
             ...
             performTransformation(comp,
                                   "O^O Denominator is 1 for ldir/lrem.\n")) {
      if (!isDivision) // remainder
      {
        generateLoad32BitConstant(cg, node, 0, dividendPair->getLowOrder(),
                                  true);
    ...
    // case A/(+-)2^x - division is done using shifts
                  ...
                  denominator)) > 0 &&
             performTransformation(
                 comp, "O^O Denominator is powerOfTwo (%d) for ldir/lrem.\n",
                 denominator)) {
      int64_t absValueOfDenomintor =
      ...
      if (isDivision) {
        if (!firstChild->isNonNegative()) {
          TR::LabelSymbol *skipSet =
              TR::LabelSymbol::create(cg->trHeapMemory(), cg);

          cursor = generateS390CompareAndBranchInstruction(
              cg, TR::InstOpCode::C, node, dividendPair->getHighOrder(),
              (signed char)0, TR::InstOpCode::COND_BNLRC, skipSet);
          cursor->setStartInternalControlFlow();

          // adjustment to dividend if dividend is negative
          dividendPair =
              laddConst(node, cg, dividendPair, absValueOfDenomintor - 1, dep);
        ...
        // divide
        if (isUnsigned)
          generateRSInstruction(cg, TR::InstOpCode::SRDL, node, dividendPair,
                                shiftAmnt, dep);
        else
          generateRSInstruction(cg, TR::InstOpCode::SRDA, node, dividendPair,
                                shiftAmnt, dep);
      ...
      } else // remainder
      {
        ...
        if (!firstChild->isNonNegative()) {
          ...
          TR::RegisterPair *tempRegisterPair1 =
              cg->allocateConsecutiveRegisterPair(tempRegister1h,
                                                  tempRegister1l);
          TR::RegisterPair *tempRegisterPair2 =
              cg->allocateConsecutiveRegisterPair(tempRegister2h,
                                                  tempRegister2l);
          generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                tempRegisterPair2->getLowOrder(),
                                dividendPair->getLowOrder());
          generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                tempRegisterPair2->getHighOrder(),
                                dividendPair->getHighOrder());
          generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                tempRegisterPair1->getLowOrder(),
                                dividendPair->getLowOrder());
          generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                tempRegisterPair1->getHighOrder(),
                                dividendPair->getHighOrder());
          generateRSInstruction(cg, TR::InstOpCode::SRDA, node,
                                tempRegisterPair2, 63);
          generateRILInstruction(cg, TR::InstOpCode::NILF, node,
                                 tempRegisterPair2->getLowOrder(),
                                 (int32_t)(absValueOfDenomintor - 1));
          generateRILInstruction(cg, TR::InstOpCode::NILF, node,
                                 tempRegisterPair2->getHighOrder(),
                                 (int32_t)((absValueOfDenomintor - 1) >> 32));
          generateRRInstruction(cg, TR::InstOpCode::ALR, node,
                                dividendPair->getLowOrder(),
                                tempRegisterPair2->getLowOrder());
          generateRRInstruction(cg, TR::InstOpCode::ALCR, node,
                                dividendPair->getHighOrder(),
                                tempRegisterPair2->getHighOrder());
          generateRILInstruction(
              cg, TR::InstOpCode::NILF, node, dividendPair->getLowOrder(),
              (int32_t)((int64_t)CONSTANT64(0xFFFFFFFFFFFFFFFF) -
                        absValueOfDenomintor + 1));
          if (absValueOfDenomintor != (int32_t)absValueOfDenomintor)
            generateRILInstruction(
                cg, TR::InstOpCode::NILF, node, dividendPair->getHighOrder(),
                (int32_t)(((int64_t)CONSTANT64(0xFFFFFFFFFFFFFFFF) -
                           absValueOfDenomintor + 1) >>
                          32));
          generateRRInstruction(cg, TR::InstOpCode::SLR, node,
                                tempRegisterPair1->getLowOrder(),
                                dividendPair->getLowOrder());
          generateRRInstruction(cg, TR::InstOpCode::SLBR, node,
                                tempRegisterPair1->getHighOrder(),
                                dividendPair->getHighOrder());
          generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                dividendPair->getLowOrder(),
          ...
          generateRSInstruction(cg, TR::InstOpCode::SLDL, node, dividendPair,
                                64 - shiftAmnt, dep);
          generateRSInstruction(cg, TR::InstOpCode::SRDL, node, dividendPair,
                                64 - shiftAmnt, dep);
        }
  ...
  if (performTransformation(comp, "O^O Using 64bit ldiv on 32bit driver.\n")) {
    ...
    TR::RegisterPair *divisorPair =
        (TR::RegisterPair *)cg->gprClobberEvaluate(secondChild);
    TR::Register *litPoolBase = NULL;
    bool highWordRADisabled = !cg->supportsHighWordFacility() ||
                              comp->getOption(TR_DisableHighWordRA);

    // 1st-6th Post Conditions as listed below
    // Setup arguments
    TR::RegisterDependencyConditions *dependencies =
        new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 8, cg);
    dependencies->addPostCondition(dividendPair, TR::RealRegister::EvenOddPair);
    dependencies->addPostCondition(dividendPair->getHighOrder(),
                                   TR::RealRegister::LegalEvenOfPair);
    dependencies->addPostCondition(dividendPair->getLowOrder(),
                                   TR::RealRegister::LegalOddOfPair);
    dependencies->addPostCondition(divisorPair, TR::RealRegister::EvenOddPair);
    dependencies->addPostCondition(divisorPair->getHighOrder(),
                                   TR::RealRegister::LegalEvenOfPair);
    dependencies->addPostCondition(divisorPair->getLowOrder(),
                                   TR::RealRegister::LegalOddOfPair);

    if (!node->getOpCode().isUnsigned() && !node->isSimpleDivCheck() &&
    ...
    cursor = generateRSInstruction(cg, TR::InstOpCode::SLLG, node,
                                   divisorPair->getHighOrder(),
                                   divisorPair->getHighOrder(), 32);

    cursor = generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                   divisorPair->getHighOrder(),
                                   divisorPair->getLowOrder());

    if (!node->getOpCode().isUnsigned()) // Signed Divide Code
    {
      if (!node->isSimpleDivCheck() && !firstChild->isNonNegative() &&
        ...
        generateS390ImmOp(cg, TR::InstOpCode::CG, node,
                          dividendPair->getHighOrder(),
                          dividendPair->getHighOrder(),
                          (int64_t)(CONSTANT64(0x8000000000000000)),
                          dependencies, litPoolBase);

        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, node, gotoDiv);
        cursor->setStartInternalControlFlow();
        ...
        if (!isDivision) {
          cursor = generateS390CompareAndBranchInstruction(
              cg, TR::InstOpCode::CG, node, divisorPair->getHighOrder(),
              (signed char)-1, TR::InstOpCode::COND_BNERC, gotoDiv);
          // Check to see if compare generated a register to build constChar
          TR::Instruction *compareInstr =
              (cursor->getOpCode().isCompare()) ? NULL : cursor->getPrev();
          if (compareInstr && compareInstr->getNumRegisterOperands() == 2) {
            dependencies->addPostCondition(compareInstr->getRegisterOperand(2),
                                           TR::RealRegister::AssignAny);
          }
          cursor = generateRIInstruction(cg, TR::InstOpCode::LGHI, node,
                                         dividendPair->getHighOrder(), 0);

          generateS390BranchInstruction(
              cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, dontDiv);
        } else {
          generateS390CompareAndBranchInstruction(
              cg, TR::InstOpCode::CG, node, divisorPair->getHighOrder(),
              (signed char)-1, TR::InstOpCode::COND_BERC, dontDiv);
        }
      ...
      // do division
      cursor = generateRRInstruction(cg, TR::InstOpCode::DSGR, node,
                                     dividendPair, divisorPair->getHighOrder());

      // Label to skip the division
      cursor = generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                            dontDiv, dependencies);
    } else // Unsigned Divide Code
    {
      ...
      // do division
      cursor = generateRRInstruction(cg, TR::InstOpCode::DLGR, node,
                                     dividendPair, divisorPair->getHighOrder());
    }

    // mainly common clean-up code
    if (isDivision) {
      cursor = generateRSInstruction(cg, TR::InstOpCode::SRLG, node,
                                     dividendPair->getHighOrder(),
                                     dividendPair->getLowOrder(), 32);
    } else // remainder
    {
      cursor = generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                     dividendPair->getLowOrder(),
    ...
    TR::RegisterPair *targetRegisterPair =
        cg->allocateConsecutiveRegisterPair(oddRegister, evenRegister);
    TR::RegisterPair *divisorPair =
        (TR::RegisterPair *)cg->gprClobberEvaluate(secondChild);

    // 1st-4th Pre Conditions as listed below
    // 1st-10th Post Conditions as listed below
    // 11th Post Condition may be added by generateDirectCall() if it uses
    // generateRegLitRefInstruction.
    TR::RegisterDependencyConditions *dependencies =
        new (cg->trHeapMemory()) TR::RegisterDependencyConditions(4, 11, cg);

    // Setup arguments
    dependencies->addPreCondition(dividendPair->getLowOrder(),
                                  TR::RealRegister::GPR3);
    dependencies->addPreCondition(dividendPair->getHighOrder(),
                                  TR::RealRegister::GPR2);
    dependencies->addPreCondition(divisorPair->getLowOrder(),
                                  TR::RealRegister::GPR9);
    dependencies->addPreCondition(divisorPair->getHighOrder(),
    ...
    // Setup return value
    if (isDivision) {
      cursor = generateDirectCall(cg, node, false,
                                  cg->symRefTab()->findOrCreateRuntimeHelper(
                                      TR_S390longDivide, false, false, false),
                                  dependencies);
    } else {
      cursor = generateDirectCall(
          cg, node, false, cg->symRefTab()->findOrCreateRuntimeHelper(
                               TR_S390longRemainder, false, false, false),
          dependencies);
    }
    cursor->setDependencyConditions(dependencies);
...
inline TR::Register *lDivRemGenericEvaluator64(TR::Node *node,
  ...
  if (handleConstDenominator) {
    ...
    if (denominator == -1) {
      ...
      if (!isDivision) // remainder
      {
        ...
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node, endDiv);
        cursor->setStartInternalControlFlow();
    ...
    // case A/(+-)2^x - division is done using shifts
      ...
      if (isDivision) {
        ...
        if (!firstChild->isNonNegative()) {
          ...
          cursor = generateS390CompareAndBranchInstruction(
              cg, TR::InstOpCode::CG, node, firstRegister, (signed char)0,
              TR::InstOpCode::COND_BNLRC, skipSet);
          cursor->setStartInternalControlFlow();
          ...
          generateS390ImmOp(cg, TR::InstOpCode::AG, node, firstRegister,
                            firstRegister, absValueOfDenomintor - 1, deps);
          skipSet->setEndInternalControlFlow();
          generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, skipSet,
                                       deps);
        }
  ...
  if (!doConditionalRemainder && !node->isSimpleDivCheck() &&
      !firstChild->isNonNegative() && !secondChild->isNonNegative() &&
      !(secondChild->getOpCodeValue() == TR::lconst &&
        secondChild->getLongInt() != CONSTANT64(0xFFFFFFFFFFFFFFFF))) {
    //  We have to check for a special case where we do
    ...
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BNERC, node, doDiv);
...
TR::Register *iDivRemGenericEvaluator(TR::Node *node, TR::CodeGenerator *cg,
  ...
  //
  // Note:  D and DR require that R1=0 before executing the division
  bool needCheck = (!node->isSimpleDivCheck() && !firstChild->isNonNegative() &&
                    !secondChild->isNonNegative());
  ...
  if (divchkDivisorMR && !needCheck) {
    ...
    if (divchkDivisorMR) {
      ...
      generateRXInstruction(cg, dtype.isInt64() ? TR::InstOpCode::LG
                                                : TR::InstOpCode::L,
  ...
  if (needCheck) {
    ...
    generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::C, node,
                                            sourceRegister, (signed char)-1,
                                            TR::InstOpCode::COND_BNERC, doDiv);
  ...
  if (sourceMR) {
    if (node->getOpCode().isUnsigned())
      cursor = generateRXInstruction(cg, TR::InstOpCode::DL, node,
                                     targetRegisterPair, sourceMR);
    else
      cursor = generateRXInstruction(cg, TR::InstOpCode::D, node,
                                     targetRegisterPair, sourceMR);
  } else {
    if (node->getOpCode().isUnsigned())
      cursor = generateRRInstruction(cg, TR::InstOpCode::DLR, node,
                                     targetRegisterPair, sourceRegister);
    else
      cursor = generateRRInstruction(cg, TR::InstOpCode::DR, node,
                                     targetRegisterPair, sourceRegister);
  }
  ...
  if (skipDiv) {
    ...
    TR::Instruction *cursor = generateS390PseudoInstruction(
 ...
 */
inline TR::Register *genericLongShift(TR::Node *node, TR::CodeGenerator *cg,
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (src2Reg->getRegisterPair())
      src2Reg = src2Reg->getRegisterPair()->getHighOrder();
...
inline TR::Register *genericLongShiftSingle(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (node->getOpCodeValue() == TR::lshl &&
        firstChild->getOpCodeValue() == TR::i2l &&
      ...
      TR::InstOpCode::Mnemonic opCode =
          cg->getS390ProcessorInfo()->supportsArch(
              ...
              : TR::InstOpCode::RISBG;
      generateRIEInstruction(cg, opCode, node, trgReg, srcReg,
                             (int8_t)(32 - value),
...
inline TR::Register *genericIntShift(TR::Node *node, TR::CodeGenerator *cg,
  ...
  // doing a
  // shift. Only generate SLLG over SLL if it saves an instruction - an LR to
  // copy a
  ...
  if (shiftOp == TR::InstOpCode::SRL || shiftOp == TR::InstOpCode::SRA) {
    if (altShiftOp != TR::InstOpCode::SRLK &&
  ...
  if (canUseAltShiftOp && (TR::InstOpCode(altShiftOp).is64bit() ||
                           TR::InstOpCode(altShiftOp).is32to64bit()))
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    if (shiftAmount != 0) {
      if (trgReg != srcReg && canUseAltShiftOp) {
        if (cg->supportsHighWordFacility() &&
            !comp->getOption(TR_DisableHighWordRA) && srcReg->assignToHPR() &&
          ...
          generateExtendedHighWordInstruction(node, cg, altShiftOp, trgReg,
                                              srcReg, shiftAmount);
        } else {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::addrAddHelper(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  if (node->getOpCodeValue() == TR::aiadd) {
    if (cg->getCurrentRegisterPressure() < CODEGEN_REGPRESSURE_THRESHOLD)
      ...
      TR_S390BinaryCommutativeAnalyser temp(cg);
      temp.integerAddAnalyser(node, TR::InstOpCode::ALR, TR::InstOpCode::AL,
                              TR::InstOpCode::LR);
    ...
    if (cg->getCurrentRegisterPressure() < CODEGEN_REGPRESSURE_THRESHOLD)
      ...
      TR_S390BinaryCommutativeAnalyser temp(cg);
      temp.integerAddAnalyser(node, TR::InstOpCode::ALGR, TR::InstOpCode::ALG,
                              TR::InstOpCode::LGR);
      return node->getRegister();
    }
  } else
 ...
 */
TR::Register *lsubHelper(TR::Node *node, TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR_ASSERT(TR::Compiler->target.is32Bit(),
            "Call lsubHelper64(...) for 64bit code-gen!");
  TR::Node *secondChild = node->getSecondChild();
  TR::Instruction *cursor = NULL;
  TR::RegisterPair *targetRegisterPair = NULL;
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if (ENABLE_ZARCH_FOR_32 &&
      ...
      if (l_value != 0) {
        ...
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, doneLSub,
                                     dependencies);
      }
    }
  ...
  if (node->getOpCodeValue() == TR::lor) {
    ...
    */
    ...
    if (lushr && lushr->getOpCodeValue() == TR::lushr &&
        ...
        lshl->getOpCodeValue() == TR::lshl && lshl->getReferenceCount() == 1 &&
  ...
  if (node->getOpCodeValue() == TR::lor && andChild &&
    ...
    if (mask1 && mask1 == (~mask2) && shiftBy <= 63 &&
        performTransformation(cg->comp(),
                              "O^O Insert bit using RISBG node [%p]\n", node)) {
      ...
      if (mask1 == mask2 &&
          performTransformation(
              cg->comp(), "O^O Insert contiguous bits using RISBG node [%p]\n",
              node)) {
...
TR::Register *genericRotateAndInsertHelper(TR::Node *node,
  ...
  if (firstChild->getOpCode().isConversion()) {
    ...
    if (TR::Compiler->target.is32Bit() &&
        firstChild->getOpCodeValue() == TR::l2i) {
      LongToInt = true;
      traceMsg(comp, "Long2Int conversion in between, need to evaluate if "
                     "RISBG can be used \n");
    }
  ...
  //
  if (node->getOpCode().isAnd() && (firstChild->getOpCode().isLeftShift() ||
                                    firstChild->getOpCode().isRightShift()) &&
      firstChild->getRegister() == NULL &&
      firstChild->getReferenceCount() <= 1 &&
      secondChild->getOpCode().isLoadConst() &&
      firstChild->getSecondChild()->getOpCode().isLoadConst()) {
    // if GRA had decided to assign HPR to these nodes, we cannot use RISBG
    ...
    if (firstChild->getOpCode().isRightShift()) {
      ...
      if (LongToInt) {
        int32_t shiftRightAmt =
            (64 - shiftAmnt); // get original shiftRight amount
    ...
    if (comp->getOption(TR_TraceCG)) {
      traceMsg(comp, "[%p] and/sh[r,l] => rotated-and-insert: tZeros %d, "
                     "lZeros %d, popCnt %d\n",
               node, tZeros, lZeros, popCnt);
      traceMsg(comp, "\t               => rotated-and-insert: shiftMsBit %d, "
                     "shiftLsBit %d \n",
    ...
    if (popCnt == (64 - lZeros - tZeros)) {
      ...
      if (msBit > lsBit) {
        ...
        generateRIEInstruction(cg, cg->getS390ProcessorInfo()->supportsArch(
                                       TR_S390ProcessorInfo::TR_zEC12)
                                       ? TR::InstOpCode::RISBGN
                                       : TR::InstOpCode::RISBG,
                               node, targetReg, sourceReg, msBit, 0x80 + lsBit,
                               shiftAmnt);
      else {
        if (!cg->canClobberNodesRegister(firstChild->getFirstChild()))
...
TR::Register *genericLongAndAsRotateHelper(TR::Node *node,
                                           TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Node *skipedConversion = NULL;
  ...
  if ((node->getOpCodeValue() == TR::land) &&
    ...
    if ((firstChild->getOpCodeValue() == TR::i2l ||
        ...
        firstChild->getReferenceCount() == 1 &&
        firstChild->getRegister() == NULL && firstChild->isHighWordZero()) {
      skipedConversion = firstChild;
      firstChild = firstChild->getFirstChild();
    ...
    if (popCnt == (64 - lZeros - tZeros)) {
      ...
      if (firstChild->getReferenceCount() > 1 ||
          skipedConversion !=
              NULL // Better of with non-destructive RISBG as AND?
          || lZeros > 31 || tZeros > 31 ||
          (lZeros > 0 && tZeros > 0)) // Better off with a single AND?
      {
        doIt = true;
      }
    ...
    if (doIt && performTransformation(
      ...
      TR::InstOpCode::Mnemonic opCode =
          cg->getS390ProcessorInfo()->supportsArch(
              ...
              ? TR::InstOpCode::RISBGN
              : TR::InstOpCode::RISBG;
      generateRIEInstruction(cg, opCode, node, targetReg, sourceReg, msBit,
                             0x80 + lsBit, 0);
 ...
 */
TR::Register *lmulHelper(TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is32Bit(),
            "lmulHelper() is for 32bit code-gen only!");
  TR::Node *firstChild = node->getFirstChild();
  ...
  if (secondChildIsConstant) {
    ...
    if (isNonNegativePowerOf2(src2Value)) {
      if (!noShift) {
        ...
        TR_ASSERT(shiftAmount <= 63 && shiftAmount > 0,
                  "lmulEvaluator - unexpected shift amount for Power of 2.");
        cursor = generateRSInstruction(cg, TR::InstOpCode::SLDL, node,
                                       trgtRegPair, shiftAmount);
      }
  ...
  if (performTransformation(comp,
                            "O^O Using 64bit lmul on 32bit platform.\n")) {
    ...
    if (!node->getOpCode().isUnsigned()) // Signed Multiply
    {
      ...
      bool useMGHI = secondChildIsConstant && value >= MIN_IMMEDIATE_VAL &&
                     value <= MAX_IMMEDIATE_VAL &&
                     secondChild->getRegister() == NULL;
      ...
      cursor = generateRSInstruction(cg, TR::InstOpCode::SLLG, node,
                                     trgtRegPair->getHighOrder(),
                                     trgtRegPair->getHighOrder(), 32);

      cursor = generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                     trgtRegPair->getHighOrder(),
                                     trgtRegPair->getLowOrder());

      if (!useMGHI) {
        dependencies->addPostCondition(src2RegPair->getHighOrder(),
                                       TR::RealRegister::AssignAny);
        dependencies->addPostCondition(src2RegPair->getLowOrder(),
                                       TR::RealRegister::AssignAny);
      }
      ...
      if (!useMGHI) {
        ...
        cursor = generateRSInstruction(cg, TR::InstOpCode::SLLG, node,
                                       src2RegPair->getHighOrder(),
                                       src2RegPair->getHighOrder(), 32);

        cursor = generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                       src2RegPair->getHighOrder(),
                                       src2RegPair->getLowOrder());

        // do multiplication
        cursor = generateRRInstruction(cg, TR::InstOpCode::MSGR, node,
                                       trgtRegPair->getHighOrder(),
                                       src2RegPair->getHighOrder());
      } else {
        cursor = generateRIInstruction(cg, TR::InstOpCode::MGHI, node,
                                       trgtRegPair->getHighOrder(), value);
      }
    ...
    } else // Unsigned Multiply Support
    {
      ...
      dependencies->addPostCondition(src2RegPair,
                                     TR::RealRegister::EvenOddPair);
      dependencies->addPostCondition(src2RegPair->getHighOrder(),
                                     TR::RealRegister::LegalEvenOfPair);
      dependencies->addPostCondition(src2RegPair->getLowOrder(),
      ...
      dependencies->addPostCondition(trgtRegPair,
                                     TR::RealRegister::EvenOddPair);
      dependencies->addPostCondition(trgtRegPair->getHighOrder(),
                                     TR::RealRegister::LegalEvenOfPair);
      dependencies->addPostCondition(trgtRegPair->getLowOrder(),
      ...
      // put value in low order because the MLGR instruction will look for the
      // multiplicand in register R1+1 (the odd in the pair)
      cursor = generateRRInstruction(cg, TR::InstOpCode::LGR, node,
                                     trgtRegPair->getLowOrder(),
                                     trgtRegPair->getHighOrder());
      ...
      // do multiplication
      cursor = generateRRInstruction(cg, TR::InstOpCode::MLGR, node,
                                     trgtRegPair, src2RegPair->getHighOrder());
    ...
    TR::RegisterPair *tempRegisterPair =
    ...
    TR::RegisterPair *src2RegPair =
        (TR::RegisterPair *)cg->gprClobberEvaluate(secondChild);

    TR::RegisterDependencyConditions *dependencies =
        new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 9, cg);
    dependencies->addPostCondition(tempRegisterPair,
                                   TR::RealRegister::EvenOddPair);
    dependencies->addPostCondition(highRegister,
                                   TR::RealRegister::LegalEvenOfPair);
    dependencies->addPostCondition(lowRegister,
                                   TR::RealRegister::LegalOddOfPair);
    ...
    generateRRInstruction(cg, TR::InstOpCode::LR, node, tempRegister,
                          trgtRegPair->getHighOrder());
    generateRRInstruction(cg, TR::InstOpCode::LTR, node,
                          tempRegisterPair->getLowOrder(),
                          trgtRegPair->getLowOrder());

    // Prod = Al*Bl
    generateRRInstruction(cg, TR::InstOpCode::MR, node, trgtRegPair,
                          src2RegPair->getLowOrder());

    cursor = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNLRC, node, lmul1);
    cursor->setStartInternalControlFlow();
    ...
    generateRIInstruction(cg, TR::InstOpCode::CHI, node,
                          src2RegPair->getLowOrder(), 0);
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BNMRC, node, lmul2);

    generateRRInstruction(cg, TR::InstOpCode::ALR, node,
                          trgtRegPair->getHighOrder(),
                          tempRegisterPair->getLowOrder());

    generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, lmul2,
                                 dependencies);
    lmul2->setEndInternalControlFlow();

    if (!(secondChildIsConstant && 0 == secondChild->getLongIntHigh()) &&
        !secondHighZero) {
      // If second child is constant and has 0's for high order bits,
      // We know for sure that Ah * Bl is 0.
      // Ah*Bl
      generateRRInstruction(cg, TR::InstOpCode::MR, node, tempRegisterPair,
                            src2RegPair->getHighOrder());
    ...
    // Al*Bh
    generateRRInstruction(cg, TR::InstOpCode::MR, node, src2RegPair,
                          tempRegister);
...
/**
 * dualMulHelper32on64 - 32bit version of dual multiply Helper for 64bit
 * registers
 */
TR::Register *dualMulHelper32on64(TR::Node *node, TR::Node *lmulNode,
                                  TR::Node *lumulhNode, TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  bool needsUnsignedHighMulOnly = (lmulNode == NULL);
  // Both parts of multiplication required if !needsUnsignedHighMulOnly, can use
  // 64bit MLGR to do it.
  // but need to extend 32bit registers to 64bit
  //
  // First combine 32bit reg pairs into a 64bit single reg for both multiplier
  // and multiplicant, do the multiply
  // then split both the high 64bits and low 64bits of the 128 bit answer into
  // 32bit reg pairs.
  //
  // ignore whether children are constant or zero, which may be suboptimal
  TR::Node *firstChild = lumulhNode->getFirstChild();
  ...
  TR::Instruction *cursor = NULL;
  TR::RegisterPair *lmulTargetRegister =
      (TR::RegisterPair *)cg->gprClobberEvaluate(firstChild);
  ...
  // TR::RegisterDependencyConditions * dependencies = new (cg->trHeapMemory())
  // TR::RegisterDependencyConditions(0, 6, cg);
  TR::RegisterDependencyConditions *dependencies =
      new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 3, cg);
  bool highWordRADisabled =
      !cg->supportsHighWordFacility() || comp->getOption(TR_DisableHighWordRA);
  // doesn't seem to need these conditions
  // dependencies->addPostCondition(lumulhTargetRegister,
  // TR::RealRegister::EvenOddPair);
  // dependencies->addPostCondition(lumulhTargetRegister->getHighOrder(),
  // TR::RealRegister::LegalEvenOfPair);
  // dependencies->addPostCondition(lumulhTargetRegister->getLowOrder(),
  // TR::RealRegister::LegalOddOfPair);

  dependencies->addPostCondition(lmulTargetRegister,
                                 TR::RealRegister::EvenOddPair);
  dependencies->addPostCondition(lmulTargetRegister->getHighOrder(),
                                 TR::RealRegister::LegalEvenOfPair);
  dependencies->addPostCondition(lmulTargetRegister->getLowOrder(),
                                 TR::RealRegister::LegalOddOfPair);

  // shift all 64 bits of multiplicand into lmulTargetRegister->getHighOrder()
  cursor = generateRSInstruction(cg, TR::InstOpCode::SLLG, node,
                                 lmulTargetRegister->getHighOrder(),
                                 lmulTargetRegister->getHighOrder(), 32);

  cursor = generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                 lmulTargetRegister->getHighOrder(),
                                 lmulTargetRegister->getLowOrder());
  ...
  cursor = generateRSInstruction(cg, TR::InstOpCode::SLLG, node,
                                 lumulhTargetRegister->getHighOrder(),
                                 lumulhTargetRegister->getHighOrder(), 32);

  cursor = generateRRInstruction(cg, TR::InstOpCode::LR, node,
                                 lumulhTargetRegister->getHighOrder(),
                                 lumulhTargetRegister->getLowOrder());

  // do multiplication
  cursor =
      generateRRInstruction(cg, TR::InstOpCode::MLGR, node, lmulTargetRegister,
                            lumulhTargetRegister->getHighOrder());

  // cannot do dependency here, because otherwise it will assume it is only a
  // 32bit register, whereas it is 64bit

  // put high part of product into the lumulhTargetRegister, highOrder and
  // lowOrder.
  cursor = generateRRInstruction(cg, TR::InstOpCode::LGR, node,
                                 lumulhTargetRegister->getHighOrder(),
                                 lmulTargetRegister->getHighOrder());
  ...
  // shift down the high 32bits of the high 64bits of the product for the
  // highOrder in lumulhTargetRegister
  // but leave the low 32bits in lowOrder alone (upper 32bits will be ignored)
  cursor = generateRSInstruction(cg, TR::InstOpCode::SRLG, node,
                                 lumulhTargetRegister->getHighOrder(),
                                 lumulhTargetRegister->getHighOrder(), 32);

  // make sure low part of product is in the lumulhTargetRegister, both its
  // highOrder and lowOrder.
  cursor = generateRRInstruction(cg, TR::InstOpCode::LGR, node,
                                 lmulTargetRegister->getHighOrder(),
                                 lmulTargetRegister->getLowOrder());

  // shift down the high 32bits of the low 64bits of the product for the
  // highOrder in lmulTargetRegister
  // but leave the low 32 bits in lowOrder alone (upper 32bits will be ignored)
  cursor = generateRSInstruction(cg, TR::InstOpCode::SRLG, node,
                                 lmulTargetRegister->getHighOrder(),
                                 lmulTargetRegister->getHighOrder(), 32);

  // have to do dependancy here, because need the 64bit context (virtual
...
/**
 * dualMulHelper32on32 - 32bit version of dual multiply Helper on 32 bit
 * platforms
 */
TR::Register *dualMulHelper32on32(TR::Node *node, TR::Node *lmulNode,
                                  TR::Node *lumulhNode, TR::CodeGenerator *cg) {
  // TR_ASSERTC( "Should never reach here,comp, 0, if 64bit registers always
  // available on 32bit.");
  // traceMsg(comp, "Should never reach here, if 64bit registers always
  // available on 32bit.\n");
  ...
  //                and an even-odd register pair used as accumulator for
  //                multiply: AC0:AC1
  // entry:
  //   RR5:RR1 = a = evaluate(firstChild)
  //   RR3:RR4 = b = evaluate(secondChild)
  //   RR1,RR3,RR4 are overwritten with result; AC1, AC0 are overwritten. RR5 is
  //   preserved.
  // exit
  //   RR4:RR3:RR2:RR1 = r = a * b
  // for simplicity will act as though RR5 is not preserved, and use
  // clobberEvaluate
  ...
  //  16  ALR    RR2, AC1      // RR2 = (al * bl)h + (al * bh)l              +
  //  (ah * bl)l
  //  17  ALCR   RR3, AC0      // RR3 =              (al * bh)h + (ah * bh)l +
  //  (ah * bl)h + [carry]
  //  18  LHI    AC1, 0
  //  18b ALCR   RR4, AC1      // RR4 =                           (ah * bh)h
  //  + [carry]
  TR::Instruction *cursor = NULL;
  TR::RegisterPair *aRegister =
      (TR::RegisterPair *)cg->gprClobberEvaluate(firstChild);
  TR::RegisterPair *bRegister =
      (TR::RegisterPair *)cg->gprClobberEvaluate(secondChild);
  ...
  dependencies->addPostCondition(accRegPair->getHighOrder(),
                                 TR::RealRegister::LegalEvenOfPair);
  dependencies->addPostCondition(accRegPair->getLowOrder(),
                                 TR::RealRegister::LegalOddOfPair);
  ...
  //   RR4:RR3:RR2:RR1 = r = a * b
  TR::RegisterPair *lmulTargetRegister =
      cg->allocateConsecutiveRegisterPair(rr1, rr2);
  TR::RegisterPair *lumulhTargetRegister =
      cg->allocateConsecutiveRegisterPair(rr3, rr4);
  //   1  LR     AC1, RR1      // AC1  = al
  cursor = generateRRInstruction(cg, TR::InstOpCode::LR, node, ac1, rr1);
  //   2  MLR    AC0, RR4      // AC0:AC1 = al * bl
  cursor =
      generateRRInstruction(cg, TR::InstOpCode::MLR, node, accRegPair, rr4);
  ...
  //   5  MLR    AC0, RR3      // AC0:AC1 = al * bh
  cursor =
      generateRRInstruction(cg, TR::InstOpCode::MLR, node, accRegPair, rr3);
  ...
  //  10  MLR    AC0, AC0       // AC0:AC1 = ah * bh
  cursor =
      generateRRInstruction(cg, TR::InstOpCode::MLR, node, accRegPair, ac0);
  ...
  //  (ah * bl)l
  cursor = generateRRInstruction(cg, TR::InstOpCode::ALR, node, rr2, ac1);
  //  17  ALCR    RR3, AC0      // RR3 =              (al * bh)l + (ah * bh)l +
  //  (ah * bl)h + [carry]
  ...
  // ac0, ac1 and rr5 are no longer required, and neither are aRegister,
  // bRegister, accRegPair
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::dualMulHelper32(TR::Node *node,
                                                     TR::Node *lmulNode,
                                                     TR::Node *lumulhNode,
                                                     TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  if (performTransformation(
          comp, "O^O Using 64bit multiply on 32bit platform for Quad mul.\n")) {
...
TR::Register *OMR::Z::TreeEvaluator::lmulhEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit()) {
    ...
    TR_ASSERT(firstRegister->getRegisterPair(),
              "lmulh expected to have reg pair as first child\n");
    TR::Register *srcRegH = firstRegister->getHighOrder();
    TR::Register *srcRegL = firstRegister->getLowOrder();

    cursor = generateRSInstruction(cg, TR::InstOpCode::SLLG, node, srcRegH,
                                   srcRegH, 32);
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    cursor = generateS390CompareAndBranchInstruction(
        cg, TR::InstOpCode::CG, node, sourceRegister, (signed char)0,
        TR::InstOpCode::COND_BNLRC, posMulh);
    cursor->setStartInternalControlFlow();
  ...
  //
  if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit()) {
    //  LMG will have put the high 64-bits of the result in
    //  targetRegisterPair->getHighOrder()
    //  We need to split this into two 32-bit values with the high word going
    //  into
    //  the even register's low word and the low word going into the odd
    //  register's low word
    //
    cursor = generateRRInstruction(cg, TR::InstOpCode::LGR, node,
                                   targetRegisterPair->getLowOrder(),
    ...
    cursor = generateRSInstruction(cg, TR::InstOpCode::SRLG, node,
                                   targetRegisterPair->getHighOrder(),
                                   targetRegisterPair->getHighOrder(), 32);
...
TR::Register *OMR::Z::TreeEvaluator::mulhEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    cursor = generateS390ImmOp(cg, TR::InstOpCode::M, node,
                               targetRegisterPair->getLowOrder(),
                               targetRegisterPair, value);
  } else {
    TR::Register *secondRegister = cg->evaluate(secondChild);
...
TR::Register *OMR::Z::TreeEvaluator::imulEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    bool create_LA = false;
    if ((firstChild->getRegister() != NULL) && (value == 2) &&
        (node->isNonNegative() && !cg->use64BitRegsOn32Bit())) {
      create_LA = true;
    }
    ...
    } else // Unsigned Multiply Support
    {
      ...
      TR::RegisterPair *trgtRegPair = cg->allocateConsecutiveRegisterPair(
          trgtRegPairFirst, trgtRegPairSecond);
      ...
      // do multiplication
      cursor = generateRRInstruction(cg, TR::InstOpCode::MLR, node, trgtRegPair,
                                     secondRegister);
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (value == -1 && !node->getOpCode().isUnsigned()) {
      ...
      if (!firstChild->isNonNegative() && !node->getOpCode().isUnsigned()) {
        ...
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNLRC, node, doShift);
        cursor->setStartInternalControlFlow();
...
OMR::Z::TreeEvaluator::integerRolEvaluator(TR::Node *node,
  ...
  bool useRegPairs = TR::Compiler->target.is32Bit() &&
                     !cg->use64BitRegsOn32Bit() &&
                     node->getOpCodeValue() == TR::lrol;
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (rotateAmount == 0) {
      ...
      sourceRegister = cg->evaluate(firstChild);
      targetRegister =
          nodeIs64Bit ? cg->allocate64bitRegister() : cg->allocateRegister();
      // generateRSYInstruction(cg, nodeIs64Bit ? TR::InstOpCode::RLLG :
      // TR::InstOpCode::RLL, node,targetRegister,0,0);
      // generateRegImmInstruction(ROLRegImm1(nodeIs64Bit), node,
      // targetRegister, rotateAmount , cg);
      generateRSInstruction(cg, nodeIs64Bit ? TR::InstOpCode::RLLG
                                            : TR::InstOpCode::RLL,
                            node, targetRegister, sourceRegister, rotateAmount);
    }
  } else if (useRegPairs) {
    sourceRegister = cg->evaluate(firstChild);
    ...
    generateRSInstruction(cg, nodeIs64Bit ? TR::InstOpCode::RLLG
                                          : TR::InstOpCode::RLL,
                          node, targetRegister, sourceRegister, memRef);
  }
...
TR::Register *OMR::Z::TreeEvaluator::landEvaluator(TR::Node *node,
  ...
  // returns false,
  // then we don't need to put the constant in literal pool, so we'll evaluate
  // first child and
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    if (cg->use64BitRegsOn32Bit()) {
      generateS390ImmOp(cg, TR::InstOpCode::NG, node, targetRegister,
                        targetRegister, secondChild->getLongInt());
    } else {
      generateS390ImmOp(cg, TR::InstOpCode::getAndOpCode(), node,
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
      ...
      temp.genericLongAnalyser(node, TR::InstOpCode::NR, TR::InstOpCode::NR,
                               TR::InstOpCode::N, TR::InstOpCode::N,
                               TR::InstOpCode::getLoadRegOpCode());
    }
    targetRegister = node->getRegister();
  }
...
TR::Register *OMR::Z::TreeEvaluator::lorEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    if (cg->use64BitRegsOn32Bit()) {
      generateS390ImmOp(cg, TR::InstOpCode::OG, node, targetRegister,
                        targetRegister, secondChild->getLongInt());
    } else {
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
      ...
      temp.genericLongAnalyser(node, TR::InstOpCode::OR, TR::InstOpCode::OR,
                               TR::InstOpCode::O, TR::InstOpCode::O,
                               TR::InstOpCode::getLoadRegOpCode());
    }
    targetRegister = node->getRegister();
  }
...
TR::Register *OMR::Z::TreeEvaluator::lxorEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    if (cg->use64BitRegsOn32Bit()) {
      generateS390ImmOp(cg, TR::InstOpCode::XG, node, targetRegister,
                        targetRegister, secondChild->getLongInt());
    } else {
      generateS390ImmOp(cg, TR::InstOpCode::getXOROpCode(), node,
                        targetRegister, targetRegister,
                        secondChild->getLongInt());
    }
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
      ...
      temp.genericLongAnalyser(node, TR::InstOpCode::XR, TR::InstOpCode::XR,
                               TR::InstOpCode::X, TR::InstOpCode::X,
                               TR::InstOpCode::getLoadRegOpCode());
    }
    targetRegister = node->getRegister();
  }
