...
/**
 * 32bit version lconstHelper: load long integer constant (64-bit signed 2's
 * complement)
 */
TR::Register *lconstHelper(TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is32Bit() || cg->evaluateNodeInRegPair(node),
            "lconstHelper() is for 32bit only!");
...
TR::Register *OMR::Z::TreeEvaluator::sconstEvaluator(TR::Node *node,
  ...
  TR::Register *tempReg = node->setRegister(cg->allocateRegister());
  int32_t value = (node->isUnsigned() || node->isUnsignedLoad())
...
#ifdef J9_PROJECT_SPECIFIC
TR::Register *OMR::Z::TreeEvaluator::o2xEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  ...
  if (cg->traceBCDCodeGen())
    traceMsg(cg->comp(),
             "\to2xEval : %s (%p) srcReg->aggrSize = %d, destSize = %d\n",
             node->getOpCode().getName(), node, srcSize, node->getSize());

  TR::Register *targetReg = cg->evaluateAggregateToGPR(
      destSize, srcNode, srcAggrReg, NULL); // srcMR = NULL
...
TR::Register *OMR::Z::TreeEvaluator::x2oEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  ...
  TR_OpaquePseudoRegister *targetReg =
  ...
  if (hint)
    ...
    targetStorageReference =
        TR_StorageReference::createTemporaryBasedStorageReference(dstAggrSize,
                                                                  comp);
  targetReg->setStorageReference(targetStorageReference, node);

  TR::MemoryReference *destMR = generateS390MemRefFromStorageRef(
      node, targetStorageReference, cg, false); // enforceSSLimits=false

  if (cg->traceBCDCodeGen())
    traceMsg(comp, "\tx2oEval : %s (%p) srcSize = %d (srcAddrPrec %d), "
                   "destNode->aggrSize = %d\n",
             node->getOpCode().getName(), node, srcNode->getSize(),
             srcNode->getType().isAddress()
  ...
  if (storeSize > srcSize) {
    ...
    if (cg->traceBCDCodeGen())
      traceMsg(comp, "\tisWidening=true : clear upper %lld bytes, set "
                     "storeSize to %d, add %lld to destMR offset (dstAggrSize "
                     "%d > srcIntSize %d)\n",
               bytesToClear, srcSize, bytesToClear, dstAggrSize, srcSize);
    cg->genMemClear(destMR, node, bytesToClear);
    storeSize = srcSize;
    destMR = reuseS390MemoryReference(destMR, bytesToClear, node, cg,
                                      true); // enforceSSLimits=true
  }

  switch (storeSize) {
  case 1:
    generateRXInstruction(cg, TR::InstOpCode::STC, node,
                          srcIsRegPair ? srcRegLow : srcReg64, destMR);
    break;
  case 2:
    generateRXInstruction(cg, TR::InstOpCode::STH, node,
                          srcIsRegPair ? srcRegLow : srcReg64, destMR);
    break;
  case 3:
    generateRSInstruction(cg, TR::InstOpCode::STCM, node,
                          srcIsRegPair ? srcRegLow : srcReg64, (uint32_t)0x7,
                          destMR);
    break;
  case 4:
    generateRXInstruction(cg, TR::InstOpCode::ST, node,
                          srcIsRegPair ? srcRegLow : srcReg64, destMR);
    break;
  case 5:
  case 6:
  case 7:
    generateRSYInstruction(cg, srcIsRegPair ? TR::InstOpCode::STCM
                                            : TR::InstOpCode::STCMH,
                           node, srcIsRegPair ? srcRegHi : srcReg64,
                           (uint32_t)((1 << (dstAggrSize - 4)) - 1), destMR);
    generateRXInstruction(
        cg, TR::InstOpCode::ST, node, srcIsRegPair ? srcRegLow : srcReg64,
        generateS390MemoryReference(*destMR, dstAggrSize - 4, cg));
    break;
  case 8:
    if (srcIsRegPair) {
      generateRXInstruction(cg, TR::InstOpCode::ST, node, srcRegHi, destMR);
      generateRXInstruction(cg, TR::InstOpCode::ST, node, srcRegLow,
                            generateS390MemoryReference(*destMR, 4, cg));
    } else {
...
TR::Register *OMR::Z::TreeEvaluator::labsEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    ...
    TR_ASSERT(targetRegister->getRegisterPair() != NULL,
              "in 32-bit mode, a 64-bit value is not in a register pair");
    ...
    generateRRInstruction(cg, TR::InstOpCode::XR, node,
                          targetRegister->getHighOrder(), tempReg);
    generateRRInstruction(cg, TR::InstOpCode::XR, node,
                          targetRegister->getLowOrder(), tempReg);
...
TR::Register *OMR::Z::TreeEvaluator::iabsEvaluator(TR::Node *node,
  ...
  if (node->getOpCodeValue() == TR::deabs) {
    ...
    // load positive here is setting
    // the highest bit in the high-order register, so the low half can remain
    // unchanged
    if (!cg->canClobberNodesRegister(firstChild))
