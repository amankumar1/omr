...
//  We are temporarily accounting for ZOS XPLink NOP to Call Descriptor distance
//  too.
...
////////////////////////////////////////////////////////////////////////////////
// TR_S90ProcessorInfo::checkz900: returns true if it's zArchitecture, otherwise
// false.
////////////////////////////////////////////////////////////////////////////////
bool TR_S390ProcessorInfo::checkz900() {
...
////////////////////////////////////////////////////////////////////////////////
bool TR_S390ProcessorInfo::checkz990() {
  return TR::Compiler->target.cpu.getS390SupportsZ990();
}

////////////////////////////////////////////////////////////////////////////////
// TR_S390ProcessorInfo::checkz9: returns true if it's Golden Eagle, otherwise
// false.
////////////////////////////////////////////////////////////////////////////////
bool TR_S390ProcessorInfo::checkz9() {
...
////////////////////////////////////////////////////////////////////////////////
bool TR_S390ProcessorInfo::checkz10() {
  return TR::Compiler->target.cpu.getS390SupportsZ10();
}

////////////////////////////////////////////////////////////////////////////////
// TR_S90ProcessorInfo::checkz196: returns true if it's zGryphon, otherwise
// false.
////////////////////////////////////////////////////////////////////////////////
bool TR_S390ProcessorInfo::checkz196() {
  return TR::Compiler->target.cpu.getS390SupportsZ196();
}

////////////////////////////////////////////////////////////////////////////////
// TR_S90ProcessorInfo::checkzEC12: returns true if it's zHelix, otherwise
// false.
////////////////////////////////////////////////////////////////////////////////
bool TR_S390ProcessorInfo::checkzEC12() {
...
////////////////////////////////////////////////////////////////////////////////
bool TR_S390ProcessorInfo::checkZ13() {
  return TR::Compiler->target.cpu.getS390SupportsZ13();
}

////////////////////////////////////////////////////////////////////////////////
//  TR_S90ProcessorInfo::getProcessor: returns the TR_s390gpX for current
//  processorInfo
////////////////////////////////////////////////////////////////////////////////
TR_Processor TR_S390ProcessorInfo::getProcessor() {
...
OMR::Z::CodeGenerator::CodeGenerator()
    : OMR::CodeGenerator(), _processorInfo(), _extentOfLitPool(-1),
      _recompPatchInsnList(
          getTypedAllocator<TR::Instruction *>(self()->comp()->allocator())),
      _targetList(getTypedAllocator<TR::S390TargetAddressSnippet *>(
          self()->comp()->allocator())),
      ...
      _outOfLineCodeSectionList(
          getTypedAllocator<TR_S390OutOfLineCodeSection *>(
              self()->comp()->allocator())),
      _returnTypeInfoInstruction(NULL), _ARSaveAreaForTM(NULL),
      _notPrintLabelHashTab(NULL), _interfaceSnippetToPICsListHashTab(NULL),
      _currentCheckNode(NULL),
      _internalControlFlowRegisters(
          getTypedAllocator<TR::Register *>(self()->comp()->allocator())),
      _nodesToBeEvaluatedInRegPairs(self()->comp()->allocator()),
      _killedRestrictedRegisters(NULL), _currentlyRestrictedRegisters(NULL),
      _nodeAddressOfCachedStatic(NULL), _ccInstruction(NULL),
      _previouslyAssignedTo(self()->comp()->allocator("LocalRA")),
      _bucketPlusIndexRegisters(self()->comp()->allocator()),
      _currentDEPEND(NULL), _outgoingArgLevelDuringTreeEvaluation(0) {
  ...
  if (enableBranchPreload ||
      (!disableBranchPreload && comp->isOptServer() &&
       _processorInfo.supportsArch(TR_S390ProcessorInfo::TR_zEC12)))
  ...
  // Check if platform supports highword facility - both hardware and OS
  // combination.
  if (!comp->getOption(TR_MimicInterpreterFrameShape)) {
    // On 31-bit zlinux we need to check RAS support
    if (!TR::Compiler->target.isLinux() || TR::Compiler->target.is64Bit() ||
        TR::Compiler->target.cpu.getS390SupportsHPRDebug())
  ...
  if (!comp->getOptions()->getOption(TR_NoResumableTrapHandler) &&
  ...
  if (_processorInfo.supportsArch(TR_S390ProcessorInfo::TR_zEC12)) {
    ...
    if (TR::Compiler->target.cpu.getS390SupportsTM() &&
  ...
  if (_processorInfo.supportsArch(TR_S390ProcessorInfo::TR_z13) &&
  ...
  if (!(self()->getS390ProcessorInfo()->supportsArch(
            TR_S390ProcessorInfo::TR_z13) &&
  ...
  if (comp->getOptions()->getRegisterAssignmentTraceOption(
    ...
    self()->setGPRegisterIterator(
        new (self()->trHeapMemory())
            TR::RegisterIterator(self()->machine(), TR_GPR));
    self()->setFPRegisterIterator(
        new (self()->trHeapMemory())
            TR::RegisterIterator(self()->machine(), TR_FPR));
    self()->setARegisterIterator(new (
        self()->trHeapMemory()) TR::RegisterIterator(self()->machine(), TR_AR));
    self()->setHPRegisterIterator(
        new (self()->trHeapMemory())
            TR::RegisterIterator(self()->machine(), TR_HPR));
    self()->setVRFRegisterIterator(
        new (self()->trHeapMemory())
            TR::RegisterIterator(self()->machine(), TR_VRF));
  }
...
OMR::Z::CodeGenerator::getGlobalHPRFromGPR(TR_GlobalRegisterNumber n) {
  ...
  TR::RealRegister *gpr =
      toRealRegister(self()->machine()->getGPRFromGlobalRegisterNumber(n));
...
OMR::Z::CodeGenerator::getGlobalGPRFromHPR(TR_GlobalRegisterNumber n) {
  ...
  TR::RealRegister *hpr =
      toRealRegister(self()->machine()->getHPRFromGlobalRegisterNumber(n));
...
void OMR::Z::CodeGenerator::setupSpecializedEpilogues() {

  if (self()->specializedEpilogues()) {
    int32_t numRegs = TR::RealRegister::NumRegisters;

    _blocksThatModifyRegister =
        (TR_BitVector **)self()->trMemory()->allocateHeapMemory(
            numRegs * sizeof(TR_BitVector *), TR_Memory::CodeGenerator);

    int32_t reg;
    TR_ASSERT(self()->comp()->getFlowGraph(),
              "TR::CodeGenerator - %s needs a flow graph\n",
              self()->comp()->signature());
    for (reg = 0; reg < numRegs; reg++)
      _blocksThatModifyRegister[reg] = new (self()->trHeapMemory())
          TR_BitVector(self()->comp()->getFlowGraph()->getNextNodeNumber(),
                       self()->trMemory(), heapAlloc, growable);
  }
}
...
bool OMR::Z::CodeGenerator::isStackBased(TR::MemoryReference *mr) {
  if (mr->getBaseRegister() &&
      mr->getBaseRegister()
          ->getGPRofArGprPair()
          ->getRealRegister()) // need to consider AR register pair as base reg
  {
    return (toRealRegister(
                mr->getBaseRegister()->getGPRofArGprPair()->getRealRegister())
                ->getRegisterNumber() ==
...
bool OMR::Z::CodeGenerator::prepareForGRA() {
  bool enableHighWordGRA = self()->supportsHighWordFacility() &&
  ...
  if (!_globalRegisterTable) {
    ...
    self()->setFirstGlobalHPR(
        self()->machine()->getFirstGlobalHPRRegisterNumber());
    self()->setLastGlobalHPR(
        self()->machine()->getLastGlobalHPRRegisterNumber());
    ...
    self()->setFirstGlobalAR(
        self()->machine()->getFirstGlobalAccessRegisterNumber());
    self()->setLastGlobalAR(
        self()->machine()->getLastGlobalAccessRegisterNumber());
    ...
    // _globalFPRsPreservedAcrossCalls
    // We call init here because getNumberOfGlobal[FG]PRs() is initialized
    // during the call to initialize() above.
    //
    if (enableHighWordGRA) {
      _globalGPRsPreservedAcrossCalls.init(NUM_S390_GPR + NUM_S390_FPR +
                                               NUM_S390_AR + NUM_S390_HPR,
                                           self()->trMemory());
      _globalFPRsPreservedAcrossCalls.init(NUM_S390_GPR + NUM_S390_FPR +
                                               NUM_S390_AR + NUM_S390_HPR,
                                           self()->trMemory());
    } else {
      _globalGPRsPreservedAcrossCalls.init(
          NUM_S390_GPR + NUM_S390_FPR + NUM_S390_AR, self()->trMemory());
      _globalFPRsPreservedAcrossCalls.init(
          NUM_S390_GPR + NUM_S390_FPR + NUM_S390_AR, self()->trMemory());
    }
    ...
    for (grn = self()->getFirstGlobalGPR(); grn <= self()->getLastGlobalGPR();
      ...
      if (globalReg != -1 &&
          self()->getS390Linkage()->getPreserved(
    ...
    if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
      ...
      for (grn = 0; grn < self()->getNumberOfGlobalRegisters(); grn++) {
        ...
        TR_ASSERT(reg != -1, "Register pressure simulator doesn't support gaps "
                             "in the global register table; reg %d must be "
                             "removed",
                  grn);
        ...
        if (linkage->getIntegerArgument(reg) ||
          ...
          if ((enableHighWordGRA) && (grn >= self()->getFirstGlobalGPR() &&
                                      grn <= self()->getLastGlobalGPR())) {
            TR_GlobalRegisterNumber grnHPR =
                self()->getFirstGlobalHPR() - self()->getFirstGlobalGPR() + grn;
...
TR_S390PrivateLinkage *OMR::Z::CodeGenerator::getS390PrivateLinkage() {
  return toS390PrivateLinkage(self()->getLinkage());
}

TR::SystemLinkage *OMR::Z::CodeGenerator::getS390SystemLinkage() {
  return toSystemLinkage(self()->getLinkage());
}
 ...
 */
bool OMR::Z::CodeGenerator::mayImmedInstructionCauseOverFlow(TR::Node *node) {
...
// Z
void OMR::Z::CodeGenerator::changeRegisterKind(TR::Register *temp,
                                               TR_RegisterKinds rk) {
  if (rk == temp->getKind())
    return;
  if (_liveRegisters[temp->getKind()] && _liveRegisters[rk])
    TR_LiveRegisters::moveRegToList(_liveRegisters[temp->getKind()],
                                    _liveRegisters[rk], temp);
  temp->setKind(rk);
}

void OMR::Z::CodeGenerator::ensure64BitRegister(TR::Register *reg) {
...
bool OMR::Z::CodeGenerator::inlinePackedLongConversion() {
  return !self()->comp()->getOptions()->getOption(
      TR_DisablePackedLongConversion);
}
...
void OMR::Z::CodeGenerator::insertPad(TR::Node *theNode,
                                      TR::Instruction *insertionPoint,
                                      int32_t padSize, bool before) {
  if (0 == padSize) {
    return;
  }
  ...
  switch (padSize) {
  case 4:
    (void)new (self()->trHeapMemory()) TR::S390NOPInstruction(
        TR::InstOpCode::NOP, 4, theNode, insertionPoint, self());
    break;
  ...
  case 6:
    (void)new (self()->trHeapMemory()) TR::S390NOPInstruction(
        TR::InstOpCode::NOP, 4, theNode, insertionPoint, self());
    (void)new (self()->trHeapMemory()) TR::S390NOPInstruction(
        TR::InstOpCode::NOP, 2, theNode, insertionPoint, self());
    break;
...
// TODO vm thread work: need to add a vm thread dependency at the top of the
// instruction strea.
// x86 does this by adding a dependency on the TR::InstOpCode::PROC instruction.
// This causes an assertion
// failure on 390 (error: node does not have block set).  One solution is to add
// the dependency
// to the first instruction that has a block set.  This can be implemented in
...
void OMR::Z::CodeGenerator::doInstructionSelection() {
  ...
  _outgoingArgLevelDuringTreeEvaluation =
      self()->getLinkage()->getNumberOfAllocatedOutgoingArgumentAreas();
...
void OMR::Z::CodeGenerator::insertInstructionPrefetches() {
  if (self()->supportsBranchPreload()) {
    if (_hottestReturn._frequency > 6) {
      ...
      // if we encountered a call, insert after the call or after the lable that
      ...
      TR::RealRegister *spReg = self()->getS390Linkage()->getS390RealRegister(
      ...
      cursor = generateRXYInstruction(self(),
...
void OMR::Z::CodeGenerator::insertInstructionPrefetchesForCalls(
  ...
  // _currentCodeCache in the compilation object is set right before Instruction
  // selection.
  // if it returns null, we have a call during optimizer stage, in which case,
  // we use
   ...
   * header or a block above
   * if found a loop, don't mix with push higher into other blocks for all
   * possible edges
  ...
  if (true) //! isInLoop || !bppMaxPath)
  {
    ...
    while (first != cursor) {
      ...
      if (real != NULL && op.getOpCodeValue() == TR::InstOpCode::DEPEND &&
        ...
        real = cursor;
        if (cursor->getNext() &&
            first != cursor->getNext()) // op.getOpCodeValue() ==
                                        // TR::InstOpCode::BPRP ||
                                        // op.getOpCodeValue() ==
                                        // TR::InstOpCode::BPP)
        {
    ...
    for (auto e = block->getPredecessors().begin();
      ...
      while (first != cursor) {
        ...
        if (real != NULL && op.getOpCodeValue() == TR::InstOpCode::DEPEND &&
            real->getOpCode().isLabel()) {
          real = cursor;
          if (cursor->getNext() && first != cursor) // op.getOpCodeValue() ==
                                                    // TR::InstOpCode::BPRP ||
                                                    // op.getOpCodeValue() ==
                                                    // TR::InstOpCode::BPP)
          {
 ...
 */
bool OMR::Z::CodeGenerator::isExtCodeBaseFreeForAssignment() {
  bool extCodeBaseIsFree = false;

  // Codegen's extCodeBase flag has ultimate veto
  // For N3+ we use a stack slot to spill the ExtCodeBase, compute the disp, use
  // it, and unspill
...
bool OMR::Z::CodeGenerator::isLitPoolFreeForAssignment() {
  ...
  if ((self()->isLiteralPoolOnDemandOn() ||
       (!self()->comp()->hasNativeCall() &&
        (self()->getFirstSnippet() == NULL))) &&
...
OMR::Z::CodeGenerator::upgradeToHPRInstruction(TR::Instruction *inst) {
  ...
  if (targetUpgradable) {
    ...
    case TR::InstOpCode::AR:
      ...
      newInst = generateRRRInstruction(self(), newOpCode, node, targetReg,
                                       targetReg, srcReg, inst->getPrev());
    ...
    case TR::InstOpCode::AFI:
      ...
      newInst = generateRILInstruction(self(), newOpCode, node, targetReg,
                                       immValue, inst->getPrev());
      targetReg->decTotalUseCount();
      break;
    case TR::InstOpCode::BRCT:
      newOpCode = TR::InstOpCode::BRCTH;
      // BRCTH has extended immediate
      newInst = generateS390BranchInstruction(
          self(), TR::InstOpCode::BRCTH, node, targetReg,
          ((TR::S390LabeledInstruction *)inst)->getLabelSymbol(),
          inst->getPrev());
      targetReg->decTotalUseCount();
      break;
    case TR::InstOpCode::CR:
      ...
      newInst = generateRRInstruction(self(), newOpCode, node, targetReg,
                                      srcReg, inst->getPrev());
      targetReg->decTotalUseCount();
      srcReg->decTotalUseCount();
      break;
    ...
    case TR::InstOpCode::CY:
      ...
      newInst = generateRXYInstruction(self(), newOpCode, node, targetReg, mr,
                                       inst->getPrev());
      targetReg->decTotalUseCount();
    ...
    case TR::InstOpCode::LR:
      ...
      newInst = generateExtendedHighWordInstruction(
          node, self(), newOpCode, targetReg, srcReg, 0, inst->getPrev());
      targetReg->decTotalUseCount();
      srcReg->decTotalUseCount();
      break;
 ...
 case TR::InstOpCode::NR:
    ...
    newInst = generateExtendedHighWordInstruction(node, self(), newOpCode,
 targetReg, srcReg, 0, inst->getPrev());
    targetReg->decTotalUseCount();
    srcReg->decTotalUseCount();
    break;
 ...
 targetReg, srcReg, 0, inst->getPrev());
  ...
  if (newInst) {
    // make sure the registers will keep on getting upgraded for other
    // instructions
    if (targetUpgradable)
    ...
    if (s390Inst->getDependencyConditions())
      s390NewInst->setDependencyConditionsNoBookKeeping(
          s390Inst->getDependencyConditions());
...
bool OMR::Z::CodeGenerator::supportsNamedVirtualRegisters() {
  return false;
} // TODO : Identitiy needs folding
...
TR_RegisterKinds OMR::Z::CodeGenerator::prepareRegistersForAssignment() {
  ...
  if (!self()->isOutOfLineColdPath() && _extentOfLitPool < 0) {
    ...
    TR_ASSERT(_extentOfLitPool < 0, "OMR::Z::CodeGenerator::"
                                    "doRegisterAssignment -- extentOfLitPool "
    ...
    if (self()->comp()->getOption(TR_Randomize)) {
      if (randomizer.randomBoolean(300) &&
          performTransformation(self()->comp(), "O^O Random Codegen - Added "
                                                "6000 dummy slots to  Literal "
                                                "Pool frame to test  Literal "
                                                "Pool displacement.\n"))
    ...
    _extentOfLitPool =
        self()->setEstimatedOffsetForConstantDataSnippets(_extentOfLitPool);
    ...
    if (self()->isExtCodeBaseFreeForAssignment()) {
      s390PrivateLinkage->unlockRegister(
          s390PrivateLinkage->getExtCodeBaseRealRegister());
    } else {
      s390PrivateLinkage->lockRegister(
          s390PrivateLinkage->getExtCodeBaseRealRegister());
    }
  ...
  // artificially increase register pressure
  // We go from the end since in most cases, we want to test functionality of
  // the first half i.e 0-15 for overlap with FPR etc
...
void OMR::Z::CodeGenerator::doRegisterAssignment(
  ...
  if (self()->supportsHighWordFacility() &&
    ...
    while (currInst) {
      TR::LabelSymbol *labelSym =
          currInst->isLabel()
              ? toS390LabelInstruction(currInst)->getLabelSymbol()
              : NULL;
      if (currInst->isEndInternalControlFlow() ||
          (labelSym && labelSym->isEndInternalControlFlow())) {
        if (currInst->getDependencyConditions()) {
          // traceMsg(comp(), "\nsetting all conds on [%p] to be 64-bit regs",
          // currInst);
  ...
  // Be stingy with allocating the blocked list (for HPR upgrades). Space
  // requirement is small, but it adds up
  self()->machine()->allocateUpgradedBlockedList(
      new (self()->comp()->trStackMemory()) TR_Stack<TR::RealRegister *>(
          self()->comp()->trMemory(), 16, true, stackAlloc));
  ...
  while (instructionCursor) {
    ...
    if (instructionCursor->getOpCodeValue() == TR::InstOpCode::FENCE &&
      ...
      for (int8_t i = TR::RealRegister::GPR0; i != TR::RealRegister::GPR15 + 1;
           i++) {
        if (self()->machine()->getRegisterFile(i)->getState() ==
            TR::RealRegister::Assigned)
          regMask |= (1 << (7 - ((i - 1) >> 1))); // bit 0 = GPR0/1, GPR0=1,
                                                  // GPR15=16. 'Or' with bit
                                                  // [(i-1)>>1]
      }
    ...
    */
      ...
      for (int32_t i = first; i <= last; i++) {
        ...
        if (realReg->getState() == TR::RealRegister::Free &&
            realReg->getHighWordRegister()->getState() ==
    ...
    if (self()->supportsHighWordFacility() &&
      ...
      TR::Instruction *newInst =
          self()->upgradeToHPRInstruction(instructionCursor);
      if (newInst) {
    ...
    if (instructionCursor->isStartInternalControlFlow()) {
      ...
      if (_internalControlFlowNestingDepth == 0)
        self()->endInternalControlFlow(
            instructionCursor); // Walking backwards so start is end
    }
      ...
      if (li->getLabelSymbol() != NULL) {
        if (li->getLabelSymbol()->isStartInternalControlFlow()) {
          TR_ASSERT(!li->isStartInternalControlFlow(),
                    "An instruction should not be both the start of internal "
                    "control flow and have a lebel that is pegged as the start "
                    "of internal control flow, because we should only count it "
                    "once");
          _internalControlFlowNestingDepth--;
          if (_internalControlFlowNestingDepth == 0)
            self()->endInternalControlFlow(
        ...
        if (li->getLabelSymbol()->isEndInternalControlFlow()) {
          TR_ASSERT(!li->isEndInternalControlFlow(),
                    "An instruction should not be both the end of internal "
                    "control flow and have a lebel that is pegged as the end "
                    "of internal control flow, because we should only count it "
                    "once");
          _internalControlFlowNestingDepth++;
          self()->startInternalControlFlow(instructionCursor);
            ...
            if (instructionCursor->getNode()->getBranchDestination() != NULL) {
              if (instructionCursor->getNode()
                ...
                if ((instructionCursor->getNode()
                  ...
                  // be located inside ICF
                  // otherwise it's potentially not a case where the branch has
                  // to be inside Internal Control Flow
                  if (instructionCursor->getNode()
                          ->getBranchDestination()
                          ->getNode()
                          ->getBlock()
  ...
  if (!self()->isOutOfLineColdPath()) {
    for (uint32_t i = TR::RealRegister::FirstGPR;
      ...
      TR_ASSERT(self()->machine()->getS390RealRegister(i)->getState() !=
                    TR::RealRegister::Assigned,
                "ASSERTION: GPR registers are still assigned at end of first "
                "RA pass! %s regNum=%s\n",
                self()->comp()->signature(),
                self()->getDebug()->getName(
                    self()->machine()->getS390RealRegister(i)));
    }
...
void OMR::Z::CodeGenerator::printStats(int32_t instCount) {
  ...
  fprintf(stderr, "REGISTER MOTION Compiled as [%10.10s] Factor [%4.4f] "
                  "Contains hot blocks with "
                  "[spills:%4.4d,xfers:%4.4d,moves:%4.4d] cold blocks with "
...
TR::Instruction *realInstructionWithLabels(TR::Instruction *inst,
                                           bool forward) {
  while (inst &&
         (inst->getKind() == TR::Instruction::IsPseudo && !inst->isAsmGen() ||
          inst->getKind() == TR::Instruction::IsNotExtended)) {
    inst = forward ? inst->getNext() : inst->getPrev();
  }
...
bool TR_S390Peephole::isBarrierToPeepHoleLookback(TR::Instruction *current) {
  if (NULL == current)
    return true;
...
bool TR_S390Peephole::AGIReduction() {
  if (comp()->getOption(TR_Randomize)) {
  ...
  TR::RealRegister *gpr0 =
      _cg->machine()->getS390RealRegister(TR::RealRegister::GPR0);
  ...
  if (_cg->getCurrentlyRestrictedRegisters() &&
      _cg->getCurrentlyRestrictedRegisters()->isSet(
          toRealRegister(lgrTargetReg)->getRegisterNumber() - 1))
    return performed;
...
///////////////////////////////////////////////////////////////////////////////
bool TR_S390Peephole::ICMReduction() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
...
bool TR_S390Peephole::LAReduction() {
  ...
  if (s390Instr->getKind() == TR::Instruction::IsRX) {
    ...
    if (mr && mr->isBucketBaseRegMemRef() && mr->getOffset() == 0 &&
        laBaseReg && laTargetReg && laBaseReg == laTargetReg &&
        laIndexReg == NULL && (symRef == NULL || symRef->getOffset() == 0) &&
      ...
      if (performTransformation(
              comp(), "O^O S390 PEEPHOLE: Removing redundant LA (0x%p) with "
                      "matching target and base registers (%s)\n",
...
bool TR_S390Peephole::duplicateNILHReduction() {
        ...
        if (cast_na->isImm()) {
          if (cast_na->getSourceImmediate() == cast_nb->getSourceImmediate()) {
            if (cast_na->matchesTargetRegister(
              ...
              if (performTransformation(comp(), "O^O S390 PEEPHOLE: deleting "
                                                "duplicate NILH from pair %p "
                                                "%p*\n",
                                        _cursor, _cursor->getNext())) {
...
bool TR_S390Peephole::clearsHighBitOfAddressInReg(TR::Instruction *inst,
...
bool TR_S390Peephole::unnecessaryNILHReduction() { return false; }

bool TR_S390Peephole::NILHReduction() {
...
bool TR_S390Peephole::branchReduction() { return false; }

/**
 * Peek ahead in instruction stream to see if we find register being used
 * in a memref
 */
bool TR_S390Peephole::seekRegInFutureMemRef(int32_t maxWindowSize,
                                            TR::Register *targetReg) {
  TR::Instruction *current = _cursor->getNext();
  int32_t windowSize = 0;
 ...
 * in SignalHandler.c
 */
bool TR_S390Peephole::removeMergedNullCHK() {
  if (TR::Compiler->target.isZOS()) {
    // CLT cannot do the job in zOS because in zOS it is legal to read low
    // memory address (like 0x000000, literally NULL),
    // and CLT will read the low memory address legally (in this case NULL) to
    // compare it with the other operand.
 ...
 *       LTR GPRx, GPRy
 */
bool TR_S390Peephole::LRReduction() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
  ...
  if (lgrTargetReg == lgrSourceReg &&
    ...
    if (isAGI && performTransformation(
    ...
    if ((prev->getOpCodeValue() == TR::InstOpCode::LR &&
      ...
      if ((lgrTargetReg == prevTargetReg || lgrTargetReg == prevSourceReg) &&
          performTransformation(comp(), "\nO^O S390 PEEPHOLE: Transforming "
                                        "load register into load and test "
                                        "register and removing current at %p\n",
                                _cursor)) {
    ...
    // an arithmetic logical operation, ie. Add/Subtract Logical
    // this is also done by isActiveLogicalCC, and the end of
    // generateS390CompareAndBranchOpsHelper when the virtual registers match
    // but those cannot handle the case when the virtual registers are not the
    // same but we do have the same restricted register
    // which is why we are handling it here when all the register assignments
    // are done, and the redundant LR's from the
    ...
    if (prev->getOpCode().setsCC() && prev->getOpCode().setsCarryFlag() &&
      ...
      if ((branchCond == TR::InstOpCode::COND_BERC ||
           branchCond == TR::InstOpCode::COND_BNERC) &&
          performTransformation(comp(), "\nO^O S390 PEEPHOLE: Removing "
                                        "redundant Load and Test instruction "
                                        "at %p, because CC can be reused from "
 ...
 *    LLGC  Rs, Mem
 */
bool TR_S390Peephole::LLCReduction() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
 ...
 *    LGFR  Rt, Rs
 */
bool TR_S390Peephole::LGFRReduction() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
 ...
 *    ...    ...
 */
bool TR_S390Peephole::ConditionalBranchReduction(
  ...
  if (!_cg->getS390ProcessorInfo()->supportsArch(
          TR_S390ProcessorInfo::TR_z13) ||
      disabled)
    return false;
  ...
  TR::InstOpCode::S390BranchCondition cond = getBranchConditionForMask(
      0xF -
      ((getMaskForBranchCondition(branchInst->getBranchCondition()) >> 4) &
       0xF));

  if (performTransformation(comp(), "O^O S390 PEEPHOLE: Conditionalizing "
                                    "fall-through block following %p.\n",
                            currInst)) {
    ...
    while (currInst = realInstructionWithLabelsAndRET(nextInst, true)) {
      ...
      _cg->replaceInst(
          ...
          _cursor = generateRIEInstruction(
              _cg, RIInst->getOpCode().getOpCodeValue() == TR::InstOpCode::LHI
                       ? TR::InstOpCode::LOCHI
 ...
 *    CRJ  R1,R2,Lable
 */
bool TR_S390Peephole::CompareAndBranchReduction() {
  bool performed = false;
  ...
  if ((curOpCode == TR::InstOpCode::CR || curOpCode == TR::InstOpCode::CGR ||
       curOpCode == TR::InstOpCode::CGFR) &&
      (nextOpCode == TR::InstOpCode::BRC ||
       nextOpCode == TR::InstOpCode::BRCL)) {
 ...
 *    LZOpCode  Rx, Mem
 */
bool TR_S390Peephole::LoadAndMaskReduction(TR::InstOpCode::Mnemonic LZOpCode) {
  bool disabled = comp()->getOption(TR_DisableZ13) ||
  ...
  if (_cursor->getNext()->getOpCodeValue() == TR::InstOpCode::NILL) {
    ...
    if (performTransformation(
      ...
      _cg->replaceInst(loadInst,
                       _cursor = generateRXYInstruction(
 ...
 * branch to the OOL path.
 * Since Peephole optimizations come after RA, we can safely remove this
 * instruction.
 *
 * Note that for 4-byte instructions such as CVB there is an extra 2-byte NOP
 * used as padding following
 * the 4-byte instruction.
 */
bool TR_S390Peephole::DAARemoveOutlinedLabelNop(bool hasPadding) {
  ...
  if (_cursor->throwsImplicitException()) {
    ...
    if (hasPadding) {
      if (nextInst->getOpCodeValue() == TR::InstOpCode::NOP)
        ...
        TR_ASSERT(0, "S390 PEEPHOLE: DAA instruction padding was not found "
    ...
    if ((nextInst->getOpCodeValue() == TR::InstOpCode::BRC ||
      ...
      if (OOLbranchInst->getBranchCondition() == TR::InstOpCode::COND_MASK0)
        if (performTransformation(comp(), "O^O S390 PEEPHOLE: Eliminating "
                                          "redundant DAA NOP BRC/BRCL to OOL "
                                          "path %p.\n",
                                  OOLbranchInst)) {
 ...
 * hardcoded offset it will
 * instead encounter the memory reference spill code in the place where the
 * RestoreGPR7 Snippet BRCL
 ...
 * the 4-byte instruction.
 */
bool TR_S390Peephole::DAAHandleMemoryReferenceSpill(bool hasPadding) {
  // Make sure the current instruction is a DAA intrinsic instruction
  if (_cursor->throwsImplicitException()) {
    TR::Instruction *currInst = _cursor;
    TR::Instruction *nextInst = _cursor->getNext();
    ...
    if (nextInst->getOpCodeValue() != TR::InstOpCode::BRC &&
        nextInst->getOpCodeValue() != TR::InstOpCode::BRCL) {
      if (performTransformation(comp(), "O^O S390 PEEPHOLE: Eliminating DAA "
                                        "intrinsic due to a memory reference "
                                        "spill near instruction %p.\n",
                                currInst)) {
        currInst = _cursor;
        nextInst = _cursor->getNext();
 ...
 *         false otherwise
 */
bool TR_S390Peephole::revertTo32BitShift() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
            comp(),
  ...
  if ((oldOpCode == TR::InstOpCode::SLLG ||
      ...
      !(instr->getNode()->getOpCodeValue() == TR::ishl ||
        instr->getNode()->getOpCodeValue() == TR::iushl)) {
    return false;
  }
  ...
  if (performTransformation(comp(), "O^O S390 PEEPHOLE: Reverting int shift at "
                                    "%p from SLLG/SLAG/S[LR][LA]K to "
                                    "SLL/SLA/SRL/SRA.\n",
                            instr)) {
    reverted = true;
    TR::InstOpCode::Mnemonic newOpCode = TR::InstOpCode::BAD;
    ...
    if (comp()->getOption(TR_TraceCG))
      traceMsg(comp(), "\nTransforming int shift from SLLG/SLAG/S[LR][LA]K "
                       "(%p) to SLL/SLA/SRL/SRA %p.\n",
               instr, newInstr);
  }
  return reverted;
}
 ...
 * runtime performance.
 */
bool TR_S390Peephole::inlineEXtargetHelper(TR::Instruction *inst,
  ...
  if (performTransformation(
    ...
    // register which is then killed
    // immediately after its use in the EX instruction. Since we are
    // transforming the EX to an EXRL, this
    // load is no longer needed. We search backwards for this load of the
    // literal pool and remove it.
...
bool TR_S390Peephole::inlineEXtarget() {
  ...
  if (!comp()->getOption(TR_DisableForcedEXInlining) &&
      performTransformation(comp(), "O^O S390 PEEPHOLE: Inserting "
                                    "unconditional branch to hide EX target "
                                    "instr=[%p]\n",
                            _cursor)) {
 ...
 * AHIK    GPR6,GPR0, -1
 */
bool TR_S390Peephole::attemptZ7distinctOperants() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(comp(), "O^O Random Codegen  - Disable "
  ...
  while ((current != NULL) && !current->isLabel() && !current->isCall() &&
    ...
    if (!comp()->getOption(TR_DisableHighWordRA) &&
        instr->getOpCodeValue() == TR::InstOpCode::LGR) {
      // handles this case:
      // LGR     GPR2,GPR9        ; LR=Clobber_eval
      // LFH     HPR9,#366#SPILL8 Auto[<spill temp 0x84571FDB0>] ?+0(GPR5) ;
      // Load Spill
      // AGHI    GPR2,16
      //
      // cannot transform this into:
      // LFH     HPR9,#366#SPILL8 Auto[<spill temp 0x84571FDB0>] 96(GPR5) ; Load
      // Spill
      // AGHIK   GPR2,GPR9,16,

      TR::RealRegister *lgrSourceHighWordRegister =
          toRealRegister(lgrSourceReg)->getHighWordRegister();
...
void TR_S390Peephole::markBlockThatModifiesRegister(TR::Instruction *cursor,
                                                    TR::Register *targetReg,
                                                    int32_t blockNum) {
  ...
  if (targetReg && !cursor->isStore() && !cursor->isCompare()) {
    if (targetReg->getRegisterPair()) {
      ...
      if (cursor->getOpCodeValue() == TR::InstOpCode::getLoadMultipleOpCode()) {
        ...
        if (numRegs > 0) {
          for (uint8_t i = highReg->getRegisterNumber() + 1; i++;
               i <= numRegs) {
            _cg->getS390Linkage()->getS390RealRegister(REGNUM(i))->setModified(
                true);
      ...
      if (specializedEpilogues) {
        _cg->markBlockThatModifiesRegister(lowReg->getRegisterNumber(),
                                           blockNum);
        _cg->markBlockThatModifiesRegister(highReg->getRegisterNumber(),
                                           blockNum);
      }
    } else {
      // some stores use targetReg as part of source
...
void TR_S390Peephole::reloadLiteralPoolRegisterForCatchBlock() {
  // When dynamic lit pool reg is disabled, we lock R6 as dedicated lit pool
  // reg.
  // This causes a failure when we come back to a catch block because the
  // register context will not be preserved.
  // Hence, we can not assume that R6 will still contain the lit pool register
  // and hence need to reload it.
    ...
    if ((_cg->getLinkage())->setupLiteralPoolRegister(firstSnippet) > 0) {
      ...
      TR::S390RILInstruction *inst =
          (TR::S390RILInstruction *)generateRILInstruction(
              ...
              _cg->getLitPoolRealRegister(), 0xBABE, _cursor);
...
void TR_S390Peephole::setupSpecializedEpilogues() {
  _cg->setSpecializedEpilogues(
      comp()->getOption(TR_EnableSpecializedEpilogues) &&
...
bool TR_S390Peephole::DeadStoreToSpillReduction() { return false; }

bool TR_S390Peephole::tryMoveImmediate() { return false; }

/** \details
 *     This transformation may not always be possible because the LHI
 * instruction does not modify the condition
 *     code while the XR instruction does. We must be pessimistic in our
 * algorithm and carry out the transformation
 *     if and only if there exists an instruction B that sets the condition code
 * between the LHI instruction A and
 *     some instruction C that reads the condition code.
 *
 *     That is, we are trying to find instruction that comes after the LHI in
 * the execution order that will clobber
...
void TR_S390Peephole::perform() {
  ...
  if (_cg->getCurrentlyRestrictedRegisters())
    _cg->getCurrentlyRestrictedRegisters()->empty();
  while (_cursor != NULL) {
    ...
    if (specializedEpilogues) {
      ...
      if (comp()->getOption(TR_EnableTailCallOpt) && !catchBlock &&
    ...
    case TR::InstOpCode::LOCK: {
      int32_t regNum =
          ((TR::S390PseudoInstruction *)_cursor)->getLockedRegisterNumber();
      if (_cg->getCurrentlyRestrictedRegisters())
        _cg->getCurrentlyRestrictedRegisters()->set(regNum);
      break;
    }
    case TR::InstOpCode::UNLOCK: {
      int32_t regNum =
          ((TR::S390PseudoInstruction *)_cursor)->getLockedRegisterNumber();
      if (_cg->getCurrentlyRestrictedRegisters())
        _cg->getCurrentlyRestrictedRegisters()->reset(regNum);
      break;
    }
    ...
    case TR::InstOpCode::LHI: {
      ...
      // which we cannot determine whether this transformation
      // is functionally valid or not. The issue resides in the various runtime
      // patching sequences using the LHI instruction as a
      // runtime patch point for an offset. One concrete example can be found in
      // the virtual dispatch sequence for unresolved calls
      // on 31-bit platforms where an LHI instruction is used and is patched at
      // runtime.
      //
      // TODO (Issue #255): To enable this optimization we need to implement an
      // API which marks instructions that will be patched at
      // runtime and prevent ourselves from modifying such instructions in any
      // way.
      //
      // ReduceLHIToXR();
    } break;
...
void OMR::Z::CodeGenerator::replaceInst(TR::Instruction *old,
  ...
  // it is, then
  // change pointers that go to old to point to curr, and curr must point to
  // what
...
bool OMR::Z::CodeGenerator::needsVMThreadDependency() {
  if (self()->comp()->getOption(TR_Enable390FreeVMThreadReg))
    return true;
  return false;
}

void OMR::Z::CodeGenerator::doPeephole() {
  ...
  if (_stackOfMemoryReferencesCreatedDuringEvaluation.contains(mr)) {
    for (size_t i = 0;
      ...
      if (_stackOfMemoryReferencesCreatedDuringEvaluation.element(i) == mr) {
        ...
        // is called from
        // an evaluator so that a memory reference's registers can ostensibly
        // "be killed" earlier than
        // they would be by this function.
        //
        // Any stack entries pushed after mr should not exist earlier in the
        // stack because memory references
        // should not be allowed to escape their evaluators - reuse is not
        // possible. This means that
        // there should be no way for remove(i) to delete something before the
        // top of stack before evaluation mark.
...
void OMR::Z::CodeGenerator::StopUsingEscapedMemRefsRegisters(
  ...
  while (_stackOfMemoryReferencesCreatedDuringEvaluation.topIndex() >
    ...
    // _indexRegister, this
    // this should be safe to call / it should be safe to "stop using" a
...
bool OMR::Z::CodeGenerator::supportsMergingOfHCRGuards() {
...
void OMR::Z::CodeGenerator::markBlockThatModifiesRegister(
    TR::RealRegister::RegNum reg, int32_t blockNum) {
...
////////////////////////////////////////////////////////////////////////////////
bool OMR::Z::CodeGenerator::restoreRegister(TR::RealRegister::RegNum reg,
                                            int32_t blockNumber) {
  bool test1 = _reachingBlocks->_blockAnalysisInfo[blockNumber]->intersects(
      *_blocksThatModifyRegister[reg]);
...
void OMR::Z::CodeGenerator::performReachingBlocks() {
  _reachingBlocks = new (self()->comp()->allocator()) TR_ReachingBlocks(
      self()->comp(), (TR::Optimizer *)self()->comp()->getOptimizer());
...
bool OMR::Z::CodeGenerator::getSupportsEncodeUtf16BigWithSurrogateTest() {
  return (
      !self()->comp()->getOptions()->getOption(TR_DisableUTF16BEEncoder) ||
...
void OMR::Z::CodeGenerator::doBinaryEncoding() {
  ...
  while (data.cursorInstruction) {
            ...
            if (s390Inst->getKind() == TR::Instruction::IsLabel) {
              if (self()->comp()->getOption(TR_TraceLabelTargetNOPs))
                traceMsg(
                    ...
                    s390Inst, self()->comp()->getDebug()->getOpCodeName(
                                  &s390Inst->getOpCode()));
    ...
    if (data.cursorInstruction->isLastWarmInstruction() &&
        !self()->comp()->getOption(TR_AOT) &&
        self()->comp()->getOption(TR_EnableTieredCodeCache)) {
      // Estimate Warm Snippets.
      data.estimate =
          self()->setEstimatedLocationsForSnippetLabels(data.estimate, true);
  ...
  if (warmEstimate) {
    ...
    self()->setEstimatedColdLength(data.estimate - warmEstimate -
                                   MIN_DISTANCE_BETWEEN_WARM_AND_COLD_CODE);
  } else {
  ...
  uint8_t *temp =
      self()->allocateCodeMemory(self()->getEstimatedWarmLength(),
                                 self()->getEstimatedColdLength(), &coldCode);

  if (!self()->allowSplitWarmAndColdBlocks()) {
    // Only java split warm and cold blocks
    TR_ASSERT(
        self()->getEstimatedWarmLength() == 0,
        "Warm estimate should be zero if we don't split warm and cold blocks");
  ...
  if (!disableAlignJITEP && !self()->comp()->compileRelocatableCode() &&
      self()->allowSplitWarmAndColdBlocks()) {
    int32_t alignedBase = 256 - self()->getPreprologueOffset();
    int32_t padBase = (256 + alignedBase - ((intptrj_t)temp) % 256) % 256;

    // Threshold determines the maximum number of bytes to align.  If the JIT EP
    // is already close
    // to the beginning of the cache line (i.e. pad bytes is big), then we might
    // not benefit from
    // aligning JIT EP to the true cache boundary.  By default, we align only if
    // padByte exceed 192.
  ...
  if (doBinaryEncoding) {
    TR_HashTab *branchHashTable = new (self()->trStackMemory())
        TR_HashTab(self()->comp()->trMemory(), stackAlloc, 60, true);
    TR_HashTab *labelHashTable = new (self()->trStackMemory())
        TR_HashTab(self()->comp()->trMemory(), stackAlloc, 60, true);
    TR_HashTab *notPrintLabelHashTable = new (self()->trStackMemory())
        TR_HashTab(self()->comp()->trMemory(), stackAlloc, 60, true);
    ...
    while (data.cursorInstruction) {
      ...
      if (self()->comp()->cg()->isBranchInstruction(data.cursorInstruction)) {
        TR::LabelSymbol *branchLabelSymbol =
            ((TR::S390BranchInstruction *)data.cursorInstruction)
                ->getLabelSymbol();
        if (data.cursorInstruction->getKind() == TR::Instruction::IsRIE &&
            (toS390RIEInstruction(data.cursorInstruction)->getRieForm() ==
                 TR::S390RIEInstruction::RIE_RR ||
             toS390RIEInstruction(data.cursorInstruction)->getRieForm() ==
                 TR::S390RIEInstruction::RIE_RI8)) {
          branchLabelSymbol = toS390RIEInstruction(data.cursorInstruction)
                                  ->getBranchDestinationLabel();
        }
        if (branchLabelSymbol) {
          TR_HashId hashIndex = 0;
          branchHashTable->add((void *)branchLabelSymbol, hashIndex,
                               (void *)branchLabelSymbol);
        }
      } else if (data.cursorInstruction->getKind() ==
                 TR::Instruction::IsRIL) // e.g. LARL/EXRL
      {
        if (((TR::S390RILInstruction *)data.cursorInstruction)
                ->getTargetLabel() != NULL) {
          TR::LabelSymbol *targetLabel =
              ((TR::S390RILInstruction *)data.cursorInstruction)
                  ->getTargetLabel();
          TR_HashId hashIndex = 0;
          branchHashTable->add((void *)targetLabel, hashIndex,
                               (void *)targetLabel);
        }
        ...
        TR::LabelSymbol *labelSymbol =
            ((TR::S390BranchInstruction *)data.cursorInstruction)
                ->getLabelSymbol();
        TR_HashId hashIndex = 0;
        labelHashTable->add((void *)labelSymbol, hashIndex,
                            (void *)labelSymbol);
      }
      ...
      TR_ASSERT(data.cursorInstruction->getEstimatedBinaryLength() >=
                ...
                data.cursorInstruction,
                self()->getDebug()
                    ? self()->getDebug()->getOpCodeName(
                          &data.cursorInstruction->getOpCode())
                    : "(unknown)",
      ...
      if (data.cursorInstruction == data.preProcInstruction) {
        ...
        if (!self()->comp()->getOptions()->getOption(
                TR_DisableGuardedCountingRecompilations) &&
      ...
      // If this is the last warm instruction, save info about the warm code
      // range
      ...
      if (data.cursorInstruction->isLastWarmInstruction() &&
        ...
        //
        self()->addAccumulatedInstructionLengthError(
            self()->getWarmCodeEnd() - coldCode +
            MIN_DISTANCE_BETWEEN_WARM_AND_COLD_CODE);
      }
      ...
      if (isPrivateLinkage && data.cursorInstruction == data.jitTojitStart) {
        ...
        // first word is the code size of intrepeter-to-jit glue in bytes and
        // the second word is the return info
    ...
    for (TR::LabelSymbol *labelSymbol = (TR::LabelSymbol *)lit.getFirst();
         labelSymbol; labelSymbol = (TR::LabelSymbol *)lit.getNext()) {
      TR_HashId hashIndex = 0;
      if (!(branchHashTable->locate((void *)labelSymbol, hashIndex)))
        notPrintLabelHashTable->add((void *)labelSymbol, hashIndex,
                                    (void *)labelSymbol);
    }

    self()->setLabelHashTable(notPrintLabelHashTable);
  } else { // sets values for warmCodeEnd and coldCodeStart to avoid failing
           // later assertions
 ...
 * hints
 * for the register allocator.  This does not guarantee that the pair will be
 * consecutive.
 * You must use a dependency on the instruction that requires the pair if the
 * instruction
...
TR::RegisterPair *OMR::Z::CodeGenerator::allocateConsecutiveRegisterPair(
  ...
  if (lowRegister->getKind() == TR_GPR64 &&
      highRegister->getKind() == TR_GPR64) {
...
TR::RegisterPair *
OMR::Z::CodeGenerator::allocateArGprPair(TR::Register *lowRegister,
                                         TR::Register *highRegister) {
  TR::RegisterPair *regPair = NULL;
  TR_ASSERT((highRegister->getKind() == TR_GPR64 ||
             highRegister->getKind() == TR_GPR) &&
                lowRegister->getKind() == TR_AR,
            "expecting AR<->GPR pair\n");
  ...
  if (node) {
    ...
    if (node->getOpaquePseudoRegister()) {
      ...
      if (self()->traceBCDCodeGen())
        traceMsg(
            self()->comp(), "\tprocessUnusedNodeDuringEvaluation : bcd/aggr "
                            "const/ixload %s (%p) reg %s - handle extra ref to "
    ...
    // and constants
    // the node must be evaluated so any loads that could be killed are
    // privatised to a register
    if (node->safeToDoRecursiveDecrement()) {
      ...
      if (node->getOpaquePseudoRegister() &&
        ...
        if (self()->traceBCDCodeGen())
          traceMsg(
              self()->comp(), "\tprocessUnusedNodeDuringEvaluation : bcd/aggr "
                              "const/ixload %s (%p) reg %s - handle extra ref "
 ...
 * need special handling in AR mode if both src and dest pair use the same GPR
 * but different AR's
 * as well as the opposite: same ARs but different GPR's:
 *
 * for example for the MVCL below, we need to create copy of AR_1600 via CPYA
 * and use different ARs for different GPRs
 *
 * MVCL    GPR_1792:AR_1600(GPR_1792:AR_1600),GPR_1760:AR_1600(GPR_1760:AR_1600)
 */
void OMR::Z::CodeGenerator::splitBaseRegisterPairsForRRMemoryInstructions(
  ...
  if (!sourceRegHigh->isArGprPair() || !targetRegHigh->isArGprPair())
    return;

  if (sourceRegHigh->getGPRofArGprPair() ==
      targetRegHigh->getGPRofArGprPair()) {
    // same GPR's and same AR's - all good
    if (sourceRegHigh->getARofArGprPair() == targetRegHigh->getARofArGprPair())
      return;
    ...
    TR::Register *tempReg =
        self()->allocateRegister(sourceRegHighGPR->getKind());

    TR::InstOpCode::Mnemonic copyOp = TR::InstOpCode::LR;

    if (sourceRegHighGPR->getKind() == TR_GPR64)
      copyOp = TR::InstOpCode::LGR;

    generateRRInstruction(self(), copyOp, node, tempReg, sourceRegHighGPR);

    TR::RegisterPair *regpair =
        self()->allocateArGprPair(sourceRegHighAR, tempReg);
    ...
    TR::Register *tempReg =
        self()->allocateRegister(sourceRegHighAR->getKind());

    generateRRInstruction(self(), TR::InstOpCode::CPYA, node, tempReg,
                          sourceRegHighAR);

    TR::RegisterPair *regpair =
        self()->allocateArGprPair(tempReg, sourceRegHighGPR);
...
OMR::Z::CodeGenerator::createDepsForRRMemoryInstructions(
    TR::Node *node, TR::RegisterPair *sourceReg, TR::RegisterPair *targetReg,
    uint8_t extraDeps) {
  uint8_t numDeps = 8 + extraDeps; // TODO: only 6 deps are created below so why
                                   // is there is always an implicit 2 extra
                                   // deps created?
...
TR_GlobalRegisterNumber OMR::Z::CodeGenerator::getLinkageGlobalRegisterNumber(
  ...
  if (self()->getS390Linkage()->isCAASpecialArgumentRegister(
          linkageRegisterIndex)) {
    result = self()->machine()->getGlobalCAARegisterNumber();
  } else if (self()->getS390Linkage()->isEnvironmentSpecialArgumentRegister(
                 linkageRegisterIndex)) {
    result = self()->machine()->getGlobalEnvironmentRegisterNumber();
  } else if (self()->getS390Linkage()->isParentDSASpecialArgumentRegister(
                 linkageRegisterIndex)) {
    ...
    //      traceMsg(TR::comp(), "lastLinkageGPR = %d linkaeRegisterIndex = %d
    //      getGlobalRegisterNumber(getS390Linkage()->getIntegerArgumentRegister(linkageRegisterIndex)-1)
    //      = %d\n",machine()->getLastLinkageGPR(),linkageRegisterIndex,
    //      self()->getGlobalRegisterNumber(getS390Linkage()->getIntegerArgumentRegister(linkageRegisterIndex)-1));
    //      traceMsg(TR::comp(),
...
int32_t OMR::Z::CodeGenerator::getMaximumNumberOfAssignableGPRs() {
  ...
  maxGPRs =
      11 +
      (self()->isLiteralPoolOnDemandOn() ? 1 : 0) // => Litpool ptr is available
      + ((self()->getExtCodeBaseRegisterIsFree() &&
          !self()->comp()->getOption(TR_DisableLongDispStackSlot))
             ? 1
             : 0) // => ExtCodeBase is free
      + (self()->needsVMThreadDependency()
             ? 1
             : 0); // VM Thread Register is available
...
OMR::Z::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node *node) {
  ...
  int32_t numRestrictedRegs =
      self()->comp()->getOptions()->getNumRestrictedGPRs();
  maxGPRs -= numRestrictedRegs;
  bool longNeeds1Reg =
      TR::Compiler->target.is64Bit() || self()->use64BitRegsOn32Bit();
  ...
  if (node->getOpCode().isSwitch()) {
    ...
    if (node->getSecondChild()->getOpCode().isLoadConst()) {
      ...
      } else // 32bit
      {
        ...
        if ((valueHigh >= MIN_IMMEDIATE_VAL &&
             valueHigh <= MAX_IMMEDIATE_VAL) &&
    ...
    if (node->getSecondChild()->getOpCode().isLoadConst()) {
      ...
      if (self()->comp()->cg()->getS390ProcessorInfo()->supportsArch(
              TR_S390ProcessorInfo::TR_zEC12) &&
...
void OMR::Z::CodeGenerator::registerSymbolSetup() {
  ...
  for (grn = 0; grn <= lastGrn; grn++) {
    TR::RealRegister *rr =
        (TR::RealRegister *)self()->machine()->getRealRegister(grn);
  ...
  // Now visit all function symbol references and add the volatile register
  // symbols as aliases
  int32_t i, n;
  ...
  for (i = symRefTab->getIndexOfFirstSymRef(); i < n; i++) {
    ...
    if (symRef && symRef->getSymbol()->isMethod()) {
      ...
      //      traceMsg(comp(),"Aliasing volatile regs with SymRef
...
bool OMR::Z::CodeGenerator::isInternalControlFlowReg(TR::Register *reg) {
  for (auto cursor = _internalControlFlowRegisters.begin();
       cursor != _internalControlFlowRegisters.end(); ++cursor) {
    if (reg == *cursor)
      return true;
  }
  return false;
...
void OMR::Z::CodeGenerator::startInternalControlFlow(TR::Instruction *instr) {
  // Visit all the post conditions of instr. All virtual registers in this post
    ...
    for (i = 0; i < n; i++) {
      ...
      if (!self()->afterRA())
        ...
        TR::RealRegister::RegNum rr =
            postConds->getRegisterDependency(i)->getRealRegister();
        if (rr > TR::RealRegister::NoReg && rr <= TR::RealRegister::MISC)
          r = mach->getS390RealRegister(rr);
      }
      if (r)
        _internalControlFlowRegisters.push_back(r);
    }
  }
  // At the outermost internal control flow bottom remember the instruction for
  // any spill insertion
...
OMR::Z::CodeGenerator::allocateClobberableRegister(TR::Register *srcRegister) {
  ...
  if (srcRegister->isArGprPair())
    return self()->allocateArGprPair(srcRegister->getARofArGprPair(),
                                     targetRegister);
  ...
  if (!self()->canClobberNodesRegister(node, 1, &data, ignoreRefCount) ||
    ...
    if ((node->getType().isInt64() || TR::Compiler->target.is64Bit()) &&
      ...
      if (self()->use64BitRegsOn32Bit()) {
        ...
        cursor = generateRRInstruction(self(), TR::InstOpCode::LGR, node,
        ...
        if (!data.canClobberLowWord()) {
          ...
          self()->stopUsingRegister(
              lowRegister); // Allocate pair will make these live again
          allocatePair = true;
        }
        if (!data.canClobberHighWord()) {
          highRegister = self()->allocateRegister();
          self()->stopUsingRegister(
              highRegister); // Allocate pair will make these live again
          allocatePair = true;
        }

        TR::RegisterPair *tempRegPair =
            allocatePair
                ? self()->allocateConsecutiveRegisterPair(lowRegister,
                                                          highRegister)
                : trgRegPair;
        if (!data.canClobberLowWord()) {
          cursor = generateRRInstruction(self(), TR::InstOpCode::LR, node,
                                         tempRegPair->getLowOrder(),
                                         trgRegPair->getLowOrder());
          if (debugObj) {
        ...
        if (!data.canClobberHighWord()) {
          cursor = generateRRInstruction(self(), TR::InstOpCode::LR, node,
                                         tempRegPair->getHighOrder(),
                                         trgRegPair->getHighOrder());
          if (debugObj) {
            debugObj->addInstructionComment(toS390RRInstruction(cursor),
                                            CLOBBER_EVAL);
          }
      ...
      cursor = generateRRInstruction(self(), loadRegOpCode, node,
                                     targetRegister->getGPRofArGprPair(),
                                     srcRegister);
    ...
    TR::Node *node, TR::MemoryReference *sourceMR) // sourceMR can be NULL
{
  ...
  if (self()->traceBCDCodeGen())
    traceMsg(self()->comp(), "\tneedsClobberEval : %s (%p), storageRef #%d "
                             "(%p) (isBCD = %s, isReadOnlyTemp = %s, "
                             "tempRefCount = %d)\n",
             node->getOpCode().getName(), node,
  ...
  if (node->getReferenceCount() > 1) {
    ...
    if (self()->traceBCDCodeGen())
      traceMsg(self()->comp(), "\t\t-->needsClobber=true due to "
                               "srcStorageReference #%d (%s) owningRegCount "
                               "%d\n",
               srcStorageReference->getReferenceNumber(),
    ...
    if (self()->traceBCDCodeGen())
      traceMsg(self()->comp(), "\t\t-->needsClobber=true due to reuse of "
                               "readOnlyTemp %s with refCount %d > 1\n",
               self()->getDebug()->getName(srcStorageReference->getSymbol()),
               srcStorageReference->getTemporaryReferenceCount());
    ...
    if (self()->traceBCDCodeGen()) {
      traceMsg(
          self()->comp(), "\t\t-->needsClobber=false, srcStorageReference ref "
                          "#%d (%s) owningRegCount=%d)\n",
          srcStorageReference->getReferenceNumber(),
  ...
  if (needsClobberDueToRefCount || needsClobberDueToRegCount ||
    ...
    if (self()->traceBCDCodeGen())
      traceMsg(self()->comp(), "ssrClobberEvaluate: node %s (%p) with srcReg "
                               "%s (srcRegSize %d, srcReg->validSymSize %d, "
                               "srcReg->liveSymSize %d) and "
    ...
    if (srcStorageReference->isReadOnlyTemporary() &&
      ...
      // been invalidated/clobbered).
      // The subsequent uses of this copied field (via copyStorageReference)
      // will adjust based on the owning registers' ignoredBytes/deadBytes
      // setting
      int32_t regIgnoredBytes = srcRegister->getRightAlignedIgnoredBytes();
      if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(), "\tremove %d ignoredBytes from "
                                 "sourceMR->_offset (%d->%d) for readOnlyTemp "
                                 "#%d (%s)\n",
    ...
    if (self()->traceBCDCodeGen() && alreadyClearedDigits) {
      traceMsg(
          ...
          srcStorageReference->getReferenceNumber());
      traceMsg(self()->comp(), "\t\t* setting surviving leftAlignedZeroDigits "
                               "%d on srcRegister %s with new copy ref #%d "
    ...
    if (srcStorageReference->getNodesToUpdateOnClobber()) {
      ...
      //       (pdcleanA), increment t1 refCount by 2 (pdaddA nodeRefCount)
      //       nop         mark t1 as readOnly (redundant), add pdcleanB
      //       (w/regB) to list (pdcleanB), increment t1 refCount by 3 (pdcleanA
      //       nodeRefCount)
      ...
      for (TR::Node *listNode = listIt.getFirst(); listNode;
        ...
        if (listReg != srcRegister) {
          if (listReg->getStorageReference() != srcStorageReference) {
            ...
            // pdstoreEvaluator
            // so skip this update (the reference counts of the address subtree
            // will be wrong if the refCount is update here.
            ...
            if (self()->comp()->cg()->traceBCDCodeGen())
              traceMsg(
                  self()->comp(), "\ty^y : skip update on reg %s node %s (%p), "
                                  "storageRef has already changed -- was #%d "
                                  "(%s), now #%d (%s)\n",
            ...
            if (self()->comp()->cg()->traceBCDCodeGen())
              traceMsg(self()->comp(), "\ty^y : update reg %s with ref #%d "
                                       "(%s) on node %s (%p) refCount %d from "
                                       "_nodesToUpdateOnClobber with copy ref "
                                       "#%d (%s) (skip if refCount == 0)\n",
                       self()->getDebug()->getName(listReg),
                       listReg->getStorageReference()->getReferenceNumber(),
                       self()->getDebug()->getName(
                           listReg->getStorageReference()->getSymbol()),
            ...
            if (listNode->getReferenceCount() > 0)
              listReg->setStorageReference(copyStorageReference,
                                           listNode); // TODO: safe to transfer
                                                      // alreadyClearedDigits in
                                                      // this case too?
          }
        }
      }
...
bool OMR::Z::CodeGenerator::useMVCLForMemcpyWithPad(TR::Node *node,
  ...
  if (maxLen != 0 && maxLen < MVCL_THRESHOLD) // 0 is an unknown maxLength so
                                              // have to be conservative and not
                                              // use MVCL
  {
    useMVCL = true;
...
bool OMR::Z::CodeGenerator::isMemcpyWithPadIfFoldable(TR::Node *node,
                                                      TR_MemCpyPadTypes type) {
  ...
  if (type == OneByte || type == TwoByte) // non-ND cases are always foldable
  {
    ...
    // In the equal case of NDmemcpyWithPad, the evaluation to MVCL can be
    // avoided. Thus CC is not established.
    // Therefore NDmemcpyWithPad is not foldable. Ref:
    // inlineMemcpyWithPadEvaluator.
    return false;
  } else if (self()->useMVCLForMemcpyWithPad(
                 node,
                 type)) // smaller (fits in MVCL) ND cases are also foldable
  {
    return true;
  } else {
    // what remains is large or unknown max length ND cases that require an
    // explicit overlap check in the evaluator
    // that make if folding not possible (leads to reg assigner bugs due to
    // complex control flow and avoiding this
...
void OMR::Z::CodeGenerator::apply12BitLabelRelativeRelocation(
  ...
  if (s != NULL && (s->getKind() == TR::Snippet::IsConstantData ||
    ...
    TR_ASSERT(estOffset == disp, "apply12BitLabelRelativeRelocation -- [%p] "
                                 "disp (%d) did not equal estimated offset "
                                 "(%d)\n",
              s, disp, estOffset);
  } else {
    TR_ASSERT(0, "apply12BitLabelRelativeRelocation -- Unexpected use of 12bit "
                 "rellocation by [%p]\n",
              label);
  }
}
 ...
 * producing the 4 byte NOP with last two bytes az
 ...
 * relocations,
 * where the relocation is 2 bytes (hardcoded value) form the start of the
 * instruction
 */
...
OMR::Z::CodeGenerator::findOrCreateConstant(TR::Node *node, void *c,
  ...
  // for now conservatively create a new literal pool data snippet per unique
  // node
...
OMR::Z::CodeGenerator::createLabelTable(TR::Node *node, int32_t size) {
  TR::S390LabelTableSnippet *labelTableSnippet = new (self()->trHeapMemory())
      TR::S390LabelTableSnippet(self(), node, size);
...
int32_t OMR::Z::CodeGenerator::setEstimatedOffsetForConstantDataSnippets(
    int32_t targetAddressSnippetSize, bool isWarm) {
  // *this    swipeable for debugging purposes
  TR::S390ConstantDataSnippet *cursor;
  bool first;
  int32_t size;
...
#define HANDLE_CONSTANT_SNIPPET(cursor, isWarmFlag, pow2Size)                  \
  ((cursor->isWarmSnippet() == isWarmFlag) &&                                  \
...
int32_t OMR::Z::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(
    int32_t estimatedSnippetStart, bool isWarm) {
  // *this    swipeable for debugging purposes
  TR::S390ConstantDataSnippet *cursor;
  bool first;
  int32_t size;
  ...
  // from end of snippet
  // Need this padding since the alignment of our estimate and our real
...
void OMR::Z::CodeGenerator::emitDataSnippets(bool isWarm) {
  // *this    swipeable for debugging purposes
  ...
  for (exp = self()->constantDataSnippetExponent(); exp > 0; exp--) {
    ...
    for (auto iterator = _constantList.begin(); iterator != _constantList.end();
      ...
      if (HANDLE_CONSTANT_SNIPPET(
              (*iterator), false,
              size)) // false matches case where !(*iterator)->isWarmSnippet()
      {
        return *iterator;
      }
    }
    ...
    for (hi = _constantHashCur.SetToFirst(); _constantHashCur.Valid();
      ...
      if (HANDLE_CONSTANT_SNIPPET(
              cursor, false,
              size)) // false matches case where !cursor->isWarmSnippet()
      {
        return cursor;
      }
    }
 ...
 * but the first constant with biggest size
 */
int32_t OMR::Z::CodeGenerator::setEstimatedOffsetForTargetAddressSnippets() {
  // *this    swipeable for debugging purposes
  int32_t estimatedOffset = 0;

  for (auto iterator = _targetList.begin(); iterator != _targetList.end();
       ++iterator) {
...
////////////////////////////////////////////////////////////////////////////////
int32_t
OMR::Z::CodeGenerator::setEstimatedLocationsForTargetAddressSnippetLabels(
    int32_t estimatedSnippetStart, bool isWarm) {
  // *this    swipeable for debugging purposes
...
void OMR::Z::CodeGenerator::emitTargetAddressSnippets(bool isWarm) {
  // *this    swipeable for debugging purposes
  uint8_t *codeOffset;
  int8_t size = 8;
  ...
  //
  self()->setBinaryBufferCursor(
      (uint8_t *)(((uintptrj_t)(self()->getBinaryBufferCursor() + size - 1) /
                   size) *
                  size));
...
OMR::Z::CodeGenerator::CreateLookupSwitchSnippet(TR::Node *node,
                                                 TR::Snippet *s) {
...
OMR::Z::CodeGenerator::CreateTargetAddressSnippet(TR::Node *node,
                                                  TR::Snippet *s) {
  TR::S390TargetAddressSnippet *targetsnippet;
  targetsnippet = new (self()->trHeapMemory())
      TR::S390TargetAddressSnippet(self(), node, s);
...
OMR::Z::CodeGenerator::CreateTargetAddressSnippet(TR::Node *node,
                                                  TR::Symbol *s) {
  TR_ASSERT(self()->supportsOnDemandLiteralPool() == false,
            "May not be here with Literal Pool On Demand enabled\n");
  TR::S390TargetAddressSnippet *targetsnippet;
  targetsnippet = new (self()->trHeapMemory())
      TR::S390TargetAddressSnippet(self(), node, s);
  _targetList.push_front(targetsnippet);
...
TR::S390TargetAddressSnippet *
OMR::Z::CodeGenerator::findOrCreateTargetAddressSnippet(TR::Node *node,
                                                        uintptrj_t addr) {
  TR_ASSERT(self()->supportsOnDemandLiteralPool() == false,
            "May not be here with Literal Pool On Demand enabled\n");
  // A simple linear search should suffice for now since the number of FP
  // constants
  // produced is typically very small.  Eventually, this should be implemented
  // as an
  // ordered list or a hash table.
  //
  for (auto iterator = _targetList.begin(); iterator != _targetList.end();
       ++iterator) {
    if ((*iterator)->getKind() == TR::Snippet::IsTargetAddress &&
        (*iterator)->getTargetAddress() == addr) {
      return *iterator;
    }
  }
  ...
  targetsnippet = new (self()->trHeapMemory())
      TR::S390TargetAddressSnippet(self(), node, addr);
  _targetList.push_front(targetsnippet);
  return targetsnippet;
}

TR::S390TargetAddressSnippet *OMR::Z::CodeGenerator::getFirstTargetAddress() {
  if (_targetList.empty())
    return NULL;
  else
...
//
bool OMR::Z::CodeGenerator::processInstruction(TR::Instruction *instr,
                                               TR_BitVector **registerUsageInfo,
                                               int32_t &blockNum,
                                               int32_t &isFence, bool traceIt) {
    ...
    if (callNode->getSymbolReference()) {
      ...
      if (symRef->getSymbol()->isMethod()) {
        ...
        if ((m->isHelper() && !m->preservesAllRegisters()) ||
            m->isVMInternalNative() || m->isJITInternalNative() || m->isJNI() ||
            m->isSystemLinkageDispatch() ||
            (m->getLinkageConvention() == TR_System))
  ...
  case TR::Instruction::IsPseudo: {
    ...
    if (s390Instr->getOpCodeValue() == TR::InstOpCode::FENCE) {
      ...
      if (fenceNode->getOpCodeValue() == TR::BBStart) {
        ...
        if (traceIt)
          traceMsg(self()->comp(),
                   "Now generating register use information for block_%d\n",
                   blockNum);
  ...
  case TR::Instruction::IsBranchOnIndex: {
    ...
    //
    int32_t tgtRegNum = ((TR::RealRegister *)s390Instr->getRegisterOperand(1))
                            ->getRegisterNumber();
    if (traceIt)
    ...
    if (regPair) {
      int32_t srcHighNum =
          toRealRegister(sourceRegister->getHighOrder())->getRegisterNumber();
      int32_t srcLowNum =
          toRealRegister(sourceRegister->getLowOrder())->getRegisterNumber();
      if (traceIt)
        traceMsg(self()->comp(),
                 "branchOnIndex instr [%p] USES register pair [%d, %d]\n",
                 s390Instr, srcHighNum, srcLowNum);
    } else {
      int32_t srcNum = toRealRegister(sourceRegister)->getRegisterNumber();
      if (traceIt)
        traceMsg(self()->comp(),
                 "branchOnIndex instr [%p] USES register [%d]\n", s390Instr,
                 srcNum);
    }
    return true;
  }
  case TR::Instruction::IsReg: {
    ...
    if (regPair) {
      int32_t tgtHighNum =
          toRealRegister(targetRegister->getHighOrder())->getRegisterNumber();
      int32_t tgtLowNum =
          toRealRegister(targetRegister->getLowOrder())->getRegisterNumber();
      if (traceIt)
  ...
  case TR::Instruction::IsRR: {
    ...
    if (((TR::S390RRInstruction *)s390Instr)->getSecondConstant() >= 0)
      ...
      TR::Register *sourceRegister =
          ((TR::S390RRInstruction *)instr)->getRegisterOperand(2);
      TR::RegisterPair *regPair = sourceRegister->getRegisterPair();
      int32_t srcRegNum = 0;
      if (regPair) {
        srcRegNum =
  ...
  case TR::Instruction::IsRRE: {
    ...
    int32_t srcRegNum =
        toRealRegister(s390Instr->getRegisterOperand(2))->getRegisterNumber();
    if (traceIt)
      traceMsg(self()->comp(), "RRE instr [%p] USES register [%d]\n", s390Instr,
               srcRegNum);
    return true;
  ...
  case TR::Instruction::IsRRF5: {
    ...
    int32_t tgtRegNum =
        toRealRegister(s->getRegisterOperand(1))->getRegisterNumber();
    if (traceIt)
    ...
    {
      int32_t srcRegNum =
          toRealRegister(s->getRegisterOperand(3))->getRegisterNumber();
      if (traceIt)
        traceMsg(self()->comp(), "RRF instr [%p] USES register [%d]\n", s,
                 srcRegNum);
    }
    ...
    int32_t srcRegNum =
        toRealRegister(s->getRegisterOperand(2))->getRegisterNumber();
    if (traceIt)
      traceMsg(self()->comp(), "RRF instr [%p] USES register [%d]\n", s,
               srcRegNum);
    return true;
  ...
  case TR::Instruction::IsRRR: {
    ...
    if (traceIt)
      traceMsg(self()->comp(), "RRR instr [%p] USES register [%d %d %d]\n", s,
               srcRegNum, srcReg2Num, tgtRegNum);
  ...
  case TR::Instruction::IsRIL: {
    if (s390Instr->getRegisterOperand(1)) {
      int32_t tgtRegNum =
          toRealRegister(s390Instr->getRegisterOperand(1))->getRegisterNumber();
      registerUsageInfo[blockNum]->set(tgtRegNum);
      if (traceIt)
  ...
  case TR::Instruction::IsRSY: {
    ...
    if (s->getSourceImmediate()) {
      if (s->getRegisterOperand(1)->getRegisterPair()) {
        int32_t tgtHighNum =
            toRealRegister(s->getRegisterOperand(1)->getHighOrder())
                ->getRegisterNumber();
        int32_t tgtLowNum =
            toRealRegister(s->getRegisterOperand(1)->getLowOrder())
                ->getRegisterNumber();
        registerUsageInfo[blockNum]->set(tgtHighNum);
        registerUsageInfo[blockNum]->set(tgtLowNum);
        if (traceIt)
      ...
      if (!s->isTargetPair() && s->getLastRegister() != NULL) {
        int32_t lastRegNum =
            toRealRegister(s->getLastRegister())->getRegisterNumber();
      ...
      int32_t firstRegNum =
          toRealRegister(s->getFirstRegister())->getRegisterNumber();
      ...
      if (mr->getBaseRegister()) {
        if (traceIt)
          traceMsg(
              self()->comp(), "RS instr [%p] USES mr baseRegister [%d]\n", s,
              ((TR::RealRegister *)mr->getBaseRegister())->getRegisterNumber());
      }
      if (mr->getIndexRegister()) {
        if (traceIt)
          traceMsg(self()->comp(), "RS instr [%p] USES mr indexRegister [%d]\n",
                   s, ((TR::RealRegister *)mr->getIndexRegister())
                          ->getRegisterNumber());
      }
    } else if (s->getOpCode().usesRegPairForTarget() &&
               s->getOpCode().usesRegPairForSource()) {
      int32_t tgtHighNum =
          toRealRegister(s->getRegisterOperand(1)->getHighOrder())
              ->getRegisterNumber();
      int32_t tgtLowNum =
      ...
      if (traceIt)
        traceMsg(self()->comp(), "RS instr [%p] USES register pair [%d %d]\n",
                 s, tgtLowNum, tgtHighNum);
      int32_t secondRegNum =
          toRealRegister(s->getSecondRegister()->getHighOrder())
              ->getRegisterNumber();
    ...
    } else if (s->getLastRegister() == NULL ||
               s->getOpCode().usesRegPairForTarget()) {
      int32_t firstRegNum =
          toRealRegister(s->getFirstRegister())->getRegisterNumber();
      registerUsageInfo[blockNum]->set(firstRegNum);
      if (traceIt)
  ...
  case TR::Instruction::IsS: {
    ...
    if (mr->getBaseRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "RSL instr [%p] USES mr baseRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getBaseRegister())->getRegisterNumber());
    }
    if (mr->getIndexRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "RSL instr [%p] USES mr indexRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getIndexRegister())->getRegisterNumber());
    }
    return true;
  }
  ...
  case TR::Instruction::IsRXY: {
    if (s390Instr->getRegisterOperand(1)->getRegisterPair()) {
      int32_t tgtHighNum =
          toRealRegister(s390Instr->getRegisterOperand(1)->getHighOrder())
              ->getRegisterNumber();
      int32_t tgtLowNum =
          toRealRegister(s390Instr->getRegisterOperand(1)->getLowOrder())
              ->getRegisterNumber();
      registerUsageInfo[blockNum]->set(tgtHighNum);
      registerUsageInfo[blockNum]->set(tgtLowNum);
      if (traceIt)
    ...
    if (mr) {
      ...
      if (mr->getIndexRegister()) {
        if (traceIt)
          traceMsg(self()->comp(),
                   "RSL instr [%p] USES mr indexRegister [%d]\n", s390Instr,
                   ((TR::RealRegister *)mr->getIndexRegister())
                       ->getRegisterNumber());
      }
    }
  ...
  case TR::Instruction::IsRXF: {
    ...
    if (mr->getBaseRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "RXF instr [%p] USES mr baseRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getBaseRegister())->getRegisterNumber());
    }
    if (mr->getIndexRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "RXF instr [%p] USES mr indexRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getIndexRegister())->getRegisterNumber());
    }
    return true;
  ...
  case TR::Instruction::IsMem: {
    ...
    if (mr->getBaseRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "Mem instr [%p] USES mr baseRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getBaseRegister())->getRegisterNumber());
    }
    if (mr->getIndexRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "Mem instr [%p] USES mr indexRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getIndexRegister())->getRegisterNumber());
    }
    return true;
  ...
  case TR::Instruction::IsSS1: {
    ...
    if (mr->getBaseRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "SS1 instr [%p] USES mr1 baseRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getBaseRegister())->getRegisterNumber());
    }
    if (mr->getIndexRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "SS1 instr [%p] USES mr1 indexRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getIndexRegister())->getRegisterNumber());
    }
    ...
    if (mr->getBaseRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "SS1 instr [%p] USES mr2 baseRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getBaseRegister())->getRegisterNumber());
    }
    if (mr->getIndexRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "SS1 instr [%p] USES mr2 indexRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getIndexRegister())->getRegisterNumber());
    }
    return true;
  ...
  case TR::Instruction::IsSSE: {
    ...
    if (mr->getBaseRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "SS2 instr [%p] USES mr baseRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getBaseRegister())->getRegisterNumber());
    }
    if (mr->getIndexRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "SS2 instr [%p] USES mr indexRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getIndexRegister())->getRegisterNumber());
    }
    ...
    if (mr) {
      if (mr->getBaseRegister()) {
        if (traceIt)
          traceMsg(
              self()->comp(), "SS2 instr [%p] USES mr2 baseRegister [%d]\n",
              s390Instr,
              ((TR::RealRegister *)mr->getBaseRegister())->getRegisterNumber());
      }
      if (mr->getIndexRegister()) {
        if (traceIt)
          traceMsg(self()->comp(),
                   "SS2 instr [%p] USES mr2 indexRegister [%d]\n", s390Instr,
                   ((TR::RealRegister *)mr->getIndexRegister())
                       ->getRegisterNumber());
      }
    }
  ...
  case TR::Instruction::IsSS4: {
    ...
    int32_t srcKeyRegNum =
        toRealRegister(s->getSourceKeyReg())->getRegisterNumber();
    registerUsageInfo[blockNum]->set(lenRegNum);
    if (traceIt)
      traceMsg(self()->comp(), "SS4 instr [%p] USES register [%d %d]\n", s,
               lenRegNum, srcKeyRegNum);
    ...
    if (mr->getBaseRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "SS4 instr [%p] USES mr baseRegister [%d]\n", s,
            ((TR::RealRegister *)mr->getBaseRegister())->getRegisterNumber());
    }
    if (mr->getIndexRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "SS4 instr [%p] USES mr indexRegister [%d]\n", s,
            ((TR::RealRegister *)mr->getIndexRegister())->getRegisterNumber());
    }
    return true;
  }
  case TR::Instruction::IsSSF: {
    ...
    if (mr->getBaseRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "SSF instr [%p] USES mr1 baseRegister [%d]\n", s,
            ((TR::RealRegister *)mr->getBaseRegister())->getRegisterNumber());
    }
    if (mr->getIndexRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "SSF instr [%p] USES mr1 indexRegister [%d]\n", s,
            ((TR::RealRegister *)mr->getIndexRegister())->getRegisterNumber());
    }
    ...
    if (mr->getBaseRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "SSF instr [%p] USES mr2 baseRegister [%d]\n", s,
            ((TR::RealRegister *)mr->getBaseRegister())->getRegisterNumber());
    }
    if (mr->getIndexRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "SSF instr [%p] USES mr2 indexRegister [%d]\n", s,
            ((TR::RealRegister *)mr->getIndexRegister())->getRegisterNumber());
    }
    return true;
  }
  ...
  case TR::Instruction::IsRIS: {
    ...
    if (mr->getBaseRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "RIS instr [%p] USES mr baseRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getBaseRegister())->getRegisterNumber());
    }
    if (mr->getIndexRegister()) {
      if (traceIt)
        traceMsg(
            self()->comp(), "RIS instr [%p] USES mr indexRegister [%d]\n",
            s390Instr,
            ((TR::RealRegister *)mr->getIndexRegister())->getRegisterNumber());
    }
    return true;
...
uint32_t OMR::Z::CodeGenerator::isPreservedRegister(int32_t regIndex) {
  // is register preserved in prologue
  //
  for (int32_t pindex = TR::RealRegister::GPR6;
       pindex <= TR::RealRegister::GPR12; pindex++) {
...
bool OMR::Z::CodeGenerator::isReturnInstruction(TR::Instruction *instr) {
  return instr->getOpCodeValue() == TR::InstOpCode::RET;
}

bool OMR::Z::CodeGenerator::isBranchInstruction(TR::Instruction *instr) {
  return instr->isBranchOp();
}

bool OMR::Z::CodeGenerator::isLabelInstruction(TR::Instruction *instr) {
  return instr->isLabel();
}

int32_t OMR::Z::CodeGenerator::isFenceInstruction(TR::Instruction *instr) {
  TR::Instruction *s390Instr = instr;
...
bool OMR::Z::CodeGenerator::isAlignmentInstruction(TR::Instruction *instr) {
  return false;
}

TR::Instruction *
OMR::Z::CodeGenerator::splitEdge(TR::Instruction *instr, bool isFallThrough,
                                 bool needsJump, TR::Instruction *newSplitLabel,
  ...
  if (isFallThrough) {
    ...
    if (instr->getKind() == TR::Instruction::IsRIE) {
      ...
      TR::S390LabeledInstruction *labelInstr =
          (TR::S390LabeledInstruction *)instr;
    ...
    //
    for (auto jmpItr = jmpInstrs->begin(); jmpItr != jmpInstrs->end();
         ++jmpItr) {
  ...
  if (!cursor)
    cursor = generateS390LabelInstruction(
        self(), TR::InstOpCode::LABEL, location->getNode(), newLabel, location);

  if (!isFallThrough && needsJump) {
    TR::Instruction *jmpLocation = cursor->getPrev();
    TR::Instruction *i = generateS390BranchInstruction(
        self(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC,
        jmpLocation->getNode(), targetLabel, jmpLocation);
...
OMR::Z::CodeGenerator::splitBlockEntry(TR::Instruction *instr) {
  ...
  return generateS390LabelInstruction(self(), TR::InstOpCode::LABEL,
                                      location->getNode(), newLabel, location);
}

int32_t
OMR::Z::CodeGenerator::computeRegisterSaveDescription(TR_BitVector *regs,
                                                      bool populateInfo) {
  uint32_t rsd = 0;
...
void OMR::Z::CodeGenerator::processIncomingParameterUsage(
    TR_BitVector **registerUsageInfo, int32_t blockNum) {
  ...
  int32_t litPoolNumber =
      self()
          ->getLinkage(bodySymbol->getLinkageConvention())
          ->setupLiteralPoolRegister(self()->getFirstSnippet());
  if (litPoolNumber > 0)
    registerUsageInfo[blockNum]->set(litPoolNumber);
}

void OMR::Z::CodeGenerator::updateSnippetMapWithRSD(TR::Instruction *instr,
                                                    int32_t rsd) {
  ...
  TR_S390OutOfLineCodeSection *oiCursor =
      self()->findOutLinedInstructionsFromLabel(targetLabel);
  if (oiCursor) {
...
bool OMR::Z::CodeGenerator::isTargetSnippetOrOutOfLine(TR::Instruction *instr,
...
void OMR::Z::CodeGenerator::buildRegisterMapForInstruction(TR_GCStackMap *map) {
  ...
  if (self()->supportsHighWordFacility() &&
      !self()->comp()->getOption(TR_DisableHighWordRA)) {
    for (int32_t i = TR::RealRegister::FirstHPR; i <= TR::RealRegister::LastHPR;
         i++) {
      TR::RealRegister *realReg =
          self()->machine()->getS390RealRegister((TR::RealRegister::RegNum)i);
      if (realReg->getHasBeenAssignedInMethod()) {
        TR::Register *virtReg = realReg->getAssignedRegister();
        if (virtReg && virtReg->containsCollectedReference() &&
            virtReg->getAssignedRegister() == NULL) {
          // 2 bits per register, '10' means HPR has collectible, '11' means
          // both HPR and GPR have collectibles
          map->setHighWordRegisterBits(
              1 << ((i - TR::RealRegister::FirstHPR) * 2 + 1));
          // traceMsg(comp(), "\nsetting HPR regmap 0x%x\n", 1 << ((i -
          // TR::RealRegister::FirstHPR)*2 + 1));
        }
      }
    }
  }
...
TR::Register *OMR::Z::CodeGenerator::copyRestrictedVirtual(
    TR::Register *virtReg, TR::Node *node, TR::Instruction **preced) {
  TR::Register *copyReg;
  TR::Instruction *i = NULL;
  if (virtReg->getKind() == TR_GPR64) {
    copyReg = self()->allocate64bitRegister();
    i = generateRRInstruction(self(), TR::InstOpCode::LGR, node, copyReg,
                              virtReg, preced ? *preced : NULL);
  } else {
    copyReg = self()->allocateRegister();
    i = generateRRInstruction(self(), TR::InstOpCode::LR, node, copyReg,
                              virtReg, preced ? *preced : NULL);
  }
  if (preced && *preced)
    *preced = i;
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::CodeGenerator::dumpDataSnippets(TR::FILE *outFile, bool isWarm) {
  // *this    swipeable for debugging purposes
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::CodeGenerator::dumpTargetAddressSnippets(TR::FILE *outFile,
                                                      bool isWarm) {
  // *this    swipeable for debugging purposes
...
bool OMR::Z::CodeGenerator::constLoadNeedsLiteralFromPool(TR::Node *node) {
  if (node->isClassUnloadingConst() || node->getType().isIntegral() ||
      node->getType().isAddress())
    return false;
  else
...
bool OMR::Z::CodeGenerator::isAddressOfStaticSymRefWithLockedReg(
    TR::SymbolReference *symRef) {
  return false;
}

bool OMR::Z::CodeGenerator::isAddressOfPrivateStaticSymRefWithLockedReg(
    TR::SymbolReference *symRef) {
  return false;
}
 ...
 * is reachable.
 * If it is reachable from the start AND the end of the code cache, then we are
 * sure it's reachable.
 * Otherwise we'll generate the slow path.
 * This works quite well for BRASL in that it will generate the good code for
 * jit-to-jit(for single code cache case) call and usually to the fe helper
 * calls
 * as well.
...
bool OMR::Z::CodeGenerator::canUseRelativeLongInstructions(int64_t value) {
  ...
  if (TR::Compiler->target.is32Bit() && !self()->use64BitRegsOn32Bit())
    return true;

  if (TR::Compiler->target.isLinux()) {
    TR_FrontEnd *fe = self()->comp()->fe();
    intptrj_t codeCacheBase, codeCacheTop;
    // _currentCodeCache in the compilation object is set right before
    ...
    if (self()->comp()->getCurrentCodeCache() == NULL) {
      ...
      codeCacheBase = (intptrj_t)fe->getCodeCacheBase(
          self()->comp()->getCurrentCodeCache());
      codeCacheTop =
          (intptrj_t)fe->getCodeCacheTop(self()->comp()->getCurrentCodeCache());
    }
 ...
 */
bool OMR::Z::CodeGenerator::bndsChkNeedsLiteralFromPool(TR::Node *node) {
...
bool OMR::Z::CodeGenerator::isActiveCompareCC(TR::InstOpCode::Mnemonic opcd,
  ...
  if (self()->hasCCInfo() && self()->hasCCCompare()) {
    ...
    if (opcd == ccInst->getOpCodeValue() && tReg == ccTgtReg &&
        sReg == ccSrcReg &&
        performTransformation(
            self()->comp(), "O^O isActiveCompareCC: RR Compare Op [%s\t %s, "
                            "%s] can reuse CC from ccInstr [%p]\n",
            self()->getDebug()->getOpCodeName(&ccInst->getOpCode()),
...
bool OMR::Z::CodeGenerator::isActiveArithmeticCC(TR::Register *tstReg) {
  if (self()->hasCCInfo() && self()->hasCCSigned() &&
      ...
      performTransformation(self()->comp(), "O^O Arithmetic CC: CCInfo(%d) "
                                            "CCSigned(%d) CCOverflow(%d) "
                                            "CCCarry(%d)\n",
...
bool OMR::Z::CodeGenerator::signExtendedHighOrderBits(
  ...
  case 48:
    TR_ASSERT(TR::Compiler->target.is64Bit() || is64BitReg,
              "This shift can only work on 64 bits register!\n");
...
TR::RegisterDependencyConditions *
OMR::Z::CodeGenerator::addVMThreadPreCondition(
    TR::RegisterDependencyConditions *deps, TR::Register *reg) {
  if (self()->needsVMThreadDependency()) {
    ...
    // replace this IF statement with an TR_ASSERTC( reg,comp(), "reg cannot be
    // NULL\n");
    if (reg == NULL) {
    ...
    if (deps == NULL) {
      ...
      traceMsg(
          self()->comp(),
          "copying and expanding dependency list, numPre = %i, numPost = %i\n",
          deps->getNumPreConditions(), deps->getNumPostConditions());
      deps = new (self()->trHeapMemory())
          TR::RegisterDependencyConditions(deps, 1, 0, self());
    }
    // TODO: using addPreConditionIfNotAlreadyInserted() is slower than using
    // addPreCondition()
    // think about changing the name of this method to
    // addVMThreadPreConditionIfNotAlreadyInserted()
    // and add one for addVMThreadPreCondition, which uses
    // deps->addPreCondition()
    deps->addPreConditionIfNotAlreadyInserted(reg,
                                              (TR::RealRegister::RegNum)self()
                                                  ->getVMThreadRegister()
                                                  ->getAssociation());
  }
  return deps;
...
TR::RegisterDependencyConditions *
OMR::Z::CodeGenerator::addVMThreadPostCondition(
    TR::RegisterDependencyConditions *deps, TR::Register *reg) {
  if (self()->needsVMThreadDependency()) {
    // TODO This IF statement is for codegen level vm thread work, where we
    // allocate a fixed virtual register to represent the vm thread register.
    ...
    // TODO: using addPostConditionIfNotAlreadyInserted() is slower than using
    // addPostCondition()
    // think about changing the name of this method to
    // addVMThreadPostConditionIfNotAlreadyInserted()
    // and add one for addVMThreadPostCondition, which uses
    // deps->addPostCondition()
    deps->addPostConditionIfNotAlreadyInserted(reg,
                                               (TR::RealRegister::RegNum)self()
                                                   ->getVMThreadRegister()
                                                   ->getAssociation());
  }
...
TR::RegisterDependencyConditions *
OMR::Z::CodeGenerator::addVMThreadDependencies(
    TR::RegisterDependencyConditions *deps, TR::Register *reg) {
...
void OMR::Z::CodeGenerator::setVMThreadRequired(bool v) {
  if (self()->needsVMThreadDependency())
...
void OMR::Z::CodeGenerator::setUnavailableRegistersUsage(
  ...
  do {
    ...
    while (bvi.hasMoreElements()) {
      ...
      int32_t globalRegNumAR = globalRegNum - self()->getFirstGlobalGPR() +
                               self()->getFirstGlobalAR();
 ...
 */
int8_t OMR::Z::CodeGenerator::getCurrentRegisterPressure(TR_RegisterKinds rk) {
  ...
  int8_t numOfRegs = 0;
  if (rk == TR_GPR) {
    for (uint8_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR;
         ++i) {
    ...
    int8_t realRegNum = 0;
    if (liveRegisters) {
      for (info = liveRegisters->getFirstLiveRegister(); info;
           info = info->getNext()) {
        numOfRegs++;
      }
    }
...
void OMR::Z::CodeGenerator::removeUnavailableRegisters(
  ...
  while (loe.hasMoreElements()) {
                                          ...
                                          // registers from available registers
    {
      ...
      TR_BitVector *volatileRegs = self()->comp()->cg()->getGlobalRegisters(
          TR_volatileSpill, callLinkage);
...
int32_t OMR::Z::CodeGenerator::biasDecimalFloatFrac(TR::DataType dt,
                                                    int32_t frac) {
  switch (dt) {
#ifdef J9_PROJECT_SPECIFIC
  case TR::DecimalFloat:
    return TR_DECIMAL_FLOAT_BIAS - frac;
  case TR::DecimalDouble:
    return TR_DECIMAL_DOUBLE_BIAS - frac;
  case TR::DecimalLongDouble:
    return TR_DECIMAL_LONG_DOUBLE_BIAS - frac;
#endif
  default:
    TR_ASSERT(0, "unexpected biasDecimalFloatFrac dt %s", dt.toString());
  }
  return 0;
}
...
TR::Instruction *OMR::Z::CodeGenerator::genLoadAddressToRegister(
  ...
  if (offset >= TR_MIN_RX_DISP && offset <= TR_MAX_RX_DISP) {
    ...
    preced = generateRXYInstruction(cg, TR::InstOpCode::LAY, node, reg, origMR,
                                    preced);
  } else {
...
bool OMR::Z::CodeGenerator::nodeRequiresATemporary(TR::Node *node) {
  ...
  if (node->getOpCode().isPackedRightShift()) {
    ...
    // non-zero even amount when the round amount
    // is also zero is to use an MVN instruction.
    // However since this instruction will perform the 'shift' by moving the
    // sign code to the left there are right aligned dead bytes
    // and implied left aligned zero bytes introduced.
    // These dead and implied bytes cannot be within a user variable as the
    // actual full value must be materialized therefore require a
    ...
    if (roundAmount == 0 && shiftAmount != 0 && ((shiftAmount & 0x1) == 0)) {
      ...
      return true;
             ...
             node->getOpCodeValue() == TR::pddivSelect) {
...
bool OMR::Z::CodeGenerator::endAccumulatorSearchOnOperation(TR::Node *node) {
  ...
  if (self()->comp()->getOption(TR_PrivatizeOverlaps)) {
    ...
    TR_ASSERT(self()->getCodeGeneratorPhase() >
                  TR::CodeGenPhase::SetupForInstructionSelectionPhase,
              "must be past "
              "TR::CodeGenPhase::SetupForInstructionSelectionPhase (current is "
              "%s)\n",
              self()->getCodeGeneratorPhase().getName());
    if (node->getRegister() == NULL && endHint) {
      // do not have to fully descend a tree looking for overlapping operations
      // in the special (and actually quite common)
      // cases where a node will initialize to a new storageReference
      // (endHintOnOperation check) and one of the conditions below is true
      // 1) a conversion between types that use storageRefs (BCD and Aggr) and
      // types that don't (e.g. int,float) -- i2pd,i2o etc. It is always safe to
      // end the search
      ...
      // not overlap
      // endAccumulatorSearchOnOperation will return true to allow the pd2zd to
      // UNPK (accumulate) right to the store location "a" as the
...
bool OMR::Z::CodeGenerator::endHintOnOperation(TR::Node *node) {
           ...
           isOdd(node->getDecimalAdjust())) // odd left shifts need a new temp
  {
    ...
    // overlap a new temp is used
    // return false here to avoid a child of the shift from getting a doomed
    // store hint (one that will have to
 ...
 * Communicates with the evaluator to do MVC semantics under certain condition
 * no matter how the overlap is
 */
bool OMR::Z::CodeGenerator::inlineNDmemcpyWithPad(TR::Node *node,
                                                  int64_t *maxLengthPtr) {
  return false;
}
...
void handleLoadWithRegRanges(TR::Instruction *inst, TR::CodeGenerator *cg) {
  ...
  uint32_t availForShuffle = cg->machine()->genBitVectOfAssignableGPRs() &
                             ~(getRegMaskFromRange(inst));

  for (uint32_t i = lowRegNum; i != highRegNum;
       i = ((i == (isVector ? numVRFs - 1 : 15)) ? 0 : i + 1)) {
    TR::RealRegister *reg = cg->machine()->getS390RealRegister(
        i + (isAR ? TR::RealRegister::AR0 : TR::RealRegister::GPR0));

    // Registers that are assigned need to be checked whether they have a
    // matching STM--otherwise we spill.
    // Since we are called post RA for the LM, we check if assigned registers
    // are last used on the LM so we can
    // avoid needlessly spilling them. (ex.
    // LM(GPR00F,GPR14P,PSALCCAV->LCCAEMS0); ! PSALCCAV needs a reg and only
    // needed on LM)
    ...
    if (reg->getState() == TR::RealRegister::Assigned &&
      ...
      // reg before load clobbers it
      // start of range will be NULL in some cases, we are cautious and spill
      // (def# 100246)
      if (!found) {
...
bool OMR::Z::CodeGenerator::suppressInliningOfRecognizedMethod(
  ...
  if (method == TR::java_lang_Long_reverseBytes ||
      ...
      method ==
          TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_incrementAndGet ||
      method ==
          TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_decrementAndGet ||
      method ==
          TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_addAndGet ||
      method ==
          TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndIncrement ||
      method ==
          TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndDecrement ||
      method ==
          TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndAdd) {
    return true;
  }
...
bool OMR::Z::CodeGenerator::globalAccessRegistersSupported() {
  return false; // TODO : Identitiy needs folding
}

void OMR::Z::CodeGenerator::setRAPassAR() {
  TR_ASSERT(0, "ASC Mode AR not supported");
}

void OMR::Z::CodeGenerator::resetRAPassAR() {
  TR_ASSERT(0, "ASC Mode AR not supported");
}
...
bool TR_S390Peephole::forwardBranchTarget() {
  ...
  while (tmp->isLabel() ||
         _cg->isFenceInstruction(tmp)) // skip labels and fences
    tmp = tmp->getNext();
...
bool OMR::Z::CodeGenerator::canOptimizeEdit(uint8_t *parm, int32_t length) {
  bool leadingSigs = false, trailingStr = false;
  uint8_t fill = 0;
  int32_t numDigits = 0, numSigs = 0, sigOff = 0, lastDigit = 0,
          digitsAfterSig = 0;
  return self()->parseEditParm(parm, length, leadingSigs, trailingStr, fill,
                               numDigits, numSigs, sigOff, lastDigit,
                               digitsAfterSig);
}

bool OMR::Z::CodeGenerator::parseEditParm(uint8_t *parm, int32_t length,
                                          bool &leadingSigs, bool &trailingStr,
                                          uint8_t &fill, int32_t &numDigits,
                                          int32_t &numSigs, int32_t &sigOff,
                                          int32_t &lastDigit,
  ...
  fill = *parm;
  if (fill == 0x20 || fill == 0x21 ||
      fill == 0x22) // fill-byte of a control function is not supported
    return false;
...
// some mechanism, e.g. _symRefsToCheckForKills) that
...
bool OMR::Z::CodeGenerator::isOutOf32BitPositiveRange(int64_t value,
  ...
  if (value < 0 || value > INT_MAX) {
    if (trace)
      traceMsg(self()->comp(), "\tisOutOf32BitPositiveRange = true : value "
                               "%lld < 0 (or > INT_MAX)\n",
               value);
    return true;
  }
...
// different problems
// 1) circular evaluation (asserts) : node->getRegister() == NULL &&
// node->getOpCode().canHaveStorageReferenceHint() below
// 2) clobbering of value child during address child evaluation :
// node->getType().isBCD() below
// A 'conflicting' address node is a BCD/Aggr or canHaveStorageReferenceHint
// node that is commoned some where under both
...
// is evaluated (as it's using a hint)
// and the same pd2zd node is encountered and an assert happens eventually as
// the storageRef nodeRefCount underflows
//
...
// or a pdloadi when forceBCDInit is on
// In either case what happens is that the pd2zd is evaluated first and evaluate
// its child pdX and use it right away to from the source memRef
// Then shortly after, when a hint is used, the address child is evaluated to
// form the dest memRef. When the parent of the pdX in the address child
// (the pdadd here) has an already initialized child then it does a clobber
// evaluate.
// The clobber evaluate will update the storageRef on the pdX register to a new
// temp and then clobber the storageRef already on the pdX
...
bool OMR::Z::CodeGenerator::possiblyConflictingNode(TR::Node *node) {
  ...
  if (node->getReferenceCount() > 1) {
    if (node->getRegister() == NULL &&
      ...
      if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(), "\t\t\tpossiblyConflicting=true %s (%p) : "
                                 "reg==NULL and storageRefHint_Case, refCount "
                                 "%d\n",
      ...
      if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(), "\t\t\tpossiblyConflicting=true %s (%p) : "
                                 "BCDOrAggrType_Case, reg=%s, refCount %d\n",
                 node->getOpCode().getName(), node,
                 node->getRegister()
...
bool OMR::Z::CodeGenerator::loadOrStoreAddressesMatch(TR::Node *node1,
  ...
  if (node1->getSize() != node2->getSize()) {
    if (self()->comp()->getOption(TR_TraceVIP))
      traceMsg(self()->comp(), "\t\tloadOrStoreAddressesMatch = false (sizes "
                               "differ) : node1 %s (%p) size = %d and node2 %s "
                               "(%p) size = %d\n",
  ...
  if (self()->comp()->getOption(TR_TraceVIP))
    traceMsg(self()->comp(), "\t\tloadOrStoreAddressesMatch = %s : node1 %s "
                             "(%p) and node2 %s (%p)\n",
  ...
  if (loadNode->getOpCode().isIndirect() &&
    ...
    return false;
             ...
             !storeNode->getFirstChild()
                  ...
                  .isLoadConst()) {
    return false;
  } else if (loadNode->getOpCode().hasSymbolReference() &&
             loadNode->getSymbolReference()->getSymbol()->isRegisterSymbol()) {
    return false;
    ...
    return false;
             ...
             loadNode->getReferenceCount() == 1) { // The const may be loaded
                                                   // into reg and shared. So
                                                   // dont do it.
    return true;
...
void OMR::Z::CodeGenerator::setBranchOnCountFlag(TR::Node *node,
  ...
  // to zero, while
  // the pattern we're matching here is for the counter to be > 0. If the
  // counter is initially <= 0, the IL
  // would decrement and then fall through the branch, while the BRCT would
  // decrement and take the branch.
  // So, we require that the counter is > 0 before it's decremented, or that
  // it's >= 0 after it's decremented.
  ...
  // expect: under the iRegStore, under the compare, and in the
  // GLRegDeps under the compare. futureUseCount should've been initialized to
  // be referenceCount - 1 (which will have to be an iRegStore in order
  // for us to see the add node under GLRegDeps here). The compare node here
  // will account for the other two uses, so as long as there's no other
