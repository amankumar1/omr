...
public:
  ...
  void setIsNotHighWordUpgradable(bool b = true) {
    _flags.set(IsNotHighWordUpgradable, b);
  }
  ...
  void setIsUpperBitsAreDirty(bool b = true) {
    _flags.set(UpperBitsAreDirty, b);
  }
...
private:
  enum {
    ...
    Is64Bit = 0x0002,     // 390 flag indicates that this Register contained a
                          // 64-bit value
    AssignToHPR = 0x0004, // 390 flag indicates that this virtual register needs
                          // to be assigned to an HPR
    IsNotHighWordUpgradable =
        0x2000, // 390 flag indicates that this Reg can be upgraded to use HPR
    SpilledToHPR =
        0x4000, // 390 flag indicates that this virtual register is spilled to
    ...
    PendingSpillOnDef = 0x100000, // In true register model we will reverse
                                  // spill only when we see a def
