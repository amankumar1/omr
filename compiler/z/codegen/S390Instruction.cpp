...
void TR::S390RSInstruction::generateAdditionalSourceRegisters(
  ...
  if (firstRegNum != lastRegNum && lastRegNum - firstRegNum > 1) {
    ...
    for (int8_t i = 0; i < numRegsToAdd; i++) {
      ...
      // machine->getS390RealRegister(((TR::RealRegister::RegNum)curReg));
      TR::Register *temp =
          machine->getS390RealRegister(((TR::RealRegister::RegNum)curReg));
...
bool isLoopEntryAlignmentEnabled(TR::Compilation *comp) {
  ...
  // if a loop at warm would have
  // exhibited benefit from alignment then the loop must have been hot enough
  // for us to recompile the method
  // to > warm in the first place.

  if (comp->getOptLevel() <= warm)
    return false;

  if (comp->getOption(TR_EnableLabelTargetNOPs)) // only one type of NOP
                                                 // insertion is currently
                                                 // supported
    return false;
...
uint8_t *TR::S390LabelInstruction::generateBinaryEncoding() {
  ...
  if (tryToUseLabelTargetNOPs) {
    ...
    if (tryToUseLabelTargetNOPs) {
      ...
      // already
      // compiled separate programs (batchObjCodeSize) and any offset for any
      // already compiled parent programs for the nested case (getCodeSize)
      size_t batchObjCodeSize = 0;
      size_t objCodeSize = 0;

      if (traceLabelTargetNOPs)
        traceMsg(comp, "\toffsetForLabelTargetNOPs = absOffset 0x%p + "
      ...
      if (traceLabelTargetNOPs) {
        ...
        traceMsg(comp, "\tTR::S390LabeledInstruction %p (%s) at cursor %p "
        ...
        traceMsg(comp, "\tprev %p (%s)\n", prevInst,
                 comp->getDebug()->getOpCodeName(&prevInst->getOpCode()));
      }
    }
  }
  ...
  if (getOpCode().getOpCodeValue() == TR::InstOpCode::DC) {
    ...
    cg()->addRelocation(new (cg()->trHeapMemory())
                            TR_LabelAbsoluteRelocation(cursor, label));
  ...
  } else // must be real LABEL instruction
  {
    ...
    if (_alignment != 0) {
      if (tryToUseLabelTargetNOPs && traceLabelTargetNOPs)
        traceMsg(comp, "force tryToUseLabelTargetNOPs to false because "
                       "_alignment already needed on inst %p\n",
                 this);
  ...
  if (tryToUseLabelTargetNOPs) {
    ...
    if (traceLabelTargetNOPs)
      traceMsg(comp, "\tcomparing offset %lld >? labelTargetNOPLimit %d "
    ...
    if (offset > labelTargetNOPLimit) {
      ...
      if (traceLabelTargetNOPs)
        traceMsg(comp, "\tset useLabelTargetNOPs = true : "
                       "offsetForLabelTargetNOPs=%p > labelTargetNOPLimit %d, "
                       "offset=%lld, cursor %p -> %p, "
                       "labelTargetBytesInserted=%d\n",
  ...
  if (offset && (doUseLabelTargetNOPs || isNopCandidate())) {
    ...
    if (doUseLabelTargetNOPs) {
      if (traceLabelTargetNOPs)
        traceMsg(comp, "\tfinished NOP insertion : setBinaryLength to 0, "
                       "addAccumError = estBinLen %d - bytesInserted %d = %d, "
      ...
      TR_ASSERT(getEstimatedBinaryLength() >= labelTargetBytesInserted,
                "lable inst %p : estimatedBinaryLength %d must be >= "
...
bool TR::S390LabelInstruction::considerForLabelTargetNOPs(
  ...
  if (traceLabelTargetNOPs)
    traceMsg(comp, "considerForLabelTargetNOPs check during %s phase : %p "
                   "(%s), prev  %p (%s)\n",
             inEncodingPhase ? "encoding" : "estimating", this,
             comp->getDebug()->getOpCodeName(&thisInst->getOpCode()), prevInst,
             comp->getDebug()->getOpCodeName(&prevInst->getOpCode()));

  if (isSkipForLabelTargetNOPs()) {
    if (traceLabelTargetNOPs)
      traceMsg(comp, "\t%p (%s) marked with isSkipForLabelTargetNOPs = true : "
                     "set doConsider=false\n",
               this, comp->getDebug()->getOpCodeName(&thisInst->getOpCode()));
    doConsider = false;
  } else if (prevInst &&
    ...
    // been inserted between the estimate and encoding phases
    // and therefore this routine would return false during estimation and true
    // during encoding and the estimate bump for NOPs would have been missed
    if (traceLabelTargetNOPs)
      traceMsg(comp, "\t%p (%s) marked with wasEstimateDoneForLabelTargetNOPs "
...
TR::S390LabelInstruction::estimateBinaryLength(int32_t currentEstimate) {
  ...
  if (comp->getOption(TR_EnableLabelTargetNOPs)) {
            ...
            false)) // inEncodingPhase=false (in estimating phase now)
    {
      ...
      if (getLabelSymbol() != NULL) {
        if (traceLabelTargetNOPs)
          traceMsg(comp, "\tupdate label %s (%p) estimatedCodeLocation with "
                         "estimate %d + maxNOPBump %d = %d\n",
...
void TR::S390BranchInstruction::assignRegistersAndDependencies(
  ...
  if (getOpCode().isBranchOp() &&
    ...
    // branches to the
    // OOL section merging point. Register snapshot is a snapshot of register
    // states taken at
    // OOL merge label. Using this snapshot RA can enforce the similarity of
    // register states
    // at the end of main-stream code and OOL path.
    // Generally the safer option is to not reuse OOL merge label for any other
    // purpose. This
    ...
    if (cg()->getIsInOOLSection()) {
      ...
      // allowed. Branches
      // in the OOL section can jump to the end of section and then only one
      // branch (the
      // last instruction of an OOL section) jumps to the merge-point. In other
      // words, OOL
      // section must contain exactly one exit point.
      TR_ASSERT(comp->getAppendInstruction() == this,
 ...
 * instruction.
 *     If the branch distance is within +/- 2**16, it will be done with a
 * relative branch.
 ...
 * definition), slow muck
 *     will not be generated. Note this is different than BranchOnCount and
 * BranchOnIndex
...
uint8_t *TR::S390BranchInstruction::generateBinaryEncoding() {
  ...
  if (doRelocation) {
    if (shortRelocation) {
      cg()->addRelocation(
          new (cg()->trHeapMemory())
              TR_16BitLabelRelativeRelocation(relocationPoint, label));
    } else if (longRelocation) {
      cg()->addRelocation(
          new (cg()->trHeapMemory())
              TR_32BitLabelRelativeRelocation(relocationPoint, label));
    } else {
      AOTcgDiag1(comp, "add TR_AbsoluteMethodAddress cursor=%x\n", cursor);
      cg()->addRelocation(new (cg()->trHeapMemory()) TR_LabelAbsoluteRelocation(
          relocationPoint, label));
    ...
    *(int32_t *)(cursor + 2) |= boi(distance);
    ...
    if (doRelocation) {
      if (shortRelocation) {
        ...
        cg()->addRelocation(
            new (cg()->trHeapMemory())
                TR_LabelAbsoluteRelocation(relocationPoint, label));
        cg()->addProjectSpecializedRelocation(relocationPoint, NULL, NULL,
  ...
  if (distance >= MIN_IMMEDIATE_VAL && distance <= MAX_IMMEDIATE_VAL) {
    ...
    *(int16_t *)(cursor + 2) |= bos(distance);
    ...
    TR_ASSERT(cg()->isExtCodeBaseFreeForAssignment() == false,
              "TR::S390BranchOnIndexInstruction::generateBinaryEncoding -- Ext "
              "Code Base was wrongly released\n");

    TR::InstOpCode::Mnemonic opCode = getOpCodeValue();
    TR_ASSERT((opCode == TR::InstOpCode::BRXLE) ||
                  (opCode == TR::InstOpCode::BRXH) ||
                  (opCode == TR::InstOpCode::BRXLG) ||
                  (opCode == TR::InstOpCode::BRXHG),
              "Only "
              "TR::InstOpCode::BRXLE/TR::InstOpCode::BRXH/"
              "TR::InstOpCode::BRXLG/TR::InstOpCode::BRXHG are handled here\n");
    ...
    relocationPoint = cursor;
    cursor += sizeof(intptrj_t); // This should be correct when targeting a 64
                                 // bit platform from a 32 bit build
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      (*(uint32_t *)cursor) =
          boi(0xEB007000 | (regMask << 16)); // BXHG Rx,Ry,0(r7)
      cursor += 4;
...
uint8_t *TR::S390PseudoInstruction::generateBinaryEncoding() {
      ...
      NULL) // We have to emit a branch around a 8-byte aligned call descriptor.
  {
    ...
    // call NOP, we have
    // to manually emit a branch around the call descriptor in the main line
    // code.
    //    BRC  4 + Padding
    //    <Padding>
 ...
 * address of the debug counter count.
 ...
 * required:
 *                                                    STG     Rscrtch,
 * offToLongDispSlot(,GPR5)
 * LGRL    Rscrtch,   counterRelocation               LGRL    Rscrtch,
 * counterRelocation
...
uint8_t *TR::S390DebugCounterBumpInstruction::generateBinaryEncoding() {
  ...
  if (scratchReg) {
    ...
    TR_ASSERT(!comp->getOption(TR_DisableLongDispStackSlot),
              "TR_S390DebugCounterBumpInstruction::generateBinaryEncoding -- "
              "Spill slot should be enabled.");
    scratchReg = assignBestSpillRegister();
  }

  TR_ASSERT(scratchReg != NULL, "TR_S390DebugCounterBumpInstruction::"
  ...
  *(int16_t *)cursor = bos(0xC408); // LGRL Rscrtch, counterRelocation
  scratchReg->setRegisterField((uint32_t *)cursor); //
  cg()->addRelocation(new (cg()->trHeapMemory())    //
                      TR_32BitLabelRelativeRelocation(
 ...
 * halfwords. This value will
 * be used by binary encoding to set the correct target. Example of instruction
 * that uses this are
 * BRCL and BRASL instruction. Usually its used for getting helper method's
 * target addresses.
 *
...
TR::S390RILInstruction::adjustCallOffsetWithTrampoline(int32_t offset,
  ...
  if (!CHECK_32BIT_TRAMPOLINE_RANGE(getTargetPtr(), (uintptrj_t)currentInst)) {
    ...
    if (getSymbolReference()->getReferenceNumber() < TR_S390numRuntimeHelpers)
      targetAddr = cg()->fe()->indexedTrampolineLookup(
    ...
    TR_ASSERT(CHECK_32BIT_TRAMPOLINE_RANGE(targetAddr, (uintptrj_t)currentInst),
...
uint8_t *TR::S390RILInstruction::generateBinaryEncoding() {
  ...
  if (getTargetSnippet() &&
      ...
      } else // We need to do things the old fashioned way
      {
        ...
        if (comp->getOption(TR_DisableLongDispStackSlot)) {
          scratchReg = cg()->getExtCodeBaseRealRegister();
                   ...
                   getOpCode().getOpCodeValue() == TR::InstOpCode::LGFRL ||
                   getOpCode().getOpCodeValue() == TR::InstOpCode::LLGFRL) {
          scratchReg = (TR::RealRegister *)((TR::S390RegInstruction *)this)
        ...
        if (getOpCode().getOpCodeValue() == TR::InstOpCode::LGRL) {
          ...
          //
          *(int32_t *)cursor =
              boi(0xE3000000); // LG  scratchReg, 0(,scratchReg)
          scratchReg->setRegisterField((uint32_t *)cursor);
          ...
          *(int16_t *)cursor = bos(0x0004);
          ...
          //
          *(int32_t *)cursor =
              boi(0xE3000000); // LGF scratchReg, 0(,scratchReg)
          scratchReg->setRegisterField((uint32_t *)cursor);
          ...
          *(int16_t *)cursor = bos(0x0014);
          ...
          //
          *(int32_t *)cursor =
              boi(0xE3000000); // LLGF scratchReg, 0(,scratchReg)
          scratchReg->setRegisterField((uint32_t *)cursor);
          scratchReg->setRegister2Field((uint32_t *)cursor);
          cursor += 4;
    ...
    if (getTargetSnippet() != NULL) {
      cg()->addRelocation(
          new (cg()->trHeapMemory()) TR_32BitLabelRelativeRelocation(
              cursor, getTargetSnippet()->getSnippetLabel()));
    } else if (getTargetLabel() != NULL) {
      cg()->addRelocation(
          new (cg()->trHeapMemory())
              TR_32BitLabelRelativeRelocation(cursor, getTargetLabel()));
    }
    ...
    if (getTargetLabel() != NULL) {
      ...
      if (getTargetSnippet() != NULL) {
        ...
        // trampoline.
        if (!isImmediateOffsetInBytes() &&
            !CHECK_32BIT_TRAMPOLINE_RANGE(getTargetPtr(), (uintptrj_t)cursor)) {
    ...
    // immediate field so that it can be
...
// When a method is recompiled the entry point of the old implementation gets
// patched with a branch
// to the preprologue. The callee is then responsible for patching the address
// of the callers BRASL
// to point to the new implementation, and subsequently branch to the new
// location. Because this
// patching happens at runtime we must ensure that the store to the relative
// immediate offset in the
// BRASL is done atomically. On zOS we allow atomic patching on both a 4 and an
// 8 byte boundary. On
// zLinux 64 bit we only allow atomic patching on a 4 byte boundary as we use
// Multi-Code Caches and
// require trampolines.
    ...
    if (getTargetSnippet() != NULL) {
       ...
       * has not set yet
       * 2- 'targetAddress' is provided: We use provided address to generate the
       * brasl address
       * 3- 'targetAddress' is not provided: In this case the address is
       * extracted from TargetSymbol.
       *     If targetAddress is not provided, TargetSymbol should be a method
       * symbol.
       */
      TR_ASSERT(
          getTargetPtr() ||
      ...
      if (getTargetSymbol()) {
        ...
        TR::ResolvedMethodSymbol *sym =
            (callSymbol) ? callSymbol->getResolvedMethodSymbol() : NULL;
        TR_ResolvedMethod *fem = sym ? sym->getResolvedMethod() : NULL;
        isRecursiveCall =
            (fem != NULL && fem->isSameMethod(comp->getCurrentMethod()) &&
             !comp->isDLT());
      }
      if (isRecursiveCall) {
        // call myself case
 ...
 * ... (SLLG,...)
 *       targetReg => R1,                          , secondReg => R3 sourceImm
 * => D2  ... (SLLG,...)
 *       targetReg => R1,      MemRef    => (B2,D2) ... (SLL,...)
 *       targetReg => R1,                                            sourceImm
 * => D2  ... (SLL,...)
 */
...
uint8_t *TR::S390RIEInstruction::generateBinaryEncoding() {
  ...
  if (getBranchDestinationLabel()) {
    ...
    if (getWarmToColdTrampolineSnippet()) {
      trampolineDistance =
          (cg()->getBinaryBufferStart() +
           getWarmToColdTrampolineSnippet()
               ->getSnippetLabel()
               ->getEstimatedCodeLocation()) -
          (instructionStart + cg()->getAccumulatedInstructionLengthError());
    }
    ...
    if ((distance / 2) >= MIN_IMMEDIATE_VAL &&
      ...
      if (doRelocation) {
        cg()->addRelocation(new (cg()->trHeapMemory())
                                TR_16BitLabelRelativeRelocation(cursor, label));
      } else {
        *(int16_t *)(cursor) |= bos(distance / 2);
      }
    } else if (getWarmToColdTrampolineSnippet() &&
               trampolineDistance >= MIN_IMMEDIATE_VAL &&
               trampolineDistance <= MAX_IMMEDIATE_VAL) {
      cg()->addRelocation(
          new (cg()->trHeapMemory()) TR_16BitLabelRelativeRelocation(
              cursor, getWarmToColdTrampolineSnippet()->getSnippetLabel()));
      ((TR::S390WarmToColdTrampolineSnippet *)getWarmToColdTrampolineSnippet())
          ->setIsUsed(true);
    } else {
      ...
      *(cursor + 1) |= (uint8_t)getMask();
      ...
      if (doRelocation) {
        cg()->addRelocation(new (cg()->trHeapMemory())
                                TR_32BitLabelRelativeRelocation(cursor, label));
      } else {
...
uint8_t *TR::S390RIEInstruction::splitIntoCompareAndLongBranch(void) {
  ...
  // pointer to the instruction after the inserted BRC/BRCL
  // so the main cg generateBinaryEncoding loop does not encode the new CHI and
  // BRC/BRCL instruction a second time
...
bool TR::S390RXInstruction::refsRegister(TR::Register *reg) {
  ...
  bool enableHighWordRA = cg()->supportsHighWordFacility() &&
                          !comp->getOption(TR_DisableHighWordRA) &&
                          reg->getKind() != TR_FPR && reg->getKind() != TR_VRF;
  if (enableHighWordRA && getMemoryReference() && reg->getRealRegister() &&
      TR::Compiler->target.is64Bit()) {
    TR::RealRegister *realReg = (TR::RealRegister *)reg;
    TR::Register *regMem = reg;
    if (realReg->isHighWordRegister()) {
...
uint8_t *TR::S390RXInstruction::generateBinaryEncoding() {
  ...
  if (getMemoryReference() != NULL) {
    ...
    TR_ASSERT((getConstForMRField() & 0xFFF00000) == 0,
              "TR::S390RXInstruction::_constForMRField greater than 0x000FFFFF "
              "for instruction 0x%x",
              this);
...
uint8_t *TR::S390RXEInstruction::generateBinaryEncoding() {
  ...
  if (getMemoryReference() != NULL) {
    ...
    TR_ASSERT(
        (getConstForMRField() & 0xFFF00000) == 0,
        "TR::S390RXEInstruction::_constForMRField greater than 0x000FFFFF");
...
uint8_t *TR::S390RXYInstruction::generateBinaryEncoding() {
  ...
  if (getMemoryReference() != NULL) {
    ...
    TR_ASSERT(
        (getConstForMRField() & 0xFFF00000) == 0,
        "TR::S390RXYInstruction::_constForMRField greater than 0x000FFFFF");
    (*(uint32_t *)cursor) &= boi(0xFF000000);
    (*(uint32_t *)cursor) |= boi(0x000FFFFF & getConstForMRField());
  }
...
uint8_t *TR::S390VRIInstruction::generateBinaryEncoding() {
  // *this    swipeable for debugging purposes
  uint8_t *instructionStart = cg()->getBinaryBufferCursor();
  uint8_t *cursor = instructionStart;
  memset((void *)cursor, 0, getEstimatedBinaryLength());
  ...
  if (getRegisterOperand(3) != NULL)
    toRealRegister(getRegisterOperand(3))
        ->setRegister3Field((uint32_t *)cursor);
  ...
  setMaskField((uint32_t *)cursor, maskIn3, 3); // if the format doesn't have
...
uint8_t *TR::S390VRRInstruction::generateBinaryEncoding() {
  ...
  if (getRegisterOperand(4) != NULL)
    toRealRegister(getRegisterOperand(4))
        ->setRegister4Field(
            (uint32_t *)cursor); // Will cause assertion failure by now
  else
    setMaskField((uint32_t *)cursor, maskIn3,
                 3); // This mask is aliased with register operand 4
...
uint8_t *TR::S390VStorageInstruction::generateBinaryEncoding() {
  ...
  // if the format doesn't have this field, the default value of maskField will
  // always be zero.
 ...
 * VRS-a
 */
uint8_t *TR::S390VRSaInstruction::generateBinaryEncoding() {
 ...
 * VRS-b
 */
uint8_t *TR::S390VRSbInstruction::generateBinaryEncoding() {
  return TR::S390VStorageInstruction::generateBinaryEncoding();
}
 ...
 * VRS-c
 */
uint8_t *TR::S390VRScInstruction::generateBinaryEncoding() {
  return TR::S390VStorageInstruction::generateBinaryEncoding();
}
...
uint8_t *TR::S390VRXInstruction::generateBinaryEncoding() {
  return TR::S390VStorageInstruction::generateBinaryEncoding();
}

int32_t TR::S390VRXInstruction::estimateBinaryLength(int32_t currentEstimate) {
  if (getMemoryReference() != NULL) {
    return getMemoryReference()->estimateBinaryLength(currentEstimate, cg(),
                                                      this);
  } else {
...
int32_t TR::S390NOPInstruction::estimateBinaryLength(int32_t currentEstimate) {
  ...
  // Save the offset for Call Descriptor NOP (zOS-31 XPLINK), so that we can
...
uint8_t *TR::S390NOPInstruction::generateBinaryEncoding() {
  ...
  if (getKindNOP() == FastLinkCallNOP) { // assumes 4 byte length
    ...
    (*(uint32_t *)cursor) = boi(nopInst);
    if (getTargetSnippet() != NULL) {
      // Check Relocation Distance, and see whether it's within 16 bits
      // distance.
      // The offset is a 16 bit field containg the offset in double worlds from
      // the call site to the call descriptor.
      // Our snippet is always emitted later than our NOP.
      intptrj_t snippetOffset =
          getTargetSnippet()->getSnippetLabel()->getEstimatedCodeLocation();
      intptrj_t currentOffset = _estimatedOffset;

      intptrj_t distance =
          snippetOffset - currentOffset +
          8; // Add 8 to conservatively estimate the alginment of NOP.

      // TODO: confirm that it's okay to have negative offset (for cases when
      // JNI call is outofline)
      TR_ASSERT(distance > 0, "XPLINK Call Descriptor Snippet is before NOP "
                              "instruction (JNI Call).");

      // If within range, we don't have to emit our psuedo branch around
      // instruction.
      if (distance < 0x3FFF8 && distance > 0) {
        cg()->addRelocation(
            new (cg()->trHeapMemory()) TR_16BitLabelRelativeRelocation(
                cursor + 2, getTargetSnippet()->getSnippetLabel(), 8));
      } else {
        // Snippet is beyond 16 bit relative relocation distance.
        // In this case, we have to activate our pseudo branch around call
        // descriptor
        getCallDescInstr()->setCallDescValue(
            ((TR::S390ConstantDataSnippet *)getTargetSnippet())
                ->getDataAs8Bytes(),
            cg()->trMemory());
        cg()->addRelocation(
            new (cg()->trHeapMemory()) TR_16BitLabelRelativeRelocation(
                cursor + 2, getCallDescInstr()->getCallDescLabel(), 8));
      }
    }
    cursor += 4;
...
int32_t TR::S390MIIInstruction::estimateBinaryLength(int32_t currentEstimate) {
  ...
  // support for longer disposition
...
uint8_t *TR::S390MIIInstruction::generateBinaryEncoding() {
  ...
  if (distance >= MIN_12_RELOCATION_VAL && distance <= MAX_12_RELOCATION_VAL &&
    ...
    cg()->addRelocation(
        new (cg()->trHeapMemory())
            TR_12BitLabelRelativeRelocation(cursor, getLabelSymbol(), false));
...
uint8_t *TR::S390SMIInstruction::generateBinaryEncoding() {
  ...
  if (distance >= MIN_RELOCATION_VAL && distance <= MAX_RELOCATION_VAL &&
    ...
    *(cursor) |= ((uint8_t)getMask() << 4);
    ...
    cg()->addRelocation(
        new (cg()->trHeapMemory())
            TR_16BitLabelRelativeRelocation(cursor, getLabelSymbol(), 4, true));
 ...
 * BRCL
 * This branch will be to a label.  It is assumed it is within range of a BRC on
 * a G5--
 * previous checking will ensure this.  If on a freeway, we are free to choose
 * either
...
uint8_t *TR::S390VirtualGuardNOPInstruction::generateBinaryEncoding() {
  ...
  // patch
  // point so setup the patching location to use this previous guard and
  // generate no
  ...
  // patch is written
  // in) if the patching occurs during GC pause times.  The patching of up to
  // 6-bytes is potentially
  // not atomic.

  bool performEmptyPatch =
      getNode()->isHCRGuard() || getNode()->isProfiledGuard();

  // HCR guards that are merged with profiled guards never need to generate NOPs
  // for patching because
  // the profiled guard will generate the NOP branch to the same location the
  // HCR guard needs to branch
  // to, so we can always use the NOP branch form the profiled guard as our HCR
  // patch point.

  int32_t sumEstimatedBinaryLength = getNode()->isProfiledGuard() ? 6 : 0;

  if (performEmptyPatch) {
    // so at this point we know we are an HCR guard and that there may be other
    // HCR guards
    // after us that will use us as their patch point. We now calculate how much
    // space we
    ...
    for (TR::Instruction *nextI = getNext(); nextI != NULL;
      ...
      // destinations etc
      // and so we cannot include them in the region for an HCR guard to
      // overwrite because
      // the runtime patching could take place after the HCR guard has been
      // patched
      ...
      // non-zero length so
      // skip PSEUDO instructions from length consideration because in practice
      // they will generally
      // be zero
  ...
  // return point of slow path is right after the guard -
...
TR::MemoryReference *getFirstReadWriteMemoryReference(TR::Instruction *i) {
  ...
  case TR::Instruction::IsOpCodeOnly:
    ...
    break;
  default:
    TR_ASSERT(0, "This type of instruction needs to be told how to get memory "
