...
class S390PseudoInstruction : public TR::Instruction {
  ...
  union {
    uint64_t
        _callDescValue; ///< Call Descriptor for XPLINK on zOS 31 JNI Calls.
    int32_t
        _register_num; ///< contains machine register number for LOCK & UNLOCK
  };
...
public:
  S390PseudoInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : TR::Instruction(op, n, cg), _fenceNode(fenceNode), _callDescValue(0),
        _padbytes(0), _callDescLabel(NULL), _asmDataEncoding(NULL),
        _asmDataEncodingLength(0), _shouldBeginNewLine(false) {}
  ...
  S390PseudoInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : TR::Instruction(op, n, cond, cg), _fenceNode(fenceNode),
        ...
        _asmDataEncoding(NULL), _asmDataEncodingLength(0),
  ...
  S390PseudoInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : TR::Instruction(op, n, cond, precedingInstruction, cg),
        ...
        _callDescLabel(NULL), _asmDataEncoding(NULL), _asmDataEncodingLength(0),
        _shouldBeginNewLine(false) {}
  ...
  void setRegisterNumber(int32_t regnum) {
    TR_ASSERT(getOpCodeValue() == TR::InstOpCode::LOCK ||
                  getOpCodeValue() == TR::InstOpCode::UNLOCK,
              "assertion failure");
  ...
  void setASMDataEncodingLength(int32_t encodingLength) {
    _asmDataEncodingLength = encodingLength;
  }
...
public:
  S390RegInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
    ...
    // require consequtive even-odd register pairs.
    // CanUseRegPairForTarget returns true for all the above instructions +
    // STM/LTM (which can potentially take a register pair range).

    // So, if we find a register pair passed in, we check to make sure the
    // instruction CAN use a register pair.
    // If we do not find a register pair, we check to make sure the instruction
    // SHOULD NOT use a register pair.
    TR_ASSERT((!_targetPairFlag && !getOpCode().shouldUseRegPairForTarget()) ||
              ...
              cg->getDebug() ? cg->getDebug()->getOpCodeName(&this->getOpCode())
                             : "(unknown)",
  ...
  //   != 0) ? tgtRegArrElem(0) : NULL; }
  //   TR::Register  *setTargetRegister(TR::Register *r) {assume0(_targetRegSize
  //   != 0); return (targetRegBase())[0] = r;}

  /**
   * Given that instruction expresses a register range (eg. LM or STM) the first
   * and last register of the range
   * is fetched from first operand as a register pair or from the first two
   * operands
   */
  ...
  bool matchesTargetRegister(TR::Register *reg) {
    ...
    if (isTargetPair()) {
      ...
      if (enableHighWordRA && getRegisterOperand(1)->getRealRegister()) {
        targetReg1 =
            ((TR::RealRegister *)getRegisterOperand(1))->getLowWordRegister();
        return realReg == targetReg1;
      }

      if (reg->getRealRegister() && getRegisterOperand(1)->getRealRegister() &&
          TR::RealRegister::isAR(((TR::RealRegister *)getRegisterOperand(1))
                                     ->getRegisterNumber()) &&
          GPRmatchesAR(toRealRegister(reg),
  ...
  bool GPRmatchesAR(TR::RealRegister *gprReg, TR::RealRegister *arReg) {
    ...
    if (gprReg->getRegisterNumber() ==
        arReg->getRegisterNumber() - TR::RealRegister::FirstAR + 1)
      return true;
    return false;
  }
...
public:
  ...
  S390RRInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    ...
                    TR::RegisterDependencyConditions *cond,
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, cond, cg), _flagsRR(0),
        _secondConstant(-1) {
    checkRegForGPR0Disable(op, sreg);
    if (!getOpCode().setsOperand2())
      useSourceRegister(sreg);
  ...
  S390RRInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    ...
                    TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, precedingInstruction, cg), _flagsRR(0),
        _secondConstant(-1) {
    checkRegForGPR0Disable(op, sreg);
    if (!getOpCode().setsOperand2())
      useSourceRegister(sreg);
  ...
  S390RRInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Register *treg, int8_t secondConstant,
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, cg), _flagsRR(0),
        _secondConstant(secondConstant) {}
  ...
  S390RRInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : S390RegInstruction(op, n, firstConstant, precedingInstruction, cg),
        _flagsRR(0), _secondConstant(secondConstant) {}

  S390RRInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    int8_t firstConstant, int8_t secondConstant,
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, firstConstant, cg), _flagsRR(0),
        _secondConstant(secondConstant) {}
  ...
  //   (_sourceRegSize!=0) ? (sourceRegBase())[0] : NULL;  }
  //   TR::Register  *setSourceRegister(TR::Register *sr) { (sourceRegBase())[0]
  //   = sr; return sr; }
  ...
  //
  // Note: these functions are not used anymore, will do addInstructionComment
  // with the annotation instead.
  // Will leave _flagsRR for now until next change, but these flags won't be
  // used in tr.dev.
...
public:
  ...
  // TR::Register *getSourceRegister()                {return
  // (sourceRegBase())[tr_srcidx]; }
  // TR::Register *setSourceRegister(TR::Register *sr) {return
  // (sourceRegBase())[tr_srcidx] = sr;}

  //   TR::Register *getTargetRegister()                { return
  //   (targetRegBase())[tr_targidx];}
  //   TR::Register *setTargetRegister(TR::Register *sr) { return
...
public:
  ...
  // (_sourceRegSize==2) ? (sourceRegBase())[1] : NULL; }
  // TR::Register *setSourceRegister2(TR::Register *sr) {return
  // (sourceRegBase())[1]=sr;}
...
public:
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, uintptrj_t tp, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _targetSnippet(NULL), _targetSymbol(NULL),
        _flagsRIL(0), _mask(0xffffffff), _targetLabel(NULL),
        _symbolReference(NULL) {

    checkRegForGPR0Disable(op, treg);
    if (getOpCode().isExtendedImmediate()) {
      // If extended immediate instruction, store parm as source immediate
      // value.
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, uintptrj_t tp, TR::SymbolReference *sr,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _targetPtr(tp), _targetSnippet(NULL),
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg), _targetPtr(tp),
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, uintptrj_t tp,
                     TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg), _targetPtr(tp),
        _targetSnippet(NULL), _targetSymbol(NULL), _flagsRIL(0),
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : TR::Instruction(op, n, cg), _targetPtr((uintptrj_t)NULL),
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint32_t mask,
                     uintptrj_t targetPtr, TR::CodeGenerator *cg)
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint32_t mask,
      ...
      : TR::Instruction(op, n, cg), _targetPtr((uintptrj_t)NULL),
        _targetSnippet(ts), _targetSymbol(NULL), _flagsRIL(0), _mask(mask),
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg),
        _targetPtr((uintptrj_t)NULL), _targetSnippet(ts), _targetSymbol(NULL),
        _flagsRIL(0), _mask(0xffffffff), _targetLabel(NULL),
        _symbolReference(NULL), _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::RegisterDependencyConditions *cond,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, cg), _targetPtr((uintptrj_t)NULL),
        _targetSnippet(ts), _targetSymbol(NULL), _flagsRIL(0),
        _mask(0xffffffff), _targetLabel(NULL), _symbolReference(NULL),
        _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::RegisterDependencyConditions *cond,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, cg), _targetPtr(tp), _targetSnippet(NULL),
        _targetSymbol(NULL), _flagsRIL(0), _mask(0xffffffff),
        _targetLabel(NULL), _symbolReference(NULL), _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg),
        _targetPtr((uintptrj_t)NULL), _targetSnippet(NULL), _targetSymbol(sym),
        _flagsRIL(0), _mask(0xffffffff), _targetLabel(NULL),
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint32_t mask,
      ...
      : TR::Instruction(op, n, precedingInstruction, cg),
        _targetPtr((uintptrj_t)NULL), _targetSnippet(NULL), _targetSymbol(sym),
        _flagsRIL(0), _mask(mask), _targetLabel(NULL), _symbolReference(sr),
        _sourceImmediate(0) {
    if (sym->isLabel())
      _targetLabel = sym->castToLabelSymbol();
  }
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg),
        _targetPtr((uintptrj_t)NULL), _targetSnippet(NULL), _targetSymbol(NULL),
        _flagsRIL(0), _mask(0xffffffff), _targetLabel(label),
        _symbolReference(NULL), _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
      useSourceRegister(treg);
  ...
  //   TR::Register  *getTargetRegister()               { return
  //   (_targetRegSize!=0) ? (targetRegBase())[0] : NULL;}
  //   TR::Register  *setTargetRegister(TR::Register *r) {
  //   assume0(_targetRegSize!=0);  (targetRegBase())[0] = r; return r; }
  bool matchesTargetRegister(TR::Register *reg) {
    TR::RealRegister *realReg = NULL;
    ...
    // if we are matching real regs
    if (enableHighWordRA && getRegisterOperand(1) &&
...
public:
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : S390RegInstruction(op, n, treg, cg), _sourceImmediate(imm),
        _maskImmediate(0), _idx(-1), _hasMask(false), _kind(IsRS) {
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : S390RegInstruction(op, n, treg, _conditions, precedingInstruction, cg),
        _kind(IsRS), _sourceImmediate(imm), _maskImmediate(0), _idx(-1),
        _hasMask(false) {
    // 1 Register is specified - If it is not a register pair, make sure
    // instruction doesn't take a range (i.e. STM).
    TR_ASSERT(treg->getRegisterPair() || !getOpCode().usesRegRangeForTarget(),
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    ...
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, precedingInstruction, cg),
        _sourceImmediate(0), _maskImmediate(0), _idx(-1), _hasMask(false),
        _kind(IsRS) {
    useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
    if (mf->getUnresolvedSnippet() != NULL)
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    ...
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, precedingInstruction, cg),
        _sourceImmediate(0), _maskImmediate(mask), _idx(-1), _hasMask(true),
        _kind(IsRS) {
    useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
    if (mf->getUnresolvedSnippet() != NULL)
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    ...
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, cg), _sourceImmediate(0),
        _maskImmediate(mask), _idx(-1), _hasMask(true), _kind(IsRS) {
    useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
    if (mf->getUnresolvedSnippet() != NULL)
      (mf->getUnresolvedSnippet())->setDataReferenceInstruction(this);
...
public:
  ...
  S390RRSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : S390RRInstruction(op, n, targetRegister, sourceRegister, cg),
        _kind(IsRRS), _branchDestination(branchDestination),
        _branchCondition(branchCondition) {
...
private:
  ...
  TR::InstOpCode
      _extendedHighWordOpCode; ///< for zG highword rotate instructions
...
public:
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::InstOpCode::S390BranchCondition branchCondition,
                     TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, targetRegister, cg), _kind(IsRIE),
        _instructionFormat(RIE_RR), _branchDestination(branchDestination),
        _branchCondition(branchCondition), _warmToColdTrampolineSnippet(NULL) {
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : S390RegInstruction(op, n, targetRegister, precedingInstruction, cg),
        _kind(IsRIE), _instructionFormat(RIE_RR),
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, targetRegister, precedingInstruction, cg),
        _kind(IsRIE), _instructionFormat(RIE_RI8),
        _branchDestination(branchDestination),
        _branchCondition(branchCondition), _sourceImmediate8(sourceImmediate),
        _warmToColdTrampolineSnippet(NULL) {
    // note that _targetRegister is registered for use via the
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : S390RegInstruction(op, n, targetRegister, cg), _kind(IsRIE),
        ...
        _extendedHighWordOpCode(TR::InstOpCode::BAD), _branchDestination(0),
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, targetRegister, precedingInstruction, cg),
        _kind(IsRIE), _instructionFormat(RIE_IMM),
        _extendedHighWordOpCode(TR::InstOpCode::BAD), _branchDestination(0),
        _branchCondition(TR::InstOpCode::COND_NOPR),
        _sourceImmediate8(sourceImmediate),
        _sourceImmediate8One(sourceImmediateOne),
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, targetRegister, precedingInstruction, cg),
        _kind(IsRIE), _instructionFormat(RIE_RI16A), _branchDestination(0),
        _branchCondition(branchCondition), _sourceImmediate16(sourceImmediate),
        _warmToColdTrampolineSnippet(NULL) {
    if (isTrap())
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, targetRegister, precedingInstruction, cg),
        _kind(IsRIE), _instructionFormat(RIE_RRI16),
        _extendedHighWordOpCode(TR::InstOpCode::BAD), _branchDestination(0),
  ...
  virtual void setExtendedHighWordOpCode(TR::InstOpCode op) {
    _extendedHighWordOpCode = op;
  }
  virtual TR::InstOpCode &getExtendedHighWordOpCode() {
    return _extendedHighWordOpCode;
  }
  ...
  /** warm to Cold Trampoline Snippet */
  virtual TR::Snippet *getWarmToColdTrampolineSnippet() {
    return _warmToColdTrampolineSnippet;
  }
  virtual TR::Snippet *setWarmToColdTrampolineSnippet(TR::Snippet *snippet) {
    return _warmToColdTrampolineSnippet = snippet;
  }
...
public:
  ...
  S390RISInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : S390RIInstruction(op, n, targetRegister, sourceImmediate, cg),
        _kind(IsRIS), _branchDestination(branchDestination),
        _branchCondition(branchCondition) {
    setupThrowsImplicitNullPointerException(n, branchDestination);
  }
...
public:
  ...
  S390SIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : S390MemInstruction(op, n, mf, precedingInstruction, cg, false),
        _sourceImmediate(imm), _kind(IsSI) {
    useSourceMemoryReference(mf);
  };
...
public:
  S390SS5WithImplicitGPRsInstruction(
    ...
    // followed by the up to two memory operands
    // Move all the added implicit registers adjacents to the first two register
    // operands
...
public:
  ...
  S390RXInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Register *treg, uint32_t constMR, TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, cg), _kind(IsRX),
        _constForMRField(constMR) {}

  S390RXInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Register *treg, uint32_t constMR,
                    TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, precedingInstruction, cg), _kind(IsRX),
        _constForMRField(constMR) {}
...
protected:
  S390VInstruction(TR::CodeGenerator *cg = NULL,
                   TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                   TR::Node *n = NULL, TR::Register *reg1 = NULL)
  ...
  virtual void setMaskField(uint32_t *instruction, uint8_t mask, int nField) {
    TR_ASSERT(nField >= 0 && nField <= 3,
              "Field index out of range."); // field 0 to 3 corresponding to bit
                                            // 20 to bit 35, 4 bits for each
                                            // field
...
public:
  S390VRIbInstruction(TR::CodeGenerator *cg = NULL,
      ...
      : S390VRIInstruction(cg, op, n, targetReg,
                           CAT8TO16(constantImm2, constantImm3), 0, mask4, 0) {}
...
public:
  S390VRIeInstruction(TR::CodeGenerator *cg = NULL,
                      ...
                      uint8_t mask5 = 0,         /*  4 bits */
                      uint8_t mask4 = 0)         /*  4 bits */
      : S390VRIInstruction(cg, op, n, targetReg, (constantImm3 << 4), 0, mask4,
                           mask5) {
    // Error Checking
    TR_ASSERT((constantImm3 & 0xf000) == 0,
              "Incorrect length in immediate value");
    if (getOpCode().setsOperand2())
      useTargetRegister(sourceReg2);
    else
      useSourceRegister(sourceReg2);
...
protected:
  S390VStorageInstruction(TR::CodeGenerator *cg = NULL,
                          TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                          TR::Node *n = NULL,
  ...
  S390VStorageInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
                          ...
                          TR::Instruction *precedingInstruction)
      : S390VInstruction(cg, op, n, targetReg, precedingInstruction),
        _displacement2(0), maskField(mask) {
    if (sourceReg)
      useSourceRegister(sourceReg);
    useSourceMemoryReference(mr);
    setupThrowsImplicitNullPointerException(n, mr);
...
public:
  ...
  //   virtual int32_t estimateBinaryLength(int32_t currentEstimate);
  //   virtual uint8_t * generateBinaryEncoding();
  //   virtual bool getUsedRegisters(CS2::ListOf<TR::Register*, TR::Allocator>&
  //   usedRegs);
  //   virtual bool getDefinedRegisters(CS2::ListOf<TR::Register *,
  //   TR::Allocator> & defedRegs);
  //   virtual bool getKilledRegisters(CS2::ListOf<TR::Register *,
  //   TR::Allocator> & killedRegs);
...
class S390NOPInstruction : public TR::Instruction {
  ...
  enum KindNOP { UnknownNOP = 0, XPLinkCallNOP, FastLinkCallNOP };
  KindNOP _kindNOP;
...
public:
  ...
  S390NOPInstruction(TR::InstOpCode::Mnemonic op, int32_t numbytes,
                     TR::Snippet *ts, TR::Node *n, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg) {
    setTargetSnippet(ts);
    setBinaryLength(numbytes);
    setEstimatedBinaryLength(numbytes);
  ...
  /** Fastlink flavor */
  S390NOPInstruction(TR::InstOpCode::Mnemonic op, int32_t numbytes,
                     int32_t argumentsLengthOnCall, TR::Node *n,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg) {
    setTargetSnippet(NULL);
    setBinaryLength(numbytes);
  ...
  S390PseudoInstruction *getCallDescInstr() { return _callDescInstr; }
  S390PseudoInstruction *setCallDescInstr(S390PseudoInstruction *cdi) {
    return _callDescInstr = cdi;
  }

  int8_t getCallType() { return _callType; }
  void setCallType(uint8_t callType) { _callType = callType; }

  void setArgumentsLengthOnCall(int32_t argumentsLengthOnCall) {
    _argumentsLengthOnCall = argumentsLengthOnCall;
  }
  int32_t getArgumentsLengthOnCall() { return _argumentsLengthOnCall; }
  enum KindNOP getKindNOP() { return _kindNOP; }
  void setKindNOP(KindNOP kindNOP) { _kindNOP = kindNOP; }
...
public:
  ...
  /**
   * Following constructor is for PFPO instructino, where FPR0/FPR2 and GPR1 are
   * target registers;
   * FPR4/FPR6 and GPR0 are source registers;
   * they are implied by the opcode, purpose of adding these is to notify RA the
   * reg use/def dependencies
   */
  S390EInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
