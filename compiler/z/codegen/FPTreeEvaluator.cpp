...
inline void genLogicalConversionForInt(TR::Node *node, TR::CodeGenerator *cg,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    if (cg->getS390ProcessorInfo()->supportsArch(
    ...
    generateRIEInstruction(cg, TR::InstOpCode::RISBLG, node, targetRegister,
                           targetRegister, 32 + shift_amount,
                           (int8_t)(63 | 0x80), 0);
  }
}
...
inline TR::Register *convertToFixed(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  case TR::Float:
    ...
    roundToIntOp = TR::InstOpCode::FIDBR; // Double versions. Unsigned
                                          // conversion will need to use long
                                          // form
  ...
  if (isUnsignedConversion) {
    if (floatToIntLogicalConverted) {
       ...
       * means that MSB will be 0 for value >= 2^31.
       *                 If value < 2^31, MSB will be 1 due to borrow and lower
       * bits will NOT be touched.
       *                 The XOR MSB test will put the sign back if original
       * exceeded max integral value.
       *                 xor will take care of -inf<a<min case (result is min
       * signed value as above)
...
inline TR::Register *convertFromFixed(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  case TR::s2d:
    // case TR_SInt16:
    nshift =
        (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) ? 48 : 16;
  ...
  case TR::b2d:
    // case TR_SInt8    :
    nshift =
        (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) ? 56 : 24;
    genArithmeticConversionForInt(node, cg, srcRegister, nshift);
    break;
  case TR::bu2f:
  case TR::bu2d:
 ...
 * Handles l2f,l2d,l2e
 */
TR::Register *commonLong2FloatEvaluator(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  TR::RegisterDependencyConditions *deps =
      depsNeeded > 0 ? new (cg->trHeapMemory())
                           TR::RegisterDependencyConditions(0, depsNeeded, cg)
                     : NULL;
  TR::Node *srcNode = node->getFirstChild();
  TR::Register *srcReg = cg->evaluate(srcNode);
  TR::Register *gprTemp64 = cg->allocate64bitRegister();

  // the 64 bit register gprTemp64 is going to be clobbered and R0 is safe to
  // clobber (so are R1,R15)
  if (restrictToGPR0)
    deps->addPostCondition(gprTemp64, TR::RealRegister::GPR0);
  generateRSInstruction(cg, TR::InstOpCode::SLLG, node, gprTemp64,
                        srcReg->getHighOrder(), 32);
  TR::Instruction *cursor = generateRRInstruction(
      cg, TR::InstOpCode::LR, node, gprTemp64, srcReg->getLowOrder());
  ...
  TR::LabelSymbol *doneLabel =
      deps ? TR::LabelSymbol::create(cg->trHeapMemory(), cg)
           : NULL; // attach all deps to this label at end
  if (isUnsigned) {
    TR::Register *litBase = NULL;
    bool stopUsingMemRefRegs = false;
    if (litBaseNode) {
    ...
    if (is128) {
      ...
      deps->addPostCondition(targetReg->getHighOrder(),
                             TR::RealRegister::LegalFirstOfFPPair);
      deps->addPostCondition(targetReg->getLowOrder(),
                             TR::RealRegister::LegalSecondOfFPPair);

      tempFloatReg = cg->allocateFPRegisterPair();

      deps->addPostCondition(tempFloatReg, TR::RealRegister::FPPair);
      deps->addPostCondition(tempFloatReg->getHighOrder(),
                             TR::RealRegister::LegalFirstOfFPPair);
      deps->addPostCondition(tempFloatReg->getLowOrder(),
                             TR::RealRegister::LegalSecondOfFPPair);
      two_pow_64_hi = generateS390MemoryReference(
          (int64_t)CONSTANT64(0x403f000000000000), TR::Int64, cg, litBase);
      two_pow_64_lo =
          generateS390MemoryReference((int64_t)0, TR::Int64, cg, litBase);
    } else { // float add -2^63 + 2^64 triggers inexact exception in 31bit.
             // Workaround is lengthen to double and add

      deps->addPostCondition(targetReg, TR::RealRegister::AssignAny);
      two_pow_64_hi = generateS390MemoryReference(
          (int64_t)CONSTANT64(0x43f0000000000000), TR::Int64, cg, litBase);
    }
    ...
    // the fixed to float convertOp only handles signed source operands so if
    // the converted value is negative and the
    ...
    generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, startLabel,
                                 deps);
    TR::Instruction *cursor = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNLRC, node, doneLabel);
    if (is128) {
      generateRXInstruction(cg, TR::InstOpCode::LD, node,
                            tempFloatReg->getHighOrder(), two_pow_64_hi);
      generateRXInstruction(cg, TR::InstOpCode::LD, node,
                            tempFloatReg->getLowOrder(), two_pow_64_lo);
      generateRRInstruction(cg, addOp, node, targetReg, tempFloatReg);
    } else if (is64) {
      generateRXInstruction(cg, addOp, node, targetReg, two_pow_64_hi);
    } else {
      // float add -2^63 + 2^64 triggers inexact exception in 31bit. Workaround
      // is lengthen to double and add
      addOp = TR::InstOpCode::ADB;
      generateRRInstruction(cg, TR::InstOpCode::LDEBR, node, targetReg,
                            targetReg);
      generateRXInstruction(cg, addOp, node, targetReg, two_pow_64_hi);
      generateRRInstruction(cg, TR::InstOpCode::LEDBR, node, targetReg,
                            targetReg);
    }
...
TR::Register *OMR::Z::TreeEvaluator::floadEvaluator(TR::Node *node,
  ...
  // traceMsg(comp,"In fload evaluator for Node %p. isRegisterSymbol =
...
TR::Register *OMR::Z::TreeEvaluator::floatRemHelper(TR::Node *node,
  ...
  if (node->getDataType() == TR::Float) {
    ...
    cursor = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BCRC, node,
        labelNotExact); // remainder not exact
    cursor->setStartInternalControlFlow();
    generateRXInstruction(cg, TR::InstOpCode::TCEB, node, targetRegister,
                          0x800); // c is +0 ?
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BERC, node,
                                  labelOK); // it is not +0
    generateRXInstruction(cg, TR::InstOpCode::TCEB, node, firstRegister,
                          0x555); // a is < +0 ?
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BERC, node,
                                  labelOK); // it is not < +0
    ...
    cursor = generateS390BranchInstruction(
        ...
        labelNotExact); // remainder not exact
    cursor->setStartInternalControlFlow();
    generateRXInstruction(cg, TR::InstOpCode::TCDB, node, targetRegister,
                          0x800); // c is +0 ?
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BERC, node,
                                  labelOK); // it is not +0
    generateRXInstruction(cg, TR::InstOpCode::TCDB, node, firstRegister,
                          0x555); // a is < +0 ?
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BERC, node,
                                  labelOK); // it is not < +0
  ...
  void *remHelper;
  if (node->getDataType() == TR::Float)
    remHelper = (void *)cg->symRefTab()
                    ->findOrCreateRuntimeHelper(TR_S390floatRemainder, false,
                                                false, false)
                    ->getMethodAddress();
  else
    remHelper = (void *)cg->symRefTab()
                    ->findOrCreateRuntimeHelper(TR_S390doubleRemainder, false,
                                                false, false)
                    ->getMethodAddress();

  TR::RegisterDependencyConditions *postDeps =
      generateRegisterDependencyConditions(0, 12, cg);

  genLoadAddressConstant(cg, node, (uintptrj_t)remHelper, raReg, NULL, postDeps,
                         NULL);
...
TR::Register *OMR::Z::TreeEvaluator::fbits2iEvaluator(TR::Node *node,
  ...
  TR::LabelSymbol *infinityNumber =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *negativeInfinityNumber =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *positiveInfinityNumber =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *cleansedNumber =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::Register *litBase = NULL;
  ...
  generateRXInstruction(cg, TR::InstOpCode::TCEB, node, sourceReg,
                        (uint32_t)0x03f);
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BZRC, node, cleansedNumber);
  cursor->setStartInternalControlFlow();
  TR::MemoryReference *positiveInfinity =
      generateS390MemoryReference((int32_t)0x7f800000, TR::Int32, cg, litBase);
  TR::MemoryReference *negativeInfinity =
      generateS390MemoryReference((int32_t)0xff800000, TR::Int32, cg, litBase);
  TR::MemoryReference *NaN =
      generateS390MemoryReference((int32_t)0x7fc00000, TR::Int32, cg, litBase);
  if (litBase) {
    deps->addPostCondition(litBase, TR::RealRegister::AssignAny);
  }
  generateRXInstruction(cg, TR::InstOpCode::TCEB, node, sourceReg,
  ...
  generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cleansedNumber,
                               deps);
  ...
  if ((TR::Compiler->target.cpu.getS390SupportsFPE()) && (!disabled)) {
    ...
    generateRSInstruction(cg, TR::InstOpCode::SRLG, node, targetReg, targetReg,
                          32);
    node->setRegister(targetReg);
    cg->decReferenceCount(node->getFirstChild());
    if (node->getNumChildren() == 2)
...
TR::Register *OMR::Z::TreeEvaluator::lbits2dEvaluator(TR::Node *node,
  ...
  if ((TR::Compiler->target.cpu.getS390SupportsFPE()) && (!disabled)) {
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
      sourceReg = cg->evaluate(firstChild);
      TR::Register *targetReg = cg->allocateRegister(TR_FPR);
      generateRRInstruction(cg, TR::InstOpCode::LDGR, node, targetReg,
                            sourceReg);
      node->setRegister(targetReg);
      cg->decReferenceCount(firstChild);
      return targetReg;
    } else {
      TR_ASSERT(false, "must ensure sourceReg->getHighOrder() is a 64BitReg in "
                       "SLLG so local RA will use 64 bit instructions\n");
      TR::Register *targetReg = cg->allocateRegister(TR_FPR);
      ...
      ldgrDeps->addPostCondition(sourceReg->getHighOrder(),
                                 TR::RealRegister::LegalEvenOfPair);
      ldgrDeps->addPostCondition(sourceReg->getLowOrder(),
                                 TR::RealRegister::LegalOddOfPair);
      generateRSInstruction(cg, TR::InstOpCode::SLLG, node,
                            sourceReg->getHighOrder(),
                            sourceReg->getHighOrder(), 32);
      generateRRInstruction(cg, TR::InstOpCode::LR, node,
                            sourceReg->getHighOrder(),
                            sourceReg->getLowOrder());
      TR::Instruction *cursor = generateRRInstruction(
          cg, TR::InstOpCode::LDGR, node, targetReg, sourceReg);
...
TR::Register *OMR::Z::TreeEvaluator::dbits2lEvaluator(TR::Node *node,
  ...
  TR::MemoryReference *positiveInfinity = generateS390MemoryReference(
      (int64_t)CONSTANT64(0x7ff0000000000000), TR::Int64, cg, litBase);
  TR::MemoryReference *negativeInfinity = generateS390MemoryReference(
      (int64_t)CONSTANT64(0xfff0000000000000), TR::Int64, cg, litBase);
  TR::MemoryReference *NaN = generateS390MemoryReference(
      (int64_t)CONSTANT64(0x7ff8000000000000), TR::Int64, cg, litBase);
  if ((TR::Compiler->target.cpu.getS390SupportsFPE()) && (!disabled)) {
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
      TR::RegisterDependencyConditions *deps =
          new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 2, cg);
      ...
      sourceReg = cg->evaluate(firstChild);
      cleansedNumber->setEndInternalControlFlow();
      generateRXInstruction(cg, TR::InstOpCode::TCDB, node, sourceReg,
                            (uint32_t)0x03f);
      cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                             TR::InstOpCode::COND_BZRC, node,
                                             cleansedNumber);
      ...
      generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                   negativeInfinityNumber);
      generateRXInstruction(cg, TR::InstOpCode::LD, node, sourceReg,
                            negativeInfinity);
      generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                    TR::InstOpCode::COND_BRC, node,
                                    cleansedNumber);
      ...
      generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                   cleansedNumber, deps);
      generateRRInstruction(cg, TR::InstOpCode::LGDR, node, targetReg,
                            sourceReg);
      if (node->getNumChildren() == 2) {
        cg->decReferenceCount(node->getSecondChild());
      }
      ...
      TR::Register *targetReg =
          cg->allocateConsecutiveRegisterPair(oddReg, evenReg);
      ...
      cursor = generateRSInstruction(cg, TR::InstOpCode::SRLG, node, evenReg,
                                     evenReg, 32);
      lgdrDeps =
          new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 3, cg);
...
#ifdef ZOS_LOAD
    targetReg =
        genericLoadHelper<64, 64, MemReg>(node, cg, tempMR1, NULL, false, true);
    node->setRegister(targetReg);
...
TR::Register *l2dHelper64(TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(),
            "l2dHelper64() is for 64bit code-gen only!");
  TR::Instruction *cursor;
  TR::Node *firstChild = node->getFirstChild();
...
TR::Register *l2fHelper64(TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(),
            "l2fHelper64() is for 64bit code-gen only!");
  TR::Instruction *cursor;
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *longRegister = cg->evaluate(firstChild);
  TR::Register *targetFloatRegister = cg->allocateRegister(TR_FPR);
...
TR::Register *OMR::Z::TreeEvaluator::l2fEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    ...
    node->setSymbolReference(cg->symRefTab()->findOrCreateRuntimeHelper(
        TR_S390jitMathHelperConvertLongToFloat, false, false, false));
...
TR::Register *OMR::Z::TreeEvaluator::l2dEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  return (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit())
             ? l2dHelper64(node, cg)
             : commonLong2FloatEvaluator(node, cg);
}
...
TR::Register *f2lHelper(TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is32Bit(),
            "f2lHelper() is for 32bit code-gen only!");
  ...
  TR::Register *targetRegisterPair =
  ...
  dependencies->addPostCondition(evenRegister,
                                 TR::RealRegister::LegalEvenOfPair);
  dependencies->addPostCondition(oddRegister, TR::RealRegister::LegalOddOfPair);

  // We need to assign these registers early so that any spilling will occur at
  // the
  // bottom of the loop.  Because the pair lives across the backedge, register
  // pressure
  ...
  dependencies->addPostCondition(tempFloatRegister,
  ...
  cursor =
      generateRRFInstruction(cg, TR::InstOpCode::FIEBR, node, tempFloatRegister,
                             floatRegister, (int8_t)0x5, true);
  cursor = generateRXInstruction(cg, TR::InstOpCode::TCEB, node,
                                 tempFloatRegister, (uint32_t)0xccf);
  // if result 0, done
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BMRC, node, label7);
  cursor->setStartInternalControlFlow();
  TR::MemoryReference *evenMRcopy = generateS390MemoryReference(
      *evenMR, 0, cg); // prevent duplicate use of memory reference
  cursor = generateRXInstruction(cg, TR::InstOpCode::STE, node,
                                 tempFloatRegister, evenMRcopy);
  // Test if NaN
  cursor = generateRXInstruction(cg, TR::InstOpCode::TCEB, node,
                                 tempFloatRegister, (uint32_t)0x30);
  // If NaN , go to label6
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, node, label6);
  // Load as positive FP
  cursor = generateRRInstruction(cg, TR::InstOpCode::LPEBR, node,
                                 tempFloatRegister, tempFloatRegister);
  cursor = generateRXInstruction(cg, TR::InstOpCode::STE, node,
                                 tempFloatRegister, oddMR);
  TR::MemoryReference *oddMRcopy = generateS390MemoryReference(
      *oddMR, 0, cg); // prevent duplicate use of memory reference
  cursor = generateRXInstruction(cg, TR::InstOpCode::L, node, oddRegister,
                                 oddMRcopy);
  ...
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNLRC, node, label6);
  // clear exponent bits, odd register will have only fraction
  cursor = generateS390ImmOp(cg, TR::InstOpCode::N, node, oddRegister,
                             oddRegister, (int32_t)0x007fffff, dependencies);
  cursor =
  ...
  cursor = generateRRInstruction(cg, TR::InstOpCode::OR, node, oddRegister,
                                 tempRegister2);
  ...
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node, label5);
  // if exponent value is >23, go to label3
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BHRC, node, label3);
  // exponent value is less than 23
  cursor = generateRRInstruction(cg, TR::InstOpCode::LCR, node, tempRegister,
                                 tempRegister);
  ...
  cursor = generateRSInstruction(cg, TR::InstOpCode::SRDL, node,
                                 targetRegisterPair, 1);
  cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRCT, node,
                                         tempRegister, label2);

  // go to label5
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, label5);
  ...
  TR::MemoryReference *evenMRcopy2 = generateS390MemoryReference(
      *evenMR, 0, cg); // prevent duplicate use of memory reference
  cursor = generateSIInstruction(cg, TR::InstOpCode::TM, node, evenMRcopy2,
                                 (uint32_t)0x00000080);
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BZRC, node, label7);

  cursor = generateRRInstruction(cg, TR::InstOpCode::LCR, node, evenRegister,
                                 evenRegister);
  cursor = generateRRInstruction(cg, TR::InstOpCode::LCR, node, oddRegister,
                                 oddRegister);

  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node, label7);
  cursor =
      generateRIInstruction(cg, TR::InstOpCode::AHI, node, evenRegister, -1);
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, label7);
  ...
  cursor = generateRSInstruction(cg, TR::InstOpCode::SLDL, node,
                                 targetRegisterPair, 63);
  TR::MemoryReference *evenMRcopy3 = generateS390MemoryReference(
      *evenMR, 0, cg); // prevent duplicate use of memory reference
  cursor = generateSIInstruction(cg, TR::InstOpCode::TM, node, evenMRcopy3,
                                 (uint32_t)0x00000080);
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BORC, node, label7);
  ...
  cursor = generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, label7,
                                        dependencies);
...
TR::Register *f2lHelper64(TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(),
            "f2lHelper64() is for 64bit code-gen only!");
  TR::Instruction *cursor;
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *floatRegister = cg->evaluate(firstChild);
  ...
  doneLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  cursor = generateRRInstruction(cg, TR::InstOpCode::CEBR, node, floatRegister,
                                 floatRegister);
  // if NaN, then done
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BORC, node, doneLabel);
...
TR::Register *d2lHelper(TR::Node *node, TR::CodeGenerator *cg) {
  TR::Instruction *cursor;
  ...
  generateRRFInstruction(cg, TR::InstOpCode::FIDBR, node, tempFloatRegister,
                         floatRegister, (int8_t)0x5, true);
  generateRXInstruction(cg, TR::InstOpCode::TCDB, node, tempFloatRegister,
                        (uint32_t)0xccf);
  // if 0 then done
  cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, node, label7);
  cursor->setStartInternalControlFlow();

  generateRXInstruction(cg, TR::InstOpCode::STE, node, tempFloatRegister,
                        tempMR1);
  generateRXInstruction(cg, TR::InstOpCode::TCDB, node, tempFloatRegister,
                        (uint32_t)0x30);
  // If NaN, go to label6
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_BLRC, node, label6);
  // Load as positive
  generateRRInstruction(cg, TR::InstOpCode::LPDBR, node, tempFloatRegister,
                        tempFloatRegister);
  TR::MemoryReference *tempMR2copy =
      generateS390MemoryReference(*tempMR2, 0, cg);
  generateRXInstruction(cg, TR::InstOpCode::STD, node, tempFloatRegister,
                        tempMR2copy);
  TR::MemoryReference *tempMR2copy2 =
      generateS390MemoryReference(*tempMR2, 0, cg);
  generateRSInstruction(cg, TR::InstOpCode::LM, node, targetRegisterPair,
                        tempMR2copy2);
  ...
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_BNLRC, node, label6);
  // clear exponent bits, evenRegister will have only fraction

  generateS390ImmOp(cg, TR::InstOpCode::N, node, evenRegister, evenRegister,
                    (int32_t)0x000fffff, dependencies);
  generateRIInstruction(cg, TR::InstOpCode::LA, node, tempRegister2, 1);
  generateRSInstruction(cg, TR::InstOpCode::SLL, node, tempRegister2, 20);
  generateRRInstruction(cg, TR::InstOpCode::OR, node, evenRegister,
                        tempRegister2);
  ...
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_BERC, node, label5);
  // exponent > 52 -> go to label3
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_BHRC, node, label3);
  generateRRInstruction(cg, TR::InstOpCode::LCR, node, tempRegister,
                        tempRegister);
  ...
  generateS390BranchInstruction(cg, TR::InstOpCode::BRCT, node, tempRegister,
                                label2);
  // go to label5
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_BRC, node, label5);
  ...
  // LABEL5
  generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, label5);
  TR::MemoryReference *tempMR1copy =
      generateS390MemoryReference(*tempMR1, 0, cg);
  generateSIInstruction(cg, TR::InstOpCode::TM, node, tempMR1copy,
                        (uint32_t)0x00000080);
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_BZRC, node, label7);
  generateRRInstruction(cg, TR::InstOpCode::LCR, node, evenRegister,
                        evenRegister);
  generateRRInstruction(cg, TR::InstOpCode::LCR, node, oddRegister,
                        oddRegister);
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_BERC, node, label7);
  generateRIInstruction(cg, TR::InstOpCode::AHI, node, evenRegister, -1);
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_BRC, node, label7);
  // LABEL6
  generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, label6);
  ...
  TR::MemoryReference *tempMR1copy2 =
      generateS390MemoryReference(*tempMR1, 0, cg);
  generateSIInstruction(cg, TR::InstOpCode::TM, node, tempMR1copy2,
                        (uint32_t)0x00000080);
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_BORC, node, label7);
  generateRIInstruction(cg, TR::InstOpCode::AHI, node, evenRegister, -1);
  generateRIInstruction(cg, TR::InstOpCode::AHI, node, oddRegister, -1);
...
TR::Register *d2lHelper64(TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(),
            "d2lHelper64(...) is for 64bit code-gen only");
  bool checkNaN = true;
  ...
  if (checkNaN) {
    ...
    // if NaN
    generateRXInstruction(cg, TR::InstOpCode::TCDB, node, floatRegister,
                          (uint32_t)0x00f);

    TR::Instruction *cursor = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNZRC, node, label1);
    cursor->setStartInternalControlFlow();
...
TR::Register *OMR::Z::TreeEvaluator::iffcmpeqEvaluator(TR::Node *node,
  ...
  return generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                                   TR::InstOpCode::COND_BERC,
                                   TR::InstOpCode::COND_BERC, false);
}

TR::Register *OMR::Z::TreeEvaluator::iffcmpneEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                                   TR::InstOpCode::COND_BNERC,
                                   TR::InstOpCode::COND_BNERC, false);
}

TR::Register *OMR::Z::TreeEvaluator::iffcmpltEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                                   TR::InstOpCode::COND_BLRC,
                                   TR::InstOpCode::COND_BHRC, false);
}

TR::Register *OMR::Z::TreeEvaluator::iffcmpgeEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                                   TR::InstOpCode::COND_BNLRC,
                                   TR::InstOpCode::COND_BNHRC, false);
}

TR::Register *OMR::Z::TreeEvaluator::iffcmpgtEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                                   TR::InstOpCode::COND_BHRC,
                                   TR::InstOpCode::COND_BLRC, false);
}
...
TR::Register *OMR::Z::TreeEvaluator::iffcmpleEvaluator(TR::Node *node,
  ...
  return generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                                   ...
                                   TR::InstOpCode::COND_BNLRC, false);
}
