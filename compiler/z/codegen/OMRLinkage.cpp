...
OMR::Z::Linkage::Linkage(TR::CodeGenerator *codeGen,
    ...
    : OMR::Linkage(), _codeGen(codeGen), _explicitLinkageType(elc),
      ...
      _firstPrologueInstr(NULL), _frameType(standardFrame) {
...
bool OMR::Z::Linkage::restoreRegister(TR::RealRegister::RegNum reg,
                                      int32_t blockNumber) {
...
uint8_t OMR::Z::Linkage::getRestoreBump(TR::RealRegister::RegNum firstSaved,
  ...
  for (int8_t curReg = firstReg; curReg != lastReg; curReg += dir) {
    ...
    if (!self()->restoreRegister(REGNUM(curReg), blockNumber)) {
      ...
      if ((curReg == firstSaved && !backwards) ||
          (curReg == lastSaved &&
           backwards)) // can't do anything without the ends
      {
        restoreEndReg = true;
      }
      stillCons = false;
...
void OMR::Z::Linkage::markPreservedRegsInDep(
    TR::RegisterDependencyConditions *deps, int32_t blockNum) {
  ...
  for (int32_t curReg = TR::RealRegister::FirstGPR;
    ...
    if (self()->getPreserved(REGNUM(curReg)) &&
      ...
      if (specializedEpilogues) {
        ...
        self()->cg()->markBlockThatModifiesRegister(rReg->getRegisterNumber(),
                                                    blockNum);
      }
    }
  }
  ...
  // and the RAReg
  if (deps &&
      deps->searchPostConditionRegister(
          self()->cg()->getReturnAddressRegister())) {
    self()
        ->getS390RealRegister(self()->getReturnAddressRegister())
        ->setModified(true);
    if (specializedEpilogues) {
      self()->cg()->markBlockThatModifiesRegister(
          self()->getReturnAddressRegister(), blockNum);
    }
  }
}
...
// heuristic to reduce the path length for static variable access in the absence
// of global optimization
bool OMR::Z::Linkage::useCachedStaticAreaAddresses(TR::Compilation *c) {
...
void OMR::Z::Linkage::analyzePrologue() {
  if (self()->cg()->specializedEpilogues()) {
    ...
    while (instr && instr != lastInstr->getNext()) {
      ...
      if (reg && reg->getKind() != TR_FPR && !reg->getRegisterPair() &&
          reg->getKind() != TR_VRF)
      ...
      // the target reg for extendedLoadOp can only be GPR
      {
        ...
        if (!instr->isStore()) {
          ...
          // example.: because we reload GPR14 after the stack overflow check
          // with specializedEpilogues
          // <bug fix> remove extended load opcode check (old code commented
          // out) as other
          // <bug fix> opcodes can set register and need to be considered
          // <bug fix> if (instr->getOpCodeValue() ==
          ...
          if (!(toRealRegister(reg)->getRegisterNumber() == raRegNum)) {
            ...
            self()->cg()->markBlockThatModifiesRegister(
                rReg->getRegisterNumber(), curBlockNum);
          }
        }
      }
      instr = instr->getNext();
...
void *OMR::Z::Linkage::saveArguments(void *cursor, bool genBinary,
  ...
  if (enableHighWordRA) {
    ...
    globalAllocatedRegisters.init(TR::RealRegister::LastHPR + 1,
                                  self()->trMemory());
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
    ...
    // preserved.
    // If it has no global reg number, locate it on local save area instead of
    // register save area
    ...
    if (self()->comp()->getOption(TR_TraceCG)) {
      traceMsg(self()->comp(), "save argument: %s, lri: %d, ai: %d, offset: "
                               "%d, isReferenced: %d, hasToBeOnStack: %d\n",
    ...
    case 8:
      if (TR::Compiler->target.is64Bit() ||
          (self()->cg()->use64BitRegsOn32Bit() && !argsAre32Bits)) {
    ...
    if (ai >= 0 && loadOpCode == TR::InstOpCode::L && enableHighWordRA &&
        self()->getS390RealRegister(REGNUM(ai))->isHighWordRegister())
    ...
    if (lri >= 0) {
      ...
      // on to the stack.
      // This query here is actually not ideal.  originally
      // isReferencedParameter (which actually means
      // isReferencedParameterAtIlGenTime) was replaced with
      // isParmHasToBeOnStack, which would make sense.
      // However, this broke some java modes (OSR,HCR).  Likely, those modes
      // should rely on 'unconditionalSave' to ensure we always store out to the
      // stack for whenever we need all variables on the stack.
      // This investigation will be a future todo .

      if (!self()->getIsLeafRoutine() &&
          ((ai < 0 && paramCursor->isReferencedParameter()) ||
            ...
            if (genBinary) {
              ...
              if (storeOpCode == TR::InstOpCode::STCM)
                cursor = generateRSInstruction(
                    ...
                    self()->getS390RealRegister(regNum), opcodeMask, mr,
                    (TR::Instruction *)cursor);

              else
                cursor =
                    generateRXInstruction(self()->cg(), storeOpCode, firstNode,
                                          self()->getS390RealRegister(regNum),
              ...
              if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                self()->removeOSCOnSavedArgument(
                    (TR::Instruction *)cursor,
                    self()->getS390RealRegister(regNum), offset);
            }
          }
          ...
          if (secondStore && fullLong && TR::Compiler->target.is32Bit() &&
            ...
            if (genBinary) {
              ...
              cursor = generateRXInstruction(
                  ...
                  self()->getS390RealRegister(REGNUM(regNum + 1)), mr,
                  (TR::Instruction *)cursor);
              ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
              if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                self()->removeOSCOnSavedArgument(
        ...
        case TR::DecimalDouble:
          ...
          if (genBinary) {
            ...
            cursor = (void *)generateRXInstruction(
                ...
                self()->getS390RealRegister(regNum), mr,
                (TR::Instruction *)cursor);
            ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
            if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
              self()->removeOSCOnSavedArgument(
        ...
        case TR::DecimalLongDouble:
          if (genBinary) {
            ...
            cursor = (void *)TR::S390CallSnippet::storeArgumentItem(
                TR::InstOpCode::STD, (uint8_t *)cursor,
                self()->getS390RealRegister(REGNUM(regNum + 2)), offset + 8,
                self()->cg());
          } else {
            TR::MemoryReference *mr = generateS390MemoryReference(
                stackPtr, offset, self()->cg(), param_name);
            ...
            cursor = (void *)generateRXInstruction(
                self()->cg(), TR::InstOpCode::STD, firstNode,
                self()->getS390RealRegister(REGNUM(regNum + 2)), hiMR,
                (TR::Instruction *)cursor);
      ...
      if (ai >= 0 && !InPreProlog) {
        ...
        // moves to fix up
        // Or we have long reg on 31bir, so we need to build the 64bit register
        // from the two
        // arguments (reg + reg) or (reg + mem)
        // also need to take care of longdouble/complex types which takes 2 or
        // more slots
        if (regNum != ai ||
            ((dtype == TR::Int64) && self()->cg()->use64BitRegsOn32Bit())) {
          ...
          if (freeScratchable.isSet(ai)) {
            if (dtype == TR::Float || dtype == TR::Double
              ...
              cursor = generateRRInstruction(
                  ...
                  self()->getS390RealRegister(REGNUM(ai)),
                  self()->getS390RealRegister(regNum),
                  (TR::Instruction *)cursor);
              ...
              int32_t ai_l =
                  paramCursor->getAllocatedLow(); //  low reg of a pair
              TR_ASSERT((ai_l == (ai + 2)),
              ...
              cursor = generateRRInstruction(
                  ...
                  self()->cg()->allocateFPRegisterPair(
                      ...
                      self()->getS390RealRegister(REGNUM(ai))),
                  self()->cg()->allocateFPRegisterPair(
                      self()->getS390RealRegister(REGNUM(regNum + 2)),
                      self()->getS390RealRegister(REGNUM(regNum))),
                  (TR::Instruction *)cursor);
              ...
              if ((dtype == TR::Int64) && self()->cg()->use64BitRegsOn32Bit()) {
                cursor = generateRSInstruction(
                    ...
                    self()->getS390RealRegister(REGNUM(ai)),
                    self()->getS390RealRegister(regNum), 32,
                    (TR::Instruction *)cursor);
                ...
                if (fullLong) {
                  cursor = generateRRInstruction(
                      ...
                      self()->getS390RealRegister(REGNUM(ai)),
                      self()->getS390RealRegister(REGNUM(regNum + 1)),
                      (TR::Instruction *)cursor);
                ...
                if (enableHighWordRA &&
                    self()
                        ->getS390RealRegister(REGNUM(ai))
                        ->isHighWordRegister()) {
                  cursor = generateExtendedHighWordInstruction(
                      firstNode, self()->cg(), TR::InstOpCode::LHLR,
                      self()->getS390RealRegister(REGNUM(ai)),
                      self()->getS390RealRegister(regNum), 0,
                      (TR::Instruction *)cursor);
                } else {
                  cursor = generateRRInstruction(
                      self()->cg(), TR::InstOpCode::getLoadRegOpCode(),
                      firstNode, self()->getS390RealRegister(REGNUM(ai)),
                      self()->getS390RealRegister(regNum),
                      (TR::Instruction *)cursor);
                }
...
//  - complex:  can pass real part in FP reg (or FP reg pair for complex long
//  double) and imag part on stack
...
//
    ...
    //
      ...
      case TR::Aggregate: // Should only happen on zLinux
        if (freeScratchable.isSet(ai)) {
          ...
          if (loadOpCode == TR::InstOpCode::ICM)
            cursor = generateRSInstruction(
                ...
                self()->getS390RealRegister(REGNUM(ai)), opcodeMask, mr,
                (TR::Instruction *)cursor);
          else
            cursor =
        ...
        if (ai_l >= 0 && TR::Compiler->target.is32Bit() &&
    ...
    for (i1 = 0; i1 < busyIndex; i1++) {
      ...
      if (!(target < 0) && (freeScratchable.isSet(target) ||
        ...
        case 0: // Reg 2 Reg
          if (enableHighWordRA &&
              self()
                  ->getS390RealRegister(REGNUM(target))
                  ->isHighWordRegister()) {
            cursor = generateExtendedHighWordInstruction(
                firstNode, self()->cg(), TR::InstOpCode::LHLR,
                self()->getS390RealRegister(REGNUM(target)),
                self()->getS390RealRegister(REGNUM(source)), 0,
                (TR::Instruction *)cursor);
          } else {
            cursor = generateRRInstruction(
                self()->cg(), TR::InstOpCode::getLoadRegOpCode(), firstNode,
                self()->getS390RealRegister(REGNUM(target)),
                self()->getS390RealRegister(REGNUM(source)),
                (TR::Instruction *)cursor);
          }
        ...
        case 1: // Int or Addr  Reg from Stack Slot
        {
          ...
          if ((TR::InstOpCode::Mnemonic)busyMoves[3][i1] == TR::InstOpCode::ICM)
            cursor = generateRSInstruction(
                self()->cg(), TR::InstOpCode::ICM, firstNode,
                self()->getS390RealRegister(REGNUM(target)), busyMoves[4][i1],
                mr, (TR::Instruction *)cursor);
            ...
            cursor = generateRXYInstruction(
                ...
                self()->getS390RealRegister(REGNUM(target)), mr,
                (TR::Instruction *)cursor);

          else
            cursor = generateRXInstruction(
                self()->cg(), (TR::InstOpCode::Mnemonic)busyMoves[3][i1],
                firstNode, self()->getS390RealRegister(REGNUM(target)), mr,
                (TR::Instruction *)cursor);
        ...
        case 6: // 64bit reg on 31bit
          cursor = generateRSInstruction(
              self()->cg(), TR::InstOpCode::SLLG, firstNode,
              self()->getS390RealRegister(REGNUM(target)),
              self()->getS390RealRegister(REGNUM(source)), 32,
              (TR::Instruction *)cursor);
        ...
        case 9: // LongDouble
          ...
          cursor = generateRXInstruction(
              self()->cg(), TR::InstOpCode::LD, firstNode,
              self()->getS390RealRegister(REGNUM(target + 2)),
        ...
        case 10: // LongDouble
          cursor = generateRRInstruction(
              ...
              self()->cg()->allocateFPRegisterPair(
                  ...
                  self()->getS390RealRegister(REGNUM(target))),
              self()->cg()->allocateFPRegisterPair(
                  self()->getS390RealRegister(REGNUM(source + 2)),
                  self()->getS390RealRegister(REGNUM(source))),
              (TR::Instruction *)cursor);
          freeScratchable.set(source);
...
void OMR::Z::Linkage::setParameterLinkageRegisterIndex(
  ...
  if (self()->isFloatParmDescriptors()) {
    ...
    xplinkInterfaceMappingFlags =
        zosLinkage->calculateInterfaceMappingFlags(method);
  }
  ...
  while ((paramCursor != NULL) &&
    ...
    case TR::DecimalFloat:
      ...
      if (numFloatArgs < self()->getNumFloatArgumentRegisters()) {
        if (self()->isFloatParmDescriptors()) { // XPLink: handle large
                                                // separation of float args
                                                // constraint
          int32_t val = TR_S390zOSSystemLinkage::getFloatParmDescriptorFlag(
              xplinkInterfaceMappingFlags, numFloatArgs);
    ...
    case TR::DecimalLongDouble:
      ...
      if ((numFloatArgs & 0x1) != 0) // if there are odd number of fp args in
                                     // before a long double arg, need to skip
                                     // one
        numFloatArgs++;
      if (numFloatArgs < self()->getNumFloatArgumentRegisters()) {
        if (self()->isFloatParmDescriptors()) { // XPLink: handle large
    ...
    if (self()->isFastLinkLinkageType()) {
      if ((numFloatArgs == 1) ||
        ...
        numFloatArgs = self()->getNumFloatArgumentRegisters(); // no more float
                                                               // args possible
                                                               // now
      }
    }
...
TR::InstOpCode::Mnemonic OMR::Z::Linkage::getOpCodeForLinkage(TR::Node *child,
  ...
  case TR::Int64:
    if (TR::Compiler->target.is32Bit())
      return (isStore) ? TR::InstOpCode::STM : (isRegReg) ? TR::InstOpCode::LR
                                                          : TR::InstOpCode::LM;
    else
      return (isStore) ? TR::InstOpCode::STG : (isRegReg) ? TR::InstOpCode::LGR
                                                          : TR::InstOpCode::LG;
    break;
  case TR::Address:
  ...
  case TR::Int32:
    if (TR::Compiler->target.is32Bit())
      return (isStore) ? TR::InstOpCode::ST : (isRegReg) ? TR::InstOpCode::LR
                                                         : TR::InstOpCode::L;
    else
      return (isStore)
  ...
  case TR::DecimalFloat:
#endif
    return (isStore) ? TR::InstOpCode::STE : (isRegReg) ? TR::InstOpCode::LER
                                                        : TR::InstOpCode::LE;
  case TR::Double:
#ifdef J9_PROJECT_SPECIFIC
  case TR::DecimalDouble:
#endif
    return (isStore) ? TR::InstOpCode::STD : (isRegReg) ? TR::InstOpCode::LDR
                                                        : TR::InstOpCode::LD;
#ifdef J9_PROJECT_SPECIFIC
  case TR::DecimalLongDouble:
    return (isStore) ? TR::InstOpCode::STD : (isRegReg) ? TR::InstOpCode::LXR
                                                        : TR::InstOpCode::LD;
#endif
  case TR::VectorInt8:
  case TR::VectorInt16:
  ...
  case TR::VectorInt64:
  case TR::VectorDouble:
    return (isStore) ? TR::InstOpCode::VST : (isRegReg) ? TR::InstOpCode::VLR
                                                        : TR::InstOpCode::VL;
  }
...
TR::Register *OMR::Z::Linkage::copyArgRegister(TR::Node *callNode,
  ...
  if (!self()->cg()->canClobberNodesRegister(child, 0)) {
    if (!(TR::Compiler->target.is64Bit()) &&
      ...
      cursor =
          generateRRInstruction(self()->cg(), copyOpCode, callNode, tempRegH,
      ...
      cursor =
          generateRRInstruction(self()->cg(), copyOpCode, callNode, tempRegL,
...
TR::Register *OMR::Z::Linkage::pushLongArg32(
  ...
  if (self()->cg()->use64BitRegsOn32Bit()) {
    ...
    generateRSInstruction(self()->cg(), TR::InstOpCode::SRLG, callNode,
                          highRegister, argRegister, 32);
    TR::Register *tempReg = argRegister;

    argRegister =
        self()->cg()->allocateConsecutiveRegisterPair(tempReg, highRegister);
...
/**
 * justClearSlot=true     means init variable slot to null only (used at start
 * of argument processing)
 * justClearSlot=false    means destroy register and clear variable slot (used
 * at end of argument processing)
 */
void OMR::Z::Linkage::clearCachedStackRegisterForOutgoingArguments(
    bool justClearSlot) {
  if (_cachedStackRegisterForOutgoingArguments != NULL) {
    if (!justClearSlot && self()->isFastLinkLinkageType())
      self()->cg()->stopUsingRegister(
          _cachedStackRegisterForOutgoingArguments); // faslink casued
    ...
    if (argRegister == NULL && child->getOpCode().isLoadConst() &&
      ...
      if (storeHelperImmediateInstruction(
              ...
              (storeOp == TR::InstOpCode::ST) ? InstOpCode::MVHI
                                              : InstOpCode::MVGHI,
              callNode, generateS390MemoryReference(
...
TR::Register *OMR::Z::Linkage::pushJNIReferenceArg(
  ...
  if (isNull) {
    ...
    TR::Instruction *br = generateS390BranchInstruction(
        self()->cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, child,
        nonNullLabel);
    ...
    TR::Instruction *labelInst = generateS390LabelInstruction(
 ...
 * each time anymore.
 */
int32_t OMR::Z::Linkage::computePreservedRegMask() {
...
OMR::Z::Linkage::buildArgs(TR::Node *callNode,
  ...
  self()->clearCachedStackRegisterForOutgoingArguments(
      true); // outgoing arguments may be accessed via stack register realized
             // lazily via getStackRegisterForOutgoingArgument()
  ...
  // sequence.
  // Simply evaluating them would be enough. Care must be taken when we begin to
  // use them,
  ...
  bool rightToLeft =
      self()->isParmsInReverseOrder() &&
      // we want the arguments for induceOSR to be passed from left to right as
      // in any other non-helper call
      callNode->getSymbolReference() !=
          self()->cg()->symRefTab()->element(TR_induceOSRAtCurrentPC);
  if (rightToLeft) {
    from = numChildren;
  ...
  if (!self()->isFirstParmAtFixedOffset()) {
    ...
    if (isXPLinkToPureOSLinkageCall) {
      ...
      // argument area
      // at offset 8 instead of 0 (to bypass collision with long displacement
      // slot)
  ...
  if (self()->isFloatParmDescriptors()) { // these flags are used to help
                                          // determine if float parm is needs to
                                          // be put in memory
    TR_ASSERT(self()->isXPLinkLinkageType(),
              "invalid platform target for float parm descriptors");
    TR_S390zOSSystemLinkage *zosLinkage = (TR_S390zOSSystemLinkage *)this;
    xplinkCallDescriptorFlags =
        zosLinkage->calculateCallDescriptorFlags(callNode);
  }
  ...
  for (i = from; (rightToLeft && i >= to) || (!rightToLeft && i <= to);
...
#endif
    {
      if (self()->isFloatParmDescriptors() &&
          (numFloatArgs <
           self()->getNumFloatArgumentRegisters())) { // XPLink: handle large
                                                      // separation of float
                                                      // args constraint
        uint32_t val = TR_S390zOSSystemLinkage::getFloatParmDescriptorFlag(
            xplinkCallDescriptorFlags, numFloatArgs);
        if (TR_S390zOSSystemLinkage::isFloatDescriptorFlagUnprototyped(val))
          self()->setProperty(AllParmsOnStack); // temporarily set so float gets
                                                // flushed to stack
      }
      argRegister = self()->pushArg(callNode, child, numIntegerArgs,
                                    numFloatArgs, &stackOffset, dependencies);
    ...
    case TR::DecimalLongDouble:
      ...
      if (self()->isFloatParmDescriptors() &&
          (numFloatArgs <
           (self()->getNumFloatArgumentRegisters() -
            1))) { // XPLink: handle large separation of float args constraint
        int32_t val = TR_S390zOSSystemLinkage::getFloatParmDescriptorFlag(
            xplinkCallDescriptorFlags, numFloatArgs);
        if (TR_S390zOSSystemLinkage::isFloatDescriptorFlagUnprototyped(val))
          self()->setProperty(AllParmsOnStack); // temporarily set so float gets
  ...
  if (self()->cg()->supportsHighWordFacility() &&
    ...
    dummyReg = NULL;
    self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                  REGNUM(TR::RealRegister::HPR0), self()->cg(),
                                  true, true);
    self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                  REGNUM(TR::RealRegister::HPR1), self()->cg(),
                                  true, true);
    self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                  REGNUM(TR::RealRegister::HPR2), self()->cg(),
                                  true, true);
    self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                  REGNUM(TR::RealRegister::HPR3), self()->cg(),
                                  true, true);
    self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                  REGNUM(TR::RealRegister::HPR14), self()->cg(),
                                  true, true);

    // consider all HPR volatile on 31-bit
    if (TR::Compiler->target.is32Bit()) {
      self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                    REGNUM(TR::RealRegister::HPR6),
                                    self()->cg(), true, true);
      self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                    REGNUM(TR::RealRegister::HPR7),
                                    self()->cg(), true, true);
      self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                    REGNUM(TR::RealRegister::HPR8),
                                    self()->cg(), true, true);
      self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                    REGNUM(TR::RealRegister::HPR9),
                                    self()->cg(), true, true);
      self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                    REGNUM(TR::RealRegister::HPR10),
                                    self()->cg(), true, true);
      self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                    REGNUM(TR::RealRegister::HPR11),
                                    self()->cg(), true, true);
      self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                    REGNUM(TR::RealRegister::HPR12),
                                    self()->cg(), true, true);
    }

    if (TR::Compiler->target.isZOS()) {
      self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                    REGNUM(TR::RealRegister::HPR15),
                                    self()->cg(), true, true);
      if (self()->cg()->supportsJITFreeSystemStackPointer()) {
        self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                      REGNUM(TR::RealRegister::HPR4),
                                      self()->cg(), true, true);
      }
    } else {
      self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
...
int64_t OMR::Z::Linkage::killAndAssignRegister(
  ...
  if (depVirReg) {
    ...
    *virtualRegPtr = depVirReg;
    ...
    if (isAllocate) {
      if (regNum >= TR::RealRegister::FirstHPR &&
          regNum <= TR::RealRegister::LastHPR)
...
void OMR::Z::Linkage::setupBuildArgForLinkage(
  ...
  if (hasGlRegDeps) {
    ...
    for (int i = 0; i < glRegDeps->getAddCursorForPre(); i++) {
      ...
      deps->addPreConditionIfNotAlreadyInserted(
          regDep->getRegister(self()->cg()), regDep->getRealRegister());
      //         TR_ASSERTC(deps->addPreConditionIfNotAlreadyInserted(regDep->getRegister(cg()),
      //         regDep->getRealRegister()), cg()->comp(),
      //                     "Duplicate precondition found while merging call
      //                     deps with global reg deps");
    }
    ...
    for (int i = 0; i < glRegDeps->getAddCursorForPost(); i++) {
      ...
      deps->addPostConditionIfNotAlreadyInserted(
          regDep->getRegister(self()->cg()), regDep->getRealRegister());
      //         TR_ASSERTC(deps->addPostConditionIfNotAlreadyInserted(regDep->getRegister(cg()),
      //         regDep->getRealRegister()), cg()->comp(),
      //                     "Duplicate postcondition found while merging call
      //                     deps with global reg deps");
    }
  }
}
...
TR::Register *OMR::Z::Linkage::buildNativeDispatch(
  ...
  // todo: can be cleaned up better (i.e, get rid of some j9 only params) once
  // other related linkage methods are cleaned up
  ...
  // TR_S390PrivateLinkage::setupBuildArgForLinkage and JNI Dispatch
...
void OMR::Z::Linkage::lockAccessRegisters() {
  // AR0/AR1 are used by gclibs, for now don't use them
  if (TR::Compiler->target.isLinux()) {
    self()
        ->cg()
        ->machine()
        ->getS390RealRegister(TR::RealRegister::AR0)
        ->setState(TR::RealRegister::Locked);
    self()
        ->cg()
        ->machine()
        ->getS390RealRegister(TR::RealRegister::AR1)
        ->setState(TR::RealRegister::Locked);
  }
}
...
void OMR::Z::Linkage::lockRegister(TR::RealRegister *lpReal) {
  ...
  if (self()->cg()->supportsHighWordFacility() &&
    ...
    TR::RealRegister *lpRealHigh =
        toRealRegister(lpReal)->getHighWordRegister();
...
void OMR::Z::Linkage::lockGPR(int32_t registerNo) {
  // +1 beacuse TR::RealRegister::GPR0 is 1
  TR::RealRegister *tempRegister =
      self()->cg()->machine()->getS390RealRegister(REGNUM(registerNo + 1));
...
void OMR::Z::Linkage::unlockGPR(int32_t registerNo) {
  // +1 beacuse TR::RealRegister::GPR0 is 1
  TR::RealRegister *tempRegister =
      self()->cg()->machine()->getS390RealRegister(REGNUM(registerNo + 1));
...
void OMR::Z::Linkage::lockAR(int32_t registerNo) {
  // registerNo is 1 less than the ARxx number.  i.e. AR05, registerNo = 4
  TR::RealRegister *tempRegister = self()->cg()->machine()->getS390RealRegister(
      REGNUM(registerNo + TR::RealRegister::FirstAR));
...
void OMR::Z::Linkage::unlockAR(int32_t registerNo) {
  // registerNo is 1 less than the ARxx number.  i.e. AR05, registerNo = 4
  TR::RealRegister *tempRegister = self()->cg()->machine()->getS390RealRegister(
      REGNUM(registerNo + TR::RealRegister::FirstAR));
...
TR::RealRegister::RegNum OMR::Z::Linkage::getFirstSavedRegister(int32_t fromreg,
  ...
  for (int32_t i = fromreg; i <= toreg; ++i) {
    if ((self()->getS390RealRegister(REGNUM(i)))
...
TR::Instruction *OMR::Z::Linkage::restorePreservedRegs(
  ...
  if (specializedEpilogues) {
    ...
    backBump =
        self()->getRestoreBump(firstUsedReg, lastUsedReg, blockNumber, true);
  }
  ...
  if (frontBump > 0) {
    rsa = generateS390MemoryReference(
        spReg, (rsa->getOffset() +
                (frontBump * self()->cg()->machine()->getGPRSize())),
        self()->cg());
  }
  ...
  if (break64 || newNumUsedRegs == 1) {
    ...
    for (uint8_t i = 0; i < newNumUsedRegs; i++) {
      if (specializedEpilogues &&
          self()->restoreRegister(REGNUM(curReg), blockNumber)) {
        cursor = generateRXInstruction(
            self()->cg(), TR::InstOpCode::getLoadOpCode(), nextNode,
            self()->getS390RealRegister(REGNUM(curReg)), rsa, cursor);
  ...
  } else // 32-bit (try to use bumps)
  {
    ...
    cursor = generateRXInstruction(
        self()->cg(), TR::InstOpCode::getLoadOpCode(), nextNode,
        self()->getS390RealRegister(REGNUM(newFirstUsedReg)), rsa, cursor);

    rsa = generateS390MemoryReference(
        spReg, rsa->getOffset() + self()->cg()->machine()->getGPRSize(),
        self()->cg());
    cursor = generateRSInstruction(
        self()->cg(), TR::InstOpCode::getLoadMultipleOpCode(), nextNode,
...
OMR::Z::Linkage::getFormalParameterGlobalRegister(TR::ParameterSymbol *sym) {
  TR_ASSERT(sym->getLinkageRegisterIndex() >= 0,
            "ParameterSymbol is not mapped to register\n");
  ...
  TR_ASSERT(grn >= 0, "There is no GlobalRegisterNumber for register %s\n",
            self()->cg()->getDebug()->getName(
                self()->cg()->machine()->getS390RealRegister(r)));
  return grn;
}

bool OMR::Z::Linkage::getIsLeafRoutine() {
  return (self()->getFrameType() == StackLeafFrame) ||
         (self()->getFrameType() == noStackLeafFrame) ||
         (self()->getFrameType() == noStackForwardingFrame);
}
...
uint8_t OMR::Z::Linkage::getNumSpecialArgumentRegisters() {
  return self()->isSpecialArgumentRegisters() ? _numSpecialArgumentRegisters
                                              : 0;
}
...
OMR::Z::Linkage::getSpecialArgumentRegister(uint32_t specialIndex) {
  if (self()->isSpecialArgumentRegisters() &&
      ((specialIndex - TR_FirstSpecialLinkageIndex) <
       _numSpecialArgumentRegisters))
    return _specialArgRegisters[specialIndex - TR_FirstSpecialLinkageIndex];
  else
    return TR::RealRegister::NoReg;
}

void OMR::Z::Linkage::setSpecialArgumentRegister(uint32_t index,
                                                 TR::RealRegister::RegNum r) {
  _specialArgRegisters[index] = r;
  self()->setRegisterFlag(r, IntegerArgument);
}

bool OMR::Z::Linkage::isSpecialArgumentRegister(int8_t linkageRegisterIndex) {
  bool result = self()->isSpecialArgumentRegisters() &&
...
TR::RealRegister *OMR::Z::Linkage::getExtCodeBaseRealRegister() {
...
TR::Compilation *OMR::Z::Linkage::comp() { return self()->cg()->comp(); }

TR_FrontEnd *OMR::Z::Linkage::fe() { return self()->cg()->fe(); }

TR_Memory *OMR::Z::Linkage::trMemory() { return self()->cg()->trMemory(); }

TR_HeapMemory OMR::Z::Linkage::trHeapMemory() { return self()->trMemory(); }

TR_StackMemory OMR::Z::Linkage::trStackMemory() { return self()->trMemory(); }
