...
/**
 * When calling the @@pcmp routine the enum below is the mapping for the
 * requested compare type
 */
...
#define ParmBlockRegister                                                      \
  0x8000 ///< Has a parameter block register: OS Linkage (non-Java)
...
enum TR_S390LinkageRelocations {
  TR_S390RelocXPLinkOffsetToEntryPtMarker =
      1 ///<XPLink specific relocation types
};
 ...
 */
inline TR_S390PrivateLinkage *toS390PrivateLinkage(TR::Linkage *l) {
...
public:
  ...
  virtual void removeOSCOnSavedArgument(TR::Instruction *instr,
                                        TR::Register *sReg, int stackOffset);
  ...
  virtual bool isAutoMappedBeforeAdjustedArea(TR::AutomaticSymbol *) {
    return false;
  }

public:
  virtual bool findPossibleCallInstruction(TR::Instruction *&cursor,
                                           int32_t &numToCheck,
                                           TR::Instruction **callInstruction,
                                           bool *regs = 0,
                                           int32_t regsSize = 0);
  virtual void setUsedRegisters(TR::Instruction *instruction, bool *regs,
                                int32_t regsSize);
...
public:
  ...
  int32_t getStrictestAutoSymbolAlignment() {
    return _strictestAutoSymbolAlignment;
  }
  ...
  int32_t isFloatParmDescriptors() {
    return _properties & FloatParmDescriptors;
  }
  ...
  int32_t isSpecialArgumentRegisters() {
    return ((_properties & SpecialArgumentRegisters) != 0) &&
           (_numSpecialArgumentRegisters > 0);
  }
  ...
  virtual void setNumSpecialArgumentRegisters(uint8_t n) {
    _numSpecialArgumentRegisters = n;
  }
  ...
  getSpecialArgumentRegister(uint32_t specialIndex);

  virtual void setSpecialArgumentRegister(uint32_t index,
                                          TR::RealRegister::RegNum r);

  virtual bool isSpecialArgumentRegister(int8_t linkageRegisterIndex);

  virtual bool
  isEnvironmentSpecialArgumentRegister(int8_t linkageRegisterIndex) {
    return false;
  }
  ...
  virtual TR_GlobalRegisterNumber
  getFormalParameterGlobalRegister(TR::ParameterSymbol *sym);

  virtual int64_t getLengthStartForSSInstruction() { return 1; }
  ...
  virtual TR::RealRegister::RegNum
  setExtCodeBaseRegister(TR::RealRegister::RegNum r) {
    return _extCodeBaseRegister = r;
  }
  virtual TR::RealRegister::RegNum getExtCodeBaseRegister() {
    return _extCodeBaseRegister;
  }
  ...
  virtual TR::RealRegister::RegNum getEnvironmentPointerRegister() {
    return TR::RealRegister::NoReg;
  }
...
protected:
  ...
  uint8_t getRestoreBump(TR::RealRegister::RegNum, TR::RealRegister::RegNum,
                         int32_t, bool backwards = false);
  int8_t getNumRegsToRestore(TR::RealRegister::RegNum, TR::RealRegister::RegNum,
                             int32_t);
