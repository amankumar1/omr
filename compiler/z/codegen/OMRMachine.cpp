...
TR_Debug *OMR::Z::Machine::getDebug() { return _cg->getDebug(); }
...
OMR::Z::Machine::registerCopy(TR::Instruction *precedingInstruction,
                              TR_RegisterKinds rk, TR::Register *targetReg,
                              TR::Register *sourceReg, TR::CodeGenerator *cg,
                              flags32_t instFlags) {
  TR::Compilation *comp = cg->comp();
  TR::Node *currentNode = precedingInstruction->getNode();
  TR::Instruction *currentInstruction = NULL;
  TR::RealRegister *targetRealReg =
      toRealRegister(targetReg->getRealRegister());
  TR::RealRegister *sourceRealReg =
      toRealRegister(sourceReg->getRealRegister());
  ...
  case TR_GPR64:
    currentInstruction =
        generateRRInstruction(cg, TR::InstOpCode::LGR, currentNode, targetReg,
                              sourceReg, precedingInstruction);

    if (sourceRealReg)
      sourceRealReg->setAssignedHigh(true);
    if (targetRealReg)
      targetRealReg->setAssignedHigh(true);
  ...
  case TR_GPR:
    currentInstruction = generateRRInstruction(
        cg, (comp->getOption(TR_ForceLargeRAMoves))
                ? TR::InstOpCode::LGR
                : TR::InstOpCode::getLoadRegOpCode(),
        currentNode, targetReg, sourceReg, precedingInstruction);
    if (enableHighWordRA && targetRealReg && sourceRealReg)
      TR_ASSERT(!targetRealReg->isHighWordRegister() &&
                    !sourceRealReg->isHighWordRegister(),
    ...
    break;
  case TR_HPR:
    // TR_ASSERTC( "Highword RA is disabled,comp, enableHighWordRA, but REG COPY
    // is working on HPR??");
    // TR_ASSERTC(comp, sourceReg->isHighWordRegister() &&
    // targetReg->isHighWordRegister(),
    //"REG COPY HPR: both target and source Regs have to be HPR!");
    if (sourceRealReg && sourceRealReg->isLowWordRegister())
      currentInstruction = generateExtendedHighWordInstruction(
          currentNode, cg, TR::InstOpCode::LHLR, targetReg, sourceReg, 0,
          precedingInstruction);
    else
      currentInstruction = generateExtendedHighWordInstruction(
          currentNode, cg, TR::InstOpCode::LHHR, targetReg, sourceReg, 0,
          precedingInstruction);
    cg->traceRAInstruction(currentInstruction);
#ifdef DEBUG
    if (debug("traceMsg90GPR") || debug("traceGPRStats")) {
  ...
  case TR_GPRL:
    // TR_ASSERTC( "Highword RA is disabled,comp, enableHighWordRA, but REG COPY
    // is working on GPR Low word??");
    // TR_ASSERTC(comp, sourceReg->isLowWordRegister() &&
    // targetReg->isLowWordRegister(),
    //"REG COPY HPR: both target and source Regs have to be Low word GPR!");
    if (sourceRealReg && sourceRealReg->isLowWordRegister())
      currentInstruction =
          generateRRInstruction(cg, TR::InstOpCode::LR, currentNode, targetReg,
                                sourceReg, precedingInstruction);
    else
      currentInstruction = generateExtendedHighWordInstruction(
          currentNode, cg, TR::InstOpCode::LLHFR, targetReg, sourceReg, 0,
          precedingInstruction);
    cg->traceRAInstruction(currentInstruction);
#ifdef DEBUG
    if (debug("traceMsg90GPR") || debug("traceGPRStats")) {
  ...
  case TR_AR:
    currentInstruction =
        generateRRInstruction(cg, TR::InstOpCode::CPYA, currentNode, targetReg,
                              sourceReg, precedingInstruction);
    cg->traceRAInstruction(currentInstruction);
    break;
  case TR_VRF:
    currentInstruction =
        generateVRRaInstruction(cg, TR::InstOpCode::VLR, currentNode, targetReg,
                                sourceReg, precedingInstruction);
    cg->traceRAInstruction(currentInstruction);
    break;
  }
  ...
  if (debugObj) {
    if (rk == TR_VRF)
      debugObj->addInstructionComment(toS390VRRaInstruction(currentInstruction),
                                      VREG_MOVE);
    else
...
TR::Instruction *OMR::Z::Machine::registerExchange(
    TR::Instruction *precedingInstruction, TR_RegisterKinds rk,
    TR::RealRegister *targetReg, TR::RealRegister *sourceReg,
    TR::RealRegister *middleReg, TR::CodeGenerator *cg, flags32_t instFlags) {
  ...
  if (rk == TR_FPR) {
    if (middleReg != NULL) {
      ...
      currentInstruction = machine->registerCopy(
          precedingInstruction, rk, sourceReg, middleReg, cg, instFlags);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction = machine->registerCopy(
          precedingInstruction, rk, targetReg, sourceReg, cg, instFlags);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction = machine->registerCopy(
          precedingInstruction, rk, middleReg, targetReg, cg, instFlags);
      cg->traceRAInstruction(currentInstruction);
    } else {
  ...
  // exchange access registers
    ...
    if (middleReg != NULL) {
      ...
      currentInstruction = generateRSInstruction(
          cg, TR::InstOpCode::STAM, currentNode, targetReg, targetReg, tempMR,
          currentInstruction);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction =
          generateRRInstruction(cg, TR::InstOpCode::CPYA, currentNode,
                                targetReg, sourceReg, currentInstruction);
      cg->traceRAInstruction(currentInstruction);
      TR::MemoryReference *tempMR2 =
          generateS390MemoryReference(*tempMR, 0, cg);
      currentInstruction =
          generateRSInstruction(cg, TR::InstOpCode::LAM, currentNode, sourceReg,
                                sourceReg, tempMR2, currentInstruction);
      cg->traceRAInstruction(currentInstruction);

      cg->freeSpill(location, 4, 0);
    }

    cg->generateDebugCounter("RegisterAllocator/Exchange/AR", 1,
                             TR::DebugCounter::Free);
  } else {
    ...
    if (middleReg == NULL) {
      ...
      if (rk == TR_GPR64) {
        ...
        location = cg->allocateSpill(TR::Compiler->om.sizeofReferenceAddress(),
                                     false, NULL); // No chance of a gcpoint
      }
      TR::MemoryReference *tempMR = generateS390MemoryReference(
          currentNode, location->getSymbolReference(), cg);
      location->getSymbolReference()->getSymbol()->setSpillTempLoaded();
      ...
      if (enableHighWordRA && rk != TR_GPR64) {
        if (srcRegIsHPR != middleRegIsHPR) {
          currentInstruction = generateRRInstruction(
              cg, srcRegIsHPR ? TR::InstOpCode::LHLR : InstOpCode::LLHFR,
              currentNode, sourceReg, middleReg, precedingInstruction);
        } else {
          currentInstruction = generateRRInstruction(
              cg, srcRegIsHPR ? TR::InstOpCode::LHHR : InstOpCode::LR,
              currentNode, sourceReg, middleReg, precedingInstruction);
        }
        cg->traceRAInstruction(currentInstruction);
        if (debugObj) {
          debugObj->addInstructionComment(
              toS390RRInstruction(currentInstruction), REG_EXCHANGE);
        }

        if (srcRegIsHPR != tgtRegIsHPR) {
          currentInstruction = generateRRInstruction(
              cg, tgtRegIsHPR ? TR::InstOpCode::LHLR : InstOpCode::LLHFR,
              currentNode, targetReg, sourceReg, precedingInstruction);
        } else {
          currentInstruction = generateRRInstruction(
              cg, tgtRegIsHPR ? TR::InstOpCode::LHHR : InstOpCode::LR,
              currentNode, targetReg, sourceReg, precedingInstruction);
        }
        cg->traceRAInstruction(currentInstruction);
        if (debugObj) {
        ...
        if (middleRegIsHPR != tgtRegIsHPR) {
          currentInstruction = generateRRInstruction(
              cg, middleRegIsHPR ? TR::InstOpCode::LHLR : InstOpCode::LLHFR,
              currentNode, middleReg, targetReg, precedingInstruction);
        } else {
          currentInstruction = generateRRInstruction(
              cg, middleRegIsHPR ? TR::InstOpCode::LHHR : InstOpCode::LR,
              currentNode, middleReg, targetReg, precedingInstruction);
        }
        ...
        if (debugObj && instFlags.testAny(PAIRREG)) {
          debugObj->addInstructionComment(
              toS390RRInstruction(currentInstruction), REG_PAIR);
        }
 ...
 */
static bool checkOOLInterference(TR::Instruction *currentInstruction,
                                 TR::Register *virtReg) {
  ...
  while (cursor != virtReg->getStartOfRange()) {
    ...
    if (cursor->isLabel() &&
        toS390LabelInstruction(cursor)
            ->getLabelSymbol()
            ->isEndOfColdInstructionStream()) {
      cursor->cg()->traceRegisterAssignment(
          "Found %R used in OOL [0x%p]: will not spill to HPR", virtReg,
          cursor);
      return true;
    }
...
///////////////////////////////////////////////////////////////////////////////
static uint32_t
updateInterference (TR::Instruction * currentInstruction, TR::Register *
virtualReg,
   ...
   while (cursor->getOpCodeValue() != TR::InstOpCode::PROC)
      ...
      if ((conditions = cursor->getDependencyConditions()) != NULL)
         ...
         for (int32_t i=first; i<=last; i++)
            {
            if ((interference & (1 << (i - maskI))) && !(checkRegList & (1 << (i
- maskI))))
               {
               TR::RealRegister::RegNum realReg = (TR::RealRegister::RegNum) i;
               TR::Register * interferedVReg =
conditions->searchPostConditionRegister(realReg);
               if (interferedVReg == NULL)
                  {
                  interferedVReg =
conditions->searchPreConditionRegister(realReg);
                  }
               if (interferedVReg != NULL)
                  {
                  // updateHW == true: remove interferences that do not clobber
HW
                  //            false: remove interferences that do not clobber
LW
                  if (interferedVReg->assignToHPR() && updateHW)
                     {
                     // if the highWord is free, this is not an interference
                     interference &= ~(1 << (i - maskI));
                     traceMsg(cursor->cg()->comp(),"removed GPR%d, does not
interfere HW\n", i - maskI);
                     }
...
OMR::Z::Machine::Machine(TR::CodeGenerator *cg)
    : OMR::Machine(NUM_S390_GPR, NUM_S390_FPR, NUM_S390_VRF), _cg(cg),
      _lastGlobalGPRRegisterNumber(-1), _last8BitGlobalGPRRegisterNumber(-1),
      _lastGlobalFPRRegisterNumber(-1), _lastGlobalCCRRegisterNumber(-1),
      _lastVolatileNonLinkGPR(-1), _lastLinkageGPR(-1),
      _lastVolatileNonLinkFPR(-1), _lastLinkageFPR(-1),
      _firstGlobalAccessRegisterNumber(-1), _lastGlobalAccessRegisterNumber(-1),
      ...
      _globalEntryPointRegisterNumber(-1), _lastGlobalHPRRegisterNumber(-1),
      _firstGlobalHPRRegisterNumber(-1) {
 ...
 *  It looks for blocked regs that are assigned dummy virt regs or for free
 * regs.
 ...
 */
uint32_t OMR::Z::Machine::genBitVectOfAssignableGPRs() {
  int32_t first = TR::RealRegister::FirstGPR;
  int32_t last = TR::RealRegister::LastAssignableGPR;
...
bool OMR::Z::Machine::isLegalEvenOddPair(TR::RealRegister *evenReg,
  ...
  if (toRealRegister(evenReg)->isHighWordRegister() ||
      toRealRegister(oddReg)->isHighWordRegister()) {
    return false;
  }
...
bool OMR::Z::Machine::isLegalEvenOddRestrictedPair(TR::RealRegister *evenReg,
                                                   TR::RealRegister *oddReg,
                                                   uint64_t availRegMask) {
  if (evenReg == NULL || oddReg == NULL) {
    return false;
...
bool OMR::Z::Machine::isLegalEvenRegister(TR::RealRegister *reg,
  ...
  if (regNum % 2 != 0 ||
      (TR::RealRegister::getBitMask(toRealRegister(reg)->getRegisterNumber()) &
       availRegMask) == 0 ||
...
OMR::Z::Machine::findBestLegalSiblingFPRegister(bool isFirst,
  ...
  if (freeRegister == NULL) {
    _cg->traceRegisterAssignment("BEST LEGAL sibling FP Reg: %R",
                                 lastSiblingFPReg);
...
TR::RealRegister *OMR::Z::Machine::findBestRegisterForShuffle(
    ...
    uint64_t availRegMask) {
  TR_ASSERT(currentAssignedRegister, "Register shuffling at instruction %P, "
                                     "but the register is not currently "
                                     "assigned?\n",
            currentInstruction);
  ...
  // instruction for shuffling
  // Usually, other real registers assigned to the current instruction are
  // block, except in situation where
  // we have exact one, non-pair target register. So temporary block it if it is
  // not blocked already
  ...
  if ((newRegister = self()->findBestFreeRegister(
           ...
           availRegMask)) == NULL) {
    newRegister =
        self()->freeBestRegister(currentInstruction, currentAssignedRegister,
  ...
  _cg->traceRegisterAssignment("%R is assigned to invalid register in this "
                               "instruction. Shuffling %R => %R...",
...
TR::RealRegister *OMR::Z::Machine::shuffleOrSpillRegister(
  ...
  if (bestRegister == NULL)
    ...
    TR::Instruction *cursor = self()->registerCopy(
        currInst, toFreeReg->getKind(), assignedRegister, bestRegister, _cg, 0);
...
TR::Register *OMR::Z::Machine::assignBestRegisterSingle(
  ...
  bool enableHighWordRA = _cg->supportsHighWordFacility() &&
                          !comp->getOption(TR_DisableHighWordRA) &&
  ...
  if (assignedRegister == NULL) {
    if (_cg->insideInternalControlFlow()) {
      TR_ASSERT(0, "ASSERTION assignBestRegisterSingle inside Internal Control "
                   "Flow on currInst=%p.\n"
                   "Ensure all registers within ICF have a dependency anchored "
                   "at the end-ICF label\n",
                currInst);
    }
  }
  if (enableHighWordRA && assignedRegister != NULL) {
    // In case we need a register shuffle, if the register we are assigning is a
    // source register of the instruction,
    ...
    if (targetRegister->is64BitReg()) {
      if ((TR::RealRegister::getBitMask(
        ...
        TR::RealRegister *newAssignedRegister =
            self()->findBestRegisterForShuffle(currInst, targetRegister,
                                               availRegMask);
        TR::Instruction *cursor = self()->registerCopy(
            appendInst, kindOfRegister, toRealRegister(assignedRegister),
            newAssignedRegister, _cg, 0);
        assignedRegister->setAssignedRegister(NULL);
        ...
        assignedRegister = newAssignedRegister;
                 ...
                 toRealRegister(assignedRegister)
                         ->getHighWordRegister()
                         ->getAssignedRegister() != targetRegister) {
        _cg->traceRegisterAssignment("%R is 64bit but HPR is assigned to a "
                                     "different vreg, freeing up HPR",
                                     targetRegister);
        if (toRealRegister(assignedRegister)
                ->getHighWordRegister()
                ->getState() == TR::RealRegister::Assigned) {
          assignedRegister->block();
          TR::Instruction *cursor = self()->freeHighWordRegister(
              currInst, toRealRegister(assignedRegister)->getHighWordRegister(),
              0);
          assignedRegister->unblock();
          _cg->traceRAInstruction(cursor);
        } else if (toRealRegister(assignedRegister)
                           ->getHighWordRegister()
                           ->getState() == TR::RealRegister::Blocked ||
                   toRealRegister(assignedRegister)
                           ->getHighWordRegister()
        ...
        _cg->traceRegisterAssignment(
            "%R is 64bit but currently assigned to HPR, shuffling",
            targetRegister);

        // find a new 64-bit register and shuffle HPR there
        assignedRegister->block();
        TR::RealRegister *assignedRegister64 =
            self()->findBestRegisterForShuffle(currInst, targetRegister,
                                               availRegMask);
        assignedRegister->unblock();
        TR::Instruction *cursor = generateExtendedHighWordInstruction(
            currInst->getNode(), _cg, TR::InstOpCode::LHLR, assignedRegister,
            assignedRegister64, 0, appendInst);
        _cg->traceRAInstruction(cursor);
        assignedRegister->setAssignedRegister(NULL);
        assignedRegister->setState(TR::RealRegister::Free);
        assignedRegister = assignedRegister64;
        // todo: in regdepds make sure to do the other half: if the register
        // comes in as 64-bit but requires an HPR, need to do shuffling
      }
      targetRegister->setAssignedRegister(assignedRegister);
      toRealRegister(assignedRegister)
          ->getHighWordRegister()
          ->setAssignedRegister(targetRegister);
      toRealRegister(assignedRegister)
          ->getLowWordRegister()
          ->setAssignedRegister(targetRegister);
      toRealRegister(assignedRegister)
          ->getHighWordRegister()
          ->setState(TR::RealRegister::Assigned);
      toRealRegister(assignedRegister)
          ->getLowWordRegister()
          ->setState(TR::RealRegister::Assigned);
    } else {
      // if the register we are assigning is a source register of the
      // instruction,
      ...
      // for 32-bit Registers, we need to make sure that the register is in the
      // correct low/high word
      if ((toRealRegister(assignedRegister))->isLowWordRegister() &&
          targetRegister->assignToHPR()) {
        // need to find a free HPR and move assignedRegister there
        TR::RealRegister *assignedHighWordRegister = NULL;

        if ((assignedHighWordRegister = self()->findBestFreeRegister(
                 currInst, kindOfRegister, targetRegister, availRegMask)) ==
            NULL) {
          // assignedRegister->block();
          assignedHighWordRegister = self()->freeBestRegister(
              currInst, targetRegister, kindOfRegister, availRegMask);
          // assignedRegister->unblock();
        }

        TR::Instruction *cursor = generateExtendedHighWordInstruction(
            currInst->getNode(), _cg, TR::InstOpCode::LLHFR, assignedRegister,
            assignedHighWordRegister, 0, appendInst);
        ...
        _cg->traceRAInstruction(cursor);
                 ...
                 targetRegister->assignToGPR()) {
        // special case for RISBG, we can change the rotate amount to shuffle
        // low word/ high word
        if (currInst->getOpCodeValue() == TR::InstOpCode::RISBG ||
            currInst->getOpCodeValue() == TR::InstOpCode::RISBGN) {
          uint8_t rotateAmnt =
              ((TR::S390RIEInstruction *)currInst)->getSourceImmediate8();
          ((TR::S390RIEInstruction *)currInst)
              ->setSourceImmediate8(rotateAmnt + 32);
        } else {
          // need to find a free GPR and move assignedRegister there
          TR::RealRegister *assignedLowWordRegister = NULL;
          if ((assignedLowWordRegister = self()->findBestFreeRegister(
                   currInst, kindOfRegister, targetRegister, availRegMask)) ==
              NULL) {
            // assignedRegister->block();
            assignedLowWordRegister = self()->freeBestRegister(
                currInst, targetRegister, kindOfRegister, availRegMask);
            // assignedRegister->unblock();
          }
          ...
          TR::Instruction *cursor = generateExtendedHighWordInstruction(
              currInst->getNode(), _cg, TR::InstOpCode::LHLR, assignedRegister,
              assignedLowWordRegister, 0, appendInst);
      ...
      } else if ((TR::RealRegister::getBitMask(
                      toRealRegister(assignedRegister)->getRegisterNumber()) &
                  availRegMask) == 0) {
        // Oh no.. targetRegister is assigned something it shouldn't be assigned
        // to. Do some shuffling
    ...
    assignedRegister->unblock();
    TR::Instruction *cursor = self()->registerCopy(
        currInst, kindOfRegister, toRealRegister(assignedRegister),
        newAssignedRegister, _cg, 0);
  ...
  if (assignedRegister == NULL) {
    // True register model will mark register with a pending restoreSpillState
    // and only when we see a def of this
    // register will we store to spill.
    if ((comp->getOption(TR_EnableTrueRegisterModel)) &&
        targetRegister->isLive() && (targetRegister->isValueLiveOnExit() ||
                                     !targetRegister->isNotUsedInThisBB()))
      targetRegister->setPendingSpillOnDef();

    // These values are only equal upon the first assignment.  Hence, if they
    // arn't
    // the same, and there is no assigned reg, we must have spilled the reg, so
    // invert
    // the spill state to get the reg back
    if (!comp->getOption(TR_EnableTrueRegisterModel) &&
        targetRegister->getTotalUseCount() !=
      ...
      if (cr && cr->getAssignedRegister() == NULL &&
          cr->getRealRegister()->getState() != TR::RealRegister::Blocked) {
      ...
      if (assignedRegister == NULL &&
          (assignedRegister = self()->findBestFreeRegister(
               currInst, kindOfRegister, targetRegister, availRegMask)) ==
              NULL) {
    ...
    if (enableHighWordRA && targetRegister->is64BitReg() && !reverseSpilled) {
      // TR_ASSERTC(comp,
      // toRealRegister(assignedRegister)->getHighWordRegister()->getState() ==
  ...
  // Handle a definition that requires the register's spill location to be
  // updated
  if (defsRegister && targetRegister->isPendingSpillOnDef()) {
    // traceMsg(cg()->comp(),"Handling ValueIsLiveOnExit() virtReg=%s
    ...
    if (self()->cg()->insideInternalControlFlow())
      self()->reverseSpillState(
          self()->cg()->getInstructionAtEndInternalControlFlow(),
          targetRegister, toRealRegister(assignedRegister));
    else
      self()->reverseSpillState(currInst, targetRegister,
                                toRealRegister(assignedRegister));
  }
  ...
  if (doBookKeeping &&
    ...
    // matter how many futureUseCount left (in the code path)
    // the register is considered as dead now in the hot path, so GC map
    // contains the correct list of live registers
    ...
    if (self()->cg()->isOutOfLineHotPath() &&
      ...
      // virtRegx,virtRegx
      // We assign target registers first, but in this case we do not want to
      // kill the virtual register
      // because the source register (which has the same virtual reg) is not yet
      // assigned
    ...
    if ((targetRegister->getFutureUseCount() == 0) || killOOLReg ||
        ((comp->getOption(TR_EnableTrueRegisterModel)) &&
         currInst->startOfLiveRange(targetRegister))) {
#if DEBUG
      if (comp->getOption(TR_EnableTrueRegisterModel) &&
          currInst->startOfLiveRange(targetRegister) &&
          targetRegister->getFutureUseCount() > 0 &&
          !currInst->getOpCode().isRegCopy()) {
        traceMsg(comp, "Start of live range for a non global virtual yet its "
                       "future count is not zero. Reg=%s Instr=[%p]\n",
                 _cg->getDebug()->getName(targetRegister), currInst);
      }
#endif
      ...
      if (enableHighWordRA && targetRegister->is64BitReg()) {
        toRealRegister(assignedRegister)
            ->getHighWordRegister()
            ->setAssignedRegister(NULL);
        toRealRegister(assignedRegister)
            ->getHighWordRegister()
            ->setState(TR::RealRegister::Free);
        _cg->traceRegFreed(
            targetRegister,
            toRealRegister(assignedRegister)->getHighWordRegister());
      }
 ...
 * passed in.
 * If both virt regs are not assigned, assign a consecutive free pair (curr we
 * do not handle
 * the case where a free cons pair is not available).
 * If either reg is already assigned, coerce assignment of respective even/odd
 * register.
 *
...
TR::Register *OMR::Z::Machine::assignBestRegisterPair(TR::Register *regPair,
  ...
  if (regPair->isArGprPair()) {
    if (_cg->getRAPassAR()) {
      ...
      TR::Register *targetReg = self()->assignBestRegisterSingle(
          regPair->getGPRofArGprPair(), currInst, doBookKeeping, availRegMask);
      currInst->addARDependencyCondition(regPair->getARofArGprPair(),
                                         targetReg);
  ...
  if (freeRegisterHigh == NULL || freeRegisterLow == NULL) {
    if (_cg->insideInternalControlFlow()) {
      TR_ASSERT(0, "ASSERTION assignBestRegisterPair inside Internal Control "
                   "Flow for inst %p.\n"
                   "Ensure all registers within ICF have a dependency anchored "
                   "at the end-ICF label\n",
                currInst);
  ...
  TR::RegisterPair *assignedRegPair =
      new (_cg->trHeapMemory(), TR_MemoryBase::RegisterPair)
  ...
  // avoid runspill of siblings
  // triping over each other.
  if (!comp->getOption(TR_EnableTrueRegisterModel) && freeRegisterLow == NULL &&
  ...
  // True register model will mark register with a pending restoreSpillState and
  // only when we see a def of this
  // register will we store to spill.
  if (freeRegisterLow == NULL &&
      (comp->getOption(TR_EnableTrueRegisterModel)) && lastReg->isLive() &&
      (lastReg->isValueLiveOnExit() || !lastReg->isNotUsedInThisBB()))
    lastReg->setPendingSpillOnDef();
  if (freeRegisterHigh == NULL &&
      (comp->getOption(TR_EnableTrueRegisterModel)) && firstReg->isLive() &&
  ...
  // invert
  // the spill state to get the reg back
  if (freeRegisterLow == NULL && !comp->getOption(TR_EnableTrueRegisterModel) &&
      (lastReg->getTotalUseCount() != lastReg->getFutureUseCount())) {
    if (freeRegisterHigh) {
  ...
  if (freeRegisterHigh == NULL) {
    ...
    // Lets check to see if partner is assigned to a legal register and
    // partner's match is free
    bool colouredIsGood = true;
    if (freeRegisterLow && TR_FPR != regPair->getKind() &&
        self()->isLegalOddRegister(freeRegisterLow, DISALLOWBLOCKED,
                                   availRegMask) &&
        toRealRegister(toRealRegister(freeRegisterLow)->getSiblingRegister())
                ->getState() == TR::RealRegister::Free)
      colouredIsGood = false; // Default algorithm should pick a better register
    else if (freeRegisterLow && TR_FPR == regPair->getKind() &&
             self()->isLegalSecondOfFPRegister(freeRegisterLow, DISALLOWBLOCKED,
                                               availRegMask) &&
             toRealRegister(
      ...
      colouredIsGood = false; // Default algorithm should pick a better register
    // if nothing is using my coloured register use it
    if (colouredIsGood && firstCr &&
        toRealRegister(firstCr)->getState() != TR::RealRegister::Blocked) {
      if (firstCr->getAssignedRegister() == NULL) {
        ...
        freeRegisterHigh = firstCr->getRealRegister();
        // make sure to block low order reg so whoever is in my register doesn't
        // get placed in my partner's reg
        freeRegisterHigh->getSiblingRegister()->block();
        self()->coerceRegisterAssignment(
            currInst, firstReg,
            toRealRegister(freeRegisterHigh)->getRegisterNumber(), PAIRREG);
  ...
  if (freeRegisterLow == NULL) {
    ...
    // partner's match is free
    bool colouredIsGood = true;
    if (freeRegisterHigh && TR_FPR != regPair->getKind() &&
        self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED,
                                    availRegMask) &&
        toRealRegister(toRealRegister(freeRegisterHigh)->getSiblingRegister())
                ->getState() == TR::RealRegister::Free)
      colouredIsGood = false; // Default algorithm should pick a better register
    else if (freeRegisterHigh && TR_FPR == regPair->getKind() &&
             self()->isLegalFirstOfFPRegister(freeRegisterHigh, DISALLOWBLOCKED,
                                              availRegMask) &&
             toRealRegister(
      ...
      colouredIsGood = false; // Default algorithm should pick a better register
    // if nothing is using my coloured register use it
    if (colouredIsGood && lastCr &&
        toRealRegister(lastCr)->getState() != TR::RealRegister::Blocked) {
      if (lastCr->getAssignedRegister() == NULL) {
        ...
        self()->coerceRegisterAssignment(
            currInst, lastReg,
            toRealRegister(freeRegisterLow)->getRegisterNumber(), PAIRREG);
  ...
  if (((regPair->getKind() != TR_FPR) && (regPair->getKind() != TR_VRF) &&
    ...
    if (enableHighWordRA && firstReg->is64BitReg()) {
      toRealRegister(freeRegisterHigh)
          ->getHighWordRegister()
          ->setState(TR::RealRegister::Assigned);
      toRealRegister(freeRegisterHigh)
          ->getHighWordRegister()
          ->setAssignedRegister(firstReg);
    }

    if (enableHighWordRA && lastReg->is64BitReg()) {
      toRealRegister(freeRegisterLow)
          ->getHighWordRegister()
          ->setState(TR::RealRegister::Assigned);
      toRealRegister(freeRegisterLow)
          ->getHighWordRegister()
          ->setAssignedRegister(lastReg);
    }
    ...
    if (!self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED,
      ...
      if (!self()->isLegalOddRegister(freeRegisterLow, DISALLOWBLOCKED,
        ...
        self()->coerceRegisterAssignment(
            currInst, lastReg, newOddReg->getRegisterNumber(), PAIRREG);
      ...
      self()->coerceRegisterAssignment(
          ...
          PAIRREG);
      freeRegisterHigh = self()->getS390RealRegister((TR::RealRegister::RegNum)(
          toRealRegister(freeRegisterLow)->getRegisterNumber() - 1));
      ...
      self()->coerceRegisterAssignment(
          ...
          PAIRREG);
      freeRegisterLow = self()->getS390RealRegister((TR::RealRegister::RegNum)(
          toRealRegister(freeRegisterHigh)->getRegisterNumber() + 1));
      ...
      if (self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED,
        ...
        self()->coerceRegisterAssignment(
            ...
            PAIRREG);
        freeRegisterLow =
            self()->getS390RealRegister((TR::RealRegister::RegNum)(
                toRealRegister(freeRegisterHigh)->getRegisterNumber() + 1));
      } else if (self()->isLegalOddRegister(freeRegisterLow, DISALLOWBLOCKED,
                                            availRegMask)) {
        if (lastReg) {
        ...
        self()->coerceRegisterAssignment(
            currInst, firstReg,
            (toRealRegister(tfreeRegisterHigh)->getRegisterNumber()), PAIRREG);
        self()->coerceRegisterAssignment(
            currInst, lastReg,
            (toRealRegister(tfreeRegisterLow)->getRegisterNumber()), PAIRREG);
    ...
    if (!self()->isLegalFirstOfFPRegister(freeRegisterHigh, DISALLOWBLOCKED,
      ...
      if (!self()->isLegalSecondOfFPRegister(freeRegisterLow, DISALLOWBLOCKED,
        ...
        self()->coerceRegisterAssignment(
            currInst, lastReg, newLowFPReg->getRegisterNumber(), PAIRREG);
      ...
      if (!self()->isLegalFirstOfFPRegister(freeRegisterHigh, DISALLOWBLOCKED,
        ...
        self()->coerceRegisterAssignment(
            currInst, firstReg, newHighFPReg->getRegisterNumber(), PAIRREG);
  ...
  // Handle a definition that requires the register's spill location to be
  // updated
  if (currInst->defsAnyRegister(firstReg) && firstReg->isPendingSpillOnDef()) {
    if (self()->cg()->insideInternalControlFlow())
      self()->reverseSpillState(
          self()->cg()->getInstructionAtEndInternalControlFlow(), firstReg,
          toRealRegister(freeRegisterHigh));
    else
      self()->reverseSpillState(currInst, firstReg,
                                toRealRegister(freeRegisterHigh));
  }
  if (currInst->defsAnyRegister(lastReg) && lastReg->isPendingSpillOnDef()) {
    if (self()->cg()->insideInternalControlFlow())
      self()->reverseSpillState(
          self()->cg()->getInstructionAtEndInternalControlFlow(), lastReg,
          toRealRegister(freeRegisterLow));
    else
      self()->reverseSpillState(currInst, lastReg,
                                toRealRegister(freeRegisterLow));
  }

  // OOL: if a register is first defined (became live) in the hot path, no
  // matter how many futureUseCount left (in the code path)
  // the register is considered as dead now in the hot path, so GC map contains
  // the correct list of live registers
  if (doBookKeeping) {
    firstReg->setIsLive();
    if (((firstReg->decFutureUseCount() == 0) ||
         ((comp->getOption(TR_EnableTrueRegisterModel)) &&
          currInst->startOfLiveRange(firstReg)) ||
    ...
    if (((lastReg->decFutureUseCount() == 0) ||
         ((comp->getOption(TR_EnableTrueRegisterModel)) &&
          currInst->startOfLiveRange(lastReg)) ||
         (self()->cg()->isOutOfLineHotPath() &&
...
bool OMR::Z::Machine::findBestFreeRegisterPair(TR::RealRegister **firstRegister,
  ...
  if (rk != TR_FPR && rk != TR_VRF) {
    ...
    for (int32_t i = first; i <= last; i += 2) {
      ...
      if (enableHighWordRA && rk == TR_GPR64) {
        ...
        if ((_registerFile[i]->getHighWordRegister()->getState() ==
                 TR::RealRegister::Free ||
             _registerFile[i]->getHighWordRegister()->getState() ==
                 TR::RealRegister::Unlatched) &&
            (_registerFile[i + 1]->getHighWordRegister()->getState() ==
                 TR::RealRegister::Free ||
             _registerFile[i + 1]->getHighWordRegister()->getState() ==
                 TR::RealRegister::Unlatched))
      ...
      if ((_registerFile[i]->getState() == TR::RealRegister::Free ||
          ...
          (_registerFile[i + 1]->getState() == TR::RealRegister::Free ||
           _registerFile[i + 1]->getState() == TR::RealRegister::Unlatched) &&
          highWordPairIsFree &&
          ((_registerFile[i]->getWeight() + _registerFile[i + 1]->getWeight()) <
           bestWeightSoFar)) {
  ...
  } else // if FP Reg Pair
  {
    ...
    for (int32_t k = 0; k < NUM_S390_FPR_PAIRS; k++) {
      ...
      if ((_registerFile[i]->getState() == TR::RealRegister::Free ||
          ...
          ((_registerFile[i]->getWeight() + _registerFile[i + 2]->getWeight()) <
           bestWeightSoFar)) {
        freeRegisterHigh = _registerFile[i];
  ...
  if (enableHighWordRA && rk == TR_GPR64) {
    // If unlatched, set it free
    if (freeRegisterHigh != NULL &&
        freeRegisterHigh->getHighWordRegister()->getState() ==
            TR::RealRegister::Unlatched) {
    ...
    // If unlatched, set it free
    if (freeRegisterLow != NULL &&
        freeRegisterLow->getHighWordRegister()->getState() ==
            TR::RealRegister::Unlatched) {
  ...
  if (freeRegisterLow != NULL && freeRegisterHigh != NULL) {
    ...
    *lastRegister = freeRegisterLow;
    ...
    _cg->traceRegisterAssignment("BEST FREE PAIR: (%R, %R)", freeRegisterHigh,
                                 freeRegisterLow);
    return true;
  } else {
    return false;
...
void OMR::Z::Machine::freeBestFPRegisterPair(TR::RealRegister **firstReg,
  ...
  if (bestVirtCandidateLow != NULL) {
    ...
    // register will we store to spill.
    if ((comp->getOption(TR_EnableTrueRegisterModel)) &&
        (bestVirtCandidateLow->isValueLiveOnExit() ||
         !bestVirtCandidateLow->isNotUsedInThisBB()))
    ...
    if (!comp->getOption(TR_DisableOOL)) {
      if (!self()->cg()->isOutOfLineColdPath()) {
        ...
        // before entering
        // the OOL cold path, post dependencies will be generated using this
        // list
        ...
        // the spill slot
        // because the post condition at OOL entry does not expect this register
        // to be spilled
        if (locationLow->getMaxSpillDepth() != 1 &&
  ...
  if (bestVirtCandidateHigh != NULL) {
    ...
    // register will we store to spill.
    if ((comp->getOption(TR_EnableTrueRegisterModel)) &&
        (bestVirtCandidateHigh->isValueLiveOnExit() ||
         !bestVirtCandidateHigh->isNotUsedInThisBB()))
...
void OMR::Z::Machine::freeBestRegisterPair(TR::RealRegister **firstReg,
  ...
  if (bestCandidateHigh == NULL && bestCandidateLow == NULL) {
    self()->cg()->getDebug()->printGPRegisterStatus(comp->getOutFile(),
                                                    machine);
    traceMsg(comp, "OMR::Z::Machine::freeBestRegisterPair -- Ran out of regs "
                   "to use as pair on Inst %p.\n",
             currInst);

    TR_ASSERT(0, "OMR::Z::Machine::freeBestRegisterPair -- Ran out of regs to "
                 "use as a pair on Inst %p.\n",
              currInst);
  }
  ...
  if (enableHighWordRA && rk == TR_GPR64) {
    ...
    TR::RealRegister::RegNum hprHigh =
        bestCandidateHigh->getHighWordRegister()->getRegisterNumber();
    TR::RealRegister::RegNum hprLow =
        bestCandidateLow->getHighWordRegister()->getRegisterNumber();
  ...
  if (bestVirtCandidateLow != NULL) {
    ...
    // if we selected the VM Thread Register to be freed, check to see if the
    // value has already been spilled
    locationLow = bestVirtCandidateLow->getBackingStorage();
    if (_cg->needsVMThreadDependency() &&
        bestVirtCandidateLow == _cg->getVMThreadRegister()) {
      traceMsg(comp,
      ...
      if (bestVirtCandidateLow->getBackingStorage() == NULL) {
        ...
        traceMsg(comp, "\ns390machine: allocateVMThreadSpill call completed\n");
               ...
               !bestVirtCandidateLow->containsInternalPointer()) {
      if (bestVirtCandidateLow->getKind() == TR_GPR64) {
        locationLow = self()->cg()->allocateSpill(
            8, bestVirtCandidateLow->containsCollectedReference(), NULL, true);
        opCodeLow = TR::InstOpCode::LG;
      } else {
        if (enableHighWordRA) {
          if (bestVirtCandidateLow->is64BitReg()) {
            locationLow = self()->cg()->allocateSpill(
                8, bestVirtCandidateLow->containsCollectedReference(), NULL,
                true);
            opCodeLow = TR::InstOpCode::LG;
          } else {
            locationLow = self()->cg()->allocateSpill(
                4, bestVirtCandidateLow->containsCollectedReference(), NULL,
                true);
          ...
          locationLow = self()->cg()->allocateSpill(
              ...
              bestVirtCandidateLow->containsCollectedReference(), NULL, true);
        }
      }
    } else if (locationLow == NULL) {
    ...
    if (enableHighWordRA) {
      if (rk == TR_GPR64 &&
        ...
        if (bestCandidateLow->getHighWordRegister()->getAssignedRegister() !=
            bestVirtCandidateLow) {
          self()->cg()->traceRegisterAssignment(
              "spilling HPR %R for reg pair low",
              bestCandidateLow->getHighWordRegister()->getAssignedRegister());
          currInst = self()->freeHighWordRegister(
              currInst, bestCandidateLow->getHighWordRegister(), 0);
        }
      }

      // TODO: change the logic here so that we don't generate 2 separate Loads
      // to spill a 64-bit reg
      /*
      if (bestVirtCandidateLow->is64BitReg())
         opCodeLow = TR::InstOpCode::LG;
      else
    ...
    locationLow->getSymbolReference()->getSymbol()->setSpillTempLoaded();
    bestVirtCandidateLow->setBackingStorage(locationLow);
    cursor = generateRXInstruction(self()->cg(), opCodeLow, currentNode,
                                   bestCandidateLow, tempMRLow, currInst);
  ...
  if (bestVirtCandidateHigh != NULL) {
    ...
    locationHigh = bestVirtCandidateHigh->getBackingStorage();
    if (_cg->needsVMThreadDependency() &&
        bestVirtCandidateHigh == _cg->getVMThreadRegister()) {
      traceMsg(comp,
      ...
      if (bestVirtCandidateHigh->getBackingStorage() == NULL) {
        ...
        traceMsg(comp, "\ns390machine: allocateVMThreadSpill call completed\n");
               ...
               !bestVirtCandidateHigh->containsInternalPointer()) {
      if (bestVirtCandidateHigh->getKind() == TR_GPR64) {
        locationHigh = self()->cg()->allocateSpill(
            8, bestVirtCandidateHigh->containsCollectedReference(), NULL, true);
        opCodeHigh = TR::InstOpCode::LG;
      } else {
        if (enableHighWordRA) {
          if (bestVirtCandidateHigh->is64BitReg()) {
            locationHigh = self()->cg()->allocateSpill(
                8, bestVirtCandidateHigh->containsCollectedReference(), NULL,
                true);
            opCodeHigh = TR::InstOpCode::LG;
          } else {
            locationHigh = self()->cg()->allocateSpill(
                4, bestVirtCandidateHigh->containsCollectedReference(), NULL,
                true);
          ...
          locationHigh = self()->cg()->allocateSpill(
              TR::Compiler->om.sizeofReferenceAddress(),
              bestVirtCandidateHigh->containsCollectedReference(), NULL, true);
        }
      }
    } else if (locationHigh == NULL) {
    ...
    if (enableHighWordRA) {
      if (rk == TR_GPR64 &&
          bestCandidateHigh->getHighWordRegister()->getAssignedRegister() !=
              NULL) {
        // if the candidate is assigned on both GPR and HPR, need to spill HPR
        if (bestCandidateHigh->getHighWordRegister()->getAssignedRegister() !=
            bestVirtCandidateHigh) {
          self()->cg()->traceRegisterAssignment(
              "spilling HPR %R for reg pair high",
              bestCandidateHigh->getHighWordRegister()->getAssignedRegister());
          // todo: inst flags?
          currInst = self()->freeHighWordRegister(
              currInst, bestCandidateHigh->getHighWordRegister(), 0);
        }
      }
      // TODO: change the logic here so that we don't generate 2 separate Loads
      // to spill a 64-bit reg
    ...
    locationHigh->getSymbolReference()->getSymbol()->setSpillTempLoaded();
    bestVirtCandidateHigh->setBackingStorage(locationHigh);
    cursor = generateRXInstruction(self()->cg(), opCodeHigh, currentNode,
                                   bestCandidateHigh, tempMRHigh, currInst);

    if (enableHighWordRA && bestVirtCandidateHigh->is64BitReg()) {
      bestCandidateHigh->getHighWordRegister()->setState(
          TR::RealRegister::Free);
...
TR::RealRegister *OMR::Z::Machine::findBestFreeRegister(
    ...
    TR::Register *virtualReg, uint64_t availRegMask, bool needsHighWord) {
  // *this    swipeable for debugging purposes
  uint32_t interference = 0;
  ...
  if (comp->getOption(TR_Randomize)) {
    ...
    if (TR::RealRegister::isHPR((TR::RealRegister::RegNum)preference)) {
      randomPreference = _cg->randomizer.randomInt(TR::RealRegister::FirstHPR,
                                                   TR::RealRegister::LastHPR);
    } else if (TR::RealRegister::isFPR((TR::RealRegister::RegNum)preference)) {
      randomPreference = _cg->randomizer.randomInt(TR::RealRegister::FirstFPR,
                                                   TR::RealRegister::LastFPR);
    } else if (TR::RealRegister::isVRF((TR::RealRegister::RegNum)preference)) {
      randomPreference = _cg->randomizer.randomInt(TR::RealRegister::FirstVRF,
                                                   TR::RealRegister::LastVRF);
    } else if (TR::RealRegister::isGPR((TR::RealRegister::RegNum)preference)) {
      if (useGPR0) {
        randomPreference = _cg->randomizer.randomInt(TR::RealRegister::GPR0,
                                                     TR::RealRegister::LastGPR);
      } else {
        randomPreference = _cg->randomizer.randomInt(TR::RealRegister::GPR1,
                                                     TR::RealRegister::LastGPR);
      }
    }
  ...
  if (needsHighWord && preference != 0 &&
      !TR::RealRegister::isHPR((TR::RealRegister::RegNum)preference)) {
    preference = 0;
  }
  ...
  if (liveRegOn && virtualReg != NULL) {
    ...
    if (comp->getOption(TR_Randomize)) {
      ...
      if (performTransformation(comp, "O^O Random Codegen - Randomizing "
                                      "Interference for %s: Original=%x "
                                      "Random=%x\n",
                                _cg->getDebug()->getName(virtualReg),
  ...
  if (self()->cg()->enableRegisterPairAssociation() &&
      preference == TR::RealRegister::LegalEvenOfPair && !needsHighWord) {
    ...
    if ((virtualReg->getSiblingRegister()) &&
      ...
      uint64_t sibRegMask = TR::RealRegister::getBitMask(
          toRealRegister(realSibling)->getRegisterNumber() - 1);
    ...
    if (self()->cg()->supportsHighWordFacility() &&
        !comp->getOption(TR_DisableHighWordRA) && virtualReg->is64BitReg()) {
      if (bestRegister != NULL &&
          (bestRegister->getState() == TR::RealRegister::Free ||
           bestRegister->getState() == TR::RealRegister::Unlatched) &&
          (bestRegister->getHighWordRegister()->getState() ==
               TR::RealRegister::Free ||
           bestRegister->getHighWordRegister()->getState() ==
               TR::RealRegister::Unlatched)) {
        ...
        if (bestRegister->getHighWordRegister()->getState() ==
            TR::RealRegister::Unlatched) {
    ...
    } else // Pre zG+ machine don't support Highword facility
    {
      if (bestRegister != NULL &&
        ...
        return bestRegister;
             ...
             preference == TR::RealRegister::LegalOddOfPair && !needsHighWord) {
    // Check to see if there is a sibling already assigned
    if ((virtualReg->getSiblingRegister()) &&
        (realSibling =
             virtualReg->getSiblingRegister()->getAssignedRegister()) &&
    ...
    if ((virtualReg->getSiblingRegister()) &&
        ...
        self()->isLegalSecondOfFPRegister((TR::RealRegister *)realSibling,
                                          ALLOWBLOCKED, availRegMask)) {
      uint64_t sibRegMask = TR::RealRegister::getBitMask(
          toRealRegister(realSibling)->getRegisterNumber() - 2);
      // If the sibling is assigned we choose the even register next to it
      if (availRegMask & sibRegMask) {
        bestRegister =
    ...
    // HighWord RA stuff
      ...
      if (virtualReg->is64BitReg() && !needsHighWord) {
        ...
        if (preference != 0 && (prefRegMask & availRegMask) &&
            _registerFile[preference] != NULL) {
          candidateLWFree = (_registerFile[preference]->getState() ==
                             TR::RealRegister::Free) ||
                            (_registerFile[preference]->getState() ==
                             TR::RealRegister::Unlatched);
          candidateHWFree =
              (_registerFile[preference]->getHighWordRegister()->getState() ==
               TR::RealRegister::Free) ||
              (_registerFile[preference]->getHighWordRegister()->getState() ==
               TR::RealRegister::Unlatched);
        }
        ...
        if ((prefRegMask & availRegMask) && candidateLWFree &&
            candidateHWFree && _registerFile[preference] != NULL) {
          bestWeightSoFar = 0x0fffffff;
          bestRegister = _registerFile[preference];
          if (bestRegister != NULL &&
              bestRegister->getState() == TR::RealRegister::Unlatched) {
          ...
          if (bestRegister != NULL &&
              bestRegister->getHighWordRegister()->getState() ==
                  TR::RealRegister::Unlatched) {
            bestRegister->getHighWordRegister()->setAssignedRegister(NULL);
            bestRegister->getHighWordRegister()->setState(
                TR::RealRegister::Free);
          }
          _cg->setRegisterAssignmentFlag(TR_ByAssociation);

          if (bestRegister != NULL)
            _cg->traceRegisterAssignment("BEST FREE REG by pref for %R is %R",
                                         virtualReg, bestRegister);
          else
            _cg->traceRegisterAssignment("BEST FREE REG by pref for %R is NULL",
                                         virtualReg);
        ...
        if (candidate != NULL && (prefRegMask & availRegMask) &&
  ...
  for (int32_t i = first; i <= last; i++) {
    ...
    if (!enableHighWordRA) {
      ...
      if ((_registerFile[i]->getState() == TR::RealRegister::Free ||
        ...
        if (comp->getOption(TR_Randomize)) {
          ...
          if (performTransformation(
                  comp, "O^O Random Codegen - Randomizing Weight for %s, "
                        "Original bestWeightSoFar: %x randomized to: %x\n",
                  _cg->getDebug()->getName(_registerFile[i]), bestWeightSoFar,
                  randomWeight)) {
      ...
      if (candidate->getHighWordRegister()->getState() ==
          TR::RealRegister::Locked) {
      ...
      if (virtualReg->is64BitReg() && !needsHighWord) {
        bool candidateLWFree =
            (candidate->getState() == TR::RealRegister::Free) ||
            (candidate->getState() == TR::RealRegister::Unlatched);
        bool candidateHWFree = (candidate->getHighWordRegister()->getState() ==
                                TR::RealRegister::Free) ||
                               (candidate->getHighWordRegister()->getState() ==
                                TR::RealRegister::Unlatched);
        ...
        if (candidateLWFree && candidateHWFree &&
            (freeRegister == NULL || (iOld && !iNew) ||
             ((iOld || !iNew) && candidate->getWeight() < bestWeightSoFar))) {
          iOld = iNew;
        ...
        if (virtualReg->assignToHPR() || needsHighWord) {
          ...
          tRegMask =
              TR::RealRegister::getBitMask(candidate->getRegisterNumber());
        } else {
  ...
  if (enableHighWordRA && virtualReg->is64BitReg()) {
    ...
    if (freeRegister != NULL &&
        freeRegister->getHighWordRegister()->getState() ==
            TR::RealRegister::Unlatched) {
  ...
  if (freeRegister != NULL)
    _cg->traceRegisterAssignment("BEST FREE REG for %R is %R", virtualReg,
                                 freeRegister);
  else
    _cg->traceRegisterAssignment(
...
TR::RealRegister *OMR::Z::Machine::findRegNotUsedInInstruction(
  ...
  for (int32_t i = first; i <= last, spill == NULL; i++) {
    TR::RealRegister *realReg =
        self()->getS390RealRegister((TR::RealRegister::RegNum)i);
...
OMR::Z::Machine::findVirtRegInHighWordRegister(TR::Register *virtReg) {
  ...
  for (int32_t i = first; i <= last; i++) {
    ...
    if (realReg->getAssignedRegister() &&
...
void OMR::Z::Machine::allocateUpgradedBlockedList(
    TR_Stack<TR::RealRegister *> *mem) {
  _blockedUpgradedRegList = mem;
}

bool OMR::Z::Machine::addToUpgradedBlockedList(TR::RealRegister *reg) {
  if (reg == NULL)
    return false;
  self()->cg()->traceRegisterAssignment("Adding %s (0x%p) to blocked reg list",
                                        getRegisterName(reg, self()->cg()),
                                        reg);
  _blockedUpgradedRegList->push(reg);
  return true;
}

TR::RealRegister *OMR::Z::Machine::getNextRegFromUpgradedBlockedList() {
  if (_blockedUpgradedRegList->isEmpty())
    return NULL;
  TR::RealRegister *reg = _blockedUpgradedRegList->pop();
  TR_ASSERT(reg->getAssignedRegister() == NULL,
            "Register %s (0x%p) from Blocked-list has assigned reg %p",
            getRegisterName(reg, self()->cg()), reg,
            reg->getAssignedRegister());
  TR_ASSERT(reg->getState() == TR::RealRegister::Blocked,
            "Register %s (0x%p) from Blocked-list is not in Blocked state",
            getRegisterName(reg, self()->cg()), reg);
  return reg;
}
 ...
 */
void OMR::Z::Machine::blockVolatileAccessRegisters() {
  ...
  for (int32_t i = first; i <= last; i++) {
    ...
    if (volatileReg) {
      TR_ASSERT(realReg->getState() == TR::RealRegister::Free ||
                    realReg->getState() == TR::RealRegister::Locked,
                "OMR::Z::Machine::blockVolatileAccessRegister -- all volatile "
                "access regs should be free.");
 ...
 */
void OMR::Z::Machine::unblockVolatileAccessRegisters() {
  int32_t first = TR::RealRegister::FirstAR;
  int32_t last = TR::RealRegister::LastAR;
  TR::Machine *machine = self()->cg()->machine();
  TR::Linkage *linkage = _cg->getS390Linkage();
 ...
 */
void OMR::Z::Machine::blockVolatileHighRegisters() {
  int32_t first = TR::RealRegister::FirstGPR;
  int32_t last = TR::RealRegister::LastGPR;
  TR::Machine *machine = self()->cg()->machine();
  TR::Linkage *linkage = _cg->getS390Linkage();
 ...
 */
void OMR::Z::Machine::unblockVolatileHighRegisters() {
  int32_t first = TR::RealRegister::FirstGPR;
  int32_t last = TR::RealRegister::LastGPR;
  TR::Machine *machine = self()->cg()->machine();
  TR::Linkage *linkage = _cg->getS390Linkage();
...
OMR::Z::Machine::freeBestRegister(TR::Instruction *currentInstruction,
                                  ...
                                  uint64_t availRegMask, bool allowNullReturn,
                                  bool doNotSpillToSiblingHPR) {
  ...
  for (int32_t i = first; i <= last; i++) {
    ...
    if (!enableHighWordRA) {
      if (realReg->getState() == TR::RealRegister::Assigned) {
        ...
        if (currentInstruction->getDependencyConditions() &&
            currentInstruction->getDependencyConditions()
                ->searchPostConditionRegister(associatedVirtual)) {
          // we just assigned this virtual in the reg deps, do not free it
          traceMsg(_cg, "  Reg[@%d] associatedVirtual[%s] was excluded from "
                        "spill target because of reg dependency\n",
                   realReg->getRegisterNumber() - 1,
                   _cg->getDebug()->getName(associatedVirtual));
          continue;
        }
    ...
    // HighWord RA stuff
      ...
      if (virtReg->assignToHPR()) {
        ...
        bool assignedToTwoRegs = false; // LW and HW are assigned to 2 different
                                        // virtRegs, least preferred candidate
        if (realReg->getState() == TR::RealRegister::Assigned &&
            realRegHW->getState() == TR::RealRegister::Assigned) {
          if (realReg->getAssignedRegister() !=
              realRegHW->getAssignedRegister()) {
        ...
        if (realReg->getState() == TR::RealRegister::Assigned) {
          // candidate is LW's virtReg in case if both LW and HW need to be
          // spilled
          associatedVirtual = realReg->getAssignedRegister();
          usedInMemRef = associatedVirtual->isUsedInMemRef();
        }

        bool doNotSpillHPR = realRegHW->getAssignedRegister() == virtReg &&
                             realReg->getAssignedRegister() != virtReg;

        /*
        // do not to spill an HPR that contains a compressed ref when shift !=0.
        // We have to first decompress the pointer, which means we need an extra
        64-bit register or at least the low word register.
        ...
        */
        ...
        if ((realReg->getState() == TR::RealRegister::Assigned ||
             realReg->getState() == TR::RealRegister::Free) &&
            (realRegHW->getState() == TR::RealRegister::Assigned ||
             realRegHW->getState() == TR::RealRegister::Free) &&
            !doNotSpillHPR) {
          if ((!iInterfere && i == preference && pref_favored) ||
              (!usedInMemRef && !assignedToTwoRegs)) {
            if (numCandidates == 0) {
              candidates[0] = associatedVirtual;
            } else {
              tempReg = candidates[0];
  ...
  if (numCandidates == 0) {
    if (!allowNullReturn) {
      ...
      traceMsg(
          comp,
          "OMR::Z::Machine::freeBestRegister -- Ran out of regs on Inst %p.\n",
          currentInstruction);
  ...
  while (numCandidates > 1 && cursor != NULL &&
    ...
    for (int32_t i = 0; i < numCandidates; i++) {
      ...
      // also need to check HW. if it's assigned, push the candidate back
      else if (enableHighWordRA && virtReg->is64BitReg() &&
               candidates[i]->getAssignedRegister()) {
        TR::RealRegister *candidateHW =
            toRealRegister(candidates[i]->getAssignedRegister())
                ->getHighWordRegister();
        if (candidateHW->getState() == TR::RealRegister::Assigned &&
          ...
          if (cursor->refsRegister(candidateHW->getAssignedRegister())) {
            candidates[i] = candidates[--numCandidates];
            i--; // on continue repeat test for candidate[i] as candidate[i] is
                 // now new.
          }
        }
      }
  ...
  if (candidates[0]->getAssignedRegister()) {
    ...
    TR_ASSERT(enableHighWordRA,
              "freeBestRegister:a virtual reg is assigned to NULL?");
    // if the best candidate is a previously spilled HPR
    best = toRealRegister(self()->findVirtRegInHighWordRegister(candidates[0]));
  }

  // Shortcut the selection when a virtReg is specified, and it is already
  // assigned a real, we spill
  // that real
  ...
  // todo fix this
    ...
    _cg->traceRegisterAssignment(
        "HW RA: freeBestReg Spill %R for fullsize reg: %R ",
        best->getHighWordRegister(), virtReg);
    ...
    best = best->getHighWordRegister();
             ...
             (virtReg->is64BitReg() || virtReg->assignToGPR())) {
    best = best->getLowWordRegister();
  }

  // If we spill into highword, make sure to not to spill it into the one that
  // will clobber fullsize reg
  if (enableHighWordRA && (virtReg->is64BitReg() || doNotSpillToSiblingHPR)) {
    uint32_t availHighWordRegMap =
        ~(TR::RealRegister::getBitMask(
            toRealRegister(best->getHighWordRegister())->getRegisterNumber())) &
        availRegMask & 0xffff0000;
 ...
 */
void OMR::Z::Machine::freeRealRegister(TR::Instruction *currentInstruction,
  ...
  bool enableHighWordRA = _cg->supportsHighWordFacility() &&
                          !comp->getOption(TR_DisableHighWordRA) &&
                          targetReal->getKind() != TR_FPR &&
                          targetReal->getKind() != TR_VRF;

  if (virtReg) {
    if (enableHighWordRA && is64BitReg) {
      uint32_t availHighWordRegMap =
          ~(TR::RealRegister::getBitMask(
              toRealRegister(targetReal->getHighWordRegister())
                  ->getRegisterNumber())) &
          0xffff0000;
...
TR::Instruction *
OMR::Z::Machine::freeHighWordRegister(TR::Instruction *currentInstruction,
                                      TR::RealRegister *targetRegisterHW,
                                      flags32_t instFlags) {
  ...
  _cg->traceRegisterAssignment(" freeHighWordRegister: %R:%R ", virtRegHW,
                               targetRegisterHW);

  TR_ASSERT(targetRegisterHW != NULL,
            "freeHighWordRegister called but targetReg HW is null?");
  TR_ASSERT(targetRegisterHW->isHighWordRegister(),
            "freeHighWordRegister called for non HPR?");

  // if we are trying to free up the HPR that was used for a 32-bit GPR spill
  if (virtRegHW->getAssignedRegister() == NULL &&
      targetRegisterHW->getState() == TR::RealRegister::Assigned) {
    _cg->traceRegisterAssignment(
        " HW RA %R was spilled to %R, now need to spill again", virtRegHW,
        targetRegisterHW);

    // try to find another free HPR
    spareReg = self()->findBestFreeRegister(
        currentInstruction, rk, virtRegHW,
        self()->cg()->getAvailableHPRSpillMask(), true);

    if (spareReg) {
      cursor =
          self()->registerCopy(currentInstruction, TR_HPR, targetRegisterHW,
                               spareReg, self()->cg(), instFlags);
      ...
      // if no more free regs, spill this one to stack, do not try to HPR spill
      // again
  ...
  if (targetRegisterHW->getState() == TR::RealRegister::Blocked) {
    if (spareReg == NULL) {
      virtRegHW->block();
      spareReg =
          self()->freeBestRegister(currentInstruction, virtRegHW, rk,
                                   self()->cg()->getAvailableHPRSpillMask());
      virtRegHW->unblock();
    }
    TR_ASSERT(spareReg != NULL,
              "freeHighWordRegister: blocked, must find a spareReg");

    cursor = self()->registerCopy(currentInstruction, TR_HPR, targetRegisterHW,
                                  spareReg, self()->cg(), instFlags);
...
void OMR::Z::Machine::spillRegister(TR::Instruction *currentInstruction,
  ...
  if (enableHighWordRA && virtReg->getAssignedRegister() == NULL) {
    ...
    TR_ASSERT(best != NULL,
              "HW RA: spillRegister cannot find real target reg\n");
  } else {
  ...
  if (virtReg->containsCollectedReference() && TR::Compiler->target.is64Bit() &&
      ...
      !checkOOLInterference(currentInstruction, virtReg)) {
    canSpillPointerToHPR = true;
  }

  // try to spill to highword
  if (enableHighWordRA && !comp->getOption(TR_DisableHPRSpill) &&
      ((!virtReg->is64BitReg() && !virtReg->containsCollectedReference()) ||
       canSpillPointerToHPR) &&
      best->isLowWordRegister() && !virtReg->containsInternalPointer() &&
      !self()->cg()->isOutOfLineColdPath()) {
    // try to find a free HW reg
    availHighWordRegMap &= self()->cg()->getAvailableHPRSpillMask();
    freeHighWordReg = self()->findBestFreeRegister(
        currentInstruction, rk, virtReg, availHighWordRegMap, true);

    if (freeHighWordReg) {
      if (debugHPR) {
        printf("\nHW RA: spilling into HPR ");
        fflush(stdout);
      }
      self()->cg()->traceRegisterAssignment(
      ...
      if (alreadySpilledToHPR) {
        cursor = generateExtendedHighWordInstruction(
            currentNode, _cg, TR::InstOpCode::LHHR, best, freeHighWordReg, 0,
            currentInstruction);
      } else {
        if (virtReg->containsCollectedReference()) {
          if (debugHPR) {
          ...
          if (compressShift == 0) {
            cursor = generateExtendedHighWordInstruction(
                currentNode, _cg, TR::InstOpCode::LLHFR, best, freeHighWordReg,
                0, currentInstruction);
            self()->cg()->traceRAInstruction(cursor);
            cursor =
                generateRILInstruction(_cg, TR::InstOpCode::IIHF, currentNode,
                                       best, (uintptrj_t)0, cursor);
            self()->cg()->traceRAInstruction(cursor);
          } else {
            cursor = generateRIEInstruction(
                _cg, TR::InstOpCode::RISBLG, currentNode, best, freeHighWordReg,
                0, 31 + 0x80 - compressShift, 32 + compressShift,
                currentInstruction);
            self()->cg()->traceRAInstruction(cursor);
            cursor = generateRIEInstruction(
                _cg, TR::InstOpCode::RISBHG, currentNode, best, freeHighWordReg,
                32 - compressShift, 31 + 0x80, 32 + compressShift,
                currentInstruction);
            self()->cg()->traceRAInstruction(cursor);
          }
        } else {
          cursor = generateExtendedHighWordInstruction(
              currentNode, _cg, TR::InstOpCode::LLHFR, best, freeHighWordReg, 0,
              currentInstruction);
        }
      }
      if (debugHPR) {
        printf("in %s ,0x%x, %s", comp->signature(), availHighWordRegMap,
...

  {
    ...
    case TR_GPR64:
      ...
      } else if ((!comp->getOption(TR_EnableTrueRegisterModel)) ||
                 location == NULL) {
    ...
    case TR_GPR:
      ...
      // value has already been spilled
               ...
               _cg->needsVMThreadDependency() &&
               virtReg == _cg->getVMThreadRegister()) {
        ...
        if ((!comp->getOption(TR_EnableTrueRegisterModel)) ||
          ...
          if ((enableHighWordRA && virtReg->is64BitReg()) ||
              comp->getOption(TR_ForceLargeRAMoves)) {
            location = self()->cg()->allocateSpill(
                8, virtReg->containsCollectedReference(), NULL, true);
          } else {
            location = self()->cg()->allocateSpill(
                TR::Compiler->om.sizeofReferenceAddress(),
                virtReg->containsCollectedReference(), NULL, true);
          }
          if (debugObj)
            self()->cg()->traceRegisterAssignment("\nSpilling %s to (%p)\n",
                                                  debugObj->getName(virtReg),
      ...
      if (enableHighWordRA) {
        ...
        } else if (best->isLowWordRegister() &&
                   best->getHighWordRegister()->getAssignedRegister() !=
                       virtReg) {
        ...
        // incorrect spill slot size");
        // this assume kicks in for SLLG, MGHI etc on 31bit
        if (debugObj)
          self()->cg()->traceRegisterAssignment(" HW RA: spilling %R:%R",
                                                virtReg, best);
      }
      break;
    case TR_FPR:
    ...
    if (enableHighWordRA && alreadySpilledToHPRCollectible) {
      ...
      TR::MemoryReference *mr =
          generateS390MemoryReference(*tempMR, 4, self()->cg());
      cursor =
          generateRXYInstruction(self()->cg(), TR::InstOpCode::LFH, currentNode,
                                 best, mr, currentInstruction);
      virtReg->setSpilledToHPR(false);
    } else {
      if (opCode == TR::InstOpCode::LAM)
        cursor = generateRSInstruction(self()->cg(), opCode, currentNode, best,
                                       best, tempMR, currentInstruction);
...
OMR::Z::Machine::reverseSpillState(TR::Instruction *currentInstruction,
  ...
  if (enableHighWordRA) {
    // TR_ASSERTC( !location,_cg->comp(), "\nHW RA: reg spilled to both HW and
    // stack??\n");
    freeHighWordReg = self()->findVirtRegInHighWordRegister(spilledRegister);
    if (freeHighWordReg)
      _cg->traceRegisterAssignment("reverseSpillState: found GPR spilled to %R",
                                   freeHighWordReg);
  }
  ...
  if (_cg->isOutOfLineColdPath()) {
    ...
    // state
    // for example a new register assignment in the hot path would cause FC !=
    // TC
    ...
    if (!freeHighWordReg) {
      if (!location) {
        ...
        if (enableHighWordRA && spilledRegister->is64BitReg()) {
          targetRegister->getHighWordRegister()->setState(
              TR::RealRegister::Assigned);
          targetRegister->getHighWordRegister()->setAssignedRegister(
              spilledRegister);
        }
        return targetRegister;
      }
  ...
  if (location == NULL && freeHighWordReg == NULL) {
    if (rk == TR_GPR) {
      if (enableHighWordRA && spilledRegister->is64BitReg())
        ...
        location = self()->cg()->allocateSpill(
            TR::Compiler->om.sizeofReferenceAddress(),
            spilledRegister->containsCollectedReference(), NULL, true);
    } else if (rk == TR_GPR64) {
      location = self()->cg()->allocateSpill(
          8, spilledRegister->containsCollectedReference(), NULL, true);
    } else if (rk == TR_VRF) {
  ...
  if (comp->getOption(TR_Enable390FreeVMThreadReg) &&
      spilledRegister == self()->cg()->getVMThreadRegister()) {
    ...
    if (freeHighWordReg) {
      if (targetRegister->isHighWordRegister()) {
        cursor = generateExtendedHighWordInstruction(
            currentNode, _cg, TR::InstOpCode::LHHR, freeHighWordReg,
            targetRegister, 0, currentInstruction);
      } else {
        if (spilledRegister->containsCollectedReference()) {
          ...
          if (compressShift == 0) {
            cursor = generateExtendedHighWordInstruction(
                currentNode, _cg, TR::InstOpCode::LHLR, freeHighWordReg,
                targetRegister, 0, currentInstruction);
          } else {
            cursor = generateRIEInstruction(
                _cg, TR::InstOpCode::RISBHG, currentNode, freeHighWordReg,
                targetRegister, 0, 31 + 0x80, 32 - compressShift,
                currentInstruction);
          }
        } else {
          cursor = generateExtendedHighWordInstruction(
    ...
    case TR_GPR:
      ...
      if (enableHighWordRA) {
        if (spilledRegister->assignToHPR() ||
          ...
          if (spilledRegister->containsCollectedReference()) {
            ...
            TR_ASSERT(!spilledRegister->is64BitReg(),
                      "ReverseSpill: HPR cannot be 64 bit!\n");
          }
        }
        if (spilledRegister->assignToGPR() ||
            targetRegister->isLowWordRegister()) {
    ...
    if (true) // Check to see if we should free spill location
    {
      if (comp->getOption(TR_DisableOOL)) {
        ...
        if (self()->cg()->isOutOfLineColdPath()) {
          ...
          // OOL: only free the spill slot if the register was spilled in the
          // same or less dominant path
          // ex: spilled in cold path, reverse spill in hot path or main line
          // we have to spill this register again when we reach OOL entry point
          // due to post
          // conditions. We want to guarantee that the same spill slot will be
          ...
          // the spilledRegisterList contains all registers that are spilled
          // before entering
          // the OOL path (in backwards RA). Post dependencies will be generated
          // using this list.
          // Any registers reverse spilled before entering OOL should be removed
          // from the spilled list
          if (debugObj)
            self()->cg()->traceRegisterAssignment(
    ...
    if (needMVHI) {
      cursor =
          generateSILInstruction(self()->cg(), TR::InstOpCode::MVHI,
                                 currentNode, tempMR, 0, currentInstruction);
      self()->cg()->traceRAInstruction(cursor);
      cursor = generateRXInstruction(
          self()->cg(), TR::InstOpCode::STFH, currentNode, targetRegister,
          generateS390MemoryReference(*tempMR, 4, self()->cg()), cursor);
      self()->cg()->traceRAInstruction(cursor);
      spilledRegister->setSpilledToHPR(true);
    } else {
      if (opCode == TR::InstOpCode::STAM)
        cursor = generateRSInstruction(self()->cg(), opCode, currentNode,
                                       targetRegister, targetRegister, tempMR,
                                       currentInstruction);
      else if (opCode == TR::InstOpCode::VST)
        cursor = generateVRXInstruction(self()->cg(), opCode, currentNode,
                                        targetRegister, tempMR, 0,
                                        currentInstruction);
      else
        cursor =
            generateRXInstruction(self()->cg(), opCode, currentNode,
...
bool OMR::Z::Machine::isAssignable(TR::Register *virtReg,
  ...
  if (virtReg->isUsedInMemRef() &&
    ...
    if (_cg->supportsHighWordFacility() &&
        !TR::comp()->getOption(TR_DisableHighWordRA) &&
        virtReg->getKind() != TR_FPR && virtReg->getKind() != TR_VRF) {
      if ((virtReg->is64BitReg() &&
           realReg->getLowWordRegister()->getAssignedRegister() ==
               realReg->getHighWordRegister()->getAssignedRegister()) ||
          (!virtReg->is64BitReg() &&
           realReg->getLowWordRegister()->getAssignedRegister() !=
               realReg->getHighWordRegister()->getAssignedRegister())) {
        return true;
...
TR::Instruction *OMR::Z::Machine::coerceRegisterAssignment(
    ...
    TR::RealRegister::RegNum registerNumber, flags32_t instFlags) {
  // *this    swipeable for debugging purposes
  ...
  if (virtualRegister->isArGprPair())
    realReg = virtualRegister->getGPRofArGprPair()->getAssignedRealRegister();
  ...
  TR_RegisterKinds currentAssignedRegisterRK =
      rk; // used for register Copy, todo: use it for freeBestReg and
          // findFreeReg
  ...
  if (!_cg->getRAPassAR() &&
      (rk == TR_AR || targetRegister->getKind() == TR_AR))
    TR_ASSERT(
        false,
        "Should only process AR dependencies in AR register allocator pass");
  ...
  if (enableHighWordRA) {
    availHighWordRegMap =
        ~(TR::RealRegister::getBitMask(toRealRegister(targetRegister)
                                           ->getHighWordRegister()
                                           ->getRegisterNumber()));
  }
  _cg->traceRegisterAssignment("COERCE %R into %R", virtualRegister,
                               targetRegister);

  // If either the virtual we are coercing or the assigned reg we are
  //
  //
  if (targetRegister->getAssignedRegister() &&
      targetRegister->getAssignedRegister()->getKind() == TR_GPR64) {
    rk = TR_GPR64;
  ...
  // in addition to the 4 GPR's involved, we have 4 more registers for high
  // words:
  //                      REAL                    VIRTUAL
  ...
  if (enableHighWordRA) {
    ...
    if (currentAssignedRegister != NULL) {
      currentAssignedRegisterHW =
          currentAssignedRegister->getHighWordRegister();
      virtualRegisterHW = currentAssignedRegisterHW->getAssignedRegister();
    }

    if (virtualRegister->is64BitReg()) {
      _cg->traceRegisterAssignment(" HW RA coerceRA: %R needs 64 bit reg ",
                                   virtualRegister);
    } else {
      _cg->traceRegisterAssignment(" HW RA coerceRA: %R needs 32 bit reg ",
                                   virtualRegister);
    }
  }
  ...
  // the target reg is free
    ...
    if (virtualRegister->isPlaceholderReg())
      targetRegister->setIsAssignedMoreThanOnce(); // Register is killed
                                                   // invalidate it for moving
                                                   // spill out of loop
    ...
    if (enableHighWordRA && virtualRegister->is64BitReg()) {
      if (targetRegister->isHighWordRegister() &&
        ...
        // this could happen for OOL, when the reg deps on the top of slow path
        // dictates that a collectible register must be
        // spilled to a specific HPR
        TR_ASSERT(virtualRegister->containsCollectedReference(),
                  " OOL HPR spill: spilling a 64 bit scalar into HPR");

        TR::RealRegister *currentHighWordReg =
            self()->findVirtRegInHighWordRegister(virtualRegister);

        if (virtualRegister->getAssignedRegister() == NULL &&
            currentHighWordReg) {
          // already spilled to HPR, so simply move
          cursor = generateExtendedHighWordInstruction(
              currentNode, _cg, TR::InstOpCode::LHHR, currentHighWordReg,
              targetRegister, 0, currentInstruction);
          _cg->traceRAInstruction(cursor);
          ...
          if (currentAssignedRegister == NULL) {
            if (virtualRegister->getTotalUseCount() !=
              ...
              // the virtual register is currently spilled to stack, now we need
              // to spill it onto HPR
              // load it back from the stack into HPR with STFH
              // since we are working with compressed refs shift = 0, simply
              // load 32-bit value into HPR.
              TR::MemoryReference *tempMR = generateS390MemoryReference(
                  currentNode,
                  virtualRegister->getBackingStorage()->getSymbolReference(),
                  _cg);

              // is the offset correct?  +4 big endian?
              TR::MemoryReference *mr =
                  generateS390MemoryReference(*tempMR, 4, _cg);

              cursor = generateSILInstruction(self()->cg(),
                                              TR::InstOpCode::MVHI, currentNode,
                                              tempMR, 0, currentInstruction);
              _cg->traceRAInstruction(cursor);
              cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::STFH,
                                             currentNode, targetRegister, mr,
                                             cursor);
              _cg->traceRAInstruction(cursor);

              // fix up states
              // don't need to worry about protecting backing storage because we
              // are leaving cold path OOL now
              ...
              TR_ASSERT(comp, " OOL HPR spill: currentAssignedRegister is NULL "
                              "but virtual reg is not spilled?");
            }
          } else {
            // the virtual register is currently assigned to a 64 bit real reg
            // simply spill it to HPR and decompress
            TR_ASSERT(currentAssignedRegister->isLowWordRegister(),
                      " OOL HPR spill: 64-bit reg assigned to HPR and is not "
                      "spilled to HPR");
            cursor = generateExtendedHighWordInstruction(
                currentNode, _cg, TR::InstOpCode::LLHFR,
                currentAssignedRegister, targetRegister, 0, currentInstruction);
            _cg->traceRAInstruction(cursor);
            cursor = generateRILInstruction(
                _cg, TR::InstOpCode::IIHF, currentNode, currentAssignedRegister,
                (uintptrj_t)0, cursor);
            _cg->traceRAInstruction(cursor);
      ...
      if (targetRegisterHW->getState() != TR::RealRegister::Free &&
        ...
        cursor = self()->freeHighWordRegister(currentInstruction,
                                              targetRegisterHW, instFlags);
    ...
    if (currentAssignedRegister == NULL) {
      ...
      cursor = self()->registerCopy(
          currentInstruction, currentAssignedRegisterRK,
          currentAssignedRegister, targetRegister, self()->cg(), instFlags);
      if (enableHighWordRA && virtualRegister->is64BitReg()) {
        currentAssignedRegisterHW->setState(TR::RealRegister::Free);
        currentAssignedRegisterHW->setAssignedRegister(NULL);
      }
  ...
  // the target reg is blocked so we need to guarantee it stays in a reg
    ...
    _cg->traceRegisterAssignment("target %R is blocked, assigned to %R",
                                 targetRegister, currentTargetVirtual);

    if (enableHighWordRA && currentTargetVirtual) {
      if (targetRegister->isHighWordRegister() &&
          targetRegister->getLowWordRegister()->getAssignedRegister() !=
    ...
    if (enableHighWordRA && virtualRegister->is64BitReg())
      spareReg = self()->findBestFreeRegister(
          currentInstruction, rk, currentTargetVirtual, availHighWordRegMap);
    else
    ...
    if (spareReg == NULL) {
      ...
      if (enableHighWordRA && virtualRegister->is64BitReg()) {
        // if we end up spilling currentTargetVirtual to HPR, make sure to not
        // pick the HPR of targetRegister since it will not
        // free up the full 64 bit register
        spareReg = self()->freeBestRegister(
            currentInstruction, currentTargetVirtual,
            currentTargetVirtual->getKind(), availHighWordRegMap, true, true);
      } else {
        // we can allow freeBestRegister() to return NULL here, as long as we
        // can perform register exchange later via memory
        // and take a bad OSC penalty
        // todo: fix HW RA to enable register exchange later
        spareReg = self()->freeBestRegister(
            currentInstruction, currentTargetVirtual,
            currentTargetVirtual->getKind(), 0xffff, true);
      }
      virtualRegister->unblock();
    ...
    // but virtual Reg could require full size, in this case, need to free the
    // HW of target Reg
    if (enableHighWordRA && virtualRegister->is64BitReg()) {

      // There is an extra check at the end to avoid the scenario in which
      // virtualRegister is assigned to
      // HPRx and we are attempting to coerce it to GPRx. In this case we do not
      // need to spill HPRx, rather just copy it
      if (targetRegisterHW->getState() != TR::RealRegister::Free &&
          targetRegisterHW->getState() != TR::RealRegister::Unlatched &&
          targetRegisterHW->getAssignedRegister() != virtualRegister) {
        if (targetRegisterHW->getAssignedRegister() &&
            targetRegisterHW->getAssignedRegister() !=
                targetRegister->getAssignedRegister()) {
          // TR_ASSERTC( currentTargetVirtual->isLowWordOnly(),_cg->comp(),
          // "currentTargetVirtual is not LWOnly but HW is clobbered by another
          // vreg?");
          virtualRegister->block();
          currentTargetVirtual->block();
          spareReg->block(); // to do: is this necessary? only need to block
                             // HPR?
          // free up this highword register by either spilling it or moving it
          cursor = self()->freeHighWordRegister(currentInstruction,
                                                targetRegisterHW, instFlags);
    ...
    if (currentAssignedRegister != NULL) {
      ...
      if (!self()->isAssignable(currentTargetVirtual,
                                currentAssignedRegister)) {
        if (enableHighWordRA && spareReg == NULL &&
            currentTargetVirtualRK != currentAssignedRegisterRK) {
          // register kinds mismatch and we do not have a free spare reg, take
          // the OSC penalty...
          TR_BackingStore *location;

          location = self()->cg()->allocateSpill(
              8, false, NULL, true); // No chance of a gcpoint
          TR::MemoryReference *tempMR = generateS390MemoryReference(
              currentNode, location->getSymbolReference(), self()->cg());
          ...
          cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::LG,
                                         currentNode, currentAssignedRegister,
                                         tempMR, currentInstruction);
          self()->cg()->traceRAInstruction(cursor);
          cursor = generateRRInstruction(
              self()->cg(), TR::InstOpCode::LGR, currentNode, targetRegister,
              currentAssignedRegister, currentInstruction);
          self()->cg()->traceRAInstruction(cursor);
          TR::MemoryReference *tempMR2 =
              generateS390MemoryReference(*tempMR, 0, self()->cg());
          cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::STG,
                                         currentNode, targetRegister, tempMR2,
                                         currentInstruction);
          self()->cg()->traceRAInstruction(cursor);
          ...
          TR::RealRegister::RegState tempState =
              currentAssignedRegisterHW->getState();
          TR::Register *tempAssignedReg =
              currentAssignedRegisterHW->getAssignedRegister();

          currentAssignedRegisterHW->setState(targetRegisterHW->getState());
          currentAssignedRegisterHW->setAssignedRegister(
              targetRegisterHW->getAssignedRegister());

          // If the target register contained a 64-bit value then the Low-Word
          // and High-Word real registers
          // would point to the same virtual register. As such the following is
          // true:
          //
          // currentTargetVirtual == targetRegisterHW->getAssignedRegister()
          //
          // hence the statement in the if block below would set
          // currentTargetVirtual to point to the High-Word
          // of the currentAssignedRegister which is not what we want (since it
          // is a 64-bit value).

          if (targetRegisterHW->getAssignedRegister() &&
              !targetRegisterHW->getAssignedRegister()->is64BitReg()) {
            targetRegisterHW->getAssignedRegister()->setAssignedRegister(
                currentAssignedRegisterHW);
          }
          ...
          cursor = self()->registerCopy(currentInstruction,
                                        currentTargetVirtualRK, targetRegister,
                                        spareReg, self()->cg(), instFlags);
          cursor = self()->registerCopy(
              currentInstruction, currentAssignedRegisterRK,
              currentAssignedRegister, targetRegister, self()->cg(), instFlags);
          ...
          if (enableHighWordRA && currentTargetVirtual->is64BitReg()) {
            targetRegister->getLowWordRegister()->setState(
                TR::RealRegister::Unlatched);
            targetRegister->getLowWordRegister()->setAssignedRegister(NULL);
            targetRegisterHW->setState(TR::RealRegister::Unlatched);
            targetRegisterHW->setAssignedRegister(NULL);

            spareReg->getHighWordRegister()->setState(
                TR::RealRegister::Assigned); // no need to block the HW half, we
                                             // already blocked the Lw
            spareReg->getHighWordRegister()->setAssignedRegister(
                currentTargetVirtual);
          }
        ...
        if (enableHighWordRA) {
          cursor = self()->registerExchange(
              currentInstruction, currentAssignedRegisterRK, targetRegister,
              currentAssignedRegister, spareReg, self()->cg(), instFlags);
          if (currentTargetVirtual->is64BitReg()) {
            // currentAssignedRegister->getHighWordRegister()->setState(TR::RealRegister::Blocked);
            // //not necessary
            currentAssignedRegister->getHighWordRegister()->setAssignedRegister(
                currentTargetVirtual);
          }
        } else {
          // Vector coercion most likely to take this path.
          cursor = self()->registerExchange(
              currentInstruction, rk, targetRegister, currentAssignedRegister,
              spareReg, self()->cg(), instFlags);
        }
        currentAssignedRegister->setState(TR::RealRegister::Blocked);
        currentAssignedRegister->setAssignedRegister(currentTargetVirtual);
      ...
      cursor = self()->registerCopy(currentInstruction, currentTargetVirtualRK,
                                    targetRegister, spareReg, self()->cg(),
                                    instFlags);
      ...
      if (enableHighWordRA && currentTargetVirtual->is64BitReg()) {
        ...
        //        "currentTargetVirtual is blocked and is fullsize, but the HW
        //        is not blocked?");
  ...
  // the target reg is assigned
    ...
    currentTargetVirtual = targetRegister->getAssignedRegister();
    _cg->traceRegisterAssignment("target %R is assigned, assigned to %R",
                                 targetRegister, currentTargetVirtual);
    ...
    if (enableHighWordRA &&
        ...
        targetRegister->isHighWordRegister()) {
      _cg->traceRegisterAssignment(
          " HW RA %R was spilled to %R, now need to spill again",
          currentTargetVirtual, targetRegister);
      ...
      if (virtualRegister->is64BitReg() &&
        ...
        // we need to either assign or spill a 32-bit virtual register into a
        // HPR
        ...
        if (currentAssignedRegister == NULL) {
          ...
          if (currentHighWordReg) {
          ...
          } else if (virtualRegister->getTotalUseCount() !=
                     virtualRegister->getFutureUseCount()) {
            // if it is spilled to stack, load it back into HPR
            TR_ASSERT(
                virtualRegister->getBackingStorage(),
                " OOL HPR spill: virtual reg is not spilled to stack nor HPR");
            TR::MemoryReference *tempMR = generateS390MemoryReference(
                currentNode,
                virtualRegister->getBackingStorage()->getSymbolReference(),
                _cg);
            ...
            cursor = generateSILInstruction(self()->cg(), TR::InstOpCode::MVHI,
                                            currentNode, tempMR, 0,
                                            currentInstruction);
            _cg->traceRAInstruction(cursor);
            cursor =
                generateRXInstruction(self()->cg(), TR::InstOpCode::STFH,
                                      currentNode, targetRegister, mr, cursor);
            _cg->traceRAInstruction(cursor);
          ...
          // the 32-bit virtual register is currently assigned to a real
          // register
          // do register copy
          cursor = self()->registerCopy(
              currentInstruction, currentAssignedRegisterRK,
    ...
    if (currentAssignedRegister != NULL) {
      ...
      if (!self()->isAssignable(currentTargetVirtual,
        ...
        if (spareReg == NULL) {
          ...
          virtualRegister->block();
          if (enableHighWordRA && virtualRegister->is64BitReg()) {
            // if we end up spilling currentTargetVirtual to HPR, make sure to
            // not pick the HPR of targetRegister since it will not
            // free up the full 64 bit register
            spareReg = self()->freeBestRegister(
                currentInstruction, currentTargetVirtual,
                currentTargetVirtual->getKind(), availHighWordRegMap, false,
                true);
          } else {
            spareReg = self()->freeBestRegister(
                currentInstruction, currentTargetVirtual,
                currentTargetVirtual->getKind(), availHighWordRegMap, true);
          }
          ...
          // freeBestRegister could spill currentTargetVirtual directly to free
          // up targetRegister.
          // If currentTargetVirtual is 64-bit, spareReg will be a GPR
          // (targetRegister's low word)
          // however, targetRegister could be an HPR because we might only need
          // its high word. In this case
          // we must not generate register move of LGR targetRegister,spareReg
          // because they will be the same register
          // and we must leave currentTargetVirtual spilled instead of assigning
          // it to spareReg.
          if (targetRegister->isHighWordRegister() &&
              currentTargetVirtual->is64BitReg() &&
              targetRegister->getRegisterNumber() ==
                  spareReg->getHighWordRegister()->getRegisterNumber())
            doNotRegCopy = true;
          virtualRegister->unblock();
        }

        // if currentTargetVirtual is low word only, spare Reg will be low word
        // but virtual Reg could require full size, in this case, need to free
        // the HW of target Reg
        if (enableHighWordRA && virtualRegister->is64BitReg()) {
          if (targetRegisterHW->getState() != TR::RealRegister::Free &&
              targetRegisterHW->getState() != TR::RealRegister::Unlatched) {
            // if the targetReg and targetRegHW are assigned to two different
            // virtual regs
            if (targetRegisterHW->getAssignedRegister() &&
                targetRegisterHW->getAssignedRegister() !=
                    targetRegister->getAssignedRegister()) {
              // TR_ASSERTC( currentTargetVirtual->isLowWordOnly(),_cg->comp(),
              // "currentTargetVirtual is not LWOnly but HW is clobbered by
              // another vreg?");
              virtualRegister->block();
              currentTargetVirtual->block();
              spareReg->block(); // to do: is this necessary? only need to block
                                 // HPR?
              // free up this highword register by either spilling it or moving
              // it
              cursor = self()->freeHighWordRegister(
                  currentInstruction, targetRegisterHW, instFlags);
              spareReg->unblock();
              currentTargetVirtual->unblock();
              virtualRegister->unblock();
            }
        ...
        if (spareReg == NULL) {
          ...
          cursor = self()->registerCopy(currentInstruction, rk,
                                        currentAssignedRegister, targetRegister,
                                        self()->cg(), instFlags);
          currentAssignedRegister->setState(TR::RealRegister::Unlatched);
          currentAssignedRegister->setAssignedRegister(NULL);
        } else {
      ...
      if (spareReg == NULL && virtualRegister->isPlaceholderReg()) {
        ...
        // In zLinux, GPR6 may be used for param passing, in this case, we can't
        // spill to it
        if (TR::Compiler->target.isLinux()) {
          // HPR7-12 are available
          self()->spillRegister(currentInstruction, currentTargetVirtual,
                                availHWRegs & 0x1F800000);
        } else {
          // can only spill to preserved registers (HPR6-HPR12)
          self()->spillRegister(currentInstruction, currentTargetVirtual,
                                availHWRegs & 0x1FC00000);
          // spillRegister(currentInstruction, currentTargetVirtual,
          // 0x1FC00000);
        }
        ...
        if (spillTargetHWReg) {
          self()->spillRegister(currentInstruction,
                                targetRegisterHW->getAssignedRegister(), 0);
        }
        ...
        if (spareReg == NULL) {
          ...
          // its high word. In this case
          // we must not generate register move of LGR targetRegister,spareReg
          // because they are the same register
          // and we must leave currentTargetVirtual spilled instead of assigning
          // it to spareReg.
          if (targetRegister->isHighWordRegister() &&
              currentTargetVirtual->is64BitReg() &&
  ...
  if (targetRegister->getState() != TR::RealRegister::Locked) {
    ...
    traceMsg(comp, "             This assignment is equivalent to using a hard "
                   "coded real register.\n");
    if (_cg->getRAPassAR() && virtualRegister->getAssignedRegister() &&
        virtualRegister->getAssignedRegister() != targetRegister)
    ...
    if (self()->supportLockedRegisterAssignment()) {
      ...
      if (!_cg->getRAPassAR() &&
        ...
        // register is locked but assigned to a VR, need to re-assign VR to
        // another reg via reg copy or spill it
        _cg->traceRegisterAssignment(" Freeing locked register %R ",
                                     targetRegister);
        uint64_t availRegMask = 0xffffffff;
        if (toFreeRegister->isUsedInMemRef())
          availRegMask &=
              ~(TR::RealRegister::getBitMask(TR::RealRegister::GPR0));

        TR::RealRegister *bestRegister = NULL;
        if ((bestRegister = self()->findBestFreeRegister(
                 currentInstruction, toFreeRegister->getKind(), toFreeRegister,
                 availRegMask)) == NULL) {
          bestRegister =
              self()->freeBestRegister(currentInstruction, toFreeRegister,
                                       toFreeRegister->getKind(), availRegMask);
        }
        self()->registerCopy(currentInstruction, toFreeRegister->getKind(),
                             toRealRegister(targetRegister), bestRegister, _cg,
                             0);
...
uint64_t
OMR::Z::Machine::filterColouredRegisterConflicts(TR::Register *targetRegister,
                                                 TR::Register *siblingRegister,
                                                 TR::Instruction *currInst) {
  uint64_t mask = 0xffffffff;
  TR::Compilation *comp = TR::comp();
  TR::list<TR::Register *> conflictRegs(
  ...
  if (currInst->defsAnyRegister(targetRegister)) {
    ...
    for (auto reg = conflictRegs.begin(); reg != conflictRegs.end(); ++reg) {
      TR::Register *cr =
          (*reg)->getRealRegister() ? NULL : (*reg)->getAssignedRegister();
      if (cr == NULL)
        cr = (*reg)->getRealRegister() ? NULL : (*reg)->getColouredRegister();
      if (cr && targetRegister != (*reg) &&
          (*reg)->getAssignedRegister() != targetRegister &&
          (siblingRegister == NULL || (*reg) != siblingRegister))
        mask &= ~TR::RealRegister::getBitMask(
            toRealRegister(cr)->getRegisterNumber());
    }
  }
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::Machine::initialiseRegisterFile() {
  // *this    swipeable for debugging purposes
  ...
  _registerFile[TR::RealRegister::GPR0] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR0, TR::RealRegister::GPR0Mask, _cg);

  _registerFile[TR::RealRegister::GPR1] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR1, TR::RealRegister::GPR1Mask, _cg);

  _registerFile[TR::RealRegister::GPR2] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR2, TR::RealRegister::GPR2Mask, _cg);

  _registerFile[TR::RealRegister::GPR3] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR3, TR::RealRegister::GPR3Mask, _cg);

  _registerFile[TR::RealRegister::GPR4] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR4, TR::RealRegister::GPR4Mask, _cg);

  _registerFile[TR::RealRegister::GPR5] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR5, TR::RealRegister::GPR5Mask, _cg);

  _registerFile[TR::RealRegister::GPR6] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR6, TR::RealRegister::GPR6Mask, _cg);

  _registerFile[TR::RealRegister::GPR7] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR7, TR::RealRegister::GPR7Mask, _cg);

  _registerFile[TR::RealRegister::GPR8] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR8, TR::RealRegister::GPR8Mask, _cg);

  _registerFile[TR::RealRegister::GPR9] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR9, TR::RealRegister::GPR9Mask, _cg);

  _registerFile[TR::RealRegister::GPR10] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR10, TR::RealRegister::GPR10Mask,
                       _cg);

  _registerFile[TR::RealRegister::GPR11] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR11, TR::RealRegister::GPR11Mask,
                       _cg);

  _registerFile[TR::RealRegister::GPR12] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR12, TR::RealRegister::GPR12Mask,
                       _cg);

  _registerFile[TR::RealRegister::GPR13] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR13, TR::RealRegister::GPR13Mask,
                       _cg);

  _registerFile[TR::RealRegister::GPR14] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR14, TR::RealRegister::GPR14Mask,
                       _cg);

  _registerFile[TR::RealRegister::GPR15] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
  ...
  _registerFile[TR::RealRegister::FPR0] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR0, TR::RealRegister::FPR0Mask, _cg);

  _registerFile[TR::RealRegister::FPR1] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR1, TR::RealRegister::FPR1Mask, _cg);

  _registerFile[TR::RealRegister::FPR2] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR2, TR::RealRegister::FPR2Mask, _cg);

  _registerFile[TR::RealRegister::FPR3] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR3, TR::RealRegister::FPR3Mask, _cg);

  _registerFile[TR::RealRegister::FPR4] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR4, TR::RealRegister::FPR4Mask, _cg);

  _registerFile[TR::RealRegister::FPR5] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR5, TR::RealRegister::FPR5Mask, _cg);

  _registerFile[TR::RealRegister::FPR6] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR6, TR::RealRegister::FPR6Mask, _cg);

  _registerFile[TR::RealRegister::FPR7] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR7, TR::RealRegister::FPR7Mask, _cg);

  _registerFile[TR::RealRegister::FPR8] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR8, TR::RealRegister::FPR8Mask, _cg);

  _registerFile[TR::RealRegister::FPR9] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR9, TR::RealRegister::FPR9Mask, _cg);

  _registerFile[TR::RealRegister::FPR10] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR10, TR::RealRegister::FPR10Mask,
                       _cg);

  _registerFile[TR::RealRegister::FPR11] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR11, TR::RealRegister::FPR11Mask,
                       _cg);

  _registerFile[TR::RealRegister::FPR12] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR12, TR::RealRegister::FPR12Mask,
                       _cg);

  _registerFile[TR::RealRegister::FPR13] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR13, TR::RealRegister::FPR13Mask,
                       _cg);

  _registerFile[TR::RealRegister::FPR14] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR14, TR::RealRegister::FPR14Mask,
                       _cg);

  _registerFile[TR::RealRegister::FPR15] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR15, TR::RealRegister::FPR15Mask,
                       _cg);

  // Initialize Access Regs
  _registerFile[TR::RealRegister::AR0] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR0,
                       TR::RealRegister::AR0Mask, _cg);

  _registerFile[TR::RealRegister::AR1] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR1,
                       TR::RealRegister::AR1Mask, _cg);

  _registerFile[TR::RealRegister::AR2] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR2,
                       TR::RealRegister::AR2Mask, _cg);

  _registerFile[TR::RealRegister::AR3] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR3,
                       TR::RealRegister::AR3Mask, _cg);

  _registerFile[TR::RealRegister::AR4] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR4,
                       TR::RealRegister::AR4Mask, _cg);

  _registerFile[TR::RealRegister::AR5] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR5,
                       TR::RealRegister::AR5Mask, _cg);

  _registerFile[TR::RealRegister::AR6] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR6,
                       TR::RealRegister::AR6Mask, _cg);

  _registerFile[TR::RealRegister::AR7] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR7,
                       TR::RealRegister::AR7Mask, _cg);

  _registerFile[TR::RealRegister::AR8] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR8,
                       TR::RealRegister::AR8Mask, _cg);

  _registerFile[TR::RealRegister::AR9] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR9,
                       TR::RealRegister::AR9Mask, _cg);

  _registerFile[TR::RealRegister::AR10] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::AR10, TR::RealRegister::AR10Mask, _cg);

  _registerFile[TR::RealRegister::AR11] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::AR11, TR::RealRegister::AR11Mask, _cg);

  _registerFile[TR::RealRegister::AR12] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::AR12, TR::RealRegister::AR12Mask, _cg);

  _registerFile[TR::RealRegister::AR13] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::AR13, TR::RealRegister::AR13Mask, _cg);

  _registerFile[TR::RealRegister::AR14] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::AR14, TR::RealRegister::AR14Mask, _cg);

  _registerFile[TR::RealRegister::AR15] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::AR15, TR::RealRegister::AR15Mask, _cg);

  // Initialize High Regs
  _registerFile[TR::RealRegister::HPR0] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR0, TR::RealRegister::HPR0Mask, _cg);

  _registerFile[TR::RealRegister::HPR1] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR1, TR::RealRegister::HPR1Mask, _cg);

  _registerFile[TR::RealRegister::HPR2] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR2, TR::RealRegister::HPR2Mask, _cg);

  _registerFile[TR::RealRegister::HPR3] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR3, TR::RealRegister::HPR3Mask, _cg);

  _registerFile[TR::RealRegister::HPR4] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR4, TR::RealRegister::HPR4Mask, _cg);

  _registerFile[TR::RealRegister::HPR5] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR5, TR::RealRegister::HPR5Mask, _cg);

  _registerFile[TR::RealRegister::HPR6] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR6, TR::RealRegister::HPR6Mask, _cg);

  _registerFile[TR::RealRegister::HPR7] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR7, TR::RealRegister::HPR7Mask, _cg);

  _registerFile[TR::RealRegister::HPR8] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR8, TR::RealRegister::HPR8Mask, _cg);

  _registerFile[TR::RealRegister::HPR9] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR9, TR::RealRegister::HPR9Mask, _cg);

  _registerFile[TR::RealRegister::HPR10] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR10, TR::RealRegister::HPR10Mask,
                       _cg);

  _registerFile[TR::RealRegister::HPR11] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR11, TR::RealRegister::HPR11Mask,
                       _cg);

  _registerFile[TR::RealRegister::HPR12] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR12, TR::RealRegister::HPR12Mask,
                       _cg);

  _registerFile[TR::RealRegister::HPR13] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR13, TR::RealRegister::HPR13Mask,
                       _cg);

  _registerFile[TR::RealRegister::HPR14] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR14, TR::RealRegister::HPR14Mask,
                       _cg);

  _registerFile[TR::RealRegister::HPR15] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::HPR15, TR::RealRegister::HPR15Mask,
                       _cg);
  ...
  _registerFile[TR::RealRegister::VRF16] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF16, TR::RealRegister::VRF16Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF17] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF17, TR::RealRegister::VRF17Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF18] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF18, TR::RealRegister::VRF18Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF19] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF19, TR::RealRegister::VRF19Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF20] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF20, TR::RealRegister::VRF20Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF21] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF21, TR::RealRegister::VRF21Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF22] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF22, TR::RealRegister::VRF22Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF23] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF23, TR::RealRegister::VRF23Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF24] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF24, TR::RealRegister::VRF24Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF25] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF25, TR::RealRegister::VRF25Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF26] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF26, TR::RealRegister::VRF26Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF27] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF27, TR::RealRegister::VRF27Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF28] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF28, TR::RealRegister::VRF28Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF29] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF29, TR::RealRegister::VRF29Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF30] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF30, TR::RealRegister::VRF30Mask,
                       _cg);

  _registerFile[TR::RealRegister::VRF31] = new (_cg->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF31, TR::RealRegister::VRF31Mask,
                       _cg);
  _registerFile[TR::RealRegister::HPR0]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR0]);
  _registerFile[TR::RealRegister::HPR1]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR1]);
  _registerFile[TR::RealRegister::HPR2]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR2]);
  _registerFile[TR::RealRegister::HPR3]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR3]);
  _registerFile[TR::RealRegister::HPR4]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR4]);
  _registerFile[TR::RealRegister::HPR5]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR5]);
  _registerFile[TR::RealRegister::HPR6]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR6]);
  _registerFile[TR::RealRegister::HPR7]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR7]);
  _registerFile[TR::RealRegister::HPR8]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR8]);
  _registerFile[TR::RealRegister::HPR9]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR9]);
  _registerFile[TR::RealRegister::HPR10]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR10]);
  _registerFile[TR::RealRegister::HPR11]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR11]);
  _registerFile[TR::RealRegister::HPR12]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR12]);
  _registerFile[TR::RealRegister::HPR13]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR13]);
  _registerFile[TR::RealRegister::HPR14]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR14]);
  _registerFile[TR::RealRegister::HPR15]->setLowWordRegister(
      _registerFile[TR::RealRegister::GPR15]);

  _registerFile[TR::RealRegister::GPR0]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR0]);
  _registerFile[TR::RealRegister::GPR1]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR1]);
  _registerFile[TR::RealRegister::GPR2]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR2]);
  _registerFile[TR::RealRegister::GPR3]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR3]);
  _registerFile[TR::RealRegister::GPR4]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR4]);
  _registerFile[TR::RealRegister::GPR5]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR5]);
  _registerFile[TR::RealRegister::GPR6]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR6]);
  _registerFile[TR::RealRegister::GPR7]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR7]);
  _registerFile[TR::RealRegister::GPR8]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR8]);
  _registerFile[TR::RealRegister::GPR9]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR9]);
  _registerFile[TR::RealRegister::GPR10]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR10]);
  _registerFile[TR::RealRegister::GPR11]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR11]);
  _registerFile[TR::RealRegister::GPR12]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR12]);
  _registerFile[TR::RealRegister::GPR13]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR13]);
  _registerFile[TR::RealRegister::GPR14]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR14]);
  _registerFile[TR::RealRegister::GPR15]->setHighWordRegister(
      _registerFile[TR::RealRegister::HPR15]);
...
uint32_t *OMR::Z::Machine::initializeGlobalRegisterTable() {
  ...
  if (!comp->getOption(TR_DisableRegisterPressureSimulation)) {
    int32_t p = 0;
    static char *dontInitializeGlobalRegisterTableFromLinkage =
        feGetEnv("TR_dontInitializeGlobalRegisterTableFromLinkage");
    bool enableHighWordGRA = _cg->supportsHighWordFacility() &&
                             !comp->getOption(TR_DisableHighWordRA);
    if (dontInitializeGlobalRegisterTableFromLinkage) {
      ...
      //
      // p = addGlobalReg(TR::RealRegister::GPR0, p); // Local register assigner
      // can't handle virtuals assigned to GPR0 appearing in memrefs
      ...
      //
      p = self()->addGlobalReg(TR::RealRegister::GPR6, p); // NOTE: GPR6 must be
                                                           // avoided if
                                                           // on-demand literal
                                                           // pool opt isn't run
      static char *noGraFIX = feGetEnv("TR_NOGRAFIX");
      // Exclude GPR7 if we are not on Freeway+ hardware
      if (!noGraFIX && !comp->getOption(TR_DisableLongDispStackSlot) &&
      ...
      if (enableHighWordGRA) {
        ...
        // for preserved regs, we can only use HPR6-12 because VM only
        // saves/restores those
      ...
      if (comp->getOption(TR_Enable390AccessRegs)) {
        for (int32_t i = TR::RealRegister::FirstAR;
             i <= TR::RealRegister::LastAR; i++) {
...
//
      ...
      if (linkage
              ->isZLinuxLinkageType()) // ordering seems to make crashes on zos.
      {
      ...
      if ((_cg->isLiteralPoolOnDemandOn() && !linkage->isZLinuxLinkageType()) ||
      ...
      if (linkage
              ->isZLinuxLinkageType()) // ordering seems to make crashes on zos.
      {
        for (i = TR::RealRegister::LastAssignableGPR;
        ...
        //
        for (i = TR::RealRegister::FirstGPR;
      ...
      if (TR::Compiler->target.isZOS()) {
        p = self()->addGlobalRegLater(
            _cg->getS390Linkage()->getStackPointerRegister(), p);
      }

      // Special regs that add to prologue cost
      //
      p = self()->addGlobalRegLater(linkage->getEnvironmentPointerRegister(),
                                    p);
      ...
      if (_cg->globalAccessRegistersSupported()) {
        ...
        for (i = self()->getFirstGlobalGPRRegisterNumber();
             i <= self()->getLastGlobalGPRRegisterNumber(); i++) {
          reg = (TR::RealRegister::RegNum)(
              _globalRegisterNumberToRealRegisterMap[i] -
              TR::RealRegister::FirstGPR + TR::RealRegister::FirstAR);
          p = self()->addGlobalReg(reg, p);
        }
      ...
      // initGlobalVectorRegisterMap sets first/last global grns and overlapped
      // grns
      ...
      for (int32_t i = 0; i < p; i++) {
        ...
        if (_globalRegisterNumberToRealRegisterMap[i] ==
            linkage->getEnvironmentPointerRegister())
  ...
  //   _globalRegisterNumberToRealRegisterMap = new
  //   uint32_t[NUM_S390_GPR+NUM_S390_FPR]; // Make room for max num GPRs + FPRs

  // GPRs

  self()->setLastVolatileNonLinkGPR(0);

  _globalRegisterNumberToRealRegisterMap[0] =
      TR::RealRegister::GPR3; // volatile and 3rd param
  _globalRegisterNumberToRealRegisterMap[1] =
      TR::RealRegister::GPR2; // volatile and 3rd param
  _globalRegisterNumberToRealRegisterMap[2] =
      TR::RealRegister::GPR1; // volatile and 3rd param
  ...
  _globalRegisterNumberToRealRegisterMap[GLOBAL_REG_FOR_LITPOOL] =
      (uint32_t)(-1); // preserved

  static char *noGraFIX = feGetEnv("TR_NOGRAFIX");
  // Exclude GPR7 if we are not on Freeway+ hardware
  if (!noGraFIX && !comp->getOption(TR_DisableLongDispStackSlot) &&
      _cg->getExtCodeBaseRegisterIsFree()) {
    _globalRegisterNumberToRealRegisterMap[4] =
        TR::RealRegister::GPR7; // preserved
  } else {
    _globalRegisterNumberToRealRegisterMap[4] = (uint32_t)(-1); // preserved
  }

  _globalRegisterNumberToRealRegisterMap[5] =
      TR::RealRegister::GPR8; // preserved
  _globalRegisterNumberToRealRegisterMap[6] =
      TR::RealRegister::GPR9; // preserved
  _globalRegisterNumberToRealRegisterMap[7] =
      TR::RealRegister::GPR10; // preserved
  _globalRegisterNumberToRealRegisterMap[8] =
      TR::RealRegister::GPR11; // preserved -- non-Java may lock
  _globalRegisterNumberToRealRegisterMap[9] =
      TR::RealRegister::GPR12; // preserved -- non-Java may lock

  // Access Registers
  self()->setFirstGlobalAccessRegisterNumber(10);
  _globalRegisterNumberToRealRegisterMap[10] =
      (uint32_t)(-1); // TR::RealRegister::AR0; locked on zLinux
  _globalRegisterNumberToRealRegisterMap[11] =
      (uint32_t)(-1); // TR::RealRegister::AR1; locked on zLinux
  // /// /// Disable and test on only on ZOS where the other ARs are protected
  // by system linkage
  _globalRegisterNumberToRealRegisterMap[12] =
      (uint32_t)(-1); // TR::RealRegister::AR2;
  _globalRegisterNumberToRealRegisterMap[13] =
      (uint32_t)(-1); // TR::RealRegister::AR3;
  _globalRegisterNumberToRealRegisterMap[14] =
      (uint32_t)(-1); // TR::RealRegister::AR4;
  _globalRegisterNumberToRealRegisterMap[15] =
      (uint32_t)(-1); // TR::RealRegister::AR5;
  _globalRegisterNumberToRealRegisterMap[16] =
      (uint32_t)(-1); // TR::RealRegister::AR6;
  _globalRegisterNumberToRealRegisterMap[17] =
      (uint32_t)(-1); // TR::RealRegister::AR7;
  // /// ///

  _globalRegisterNumberToRealRegisterMap[18] =
      TR::RealRegister::AR8; // preserved ZOS
  _globalRegisterNumberToRealRegisterMap[19] =
      (uint32_t)(-1); // TR::RealRegister::AR9;      // preserved ZOS
  _globalRegisterNumberToRealRegisterMap[20] =
      (uint32_t)(-1); // TR::RealRegister::AR10;     // preserved ZOS
  _globalRegisterNumberToRealRegisterMap[21] =
      (uint32_t)(-1); // TR::RealRegister::AR11;     // preserved ZOS
  _globalRegisterNumberToRealRegisterMap[22] =
      (uint32_t)(-1); // TR::RealRegister::AR12;     // preserved ZOS
  _globalRegisterNumberToRealRegisterMap[23] =
      (uint32_t)(-1); // TR::RealRegister::AR13;     // preserved ZOS
  _globalRegisterNumberToRealRegisterMap[24] =
      (uint32_t)(-1); // TR::RealRegister::AR14;     // preserved ZOS
  _globalRegisterNumberToRealRegisterMap[25] =
      (uint32_t)(-1); // TR::RealRegister::AR15;     // preserved ZOS
  self()->setLastGlobalAccessRegisterNumber(25);

  self()->setLastGlobalGPRRegisterNumber(25); // Index of last global GPR
  self()->setLast8BitGlobalGPRRegisterNumber(
      25); // Index of last global 8bit Reg

  // additional (forced) restricted regs.
  // Similar code in TR_S390PrivateLinkage::initS390RealRegisterLinkage() for RA

  for (int32_t i = self()->getFirstGlobalGPRRegisterNumber();
       i < self()->getLastGlobalGPRRegisterNumber(); ++i) {
  ...
  if (TR::Compiler->target.is64Bit() ||
    ...
    for (int32_t i = self()->getFirstGlobalAccessRegisterNumber();
         i <= self()->getLastGlobalAccessRegisterNumber(); ++i) {
      _globalRegisterNumberToRealRegisterMap[i] = (uint32_t)(-1);
    }
  }

  // FPRs
  _globalRegisterNumberToRealRegisterMap[26] =
      TR::RealRegister::FPR7; // volatile float
  _globalRegisterNumberToRealRegisterMap[27] =
      TR::RealRegister::FPR5; // volatile float
  _globalRegisterNumberToRealRegisterMap[28] =
      TR::RealRegister::FPR3; // volatile float
  _globalRegisterNumberToRealRegisterMap[29] =
      TR::RealRegister::FPR1; // volatile float
  self()->setLastVolatileNonLinkFPR(29);

#if defined(ENABLE_PRESERVED_FPRS)
  setLastVolatileNonLinkFPR(29);

  _globalRegisterNumberToRealRegisterMap[30] =
      TR::RealRegister::FPR6; // volatile and 4th param float
  _globalRegisterNumberToRealRegisterMap[31] =
      TR::RealRegister::FPR4; // volatile and 3rd param float
  _globalRegisterNumberToRealRegisterMap[32] =
      TR::RealRegister::FPR2; // volatile and 2nd param float
  _globalRegisterNumberToRealRegisterMap[33] =
      TR::RealRegister::FPR0; // volatile and 1st param float
  setLastLinkageFPR(33);

  _globalRegisterNumberToRealRegisterMap[34] =
      TR::RealRegister::FPR15; // preserved float or vector
  _globalRegisterNumberToRealRegisterMap[35] =
      TR::RealRegister::FPR14; // preserved float or vector
  _globalRegisterNumberToRealRegisterMap[36] =
      TR::RealRegister::FPR13; // preserved float or vector
  _globalRegisterNumberToRealRegisterMap[37] =
      TR::RealRegister::FPR12; // preserved float or vector
  _globalRegisterNumberToRealRegisterMap[38] =
      TR::RealRegister::FPR11; // preserved float or vector
  _globalRegisterNumberToRealRegisterMap[39] =
      TR::RealRegister::FPR10; // preserved float or vector
  _globalRegisterNumberToRealRegisterMap[40] =
      TR::RealRegister::FPR9; // preserved float or vector
  _globalRegisterNumberToRealRegisterMap[41] =
      TR::RealRegister::FPR8; // preserved float or vector
#else
  _globalRegisterNumberToRealRegisterMap[30] =
      TR::RealRegister::FPR15; // volatile float or vector
  _globalRegisterNumberToRealRegisterMap[31] =
      TR::RealRegister::FPR14; // volatile float or vector
  _globalRegisterNumberToRealRegisterMap[32] =
      TR::RealRegister::FPR13; // volatile float or vector
  _globalRegisterNumberToRealRegisterMap[33] =
      TR::RealRegister::FPR12; // volatile float or vector
  _globalRegisterNumberToRealRegisterMap[34] =
      TR::RealRegister::FPR11; // volatile float or vector
  _globalRegisterNumberToRealRegisterMap[35] =
      TR::RealRegister::FPR10; // volatile float or vector
  _globalRegisterNumberToRealRegisterMap[36] =
      TR::RealRegister::FPR9; // volatile float or vector
  _globalRegisterNumberToRealRegisterMap[37] =
      TR::RealRegister::FPR8; // volatile float or vector
 ...
 * (zOS/zLinux 31/64) have the same linkage conventions
 * Each register is explicitly set to stay consistent with a majority of the
 * global register number setup procedures across z-supported
...
uint32_t OMR::Z::Machine::initGlobalVectorRegisterMap(uint32_t vectorOffset) {
  if (!_cg->getSupportsVectorRegisters() &&
      !TR::comp()->getOption(TR_DisableVectorRegGRA)) {
  ...
  const bool useEntireRegFile =
      ((hideLowerHalf == NULL) && _cg->getSupportsVectorRegisters());
...
#define addToGRAMap(regNum)                                                    \
  ...
  // vector in GRA depends on it.
  // This greatly simplifies GRA.
  // By setting first VRF reg num to first FPR, we'll essentially see 16 + 32
  // grn's for vectors BUT gra takes care
  // to ensure we do things right i.e if any of the 16 fpr grn's are assigned,
  // remove the corresponding reg in the 32.
  ...
  // Similar to HPR/GPR overlap, FPR and VRF will have same 'lastOverlapping'
  // grn.
...
void OMR::Z::Machine::lockGlobalRegister(int32_t globalRegisterTableIndex) {
  TR::Compilation *comp = TR::comp();
  if (comp->getOption(TR_DisableRegisterPressureSimulation)) {
    _globalRegisterNumberToRealRegisterMap[globalRegisterTableIndex] =
        (uint32_t)(-1);
  } else {
    // TODO: make sure this method is not called without
    // TR_DisableRegisterPressureSimulation
    // TR_ASSERTC( false,comp, "lockGlobalRegister() does not work with new
    // pickRegister\n");
  }
}

void OMR::Z::Machine::releaseGlobalRegister(int32_t globalRegisterTableIndex,
                                            TR::RealRegister::RegNum gReg) {
  _globalRegisterNumberToRealRegisterMap[globalRegisterTableIndex] = gReg;
}

int OMR::Z::Machine::findGlobalRegisterIndex(TR::RealRegister::RegNum gReg) {
  int32_t index = -1;
...
TR_GlobalRegisterNumber OMR::Z::Machine::setFirstGlobalAccessRegisterNumber(
    TR_GlobalRegisterNumber reg) {
  self()->setFirstGlobalRegisterNumber(TR_AR, reg);
  return _firstGlobalAccessRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setLastGlobalAccessRegisterNumber(
    TR_GlobalRegisterNumber reg) {
...
TR::Register *OMR::Z::Machine::getAccessRegisterFromGlobalRegisterNumber(
    TR_GlobalRegisterNumber reg) {
  if (reg >= self()->getFirstGlobalAccessRegisterNumber() &&
...
OMR::Z::Machine::getGPRFromGlobalRegisterNumber(TR_GlobalRegisterNumber reg) {
  if (reg >= self()->getFirstGlobalGPRRegisterNumber() &&
...
OMR::Z::Machine::getHPRFromGlobalRegisterNumber(TR_GlobalRegisterNumber reg) {
  if (reg >= self()->getFirstGlobalHPRRegisterNumber() &&
...
void OMR::Z::Machine::createRegisterAssociationDirective(
  ...
  if (_cg->supportsHighWordFacility() &&
      !comp->getOption(TR_DisableHighWordRA)) {
    int32_t lastHPR =
        last + TR::RealRegister::LastHPR - TR::RealRegister::FirstHPR;
    associations =
        new (_cg->trHeapMemory(), TR_MemoryBase::RegisterDependencyConditions)
            TR::RegisterDependencyConditions(0, lastHPR, _cg);
  } else {
    associations =
        new (_cg->trHeapMemory(), TR_MemoryBase::RegisterDependencyConditions)
            TR::RegisterDependencyConditions(0, last, _cg);
  }
  ...
  if (_cg->supportsHighWordFacility() &&
      !comp->getOption(TR_DisableHighWordRA)) {
    for (int32_t i = TR::RealRegister::FirstHPR;
         i < TR::RealRegister::LastHPR + 1; i++) {
      TR::RealRegister::RegNum regNum = (TR::RealRegister::RegNum)(i);
      associations->addPostCondition(
          self()->getVirtualAssociatedWithReal(regNum), regNum);
    }
  }

  TR::Instruction *cursor1 =
      new (_cg->trHeapMemory(), TR_MemoryBase::S390Instruction) TR::Instruction(
 ...
 */
bool OMR::Z::Machine::isRestrictedReg(TR::RealRegister::RegNum reg) {
  static const TR::RealRegister::RegNum regList[] = {
      TR::RealRegister::GPR9, TR::RealRegister::GPR10, TR::RealRegister::GPR11,
      TR::RealRegister::GPR12,
  };
  TR::Compilation *comp = TR::comp();
  static const int32_t regListSize =
      (sizeof(regList) / sizeof(TR::RealRegister::RegNum));
...
#ifdef DEBUG
    if (!printed) {
      fprintf(stderr, "Invalid value for numRestrictedRegs or on-demand lit "
                      "pool is disabled. Needs to range from 0 to %d\n",
              regListSize);
      printed = true;
    }
#endif
...
bool OMR::Z::Machine::supportLockedRegisterAssignment() {
  return false; // TODO : Identity needs folding
}

TR::RealRegister *OMR::Z::Machine::getRegisterFile(int32_t i) {
  return _registerFile[i];
}
...
void OMR::Z::Machine::takeRegisterStateSnapShot() {
  for (int32_t i = TR::RealRegister::FirstGPR;
    ...
    if (_assignedRegisterSnapShot[i] &&
        _assignedRegisterSnapShot[i]->getAssignedRegister() == NULL) {
      _cg->traceRegisterAssignment("\nOOL: %R : %R", _registerFile[i],
                                   _assignedRegisterSnapShot[i]);
      TR_ASSERT(_registerFile[i]->isHighWordRegister(), "OOL: HPR spill?? %d",
                i);
  ...
  for (int32_t i=0;i<(NUM_S390_GPR+NUM_S390_FPR+NUM_S390_AR);i++)
     {
     _globalRegisterNumberToRealRegisterMapSnapShot[i] =
  _globalRegisterNumberToRealRegisterMap[i];
     }
  */
}
  ...
  for (int32_t i = TR::RealRegister::FirstGPR;
    ...
    if (_registerFile[i]->getState() == TR::RealRegister::Free) {
      ...
      if (_registerFile[i]->getAssignedRegister() != NULL &&
        ...
        // assigned to the current real register
        ...
        //  assignment of GPR_3545 (currently to GPR15) to GPR12.
        //  When we subsequently handle _registerFile[15], we cannot blindly
        //  reset GPR_3545's assigned register to NULL,
    ...
    if (_registerFile[i]->getState() == TR::RealRegister::Assigned &&
        !_containsHPRSpillSnapShot[i]) {
      //_cg->traceRegisterAssignment("\nOOL: restoring %R : %R",
      //_registerFile[i], _registerFile[i]->getAssignedRegister());
      if (!_registerFile[i]->getAssignedRegister()->is64BitReg() ||
          !_registerFile[i]->isHighWordRegister()) {
    ...
    // in one OOL hot path
    // the dead in a later OOL hot path) and has never been used in any OOL cold
    // path or main line, then we need
    // to make sure to not include this register in the snapshot once the
    // register is dead, otherwise
    // we will end up having a virtual register with futureUseCount=0 assigned
    // to a real reg dangling in the
...
TR::RegisterDependencyConditions *OMR::Z::Machine::createDepCondForLiveGPRs(
  ...
  if (_cg->supportsHighWordFacility() &&
    ...
    for (i = TR::RealRegister::FirstHPR; i <= TR::RealRegister::LastHPR; i++) {
      if (self()->getS390RealRegister(i)->getState() ==
              TR::RealRegister::Assigned &&
          self()->getS390RealRegister(i)->getAssignedRegister()) {
        if (self()->getS390RealRegister(i)->getAssignedRegister() !=
            self()
                ->getS390RealRegister(i)
                ->getLowWordRegister()
                ->getAssignedRegister())
          c++;
        // if a HPR is assigned to a virtReg but a virtReg is not assigned to
        // HPR, we must have spilled the virtReg to HPR
        if (!self()
                 ->getS390RealRegister(i)
                 ->getAssignedRegister()
                 ->getAssignedRegister())
  ...
  if (c) {
    deps =
        new (_cg->trHeapMemory(), TR_MemoryBase::RegisterDependencyConditions)
            TR::RegisterDependencyConditions(0, c, _cg);
    for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastVRF;
         i = ((i == TR::RealRegister::LastAssignableGPR)
                  ? TR::RealRegister::FirstVRF
                  : i + 1)) {
    ...
    if (_cg->supportsHighWordFacility() &&
        !comp->getOption(TR_DisableHighWordRA)) {
      for (i = TR::RealRegister::FirstHPR; i <= TR::RealRegister::LastHPR;
           i++) {
        if (self()->getS390RealRegister(i)->getState() ==
                TR::RealRegister::Assigned &&
            self()->getS390RealRegister(i)->getAssignedRegister()) {
          if (self()->getS390RealRegister(i)->getAssignedRegister() !=
            ...
            deps->addPostCondition(
                self()->getS390RealRegister(i)->getAssignedRegister(),
                self()->getS390RealRegister(i)->getRegisterNumber());
            self()
                ->getS390RealRegister(i)
                ->getAssignedRegister()
                ->incFutureUseCount();
          ...
          if (!self()
                   ...
                   ->getAssignedRegister()
                   ->getAssignedRegister()) {
            // this is not a conflicting dependency rather a directive to tell
            // RA to mark the virt reg as a HPR spill
            deps->addPostCondition(self()->getS390RealRegister(i),
