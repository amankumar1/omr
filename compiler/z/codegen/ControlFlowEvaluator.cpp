...
extern TR::Instruction *generateS390AggregateCompareAndBranchOps(
    TR::Node *node, TR::CodeGenerator *cg,
    TR::InstOpCode::S390BranchCondition fBranchOpCond,
    TR::InstOpCode::S390BranchCondition rBranchOpCond,
    TR::InstOpCode::S390BranchCondition &retBranchOpCond,
...
void killRegisterIfNotLocked(TR::CodeGenerator *cg,
  ...
  if (cg->machine()->getS390RealRegister(reg)->getState() !=
 ...
 * NOP instruction,
 * a BRC, has only a +ve displacement of 2^12 (4k), and so in some cases it is
 * not safe to use it
 * on a G5 platform (no problems with BRCL).  At this point, since instructions
 * have not yet
 * been generated, there is no good way to guage how far away the label will be,
 * so we cannot
...
static bool virtualGuardHelper(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (comp->compileRelocatableCode()) {
    ...
    default:
      TR_ASSERT(0, "got AOT guard in node but virtual guard not one of known "
  ...
  traceMsg(
      comp, "virtualGuardHelper for %s %s\n",
      comp->getDebug()
...
static TR::Register *generateS390lcmpEvaluator(
    TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic brOpCode,
    TR::InstOpCode::S390BranchCondition condCmpHighFalse,
    TR::InstOpCode::S390BranchCondition condCmpHighTrue,
  ...
  if (!isBoolean && numGlobalDeps < 8 && //  Needs to be tuned
    ...
    if (cg->canClobberNodesRegister(firstChild, 1, &data) &&
      ...
      generateRRInstruction(cg, TR::InstOpCode::LR, node, workReg,
    ...
    if (node->getNumChildren() == 3) {
      ...
      deps->addPostConditionIfNotAlreadyInserted(workReg,
                                                 TR::RealRegister::AssignAny);

      cg->decReferenceCount(thirdChild);
    }

    generateRRInstruction(cg, TR::InstOpCode::OR, node, workReg,
    ...
    generateS390BranchInstruction(cg, brOpCode, cmpOpCode, node, trueTarget,
                                  deps);
    ...
    if (tempLogicalOp.getOpReg1Reg2()) {
      generateRRInstruction(cg, logicalRR, firstChild,
      ...
      TR::RegisterPair *tempReg = cg->allocateConsecutiveRegisterPair(
          cg->allocateRegister(), cg->allocateRegister());

      generateRRInstruction(cg, TR::InstOpCode::LR, firstChild,
      ...
      generateRRInstruction(cg, logicalRR, firstChild, tempReg->getHighOrder(),
                            secondLogicalOpRegister->getHighOrder());
      targetLogicalOpReg = tempReg->getLowOrder();
      sourceLogicalOpReg = secondLogicalOpRegister->getLowOrder();
      firstChild->setRegister(tempReg);
    } else {
      TR_ASSERT(!tempLogicalOp.getInvalid(),
                "TR_S390ControlFlowEvaluatore::invalid case\n");
      if (tempLogicalOp.getOpReg3Mem2() || tempLogicalOp.getOpReg1Mem2()) {
        ...
        generateRXInstruction(cg, logicalRX, firstChild,
                              tempReg->getHighOrder(), highLogicalOpMR);
        targetLogicalOpReg = tempReg->getLowOrder();
        firstChild->setRegister(tempReg);
      } else {
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    // if second child is complicated then control flow region start will be
    // generated later
    TR::LabelSymbol *cflowRegionStart =
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (node->getNumChildren() == 3) {
      ...
      deps->addPostConditionIfNotAlreadyInserted(highOrder,
                                                 TR::RealRegister::AssignAny);
      deps->addPostConditionIfNotAlreadyInserted(lowOrder,
                                                 TR::RealRegister::AssignAny);
    } else {
      deps =
          new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 9, cg);
      //         deps->addPostCondition(targetRegisterPair,
      //         TR::RealRegister::EvenOddPair);
      //         deps->addPostCondition(highOrder,
      //         TR::RealRegister::LegalEvenOfPair);
      //         deps->addPostCondition(lowOrder,
    ...
    // HIGH ORDER
    if (isUnsignedCmp) {
      generateS390ImmOp(cg, TR::InstOpCode::CL, node, highOrder, NULL, h_value,
                        deps);
    } else {
      generateS390ImmOp(cg, TR::InstOpCode::C, node, highOrder, highOrder,
                        h_value, deps);
    }

    // See if condition is FALSE
    if (condCmpHighFalse != TR::InstOpCode::COND_NOP) {
      generateS390BranchInstruction(cg, brOpCode, condCmpHighFalse, node,
                                    falseTarget);
    }

    // See if we can assert the condition as TRUE
    if (condCmpHighTrue != TR::InstOpCode::COND_NOP) {
      generateS390BranchInstruction(cg, brOpCode, condCmpHighTrue, node,
                                    trueTarget);
    }
    ...
    if (splitLogicalOp) {
      ...
      if (lowLogicalOpMR) {
        generateRXInstruction(cg, logicalRX, firstChild, targetLogicalOpReg,
                              lowLogicalOpMR);
        if (lowLogicalOpMR->getIndexRegister()) {
          deps->addPostConditionIfNotAlreadyInserted(
              lowLogicalOpMR->getIndexRegister(), TR::RealRegister::AssignAny);
        }
        if (lowLogicalOpMR->getBaseRegister()) {
          deps->addPostConditionIfNotAlreadyInserted(
              lowLogicalOpMR->getBaseRegister(), TR::RealRegister::AssignAny);
        }
        deps->addPostConditionIfNotAlreadyInserted(targetLogicalOpReg,
                                                   TR::RealRegister::AssignAny);
      } else {
        generateRRInstruction(cg, logicalRR, firstChild, targetLogicalOpReg,
                              sourceLogicalOpReg);
        deps->addPostConditionIfNotAlreadyInserted(sourceLogicalOpReg,
                                                   TR::RealRegister::AssignAny);
        deps->addPostConditionIfNotAlreadyInserted(targetLogicalOpReg,
                                                   TR::RealRegister::AssignAny);
      }
    } else {
      generateS390ImmOp(cg, TR::InstOpCode::CL, node, lowOrder, lowOrder,
                        l_value, deps);
    }

    // Perform required branch on compare of low values
    if (condCmpLowTrue != 0) {
      generateS390BranchInstruction(cg, brOpCode, condCmpLowTrue, node,
                                    trueTarget);
    }
    ...
    deps = temp.longOrderedCompareAndBranchAnalyser(
        node, brOpCode, condCmpLowTrue, condCmpHighTrue, condCmpHighFalse,
        trueTarget, falseTarget, internalControlFlowStarted);
  }
  ...
  if (!isBoolean) {
    ...
    // We don't need an equivalent dep on the gra deps as it is by defn not
    // generated
...
static TR::Register *generateS390lcmpEvaluator64(
  ...
  TR_ASSERT(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(),
            "lcmpEvaluator64() is for 64bit code-gen only!");
  TR::RegisterDependencyConditions *deps = NULL;
  TR::LabelSymbol *isTrue = TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  isTrue->setEndInternalControlFlow();
  ...
  // TRUE
  instr = generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, isTrue,
                                       deps);
  isTrue->setEndInternalControlFlow();
...
TR::Register *OMR::Z::TreeEvaluator::fcmplEvaluator(TR::Node *node,
  ...
  brCond = generateS390CompareOps(node, cg, TR::InstOpCode::COND_BHRC,
  ...
  TR::Instruction *cursor = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node, doneCmp);
  ...
  if (node->getOpCodeValue() == TR::fcmpg ||
    ...
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BNANRC, node, doneCmp);
  }
...
/**
 * 32bit version lcmpEvaluator: long compare (1 if child1 > child2, 0 if child1
 * == child2,
 *    -1 if child1 < child2 or unordered)
 */
TR::Register *lcmpHelper(TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is32Bit(), "this is for 32bit code-gen only!");
  // There is probably a better way to implement this.  Should re-visit if we
  // get the chance.
  // As things stand now, we will to LCR R1,R1 when R1=0 which is useless...
  // But still probably
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    //      dependencies->addPostCondition(src1RegPair,
    //      TR::RealRegister::EvenOddPair);
    // Forcing these registers to be pairs is too strict because we treat them
    // independently anyways
    dependencies->addPostCondition(src1RegPair->getHighOrder(),
                                   TR::RealRegister::AssignAny);
    dependencies->addPostCondition(src1RegPair->getLowOrder(),
                                   TR::RealRegister::AssignAny);
  } else {
    ...
    dependencies->addPostCondition(src1RegPair->getLowOrder(),
                                   TR::RealRegister::AssignAny);
    // Fix the case when second child register == first register to avoid
    // duplicates.
    // dependencies->addPostConditionIfNotAlreadyInserted(src2RegPair,
    // TR::RealRegister::EvenOddPair);
    dependencies->addPostConditionIfNotAlreadyInserted(
        src2RegPair->getHighOrder(), TR::RealRegister::AssignAny);
    dependencies->addPostConditionIfNotAlreadyInserted(
        src2RegPair->getLowOrder(), TR::RealRegister::AssignAny);
  }
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    generateRRInstruction(cg, TR::InstOpCode::XR, node, tempRegister,
                          tempRegister);
    generateRRInstruction(cg, TR::InstOpCode::XR, node, targetRegister,
                          targetRegister);
    generateRRInstruction(cg, TR::InstOpCode::SLR, node, tempRegister,
                          src1RegPair->getLowOrder());
    generateRRInstruction(cg, TR::InstOpCode::SLBR, node, targetRegister,
                          src1RegPair->getHighOrder());

    TR::Register *tempHORegister = src1RegPair->getHighOrder();
    TR_ClobberEvalData data;
    bool clobberHighWord = cg->canClobberNodesRegister(firstChild, 1, &data) &&
                           data.canClobberHighWord();
    if (!clobberHighWord) {
      tempHORegister = cg->allocateRegister();
      generateRRInstruction(cg, TR::InstOpCode::LR, node, tempHORegister,
                            src1RegPair->getHighOrder());
    }
    generateRSInstruction(cg, TR::InstOpCode::SRA, node, tempHORegister,
                          tempHORegister, 31);
    generateRSInstruction(cg, TR::InstOpCode::SRL, node, targetRegister,
                          targetRegister, 31);
    generateRRInstruction(cg, TR::InstOpCode::OR, node, targetRegister,
                          tempHORegister);
    ...
    if (secondChild->getOpCode().isLoadConst() &&
      ...
      generateS390ImmOp(
          cg, TR::InstOpCode::C, node, src1RegPair->getHighOrder(),
          src1RegPair->getHighOrder(), secondChild->getLongIntHigh());
    } else {
      // Set the CC using a comp.
    ...
    cursor = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, node, labelLT);

    // If GT, we invert the result register
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BHRC, node, labelGT);
    ...
    if (secondChild->getOpCode().isLoadConst() &&
      ...
      generateS390ImmOp(cg, TR::InstOpCode::C, node, src1RegPair->getLowOrder(),
                        src1RegPair->getLowOrder(),
    ...
    // If LT, we are done
    cursor2 = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, node, labelLT);
    if (cursor)
      cursor->setStartInternalControlFlow();
    else
...
TR::Register *OMR::Z::TreeEvaluator::maxEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  TR_ASSERT(false,
            "Max opcode should have been expanded by TR_OpCodeExpansion\n");
  return NULL;
}
...
TR::Register *OMR::Z::TreeEvaluator::minEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  TR_ASSERT(false,
            "Min opcode should have been expanded by TR_OpCodeExpansion\n");
  return NULL;
}
...
TR::Register *lcmpHelper64(TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(),
            "this is for 64bit code-gen only!");
 ...
 * (return, areturn, ireturn, lreturn, freturn, dreturn, iureturn, lureturn,
 * oreturn)
 */
TR::Register *OMR::Z::TreeEvaluator::returnEvaluator(TR::Node *node,
  ...
  if ((node->getOpCodeValue() != TR::Return) && !returnCodeDeferred)
  ...
  if (cg->supportsHighWordFacility() &&
      !comp->getOption(TR_DisableHighWordRA) && returnValRegister != NULL &&
  ...
  case TR::iureturn:
    ...
    if (linkage->isNeedsWidening())
      new (cg->trHeapMemory())
          TR::S390RRInstruction(TR::InstOpCode::LLGFR, node, returnValRegister,
...
/**
 * This helper is for generating a VGNOP for HCR guard when the guard it was
 * merged with cannot be NOPed.
 ...
 *
 * When the condition is switched on node from eq to ne, HCR guard has to be
 * patch to go to the fall through path
 */
static inline void generateMergedHCRGuardCodeIfNeeded(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
#ifdef J9_PROJECT_SPECIFIC
  TR::Compilation *comp = cg->comp();
  if (node->isTheVirtualGuardForAGuardedInlinedCall() &&
    ...
    if (virtualGuard && virtualGuard->mergedWithHCRGuard()) {
      ...
      if (instr && instr->getNode() == node && instr->getDependencyConditions())
        mergedHCRDeps = new (cg->trHeapMemory())
...

      {
        ...
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                     fallThroughLabel, mergedHCRDeps);
      } else {
        TR::LabelSymbol *label =
            node->getBranchDestination()->getNode()->getLabel();

        TR::Instruction *vgnopInstr = generateVirtualGuardNOPInstruction(
            cg, node, site, mergedHCRDeps, label,
            instr ? instr->getPrev() : NULL);
        vgnopInstr->setNext(instr);
        comp->setAppendInstruction(instr);
      }
      traceMsg(comp, "generateMergedHCRGuardCodeIfNeeded for %s %s\n",
               comp->getDebug()
                   ? comp->getDebug()->getVirtualGuardKindName(
                         virtualGuard->getKind())
                   : "???Guard",
               virtualGuard->mergedWithHCRGuard() ? "merged with HCRGuard"
                                                  : "");
...
TR::Register *OMR::Z::TreeEvaluator::iflcmpeqEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    ...
    generateS390lcmpEvaluator(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC,
        TR::InstOpCode::COND_NOP, TR::InstOpCode::COND_BERC, CMP4CONTROLFLOW);
  }
  generateMergedHCRGuardCodeIfNeeded(node, cg);
  return NULL;
}
...
TR::Register *OMR::Z::TreeEvaluator::iflcmpneEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    ...
    generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC,
                              TR::InstOpCode::COND_NOP,
                              TR::InstOpCode::COND_BNERC,
                              TR::InstOpCode::COND_BNERC, CMP4CONTROLFLOW);
  }
...
TR::Register *OMR::Z::TreeEvaluator::iflcmpleEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    ...
    generateS390lcmpEvaluator(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BHRC,
        TR::InstOpCode::COND_BLRC, TR::InstOpCode::COND_BNHRC, CMP4CONTROLFLOW);
  }
  return NULL;
}
...
TR::Register *OMR::Z::TreeEvaluator::iflcmpltEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    ...
    generateS390lcmpEvaluator(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BHRC,
        TR::InstOpCode::COND_BLRC, TR::InstOpCode::COND_BLRC, CMP4CONTROLFLOW);
  }
  return NULL;
}
...
TR::Register *OMR::Z::TreeEvaluator::iflcmpgeEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    ...
    generateS390lcmpEvaluator(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC,
        TR::InstOpCode::COND_BHRC, TR::InstOpCode::COND_BNLRC, CMP4CONTROLFLOW);
  }
  return NULL;
}
...
TR::Register *OMR::Z::TreeEvaluator::iflcmpgtEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    ...
    generateS390lcmpEvaluator(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC,
        TR::InstOpCode::COND_BHRC, TR::InstOpCode::COND_BHRC, CMP4CONTROLFLOW);
  }
  return NULL;
}
...
TR::Register *OMR::Z::TreeEvaluator::ifbcmpeqEvaluator(TR::Node *node,
  ...
  if (node->getOpCodeValue() == TR::ifbcmpeq ||
      node->getOpCodeValue() == TR::ifbucmpeq) {
    return generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                                     TR::InstOpCode::COND_BERC,
                                     TR::InstOpCode::COND_BERC);
  } else {
  ...
  if (node->getOpCodeValue() == TR::icmpeq ||
    ...
    if (node->getOpCodeValue() != TR::acmpeq &&
      ...
      if ((val1 & (val1 - 1)) == 0) {
        ...
        generateRIEInstruction(cg, TR::InstOpCode::RXSBG, node, target, source,
                               63, 128 + 63, rotBy);
...
TR::Register *OMR::Z::TreeEvaluator::lcmpeqEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    return generateS390lcmpEvaluator64(node, cg, TR::InstOpCode::BRC,
                                       TR::InstOpCode::COND_BERC, CMP4BOOLEAN);
  } else {
    return generateS390lcmpEvaluator(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC,
        TR::InstOpCode::COND_NOP, TR::InstOpCode::COND_BERC, CMP4BOOLEAN);
  }
}
...
TR::Register *OMR::Z::TreeEvaluator::lcmpneEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    return generateS390lcmpEvaluator64(node, cg, TR::InstOpCode::BRC,
                                       TR::InstOpCode::COND_BNERC, CMP4BOOLEAN);
  } else {
    return generateS390lcmpEvaluator(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_NOP,
        TR::InstOpCode::COND_BNERC, TR::InstOpCode::COND_BNERC, CMP4BOOLEAN);
  }
}
...
TR::Register *OMR::Z::TreeEvaluator::lcmpleEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    return generateS390lcmpEvaluator64(node, cg, TR::InstOpCode::BRC,
                                       TR::InstOpCode::COND_BNHRC, CMP4BOOLEAN);
  } else {
    return generateS390lcmpEvaluator(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BHRC,
        TR::InstOpCode::COND_BLRC, TR::InstOpCode::COND_BNHRC, CMP4BOOLEAN);
  }
}
...
TR::Register *OMR::Z::TreeEvaluator::lcmpltEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    return generateS390lcmpEvaluator64(node, cg, TR::InstOpCode::BRC,
                                       TR::InstOpCode::COND_BLRC, CMP4BOOLEAN);
  } else {
    return generateS390lcmpEvaluator(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BHRC,
        TR::InstOpCode::COND_BLRC, TR::InstOpCode::COND_BLRC, CMP4BOOLEAN);
  }
}
...
TR::Register *OMR::Z::TreeEvaluator::lcmpgeEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    return generateS390lcmpEvaluator64(node, cg, TR::InstOpCode::BRC,
                                       TR::InstOpCode::COND_BNLRC, CMP4BOOLEAN);
  } else {
    return generateS390lcmpEvaluator(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC,
        TR::InstOpCode::COND_BHRC, TR::InstOpCode::COND_BNLRC, CMP4BOOLEAN);
  }
}
...
TR::Register *OMR::Z::TreeEvaluator::lcmpgtEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
    return generateS390lcmpEvaluator64(node, cg, TR::InstOpCode::BRC,
                                       TR::InstOpCode::COND_BHRC, CMP4BOOLEAN);
  } else {
    return generateS390lcmpEvaluator(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC,
        TR::InstOpCode::COND_BHRC, TR::InstOpCode::COND_BHRC, CMP4BOOLEAN);
  }
}
...
TR::Register *OMR::Z::TreeEvaluator::bcmpeqEvaluator(TR::Node *node,
  ...
  if (node->getOpCodeValue() == TR::bcmpeq ||
      node->getOpCodeValue() == TR::bucmpeq) {
    return generateS390CompareBool(node, cg, TR::InstOpCode::BRC,
                                   TR::InstOpCode::COND_BERC,
                                   TR::InstOpCode::COND_BERC);
  } else {
...
TR::Register *OMR::Z::TreeEvaluator::lucmpEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  PRINT_ME("lucmp", node, cg);
  return badILOpEvaluator(node, cg);
}

TR::Register *OMR::Z::TreeEvaluator::icmpEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  PRINT_ME("icmp", node, cg);
  return badILOpEvaluator(node, cg);
}

TR::Register *OMR::Z::TreeEvaluator::iucmpEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
...
TR::Register *OMR::Z::TreeEvaluator::tableEvaluator(TR::Node *node,
   ...
   * where we load an address out of the table and branch to it
   * since cases are indexed by 1, we need to shift by either 4 or 8 to get the
  ...
  case AddressTable32bit: {
    if (!node->chkCannotOverflow())
      generateS390CompareAndBranchInstruction(
          cg, TR::InstOpCode::CL, node, selectorReg, numBranchTableEntries,
          TR::InstOpCode::COND_BNLRC,
  ...
  case AddressTable64bitLongLookup: {
    if (!node->chkCannotOverflow())
      generateS390CompareAndBranchInstruction(
          cg, TR::InstOpCode::CLG, node, selectorReg, numBranchTableEntries,
          TR::InstOpCode::COND_BNLRC,
          node->getSecondChild()->getBranchDestination()->getNode()->getLabel(),
          false, false);
    generateRSInstruction(cg, TR::InstOpCode::SLLG, node, selectorReg,
  ...
  tableEvaluatorCaseLabelHelper(node, cg, tableKindToBeEvaluated,
                                numBranchTableEntries, selectorReg,
                                branchTableReg, reg1);
      ...
      if (type.isInt64()) {
        if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit())
          ...
          generateS390ImmOp(cg, TR::InstOpCode::C, node,
                            selectorReg->getHighOrder(),
                            selectorReg->getHighOrder(),
                            (int32_t)((child->getCaseConstant() >> 32) &
                                      0x00000000FFFFFFFFl));
          generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                        TR::InstOpCode::COND_BNERC, node,
                                        skipLoCompareLabel);
          generateS390ImmOp(
              cg, TR::InstOpCode::C, node, selectorReg->getLowOrder(),
              selectorReg->getLowOrder(),
              (int32_t)(child->getCaseConstant() & 0x00000000FFFFFFFFl));
        }
      } else
...
TR::Register *OMR::Z::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(
  ...
  if (cg->getSupportsImplicitNullChecks() && !firstChild->isUnneededIALoad()) {
    if (opCode.isLoadVar() || (TR::Compiler->target.is64Bit() &&
  ...
  if (needExplicitCheck) {
    ...
    if (cg->getHasResumableTrapHandler()) {
      ...
      if (cg->getS390ProcessorInfo()->supportsArch(
        ...
        appendTo = generateRXYInstruction(
      ...
      } else if (cg->getS390ProcessorInfo()->supportsArch(
                     TR_S390ProcessorInfo::TR_zEC12) &&
                 reference->getRegister() == NULL &&
  ...
  if (needLateEvaluation) {
    ...
    if (opCode.isLoad() && firstChild->getReferenceCount() == 1 &&
      ...
      if (needExplicitCheck && firstChild->getRegister() == NULL) {
        ...
        // it should not be evaluated yet. Its reference counter will be
        // decrease later.
        cg->decReferenceCount(reference);
      } else {
        cg->decReferenceCount(firstChild);
...
TR::Register *OMR::Z::TreeEvaluator::ZEROCHKEvaluator(TR::Node *node,
  ...
  TR::LabelSymbol *slowPathOOLLabel =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  ;
  ...
  if (valueToCheck->getOpCode().getOpCodeValue() == TR::acmpeq) {
    TR::InstOpCode::S390BranchCondition opBranchCond =
        generateS390CompareOps(valueToCheck, cg, TR::InstOpCode::COND_BNERC,
                               TR::InstOpCode::COND_BNERC);
 ...
 * check
 *    the underlying pointer.  child 1 is reference to be resolved. Symbolref
 * indicates
...
TR::Register *OMR::Z::TreeEvaluator::inlineIfBifEvaluator(
  ...
  if (ifNode->getFirstChild()->getOpCodeValue() == TR::bu2i) {
    ...
    // therefore the bu2i must also be isSingleRefEvaluated()
...
OMR::Z::TreeEvaluator::getBranchConditionFromCompareOpCode(
  ...
  default: {
    ...
    return TR::InstOpCode::COND_BERC; // not a valid return value.  We should
                                      // never ever get here.
  } break;
  }
}
 ...
 * is true,
 * we must do the opposite to mimic branch behaviour, which would branch around
 * the load when one of the following is true.
 */
TR::InstOpCode::S390BranchCondition
...
TR::Register *OMR::Z::TreeEvaluator::ternaryEvaluator(TR::Node *node,
  ...
  if (comp->getOption(TR_TraceCG))
    traceMsg(comp, "Starting evaluation of ternary node %p condition %p (in "
                   "reg %p) trueVal %p (in reg %p) falseVal %p (in reg %p)\n",
  ...
  if (treeContainsAllOtherUsesForNode(condition, trueVal) &&
      (!trueVal->isUnneededConversion() ||
       treeContainsAllOtherUsesForNode(condition, trueVal->getFirstChild()))) {
    if (comp->getOption(TR_TraceCG))
      traceMsg(comp, "Calling evaluate (instead of clobber evaluate for node "
                     "%p because all other uses are in the compare tree)\n",
               trueVal);
  ...
  if (condition->getOpCode().isBooleanCompare() &&
    ...
    TR::InstOpCode::S390BranchCondition bc =
        getBranchConditionFromCompareOpCode(condition->getOpCodeValue());
    ...
    if (comp->getOption(TR_TraceCG))
      traceMsg(
          comp,
          "Emitting a Load on condition with condtion based on compare type\n");
    if (trueReg->getRegisterPair() || falseReg->getRegisterPair()) {
      TR_ASSERT(trueReg->getRegisterPair() && falseReg->getRegisterPair(),
                "Both (or none) false and true regs should be register pairs");
      bool is64BitRegister =
          (trueReg->getKind() == TR_GPR64 || cg->use64BitRegsOn32Bit());
      generateRRFInstruction(
          cg, is64BitRegister ? TR::InstOpCode::LOCGR : TR::InstOpCode::LOCR,
          node, trueReg->getHighOrder(), falseReg->getHighOrder(),
          getMaskForBranchCondition(TR::InstOpCode::COND_BERC) >> 4, true);
      generateRRFInstruction(
          cg, is64BitRegister ? TR::InstOpCode::LOCGR : TR::InstOpCode::LOCR,
          node, trueReg->getLowOrder(), falseReg->getLowOrder(),
          getMaskForBranchCondition(TR::InstOpCode::COND_BERC) >> 4, true);
    } else
      generateRRFInstruction(
          cg, trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LOCGR
                                             : TR::InstOpCode::LOCR,
          node, trueReg, falseReg,
          getMaskForBranchCondition(mapBranchConditionToLOCRCondition(bc)) >> 4,
          true);
  } else {
    if (comp->getOption(TR_TraceCG))
    ...
    TR::Instruction *compareInst = generateRILInstruction(
        cg, condition->getOpCode().isLongCompare() ? TR::InstOpCode::CGFI
                                                   : TR::InstOpCode::CFI,
        condition, condition->getRegister(), (uintptrj_t)0);

    if (comp->getOption(TR_TraceCG))
      traceMsg(
          comp,
          "Emmitting a LOCR with resulReg = %p falseReg->getRegister = %p\n",
    ...
    if (trueReg->getRegisterPair() || falseReg->getRegisterPair()) {
      ...
      generateRRFInstruction(
          cg, trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LOCGR
                                             : TR::InstOpCode::LOCR,
          node, trueReg, falseReg->getRegister(),
          getMaskForBranchCondition(TR::InstOpCode::COND_BERC) >> 4, true);
  }
...
TR::Register *OMR::Z::TreeEvaluator::dternaryEvaluator(TR::Node *node,
  ...
  if (conditionNode->getOpCode().isCompareDouble()) {
    ...
    // vector compare instructions return false for unordered.
...
/**
 * Generates always trapping sequence - for conditions in WCODE Path that
 * require always trap
 * If used in internal control flow section, caller should merge register
 * dependencies with returned deps.
 */
TR::Instruction *
generateAlwaysTrapSequence(TR::Node *node, TR::CodeGenerator *cg,
                           TR::RegisterDependencyConditions **retDeps) {
  // ** Generate a NOP LR R0,R0.  The signal handler has to walk backwards to
  // pattern match
  // the trap instructions.  All trap instructions besides CRT/CLRT are 6-bytes
  // in length.
  // Insert 2-byte NOP in front of the 4-byte CLRT to ensure we do not mismatch
  // accidentally.
  TR::Instruction *cursor = new (cg->trHeapMemory())
      TR::S390NOPInstruction(TR::InstOpCode::NOP, 2, node, cg);

  TR::Register *zeroReg = cg->allocateRegister();
  TR::RegisterDependencyConditions *regDeps =
      new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 1, cg);
  regDeps->addPostCondition(zeroReg, TR::RealRegister::AssignAny);
  cursor = new (cg->trHeapMemory()) TR::S390RRFInstruction(
      TR::InstOpCode::CLRT, node, zeroReg, zeroReg,
      getMaskForBranchCondition(TR::InstOpCode::COND_BERC) >> 4, true, cg);
