...
public:
  S390WarmEyeCatcherDataSnippet(TR::CodeGenerator *cg, TR::Node *,
                                TR::LabelSymbol *);
...
/**
 * TargetAddress Snippet is used to hold address of snippet which exceed the
 * branch limit
 */
...
public:
  S390TargetAddressSnippet(TR::CodeGenerator *cg, TR::Node *,
...
public:
  S390InterfaceCallDataSnippet(TR::CodeGenerator *, TR::Node *, uint8_t,
                               bool useCLFIandBRCL = false);

  S390InterfaceCallDataSnippet(TR::CodeGenerator *, TR::Node *, uint8_t,
                               uint8_t *, bool useCLFIandBRCL = false);
...
public:
  ...
  TR::LabelSymbol *getLabel(int32_t index) {
    TR_ASSERT(index < _size, "out of range, index %d, size %d", index, _size);
    return _labelTable[index];
  }
  TR::LabelSymbol *setLabel(int32_t index, TR::LabelSymbol *label) {
    TR_ASSERT(index < _size, "out of range, index %d, size %d", index, _size);
    return _labelTable[index] = label;
  }
