...
public:
  ...
  // exception
  // currently this is only implemented for null pointer exceptions, but it
  // should
  ...
  virtual void setKind(Kind kind) {
    TR_ASSERT(0, "Should not be called unless working with an RX, RS or RI "
                 "instructions");
  }
  ...
  virtual bool startOfLiveRange(TR::Register *reg); // True if virtual register
                                                    // is dead before current
                                                    // instruction
  ...
  void setBreakPoint(bool v) {
    v ? _index |= BreakPoint : _index &= ~BreakPoint;
  }
  ...
  void readCCInfo();
  void clearCCInfo();
  void addARDependencyCondition(TR::Register *virtAR,
...
protected:
   ...
   * If a register operand is set it will be in target category, if register
   *     is set and used it will also be in target category but not in source.
   * The opcode property UsesTarget indicates that all target registers are
   *     used for input. 390 Instructions have 0,1,2, or 3 register operands
   * often refered in manual as R1,R2,R3. These operands can be retreived using
  ...
  template <typename T>
  class RegisterArray
      : public TR::Allocatable<RegisterArray<T>, TR::Allocator> {
  public:
  ...
  class RegisterBitVector
      : public TR::Allocatable<RegisterBitVector, TR::Allocator> {
  public:
    TR::Allocator allocator() { return TR::comp()->allocator(); }
