...
void TR::S390ConstantDataSnippet::addMetaDataForCodeAddress(uint8_t *cursor) {
  ...
  case TR_MethodObject: {
    ...
    TR::SymbolReference *reloSymRef = (reloType == TR_ClassAddress)
                                          ...
                                          : getSymbolReference();
    cg()->addAOTRelocation(
        new (cg()->trHeapMemory()) TR_ExternalRelocation(
  ...
  case TR_ClassPointer: {
    AOTcgDiag2(comp, "add relocation (%d) cursor=%x\n", reloType, cursor);
    TR_Relocation *relo;
    // for optimizations where we are trying to relocate either profiled j9class
    // or getfrom signature we can't use node to get the target address
    // so we need to pass it to relocation in targetaddress2 for now
    // two instances where use this relotype in such way are: profile checkcast
    // and arraystore check object check optimiztaions
    ...
    relo = new (cg()->trHeapMemory())
        TR_ExternalRelocation(cursor, (uint8_t *)getNode(), targetAdress2,
                              (TR_ExternalRelocationTargetKind)reloType, cg());
  ...
  for (auto it = comp->getSnippetsToBePatchedOnRegisterNative()->begin();
       it != comp->getSnippetsToBePatchedOnRegisterNative()->end(); ++it) {
    TR::Snippet *s = (*it)->getKey();
    if (this == s) {
      TR_OpaqueMethodBlock *method =
          (*it)->getValue()->getPersistentIdentifier();
 ...
 */
TR::S390WarmEyeCatcherDataSnippet::S390WarmEyeCatcherDataSnippet(
    TR::CodeGenerator *cg, TR::Node *n,
    TR::LabelSymbol *fullEyeCatcherSnippetLabel)
    : TR::S390ConstantDataSnippet(cg, n, NULL, 0) {
  _fullEyeCatcherSnippet = fullEyeCatcherSnippetLabel;

  setWarmSnippet(); // Set as Warm Snippet - this is always warm... If TCC is
                    // disabled, only the full eyecatcher will be emitted then.

  int32_t totalLength = (8 + sizeof(intptrj_t));
  setLength(totalLength);
}

uint8_t *TR::S390WarmEyeCatcherDataSnippet::emitSnippetBody() {
  uint8_t *cursor = cg()->getBinaryBufferCursor();
  getSnippetLabel()->setCodeLocation(cursor);
  ...
  TR_ASSERT(_fullEyeCatcherSnippet != NULL,
            "Warm EyeCatcher does not have a corresponding full EyeCatcher.");
  cg()->addRelocation(new (cg()->trHeapMemory()) TR_LabelAbsoluteRelocation(
      cursor, _fullEyeCatcherSnippet));
...
//////////////////////////////////////////////////////////////////////////////////////
TR::S390TargetAddressSnippet::S390TargetAddressSnippet(
    TR::CodeGenerator *cg, TR::Node *n, TR::LabelSymbol *targetLabel)
    : TR::Snippet(cg, n, TR::LabelSymbol::create(cg->trHeapMemory(), cg),
                  false),
      _targetsnippet(NULL), _targetaddress(0), _targetsym(NULL),
      _targetlabel(targetLabel) {}

TR::S390TargetAddressSnippet::S390TargetAddressSnippet(TR::CodeGenerator *cg,
                                                       TR::Node *n,
                                                       TR::Snippet *s)
    : TR::Snippet(cg, n, TR::LabelSymbol::create(cg->trHeapMemory(), cg),
                  false),
      _targetsnippet(s), _targetaddress(0), _targetsym(NULL),
      _targetlabel(NULL) {}
...
TR::S390TargetAddressSnippet::S390TargetAddressSnippet(TR::CodeGenerator *cg,
    ...
    : TR::Snippet(cg, n, TR::LabelSymbol::create(cg->trHeapMemory(), cg),
                  false),
      _targetsnippet(NULL), _targetaddress(addr), _targetsym(NULL),
      _targetlabel(NULL) {}
...
TR::S390TargetAddressSnippet::S390TargetAddressSnippet(TR::CodeGenerator *cg,
    ...
    : TR::Snippet(cg, n, TR::LabelSymbol::create(cg->trHeapMemory(), cg),
                  false),
      _targetsnippet(NULL), _targetaddress(0), _targetsym(sym),
      _targetlabel(NULL) {}

uint8_t *TR::S390TargetAddressSnippet::emitSnippetBody() {
  // *this   swipeable for debugging purposes
  uint8_t *cursor = cg()->getBinaryBufferCursor();
  AOTcgDiag1(cg()->comp(),
             "TR::S390TargetAddressSnippet::emitSnippetBody cursor=%x\n",
             cursor);
  getSnippetLabel()->setCodeLocation(cursor);
  TR::Compilation *comp = cg()->comp();
  ...
  if (_targetsnippet != NULL) {
    *(intptrj_t *)cursor = 0;
    ...
    cg()->addRelocation(new (cg()->trHeapMemory()) TR_LabelAbsoluteRelocation(
        cursor, _targetsnippet->getSnippetLabel()));
    ...
    *(intptrj_t *)cursor = 0;
    if (_targetlabel->getCodeLocation() != NULL) {
      *(intptrj_t *)cursor = (intptrj_t)(_targetlabel->getCodeLocation());
      ...
      cg()->addRelocation(new (cg()->trHeapMemory())
                              TR_LabelAbsoluteRelocation(cursor, _targetlabel));
      cg()->addProjectSpecializedRelocation(cursor, NULL, NULL,
                                            TR_AbsoluteMethodAddress, __FILE__,
                                            __LINE__, getNode());
    }
    ...
    if (_targetsym != NULL) {
      TR_ResolvedMethod *resolvedMethod =
          _targetsym->getResolvedMethodSymbol()->getResolvedMethod();

      if (resolvedMethod != NULL &&
          resolvedMethod->isSameMethod(comp->getCurrentMethod())) {
        uint8_t *jitTojitStart = cg()->getCodeStart();

        // Calculate jit-to-jit entry point
        jitTojitStart +=
            ((*(intptrj_t *)(jitTojitStart - 4)) >> 16) & 0x0000ffff;
...
TR::S390LookupSwitchSnippet::S390LookupSwitchSnippet(TR::CodeGenerator *cg,
                                                     TR::Node *switchNode)
    : TR::S390TargetAddressSnippet(
          cg, switchNode, TR::LabelSymbol::create(cg->trHeapMemory(), cg)) {
  // *this   swipeable for debugger
}

uint8_t *TR::S390LookupSwitchSnippet::emitSnippetBody() {
  ...
  for (int32_t i = 1; i <= numChildren; i++) {
    ...
    intptrj_t caseKeyTarget = (intptrj_t)caseKeyNode->getBranchDestination()
                                  ->getNode()
                                  ->getLabel()
                                  ->getCodeLocation();
...
uint32_t TR::S390LookupSwitchSnippet::getLength(int32_t estimatedSnippetStart) {
  TR::Compilation *comp = cg()->comp();
  return (getNode()->getCaseIndexUpperBound() - 1) *
         (sizeof(int32_t) + TR::Compiler->om.sizeofReferenceAddress());
}

///////////////////////////////////////////////////////////////////////////////
// TR::S390InterfaceCallDataSnippet member functions
///////////////////////////////////////////////////////////////////////////////
TR::S390InterfaceCallDataSnippet::S390InterfaceCallDataSnippet(
    TR::CodeGenerator *cg, TR::Node *node, uint8_t n, bool useCLFIandBRCL)
    : TR::S390ConstantDataSnippet(
          cg, node, TR::LabelSymbol::create(cg->trHeapMemory(), cg), 0),
      _numInterfaceCallCacheSlots(n), _codeRA(NULL), thunkAddress(NULL),
      _useCLFIandBRCL(useCLFIandBRCL) {}

TR::S390InterfaceCallDataSnippet::S390InterfaceCallDataSnippet(
    TR::CodeGenerator *cg, TR::Node *node, uint8_t n, uint8_t *thunkPtr,
    bool useCLFIandBRCL)
    : TR::S390ConstantDataSnippet(
          cg, node, TR::LabelSymbol::create(cg->trHeapMemory(), cg), 0),
      _numInterfaceCallCacheSlots(n), _codeRA(NULL), thunkAddress(thunkPtr),
      _useCLFIandBRCL(useCLFIandBRCL) {}

uint8_t *TR::S390InterfaceCallDataSnippet::emitSnippetBody() {
  ...
  // If we use CLFI / BRCL
  //  0x00000100211cfea0     DC  0x00000000       ; Patch slot of the first CLFI
  //  (Class 1)
  //  0x00000100211cfea0     DC  0x00000000       ; Patch slot of the second
  //  CLFI (Class 2)
  //  0x00000100211cfea0     DC  0x00000000       ; Patch slot of the third CLFI
  //  (Class 3)
  // and so on ...
  ...
  uint8_t *cursor = cg()->getBinaryBufferCursor();
  AOTcgDiag1(comp,
             "TR::S390InterfaceCallDataSnippet::emitSnippetBody cursor=%x\n",
             cursor);

  // Class Pointer must be double word aligned.
  int32_t padBytes = ((intptrj_t)cursor + 5 * sizeof(intptrj_t) + 7) / 8 * 8 -
                     ((intptrj_t)cursor + 5 * sizeof(intptrj_t));
  cursor += padBytes;
  ...
  TR_ASSERT(
      _codeRA != NULL,
      "Interface Call Data Constant's code return address not initialized.\n");
  *(uintptrj_t *)cursor = (uintptrj_t)_codeRA;
  ...
  // constant pool
  *(uintptrj_t *)cursor = (uintptrj_t)callNode->getSymbolReference()
  ...
  cg()->addProjectSpecializedRelocation(
      ...
      callNode ? (uint8_t *)(intptr_t)callNode->getInlinedSiteIndex()
               : (uint8_t *)-1,
      TR_Thunks, __FILE__, __LINE__, callNode);
  cursor += TR::Compiler->om.sizeofReferenceAddress();

  //  save CPIndex as sign extended 8 byte value on 64bit as it's assumed in J9
  //  helpers
  // cp index
  *(intptrj_t *)cursor =
      (intptrj_t)callNode->getSymbolReference()->getCPIndex();
  cursor += TR::Compiler->om.sizeofReferenceAddress();
  ...
  *(intptrj_t *)cursor = 0;
  ...
  TR::list<TR_OpaqueClassBlock *> *profiledClassesList =
      cg()->getPICsListForInterfaceSnippet(this);
  if (profiledClassesList) {
    for (auto valuesIt = profiledClassesList->begin();
         valuesIt != profiledClassesList->end(); ++valuesIt) {
      TR::SymbolReference *methodSymRef = callNode->getSymbolReference();
      TR_ResolvedMethod *profiledMethod =
          methodSymRef->getOwningMethod(comp)->getResolvedInterfaceMethod(
              comp, (TR_OpaqueClassBlock *)(*valuesIt),
              methodSymRef->getCPIndex());
      ...
      if (cg()->fe()->isUnloadAssumptionRequired(
              (TR_OpaqueClassBlock *)(*valuesIt), comp->getCurrentMethod())) {
      ...
      // Method Pointer
      *(uintptrj_t *)(cursor) =
          (uintptrj_t)profiledMethod->startAddressForJittedMethod();
      cursor += TR::Compiler->om.sizeofReferenceAddress();
    }
  }

  // Skip the top cache slots that are filled with IProfiler data by setting the
  // cursorlastCachedSlot to point to the fist dynamic cache slot
  if (updateField) {
    *(intptrj_t *)(cursorlastCachedSlot) =
        snippetStart + getFirstSlotOffset() +
        ((getNumInterfaceCallCacheSlots() - numInterfaceCallCacheSlots - 1) *
         2 * TR::Compiler->om.sizeofReferenceAddress());
  }
  ...
  for (i = 0; i < numInterfaceCallCacheSlots; i++) {
    if (isUseCLFIandBRCL()) {
      ...
      // jitAddPicToPatchOnClassUnload is called by PicBuilder code when the
      // cache is populated.
      *(intptrj_t *)cursor = (intptrj_t)(getFirstCLFI()->getBinaryEncoding()) +
                             (intptrj_t)(i * 12 + 2);
      cursor += TR::Compiler->om.sizeofReferenceAddress();
    } else {
      // Class Pointer - jitAddPicToPatchOnClassUnload is called by PicBuilder
      // code when the cache is populated.
      *(intptrj_t *)cursor = 0;
      cursor += TR::Compiler->om.sizeofReferenceAddress();
...
uint32_t TR::S390InterfaceCallDataSnippet::getCallReturnAddressOffset() {
  return 0;
}

uint32_t TR::S390InterfaceCallDataSnippet::getSingleDynamicSlotOffset() {
  return getCallReturnAddressOffset() +
         5 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390InterfaceCallDataSnippet::getLastCachedSlotFieldOffset() {
  return getCallReturnAddressOffset() +
         6 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390InterfaceCallDataSnippet::getFirstSlotFieldOffset() {
  return getLastCachedSlotFieldOffset() +
         TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390InterfaceCallDataSnippet::getLastSlotFieldOffset() {
  return getFirstSlotFieldOffset() + TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390InterfaceCallDataSnippet::getFirstSlotOffset() {
  return getLastSlotFieldOffset() + TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390InterfaceCallDataSnippet::getLastSlotOffset() {
  return (getFirstSlotOffset() +
          (getNumInterfaceCallCacheSlots() - 1) *
              ((isUseCLFIandBRCL() ? 1 : 2) *
               TR::Compiler->om.sizeofReferenceAddress()));
}

uint32_t TR::S390InterfaceCallDataSnippet::getLength(int32_t) {
  TR::Compilation *comp = cg()->comp();
  // the 1st item is for padding...
  if (getNumInterfaceCallCacheSlots() == 0)
    return 2 * TR::Compiler->om.sizeofReferenceAddress() +
           8 * TR::Compiler->om.sizeofReferenceAddress();

  if (comp->getOption(TR_enableInterfaceCallCachingSingleDynamicSlot))
    return 2 * TR::Compiler->om.sizeofReferenceAddress() +
           getSingleDynamicSlotOffset() +
           4 * TR::Compiler->om.sizeofReferenceAddress();

  if (getNumInterfaceCallCacheSlots() > 0)
    return 2 * TR::Compiler->om.sizeofReferenceAddress() + getLastSlotOffset() +
           TR::Compiler->om.sizeofReferenceAddress();
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::S390TargetAddressSnippet *snippet) {
  // *this   swipeable for debugger
  if (pOutFile == NULL) {
    return;
  }
...
void TR_Debug::print(TR::FILE *pOutFile, TR::S390LookupSwitchSnippet *snippet) {
  ...
  for (int i = 1; i < upperBound; i++) {
    ...
    int32_t caseKeyTarget = (intptrj_t)caseKeyNode->getBranchDestination()
                                ->getNode()
                                ->getLabel()
                                ->getCodeLocation();
...
void TR_Debug::print(TR::FILE *pOutFile, TR::S390ConstantDataSnippet *snippet) {
  ...
  if (snippet->getKind() == TR::Snippet::IsWritableData) {
    ...
    printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                      "Label Table Snippet");
    trfprintf(pOutFile, "\n");
    TR::S390LabelTableSnippet *labelTable =
        (TR::S390LabelTableSnippet *)snippet;
    ...
    printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                      "Declarative Trampoline Snippet");
    trfprintf(pOutFile, "\n");
    TR::S390DeclTrampSnippet *declTrampSnippet =
        (TR::S390DeclTrampSnippet *)snippet;
    ...
    printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                      "Sort Jump Trampoline Snippet");
    trfprintf(pOutFile, "\n");
    TR::S390SortJumpTrampSnippet *sortJumpTrampSnippet =
        (TR::S390SortJumpTrampSnippet *)snippet;
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::S390InterfaceCallDataSnippet *snippet) {
  uint8_t *bufferPos = snippet->getSnippetLabel()->getCodeLocation();
  ...
  if (snippet->getLength(0) ==
      6 * sizeof(intptrj_t) + snippet->getSingleDynamicSlotOffset()) {
    isSingleDynamicSlot = 1;
  }

  printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                    "Interface Call Data Snippet");

  printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile, "DC   \t%p \t\t# Call Site RA",
            *((intptrj_t *)bufferPos));
  bufferPos += sizeof(intptrj_t);

  printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile, "DC   \t%p \t\t# Address Of Constant Pool",
            *((intptrj_t *)bufferPos));
  bufferPos += sizeof(intptrj_t);
  ...
  printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile, "DC   \t%p \t\t# Interface Class",
            *((intptrj_t *)bufferPos));
  bufferPos += sizeof(intptrj_t);

  printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile, "DC   \t%p \t\t# Method Index",
            *((intptrj_t *)bufferPos));
  bufferPos += sizeof(intptrj_t);

  if (snippet->getNumInterfaceCallCacheSlots() == 0) {
    printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
    trfprintf(pOutFile, "DC   \t%p \t\t# Resolved Flag",
              *((intptrj_t *)bufferPos));
  } else {
    if (!isSingleDynamicSlot) {
      ...
      printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
      trfprintf(pOutFile, "DC   \t%p \t\t# lastCachedSlotField",
                *((intptrj_t *)bufferPos));
      bufferPos += sizeof(intptrj_t);

      printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
      trfprintf(pOutFile, "DC   \t%p \t\t# firstSlotField",
                *((intptrj_t *)bufferPos));
      bufferPos += sizeof(intptrj_t);

      printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
      trfprintf(pOutFile, "DC   \t%p \t\t# lastSlotField",
                *((intptrj_t *)bufferPos));
      bufferPos += sizeof(intptrj_t);
    }
    ...
    for (i = 0; i < snippet->getNumInterfaceCallCacheSlots(); i++) {
      if (snippet->isUseCLFIandBRCL()) {
        ...
        trfprintf(
            pOutFile,
            "DC   \t%p \t\t# Addr of CLFI immed field for cached class %d",
            *((intptrj_t *)bufferPos), i + 1);
        bufferPos += sizeof(intptrj_t);
      } else {
        printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
        trfprintf(pOutFile, "DC   \t%p \t\t# cached class%d",
                  *((intptrj_t *)bufferPos), i + 1);
        bufferPos += sizeof(intptrj_t);

        printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
        trfprintf(pOutFile, "DC   \t%p \t\t# cached method%d",
                  *((intptrj_t *)bufferPos), i + 1);
        bufferPos += sizeof(intptrj_t);
      }
    }
...
TR::S390LabelTableSnippet::S390LabelTableSnippet(TR::CodeGenerator *cg,
                                                 TR::Node *node, uint32_t size)
    : TR::S390ConstantDataSnippet(cg, node, NULL, 0), _size(size) {
  TR_ASSERT(TR::Compiler->target.is32Bit(),
            "only 31bit mode supported for TR::S390LabelTableSnippet\n");
  _labelTable = (TR::LabelSymbol **)cg->trMemory()->allocateMemory(
      sizeof(TR::LabelSymbol *) * size, heapAlloc);
  setLength(0);
}

uint8_t *TR::S390LabelTableSnippet::emitSnippetBody() {
  ...
  for (int32_t i = 0; i < _size; i++) {
    if (_labelTable[i]) {
      cg()->addRelocation(
          new (cg()->trHeapMemory())
              TR_32BitLabelTableRelocation(cursor, _labelTable[i]));
...
uint32_t TR::S390LabelTableSnippet::getLength(int32_t estimatedSnippetStart) {
  return _size * 4;
}

TR::S390DeclTrampSnippet::S390DeclTrampSnippet(TR::CodeGenerator *cg,
                                               TR::LabelSymbol *label)
    : TR::S390ConstantDataSnippet(cg, NULL, NULL, 0), _label(label) {
  TR_ASSERT(TR::Compiler->target.is32Bit(),
            "only 31bit mode supported for TR::S390DeclTrampSnippet\n");
  setLength(16);
}

uint8_t *TR::S390DeclTrampSnippet::emitSnippetBody() {
  uint8_t *snip = cg()->getBinaryBufferCursor();
  getSnippetLabel()->setCodeLocation(snip);
  uint8_t *cursor = snip;
  ...
  *(uint16_t *)cursor = 0xc0f4;
  cg()->addRelocation(new (cg()->trHeapMemory())
                          TR_32BitLabelRelativeRelocation(cursor, _label));
  cursor += 6;
...
TR::S390SortJumpTrampSnippet::S390SortJumpTrampSnippet(TR::CodeGenerator *cg,
                                                       TR::LabelSymbol *label)
    : TR::S390ConstantDataSnippet(cg, NULL, NULL, 0), _label(label) {
  TR_ASSERT(TR::Compiler->target.is32Bit(),
            "only 31bit mode supported for TR::S390SortJumpTrampSnippet\n");
  setLength(16);
}

uint8_t *TR::S390SortJumpTrampSnippet::emitSnippetBody() {
  uint8_t *snip = cg()->getBinaryBufferCursor();
  getSnippetLabel()->setCodeLocation(snip);
  uint8_t *cursor = snip;
