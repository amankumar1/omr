...
void TR_S390CompareAnalyser::integerCompareAnalyser(
  ...
  TR_ASSERT(
      0, "TR_S390CompareAnalyser::integerCompareAnalyser: Not implemented yet");
  return;
}
...
TR_S390CompareAnalyser::longOrderedCompareAndBranchAnalyser(
  ...
  if (firstChild->isHighWordZero() && !isUnsignedCmp &&
      (brCmpLowTrueCond == TR::InstOpCode::COND_BERC ||
       brCmpLowTrueCond == TR::InstOpCode::COND_BNERC)) {
    firstHighZero = true;
  }
  ...
  if (root->getFirstChild() != firstChild) // May have skipped original first
                                           // child earlier (i.e. if 1st child
                                           // is iu2l)
