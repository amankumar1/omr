...
void *getCurrPC();
#pragma mc_func getCurrPC{"48000005" /* bl .+4   */                            \
                          "7C6802A6" /* mfspr r3,lr */                         \
}

void *getCurrTos();
#pragma mc_func getCurrTos{"38610000" /* mr r3, r1   */                        \
}
...
void TR_LinuxCallStackIterator::printSymbol(int32_t frame, char *sig,
  ...
  int rc = sscanf(sig, "%255[^(](%255[^+]+%llx) [%llx]", lib, func, &offset,
                  &address);
  if (rc == 4) {
...
#define SYMOPT_DEBUG 0x80000000
  ...
  if (!(*SymInitialize)(hProcess, userSearchPath, TRUE)) {
    ...
    fprintf(stderr, "Unable to retrieve symbol informaton. SymInitialize "
                    "returned error : %d\n",
            GetLastError());
  }
