...
// returned in optionsArray. if the size of the array is insufficient to hold
// them
// all, the required size is returned so that the caller can provide a bigger
// array
...
void TR_Debug::dumpOptions(char *optionsType, char *options, char *envOptions,
  ...
  while (ojit->name || ofe->name) {
    if (ojit->name && (!ojit->msg || (ojit->msg[0] == 'F' &&
                                      !(ojit->msgInfo & OPTION_FOUND))) &&
        !ojit->enabled) {
      ojit++;
      continue;
    }
