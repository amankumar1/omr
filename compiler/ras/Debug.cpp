...
bool TR_Debug::performTransformationImpl(bool canOmitTransformation,
  ...
  if (canOmitTransformation) {
    ...
    if ((format[0] == '%' && format[1] == 's') ||
      ...
      // transformations
      // this should already be in most transformations but it could get
...
void TR_Debug::printAsmDumpHeader() {
  if (_file == NULL)
    return;
  ...
  trfprintf(_file, "\n  |%*s +----------------------------------------- binary "
                   "representation",
            addressFieldWidth - 3, " ", addressFieldWidth - 1, " ");
  trfprintf(_file,
            "\n  |%*s | %*s+----------------------------- opcode and operands",
            addressFieldWidth - 3, " ", codeByteColumnWidth + 4, " ");
  trfprintf(_file, "\n  |%*s | %*s|\t\t\t\t    +------- additional information",
            addressFieldWidth - 3, " ", codeByteColumnWidth + 4, " ");
  trfprintf(_file, "\n  |%*s | %*s|\t\t\t\t    |", addressFieldWidth - 3, " ",
            codeByteColumnWidth + 4, " ");
  trfprintf(_file, "\n  V%*s V %*sV\t\t\t\t    V\n", addressFieldWidth - 3, " ",
            codeByteColumnWidth + 4, " ");
}
#endif
...
void TR_Debug::printSnippetLabel(TR::FILE *pOutFile, TR::LabelSymbol *label,
  ...
  trfprintf(pOutFile, ":");
  if (comment1) {
    trfprintf(pOutFile, "\t\t%c %s", (TR::Compiler->target.cpu.isX86() &&
...
const char *TR_Debug::getAutoName(TR::SymbolReference *symRef) {
  ...
  if (symRef->getSymbol()->isRegisterSymbol()) {
    ...
    if (symReg->isRealRegister())
      sprintf(name, "<GlobalReg%d %s>", symReg->getGlobalRegisterNumber(),
              getGlobalRegisterName(symReg->getGlobalRegisterNumber()));
    else
    ...
    if (symRef->getSymbol()->getDataType() == TR::Float ||
      ...
      sprintf(symName, "#FPSPILL%d_%d", symRef->getSymbol()->getSize(),
              symRef->getReferenceNumber());
    else
      sprintf(symName, "#SPILL%d_%d", symRef->getSymbol()->getSize(),
              symRef->getReferenceNumber());
    ...
    sprintf(name, "<auto marker symbol " POINTER_PRINTF_FORMAT ": %s>", symbol,
            symbol->getName());
...
const char *TR_Debug::getShadowName(TR::SymbolReference *symRef) {
  ...
  if (symRef->getSymbol()) {
    ...
    if (symRef->getSymbol()->isPythonLocalVariableShadowSymbol()) {
      auto comp = TR::comp();
      return symRef->getOwningMethod(comp)->localName(
          symRef->getOffset() / TR::DataType::getSize(TR::Address), -1,
          comp->trMemory());
    }
      ...
      while (cursor) {
        ...
        if (outlinedCursor) {
          ...
          if (outlinedCursor != lastOutlinedTarget) {
            ...
            // by keeping track of the last sequence we dumped
  ...
  while (instr) {
    ...
    if (printLinenos) {
      ...
      if ((crtLineNo != -1) &&
        ...
        // the first line number of the method. Line numbrers for includes are
        // absoluted .
        // Since we treat Inlined methods (like include files - see previous),
        // such line numbers
...
const char *TR_Debug::getName(TR::Register *reg, TR_RegisterSizes size) {
  ...
  if (reg->getRegisterPair()) {
    ...
    // can't contain collected refs or internal pointers anyway.  Let's not
    // worry about it.
    sprintf(prefix, "");
  } else {
...
void TR_Debug::printFullRegInfo(TR::FILE *pOutFile, TR::Register *reg) {
  ...
  if (reg->getRealRegister()) {
    ...
    trfprintf(pOutFile, "%-12s", reg->getAssignedRegister()
                                     ? getName(reg->getAssignedRegister())
                                     : " ");
...
void TR_Debug::print(TR::FILE *pOutFile, TR_GCStackMap *map,
  ...
  if (internalPtrMap) {
    ...
    for (; currElement; currElement = currElement->getNextElement())
      trfprintf(
          pOutFile, "      pinning array GC stack map index = %d Internal "
                    "pointer regnum = %d\n",
  ...
  if (!inDebugExtension() && !vguards.empty()) {
    ...
    for (auto info = vguards.begin(); info != vguards.end(); ++info, ++i) {
      ...
      if ((*info)->getKind() == TR_OSRGuard)
        ...
        trfprintf(pOutFile,
                  "[%4d] %-38s %scalleeSymbol=" POINTER_PRINTF_FORMAT "\n", i,
      ...
      for (TR_InnerAssumption *inner = innerIt.getFirst(); inner;
        ...
        trfprintf(
            pOutFile, "\tInner Assumption: calleeSymbol=" POINTER_PRINTF_FORMAT
                      " for parm ordinal=%d\n",
...
const char *TR_Debug::getRuntimeHelperName(int32_t index) {
  ...
  if (index <= PyHelper_Last) {
    switch (index) {
    case PyHelper_TupleGetItem:
      return "PyTuple_GetItem";
    case PyHelper_UnaryNot:
      return "Py_doUnaryNot";
    case PyHelper_BinaryRemainder:
      return "Py_doBinaryRemainder";
    case PyHelper_BinaryAdd:
      return "Py_doBinaryAdd";
    case PyHelper_InPlaceAdd:
      return "Py_doInPlaceAdd";
    case PyHelper_LoadGlobal:
      return "Py_LoadGlobal";
    case PyHelper_ListAppend:
      return "PyList_Append";
    case PyHelper_ObjectDelItem:
      return "PyObject_DelItem";
    case PyHelper_ObjectGetItem:
      return "PyObject_GetItem";
    case PyHelper_ObjectGetIter:
      return "PyObject_GetIter";
    case PyHelper_ObjectRichCompare:
      return "PyObject_RichCompare";
    case PyHelper_NumberPositive:
      return "PyNumber_Positive";
    case PyHelper_NumberNegative:
      return "PyNumber_Negative";
    case PyHelper_NumberInvert:
      return "PyNumber_Invert";
    case PyHelper_NumberPower:
      return "PyNumber_Power";
    case PyHelper_NumberMultiply:
      return "PyNumber_Multiply";
    case PyHelper_NumberTrueDivide:
      return "PyNumber_TrueDivide";
    case PyHelper_NumberFloorDivide:
      return "PyNumber_FloorDivide";
    case PyHelper_NumberSubtract:
      return "PyNumber_Subtract";
    case PyHelper_NumberLshift:
      return "PyNumber_Lshift";
    case PyHelper_NumberRshift:
      return "PyNumber_Rshift";
    case PyHelper_NumberAnd:
      return "PyNumber_And";
    case PyHelper_NumberXor:
      return "PyNumber_Xor";
    case PyHelper_NumberOr:
      return "PyNumber_Or";
    case PyHelper_NumberInPlacePower:
      return "PyNumber_InPlacePower";
    case PyHelper_NumberInPlaceMultiply:
      return "PyNumber_InPlaceMultiply";
    case PyHelper_NumberInPlaceTrueDivide:
      return "PyNumber_InPlaceTrueDivide";
    case PyHelper_NumberInPlaceFloorDivide:
      return "PyNumber_InPlaceFloorDivide";
    case PyHelper_NumberInPlaceRemainder:
      return "PyNumber_InPlaceRemainder";
    case PyHelper_NumberInPlaceSubtract:
      return "PyNumber_InPlaceSubtract";
    case PyHelper_NumberInPlaceLshift:
      return "PyNumber_InPlaceLshift";
    case PyHelper_NumberInPlaceRshift:
      return "PyNumber_InPlaceRshift";
    case PyHelper_NumberInPlaceAnd:
      return "PyNumber_InPlaceAnd";
    case PyHelper_NumberInPlaceXor:
      return "PyNumber_InPlaceXor";
    case PyHelper_NumberInPlaceOr:
      return "PyNumber_InPlaceOr";
    case PyHelper_SequenceContains:
      return "PySequence_Contains";
    case PyHelper_SetAdd:
      return "PySet_Add";
    case PyHelper_exceptionFromFrame:
      return "Py_exceptionFromFrame";
    case PyHelper_exceptionOnByteCode:
      return "Py_exceptionOnByteCode";
    case PyHelper_exceptionOnByteCodeAndReturn:
      return "Py_exceptionOnByteCode";
    case PyHelper_exceptionLoadFast:
      return "Py_exceptionLoadFast";
    case PyHelper_exceptionLoadFastAndReturn:
      return "Py_exceptionLoadFastNoHandler";
    case PyHelper_compareOpForExceptionMatch:
      return "Py_compareOpForExceptionMatch";
    case PyHelper_ObjectSetItem:
      return "PyObject_SetItem";
    case PyHelper_DictSetItem:
      return "PyDict_SetItem";
    case PyHelper_ObjectIsTrue:
      return "PyObject_IsTrue";
    case PyHelper_callfunction:
      return "call_function";
    case PyHelper_long_neg:
      return "long_neg";
    case PyHelper_long_long:
      return "long_long";
    case PyHelper_long_add:
      return "long_add";
    case PyHelper_long_sub:
      return "long_sub";
    case PyHelper_long_mul:
      return "long_mul";
    case PyHelper_long_div:
      return "long_div";
    case PyHelper_long_true_divide:
      return "long_true_divide";
    case PyHelper_long_mod:
      return "long_mod";
    case PyHelper_long_lshift:
      return "long_lshift";
    case PyHelper_long_rshift:
      return "long_rshift";
    case PyHelper_long_and:
      return "long_and";
    case PyHelper_long_xor:
      return "long_xor";
    case PyHelper_long_or:
      return "long_or";
    case PyHelper_UnpackSequence:
      return "unpack_iterable";
    case PyHelper_TupleNew:
      return "PyTuple_New";
    case PyHelper_TupleSetItem:
      return "PyTuple_SetItem";
    case PyHelper_ListNew:
      return "PyList_New";
    case PyHelper_ListSetItem:
      return "PyList_SetItem";
    case PyHelper_SetNew:
      return "PySet_New";
    case PyHelper_DictNewPresized:
      return "_PyDict_NewPresized";
    case PyHelper_SliceNew:
      return "PySlice_New";
    case PyHelper_ObjectGetAttr:
      return "PyObject_GetAttr";
    case PyHelper_ObjectSetAttr:
      return "PyObject_SetAttr";
    case PyHelper_DestroyObject:
      return "_Py_Dealloc";
    case PyHelper_DictDelItem:
      return "PyDict_DelItem";
    case PyHelper_IncRef:
      return "Py_IncRef";
    case PyHelper_DecRef:
      return "Py_DecRef";
    case PyHelper_StopIterating:
  ...
  if (index < TR_FSRH) {
    switch (index) {
    case RubyHelper_rb_funcallv:
      return "rb_funcallv";
    case RubyHelper_vm_send:
      return "vm_send";
    case RubyHelper_vm_send_without_block:
      return "vm_send_without_block";
    case RubyHelper_vm_getspecial:
      return "vm_getspecial";
    case RubyHelper_lep_svar_set:
      return "vm_lep_svar_set";
    case RubyHelper_vm_getivar:
      return "vm_getivar";
    case RubyHelper_vm_setivar:
      return "vm_setivar";
    case RubyHelper_vm_opt_plus:
      return "vm_opt_plus";
    case RubyHelper_vm_opt_minus:
      return "vm_opt_minus";
    case RubyHelper_vm_opt_mult:
      return "vm_opt_mult";
    case RubyHelper_vm_opt_div:
      return "vm_opt_div";
    case RubyHelper_vm_opt_mod:
      return "vm_opt_mod";
    case RubyHelper_vm_opt_eq:
      return "vm_opt_eq";
    case RubyHelper_vm_opt_neq:
      return "vm_opt_neq";
    case RubyHelper_vm_opt_lt:
      return "vm_opt_lt";
    case RubyHelper_vm_opt_le:
      return "vm_opt_le";
    case RubyHelper_vm_opt_gt:
      return "vm_opt_gt";
    case RubyHelper_vm_opt_ge:
      return "vm_opt_ge";
    case RubyHelper_vm_opt_ltlt:
      return "vm_opt_ltlt";
    case RubyHelper_vm_opt_not:
      return "vm_opt_not";
    case RubyHelper_vm_opt_aref:
      return "vm_opt_aref";
    case RubyHelper_vm_opt_aset:
      return "vm_opt_aset";
    case RubyHelper_vm_opt_length:
      return "vm_opt_length";
    case RubyHelper_vm_opt_size:
      return "vm_opt_size";
    case RubyHelper_vm_opt_empty_p:
      return "vm_opt_empty_p";
    case RubyHelper_vm_opt_succ:
      return "vm_opt_succ";
    case RubyHelper_rb_ary_new_capa:
      return "rb_ary_new_capa";
    case RubyHelper_rb_ary_new_from_values:
      return "rb_ary_new_from_values";
    case RubyHelper_vm_expandarray:
      return "vm_expandarray";
    case RubyHelper_rb_ary_resurrect:
      return "rb_ary_resurrect";
    case RubyHelper_vm_concatarray:
      return "vm_concatarray";
    case RubyHelper_vm_splatarray:
      return "vm_splatarray";
    case RubyHelper_rb_range_new:
      return "rb_range_new";
    case RubyHelper_rb_hash_new:
      return "rb_hash_new";
    case RubyHelper_rb_hash_aset:
      return "rb_hash_aset";
    case RubyHelper_vm_trace:
      return "vm_trace";
    case RubyHelper_rb_str_new:
      return "rb_str_new";
    case RubyHelper_rb_str_new_cstr:
      return "rb_str_new_cstr";
    case RubyHelper_rb_str_resurrect:
      return "rb_str_resurrect";

    case RubyHelper_vm_get_ev_const:
      return "vm_get_ev_const";
    case RubyHelper_vm_check_if_namespace:
      return "vm_check_if_namespace";
    case RubyHelper_rb_const_set:
      return "rb_const_set";
    case RubyHelper_rb_gvar_get:
      return "rb_gvar_get";
    case RubyHelper_rb_gvar_set:
      return "rb_gvar_set";
    case RubyHelper_rb_iseq_add_mark_object:
      return "rb_iseq_add_mark_object";
    case RubyHelper_vm_setinlinecache:
      return "vm_setinlinecache";
    case RubyHelper_vm_throw:
      return "vm_throw";
    case RubyHelper_rb_threadptr_execute_interrupts:
      return "rb_threadptr_execute_interrupts";
    case RubyHelper_rb_vm_ep_local_ep:
      return "rb_vm_ep_local_ep";
    case RubyHelper_vm_get_cbase:
      return "vm_get_cbase";
    case RubyHelper_vm_get_const_base:
      return "vm_get_const_base";
    case RubyHelper_rb_vm_get_cref:
      return "rb_vm_get_cref";
    case RubyHelper_vm_get_cvar_base:
      return "vm_get_cvar_base";
    case RubyHelper_rb_cvar_get:
      return "rb_cvar_get";
    case RubyHelper_rb_cvar_set:
      return "rb_cvar_set";
    case RubyHelper_vm_checkmatch:
      return "vm_checkmatch";
    case RubyHelper_rb_obj_as_string:
      return "rb_obj_as_string";
    case RubyHelper_rb_str_append:
      return "rb_str_append";
    case RubyHelper_rb_ary_tmp_new:
      return "rb_ary_tmp_new";
    case RubyHelper_rb_ary_store:
      return "rb_ary_store";
    case RubyHelper_rb_reg_new_ary:
      return "rb_reg_new_ary";
    case RubyHelper_vm_opt_regexpmatch1:
      return "vm_opt_regexpmatch1";
    case RubyHelper_vm_opt_regexpmatch2:
      return "vm_opt_regexpmatch2";
    case RubyHelper_vm_defined:
      return "vm_defined";
    case RubyHelper_vm_invokesuper:
      return "vm_invokesuper";
    case RubyHelper_vm_invokeblock:
      return "vm_invokeblock";
    case RubyHelper_vm_get_block_ptr:
      return "vm_get_block_ptr";
    case RubyHelper_rb_bug:
      return "rb_bug";
    case RubyHelper_vm_exec_core:
      return "vm_exec_core";
    case RubyHelper_rb_class_of:
      return "rb_class_of";
    case RubyHelper_vm_send_woblock_inlineable_guard:
      return "vm_send_woblock_inlineable_guard";
    case RubyHelper_ruby_omr_is_valid_object:
      return "ruby_omr_is_valid_object";
    case RubyHelper_rb_class2name:
      return "rb_class2name";
    case RubyHelper_vm_opt_aref_with:
      return "vm_opt_aref_with";
    case RubyHelper_vm_opt_aset_with:
      return "vm_opt_aset_with";
    }
  }
#endif
  ...
  if (index < TR_FSRH) {
    ...
    case TR_tenantInitialize:
      return "jitTenantInitialize";
    ...
    case TR_MTUnresolvedInt32Load:
      return "MTUnresolvedInt32Load";
    case TR_MTUnresolvedInt64Load:
      return "MTUnresolvedInt64Load";
    case TR_MTUnresolvedFloatLoad:
      return "MTUnresolvedFloatLoad";
    case TR_MTUnresolvedDoubleLoad:
      return "MTUnresolvedDoubleLoad";
    case TR_MTUnresolvedAddressLoad:
      return "MTUnresolvedAddressLoad";

    case TR_MTUnresolvedInt32Store:
      return "MTUnresolvedInt32Store";
    case TR_MTUnresolvedInt64Store:
      return "MTUnresolvedInt64Store";
    case TR_MTUnresolvedFloatStore:
      return "MTUnresolvedFloatStore";
    case TR_MTUnresolvedDoubleStore:
      return "MTUnresolvedDoubleStore";
    case TR_MTUnresolvedAddressStore:
      return "MTUnresolvedAddressStore";
    ...
    case TR_newObjectNoTenantInit:
      return "jitNewObjectNoTenantInit";
    ...
    case TR_AbstractMethodError:
      return "jitThrowAbstractMethodError";
    case TR_IllegalAccessError:
      return "jitThrowIllegalAccessError";
    ...
    if (index < TR_LXRH) {
      ...
      case TR_X86OutlinedNew:
        return "outlinedNew";
      case TR_X86OutlinedNewArray:
        return "outlinedNewArray";
      case TR_X86OutlinedNewNoZeroInit:
        return "outlinedNewNoZeroInit";
      case TR_X86OutlinedNewArrayNoZeroInit:
        return "outlinedNewArrayNoZeroInit";
      ...
      switch (index) {
      case TR_IA32longMultiply:
        return "__longMultiply";
      case TR_IA32longDivide:
        return "__longDivide";
      case TR_IA32longRemainder:
        return "__longRemainder";
      case TR_IA32longShiftLeft:
        return "__longShiftLeft";
      case TR_IA32longRotateLeft:
        return "__longRotateLeft";
      case TR_IA32longShiftRightLogical:
        return "__longShiftRightLogical";
      case TR_IA32longShiftRightArithmetic:
      ...
      case TR_IA32jitThrowCurrentException:
        return "_jitThrowCurrentException";
      case TR_IA32jitCollapseJNIReferenceFrame:
        return "_jitCollapseJNIReferenceFrame";
      case TR_IA32arrayCopy:
        return "__arrayCopy";
      case TR_IA32wordArrayCopy:
        return "__wordAarrayCopy";
      case TR_IA32halfWordArrayCopy:
        return "__halfWordArrayCopy";
      case TR_IA32forwardArrayCopy:
        return "__forwardArrayCopy";
      case TR_IA32forwardWordArrayCopy:
        return "__forwardWordArrayCopy";
      case TR_IA32forwardHalfWordArrayCopy:
        return "__forwardHalfWordArrayCopy";

      case TR_IA32shortArrayCopy:
        return "__shortArrayCopy";
      case TR_IA32forwardSSEArrayCopy:
        return "__forwardSSEArrayCopy";
      case TR_IA32forwardSSEArrayCopyNoAlignCheck:
        return "__forwardSSEArrayCopyNoAlignCheck";
      case TR_IA32forwardArrayCopy2:
        return "__forwardArrayCopy2";
      case TR_IA32SSEforwardArrayCopy:
        return "_SSEforwardArrayCopy";
      case TR_IA32SSEforwardHalfWordArrayCopy:
        return "_SSEforwardHalfWordArrayCopy";
      case TR_IA32SSEforwardArrayCopyAMDOpteron:
        return "_SSEforwardArrayCopyAMDOpteron";
      case TR_IA32SSEforwardArrayCopyAggressive:
        return "_SSEforwardArrayCopyAggressive";
      ...
      case TR_IA32arrayXor:
        return "arrayxor";
      case TR_IA32arrayOr:
        return "arrayor";
      case TR_IA32arrayAnd:
        return "arrayand";
      case TR_IA32arrayCmp:
        return "arraycmp";
      case TR_IA32overlapArrayCopy:
        return "overlapArrayCopy";
      case TR_IA32getTimeOfDay:
        return "gettimeofday";
      }
    } else {
      switch (index) {
      ...
      case TR_AMD64doubleToLong:
        return "__doubleToLong";
      case TR_AMD64doubleToInt:
        return "__doubleToInt";
      case TR_AMD64floatToLong:
        return "__floatToLong";
      case TR_AMD64floatToInt:
        return "__floatToInt";
      ...
      case TR_AMD64jitThrowCurrentException:
        return "_jitThrowCurrentException";
      case TR_AMD64jitCollapseJNIReferenceFrame:
        return "_jitCollapseJNIReferenceFrame";
      case TR_AMD64arrayCopy:
        return "__arrayCopy";
      case TR_AMD64BCarrayCopy:
        return "__BC_arrayCopy";
      case TR_AMD64byteArrayCopy:
        return "__byteArrayCopy";
      case TR_AMD64wordArrayCopy:
        return "__wordArrayCopy";
      case TR_AMD64halfWordArrayCopy:
        return "__halfWordArrayCopy";
      case TR_AMD64forwardArrayCopy:
        return "__forwardArrayCopy";
      case TR_AMD64forwardWordArrayCopy:
        return "__forwardWordArrayCopy";
      case TR_AMD64forwardHalfWordArrayCopy:
        return "__forwardHalfWordArrayCopy";
      case TR_AMD64forwardArrayCopyAMDOpteron:
        return "_forwardArrayCopyAMDOpteron";
      ...
      case TR_AMD64induceRecompilation:
        return "__induceRecompilation";
      case TR_AMD64arrayXor:
        return "arrayxor";
      case TR_AMD64arrayOr:
        return "arrayor";
      case TR_AMD64arrayAnd:
        return "arrayand";
      case TR_AMD64noOverlapArrayXor:
        return "noOverlapArrayXor";
      case TR_AMD64noOverlapArrayOr:
        return "noOverlapArrayOr";
      case TR_AMD64noOverlapArrayAnd:
        return "noOverlapArrayAnd";
      case TR_AMD64overlapArrayXor:
        return "overlapArrayXor";
      case TR_AMD64overlapArrayOr:
        return "overlapArrayOr";
      case TR_AMD64overlapArrayAnd:
        return "overlapArrayAnd";
      case TR_AMD64arrayCmp:
        return "arraycmp";
      case TR_AMD64overlapArrayCopy:
        return "overlapArrayCopy";
      }
    }
  }
...
/* Work in progress side-by side traceRA={*} */
        ...
        if (_registerKindsToAssign & TR_HPR_Mask) {
          ...
          for (TR::Register *hpr = iter->getFirst(); hpr;
               hpr = iter->getNext()) {
        ...
        if (_registerKindsToAssign & TR_X87_Mask) {
          ...
          for (TR::Register *reg = iter->getFirst(); reg;
               reg = iter->getNext()) {
  ...
  if (::feGetEnv("DEBUG_PROG") != NULL) {
    ...
    if (!started) {
      ...
      if (!ipAddress || !libPath || !lib) {
        fprintf(stderr, "You must specify a valid TCP/IP address for the "
                        "debugger to connect to\n");
        fprintf(
            stderr,
            "You must specify a valid Library Path to get the debugger from\n");
        fprintf(stderr, "You must specify a valid Library to load\n");
        fprintf(stderr, "... these are DER_DBG_ADDR, DER_DBG_LIBPATH, "
                        "DER_DBG_LIB respectively\n");
        return;
      }
      char debugger[2050];
      ...
      if (!attach || !stop) {
        fprintf(stderr, "One of attach (%p) or stop (%p) could not be located "
                        "in %s shared library\n",
      ...
      if (rc != 1) {
        fprintf(stderr,
                "Return code of %d (0x%x) on stop_at request of debugger\n", rc,
                rc);
        return;
      }
    }
...
#define BUFFER_SIZE 1024
  ...
  if (!started) {
    ...
    *startupInfo = 17 * 4;            // ditto...
    ...
    size = sprintf(appBuffer, "\"C:\\Program Files\\Debugging Tools For "
                              "Windows\\windbg\" -p %d -srcpath %s -c \"%s\"",
...
static int counterCompare(const char *left, const char *right) {
  ...
  // where indicated
  // Note: '.' doesn't work like an actual decimal point, ".12" > ".2" because
  // 12 > 2
  ...
  while (*left != 0 && *right != 0) {
    ...
    if (leftSectionLength != rightSectionLength) {
      return numericComparisonMode
                 ? leftSectionLength - rightSectionLength // Assume that numbers
                                                          // are not 0-padded,
                                                          // so any longer
                                                          // number must be
                                                          // bigger
...
void TR_Debug::printDebugCounters(TR::DebugCounterGroup *counterGroup,
  ...
  // denominator D,
  // no other nonzero counter has denominator D, and C's count equals D's count,
  // then D
