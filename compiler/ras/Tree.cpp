...
// the last optimization and also sets TR::Compilationl::_prevSymRefTabSize
// to the current size of symbol reference table as it is the end of this
// optimization.
//
...
int32_t TR_Debug::printWithFixedPrefix(TR::FILE *pOutFile, TR::Node *node,
  ...
  if (_nodeChecklist.isSet(node->getGlobalIndex())) {
    ...
    if (_comp->getAppendInstruction() != NULL &&
...
void TR_Debug::printBasicPreNodeInfoAndIndent(TR::FILE *pOutFile,
  ...
  if (node->getOpCodeValue() == TR::BBStart &&
      ...
      node->getBlock()->getPrevBlock()) // a block that is not an extension of
                                        // previous block and is not the first
                                        // block
  {
    trfprintf(pOutFile, "\n");
  }
...
void TR_Debug::printBasicPostNodeInfo(TR::FILE *pOutFile, TR::Node *node,
  ...
  if ((_comp->getNodeOpCodeLength() + indentation) <
    ...
    output.append("%*s", DEFAULT_NODE_LENGTH + DEFAULT_INDENT_INCREMENT -
...
void TR_Debug::printNodeInfo(TR::Node *node, TR_PrettyPrinterString &output,
  ...
  if (node->getOpCode().hasSymbolReference() && node->getSymbolReference()) {
    ...
    if ((nextBlock = block->getNextBlock()) &&
        !nextBlock->isExtensionOfPreviousBlock()) // end of a block that is not
                                                  // the last block and the next
                                                  // block is not an extension
                                                  // of it
      output.append(" =====");
...
void TR_Debug::printBCDNodeInfo(TR::Node *node,
  ...
  if (node->getType().isBCD()) {
    ...
    } else if (node->getOpCode().isStore() || node->getOpCode().isCall() ||
    ...
    if (!node->getOpCode().isStore()) {
      ...
      if (node->hasKnownOrAssumedCleanSign() ||
        ...
        if (node->hasKnownOrAssumedSignCode())
          output.append("%s%s", node->hasKnownOrAssumedCleanSign() ||
                                        node->hasKnownOrAssumedPreferredSign()
                                    ? "/"
                                    : "",
...
void TR_Debug::printVCG(TR::FILE *pOutFile, TR::CFG *cfg, const char *sig) {
  ...
  if (debug("nestedVCG")) {
    ...
    trfprintf(pOutFile, "node: {title: \"Top1\" label: \"%s\" vertical_order: "
                        "0 horizontal_order: 0 textcolor: blue borderwidth: "
                        "1}\n",
              sig);
    ...
    trfprintf(pOutFile, "edge.color: black\n");
    trfprintf(pOutFile, "node: {title: \"Top1\" label: \"%s\" vertical_order: "
                        "0 textcolor: blue borderwidth: 1}\n",
              sig);
...
void TR_Debug::printVCG(TR::FILE *pOutFile, TR::Block *block, int32_t vorder,
  ...
  for (auto edge = block->getExceptionSuccessors().begin();
    ...
    if (b->getNumber() >= 0) {
      trfprintf(
          pOutFile, "edge: { sourcename: \"%d\" targetname: \"%d\" linestyle: "
    ...
    for (int32_t i = node->getNumChildren() - 1; i >= 0; --i) {
      ...
      if (debug("checkTypes") && expectedType != TR::NoType) {
        ...
        if (childType != expectedType && childType != TR::NoType &&
          ...
          if (getFile() != NULL) {
            trfprintf(getFile(), "TREE VERIFICATION ERROR -- node [%s] has "
                                 "wrong type for child [%s] (%s), expected "
                                 "%s\n",
  ...
  if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
    ...
    if (isTreeTop) {
      if (node->getReferenceCount() != 0) {
        if (getFile() != NULL)
          trfprintf(getFile(), "TREE VERIFICATION ERROR -- treetop node [%s] "
                               "with ref count %d\n",
    ...
    if (node->getReferenceCount() > 1 &&
      ...
      if (getFile() != NULL)
        trfprintf(getFile(), "TREE VERIFICATION ERROR -- void call node [%s] "
                             "with ref count %d\n",
                  getName(node), node->getReferenceCount());
      TR_ASSERT(debug("fixTrees"), "Tree verification error");
    }
    ...
    if (node->getReferenceCount() != node->getLocalIndex()) {
      if (getFile() != NULL)
        trfprintf(getFile(), "TREE VERIFICATION ERROR -- node [%s] ref count "
...
void TR_Debug::verifyBlocks(TR::ResolvedMethodSymbol *methodSymbol) {
  ...
  for (tt = methodSymbol->getFirstTreeTop(); tt;
    ...
    // it is 0
    // do not walk the tree backwards as this causes huge stack usage in
    // verifyBlocksPass2
    _nodeChecklist.empty();
...
// each child is
// decremented for each visit.  The second pass is to make sure that the
// verifyRefCount is zero
...
void TR_Debug::verifyBlocksPass2(TR::Node *node) {
  ...
  if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
    ...
    if (node->getLocalIndex() != 0) {
      char buffer[100];
      sprintf(buffer, "BLOCK VERIFICATION ERROR -- node [%s] accessed outside "
                      "of its (extended) basic block: %d time(s)",
