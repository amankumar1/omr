...
class DebugCounter : public DebugCounterBase {
  ...
  DebugCounter(const char *name, int8_t fidelity,
      ...
      : _name(name), _fidelity(fidelity), _denominator(denominator),
...
public:
  ...
  // memory;
  // these functions will scan existing counter names and reuse them where
  // possible
  ...
  void getInsertionCounterNames(TR::Compilation *comp, TR::Node *node,
...
class DebugCounterAggregation : public DebugCounterBase {
  ...
  void aggregateDebugCounterInsertions(TR::Compilation *comp, TR::Node *node,
                                       DebugCounter *counter, int32_t delta,
                                       int8_t fidelity, int32_t staticDelta);
  void aggregateDebugCounterHistogram(TR::Compilation *comp, TR::Node *node,
                                      DebugCounter *counter, int32_t delta,
                                      int8_t fidelity, int32_t staticDelta);

protected:
  DebugCounterAggregation(TR_Memory *mem)
      : _mem(mem), _counterDeltas(new (mem->trPersistentMemory())
                                      TR_PersistentList<CounterDelta>()),
...
class DebugCounterGroup {
  ...
  TR::Monitor *_countersMutex; /**< Monitor used to synchronize read/write
                                  actions to _countersHashTable, otherwise we
                                  may have a race */
