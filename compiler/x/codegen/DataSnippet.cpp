...
TR::IA32DataSnippet::IA32DataSnippet(TR::CodeGenerator *cg, TR::Node *n,
                                     void *c, uint8_t size)
...
void TR::IA32DataSnippet::addMetaDataForCodeAddress(uint8_t *cursor) {
...
void TR_Debug::print(TR::FILE *pOutFile, TR::IA32DataSnippet *snippet) {
  ...
  trfprintf(pOutFile, "%s \t%s",
            snippet->getDataSize() == 8
                ? dqString()
                : (snippet->getDataSize() == 4 ? ddString() : dwString()),
            hexPrefixString());
  ...
  if (snippet->getDataSize() == 8) {
    ...
    trfprintf(pOutFile, "\t%s %gD", commentString(), *d);
  } else if (snippet->getDataSize() == 4) {
    float *f = (float *)bufferPos;
    trfprintf(pOutFile, "\t\t%s %gF", commentString(), *f);
  } else if (snippet->getDataSize() == 2) {
    int16_t *s = (int16_t *)bufferPos;
    trfprintf(pOutFile, "\t\t\t%s 0x%04x", commentString(), *s);
  }
}
