...
struct TR_MovStatus // What kind of RegReg movs must be applied to a given
                    // register
{
...
public:
  ...
  void alignCollectedLocalObject(uint8_t pointerSize, TR::GCStackAtlas *atlas,
                                 uint32_t &stackIndex) {}
  void alignUncollectedLocalObject(TR::AutomaticSymbol *p, uint32_t size,
                                   uint32_t &stackIndex) {}
...
protected:
  Linkage(TR::CodeGenerator *cg) : _cg(cg), _minimumFirstInstructionSize(0) {
    ...
    // target.
    //
    TR_X86OpCodes op = cg->getXMMDoubleLoadOpCode()
