...
TR::X86FPConversionSnippet::emitCallToConversionHelper(uint8_t *buffer) {
  ...
  *buffer++ = 0xe8; // CallImm4
  ...
  if (NEEDS_TRAMPOLINE(helperAddress, buffer + 4, cg())) {
    helperAddress = cg()->fe()->indexedTrampolineLookup(
        getHelperSymRef()->getReferenceNumber(), (void *)buffer);
    TR_ASSERT(IS_32BIT_RIP(helperAddress, buffer + 4),
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86fbits2iSnippet *snippet) {
  // *this    swipeable for debugging purposes
  ...
  trfprintf(pOutFile, "test\t%s, %sfffffe00%s", getName(snippet->getRegister()),
            hexPrefixString(), hexSuffixString());
  bufferPos += 6;
  ...
  printPrefix(pOutFile, NULL, bufferPos, 5);
  trfprintf(pOutFile, "mov\t%s, " POINTER_PRINTF_FORMAT,
            getName(snippet->getRegister()), FLOAT_NAN);
  bufferPos += 5;
...
uint32_t TR::X86fbits2iSnippet::getLength(int32_t estimatedSnippetStart) {
  ...
  if (getRestartLabel()->getCodeLocation() != 0) {
    location =
        getRestartLabel()->getCodeLocation() - cg()->getBinaryBufferStart();
  }
  int32_t distance = location - (estimatedSnippetStart + length +
                                 2); // 2 is size of short branch
