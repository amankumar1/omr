...
class TR_GCStackMap;
namespace TR {
class IA32ConstantDataSnippet;
}
namespace TR {
class IA32DataSnippet;
}
...
public:
  ...
  bool supportsVirtualModeExtension() {
    return _featureFlags.testAny(TR_VirtualModeExtension);
  }
  bool supportsDebuggingExtension() {
    return _featureFlags.testAny(TR_DebuggingExtension);
  }
  bool supportsPageSizeExtension() {
    return _featureFlags.testAny(TR_PageSizeExtension);
  }
  bool supportsRDTSCInstruction() {
    return _featureFlags.testAny(TR_RDTSCInstruction);
  }
  bool hasModelSpecificRegisters() {
    return _featureFlags.testAny(TR_ModelSpecificRegisters);
  }
  bool supportsPhysicalAddressExtension() {
    return _featureFlags.testAny(TR_PhysicalAddressExtension);
  }
  bool supportsMachineCheckException() {
    return _featureFlags.testAny(TR_MachineCheckException);
  }
  bool supportsCMPXCHG8BInstruction() {
    return _featureFlags.testAny(TR_CMPXCHG8BInstruction);
  }
  bool supportsCMPXCHG16BInstruction() {
    return _featureFlags2.testAny(TR_CMPXCHG16BInstruction);
  }
  bool hasAPICHardware() { return _featureFlags.testAny(TR_APICHardware); }
  bool hasMemoryTypeRangeRegisters() {
    return _featureFlags.testAny(TR_MemoryTypeRangeRegisters);
  }
  bool supportsPageGlobalFlag() {
    return _featureFlags.testAny(TR_PageGlobalFlag);
  }
  bool hasMachineCheckArchitecture() {
    return _featureFlags.testAny(TR_MachineCheckArchitecture);
  }
  bool supportsCMOVInstructions() {
    return _featureFlags.testAny(TR_CMOVInstructions);
  }
  bool supportsFCOMIInstructions() {
    return _featureFlags.testAll(TR_BuiltInFPU | TR_CMOVInstructions);
  }
  bool hasPageAttributeTable() {
    return _featureFlags.testAny(TR_PageAttributeTable);
  }
  bool has36BitPageSizeExtension() {
    return _featureFlags.testAny(TR_36BitPageSizeExtension);
  }
  bool hasProcessorSerialNumber() {
    return _featureFlags.testAny(TR_ProcessorSerialNumber);
  }
  bool supportsCLFLUSHInstruction() {
    return _featureFlags.testAny(TR_CLFLUSHInstruction);
  }
  bool supportsDebugTraceStore() {
    return _featureFlags.testAny(TR_DebugTraceStore);
  }
  bool hasACPIRegisters() { return _featureFlags.testAny(TR_ACPIRegisters); }
  bool supportsMMXInstructions() {
    return _featureFlags.testAny(TR_MMXInstructions);
  }
  bool supportsFastFPSavesRestores() {
    return _featureFlags.testAny(TR_FastFPSavesRestores);
  }
  ...
  bool supportsHyperThreading() {
    return _featureFlags.testAny(TR_HyperThreading);
  }
  ...
  bool supportsSFence() {
    return _featureFlags.testAny(TR_SSE | TR_MMXInstructions);
  }
...
public:
  ...
  TR::RegisterIterator *getX87RegisterIterator() {
    return _x87RegisterIterator;
  }
  TR::RegisterIterator *setX87RegisterIterator(TR::RegisterIterator *iter) {
    return (_x87RegisterIterator = iter);
  }
  ...
  // Late edge splitting
  //
  void addDeferredSplit(TR::X86LabelInstruction *branch) {
    _deferredSplits.push_front(branch);
  }
  ...
  TR::LabelSymbol *
  splitLabel(TR::LabelSymbol *targetLabel,
             TR::X86LabelInstruction *instructionToDefer = NULL);
  ...
  bool supportsCMOV() {
    return (_targetProcessorInfo.supportsCMOVInstructions());
  }
  ...
  TR_OutlinedInstructions *
  findOutlinedInstructionsFromLabelForShrinkWrapping(TR::LabelSymbol *label);
...
private:
  ...
  findOrCreateConstant(TR::Node *, void *c, uint8_t size, bool isWarm = 0);
...
public:
  ...
  void setUseGPRsForWin32CTMConversion() {
    _flags.set(UseGPRsForWin32CTMConversion);
  }

  bool useLongDivideHelperForWin32CTMConversion() {
    return _flags.testAny(UseLongDivideHelperForWin32CTMConversion);
  }
  void setUseLongDivideHelperForWin32CTMConversion() {
    _flags.set(UseLongDivideHelperForWin32CTMConversion);
  }
...
#define NEEDS_TRAMPOLINE(target, rip, cg)                                      \
  (cg->alwaysUseTrampolines() || !IS_32BIT_RIP((target), (rip)))
#else
...
public:
  TR_X86FPStackIterator(TR::Machine *machine,
                        TR_RegisterKinds kind = TR_NoRegister)
  ...
  TR::Register *getFirst() {
    return _machine->getFPStackLocationPtr(
        _cursor = TR_X86FPStackRegister::fpFirstStackReg);
  }
  TR::Register *getCurrent() {
    return _machine->getFPStackLocationPtr(_cursor);
  }
  TR::Register *getNext() {
    return _cursor > TR_X86FPStackRegister::fpLastStackReg
               ? NULL
               : _machine->getFPStackLocationPtr(++_cursor);
  }
