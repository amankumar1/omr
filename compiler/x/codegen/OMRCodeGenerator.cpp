...
bool TR_X86ProcessorInfo::isIntelOldMachine() {
  if (isIntelPentium() || isIntelP6() || isIntelPentium4() || isIntelCore2() ||
      isIntelTulsa() || isIntelNehalem())
    return true;
...
void TR_X86ProcessorInfo::initialize(TR::Compilation *comp) {
  ...
  _featureFlags2.set(
      TR::Compiler->target.cpu.getX86ProcessorFeatureFlags2(comp));
  ...
  uint32_t _processorSignature =
      TR::Compiler->target.cpu.getX86ProcessorSignature(comp);
...
void OMR::X86::CodeGenerator::initialize(TR::Compilation *comp) {
  ...
  if (_targetProcessorInfo.supportsSSE2() &&
      TR::Compiler->target.cpu.getX86OSSupportsSSE2(comp))
    supportsSSE2 = true;
#endif // defined(TR_TARGET_X86) && !defined(J9HAMMER)

  if (TR::Compiler->target.cpu.getX86SupportsTM(comp) &&
      !comp->getOption(TR_DisableTM)) {
    if (TR::Compiler->target.is64Bit())
      self()->setSupportsTM(); // disable tm on 32bits for now
  }

  if (!forceX87 && (TR::Compiler->target.is64Bit() ||
#if defined(TR_TARGET_X86) && !defined(J9HAMMER)
                    supportsSSE2 ||
#endif
                    forceSSE2)) {
  ...
  if (vmThreadIndex != TR::RealRegister::NoReg) {
    TR::RealRegister *vmThreadReal =
        self()->machine()->getX86RealRegister(vmThreadIndex);
  ...
  if (comp->getOptions()->getRegisterAssignmentTraceOption(
    ...
    self()->setX87RegisterIterator(
        new (self()->trHeapMemory()) TR_X86FPStackIterator(self()->machine()));
  }
...
OMR::X86::CodeGenerator::CodeGenerator()
    : OMR::CodeGenerator(), _wordConversionTemp(NULL),
      _doubleWordConversionTemp(NULL), _currentTimeMillisTemp(NULL),
      _nanoTimeTemp(NULL), _assignmentDirection(Backward),
      _lastCatchAppendInstruction(NULL), _betterSpillPlacements(NULL),
      _dataSnippetList(
          getTypedAllocator<TR::IA32DataSnippet *>(TR::comp()->allocator())),
      ...
      _outlinedInstructionsList(getTypedAllocator<TR_OutlinedInstructions *>(
          TR::comp()->allocator())),
      _deferredSplits(getTypedAllocator<TR::X86LabelInstruction *>(
          TR::comp()->allocator())),
      _flags(0) {
...
#if defined(TR_TARGET_64BIT)
      linkage =
          new (self()->trHeapMemory()) TR_AMD64Win64FastCallLinkage(self());
#else
      linkage = new (self()->trHeapMemory()) TR_IA32SystemLinkage(self());
#endif
    } else {
...
void OMR::X86::CodeGenerator::beginInstructionSelection() {
  ...
  if (recompilation && recompilation->generatePrePrologue() != NULL) {
    ...
    if (methodSymbol->getLinkageConvention() == TR_Private)
      _returnTypeInfoInstruction =
          (TR::X86ImmInstruction *)comp->getAppendInstruction();
  ...
  TR::RegisterDependencyConditions *deps =
      generateRegisterDependencyConditions((uint8_t)0, (uint8_t)1, self());
  if (_linkageProperties->getMethodMetaDataRegister() !=
  ...
  if (comp->getAppendInstruction())
    ...
    new (self()->trHeapMemory())
        TR::Instruction(deps, PROCENTRY, (TR::Instruction *)NULL, self());
  ...
  if (self()->enableSinglePrecisionMethods() &&
    ...
    TR::IA32ConstantDataSnippet *cds = self()->findOrCreate2ByteConstant(
...
/*
 * this method returns TRUE for all the cases we decide NOT to inline the CAS
 * native
 * it checks the same condition as inlineCompareAndSwapNative in
 * tr.source/trj9/x/codegen/J9TreeEvaluator.cpp
 * so that GRA and Evaluator should be consistent about whether to inline CAS
 * natives
 */
...
// some recognized methods might be transformed into very simple hardcoded
// assembly sequence which doesn't need register spills as a real call does
...
// this to be hoisted within CG,
// this method is placed here as a dummy until platform specific code is removed
// from LocalOpt
int32_t OMR::X86::CodeGenerator::getMaximumNumbersOfAssignableVRs() {
  // TR_ASSERT(false,"getMaximumNumbersOfAssignableVRs should never be
  // called.");
  return INT_MAX;
}
...
void OMR::X86::CodeGenerator::clobberLiveDiscardableRegisters(
  ...
  if (symbol) {
    ...
    // register(s),
    // those registers must be deactivated (marked as non-discardable) after
    // this point.
    // We record which registers are deactivated, so that we can re-activate
    // them when
    ...
    // deactivated
    // registers, it (and those that depend on it if any) must also be
    // deactivated.
    //
...
bool OMR::X86::CodeGenerator::enableAESInHardwareTransformations() {
  if (TR::CodeGenerator::getX86ProcessorInfo().supportsAESNI() &&
      !self()->comp()->getOptions()->getOption(TR_DisableAESInHardware) &&
      !self()->comp()->getCurrentMethod()->isJNINative())
    return true;
  else
    return false;
}
...
bool OMR::X86::CodeGenerator::supportsMergingOfHCRGuards() {
...
TR::SymbolReference *OMR::X86::CodeGenerator::getWordConversionTemp() {
...
TR::SymbolReference *OMR::X86::CodeGenerator::getDoubleWordConversionTemp() {
...
OMR::X86::CodeGenerator::findOrCreateCurrentTimeMillisTempSymRef() {
  if (_currentTimeMillisTemp == NULL) {
    ...
    TR::AutomaticSymbol *sym = TR::AutomaticSymbol::create(
        self()->trHeapMemory(), TR::Aggregate, symSize);
    self()->comp()->getMethodSymbol()->addAutomatic(sym);
    _currentTimeMillisTemp = new (self()->trHeapMemory())
...
void OMR::X86::CodeGenerator::saveBetterSpillPlacements(
  ...
  for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR;
       i++) {
    realReg =
        self()->machine()->getX86RealRegister((TR::RealRegister::RegNum)i);
...
void OMR::X86::CodeGenerator::performNonLinearRegisterAssignmentAtBranch(
  ...
  if (deps) {
    ...
    if (self()->comp()->getOptions()->getOption(
            TR_TraceNonLinearRegisterAssigner)) {
      traceMsg(self()->comp(),
  ...
  // happen
  // when a register is live at the merge point, dies in the mainline path, and
  // is not
  ...
  // virtual if
  // virtual is no longer spilled.  This occurs because the the free spill list
  // was
  // locked.
  //
...
void OMR::X86::CodeGenerator::doBackwardsRegisterAssignment(
  ...
  while (instructionCursor && instructionCursor != appendInstruction) {
    ...
    // Detect BBEnd end of a non-extended block or the last BBEnd end of an
    // extended block
    if (comp->cg()->getSupportsVMThreadGRA() &&
        inst->getKind() == TR::Instruction::IsLabel &&
        vmThreadIndex != TR::RealRegister::NoReg) {
      TR::Node *node = inst->getNode();
      if (node && node->getOpCodeValue() == TR::BBEnd) {
        TR::Block *block = node->getBlock();
        if (block && (!block->getNextBlock() ||
                      !block->getNextBlock()
                           ->isExtensionOfPreviousBlock())) { // Reset vmThread
                                                              // register state.
          TR::RealRegister *vmThreadRealReg =
              self()->machine()->getX86RealRegister(TR::RealRegister::ebp);
...
void OMR::X86::CodeGenerator::doRegisterAssignment(
  ...
  if (kindsToAssign) {
    ...
    self()->doBackwardsRegisterAssignment(
        kindsToAssign, self()->comp()->getAppendInstruction());
  }
}
...
void OMR::X86::CodeGenerator::doBinaryEncoding() {
  ...
  if (vmThreadSpillCursor &&
      self()->getVMThreadRegister()->getBackingStorage()) {
    if (vmThreadSpillCursor == (TR::Instruction *)0xffffffff ||
        self()->comp()->mayHaveLoops()) {
      vmThreadSpillCursor = estimateCursor;
    }

    new (self()->trHeapMemory()) TR::X86MemRegInstruction(
        vmThreadSpillCursor, SMemReg(),
        generateX86MemoryReference(self()
                                       ->getVMThreadRegister()
                                       ->getBackingStorage()
                                       ->getSymbolReference(),
                                   self()),
        self()->machine()->getX86RealRegister(
            _linkageProperties->getMethodMetaDataRegister()),
        self());
  }
...
#define OVER_ESTIMATATION 4
  if (warmEstimate) {
    ...
    self()->setEstimatedColdLength(estimate - warmEstimate -
  ...
  if (TR::Compiler->target.is64Bit() &&
    ...
    TR_ASSERT(numTrampolinesToReserve >= 0,
              ...
              self()->getPicSlotCount(),
              self()->comp()->getNumReservedIPICTrampolines());
    self()->comp()->fe()->reserveNTrampolines(self()->comp(),
                                              numTrampolinesToReserve, true);
  }
  ...
  if ((!self()->comp()->getOptions()->getOption(
      ...
      (self()->comp()->getOptions()->getEnableGPU(TR_EnableGPU) &&
       self()->comp()->hasIntStreamForEach()))
#else
  if (!self()->comp()->getOptions()->getOption(
          TR_DisableGuardedCountingRecompilations) &&
      TR::Options::getCmdLineOptions()->allowRecompilation())
  ...
  while (oiIterator != self()->getOutlinedInstructionsList().end()) {
    ...
    bool needsETE =
        (*oiIterator)->getCallNode() &&
        (*oiIterator)->getCallNode()->getSymbolReference()->canCauseGC();
  ...
  if (self()->comp()->getOption(TR_EnableGCRPatching) &&
    ...
    if (!addrToPatch) {
      TR_ASSERT(false, "Must have updated gcrPatchPointSymbol with the correct "
                       "address by now\n");
      traceMsg(self()->comp(), "Must have updated gcrPatchPointSymbol with the "
                               "correct address by now");
...
TR_OutlinedInstructions *
OMR::X86::CodeGenerator::findOutlinedInstructionsFromLabelForShrinkWrapping(
    TR::LabelSymbol *label) {
  auto oiIterator = self()->getOutlinedInstructionsList().begin();
  while (oiIterator != self()->getOutlinedInstructionsList().end()) {
    if ((*oiIterator)->getEntryLabel() == label ||
        (*oiIterator)->getEntryLabel()->getVMThreadRestoringLabel() == label)
      return *oiIterator;
    ++oiIterator;
  }
...
TR::IA32ConstantDataSnippet *
OMR::X86::CodeGenerator::findOrCreateConstant(TR::Node *n, void *c,
                                              uint8_t size, bool isWarm) {
  // *this    swipeable for debugging purposes
  ...
  for (auto iterator = _dataSnippetList.begin();
    ...
    if ((*iterator)->getKind() == TR::Snippet::IsConstantData &&
        ((TR::IA32ConstantDataSnippet *)(*iterator))->getConstantSize() ==
            size &&
  ...
  cursor = new (self()->trHeapMemory())
      TR::IA32ConstantDataSnippet(self(), n, c, size);
...
TR::IA32DataSnippet *
OMR::X86::CodeGenerator::create2ByteData(TR::Node *n, int16_t c, bool isWarm) {
  TR::IA32DataSnippet *cursor =
      new (self()->trHeapMemory()) TR::IA32DataSnippet(self(), n, &c, 2);
  _dataSnippetList.push_front(cursor);
  return cursor;
}

TR::IA32DataSnippet *
OMR::X86::CodeGenerator::create4ByteData(TR::Node *n, int32_t c, bool isWarm) {
  TR::IA32DataSnippet *cursor =
      new (self()->trHeapMemory()) TR::IA32DataSnippet(self(), n, &c, 4);
  _dataSnippetList.push_front(cursor);
  return cursor;
}

TR::IA32DataSnippet *
OMR::X86::CodeGenerator::create8ByteData(TR::Node *n, int64_t c, bool isWarm) {
  TR::IA32DataSnippet *cursor =
      new (self()->trHeapMemory()) TR::IA32DataSnippet(self(), n, &c, 8);
  _dataSnippetList.push_front(cursor);
  return cursor;
}

TR::IA32DataSnippet *
OMR::X86::CodeGenerator::create16ByteData(TR::Node *n, void *c, bool isWarm) {
  TR::IA32DataSnippet *cursor =
      new (self()->trHeapMemory()) TR::IA32DataSnippet(self(), n, c, 16);
  _dataSnippetList.push_front(cursor);
  return cursor;
}
...
bool OMR::X86::CodeGenerator::processInstruction(
    TR::Instruction *instr, TR_BitVector **registerUsageInfo, int32_t &blockNum,
  ...
  if (x86Instr->getOpCode().isCallOp()) {
    ...
    if (callNode->getOpCode().hasSymbolReference()) // getSymbolReference())
    {
      ...
      if (x86Instr->getKind() == TR::Instruction::IsImmSym) {
        ...
        if (traceIt)
          traceMsg(self()->comp(),
                   "call instr (ImmSym) %p has method symbol %p\n", x86Instr,
                   m);
      } else {
      ...
      if (m && ((m->isHelper() && !m->preservesAllRegisters()) ||
                m->isVMInternalNative() || m->isJITInternalNative() ||
                m->isJNI() || m->isSystemLinkageDispatch() ||
      ...
      if (!callPreservesRegisters) {
        if (traceIt)
          ////traceMsg(comp(), "call instr [%p] does not preserve regs really
  ...
  case TR::Instruction::IsFPReg: {
    ...
    //
    int32_t tgtRegNum = ((TR::RealRegister *)x86Instr->getTargetRegister())
                            ->getRegisterNumber();
    if (traceIt)
  ...
  case TR::Instruction::IsFPRemainderRegReg: {
    ...
    int32_t tgtRegNum = ((TR::RealRegister *)x86Instr->getTargetRegister())
                            ->getRegisterNumber();
    int32_t srcRegNum = ((TR::RealRegister *)x86Instr->getSourceRegister())
                            ->getRegisterNumber();
    if (traceIt)
      traceMsg(self()->comp(),
               "instr [%p] USES register [%d] ; register [%d]\n", x86Instr,
               tgtRegNum, srcRegNum);
    registerUsageInfo[blockNum]->set(tgtRegNum);
  ...
  case TR::Instruction::IsMemImmSnippet: {
    ...
    if (TR::Compiler->target.is64Bit() && mr->getAddressRegister()) {
      if (traceIt)
        traceMsg(self()->comp(), "instr [%p] USES mr addressRegister [%d]\n",
                 x86Instr, ((TR::RealRegister *)mr->getAddressRegister())
                               ->getRegisterNumber());
      registerUsageInfo[blockNum]->set(
          ((TR::RealRegister *)mr->getAddressRegister())->getRegisterNumber());
    } else if (traceIt)
      traceMsg(self()->comp(), "instr [%p] USES mr no addressRegister\n",
               x86Instr);
  ...
  case TR::Instruction::IsMemRegReg: {
    // get sourceRegister, sourceRightRegister and memoryReference:
    ...
    int32_t srcRightRegNum =
        ((TR::RealRegister *)x86Instr->getSourceRightRegister())
            ->getRegisterNumber();
    if (traceIt)
      traceMsg(self()->comp(), "instr [%p] USES register [%d]\n", x86Instr,
               srcRightRegNum);
...
uint32_t OMR::X86::CodeGenerator::isPreservedRegister(int32_t regIndex) {
  ...
  int32_t numPreserved =
      self()->getProperties().getMaxRegistersPreservedInPrologue();
  for (int32_t pindex = numPreserved - 1; pindex >= 0; pindex--) {
    if (self()->getProperties().getPreservedRegister((uint32_t)pindex) ==
...
//
TR::Instruction *
OMR::X86::CodeGenerator::splitBlockEntry(TR::Instruction *instr) {
  ...
  if (instr->getKind() == TR::Instruction::IsLabel) {
    TR::LabelSymbol *label =
        ((TR::X86LabelInstruction *)instr)->getLabelSymbol();
...
TR::Instruction *OMR::X86::CodeGenerator::splitEdge(
    TR::Instruction *instr, bool isFallThrough, bool needsJump,
    TR::Instruction *newSplitLabel, TR::list<TR::Instruction *> *jmpInstrs,
    bool firstJump) {
  ...
  if (!cursor)
    cursor = generateLabelInstruction(location, LABEL, newLabel, self());
...
bool OMR::X86::CodeGenerator::isTargetSnippetOrOutOfLine(
    TR::Instruction *instr, TR::Instruction **start, TR::Instruction **end) {
  TR::X86LabelInstruction *labelInstr = (TR::X86LabelInstruction *)instr;
  TR::LabelSymbol *targetLabel = labelInstr->getLabelSymbol();
  TR_OutlinedInstructions *oiCursor =
      self()->findOutlinedInstructionsFromLabelForShrinkWrapping(targetLabel);
  if (oiCursor) {
...
void OMR::X86::CodeGenerator::updateSnippetMapWithRSD(TR::Instruction *instr,
                                                      int32_t rsd) {
...
int32_t
OMR::X86::CodeGenerator::computeRegisterSaveDescription(TR_BitVector *regs,
                                                        bool populateInfo) {
  uint32_t rsd = 0;
  ...
  while (regIt.hasMoreElements()) {
    ...
    if (regIndex != -1) {
      TR::RealRegister::RegNum idx =
          self()->getProperties().getPreservedRegister((uint32_t)regIndex);
      /// traceMsg(comp(), "regIndex %d idx %d\n", regIndex, idx);
      TR::RealRegister *reg =
          self()->machine()->getX86RealRegister((TR::RealRegister::RegNum)idx);
...
void OMR::X86::CodeGenerator::processIncomingParameterUsage(
...
// movInstruction's immediate field
// deltaToStartLabel is the byte distance from the start of the instruction
// following the mov to the startLabel address
...
int32_t OMR::X86::CodeGenerator::branchDisplacementToHelperOrTrampoline(
  ...
  if (NEEDS_TRAMPOLINE(helperAddress, nextInstructionAddress, self())) {
    helperAddress = self()->fe()->indexedTrampolineLookup(
        helper->getReferenceNumber(), (void *)(nextInstructionAddress - 4));
    TR_ASSERT(IS_32BIT_RIP(helperAddress, nextInstructionAddress),
...
OMR::X86::CodeGenerator::pickNOPRegister(TR::Instruction *successor) {
  ...
  TR::RealRegister *ebx =
      self()->machine()->getX86RealRegister(TR::RealRegister::ebx);
  TR::RealRegister *esi =
      self()->machine()->getX86RealRegister(TR::RealRegister::esi);
  TR::RealRegister *edi =
      self()->machine()->getX86RealRegister(TR::RealRegister::edi);
...
OMR::X86::CodeGenerator::nodeResultGPRCount(TR::Node *node,
  ...
  //
  if (node->getOpCode().isLoadConst() && node->getSize() <= 4 &&
...
bool OMR::X86::CodeGenerator::suppressInliningOfRecognizedMethod(
...
void OMR::X86::CodeGenerator::performDeferredSplits() {
  ...
  for (auto li = _deferredSplits.begin(); li != _deferredSplits.end(); ++li) {
    TR::LabelSymbol *newLabelSymbol =
        self()->splitLabel((*li)->getLabelSymbol());
    if (self()->getTraceRAOption(TR_TraceRALateEdgeSplitting))
      traceMsg(self()->comp(), "LATE EDGE SPLITTING: Pointed branch %s at "
                               "vmThread-restoring label %s\n",
...
void OMR::X86::CodeGenerator::processDeferredSplits(bool clear) {
  if (clear)
...
TR::LabelSymbol *OMR::X86::CodeGenerator::splitLabel(
    TR::LabelSymbol *targetLabel, TR::X86LabelInstruction *instructionToDefer) {
  ...
  TR::RealRegister *ebp = self()->machine()->getX86RealRegister(
      self()->getProperties().getMethodMetaDataRegister());
  if (instructionToDefer && !ebp->getAssignedRegister() &&
      performTransformation(
          self()->comp(),
          "O^O LATE EDGE SPLITTING: Defer splitting %s for %s\n",
          self()->getDebug()->getName(targetLabel),
          self()->getDebug()->getName(instructionToDefer))) {
    TR_ASSERT(instructionToDefer->getOpCode().isBranchOp() &&
                  instructionToDefer->getLabelSymbol() == targetLabel,
  ...
  if (!targetLabel->getVMThreadRestoringLabel()) {
    ...
    newLabel->setInstruction(generateLabelInstruction(
        targetLabel->getInstruction()->getPrev(), LABEL, newLabel, self()));
    self()->generateDebugCounter(targetLabel->getInstruction(),
                                 "cg.lateSplitEdges", 1,
                                 TR::DebugCounter::Exorbitant);
    if (self()->getTraceRAOption(TR_TraceRALateEdgeSplitting))
      traceMsg(self()->comp(), "LATE EDGE SPLITTING: Inserted "
                               "vmThread-restoring label %s before %s\n",
               self()->getDebug()->getName(newLabel),
               self()->getDebug()->getName(targetLabel));
  }

  // Conservatively store ebp in the prologue just in case any of these split
  // labels decide they need to load it
  // That decision will occur at binary encoding time, at which point it's too
  ...
  if (instructionCursor->totalReferencedGPRegisters(self()) > 0) {
    ...
    if (debug("dumpGPRegStatus")) {
      self()->machine()->printGPRegisterStatus(
          self()->fe(), self()->machine()->getRegisterFile(),
          self()->comp()->getOutFile());
    }
  }
}
