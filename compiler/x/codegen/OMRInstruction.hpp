...
public:
  ...
  enum TR_UpperHalfRefConditions {
    ...
    // Some other conditions under which the top half of a register might be
    // used
    TR_if64bitSource,
    TR_ifUses64bitTarget,
    TR_ifUses64bitSourceOrTarget, // A conservative default for when to
                                  // setIsUpperHalfDead(true).  Shouldn't really
                                  // be needed.
  ...
  void setIsUpperHalfDead(TR::Register *reg, bool value,
                          TR_UpperHalfRefConditions when = TR_always);
  void aboutToAssignUsedRegister(
      TR::Register *reg,
      TR_UpperHalfRefConditions usesUpperHalf = TR_ifUses64bitSourceOrTarget);
  void
  aboutToAssignDefdRegister(TR::Register *reg,
                            TR_UpperHalfRefConditions defsUpperHalf = TR_never);

  void aboutToAssignRegister(
      TR::Register *reg,
      TR_UpperHalfRefConditions usesUpperHalf = TR_ifUses64bitSourceOrTarget,
      TR_UpperHalfRefConditions defsUpperHalf = TR_never);
