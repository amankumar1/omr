    ...
    for (candidate = 0; candidate < cursor; candidate++) {
      ...
      if (dep->getRealRegister() == vmThreadRealRegisterIndex) {
        ...
        if (dep->getRegister() == vmThreadRegister) {
          ...
          // reg %s\n      {\"%s\"}",
          //   getDebug()->getName(dep->getRegister()),getDebug()->getName(machine->getX86RealRegister(rr)),getDebug()->getName(vr),
          ...
          // {%s}}\n",
          //   getDebug()->getName(vr),getDebug()->getName(machine->getX86RealRegister(rr)),getDebug()->getName(dep->getRegister()),
          //   cg->comp()->getCurrentMethod()->signature());
...
void TR_X86RegisterDependencyGroup::unblockRealDependencyRegisters(
  ...
  for (TR_X86RegisterDependencyIndex i = 0; i < numberOfRegisters; i++) {
    if (_dependencies[i].getRealRegister() != TR::RealRegister::NoReg) {
      machine->getX86RealRegister(_dependencies[i].getRealRegister())
          ->unblock();
    }
  }
}
...
void TR_X86RegisterDependencyGroup::assignRegisters(
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    TR::RealRegister *vmThreadRealReg =
        machine->getX86RealRegister(TR::RealRegister::ebp);
    if (cg->getSupportsVMThreadGRA() &&
        dependentRegNum == TR::RealRegister::ebp && virtReg &&
        virtReg != vmThreadReg &&
      ...
      if (debug("vmThreadGRA"))
        cg->traceRegisterAssignment("Force spill of vmThread at inst %p",
                                    currentInstruction);
      TR::MemoryReference *tempMR =
          generateX86MemoryReference(vmThreadSill->getSymbolReference(), cg);
      TR::X86RegMemInstruction *inst = generateRegMemInstruction(
          currentInstruction, LRegMem(), vmThreadRealReg, tempMR, cg);
  ...
  // previous
  // loop, but that could lead to some naive changes that would have been easier
  // once
...
void TR_X86RegisterDependencyGroup::assignFPRegisters(
  ...
  if (numberOfRegisters > 0) {
    ...
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
        if (virtReg->getTotalUseCount() != virtReg->getFutureUseCount()) {
          ...
          // caller
          // side return value.  Assume it already exists on the FP stack if not
          // a global
          // FP register; else coerce it to the right stack location. The
          // required stack
    ...
    // certain cases;
    // in particular if there are 2 global registers having the same value (same
    // child C
    // in the GLRegDeps on a branch) and one of them is never used in the
    // extended block after
    // the branch; then only one value will be popped off the stack when the
    // last reference
    // to child C is seen in the extended basic block. This case cannot be
    // detected during
...
// happen
// if there are N/2 pairwise incorrect global registers. This could possibly be
// done in
// N+1 approx maybe (?)
//
...
uint32_t OMR::X86::RegisterDependencyConditions::numReferencedFPRegisters(
  ...
  for (i = 0; i < _numPreConditions; i++) {
    ...
    if ((reg && reg->getKind() == TR_X87) ||
        (!reg &&
         _preConditions->getRegisterDependency(i)->getRealRegister() ==
             TR::RealRegister::AllFPRegisters)) {
      total++;
    }
  }
  ...
  for (i = 0; i < _numPostConditions; i++) {
    ...
    if ((reg && reg->getKind() == TR_X87) ||
        (!reg &&
         _postConditions->getRegisterDependency(i)->getRealRegister() ==
             TR::RealRegister::AllFPRegisters)) {
      total++;
    }
  }
