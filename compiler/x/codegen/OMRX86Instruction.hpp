...
public:
  ...
  // reason is
  // that we don't want the inherited logic to stop silently when it reaches
  // this limit;
  // rather, we want it to try to add more padding, and trip on an assertion
  // inside
  ...
  static TR_AtomicRegion CALLImm4AtomicRegions[]; // For patching the
                                                  // displacement of a 5-byte
                                                  // call instruction
};
...
public:
  X86LabelInstruction(TR::LabelSymbol *sym, TR::Node *node, TR_X86OpCodes op,
  ...
  X86LabelInstruction(TR::LabelSymbol *sym, TR_X86OpCodes op,
...
public:
  X86RestoreVMThreadInstruction(TR_X86OpCodes op, TR::Node *n,
                                TR::CodeGenerator *cg);
...
protected:
  void aboutToAssignTargetRegister() {
    aboutToAssignRegister(getTargetRegister(), TR_ifUses64bitTarget,
...
protected:
  void aboutToAssignSourceRegister() {
    aboutToAssignRegister(getSourceRegister(), TR_if64bitSource,
...
public:
  ...
  X86RegRegRegInstruction(TR::RegisterDependencyConditions *cond,
                          ...
                          TR::Instruction *precedingInstruction,
                          TR::CodeGenerator *cg)
      : TR::X86RegRegInstruction(cond, slreg, treg, op, precedingInstruction,
                                 cg),
        _sourceRightRegister(srreg) {
    useRegister(srreg);
  }
  ...
  virtual TR::Register *getSourceRightRegister() {
    return _sourceRightRegister;
  }
  TR::Register *setSourceRightRegister(TR::Register *srr) {
    return (_sourceRightRegister = srr);
  }
...
protected:
  void aboutToAssignSourceRightRegister() {
    aboutToAssignRegister(getSourceRightRegister(), TR_if64bitSource, TR_never);
  }
};
...
public:
  X86MemTableInstruction(TR_X86OpCodes op, TR::Node *node,
    ...
    _relocations = (TR_LabelRelocation **)cg->trMemory()->allocateHeapMemory(
  ...
  void addRelocation(TR_LabelRelocation *r) {
    TR_ASSERT(_numRelocations < _capacity, "Can't add another relocation to a "
                                           "X86MemTableInstruction that is "
                                           "already full");
    _relocations[_numRelocations++] = r;
  }

  TR_LabelRelocation *getRelocation(ncount_t index) {
    TR_ASSERT(0 <= index && index < _numRelocations, "X86MemTableInstruction::"
                                                     "getRelocation - index %d "
                                                     "is out of range (0-%d)",
...
public:
  X86MemRegRegInstruction(TR::Register *srreg, TR::Register *slreg,
                          TR::MemoryReference *mr, TR::Node *node,
                          TR_X86OpCodes op, TR::CodeGenerator *cg)
  ...
  X86MemRegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                          TR::MemoryReference *mr, TR::Register *slreg,
...
public:
  ...
  AMD64Imm64SymInstruction(TR::Instruction *precedingInstruction,
      ...
      : TR::AMD64Imm64Instruction(imm, op, precedingInstruction, cg),
        _symbolReference(sr), _thunkAddress(NULL) {}
...
class X86VFPDedicateInstruction : public TR::X86RegMemInstruction {
  ...
  TR::MemoryReference *memref(TR::CodeGenerator *cg) {
    ...
    return generateX86MemoryReference(
        machine->getX86RealRegister(TR::RealRegister::vfp), 0, cg);
  }
...
generateRestoreVMThreadInstruction(TR::Node *, TR::CodeGenerator *);
...
TR::X86LabelInstruction *generateLabelInstruction(TR::Instruction *,
                                                  TR_X86OpCodes op,
                                                  TR::LabelSymbol *sym,
                                                  bool needsVMThreadRegister,
                                                  TR::CodeGenerator *cg);
inline TR::X86LabelInstruction *
generateLabelInstruction(TR::Instruction *i, TR_X86OpCodes op,
                         TR::LabelSymbol *sym, TR::CodeGenerator *cg) {
...
generateLabelInstruction(TR_X86OpCodes op, TR::Node *, TR::LabelSymbol *sym,
                         ...
                         bool needsVMThreadRegister, bool evaluateGlRegDeps,
                         TR::CodeGenerator *cg);
...
generateLabelInstruction(TR_X86OpCodes op, TR::Node *node, TR::LabelSymbol *sym,
                         TR::Node *glRegDep, List<TR::Register> *popRegs,
                         bool needsVMThreadRegister, TR::CodeGenerator *cg) {
  return generateLabelInstruction(op, node, sym, glRegDep, popRegs,
                                  needsVMThreadRegister, true, cg);
}
...
generateLabelInstruction(TR_X86OpCodes op, TR::Node *node, TR::LabelSymbol *sym,
                         TR::Node *glRegDep, List<TR::Register> *popRegs,
                         TR::CodeGenerator *cg) {
  return generateLabelInstruction(op, node, sym, glRegDep, popRegs, false, true,
                                  cg);
}
