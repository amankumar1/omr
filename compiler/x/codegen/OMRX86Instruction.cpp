...
TR::RealRegister *assignGPRegister(TR::Instruction *instr,
  ...
  if (assignedRegister->getRegisterNumber() ==
      cg->getProperties().getMethodMetaDataRegister())
...
TR::X86LabelInstruction::X86LabelInstruction(TR_X86OpCodes op, TR::Node *node,
    ...
    : TR::Instruction(node, op, cg), _symbol(sym), _needToClearFPStack(b),
      _outlinedInstructionBranch(NULL), _reloType(TR_NoRelocation),
...
TR::X86LabelInstruction::X86LabelInstruction(
    ...
    : TR::Instruction(op, precedingInstruction, cg), _symbol(sym),
      _needToClearFPStack(b), _outlinedInstructionBranch(NULL),
      _reloType(TR_NoRelocation), _permitShortening(true) {
  // *this    swipeable for debugging purposes
  if (sym && op == LABEL)
    sym->setInstruction(this);
  else if (sym)
...
TR::X86LabelInstruction::X86LabelInstruction(
    ...
    : TR::Instruction(cond, node, op, cg), _symbol(sym), _needToClearFPStack(b),
      _outlinedInstructionBranch(NULL), _reloType(TR_NoRelocation),
      _permitShortening(true) {
  // *this    swipeable for debugging purposes
  if (sym && op == LABEL)
...
TR::X86LabelInstruction::X86LabelInstruction(
    ...
    TR::CodeGenerator *cg, bool b)
    : TR::Instruction(cond, op, precedingInstruction, cg), _symbol(sym),
      _needToClearFPStack(b), _outlinedInstructionBranch(NULL),
      _reloType(TR_NoRelocation), _permitShortening(true) {
  // *this    swipeable for debugging purposes
  if (sym && op == LABEL)
...
void TR::X86LabelInstruction::assignOutlinedInstructions(
  ...
  TR_ASSERT(oi, "Could not find OutlinedInstructions stream from label.  "
                "instr=%p, label=%p\n",
...
void TR::X86LabelInstruction::assignRegisters(
  ...
  if (kindsToBeAssigned & TR_GPR_Mask) {
    if (getDependencyConditions()) {
      ...
      // register
      // contains the class object reference.  This must be used by the snippet
      // to generate
        ...
        if (classObjectRegister != NULL &&
          ...
          if (assignedClassObjectRegister == NULL &&
            ...
            if ((assignedClassObjectRegister = machine->findBestFreeGPRegister(
              ...
              // so that it doesn't look like the first time a register is seen.
              // That is
              // make it fail the getTotalUseCount() == getFutureUseCount() test
              // used
              // to detect the first reference to a register in the assignment
              // direction
              //
    ...
    if (!comp->getOption(TR_DisableLateEdgeSplitting) &&
      ...
      // 1. Identify block exits where the vmthread is not in ebp.  At that
      // point,
      // we'll split the edge and add a second label.
      // 2. Identify labels where the vmthread must be in ebp.  Mark such labels
      // with IsVMThreadLive.
      // 3. (After register assignment, during size estimation.)  Find labels
      // that
      // have been split AND have IsVMThreadLive.  Insert an ebp load between
      // the labels.

      TR::RealRegister *ebp = cg()->machine()->getX86RealRegister(
          cg()->getProperties().getMethodMetaDataRegister());
      bool ebpIsVMThreadRegister =
          ebp->getAssignedRegister() == cg()->getVMThreadRegister();
      ...
      if (getOpCodeValue() == LABEL && ebpIsVMThreadRegister) {
      ...
      } else if (getNode()->getOpCodeValue() == TR::BBStart &&
                 !getNode()->getBlock()->isExtensionOfPreviousBlock() &&
                 cg()->hasDeferredSplits()) {
        ...
        if (cg()->getTraceRAOption(TR_TraceRALateEdgeSplitting))
          traceMsg(comp, "O^O LATE EDGE SPLITTING: BBStart label %s needs "
                         "vmthread in ebp because of deferred splits\n",
        ...
        if (getNode()->getOpCodeValue() == TR::BBEnd) {
          ...
          if (nextBlock && getNode()->getBlock()->hasSuccessor(nextBlock) &&
              !nextBlock->isExtensionOfPreviousBlock()) {
          ...
          if (_symbol->getInstruction() &&
              !cg()->findOutlinedInstructionsFromLabel(_symbol)) {
            TR::LabelSymbol *newLabelSymbol = cg()->splitLabel(_symbol, this);

            if (cg()->getTraceRAOption(TR_TraceRALateEdgeSplitting) &&
                newLabelSymbol != _symbol)
              traceMsg(comp, "O^O LATE EDGE SPLITTING: Pointed branch %s at "
...
TR::X86RestoreVMThreadInstruction::X86RestoreVMThreadInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::CodeGenerator *cg)
...
void TR::X86RegRegInstruction::assignRegisters(
  ...
  if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
    ...
    TR_RegisterSizes firstRequestedRegSize =
        getOpCode().hasByteTarget() ? TR_ByteReg : getOpCode().hasXMMTarget()
                                                       ? TR_QuadWordReg
                                                       : TR_WordReg;

    TR_RegisterSizes secondRequestedRegSize =
        getOpCode().hasByteSource() ? TR_ByteReg : getOpCode().hasXMMSource()
                                                       ? TR_QuadWordReg
                                                       : TR_WordReg;
    ...
    if (feGetEnv("TR_UseOutOfDatedRegRevMoveList")) {
      if (opCode == MOVAPSRegReg || opCode == MOV8RegReg ||
          opCode == MOV4RegReg || opCode == MOV2RegReg ||
          opCode == MOV1RegReg) {
        regRegCopy = true;
      } else {
        regRegCopy = false;
    ...
    if (assignedSecondRegister == NULL) {
      ...
      if (toRealRegister(assignedSecondRegister)->getRegisterNumber() ==
          cg()->getProperties().getMethodMetaDataRegister())
        cg()->processDeferredSplits(secondRegister ==
...
TR::X86RegRegRegInstruction::X86RegRegRegInstruction(
    ...
    : TR::X86RegRegInstruction(slreg, treg, node, op, cg),
...
TR::X86RegRegRegInstruction::X86RegRegRegInstruction(
    ...
    TR::CodeGenerator *cg)
    : TR::X86RegRegInstruction(cond, slreg, treg, node, op, cg),
      _sourceRightRegister(srreg) {
  // *this    swipeable for debugging purposes
  useRegister(srreg);
}
...
bool TR::X86RegRegRegInstruction::usesRegister(TR::Register *reg) {
  // *this    swipeable for debugging purposes
  if ((reg == getTargetRegister() && getOpCode().usesTarget()) ||
      reg == getSourceRegister() || reg == getSourceRightRegister()) {
    return true;
  } else if (getDependencyConditions()) {
    return getDependencyConditions()->usesRegister(reg);
  }
...
void TR::X86RegRegRegInstruction::assignRegisters(
  ...
  if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
    ...
    aboutToAssignRegister(firstRegister, TR_ifUses64bitTarget,
                          TR_ifModifies32or64bitTarget);
    aboutToAssignRegister(secondRegister, TR_if64bitSource,
                          TR_ifModifies32or64bitSource);
    aboutToAssignRegister(thirdRegister, TR_if64bitSource,
                          TR_ifModifies32or64bitSource);
...
void insertUnresolvedReferenceInstructionMemoryBarrier(
  ...
  if (is5ByteFence) {
    ...
    TR::RealRegister *espReal =
        cg->machine()->getX86RealRegister(TR::RealRegister::esp);
    ...
    if (!ebpIsVMThreadRegister) {
      ...
      for (ncount_t i = 0; i < getNumRelocations(); i++) {
        ...
        if (targetLabel->getInstruction()) {
          ...
          if (cg()->getTraceRAOption(TR_TraceRALateEdgeSplitting))
            traceMsg(cg()->comp(), "O^O LATE EDGE SPLITTING: Pointed jump "
                                   "table entry %d of %s at vmThread-restoring "
                                   "label %s\n",
                     i, cg()->getDebug()->getName(this),
  ...
  if ((cg()->getAssignmentDirection() == cg()->Backward)) {
    ...
    if (kindsToBeAssigned & getSourceRegister()->getKindAsMask()) {
      ...
      if ((getMemoryReference()->getBaseRegister() ==
        ...
        blockedEbp = true;
        oldState = cg()->machine()
                       ->getX86RealRegister(TR::RealRegister::ebp)
                       ->getState();
        cg()->machine()
            ->getX86RealRegister(TR::RealRegister::ebp)
      ...
      // reset
      // its rematerializability before we allocate registers for this
      // instruction.
      //
...
TR::X86MemRegRegInstruction::X86MemRegRegInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr,
    TR::Register *slreg, TR::Register *srreg, TR::CodeGenerator *cg)
    : TR::X86MemRegInstruction(slreg, mr, node, op, cg),
      _sourceRightRegister(srreg) {
  // *this    swipeable for debugging purposes
  useRegister(srreg);
}
...
TR::X86MemRegRegInstruction::X86MemRegRegInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op,
    TR::MemoryReference *mr, TR::Register *slreg, TR::Register *srreg,
    TR::CodeGenerator *cg)
    : TR::X86MemRegInstruction(slreg, mr, op, precedingInstruction, cg),
      _sourceRightRegister(srreg) {
  // *this    swipeable for debugging purposes
  useRegister(srreg);
}
...
TR::X86MemRegRegInstruction::X86MemRegRegInstruction(
    ...
    TR::Register *slreg, TR::Register *srreg,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
    : TR::X86MemRegInstruction(cond, slreg, mr, node, op, cg),
      _sourceRightRegister(srreg) {
  // *this    swipeable for debugging purposes
  useRegister(srreg);
}
...
TR::X86MemRegRegInstruction::X86MemRegRegInstruction(
    ...
    TR::MemoryReference *mr, TR::Register *slreg, TR::Register *srreg,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
    : TR::X86MemRegInstruction(cond, slreg, mr, op, precedingInstruction, cg),
      _sourceRightRegister(srreg) {
  // *this    swipeable for debugging purposes
  useRegister(srreg);
}
...
void TR::X86MemRegRegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  aboutToAssignRegDeps();
  aboutToAssignSourceRegister();
  aboutToAssignSourceRightRegister();
  ...
  if ((cg()->getAssignmentDirection() == cg()->Backward)) {
    ...
    if (kindsToBeAssigned & getSourceRegister()->getKindAsMask()) {
      ...
      assignedSourceRightRegister =
          _sourceRightRegister->getAssignedRealRegister();
      if (assignedSourceRightRegister == NULL) {
        assignedSourceRightRegister =
            assignGPRegister(this, _sourceRightRegister, TR_WordReg, cg());
      }
      ...
      if (assignedSourceRegister != NULL) {
        ...
        if (_sourceRightRegister->decFutureUseCount() == 0 &&
            assignedSourceRightRegister->getState() !=
                TR::RealRegister::Locked) {
          cg()->traceRegFreed(_sourceRightRegister,
                              assignedSourceRightRegister);
...
void TR::X86FPST0ST1RegRegInstruction::assignRegisters(
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    TR_ASSERT(result & kSourceOnFPStack, "TR::X86FPST0STiRegRegInstruction::"
                                         "assignRegisters ==> source not on FP "
                                         "stack!");

    TR_ASSERT(result & kTargetOnFPStack, "TR::X86FPST0STiRegRegInstruction::"
                                         "assignRegisters ==> target not on FP "
                                         "stack!");
...
void TR::X86FPSTiST0RegRegInstruction::assignRegisters(
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    TR_ASSERT(result & kSourceOnFPStack, "TR::X86FPSTiST0RegRegInstruction::"
                                         "assignRegisters ==> source not on FP "
                                         "stack!");

    TR_ASSERT(result & kTargetOnFPStack, "TR::X86FPSTiST0RegRegInstruction::"
                                         "assignRegisters ==> target not on FP "
                                         "stack!");
...
void TR::X86FPST0STiRegRegInstruction::assignRegisters(
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    if (!(result & kTargetOnFPStack)) {
      ...
      // stack
      // here because it only appears on the stack after the instruction
      // executes.
      //
      lateTargetPush = true;
      sourceRegister->block();
...
void TR::X86FPArithmeticRegRegInstruction::assignRegisters(
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    TR_ASSERT(result & kSourceOnFPStack, "TR::X86FPArithmeticRegRegInstruction:"
                                         ":assignRegisters ==> source not on "
                                         "FP stack!");

    TR_ASSERT(result & kTargetOnFPStack, "TR::X86FPArithmeticRegRegInstruction:"
                                         ":assignRegisters ==> target not on "
                                         "FP stack!");
...
void TR::X86FPCompareRegRegInstruction::assignRegisters(
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    TR_ASSERT(result & kSourceOnFPStack, "TR::X86FPCompareRegRegInstruction::"
                                         "assignRegisters ==> source not on FP "
                                         "stack!");

    TR_ASSERT(result & kTargetOnFPStack, "TR::X86FPCompareRegRegInstruction::"
                                         "assignRegisters ==> target not on FP "
                                         "stack!");
...
void TR::X86FPCompareEvalInstruction::assignRegisters(
  ...
  if (kindsToBeAssigned & TR_GPR_Mask) {
    TR_ASSERT(_accRegister != NULL, "TR::X86FPCompareEvalInstruction::"
                                    "assignRegisters() ==> _accRegister is "
                                    "NULL\n");

    // eax should have been assigned to _accRegister at this point.
    //
    TR::Register *accRegister = _accRegister->getAssignedRegister();
    TR_ASSERT(accRegister != NULL, "TR::X86FPCompareEvalInstruction::"
                                   "assignRegisters() ==> _accRegister has not "
                                   "been assigned!\n");
...
void TR::X86FPRemainderRegRegInstruction::assignRegisters(
  ...
  if (kindsToBeAssigned & TR_GPR_Mask) // TODO: Move this code generation in
                                       // FPTreeEvaluator.cpp rather than doing
                                       // it here
  {
    OMR::X86::Instruction::assignRegisters(kindsToBeAssigned);
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    if (targetRegister->decFutureUseCount() == 0) {
      if (getOpCodeValue() == FLDRegMem || getOpCodeValue() == DLDRegMem) {
        ...
        // can happen after
        // the optimizer runs and eliminates unnecessary stores of unresolved
        // data.
        //
        new (cg()->trHeapMemory())
...
generateLabelInstruction(TR_X86OpCodes op, TR::Node *node, TR::LabelSymbol *sym,
                         bool needsVMThreadRegister, TR::CodeGenerator *cg) {
  TR::X86LabelInstruction *instr;
  if (needsVMThreadRegister &&
      cg->getLinkage()->getProperties().getMethodMetaDataRegister() !=
    ...
    cond->addPreCondition(
        vmThreadRegister,
        (TR::RealRegister::RegNum)vmThreadRegister->getAssociation(), cg);
...
TR::X86LabelInstruction *generateLabelInstruction(TR::Instruction *pred,
  ...
  if (needsVMThreadRegister &&
    ...
    cond->addPostCondition(
        ...
        (TR::RealRegister::RegNum)vmThreadRegister->getAssociation(), cg);
    instr = new (cg->trHeapMemory())
        TR::X86LabelInstruction(pred, op, sym, cond, cg);
  } else {
...
TR::X86LabelInstruction *
generateLongLabelInstruction(TR_X86OpCodes op, TR::Node *node,
                             TR::LabelSymbol *sym, bool needsVMThreadRegister,
                             TR::CodeGenerator *cg) {
  TR::X86LabelInstruction *instr;
  if (needsVMThreadRegister &&
      cg->getLinkage()->getProperties().getMethodMetaDataRegister() !=
...
TR::X86LabelInstruction *generateLabelInstruction(
    ...
    bool needsVMThreadRegister, bool evaluateGlRegDeps, TR::CodeGenerator *cg) {
  TR::X86LabelInstruction *instr;
  ...
  if (needsVMThreadRegister &&
      cg->getLinkage()->getProperties().getMethodMetaDataRegister() !=
          TR::RealRegister::NoReg) {
    TR::RegisterDependencyConditions *deps =
        generateRegisterDependencyConditions(glRegDep, cg, 1, popRegisters);
    TR::Register *vmThreadRegister = cg->getVMThreadRegister();
    ...
    deps->stopAddingConditions();
    instr = new (cg->trHeapMemory())
        TR::X86LabelInstruction(opCode, node, label, deps, cg);
  } else {
    instr = generateLabelInstruction(
        opCode, node, label,
...
generateRestoreVMThreadInstruction(TR::Node *node, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RestoreVMThreadInstruction(RestoreVMThread, node, cg);
}
...
TR::X86LabelInstruction *generateJumpInstruction(TR_X86OpCodes opCode,
  ...
  (jumpNode->getNumChildren() > 0)
      ? inst = generateLabelInstruction(
            opCode, jumpNode, destinationLabel, jumpNode->getFirstChild(), 0,
            needsVMThreadRegister, evaluateGlRegDeps, cg)
      : inst = generateLabelInstruction(opCode, jumpNode, destinationLabel,
                                        needsVMThreadRegister, cg);
...
generateConditionalJumpInstruction(TR_X86OpCodes opCode, TR::Node *ifNode,
  ...
  if (ifNode->getNumChildren() == 3) {
    ...
    inst = generateLabelInstruction(opCode, ifNode, destinationLabel, glRegDep,
                                    &popRegisters, needsVMThreadRegister, cg);
    ...
    inst = generateLabelInstruction(opCode, ifNode, destinationLabel,
                                    needsVMThreadRegister, cg);
  }
...
generateImm64SymInstruction(TR_X86OpCodes op, TR::Node *node, uint64_t imm,
                            ...
                            uint8_t *thunk) {
  return new (cg->trHeapMemory())
      TR::AMD64Imm64SymInstruction(op, node, imm, sr, cg, thunk);
}
