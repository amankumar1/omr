...
static inline TR::Instruction *generateWiderCompare(TR::Node *node,
  ...
  // the length of an LCP instruction in one cycle, therefore it initiates slow
  // decoding,
...
bool isConditionCodeSetForCompareToZero(TR::Node *node, bool justTestZeroFlag) {
  ...
  for (prevInstr = comp->getAppendInstruction(); prevInstr;
...
static void binarySearchCaseSpace(TR::Register *selectorReg,
  ...
  if (pivot >= lowChild) {
    ...
    //                [------------------------|--------------------------)
    //                signed
    //   TR::getMinSigned<TR::Int32>()         0
    //   TR::getMaxSigned<TR::Int32>()      TR::getMaxUnsigned<TR::Int32>()
    //                                         [--------------------------|------------------------------------)
    //                                         unsigned
    ...
    // For the signed case, we better use JG if the numbers straddle the 0
    // boundary; we already do.
    // For the unsigned case, we better use JA if the numbers straddle the
    // TR::getMaxSigned<TR::Int32>() boundary. If that's
    // the case, the highVal will be less than (in the signed sense) than
    // lowVal.
    //
    bool isUnsigned;
...
TR::Register *OMR::X86::TreeEvaluator::tableEvaluator(TR::Node *node,
  ...
  TR::MemoryReference *tempMR = generateX86MemoryReference(
      ...
      (uint8_t)(TR::Compiler->target.is64Bit() ? 3 : 2), (intptrj_t)branchTable,
      cg);
  ...
  if (cg->getLinkage()->getProperties().getMethodMetaDataRegister() !=
    ...
    if (secondChild->getNumChildren() > 0) {
      deps = generateRegisterDependencyConditions(
          secondChild->getFirstChild(), cg, needVMThreadDep ? 1 : 0, NULL);
    ...
    jmpTableInstruction = generateMemTableInstruction(
        JMPMem, node, tempMR, numBranchTableEntries, deps, cg);
  } else {
...
TR::Register *OMR::X86::TreeEvaluator::minmaxEvaluator(TR::Node *node,
  ...
  TR_X86OpCodes move_op =
      MOVRegReg(TR::Compiler->target.is64Bit() && type.isInt64());
  TR_X86OpCodes cmp_op =
      CMPRegReg(TR::Compiler->target.is64Bit() && type.isInt64());
  bool two_reg = (TR::Compiler->target.is32Bit() && type.isInt64());
  bool isMin = false;
  if ((node->getOpCodeValue() == TR::imin) ||
  ...
  if (cg->canClobberNodesRegister(child)) {
    ...
    if (two_reg) {
      generateRegRegInstruction(move_op, node, trgReg->getLowOrder(),
                                reg->getLowOrder(), cg);
      generateRegRegInstruction(move_op, node, trgReg->getHighOrder(),
                                reg->getHighOrder(), cg);
    } else {
  ...
  if (two_reg) {
    ...
    generateRegRegInstruction(cmp_op, node, reg->getHighOrder(),
                              trgReg->getHighOrder(), cg);
    generateLabelInstruction(isMin ? JG4 : JL4, node, doneLabel, false, cg);
    generateLabelInstruction(JNE4, node, gtLabel, false, cg);
    generateRegRegInstruction(cmp_op, node, reg->getLowOrder(),
                              trgReg->getLowOrder(), cg);
...
void OMR::X86::TreeEvaluator::setupProfiledGuardRelocation(
  ...
  // this inlined site will be created
    ...
    if (constValue >= -128 && constValue <= 127) {
      ...
      //
      {
        if (firstChild->getOpCode().isAnd() &&
          ...
          // compare a 2-byte
          // value with zero.  Ordinarily, this wouldn't work because the top 2
          // bytes of
          // the value are garbage. To make this work, we must zero-extend the
          // mask immediate
          // to 4 bytes, thereby ensuring the top two bytes of the masked value
          // will be
          // zero and won't affect the subsequent compare with zero.

          // Therefore, counterintuitively, zero-entending the mask is always
          // the correct thing
          // to do, regardless of whether the compare node claims to be a signed
          // operation!
          uint64_t mask;
                         ...
                         // and may be used in a 4 byte compare
          {
            ...
            if (andFirstChild->getRegister() == NULL &&
              ...
              // integral
              // conversion that is redundant because of the constant value in
              // the compare
          ...
          if (isConditionCodeSetForCompareToZero(firstChild, true)) {
            ...
            if (compareSize > 1 && (firstChild->getOpCodeValue() == TR::b2i ||
            ...
            if (compareSize > 2 && (firstChild->getOpCodeValue() == TR::su2i ||
            ...
            if (firstChild->getOpCode().isMemoryReference() &&
              ...
              if (isConditionCodeSetForCompareToZero(firstChild, true)) {
                ...
                if (compareSize == 1)
                  ...
                  compareGPRegisterToImmediateForEquality(node, firstChildReg,
                                                          0, cg);
              }
            }
          }
        ...
        if (firstChild->getOpCode().isMemoryReference() &&
          ...
          if (compareSize == 1)
            ...
            // shouldn't use Imm2 instructions
            TR::Register *tempReg = cg->allocateRegister();
            loadConstant(node, constValue, TR_RematerializableShort, cg,
                         tempReg);
      ...
      if (isPICCandidate &&
          cg->wantToPatchClassPointer((TR_OpaqueClassBlock *)constValue,
                                      secondChild))
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (constValue != 0 ||
      ...
      if (!node->getOpCode().isSpineCheck() &&
        ...
        compareGPRegisterToImmediate(node, cg->evaluate(firstChild), constValue,
                                     cg);
      }
    }
...
void OMR::X86::TreeEvaluator::compareIntegersForOrder(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  compareIntegersForOrder(node, node->getFirstChild(), node->getSecondChild(),
                          cg);
}
...
TR::Register *OMR::X86::TreeEvaluator::returnEvaluator(TR::Node *node,
  ...
  TR::RegisterDependencyConditions *dependencies =
      generateRegisterDependencyConditions((uint8_t)1, 0, cg);
...
OMR::X86::TreeEvaluator::iternaryEvaluator(TR::Node *node,
  ...
  if ((node->getOpCodeValue() == TR::buternary ||
       node->getOpCodeValue() == TR::bternary) &&
...
static bool canBeHandledByIfInstanceOfHelper(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  if (secondChild->getOpCode().isLoadConst() &&
...
static bool canBeHandledByIfArrayCmpHelper(TR::Node *node,
                                           TR::CodeGenerator *cg) {
  return false;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    return (firstChild->getOpCodeValue() == TR::arraycmp &&
            !firstChild->isArrayCmpLen() && firstChild->getRegister() == NULL &&
            firstChild->getReferenceCount() == 1 && constValue == 0);
  } else {
    return false;
  }
}
...
static inline bool needsMergedHCRGuardCode(TR::Node *node,
                                           TR::CodeGenerator *cg) {
  if (node->isTheVirtualGuardForAGuardedInlinedCall() &&
...
static inline void
generateMergedHCRGuardCodeIfNeeded(TR::Node *node, TR::CodeGenerator *cg,
...
OMR::X86::TreeEvaluator::integerIfCmpneEvaluator(TR::Node *node,
  ...
  if (virtualGuardHelper(node, cg)) {
    ...
    if (node->getFirstChild()->getOpCodeValue() == TR::ishr &&
        ...
        performTransformation(
            comp, "O^O SHIFT PEEPHOLE: detected shift pattern for node %p "
                  "shifting so mask = %p shift amount = %d \n",
            node, (((int32_t)-1)
    ...
    // compareIntegersForEquality
    // Below test was risky as the append instruction might not be the
    // instruction we want
    //
    // if (node->isTheVirtualGuardForAGuardedInlinedCall())
    //         {
...
bool OMR::X86::TreeEvaluator::generateIAddOrSubForOverflowCheck(
  ...
  if (matches && (u.operationNode->getOpCode().isAdd() ||
...
OMR::X86::TreeEvaluator::integerIfCmpltEvaluator(TR::Node *node,
                                                 TR::CodeGenerator *cg) {
  if (getNodeIs64Bit(node, cg) ? generateLAddOrSubForOverflowCheck(node, cg)
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if ((firstChild->getReferenceCount() == 1) &&
      ...
      cg->decReferenceCount(secondChild);
               ...
               (value == 0 || (value ==
                                   (int32_t)firstChild->getSecondChild()
...
static bool virtualGuardHelper(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (!((comp->performVirtualGuardNOPing() || node->isHCRGuard() ||
         node->isOSRGuard() || cg->needClassAndMethodPointerRelocations()) &&
  ...
  if (node->getOpCodeValue() != TR::ificmpne &&
    ...
    // Raise an assume if the optimizer requested that this virtual guard must
    // be NOPed
    //
    TR_ASSERT(
        virtualGuard->canBeRemoved(),
        "virtualGuardHelper: a non-removable virtual guard cannot be NOPed");
  ...
  if (node->isSideEffectGuard()) {
    ...
    default:
      TR_ASSERT(0, "AOT guard in node but not one of known guards supported "
                   "for AOT. Guard: %d",
                reloKind);
      break;
    }
  ...
  if (TR::Compiler->target.isSMP() && !node->isHCRGuard() &&
      !node->isOSRGuard()) {
    // the compiler is now capable of generating a train of vgnops all looking
    // to patch the
    // same point with different constraints. alignment is required before the
    // delegated patch
    // instruction if this collaborative merging is to work
    // we only want one alignment instruction because the instructions always
    // try to move
    // next to their target instruction and we can end up with an infinite loop
    // if two or more
    // dual. for now we know that all patch points have the same alignment
    // requirement so just
...
void OMR::X86::CodeGenerator::addMetaDataForBranchTableAddress(
  ...
  self()->addAOTRelocation(
      new (self()->trHeapMemory())
          TR_ExternalRelocation(target, 0, TR_AbsoluteMethodAddress, self()),
      __FILE__, __LINE__, caseNode->getBranchDestination()->getNode());

  TR::LabelSymbol *label =
      caseNode->getBranchDestination()->getNode()->getLabel();
  TR_LabelRelocation *relocation =
      new (self()->trHeapMemory()) TR_LabelAbsoluteRelocation(target, label);
