  ...
  for (i = 0; i < numConditions; i++) {
    ...
    *(cursor++) = '(';
    ...
    if (r == TR::RealRegister::AllFPRegisters) {
      ...
      len =
          sprintf(cursor, "%s", getName(_cg->machine()->getX86RealRegister(r)));
    }
    ...
    if (r == TR::RealRegister::AllFPRegisters) {
      ...
      if (r == TR::RealRegister::NoReg)
        ...
        trfprintf(pOutFile, "%s]",
                  getName(_cg->machine()->getX86RealRegister(r)));
    }
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::X86RestoreVMThreadInstruction *instr) {
  // *this    swipeable for debugging purposes
  if (pOutFile == NULL)
    return;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86RegRegRegInstruction *instr) {
  ...
  if (instr->getOpCodeValue() == SHLD4RegRegCL ||
      instr->getOpCodeValue() <= SHRD4RegRegCL)
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86MemRegRegInstruction *instr) {
  // *this    swipeable for debugging purposes
  if (pOutFile == NULL)
    return;
  ...
  if (instr->getOpCodeValue() == SHLD4MemRegCL ||
      instr->getOpCodeValue() == SHRD4MemRegCL)
    trfprintf(pOutFile, "cl");
  else
    print(pOutFile, instr->getSourceRightRegister(), sourceSize);
...
void TR_Debug::printReferencedRegisterInfo(TR::FILE *pOutFile,
                                           TR::X86MemRegRegInstruction *instr) {
  if (pOutFile == NULL)
    return;
...
int32_t TR_Debug::printFPConstant(TR::FILE *pOutFile, void *value,
                                  int8_t numBits, bool printAsBytes) {
  return 0;
}
...
void TR_Debug::printX86GCRegisterMap(TR::FILE *pOutFile,
  ...
  for (int i = TR::RealRegister::FirstGPR;
    ...
    if (map->getMap() & (1 << (i - 1))) // TODO:AMD64: Use the proper mask value
      trfprintf(pOutFile, "%s ", getName(machine->getX86RealRegister(
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    for (count = 0; count < callNode->getNumChildren(); count++) {
      ...
      case TR::Int32:
        ...
        trfprintf(pOutFile, "mov \tdword ptr[rsp+%d], %s\t\t#save registers "
                            "for interpreter call snippet",
      ...
      case TR::Int64:
        ...
        trfprintf(pOutFile, "mov \tqword ptr[rsp+%d], %s\t\t#save registers "
                            "for interpreter call snippet",
    ...
    case TR::Int64:
      if (numGPArgs < linkageProperties.getNumIntegerArgumentRegisters()) {
        ...
        TR::RealRegister::RegNum reg =
            linkageProperties.getIntegerArgumentRegister(numGPArgs);
        regName =
            getName(_cg->machine()->getX86RealRegister(reg), TR_DoubleWordReg);
      }
      numGPArgs++;
      break;
    case TR::Float:
      if (numFPArgs < linkageProperties.getNumFloatArgumentRegisters()) {
        ...
        regName =
            getName(_cg->machine()->getX86RealRegister(reg), TR_QuadWordReg);
      }
      numFPArgs++;
      break;
    case TR::Double:
...
#ifdef J9_PROJECT_SPECIFIC
uint8_t *TR_Debug::printArgs(TR::FILE *pOutFile,
                             TR::AMD64WriteBarrierSnippet *snippet,
                             bool restoreRegs, uint8_t *bufferPos) {
  ...
  TR::RealRegister *r1Real =
      toRealRegister(snippet->getDestOwningObjectRegister());
  ...
  // Fast path the single argument case.  Presently, only a batch arraycopy
  // write barrier applies.
  //
  if (deps->getNumPostConditions() == 1) {
    if (r1 != TR::RealRegister::eax) {
      // Register is not in RAX so exchange it.  This applies to both building
      // and restoring args.
      //
  ...
  action = (restoreRegs ? 32 : 0) | (r1 == TR::RealRegister::eax ? 16 : 0) |
           (r1 == TR::RealRegister::esi ? 8 : 0) |
           (r2 == TR::RealRegister::esi ? 4 : 0) |
           (r2 == TR::RealRegister::eax ? 2 : 0) | (r1 == r2 ? 1 : 0);
...
static uint8_t *estimateJumpSize(TR::AMD64WriteBarrierSnippet *snippet,
                                 uint8_t *bufferPos) {
...
void TR_Debug::print(TR::FILE *pOutFile,
  ...
  printPrefix(pOutFile, NULL, bufferPos, 5);
  trfprintf(pOutFile,
            "call\t\tjitWriteBarrierStoreGenerationalAndConcurrentMark");
  bufferPos += 5;
  bufferPos = printArgs(pOutFile, snippet, true, bufferPos);
