...
TR::Register *OMR::X86::TreeEvaluator::fconstEvaluator(TR::Node *node,
  ...
  if (cg->useSSEForSinglePrecision()) {
    ...
    if (node->getFloatBits() == 0) {
      ...
      TR::IA32ConstantDataSnippet *cds =
          cg->findOrCreate4ByteConstant(node, node->getFloatBits());
...
TR::Register *OMR::X86::TreeEvaluator::dconstEvaluator(TR::Node *node,
  ...
  if (cg->useSSEForDoublePrecision()) {
    ...
    if (node->getLongInt() == 0) {
      ...
      TR::IA32ConstantDataSnippet *cds =
          cg->findOrCreate8ByteConstant(node, node->getLongInt());
      TR::MemoryReference *tempMR = generateX86MemoryReference(cds, cg);
...
TR::Register *OMR::X86::TreeEvaluator::performFload(
  ...
  if (cg->useSSEForSinglePrecision()) {
    if (TR::Compiler->target.is64Bit() &&
      ...
      // assignment.
      // Hence, the unresolved memory reference must be evaluated into a
      // register
      // first.
      //
...
OMR::X86::TreeEvaluator::fpReturnEvaluator(TR::Node *node,
  ...
  if (comp->getJittedMethodSymbol()->usesSinglePrecisionMode() &&
    ...
    generateMemInstruction(LDCWMem, node, generateX86MemoryReference(cds, cg),
                           cg);
  }
  ...
  // Protect VMThread register fetch, it could be NULL if we are generating from
  // WCode
  //
  if (linkageProperties.getMethodMetaDataRegister() !=
      TR::RealRegister::NoReg) {
    dependencies->addPreCondition(
        cg->getVMThreadRegister(),
        (TR::RealRegister::RegNum)cg->getVMThreadRegister()->getAssociation(),
        cg);
  }
...
TR::Register *OMR::X86::TreeEvaluator::fpRemEvaluator(TR::Node *node,
  ...
  TR::Register *targetRegister;
  TR::Compilation *comp = cg->comp();
  const TR_X86LinkageProperties &linkageProperties =
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      TR::SymbolReference *helperSymRef =
          cg->symRefTab()->findOrCreateRuntimeHelper(
              ...
              false, false, false);
      targetRegister = performHelperCall(
...
TR::Register *OMR::X86::TreeEvaluator::fnegEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  ...
  if (sourceRegister->getKind() == TR_FPR) {
    TR::IA32ConstantDataSnippet *cds =
        cg->findOrCreate4ByteConstant(node, FLOAT_NEG_ZERO);
    targetRegister = cg->allocateSinglePrecisionRegister(TR_FPR);
    generateRegMemInstruction(MOVSSRegMem, node, targetRegister,
                              generateX86MemoryReference(cds, cg), cg);
    generateRegRegInstruction(XORPSRegReg, node, targetRegister, sourceRegister,
                              cg);
  } else {
...
TR::Register *OMR::X86::TreeEvaluator::dnegEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *sourceRegister = cg->evaluate(child);
  TR::Register *targetRegister;
  if (sourceRegister->getKind() == TR_FPR) {
    TR::IA32ConstantDataSnippet *cds =
        cg->findOrCreate8ByteConstant(node, DOUBLE_NEG_ZERO);
    targetRegister = cg->allocateRegister(TR_FPR);
    generateRegMemInstruction(cg->getXMMDoubleLoadOpCode(), node,
                              targetRegister,
                              generateX86MemoryReference(cds, cg), cg);
    generateRegRegInstruction(XORPDRegReg, node, targetRegister, sourceRegister,
                              cg);
  } else {
...
TR::Register *OMR::X86::TreeEvaluator::fpConvertToInt(
  ...
  if (!optimizeF2IWithSSE && !optimizeD2IWithSSE2) {
    ...
    TR::IA32ConstantDataSnippet *cds1 =
        cg->findOrCreate2ByteConstant(node, fpcw);

    fpcw = comp->getJittedMethodSymbol()->usesSinglePrecisionMode()
               ? SINGLE_PRECISION_ROUND_TO_NEAREST
               : DOUBLE_PRECISION_ROUND_TO_NEAREST;

    TR::IA32ConstantDataSnippet *cds2 =
        cg->findOrCreate2ByteConstant(node, fpcw);

    tempMR = (cg->machine())->getDummyLocalMR(TR::Int32);

    generateMemInstruction(LDCWMem, node, generateX86MemoryReference(cds1, cg),
                           cg);
    generateFPMemRegInstruction(FISTMemReg, node, tempMR, floatReg, cg);
    generateMemInstruction(LDCWMem, node, generateX86MemoryReference(cds2, cg),
                           cg);
...
TR::Register *OMR::X86::TreeEvaluator::fpConvertToLong(
  ...
  if (child->getOpCode().isDouble() && cg->useSSEForDoublePrecision()) {
    ...
    // CVTTSD2SI.
    // If the conversion succeeds, put the integer in lowReg and sign-extend it
    // to highReg.
    ...
    //    The code that uses optLevel>0 doesn't work because it's allocating the
    //    floating point registers without
    //    putting dependency on the end label. FP stack register dependencies
    //    aren't supported
    //    in the current register assigner implementation (except for ST0) and
    //    we can't use the sequence below.
    //    The current code will use slower sequence with reseting the control
    //    word for processors
    //    that don't support SSE and SSE2 (see above). In case we get
    //    performance complain we can
    //    engineer a better solution, which could involve adding FP stack
    //    dependencies support or making sure
    //    we have 2 slots on the FP stack and generating the code with fixed
    //    registers in some snippet.
    ...
    // original double register.
    // If this instruction changes, or if you add any instructions that
    // manipulate the FP stack between
    // here and the call to the snippet, you may need to change the snippet
    // also.
    //
    ...
    if (!optLevel) {
      ...
      generateMemInstruction(LDCWMem, node,
                             generateX86MemoryReference(cds1, cg), cg);
    }
    ...
    if (!optLevel) {
      ...
      TR::IA32ConstantDataSnippet *cds2 =
          cg->findOrCreate2ByteConstant(node, fpcw);
      generateMemInstruction(LDCWMem, node,
                             generateX86MemoryReference(cds2, cg), cg);
    }
    ...
    // If they or their format is changed, you may need to change the snippet
    // also.
    //
    ...
    generateLabelInstruction(JNE4, node, optLevel ? nonNanLabel : reStartLabel,
                             cg);
    generateRegRegInstruction(TEST4RegReg, node, lowReg, lowReg, cg);
    generateLabelInstruction(JE4, node, snippetLabel, cg);

    // For fast conversion only, we need to adjust the rounded long to emulate
    // truncation.
    //
    if (optLevel) {
      ...
      if (optLevel > 1 &&
          cg->getX86ProcessorInfo().supportsFCOMIInstructions()) {
        TR::Register *tempFPR2 = cg->allocateSinglePrecisionRegister(TR_X87);
        generateFPRegInstruction(FLD0Reg, node, tempFPR2, cg);
        generateFPCompareRegRegInstruction(FCOMIRegReg, node, tempFPR2,
                                           doubleReg, cg);
        TR::LabelSymbol *negativeLabel =
            TR::LabelSymbol::create(cg->trHeapMemory(), cg);
        ...
        generateFPRegMemInstruction(
            FLLDRegMem, node, tempFPR3,
            generateX86MemoryReference(*convertedLongMR, 0, cg), cg);
        generateFPCompareRegRegInstruction(FCOMIRegReg, node, tempFPR3,
                                           doubleReg, cg);
        ...
        generateFPRegMemInstruction(
            FLLDRegMem, node, tempFPR4,
            generateX86MemoryReference(*convertedLongMR, 0, cg), cg);
        generateFPCompareRegRegInstruction(FCOMIRegReg, node, tempFPR4,
                                           doubleReg, cg);
        ...
        TR::Register *tempFPR2 = cg->allocateSinglePrecisionRegister(TR_X87);
        generateFPRegInstruction(FLD0Reg, node, tempFPR2, cg);
        generateFPCompareRegRegInstruction(FCOMRegReg, node, tempFPR2,
                                           doubleReg, cg);
        cg->stopUsingRegister(tempFPR2);
        ...
        generateFPRegMemInstruction(
            FLLDRegMem, node, tempFPR3,
            generateX86MemoryReference(*convertedLongMR, 0, cg), cg);
        generateFPArithmeticRegRegInstruction(FSUBRRegReg, node, tempFPR3,
                                              doubleReg, cg);
        generateFPMemRegInstruction(FSTMemReg, node, convertedLongMR, tempFPR3,
                                    cg);
        cg->stopUsingRegister(tempFPR3);
...
TR::Register *OMR::X86::TreeEvaluator::f2iEvaluator(TR::Node *node,
  ...
  if (cg->useSSEForSinglePrecision()) {
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      switch (node->getOpCodeValue()) {
      case TR::f2i:
        helperSymRef = cg->symRefTab()->findOrCreateRuntimeHelper(
            TR_AMD64floatToInt, false, false, false);
        cvttOpCode = CVTTSS2SIReg4Reg;
        break;
      case TR::f2l:
        helperSymRef = cg->symRefTab()->findOrCreateRuntimeHelper(
            TR_AMD64floatToLong, false, false, false);
        cvttOpCode = CVTTSS2SIReg8Reg;
        break;
      case TR::d2i:
        helperSymRef = cg->symRefTab()->findOrCreateRuntimeHelper(
            TR_AMD64doubleToInt, false, false, false);
        cvttOpCode = CVTTSD2SIReg4Reg;
        break;
      case TR::d2l:
        helperSymRef = cg->symRefTab()->findOrCreateRuntimeHelper(
            TR_AMD64doubleToLong, false, false, false);
      ...
      helperSymRef = cg->symRefTab()->findOrCreateRuntimeHelper(
          TR_IA32floatToInt, false, false, false);
      cvttOpCode = CVTTSS2SIReg4Reg;
    }
    ...
    if (sourceRegister->getKind() == TR_X87 &&
      ...
      instr = generateRegMemInstruction(
          CVTTSS2SIReg4Mem, node, targetRegister,
          generateX86MemoryReference(*tempMR, 0, cg), cg);
    } else {
      instr = generateRegRegInstruction(cvttOpCode, node, targetRegister,
                                        sourceRegister, cg);
    }

    if (TR::Compiler->target.is64Bit()) {
      cg->addSnippet(new (cg->trHeapMemory()) TR::AMD64FPConversionSnippet(
          reStartLabel, snippetLabel, helperSymRef, instr, cg));
    } else {
      cg->addSnippet(new (cg->trHeapMemory()) TR::X86FPConvertToIntSnippet(
          reStartLabel, snippetLabel, helperSymRef, instr, cg));
    }
    ...
    TR_ASSERT(TR::Compiler->target.is32Bit(), "assertion failure");
    return fpConvertToInt(node, cg->symRefTab()->findOrCreateRuntimeHelper(
                                    TR_IA32floatToInt, false, false, false),
                          cg);
  }
}
...
TR::Register *OMR::X86::TreeEvaluator::f2lEvaluator(TR::Node *node,
  ...
  return fpConvertToLong(node, cg->symRefTab()->findOrCreateRuntimeHelper(
...
TR::Register *OMR::X86::TreeEvaluator::d2iEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is32Bit(), "AMD64 uses f2iEvaluator for this");

  TR::SymbolReference *helperSymRef =
      cg->symRefTab()->findOrCreateRuntimeHelper(TR_IA32doubleToInt, false,
                                                 false, false);
  ...
  if (cg->useSSEForDoublePrecision()) {
    ...
    if (sourceRegister->getKind() == TR_X87 &&
      ...
      instr = generateRegMemInstruction(
          CVTTSD2SIReg4Mem, node, targetRegister,
          generateX86MemoryReference(*tempMR, 0, cg), cg);
    } else {
      instr = generateRegRegInstruction(CVTTSD2SIReg4Reg, node, targetRegister,
                                        sourceRegister, cg);
    }
    ...
    return fpConvertToInt(node, cg->symRefTab()->findOrCreateRuntimeHelper(
                                    TR_IA32doubleToInt, false, false, false),
                          cg);
  }
}
...
TR::Register *OMR::X86::TreeEvaluator::fbits2iEvaluator(TR::Node *node,
  ...
  if (node->normalizeNanValues()) {
    ...
    if (debug("newfbits2i")) {
      ...
      generateRegImmInstruction(ROR4RegImm1, node, target, 23,
                                cg); // move exponent to the low order byte
      ...
      TR::X86RegImmInstruction *instr = generateRegImmInstruction(
          ROL4RegImm1, node, target, 23, cg); // restore bits to proper order
      TR::RegisterDependencyConditions *deps =
          generateRegisterDependencyConditions((uint8_t)0, (uint8_t)1, cg);
      deps->addPostCondition(target, TR::RealRegister::NoReg, cg);
      TR::X86fbits2iSnippet *snippet = new (cg->trHeapMemory())
          TR::X86fbits2iSnippet(restartLabel, snippetLabel, instr, cg);
      ...
      // Slow path
      //
      TR_OutlinedInstructions *slowPath =
          new (cg->trHeapMemory()) TR_OutlinedInstructions(slowPathLabel, cg);
...
TR::Register *OMR::X86::TreeEvaluator::generateBranchOrSetOnFPCompare(
  ...
  // in AH;
  // generate an FCMPEVAL pseudo-instruction, which will emit the interpretation
  // code
  // at register assignment time (when we have finalized the sense of the
  // comparison).
  //
  if (accRegister != NULL) {
...
OMR::X86::TreeEvaluator::compareFloatAndBranchEvaluator(TR::Node *node,
  ...
  TR::Register *accRegister = compareFloatOrDoubleForOrder(
