...
OMR::X86::Machine::Machine(uint8_t numIntRegs, uint8_t numFPRegs,
    ...
    : OMR::Machine(numIntRegs, numFPRegs), _cg(cg), _registerFile(registerFile),
...
TR::RealRegister *OMR::X86::Machine::findBestFreeGPRegister(
  ...
  if (!debug("disableForceVMThreadToEBP") &&
      virtReg == self()->cg()->getVMThreadRegister()) {
    // Reg assigner gets confused if the virtual vmthread GPR gets a register
    // other than ebp

    // If the vmThread register is free then assign it straightaway.
    //
    if (_registerFile[TR::RealRegister::ebp]->getState() ==
            TR::RealRegister::Free ||
        _registerFile[TR::RealRegister::ebp]->getState() ==
...
TR::RealRegister *OMR::X86::Machine::freeBestGPRegister(
  ...
  if (!debug("disableForceVMThreadToEBP") &&
    ...
    TR_ASSERT(_registerFile[TR::RealRegister::ebp]->getState() ==
                  TR::RealRegister::Assigned,
              "ebp must be spillable for vmthread");
    ...
    if (false)
      TR_ASSERT(!self()->cg()->insideInternalControlFlow(),
                "The instruction %p from %s, should not be inside an Internal "
                "Control Flow with a depth of %d with a safe depth of %d\n",
                currentInstruction,
                currentInstruction->getNode()->getOpCode().getName(),
                self()->cg()->internalControlFlowNestingDepth(),
                self()->cg()->internalControlFlowSafeNestingDepth());
    ...
    for (i = first; i <= last; i++) {
      ...
      if (i == TR::RealRegister::ebp &&
          (interference & TR::RealRegister::ebpMask) &&
          realReg->getAssignedRegister() ==
              self()->cg()->getVMThreadRegister()) {
        // The interference to the VM thread register is absolute.  Don't
        // consider the thread
      ...
      if (realReg->getState() == TR::RealRegister::Assigned) {
        ...
        if (targetRegister == i) {
          if (useRegisterInterferences &&
            ...
            bestRegisters[NonInterferingTarget] =
                realReg->getAssignedRegister();
        }
      }
    }
    ...
    if (numCandidates == 0) {
      ...
      // register required in a register dependency, there may not be any
      // suitable spill
      // candidates if all available real registers are requested in the
      // register dependency.  In
      // such a case, where our virtual need not be in a register across the
      // dependency, choose the
      // virtual itself as a spill candidate.
      //
      TR_ASSERT(considerVirtAsSpillCandidate, "freeBestGPRegister(): could not "
                                              "find any GPR spill candidates "
                                              "for %s\n",
  ...
  if (enableRematerialisation && bestDiscardableRegister) {
    ...
    if (info->isRematerializableFromMemory()) {
      ...
      if (info->getDataType() == TR_RematerializableFloat) {
        ...
        TR::MemoryReference *tempMR =
            generateX86MemoryReference(cds, self()->cg());
        instr = generateRegMemInstruction(currentInstruction, MOVSSRegMem, best,
                                          tempMR, self()->cg());
#ifdef DEBUG
        self()->cg()->incNumRematerializedXMMRs();
    ...
    if (bestRegister == self()->cg()->getVMThreadRegister()) {
      if (bestRegister->getBackingStorage() == NULL) {
        ...
        self()->cg()->setVMThreadSpillInstruction(
            (TR::Instruction *)0xffffffff);
      } else {
        location = bestRegister->getBackingStorage(); // note: vmthread is never
                                                      // spilled to a slot's
                                                      // second half
      }
...
TR::RealRegister *OMR::X86::Machine::reverseGPRSpillState(
  ...
  if (spilledRegister != self()->cg()->getVMThreadRegister()) {
    ...
    if (self()->getDebug()) {
      self()->cg()->traceRegisterAssignment(
          "VMThread spill instruction is now %s\n",
          (self()->cg()->getVMThreadSpillInstruction() ==
           (TR::Instruction *)0xffffffff)
              ? "sentinel"
...
void OMR::X86::Machine::coerceGPRegisterAssignment(
  ...
  if (targetRegister->getState() == TR::RealRegister::Free) {
    ...
    if (currentAssignedRegister != NULL) {
      ...
      if (toRealRegister(currentAssignedRegister)->getRegisterNumber() ==
          self()->cg()->getProperties().getMethodMetaDataRegister())
        self()->cg()->processDeferredSplits(
            currentTargetVirtual == self()->cg()->getVMThreadRegister());
    } else {
      ...
      if (candidate) {
        ...
        // second
        // register whence the virtual currently occupying the target may be
        // moved,
      ...
      if ((targetRegister != candidate) &&
        ...
        if (candidate->getRegisterNumber() ==
            self()->cg()->getProperties().getMethodMetaDataRegister())
          self()->cg()->processDeferredSplits(
              currentTargetVirtual == self()->cg()->getVMThreadRegister());
      }
  ...
  if (registerNumber ==
      self()->cg()->getProperties().getMethodMetaDataRegister())
    self()->cg()->processDeferredSplits(virtualRegister ==
                                        self()->cg()->getVMThreadRegister());
}
...
void OMR::X86::Machine::coerceXMMRegisterAssignment(
  ...
  if (targetRegister->getState() == TR::RealRegister::Free) {
    ...
    if (currentAssignedRegister != NULL) {
      ...
      //
      TR_X86OpCodes xchgOp =
          virtualRegister->isSinglePrecision() ? XORPSRegReg : XORPDRegReg;
      self()->cg()->traceRegAssigned(currentTargetVirtual,
                                     currentAssignedRegister);
      instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
...
void OMR::X86::Machine::setGPRWeightsFromAssociations() {
  ...
  for (int i = TR::RealRegister::FirstGPR;
    ...
    //
    if (self()->getX86RealRegister((TR::RealRegister::RegNum)i)->getState() ==
        TR::RealRegister::Locked)
      continue;
    ...
    if ((linkageProperties.isPreservedRegister((TR::RealRegister::RegNum)i) ||
      ...
      assocReg->setAssociation(i);
      if (assocReg->isPlaceholderReg()) { // placeholder register and is only
                                          // needed at the specific dependency
...
void OMR::X86::Machine::createRegisterAssociationDirective(
  ...
  for (int i = 0; i < TR::RealRegister::LastAssignableGPR; i++) {
    ...
    // Skip non-assignable registers
    //
    if (self()->getX86RealRegister(regNum)->getState() ==
        TR::RealRegister::Locked)
      continue;
...
void OMR::X86::Machine::initialiseRegisterFile(
    const struct TR_X86LinkageProperties &properties) {
  // *this    swipeable for debugging purposes
  int reg;
  ...
  for (reg = TR::RealRegister::r8; reg <= TR::RealRegister::LastAssignableGPR;
    ...
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, properties.isPreservedRegister((TR::RealRegister::RegNum)reg)
                    ? PRESERVED_WEIGHT
                    : NONPRESERVED_WEIGHT,
  ...
  for (reg = TR::RealRegister::FirstFPR; reg <= TR::RealRegister::LastFPR;
       reg++) {
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_X87, properties.isPreservedRegister((TR::RealRegister::RegNum)reg)
                    ? PRESERVED_WEIGHT
                    : NONPRESERVED_WEIGHT,
        TR::RealRegister::Free, (TR::RealRegister::RegNum)reg,
  ...
  for (reg = TR::RealRegister::FirstXMMR; reg <= TR::RealRegister::LastXMMR;
       reg++) {
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, properties.isPreservedRegister((TR::RealRegister::RegNum)reg)
                    ? PRESERVED_WEIGHT
                    : NONPRESERVED_WEIGHT,
        TR::RealRegister::Free, (TR::RealRegister::RegNum)reg,
...
uint32_t *OMR::X86::Machine::getGlobalRegisterTable(
    const struct TR_X86LinkageProperties &property) {
...
OMR::X86::Machine::cloneRegisterFile(TR::RealRegister **registerFile,
  ...
  // _frameRegister pointer so the memRef assignRegisters check
  // if (_baseRegister ==
  // cg()->machine()->getX86RealRegister(TR::RealRegister::vfp)
  // works correctly
  ...
  // registerFile values for the XMMRegs so
...
OMR::X86::Machine::createDepCondForLiveGPRs() {
  ...
  for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastXMMR;
       i = ((i == TR::RealRegister::LastAssignableGPR)
                ...
                : i + 1)) {
    TR::RealRegister *realReg =
        self()->getX86RealRegister((TR::RealRegister::RegNum)i);
    if (realReg->getState() == TR::RealRegister::Assigned ||
...
TR::RealRegister **OMR::X86::Machine::captureRegisterFile() {
  ...
  registerFileClone[TR::RealRegister::vfp] =
      (TR::RealRegister *)self()->cg()->trMemory()->allocateMemory(
          sizeof(TR::RealRegister), heapAlloc);
  memcpy(registerFileClone[TR::RealRegister::vfp],
         _registerFile[TR::RealRegister::vfp], sizeof(TR::RealRegister));
...
void OMR::X86::Machine::installRegisterFile(
  ...
  memcpy(_registerFile[TR::RealRegister::vfp],
         registerFileCopy[TR::RealRegister::vfp], sizeof(TR::RealRegister));
}
...
TR::Register **OMR::X86::Machine::captureRegisterAssociations() {
  ...
  for (int32_t i = TR::RealRegister::FirstGPR; i <= endReg;
    ...
    if (_registerAssociations[i] != NULL) {
      ...
      memcpy(registerAssociationsClone[i], _registerAssociations[i],
             sizeof(TR::Register));
    } else {
  ...
  if (_registerAssociations[TR::RealRegister::vfp] != NULL) {
    registerAssociationsClone[TR::RealRegister::vfp] =
        (TR::Register *)self()->cg()->trMemory()->allocateMemory(
            sizeof(TR::Register), heapAlloc);
    memcpy(registerAssociationsClone[TR::RealRegister::vfp],
           _registerAssociations[TR::RealRegister::vfp], sizeof(TR::Register));
  } else {
...
TR_RegisterAssignerState::createDependenciesFromRegisterState(
  ...
  if (comp->getOptions()->getOption(TR_TraceNonLinearRegisterAssigner)) {
    traceMsg(comp, "createDependenciesFromRegisterState : %d live registers: "
                   "%d assigned, %d spilled\n",
  ...
  for (i = TR::RealRegister::FirstGPR; i <= endReg;
    ...
    if (realReg->getState() == TR::RealRegister::Assigned) {
      ...
      // be adjusted
      // to account for this new use.  The total use count will be adjusted when
      // this
      ...
      TR_ASSERT(!_spilledRegistersList || !found, "a register should not be in "
                                                  "both an assigned state and "
                                                  "in the spilled list\n");
    }
  }
...
void OMR::X86::Machine::adjustRegisterUseCountsUp(
  ...
  for (auto iter = rul->begin(); iter != rul->end(); ++iter) {
    if (comp->getOptions()->getOption(TR_TraceNonLinearRegisterAssigner)) {
      traceMsg(comp, "Adjusting UP register use counts of reg %p "
...
void OMR::X86::Machine::adjustRegisterUseCountsDown(
  ...
  for (auto iter = rul->begin(); iter != rul->end(); ++iter) {
    if (comp->getOptions()->getOption(TR_TraceNonLinearRegisterAssigner)) {
      traceMsg(comp, "Adjusting DOWN register use counts of reg %p "
                     "(fuc=%d:tuc=%d:adjustFuture=%d) by %d -> ",
               (*iter)->virtReg, (*iter)->virtReg->getFutureUseCount(),
               (*iter)->virtReg->getTotalUseCount(), adjustFuture,
               (*iter)->useCount);
...
void OMR::X86::Machine::disassociateUnspilledBackingStorage() {
  ...
  for (i = TR::RealRegister::FirstGPR; i <= endReg;
    ...
    if (_registerFile[i]->getState() == TR::RealRegister::Assigned) {
      ...
      if (location) {
        int32_t size = (virtReg->getKind() == TR_FPR)
                           ? (virtReg->isSinglePrecision() ? 4 : 8)
                           : TR::Compiler->om.sizeofReferenceAddress();
...
// stack
// and generate code.  It is assumed that both registers of interest are on the
// FP stack.
//
TR::Instruction *
OMR::X86::Machine::fpStackFXCH(TR::Instruction *prevInstruction,
...
// will occupy ST1.
// The orientation will be chosen based on whichever results in the fewest
// possible register
// FXCHs to achieve.
//
