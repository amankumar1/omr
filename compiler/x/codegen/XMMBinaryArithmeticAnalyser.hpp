...
public:
  ...
  TR_X86XMMBinaryArithmeticAnalyser(TR::CodeGenerator *codeGen)
      : _codeGen(codeGen), _package(kBADpackage), _inputs(0) {}

  TR_X86XMMBinaryArithmeticAnalyser(uint8_t package, TR::CodeGenerator *codeGen)
      : _codeGen(codeGen), _package(package), _inputs(0) {}

  TR_X86XMMBinaryArithmeticAnalyser(TR::Node *node, TR::CodeGenerator *codeGen)
      : _codeGen(codeGen), _package(getX86XMMOpPackage(node)), _inputs(0) {}
  ...
  TR_X86OpCodes getRegRegOp() { return _opCodePackage[_package][kOpRegReg]; }

  TR_X86OpCodes getRegMemOp() { return _opCodePackage[_package][kOpRegMem]; }
  ...
  bool evaluatesTarget() {
    return (_actionMap[_inputs] & kEvalTarget) ? true : false;
  }
  bool evaluatesSource() {
    return (_actionMap[_inputs] & kEvalSource) ? true : false;
  }
  bool copiesTargetReg() {
    return (_actionMap[_inputs] & kCopyTarget) ? true : false;
  }
  bool mustUseRegRegOp() {
    return (_actionMap[_inputs] & kRegReg) ? true : false;
  }
  bool canUseRegMemOp() {
    return (_actionMap[_inputs] & kRegMem) ? true : false;
  }
  bool resetsInputs() {
    return (_actionMap[_inputs] & kResetInputs) ? true : false;
  }
...
private:
  ...
  static const TR_X86OpCodes _opCodePackage[kNumXMMArithPackages]
                                           [kNumXMMArithVariants];
  uint8_t _package;
  uint8_t _inputs;
