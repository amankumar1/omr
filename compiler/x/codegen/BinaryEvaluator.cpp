  ...
  if (indexNode) {
    ...
    if (constNode) {
      ...
      tempNode = NULL;
               ...
               constNodeValueIs32BitSigned(baseNode->getSecondChild(),
                                           &dummyConstValue, cg)) {
      // add            (** children may be reversed **)
      //    add            (baseNode)
      //       ?
    ...
    if (firstChild->getRegister() == NULL &&
      ...
      // firstChild's grandchildren's registers, and hence adding register
      // pressure.
      //
      ...
      if (stride1 | stride2) {
        ...
        if (indexNode->getOpCode().isLoadConst()) {
          ...
          leaMR = generateX86MemoryReference(
              cg->evaluate(baseNode), integerConstNodeValue(constNode, cg) +
                                          integerConstNodeValue(indexNode, cg),
              cg);
        } else {
          leaMR = generateX86MemoryReference(
    ...
    targetRegister = generateLEAForLoadAddr(
    ...
    return true;
             ...
             constNodeValueIs32BitSigned(firstChild->getSecondChild(),
                                         &dummyConstValue, cg) &&
             (firstChild->getReferenceCount() == 1)) {
    // add
    //    sub/add    (firstChild)
...
OMR::X86::TreeEvaluator::integerAddEvaluator(TR::Node *node,
  ...
  if (!computesCarry && analyseAddForLEA(node, cg)) {
    ...
    if (!targetRegister && secondChild->getOpCode().isLoadConst() &&
        ...
        performTransformation(comp, "O^O analyseAddForLEA: checking that "
                                    "second node is a memory reference %x\n",
                              constValue)) {
      if (!isMemOp)
      ...
      if (targetRegister &&
        ...
        // needs to be marked as a collected reference for GC to behave
        // correctly;
        // note that support for internal pointer TR::aiadd (e.g. array access)
        // is not present
        // still
        //
...
TR::Register *OMR::X86::TreeEvaluator::baddEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL &&
      performTransformation(comp, "O^O BaddEvaluator: checking that the store "
                                  "has not happened yet. Target register: %x\n",
                            testRegister)) {
...
TR::Register *OMR::X86::TreeEvaluator::saddEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL &&
      performTransformation(comp, "O^O SaddEvaluator: checking that the store "
                                  "has not happened yet. Target register: %x\n",
                            testRegister)) {
...
TR::Register *OMR::X86::TreeEvaluator::caddEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCodeValue() == TR::cconst && testRegister == NULL &&
      performTransformation(comp, "O^O CaddEvaluator: checking that the store "
                                  "has not happened yet. Target register:  "
                                  "%x\n",
                            testRegister)) {
...
OMR::X86::TreeEvaluator::integerSubEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst() && testRegister == NULL &&
      constNodeValueIs32BitSigned(secondChild, &constValue, cg) &&
      performTransformation(comp, "O^O IntegerSubEvaluator: register is not "
                                  "NULL, or second operand is not a 32 bit "
                                  "constant. Register value: %x\n",
                            testRegister)) {
...
TR::Register *OMR::X86::TreeEvaluator::bsubEvaluator(TR::Node *node,
  ...
  TR::Register *testRegister = secondChild->getRegister();
  if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL &&
      performTransformation(comp, "O^O BSUBEvaluator: checking that the store "
                                  "has not happened yet. Target register:  "
                                  "%x\n",
                            testRegister)) {
    int32_t value = secondChild->getByte();
...
TR::Register *OMR::X86::TreeEvaluator::ssubEvaluator(TR::Node *node,
  ...
  TR::Register *testRegister = secondChild->getRegister();
  if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL &&
      performTransformation(comp, "O^O SSUBEvaluator: checking that the store "
                                  "has not happened yet. Target register:  "
                                  "%x\n",
                            testRegister)) {
    int32_t value = secondChild->getShortInt();
...
TR::Register *OMR::X86::TreeEvaluator::csubEvaluator(TR::Node *node,
  ...
  TR::Register *testRegister = secondChild->getRegister();
  if (secondChild->getOpCodeValue() == TR::cconst && testRegister == NULL &&
      performTransformation(comp, "O^O CSubEvaluator: checking that the store "
                                  "has not happened yet. Target register: %x\n",
                            testRegister)) {
    int32_t value = secondChild->getConst<uint16_t>();
    if (!isMemOp)
...
OMR::X86::TreeEvaluator::integerDualMulEvaluator(TR::Node *node,
  ...
  if (node->isDualCyclic() || needsHighMulOnly) {
    ...
    // evaluation of lumulh
    // (or result/by-product may be reversed, if lumulh encountered first, and
    // RAX may not be required if lumulh is on its own)
...
OMR::X86::TreeEvaluator::integerMulhEvaluator(TR::Node *node,
  ...
  //
  TR::Register *targetRegister =
      intOrLongClobberEvaluate(secondChild, nodeIs64Bit, cg);
  TR::Register *eaxRegister =
      intOrLongClobberEvaluate(firstChild, nodeIs64Bit, cg);
...
OMR::X86::TreeEvaluator::signedIntegerDivOrRemAnalyser(TR::Node *node,
  ...
  TR::Register *dividendRegister =
      intOrLongClobberEvaluate(dividend, nodeIs64Bit, cg);
  ...
  if (isPowerOf2(dvalue)) {
    ...
    // right result from the SAR.
    // We use CDQ (or CQO) to allow us to do this without introducing control
    // flow.
    //
    if ((dividend->isNonNegative() == false) && (dvalue > 0 || isMinInt)) {
      ...
      if (dvalue == 2) // special case when working with 2
      {
        ...
        TR_ASSERT(mask >= 0, "AMD64: mask must have sign bit clear so that, "
                             "when sign-extended, it clears the upper 32 bits "
                             "of tempRegister");
...
TR::X86RegInstruction *OMR::X86::TreeEvaluator::generateRegisterShift(
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    intptrj_t shiftAmount =
        integerConstNodeValue(secondChild, cg) & shiftMask(nodeIs64Bit);
    if (shiftAmount == 0) {
      targetRegister = cg->evaluate(firstChild);
    } else {
      targetRegister = cg->evaluate(firstChild);
      ...
      if (SHIFT_MAY_HAVE_ADDRESS_CHILD && node->getOpCode().isLeftShift() &&
        ...
        targetRegister = intOrLongClobberEvaluate(
            firstChild,
            (SHIFT_MAY_HAVE_ADDRESS_CHILD) ? getNodeIs64Bit(firstChild, cg)
                                           : nodeIs64Bit,
            cg);
      }
...
OMR::X86::TreeEvaluator::integerShlEvaluator(TR::Node *node,
  ...
  if (node->isDirectMemoryUpdate()) {
    generateMemoryShift(node, SHLMemImm1(nodeIs64Bit), SHLMemCL(nodeIs64Bit),
                        cg);
  } else if (shiftAmountNode->getOpCode().isLoadConst() &&
             (shiftAmount = integerConstNodeValue(shiftAmountNode, cg) &
                            shiftMask(nodeIs64Bit),
    ...
    generateRegisterShift(node, SHLRegImm1(nodeIs64Bit), SHLRegCL(nodeIs64Bit),
                          cg);
  }
  return node->getRegister();
}
...
OMR::X86::TreeEvaluator::integerRolEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    intptrj_t rotateAmount =
        integerConstNodeValue(secondChild, cg) & rotateMask(nodeIs64Bit);
    if (rotateAmount == 0) {
      targetRegister = cg->evaluate(firstChild);
    } else {
      targetRegister = intOrLongClobberEvaluate(firstChild, nodeIs64Bit, cg);
...
OMR::X86::TreeEvaluator::integerShrEvaluator(TR::Node *node,
  ...
  bool nodeIs64Bit = getNodeIs64Bit(node, cg);
  if (node->isDirectMemoryUpdate()) {
    generateMemoryShift(node, SARMemImm1(nodeIs64Bit), SARMemCL(nodeIs64Bit),
                        cg);
  } else {
    generateRegisterShift(node, SARRegImm1(nodeIs64Bit), SARRegCL(nodeIs64Bit),
                          cg);
  }
  return node->getRegister();
}
...
OMR::X86::TreeEvaluator::integerUshrEvaluator(TR::Node *node,
  ...
  if (node->isDirectMemoryUpdate()) {
    TR::Instruction *shiftInstruction = generateMemoryShift(
        node, SHRMemImm1(nodeIs64Bit), SHRMemCL(nodeIs64Bit), cg);
    if (shiftInstruction)
      faultingInstruction = shiftInstruction;
  } else {
    generateRegisterShift(node, SHRRegImm1(nodeIs64Bit), SHRRegCL(nodeIs64Bit),
                          cg);
  }
...
TR::Register *OMR::X86::TreeEvaluator::bshlEvaluator(TR::Node *node,
  ...
  if (testOpCode == TR::bconst &&
      performTransformation(comp, "O^O BSHLEvaluator: second child is not an "
                                  "8-bit signed, two's complement number: %x\n",
                            testOpCode)) {
    int32_t value = secondChild->getByte();
    if (isMemOp) {
      if (value != 0) {
...
TR::Register *OMR::X86::TreeEvaluator::sshlEvaluator(TR::Node *node,
  ...
  if (testOpcode == TR::iconst &&
      performTransformation(comp, "O^O SSHLEvaluator: second child is not a "
                                  "16-bit integer constant: %x\n",
                            testOpcode)) {
...
TR::Register *OMR::X86::TreeEvaluator::bshrEvaluator(TR::Node *node,
  ...
  TR::ILOpCodes testOpcode = secondChild->getOpCodeValue();
  if (testOpcode == TR::bconst &&
      performTransformation(comp, "O^O BSHREvaluator: second child is not an "
                                  "8-bit signed Two's complement opcode %x\n",
                            testOpcode)) {
    int32_t value = secondChild->getByte();
    if (value != 0) {
      if (isMemOp) {
...
TR::Register *OMR::X86::TreeEvaluator::sshrEvaluator(TR::Node *node,
  ...
  TR::ILOpCodes testOpcode = secondChild->getOpCodeValue();
  if (testOpcode == TR::iconst &&
      performTransformation(comp, "O^O SSHREvaluator: second child is not a "
                                  "16-bit signed two's complement number %x\n",
                            testOpcode)) {
    int32_t value = secondChild->getInt();
    if (value != 0) {
      if (isMemOp) {
...
TR::Register *OMR::X86::TreeEvaluator::bushrEvaluator(TR::Node *node,
  ...
  if (isMemOp) {
    ...
    if (firstChild->getReferenceCount() > 1) {
      ...
      tempMR = generateX86MemoryReference(firstChild, cg, false);
             ...
             performTransformation(
                 comp, "O^O BUSHREvaluator: first child is not an 8-bit signed "
                       "two's complement, or an 8 bit unsigned %x\n",
                 testOpcode1)) {
    targetRegister = cg->allocateRegister();
  ...
  if ((testOpcode2 == TR::bconst || testOpcode2 == TR::buconst) &&
      performTransformation(comp, "O^O BUSHREvaluator: first child is not an "
                                  "8-bit signed two's complement, or an 8 bit "
                                  "unsigned %x\n",
                            testOpcode2)) {
...
TR::Register *OMR::X86::TreeEvaluator::sushrEvaluator(TR::Node *node,
  ...
  TR::ILOpCodes testOpcode = secondChild->getOpCodeValue();
  if (testOpcode == TR::iconst &&
      performTransformation(comp, "O^O SUSHREvaluator: opcode is not a 16-bit "
                                  "signed two's complement %x\n",
                            testOpcode)) {
    int32_t value = secondChild->getInt();
    if (isMemOp) {
...
TR::Register *OMR::X86::TreeEvaluator::logicalEvaluator(TR::Node *node,
  ...
  TR::Register *testRegister = secondChild->getRegister();
  if (secondChild->getOpCode().isLoadConst() && testRegister == NULL &&
      performTransformation(comp, "O^O LogicalEvaluator: checking that the "
                                  "store has not happened yet. Target "
                                  "register: %x\n",
                            testRegister)) {
  ...
  if (secondChild->getOpCode().isLoadConst() && testRegister == NULL &&
      ...
      performTransformation(comp, "O^O checking that the store has not "
                                  "happened yet. Target register: %x\n",
                            testRegister)) {
...
TR::Register *OMR::X86::TreeEvaluator::bandEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister =
      logicalEvaluator(node, _logicalOpPackage[bandOpPackage], cg);
...
TR::Register *OMR::X86::TreeEvaluator::borEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Register *targetRegister =
      logicalEvaluator(node, _logicalOpPackage[borOpPackage], cg);
...
TR::Register *OMR::X86::TreeEvaluator::bxorEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister =
      logicalEvaluator(node, _logicalOpPackage[bxorOpPackage], cg);
