...
TR_OutlinedInstructions::TR_OutlinedInstructions(TR::LabelSymbol *entryLabel,
    ...
    : _entryLabel(entryLabel), _cg(cg), _firstInstruction(NULL),
      ...
      _rematerializeVMThread(false), _postDependencyMergeList(NULL),
...
TR_OutlinedInstructions::TR_OutlinedInstructions(TR::Node *callNode,
    ...
    : _restartLabel(restartLabel), _firstInstruction(NULL),
      ...
      _targetRegMovOpcode(MOVRegReg()), _hasBeenRegisterAssigned(false),
      _rematerializeVMThread(false), _postDependencyMergeList(NULL),
      _outlinedPathRegisterUsageList(NULL),
      _mainlinePathRegisterUsageList(NULL), _registerAssignerStateAtMerge(NULL),
      _cg(cg) {
  _entryLabel->setStartOfColdInstructionStream();
  _block = callNode->getOpCode().hasSymbolReference() &&
...
TR_OutlinedInstructions::TR_OutlinedInstructions(
    ...
    bool rematerializeVMThread, TR::CodeGenerator *cg)
    : _restartLabel(restartLabel), _firstInstruction(NULL),
      _appendInstruction(NULL), _targetReg(targetReg), _entryLabel(entryLabel),
      _targetRegMovOpcode(MOVRegReg()), _hasBeenRegisterAssigned(false),
...
TR_OutlinedInstructions::TR_OutlinedInstructions(
    TR::Node *callNode, TR::ILOpCodes callOp, TR::Register *targetReg,
    TR::LabelSymbol *entryLabel, TR::LabelSymbol *restartLabel,
    TR_X86OpCodes targetRegMovOpcode, TR::CodeGenerator *cg)
    : _restartLabel(restartLabel), _firstInstruction(NULL),
      _appendInstruction(NULL), _targetReg(targetReg), _entryLabel(entryLabel),
      _targetRegMovOpcode(targetRegMovOpcode), _hasBeenRegisterAssigned(false),
      _rematerializeVMThread(false), _postDependencyMergeList(NULL),
      _outlinedPathRegisterUsageList(NULL),
      _mainlinePathRegisterUsageList(NULL), _registerAssignerStateAtMerge(NULL),
      _cg(cg) {
...
void TR_OutlinedInstructions::generateOutlinedInstructionsDispatch() {
  ...
  if (_rematerializeVMThread) {
    ...
    TR::MemoryReference *vmThreadMR = generateX86MemoryReference(
        vmThreadReg, (TR::Compiler->target.is64Bit()) ? 16 : 8, _cg);
    generateRegMemInstruction(LRegMem(), _callNode, vmThreadReg, vmThreadMR,
                              _cg);
  }
  ...
  if (_targetReg) {
    ...
    if (targetRegPair) {
      TR_ASSERT(resultRegPair, "OutlinedInstructions: targetReg is a register "
                               "pair and resultReg is not");
      generateRegRegInstruction(_targetRegMovOpcode, _callNode,
      ...
      TR_ASSERT(!resultRegPair, "OutlinedInstructions: resultReg is a register "
                                "pair and targetReg is not");
      generateRegRegInstruction(_targetRegMovOpcode, _callNode, _targetReg,
                                resultReg, _cg);
    }
  }
...
// into a register.
// Ignore children that do not have a register since their live range should not
// persist outside of
...
void TR_OutlinedInstructions::assignRegisters(
  ...
  // captures all
  // current real register associations.  This is necessary to get the register
  // assigner
  ...
  //
  generateVFPRestoreInstruction(comp()->getAppendInstruction(),
    ...
    if (child->getRegister() != NULL) {
      ...
      if ((child->getOpCodeValue() == TR::loadaddr) &&
          /*(callNode->getOpCodeValue() == TR::instanceof ||
             ...
             callNode->getOpCodeValue() == TR::New || callNode->getOpCodeValue()
             == TR::anewarray)    &&*/
        ...
        // make it less so.  For example, this will catch
        // the case of an unevaluated argument not persisting outside of the
        // outlined region even though one of its subtrees will.
        //
        (void)_cg->evaluate(child);

        // Do not decrement the reference count here.  It will be decremented
        // when the call node is evaluated
