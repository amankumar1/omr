...
int32_t TR_X86SystemLinkage::computeMemoryArgSize(TR::Node *callNode,
...
TR_X86SystemLinkage::copyParametersToHomeLocation(TR::Instruction *cursor) {
  ...
  TR::RealRegister *framePointer =
      machine->getX86RealRegister(TR::RealRegister::vfp);
  ...
  TR_MovStatus movStatus[TR::RealRegister::NumRegisters] = {
    ...
    if (lri == NOT_LINKAGE) // It's on the stack
    {
      ...
      } else // Method body expects it to be in the ai register
      {
        ...
        loadCursor = generateRegMemInstruction(
            loadCursor, movOpcodes[RegMem][movDataType],
    ...
    } else // It's in a linkage register
    {
      ...
      if (ai == NOT_ASSIGNED || hasToBeOnStack(paramCursor)) {
        if (comp()->getOption(TR_TraceCG))
          traceMsg(comp(), "copyToHomeLocation param %p, linkage reg index %d, "
                           "allocated index %d, parameter offset %d, "
                           "hasToBeOnStack %d, parm->isParmHasToBeOnStack() "
                           "%d.\n",
        ...
        cursor = generateMemRegInstruction(
            cursor, movOpcodes[MemReg][movDataType],
            generateX86MemoryReference(framePointer, offset, cg()),
            machine->getX86RealRegister(sourceIndex), cg());
      }
    ...
    if (movStatus[paramReg].targetReg == 0) {
      ...
      while (movStatus[regCursor].sourceReg != noReg) {
        ...
        cursor = generateRegRegInstruction(
            cursor, movOpcodes[RegReg][movStatus[source].outgoingDataType],
            machine->getX86RealRegister(regCursor),
            machine->getX86RealRegister(source), cg());
...
TR_X86SystemLinkage::savePreservedRegisters(TR::Instruction *cursor) {
  // For IA32, if disableShrinkWrapping, usePushForPreservedRegs will be true;
  // otherwise false;
  //          For X64,  shrinkWraping is always on, and usePushForPreservedRegs
  //          always false;
  // TR_ASSERT(!getProperties().getUsesPushesForPreservedRegs(), "assertion
  // failure");
  TR::ResolvedMethodSymbol *bodySymbol = comp()->getJittedMethodSymbol();
  ...
  if (_properties.getUsesPushesForPreservedRegs()) {
    ...
    for (int32_t pindex =
      ...
      TR::RealRegister::RegNum idx =
          _properties.getPreservedRegister((uint32_t)pindex);
      TR::RealRegister *reg = machine()->getX86RealRegister(
          getProperties().getPreservedRegister((uint32_t)pindex));
      if (reg->getHasBeenAssignedInMethod() &&
          reg->getState() != TR::RealRegister::Locked) {
        if (!p || p->get(idx)) {
          cursor = generateMemRegInstruction(
              cursor, movOpcodes[MemReg][fullRegisterMovType(reg)],
...
void TR_X86SystemLinkage::createPrologue(TR::Instruction *cursor) {
#if defined(DEBUG)
  ...
  if (_properties.getOutgoingArgAlignment()) {
    ...
    TR_ASSERT(_properties.getOutgoingArgAlignment() == 16 ||
                  _properties.getOutgoingArgAlignment() == 4,
              "AMD64 SysV linkage require outgoingArgAlignment be 16/32 bytes "
              "aligned, while IA32 linkage require 4 bytes aligned.  We "
              "currently haven't support 32 bytes alignment for AMD64 SysV ABI "
              "yet.\n");
  ...
  if (comp()->getOption(TR_TraceCG)) {
    traceMsg(comp(), "create prologue using system linkage, after "
  ...
  debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(
      comp(), properties.getOffsetToFirstLocal() - localSize -
                  preservedRegsSize - adjust,
  ...
  if (cg()->pushPreservedRegisters()) {
    ...
    for (int32_t pindex = _properties.getMaxRegistersPreservedInPrologue() - 1;
      ...
      if (comp()->getOption(TR_TraceCG)) {
        traceMsg(
            comp(),
            "reg %d, getHasBeenAssignedInMethod %d, p %x, p->get(idx) %d.\n",
      ...
      if (reg->getHasBeenAssignedInMethod()) {
        if (!p || p->get(idx)) {
          cursor = generateRegMemInstruction(
              cursor, movOpcodes[RegMem][fullRegisterMovType(reg)], reg,
              generateX86MemoryReference(
                  machine()->getX86RealRegister(TR::RealRegister::vfp),
                  offsetCursor, cg()),
              cg());
...
void TR_X86SystemLinkage::createEpilogue(TR::Instruction *cursor) {
  TR::RealRegister *espReal =
      machine()->getX86RealRegister(TR::RealRegister::esp);
  TR::ResolvedMethodSymbol *bodySymbol = comp()->getJittedMethodSymbol();
  ...
  if (comp()->getOption(TR_TraceCG)) {
    traceMsg(comp(), "create epilogue using system linkage, after "
  ...
  if (_properties.getAlwaysDedicateFramePointerRegister()) {
    ...
    cursor = new (trHeapMemory()) TR::X86RegRegInstruction(
        ...
        machine()->getX86RealRegister(_properties.getFramePointerRegister()),
        cg());
    cursor = new (trHeapMemory()) TR::X86RegInstruction(
        cursor, POPReg,
        machine()->getX86RealRegister(_properties.getFramePointerRegister()),
  ...
  if (comp()->getOption(TR_TraceCG)) {
    traceMsg(comp(), "create epilogue using system linkage, after delocating "
                     "stack frame, cursor is %x.\n",
             cursor);
  }
...
void TR_X86SystemLinkage::copyLinkageInfoToParameterSymbols() {
  TR::ResolvedMethodSymbol *bodySymbol = comp()->getJittedMethodSymbol();
  ListIterator<TR::ParameterSymbol> paramIterator(
      &(bodySymbol->getParameterList()));
  const TR_X86LinkageProperties &properties = getProperties();
...
void TR_X86SystemLinkage::mapIncomingParms(TR::ResolvedMethodSymbol *method) {
...
void TR_X86SystemLinkage::copyGlRegDepsToParameterSymbols(
  ...
  if (bbStart->getNumChildren() > 0) {
    ...
    for (childNum = 0; childNum < glRegDeps->getNumChildren(); childNum++) {
      ...
      sym->setAllocatedIndex(
          cg->getGlobalRegister(child->getGlobalRegisterNumber()));
    }
  }
}

int32_t TR_X86SystemLinkage::getParameterStartingPos(int32_t &dataCursor,
                                                     uint32_t align) {
...
int32_t TR_X86SystemLinkage::layoutTypeOnStack(TR::DataType type,
                                               int32_t &dataCursor,
                                               parmLayoutResult &layoutResult) {
