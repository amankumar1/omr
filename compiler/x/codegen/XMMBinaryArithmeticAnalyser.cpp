...
static TR::Register *copyRegister(TR::Node *root, TR::Register *reg,
                                  TR::CodeGenerator *cg) {
  TR::Register *copyReg;
...
void TR_X86XMMBinaryArithmeticAnalyser::setInputs(
  ...
  if (firstChild->getReferenceCount() == 1) {
    ...
    if (!(_codeGen->useSSEForSinglePrecision() &&
          _codeGen->useSSEForDoublePrecision()) &&
...
uint8_t TR_X86XMMBinaryArithmeticAnalyser::getX86XMMOpPackage(TR::Node *node) {
  uint8_t package;
...
void TR_X86XMMBinaryArithmeticAnalyser::genericXMMAnalyser(TR::Node *root) {
  ...
  TR::Register *targetRegister = targetChild->getRegister(),
               *sourceRegister = sourceChild->getRegister(), *tempReg;
  TR::SymbolReference *targetSymRef = NULL, *sourceSymRef = NULL;
  ...
  //
  if ((isStrictFP(_compil->getCurrentMethod()) ||
       _compil->getOption(TR_StrictFP)) &&
  ...
  //
  if (targetRegister && targetRegister->getKind() == TR_X87 && sourceRegister &&
  ...
  if (isReversed() || (isCommutative() && canUseRegMemOp() && targetRegister &&
                       targetRegister->getKind() == TR_X87 &&
                       targetChild->getReferenceCount() == 1 &&
    ...
    if (sourceRegister->getKind() == TR_X87)
      sourceRegister = TR::TreeEvaluator::coerceFPRToXMMR(
          sourceChild, sourceRegister, _codeGen);
#ifdef ENABLE_FULL_STRICTFP
    ...
    if (mustUseRegRegOp()) {
      ...
      if (targetRegister->getKind() == TR_X87)
        targetRegister = TR::TreeEvaluator::coerceFPRToXMMR(
            targetChild, targetRegister, _codeGen);

      generateRegRegInstruction(getRegRegOp(), root, sourceRegister,
                                targetRegister, _codeGen);
      ...
      if (targetRegister && targetRegister->getKind() == TR_X87 &&
        ...
        if (targetRegister->isSinglePrecision()) {
          ...
          generateFPMemRegInstruction(FSTPMemReg, root, tempMR, targetRegister,
                                      _codeGen);
        } else {
          tempMR = _codeGen->machine()->getDummyLocalMR(TR::Double);
          generateFPMemRegInstruction(DSTPMemReg, root, tempMR, targetRegister,
                                      _codeGen);
        }
        ...
        TR_ASSERT(targetChild->getOpCode().isMemoryReference(),
                  "target operand should be a memory load\n");
        tempMR = generateX86MemoryReference(targetChild, _codeGen);
      }

      if (tempMR) {
        generateRegMemInstruction(getRegMemOp(), root, sourceRegister, tempMR,
                                  _codeGen);
    ...
    if (mustUseRegRegOp()) {
      ...
      generateRegRegInstruction(getRegRegOp(), root, targetRegister,
                                sourceRegister, _codeGen);
    } else if (canUseRegMemOp()) {
      TR::MemoryReference *tempMR;
      ...
      if (sourceRegister && sourceRegister->getKind() == TR_X87 &&
        ...
        if (sourceRegister->isSinglePrecision()) {
          tempMR = _codeGen->machine()->getDummyLocalMR(TR::Float);
          generateFPMemRegInstruction(FSTPMemReg, root, tempMR, sourceRegister,
                                      _codeGen);
        } else {
          tempMR = _codeGen->machine()->getDummyLocalMR(TR::Double);
          generateFPMemRegInstruction(DSTPMemReg, root, tempMR, sourceRegister,
                                      _codeGen);
        }
        tempMR = generateX86MemoryReference(*tempMR, 0, _codeGen);
        ...
        TR_ASSERT(sourceChild->getOpCode().isMemoryReference(),
                  "source operand should be a memory load\n");
        tempMR = generateX86MemoryReference(sourceChild, _codeGen);
      }

      if (tempMR) {
        generateRegMemInstruction(getRegMemOp(), root, targetRegister, tempMR,
                                  _codeGen);
        tempMR->decNodeReferenceCounts(_codeGen);
      } else {
        generateRegRegInstruction(getRegRegOp(), root, targetRegister,
...
const TR_X86OpCodes TR_X86XMMBinaryArithmeticAnalyser::_opCodePackage
    [kNumXMMArithPackages][kNumXMMArithVariants] = {
        // PACKAGE
        //        regReg        regMem
...
const uint8_t
    TR_X86XMMBinaryArithmeticAnalyser::_actionMap[NUM_XMM_ACTION_SETS] = {
        ...
        /* 0  0  0  0  0  1  0 */ kEvalTarget | kEvalSource | kCopyTarget |
            kRegReg,
        ...
        /* 0  0  1  0  0  1  0 */ kEvalTarget | kEvalSource | kCopyTarget |
            kRegReg,
        ...
        /* 1  0  0  0  0  1  0 */ kEvalTarget | kEvalSource | kCopyTarget |
            kRegReg,
        ...
        /* 1  0  1  0  0  1  0 */ kEvalTarget | kEvalSource | kCopyTarget |
            kRegReg,
