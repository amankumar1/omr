...
OMR::X86::MemoryReference::MemoryReference(TR::IA32DataSnippet *cds,
                                           TR::CodeGenerator *cg)
...
TR::IA32DataSnippet *OMR::X86::MemoryReference::getDataSnippet() {
  return (self()->hasUnresolvedDataSnippet() ||
          self()->hasUnresolvedVirtualCallSnippet())
             ? NULL
             : (TR::IA32DataSnippet *)_dataSnippet;
}
  ...
  if (_baseRegister != NULL) {
    if (_baseRegister ==
      ...
      if (assignedBaseRegister == NULL) {
        ...
        // copy using
        // generateX86MemoryReference(OMR::X86::MemoryReference  &, intptrj_t,
        // TR::CodeGenerator *cg).
  ...
  if (self()->getBaseRegister()) {
    ...
    if (registerNumber == TR::RealRegister::vfp) {
      TR_ASSERT(cg->machine()
                    ->getX86RealRegister(registerNumber)
...
OMR::X86::EnlargementResult OMR::X86::MemoryReference::enlarge(
  ...
  if (!self()->isForceWideDisplacement()) {
    ...
    if (potentialPatchGrowth > 0 &&
        ...
        performTransformation(cg->comp(), "O^O Enlarging memory reference by "
                                          "%d bytes by forcing wide "
                                          "displacement - allowpartial was %d",
...
void OMR::X86::MemoryReference::addMetaDataForCodeAddress(
  ...
  case 2: {
            ...
            NULL, cursor)) // might not point to beginning of class
    {
      cg->jitAdd32BitPicToPatchOnClassRedefinition(
          (void *)*(int32_t *)cursor, (void *)cursor,
      ...
      if (staticSym) {
        if (self()->getUnresolvedDataSnippet() == NULL) {
          if (symbol->isConst()) {
            ...
            {
              cg->jitAdd32BitPicToPatchOnClassRedefinition(
                  (void *)*(int32_t *)cursor, (void *)cursor, false);
            }
          } else {
            if (staticSym->isCountForRecompile()) {
              ...
              cg->addAOTRelocation(
                  new (cg->trHeapMemory())
                      TR_ExternalRelocation(cursor, 0, TR_BodyInfoAddress, cg),
                  __FILE__, __LINE__, node);
            } else if (staticSym->isGCRPatchPoint()) {
              TR_ExternalRelocation *r = new (cg->trHeapMemory())
                  TR_ExternalRelocation(cursor, 0, TR_AbsoluteMethodAddress,
                                        cg);
              ...
              cg->addAOTRelocation(
                  new (cg->trHeapMemory()) TR_ExternalRelocation(
                      cursor, (uint8_t *)&self()->getSymbolReference(),
                      node ? (uint8_t *)node->getInlinedSiteIndex()
                           : (uint8_t *)-1,
                      TR_DataAddress, cg),
                  __FILE__, __LINE__, node);
            }
  ...
  case 5: {
    ...
    if (!(displacement == 0 && !base->needsDisp() && !base->needsDisp() &&
      ...
      if (self()->getSymbolReference().getSymbol() &&
          self()->getSymbolReference().getSymbol()->isClassObject() &&
          cg->wantToPatchClassPointer(NULL, cursor)) // possibly unresolved, may
                                                     // not point to beginning
                                                     // of class
      {
        cg->jitAdd32BitPicToPatchOnClassRedefinition(
            (void *)*(int32_t *)cursor, (void *)cursor,
...
uint8_t *OMR::X86::MemoryReference::generateBinaryEncoding(
  ...
  case 1: {
    ...
    if (baseRegisterNumber == TR::RealRegister::vfp) {
      TR_ASSERT(
          cg->machine()
              ->getX86RealRegister(baseRegisterNumber)
              ->getAssignedRealRegister(),
      ...
      if (label != NULL) {
        if (TR::Compiler->target.is64Bit()) {
          ...
          *(int32_t *)cursor = -(int32_t)(intptrj_t)(
              cursor + 4 + TR_X86OpCode::getImmediateSize(
                               containingInstruction->getOpCodeValue()));
        } else {
...
static void rematerializeAddressAdds(TR::Node *rootLoadOrStore,
  ...
  // register
  // pressure. Use the existing register if one has been allocated for a
  // previous use of
...
TR::MemoryReference *generateX86MemoryReference(TR::IA32DataSnippet *cds,
                                                TR::CodeGenerator *cg) {
