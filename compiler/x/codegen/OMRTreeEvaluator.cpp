...
TR::Register *OMR::X86::TreeEvaluator::intOrLongClobberEvaluate(
  ...
  if (nodeIs64Bit) {
    TR_ASSERT(getNodeIs64Bit(node, cg),
...
TR::Instruction *OMR::X86::TreeEvaluator::compareGPMemoryToImmediate(
  ...
  bool is64Bit = TR::Compiler->target.is64Bit()
                     ? getNodeIs64Bit(node->getFirstChild(), cg)
                     : false;
...
TR::Instruction *OMR::X86::TreeEvaluator::insertLoadConstant(
  ...
  if (currentInstruction) {
    ...
    // checked
    // to ensure they don't modify any eflags needed by surrounding
    // instructions.
    //
    if ((value == 0 || value == -1)) {
...
TR::Register *OMR::X86::TreeEvaluator::loadConstant(
  ...
  TR::Instruction *instr =
      insertLoadConstant(node, targetRegister, value, type, cg);
...
// patchable
// instruction (such as a virtual guard nop) with a different alignment boundary
// size
...
TR::Register *OMR::X86::TreeEvaluator::performIload(
  ...
  TR::Register *reg = loadMemory(node, sourceMR, TR_RematerializableInt,
...
TR::Register *OMR::X86::TreeEvaluator::aloadEvaluator(TR::Node *node,
  ...
  TR::Register *reg = loadMemory(node, sourceMR, TR_RematerializableAddress,
                                 node->getOpCode().isIndirect(), cg);
...
TR::Register *OMR::X86::TreeEvaluator::bloadEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::MemoryReference *sourceMR = generateX86MemoryReference(node, cg);
  TR::Register *reg = loadMemory(node, sourceMR, TR_RematerializableByte,
                                 node->getOpCode().isIndirect(), cg);
...
TR::Register *OMR::X86::TreeEvaluator::sloadEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::MemoryReference *sourceMR = generateX86MemoryReference(node, cg);
  TR::Register *reg = loadMemory(node, sourceMR, TR_RematerializableShort,
                                 node->getOpCode().isIndirect(), cg);
...
OMR::X86::TreeEvaluator::integerStoreEvaluator(TR::Node *node,
  ...
  if (tempMR)
    ...
    // since this was not done by the child evaluator in its
...
OMR::X86::TreeEvaluator::arraycmpEvaluator(TR::Node *node,
  ...
  if (cg->getX86ProcessorInfo().supportsSSE2()) {
    return node->isArrayCmpLen() ? SSE2ArraycmpLenEvaluator(node, cg)
                                 : SSE2ArraycmpEvaluator(node, cg);
  }
  ...
  TR::Register *src1AddrReg = intOrLongClobberEvaluate(
      src1AddrNode, getNodeIs64Bit(src1AddrNode, cg), cg);
  TR::Register *src2AddrReg = intOrLongClobberEvaluate(
      src2AddrNode, getNodeIs64Bit(src2AddrNode, cg), cg);
  byteLenRegister = intOrLongClobberEvaluate(
      byteLenNode, getNodeIs64Bit(byteLenNode, cg), cg);

  byteLenRemainingRegister = cg->allocateRegister(TR_GPR);

  generateRegRegInstruction(MOVRegReg(), node, byteLenRemainingRegister,
                            byteLenRegister, cg);
  ...
  generateRegImmInstruction(CMPRegImms(), node, byteLenRemainingRegister,
                            byteLen, cg);
  generateLabelInstruction(JL4, node, residueLoopStartLabel, cg);

  generateRegImmInstruction(ADDRegImms(), node, byteLenRemainingRegister,
                            -byteLen, cg);
  ...
  generateRegImmInstruction(CMPRegImms(), node, byteLenRemainingRegister, 0,
                            cg);
  generateLabelInstruction(JE4, node, result3Label, cg);

  generateLabelInstruction(LABEL, node, residueLoopStartLabel, cg);

  generateRegMemInstruction((byteLen == 4) ? MOVZXReg4Mem1 : MOVZXReg8Mem1,
                            node, src1Reg,
                            generateX86MemoryReference(src1AddrReg, 0, cg), cg);
  generateRegMemInstruction(
      (byteLen == 4) ? MOVZXReg4Mem1 : MOVZXReg8Mem1, node, src2Reg,
      generateX86MemoryReference(src1AddrReg, src2AddrReg, 0, 0, cg), cg);
  ...
  generateRegImmInstruction(ADDRegImms(), node, byteLenRemainingRegister, -1,
                            cg);
  ...
  if (node->isArrayCmpLen()) {
    ...
    generateRegRegInstruction(SUBRegReg(), node, src1Reg,
                              byteLenRemainingRegister, cg);
    resultRegister = src1Reg;
  } else {
    generateRegImmInstruction(CMPRegImms(), node, byteLenRemainingRegister, 0,
                              cg);
    generateLabelInstruction(JNE4, node, result2Label, cg);
    generateRegImmInstruction(MOVRegImm4(), node, byteLenRemainingRegister, 0,
                              cg);
    generateLabelInstruction(JMP4, node, residueEndLabel, cg);
    generateLabelInstruction(LABEL, node, result2Label, cg);
    generateRegImmInstruction(MOVRegImm4(), node, byteLenRemainingRegister, 1,
                              cg);
    generateRegRegInstruction(CMPRegReg(), node, src1Reg, src2Reg, cg);
    generateLabelInstruction(JL4, node, residueEndLabel, cg);
    generateRegImmInstruction(MOVRegImm4(), node, byteLenRemainingRegister, 2,
                              cg);
  ...
  dependencies->addPostCondition(byteLenRemainingRegister,
  ...
  if ((byteLenRemainingRegister) &&
      (byteLenRemainingRegister != resultRegister)) {
...
TR::Register *OMR::X86::TreeEvaluator::SSE2IfArraycmpEvaluator(
    TR::Node *node, TR::LabelSymbol *targetLabel, bool branchonEqual,
    TR::CodeGenerator *cg) {
  ...
  generateRegMemInstruction(LEARegMem(), node, s1Reg,
                            generateX86MemoryReference(s1Reg, 0x10, cg), cg);
...
void genCodeToPerformLeftToRightAndBlockConcurrentOpIfNeeded(
    TR::Node *node, TR::MemoryReference *memRef, TR::Register *vReg,
    TR::Register *tempReg, TR::Register *tempReg1, TR::Register *tempReg2,
    TR::LabelSymbol *nonLockedOpLabel, TR::LabelSymbol *&opDoneLabel,
    TR::RegisterDependencyConditions *&deps, uint8_t size,
    TR::CodeGenerator *cg, bool isLoad, bool genOutOfline,
    bool keepValueRegAlive, TR::LabelSymbol *startControlFlowLabel) {}

void constLengthArrayCopy(TR::Node *node, TR::CodeGenerator *cg,
                          TR::Register *byteSrcReg, TR::Register *byteDstReg,
                          TR::Node *byteLenNode, bool preserveSrcPointer,
  ...
  static char *disableConstArrayCopyLoop =
      feGetEnv("TR_DisableConstArrayCopyLoop");

  uintptrj_t byteLength =
      TR::TreeEvaluator::integerConstNodeValue(byteLenNode, cg);
  TR::Compilation *comp = cg->comp();
  TR::RegisterDependencyConditions *deps = NULL;
  TR::LabelSymbol *overlapCopyLabel = NULL;

  // 160-byte copies occur in console workloads dealing with 80-character
  // strings, so
  ...
  if (node->isForwardArrayCopy() && byteLength >= 64 && byteLength <= 160) {
    struct TR_FancyConstLengthStrategy {
      ...
      int8_t getLatency(TR_X86ProcessorInfo &p) {
        return (p.isIntelCore2() || p.isIntelNehalem() || p.isIntelWestmere() ||
                p.isIntelSandyBridge() || p.isAMD15h())
                   ? _core2Latency
                   : p.isAMDOpteron() ? _opteronLatency : _pentium4Latency;
      }
    };
    static TR_FancyConstLengthStrategy fancyConstLengthStrategies[] = {
        {"none"},
        {"4-byte mov", L4RegMem, S4MemReg, 2, TR_GPR, 3, 3, 3, 4},
        {"8-byte mov", L8RegMem, S8MemReg, 3, TR_GPR, 3, 3, 3,
         4}, // Only valid when is64BitTarget
        {"movq", MOVQRegMem, MOVQMemReg, 3, TR_FPR, 4, 4, 4,
         4}, // Not the latencies from the proc manuals... beyond 4 regs it
    ...
    if (p.isIntelSandyBridge() || p.isIntelWestmere() || p.isAMD15h())
      stratNum = 4;
             ...
             (liveGPRs->getNumberOfLiveRegisters() <
              (15 - fancyConstLengthStrategies[2].getLatency(p))))
      stratNum = 2;
    ...
    if ((stratNum > 0) && ((byteLength & strideMask) == 0) &&
        performTransformation(
            comp, "O^O FANCY CONST LENGTH ARRAYCOPY: Copy %d bytes\n",
            byteLength)) {
      ...
      int32_t numIterations =
          numCopies / numRegs; // An "iteration" is a sequence of loads and
                               // stores using all the registers.  Has nothing
                               // to do with loops (though it is true that a
                               // loop, if generated, does exactly one
                               // iteration).
      int32_t residue = numCopies - numIterations * numRegs;

      if (comp->getOption(TR_TraceCG)) {
        traceMsg(comp,
                 "   strategy:%d (%s) stride:%d bytes:%d architecture:%x\n",
                 stratNum, strat->_name, strat->_stride, 1 << strat->_stride,
                 p.getX86Architecture());
        traceMsg(comp,
                 "   copies:%d regs:%d ramp:%d iterations:%d residue:%d\n",
      ...
      for (copyNum = 0; copyNum < ramp; copyNum++) {
        ...
        generateRegMemInstruction(
            strat->_loadOpCode, node, regs[regIndex],
            generateX86MemoryReference(byteSrcReg, copyNum << strat->_stride,
                                       cg),
            cg);
      }
      ...
      //
      // DISABLED becuase it doesn't handle the internal control flow properly
      // yet
      //
      ...
      for (copyNum = ramp; copyNum < numCopies; copyNum++) {
        ...
        generateMemRegInstruction(
            strat->_storeOpCode, node,
            generateX86MemoryReference(byteDstReg, inductionReg, 0,
                                       (copyNum - numRegs) << strat->_stride,
                                       cg),
            regs[regIndex], cg);
        generateRegMemInstruction(
            strat->_loadOpCode, node, regs[regIndex],
            generateX86MemoryReference(byteSrcReg, inductionReg, 0,
                                       copyNum << strat->_stride, cg),
            cg);
      }
      ...
      if (inductionReg) {
        ...
        generateRegImmInstruction(opCode, node, inductionReg,
                                  inductionIncrement, cg);
      ...
      if (ramp < numRegs) {
        ...
        // Just do the stores corresponding to the loads we actually did in the
        // partial ramp-up.
        //
        for (copyNum = 0; copyNum < ramp; copyNum++) {
          regIndex = copyNum % numRegs;
          generateMemRegInstruction(
              strat->_storeOpCode, node,
              generateX86MemoryReference(byteDstReg, copyNum << strat->_stride,
                                         cg),
              regs[regIndex], cg);
        }
      } else {
        // We completed the ramp-up, then did zero or more iterations.
        // The idea here is to pretend we're going to keep on copying, but only
        // do the stores.
        //
  ...
  if (useSSECopy && byteLength > 48) {
    ...
    TR::MemoryReference *leaMR =
        generateX86MemoryReference(byteDstReg, byteSrcReg, 0, cg);
    generateRegRegInstruction(SUBRegReg(nodeIs64Bit), node, byteDstReg,
                              byteSrcReg, cg);
    generateRegImmInstruction(TESTRegImm4(nodeIs64Bit), node, byteDstReg, 0xf,
                              cg);

    // Restore contents of byteDstReg without modifying any flags.
    generateRegMemInstruction(LEARegMem(nodeIs64Bit), node, byteDstReg, leaMR,
                              cg);
    ...
    TR::X86ImmSymInstruction *callInstr = generateHelperCallInstruction(
        node, TR_IA32forwardSSEArrayCopyNoAlignCheck, sseDeps, cg);
  ...
  TR_ASSERT(!(srcIsEsp && dstIsEsp),
            "Copying to ESP from ESP is not supported");
  ...
  bool srcPtrChanged =
      true; // assume that the source and destination pointers get changed
  ...
  if (wordCount <= 3) {
    ...
    while (wordCount--) {
      TR::MemoryReference *tempSrcMR = generateX86MemoryReference(
          byteSrcReg,
          constWordSize * (originalWordCount - wordCount) - constWordSize, cg);
      generateRegMemInstruction(LRegMem(), node, lenReg, tempSrcMR, cg);
      TR::MemoryReference *tempDstMR = generateX86MemoryReference(
          byteDstReg,
          constWordSize * (originalWordCount - wordCount) - constWordSize, cg);
    ...
    if (reportConstArrayCopy)
      diagnostic("constArrayCopy: found one inline with bytelength = %d in "
                 "method %s\n",
                 byteLength, comp->signature());
  } else if (!disableConstArrayCopyLoop && wordCount < 64 &&
             (byteSrcReg != byteDstReg)) // word count must be > 1 or pipelined
                                         // loop below is bad
  {
    TR_ASSERT(
        wordCount > 1,
        "Const Arraycopy tried to do pipelined loop with word count <= 1");
    lenReg = TR::TreeEvaluator::loadConstant(
        byteLenNode, wordCount, TR_RematerializableInt, cg, lenReg);
    if (!temp)
    ...
    generateRegImmInstruction(ADDRegImms(), node,
                              (srcIsOffset) ? byteDstReg : byteSrcReg,
                              constWordSize, cg);
    ...
    // add base to offset, so that both src and dst point to the end of their
    // arrays
    // and, if we need to preserve the pointer, subtract the # bytes moved
    if (srcIsOffset && preserveSrcPointer) {
      generateRegMemInstruction(LEARegMem(), node, byteSrcReg,
                                generateX86MemoryReference(byteDstReg,
                                                           byteSrcReg, 0,
                                                           -numBytesMoved, cg),
                                cg);
      srcPtrChanged = false;
    ...
    {
      generateRegMemInstruction(LEARegMem(), node, byteDstReg,
                                generateX86MemoryReference(byteDstReg,
                                                           byteSrcReg, 0,
                                                           -numBytesMoved, cg),
                                cg);
    ...
    if (reportConstArrayCopy)
      diagnostic(
          "constArrayCopy: found one loop with bytelength = %d in method %s\n",
          byteLength, comp->signature());
  } else {
    TR_RematerializableTypes type = byteLenNode->getOpCode().isLong()
                                        ? TR_RematerializableLong
                                        : TR_RematerializableInt;
    lenReg = TR::TreeEvaluator::loadConstant(byteLenNode, wordCount, type, cg,
                                             lenReg);

    // we can't modify the value of esp. Unless, of course, you want _weird_
    // data corruption.
    if (srcIsEsp) {
    ...
    if (reportConstArrayCopy)
      diagnostic("constArrayCopy: found one repmovs%s with bytelength = %d in "
                 "method %s\n",
                 TR::Compiler->target.is64Bit() ? "q" : "d", byteLength,
                 comp->signature());
  }
  ...
  TR_ASSERT(residue < 8,
            "Residue (%d) should be < 8, otherwise its not residue", residue);

  if (residue >= 4) {
    if (!lenReg)
      lenReg = cg->allocateRegister();

    TR::MemoryReference *loadRef =
        generateX86MemoryReference(byteSrcReg, srcOffset, cg);
    generateRegMemInstruction(L4RegMem, node, lenReg, loadRef, cg);
    TR::MemoryReference *storeRef =
        generateX86MemoryReference(byteDstReg, dstOffset, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::constLengthArrayCopyEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  ...
  if (isPrimitiveCopy) {
    ...
    TR_ASSERT(node->isNoArrayStoreCheckArrayCopy(),
              "expecting arraycopy not requiring an array store check");
  ...
  TR_ASSERT(byteLenNode->getOpCode().isLoadConst(),
            "expecting constant length arraycopy");
  ...
  constLengthArrayCopy(node, cg, byteSrcReg, byteDstReg, byteLenNode,
                       preserveSrcPointer, preserveDstPointer);
...
TR::Register *OMR::X86::TreeEvaluator::compressStringEvaluator(
    TR::Node *node, TR::CodeGenerator *cg, bool japaneseMethod) {
...
TR::Register *OMR::X86::TreeEvaluator::compressStringNoCheckEvaluator(
    TR::Node *node, TR::CodeGenerator *cg, bool japaneseMethod) {
  TR::Node *srcObjNode, *dstObjNode, *startNode, *lengthNode;
  TR::Register *srcObjReg, *dstObjReg, *lengthReg, *startReg;
  bool stopUsingCopyReg1, stopUsingCopyReg2, stopUsingCopyReg3,
  ...
  TR_RuntimeHelper helper;
  if (TR::Compiler->target.is64Bit())
    helper = japaneseMethod ? TR_AMD64compressStringNoCheckJ
                            : TR_AMD64compressStringNoCheck;
  else
    helper = japaneseMethod ? TR_IA32compressStringNoCheckJ
                            : TR_IA32compressStringNoCheck;
...
TR::Register *
OMR::X86::TreeEvaluator::andORStringEvaluator(TR::Node *node,
                                              TR::CodeGenerator *cg) {
...
OMR::X86::TreeEvaluator::OverflowCHKEvaluator(TR::Node *node,
  ...
  bool nodeIs64Bit =
      TR::Compiler->target.is32Bit() ? false : getNodeIs64Bit(operand1, cg);
  ...
  case TR::lmul:
    ...
    // TR_X86BinaryCommutativeAnalyser::longMultiplyAnalyser
    // Therefore the usual way of only detecting the OF for the last instruction
    // of sequence won't work for this case.
    // The implementation needs to detect OF flags after all the instructions
    // involving higher parts of the registers for
  ...
  TR::list<TR::CFGEdge *> excepSucc = cg->getCurrentEvaluationTreeTop()
                                          ->getEnclosingBlock()
  ...
  // TODO: there is still a chance that the flags might still be avaiable and we
  // could detect it and avoid repeating
  // the operantion
  {
  ...
  generateLabelInstruction(
      JO4, node, overflowCatchBlock->getEntry()->getNode()->getLabel(), cg);
  cg->setVMThreadRequired(false);
...
OMR::X86::TreeEvaluator::arraycopyEvaluator(TR::Node *node,
  ...
  bool isArrayStoreCheckUnnecessary =
      isPrimitiveCopy || node->isNoArrayStoreCheckArrayCopy();
  bool shortCopy = true;

  static char *disableConstantLengthArrayCopy =
      feGetEnv("TR_disableConstantLengthArrayCopy");
  ...
  static char *disableOptSeverCheckForRepeatMove =
      feGetEnv("TR_disableOptSeverCheckForRepeatMove");
  ...
  if (caller) {
    m = comp->fe()->createResolvedMethod(
        cg->trMemory(), caller,
        node->getSymbolReference()->getOwningMethod(comp));
  }
  ...
  if (m &&
      (m->getRecognizedMethod() == TR::java_util_Arrays_copyOf_byte ||
       ...
       m->getRecognizedMethod() ==
           TR::java_io_ObjectInputStream_BlockDataInputStream_read)) {
    useSSECopy = true;
  }
#endif

  // disable SSE arraycopy for AMD64 but use it for aggressive liberty mode and
  // for hot compiles
  if (TR::Compiler->target.is64Bit()) {
    if (comp->getOptLevel() >= hot || disableOptSeverCheckForRepeatMove ||
        !comp->isOptServer())
      useSSECopy = true;
  ...
  if (!useSSECopy &&
      (comp->isOptServer() && !disableOptSeverCheckForRepeatMove)) {
    shortCopy = false;
  }
  ...
  if (isArrayStoreCheckUnnecessary && !shortCopy &&
      node->isForwardArrayCopy() &&
      (node->isWordElementArrayCopy() || node->isHalfWordElementArrayCopy()) &&
      !useSSECopy) {
    intptrj_t size = 0;
    if (byteLenNode && byteLenNode->getRegister() == NULL &&
  ...
  if (byteLenNode->getReferenceCount() > 1) {
    ...
    generateRegRegInstruction(MOVRegReg(), node, copyByteLenReg, byteLenReg,
                              cg);
  ...
  if (byteSrcNode->getReferenceCount() > 1) {
    if (byteSrcReg->containsInternalPointer()) {
      ...
      copyByteSrcReg->setPinningArrayPointer(
          byteSrcReg->getPinningArrayPointer());
      copyByteSrcReg->setContainsInternalPointer();
    } else
      copyByteSrcReg = cg->allocateCollectedReferenceRegister();

    generateRegRegInstruction(MOVRegReg(), node, copyByteSrcReg, byteSrcReg,
                              cg);
  ...
  if (byteDstNode->getReferenceCount() > 1) {
    if (byteDstReg->containsInternalPointer()) {
      ...
      copyByteDstReg->setPinningArrayPointer(
          byteDstReg->getPinningArrayPointer());
      copyByteDstReg->setContainsInternalPointer();
    } else
      copyByteDstReg = cg->allocateCollectedReferenceRegister();

    generateRegRegInstruction(MOVRegReg(), node, copyByteDstReg, byteDstReg,
                              cg);
  ...
  if (node->isHalfWordElementArrayCopy() && shortCopy &&
  ...
  dependencies =
      generateRegisterDependencyConditions((uint8_t)0, numPostDeps, cg);
  ...
  if (node->isForwardArrayCopy()) {
    ...
    if (useSSECopy) {
      ...
      dependencies->stopAddingConditions();
      if (TR::Compiler->target.is64Bit())
        generateHelperCallInstruction(
            node, TR_AMD64SSEforwardArrayCopyAggressive, dependencies, cg);
      else {
        generateHelperCallInstruction(
            node, TR_IA32SSEforwardArrayCopyAggressive, dependencies, cg);
      }
      cg->stopUsingRegister(tempReg);
      ...
      if (!shortCopy) {
        ...
        // TODO: need an absolute relocation on the table base address and then
        // AOT will work
        //
    ...
    } else { // doing byte array copy or 8 byte reference copy on 64bit
      dependencies->stopAddingConditions();
      TR_X86ProcessorInfo p = cg->getX86ProcessorInfo();
      // TR_ASSERT(shortCopy, "can't use long copy assumption here, code was
      // invalid for 64bit reference copies");
      if (!shortCopy) {
        ...
        helper = (p.isAMDOpteron() || p.isAMD15h())
                     ? TR_IA32SSEforwardArrayCopyAMDOpteron
                     : TR_IA32SSEforwardArrayCopy;
      else
    ...
    if (shortCopy && !generateInlineJumpTable && !useSSECopy) {
      generateHelperCallInstruction(node, helper, dependencies, cg)
          ->setAdjustsFramePointerBy(-argSize);
      ...
      TR::SymbolReference *helperSymRef =
          cg->symRefTab()->findOrCreateRuntimeHelper(helper, false, false,
                                                     false);

      TR::X86HelperCallSnippet *snippet = new (cg->trHeapMemory())
          TR::X86HelperCallSnippet(cg, node, restartLabel, snippetLabel,
                                   helperSymRef);
      ...
      generateRegMemInstruction(
          LEARegMem(), node, byteDstReg,
          generateX86MemoryReference(byteDstReg, byteLenReg, 0, cg), cg);

      generateRegMemInstruction(
          LEARegMem(), node, byteSrcReg,
          generateX86MemoryReference(byteSrcReg, byteLenReg, 0, cg), cg);

      int32_t scale = TR::Compiler->target.is64Bit() ? 3 : 2;

      // TODO: need an absolute relocation on the table base address
      //
      TR::MemoryReference *mr = generateX86MemoryReference(
          NULL, byteLenReg, scale, (intptrj_t)&fwdHalfWordCopyTable, cg);

      if (TR::Compiler->target.is64Bit()) {
        TR::Register *addressReg = mr->getAddressRegister();
        if (addressReg) {
          dependencies->addPostCondition(addressReg, TR::RealRegister::NoReg,
                                         cg);
        }
      }
    ...
    TR::Register *xmm0Register = NULL, *xmm1Register = NULL,
                 *xmm2Register = NULL, *xmm3Register = NULL;
    if (useSSECopy) {
    ...
    if (!shortCopy && !(elementSize == 8 && TR::Compiler->target.is32Bit())) {
      ...
      generateRegMemInstruction(
          LEARegMem(), node, byteDstReg,
          generateX86MemoryReference(byteDstReg, byteSrcReg, 0, cg), cg);
    ...
    if (elementSize == 8) {
      ...
      helper = TR::Compiler->target.is64Bit()
                   ? (useSSECopy) ? TR_AMD64wordArrayCopyAggressive
                                  : TR_AMD64wordArrayCopy
                   : (useSSECopy) ? TR_IA32wordArrayCopyAggressive
                                  : TR_IA32wordArrayCopy;
    } else if (elementSize == 4) {
      if (!shortCopy) {
        generateRegImmInstruction(SHRRegImm1(), node, byteLenReg, 2, cg);
        generateInstruction(REPMOVSD, node, dependencies, cg);
      ...
      helper = TR::Compiler->target.is64Bit()
                   ? (useSSECopy) ? TR_AMD64halfWordArrayCopyAggressive
                                  : TR_AMD64halfWordArrayCopy
                   : (useSSECopy) ? TR_IA32halfWordArrayCopyAggressive
                                  : TR_IA32halfWordArrayCopy;
    } else {
      if (!shortCopy) {
        generateInstruction(REPMOVSB, node, dependencies, cg);
      }
      helper =
          TR::Compiler->target.is64Bit()
              ? (false ? TR_AMD64BCarrayCopy : (useSSECopy)
                                                   ? TR_AMD64arrayCopyAggressive
                                                   : TR_AMD64arrayCopy)
              : (useSSECopy) ? TR_IA32arrayCopyAggressive : TR_IA32arrayCopy;
    }

    TR_ASSERT((helperCallLabel != NULL) ==
                  (!shortCopy &&
                   !(elementSize == 8 && TR::Compiler->target.is32Bit())),
              "arraycopy evaluator control flow disaster!");
    TR_OutlinedInstructions *outlinedCall = NULL;
    if (helperCallLabel != NULL) {
      outlinedCall =
          new (cg->trHeapMemory()) TR_OutlinedInstructions(helperCallLabel, cg);
  ...
  // If we made copies of the derived registers, these copies are now dead. At
  // this point,
  // copy{ByteSrc,ByteDst,ByteLen,Dst}Reg == {byteSrc,byteDst,byteLen,dst}Reg,
  // respectively.
  //
  if (copyByteSrcReg)
...
TR::Register *
OMR::X86::TreeEvaluator::reverseLoadEvaluator(TR::Node *node,
                                              TR::CodeGenerator *cg) {
...
TR::Register *
OMR::X86::TreeEvaluator::reverseStoreEvaluator(TR::Node *node,
                                               TR::CodeGenerator *cg) {
...
TR::Register *
OMR::X86::TreeEvaluator::arraytranslateAndTestEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
...
OMR::X86::TreeEvaluator::arraytranslateEvaluator(TR::Node *node,
  ...
  bool stopUsingCopyReg1 =
      stopUsingCopyRegAddr(node->getChild(0), srcPtrReg, cg);
  bool stopUsingCopyReg2 =
      stopUsingCopyRegAddr(node->getChild(1), dstPtrReg, cg);
  bool stopUsingCopyReg4 =
      stopUsingCopyRegInteger(node->getChild(3), termCharReg, cg);
  bool stopUsingCopyReg5 =
      stopUsingCopyRegInteger(node->getChild(4), lengthReg, cg);
  TR::Register *resultReg = cg->allocateRegister();
...
#ifdef J9_PROJECT_SPECIFIC
TR::Register *
OMR::X86::TreeEvaluator::encodeUTF16Evaluator(TR::Node *node,
                                              TR::CodeGenerator *cg) {
  // tree looks like:
  ...
  bool bigEndian = symbol->getRecognizedMethod() ==
                   TR::com_ibm_jit_JITHelpers_transformedEncodeUTF16Big;

  // Set up register dependencies
  const int gprClobberCount = 2;
  const int maxFprClobberCount = 5;
  const int fprClobberCount =
      bigEndian ? 5 : 4; // xmm4 only needed for big-endian
...
OMR::X86::TreeEvaluator::arraysetEvaluator(TR::Node *node,
  ...
  TR::Register *addressReg =
      intOrLongClobberEvaluate(addressNode, TR::Compiler->target.is64Bit(), cg);
  TR::Register *valueReg = cg->evaluate(valueNode);
  TR::Register *sizeReg =
      intOrLongClobberEvaluate(sizeNode, getNodeIs64Bit(sizeNode, cg), cg);
  ...
  if (valueNode->getSize() == 8 && TR::Compiler->target.is32Bit()) {
    ...
    switch (valueReg->getKind()) {
    case TR_GPR:
      generateRegRegInstruction(MOVRegReg(valueNode->getSize() == 8), node, EAX,
                                valueReg, cg);
      break;
    case TR_FPR:
      generateRegRegInstruction(valueNode->getSize() == 8 ? MOVQReg8Reg
                                                          : MOVDReg4Reg,
                                node, EAX, valueReg, cg);
      break;
    default:
      TR_ASSERT(0, "Arrayset Evaluator: unsupported register type");
      break;
...
bool OMR::X86::TreeEvaluator::getNodeIs64Bit(TR::Node *node,
   ...
   * code
   * to be reused, almost unchanged, to do the corresponding 64-bit logic on
   * AMD64.
   * It compiles away to nothing on IA32, thus preserving performance and code
   * size
...
OMR::X86::TreeEvaluator::integerConstNodeValue(TR::Node *node,
  ...
  if (getNodeIs64Bit(node, cg)) {
    return (intptrj_t)node->getLongInt(); // Cast to satisfy 32-bit compilers,
                                          // even though they never take this
                                          // path
  } else {
    TR_ASSERT(node->getSize() <= 4, "For efficiency on IA32, only call "
                                    "integerConstNodeValue for 32-bit "
                                    "constants");
    return node->getInt();
  }
}
...
static TR::Register *inlineSinglePrecisionSQRT(TR::Node *node,
  ...
  if (opRegister->getKind() == TR_FPR) {
    ...
    generateRegRegInstruction(SQRTSFRegReg, node, targetRegister, opRegister,
                              cg);
  } else {
...
OMR::X86::TreeEvaluator::directCallEvaluator(TR::Node *node,
  ...
  case TR::java_nio_Bits_keepAlive: {
    TR_ASSERT(node->getNumChildren() == 1,
              "keepAlive is assumed to have just one argument");
    ...
    new (cg->trHeapMemory())
        TR::X86LabelInstruction(LABEL, node, generateLabelSymbol(cg), deps, cg);
    cg->decReferenceCount(node->getFirstChild());
  ...
  if (cg->enableAESInHardwareTransformations() &&
      (symbol->getRecognizedMethod() ==
           TR::com_ibm_jit_crypto_JITAESCryptInHardware_doAESInHardware ||
       symbol->getRecognizedMethod() ==
           TR::com_ibm_jit_crypto_JITAESCryptInHardware_expandAESKeyInHardware)) {
    return TR::TreeEvaluator::VMAESHelperEvaluator(node, cg);
  }

  if (symbol->getMandatoryRecognizedMethod() ==
          TR::com_ibm_jit_JITHelpers_transformedEncodeUTF16Big ||
      symbol->getMandatoryRecognizedMethod() ==
          TR::com_ibm_jit_JITHelpers_transformedEncodeUTF16Little) {
  ...
  if (cg->getSupportsBDLLHardwareOverflowCheck() &&
    ...
    // Eat this call as its only here to anchor where a long lookaside overflow
    // check
  ...
  //
      ...
      symbol->isVMInternalNative() || symbol->isJITInternalNative() ||
      ...
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicMarkableReference_doubleWordCAS) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicStampedReference_doubleWordCAS) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicMarkableReference_doubleWordSet) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicStampedReference_doubleWordSet) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicMarkableReference_doubleWordCASSupported) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicStampedReference_doubleWordCASSupported) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicMarkableReference_doubleWordSetSupported) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicStampedReference_doubleWordSetSupported) ||
      ...
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicInteger_getAndAdd) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicInteger_getAndIncrement) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicInteger_getAndDecrement) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicInteger_getAndSet) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicInteger_addAndGet) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicInteger_decrementAndGet) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicInteger_incrementAndGet) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicReference_getAndSet) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicIntegerArray_getAndAdd) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicIntegerArray_getAndIncrement) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicIntegerArray_getAndDecrement) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicIntegerArray_getAndSet) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicIntegerArray_addAndGet) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicIntegerArray_decrementAndGet) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicIntegerArray_incrementAndGet) ||
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicReferenceArray_getAndSet) ||
      (symbol->getRecognizedMethod() ==
      ...
      (symbol->getRecognizedMethod() ==
       TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndSet) ||
      (symbol->getRecognizedMethod() ==
    ...
    return compressStringEvaluator(node, cg, useJapaneseCompression);
             ...
             TR::java_lang_String_compressNoCheck) {
...
TR::Register *
OMR::X86::TreeEvaluator::ifInstanceOfHelper(TR::Node *node,
                                            TR::CodeGenerator *cg) {
...
OMR::X86::TreeEvaluator::loadaddrEvaluator(TR::Node *node,
  ...
  TR::Register *targetRegister =
      generateLEAForLoadAddr(node, memRef, symRef, cg, false);
...
OMR::X86::TreeEvaluator::iRegStoreEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() && node->getDataType() == TR::Int32) {
    ...
    // zero-extending
    // at iRegStores, we are free to setUpperBitsAreZero at every iRegLoad, and
    // lots
    // of goodness results.
    //
...
TR::Register *OMR::X86::TreeEvaluator::BBStartEvaluator(TR::Node *node,
  ...
  if (!block->isExtensionOfPreviousBlock()) {
    ...
    if (comp->getOption(TR_DisableLateEdgeSplitting)) {
      // Check the cfg edge for the mustRestoreVMThreadRegister attribute. If
      // markked then
      // there can be only one incoming edge.
      TR::CFGEdge *edge = (!block->getPredecessors().empty())
                              ? block->getPredecessors().front()
                              : NULL;
      if (cg->getSupportsVMThreadGRA() && edge &&
          edge->mustRestoreVMThreadRegister()) {
        ...
        if (node->getNumChildren() > 0) {
          ...
          glRegDeps = generateRegisterDependencyConditions(
              node->getFirstChild(), cg, 1, &popRegisters);
        } else
          glRegDeps =
        ...
        inst = new (cg->trHeapMemory())
            TR::X86LabelInstruction(LABEL, node, label, glRegDeps, cg);
        cg->stopUsingRegister(dummyReg);
      } else {
        if (node->getNumChildren() > 0)
          inst = generateLabelInstruction(LABEL, node, label,
                                          node->getFirstChild(), &popRegisters,
                                          true, cg);
        else
          inst =
              generateLabelInstruction(LABEL, node, node->getLabel(), true, cg);
      }
    } else {
      bool needVMThreadDep = !performTransformation(
          comp, "O^O LATE EDGE SPLITTING: Omit ebp dependency for %s node %s\n",
          node->getOpCode().getName(), cg->getDebug()->getName(node));
      if (node->getNumChildren() > 0)
        inst =
            generateLabelInstruction(LABEL, node, label, node->getFirstChild(),
                                     &popRegisters, needVMThreadDep, cg);
      else
        inst = generateLabelInstruction(LABEL, node, node->getLabel(),
                                        needVMThreadDep, cg);
    }
  ...
  TR::Instruction *fence = generateFenceInstruction(
      FENCE, node, TR::Node::createRelative32BitFenceNode(
  ...
  if (comp->getOption(TR_BreakBBStart)) {
    ...
    generateRegImmInstruction(
        TEST4RegImm4, node, machine->getX86RealRegister(TR::RealRegister::esp),
        block->getNumber(), cg);
...
TR::Register *OMR::X86::TreeEvaluator::BBEndEvaluator(TR::Node *node,
  ...
  if (!nextTT || !nextTT->getNode()->getBlock()->isExtensionOfPreviousBlock()) {
    ...
    bool needVMThreadDep =
        ...
        !performTransformation(
            comp,
            "O^O LATE EDGE SPLITTING: Omit ebp dependency for %s node %s\n",
            node->getOpCode().getName(), cg->getDebug()->getName(node));
...
TR::Register *OMR::X86::TreeEvaluator::icmpsetEvaluator(TR::Node *node,
  ...
  TR::Register *compareReg =
      intOrLongClobberEvaluate(compareValue, nodeIs64Bit, cg);
...
void TR_X86ComputeCC::bitwise32(TR::Node *node, TR::Register *ccReg,
                                TR::Register *target, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::TreeEvaluator::VMarrayStoreCheckArrayCopyEvaluator(
    TR::Node *, TR::CodeGenerator *) {
  return 0;
}

TR::Register *
OMR::X86::TreeEvaluator::VMAESHelperEvaluator(TR::Node *node,
                                              TR::CodeGenerator *cg) {
  return 0;
}
...
TR::Register *
OMR::X86::TreeEvaluator::VMifInstanceOfEvaluator(TR::Node *,
                                                 TR::CodeGenerator *) {
  return 0;
}

TR::Register *
OMR::X86::TreeEvaluator::VMifArrayCmpEvaluator(TR::Node *,
                                               TR::CodeGenerator *) {
  return 0;
}
