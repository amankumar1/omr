 ...
 * the operands
 * are given explicitly by the caller and are not the first and second child of
 * the given root node
 *
 * \param root
 *     the check node
...
void TR_X86BinaryCommutativeAnalyser::genericLongAnalyser(
  ...
  // their
  // high order word as a zero. Examples are xu2l conversion operators, right
  // unsigned shifts by 32 or more bits
  // and just things that value propagation has found are in the range of an
  // unsigned int.
  // we can avoid the evaluation when we don't need the result of such nodes for
  // another parent.
  //
...
void TR_X86BinaryCommutativeAnalyser::longDualMultiplyAnalyser(TR::Node *root) {
  ...
  generateRegRegInstruction(ADC4RegReg, root, r3, edx,
                            _cg); //  17  ADC    R3, EDX     // R3 =
                                  //  (al * bh)l + (ah * bh)l + (ah * bl)h +
                                  //  [carry]
  generateRegImmInstruction(ADC4RegImms, root, r4, 0, _cg); //  18  ADC    R4, 0
                                                            //  // R4 =
                                                            //  (ah * bh)h
                                                            //  + [carry]
...
void TR_X86BinaryCommutativeAnalyser::longMultiplyAnalyser(TR::Node *root) {
  ...
  if (getOpReg1Reg2()) {
    ...
    // instructon between Al and Bl
    //
    // both high zero: compute                               Ah:Al=Al*Bl
    // (clobber Ah,Al)
    // Ah zero:        compute Bh=Bh*Al,                     Ah:Al=Al*Bl,
    // Ah=Ah+Bh  (clobber Ah,Al,Bh)
    // Bh zero:        compute Ah=Ah*Bl,                     Bh:Bl=Bl*Al,
    // Bh=Bh+Ah  (clobber Ah,Bh,Bl)
    // neither zero:   compute Ah=Ah*Bl, Bh=Bh*Al, Bh=Bh+Ah, Ah:Al=Al*Bl,
    // Ah=Ah+Bh  (clobber Ah,Bh,Al)
