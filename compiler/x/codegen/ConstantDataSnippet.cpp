...
TR::IA32ConstantDataSnippet::IA32ConstantDataSnippet(TR::CodeGenerator *cg,
                                                     TR::Node *n, void *c,
                                                     uint8_t size)
    : TR::IA32DataSnippet(cg, n, c, size) {}

void TR_Debug::print(TR::FILE *pOutFile, TR::IA32ConstantDataSnippet *snippet) {
  ...
  trfprintf(pOutFile, "%s \t%s",
            snippet->getConstantSize() == 8
                ? dqString()
                : (snippet->getConstantSize() == 4 ? ddString() : dwString()),
            hexPrefixString());
