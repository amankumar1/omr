...
int32_t memoryBarrierRequired(TR_X86OpCode &op, TR::MemoryReference *mr,
  ...
  // otherwise.
  // The memory barrier will be removed by PicBuilder when the reference is
  // resolved and
...
int32_t OMR::X86::Instruction::estimateBinaryLength(int32_t currentEstimate) {
  ...
  self()->setEstimatedBinaryLength(
      self()->getOpCode().getOpCodeLength() + self()->rexPrefixLength() +
...
int32_t TR::X86PatchableCodeAlignmentInstruction::betterPadLength(
  ...
  TR_ASSERT(result < getBoundarySpacing(), "Patchable code has no viable "
                                           "padding that will let it be "
                                           "updated atomically");
  return result;
}
      ...
      if (distance >= -128 && distance <= 127 && getOpCode().isBranchOp() &&
        ...
        if (label->getCodeLocation() != NULL) {
          *(int8_t *)cursor = (int8_t)distance;
          ...
          cg()->addRelocation(
              new (cg()->trHeapMemory())
                  TR_8BitLabelRelativeRelocation(cursor, label));
        ...
        if (getOpCode().isShortBranchOp()) {
          ...
          //
          TR_ASSERT(0, "short form branch displacement too large: instr=%p, "
                       "distance=%d\n",
                    this, distance);
...
TR::X86LabelInstruction::enlarge(int32_t requestedEnlargementSize,
  ...
  // the binary encoding will, usually try to use the smallest instruction it
  // can
...
int32_t TR::X86LabelInstruction::estimateBinaryLength(int32_t currentEstimate) {
  ...
  if (getOpCode().isBranchOp()) {
    ...
    if (getOpCode().hasIntImmediate()) {
      ...
      if (getLabelSymbol() && getLabelSymbol()->getEstimatedCodeLocation()) {
        ...
        if (distance >= -128 && distance < 0 && _permitShortening) {
          immediateLength = 0; // really 1, but for conditional branches (all
                               // excep JMP4) the opcode entry will be 1 too big
                               // for short branch
          // because the short branch op is 1 byte, but the long branch op for
          // conditionals is 2
    ...
    setEstimatedBinaryLength(getOpCode().getOpCodeLength() + rexPrefixLength() +
                             immediateLength);
  } else if (getOpCodeValue() == LABEL) {
    if (!comp->getOption(TR_DisableLateEdgeSplitting) &&
        _symbol->getVMThreadRestoringLabel() && _symbol->isVMThreadLive()) {
      // If this label is the second of a split pair, and vmThread is live at
      // this label,
      ...
      TR::RealRegister *vmThreadRealReg = cg()->machine()->getX86RealRegister(
          cg()->getProperties().getMethodMetaDataRegister());
      if (vmThreadRealReg->getHasBeenAssignedInMethod() ||
          !performTransformation(
              comp, "O^O LATE EDGE SPLITTING: Skip vmThread reload at %p "
                    "because ebp was never assigned\n",
              this)) // TODO: could we use getModified instead?
      {
        ...
        TR::Instruction *ebpLoad = generateRegMemInstruction(
            getPrev(), LRegMem(), vmThreadRealReg,
            generateX86MemoryReference(
                vmThreadVirtualReg->getBackingStorage()->getSymbolReference(),
                cg()),
            cg());
        currentEstimate = ebpLoad->estimateBinaryLength(currentEstimate);
        if (cg()->getTraceRAOption(TR_TraceRALateEdgeSplitting))
          traceMsg(comp, "LATE EDGE SPLITTING: Added vmThread "
                         "rematerialization %s before label %s\n",
                   cg()->getDebug()->getName(ebpLoad),
                   cg()->getDebug()->getName(_symbol));

        TR::Instruction *bump =
            cg()->generateDebugCounter(ebpLoad, "cg.lateSplitEdges:reloaded", 1,
...
int32_t TR::X86RestoreVMThreadInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
...
int32_t TR::X86VirtualGuardNOPInstruction::estimateBinaryLength(
  ...
  setEstimatedBinaryLength(
      TR::Compiler->target.is64Bit() ? 5 : 6); // TODO:AMD64: What if patched
                                               // instruction needs a Rex?  Is 6
                                               // enough?
  return currentEstimate + getEstimatedBinaryLength();
}
...
uint8_t *TR::X86VirtualGuardNOPInstruction::generateBinaryEncoding() {
  ...
  // patch point
  // so setup the patching location to use this previous guard and generate no
  // instructions
  // ourselves
  ...
  // is the Profield Guard conditional jump. Because the HCR Guard jumps to the
  // same destination
  // we are guaranteed that the patch site will be big enough for the required
  // jump and so we
  ...
  // require atomic
  // nop sequences - all other guards require a single instruction to prevent
  ...
  if (instToBePatched && _nopSize > 0 &&
    ...
    if (enlargement.getPatchGrowth() + next >= _nopSize) {
      ...
      TR_ASSERT(enlargement.getPatchGrowth() == 0,
                "Instructions should only partially enlarge when dealing with "
                "an HCR guard - any other partial enlargement is a waste of "
                "code cache!");
    }
  ...
  if (getOpCode().hasIntImmediate()) {
    ...
    if (std::find(cg()->comp()->getStaticHCRPICSites()->begin(),
      ...
      cg()->jitAdd32BitPicToPatchOnClassRedefinition(
          ((void *)getSourceImmediateAsAddress()), (void *)cursor);
    }
  }
}
...
int32_t TR::X86ImmInstruction::estimateBinaryLength(int32_t currentEstimate) {
  ...
  setEstimatedBinaryLength(getOpCode().getOpCodeLength() + prefixLength +
                           rexPrefixLength() + immediateLength);
    ...
    if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
      ...
      if (!(resolvedMethod &&
        ...
        if (labelSym) {
          cg()->addRelocation(
              new (cg()->trHeapMemory())
                  TR_32BitLabelRelativeRelocation(cursor, labelSym));
        } else {
      ...
      cg()->addAOTRelocation(
          new (cg()->trHeapMemory()) TR_ExternalRelocation(
              ...
              getNode() ? (uint8_t *)getNode()->getInlinedSiteIndex()
                        : (uint8_t *)-1,
              TR_ConstantPool, cg()),
          __FILE__, __LINE__, getNode());
      ...
      if (symbol->isConst()) {
        ...
        if (staticSym && staticSym->isCompiledMethod())
          cg()->addAOTRelocation(
              new (cg()->trHeapMemory())
                  TR_ExternalRelocation(cursor, 0, TR_RamMethod, cg()),
              __FILE__, __LINE__, getNode());
...
uint8_t *TR::X86ImmSymInstruction::generateBinaryEncoding() {
  ...
  if (getOpCode().hasIntImmediate()) {
    ...
    if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
      ...
      if (TR::Compiler->target.is64Bit() && comp->getCodeCacheSwitched() &&
      ...
      if (resolvedMethod &&
        ...
        if (TR::Compiler->target.is64Bit()) {
          ...
          TR_ASSERT(IS_32BIT_RIP(start, cursor + 4),
        ...
        if (!labelSym) {
          ...
          // way in which CALL
          // instructions are created.  On 32-bit the target is always reachable
          // from the call
          // site, so the actual 32-bit target address is encoded in the
          // immediate field of
          // the instruction.  There is no need to ask the symbol reference for
          // the actual address
          ...
          // target symbol
          // must be queried to determine the actual target address and then
          // whether it is with
          // the 32-bit displacement range.  Hence, 64-bit relies on a valid
          // target symbol to find
          // the actual target address.
          //
          // Not all CALL instructions have symbol references that describe the
          // actual target.
          // Profiled targets in static IPICs are an example.  The handshake
          // between instruction
          // selection and binary encoding is dubious, and the safe thing to do
          // at this stage is
          // to revert back to the original logic in this method on 32-bit.
          // Such CALL instructions
          ...
          if (TR::Compiler->target.is64Bit()) {
            ...
            // pointing to the RAM method.
            // Otherwise, the symbol method address points at the actual target
            // address.
            //
          ...
          if (methodSym && methodSym->isHelper()) {
            if (!IS_32BIT_RIP(targetAddress, cursor + 4) ||
                forceTrampolineUse) {
              // TODO:AMD64: Consider AOT ramifications
              targetAddress = cg()->fe()->indexedTrampolineLookup(
                  getSymbolReference()->getReferenceNumber(), (void *)cursor);
              TR_ASSERT(
                  IS_32BIT_RIP(targetAddress, cursor + 4),
            ...
            if (!IS_32BIT_RIP(targetAddress, cursor + 4) ||
              ...
              TR_ASSERT(
                  IS_32BIT_RIP(targetAddress, cursor + 4),
                  "Local method trampoline must be reachable directly.\n");
            }
          }
        }
      }
          ...
          if (sym->isStatic()) {
            ...
            *(int32_t *)cursor =
                (int32_t)
                    TR::Compiler->cls.persistentClassPointerFromClassPointer(
                        cg()->comp(),
                        (TR_OpaqueClassBlock *)getSourceImmediate());
          }
        }
      }
    }
...
uint8_t TR::X86RegInstruction::getBinaryLengthLowerBound() {
  ...
  uint8_t prefix =
      opCode.needs16BitOperandPrefix() ? 1 : opCode.needsScalarPrefix() ? 1 : 0;
  uint8_t opCodeLength =
      (opCode.isAMD64DeprecatedInc() || opCode.isAMD64DeprecatedDec())
          ? 2
          : opCode.getOpCodeLength();
...
int32_t TR::X86RegInstruction::estimateBinaryLength(int32_t currentEstimate) {
  // *this    swipeable for debugging purposes
  TR_X86OpCode &opCode = getOpCode();
  uint8_t prefixLength = (opCode.needsSSE42OpcodePrefix())
                             ? 2
                             : opCode.needs16BitOperandPrefix()
                                   ? 1
                                   : opCode.needsScalarPrefix() ? 1 : 0;
...
TR::X86RegInstruction::enlarge(int32_t requestedEnlargementSize,
  ...
  if (disableRexExpansion ||
      TR::comp()->getOption(TR_DisableZealousCodegenOpts))
    return OMR::X86::EnlargementResult(0, 0);
  ...
  if (TR::Compiler->target.is64Bit() &&
      ...
      performTransformation(comp, "O^O Enlarging instruction %p by %d bytes by "
  ...
  if (getOpCode().needsScalarPrefix()) {
    if (getOpCode().singleFPOp()) {
      *cursor++ = IA32ScalarSSEPrefix;
      ...
      TR_ASSERT(getOpCode().doubleFPOp(),
                "generateBinaryEncoding() ==> scalar prefix not applicable\n");
...
void TR::X86RegImmInstruction::addMetaDataForCodeAddress(uint8_t *cursor) {
  ...
  if (getOpCode().hasIntImmediate()) {
      ...
      void *classPointer =
          (void *)cg()
              ...
              ->createResolvedMethod(
                  cg()->trMemory(),
                  (TR_OpaqueMethodBlock *)getSourceImmediateAsAddress(),
    ...
    case TR_HEAP_BASE_FOR_BARRIER_RANGE:
      cg()->addAOTRelocation(new (cg()->trHeapMemory()) TR_ExternalRelocation(
                                 cursor, (uint8_t *)TR_HeapBaseForBarrierRange0,
                                 TR_GlobalValue, cg()),
                             __FILE__, __LINE__, getNode());
      break;
    case TR_HEAP_SIZE_FOR_BARRIER_RANGE:
      cg()->addAOTRelocation(new (cg()->trHeapMemory()) TR_ExternalRelocation(
                                 cursor, (uint8_t *)TR_HeapSizeForBarrierRange0,
                                 TR_GlobalValue, cg()),
                             __FILE__, __LINE__, getNode());
      break;
    ...
    if (cg()->needClassAndMethodPointerRelocations()) {
      ...
      *(int32_t *)cursor =
          (int32_t)TR::Compiler->cls.persistentClassPointerFromClassPointer(
              cg()->comp(), (TR_OpaqueClassBlock *)getSourceImmediate());
      cg()->addAOTRelocation(
          new (cg()->trHeapMemory()) TR_ExternalRelocation(
              cursor, (uint8_t *)getSymbolReference(),
              getNode() ? (uint8_t *)getNode()->getInlinedSiteIndex()
...
uint8_t TR::X86MemInstruction::getBinaryLengthLowerBound() {
  ...
  uint8_t length = getOpCode().needs16BitOperandPrefix()
                       ? 1
                       : getOpCode().needsScalarPrefix() ? 1 : 0;
...
int32_t TR::X86MemInstruction::estimateBinaryLength(int32_t currentEstimate) {
  int32_t barrier =
      memoryBarrierRequired(getOpCode(), getMemoryReference(), cg(), false);
  int32_t length = getOpCode().needs16BitOperandPrefix()
                       ? 1
                       : getOpCode().needsScalarPrefix() ? 1 : 0;
  ...
  setEstimatedBinaryLength(getOpCode().getOpCodeLength() + length +
                           rexPrefixLength() + patchBoundaryPadding);
...
void TR::AMD64RegImm64Instruction::addMetaDataForCodeAddress(uint8_t *cursor) {
  ...
  if (comp->fej9()->helpersNeedRelocation()) {
    if (getNode()->getOpCode().hasSymbolReference() && methodSymRef &&
      ...
      case TR_ACTIVE_CARD_TABLE_BASE:
        cg()->addAOTRelocation(new (cg()->trHeapMemory()) TR_ExternalRelocation(
                                   cursor, (uint8_t *)TR_ActiveCardTableBase,
                                   TR_GlobalValue, cg()),
                               __FILE__, __LINE__, getNode());
        break;
      case TR_ClassAddress:
...
uint8_t *TR::AMD64RegImm64Instruction::generateBinaryEncoding() {
  ...
  TR_ASSERT(!getOpCode().needs16BitOperandPrefix(),
            "Imm64 instructions must have 64-bit operands");
  ...
  if (getSymbolReference()->getSymbol()->isLabel()) {
    ...
    case TR_DataAddress: {
      cg()->addAOTRelocation(
          new (cg()->trHeapMemory()) TR_ExternalRelocation(
              cursor, (uint8_t *)getSymbolReference(),
              (uint8_t *)getNode() ? (uint8_t *)getNode()->getInlinedSiteIndex()
                                   : (uint8_t *)-1,
              (TR_ExternalRelocationTargetKind)getReloKind(), cg()),
          __FILE__, __LINE__, getNode());
