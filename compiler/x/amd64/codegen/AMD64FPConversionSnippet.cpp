...
uint8_t *TR::AMD64FPConversionSnippet::genFPConversion(uint8_t *buffer) {
  ...
  // This didn't end up as clean as I thought.  TODO:AMD64: Separate out the
  // 64-bit code into another class.
  ...
  TR_ASSERT(properties.getIntegerReturnRegister() == TR::RealRegister::eax,
            "Only support integer return in eax");

  if (targetReg != TR::RealRegister::eax) {
    // MOV R, rax
    //
    *buffer++ = TR::RealRegister::REX | TR::RealRegister::REX_W |
                targetRegister->rexBits(TR::RealRegister::REX_R, false);
    *buffer++ = 0x8b;
    *buffer = 0xc0;
  ...
  TR_ASSERT(properties.getFloatArgumentRegister(0) == TR::RealRegister::xmm0,
            "Only support 1st FP arg in xmm0");
  ...
  if (sourceReg != TR::RealRegister::xmm0) {
    TR_ASSERT(TR::Compiler->target.is64Bit(),
              "This push sequence only works on AMD64");
  ...
  if (sourceReg != TR::RealRegister::xmm0) {
    TR_ASSERT(TR::Compiler->target.is64Bit(),
              "This pop sequence only works on AMD64");
  ...
  if (targetReg != TR::RealRegister::eax) {
    ...
    //
    *buffer++ = TR::RealRegister::REX | TR::RealRegister::REX_W |
                targetRegister->rexBits(TR::RealRegister::REX_B, false);
    *buffer = 0x90;
...
// Cheat: should compute if Rex is required, but instead we just check if it's
// there
#define IS_REX(byte) (((byte)&0xf0) == TR::RealRegister::REX)

void TR_Debug::print(TR::FILE *pOutFile,
                     TR::AMD64FPConversionSnippet *snippet) {
  ...
  TR::RealRegister *sourceRegister =
      toRealRegister(snippet->getConvertInstruction()->getSourceRegister());
  TR::RealRegister *targetRegister =
  ...
  TR::ILOpCodes opCode =
      snippet->getConvertInstruction()->getNode()->getOpCodeValue();
  ...
  if (treg != TR::RealRegister::eax) {
    ...
    trfprintf(pOutFile, "\t%s preserve helper return reg", commentString());
    bufferPos += instrSize;
  }
  ...
  if (sreg != TR::RealRegister::xmm0) {
    ...
    trfprintf(pOutFile, "movsd\t[rsp], xmm0\t%s save xmm0", commentString());
    bufferPos += 9;
    int instrSize = IS_REX(*bufferPos) ? 5 : 4;
    printPrefix(pOutFile, NULL, bufferPos, instrSize);
    trfprintf(pOutFile, "movsd\txmm0, ");
    print(pOutFile, sourceRegister, TR_QuadWordReg);
    trfprintf(pOutFile, "\t%s load parameter", commentString());
    bufferPos += instrSize;
  }
  ...
  if (sreg != TR::RealRegister::xmm0) {
    printPrefix(pOutFile, NULL, bufferPos, 5);
    trfprintf(pOutFile, "movsd\txmm0, [rsp]\t%s restore xmm0", commentString());
    printPrefix(pOutFile, NULL, bufferPos, 4);
...
uint32_t
TR::AMD64FPConversionSnippet::getLength(int32_t estimatedSnippetStart) {
  // *this    swipeable for debugging purposes
  uint32_t length = 11;
  TR::Machine *machine = cg()->machine();
  ...
  TR::X86RegRegInstruction *instr =
      _convertInstruction->getIA32RegRegInstruction();
  TR_ASSERT(instr != NULL, "f2i conversion instruction must be either L4RegMem "
                           "or CVTTSS2SIRegReg\n");
  TR::RealRegister *sourceRegister = toRealRegister(instr->getSourceRegister());
  if (sourceRegister->getRegisterNumber() != TR::RealRegister::xmm0) {
    length += sizeof(pushBinary) + sizeof(popBinary) // push and pop
              + 4 + (sourceRegister->rexBits(TR::RealRegister::REX_B, false)
                         ? 1
                         : 0) // MOVSD xmm0, source
        ;
  }
