...
TR_AMD64Win64FastCallLinkage::TR_AMD64Win64FastCallLinkage(
    TR::CodeGenerator *cg)
...
TR_AMD64ABILinkage::TR_AMD64ABILinkage(TR::CodeGenerator *cg)
  ...
  _properties._properties =
      EightBytePointers | EightByteParmSlots | CallerCleanup |
      IntegersInRegisters | LongsInRegisters | FloatsInRegisters |
      ReservesOutgoingArgsInPrologue | AlwaysDedicateFramePointerRegister;
...
TR::Register *TR_AMD64SystemLinkage::buildVolatileAndReturnDependencies(
  ...
  for (i = 0; i < getProperties().getNumVolatileRegisters(); i++) {
    ...
    if (regIndex != returnRegIndex) {
      ...
      // also volatile
      // in the caller's linkage, then that flag doesn't matter much anyway.  If
      // it's preserved
      // in the caller's linkage, then we don't want to set that flag because we
      // want this
      // use of the register to count as a "real" use, thereby motivating the
      // prologue to
  ...
  for (i = 0; i < getProperties().getNumberOfPreservedGPRegisters(); i++) {
    ...
    // Note that we don't setPlaceholderReg here.  If this volatile reg is also
    // volatile
    // in the caller's linkage, then that flag doesn't matter much anyway.  If
    // it's preserved
    // in the caller's linkage, then we don't want to set that flag because we
...
//
int32_t
TR_AMD64SystemLinkage::buildArgs(TR::Node *callNode,
  ...
  TR::RealRegister *espReal =
      machine()->getX86RealRegister(TR::RealRegister::esp);
  ...
  for (i = first; i != last; i += direction) {
    ...
    if (rregIndex != noReg) {
      ...
      // a different register than the child.  In that case it is not necessary
      // to copy the temporary scratch
      ...
      if ((child->getReferenceCount() > 1) && (vreg == child->getRegister())) {
        ...
        generateRegRegInstruction(movOpcodes[RegReg]
                                            [movType(child->getDataType())],
      ...
      generateMemRegInstruction(
          movOpcodes[MemReg][fullRegisterMovType(vreg)], child,
...
TR::Register *TR_AMD64SystemLinkage::buildIndirectDispatch(TR::Node *callNode) {
  TR::SymbolReference *methodSymRef = callNode->getSymbolReference();
...
#if defined(PYTHON) && 0
  // Treat all preserved GP regs as volatile until register map support
  // available.
  //
...
TR::Register *TR_AMD64SystemLinkage::buildDirectDispatch(TR::Node *callNode,
                                                         bool spillFPRegs) {
  TR::SymbolReference *methodSymRef = callNode->getSymbolReference();
  TR::MethodSymbol *methodSymbol =
      methodSymRef->getSymbol()->castToMethodSymbol();
...
#if defined(PYTHON) && 0
  // For Python, store the instruction that contains the GC map at this site
  // into
  // the frame object.
  //
  TR::SymbolReference *frameObjectSymRef =
      comp()->getSymRefTab()->findOrCreateAutoSymbol(
          comp()->getMethodSymbol(), 0, TR::Address, true, false, true);

  TR::Register *frameObjectRegister = cg()->allocateRegister();
  generateRegMemInstruction(L8RegMem, callNode, frameObjectRegister,
  ...
  generateRegMemInstruction(LEA8RegMem, callNode, gcMapPCRegister,
  ...
  // should be chosen.
  //
  TR::RegisterDependencyConditions *gcMapDeps =
      generateRegisterDependencyConditions(0, 2, cg());
  ...
  generateMemRegInstruction(
      S8MemReg, callNode,
      generateX86MemoryReference(frameObjectRegister,
                                 fe()->getPythonGCMapPCOffsetInFrame(), cg()),
      gcMapPCRegister, gcMapDeps, cg());
  ...
  if (methodSymbol->getMethodAddress()) {
    ...
    generateRegImm64Instruction(MOV8RegImm64, callNode, scratchReg,
...
uint32_t TR_AMD64SystemLinkage::getAlignment(TR::DataType type) {
  switch (type) {
  case TR::Float:
    return 4;
  case TR::Double:
...
void TR_AMD64ABILinkage::mapIncomingParms(TR::ResolvedMethodSymbol *method,
                                          uint32_t &stackIndex) {
  ...
  for (parmCursor = parameterIterator.getFirst(); parmCursor;
    ...
    if ((parmCursor->getLinkageRegisterIndex() >= 0) &&
        (parmCursor->getAllocatedIndex() < 0 || hasToBeOnStack(parmCursor))) {
      ...
      if (comp()->getOption(TR_TraceCG))
        traceMsg(comp(), "mapIncomingParms setParameterOffset %d for param "
                         "symbol (reg param without home location) %p, "
                         "hasToBeOnStack() %d\n",
      ...
      if (comp()->getOption(TR_TraceCG))
        traceMsg(comp(), "mapIncomingParms no need to set parm %p, for it has "
                         "got register %d assigned\n",
                 parmCursor, parmCursor->getAllocatedIndex());
    }
  }
}

bool TR_AMD64SystemLinkage::layoutTypeInRegs(TR::DataType type,
                                             uint16_t &intArgs,
                                             uint16_t &floatArgs,
                                             parmLayoutResult &layoutResult) {
...
int32_t TR_AMD64SystemLinkage::layoutParm(TR::Node *parmNode,
...
int32_t TR_AMD64SystemLinkage::layoutParm(TR::ParameterSymbol *parmSymbol,
                                          int32_t &dataCursor, uint16_t &intReg,
                                          uint16_t &floatReg,
                                          parmLayoutResult &layoutResult) {
  // AMD64 SysV ABI:  if the size of an object is larger than four eightbytes,
...
void TR_AMD64SystemLinkage::setUpStackSizeForCallNode(TR::Node *node) {
  ...
  if (sizeOfOutGoingArgs > cg()->getLargestOutgoingArgSize()) {
    ...
    if (comp()->getOption(TR_TraceCG))
      traceMsg(comp(), "setUpStackSizeForCallNode setLargestOutgoingArgSize "
                       "%d(for call node %p)\n",
  ...
  if (alignment > _properties.getOutgoingArgAlignment()) {
    ...
    if (comp()->getOption(TR_TraceCG))
      traceMsg(comp(), "setUpStackSizeForCallNode setOutgoingArgAlignment "
                       "%d(for call node %p)\n",
               alignment, node);
  }
}
