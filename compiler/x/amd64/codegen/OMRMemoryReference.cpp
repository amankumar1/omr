...
OMR::X86::AMD64::MemoryReference::MemoryReference(TR::IA32DataSnippet *cds,
                                                  TR::CodeGenerator *cg)
    ...
    if (self()->getUnresolvedDataSnippet()) {
        ...
        if (cg->needClassAndMethodPointerRelocations())
          cg->addAOTRelocation(
              new (cg->trHeapMemory()) TR_ExternalRelocation(
                  displacementLocation, (uint8_t *)&_symbolReference,
                  (uint8_t *)containingInstruction->getNode()
                      ->getInlinedSiteIndex(),
                  TR_ClassAddress, cg),
      ...
      if (cg->needRelocationsForStatics())
        cg->addAOTRelocation(new (cg->trHeapMemory())
                                 TR_ExternalRelocation(displacementLocation, 0,
                                                       TR_BodyInfoAddress, cg),
                             __FILE__, __LINE__,
      ...
      if (cg->needRelocationsForStatics()) {
        ...
        cg->addAOTRelocation(r, __FILE__, __LINE__,
                             containingInstruction->getNode());
      }
...
uint8_t *OMR::X86::AMD64::MemoryReference::generateBinaryEncoding(
  ...
  if (comp->getOption(TR_TraceCG)) {
    diagnostic(
        "OMR::X86::AMD64::MemoryReference " POINTER_PRINTF_FORMAT
        " with address register %s in instruction " POINTER_PRINTF_FORMAT "\n",
        this, _addressRegister
  ...
  // See x86-64 Architecture Programmer's Manual, Volume 3, sections 1.1 and
  // 1.7.1.
  //
  intptrj_t rip =
      (intptrj_t)(modRM + 5) +
      TR_X86OpCode::getImmediateSize(containingInstruction->getOpCodeValue());
  ...
  if (self()->getDataSnippet() || self()->getLabel()) {
    ...
    TR_ASSERT(_addressRegister != NULL, "OMR::X86::AMD64::MemoryReference "
                                        "should have allocated an address "
                                        "register");
    ...
    if (_symbolReference.getSymbol()) {
      ...
      //
      TR::SymbolReference *symRef = new (cg->trHeapMemory())
    ...
    self()->addMetaDataForCodeAddressWithLoad(displacementLocation,
                                              containingInstruction, cg);
    ...
    self()->setUnresolvedDataSnippet(NULL); // Otherwise it will get damaged
                                            // when we re-emit
                                            // containingInstruction
    ...
    if (self()->getUnresolvedDataSnippet()) {
      ...
      traceMsg(
          comp, "found unresolved shadow with NULL base object : data "
                "reference instruction=%p, cursor=%p\n",
