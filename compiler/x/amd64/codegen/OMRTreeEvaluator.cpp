...
OMR::X86::AMD64::TreeEvaluator::aconstEvaluator(TR::Node *node,
  ...
  TR::Register *targetRegister =
      loadConstant(node, node->getLongInt(), TR_RematerializableAddress, cg);
...
OMR::X86::AMD64::TreeEvaluator::lconstEvaluator(TR::Node *node,
                                                TR::CodeGenerator *cg) {
  TR::Register *targetRegister =
      loadConstant(node, node->getLongInt(), TR_RematerializableLong, cg);
...
OMR::X86::AMD64::TreeEvaluator::lloadEvaluator(TR::Node *node,
  ...
  TR::Register *reg = loadMemory(node, sourceMR, TR_RematerializableLong,
  ...
  if (node->getFirstChild()->getOpCode().isLoadConst()) {
    ...
    if (node->isNonNegative() ||
        (node->skipSignExtension() &&
         performTransformation(comp, "TREE EVALUATION: skipping sign extension "
                                     "on node %s despite lack of "
                                     "isNonNegative\n",
...
OMR::X86::AMD64::TreeEvaluator::iu2lEvaluator(TR::Node *node,
  ...
  if (node->getFirstChild()->getOpCode().isLoadConst()) {
    ...
    return conversionAnalyser(node, L4RegMem, MOVZXReg8Reg4,
...
OMR::X86::AMD64::TreeEvaluator::dbits2lEvaluator(TR::Node *node,
  ...
  if (node->normalizeNanValues()) {
    ...
    if (disableFastNormalizeNaNs) {
      ...
      TR::IA32ConstantDataSnippet *nan1Snippet =
          cg->findOrCreate8ByteConstant(node, DOUBLE_NAN_1_LOW);
      TR::IA32ConstantDataSnippet *nan2Snippet =
          cg->findOrCreate8ByteConstant(node, DOUBLE_NAN_2_LOW);
      ...
      TR::IA32ConstantDataSnippet *nanDetectorSnippet =
          cg->findOrCreate8ByteConstant(node, nanDetector);
      TR::MemoryReference *nanDetectorMR =
          generateX86MemoryReference(nanDetectorSnippet, cg);
...
//
TR::Register *
OMR::X86::AMD64::TreeEvaluator::ifInstanceOfHelper(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
