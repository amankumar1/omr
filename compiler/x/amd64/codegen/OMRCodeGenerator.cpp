...
OMR::X86::AMD64::CodeGenerator::CodeGenerator() : OMR::X86::CodeGenerator() {
  ...
  // the
  // runtime comparison does not sign extend (saves an instruction on array
  // allocations).
...
int32_t OMR::X86::AMD64::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(
  ...
  if (node->getOpCode().isIf() &&
    ...
    return self()->getNumberOfGlobalGPRs() - 3; // A memref in a jump for a
                                                // MemTable might need a base,
                                                // index and address register.
