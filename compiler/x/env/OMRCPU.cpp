...
TR_X86CPUIDBuffer *OMR::X86::CPU::queryX86TargetCPUID(TR::Compilation *comp) {
...
const char *OMR::X86::CPU::getX86ProcessorVendorId(TR::Compilation *comp) {
  static char buf[13];
...
uint32_t OMR::X86::CPU::getX86ProcessorSignature(TR::Compilation *comp) {
  return self()->queryX86TargetCPUID(comp)->_processorSignature;
}

uint32_t OMR::X86::CPU::getX86ProcessorFeatureFlags(TR::Compilation *comp) {
  return self()->queryX86TargetCPUID(comp)->_featureFlags;
}

uint32_t OMR::X86::CPU::getX86ProcessorFeatureFlags2(TR::Compilation *comp) {
  return self()->queryX86TargetCPUID(comp)->_featureFlags2;
}

bool OMR::X86::CPU::testOSForSSESupport(TR::Compilation *comp) { return false; }

bool OMR::X86::CPU::getX86OSSupportsSSE(TR::Compilation *comp) {
  // If the FXSR (bit 24) and SSE (bit 25) bits in the feature flags are not
  // set, SSE is unavailable
  // on this processor.
  //
...
bool OMR::X86::CPU::getX86OSSupportsSSE2(TR::Compilation *comp) {
  // If the FXSR (bit 24) and SSE2 (bit 26) bits in the feature flags are not
  // set, SSE is unavailable
  // on this processor.
  //
  uint32_t flags = self()->getX86ProcessorFeatureFlags(comp);
...
bool OMR::X86::CPU::getX86SupportsTM(TR::Compilation *comp) { return false; }
