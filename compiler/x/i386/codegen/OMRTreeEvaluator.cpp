...
TR::Register *OMR::X86::i386::TreeEvaluator::compareLongAndSetOrderedBoolean(
  ...
  if (secondChild->getOpCodeValue() == TR::lconst && testRegister == NULL &&
      performTransformation(comp, "O^O compareLongAndSetOrderedBoolean: "
                                  "checking that the second child node does "
...
void OMR::X86::i386::TreeEvaluator::compareLongsForOrder(
  ...
  TR::Register *testRegister = secondChild->getRegister();
  if (secondChild->getOpCodeValue() == TR::lconst && testRegister == NULL &&
      performTransformation(comp, "O^O compareLongsForOrder: checking that the "
                                  "second child node does not have an assigned "
                                  "register: %d\n",
                            testRegister)) {
    int32_t lowValue = secondChild->getLongIntLow();
    int32_t highValue = secondChild->getLongIntHigh();
    ...
    if (node->getNumChildren() == 3) {
      ...
      if (needVMThreadDep &&
        ...
        deps->addPostCondition(
            ...
            (TR::RealRegister::RegNum)cg->getVMThreadRegister()
                ->getAssociation(),
            cg);
      }
...
OMR::X86::i386::TreeEvaluator::integerPairMulEvaluator(TR::Node *node,
  ...
  if (lowRegister == NULL) {
    ...
    if (!p) {
      ...
      dependencies->addPostCondition(lowRegister, TR::RealRegister::eax, cg);
      dependencies->addPostCondition(highRegister, TR::RealRegister::edx, cg);
      TR_IA32PrivateLinkage *linkage =
          toIA32PrivateLinkage(cg->getLinkage(TR_Private));
      TR::IA32LinkageUtils::pushLongArg(secondChild, cg);
      TR::IA32LinkageUtils::pushLongArg(firstChild, cg);
      instr = generateHelperCallInstruction(node, TR_IA32longMultiply,
                                            dependencies, cg);
...
OMR::X86::i386::TreeEvaluator::integerPairShlEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    TR::X86ImmSymInstruction *instr = generateHelperCallInstruction(
        node, TR_IA32longShiftLeft, dependencies, cg);
    if (!linkage->getProperties().getCallerCleanup()) {
...
OMR::X86::i386::TreeEvaluator::integerPairRolEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    TR::RegisterDependencyConditions *dependencies =
        generateRegisterDependencyConditions((uint8_t)3, 3, cg);

    dependencies->addPreCondition(targetAsPair->getLowOrder(),
                                  TR::RealRegister::eax, cg);
    dependencies->addPreCondition(targetAsPair->getHighOrder(),
                                  TR::RealRegister::edx, cg);
    dependencies->addPreCondition(rotateAmountReg, TR::RealRegister::ecx, cg);

    dependencies->addPostCondition(targetAsPair->getLowOrder(),
                                   TR::RealRegister::eax, cg);
    dependencies->addPostCondition(targetAsPair->getHighOrder(),
                                   TR::RealRegister::edx, cg);
    dependencies->addPostCondition(rotateAmountReg, TR::RealRegister::ecx, cg);

    generateHelperCallInstruction(node, TR_IA32longRotateLeft, dependencies,
                                  cg);
...
OMR::X86::i386::TreeEvaluator::integerPairShrEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    TR::IA32LinkageUtils::pushIntegerWordArg(secondChild, cg);
    TR::X86ImmSymInstruction *instr = generateHelperCallInstruction(
        node, TR_IA32longShiftRightArithmetic, dependencies, cg);
    if (!linkage->getProperties().getCallerCleanup()) {
      instr->setAdjustsFramePointerBy(-12); // 1 long, 1 integer arg
    }
    targetRegister = cg->allocateRegisterPair(lowRegister, highRegister);
...
OMR::X86::i386::TreeEvaluator::integerPairUshrEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    TR::IA32LinkageUtils::pushIntegerWordArg(secondChild, cg);
    TR::X86ImmSymInstruction *instr = generateHelperCallInstruction(
        node, TR_IA32longShiftRightLogical, dependencies, cg);
    if (!linkage->getProperties().getCallerCleanup()) {
      instr->setAdjustsFramePointerBy(-12); // 1 long, 1 integer arg
    }
...
OMR::X86::i386::TreeEvaluator::iflcmpeqEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if ((lowValue | highValue) == 0) {
      ...
      // hand;
      // we cannot call generateConditionalJumpInstruction() here because we
      // need
      ...
      if (node->getNumChildren() == 3) {
        ...
        cg->evaluate(third);
        deps =
            generateRegisterDependencyConditions(third, cg, 3, &popRegisters);
        deps->setMayNeedToPopFPRegisters(true);
        if (needVMThreadDep)
          deps->addPostCondition(
              cg->getVMThreadRegister(),
        ...
        compareGPRegisterToConstantForEquality(node, highValue,
                                               cmpRegister->getHighOrder(), cg);
        generateLabelInstruction(JE4, node, destinationLabel, needVMThreadDep,
                                 cg);
        deps = generateRegisterDependencyConditions(
            (uint8_t)0, needVMThreadDep ? 3 : 2, cg);
        deps->addPostCondition(cmpRegister->getLowOrder(),
                               TR::RealRegister::NoReg, cg);
        deps->addPostCondition(cmpRegister->getHighOrder(),
                               TR::RealRegister::NoReg, cg);
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if ((lowValue | highValue) == 0) {
      ...
      if (node->getNumChildren() == 3) {
        ...
        generateLabelInstruction(JNE4, node, destinationLabel, needVMThreadDep,
                                 cg);
        compareGPRegisterToConstantForEquality(node, highValue,
                                               cmpRegister->getHighOrder(), cg);
        generateLabelInstruction(JNE4, node, destinationLabel, needVMThreadDep,
