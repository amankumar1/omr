...
TR_IA32SystemLinkage::TR_IA32SystemLinkage(TR::CodeGenerator *cg)
    : TR_X86SystemLinkage(cg) {
  _properties._properties = CallerCleanup | AlwaysDedicateFramePointerRegister;
  // for shrinkwrapping, we cannot use pushes for the preserved regs.
  // pushes/pops need to be in sequence and this is not compatible with
  // shrinkwrapping as registers need not be saved/restored in sequenc
...
uint32_t TR_IA32SystemLinkage::getAlignment(TR::DataType type) {
  switch (type) {
  case TR::Float:
    return 4;
  case TR::Double:
...
int32_t TR_IA32SystemLinkage::layoutParm(TR::Node *parmNode,
...
int32_t TR_IA32SystemLinkage::layoutParm(TR::ParameterSymbol *parmSymbol,
                                         int32_t &dataCursor, uint16_t &intReg,
                                         uint16_t &floatReg,
                                         parmLayoutResult &layoutResult) {
  layoutResult.abstract |= parmLayoutResult::ON_STACK;
...
void TR_IA32SystemLinkage::setUpStackSizeForCallNode(TR::Node *node) {
...
TR::Register *TR_IA32SystemLinkage::buildIndirectDispatch(TR::Node *callNode) {
  TR_ASSERT(false,
            "TR_IA32SystemLinkage::buildIndirectDispatch is not supported.\n");
  return NULL;
}

TR::Register *TR_IA32SystemLinkage::buildVolatileAndReturnDependencies(
...
int32_t
TR_IA32SystemLinkage::buildArgs(TR::Node *callNode,
...
TR::Register *TR_IA32SystemLinkage::buildDirectDispatch(TR::Node *callNode,
                                                        bool spillFPRegs) {
