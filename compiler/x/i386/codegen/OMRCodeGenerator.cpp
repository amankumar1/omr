...
OMR::X86::i386::CodeGenerator::CodeGenerator() : OMR::X86::CodeGenerator() {
  ...
  if (TR::Compiler->target.isWindows()) {
    ...
    if (!disableEBPasGPR && self()->allowVMThreadRematerialization() &&
        !self()->comp()->requiresSpineChecks()
        // when OSR is enabled, we don't want to generate any code in the osr
        // code block or the osr catch block
        // that causes ebp to be restored (because it was saved at some other
        // jitted code).
        // if we don't, when we jump to the osr catch block, we trash ebp by
        // that restore
    ...
    //
    static char *dontUseGPRsForWin32CTMConversion =
        feGetEnv("TR_DontUseGPRsForWin32CTMConversion");
    if (!dontUseGPRsForWin32CTMConversion) {
      self()->setUseGPRsForWin32CTMConversion();
    }

    static char *useLongDivideHelperForWin32CTMConversion =
        feGetEnv("TR_UseLongDivideHelperForWin32CTMConversion");
...
TR_GlobalRegisterNumber OMR::X86::i386::CodeGenerator::pickRegister(
  ...
  if (!self()->comp()->getOptions()->getOption(
    ...
    if (self()->comp()->getOption(TR_AssignEveryGlobalRegister) &&
        (!self()->comp()->cg()->getSupportsVMThreadGRA() ||
  ...
  if (self()->comp()->cg()->getSupportsVMThreadGRA() &&
    ...
    while (bvi.hasMoreElements()) {
      ...
      int32_t maxRegisterPressure = self()->estimateRegisterPressure(
...
#endif
  {
    ...
    if (maxRegisterPressure <
        self()->comp()->cg()->getMaximumNumbersOfAssignableGPRs())
      numExtraRegs = self()->comp()->cg()->getMaximumNumbersOfAssignableGPRs() -
                     maxRegisterPressure;
