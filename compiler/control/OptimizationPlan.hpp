...
public:
  ...
  bool isDowngradedDueToJProfiling() const {
    return _flags.testAny(DowngradedDueToJProfiling);
  }
  void setDowngradedDueToJProfiling(bool b) {
    _flags.set(DowngradedDueToJProfiling, b);
  }
  ...
  enum { // list of flags
    ...
    IsStackAllocated = 0x00008000, // Plan is allocated on the stack. Only used
    ...
    DontFailOnPurpose = 0x00100000,   // Mostly needed to avoid failing for the
                                      // purpose of upgrading to a higher opt
                                      // level
    RelaxedCompilationLimits = 0x00200000,  // Compilation can use larger limits
                                            // because method is very very hot
    DowngradedDueToJProfiling = 0x00400000, // Compilation was downgraded to
                                            // cold just because we wanted to do
                                            // JProfiling
