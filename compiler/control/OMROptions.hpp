 ...
 * generated by the automated build tasks.
 * It is declared as an external constant and it is defined in a make-time
 * generated file called TRBuildName.cpp. This
 * file is generated in the generateVersion.pl script which runs as part of the
 * make process. We could also supply this
 * definition via a compiler define macro flag (-DTR_BUILD_NAME) however that
 * would mean that from one automated build
 * to the next the command line arguments used to invoke the compiler would be
 * different. This is problematic for the
 * use of ccache to cache the object results of a compilation. This is why we
 * had to refactor this definition into a
...
enum TR_CompilationOptions {
  ...
  TR_MimicInterpreterFrameShape =
      0x00008000, // ** NOTE ** Alias for TR_FullSpeedDebug until we're prepared
  ...
  TR_UnresolvedAreNotColdAtCold = 0x00004000 + 4, // cold block marker marks
                                                  // unresolved blocks as cold
                                                  // at hotness cold or less
  ...
  TR_ReservingLocks = 0x00004000 + 6, // Can be merged with
                                      // TR_DisableLockResevation when lock
  ...
  TR_DisableNewLoopTransfer =
      0x10000000 + 6, // loop versioning for virtual guards
  ...
  TR_DisableVSSStackCompaction =
      0x00100000 + 17, // never compact variable size symbols -- if not force or
                       // disable then use heuristic based on opt level and VSS
                       // total size
  ...
  TR_DisableProloguePushes =
      0x00080000 + 18, // Use stores instead of pushes in x86 prologues
  TR_EnableOutlinedPrologues =
      0x00100000 + 18, // Call a helper to do some of the prologue logic
  TR_DisableOutlinedNew = 0x00200000 + 18, // Expand object allocation logic
                                           // inline instead of using a fast jit
                                           // helper
  // Available                                       = 0x00400000 + 18, //
  // enable register(*) support in PLX
  ...
  // Available                                       = 0x00200000 + 19,  // only
  // propagate const nodes and not conversions of const nodes in
  // VariableInitializerPropagation
  ...
  TR_EnableLateCleanFolding =
      0x40000000 +
      19, // fold pdclean flags into pdstore nodes right before codegen
  ...
  // enable support for packed objects/arrays
  // Available                                       = 0x00000800 + 20,  //
  // disable passing nested autos as parameters
...
enum TR_VerboseFlags {
  ...
  TR_VerboseCompFailure, // print extra message when a first time compilation is
                         // going to be continue interpreted
...
enum { first_method, avg_method, max_method };
}
}
...
public:
  ...
  static bool getJProfilingOption(TR_JProfilingFlags op) {
    return _jprofilingOptionFlags.isSet(op);
  }
  static void setJProfilingOption(TR_JProfilingFlags op) {
    _jprofilingOptionFlags.set(op);
  }
  static void resetJProfilingOption(TR_JProfilingFlags op) {
    _jprofilingOptionFlags.reset(op);
  }
  static bool isAnyJProfilingOptionSet() {
    return !_jprofilingOptionFlags.isEmpty();
  }
  ...
  bool getTraceRegisterITF(uint32_t mask) {
    return (_traceRegisterITF & mask) != 0;
  }
  ...
  int32_t getHWProfileSuperColdOnlySteps() {
    return _HWProfileSuperColdOnlySteps;
  }
  int32_t getHWProfileHotnessCalc() { return _HWProfileHotnessCalc; }
  int32_t getHWProfileFreqCalc() { return _HWProfileFreqCalcMethod; }
  int32_t getHWProfileFreqCalcInlineHeuristic() {
    return _HWProfileFreqCalcInlineHeuristic;
  }
...
public:
  ...
  static int32_t _queueWeightThresholdForAppThreadYield; // used only for LINUX;
                                                         // app threads will
                                                         // sleep sometimes
  ...
  static uint32_t _minBytesToLeaveAllocatedInSharedPool; // 0 to disable the
                                                         // feature and revert
                                                         // to old behavior
  static uint32_t _maxBytesToLeaveAllocatedInSharedPool; // 0 to disable the
                                                         // feature and revert
                                                         // to old behavior
  ...
  void setJitMethodEntryAlignmentBoundary(int32_t boundary) {
    _jitMethodEntryAlignmentBoundary = boundary;
  }
...
private:
  ...
  template <typename Enum, int NUM_FLAGS> class OptionFlagArray {
    ...
    // ordinary option
    // words are typecast to one of these (eg.
  ...
  // Set jprofiling bits
  //
  static char *setJProfilingBits(char *option, void *base,
                                 TR::OptionTable *entry);
...
public:
  ...
  typedef OptionFlagArray<TR_JProfilingFlags, TR_NumJProfilingFlags>
      JProfilingOptionFlagArray;
