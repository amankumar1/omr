...
static void writePerfToolEntry(void *start, uint32_t size, const char *name) {
  ...
  if (firstAttempt) {
    ...
    int numCharsWritten = snprintf(perfFilename, maxPerfFilenameSize,
                                   "/tmp/perf-%ld.map", jvmPid);
...
int32_t init_options(TR::JitConfig *jitConfig, char *cmdLineOptions) {
  ...
  if (cmdLineOptions) {
    // The callers (ruby/python) guarantee that we have at least -Xjit in
    // cmdline options
    //
...
uint8_t *compileMethodFromDetails(OMR_VMThread *omrVMThread,
  ...
  if (!methodCanBeCompiled(&fe, compilee, filterInfo, &trMemory)) {
    ...
    TR_ASSERT(TR::comp() == NULL, "there seems to be a current TLS "
                                  "TR::Compilation object %p for this thread. "
                                  "At this point there should be no current "
                                  "TR::Compilation object",
              TR::comp());
    TR::Compilation compiler(0, omrVMThread, &fe, &compilee, request, *options,
                             dispatchRegion, &trMemory, plan);
    TR_ASSERT(TR::comp() == &compiler, "the TLS TR::Compilation object %p for "
                                       "this thread does not match the one %p "
                                       "just created.",
              TR::comp(), &compiler);
    ...
    try {
      ...
      if (rc == 0) // success!
      {
        ...
        // not ready yet...
        // OMR::MethodMetaDataPOD *metaData =
        // fe.createMethodMetaData(&compiler);

        startPC = compiler.cg()->getCodeStart();

        if (TR::Options::getCmdLineOptions()->getVerboseOption(
                TR_VerboseCompileEnd) ||
            TR::Options::getCmdLineOptions()->getVerboseOption(
                TR_VerboseCompileTime)) {
          const char *signature = compilee.signature(&trMemory);
          TR_VerboseLog::writeLineLocked(
              TR_Vlog_COMP, "(%s @ %#p t=%lldms  %lld.%lldms) %s",
              compiler.getHotnessName(compiler.getMethodHotness()), startPC,
              TR::Compiler->vm.getUSecClock() - fe.getStartTime(),
              translationTime / 1000, translationTime % 1000, signature);
          trfflush(jitConfig->options.vLogFile);
        }

        if (TR::Options::getCmdLineOptions()->getOption(TR_PerfTool))
          generatePerfToolEntry(
              startPC, compiler.cg()->getCodeEnd(), compiler.signature(),
              compiler.getHotnessName(compiler.getMethodHotness()));

        if (compiler.getOutFile() != NULL && compiler.getOption(TR_TraceAll))
          traceMsg((&compiler), "<result success=\"true\" startPC=\"%#p\" "
                                "time=\"%lld.%lldms\"/>\n",
      ...
      } else /* of rc == 0 */
      {
        ...
        if (jitConfig->options.verboseFlags != 0) {
          ...
          TR_VerboseLog::writeLineLocked(TR_Vlog_COMPFAIL,
                                         "%s failed compilation", signature);
        }
        if (compiler.getOutFile() != NULL && compiler.getOption(TR_TraceAll))
          traceMsg((&compiler), "<result success=\"false\">compiler returned "
                                "with failure %d</result>\n",
                   rc);
      }
      ...
      // failed! :-(
      if (compiler.getOutFile() != NULL && compiler.getOption(TR_TraceAll))
        traceMsg((&compiler), "<result success=\"false\">longjmp invoked from "
                              "the compiler</result>\n");
    }
