...
const char *OMR::Optimization::optDetailString() {
...
TR::Node *OMR::Optimization::replaceNodeWithChild(TR::Node *node,
  ...
  if (correctBCDPrecision && node->getType().isBCD() &&
    ...
    // ensuring
    // that the precision of the replacing node (child) matches the precision of
    // the replaced
    // node (node).
    // If 'child' has a different precision than 'node' then the parent of
    // 'node', if it is call
    // for example, will pass too many or too few digits to its callee (even
    // digits known to be zero
    // are significant to calls to ensure the callee can demarshal the arguments
    // correctly).
    // If the 'child' truncates then there is the danger that this truncating
    // side-effect will be lost unless
    // a bcd modPrecOp is inserted.
    // If correctBCDPrecision=false then the caller has guaranteed that there is
    // no danger of a child truncation
    ...
    if (node->getReferenceCount() > 1) {
      ...
      dumpOptDetails(self()->comp(), "%sPrecision mismatch when replacing "
...
TR::Node *OMR::Optimization::replaceNode(TR::Node *node, TR::Node *other,
                                         ...
                                         bool anchorChildren) {
  if (!performTransformation(
          self()->comp(), "%sReplace node [" POINTER_PRINTF_FORMAT
...
bool OMR::Optimization::nodeIsOrderDependent(TR::Node *node, uint32_t depth,
  ...
  // well maintained in simplifier
  // and so shouldn't be used for functional items such as to anchor or not
  // anchor a node. FutureUseCount
  ...
  return (
      (node->getOpCode().isLoad() && node->getOpCode().hasSymbolReference() &&
       !(cachedStaticReg && node->getOpCodeValue() == TR::aload &&
         linkage->isAddressOfStaticSymRef(node->getSymbolReference())) &&
       !(cachedStaticReg && node->getOpCodeValue() == TR::aload &&
         linkage->isAddressOfPrivateStaticSymRef(node->getSymbolReference())) &&
