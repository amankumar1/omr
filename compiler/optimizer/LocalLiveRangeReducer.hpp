 ...
 * registers
 * in the code generator) with a view to reducing the likelihood of register
 * spilling.
 * Entire trees are moved if necessary and also subtrees are extracted and
 * anchored
 * under treetop nodes with a view of keeping all the references to a particular
 * node
 * as close together as possible (while also trying to do the same wrt live
 * ranges
 * of the children of the node). Since the optimization moves nodes around it
 * needs
 * to consult aliasing to ensure that the semantics of the original program are
 * not
