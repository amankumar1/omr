...
int32_t TR_ExtendBasicBlocks::orderBlocksWithoutFrequencyInfo() {
  ...
  for (; block; prevBlock = block, block = block->getNextBlock()) {
    ...
    if (cannotExtendWithCurrentFallThrough ||
      ...
      // prevBlock.
      // If so, see if we can rearrange the blocks so that the prevBlock can be
      // extended
    ...
    if (prevNode->getOpCodeValue() == TR::Goto) {
      ...
      if (nextBlock && prevNode->getOpCode().isBranch() &&
        ...
        if (prevNode->getBranchDestination() == nextBlock->getEntry() &&
          ...
          if (!performTransformation(comp(), "%sReverse branch in %d (possibly "
                                             "swinging up destintion and "
          ...
          if (!performTransformation(comp(), "%sRemove backedge goto block_%d "
                                             "by reversing the branch in "
                                             "block_%d\n",
...
TR::Block *TR_BlockManipulator::getBestChoiceForExtension(TR::Block *block1) {
  ...
  for (auto nextEdge = block1->getSuccessors().begin();
    ...
    if (trace())
      traceMsg(comp(), "    Estimating hotness for BB [%d], next BB [%d], "
                       "estimated hotness %d\n",
...
int32_t TR_BlockManipulator::performChecksAndTreesMovement(
  ...
  if (newBlock->getVisitCount() < visitCount) {
    ...
    for (cursor = newBlock, cursorNext = cursor->getNextBlock();
         cursorNext && (cursor->hasSuccessor(cursorNext) ||
    ...
    // point to block (old fallthrough)
    ...
    */
    ...
    if (endOfTreesBeingMoved && !prevNode->isNopableInlineGuard() &&
      ...
      if ( // comp()->cg()->allowBlockToBeReordered(newBlock) &&
          ...
          performTransformation(comp(), "%sswing down block_%d and break "
                                        "original fall through to join with "
                                        "block_%d\n",
...
//
    ...
    if ((node1->getOpCode().isReturn() ||
      ...
      for (auto nextEdge = block->getPredecessors().begin();
        ...
        // removed from structure/CFG after this hoisting) that could create
        // improper regions.
        // In any case if this is the last predecessor, then this block should
        // be moved
        // will block extension/trees cleansing to be adjacent to the
        // predecessor (no
        // need for hoisting)
        //
          ...
          if (synergyExists &&
            ...
            for (auto nextSuccEdge = block->getSuccessors().begin();
              ...
              if (extraGotoBlock && (succBlock == fallThroughSuccBlock)) {
                ...
                // the edge being added to the CFG but NOT to the structure
                // (which
                // checks for duplicate edges before adding them). Then removing
                // the
                // edge from prevBlock->block will fail after structure repair
                // as there
                // would be one edge in the CFG left, but NONE in the structure
                // subgraph.
                //
                if (block != succBlock)
  ...
  if (currentNode->getOpCodeValue() == TR::arraycopy) {
    ...
    if ((arraycopyDestAddressNode == addressChild) ||
      ...
      *scratch &= *readAndWrittenSymbols;
      ...
      if (scratch->isEmpty() &&
          performTransformation(comp(), "%sEliminating some stores done by "
                                        "arrayset node %p because stores done "
                                        "by a following arraycopy node %p will "
                                        "overwrite them\n",
  ...
  for (treeTop = startTree; (treeTop != endTree);
    ...
    if ((lastRealNode->getOpCode().isBranch() &&
      ...
      while (seenAndNode) {
        ...
        // (a&b && a&c) ==> ((a & b|c) == b|c)
                 ...
                 (isAndOfTwoFlags(comp(), andNode, lastRealNode, TR::ificmpeq,
                                  TR::iand) ||
                  isAndOfTwoFlags(comp(), andNode, lastRealNode, TR::ifiucmpeq,
                                  TR::iand) ||
                  isAndOfTwoFlags(comp(), andNode, lastRealNode, TR::iflcmpeq,
                                  TR::land) ||
                  isAndOfTwoFlags(comp(), andNode, lastRealNode, TR::iflucmpeq,
                                  TR::land))) {
          if (trace())
      ...
      if (!seenAndBefore) {
        ...
        // IL pattern which is expected to be more efficient from a codegen
        // point
        // of view. Note that this pattern is seen in Compressor.compress() and
        // might
            ...
            if ((firstTree == lastTree) &&
              ...
              if (firstNode->getOpCode().isStoreDirect() &&
                ...
                if (areTreesInRequiredForm &&
                    performTransformation(comp(), "%sSimplifying branch %p and "
                                                  "store %p into conditional\n",
      ...
      } else if (performTransformation(comp(), "%sRemoving conditional node %p "
        ...
        if (lastRealNode->getOpCode().isSpineCheck()) {
          ...
          if (0 && (prevNode->getChild(2) == lastRealNode->getChild(2)) &&
              (compareValues(prevNode->getChild(3), lastRealNode->getChild(3)) >
               0) &&
              performTransformation(
                  comp(), "%sRemoving BNDCHK from BNDCHKwithSpineCHK "
...
int32_t TR_EliminateRedundantGotos::process(TR::TreeTop *startTree,
  ...
  for (TR::TreeTop *treeTop = startTree, *exitTreeTop; (treeTop != endTree);
    ...
    // block have been
    // removed because the exception predecessor nodes have become empty and
    // removed.
    // Even in that case, we don't want to remove the gotos at the end of such
    // OSR blocks.
    ...
    if (rootStructure && blockRegion &&
        blockRegion->getNumber() == block->getNumber()) {
      ...
      // blockStrucutre
      // into the parent.  1) if the blockRegion is a loop region, we will end
      // up
      // with an improper region 2) otherwise, we will end up with a bigger
      // parent
      ...
      for (auto edge = block->getPredecessors().begin();
        ...
        if (predBlock->getLastRealTreeTop()
                ->getNode()
                ->getOpCode()
                .isBranch() &&
            predBlock->getLastRealTreeTop()
                    ->getNode()
                    ->getBranchDestination() == block->getEntry())
        ...
        if (predBlock->getNextBlock() == destBlock) {
          ...
          if (last->getOpCodeValue() == TR::Goto) {
            ...
            TR::TreeTop *prev =
                predBlock->getLastRealTreeTop()->getPrevTreeTop();
            TR::TreeTop *next =
                predBlock->getLastRealTreeTop()->getNextTreeTop();
            prev->join(next);
          }
        }
      }
      ...
      //
      for (auto inEdge = block->getPredecessors().begin();
           inEdge != block->getPredecessors().end();) {
        TR::CFGEdge *current = *(inEdge++);
        TR::Block *prevBlock = toBlock(current->getFrom());
        if (asyncMessagesFlag)
          placeAsyncCheck(prevBlock);

        if (prevBlock->getLastRealTreeTop()
                ->getNode()
                ->getOpCode()
                .isBranch() &&
          ...
          prevBlock->redirectFlowToNewDestination(comp(), current, destBlock,
                                                  false);
        }
        ...
        if (prevBlock->getNextBlock() == destBlock) {
          ...
          if (last->getOpCodeValue() == TR::Goto) {
            ...
            TR::TreeTop *prev =
                prevBlock->getLastRealTreeTop()->getPrevTreeTop();
            TR::TreeTop *next =
                prevBlock->getLastRealTreeTop()->getNextTreeTop();
            prev->join(next);
          }
        }
      }
...
// as it's known that no block falls through to block C. We can then eliminate
// the
// goto.
//
    ...
    if (tt2 && node->getBranchDestination() == tt2) {
      ...
      if ((tt1->getNode()->getOpCodeValue() == TR::Goto ||
          ...
          (/* (!rootStructure) || */ isBestChoiceForFallThrough(block,
                                                                block2)) &&
          performTransformation(comp(), "%sMoving trees contained in block "
                                        "that was the fall through of "
                                        "block_%d\n",
                                optDetailString(), block->getNumber())) {
        ...
        if (numElementsInfo) {
          ...
          if (totalFrequency > 0) {
            ...
            TR_ScratchList<TR_ExtraAbstractInfo> valuesSortedByFrequency(
                trMemory());
            numElementsInfo->getSortedList(
                comp(), (List<TR_ExtraAbstractInfo> *)&valuesSortedByFrequency);
            ListIterator<TR_ExtraAbstractInfo> i(
                (List<TR_ExtraAbstractInfo> *)&valuesSortedByFrequency);
            for (TR_ExtraAbstractInfo *profiledInfo = i.getFirst();
          ...
          if (doProfiling &&
              performTransformation(
                  comp(), "O^O PROFILED NODE VERSIONING: Versioning %s %s, "
                          "size %s %d %.01f%%\n",
            ...
            if (originalFrequency >= 0) {
              ...
              if (trace()) {
                traceMsg(
                    comp(), " -> Setting fast block_%d frequency = %d; slow "
            ...
            traceMsg(comp(), "Not versioning %s node %s because size node %s "
                             "top probability is %.0f%% < %.0f%%\n",
...
void TR_Rematerialization::rematerializeSSAddress(TR::Node *parent,
                                                  int32_t addrChildIndex) {
       ...
       (addressNode->getOpCode().isArrayRef() &&
        addressNode->getSecondChild()->getOpCode().isLoadConst()) &&
           cg()->getSupportsConstantOffsetInAddressing(
               addressNode->getSecondChild()->get64bitIntegralValue())))

  {
    if (performTransformation(
            comp(), "%sRematerializing SS address %s (%p)\n", optDetailString(),
            addressNode->getOpCode().getName(), addressNode)) {
      ...
      if (!(node->getOpCodeValue() == TR::BBStart ||
        ...
        if (performTransformation(comp(), "%s Counting ops for node [%p]\n",
          ...
          if (_curLongOps > 0) {
            int32_t nWeight = 1;
            bool isInLoop = (NULL !=
                             treeTop->getEnclosingBlock()
                                 ->getStructureOf()
                                 ->getContainingLoop());
            if (isInLoop) {
              TR_RegionStructure *region =
                  treeTop->getEnclosingBlock()->getParentStructureIfExists(
                      comp()->getFlowGraph());
              if (trace())
                traceMsg(comp(), "\tNode [%p] is in a loop\n", node);

              // calculate freq of exec (will take care of loops and nesting in
              // loops)
              if (region) {
                ...
                if (trace())
                  traceMsg(comp(),
                           "\tLoop node [%p] has %d longs at level %d\n", node,
                           _curLongOps, level);
              }
            }
          }
...
bool TR_Rematerialization::examineNode(TR::TreeTop *treeTop, TR::Node *parent,
  ...
  node->getNumChildren(), seenChildren.getSize());
  ...
  */
  ...
  if (shouldOnlyRunLongRegHeuristic()) {
    ...
    if (isCall || isCallDirect) {
      ...
      if (isInLoop) {
        ...
        region = treeTop->getEnclosingBlock()->getParentStructureIfExists(
            comp()->getFlowGraph());
        if (trace())
        ...
        if (region) {
          ...
          if (trace())
            traceMsg(comp(),
                     "\tCall node [%p] in loop weighted live longs=%d\n", node,
                     curLiveLongs);
        }
      }
  ...
  if (!shouldOnlyRunLongRegHeuristic()) {
    ...
    if (trace() && !shouldOnlyRunLongRegHeuristic()) {
      traceMsg(comp(), "At node %p parent %p GPR pressure is %d (child adjust "
                       "%d parent adjust %d) limit is %d\n",
               node, parent,
               numRegisters + childAdjustment +
                   adjustments.adjustmentFromParent + restrictedGPRNum,
  ...
  if (trace() && !shouldOnlyRunLongRegHeuristic() &&
    ...
    traceMsg(comp(), "At node %p FPR pressure is %d (child adjust %d parent "
                     "adjust %d) limit is %d\n",
             node, state->_currentlyCommonedFPNodes.getSize() +
  ...
  if (trace() && !shouldOnlyRunLongRegHeuristic() &&
      node->getOpCode().isVector()) {
    traceMsg(
        comp(), "At node %p VSR pressure is %d (child adjust %d parent adjust "
                "%d) limit is %d\n",
        node, state->_currentlyCommonedVector128Nodes.getSize() +
                  vector128ChildAdjustment +
...
void TR_Rematerialization::calculateLongRegWeight(bool childOfCall,
                                                  bool isLongNode) {
  _curOps++;
  if (isLongNode) {
    _curLongOps++;
...
void TR_Rematerialization::examineLongRegNode(TR::Node *node,
                                              vcount_t visitCount,
                                              bool childOfCall) {
...
void TR_LongRegAllocation::makeLongRegDecision() {
  ...
  /*if (totalRatio < LONGREG_TOTAL_LOW_WATERMARK)
      ...
      totalRatio > LONGREG_TOTAL_HIGH_WATERMARK &&
  ...
  if (numLoopOps > 0) {
    ...
    if (getNumLongAtNesting(1) > 0) {
      ...
      double nestingRatio =
          nestedLoopSum / ((double)getNumLongAtNesting(1)) * 100;
...
void TR_Rematerialization::makeEarlyLongRegDecision() {
  ...
  if (!comp()->getOptions()->getOption(TR_Disable64BitRegsOn32Bit) &&
    ...
    setLongRegDecision(true);
    dumpOptDetails(comp(),
                   "\tEarly decision - unconditionally longRegAllocable\n");
  }
  return;
}
...
int32_t TR_BlockSplitter::perform() {
  ...
  if (disableSynergy() && comp()->isOptServer()) {
    if (trace())
      traceMsg(comp(), "Large app detected, making BlockSplitter very "
                       "conservative to avoid cloning code (increases i-cache "
                       "pressure)\n");
  }
  ...
  if (trace())
    traceMsg(comp(), "  alpha=%f maxDepth=%d totalFrequency=%d "
                     "splitFrequency=%d splitPredFrequency=%d\n",
  ...
  // we should visit
  // the order is important because the index will be set in visitation order
  // and we need to sort
  ...
  while (children->size() > 0) {
    ...
    if ((toBlock(children->top())->hasExceptionPredecessors() == true) ||
    ...
    if (disableSynergy() && comp()->isOptServer() &&
      ...
      for (auto edge = mergeNode->getPredecessors().begin();
        ...
        if (predBlock->getEntry() &&
          ...
          if (trace())
            traceMsg(comp(), "    rejecting because predecessor block %d has "
                             "more than 4 treetops\n",
                     predBlock->getNumber());
          break;
        }
      }
  ...
  for (int32_t i = 0; i <= array->lastIndex(); ++i) {
    ...
    for (auto edge = mergeNode->getPredecessors().begin();
      ...
      if (toBlock(node)->getEntry()) {
        ...
        // as the target
        // of a virtual guard that we are not allowed to reverse, if this is the
        // case we must chose someone else
        ...
        // target of
        // a GCR counting guard which could be patched, we are not allowed to
        // reverse these either and must choose someone else
        if (lastRealNode->getOpCode().isIf() &&
      ...
      if (!splitPred || splitPred->getFrequency() < node->getFrequency()) {
        ...
        if (trace())
          traceMsg(comp(), "mergeNode(%d) splitPred(%d) blockfreq = %d "
                           "edgefreq = %d candidate\n",
    ...
    if (!splitPred || (splitPred->getFrequency() < splitPredFrequency) ||
        ...
        // lower than
        // block frequencies because of frequency maintenance issues elsewhere,
        // it can
    ...
    while (depth > 1) {
      ...
      for (auto edge = target->getSuccessors().begin();
        ...
        // until last
        // we want to skip as many checks as we can, so check the frequency
        // early
        ...
        if (child->getFrequency() <= (target->getFrequency() - tollerance)) {
          if (trace())
            traceMsg(comp(), "    Reject successor block_%d: frequency is too "
                             "low (%d < %d)\n",
        ...
        if (!(child->getPredecessors().size() == 1)) {
          if (trace())
            traceMsg(comp(), "    Reject successor block_%d: merge block -- "
        ...
        if (containCycle(child, &bMap)) {
          if (trace())
            traceMsg(comp(), "    Reject successor block_%d: would cause cycle "
                             "in block mapper\n",
                     child->getNumber());
          continue;
        }
        ...
        // reached as the target
        // of a virtual guard that we are not allowed to reverse, if this is the
        // case we must stop here
        if (lastRealNode->getOpCode().isIf() &&
            lastRealNode->isTheVirtualGuardForAGuardedInlinedCall() &&
            lastRealNode->getBranchDestination()
                    ->getNode()
          ...
          if (trace())
            traceMsg(comp(), "    Reject successor block_%d: virtual guard "
                             "branch that can't be reversed\n",
                     child->getNumber());
          continue;
        }
...
int32_t TR_BlockSplitter::synergisticDepthCalculator(
  ...
  if (disableSynergy()) {
    ...
    for (i = 2; i < synergies.size(); ++i) {
      ...
      // copied
      // so allow up to one alpha's worth of cost to push the merge point
      // further down
...
bool TR_BlockSplitter::hasIVUpdate(TR::Block *block) {
  ...
  if (getLastRun() || comp()->isProfilingCompilation() || !parent ||
...
TR::Block *TR_BlockSplitter::splitBlock(TR::Block *pred,
        ...
        if (lastRealNode->getBranchDestination()
          ...
          if (trace())
            traceMsg(comp(),
                     ...
                     pred->getNumber(), lastRealNode->getBranchDestination()
                                            ->getNode()
                                            ->getBlock()
                                            ->getNumber());
          ...
          if (bMap->getLast()->_from->getNumber() == pred->getNumber()) {
            ...
            lastRealNode->reverseBranch(cloneEnd->getExit()->getNextTreeTop());
            if (trace())
              traceMsg(comp(), "    Also reversing cloned branch, node %d now "
                               "jumps to block_%d\n",
  ...
  for (BlockMapper *itr = bMap->getFirst(); itr; itr = itr->getNext()) {
    ...
    if (lastRealNode->getOpCode().isBranch() &&
      ...
      // execution stream so fall through
      // to a goto
      //
...
int32_t TR_InvariantArgumentPreexistence::perform() {
...

  {
    ...
    // TR_J9EstimateCodeSize::realEstimateCodeSize,
    // so TR_InvariantArgumentPreexistence can validate preexistence args and
    // propagate info onto parmSymbols
    {
      ...
      if (argInfo) {
        if (trace())
          traceMsg(comp(), "PREX:    Populating parmInfo of inlined method %s "
                           "from argInfo %p\n",
        ...
        if (trace())
          traceMsg(comp(), "PREX:    No argInfo -- can't populate parmInfo for "
                           "inlined method %s\n",
                   feMethod->signature(trMemory()));
      }
    }
  ...
  if (methodSymbol->isComputed()) {
    ...
    if (refinedMethod) {
      ...
      if (!needsAssumptions || addAssumptions) {
        if (!refinedMethod->isSameMethod(resolvedMethod)) {
          if (performTransformation(comp(), "%sspecialize and devirtualize "
                                            "invoke [%p] on currently fixed or "
                                            "final parameter %d [%p]\n",
                                    optDetailString(), node,
                                    receiverParmOrdinal, receiverSymbol)) {
        ...
        } else if (performTransformation(comp(), "%sdevirtualize invoke [%p] "
                                                 "on currently fixed or final "
                                                 "parameter  %d [%p]\n",
                                         optDetailString(), node,
                                         receiverParmOrdinal, receiverSymbol)) {
          node->devirtualizeCall(treeTop);
    ...
    if (!isInterface && !resolvedMethod->virtualMethodIsOverridden() &&
      ...
      if (methSymbol->isInterface() || methodSymbol) {
        ...
        // In the following avoid devirtualization based on single implementor
        // if the method has
        ...
        if (method && !method->virtualMethodIsOverridden()) {
          ...
          if (addAssumptions &&
              performTransformation(comp(), "%sspecialize and devirtualize "
                                            "invoke [%p] based on only a "
                                            "single implementation for call on "
                                            "parameter %d [%p]\n",
                                    optDetailString(), node,
                                    receiverParmOrdinal, receiverSymbol)) {
...
void TR_InvariantArgumentPreexistence::processIndirectLoad(
  ...
  if (addressChild->getSymbolReference()->hasKnownObjectIndex()) {
    ...
    somethingMayHaveChanged = TR::TransformUtil::transformIndirectLoadChainAt(
        comp(), node, addressChild, (uintptrj_t *)addressChild->getSymbol()
                                        ->castToStaticSymbol()
                                        ->getStaticAddress(),
        &removedNode);
        ...
        if (virtualGuard && transformationIsValid &&
            ...
            vftLoad->getFirstChild()->getSymbolReference() ==
                checkcastTree->getNode()
                    ->getFirstChild()
                    ->getSymbolReference() &&
            performTransformation(comp(), "%s Merging checkcast [%p] and "
                                          "profiled guard [%p] (basic case)\n",
    ...
    case TR::ifacmpeq:
      ...
      if (checkcastTree) {
        if (node->getFirstChild() !=
          ...
          traceMsg(comp(), "Found suitable ifacmpeq [%p] after checkcast [%p], "
                           "safe to continue to next block in search of a "
                           "profiled guard\n",
...
void TR_CheckcastAndProfiledGuardCoalescer::doBasicCase(
  ...
  TR::Node *newCheckcastNode = TR::Node::createWithSymRef(
      ...
      classConst, 0, comp()->getSymRefTab()->findOrCreateCheckCastSymbolRef(
                         comp()->getMethodSymbol()));
...
int32_t TR_ColdBlockMarker::isBlockCold(TR::Block *block) {
  ...
  // for (TR::PostorderNodeOccurrenceIterator iter(block->getFirstRealTreeTop(),
  // this); iter != block->getExit(); ++iter)
  //
  for (TR::PreorderNodeIterator iter(block->getFirstRealTreeTop(), this);
  ...
  if (hasBeenRun != TR_maybe) {
    ...
    if (node->getSymbolReference()->getSymbol()->isClassObject() &&
      ...
      if (comp()->compileRelocatableCode() &&
        ...
        // often for AOT than non-AOT beacause of
        // literals() vs romLiterals() used in relocatableresolvedj9method
        // this optimiztaion can be removed when work to clean up ram vs rom
        // literals is done
...
void TR_ColdBlockOutlining::reorderColdBlocks() {
  ...
  for (currentTree = comp()->getStartTree(); currentTree != NULL;
    ...
    if (!coldBlock(
            currentBlock,
            comp()) /*currentBlock->isCold()*/ /* && !currentBlock->isRare(comp()) */) {
    ...
    // to long form
    // So we don't want to move the successor of a gen asm flow block if gen asm
    // block itself is not cold
    // Also we don't want to move gen asm flow block if one of it's successor is
    // not cold.
    ...
    // checks above
    // guarantee a break out of the loop before this point for the last block
    // (i.e. what was
    ...
    // entry block is cold (and hence
    // every single block in the method).
    // FIXME: the following should be an assume, but i am hesitant.. If anybody
    // runs the cold propagtion code
    // ever at fixed opt levels (count=0) then we could results with the entry
    // block as cold
    //
    if (prevOfStart == NULL)
      return;
...
TR::Block *TR_BlockManipulator::breakFallThrough(TR::Block *faller,
  ...
  if (lastNode->getOpCode().isBranch() ||
    ...
    TR::Block *gotoBlock = TR::Block::createEmptyBlock(
        lastNode, comp(), (fallee->getFrequency() < faller->getFrequency())
                              ? fallee->getFrequency()
                              : faller->getFrequency(),
        fallee);
...
void TR_TrivialDeadTreeRemoval::preProcessTreetop(
  ...
  if (ttNode->getOpCodeValue() == TR::treetop &&
    ...
    if (firstChild->getReferenceCount() == 1) {
      if (!firstChild->getOpCode().hasSymbolReference() &&
#ifdef J9_PROJECT_SPECIFIC
          !firstChild->getOpCode()
               .isPackedExponentiation() && // pdexp has a possible message side
                                            // effect in truncating or no
                                            // significant digits left cases
#endif
          performTransformation(
              ...
              firstChild)) {
        if (trace)
          traceMsg(comp, "\tfound trivially anchored ttNode %p with firstChild "
                         "%s (%p -- refCount == 1)\n",
        ...
        for (int32_t i = 0; i < firstChild->getNumChildren(); i++) {
          ...
          if (!grandChild->getOpCode().isLoadConst() ||
            ...
            if (trace)
              traceMsg(comp, "\t\tcreate new treetop for "
                             "firstChild->getChild(%d) = %s (%p)\n",
        ...
        if (trace)
          traceMsg(comp, "\t\tremove trivially anchored ttNode %p with "
                         "firstChild %s (%p) treetop\n",
                   ttNode, firstChild->getOpCode().getName(), firstChild);
        treeTop->unlink(true);
      }
      ...
      if (trace)
        traceMsg(comp, "\tadd ttNode %p with firstChild %s (%p, refCount %d) "
                       "to commonedTreeTopList\n",
                 ttNode, firstChild->getOpCode().getName(), firstChild,
...
void TR_TrivialDeadTreeRemoval::postProcessTreetop(
  ...
  if (treeTop->isPossibleDef()) {
    if (trace)
      traceMsg(comp, "\tfound a possible def at node %p so clear "
...
void TR_TrivialDeadTreeRemoval::processCommonedChild(
  ...
  if (child->getReferenceCount() > 1) {
    if (!commonedTreeTopList.isEmpty()) {
      ...
      if (trace)
        traceMsg(comp, "commonedTreeTopList is not empty and found a commoned "
                       "child %s (%p, refCount %d)\n",
      ...
      while (listTT) {
        ...
        // treetop as the tracked treetop
        // tt
        //   a
        //   =>a
        //
        if (trace)
          traceMsg(comp, "\tcomparing listTT %p with firstChild %s (%p) to "
                         "commoned child %s (%p, refCount %d) (listTT == "
                         "_currentTreeTop -- %s)\n",
                   listTT->getNode(),
        ...
        if (listTT->getNode()->getFirstChild() == child) {
          if (listTT != treeTop) {
            if (performTransformation(
                    comp, "%sFound commoned reference to child %s (%p) so "
                          "unlink %s (0x%p)\n",
            ...
            if (treeTop->getNode()->getNumChildren() == 1 &&
              ...
              if (performTransformation(
                      comp, "%sFound commoned reference to single child %s "
                            "(%p) case 1 so unlink %s (0x%p)\n",
                      optDetails, child->getOpCode().getName(), child,
      ...
      if (treeTop->getNode()->getNumChildren() == 1 &&
          ...
          treeTop->getNode()->getFirstChild() == child) {
        if (performTransformation(
                comp, "%sFound commoned reference to single child %s (%p) case "
                      "2 so unlink %s (0x%p)\n",
                optDetails, child->getOpCode().getName(), child,
...
void TR_TrivialDeadTreeRemoval::transformBlock(TR::TreeTop *entryTree,
  ...
  if (comp()->getOption(TR_TraceTrivialDeadTreeRemoval))
    traceMsg(comp(), "TrivialDeadTreeRemoval on block_%d : entryTreeNode %p -> "
                     "exitTreeNode %p\n",
...
int32_t TR_TrivialBlockExtension::performOnBlock(TR::Block *block) {
  ...
  if (block->isExtensionOfPreviousBlock()) {
    if (trace())
      traceMsg(comp(), "BlockExtension: block_%d is already an extension of "
                       "the previous block\n",
               block->getNumber());
      ...
      if (trace())
        traceMsg(comp(),
                 ...
                 pred->getNumber(), block->getNumber());
                   ...
                   .isJumpWithMultipleTargets()) {
      if (trace())
        traceMsg(comp(), "BlockExtension: block_%d ends in a switch and so we "
                         "will not mark block_%d as an extension\n",
                 pred->getNumber(), block->getNumber());
    } else {
      if (performTransformation(comp(), "O^O BLOCK EXTENSION: Mark block_%d as "
                                        "an extension of block_%d\n",
