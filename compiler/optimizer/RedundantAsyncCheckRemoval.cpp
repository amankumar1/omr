...
bool TR_RedundantAsyncCheckRemoval::shouldPerform() {
  ...
  if (comp()->getOption(TR_EnableOSR)) // cannot move async checks around
                                       // arbitrarily if OSR is possible
    return false;
  ...
  if (comp()->getMethodHotness() <= warm || !comp()->mayHaveLoops()) {
    ...
    if ((uint32_t)comp()->getNodeCount() > numNodesInLargeMethod ||
      ...
      _numAsyncChecksInserted +=
          TR_AsyncCheckInsertion::insertReturnAsyncChecks(comp());
...
TR_RedundantAsyncCheckRemoval::processBlockStructure(TR_BlockStructure *b) {
  ...
  for (treeTop = firstBlockEntry; treeTop != block->getExit();
    ...
    if (info->canHaveAYieldPoint() && containsImplicitInternalPointer(node)) {
      ...
      // we recognize portions of extended basic blocks as "grey-areas" where
      // its
...
bool TR_RedundantAsyncCheckRemoval::isMaxLoopIterationGuardedLoop(
  ...
  // explanation
  // we start at the node representing the loop in the parent region.  What we
  // want to do
  // is to find a dominating if-block that contains the max-loop-iteration guard
  // for this
  // loop.  Traditionally, the guard is not the parent region, but infact there
  // are several
  // other versioning guards intervening between the loop and the iter guard.
  // Each of
  ...
  // loop region.
  // By definition there must exist an acyclic path between the loop and the
  // guard.  Ignore
  // backedges and just walk the preds.  There cannot be a region structure en
  // route
  // since that would imply that there was some control flow not typical of
  // guarded versioned
  // loops.  If we hit the entry of the parent region, we start the same process
  // in the
  // parent of the parent region, until we hit the start-node, which implies
  // failure.
  //
  ...
  // backwards
  // reverse post order walk, however there is never more than one element in
  // the queue.
  // NOTE: there is no use of visit counts in this loop, a node can never be
  // visited twice
...
bool TR_RedundantAsyncCheckRemoval::originatesFromShortRunningMethod(
  ...
  // running unless
  // they contain loops that are inlined from methods outsides the above
  // classes.
  // The following is an algorithm to detect such loops. In the algorithm, we
  // assume that if all branches of all basic blocks of a loop
...
TR_RedundantAsyncCheckRemoval::processNaturalLoop(TR_RegionStructure *region,
  ...
  if (!isInsideImproperRegion) {
    ...
    if ((comp()->getMethodHotness() == scorching) &&
      ...
      isShortRunning = true;
      if (trace())
        traceMsg(comp(), "Loop %d originates from a trusted method, and "
                         "therefore, is tagged as short running. Skipping\n",
                 region->getNumber());
    }
...
void TR_RedundantAsyncCheckRemoval::markAncestors(
  ...
  for (auto edge = node->getPredecessors().begin();
    ...
    // yield
    // point in it - and the next block cannot have a yield point in it - then
    // mark
...
int32_t TR_RedundantAsyncCheckRemoval::processImproperRegion(
  ...
  // optimal
  // solution for an improper region, and leave it in its original form, hence
  // being
  // conservatively correct.  However, any inner loops of the region are
  // analyzed
  // normally: they execute an AC definitely, before and after the
  // transformation, so
...
// BFS, with
// the only difference that we analyzed a node only after all its successors
// have been
// analyzed
//
    ...
    if (candidates.isSet(refNum)) {
      ...
      if (einfo->isUnknownValue() && iinfo->getKind() != Geometric) {
        ...
        if (iinfo->getKind() == Geometric) {
          ...
          // number
          // of loop iterations in bounded by 32.  We don't really care about
          // the
          ...
          // messed up induction variable info
            ...
            // number of
            // iterations is calculated to be a negative number, because of
            // commoning.
            // Ignore these cases. Usually the diff is off the actual exact
            // value
