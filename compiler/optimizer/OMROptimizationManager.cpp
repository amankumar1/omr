...
OMR::OptimizationManager::OptimizationManager(
    ...
    const char *optDetailString, const OptimizationStrategy *groupOfOpts)
    : _optimizer(o), _factory(factory), _id(optNum), _groupOfOpts(groupOfOpts),
      _numPassesCompleted(0), _optData(NULL), _optPolicy(NULL), _enabled(false),
      _requested(false), _requestedBlocks(o->trMemory()),
