...
TR_LoadExtensions::TR_LoadExtensions(TR::OptimizationManager *manager)
    : TR::Optimization(manager), _counts(0), _seenLoads(0), _useDefInfo(NULL) {
  setTrace(comp()->getOptions()->getOptsToTrace() &&
  ...
  for (TR::TreeTop *tt = comp()->getStartTree(); tt;
    ...
    TR_ASSERT(node->getVisitCount() != visitCount,
              "Code Gen: error in marking load nodes");
    nodeCount = indexNodesForCodegen(node, nodeCount, visitCount);
  }

  _signExtensionFlags = new (trHeapMemory())
      TR_BitVector(nodeCount * TR::Node::numSignExtensionFlags, trMemory(),
                   heapAlloc, growable);

  TR::SparseBitVector extendedToInt64GlobalRegisters =
...
int32_t TR_LoadExtensions::perform() {
  ...
  if (
      ...
      0 &&
#endif
      (comp()->getOptLevel() > noOpt) && _seenLoads & 0x2 && optimizer() &&
      !optimizer()->cantBuildGlobalsUseDefInfo()
#ifdef REUSE_USEDEFS
      && (NULL == (_useDefInfo = optimizer()->getUseDefInfo()) ||
          !_useDefInfo->hasGlobalsUseDefs() || !_useDefInfo->_useDefForRegs)
#endif
          ) {
    if (!comp()->getFlowGraph()->getStructure())
      optimizer()->doStructuralAnalysis();

    TR::LexicalMemProfiler mp("use defs (Load Extensions)",
                              comp()->phaseMemProfiler());

    optimizer()->setUseDefInfo(NULL);

    _useDefInfo = new (comp()->allocator())
        TR_UseDefInfo(comp(), comp()->getFlowGraph(), optimizer(), false, false,
                      false, true, true);
...

  {
    ...
    _counts = (int32_t *)trMemory()->allocateStackMemory(
        comp()->getNodeCount() * sizeof(int32_t), TR_Memory::CodeGenerator);
...
void TR_LoadExtensions::countLoad(TR::Node *load, TR::Node *conversion) {
  ...
  {
    if (trace())
      traceMsg(comp(), "\t\tSigned %p %p\n", conversion, load);
    _counts[load->getGlobalIndex()] += totalFlags;
  } else if (conversion->isZeroExtension() ||
             opcode.isUnsigned()) // i.e. TR::bu2i || TR::buadd
  {
    if (trace())
      traceMsg(comp(), "\t\tUnsigned %p %p\n", conversion, load);
    _counts[load->getGlobalIndex()] -= totalFlags;
  } else // i.e. TR::isub
  {
    if (trace())
      traceMsg(comp(), "\t\tSignedd %p %p\n", conversion, load);
...
bool TR_LoadExtensions::supportedConstLoad(TR::Node *load, TR::Compilation *c) {
  TR_ASSERT(load->getOpCode().isLoadConst(),
            "Only meant to be called by const loads.");
...
#define MIN_IMMEDIATE_VAL -32768 // 0x8000
  ...
  case TR::buconst:
    value = load->getByte() & 0xFF;
    break;
  case TR::bconst:
    value = load->getByte();
    break;
  case TR::cconst:
    value = load->getConst<uint16_t>();
    break;
  case TR::sconst:
    value = load->getShortInt();
    break;
  case TR::iconst:
    value = load->getInt();
    break;
...
void TR_LoadExtensions::countLoadExtensions(TR::Node *parent,
                                            vcount_t visitCount) {
  ...
  if (opcode.isConversion() && supportedType(parent)) {
    ...
    if ((load->getOpCode().isLoadVar() || load->getOpCode().isLoadReg() ||
         load->getOpCode().isLoadConst()) &&
        supportedType(load) && parent->getSize() != load->getSize()) {
      if (load->getOpCode().isLoadReg() ||
          (load->getOpCode().isLoadVar() &&
           load->getSymbol()->isRegisterSymbol())) {
        if (optimizer() && _useDefInfo && _useDefInfo->infoIsValid() &&
            load->getUseDefIndex() &&
            _useDefInfo->isUseIndex(load->getUseDefIndex()) &&
            (parent->getSize() <
             8)) // On Java disable i2l, because of interferance with HRA
        {
          ...
          if (!info.IsZero()) {
            if (trace())
              traceMsg(
                  comp(),
                  "\t\tPeeking through RegLoad %p for Conversion %p (%s)\n",
            ...
            for (cursor.SetToFirstOne(); cursor.Valid();
              ...
              if ((defNode->getOpCode().isStoreReg() ||
                   (defNode->getOpCode().isStore() && defNode->getSymbol() &&
                    defNode->getSymbol()->isRegisterSymbol())) &&
                  NULL != (realLoad = defNode->getFirstChild()) &&
                  ((realLoad->getOpCode().isLoadVar() &&
                    !realLoad->getSymbol()->isRegisterSymbol()) ||
                   realLoad->getOpCode().isLoadConst() &&
                       supportedConstLoad(realLoad, comp())) &&
                  supportedType(realLoad)) {
                if (trace())
                  traceMsg(comp(), "\t\tPeeked through %p (%s) and found %p "
                                   "(%s) with child %p (%s), Counting.\n",
                           load, load->getOpCode().getName(), defNode,
                           defNode->getOpCode().getName(), realLoad,
                           realLoad->getOpCode().getName());
                countLoad(realLoad, parent);
              } else {
                if (trace())
                  traceMsg(comp(), "\t\tPeeked through %p (%s) and found %p "
                                   "(%s) with child %p (%s), setting Ignore.\n",
                           load, load->getOpCode().getName(), defNode,
                           defNode->getOpCode().getName(), realLoad,
                           realLoad ? realLoad->getOpCode().getName() : "NULL");
                setOverrideOpt(parent, regLoadOverride);
              }
            }
          }
  ...
  for (int32_t i = 0; i < parent->getNumChildren(); i++) {
    ...
    // See how a conversion is being used. A narrowing conversion may be
    // unneeded as long as it feeds only:
    //  - conversions
    //  - stores
    //
    // (Do counting for all conversions. Whether it is a narrowing conversion is
    // verified at the flag-setting stage)
    if (child->getOpCode().isConversion()) {
      if (opcode.isStoreReg() || (opcode.isStore() && parent->getSymbol() &&
                                  parent->getSymbol()->isRegisterSymbol())) {
        ...
        setOverrideOpt(child, narrowOverride);
      // We counted all the loads feeding into conversions (before the loop),
      // now count the other references to loads
      else if (!opcode.isConversion() && child->getOpCode().isLoadVar() &&
               supportedType(child))
        countLoad(child, parent);
    }

    // See how a load is being used. If a load feeds only to narrowing
    // conversions, we may set couldIgnoreExtend() flag on the load
    // WARNING: "Clever idea" -- set the LSB, to mean that does _not_ feed into
    // a narrowing conversion, use other bits as int
 ...
 *
 * This function returns false for patterns we might ignore a narrowing
 * conversion for
 * @param parent
 * @param conversion
 * @return
 */
bool TR_LoadExtensions::detectReverseNeededConversionPattern(
    TR::Node *parent, TR::Node *conversion) {
  TR::ILOpCode &opcode = parent->getOpCode();

  // Many programmers zero out top bits with AND. Calculate the AND mask that
  // will result in conversion being unneeeded
  // i.e.
  ...
  if (opcode.isAnd() &&
      (conversion->getSize() - conversion->getFirstChild()->getSize()) >
          0) // Must be widening conversion
  {
    TR::Node *constant;
    ...
    if ((NULL != (constant = parent->getFirstChild()) &&
        ...
        (constant->get64bitIntegralValue() == andMask2 ||
         conversion->getOpCode().isZeroExtension() &&
             constant->get64bitIntegralValue() == andMask))
      return false;
  } else if (opcode.isConversion() ||
             (opcode.isStore() && !opcode.isStoreReg() &&
              !(parent->getSymbol() &&
                parent->getSymbol()->isRegisterSymbol())))
    return false;
...
bool TR_LoadExtensions::detectUnneededConversionPattern(TR::Node *conversion,
                                                        TR::Node *child,
                                                        bool &mustForce64) {
  TR::ILOpCode &opcode = conversion->getOpCode();
  ...
  TR::Node *grandChild;
  if (trace())
    traceMsg(comp(), "\t\tLooking at conversion %p%s%s\n", conversion,
  ...
  if (supportedType(child)) {
     ...
     *   - sign of load does not match (i.e. RefCnt > 1, load is being used with
     * different signedness, odd)
     * TODO:
     *   - addresses?
     *   - conv
     *       iload <Fixed24>
    ...
    if (((childOpcode.isLoadVar() &&
          !(child->getSymbol()->isRegisterSymbol())) ||
         childOpcode.isLoadConst() && supportedConstLoad(child, comp())) &&
        isConvWide &&
        (conversion->getSize() != 8 || (TR::Compiler->target.is64Bit() ||
                                        comp()->cg()->use64BitRegsOn32Bit())) &&
        (loadIsSigned && loadIsSigned == opcode.isSignExtension() ||
         !loadIsSigned && (!loadIsSigned) == conversion->isZeroExtension()) &&
        (opcode.getOpCodeValue() != TR::a2l ||
         child->getReferenceCount() == 1 &&
             TR::Compiler->target
                 .is32Bit())) // 4 byte a2l really means 31->64! careful!
    {
      if (trace())
        traceMsg(comp(), "\t\tDetected Sign Extension pattern on conversion "
                         "and load nodes %p\n",
                 conversion);
    ...
    //  - conversions
    //  - stores
    // If above is not true, counts[node->getGlobalIndex()] would had been set
    // to true/1
    // l2i can be also ignored: will either grab the reg->getLowOrder() or keep
    // on passing GPR64
    else if (!isConvWide &&
             (!isOverriden(conversion, narrowOverride) ||
      ...
      if (trace())
        traceMsg(comp(), "\t\tDetected Sign Extension pattern on narrowing "
                         "conversion node %p\n",
                 conversion);
      return true;
    } else if (childOpcode.isRightShift() &&
               child->getSecondChild()->getOpCode().isLoadConst() &&
               child->getSecondChild()->getInt() ==
                   (child->getSize() - conversion->getSize())) //(16|24|32...?)
    {
      if (trace())
        traceMsg(comp(), "\t\tDetected Sign Extension pattern on conversion "
                         "and right shift nodes %p\n",
                 conversion);
      return true;
    } else if (childOpcode.isBooleanCompare() &&
               conversion->getSize() !=
                   8) // Needs a conversion from GPR32 to GPR64 or 2xGPR32
    {
      if (trace())
        traceMsg(comp(), "\t\tDetected Sign Extension pattern on conversion "
                         "and compare nodes %p\n",
    ...
    // Only for  PLX
    else if (isConvWide && conversion->getSize() == 4 &&
             cg()->isUsing32BitEvaluator(child) && child->cannotOverflow() &&
             ((opcode.isSignExtension() && !childOpcode.isLoadReg()) ||
              child->isNonNegative())) {
      if (trace())
        traceMsg(comp(), "\t\tDetected Sign Extension s2i/b2i pattern on "
                         "ssub/sadd, bsub/badd and/or sRegLoad/bRegLoad %p\n",
                 conversion);
      return true;
    }
#endif
...
void TR_LoadExtensions::setPreferredExtension(TR::Node *node,
                                              vcount_t visitCount) {
  ...
  if (supportedType(node)) {
    if (opcode.isLoadVar() ||
        (opcode.isLoadConst() && supportedConstLoad(node, comp()))) {
      ...
      if (child->getSize() == node->getSize()) {
        ...
        {
          if (child->getSize() >= 2 && (node->getOpCodeValue() == TR::bu2i ||
            ...
            node->setUnneededConversion(true);
                  ...
                  (childOpcode.isLoadVar() &&
                   child->getSymbol()->isRegisterSymbol())) &&
                 !(node->getSize() > 4 && TR::Compiler->target.is32Bit()) &&
                 !isOverriden(node, regLoadOverride) && supportedType(child)) {
        TR_ASSERT(optimizer() && _useDefInfo && _useDefInfo->infoIsValid() &&
                      child->getUseDefIndex() &&
                      _useDefInfo->isUseIndex(child->getUseDefIndex()),
                  "No UseDef info available, yet override flag not set!\n");

        TR_UseDefInfo::BitVector info(comp()->allocator());
        _useDefInfo->getUseDef(info, child->getUseDefIndex());
        TR_ASSERT(!info.IsZero() || !childOpcode.isLoadReg(),
                  "Could not get UseDef info for RegLoad!");
        TR_UseDefInfo::BitVector::Cursor cursor(info);
        int32_t firstDefIndex = _useDefInfo->getFirstRealDefIndex();
        int32_t firstUseIndex = _useDefInfo->getFirstUseIndex();
        isUnneeded = true;
        ...
        for (cursor.SetToFirstOne(); isUnneeded && cursor.Valid();
             cursor.SetToNextOne()) {
          int32_t defIndex = cursor;
          if (defIndex >= firstUseIndex)
            break;
          ...
          isUnneeded &=
              detectUnneededConversionPattern(node, realLoad, mustForce64Case);
          mustForce64All |= mustForce64Case;
          mustForce64 &= mustForce64Case;
          isUnneeded &= (mustForce64 == mustForce64All); // i.e. break?
          if (trace())
            traceMsg(comp(), "\t\tPeeked through %p (%s) and found %p (%s) "
                             "with child %p (%s), evaluate %s%s.\n",
                     child, child->getOpCode().getName(), defNode,
                     defNode->getOpCode().getName(), realLoad,
                     realLoad->getOpCode().getName(),
        ...
        if (isUnneeded &&
            performTransformation(
                comp(), "%sFound Unneeded conversion %p after RegLoad\n",
                OPT_DETAILS, node)) {
          if (isUnneeded && mustForce64) {
            ...
            for (cursor2.SetToFirstOne(); cursor2.Valid();
              ...
              if (!defNode)
                continue;
              if (trace())
                traceMsg(comp(), "\t\t\tSetting force64 on %p (%p)\n",
                         defNode->getFirstChild(), child);
              defNode->getFirstChild()->setForce64BitLoad(true);
            }
          }
          node->setUnneededConversion(isUnneeded);
          if (node->getType().isInt64() && node->getSize() > child->getSize()) {
            if (trace())
              traceMsg(comp(), "\t\t\tset num %d (%s) in "
            ...
            // getExtendedToInt64GlobalRegisters is used by the evaluators to
            // force a larger virtual register to be used when
            // evaluating the regload so any instructions generated by local RA
            // are the correct size to preserve the upper bits
            cg()->getExtendedToInt64GlobalRegisters()
      ...
      if (!isUnneeded) {
        ...
        if (isUnneeded &&
            performTransformation(comp(), "%sFound Unneeded conversion %p\n",
                                  OPT_DETAILS, node)) {
          if (mustForce64)
 ...
 */
ncount_t TR_LoadExtensions::indexNodesForCodegen(TR::Node *parent,
                                                 ncount_t nodeCount,
                                                 vcount_t visitCount) {
  if (parent->getVisitCount() == visitCount)
    return nodeCount;
  ...
  if (opcode.isLoadVar() && supportedType(parent) ||
    ...
    if (trace())
      traceMsg(cg(), "Marking node %p as %d [children: %d]\n", parent,
               _nodeCount, parent->getNumChildren());
    TR_ASSERT(parent->getNumChildren() < 2,
              "Cannot reuse the second child slot, already taken\n");
  ...
  if (opcode.isLoadReg() ||
      (opcode.isLoadVar() && parent->getSymbol()->isRegisterSymbol()))
    _seenLoads |= 2;

  // The following line should not be needed, especially in case somebody really
  // wants to mark conversion nodes as unneeded earlier
  // but I am tired of finding bugs with flags incorrectly set (i.e. changing
  // Node opcode and inheriting -incorrect/overloaded- flags)
  // As far as I know, nobody else is using these flags in other optimizations
  // yet.
  // Another option is to move the unneededConversion flag to its own bitvector,
  // as with load flags. That is also more work because
  // second-child in conversion nodes may already be sometimes used by
  // <code>bool TR_DynamicLiteralPool::addNewAloadChild(TR::Node *node)</code>
  // (the second child is being used instead of globalIndex)
  if (!comp()->getOptions()->getDebugEnableFlag(
  ...
  for (int32_t i = 0; i < parent->getNumChildren(); i++) {
    _nodeCount =
        indexNodesForCodegen(parent->getChild(i), _nodeCount, visitCount);
  }
