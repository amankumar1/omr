...
void TR::GlobalSet::collectReferencedAutoSymRefs(TR::Block *BB) {
  ...
  for (TR::TreeTop *tt = BB->getFirstRealTreeTop(); tt != BB->getExit();
       tt = tt->getNextTreeTop())
    collectReferencedAutoSymRefs(
        tt->getNode(), _refAutosPerBlock[BB->getNumber()], visitCount);
}

void TR::GlobalSet::collectReferencedAutoSymRefs(TR::Node *node,
                                                 Set *referencedAutoSymRefs,
                                                 vcount_t visitCount) {
      ...
      if (!indirectMethodMetaUses.IsZero()) {
        ...
        for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
          ...
          TR::SymbolReference *usedSymRef =
              _comp->getSymRefTab()->getSymRef(aliasesCursor);
...
TR_RegisterCandidate::TR_RegisterCandidate(TR::SymbolReference *sr,
                                           TR_Memory *m, TR::Allocator a)
    : _symRef(sr), _splitSymRef(NULL), _restoreSymRef(NULL), _lowRegNumber(-1),
      _highRegNumber(-1), _allBlocks(false), _failedToAssignToARegister(false),
      _liveOnEntry(), _liveOnExit(), _originalLiveOnEntry(),
      _8BitGlobalGPR(false), _reprioritized(0), _blocks(m, a),
      _valueModified(false), _liveAcrossExceptionEdge(false),
      _loopExitBlocks(m), _stores(m), _loopsWithHoles(m),
      _mostRecentValue(NULL), _lastLoad(NULL), _highWordZero(false),
      _dontAssignVMThreadRegister(false), _initialBlocksWeightComputed(false),
      _extendedLiveRange(false), _availableOnExit(NULL), _blocksVisited(NULL) {}
...
TR::Symbol *TR_RegisterCandidate::getSymbol() { return _symRef->getSymbol(); }
...
TR_RegisterCandidates::TR_RegisterCandidates(TR::Compilation *comp)
    : _compilation(comp), _trMemory(comp->trMemory()),
      _candidateTable(128, comp->allocator()),
      _internalCalls(comp->allocator()), _internalCallsComputed(false),
...
TR_RegisterCandidate *TR_RegisterCandidates::find(TR::SymbolReference *symRef) {
  if (_candidateForSymRefsSize &&
      symRef->getReferenceNumber() >
          _candidateForSymRefsSize) // some spots in GRA add more candidates
                                    // after _candidateForSymRefs was created in
                                    // certain cases (ex if there are unused
                                    // parameters, offerAllAutos may create a
                                    // new symref)
  {
...
void TR_RegisterCandidate::addBlock(TR::Block *block,
                                    ...
                                    TR_Memory *m, bool ifNotFound) {
  if (find(block)) {
    if (!ifNotFound)
      _blocks.incNumberOfLoadsAndStores(block->getNumber(),
                                        numberOfLoadsAndStores);
  } else
    _blocks.setNumberOfLoadsAndStores(block->getNumber(),
                                      numberOfLoadsAndStores);
}
...
void TR_RegisterCandidate::setWeight(
  ...
  _originalLiveOnEntry.init(numberOfBlocks, comp->trMemory(), stackAlloc,
                            growable);
  _loadsAndStores = new (comp->trStackMemory())
      TR_Array<uint32_t>(comp->trMemory(), numberOfBlocks, true, stackAlloc);
  ...
  while (bc.hasMoreElements()) {
    ...
    bool hasLoadNearStart = !isExtensionOfPreviousBlock.isSet(blockNumber) &&
                            findLoadNearStartOfBlock(b, getSymbolReference());
    TR_ASSERT((blockNumber < cfg->getNextNodeNumber()) &&
    ...
    if (((firstBlock && _allBlocks && cg->getSupportsGlRegDepOnFirstBlock()) ||
      ...
      static bool recalcWeights =
          feGetEnv("TR_GRA_RecalculateBlockWeights") ? true : false;

      TR_BlockStructure *blockStructure;
      if (recalcWeights && blockWeight > 0 &&
      ...
      TR_BlockStructure *extendedBlockStructure =
          extendedBlock->getStructureOf();
      ...
      // traceMsg(comp, "for cand %d blockWeight %d b %d extended b %d live
      // %d\n", getSymbolReference()->getReferenceNumber(), blockWeight,
      // b->getNumber(), extendedBlock->getNumber(), symbolIsLive(b));
      ...
      // We should not mark a candidate as being referenced in the extended
      // block if the
      // only references to it are in the portion of the extended block that is
      // outside a loop
      // This is important as we cannot recognize that a candidate is not
      // referenced at all in a loop
      // otherwise. We see a 'false' use in the extended basic block that
      // started inside the loop because
      ...
      if (extendedBlockFreq <= blockFreq) {
        ...
        if ((uint32_t)blockWeight >
            (*_loadsAndStores)[extendedBlock->getNumber()])
...
void TR_RegisterCandidate::processLiveOnEntryBlocks(
  ...
  // can escape via return but there is no real loads
  // so we do not want such live on entry blocks to contribute to weigh of the
  // candidate
  // TR_BitVector *ignoreLiveOnEntryBlocks = new (comp->trHeapMemory())
  ...
  while (lookForBlocksToRemove) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (trace)
        dumpOptDetails(comp, "Examining original live-on-entry block_%d with "
                             "loads and stores = %d\n",
  ...
  while (bvi.hasMoreElements()) {
    int32_t blockNumber = bvi.getNextElement();
    if (trace)
      dumpOptDetails(comp, "Examining live-on-entry block_%d with loads and "
                           "stores = %d to find maxFrequency\n",
                     blockNumber, (*_loadsAndStores)[blockNumber]);
    TR::Block *block = blocks[blockNumber];
  ...
  if ((maxFrequency > 1) && removeUnusedLoops) {
    ...
    while (lookForBlocksToRemove) {
             ...
             // maxFrequency later
      {
        ...
        // blocks
        // before we decide to discard any; experiment more with that; in that
        // case use the
                    ...
                    for (nextBlock = blocksIt.getCurrent(); nextBlock;
                      ...
                      if (nextBlock->getEntry()) {
                        ...
                        if (_liveOnEntry.get(liveBlock->getNumber())) {
                          if (((*_loadsAndStores)[liveBlock->getNumber()] !=
                               0) &&
              ...
              if (!referencedInNearestLoop) {
                ...
                if (parent && !unreferencedLoops.find(parent)) {
                  ...
                  for (TR::CFGEdge *edge = ei.getCurrent(); edge != NULL;
                    ...
                    //      {
                    //      toFrequency =
                    //      succStructure->getEntryBlock()->getFrequency();
                  ...
                  if (shouldResetThisLoop) {
                    if (trace) {
                      dumpOptDetails(
                          comp, "\n1Loop %d does not use candidate, removing "
                                "the following blocks from live ranges of "
                                "candidate #%d \n",
                          parent->getNumber(),
                          getSymbolReference()->getReferenceNumber());
                ...
                if (shouldResetThisLoop) {
                  if (trace)
                    dumpOptDetails(comp, "\n2Loop does not use candidate, "
                                         "removing block_%d from live ranges "
                                         "of candidate #%d \n",
                                   blockNumber,
                                   getSymbolReference()->getReferenceNumber());
  ...
  while (bvi.hasMoreElements()) {
    ...
    if (trace)
      dumpOptDetails(comp, "Examining live-on-entry block_%d (blockWeight %d "
                           "maxFrequency %d) numberOfBlocks %d loadsAndStores "
                           "%d",
    ...
    for (auto e = block->getPredecessors().begin();
      ...
      if (((predWeight > blockWeight) && !liveOnExitFromPred &&
          ...
          (*_loadsAndStores)[startOfExtendedBBForBB[pred->getNumber()]
                                 ->getNumber()] == 0 &&
  ...
  while (bvi.hasMoreElements()) {
    ...
    if (!_liveOnEntry.get(blockNumber) &&
      ...
      if (trace)
        traceMsg(comp, "Increased weight of candidate %d by %d (new weight %d) "
          ...
          if (((*succ)->getTo() != block->getNextBlock()) &&
            ...
            if (!_liveOnEntry.get((*succ)->getTo()->getNumber())) {
              ...
              if (!exclude && blockStructure &&
                ...
                if (trace)
                  traceMsg(comp, "Reduced weight of candidate %d by %d (new "
                                 "weight %d) because of end in live range at "
                                 "block_%d succ %d\n",
                           getSymbolReference()->getReferenceNumber(),
                           blockWeight, loadsAndStores, blockNumber,
          ...
          if ((*succ)->getTo() != block->getNextBlock()) {
            if (_liveOnEntry.get((*succ)->getTo()->getNumber())) {
              ...
              if (!exclude && blockStructure &&
                ...
                if (trace)
                  traceMsg(comp, "Reduced weight of candidate %d by %d (new "
                                 "weight %d) because of start of live range at "
                                 "block_%d succ %d\n",
                           getSymbolReference()->getReferenceNumber(),
                           blockWeight, loadsAndStores, blockNumber,
                           (*succ)->getTo()->getNumber());
  ...
  bool isPPS = false; // (comp->getOption(TR_EnableOSR) && getSymbol()->isAuto()
                      // && (getSymbolReference()->getCPIndex() < 0)); // This
                      // hurts in WAS performance; so it is disable until I can
  ...
  if (numberOfBlocks && isPPS) {
    ...
    _weight = INT_MAX - 1;
    if (trace)
      dumpOptDetails(comp, "For candidate #%d weight = %d loadsAndStores = %d "
                           "numberOfBlocks = %d\n",
...
// exit from all its
// successors. Moreover, it will be live from the time it is first referenced in
// the successor
    ...
    if (!block->isExtensionOfPreviousBlock()) {
      for (auto e = block->getPredecessors().begin();
        ...
        do {
          ...
          TR::GlobalSet::Set *autosInBlock =
              comp->getGlobalRegisterCandidates()
                  ->getReferencedAutoSymRefsInBlock(currBlock->getNumber());
          if (autosInBlock &&
        ...
        do {
          ...
          if (currBlock->isExtensionOfPreviousBlock())
            availableInPrevBlock =
                getAvailableOnExit()->get(
                    currBlock->getPrevBlock()->getNumber()) != 0;

          if (!_liveOnEntry.get(currBlock->getNumber()) && // not set already
              _liveOnExit.get(currBlock->getNumber()) && availableInPrevBlock &&
              !getAvailableOnExit()->get(currBlock->getNumber())) {
            if (trace)
              traceMsg(comp, "\tFor candidate #%d, set live on block_%d entry "
                             "due to live on exit\n",
                       getSymbolReference()->getReferenceNumber(),
                       currBlock->getNumber());
...
bool TR_RegisterCandidates::candidatesOverlap(
  ...
  // seeing a reference
  // to the exit symbol.
  // Note, since a RegStore will have to be inserted to switch from one register
  // to another,
  // if both symbol are used in the same tree, they are overlapping regardless
  // of which order
...
TR_RegisterCandidate *TR_RegisterCandidates::reprioritizeCandidates(
  ...
  if (!successorBits) {
    ...
    // TR_Array<int32_t> blockFPRCount(trMemory(),
    // numberOfBlocks,true,stackAlloc);
    ...
    if (rc->getBlocksLiveOnEntry().intersects(*successorBits)) {
      if ((!onlyReprioritizeLongs ||
          ...
          ((reprioritizeFP && isFPCandidate) ||
           (!reprioritizeFP && !isFPCandidate))) {
        if (trace) {
          dumpOptDetails(comp, "\nBefore repriortization, removing the "
                               "following blocks from live ranges of candidate "
                               "#%d \n",
...
static void scanPressureSimulatorCacheForConflicts(
  ...
  for (i = firstRegister; i <= lastRegister; ++i) {
    if ((i == comp->cg()->getVMThreadGlobalRegisterNumber()) &&
        rc->dontAssignVMThreadRegister())
      continue;
  ...
  if (trace) {
    ...
    while (regIterator.hasMoreElements()) {
      ...
      if (firstRegister <= regNum && regNum <= lastRegister) {
        ...
        // comp->getDebug()->print(comp->getOptions()->getLogFile(),
        // &liveOnEntryConflicts[regNum]);
        traceMsg(comp, "\n");
        traceMsg(
            comp,
...
void TR_RegisterCandidates::collectCfgProperties(TR::Block **blocks,
  ...
  // however, profiles have shown that
  // indexing blocks followed by extracting information from has significant
  // data cache misses. I'm hopeful
  // that bundling this info together into a bit vector to be referenced
  // repeatedly for each RegisterCandidate
...
bool TR_RegisterCandidates::assign(TR::Block **cfgBlocks,
  ...
  TR_BitVector *availableOnExit = new (comp()->trStackMemory()) TR_BitVector(
      comp()->getFlowGraph()->getNextNodeNumber(), comp()->trMemory());
  ...
  // invariant to the candidate
  // and only depend on the structure of the CFG. This will speed up computing
  // the heuristics.
   ...
   for (rc = first; rc; assign_candidate_loop_trace_increment(comp(), rc, iterationCount), rc = next)
    ...
    if (rc->getFailedToAssignToARegister()) {
      if (trace)
        traceMsg(
            comp(),
            "Leaving candidate because rc->getFailedToAssignToARegister()\n");
      continue;
    }
    ...
    if (true) {
      ...
      while (bvi.hasMoreElements()) {
        ...
        // dumpOptDetails(comp(), "For block_%d numGPRs %d numFPRs %d ext
        // block_%d numGPRs %d numFPRs %d maxVRFs %d\n", b->getNumber(),
        // totalGPRCount[b->getNumber()], totalFPRCount[b->getNumber()],
        // blockNum, totalGPRCount[blockNum], totalFPRCount[blockNum],
        // totalVRFCount[blockNum]);
        ...
        if (isFloat) {
          if (totalFPRCount[blockNum] >=
            ...
            if (trace)
              dumpOptDetails(comp(), "FPR count exceeded at some exit out of "
                                     "extended block_%d: %d >= %d\n",
            ...
            for (TR::CFGEdge *e = ebsi.getFirst(); e; e = ebsi.getNext()) {
              ...
              if (hasMultiWayBranch ||
                  (totalFPRCountOnEntry[b->getNumber()] >=
                   (maxFPRsLiveOnExit[blockNum] - (needs2Regs ? 1 : 0)))) {
                if (trace)
                  dumpOptDetails(comp(), "FPR count exceeded : resetting live "
                                         "on entry block_%d (because of "
                                         "extended block_%d)\n",
        ...
        if (isVector) {
          if (totalVRFCount[blockNum] >= maxVRFsLiveOnExit[blockNum]) {
            if (trace)
              dumpOptDetails(comp(), "VRF count exceeded at some exit out of "
                                     "extended block_%d: %d >= %d\n",
            ...
            for (TR::CFGEdge *e = ebsi.getFirst(); e; e = ebsi.getNext()) {
              ...
              if (hasMultiWayBranch || (totalVRFCountOnEntry[b->getNumber()] >=
                                        maxVRFsLiveOnExit[blockNum])) {
                if (trace)
                  dumpOptDetails(comp(), "VRF count exceeded : resetting live "
                                         "on entry block_%d (because of "
                                         "extended block_%d)\n",
                                 b->getNumber(), blockNum);
                rc->getBlocksLiveOnEntry().reset(b->getNumber());
          ...
          if (totalGPRCount[blockNum] >=
            ...
            if (trace)
              dumpOptDetails(comp(), "GPR count exceeded at some exit out of "
                                     "extended block_%d: %d >= %d\n",
            ...
            for (TR::CFGEdge *e = ebsi.getFirst(); e; e = ebsi.getNext()) {
              ...
              if (hasMultiWayBranch ||
                  (totalGPRCountOnEntry[b->getNumber()] >=
                   (maxGPRsLiveOnExit[blockNum] - (needs2Regs ? 1 : 0)))) {
                if (trace)
                  dumpOptDetails(comp(), "GPR count exceeded : resetting live "
                                         "on entry block_%d (because of "
                                         "extended block_%d)\n",
                                 b->getNumber(), blockNum);
                rc->getBlocksLiveOnEntry().reset(b->getNumber());
    ...
    /* edTODO : This check should be enabled always, not only when reg pressure
     * is enabled */
    if (!comp()->getOption(TR_DisableRegisterPressureSimulation) &&
        comp()->cg()->supportsHighWordFacility() &&
        !comp()->getOption(TR_DisableHighWordRA)) {
      // 64bit values clobber highword registers on zGryphon with HPR support
      // if the HPR is not available, we cannot assign the corresponding GPR to
      // 64bit symbols
      if (!rc->getType().isInt8() && !rc->getType().isInt16() &&
          !rc->getType().isInt32()) {
        for (int8_t i = firstRegister; i <= lastRegister; ++i) {
          if (!availableRegisters.isSet(i) && cg->isGlobalHPR(i)) {
            ...
            if (trace) {
              traceMsg(comp(), "%s is unavailable and RC is 64bit, ",
      ...
      // For symmetry, the converse is done : if the GPR is used for 64-bit
      // value, we cannot assign an HPR to its highword.
      {
        for (int8_t i = firstRegister; i <= lastRegister; ++i) {
          // HPR grn's are a part of GPR grn's but not vice versa!
          if (!availableRegisters.isSet(i) && cg->isGlobalGPR(i) &&
              !cg->isGlobalHPR(i)) {
            TR_GlobalRegisterNumber clobberedHPR = cg->getGlobalHPRFromGPR(i);
            if (trace) {
              traceMsg(comp(), "%s is unavailable, ",
                       cg->getDebug()->getGlobalRegisterName(i));
              traceMsg(comp(), "removing HPR %s from available list since its "
                               "use will clobber the corresponding GPR\n",
        ...
        if (vrbVecGRA && (cg->isGlobalFPR(i) || cg->isGlobalVRF(i)) &&
          ...
          dumpOptDetails(comp(), "ed: alias for %d %s %d %s\t", i,
                         ...
                         cg->getDebug()->getGlobalRegisterName(
                             cg->getOverlappedAliasForGRN(i)));
          dumpOptDetails(comp(), "ffpr %d lfpr %d fopf %d lopr %d fvrf %d lvrf "
                                 "%d fovrf %d lovrf %d\n",
      ...
      for (int i = firstRegister; i <= lastRegister; ++i) {
        ...
        if ((cg->isGlobalFPR(i) || cg->isGlobalVRF(i)) && cg->isAliasedGRN(i) &&
          ...
          TR_GlobalRegisterNumber overlappedReg =
              cg->getOverlappedAliasForGRN(i);
          if (trace) {
            traceMsg(comp(), "removing %s (%d) from available list since it is "
                             "aliased with %s (%d)\n",
    ...
    if ((registerNumber == -1) || (needs2Regs && (highRegisterNumber == -1))) {
      ...
      //
        ...
        intersection.empty();
        List<TR_Structure> regStructureUses(trMemory()); // list of deepest
                                                         // loops where a
                                                         // register is
                                                         // currently live
        ...
        const bool useRegisterPressureInfo =
            ...
            (!trace ||
             performTransformation(
                 comp(), "%s Including register pressure info for trimming "
                         "live range of #%d\n",
        ...
        for (i = firstRegister; i <= lastRegister; ++i) {
          if (trace) {
            traceMsg(comp(), "Searching for blocks/structures with max "
                             "frequency for reg: %d\n",
                     i);
          }
          if ((!useRegisterPressureInfo && availableRegisters.get(i)) ||
              ((i == comp()->cg()->getVMThreadGlobalRegisterNumber()) &&
               rc->dontAssignVMThreadRegister())) {
          ...
          if (!needs2Regs) {
            ...
            while (bvi.hasMoreElements()) {
              ...
              int32_t blockWeight = 1;
              TR::GlobalSet::Set *autosInBlock =
                  getReferencedAutoSymRefsInBlock(block->getNumber());
        ...
        if (conflictingRegister != -1) {
          ...
          if (trace) {
            dumpOptDetails(comp(), "\nDue to conflicts, removing the following "
                                   "blocks from live on entry ranges of "
                                   "candidate #%d conflicting register %d\n",
                           rc->getSymbolReference()->getReferenceNumber(),
                           conflictingRegister);
      ...
      if (b->getGlobalRegisters(comp())[registerNumber]
        ...
        if ((b->getGlobalRegisters(comp())[registerNumber]
            ...
            (rc !=
             b->getGlobalRegisters(comp())[registerNumber]
                 .getRegisterCandidateOnEntry())) {
          traceMsg(comp(), "Candidate %d instead of candidate %d is required "
                           "in register %d on exit out of block_%d\n",
                   b->getGlobalRegisters(comp())[registerNumber]
                       .getRegisterCandidateOnExit()
                       ->getSymbolReference()
                       ->getReferenceNumber(),
...
void TR_RegisterCandidates::computeAvailableRegisters(
  ...
  for (i = firstRegister; i <= lastRegister; ++i) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (rc1 && (!overlaps.Locate(
                      rc1->getSymbolReference()->getReferenceNumber(), hi1) ||
                  !overlaps.Locate(
                      rc->getSymbolReference()->getReferenceNumber(), hi2) ||
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (rc2 && (!overlaps.Locate(
                      rc->getSymbolReference()->getReferenceNumber(), hi1) ||
                  !overlaps.Locate(
                      rc2->getSymbolReference()->getReferenceNumber(), hi2) ||
                  overlaps[hi1].last < overlaps[hi2].first)) {
    ...
    if (_liveOnEntryConflicts[i].isEmpty() &&
        _liveOnExitConflicts[i].isEmpty() && _exitEntryConflicts[i].isEmpty() &&
