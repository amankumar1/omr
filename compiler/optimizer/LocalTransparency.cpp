...
TR_LocalTransparency::TR_LocalTransparency(TR_LocalAnalysisInfo &info, bool t)
   ...
   * returns;
   * so we should consider this when moving the stack memory region's scope
   * earlier
   */
  {
    ...
    *_hasTransparencyInfoFor |= *globalDefinedSymbolReferences;
    ...
    for (nextNode = comp()->getFlowGraph()->getFirstNode(); nextNode;
      ...
      *(_info[nextBlock->getNumber()]._analysisInfo) &= *_supportedNodes;
      ...
      // load/store) being stored into;
      // this is because we cannot compensate for an expression (o.f+1) where
      // o.f is written in the block.
      // We would adjust the load/store of o.f (say with temp t1) but to adjust
      // the value of the temp t2 that
      ...
      *tempContainer |= *definedSymbolReferencesInBlock[nextBlock->getNumber()];
      ...
      for (iter.SetToFirstOne(); iter.Valid(); iter.SetToNextOne()) {
        ...
        if (_hasTransparencyInfoFor->get(nextDefinedSymbolReference)) {
          ...
          // ONLY the store/load
          // of the stored value (say o.f) can still survive. All other
          // dependent expressions must be killed
          ...
          if (!definedSymbolReferencesInBlock[nextBlock->getNumber()]->get(
            ...
            for (bvi.SetToFirstOne(); bvi.Valid(); bvi.SetToNextOne()) {
              ...
              // transparent;
              // i.e. do not kill it because of this store as this store is
              // going to be
              // privatized
              //
        ...
        if (!_supportedNodes->get(child->getLocalIndex()))
          ...
          for (i = 0; i < comp()->getMaxAliasIndex(); i++) {
            if (!_transparencyInfo[i]->get(child->getLocalIndex()) ||
              ...
              if (trace())
                traceMsg(comp(), "Expression %d killed by symRef #%d because "
                                 "child %d is already killed by the symRef\n",
        ...
        if (!_transparencyInfo[i]->get(child->getLocalIndex())) {
          ...
          if (trace()) {
            if (TR::Compiler->target.is64Bit())
              traceMsg(comp(), "Expression %d killed by symRef #%d because "
                               "grandchild (child of aladd) %d is already "
                               "killed by the symRef\n",
                       node->getLocalIndex(), i, child->getLocalIndex());
            else
              traceMsg(comp(), "Expression %d killed by symRef #%d because "
                               "grandchild (child of aiadd) %d is already "
                               "killed by the symRef\n",
                       node->getLocalIndex(), i, child->getLocalIndex());
          }
        }
