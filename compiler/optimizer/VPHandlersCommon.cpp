...
TR::Node *constrainChildrenFirstToLast(TR::ValuePropagation *vp,
                                       TR::Node *node) {
  TR::Node *myParent = vp->getCurrentParent();
...
bool checkMethodSignature(TR::ValuePropagation *vp, TR::SymbolReference *symRef,
                          const char *sig) {
  ...
  if (strncmp(method->getResolvedMethod()->signature(vp->trMemory()), sig,
              strlen(sig)) == 0)
    return true;
  return false;
}

TR::TreeTop *searchForToStringCall(TR::ValuePropagation *vp, TR::TreeTop *tt,
                                   TR::TreeTop *exitTree, TR::Node *newBuffer,
                                   vcount_t visitCount,
                                   TR::TreeTop **toStringTree,
                                   bool useStringBuffer) {
  ...
  for (; tt != exitTree; tt = tt->getNextRealTreeTop()) {
    ...
    if (node->getNumChildren() == 1 &&
      ...
      if (checkMethodSignature(
              vp, node->getFirstChild()->getSymbolReference(),
              (useStringBuffer
...
TR::TreeTop *searchForStringAppend(TR::ValuePropagation *vp, const char *sig,
                                   TR::TreeTop *tt, TR::TreeTop *exitTree,
                                   TR::ILOpCodes opCode, TR::Node *newBuffer,
                                   vcount_t visitCount, TR::Node **string) {
  int len = 0;
  bool isGlobal = false;
  for (; tt != exitTree; tt = tt->getNextRealTreeTop()) {
    TR::Node *node = tt->getNode();
      ...
      if (checkMethodSignature(vp, node->getFirstChild()->getSymbolReference(),
        ...
        if (call->getFirstChild() == newBuffer) {
          if (vp->getConstraint(call->getSecondChild(), isGlobal)) {
            if (vp->getConstraint(call->getSecondChild(), isGlobal)
                    ->getClassType()) {
              const char *sigAppend =
                  vp->getConstraint(call->getSecondChild(), isGlobal)
                      ->getClassType()
                      ->getClassSignature(len);
              if (call->getSecondChild()->getOpCodeValue() == TR::aload &&
                  (!strncmp(sigAppend, "Ljava/lang/String;", 18))) {
...
static int cacheStringAppend(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (checkMethodSignature(vp, node->getSymbolReference(), sigInit)) {
    ...
    TR_ResolvedMethod *m =
        symbol->castToResolvedMethodSymbol()->getResolvedMethod();
    if (strncmp(m->signatureChars(), "(Ljava/lang/String;Ljava/lang/String;)V",
                m->signatureLength()) == 0) {
      vp->_cachedStringPeepHolesVcalls.add(
          new (vp->comp()->trStackMemory()) TR::ValuePropagation::VPTreeTopPair(
  ...
  while (1) {
    ...
    if (useStringBuffer)
      tt = searchForStringAppend(vp, "java/lang/StringBuffer.append(",
                                 startTree, exitTree, TR::acall, child,
                                 visitCount, appendedString + stringCount);
    else
    ...
    if (appendedString[stringCount]) // we found it
    {
      ...
      TR::Symbol *symbol =
          tt->getNode()->getFirstChild()->getSymbolReference()->getSymbol();
      ...
      } else // appending something that needs conversion using valueOf
      {
        ...
        TR_ASSERT(m->signatureLength() >= 3,
                  "The minimum signature length should be 3 for ()V");
      }
      stringCount++;
    ...
    lastAppendTree = tt;
    child = tt->getNode()->getFirstChild(); // the first node is a NULLCHK and
                                            // its child is the call
  }                                         // end while
  ...
  TR_ASSERT(lastAppendTree,
            "If stringCount <=2 then we must have found an append");
  ...
  vp->_cachedStringBufferVcalls.add(
      new (vp->comp()->trStackMemory()) TR::ValuePropagation::VPStringCached(
          appendTree[0], appendTree[1], appendedString[0], appendedString[1],
          newTree, toStringTree));
}
TR::Node *constrainVcall(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (node->getSymbolReference() == finalizeSymRef) {
    ...
    // the stack. That's ensured
    // by virtue of (indirectly) calling bool
    ...
    if (canBeRemoved &&
      ...
      /// traceMsg(vp->comp(), "found opportunity to remove call %p to
      /// checkfinalize\n", node);
      vp->removeNode(node);
