...
TR_IsolatedStoreElimination::TR_IsolatedStoreElimination(
    ...
    : TR::Optimization(manager), _mustUseUseDefInfo(false),
      ...
      _usedSymbols(NULL), _temp(NULL), _temp1(NULL), _temp2(NULL),
      _storeNodes(NULL), _defParentOfUse(NULL), _defStatus(NULL),
      _groupsOfStoreNodes(NULL), _trivialDefs(NULL) {}
...
int32_t TR_IsolatedStoreElimination::perform() {
  ...
  // The non-useDefInfo path examines direct loads/stores to autos/parms to
  // determine what
  // stores are removable.  Autos can have their address taken in WCode and this
  // aliasing
  // is only required to be communicated through PALIs (pointer aliasing) on
  // consuming calls.
  // Therefore aliasing (through useDefInfo) must be consulted to safely remove
  // stores.
  // Checking address taken is not sufficient as a frontend does not have to set
  // address taken
  // if the aliasing is correctly communicated through PALIs (address taken
  // pessimizes optimizing
  ...
  if (useDefInfo) {
    ...
    if (trace())
      traceMsg(comp(), "Not performing Global Store Elimination because there "
                       "is no use/def info\n");
  ...
  if (useDefInfo) {
    ...
    for (uint32_t groupIndex = 0; groupIndex < _groupsOfStoreNodes->size();
      ...
      if (groupOfStores &&
        ...
        while (bviUses.hasMoreElements()) {
          ...
          _trivialDefs->reset(index); // It's not a trivial def if we're
                                      // handling it as a group.  If we try to
                                      // do both, BOOM!
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (node) {
        ...
        useDefInfo->getUseDef(defsOfRhs,
...
bool TR_IsolatedStoreElimination::canRemoveStoreNode(TR::Node *node) {
  ...
  if (!comp()->cg()->allowDSEOfVolatiles() &&
...
int32_t TR_IsolatedStoreElimination::performWithUseDefInfo() {
  ...
  // for ("ToBeRemoved" and "ShouldNotBeRemoved") should be set.
  // The 4th bitvector is inTransit vector.
  // So "NotToBeExamine should set the bits in "Visited" and also on
  // "ShouldNotBeRemoved".
  // After finding the group set the right bits in "ToBeRemoved" or
...
bool TR_IsolatedStoreElimination::groupIsolatedStores(
  ...
  if (tt) {
    ...
    if (node->getValueChild()->getOpCode().isBCDLoadVar() &&
      ...
      for (TR::TreeTop *ttTemp = tt->getNextTreeTop();
        ...
        if (defNodeHasSymRef &&
            defNode->getSymbolReference()->canKill(
                valueChild->getSymbolReference())) {
          if (trace())
            traceMsg(comp(), "%s groupIsolated - DEF %d cannot be removed due "
                             "to InMemoryLoadStoreMarking \n",
  ...
  // defParent array).
  // If even one of the uses is not under store node, then it imposible to
  // remove this def, and thus return.
...
void TR_IsolatedStoreElimination::performDeadStructureRemoval(
  ...
  TR_BitVector *nodesInStructure = new (trStackMemory())
      TR_BitVector(info->getTotalNodes(), trMemory(), stackAlloc, growable);
  _temp = new (trStackMemory())
      TR_BitVector(info->getNumDefNodes(), trMemory(), stackAlloc, growable);

  _temp2 = new (trStackMemory())
      TR_BitVector(info->getTotalNodes(), trMemory(), stackAlloc, growable);

  findStructuresAndNodesUsedIn(info, rootStructure, visitCount,
                               nodesInStructure, &propagateRemovalToParent);
}
...
bool TR_IsolatedStoreElimination::findStructuresAndNodesUsedIn(
    ...
    TR_BitVector *nodesInStructure, bool *propagateRemovalToParent) {
  ...
  if (structure->asRegion()) {
    ...
    TR_BitVector *nodesInSubStructure = new (trStackMemory())
        TR_BitVector(info->getTotalNodes(), trMemory(), stackAlloc, growable);
  ...
  if (canRemoveStructure) {
    ...
    TR_BitVector *defNodesInStructure = new (trStackMemory())
        TR_BitVector(info->getNumDefNodes(), trMemory(), stackAlloc, growable);
    int32_t i;
    for (i = info->getNumDefNodes() - 1; i >= 0; --i) {
      TR::Node *node = info->getNode(i + info->getFirstDefIndex());
      if (node && nodesInStructure->get(i + info->getFirstDefIndex()))
    ...
    *_temp2 -= *nodesInStructure;
    ...
    for (i = info->getNumUseNodes() - 1; i >= 0; --i) {
      ...
      if (node && !nodesInStructure->get(node->getGlobalIndex()))
#endif
        ...
        if (!defs.IsZero()) {
          ...
          *_temp &= defs;
          if (!_temp->isEmpty()) {
            if (trace()) {
              if (structure->asRegion())
                traceMsg(comp(), "Use Node %d invalidates region structure %d "
                                 "- will propagate removal to parent\n",
  ...
  if (canRemoveStructure) {
    ...
    if (regionStructure && regionStructure->isNaturalLoop() &&
        (regionStructure->getParent() /* &&
           regionStructure->getParent()->asRegion()->isCanonicalizedLoop() */)) {
                ...
                if (loopIncrement && (loopStride == 1) &&
                  ...
                  if ((lastTreeInLoopHeader->getOpCodeValue() ==
                    ...
                    numIterations = TR::Node::create(
                        TR::isub, 2, TR::Node::createWithSymRef(
                                         secondChildOfLastTree, TR::iload, 0,
                                         inductionSymRef),
