...
int32_t TR_ShrinkWrap::perform() {
  ...
  if (!comp()->cg()->getSupportsShrinkWrapping()) {
    if (_traceSW)
      traceMsg(comp(),
               "Platform does not support shrinkWrapping of registers\n");
    return 0;
  }
  ...
  if (_preservedRegs.isEmpty()) {
    traceMsg(
        comp(),
        "No preserved registers used in this method, no shrink wrapping\n");
  ...
  //
  TR_RegisterAnticipatability registerAnticipatability(
      comp(), optimizer(), _cfg->getStructure(), _registerUsageInfo, trace());

  // second data-flow pass to
  // compute register availability
  //
  TR_RegisterAvailability registerAvailability(
      comp(), optimizer(), _cfg->getStructure(), _registerUsageInfo, trace());
...
void TR_ShrinkWrap::prePerformOnBlocks() {
  ...
  _registerUsageInfo = (TR_BitVector **)trMemory()->allocateStackMemory(
      _numberOfNodes * sizeof(TR_BitVector *));
  _swBlockInfo = new (trStackMemory()) SWBlockInfo[_numberOfNodes];
  _cfgBlocks = _cfg->createArrayOfBlocks();

  // init the save/restore sets
  //
  _saveInfo = (TR_BitVector **)trMemory()->allocateStackMemory(
      _numberOfNodes * sizeof(TR_BitVector *));
  _restoreInfo = (TR_BitVector **)trMemory()->allocateStackMemory(
      _numberOfNodes * sizeof(TR_BitVector *));

  int32_t numRegs = TR::RealRegister::NumRegisters;
  for (int32_t i = 0; i < _numberOfNodes; i++) {
    _registerUsageInfo[i] =
        new (trStackMemory()) TR_BitVector(numRegs, trMemory(), stackAlloc);
    _saveInfo[i] =
        new (trStackMemory()) TR_BitVector(numRegs, trMemory(), stackAlloc);
    _restoreInfo[i] =
        new (trStackMemory()) TR_BitVector(numRegs, trMemory(), stackAlloc);
  ...
  //
  _preservedRegsInMethod =
      new (trStackMemory()) TR_BitVector(numRegs, trMemory(), stackAlloc);
  _preservedRegsInMethod->empty();
  _preservedRegsInLinkage =
      new (trStackMemory()) TR_BitVector(numRegs, trMemory(), stackAlloc);
  ...
  TR_BitVector *p =
      new (trHeapMemory()) TR_BitVector(numRegs, trMemory(), heapAlloc);
  p->empty();
  ...
  //
  _mapRegsToStack =
      (int32_t *)trMemory()->allocateStackMemory(numRegs * sizeof(int32_t));
  memset(_mapRegsToStack, -1, (numRegs * sizeof(int32_t)));
  _linkageUsesPushes =
      comp()->cg()->getLinkage()->mapPreservedRegistersToStackOffsets(
          _mapRegsToStack, _numPreservedRegs, _preservedRegsInLinkage);

  if (_traceSW)
    traceMsg(comp(),
             "Mapping between preserved registers and stack offsets: \n");
  for (int32_t i = 0; i < numRegs; i++) {
    if (_mapRegsToStack[i] != -1) {
      ...
      p->_index = i;
      p->_saveBlocks = new (trStackMemory())
          TR_BitVector(_numberOfNodes, trMemory(), stackAlloc);
      p->_restoreBlocks = new (trStackMemory())
          TR_BitVector(_numberOfNodes, trMemory(), stackAlloc);
  ...
  if (_traceSW) {
    ...
    for (PreservedRegisterInfo *cur = _preservedRegs.getFirst(); cur;
         cur = cur->getNext())
      traceMsg(comp(), "%s ",
               comp()->getDebug()->getRealRegisterName(cur->_index - 1));
    traceMsg(comp(), "}\n");
...
TR::Instruction *TR_ShrinkWrap::findJumpInstructionsInBlock(
    int32_t blockNum, TR::list<TR::Instruction *> *jmpInstrs) {
  // Start at the end of the block, and find the last jump instruction that is
  // not to an out-of-line label
  // As we do not handle multi-target jumps in Shrink Wrapping, this is a jump
  // to label marking the successor of this block
  TR::Instruction *current = _swBlockInfo[blockNum]._endInstr;

  while (current && current != _swBlockInfo[blockNum]._startInstr &&
         (!comp()->cg()->isBranchInstruction(current) ||
          current->getLabelSymbol()->isStartOfColdInstructionStream()))
    current = current->getPrev();

  TR_ASSERT(current != _swBlockInfo[blockNum]._startInstr,
            "Current should not have been able to make it all the way back to "
            "the start instruction without encountering a jump\n");

  CS2::HashTable<TR::Instruction *, bool> setOfJumps;
  findJumpInstructionsInCodeRegion(_swBlockInfo[blockNum]._startInstr,
                                   _swBlockInfo[blockNum]._endInstr,
                                   setOfJumps);
...
void TR_ShrinkWrap::findJumpInstructionsInCodeRegion(
    TR::Instruction *firstInstr, TR::Instruction *lastInstr,
    CS2::HashTable<TR::Instruction *, bool> &setOfJumps) {
  TR::Instruction *loc = lastInstr;
  while (loc != firstInstr) {
    if (comp()->cg()->isBranchInstruction(loc) && setOfJumps.Add(loc, true)) {
      // In general, a snippet or out-of-line code segment could also have a
      // jump that might need patching.
      // The recursive call below identifies these cases, and adds any jumps in
      // snippets to the list.
      TR::Instruction *start, *end;
      if (comp()->cg()->isTargetSnippetOrOutOfLine(loc, &start, &end)) {
        if (_traceSW)
          traceMsg(comp(), "Recursing over snippet jumped to by instruction %p",
                   loc);
...
void TR_ShrinkWrap::analyzeInstructions() {
  ...
  int32_t blockNum =
      comp()->getStartTree()->getEnclosingBlock()->getNumber(); // entry block
  if (_traceSW)
  ...
  for (int32_t blockNum = 0; blockNum < _numberOfNodes; blockNum++) {
    ...
    while (cur != end) {
      ...
      bool success = comp()->cg()->processInstruction(
          cur, _registerUsageInfo, blockNum, isFence, _traceSW);
  ...
  for (int32_t i = 0; i < _numberOfNodes; i++) {
    ...
    for (; outerMost && outerMost->asRegion()->isNaturalLoop();
...
// FIXME: enable this code below
  ...
  for (TR::CFGNode *node = _cfg->getFirstNode(); node; node = node->getNext()) {
    ...
    if (!block->getExceptionPredecessors().empty()) {
      for (auto e = block->getExceptionPredecessors().begin();
           e != block->getExceptionPredecessors().end(); ++e) {
...
void TR_ShrinkWrap::processInstructionsInSnippet(TR::Instruction *instr,
                                                 int32_t blockNum) {
...
void TR_ShrinkWrap::computeSaveRestoreSets(
    TR_RegisterAnticipatability &registerAnticipatability,
    TR_RegisterAvailability &registerAvailability) {
  // For each register r, the save equation at basic block i would be: (REGS is
  // set to 1s)
  //
  // SAV(i)          = [ RANTin(i) - RAVLin(i) ] & intersection j E pred(i) [
  // REGS - RANTin(j) ]
  // RESTORE(i)      = [ RAVLout(i) - RANTout(i) ] & intersection j E succ(i) [
  // REGS - RAVLout(j) ]
  //
  int32_t numRegs = TR::RealRegister::NumRegisters;
  TR_BitVector *regs =
  ...
  for (TR::CFGNode *node = _cfg->getFirstNode(); node; node = node->getNext())
     ...
     if (loop &&
        ...
        TR_BitVector *entryAnticipatability =
  registerAnticipatability._blockAnalysisInfo[block->getNumber()];
        ...
        for (TR::CFGEdge *e = edgeIt.getFirst(); e; e = edgeIt.getNext())
           ...
           if (!loop->contains(predBlock->getStructureOf(), loop->getParent()))
              ...
              //
              *(registerAnticipatability._blockAnalysisInfo[predBlock->getNumber()])
  |= *entryAnticipatability;
           }
        }
     }
  */
  ...
  for (TR::CFGNode *node = _cfg->getFirstNode(); node; node = node->getNext()) {
    ...
    */
    ...
    for (TR::CFGEdge *e = edgeIt.getFirst(); e; e = edgeIt.getNext()) {
      ...
      ///*scratch -=
      ///*(registerAnticipatability._blockAnalysisInfo[from->getNumber()]);
      *scratch &=
          *(registerAnticipatability._blockAnalysisInfo[from->getNumber()]);
      ///*scratch |=
      ///*(registerAnticipatability._blockAnalysisInfo[from->getNumber()]);
    ...
    */
    ...
    for (TR::CFGEdge *e = edgeIt.getFirst(); e; e = edgeIt.getNext()) {
      ...
      *scratch &= *(registerAvailability._blockAnalysisInfo[to->getNumber()]);
      ///*scratch |=
      ///*(registerAvailability._blockAnalysisInfo[to->getNumber()]);
      hasSuccs = true;
    }
    ...
    for (int32_t i = 0; i < _numberOfNodes; i++) {
      ...
      while (bvIt.hasMoreElements()) {
        ...
        if (comp()->cg()->isPreservedRegister(index) != -1)
          traceMsg(comp(), "%s ",
                   comp()->getDebug()->getRealRegisterName(index - 1));
      }
      traceMsg(comp(), " }\n");
    }
...
static bool isPredecessor(TR::Block *pred, TR::Block *cur) {
  for (auto e = cur->getPredecessors().begin();
       e != cur->getPredecessors().end(); ++e) {
...
void TR_ShrinkWrap::doPlacement(
    TR_RegisterAnticipatability &registerAnticipatability,
    TR_RegisterAvailability &registerAvailability) {
  // iterate over each basic block
  // look at its SAVE/RESTORE set and save/restore each callee-saved register as
  // needed
  //   - if block is inside loop, walk up the structure hierarchy finding the
  //   first parent
  ...
  int32_t firstBlockNum =
      comp()->getStartTree()->getEnclosingBlock()->getNumber();

  TR_BitVector *visited = new (trStackMemory())
      TR_BitVector(_numberOfNodes, trMemory(), stackAlloc);
  TR_BitVector *blocksVisited = new (trStackMemory())
      TR_BitVector(_numberOfNodes, trMemory(), stackAlloc);
  TR_BitVector *splitBlocks = new (trStackMemory())
      TR_BitVector(_numberOfNodes, trMemory(), stackAlloc);
  TR_BitVector *blocksSplitSoFar = new (trStackMemory())
      TR_BitVector(_numberOfNodes, trMemory(), stackAlloc);
  ...
  for (int32_t blockNum = 0; blockNum < _numberOfNodes; blockNum++) {
    ...
    TR_YesNoMaybe isReturnBlock =
        blockEndsInReturn(blockNum, hasExceptionSuccs);
    ...
    // nothing to do
    //
    if (_saveInfo[blockNum]->isEmpty() && _restoreInfo[blockNum]->isEmpty())
      continue;
    ...
    //
    if ((blockNum == firstBlockNum) || (blockNum == cfgStartNum)) {
    ...
    // that eventually throw an exception)
    //
    if (!_restoreInfo[blockNum]->isEmpty() && (isReturnBlock == TR_no)) {
      if (_traceSW)
        traceMsg(comp(), "not restoring registers in block [%d] that does not "
                         "contain return\n",
                 blockNum);
      addInfo = false;
    }
  ...
  // is going to affect the registers saved in the prologue. if this is the
  // case,
  ...
  if (1) {
    ...
    *temp |= *_saveInfo[firstBlockNum];
    ...
    for (PreservedRegisterInfo *p = _preservedRegs.getFirst(); p;
         p = p->getNext()) {
      ...
      if (p->_saveBlocks->isEmpty()) {
        traceMsg(comp(), "         O^O Preserved reg %d has no save blocks -- "
                         "saved in prologue\n",
                 regIndex);
      ...
      // is actually the block to which a(n inner) loop exits to and then
      // restore the
      // register along that edge. this would mean either tweaking the
      // regAvailability
      // sets and then doing the right checks in the restore part of STEP3 or
      // just
      ...
      if (1) {
        ...
        while (resIt.hasMoreElements()) {
          ...
          if (curBlock->getStructureOf()->getContainingLoop()) {
            traceMsg(comp(), "         O^O Register %d is going to be restored "
                             "in a block_%d inside a loop, no shrink wrap\n",
          ...
          //
            ...
            for (auto e = curBlock->getExceptionSuccessors().begin();
                 e != curBlock->getExceptionSuccessors().end(); ++e) {
              ...
              bool catchBlockHasSinglePred =
                  (catchBlock->getExceptionPredecessors().size() == 1);
              if (r &&
                  !registerAvailability._inSetInfo[catchBlockNum]->get(
                      regIndex) &&
                  !registerAnticipatability._blockAnalysisInfo[catchBlockNum]
                       ->get(regIndex)) {
                ...
                {
                  traceMsg(comp(), "         O^O Register %d is going to be "
                                   "restored on a catch block_%d that contains "
                                   "a return and needs to be split\n",
                           regIndex, catchBlockNum);
                  found = true;
                  break;
                }
              } else if (!catchBlockHasSinglePred) {
                // FIXME: this is the case when a restore block has exception
                // succs
                // and the catch block has in turn multiple predecessors. in
                // this case
                // we cannot split this edge, so we currently have no way of
                // restoring this
                // register correctly (the register may be restored at the end
                // of the block but
                // that is not good enough if the exception happens to be taken
                // from the middle of
                // the block). conservatively mark this register as an
                // untouchable
                //
                traceMsg(comp(), "         O^O Register %d is going to be "
                                 "restored in a block_%d that has exception "
                                 "edges\n",
                         regIndex, blockNum);
                found = true;
                break;
            ...
            if (!found) {
              ...
              if ((isReturnBlock == TR_maybe) && hasExceptionSuccs) {
                traceMsg(comp(), "         O^O Register %d is going to be "
                                 "restored in a block_%d that contains a throw "
                                 "and has exception successors\n",
                         regIndex, blockNum);
                found = true;
                break;
              }
        ...
        while (savIt.hasMoreElements()) {
          ...
          if (!curBlock->getExceptionPredecessors().empty()) {
            ...
            for (auto e = curBlock->getExceptionPredecessors().begin();
                 e != curBlock->getExceptionPredecessors().end(); ++e) {
              TR::Block *predBlock = toBlock((*e)->getFrom());
              int32_t predBlockNum = predBlock->getNumber();
              if (registerAnticipatability._outSetInfo[predBlockNum]->get(
                      regIndex) ||
                  registerAvailability._blockAnalysisInfo[predBlockNum]->get(
                      regIndex)) {
                anyPredsNeedSplit = true;
                if (_traceSW)
                  traceMsg(comp(), "         O^O Edge between predecessor "
                                   "block_%d of catch block_%d needs to be "
                                   "split\n",
            ...
            if (anyPredsNeedSplit) {
              traceMsg(comp(), "         O^O Register %d is going to be saved "
                               "in a catch block_%d that needs to be split\n",
                       regIndex, blockNum);
              found = true;
              break;
            }
      ...
      bool doSave = performTransformation(
          comp(), "%s register %s [%d] at stack offset [%d]\n", OPT_DETAILS,
          comp()->getDebug()->getRealRegisterName(regIndex - 1), regIndex,
          _mapRegsToStack[regIndex]);
      if (!doSave) {
        comp()->cg()->getPreservedRegsInPrologue()->set(regIndex);
        debugMode = true;
      } else
  ...
  //
  if (shrinkWrapWillbeDone && comp()->cg()->getUsesLoadStoreMultiple()) {
    int32_t regsUsedInMethod = _preservedRegsInMethod->elementCount();
    int32_t regsNotShrinkWrapped =
        comp()->cg()->getPreservedRegsInPrologue()->elementCount();
    if ((regsUsedInMethod - regsNotShrinkWrapped) <=
        2) // no benefit to breaking the store multiple in prologue
    {
      shrinkWrapWillbeDone = false;
      traceMsg(comp(), "No benefit to shrinkwrapping %d regs out of %d "
                       "preserved regs used in method\n",
  ...
  if (!shrinkWrapWillbeDone) {
    ...
    if (_traceSW) {
      traceMsg(comp(),
               "These regs are saved (restored) in prologue (epilogue): ");
  ...
  //
  TR_BitVector *registersShrinkWrapped =
      new (trStackMemory()) TR_BitVector(numRegs, trMemory(), stackAlloc);
  ...
  for (int32_t i = 0; i < _cfg->getForwardTraversalLength(); i++) {
    ...
    //
    TR_BitVector *livePreservedRegs =
        curBlock->getRegisterSaveDescriptionBits();
    if (!livePreservedRegs)
      livePreservedRegs =
          new (trHeapMemory()) TR_BitVector(*_saveInfo[blockNum]);
    else
    ...
    for (TR::CFGEdge *e = predIt.getFirst(); e; e = predIt.getNext()) {
      ...
      if (pred->getForwardTraversalIndex() >=
          curBlock->getForwardTraversalIndex()) {
        isBackEdge = true;
        backEdges.add(e);
        if (_traceSW)
      ...
      if (!isBackEdge) {
        TR_ASSERT(visited->get(pred->getNumber()),
                  "computing register save description for node [%d] whose "
                  "pred is not processed [%d]\n",
                  blockNum, pred->getNumber());
        ...
        while (resIt.hasMoreElements()) {
          ...
          if (!registerAvailability._inSetInfo[blockNum]->get(regIndex) &&
              !registerAnticipatability._blockAnalysisInfo[blockNum]->get(
                  regIndex))
    ...
    // finally stack slots corresp. to regs that will be restored in this block
    // are
    // still live until the end of the block along with the ones saved in the
    // prologue
    //
    *livePreservedRegs |= *_restoreInfo[blockNum];
    *livePreservedRegs |= *comp()->cg()->getPreservedRegsInPrologue();
    // make sure this vector only contains bits corresponding to regs used in
    // this
    ...
    *livePreservedRegs &= *_preservedRegsInMethod;
    if (_traceSW) {
      traceMsg(comp(), "livePreservedRegs for block[%d] : ",
               curBlock->getNumber());
    ...
    if (_traceSW) {
      traceMsg(comp(), "registerDesriptionBits for block [%d] : ",
               curBlock->getNumber());
    ...
    for (TR::CFGEdge *e = succIt.getFirst(); e; e = succIt.getNext()) {
      ...
      if (backEdges.find(e)) {
        if (_traceSW)
          traceMsg(comp(),
                   "not propagating info along backedge from [%d] to [%d]\n",
                   curBlock->getNumber(), succ->getNumber());
        continue;
      }
      TR_BitVector *scratch =
          new (trHeapMemory()) TR_BitVector(*livePreservedRegs);
  ...
  if (_traceSW) {
    for (PreservedRegisterInfo *p = _preservedRegs.getFirst(); p;
      ...
      traceMsg(comp(), "For preserved register: %d [%s]\n", p->_index,
               comp()->getDebug()->getRealRegisterName(p->_index - 1));
  ...
  for (PreservedRegisterInfo *p = _preservedRegs.getFirst(); p;
    ...
    if (_traceSW)
      traceMsg(comp(),
               "      Pre-Processing save/restore for register: %d [%s]\n",
    ...
    while (savIt.hasMoreElements()) {
      ...
      TR::Block *curBlock = _swBlockInfo[blockNum]._block;
      bool isInLoop =
          curBlock->getStructureOf()->getContainingLoop() ? true : false;
      ...
      //
      if (isInLoop && !blocksVisited->get(blockNum)) {
        ...
        if (1) ///!(curBlock->getPredecessors().size() == 1))
        {
          ...
          traceMsg(comp(),
                   "         O^O Loop entry block_%d split at newentry %p\n",
                   blockNum, newEntry);

          for (auto e = curBlock->getPredecessors().begin();
               e != curBlock->getPredecessors().end(); ++e) {
            // ignore backedges
            //
            if (backEdges.find(*e))
              continue;
  ...
  for (PreservedRegisterInfo *p = _preservedRegs.getFirst(); p;
    ...
    if (p->_saveBlocks->isEmpty()) {
      ...
      if (_traceSW)
        traceMsg(comp(), "      Nothing to do for register %d [%s]\n", regIndex,
                 comp()->getDebug()->getRealRegisterName(regIndex - 1));
      continue;
    }
    ...
    if (comp()->cg()->getPreservedRegsInPrologue()->get(regIndex)) {
      if (_traceSW)
        traceMsg(comp(), "      Not shrink wrapping register %d [%s] as it is "
                         "saved in prologue\n",
                 regIndex,
                 comp()->getDebug()->getRealRegisterName(regIndex - 1));
      continue;
    ...
    /*
    bool doSave = performTransformation(comp(), "%sSaving register %s [%d] at
    stack offset [%d]\n", OPT_DETAILS,
    comp()->getDebug()->getRealRegisterName(regIndex-1), regIndex,
    _mapRegsToStack[regIndex]);
...
//
      ...
      if (!curBlock->getExceptionPredecessors().empty()) {
        ...
        for (auto e = curBlock->getExceptionPredecessors().begin();
             e != curBlock->getExceptionPredecessors().end(); ++e) {
          TR::Block *predBlock = toBlock((*e)->getFrom());
          if (!registerAnticipatability._outSetInfo[predBlock->getNumber()]
                   ->get(regIndex) &&
              !registerAvailability._blockAnalysisInfo[predBlock->getNumber()]
                   ->get(regIndex) &&
              !doneSave) {
            traceMsg(comp(), "         O^O Saving register %s [%d] at instr "
                             "[%p] in catch block [%d] with stack offset "
                             "[%d]\n",
                     comp()->getDebug()->getRealRegisterName(regIndex - 1),
                     regIndex, cursor, blockNum, _mapRegsToStack[regIndex]);
            int32_t offset =
                _linkageUsesPushes ? -1 : _mapRegsToStack[regIndex];
            comp()->cg()->getLinkage()->savePreservedRegister(cursor, regIndex,
                                                              offset);
            doneSave = true;
          }
          ...
          if (!doneSave) {
            if (_traceSW)
              traceMsg(comp(), "      register %s [%d] is used in some "
                               "exception predecessor of catch block [%d], not "
                               "saved\n",
                       comp()->getDebug()->getRealRegisterName(regIndex - 1),
                       regIndex, blockNum);
          }
        ...
        TR::Block *predBlock = toBlock(e->getFrom());
        if (!registerAnticipatability._outSetInfo[predBlock->getNumber()]->get(
                regIndex)) {
          traceMsg(comp(), "         O^O Saving register %s [%d] at instr [%p] "
                           "block [%d] with stack offset [%d]\n",
                   comp()->getDebug()->getRealRegisterName(regIndex - 1),
                   regIndex, cursor, blockNum, _mapRegsToStack[regIndex]);
          ...
          if (!_swBlockInfo[blockNum]._savedRegs) {
            _swBlockInfo[blockNum]._savedRegs = new (trStackMemory())
                TR_BitVector(numRegs, trMemory(), stackAlloc);
        ...
        if (1) {
          for (auto e = curBlock->getPredecessors().begin();
            ...
            if (isFallThrough) {
              ...
              traceMsg(comp(), "         O^O Found fall-through block_%d "
                               "needsJump %d anyPredsNeedSplit %d\n",
            ...
            traceMsg(comp(),
                     "         O^O Looked at pred %d anyPredsNeedSplit %d\n",
        ...
        for (auto e = curBlock->getPredecessors().begin();
          ...
          if (!registerAnticipatability._outSetInfo[predBlockNum]->get(
            ...
            //
            if (!registerAvailability._blockAnalysisInfo[predBlockNum]->get(
                    regIndex)) {
              ...
              if (0 && !ei && splitBlocks->get(blockNum)) {
                ...
                //
                for (auto pred = curBlock->getPredecessors().begin();
                     pred != curBlock->getPredecessors().end(); ++pred) {
                  ...
                  if (pei) {
                    traceMsg(comp(),
                             "         O^O Found pei for save predblock_%d\n",
                             pNum);
                    ...
                    *temp2 &= *temp1;
                    TR_BitVector *temp3 =
                        registerAvailability._blockAnalysisInfo[predBlockNum];
                    *temp3 &= *_preservedRegsInLinkage;
                    TR_BitVector *temp4 =
                        registerAvailability._blockAnalysisInfo[pNum];
                    *temp4 &= *_preservedRegsInLinkage;
                    traceMsg(comp(), "         O^O temp2 %d "
                                     "_restoreInfo[predBlockNum] %d "
                                     "_restoreInfo[pNum] %d isFallThrough %d\n",
                             temp2->isEmpty(),
                             _restoreInfo[predBlockNum]->isEmpty(),
                             _restoreInfo[pNum]->isEmpty(), isFallThrough);
                    // make sure
                    // a) this register is not saved already in predBlock (temp2
                    // being empty)
                    // b) this register is not being restored along edge from
                    // predBlockNum to block
                    // c) this register is not being restored along edge from
                    // pNum to block
                    //
                    if (temp2->isEmpty() && (*temp3 == *temp4) &&
                        _restoreInfo[predBlockNum]->isEmpty() &&
                        _restoreInfo[pNum]->isEmpty() && !isFallThrough) {
                      traceMsg(comp(), "         O^O Can share split for saves "
                                       "between %d and %d\n",
                               predBlockNum, pNum);
              ...
              if (!ei) {
                ...
                ei->_savedRegs = new (trStackMemory())
                    TR_BitVector(numRegs, trMemory(), stackAlloc);
                ei->_restoredRegs = new (trStackMemory())
                    TR_BitVector(numRegs, trMemory(), stackAlloc);
                ei->_savedRegs->empty();
              ...
              if (!ei->_restoreSplitLocation) //! ei->_saveSplitLocation)
              {
                ...
                if (!isPredecessor(prevBlock, curBlock) &&
                    !blocksSplitSoFar->get(curBlock->getNumber())) {
                  /// printf("found save suboptimality in %s\n",
                  /// comp()->signature());fflush(stdout);
                  traceMsg(
                      comp(),
                      "         O^O No save jump needed between %d and %d\n",
                      prevBlock->getNumber(), curBlock->getNumber());
                  needsJump = false;
                }
                cursor = _swBlockInfo[predBlockNum]._endInstr;
                TR::list<TR::Instruction *> jmpInstrs(
                    getTypedAllocator<TR::Instruction *>(
                        TR::comp()->allocator()));
                if (!isFallThrough) {
                  cursor =
                      findJumpInstructionsInBlock(predBlockNum, &jmpInstrs);
                }
                traceMsg(comp(),
                         "         O^O Split cursor %p with newSplitLoc %p\n",
                         cursor, newSplitLocation);
                cursor =
                    comp()->cg()->splitEdge(cursor, isFallThrough, needsJump,
                                            newSplitLocation, &jmpInstrs, true);
              ...
              //
              if (doSaves) {
                traceMsg(comp(), "         O^O Saving register %s [%d] for "
                                 "block [%d] with instr [%p] at predecessor "
                                 "block (split) [%d] with stack offset [%d]\n",
                         comp()->getDebug()->getRealRegisterName(regIndex - 1),
                         regIndex, blockNum, cursor, predBlockNum,
                         _mapRegsToStack[regIndex]);
                int32_t offset =
                    _linkageUsesPushes ? -1 : _mapRegsToStack[regIndex];
                comp()->cg()->getLinkage()->savePreservedRegister(
                    cursor, regIndex, offset);
                ////doneSaves = true;
                ei->_savedRegs->set(regIndex);
              }

              // saves were shared between preds, so fixup the branches if any
              // to point
              // to the new location
              //
              if (saveLocation) {
                ...
                cursor = comp()->cg()->splitEdge(cursor, isFallThrough, false,
                                                 newSaveLocation, &jmpInstrs);
              }
            } else
              traceMsg(comp(), "         O^O Not saving register %s [%d] for "
                               "block [%d] since its available in pred [%d]\n",
                       comp()->getDebug()->getRealRegisterName(regIndex - 1),
                       regIndex, blockNum, predBlockNum);
          }
        }
      }
    ...
    while (resIt.hasMoreElements()) {
      ...
      if (curBlock->getSuccessors().size() == 1) {
        ...
        if (backEdges.find(e)) {
          if (_traceSW)
            traceMsg(comp(),
                     "      not restoring along backedge [%d] -> [%d]\n",
        ...
        //
        TR::Block *succBlock = toBlock(e->getTo());
        if (registerAvailability._inSetInfo[succBlock->getNumber()]->get(
                regIndex) &&
            (succBlock != _cfg->getEnd())) {
          if (_traceSW)
            traceMsg(comp(), "      not restoring along edge [%d] -> [%d] "
                             "since reg is available\n",
                     e->getFrom()->getNumber(), e->getTo()->getNumber());
          doRestore = false;
        }
        ...
        if (doRestore) {
          bool isFallThrough = true;
          TR::Node *branchNode =
              curBlock->getExit()->getPrevRealTreeTop()->getNode();
          ...
          traceMsg(comp(), "         O^O Restoring register %s [%d] at instr "
                           "[%p] at end of block [%d] with stack offset [%d]\n",
                   comp()->getDebug()->getRealRegisterName(regIndex - 1),
                   regIndex, cursor, blockNum, _mapRegsToStack[regIndex]);
          ...
          if (r) {
            ...
            // this has a dual purpose of the instr deallocating the frame as
            // well
            // as being compact
            // FIXME: shrinkwrapping and using pushes is not compatible, so we
            // wont
            // use pushes on ia32
            //
            ...
            comp()->cg()->getLinkage()->restorePreservedRegister(
                cursor, regIndex, _mapRegsToStack[regIndex]);

          if (!_swBlockInfo[blockNum]._restoredRegs) {
            _swBlockInfo[blockNum]._restoredRegs = new (trStackMemory())
                TR_BitVector(numRegs, trMemory(), stackAlloc);
        ...
        // and then restored.
        //
        for (auto e = curBlock->getSuccessors().begin();
             e != curBlock->getSuccessors().end(); ++e) {
          ...
          if (backEdges.find(*e)) {
            if (_traceSW)
              traceMsg(
                  comp(),
                  "         O^O Not restoring along backedge [%d] -> [%d]\n",
                  (*e)->getFrom()->getNumber(), (*e)->getTo()->getNumber());
            continue;
          }
          ...
          if (!registerAvailability._inSetInfo[succBlockNum]->get(regIndex) &&
              !registerAnticipatability._blockAnalysisInfo[succBlockNum]->get(
                  regIndex)) {
            if (!(succBlock->getPredecessors().size() == 1)) {
              ...
              if (!ei && splitBlocks->get(succBlockNum)) {
                ...
                // restore sequence
                //
                for (auto pred = succBlock->getPredecessors().begin();
                     pred != succBlock->getPredecessors().end(); ++pred) {
                  ...
                  if (pei) {
                    ...
                    *temp2 &= *_preservedRegsInLinkage;
                    ...
                    // look at java/lang/ThreadGroup.remove(Ljava/lang/Thread;)V
                    // in jitt on x86-32
                    //
                    // HACKIT
                    if (_traceSW)
                    ...
                    if ((*temp1 == *temp2) && pei->_savedRegs->isEmpty() &&
                        !isFallThrough) {
                      if (_traceSW)
                        traceMsg(comp(),
                                 "no need to split between block_%d and %d\n",
                                 predBlockNum, blockNum);
                      ...
                      if (0 && isFallThrough) {
                        ...
                        fflush(stdout);
                        // if a fall-through is going to share, get rid of the
                        // preceeding jmp
                        TR::Instruction *jmpInstr =
                            pei->_restoreSplitLocation->getPrev();
              ...
              if (!ei->_restoreSplitLocation) {
                ...
                if (!isPredecessor(prevBlock, succBlock) &&
                    !blocksSplitSoFar->get(succBlockNum)) {
                  /// printf("found restore suboptimality in %s\n",
                  /// comp()->signature());fflush(stdout);
                  if (_traceSW)
                    traceMsg(comp(),
                             "no restore jump needed between %d and %d\n",
                ...
                TR_ASSERT(
                    isFallThrough || comp()->cg()->isBranchInstruction(cursor),
                    "cursor [%p] is not branch before split edge\n", cursor);
                cursor = comp()->cg()->splitEdge(
                    cursor, isFallThrough, needsJump, NULL, &jmpInstrs, true);
                ei->_restoreSplitLocation = cursor;
              ...
              if (doRestore) {
                traceMsg(comp(), "         O^O Restoring register %s [%d] for "
                                 "block [%d] with instr [%p] at successor "
                                 "block (split) [%d] with stack offset [%d]\n",
                         comp()->getDebug()->getRealRegisterName(regIndex - 1),
                         regIndex, blockNum, cursor, succBlockNum,
                         _mapRegsToStack[regIndex]);
                comp()->cg()->getLinkage()->restorePreservedRegister(
                    cursor, regIndex, _mapRegsToStack[regIndex]);
              ...
              /// else
              {
                traceMsg(comp(), "         O^O Not restoring register again %s "
                                 "[%d] for block [%d] at successor block "
                                 "(split) [%d]\n",
                         comp()->getDebug()->getRealRegisterName(regIndex - 1),
                         regIndex, blockNum, succBlockNum);
                ...
                if (!isFallThrough) {
                  ...
                  cursor = findJumpInstructionsInBlock(blockNum, &jmpInstrs);
                  cursor =
                      comp()->cg()->splitEdge(cursor, isFallThrough, false,
                                              newSplitLocation, &jmpInstrs);
                }
              }
            } else // this is not possible according to the data flow solutions
            {
              ...
              traceMsg(comp(), "         O^O Restoring register %s [%d] for "
                               "block [%d] with instr [%p] at successor block "
                               "[%d] with stack offset [%d]\n",
                       comp()->getDebug()->getRealRegisterName(regIndex - 1),
                       regIndex, blockNum, cursor, succBlockNum,
                       _mapRegsToStack[regIndex]);
              comp()->cg()->getLinkage()->restorePreservedRegister(
            ...
            ; // do nothing as the successor will take care of restoring the
              // register
        }
      }
      isWrapped = true;
  ...
  //
  if (_traceSW && comp()->cg()->getPreservedRegsInPrologue()) {
...
void TR_ShrinkWrap::composeSavesRestores() {
  ...
  for (SWEdgeInfo *cur = _swEdgeInfo.getFirst(); cur; cur = cur->getNext()) {
    ...
    if (pEnv2 && cur->_savedRegs) {
      traceMsg(comp(), "trying to use smg for saves along edge %d %d\n",
               cur->_edge->getFrom()->getNumber(),
               cur->_edge->getTo()->getNumber());
  ...
  for (int32_t i = 0; i < _numberOfNodes; i++) {
    if (pEnv4 && _swBlockInfo[i]._savedRegs) {
      ...
      findMultiples(_swBlockInfo[i]._savedRegs, _swBlockInfo[i]._startInstr,
                    true);
    }
    if (pEnv5 && _swBlockInfo[i]._restoredRegs) {
      ...
      if (!forwardWalk)
        findMultiples(_swBlockInfo[i]._restoredRegs,
                      _swBlockInfo[i]._restoreLocation, false, forwardWalk);
    }
  }
}

bool TR_ShrinkWrap::findMultiples(TR_BitVector *regs,
                                  TR::Instruction *startLocation, bool doSaves,
                                  bool forwardWalk) {
        ...
        if (curIdx == prevIdx + 1) {
          ...
          if (i > 1) {
            ...
            traceMsg(comp(), "startLocation %p offset %d\n", startLocation,
                     _mapRegsToStack[startIdx]);
            lmgInstrs = comp()->cg()->getLinkage()->composeSavesRestores(
                lmgInstrs, startIdx, endIdx, _mapRegsToStack[startIdx], numRegs,
                doSaves);
            if (forwardWalk) {
...
PreservedRegisterInfo *
TR_ShrinkWrap::findPreservedRegisterInfo(int32_t regIndex) {
  for (PreservedRegisterInfo *cur = _preservedRegs.getFirst(); cur;
       cur = cur->getNext()) {
...
ReturnBlockInfo *TR_ShrinkWrap::findReturnBlockInfo(int32_t blockNum) {
  for (ReturnBlockInfo *cur = _returnBlocks.getFirst(); cur;
       cur = cur->getNext()) {
...
void TR_ShrinkWrap::markInstrsInBlock(int32_t blockNum) {
  ...
  int32_t rsd = comp()->cg()->computeRegisterSaveDescription(
      block->getRegisterSaveDescriptionBits());
  ...
  while (cur != end) {
    ...
    if ((mightHaveSnippet != 1) && comp()->cg()->isBranchInstruction(cur))
...
// end in return
//
TR_YesNoMaybe TR_ShrinkWrap::blockEndsInReturn(int32_t blockNum,
                                               bool &hasExceptionSuccs) {
  TR::Block *exitBlock = toBlock(_cfg->getEnd());

  bool hasExitEdge = false;
  for (auto e = exitBlock->getPredecessors().begin();
       e != exitBlock->getPredecessors().end(); ++e)
