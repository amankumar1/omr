...
void TR_RegisterAvailability::analyzeNode(TR::Node *, vcount_t,
                                          TR_BlockStructure *, TR_BitVector *) {
}
...
void TR_RegisterAvailability::initializeRegisterUsageInfo() {
  ...
  _inSetInfo = (TR_BitVector **)trMemory()->allocateStackMemory(
      _numberOfNodes * sizeof(TR_BitVector *));
  ...
  for (int32_t i = 0; i < _numberOfNodes; i++) {
    ...
    _inSetInfo[i] = new (trStackMemory())
        TR_BitVector(_numberOfBits, trMemory(), stackAlloc);
    _inSetInfo[i]->empty();
  }
}
...
void TR_RegisterAvailability::analyzeBlockZeroStructure(
    TR_BlockStructure *blockStructure) {
  // initialize the analysis info by zeroing out the initial inSet for
  // availability
...
void TR_RegisterAvailability::analyzeTreeTopsInBlockStructure(
    TR_BlockStructure *blockStructure) {
