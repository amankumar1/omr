...
TR_Dominators::TR_Dominators(TR::Compilation *c, bool post)
    : _compilation(c), _info(c->getFlowGraph()->getNextNodeNumber() + 1,
                             c->allocator(), c->allocator()),
      _dfNumbers(c->getFlowGraph()->getNextNodeNumber() + 1, c->allocator(), 0),
      _dominators(c->getFlowGraph()->getNextNodeNumber() + 1, c->allocator(),
                  NULL) {
  ...
  for (block = toBlock(cfg->getFirstNode()); block;
    ...
    TR_ASSERT(_dfNumbers.ValueAt(block->getNumber()) >= 0,
...
// The following constructor needs to be shortened by finding common code with
// the
// constructor above
TR_Dominators::TR_Dominators(TR::Compilation *c,
                             TR::ResolvedMethodSymbol *methSym, bool post)
    : _compilation(c), _cfg(methSym->getFlowGraph()),
      _info(methSym->getFlowGraph()->getNextNodeNumber() + 1, c->allocator(),
            c->allocator()),
      _dfNumbers(methSym->getFlowGraph()->getNextNodeNumber() + 1,
                 c->allocator(), 0),
      _dominators(methSym->getFlowGraph()->getNextNodeNumber() + 1,
                  c->allocator(), NULL) {
  LexicalTimer tlex("TR_Dominators::TR_Dominators", _compilation->phaseTimer());
...
int TR_Dominators::dominates(TR::Block *block, TR::Block *other) {
  ...
  if (other == block)
    return 1;
  for (TR::Block *d = other; d != NULL &&
                             _dfNumbers.ValueAt(d->getNumber()) >=
                                 _dfNumbers.ValueAt(block->getNumber());
       d = getDominator(d)) {
    if (d == block)
      return 1;
  }
...
void TR_Dominators::initialize(TR::Block *start, BBInfo *nullParent) {
  ...
  TR::list<TR::CFGEdge *> dummyList(
      getTypedAllocator<TR::CFGEdge *>(comp()->allocator()));
  dummyList.push_front(&dummyEdge);
  CS2::ArrayOf<StackInfo, TR::Allocator> stack(_numNodes / 2,
                                               comp()->allocator());
...
void TR_Dominators::BBInfo::print(TR_FrontEnd *fe, TR::FILE *pOutFile) {
  ...
  trfprintf(pOutFile, "   _parent=%d, _idom=%d, _sdno=%d, _ancestor=%d, "
                      "_label=%d, _size=%d, _child=%d\n",
