...
class TR_DataFlowAnalysis
    : public TR::Allocatable<TR_DataFlowAnalysis, TR::Allocator> {
public:
...
#include "optimizer/DataFlowAnalysis.enum"
  ...
  // *asIntersectionBitVectorAnalysis();
  // virtual TR_BackwardBitVectorAnalysis *asBackwardBitVectorAnalysis();
  // virtual TR_BackwardIntersectionBitVectorAnalysis
  // *asBackwardIntersectionBitVectorAnalysis();
  // virtual TR_BackwardUnionBitVectorAnalysis
  // *asBackwardUnionBitVectorAnalysis();
...
private:
  void initializeGenAndKillSetInfoForNode(TR::Node *node,
                                          TR_UseDefInfo::BitVector &defsKilled,
...
public:
  TR_ReachingBlocks(TR::Compilation *comp, TR::Optimizer *optimizer,
                    bool trace = false);
...
public:
  ...
  bool analyzeNodeIfSuccessorsAnalyzed(TR_RegionStructure *, TR::BitVector &,
                                       TR::BitVector &);

  virtual void initializeGenAndKillSetInfo(TR_RegionStructure *,
                                           TR::BitVector &, TR::BitVector &,
                                           bool);
  virtual void initializeGenAndKillSetInfoForRegion(TR_RegionStructure *);
  virtual void initializeGenAndKillSetInfoForBlock(TR_BlockStructure *);
  virtual bool canGenAndKillForStructure(TR_Structure *);
...
private:
  ...
  // analyses
  // once they are allocated via createGenAndKillSetCaches(), the next call to
  // initializeGenAndKillSetInfo() will store
  //   the sets into these caches and set _haveCachedGenAndKillSets.  Subsequent
  //   calls to initializeGenAndKillSetInfo()
  //   will change the arrays passed in to refer to the bit vectors in these
  //   caches.
...
class TR_RegisterAnticipatability
    : public TR_BackwardIntersectionBitVectorAnalysis {
public:
  TR_RegisterAnticipatability(TR::Compilation *comp, TR::Optimizer *optimizer,
                              TR_Structure *, TR_BitVector **,
                              bool trace = false);
...
public:
  TR_RegisterAvailability(TR::Compilation *comp, TR::Optimizer *optimizer,
                          TR_Structure *, TR_BitVector **, bool trace = false);
