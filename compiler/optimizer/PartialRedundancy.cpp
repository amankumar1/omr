...
static void correctDecimalPrecision(TR::Node *store, TR::Node *child,
  ...
  if (child->getType().isBCD() &&
    ...
    if (comp->cg()->traceBCDCodeGen())
      traceMsg(comp, "%screating %s (%p) to correctDecimalPrecision (%d->%d : "
...
int32_t TR_PartialRedundancy::perform() {
      ...
      if ((_newSymbolsMap[nextOptimalComputation] < 0) &&
        ...
        if (nextOptimalNode->getOpCode().isLoadVarDirect() &&
          ...
          if (fe()->dataTypeForLoadOrStore(nextOptimalNode->getDataType()) !=
                  nextOptimalNode->getDataType() && // only do it for Java where
                                                    // we use istore for small
                                                    // intergral types
  ...
  for (bvi3.SetToFirstOne(); bvi3.Valid(); bvi3.SetToNextOne()) {
    ...
    if (cost > benefit) {
      if (trace())
        traceMsg(comp(), "Cost-benefit analysis (cost %d > benefit %d) said "
                         "ignore expression #%d\n",
...
// node with a particular
// sign setting to be stored to a temp and then the load of this temp will lose
// this sign info and
// subsequent generated code may clobber an unsigned sign code (0xf -> 0xc for
// example)
// pdSetSignA sign=?
//    x
// ...
...
// containing some sign state info
            ...
            if (!shouldAllowLimitingOfOptimalPlacement(comp()) ||
              ...
              if (comp()->useCompressedPointers()) {
                if (convertedOptimalNode->getOpCode().isLoadIndirect() &&
                  ...
                  translateTT = TR::TreeTop::create(
                      comp(), TR::Node::createCompressedRefsAnchor(
                                  convertedOptimalNode),
                      NULL, NULL);
                }
              }
        ...
        if (isSupportedOpCode(nullCheckReferenceNode, NULL) &&
          ...
          if (((nullCheckReferenceNode->getLocalIndex() != MAX_SCOUNT) &&
            ...
            if ((_newSymbolsMap[nullCheckReferenceNode->getLocalIndex()] >
              ...
              if (trace())
                traceMsg(comp(), "Duplicate null check had its new null check "
                                 "reference %p replaced by %p with symRef "
                                 "#%d\n",
...
void TR_PartialRedundancy::eliminateRedundantComputations(
  ...
  if (trace())
    traceMsg(comp(), "Eliminating redundant computations in block number %d "
                     "visit count %d\n",
        ...
        if (isExpressionRedundant(currentTree->getNode(), redundantComputations,
          ...
          if (!currentTree->getNode()->getOpCode().isStore()) {
            if (0 && performTransformation(
                         comp(), "%sEliminating redundant check computation "
                                 "(resolve or null check) : %p\n",
                         OPT_DETAILS, currentTree->getNode())) // Look at
                                                               // check-in
                                                               // comment for
                                                               // version 1.296
            {
                     ...
                     .isCheck()) // Look at check-in comment for version 1.296
      {
        ...
        if (isExpressionRedundant(currentTree->getNode(), redundantComputations,
          ...
          if (firstOpCodeInTree.isResolveOrNullCheck()) {
            if (performTransformation(comp(), "%sEliminating redundant check "
                                              "computation (resolve or null "
                                              "check) : %p\n",
                                      OPT_DETAILS, currentTree->getNode())) {
              if (firstOpCodeInTree.getOpCodeValue() == TR::NULLCHK)
                TR::Node::recreate(currentTree->getNode(), TR::treetop);
    ...
    if (isExpressionRedundant(node, redundantComputations,
      ...
      if (!isNodeAnImplicitNoOp(node)) {
        ...
        TR::SymbolReference *newSymbolReference =
            _newSymbolReferences[nodeIndex];
        if (newSymbolReference &&
            performTransformation(comp(), "%sEliminating redundant computation "
                                          "(%s) : %p in block_%d, visit count "
                                          "%d\n",
...
bool TR_PartialRedundancy::isNotPrevTreeStoreIntoFloatTemp(TR::Symbol *symbol) {
  if (_prevTree && (_prevTree->getNode()->getOpCode().isFloat() ||
...
TR::TreeTop *TR_PartialRedundancy::replaceOptimalSubNodes(
  ...
  if (isSupportedOpCode(node, parent) &&
    ...
    if (((node->getLocalIndex() != MAX_SCOUNT) &&
      ...
      // it can cause spurious failures as a result
      // of earlier optimal node placement not having been done (because of opt
      // transformation index) that interfere with last opt transformation index
      // based search.
      // This is done because optimal sub node replacement is rarely the cause
      // of bugs on it's own and this issue is just making it harder to
      // debug problems in other PRE transformations.
      // We created another option TR_TracePREForOptimalSubNodeReplacement to
      // continue to allow optimal sub node replacement to be done regardless
      // but in order to make this more useful we turn off limiting optimal
      // placements when this new option is in effect (so that it becomes safe
      // to narrow down to a
      ...
      if (shouldAllowOptimalSubNodeReplacement(comp()) &&
        ...
        if (trace())
          traceMsg(comp(), "Duplicate parent %p had its old child %p replaced "
                           "by %p with symRef #%d\n",
        ...
        if (trace())
          traceMsg(comp(), "Note : Duplicate parent %p wanted to replace its "
                           "child %p by possibly already available symRef but "
                           "FAILED to do so\n",
    ...
    if (node->getOpCode().isCheck()) {
      ...
      if (opCodeValue == TR::NULLCHK) {
        ...
        if (((!_optimisticRednSetInfo[blockNum]->get(
            ...
            !checkIfNodeCanSurvive(node->getNullCheckReference(),
                                   _indirectAccessesThatSurvive)) {
...
bool TR_ExceptionCheckMotion::includeRelevantNodes(TR::Node *node,
  ...
  if (node->getOpCode().isStore())
    rhsChild =
        (node->getNumChildren() - ((node->getOpCode().isWrtBar()) ? 2 : 1));
...
bool TR_ExceptionCheckMotion::analyzeBlockStructure(
  ...
  *_unresolvedAccessesThatSurvive = *_knownUnresolvedAccessesThatSurvive;
  ...
  for (listElem = _workingList.getListHead(); listElem != NULL;
    ...
    // list;
    // also mark this expression as one that will kill its parent (because it
    // was
  ...
  // block; this
  // is reqd as these expressions in the existing ordered opt list for this
  // block
  ...
  for (listElem = _blockInfo[blockNum]->getListHead(); listElem != NULL;
    ...
    if (!_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex())) {
      ...
      if (placedEarlierCheckOptimally) {
        ...
        // block
        // then kill this expression if it contains an indirect access as it is
        // not
        // allowed to cross this block (it is not allowed to move past the
        // check)
        //
        if (_exprsContainingIndirectAccess->get(node->getLocalIndex())) {
          if (indirectAccessesKilled) {
      ...
      // in this block. This is driven by the optimalACCESSKilled flags rather
      // than the
      // ACCESSKilled flags (which are really used to limit which expressions
      // can stay
      ...
      //
      if (canPlaceOptimally &&
          !_actualOptSetInfo[blockNum]->get(
              listElem->getData()->getLocalIndex())) {
        _tryAnotherIteration = true;
        if (trace())
          traceMsg(comp(), "IN ORDER Expr %d (representative) Node %p "
                           "(listElem %p) in Block : %d\n",
                   listElem->getData()->getLocalIndex(), listElem->getData(),
                   listElem, blockNum);
  ...
  *_unresolvedAccessesThatSurvive = *_knownUnresolvedAccessesThatSurvive;
  ...
  // Some expressions that are computed in this block are okay to keep in this
  // block but not okay to "move"
  // any further back.
  //
  _workingList.deleteAll();
  ...
  if (_exceptionCheckMotion->getTrySecondBestSolution()) {
    ...
    for (listElem = genSetInfo[blockNum]->getListHead(); listElem != NULL;
      ...
      */
      ...
      //
      if (trace())
        traceMsg(comp(), "CONSIDERING Expr %d (representative) Node %p "
                         "(listElem %p) in Block : %d\n",
                 listElem->getData()->getLocalIndex(), listElem->getData(),
                 listElem, blockNum);
