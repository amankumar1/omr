...
int32_t TR_LoopCanonicalizer::perform() {
...

  {
    ...
    /*
       ...
       canonicalizer, expecting natural loop");
          if (naturalLoop->getEntryBlock()->isCold())
             continue;
...
void TR_LoopTransformer::detectWhileLoopsInSubnodesInOrder(
  ...
  while (!_analysisStack.isEmpty() &&
    ...
    if (!_nodesInCycle->get(rootNode->getNumber())) {
      ...
      predList.insert(predList.end(),
                      rootNode->getExceptionPredecessors().begin(),
                      rootNode->getExceptionPredecessors().end());
    ...
    succList.insert(succList.end(), rootNode->getExceptionSuccessors().begin(),
                    rootNode->getExceptionSuccessors().end());
    ...
    if (!isDoWhileLoop && loopHeader && (numSubNodes > 1)) {
      ...
      if ((hdrBlock->hasExceptionPredecessors() == false) &&
        ...
        if (entryNode->getSuccessors().size() == 2) {
          ...
          TR::CFGEdge *succ2 =
              entryNode->getSuccessors().back(); // Get the 2nd successor
                  ...
                  for (exitBlock = blocksIt.getCurrent(); exitBlock;
                    ...
                    if (exitBlock != hdrBlock) {
                      if (exitBlock->getSuccessors().size() == 2) {
                        TR::CFGEdge *exitSucc1 =
                            exitBlock->getSuccessors().front();
                        TR::CFGEdge *exitSucc2 =
                            exitBlock->getSuccessors()
                                .back(); // Get the 2nd Successor
    ...
    if (parentStructure) {
      ...
      if (!(subNode->getExceptionPredecessors().empty() &&
          ...
          */
          ...
          // easily;
          // a catch block normally has some specific info (handler info etc.)
          // that are
          // created by IL generator and if we try to create new catch blocks,
          // then
...
void TR_LoopCanonicalizer::canonicalizeNaturalLoop(
  ...
  */
  ...
  if (trace()) {
    traceMsg(comp(), "Setting frequency of peeled iteration block_%d to %d, s1 "
  ...
  TR_RegionStructure *properRegion =
      new (trHeapMemory()) TR_RegionStructure(comp(), loopHeader->getNumber());
  ...
  TR_BlockStructure *clonedHdrBlockStructure = new (trHeapMemory())
  ...
  node = new (trHeapMemory()) TR_StructureSubGraphNode(
      new (trHeapMemory())
  ...
  if (reversedBranch) {
    ...
    TR_BlockStructure *extraGotoStruct = new (trHeapMemory())
 ...
 * external to the doWhileLoop, modifies
 * any branches or direct paths on their last tree top so that their new
 * destination is the targetBlock, instead
...
void TR_LoopCanonicalizer::canonicalizeDoWhileLoop(
  ...
  if (!comp()->getOption(TR_DisableEmptyPreHeaderCheck)) // Skip adding another
                                                         // pre-header if you
                                                         // don't need to.
  {
  ...
  TR_BlockStructure *invariantBlockStructure = new (trHeapMemory())
  ...
  if (isEntry) {
    ...
    dummyEntryBlockStructure = new (trHeapMemory()) TR_BlockStructure(
  ...
  // blockAtHeadOfLoop)
  // Since we just calculated the actual frequency of loopInvariantBlock, we
  // need to apply that to the edge
  // if necessary
  ...
  if (isEntry) {
    TR_StructureSubGraphNode *dummyEntryNode =
        new (trHeapMemory()) TR_StructureSubGraphNode(dummyEntryBlockStructure);
  ...
  if (primaryInductionVariable && bivIterator.getFirst()) {
    ...
    if (alteredCode) {
      if (loopInvariantBlock->getBlock()
        ...
        // == TR::Goto), "Expect the loop pre-header to end in a goto for call
        // to TR::Block::split to work correctly\n");
 ...
 * formed
 *  by the codegen will not have any space for a folded in variable
 * substitution.
 *
 *  @return true, when the induction variable is used only in simple addressing
 * modes
 *          false, when the induction variable is used in complex addressing
 * modes
 */
  ...
  if (opCode.isStoreIndirect()) {
    ...
    if (node->getOpCodeValue() == TR::imul) {
      ...
      if (node->getFirstChild()->getOpCode().hasSymbolReference() &&
        ...
        traceMsg(comp(), "\tAvoiding induction variable replacement because of "
                         "address mode complexity. Sym Ref. = %p\n",
 ...
 * block
 * would violate the CFG constraint that it can contain only one branch
 * instruction.
 * The goto inserted at the end earlier in canonicalization is removed and the
 * target
      ...
      if ((prevTreeOfTarget->getNode()->getOpCode().isBranch() &&
        ...
        // targetTree)))
        {
          ...
          if (exitTreeTop->getNextTreeTop() != targetTree) {
            ...
            while (placeToMoveTrees &&
                   placeToMoveTrees->hasSuccessor(
...
TR_LoopTransformer::getCorrectNumberOfIterations(TR::Node *node,
  ...
  case TR::ificmple: {
    ...
    // node->getFirstChild()->duplicateTree());
    // subNode = TR::Node::create(TR::iadd, 2, subNode, TR::Node::create(node,
    // TR::iconst, 0, 1));
    // break;
  }
  case TR::ificmpgt: {
    ...
    if (isInvertibleLoop(nextInductionVariableNumber, loopStructure)) {
      ...
      if (v) {
        ...
        if (entryVal && entryVal->asIntConst() &&
            ...
            // comparand(RhsOfStoreNode).
            // Workaround is only allow abs(incr)==1.   This is also a hard
            // requirement for unsigned IV to not introduce wraparound.
            // For signed case, we may allow abs(incr)>1, but exit value calc is
            // not just RhsOfStoreNode if it doesn't land on it. todo
