...
TR_UseDefInfo::TR_UseDefInfo(TR::Compilation *comp, TR::CFG *cfg,
    ...
    : _compilation(comp), _optimizer(optimizer),
      _atoms(comp->allocator(allocatorName),
             CS2::Pair<TR::Node *, TR::TreeTop *>(NULL, NULL)),
      _useDefForMemorySymbols(false),
      _useDefInfo(comp->allocator(allocatorName),
      ...
      _defUseInfo(comp->allocator(allocatorName),
                  TR_UseDefInfo::BitVector(comp->allocator(allocatorName))),
      _loadDefUseInfo(comp->allocator(allocatorName),
                      TR_UseDefInfo::BitVector(comp->allocator(allocatorName))),
      _tempsOnly(false), _trace(comp->getOption(TR_TraceUseDefs)),
      _hasLoadsAsDefs(loadsShouldBeDefs),
      _useDefs(comp->allocator(allocatorName)), _numMemorySymbols(0),
      _valueNumbersToMemorySymbolsMap(
          comp->allocator(allocatorName),
          TR_UseDefInfo::MemorySymbolList(comp->allocator(allocatorName))),
      _sideTableToSymRefNumMap(comp->allocator(allocatorName)), _cfg(cfg),
...
void TR_UseDefInfo::prepareUseDefInfo(bool requiresGlobals, bool prefersGlobals,
  ...
  if (_useDefForMemorySymbols) {
    ...
    // discarded when aux is discarded
    // but to do this TR_ValueNumberInfo should be converted to not be
...
void TR_UseDefInfo::setVolatileSybolsIndexAndRecurse(
  ...
  for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
    ...
    // I believe this to be a possible optimization.  However, it seems like
    // some pali aliases end up making us still overly conservative
    // I think more alias refinement should be done -- ie if a symbol does not
    // appear in the method, we should remove it from all alias sets.
    // if(aliasedSymRef->getSymbol()->isMethod())
    //   continue;

    //      if (symRef->getSymbol()->isVolatile())
    //         {
    //         traceMsg(comp(), "Recursing on alias
    //         %d\n",aliasedSymRef->getReferenceNumber());
    setVolatileSybolsIndexAndRecurse(volatileSymbols,
                                     aliasedSymRef->getReferenceNumber());
    //         }
...
void TR_UseDefInfo::fillInDataStructures(AuxiliaryData &aux) {
  ...
  comp()->incVisitCount();
  TR::Block *block = NULL;
  TR::SparseBitVector expandedLIndexes(comp()->allocator()); // expanded
                                                             // localIndex for
                                                             // variables
                                                             // defined by
                                                             // function
    ...
    for (int32_t i = getFirstDefIndex(); i < _numDefsOnEntry; i++) {
      ...
      for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
        ...
        if (!aliasedSym || !aliasedSym->isMethod())
          continue;
        if (aux._neverReferencedSymbols.ValueAt(
                aliasedSymRef->getReferenceNumber()))
          continue;
...
void TR_UseDefInfo::findTrivialSymbolsToExclude(TR::Node *node,
  ...
  if (node->getOpCode().hasSymbolReference() &&
      aux._neverReferencedSymbols.ValueAt(
          node->getSymbolReference()->getReferenceNumber())) {
    aux._neverReferencedSymbols[node->getSymbolReference()
                                    ->getReferenceNumber()] = false;
  }
  ...
  if (node->getOpCode().isStoreDirect()) {
    ...
    if (symRef->getSymbol()->isAutoOrParm()) {
      ...
      if (aux._neverWrittenSymbols.ValueAt(symRefNum)) {
        ...
        // following code fails assert when
        // aux._onceWrittenSymbolsIndices[symRefNum].IsZero() - needs to be
        // fixed
        // int32_t prevStoreIndex =
        // aux._onceWrittenSymbolsIndices[symRefNum].FirstOne();
...
void TR_UseDefInfo::findMemorySymbols(TR::Node *node) {
          ...
          node->getFirstChild()) // value number is shared with some other node
  {
    ...
    TR_UseDefInfo::MemorySymbolList::Cursor cursor(
        _valueNumbersToMemorySymbolsMap[valueNumber]);
    ...
    if (!found)
      _valueNumbersToMemorySymbolsMap[valueNumber].Push(
          MemorySymbol(size, offset, _numMemorySymbols++));

    if (trace())
      traceMsg(comp(), "Node %p has memory symbol index %d (%d:%d:%d)\n", node,
               _valueNumbersToMemorySymbolsMap[valueNumber].Head()->_localIndex,
...
bool TR_UseDefInfo::indexSymbolsAndNodes(AuxiliaryData &aux) {
  ...
  if (trace()) {
    traceMsg(comp(), "_neverReferencedSymbols[count = %d]: ",
             aux._neverReferencedSymbols.PopulationCount());
  ...
  CS2::HashTable<uint32_t, uint32_t, TR::Allocator> num_aliases_table(
      comp()->allocator());
  CS2::HashTable<uint32_t, uint32_t, TR::Allocator> num_method_aliases_table(
      comp()->allocator());
    ...
    if (symRef) {
      ...
      if (sym) {
        ...
        if (!shouldIndexVolatileSym(symRef, aux)) {
          if (trace())
            traceMsg(comp(), "Ignoring Symbol [%p] because it is volatile %d "
                             "or aliased to a volatile %d\n",
                     sym, sym->isVolatile(),
                     aux._volatileOrAliasedToVolatileSymbols[symRefNumber] ==
                         true);
          continue;
        }
        ...
        if (!isTrivialUseDefSymRef(symRef, aux) &&
            ((sym->isParm() &&
              !aux._neverWrittenSymbols.ValueAt(
                  symRef->getReferenceNumber())) ||
  ...
  for (refIter.SetToFirstOne(); refIter.Valid(); refIter.SetToNextOne()) {
    ...
    if (trace()) {
      traceMsg(comp(), "Non Trivial symbol Ref [%p:%d] isTrivial=%d isParm=%d "
                       "_neverWritten=%d",
  ...
  // The expanded set is used during reaching definitions analysis and the
  // normal
  ...
  // a cleared _symRefToLocalIndexMap is required for each call to
  // indexSymbolsAndNodes - would be better to create as a local variable here,
  // and pass down to findUseDefNodes, but findUseDefNodes is recursive, and
  // don't want to increase stack size, so keep in aux
...
bool TR_UseDefInfo::findUseDefNodes(TR::Block *block, TR::Node *node,
  ...
  if (symIndex != NULL_USEDEF_SYMBOL_INDEX) {
    if (parent && parent->getOpCode().isResolveCheck() && num_aliases > 1) {
      ...
      if (isTrivialUseDefNode(node, aux)) {
        if (symRef &&
            aux._onceReadSymbolsIndices[symRef->getReferenceNumber()].ValueAt(
                node->getGlobalIndex()))
          aux._onceReadSymbols[symRef->getReferenceNumber()][useDefIndex] =
              true;

        if (symRef &&
            aux._onceWrittenSymbolsIndices[symRef->getReferenceNumber()]
                .ValueAt(node->getGlobalIndex()))
          aux._onceWrittenSymbols[symRef->getReferenceNumber()][useDefIndex] =
              true;
      }
    }
    // else
    ...
    //   }
             ...
             num_aliases > 0) {
    if (!aux._volatileOrAliasedToVolatileSymbols[symRef
                                                     ->getReferenceNumber()]) {
      localIndex = _numExpandedDefOnlyNodes;
    ...
    localIndex = _numExpandedDefOnlyNodes;
    _numExpandedDefOnlyNodes += TR::NumTypes + 1; // == arraylet shadows + read
                                                  // barrier....encode this
...
bool TR_UseDefInfo::assignAdjustedNodeIndex(TR::Block *block, TR::Node *node,
  ...
  if (symRef &&
    ...
    aux._onceReadSymbols[symRef->getReferenceNumber()][nodeIndex + adjustment] =
        true;
  }
...
void TR_UseDefInfo::insertData(TR::Block *block, TR::Node *node,
  ...
  _atoms[node->getUseDefIndex()] =
      CS2::Pair<TR::Node *, TR::TreeTop *>(node, treeTop);
  if (!isTrivialUseDefNode(node, aux) && adjustArray)
    aux._expandedAtoms[node->getLocalIndex()] =
        CS2::Pair<TR::Node *, TR::TreeTop *>(node, treeTop);

  if (trace()) {
    if (!definesMultipleSymbols)
      traceMsg(comp(), "Node : %p   opCode = %s useDefIndex = %d localIndex = "
                       "%d definesMultipleSymbols=%d isTrivialUseDefNode=%d "
                       "adjustArray=%d \n",
               node, opCode.getName(), node->getUseDefIndex(),
               node->getLocalIndex(), definesMultipleSymbols,
               isTrivialUseDefNode(node, aux), adjustArray);
    else
      traceMsg(comp(), "Node : %p   opCode = %s useDefIndex = %d localIndex = "
                       "%d-%d definesMultipleSymbols=%d isTrivialUseDefNode=%d "
                       "adjustArray=%d \n",
               node, opCode.getName(), node->getUseDefIndex(),
               node->getLocalIndex(), node->getLocalIndex() + num_aliases - 1,
               definesMultipleSymbols, isTrivialUseDefNode(node, aux),
               adjustArray);
  }
...

  {
    ...
    for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
      ...
      if (trace())
        traceMsg(comp(), "    symbol (u/d index=%d) is defined by node with "
                         "localIndex %d \n",
                 j, k);
    }
    ...
    if (node->getOpCode().isStoreDirect()) {
      ...
      for (i = 0; i < num; i++) {
        aux._defsForSymbol[symRef->getSymbol()->getLocalIndex()]
                          [nodeIndex + i] = true;
        aux._expandedAtoms[nodeIndex + i] =
            CS2::Pair<TR::Node *, TR::TreeTop *>(node, treeTop);
      }
    }
  }
...
void TR_UseDefInfo::buildUseDefs(void *vblockInfo, AuxiliaryData &aux) {
  ...
  if (trace()) {
    traceMsg(comp(), "\nUse/Def info: firstDefIndex=%d lastDefIndex=%d "
                     "firstUseIndex=%d lastUseIndex=%d "
                     "getNumDefsOnEntry()=%d\n",
  ...
  for (int32_t j = getFirstUseIndex(); j <= lastUseIndex; j++) {
    ...
    if (trace()) {
      ...
      // assert as the
      // one above, not guarded by trace().  It looks like that one has been
      // adjusted without
...
void TR_UseDefInfo::dereferenceDefs(int32_t useIndex,
  ...
  if (trace()) {
    traceMsg(comp(), "New defs for use index %d : ",
             useIndex + getFirstUseIndex());
    (*comp()) << _useDefInfo[useIndex];
    traceMsg(comp(), "\n");
  }
...
void TR_UseDefInfo::dereferenceDef(TR_UseDefInfo::BitVector &useDefInfo,
  ...
  CS2::LinkedListOf<CS2::Pair<TR::Node *, TR::TreeTop *>, TR::Allocator>
      defIndices(comp()->allocator());
...
void TR_UseDefInfo::buildUseDefs(TR::Node *node, void *vanalysisInfo,
                                 TR::BitVector &nodesToBeDereferenced,
  ...
  if (node->getOpCode().hasSymbolReference() &&
    ...
    if (node->getOpCode().isLoadVarDirect() || (isLoadAddrUse(node))) {
      ...
      if (aux._neverWrittenSymbols.ValueAt(
              node->getSymbolReference()->getReferenceNumber())) {
...
TR_UseDefInfo::getUseDef_ref_body(int32_t useIndex,
                                  TR_UseDefInfo::BitVector &visitedDefs,
...
TR::Node *TR_UseDefInfo::getNode(int32_t index) {
  ...
  if (ud->isDef()) {
    ...
    if ((node->getOpCode().isCheck() ||
      ...
      // getFirstChild
      // But we must save and restore the number of children here, because if VP
...
void TR_UseDefInfo::buildDefUseInfo(bool loadAsDef) {
  ...
  for (int32_t i = getFirstUseIndex(); i <= getLastUseIndex(); i++) {
    ...
    if (!defs.IsZero()) {
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        int32_t defIndex = cursor;
        TR_ASSERT((defIndex < getNumDefNodes()), "USEDEF: found def which is "
                                                 "not store or call: useIndex "
                                                 "= %d, defIndex = %d",
                  i, defIndex);
...
int32_t TR_UseDefInfo::getSymRefIndexFromUseDefIndex(int32_t udIndex) {
  TR_ASSERT(udIndex < _sideTableToSymRefNumMap.NumberOfElements(),
