...
int32_t TR_TrivialInliner::perform() {
  ...
  if (sym->mayHaveInlineableCall() && !comp()->isDisabled(OMR::inlining)) {
    ...
    if (comp()->getOption(TR_Randomize) ||
      ...
      // getTrivialInlinerMaxSize()
      // because the result of convertNonDeterministicInput is cached the first
      // time is executed
      // and don't want this result to be determined by the first compilation
      // that comes through
...
void TR_InlinerBase::setInlineThresholds(
  ...
  if (comp()->getOption(TR_Randomize)) {
    ...
    traceMsg(comp(), "\nTR_Randomize "
                     "Enabled||TR_InlinerBase::inlineCallTarget, SeedValue:%d",
             comp()->getOptions()->getRandomSeed());
  }

  heuristicTrace(
      tracer(), "Inlining Limits:\n\tCaller Side Weight Limit "
...
// anything because we have already implanted, during
...
// return) to a goto to the OSR code block of the caller, and
...
bool OMR_InlinerUtil::validateInterfaceImplementation(
    TR_ResolvedMethod *interfaceMethod) {
  return true;
}
...
bool TR_InlinerBase::exceedsSizeThreshold(
  ...
  if (block) {
    ...
    if (allowBiggerMethods()) {
      ...
      if (blockFrequency > borderFrequency) {
        ...
        if (comp()->trace(OMR::inlining))
          heuristicTrace(tracer(), "exceedsSizeThreshold: Scaled down size for "
                                   "call block_%d from %d to %d",
  ...
  if (inlineThreshold && (uint32_t)bytecodeSize > inlineThreshold) {
    ...
    heuristicTrace(tracer(), "### Exceeding size threshold because "
                             "bytecodeSize %d > inlineThreshold %d ",
                   bytecodeSize, inlineThreshold);
    return true; // Exceeds size threshold
  }

  heuristicTrace(tracer(), "### Did not exceed size threshold, bytecodeSize %d "
                           "<= inlineThreshold %d ",
                 bytecodeSize, inlineThreshold);
...
void TR_InlinerBase::createParmMap(TR::ResolvedMethodSymbol *calleeSymbol,
  ...
  ListIterator<TR::ParameterSymbol> parms(
      &calleeSymbol->getLogicalParameterList(comp()));
...
bool TR_InlineCall::inlineCall(TR::TreeTop *callNodeTreeTop,
  ...
  if (resolvedCalleeSymbol &&
      !getPolicy()->canInlineMethodWhileInstrumenting(
  ...
  if (recursiveInlining && initialMaxSize == 0)
    initialMaxSize = isScorching(comp()) ? 140 : isHot(comp())
                                                     ? 90
                                                     : isWarm(comp()) ? 60 : 45;
  ...
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    ...
    if (initialMaxSize > 0 &&
        getPolicy()->getInitialBytecodeSize(calltarget->_calleeSymbol, comp()) >
            initialMaxSize) {
      heuristicTrace(tracer(), "Failed at Inlining. getMaxByteCodeIndex of %d "
                               "exceeds initialMaxSize of %d",
...

    {
      ...
      // for recursive
      // methods triggering a loop.  Unfortunately the callStack is not
      // pervasive
      // for all these seperate calls, so check the VM stack to prevent infinite
      // inlining
      // of recursive calls
      //
  ...
  if (!comp()->getOption(TR_DisableAdaptiveDumbInliner)) {
    int32_t callerSize =
        callerSymbol->getResolvedMethod()
            ->maxBytecodeIndex(); // getInitialBytecodeSize(callerSymbol,
                                  // comp());
    maxCallSize =
        prevCallStack == 0
            ?
            // Initial budget should be inverse proportional to the size of the
            // method
            (int32_t)_initialSize -
                callerSize * (int32_t)_initialSize /
                    comp()->getOptions()->getDumbInlinerBytecodeSizeCutoff()
            :
            // If I inlined a bigger method, my budget should decrease by a
            // bigger amount
      ...
      if (node->getOpCode().isCall() &&
        ...
        if ((isCold
          ...
          if (inlineCount >= MAX_INLINE_COUNT) {
            if (comp()->trace(OMR::inlining))
              traceMsg(comp(), "inliner: stopping inlining as max inline count "
                               "of %d reached\n",
                       MAX_INLINE_COUNT);
            break;
          }
    ...
    if ((byteCodeSize > maxBCSize)) {
      if (tryToInline("overriding getMaxBytecodeIndex check", calltarget)) {
        ...
        if (comp()->cg()->traceBCDCodeGen()) {
          traceMsg(comp(), "q^q : failing to inline %s into %s (callNode %p on "
...
static TR::TreeTop *cloneAndReplaceCallNodeReference(TR::TreeTop *, TR::Node *,
                                                     TR::Node *, TR::TreeTop *,
                                                     TR::Compilation *);
static void addSymRefsToList(List<TR::SymbolReference> &,
                             List<TR::SymbolReference> &);

static TR::Node *findPotentialDecompilationPoint(TR::Node *node,
                                                 TR::Compilation *comp) {
  ...
  if (node->getOpCode().hasSymbolReference() &&
      node->getSymbolReference()->canCauseGC())
    return result = node;
  else
...
findPotentialDecompilationPoint(TR::ResolvedMethodSymbol *calleeSymbol,
  ...
  comp->incVisitCount();
  TR::Node *result = NULL;
  for (TR::TreeTop *tt = calleeSymbol->getFirstTreeTop(); tt && !result;
       tt = tt->getNextTreeTop())
...
/*
Finds all nodes that are references to the given call node in callerSymbol and
replaces them with the resultNode
OR
...
void TR_InlinerBase::replaceCallNode(TR::ResolvedMethodSymbol *callerSymbol,
  ...
  //
  if (resultNode) {
    resultNode->setVisitCount(
        _visitCount); // make sure we don't visit this node again
...
void TR_InlinerBase::replaceCallNodeReferences(
  ...
  // changed
  // if we are inlining as we walk. We might think we have not inlined some call
  // and
  // try to inline it again and again (as was seen during dumb inlining).
  // Note that this routine we are in could be called from code reached from
  // dumb inliner where inline
  // as we walk the trees in inlineCallTargets as well as the call graph inliner
  // inlineCallTargets
  // where we may/may not inline as we walk the trees. Handling both cases is
  // something to
...
TR::Node *TR_InlinerBase::createVirtualGuard(
  ...
  if (guard->_type == TR_VftTest) {
    ...
    return TR_VirtualGuard::createRubyInlineGuard(
        guard->_kind, comp(), calleeIndex, callNode, destination,
        guard->_thisClass);
...
void TR_InlinerBase::rematerializeCallArguments(
  ...
  debugTrace(tracer(), "privatizedInlinerArg - consider arguments for "
                       "rematerialization for guard [%p] of call node [n%dn]",
  ...
  if (suitableForRemat) {
    ...
    // another
    // store of the argument that we can use for partial remat purposes - hibb
    // often
    // makes these for us
    ...
    for (uint32_t i = 0; i < argSafetyInfo.size(); ++i) {
      ...
      if (!unsafeSymRefs.Intersects(argSafetyInfo.symRefDependencies(i))) {
        ...
        // from another temp
          ...
          if (performTransformation(
                  comp(), "O^O GUARDED CALL REMAT: Partial rematerialize of "
                          "[%p] as [%p] - load of [%d]\n",
                  argStore, duplicateStore, rematTree->getNode()
                                                ->getSymbolReference()
                                                ->getReferenceNumber())) {
            rematPoint =
                TR::TreeTop::create(comp(), rematPoint, duplicateStore);
        ...
        debugTrace(tracer(), " priv arg remat failed for node [%p] - %d - due "
...
TR::TreeTop *TR_InlinerBase::addGuardForVirtual(
  ...
  //  last block in the inlined code)
  //  BBStart [3]
  //
  ...
  // TODO : This is a very hacky way of avoiding HCR guards on sensitive String
  // Compression methods which allows idiom recognition to work. Can we do
  // something better?
  ...
  if (!disableHCRGuards && comp()->getHCRMode() != TR::none &&
    ...
    if (virtualGuard && comp()->cg()->supportsMergingOfHCRGuards()) {
  ...
  // block which will
  // do a virtual call on the correct receiver for the method we want to run or
  // 2) we could
  // transfer control from the compiled code back to the interpreter using an
  // On-Stack-Replacement
  // (OSR) mechanism.
  //
  // Hot-Code-Replace or HCR mode is where the compiler is running in a mode
  // that assumes methods
  // could be redefined at runtime. The compiler can support this mode using
  // traditional virtual
  ...
  // compilation and those later processes will handle them using OSR so we
  // don't want to complicate
  // that with additional OSR at this point
  if ((comp()->getHCRMode() != TR::osr || guard->_kind != TR_HCRGuard) &&
      callerSymbol->supportsInduceOSR(callNode->getByteCodeInfo(), block1,
                                      calleeSymbol, comp(), false)) {
    ...
    if (shouldUseOSR &&
      ...
      TR::TreeTop *induceTree = callerSymbol->genInduceOSRCallForGuardedCallee(
          guardedCallNodeTreeTop, calleeSymbol,
...
bool TR_InlinerBase::heuristicForUsingOSR(
  ...
  while (osrMethodData) {
    ...
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(), "OSR caller at inlined site index %d has %d bytecodes "
                       "and %d stack slots, total callers bytecodes %d total "
                       "callers stack slots %d total callers live stack slots "
                       "%d total pending push slots %d\n",
...
TR_ParameterToArgumentMapper::TR_ParameterToArgumentMapper(
    ...
    : _callerSymbol(callerSymbol), _calleeSymbol(calleeSymbol),
      ...
      _availableTemps2(availableTemps2), _vftReplacementSymRef(0),
      _lastTempTreeTop(0), _firstTempTreeTop(0), _inliner(inliner) {}
...
void TR_ParameterToArgumentMapper::printMapping() {
  ...
  for (TR_ParameterMapping *pm = _mappings.getFirst(); pm; pm = pm->getNext()) {
    debugTrace(
        tracer(), "Mapping at addr %p:\n\tparmSymbol = %p (offset %d) "
                  "\treplacementSymRef = %d\t_parameterNode = "
                  "%p\treplacementSymRef2 = %d\treplacementSymRef3 = %d\n"
                  "\t_argIndex = %d\t_parmIsModified = %d\t_isConst = "
                  "%d\t_addressTaken =%d",
        pm, pm->_parmSymbol, pm->_parmSymbol->getOffset(),
        pm->_replacementSymRef ? pm->_replacementSymRef->getReferenceNumber()
                               : -1,
        pm->_parameterNode,
        pm->_replacementSymRef2 ? pm->_replacementSymRef2->getReferenceNumber()
                                : -1,
        pm->_replacementSymRef3 ? pm->_replacementSymRef3->getReferenceNumber()
                                : -1,
    ...
    if (parmMap->_parmSymbol->getParameterOffset() == argOffset) {
      ...
      if (!parmMap->_parmIsModified &&
        ...
        if (parmMap->_addressTaken) {
          ...
          debugTrace(tracer(), "Setting parameterNode to n%in %s, "
                               "argOffset=%d, argIndex=%d, _callNode n%in",
                     arg->getNodePoolIndex(), arg->getOpCode().getName(),
                     argOffset, argIndex, _callNode->getNodePoolIndex());
          ...
          if (disableParmTempOpt) {
            ...
            if ((arg->getOpCode().isLoadVarDirect() &&
              ...
              debugTrace(tracer(), "isCallIndirect = %d getFirstArgumentIndex "
                                   "= %d  arg isAuto = %d",
                         _callNode->getOpCode().isCallIndirect(),
        ...
        if (parmMap->_replacementSymRef)
...

          {
            tt = OMR_InlinerUtil::storeValueInATemp(
                comp(), arg, symRef, 0, _calleeSymbol, _tempList,
                _availableTemps, &_availableTemps2, false,
          ...
          if (!hasStaticCallStack && !neverNeedPrivatizedArguments &&
              tt->getNode()->getOpCode().isStoreDirectOrReg()) // compjazz
                                                               // 53912: PLX
                                                               // sometimes
                                                               // privatizes
                                                               // using indirect
...
TR::Node *TR_ParameterToArgumentMapper::map(TR::Node *node,
  ...
  for (TR_ParameterMapping *parmMap = _mappings.getFirst(); parmMap;
    ...
    if (parm == parmMap->_parmSymbol) {
      ...
      if (parmMap->_addressTaken &&
          parmMap->_replacementSymRef->getSymbol()->isAuto()) {
...
void TR_TransformInlinedFunction::transform() {
  ...
  if ((firstBlock->getPredecessors().size() > 1) ||
      ...
      comp()->fe()->isMethodEnterTracingEnabled(
  ...
  // while
  // walking the trees [this is possible, for example if the callee ends with a
  // 'throw'
  // instead of returning a value]; then create a zero const node (of the return
  // type of the
...
void TR_TransformInlinedFunction::transformNode(TR::Node *node,
                                                TR::Node *parent,
                                                uint32_t childIndex) {
    ...
    if (symbol->isParm()) {
      ...
      if (newNode && newNode != node) {
        if (newNode->getOpCode().isLoadConst() &&
          ...
          newNode->setReferenceCount(1);
          dumpOptDetails(comp(), "%screate %s (0x%p) to resolve type mismatch "
                                 "between %s (%p) and %s (%p)\n",
...
void TR_HandleInjectedBasicBlock::printNodesWithMultipleReferences() {
  ...
  for (MultiplyReferencedNode *mn = _multiplyReferencedNodes.getFirst(); mn;
       mn = mn->getNext()) {
    debugTrace(
        tracer(), "MultiplyReferencedNode = %p\ttreetop = "
                  "%p\n\treplacementSymRef =%d\treplacementSymRef2 = "
                  "%d\treplacementSymRef3 = %d\t_referencesToBeFound = %d"
                  "\tisConst = %d\tsymbolCanBeReloaded = %d",
        mn->_node, mn->_treeTop,
        mn->_replacementSymRef ? mn->_replacementSymRef->getReferenceNumber()
                               : -1,
        mn->_replacementSymRef2 ? mn->_replacementSymRef2->getReferenceNumber()
                                : -1,
        mn->_replacementSymRef3 ? mn->_replacementSymRef3->getReferenceNumber()
                                : -1,
...
void TR_HandleInjectedBasicBlock::replaceNodesReferencedFromAbove(
    TR::Block *block, vcount_t visitCount) {
...
void TR_HandleInjectedBasicBlock::createTemps(bool replaceAllReferences) {
  ...
  for (MultiplyReferencedNode *ref = _multiplyReferencedNodes.getFirst(); ref;
    ...
    if (rematConstant || opcode.getOpCodeValue() == TR::loadaddr) {
      ref->_isConst = true;
    } else if (opcode.getOpCodeValue() == TR::aload &&
               comp()->cg()->getLinkage()->isAddressOfStaticSymRef(
                   ref->_node->getSymbolReference())) {
      ref->_isConst = true;
    } else if (opcode.getOpCodeValue() == TR::aload &&
               comp()->cg()->getLinkage()->isAddressOfPrivateStaticSymRef(
                   ref->_node->getSymbolReference())) {
      ref->_isConst = true;
    } else {
      ...
      // The real underlying issue is that just because a symref is on a
      // parmMap, it doesn't mean you can automatically add it to a list of
      // injected basic block temps
      // When I enabled the ParmTempOpt, this is what happened.  An auto symbol
      // got added to the injectedBasicBlockTemps list, and then was reused in a
      // very bad manner.
      if (enabletempCreationOpt) {
        for (TR_ParameterMapping *parmMap = _mappings; parmMap;
          ...
          if (parmMap->_parameterNode == ref->_node) {
            ...
            debugTrace(_tracer, "\nadding %d to injected basic block temps for "
                                "node %p is in temp list %d",
...
TR_HandleInjectedBasicBlock::MultiplyReferencedNode::MultiplyReferencedNode(
    ...
    : _node(node), _treeTop(tt), _referencesToBeFound(refsToBeFound),
      ...
      _replacementSymRef2(0), _replacementSymRef3(0), _isConst(false) {}
...
void TR_HandleInjectedBasicBlock::replace(MultiplyReferencedNode *ref,
  ...
  if (!parent->getChild(childIndex)->getByteCodeInfo().doNotProfile())
    replacementNode->getByteCodeInfo().setDoNotProfile(0); // allow profiling of
                                                           // replacement node
                                                           // if the original
                                                           // could be profiled
  parent->setChild(childIndex, replacementNode);
...
/*
 * handles the temp sharing between caller and inlined callee by storing a value
 * from the inlined method to a temp of the caller
 */
TR::TreeTop *OMR_InlinerUtil::storeValueInATemp(
  ...
  if (comp->cg()->traceBCDCodeGen())
    traceMsg(comp, "\tcreate storeNode %p of tempSymRef #%d (possibly for node "
...
static void addSymRefsToList(List<TR::SymbolReference> &calleeSymRefs,
                             List<TR::SymbolReference> &callerSymRefs) {
...
bool TR_IndirectCallSite::hasResolvedTypeArgInfo() {
  return _ecsPrexArgInfo && _ecsPrexArgInfo->get(0) &&
         ...
         _ecsPrexArgInfo->get(0)->getFixedClass();
}
...
bool TR_DirectCallSite::findCallSiteTarget(TR_CallStack *callStack,
  ...
  if (!disableHCRGuards2 && comp()->getHCRMode() != TR::none &&
      ...
      !inliner->getPolicy()->skipHCRGuardForCallee(_initialCalleeSymbol)) {
...
void TR_InlinerBase::applyPolicyToTargets(TR_CallStack *callStack,
  ...
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    ...
    if (calltarget->_calleeSymbol &&
      ...
      if (comp()->trace(OMR::inlining))
        traceMsg(comp(), "Reducing bytecode size to %d because it's method of "
                         "FloatingDecimal\n",
                 bytecodeSize);
    }
    ...
    int32_t selfInliningLimit =
        selfInliningLimitStr
            ...
            : comp()->getMethodSymbol()->doJSR292PerfTweaks()
                  ...
                  : 3;
    if (callStack &&
        callStack->isAnywhereOnTheStack(calltarget->_calleeMethod,
                                        selfInliningLimit)) {
...
static void traceRequestToCSI(TR::Compilation *comp, TR_CallSite *callsite) {
  ...
  callerIndex])._byteCodeInfo.getByteCodeIndex();
     callerIndex = comp->getInlinedCallSite(stack[
  callerIndex])._byteCodeInfo.getCallerIndex();
     }
   */
}
...
TR_CallSite *TR_InlinerBase::findAndUpdateCallSiteInGraph(
  ...
  if (foundCall && callNode->getSymbolReference()->isUnresolved() &&
    ...
    debugTrace(tracer(), "findAndUpdateCallsiteInGraaph: call at node %p is "
                         "unresolved. Bail out",
               callNode);
    return 0;
  }
  ...
  if (callNode->isTheVirtualCallNodeForAGuardedInlinedCall()) {
    heuristicTrace(tracer(), "findAndUpdateCallSiteInGraph: this call %p is on "
                             "a cold side. Bail out",
                   callNode);
    return 0;
  }
  ...
  if (comp()->getOptions()->enableDebugCounters()) {
    if (!foundCall) {
      for (callsite = calltarget->_deletedCallees.getFirst(); callsite;
           callsite = callsite->getNext()) {
        debugTrace(tracer(), "considering deleted callee %p "
                             "callNode->getByteCodeIndex = %d "
                             "callsite->_byteCodeIndex = %d",
  ...
  if (callNode->getSymbolReference()
    ...
    debugTrace(tracer(), "findAndUpdateCallSiteInGraph: BAD: Interface call "
                         "has an initialCalleeSYmbol %p for calNode %p",
  ...
  if (!callNode->getOpCode().isCallIndirect()) {
    ...
    // callsite's calleeSymbol
    // also it seemed less invasive to assign i to 0 rather than change every
    // single occurence of (i)
    ...
    if (callsite->numTargets() > 0) {
              ...
              ->isIndirectCall()) // local opts pre-existence has been performed
      {
        ...
        if (!callsite->getTarget(i)->_calleeMethod->isSameMethod(
                callsite->_initialCalleeSymbol->getResolvedMethod())) {
          callsite->getTarget(i)->_myCallees.setFirst(0); // preventing furhter
                                                          // inlining as we now
                                                          // don't know what's
                                                          // beyond the new
                                                          // target
  ...
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    ...
    if (callsite->getTarget(i)->_guard->_kind == TR_InterfaceGuard &&
      ...
      if (tracer()->debugLevel())
        tracer()->dumpCallSite(
            callsite, "findAndUpdateCallSiteInGraph: BAD: Interface call has "
                      "an initialCalleeSYmbol %p for calNode %p",
            callsite->_initialCalleeSymbol, callNode);
...
void TR_InlinerBase::inlineFromGraph(TR_CallStack *prevCallStack,
  ...
  for (TR::TreeTop *tt = callerSymbol->getFirstTreeTop(); tt;
    ...
    if (isCall && tt->getNode()->getChild(0)->getVisitCount() != _visitCount &&
        ...
        tt->getNode()->getChild(0)->getSymbolReference() !=
            comp()->getSymRefTab()->element(TR_induceOSRAtCurrentPC)) {
      TR::Node *node = parent->getChild(0);
      ...
      if (!isCold) {
        debugTrace(tracer(), "inlineFromGraph:: about to call "
                             "findAndUpdateCallSiteInGraph on call target %p "
                             "for call at node %p",
                   calltarget, node);
        TR_CallSite *site = findAndUpdateCallSiteInGraph(
            &callStack, node->getByteCodeInfo(), tt, parent, node, calltarget);
        debugTrace(tracer(), "inlineFromGraph: found a call at parent %p child "
                             "%p, findAndUpdateCallsite returned callsite %p",
        ...
        if (calleeSymbol->getMethod())
          heuristicTrace(tracer(), "Block containing call node %p (callee "
                                   "name: %s) but either caller block or "
                                   "function is cold. Skipping call.",
                         node, calleeSymbol->getMethod()->signature(
                                   trMemory(), stackAlloc));
  ...
  debugTrace(tracer(), "Done First Tree Iteration for updating PrexInfo.  Now "
                       "starting inline Loop for calltarget %p",
             calltarget);
  ...
  for (TR::TreeTop *tt = callerSymbol->getFirstTreeTop(); tt;
    ...
    for (TR_CallTarget *target = iter.getFirst(); target;
      ...
      if (target->_myCallSite->_callNodeTreeTop == tt &&
        ...
        debugTrace(tracer(), "inlineFromGraph: Calling inlinecallTarget on "
                             "calltarget %p in callsite %p",
...
static void computeNumLivePendingSlotsAndNestingDepth(
    TR::Optimizer *optimizer, TR_CallTarget *calltarget,
    TR_CallStack *callStack, int32_t &numLivePendingPushSlots,
    int32_t &nestingDepth) {
  if (optimizer->comp()->getOption(TR_EnableOSR)) {
    TR::Block *containingBlock =
        calltarget->_myCallSite->_callNodeTreeTop->getEnclosingBlock();
    int32_t weight = 1;
    ...
    TR_OSRMethodData *osrMethodData =
        optimizer->comp()->getOSRCompilationData()->findOrCreateOSRMethodData(
            optimizer->comp()->getCurrentInlinedSiteIndex(),
            callStack->_methodSymbol);
    TR_Array<List<TR::SymbolReference>> *pendingPushSymRefs =
        callStack->_methodSymbol->getPendingPushSymRefs();
    int32_t numPendingSlots = 0;

    if (pendingPushSymRefs)
      numPendingSlots = pendingPushSymRefs->size();

    TR_BitVector *deadSymRefs = osrMethodData->getLiveRangeInfo(
        calltarget->_myCallSite->_callNode->getByteCodeIndex());
...
bool TR_InlinerBase::inlineCallTarget2(TR_CallStack *callStack,
  ...
  debugTrace(
      tracer(), "Starting Physical Inlining for call target %p at callsite %p "
  ...
  if (teststring &&
      strncmp(calleeSymbol->signature(trMemory()), teststring,
              strlen(teststring)) == 0)
  ...
  if (calltarget->_myCallSite->numTargets() > 1) {
    if (calltarget->_myCallSite->_visitCount == 1) {
      ...
      // also, it is better to cache the next treetop before things get too
      // funky.
  ...
  getUtil()->calleeTreeTopPreMergeActions(calleeSymbol,
  ...
  if (comp()->getOption(TR_FullSpeedDebug) &&
    ...
    if (decompPoint) {
      ...
      if (comp()->trace(OMR::inlining)) {
        ...
        traceMsg(comp(), "FSD inlining: decompilation point %s (%s) prevented "
                         "inlining of %s\n",
  ...
  // method is being inlined
  // as a non-virtual - add a dummy virtual guard around this
  //
  if (innerPrexInfo->hasInnerAssumptions() && guard->_kind == TR_NoGuard) {
    if (comp()->trace(OMR::inlining)) {
      traceMsg(comp(), "%sIPREX virtualize the non-virtual call %s because of "
                       "inner preexistence for inner calls\n",
  ...
  if (!performTransformation(comp(), "%sInlining qwerty %s into %s with a "
                                     "virtual guard kind=%s type=%s "
                                     "partialInline=%d\n",
                             OPT_DETAILS, calleeSymbol->signature(trMemory()),
  ...
  TR_ParameterToArgumentMapper pam(callerSymbol, calleeSymbol, callNode,
                                   tempList, _availableTemps,
                                   _availableBasicBlockTemps, this);
  ...
  /**
   ...
   and replaced with
   the caller's usage of the call to the callee - in other words, a new node has
   been created
  ...
  */
  ...
  if (lastCalleeBlock) {
    ...
    while (n = calleeCFG->getNodes().pop()) {
      ...
      callerCFG->addNode(n);
      debugTrace(tracer(), "\nAdding callee blocks into caller: callee block "
                           "%p:%p:%d --> %d (caller block) ",
  ...
  if (guard->_kind != TR_NoGuard) {
    ...
    heuristicTrace(tracer(), "calltarget %p vGuard %p "
                             "calltarget->_receiverClass %p "
    ...
    // doing
    // inner preexistence (yah, i know what you are thinking -- why create it
    // when i
    // am going to remove it later? if we are going to remove it, we still need
    // it
  ...
  if (tif->crossedBasicBlock()) {
    ...
    TR::Block *virtualCallSnippet = virtualGuard
                                        ? virtualGuard->getNode()
  ...
  // motion and
  // optimization so we now go and check the privatized arguments to see if we
  // can
  // create a duplicate of the argument in the cold path - if so we do to
  // decouple
  // the hot path from the cold path and give other opts more room to work
  // NOTE this used to be part of adding a virtual guard, but we need hibb to
  // store
  ...
  if (induceOSRCallTree) {
    ...
    induceOSRCallNode->setNumChildren(
        numOfChildren - firstArgChild); // tricky "swelling" of the node but we
                                        // allocated it of the right size when
                                        // we inserted the induce OSR call to
                                        // begin with
  ...
  if (calltarget->_myCallSite->numTargets() > 1 &&
    ...
    if (lastNodeInCallBlock->getOpCode().isReturn() ||
      ...
      bool swapped =
          false; // hopefully, compiler will get rid of this in prod build
  ...
  if (calleeSymbol->getMethod()->isArchetypeSpecimen() ||
      calleeSymbol->hasMethodHandleInvokes()) {
    // Trees from archetype specimens may not match the archetype method's
    // bytecodes,
    // so there may be some calls things that inliner missed.
    //
    // Tactically, we also inline again based on hasMethodHandleInvokes because
    // EstimateCodeSize
    // doesn't yet cope with invokeHandle, invokeHandleGeneric, and
    // invokeDynamic (but it should).
    //
    _optimizer->setRequestOptimization(OMR::methodHandleInvokeInliningGroup);
    if (comp()->trace(OMR::inlining))
      heuristicTrace(
          tracer(),
          "Requesting another pass of MethodHandle invoke inlining due to %s\n",
          tracer()->traceSignature(calleeSymbol));
  }
...
TR_CallSite::TR_CallSite(TR_ResolvedMethod *callerResolvedMethod,
                         ...
                         TR::Node *callNode, TR_Method *interfaceMethod,
...
TR_CallTarget *TR_CallSite::addTarget(TR_Memory *mem, TR_InlinerBase *inliner,
  ...
  if (inliner->tracer()->heuristicLevel()) {
    char name[1024];
    heuristicTrace(
        inliner->tracer(), "Creating a call target %p for callsite %p using a "
                           "%s and %s .  Signature %s",
...
void TR_InlinerTracer::dumpCallGraphs(TR_LinkHead<TR_CallTarget> *targets) {
  ...
  for (calltarget = targets->getFirst(); calltarget;
    ...
    traceMsg(comp(), "Total Estimated Size = %d Total Size After Multipliers = "
                     "%d Total Weight = %d\n\n",
...
void TR_InlinerTracer::dumpDeadCalls(TR_LinkHead<TR_CallSite> *sites) {
  ...
  for (callsite = sites->getFirst(); callsite; callsite = callsite->getNext()) {
    traceMsg(comp(), "^^^ Top Level Dead CallSite %p Node %p bcIndex %p "
                     "Failure Reason: %s\n",
...
void TR_InlinerTracer::dumpCallTarget(TR_CallTarget *calltarget,
  ...
  traceMsg(comp(), "guard->_thisClass = %p _receiverclass = %p   (enum in "
                   "compilation.hpp)\n",
  ...
  traceMsg(comp(),
           ...
           calltarget->_weight);
  traceMsg(comp(), "_callGraphAdjustedWeight = %f \n\t\t_frequencyAdjustment = "
                   "%f _isPartialInliningCandidate = %d _partialInline = %p\n",
...
void TR_InlinerTracer::dumpPrexArgInfo(TR_PrexArgInfo *argInfo) {
...

  {
    ...
    if (arg && arg->getFixedClass()) {
      char *className = TR::Compiler->cls.classSignature(
          comp(), arg->getFixedClass(), trMemory());
      traceMsg(comp(), "<Argument no=%d address=%p classIsFixed=%d "
                       "classIsPreexistent=%d class=%p className= %s/>\n",
               i, arg, arg->classIsFixed(), arg->classIsPreexistent(),
               arg->getFixedClass(), className);
    } else {
      traceMsg(comp(), "<Argument no=%d address=%p classIsFixed=%d "
