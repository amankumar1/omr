...
TR::TreeTop *
TR_CFGSimplifier::getNextRealTreetop(TR::TreeTop *treeTop,
                                     bool skipRestrictedRegSaveAndLoad) {
...
/* So far, we've matched the normal pattern, except that instead of an ifcmpxx,
   we have a
   branch that uses the condCode. To fully match the pattern, we need the
   following:
   ...
   to previously
    (eg. we conditionally store 0 or another value to x; we need to be comparing
   x to something here)
 */
bool TR_CFGSimplifier::simplifyCondCodeBooleanStore(TR::Block *joinBlock,
                                                    TR::Node *branchNode,
                                                    TR::Node *store1Node,
                                                    TR::Node *store2Node) {
  ...
  // Everything matches
  if (!performTransformation(comp(), "%sReplace (branch on condition code [%p] "
                                     "-> boolean stores -> branch-and-compare "
                                     "using stored boolean) with single branch "
                                     "on condition code\n",
  ...
  /* At this point, we have the following structure:
     ...
     correctness; we need to either
     reverse the cond code mask, if that's allowed, or swap the taken and
     fallthrough blocks so that
     we branch to the old fallthrough block and fall through to the old taken
     block.

     If it's safe to invert the mask bits, we can do so and behave as normal.
     Otherwise, we can
     reverse the blocks, but we also need to add a block containing only a goto
     node to ensure
     that block A falls through to block F without having to reorder the list of
     blocks.
...
bool TR_CFGSimplifier::canReverseBranchMask() { return false; }
