...
TR_LiveVariableInformation::TR_LiveVariableInformation(
    ...
    bool splitLongs, bool includeParms, bool includeMethodMetaDataSymbols,
    bool ignoreOSRUses)
...
// visited
// for walking trees backwards, use future use counts to determine the last
// reference and only look at those
...
// node's tree or not (on first call, pass in false)
// if commonedLoads is non-null, then any symbols loaded while belowCommonedNode
// is set are marked in that bit vector
// if visitCount is >= 0, then it will be used to avoid visiting nodes multiple
// times
//    NOTE: if a node has its visit count set to visitCount, it will not be
//    visited unless visitEntireTree is set
//    NOTE: for backward traversals a new visit count is needed for each treetop
//    in an EBB to visit correctly and efficiently
void TR_LiveVariableInformation::visitTreeForLocals(
  ...
  if (traceLiveVarInfo()) {
    if (movingForwardThroughTrees)
      traceMsg(comp(), "         Looking forward for uses in node %p has "
                       "visitCount = %d and comp() visitCount = %d\n",
               node, node->getVisitCount(), visitCount);
    else
      traceMsg(comp(), "         Looking backward for uses in node %p has "
                       "future use count = %d and reference count = %d\n",
  ...
  } else if (!belowCommonedNode && _includeMethodMetaDataSymbols &&
             node->getOpCode().isLoadIndirect() && node->getSymbolReference()
             // && symRef->getSymbol()->isMethodMetaData()
             ) {
    if (traceLiveVarInfo())
      traceMsg(comp(), "           found indirect methodMetaData on node %p "
                       "(belowCommonedNode = %d)\n",
  ...
  if (movingForwardThroughTrees) {
    ...
    // must be different for each tree in an EBB
    // for this to work properly ...IF the visit count isn't different for each
    // tree, this DOES NOT WORK PROPERLY
          ...
          for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
            ...
            if (usedSymbol && usedSymbol->getDataType() != TR::NoType) {
              ...
              if (usedSymbolIndex != INVALID_LIVENESS_INDEX &&
                ...
                if (traceLiveVarInfo())
                  traceMsg(comp(), "       setting symIdx %d (from symRef %d) "
                                   "on usedSymbols (in liveness)\n",
