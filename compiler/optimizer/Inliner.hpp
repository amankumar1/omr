...
protected:
  ...
  int32_t scaleSizeBasedOnBlockFrequency(
      ...
      int32_t coldBorderFrequency = 0); // not virtual because we want base
                                        // class always calling base version of
                                        // this
  virtual bool
...
private:
  ...
  // TR::Node * createVftCompareGuard(TR::Node * callNode, TR::TreeTop *
  // destination, bool branchOnOverride, void * thisClass);
  // TR::Node * createOverriddenBitCompareGuard(TR::Node * callNode, TR::TreeTop
  // * destination, bool branchOnOverride, TR::ResolvedMethodSymbol *
  // calleeSymbol);
  // TR::Node * createMethodCompareGuard(TR::Node * callNode, TR::TreeTop *
  // destination, bool branchOnOverride, TR::ResolvedMethodSymbol *
  // calleeSymbol);
...
struct TR_ParameterMapping : TR_Link<TR_ParameterMapping> {
  TR_ParameterMapping(TR::ParameterSymbol *ps)
      : _parmSymbol(ps), _replacementSymRef(0), _replacementSymRef2(0),
        _parameterNode(0), _replacementSymRef3(0), _parmIsModified(false),
        _addressTaken(false), _isConst(false) {}
...
protected:
  virtual bool
  validateInterfaceImplementation(TR_ResolvedMethod *interfaceMethod);
...
public:
  ...
  bool aggressiveSmallAppOpts() {
    return TR::Options::getCmdLineOptions()->getOption(TR_AggressiveOpts);
  }
  ...
  virtual bool skipHCRGuardForCallee(TR::ResolvedMethodSymbol *calleeSymbol) {
    return false;
  }
