...
void TR_RegionAnalysis::simpleIterator(TR_Stack<int32_t> &workStack,
  ...
  for (cursor.SetToLastOne(); cursor.Valid(); cursor.SetToPreviousOne()) {
    ...
    // recursion
    // but if we do it here, we would screw up big time since we can't push the
    // same node three times.
    // We should be able to push the same node at most twice
    // so if we come across the same node the second time (nodesInPath[next]) we
    // could say that we are done with this
...
void TR_RegionAnalysis::StructInfo::initialize(TR::Compilation *comp,
                                               int32_t index,
                                               TR::Block *block) {
  _structure = new (comp->trHeapMemory())
 ...
 * cycles.
 * A proper cyclic region is a sub-graph with a single entry node and one or
 * more
 ...
 * loops.
 * On the second pass we go bottom up in the flow graph looking for regions
 * large
 * enough to consider.
...
TR_RegionAnalysis::findNaturalLoop(StructInfo &node, WorkBitVector &regionNodes,
  ...
  TR_RegionStructure *region = new (trHeapMemory()) TR_RegionStructure(
...
void TR_RegionAnalysis::addNaturalLoopNodesIterativeVersion(
  ...
  // depth-first order
  // Also, we will be pushing the same node twice in order to update nodesInPath
  // correctly
  ...
  while (!workStack.isEmpty()) {
    ...
    if (nodesInPath.ValueAt(index)) {
      ...
      // processing
      // the node again will not have any sideeffects, so we can skip it
      // completely
...
void TR_RegionAnalysis::addRegionNodes(StructInfo &node,
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
    ...
    // block. It provides an exit for all other regions.
    //
    if ((next._succ.IsZero()) && next._originalBlock == _cfg->getEnd()) {
      if (hdrBlock->getNumber() != 0)
        continue;
    }
    if (_dominators.dominates(hdrBlock, next._originalBlock))
...
void TR_RegionAnalysis::buildRegionSubGraph(TR_RegionStructure *region,
  ...
  for (rCursor.SetToFirstOne(); rCursor.Valid(); rCursor.SetToNextOne()) {
    ...
    if (cfgNodes[fromIndex] == NULL)
      cfgNodes[fromIndex] =
          new (trHeapMemory()) TR_StructureSubGraphNode(fromNode._structure);
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
      ...
      if (cfgNodes[toIndex] == NULL) {
        if (regionNodes.ValueAt(toIndex))
          cfgNodes[toIndex] =
              new (trHeapMemory()) TR_StructureSubGraphNode(toNode._structure);
        else
          cfgNodes[toIndex] = new (trHeapMemory()) TR_StructureSubGraphNode(
              toNode._structure->getNumber(), trMemory());
      }
      to = cfgNodes[toIndex];
