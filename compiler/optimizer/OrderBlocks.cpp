...
bool TR_OrderBlocks::needBetterChoice(TR::CFG *cfg, TR::CFGNode *block,
  ...
  if (bestSucc != comp()->getStartBlock() &&
    ...
    if (trace())
      traceMsg(comp(), "\t\tneedBetterChoice: hot block_%d:cold_%d: follows a "
                       "cold block bestSucc_%d:cold_%d\n",
...
// the first valid block only if there are no
// valid hot blocks.
...
bool TR_OrderBlocks::endPathAtBlock(TR::CFGNode *block, TR::CFGNode *bestSucc,
  ...
  if (_superColdBlockOnly) {
    if (block->asBlock()->isSuperCold() != bestSucc->asBlock()->isSuperCold() &&
      ...
      if (trace())
        traceMsg(comp(), "\t\tEnd path because coldness changed from block_%d "
                         "to block_%d block->asBlock()->getExit()=%p "
  ...
  if (bestSuccStructure == NULL || blockStructure == NULL) {
    if (trace())
      traceMsg(comp(), "\t\tEnd path because structure is NULL block_%d=%p to "
                       "block_%d=%p \n",
...
bool TR_OrderBlocks::isCandidateReallyBetter(TR::CFGEdge *candEdge,
  ...
  for (auto predEdge = predecessors.begin(); predEdge != predecessors.end();
       ++predEdge) {
    TR::list<TR::CFGEdge *> &predSuccessors =
        (*predEdge)->getFrom()->getSuccessors();
    float numFactor = 1.5;
...
static bool isCandidateTheHottestSuccessor(TR::CFGEdge *candEdge,
  ...
  if (!comp->getFlowGraph()->getStructure()) // we cannot trust below checks if
                                             // someone invalidated structure
                                             // earlier in this opt
    return true;
...
bool TR_OrderBlocks::candidateIsBetterSuccessorThanBest(
  ...
  if (trace())
    traceMsg(comp(), "\tComparing candidate %d(%d) to current best %d(%d) as "
                     "successor for %d(%d)\n",
  ...
  if (!TR::Options::getCmdLineOptions()->getOption(
          TR_DisableInterpreterProfiling)) {
  ...
  if (candidate->hasSuccessor(currentBest)) {
    if (candidate->getFrequency() >
      ...
      if (trace())
        traceMsg(comp(), "\t\thas has current best succ as a succ, detecting "
                         "an if-then structure and making the if block my best "
                         "choice\n");
      return true;
    }
  }
...
TR_OrderBlocks::chooseBestFallThroughSuccessor(TR::CFG *cfg, TR::CFGNode *block,
  ...
  for (auto succEdge = successors.begin(); succEdge != successors.end();
    ...
    if (block->asBlock()->getExit() &&
      ...
      if (trace())
        traceMsg(comp(), "\t\tfound original fall-through successor %d: making "
                         "it initial best successor\n",
                 succBlock->getNumber());
    }
    ...
    if (!_changeBlockOrderBasedOnHWProfile) {
      ...
      if (candBlock->isSuperCold()) {
        if (!_hotPathList.isEmpty()) {
          if (trace())
            traceMsg(comp(), "\tcandidate %d is cold, discard because there "
                             "are still hot choices \n",
                     theCandidate->getNumber());
          return NULL;
        } else if (!_coldPathList.isEmpty() &&
                   !_coldPathList.getHeadData()->asBlock()->isSuperCold()) {
          if (trace())
            traceMsg(comp(), "\tcandidate %d is super cold, discard because "
                             "there are still choices which are not so cold \n",
                     theCandidate->getNumber());
          return NULL;
        }
      }
      ...
      if (candBlock->getFrequency() == 0 &&
        ...
        if (trace())
          traceMsg(comp(), "HW Profile: candidate %d has freq zero, discard "
                           "because there are still hot choices \n",
                   candBlock->getNumber());
        return NULL;
      }
    }
    ...
    // Make sure we still have structure info
    {
      ...
      while (innerLoop) {
        ...
        if (!outerLoop ||
          ...
          TR::list<TR::CFGEdge *> &successors =
              block->asBlock()->getSuccessors();
          ...
          if (exitEdges.getSize() > 1) {
            ...
            for (TR::CFGEdge *edge = exitIt.getFirst(); edge;
              ...
              if (edge->getTo()->asBlock()->getVisitCount() != _visitCount) {
                ...
                if (trace())
                  traceMsg(comp(), "\t -> block_%d\tfrequency %4d, current "
                                   "exit frequency %4d\n",
                ...
                if (edge->getFrequency() > exitFrequency) {
                  ...
                  if (trace())
                    traceMsg(comp(), "\t Loop %d has multiple exits, consider "
                                     "hotter exit %d\n",
  ...
  TR::list<TR::CFGEdge *> &predecessors =
      bestSuccessorEdge->getTo()->getPredecessors();
  for (auto predEdge = predecessors.begin(); predEdge != predecessors.end();
       ++predEdge) {
    ...
    if (pred != block) {
      ...
      if (predSuccEdge == successors.end()) {
        if (trace())
          traceMsg(comp(), "\t\tbut it has another predecessor %d for which it "
                           "is the hottest successor\n",
                   pred->getNumber());
        if (trace())
          traceMsg(comp(),
...
bool TR_OrderBlocks::peepHoleGotoToFollowing(TR::CFG *cfg, TR::Block *block,
  ...
  if (branchToFollowingBlock(gotoNode, block, comp())) {
    ...
    if (performTransformation(
            comp(), "%s dest of goto in block_%d is the following block_%d, "
                    "removing the goto node\n",
...
bool TR_OrderBlocks::peepHoleGotoToGoto(TR::CFG *cfg, TR::Block *block,
  ...
  if (destOfGoto->isGotoBlock(comp(), true)) {
    ...
    if (performTransformation(
            comp(), "%s in block_%d, dest of goto (%d) is also goto block, "
      ...
      if (parent && parent->isNaturalLoop() &&
        ...
        cfg->setStructure(0); // to avoid automatic structure repair creating an
                              // improper region due to temporary state after
...
bool TR_OrderBlocks::peepHoleGotoToEmpty(TR::CFG *cfg, TR::Block *block,
  ...
  if (destOfGoto->isEmptyBlock() &&
    ...
    if (destFallThroughDest != NULL) {
      ...
      if (performTransformation(
              comp(), "%s in block_%d, dest of goto is empty block, forwarding "
        ...
        if (parent && parent->isNaturalLoop() &&
            (parent->getNumber() == destOfGoto->getNumber()))
          cfg->setStructure(0); // to avoid automatic structure repair creating
                                // an improper region due to temporary state
...
bool TR_OrderBlocks::peepHoleBranchToLoopHeader(TR::CFG *cfg, TR::Block *block,
  ...
  if (destStructure && fallThroughStructure && blockStructure) {
    ...
    if (parent && parent->asRegion() && fallThroughIsSibling &&
        ...
        /* !dest->isCold() &&
*/
        performTransformation(comp(), "%s applied loop header peephole for "
                                      "block_%d fall through %d dest %d\n",
    ...
    if (!disableSPMD) {
      ...
      if (blockStructure) {
        if (blockStructure->isLoopInvariantBlock()) {
          if (trace())
            traceMsg(comp(), "\t\tavoid redirecting a jump to a pre-header "
                             "block %d for later SPMD optimization\n",
    ...
    if (!doNotRemove)
      ...
      if (takenBlock == blockAfterFallThrough &&
        ...
        if (fallThroughSuccessors.front()->getTo()->asBlock() !=
          ...
          if (performTransformation(
                  comp(), "%s in block_%d, branch taken dest (%d) is a block "
                          "after a single goto, so redirecting to its goto's "
                          "destination (%d)\n",
...
// know which order to try them in
...
void TR_OrderBlocks::peepHoleBranchBlock(TR::CFG *cfg, TR::Block *block,
  ...
  while (takenBlock->isGotoBlock(comp(), true)) {
    ...
    if ((!takenBlock->getStructureOf() ||
        ...
        performTransformation(
            comp(), "%s in block_%d, branch taken dest (%d) is a goto block, "
                    "so redirecting to its destination (%d)\n",
            title, block->getNumber(), takenBlock->getNumber(),
...
void TR_OrderBlocks::removeEmptyBlock(TR::CFG *cfg, TR::Block *block,
  ...
  if (performTransformation(comp(), "%s empty block_%d, redirecting edges "
                                    "around this block then removing it\n",
                            title, block->getNumber())) {
...
bool TR_OrderBlocks::doPeepHoleBlockCorrections(TR::Block *block, char *title) {
  ...
  if (block->getPredecessors().empty() &&
      ...
      block->getEntry() != NULL) {
    if (performTransformation(comp(), "%s block_%d has no predecessors so "
                                      "removing it and its out edges from the "
                                      "flow graph\n",
                              title, block->getNumber())) {
...
void TR_OrderBlocks::generateNewOrder(TR_BlockList &newBlockOrder) {
  ...
  while (block != NULL ||
    ...
    // right forest of paths
    // and set block to NULL so that we'll choose the best (possibly higher
    // frequency) path to follow
...
// other block
// so: insert a block following prevBLock that contains a goto node to
// "origSucc"
...
void TR_BlockOrderingOptimization::connectTreesAccordingToOrder(
  ...
  // right now, this code just assumes that it can safely rewrite the
  // trees...should probably put some assumes in here
  ...
  if (trace())
    traceMsg(comp(), "block =%d prevBlock->getEntry()->getNode()=%p "
  ...
  while (!newBlockOrder.isEmpty()) {
    ...
    if (fallThroughTT != block->getEntry()) {
      ...
      // there's no special work to do
      // also, if there is no flow edge from prevBlock to the original fall
      // through block, nothing to worry about
      if (prevBlock->getEntry() != NULL && fallThroughTT != NULL &&
        ...
        if (trace())
          traceMsg(comp(), "\t%d did not originally follow %d in trees, might "
                           "need to move trees\n",
        ...
        //     fall-through successor has already been placed elsewhere
        //     2) block is a different successor block than the original
        //     fall-through
        //        similar to 5, but we can reverse the branch in this case
        //     3) block is the only successor of prevBlock, but prevBlock ends
        //     in a goto (straightening should clean-up later)
        //     4) block is not a successor of prevBlock, but it begins the path
        //     we (arbitrarily?) chose to follow prevBlock
        //     5) block is a successor of prevBlock, but it's not the original
        //     fall through (due to endPathAtBlock)
        ...
        if (branchNode->getOpCodeValue() != TR::Goto &&
          ...
          if (!isSucc ||
              ...
              (branchNode->getOpCode().isJumpWithMultipleTargets() &&
               branchNode->getOpCode().isCall())) {
            if (trace())
              traceMsg(comp(), "\tneed to add extra goto block so that %d will "
                               "fall-through to %d\n",
        ...
        if (trace())
          traceMsg(comp(), "\t%d did not originally follow %d in trees, but no "
                           "trees manipulation needed\n",
                   block->getNumber(), prevBlock->getNumber());
      }
...
void checkOrderingConsistency(TR::Compilation *comp) {
  ...
  for (; block; prevBlock = block, block = block->getNextBlock()) {
    ...
    if (debugConsistencyCheck && prevFlowsToBlock &&
      ...
      if (rootStructure)
        fprintf(stderr, "Block %d(%d) doesn't look like the best successor "
                        "compared to %d(%d)\n",
                block->getNumber(), block->getFrequency(),
                prevsHotterSuccessor->getNumber(),
                prevsHotterSuccessor->getFrequency());
      else
        fprintf(stderr, "Block %d(%d,%d) doesn't look like the best successor "
                        "compared to %d(%d,%d)\n",
                block->getNumber(), block->getFrequency(),
                block->asBlock()->getNestingDepth(),
                prevsHotterSuccessor->getNumber(),
...
void TR_BlockShuffling::riffle(TR::Block **blocks) {
  ...
  if (!performTransformation(comp(), "O^O BLOCK SHUFFLING: Performing riffle "
                                     "shuffle, splitting at #%d/%d = "
                                     "block_%d\n",
...
void TR_BlockShuffling::reverse(TR::Block **blocks) {
  ...
  for (upper = 0, lower = _numBlocks - 1; upper < lower; upper++, lower--)
    if (performTransformation(comp(), "O^O BLOCK SHUFFLING:   swap [%3d] and "
                                      "[%3d] (block_%d and block_%d)\n",
