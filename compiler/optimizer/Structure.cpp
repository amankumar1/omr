...
static bool findCycle(TR_StructureSubGraphNode *node,
                      TR::SparseBitVector &regionNodes,
  ...
  for (auto edge = node->getSuccessors().begin();
    ...
    if (succ->getNumber() != entryNode &&
        regionNodes.ValueAt(succ->getUniqueSubGraphIndex()) &&
  ...
  for (auto edge = node->getExceptionSuccessors().begin();
    ...
    TR_ASSERT(
        ...
        "Expecting a CFG node which can be downcast to StructureSubGraphNode");
    TR_StructureSubGraphNode *succ = toStructureSubGraphNode((*edge)->getTo());
    if (/* succ->getNumber() != entryNode && */ regionNodes.ValueAt(
            succ->getUniqueSubGraphIndex()) &&
        findCycle(succ, regionNodes, nodesSeenOnPath, nodesCleared, entryNode))
      return true;
  }
...
void TR_RegionStructure::checkForInternalCycles() {
  ...
  setContainsInternalCycles(findCycle(getEntry(), _subNodes, nodesSeenOnPath,
...
TR_Structure *TR_RegionStructure::cloneStructure(
    ...
    List<TR_Structure> *whileLoops,
    List<TR_Structure> *correspondingWhileLoops) {
  TR_RegionStructure *clonedRegionStructure = new (trHeapMemory())
...
TR::CFGEdge *TR_RegionStructure::addExitEdge(TR_StructureSubGraphNode *from,
  ...
  TR::CFGNode *toNode = cursor ? cursor->getTo()
                               : new (trHeapMemory())
                                     TR_StructureSubGraphNode(to, trMemory());
...
TR_StructureSubGraphNode::create(int32_t num, TR_RegionStructure *region) {
  ...
  return new (region->trHeapMemory())
      TR_StructureSubGraphNode(num, region->trMemory());
}
...
void TR_BlockStructure::checkStructure(TR_BitVector *_blockNumbers) {
  TR_ASSERT(this->getNumber() == _block->getNumber(),
            "Number of BlockStructure is NOT the same as that of the block");
  TR_ASSERT(_blockNumbers->get(this->getNumber()) == 0,
...
void TR_RegionStructure::checkStructure(TR_BitVector *_blockNumbers) {
  TR_StructureSubGraphNode *node;
  TR_ASSERT(this->getNumber() == getEntry()->getStructure()->getNumber(),
            "Entry node does not have same number as this RegionStructure");
  TR_RegionStructure::Cursor si(*this);
  for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
    TR_ASSERT(this == node->getStructure()->getParent(),
    ...
    for (auto edge = node->getPredecessors().begin();
      ...
      TR_ASSERT(isConsistent, "predecessor of this subGraph node not not "
    ...
    for (auto edge = node->getExceptionPredecessors().begin();
      ...
      TR_ASSERT(isConsistent, "exception predecessor of this subGraph node not "
                              "not contain this node in its exception "
                              "successors");
    }
    ...
    for (auto edge = node->getSuccessors().begin();
      ...
      TR_ASSERT(isConsistent, "successor of this subGraph node not not contain "
                              "this node in its predecessors");
    }
    for (auto edge = node->getExceptionSuccessors().begin();
         edge != node->getExceptionSuccessors().end(); ++edge) {
      succ = toStructureSubGraphNode((*edge)->getTo());
      ...
      TR_ASSERT(isConsistent, "exception successor of this subGraph node not "
                              "not contain this node in its exception "
                              "predecessors");
    }
  ...
  for (edge = exitEdges.getFirst(); edge; edge = exitEdges.getNext()) {
    ...
    for (seenNode = exitNodesIt.getFirst(); seenNode;
      ...
      if ((seenNode->getNumber() == toNode->getNumber()) &&
          (seenNode != toNode))
        TR_ASSERT(0, "Exit edges to the same node number %d in region %p must "
                     "have a unique structure subgraph node (node1 %p node2 "
                     "%p)\n",
          ...
          while (bvi.hasMoreElements()) {
            ...
            if (!(optSetInfo[outerLoopInvariantBlock->getNumber()])
              ...
              if (outerLoop->isExpressionTransparentIn(nextOptimalComputation,
                ...
                dumpOptDetails(
                    comp(), "\nO^O PARTIAL REDUNDANCY ELIMINATION: Hoisting "
                            "loop invariant computation %d over nested loops "
