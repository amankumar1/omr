...
public:
  ...
  // This Optimization looks to convert locally declared arrays/structs/etc in
  // languages like C/C++ which are accessed using indirect loads/stores with
  // direct loads/stores
  // Ex:
  ...
  // the second part of this optimization will be an alias refinement which will
  // try to remove m and m[] from alias sets of temps if we are able to
  // eliminate all references to m and m[] in the trees.

  struct EPCandidate {
    EPCandidate(TR::Node *n, TR::TreeTop *tt, int32_t num)
        : node(n), rootTT(tt), valueNum(num) {}
    TR::Node *node;
    TR::TreeTop *rootTT;
    int32_t valueNum;
  };
...
private:
  ...
  TR::list<EPCandidate> _epCandidates; // A list of candidates that are in the
                                       // right form to be privatized.
  CS2::ABitVector<TR::Allocator> _epValueNumbers; // this bit vector is to keep
                                                  // track of value numbers that
                                                  // we know are 'safe' to
                                                  // transform.  Allowing us to
                                                  // catch move opportunities.
};
