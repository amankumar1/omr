...
static bool swapChildren(TR::Node *node, TR::Simplifier *s) {
  ...
  // dumpOptDetails into a performTransformation
  // because we require that a tree with a constant node have that constant as
  // the second child, unless both
...
TR::Node *_gotoSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (branchToFollowingBlock(node, block, s->comp())) {
    if (node->getNumChildren() > 0) {
      ...
      // has GlRegDeps(after GRA), can be removed only if BBExit has exaclty the
      // same GlRegDeps
 ...
 * operation and whose second child is a
 *     constant node such that no bits in the constant node could ever overlap
 * with the bits in the widened value.
 *
 *  \param simplifier
...
// (the base signedness does not matter)
// If the exp operation itself is unsigned (for example from pduexp) then the
// exponent value is interpreted as an unsigned number.
// This matters because otherwise (for example) an 8 bit exponent with the
// encoding 0xFF would be interpreted as base ** -1
// instead of base ** 255
bool isIntegralExponentInRange(TR::Node *parent, TR::Node *exponent,
  ...
  TR_ASSERT(exponent->getType().isIntegral(), "isIntegralExponentInRange only "
                                              "valid for integral exponents "
                                              "and not type %s\n",
...
TR::Node *reduceExpTwoAndGreaterToMultiplication(
  ...
  if (exponentValue <= 1) {
    TR_ASSERT(false, "reduceExpTwoAndGreaterToMultiplication only valid for "
                     "values >= 2 and not value=%d\n",
              exponentValue);
    return NULL;
  }
...
#endif
  // There are two algorithms here -- they are equivalent in the number of
  // multiply operations however the second is better
  // for platforms that have a destructive multiply instruction as less clobber
  // evaluates will be required.
  // The second has the advantage that more parallel multiply operations are
  // created
...
#endif
        dumpOptDetails(s->comp(), "%screated %s [" POINTER_PRINTF_FORMAT
                                  "] operation for exponentiation strength "
                                  "reduction (algorithmA/caseA)\n",
...
TR::Node *replaceExpWithMult(TR::Node *node, TR::Node *valueNode,
  ...
  // if base==0 then there are
  // fe/language specific rules on what the result/behaviour will be (e.g. Inf,
  // hardware expection).
  // The current expansion below generates the divide ignorant of any of these
  // rules so it cannot be enabled globally.
  ...
  if (exponentNode->getOpCode().isLoadConst() && kMaxPositiveExponent >= 0 &&
    ...
    if (isPowAndReasonableIntExponent &&
        performTransformation(
            s->comp(), "%sStrength reduce %s [" POINTER_PRINTF_FORMAT
...
#ifdef J9_PROJECT_SPECIFIC
bool propagateSignState(TR::Node *node, TR::Node *child, int32_t shiftAmount,
                        TR::Block *block, TR::Simplifier *s) {
  ...
  if (!node->hasKnownOrAssumedSignCode() &&
      ...
      performTransformation(
          s->comp(),
          "%sTransfer %sSignCode 0x%x from %s [" POINTER_PRINTF_FORMAT
          ...
          child->getOpCode().getName(), child, node->getOpCode().getName(),
          node)) {
  ...
  if (!node->hasKnownOrAssumedCleanSign() &&
      ...
      ((node->getDecimalPrecision() >=
        child->getDecimalPrecision() + shiftAmount) ||
       child->isNonNegative()) &&
      performTransformation(
          s->comp(),
...
bool propagateSignStateUnaryConversion(TR::Node *node, TR::Block *block,
                                       TR::Simplifier *s) {
  bool validateOp =
      node->getType().isBCD() &&
...
void convertStringToPacked(char *result, int32_t resultLen,
                           bool resultIsEvenPrecision, char *source,
                           int32_t sourceLen, uint32_t signCode) {
  TR_ASSERT(signCode >= TR::DataType::getFirstValidSignCode() &&
                signCode <= TR::DataType::getLastValidSignCode(),
            "invalid signCode 0x%x\n", signCode);
  TR_ASSERT(TR::DataType::isBCDSignChar(source[0]),
            "expecting a minus/plus/unsigned and not 0x%x\n", source[0]);

  memset(result, 0, resultLen);
  result[resultLen - 1] = (source[sourceLen - 1] << 4) | (uint8_t)signCode;

  int32_t firstDigitIndex = 1;
  for (int32_t i = resultLen - 2, j = sourceLen - 2;
       i >= 0 && j >= firstDigitIndex; i--, j--) {
...
void convertStringToZoned(char *result, int32_t resultLen, char *source,
                          int32_t sourceLen, uint32_t signCode,
                          bool signLeading) {
  TR_ASSERT(signCode >= TR::DataType::getFirstValidSignCode() &&
                signCode <= TR::DataType::getLastValidSignCode(),
  ...
  int32_t firstDigitIndex = 1;
  for (int32_t i = resultLen - 1, j = sourceLen - 1;
       i >= 0 && j >= firstDigitIndex; i--, j--)
...
void convertStringToZonedSeparate(char *result, int32_t resultLen, char *source,
                                  int32_t sourceLen, uint32_t signCode,
                                  bool signLeading) {
  TR_ASSERT(signCode == TR::DataType::getZonedSeparatePlus() ||
                signCode == TR::DataType::getZonedSeparateMinus(),
            "invalid signCode 0x%x\n", signCode);
  TR_ASSERT(TR::DataType::isBCDSignChar(source[0]),
            "expecting a minus/plus/unsigned and not 0x%x\n", source[0]);
  ...
  for (int32_t i = lastDigitIndex, j = sourceLen - 1;
       i >= firstDigitIndex && j >= 1; i--, j--)
    result[i] = (source[j] & 0xf) | TR::DataType::getZonedCode();
}

void convertStringToUnicode(char *result, int32_t resultLen, char *source,
                            int32_t sourceLen) {
  TR_ASSERT(TR::DataType::isBCDSignChar(source[0]),
            "expecting a minus/plus/unsigned and not 0x%x\n", source[0]);
  TR_ASSERT(isEven(resultLen),
            "Can't create a unicode constant with odd length\n");
  ...
  int32_t firstDigitIndex = 1;
  for (int32_t i = resultLen - 1, j = sourceLen - 1;
       i >= 0 && j >= firstDigitIndex; i -= 2, j--)
    result[i] = (source[j] & 0xf) | TR::DataType::getUnicodeZeroCode();
}

void convertStringToUnicodeSeparate(char *result, int32_t resultLen,
                                    char *source, int32_t sourceLen,
                                    uint32_t signCode, bool signLeading) {
  TR_ASSERT(signCode == TR::DataType::getUnicodePlusCode() ||
                signCode == TR::DataType::getUnicodeMinusCode(),
            "invalid signCode 0x%x\n", signCode);
  TR_ASSERT(TR::DataType::isBCDSignChar(source[0]),
            "expecting a minus/plus/unsigned and not 0x%x\n", source[0]);
  TR_ASSERT(isEven(resultLen),
  ...
  for (int32_t i = lastDigitIndex, j = sourceLen - 1;
       i >= firstDigitIndex && j >= 1; i -= 2, j--)
    result[i] = (source[j] & 0xf) | TR::DataType::getUnicodeZeroCode();
}

TR::Node *removeOperandWidening(TR::Node *node, TR::Node *parent,
                                TR::Block *block, TR::Simplifier *s) {
  if (s->comp()->getOption(TR_KeepBCDWidening)) // stress-testing option to
                                                // force more operations through
                                                // to the evaluators
    return node;

  // Many packed decimal node types (such as arithmetic/shift/stores) do not
  // need their operands explicitly widened so simple
  // widening operations (i.e. pdshl by 0 nodes) are be removed here.
  // This cannot be done globally because other node types (such a packed node
  // under a call) must be explicitly widened.
  if (node->isSimpleWidening()) {
    return s->replaceNodeWithChild(
        node, node->getFirstChild(), s->_curTree, block,
        false); // correctBCDPrecision=false because node is a widening
  } else if ((node->getOpCodeValue() == TR::i2pd ||
              node->getOpCodeValue() == TR::l2pd) &&
             node->hasSourcePrecision() &&
             node->getReferenceCount() == 1 && // the removal of widening may
                                               // not be valid in the other
                                               // contexts
             node->getDecimalPrecision() > node->getSourcePrecision() &&
             performTransformation(
                 s->comp(),
                 "%sReducing %s [" POINTER_PRINTF_FORMAT
                 "] precision %d to its child integer precision of %d\n",
                 s->optDetailString(), node->getOpCode().getName(), node,
                 node->getDecimalPrecision(), node->getSourcePrecision())) {
    ...
    int32_t adjust = node->getDecimalAdjust(); // adjust is < 0 for right shifts
                                               // and >= 0 for left shifts
    int32_t maxShiftedPrecision =
        adjust + node->getFirstChild()->getDecimalPrecision();
    if (node->getOpCode().isPackedRightShift() &&
        node->getDecimalRound() != 0) {
      maxShiftedPrecision++; // +1 as the rounding can propagate an extra digit
                             // across
    }
    if ((maxShiftedPrecision > 0) &&
        ...
        performTransformation(
            s->comp(),
            "%sReducing %s [" POINTER_PRINTF_FORMAT
            "] precision %d to the max shifted result precision of %d\n",
            s->optDetailString(), node->getOpCode().getName(), node,
            node->getDecimalPrecision(), maxShiftedPrecision)) {
      bool signWasKnownClean = node->hasKnownCleanSign();
      bool signWasAssumedClean = node->hasAssumedCleanSign();

      node->setDecimalPrecision(maxShiftedPrecision); // conservatively resets
                                                      // clean sign flags on an
                                                      // a trucation -- but this
                                                      // truncation cannot
                                                      // actually dirty a sign
  ...
  // TODO: figure out the more global check for removing widenings - all unary
  // operations (getNumChildren()==1), how about setSign and other shifts?
           ...
           performTransformation(
               s->comp(), "%sReducing %s [" POINTER_PRINTF_FORMAT
                          "] precision %d to its child precision of %d\n",
               s->optDetailString(), node->getOpCode().getName(), node,
               node->getDecimalPrecision(),
               node->getFirstChild()->getDecimalPrecision())) {
...
void removePaddingNode(TR::Node *node, TR::Simplifier *s) {
  ...
  if (paddingNode) {
    ...
    dumpOptDetails(s->comp(), "remove paddingNode %s (0x%p) from %s (0x%p) and "
                              "dec numChildren %d->%d\n",
...
void stopUsingSingleNode(TR::Node *node, bool removePadding,
                         TR::Simplifier *s) {
  TR_ASSERT(node->getReferenceCount() == 1, "stopUsingSingleNode only valid "
                                            "for nodes with a referenceCount "
                                            "of 1 and not %d\n",
