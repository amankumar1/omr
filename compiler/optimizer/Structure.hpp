...
public:
  ...
  TR_StructureSubGraphNode(TR_Structure *s)
      : TR::CFGNode(s->getNumber(), s->trMemory()), _structure(s), _index(0) {
...
public:
  ...
  bool isLoopInvariantBlock() {
    return _blockFlags.testAny(_isLoopInvariantBlock);
  }
  void setAsLoopInvariantBlock(bool b) {
    _blockFlags.set(_isLoopInvariantBlock, b);
  }

  bool isCreatedByVersioning() {
    return _blockFlags.testAny(_isCreatedByVersioning);
  }
  void setCreatedByVersioning(bool b) {
    _blockFlags.set(_isCreatedByVersioning, b);
  }

  bool isEntryOfShortRunningLoop() {
    return _blockFlags.testAny(_isEntryOfShortRunningLoop);
  }
  void setIsEntryOfShortRunningLoop() {
    _blockFlags.set(_isEntryOfShortRunningLoop, true);
  }

  bool wasHeaderOfCanonicalizedLoop() {
    return _blockFlags.testAny(_wasHeaderOfCanonicalizedLoop);
  }
  void setWasHeaderOfCanonicalizedLoop(bool b) {
    _blockFlags.set(_wasHeaderOfCanonicalizedLoop, b);
  }
...
public:
  TR_RegionStructure(TR::Compilation *c, int32_t index)
      : TR_Structure(c, index), _invariantSymbols(NULL),
        _blocksAtSameNestingLevel(NULL), _piv(NULL), _basicIVs(c->trMemory()),
        _exitEdges(c->trMemory()), _subNodes(c->allocator("Structures")),
...
public:
  ...
  public:
    Cursor(TR_RegionStructure &region)
        : _nodes(region._subNodes), _iter(_nodes),
          _cfg(region.comp()->getFlowGraph()) {
      _iter.SetToFirstOne();
    }

    TR_StructureSubGraphNode *getCurrent() {
      TR_StructureSubGraphNode *curr =
          (_iter.Valid()) ? _cfg->getStructureSubGraphNode(_iter) : NULL;
      return curr;
    }
