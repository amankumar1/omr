...
void TR_LoopUnroller::unrollLoopOnce(TR_RegionStructure *loop,
  ...
  for (subNode = nodeIt.getFirst(); subNode; subNode = nodeIt.getNext()) {
    ...
    TR_StructureSubGraphNode *clonedSubNode =
        new (trHeapMemory()) TR_StructureSubGraphNode(clonedSubStruct);
  ...
  if (isCountedLoop()) {
    ...
    // going
    // to happen, it is not valid to remove the branch for the last unroll (the
    // top half
    // of the loop maybe executed more number of times compared to the bottom
    // half)
    //
      ...
      } else if (succ == loop->getEntry()) // Type C
      {
        ...
        if (_iteration != 1) // for all iterations except the first iteration
        {
          if (shouldConnectToNextIteration(toStructureSubGraphNode(subNode),
            ...
            if (trace())
              traceMsg(comp(), "Redirecting cold edge from block_%d to "
                               "unrolled loop header rather than going to the "
                               "next iteration\n",
                       subNode->getNumber());
            addEdgeAndFixEverything(loop, *edge, GET_PREV_CLONE_NODE(subNode),
                                    loop->getEntry(), false, false, true,
...
void TR_LoopUnroller::modifyBranchTree(TR_RegionStructure *loop,
  ...
  if (pBlock->getEntry() == 0) {
    ...
    TR_BlockStructure *newBlockStr = new (trHeapMemory())
        TR_BlockStructure(comp(), newBlock->getNumber(), newBlock);
    TR_StructureSubGraphNode *newNode =
        new (trHeapMemory()) TR_StructureSubGraphNode(newBlockStr);
  ...
  if (originalLimitNode->getOpCode().isLoadConst() && !const_overflow) {
    ...
    //          TR::lsub  : TR::isub;
    //          tempValue = TR::Node::create(addOpCode, 2,originalLimitClone,
    //          constNode);
    ...
    TR::ILOpCodes addOpCode =
        getIndexType().isAddress()
    ...
    if (getIndexType().isAddress()) {
      newLimitNode =
          TR::Node::create(geta2xOpCode(getTestChildType()), 1, newLimitNode);
    ...
    if (lastTree->getNode()->getOpCode().isBranch()) {
      ...
      TR_BlockStructure *newBlockStructure = new (trHeapMemory())
          TR_BlockStructure(comp(), newBlock->getNumber(), newBlock);
      TR_StructureSubGraphNode *newSubNode =
          new (trHeapMemory()) TR_StructureSubGraphNode(newBlockStructure);
  ...
  if (1) /// moveEntryNode
  {
    ...
    if (getIndexType().isAddress()) {
      firstChild =
          TR::Node::create(geta2xOpCode(getTestChildType()), 1, firstChild);
...
void TR_LoopUnroller::modifyOriginalLoop(TR_RegionStructure *loop,
    ...
    if (!loop->isExitEdge(current)) {
      ...
      if (predNode->getNumber() < _numNodes) {
        if (shouldConnectToNextIteration(predNode, loop)) {
          ...
          if (trace())
            traceMsg(comp(), "Redirecting cold edge from block_%d to unrolled "
                             "loop header rather than going to the next "
                             "iteration\n",
                     predNode->getNumber());
  ...
  //       information
  //       _indVar->setIncr(new (trHeapMemory()) TR::VPIntConst(_loopStride * (1
  //       + _unrollCount)));
  //       }
  //    else
  //       {
  ...
  if (_spillLoopRequired) {
    ...
    if (1) {
      ...
      if (1) {
        ...
        newIfBlockStructure = new (trHeapMemory())
            TR_BlockStructure(comp(), newIfBlock->getNumber(), newIfBlock);
        newIfSubNode =
            new (trHeapMemory()) TR_StructureSubGraphNode(newIfBlockStructure);
        ...
        TR_StructureSubGraphNode *gotoSubNode = new (trHeapMemory())
            TR_StructureSubGraphNode(new (trHeapMemory()) TR_BlockStructure(
...
void TR_LoopUnroller::getLoopPreheaders(TR_RegionStructure *loop,
  ...
  for (TR::Block *
           pred = toBlock(preheader->getPredecessors().front()->getFrom());
          ...
          if ((firstChild->getOpCode().isMul() ||
            ...
            if (firstChild->getOpCode().isLoad()) {
              ...
              if (canMatch || canMatchAsIP) {
                if (trace())
                  traceMsg(comp(), "\tFound internal pointer %p with iv %d in "
                                   "offset node %p\n",
...
void TR_LoopUnroller::examineNode(TR::Node *node, intptrj_t visitCount) {
  ...
  if (symbol && symbol->isArrayShadowSymbol()) {
    ...
    if (trace())
      traceMsg(comp(), "\t\tFound array access node %p with sym ref %d and "
                       "internal pointer node %p\n",
...
void TR_LoopUnroller::examineArrayAccesses() {
  ...
  // To do so, we need to guarantee that the array accesses, for which we will
  // alow that,
  ...
  // if (a == b), the accesses are the same in any given iteration, and, thus,
  // always different
  ...
  // belong to entirely different arrays, and, thus, will never access to the
  // same location.
  //
  ListIterator<ListsOfArrayAccesses> it(&_listOfListsOfArrayAccesses);
  for (ListsOfArrayAccesses *loaa = it.getFirst(); loaa; loaa = it.getNext()) {
    ...
    { // and it doesn't have an internal pointer
      if (trace())
        traceMsg(comp(), "List of array accesses with sym ref %d does not "
                         "qualify for aliasing refinement\n",
                 loaa->symRefNum);
...
void TR_LoopUnroller::generateSpillLoop(TR_RegionStructure *loop,
  ...
  TR_StructureSubGraphNode *spillNode =
      new (trHeapMemory()) TR_StructureSubGraphNode(spillLoop);
...
sizeof(TR_StructureSubGraphNode *));
   ...
   cloneBlocksInRegion(loop);
   TR_RegionStructure *spillLoop = cloneStructure(loop)->asRegion();
   TR_StructureSubGraphNode *spillNode = new (trHeapMemory())
TR_StructureSubGraphNode(spillLoop);
   fixExitEdges(loop, spillLoop, branchNode);

   TR_StructureSubGraphNode *clonedBranchNode = GET_CLONE_NODE(branchNode);
   TR_BlockStructure *clonedBranchStructure =
clonedBranchNode->getStructure()->asBlock();
   ...
   TR_BlockStructure *newBranchStructure = new (trHeapMemory())
TR_BlockStructure(comp(), newBranchBlock->getNumber(),
                                                                 newBranchBlock);
   TR_StructureSubGraphNode *newBranchNode = new (trHeapMemory())
  ...
  for (TR::CFGEdge *edge = it.getFirst(); edge; edge = it.getNext()) {
    ...
    // FALLS INTO
      ...
      if (newFromNextBlock != newTo) // but the copy does not fall through
      {
        ...
        TR_BlockStructure *gotoStruct = new (trHeapMemory())
            TR_BlockStructure(comp(), gotoBlock->getNumber(), gotoBlock);
        TR_StructureSubGraphNode *gotoSubNode =
            new (trHeapMemory()) TR_StructureSubGraphNode(gotoStruct);
...
TR_LoopUnroller::cloneBlockStructure(TR_BlockStructure *blockStructure) {
  ...
  TR::Block *clonedBlock = GET_CLONE_BLOCK(block);
  TR_BlockStructure *clonedBlockStructure = new (trHeapMemory())
...
TR_LoopUnroller::cloneRegionStructure(TR_RegionStructure *region) {
  TR_RegionStructure *clonedRegion =
      new (trHeapMemory()) TR_RegionStructure(comp(), 0xDeadF00d); // for now
...
bool TR_LoopUnroller::unroll(TR::Compilation *comp, TR_RegionStructure *loop,
  ...
  if (1) {
    ...
    if (unroller._spillLoopRequired) {
      ...
      if (!backEdgeFound) {
        ...
        if (loopCanBeUnrolled) {
          ...
          // blocks in the loop are
          // the entry and the branch block
          // Further the only code other from the entry block in the loop must
          // be the branch (and an optional async check)
          //
        } else {
          dumpOptDetails(comp, "Cannot unroll loop %d: exit condition is not "
                               "in a block containing a backedge\n",
                         loop->getNumber());
          return false;
        }
      }
    ...
    if (1) {
      ...
      if (!foundExitEdge) {
        dumpOptDetails(comp, "Cannot unroll loop %d: eq loop with branch not "
                             "an exit out of the loop\n",
                       loop->getNumber());
        return false;
      }
    }
    ...
    if (TR::ILOpCode::isEqualCmp(op) || TR::ILOpCode::isNotEqualCmp(op)) {
      if (piv->getDeltaOnExitEdge() != 1 && piv->getDeltaOnExitEdge() != -1) {
        dumpOptDetails(comp, "Cannot unroll loop %d: eq loop with non unit "
  ...
  if (unrollKind == CompleteUnroll) {
    ...
    TR_Structure *destStruct = destBlock->getStructureOf();
    if (!loop->contains(destStruct, loop->getParent())) {
      dumpOptDetails(comp, "Cannot unroll loop %d: complete unroll of a loop "
                           "with a reversed branch\n",
                     loop->getNumber());
      return false;
    }
  }
  ...
  if (unroller.isInternalPointerLimitExceeded()) {
    dumpOptDetails(comp, "Cannot unroll loop %d: number of internal pointers "
                         "has been exceeded\n",
                   loop->getNumber());
    return false;
  }
  ...
  if (!performTransformation(
          comp, "%sUnrolling counted loop %d [unrollfactor:%d, peelcount:%d, "
                "spill:%s completeunroll:%s]\n",
  ...
  if (unroller._spillLoopRequired) {
    ...
    if (!backEdgeFound) {
      ...
      if (loopCanBeUnrolled) {
        ...
        // in the loop are
        // the entry and the branch block
        // Further the only code other from the entry block in the loop must be
        // the branch (and an optional async check)
        //
      } else {
        dumpOptDetails(comp, "Cannot unroll loop %d: exit condition is not in "
                             "a block containing a backedge\n",
                       loop->getNumber());
        return false;
      }
    }
...
void TR_GeneralLoopUnroller::profileNonCountedLoops(
  ...
  for (TR_RegionStructure *loop = it.getFirst(); loop; loop = it.getNext()) {
    ...
    if (!performTransformation(comp(), "%sInserting artificial counter in loop "
                                       "%d in order to profile the itercount\n",
...
int32_t TR_GeneralLoopUnroller::weighNaturalLoop(
  ...
  dumpOptDetails(comp(), "\tnumNodes = %d, numBlocks = %d, entryBlockFreq = "
                         "%d, numSubscripts = %d\n",
      ...
      for (auto e = entryNode->getPredecessors().begin();
        ...
        if (source->getStructure()->asBlock()) {
          ...
          if (!branchNode->getOpCode().isGoto() &&
            ...
            dumpOptDetails(comp(), "backedge branch [%p] is controlled using a "
                                   "biv, so unrolling as non-counted\n",
                           branchNode);
            loop->setPrimaryInductionVariable(NULL);
            break;
          }
  ...
  // loop is
  // relatively hotter than the outer loop.  Otherwise there is not sufficient
  // benefit
  // in unrolling the inner loop.  When profiling information is not available,
  // we
  // statically assume that the inner loops run more frequently.  High relative
  // freq
  ...
  if (piv) {
    ...
    if (iterCount <= 0 || unrollKind == TR_LoopUnroller::NoUnroll) {
      if (iterCount <= 0)
        dumpOptDetails(comp(), "\twe have a primary induction variable with "
                               "unknown itercount stride=%d\n",
    ...
    if ((unrollKind == TR_LoopUnroller::ExactUnroll) &&
      ...
      boostFactor = 400;
      unrollKind = TR_LoopUnroller::CompleteUnroll;
      dumpOptDetails(comp(), "\tloop unroll count %d iterCount %d, making "
                             "complete unroll out of exact unroll\n",
...
bool TR_GeneralLoopUnroller::canUnrollUnCountedLoop(
  ...
  //   <flags:"0x4" (X!=0 )/>
  //   [0x375191d4]   NULLCHK on [0x37047ef8] #18[0x36a24b1c]
  //   Method[jitThrowNullPointerException]
        ...
        if (node->getOpCodeValue() == TR::NULLCHK &&
          ...
          for (TR::TreeTop *tt1 = tt; tt1 != exit;
            ...
            if (n->getOpCode().isBooleanCompare() &&
              ...
              if (trace()) {
                traceMsg(comp(), "\tLoop %d can be unrolled because of common "
                                 "NULLCHK and compare to NIL\n",
                         loop->getNumber());
              }
