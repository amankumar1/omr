...
bool TR_BackwardDFSetAnalysis<Container *>::canGenAndKillForStructure(
  ...
  if (naturalLoop && naturalLoop->isNaturalLoop()) {
    ...
    predList.insert(predList.end(),
                    entryNode->getExceptionPredecessors().begin(),
                    entryNode->getExceptionPredecessors().end());
    for (auto pred = predList.begin(); pred != predList.end(); ++pred) {
      ...
      succList.insert(succList.end(),
                      predNode->getExceptionSuccessors().begin(),
                      predNode->getExceptionSuccessors().end());
    ...
    for (TR::CFGEdge *edge = ei.getCurrent(); edge != NULL;
      ...
      predList.insert(predList.end(),
                      toNode->getExceptionPredecessors().begin(),
...
void TR_BackwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfo(
    TR_RegionStructure *regionStructure, TR::BitVector &pendingList,
    TR::BitVector &exitNodes, bool lastIteration) {
  while (this->_analysisQueue.getListHead() &&
    ...
    TR::BitVector seenNodes(this->comp()->allocator());
    {
      ...
      succList.insert(succList.end(),
                      nodeStructure->getExceptionSuccessors().begin(),
                      nodeStructure->getExceptionSuccessors().end());
      bool firstSucc = true;
      ...
      for (auto succ = succList.begin(); succ != succList.end(); ++succ) {
        if (!regionStructure->isExitEdge(*succ)) {
          ...
          if ((pendingList.ValueAt(succStructure->getNumber()) &&
               (!alreadyVisitedNode)) /* &&
    ...
    if (traceBBVA()) {
      ...
      if (analysisInfo->_regularGenSetInfo) {
        ...
        for (pair = analysisInfo->_regularGenSetInfo->getFirst(); pair;
             pair = pair->getNext()) {
          if (pair->_container) {
            traceMsg(this->comp(), "Exit or Succ numbered %d : ",
                     pair->_nodeNumber);
      ...
      if (analysisInfo->_exceptionGenSetInfo) {
        traceMsg(this->comp(), "\nException Gen Set Info for Region or Block : "
                               "%p numbered %d is : \n",
                 nodeStructure->getStructure(),
                 nodeStructure->getStructure()->getNumber());
      ...
      if (analysisInfo->_exceptionKillSetInfo) {
        traceMsg(this->comp(), "\nException Kill Set Info for Region or Block "
                               ": %p numbered %d is : \n",
                 nodeStructure->getStructure(),
                 nodeStructure->getStructure()->getNumber());
    ...
    if (traceBBVA()) {
      ...
      if (analysisInfo->_currentRegularGenSetInfo) {
        traceMsg(this->comp(), "\nCurrent Gen Set Info for Region or Block : "
                               "%p numbered %d is : \n",
                 nodeStructure->getStructure(),
                 nodeStructure->getStructure()->getNumber());
      ...
      if (analysisInfo->_currentRegularKillSetInfo) {
        traceMsg(this->comp(), "\nCurrent Kill Set Info for Region or Block : "
                               "%p numbered %d is : \n",
                 nodeStructure->getStructure(),
                 nodeStructure->getStructure()->getNumber());
    ...
    predList.insert(predList.end(),
                    nodeStructure->getExceptionPredecessors().begin(),
                    nodeStructure->getExceptionPredecessors().end());
    for (auto pred = predList.begin(); pred != predList.end(); ++pred) {
      TR_StructureSubGraphNode *predNode =
          (TR_StructureSubGraphNode *)(*pred)->getFrom();
...
void TR_BackwardDFSetAnalysis<Container *>::
  ...
  // TR_BitVector *originalPendingList = new (this->trStackMemory())
  // TR_BitVector(this->_numberOfNodes, this->trMemory(), stackAlloc);
...
bool TR_BackwardDFSetAnalysis<Container *>::analyzeNodeIfSuccessorsAnalyzed(
  ...
  while (this->_analysisQueue.getListHead() &&
    ...
    ///////initializeOutSetInfo();
    {
      ...
      bool stopAnalyzingThisNode = false;
      for (auto succ = succList.begin(); succ != succList.end(); ++succ) {
        // dumpOptDetails(this->comp(), "1Succ %d %p pending %d already visited
        // %d\n", succ->getTo()->getNumber(), succ->getTo(),
        // pendingList->get(succ->getTo()->getNumber()), alreadyVisitedNode);
        if (!regionStructure->isExitEdge(*succ)) {
          TR_StructureSubGraphNode *succNode =
              (TR_StructureSubGraphNode *)(*succ)->getTo();
          TR_Structure *succStructure = succNode->getStructure();
          // dumpOptDetails(this->comp(), "2Succ %d %p pending %d already
          // visited %d\n", succStructure->getNumber(), succStructure,
          // pendingList->get(succStructure->getNumber()), alreadyVisitedNode);
          if (pendingList.ValueAt(succStructure->getNumber()) &&
              (!alreadyVisitedNode)) {
            this->removeHeadFromAnalysisQueue();
            this->addToAnalysisQueue(succNode, 0);
    ...
    // if (traceBVA())
    //  dumpOptDetails(this->comp(), "Region structure %d (%p) status %d node
    //  structure %d (%p) status %d\n", regionStructure->getNumber(),
    //  regionStructure, regionAnalysisInfo->_isInvalid,
    //  nodeStructure->getNumber(), nodeStructure->getStructure(),
    //  analysisInfo->_isInvalid);
    ...
    if (this->supportsGenAndKillSets() &&
      ...
      if (1 /* || isBlockStructure */) {
        ...
        for (auto succ = succList.begin(); succ != succList.end(); ++succ) {
          bool normalSucc = (++count <= nodeStructure->getSuccessors().size());
          int32_t nodeNumber = isBlockStructure ? node->getNumber()
        ...
        for (nodePair = analysisInfo->_outSetInfo->getFirst(); nodePair;
             nodePair = nodePair->getNext()) {
          int32_t nodeNumber =
              isBlockStructure ? node->getNumber() : nodePair->_nodeNumber;
          Container *killBitVector = analysisInfo->getContainer(
              analysisInfo->_regularKillSetInfo, nodeNumber);
          if (!killBitVector)
            killBitVector = analysisInfo->getContainer(
                analysisInfo->_exceptionKillSetInfo, nodeNumber);

          Container *genBitVector = analysisInfo->getContainer(
              analysisInfo->_regularGenSetInfo, nodeNumber);
          if (!genBitVector)
            genBitVector = analysisInfo->getContainer(
                analysisInfo->_exceptionGenSetInfo, nodeNumber);
