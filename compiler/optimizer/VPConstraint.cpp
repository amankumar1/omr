...
TR::VPClassType *
TR::VPUnresolvedClass::getArrayClass(TR::ValuePropagation *vp) {
  char *arraySig = (char *)vp->trMemory()->allocateStackMemory(_len + 2);
  arraySig[0] = '[';
  arraySig[_len + 1] = 0;
  memcpy(arraySig + 1, _sig, _len);
...
TR::VPRelation *
TR::VPGreaterThanOrEqual::getComplement(TR::ValuePropagation *vp) {
...
TR::VPConstraint *TR::VPConstraint::create(TR::ValuePropagation *vp,
...
TR::VPShortConst *TR::VPShortConst::create(TR::ValuePropagation *vp,
                                           int16_t v) {
...
TR::VPConstraint *TR::VPShortConst::createExclusion(TR::ValuePropagation *vp,
                                                    int16_t v) {
...
TR::VPConstraint *TR::VPIntConst::createExclusion(TR::ValuePropagation *vp,
                                                  int32_t v) {
...
TR::VPShortRange::createWithPrecision(TR::ValuePropagation *vp,
...
TR::VPIntConstraint *TR::VPIntRange::create(TR::ValuePropagation *vp,
...
TR::VPIntConstraint *
TR::VPIntRange::createWithPrecision(TR::ValuePropagation *vp, TR::DataType dt,
...
TR::VPConstraint *TR::VPLongConst::createExclusion(TR::ValuePropagation *vp,
                                                   int64_t v) {
...
TR::VPLongConstraint *TR::VPLongRange::create(TR::ValuePropagation *vp,
...
TR::VPConstraint *TR::VPClass::create(TR::ValuePropagation *vp,
                                      TR::VPClassType *type,
  ...
  if (location != NULL && location->isJavaLangClassObject() == TR_yes &&
...
TR::VPClassType *TR::VPClassType::create(TR::ValuePropagation *vp,
  ...
  if (classObject) {
    ...
    TR_PersistentClassInfo *classInfo =
        vp->comp()
            ...
            ->findClassInfoAfterLocking(classObject, vp->comp());
...
TR::VPResolvedClass *TR::VPResolvedClass::create(TR::ValuePropagation *vp,
...
TR::VPFixedClass *TR::VPFixedClass::create(TR::ValuePropagation *vp,
                                           TR_OpaqueClassBlock *klass) {
  // If the constraint does not already exist, create it
  //
  int32_t hash = (int32_t)((((uintptrj_t)klass) << 2) % VP_HASH_TABLE_SIZE);
...
TR::VPKnownObject *TR::VPKnownObject::create(TR::ValuePropagation *vp,
...
TR::VPConstString *TR::VPConstString::create(TR::ValuePropagation *vp,
                                             TR::SymbolReference *symRef) {
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (vpConstStringCriticalSection.hasVMAccess()) {
    void *string = *(
        (void **)symRef->getSymbol()->castToStaticSymbol()->getStaticAddress());
    ...
    for (int32_t i = 0; i < len && i < TR_MAX_CHARS_FOR_HASH; i++)
      hashValue += TR::Compiler->cls.getStringCharacter(vp->comp(),
                                                        (uintptrj_t)string, i);
    ...
    for (entry = vp->_constraintsHashTable[hash]; entry; entry = entry->next) {
      ...
      if (constraint &&
          string ==
              *((void **)constraint->_symRef->getSymbol()
                    ->castToStaticSymbol()
                    ->getStaticAddress())) {
        return constraint;
      }
...
uint16_t TR::VPConstString::charAt(int32_t i, TR::Compilation *comp) {
  ...
  if (charAtCriticalSection.hasVMAccess()) {
    uintptrj_t string = *(uintptrj_t *)_symRef->getSymbol()
                             ->castToStaticSymbol()
                             ->getStaticAddress();
...
TR::VPUnresolvedClass::create(TR::ValuePropagation *vp, const char *sig,
...
TR::VPPreexistentObject::create(TR::ValuePropagation *vp,
...
TR::VPArrayInfo *TR::VPArrayInfo::create(TR::ValuePropagation *vp,
...
TR::VPMergedConstraints::create(TR::ValuePropagation *vp,
...
TR::VPObjectLocation *TR::VPObjectLocation::create(TR::ValuePropagation *vp,
...
TR::VPLessThanOrEqual *TR::VPLessThanOrEqual::create(TR::ValuePropagation *vp,
                                                     int32_t incr) {
  // If the constraint does not already exist, create it
  //
  int32_t hash =
...
TR::VPGreaterThanOrEqual::create(TR::ValuePropagation *vp, int32_t incr) {
  // If the constraint does not already exist, create it
  //
  ...
  if (otherInt) {
    ...
    if (otherLong) {
      ...
      if ((int64_t)otherLong->getLow() <= (int64_t)(getHigh() + 1)) {
        ...
        if ((int64_t)otherLong->getLow() < (int64_t)getLow())
          return TR::VPMergedConstraints::create(
              vp, TR::VPIntRange::create(vp, (int32_t)otherLong->getLow(),
                                         (int32_t)otherLong->getHigh()),
              this);
        else
          return TR::VPMergedConstraints::create(
              vp, this, TR::VPIntRange::create(vp, (int32_t)otherLong->getLow(),
                                               (int32_t)otherLong->getHigh()));
      }
    }
  }
  ...
  if (otherLong) {
    ...
    if (otherInt) {
      ...
      if ((int64_t)otherInt->getLow() <= (int64_t)(getHigh() + 1)) {
        ...
        if ((int64_t)otherInt->getLow() < (int64_t)getLow())
          return TR::VPMergedConstraints::create(
              vp, TR::VPLongRange::create(vp, otherInt->getLow(),
                                          otherInt->getHigh()),
              this);
        else
          return TR::VPMergedConstraints::create(
              vp, this, TR::VPLongRange::create(vp, otherInt->getLow(),
                                                otherInt->getHigh()));
      }
    }
  }
...
TR::VPConstraint *TR::VPConstraint::intersect(TR::VPConstraint *other,
  ...
  if (!this || !other) {
    if (vp->trace())
      traceMsg(vp->comp(), "setIntersectionFailed to true because NULL "
                           "constraint found this = 0x%p, other = 0x%p\n",
               this, other);
...
// TR::ValuePropagation *vp, bool Unsigned)
//    {
//    TR::VPIntConstraint *otherInt = other->asIntConstraint();
//     if (otherInt)
//       {
...
// *otherCur, ListElement<TR::VPConstraint> *otherNext, TR::ValuePropagation
// *vp, bool isUnsigned)
//    {
//    TR_ScratchList<TR::VPConstraint>         result(vp->trMemory());
//    ListElement<TR::VPConstraint> *next = _constraints.getListHead();
...
//             otherHigh;
//             TR::VPConstraint *constraint = TR::VPIntRange::create(vp,
//             resultLow, resultHigh, true);
//             ///if (constraint)
//             ///   constraint->setIsUnsigned(true);
...
// *otherInt, TR::ValuePropagation *vp, bool Unsigned)
//    {
...
//       range = TR::VPIntRange::create(vp, TR::getMinUnsigned<TR::Int32>(),
//       low, true);
//       if (!range ||
//           !wrapRange)
//          return NULL;
...
TR::VPConstraint *TR::VPShortConstraint::getRange(int16_t low, int16_t high,
  ...
  // of the canOverflow bit, for either an addition or subtraction
  // operation. The function takes the results of the lower- and upper-bound
  // calculations for the operation, both performed in the same precision
  // arithmetic as that of the underlying operation, along with two flags
  // indicated whether or not the operations producing these two bounds
  // overflowed. From these values, a constraint can be created, with the
  // overflow flags being set as conservatively as possible.
  //
...
TR::VPConstraint *TR::VPRelation::propagateAbsoluteConstraint(
    TR::VPConstraint *constraint, int32_t relative, TR::ValuePropagation *vp) {
...
TR::VPConstraint::Tracer::Tracer(TR::ValuePropagation *vpArg,
