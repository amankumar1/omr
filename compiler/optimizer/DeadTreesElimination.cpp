...
// TR::SparseBitVector &seenNodes, bool &highGlobalIndex, TR::Compilation *comp,
// vcount_t oldCompVisitCount)
static bool fixUpTree(TR::Node *node, TR::TreeTop *treeTop,
                      TR::SparseBitVector &seenNodes, bool &highGlobalIndex,
                      TR::Optimization *opt) {
  ...
  /*if (node->getVisitCount() <= oldCompVisitCount)
    {
    // This node was from a treetop that was encountered before the current
    treetop
    // in which case it should already be anchored and so would not need to be
    anchored afresh
    //
  ...
  if ((node->getReferenceCount() > 1) &&
      ...
      !node->getOpCode().isLoadConst() && anchorLoadaddr && anchorArrayCmp) {
...
bool collectSymbolReferencesInNode(
  ...
  // (because an update was moved past
...
// decremented here without
// using visit counts so they cannot be trusted for any functional purpose but
// only to guide heuristics.
    ...
    if (visitCount == child->getVisitCount()) {
      ...
      if (callWithNoSideEffects) {
        ...
        if (child->getReferenceCount() == 1) {
          ...
          if (child->getOpCode().isPackedExponentiation()) {
            // pdexp has a possible message side effect in truncating or no
          ...
          safeToReplaceNode = isSafeToReplaceNode(
              ...
              comp(), &_targetTrees, _cannotBeEliminated);
      ...
      if (treeTopCanBeEliminated) {
        ...
        for (int32_t i = 0; i < child->getNumChildren(); ++i) {
          ...
          // temporarily revert this fix
          // if (fixUpTree(child->getChild(i), iter.currentTree(), seenNodes,
          // highGlobalIndex, comp(), compVisitCount))
          if (fixUpTree(child->getChild(i), iter.currentTree(), seenNodes,
                        highGlobalIndex, self()))
            containsFloatingPoint = true;
          if (highGlobalIndex) {
    ...
    if (treeTopCanBeEliminated) {
      ...
      if (!node->getOpCode().isStoreReg() ||
        ...
        if (performTransformation(
                comp(), "%sRemove tree : [" POINTER_PRINTF_FORMAT
        ...
        if (performTransformation(
                comp(), "%sMove tree : [" POINTER_PRINTF_FORMAT
