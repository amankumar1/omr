...
public:
  ...
  TR::Instruction *
  findJumpInstructionsInBlock(int32_t blockNum,
                              TR::list<TR::Instruction *> *jmpInstrs);
  void findJumpInstructionsInCodeRegion(
      TR::Instruction *firstInstr, TR::Instruction *endInstr,
      CS2::HashTable<TR::Instruction *, bool> &setOfJumps);
  void
  computeSaveRestoreSets(TR_RegisterAnticipatability &registerAnticipatability,
                         TR_RegisterAvailability &registerAvailability);
  void doPlacement(TR_RegisterAnticipatability &registerAnticipatability,
                   TR_RegisterAvailability &registerAvailability);
  ...
  bool findMultiples(TR_BitVector *regs, TR::Instruction *startLocation,
                     bool doSaves, bool forwardWalk = true);
  ...
  struct SWBlockInfo {
    ...
    SWBlockInfo()
        : _block(NULL), _startInstr(NULL), _endInstr(NULL), _savedRegs(NULL),
          _restoredRegs(NULL), _restoreLocation(NULL) {}
    TR::Block *_block;
