...
// using CPIndex and one  (i.e. with MetaData)
 ...
 * BCD and some aggregates)
 * The allocateFor checks are for tuning for scalability when running in limited
 * GRA mode
 */
...
int32_t TR_GlobalRegisterAllocator::perform() {
  ...
  candidates->_candidateForSymRefs =
      (TR_RegisterCandidate **)trMemory()->allocateStackMemory(
          CANDIDATE_FOR_SYMREF_SIZE * sizeof(TR_RegisterCandidate *));
  memset(candidates->_candidateForSymRefs, 0,
         CANDIDATE_FOR_SYMREF_SIZE * sizeof(TR_RegisterCandidates *));
  ...
  candidates->_startOfExtendedBBForBB.init(
      trMemory(), (uint32_t)(comp()->getFlowGraph()->getNextNodeNumber() *
                             sizeof(TR::Block *) * 1.5),
      false, stackAlloc);
  ...
  if (cg()->getSupportsGlRegDeps() && !debug("disableGRA") &&
    ...
    if (!cg()->getLiveLocals()) {
      ...
      if (comp()->allocateAtThisOptLevel()) {
        for (auto mds = methodMetaDataSymbols.getFirst(); mds != NULL;
             mds = methodMetaDataSymbols.getNext()) {
    ...
    _registerCandidates =
        (TR_RegisterCandidate **)trMemory()->allocateStackMemory(
            _origSymRefCount * sizeof(TR_RegisterCandidate *));
    ...
    if (canAffordAssignment) {
      ...
      if (!splitSymRefs.isEmpty()) {
        ...
        // splitting.
        // If source and destination locals of a copy receive registers, in
        // other words, a register-register copy exists,
        // and a value is modified in one of locals, another local must be
        // indentified as 'valueModified'.
        //
        if (trace)
        ...
        // (copies)
        // where both of operands (source and destination) do not receive any
        // register.
        //
        ...
        // not modified in the live range.
        // If a value is not modified in a local which receives a register, the
        // stores from the register is redundant.
        //
        if (trace)
...
void TR_GlobalRegisterAllocator::transformNode(
  ...
  if (node->getOpCode().isJumpWithMultipleTargets() &&
    ...
    if (changeNode) {
      ...
      if (!value) {
...

        {
          ...
          dumpOptDetails(comp(), "%s did not change load var [%p] of symRef#%d "
                                 "to load reg because float/double issues\n",
                         OPT_DETAILS, node,
                         node->getSymbolReference()->getReferenceNumber());
          return;
        }
      ...
      if (comp()->getOption(TR_CheckGRA) &&
        ...
        TR::TreeTop::create(
            ...
            TR::Node::createWithSymRef(
                TR::BNDCHK, 2, 2,
                TR::Node::iconst(oldValue, 1), // pretend we have an array of
                                               // size=1 so only zeroIfEqual==0
                                               // is acceptable
                zeroIfEqual,
    ...
    if (symbol->isInGlobalRegister() &&
      ...
      // created
      // by other optimization passes (like escape analysis), then for non FP
      // stores
      // (FP stores are handled correct as explained above because we will
      // insert store/reload
      // pairs wherever required based on GlobalFPStoreReloadOpt), we must keep
      // the store intact.
      // Another case when we must keep the store to the stack location intact
      // is if
      // it is a pinning array for an internal pointer, as the pinning array for
      // an
      ...
      if (!fpStore && !symRef->storeCanBeRemoved() ||
        ...
        if (_osrCatchSucc || symRef->getSymbol()->dontEliminateStores(comp()) ||
          ...
          // in the temp so that the value can be reloaded in code reachable
          // from
          // the catch block. Note that we ensure the candidate is not live on
          // entry
          // in the ctach block. This ensures that it will be re-loaded into the
          // global
 ...
 * register
 * is successors of the passed block should we be not able to reload it in the
 * current block
...
bool TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors(
  ...
  if (traceIt)
    traceMsg(comp(), "TR_GlobalRegisterAllocator::"
                     "markCandidateForReloadInSuccessors block=%d "
                     "GlobalReg=(%d,symRef=#%d)\n",
             block->getNumber(), i,
             rc->getSymbolReference()->getReferenceNumber());
  ...
  if (traceIt)
    traceMsg(comp(), "TR_GlobalRegisterAllocator::"
                     "markCandidateForReloadInSuccessors checking "
                     "extensions\n");
  ...
  if (nextBlock && nextBlock->isExtensionOfPreviousBlock()) {
    if (traceIt)
      traceMsg(comp(), "TR_GlobalRegisterAllocator::"
                       "markCandidateForReloadInSuccessors nextBlock=%d\n",
               nextBlock->getNumber());
    ...
    if (traceIt)
      traceMsg(comp(), "TR_GlobalRegisterAllocator::"
                       "markCandidateForReloadInSuccessors next block is not "
                       "extension\n");
...
void TR_GlobalRegisterAllocator::addStoresForCatchBlockLoads(
  ...
  // For OSR, however, we are particularly interested in eliminating those
  // stores,
  ...
  for (auto nextEdge = throwingBlock->getExceptionSuccessors().begin();
    ...
    if (succ->asBlock()->isOSRCatchBlock()) {
      ...
      if (trace())
        traceMsg(comp(), "           addStoresForCatchBlockLoads([%p], "
                         "block_%d) found OSR catch block_%d\n",
...
void TR_GlobalRegisterAllocator::addGlRegDepToExit(
  ...
  if (numLoads) {
    ...
    dumpOptDetails(comp(), "%s create TR::GlRegDeps [" POINTER_PRINTF_FORMAT
...
bool TR_GlobalRegisterAllocator::registerIsLiveAcrossEdge(
  ...
  // exit.
  // If there are more than one predecessor and their frequency is higher than
  // that of this block,
  ...
  if (!predecessors.empty() && !(predecessors.size() == 1) &&
    ...
    if (liveOnAllPredExits && allPredFreq >= successorBlock->getFrequency()) {
      if (trace())
        traceMsg(comp(), "Extended live range of #%d into successor since "
                         "candidate is available in register on all "
                         "predecessor's exits: block=%d succ=%d allPredFreq=%d "
                         "succFreq=%d\n",
                 rc->getSymbolReference()->getReferenceNumber(),
  ...
  // some effort we're
  // going to slow down the fall-thru path by inserting a store from the
  // register back into the auto
  // before the branch and/or reloading the float value (on ia32) at the next
  // use in the fall thru.
  //
  // If the target of the branch has only one predecessor then we can prepend a
  // block to the branch
  // target, add the store there and change the branch target to be the new
  // block.
  //
  // If the target of the branch has multiple predecessors and the fallthru path
  // in known to be
  // hotter than the branch target then we can append a new block, change the
  // branch to target
  // the new block, end the live range in the new block, and add a goto to the
  // original block.
  //
  ...
  if (!keepingItAliveAcrossEdgeRequiresExtraGoto) {
    ...
    // edge then
    // it should make sense to keep it alive across the edge unless doing so
    // introduces an
  ...
  if (branchIsHotter == TR_no) {
    ...
    if (!newBlock->getEntry()->getPrevTreeTop()) {
      ...
      // end of the method
      // is far from ideal.  We should try to find the first break in the fall
      // through chain from
      // 'block' onwards, and place it there instead so that the ichace is not
      // contaminated
      //
...
TR::Node *TR_GlobalRegister::createLoadFromRegister(TR::Node *n,
  ...
  setValue(load);
  if (comp->nodeNeeds2Regs(load))
    dumpOptDetails(comp, "%s create load [%p] from Register %d (low word) and "
...
TR::Node *TR_GlobalRegister::createStoreToRegister(
  ...
  if (comp->nodeNeeds2Regs(store))
    dumpOptDetails(
        comp, "%s create store [%p] of symRef#%d to Register %d (low word) and "
              "Register %d (high word)\n",
...
TR::Node *TR_GlobalRegister::createStoreFromRegister(
  ...
  if (i != -1) {
    if (comp->nodeNeeds2Regs(store))
      dumpOptDetails(comp, "%s create store [%p] from Register %d (low word) "
                           "and Register %d (high word)\n",
...
void TR_GlobalRegisterAllocator::findIfThenRegisterCandidates() {
  ...
  if (!debug("oldIfThen")) {
    ...
    // autoAndParmLiveLocalIndex.
    // For each intersected bit ensure BlockInfo exists for the candidate and
    // initialize it
  ...
  for (TR::CFGNode *block = cfg->getFirstNode(); block;
    ...
    if ((edges.size() == 2) && currBlock->getExit()) {
      ...
      // block2->getSuccessors().size() == 1))
      {
        ...
        // toBlock(block1->getSuccessors().front()->getTo());
        // TR::Block * mergeBlock2 =
        // toBlock(block2->getSuccessors().front()->getTo());
...
void TR_GlobalRegisterAllocator::offerAllAutosAndRegisterParmAsCandidates(
  ...
  TR::SymbolReference *symRef;
  TR::Symbol *sym;
  TR_Array<int32_t> interestedCFGNodes(trMemory(), cfg->getNextNodeNumber() / 2,
                                       false, stackAlloc);
  ...
  TR_RegisterCandidates *candidates = comp()->getGlobalRegisterCandidates();
  TR::GlobalSet &referencedAutoSymRefsInBlock =
  ...
  while (paramCursor != NULL) {
    TR::SymbolReference *symRef =
        methodSymbol->getParmSymRef(paramCursor->getSlot());
    ...
    if ((paramCursor->isReferencedParameter() && isTypeAvailable(symRef)) &&
      ...
      if (rc) {
        ...
        while (bc.hasMoreElements()) {
          ...
          TR_ASSERT(block_num < numberOfNodes,
          ...
          TR_ASSERT(block_num == block->getNumber(),
      ...
      if (new2Offer) {
        for (i = 0; i < numInterested; i++) {
          ...
          if (!referencedAutoSymRefsInBlock[blockNumber]->get(symRefNumber))
            weight = 0;
          // If there is a reference in some block then actually create a
          // candidate
        ...
        for (node = cfg->getFirstNode(); node; node = node->getNext()) {
          ...
          if (!referencedAutoSymRefsInBlock[block->getNumber()]->get(
                  symRefNumber))
            weight = 0;
      ...
      // traceMsg(comp(), "For rc %p symref %d getLinkageRegisterIndex =
  ...
  TR::SparseBitVector &unreferencedSym =
      comp()->getUnreferencedAutoOrParmOrExternals();
    ...
    if (symRef && isSymRefAvailable(symRef)) {
      ...
      if (sym) {
        ...
        if ((sym->isAuto() &&
          ...
          // Ensure all blocks have a LoadsAndStores count. If it already exists
          // icrement it by one.
          // If it is new then set the count to zero or one based on use in the
          // block
          if (rc) {
            TR_RegisterCandidate::BlockInfo::Cursor bc(rc->_blocks.all());
            while (bc.hasMoreElements()) {
          ...
          if (new2Offer) {
            for (i = 0; i < numInterested; i++) {
              int32_t weight = 1;
              int32_t blockNumber = interestedCFGNodes[i];
              if ((rc && rc->_blocks.find(blockNumber))) // handle reg(*) only
                                                         // in block with
                                                         // abnormal blocks
                continue;
              if (!referencedAutoSymRefsInBlock[blockNumber]->get(symRefNumber))
              ...
              if (!rc && weight != 0) {
                ...
                for (int32_t j = 0; j < i; j++) {
                  rc->_blocks.setNumberOfLoadsAndStores(interestedCFGNodes[j],
                                                        0);
                }
                rc->_blocks.setNumberOfLoadsAndStores(blockNumber, weight);
              } else if (rc)
            ...
            for (i = 0; i < numInterested; i++) {
              ...
              if (rc->_blocks.find(blockNumber))
                continue;
              if (!referencedAutoSymRefsInBlock[blockNumber]->get(
...
void TR_GlobalRegisterAllocator::offerAllFPAutosAndParmsAsCandidates(
  ...
  TR::GlobalSet &referencedAutoSymRefsInBlock =
      candidates->getReferencedAutoSymRefs();
  referencedAutoSymRefsInBlock.initialize(symRefCount, numberOfNodes);
  //   TR_BitVector **referencedAutoSymRefsInBlock; // =
  //   candidates->getReferencedAutoSymRefs();
  //   referencedAutoSymRefsInBlock = (TR_BitVector
  //   **)trMemory()->allocateStackMemory(numberOfNodes*sizeof(TR_BitVector *));
  ...
  //      referencedAutoSymRefsInBlock[i] = new (trStackMemory())
...
void TR_GlobalRegisterAllocator::findLoopAutoRegisterCandidates() {
  ...
  TR_RegisterCandidate **registerCandidates =
      (TR_RegisterCandidate **)trMemory()->allocateStackMemory(
          comp()->getSymRefCount() * sizeof(TR_RegisterCandidate *));
  memset(registerCandidates, 0,
         comp()->getSymRefCount() * sizeof(TR_RegisterCandidate *));
  ...
  if (structure->asRegion()) {
    ...
    if (!regionStructure->isAcyclic() && structureNode) {
      ...
      for (auto succ = structureNode->getSuccessors().begin();
        ...
        // the
        // exit blocks may lead to slowdowns if the loop (whose exit we are
        // extending
        // the live range to) was actually run for only a few iterations and
        // the outer loop was in fact long running. Making the candidate live on
        // entry
        // to an exit out of the inner loop may cause it to be loaded up into
        // the register
        // on some hot path in the outer loop and this extra load could degrade
        // performance.
        // This scenario actually occurred in db shell sort, where the innermost
        // loop is
        // String compareTo code which does not run too long, but the outer
        // loops
        // are very hot.
        //
          ...
          if (!symsThatShouldBeAssignedInCurrentLoop->get(nextCandidate)) {
            *_temp2 = *symsThatShouldBeAssignedInCurrentLoop;
            ...
            for (p = pairs.getFirst(); p; p = pairs.getNext()) {
              ...
              if (otherSymRef &&
                  symsThatShouldNotBeAssignedInCurrentLoop->get(
...
void TR_GlobalRegisterAllocator::markAutosUsedIn(
    ...
    int32_t executionFrequency, TR_RegisterCandidate **registerCandidates,
  ...
  if (enableNewSignExtGRA) {
    ...
    if (node->getOpCode().hasSymbolReference() &&
      ...
      if (child->getOpCodeValue() == TR::aladd) {
        ...
        if (((offsetChild->getOpCodeValue() == TR::ladd) ||
             (offsetChild->getOpCodeValue() == TR::lsub)) &&
    ...
    if (!canSkip && ((node->getOpCodeValue() == TR::iadd) ||
                     (node->getOpCodeValue() == TR::isub) ||
                     (node->getOpCodeValue() == TR::iand) ||
                     (node->getOpCodeValue() == TR::iloadi) ||
                     (node->getOpCodeValue() == TR::ixor)) ||
        ((node->getOpCodeValue() ==
          TR::iload) /* && !node->getSymbolReference()->getSymbol()->isAutoOrParm() */)) {
        ...
        if (comp()->getFlowGraph()->getStructure() && blockStructure) {
          ...
          if (containingLoop) {
            ...
            if (loopInvariantBlock) {
              ...
              if (_candidatesSignExtendedInThisLoop->get(
                      storeSymRef->getReferenceNumber())) {
                TR::TreeTop *cursorTree = loopInvariantBlock->getExit();
                while (cursorTree) {
                  TR::Node *cursorNode = cursorTree->getNode();
                  if ((cursorNode->getOpCodeValue() ==
                       comp()->il.opCodeForDirectStore(node->getDataType())) &&
                      (cursorNode->getSymbolReference() == storeSymRef)) {
                    if (cursorNode->getFirstChild()->getOpCode().isLoadVar() &&
                        (cursorNode->getFirstChild()->getSymbolReference() ==
                         cursorNode->getSymbolReference()) &&
                ...
                signExtStore = TR::Node::createWithSymRef(
                    comp()->il.opCodeForDirectStore(node->getDataType()), 1, 1,
                    TR::Node::createWithSymRef(
                        node, comp()->il.opCodeForDirectLoad(
                                  storeSymRef->getSymbol()->getDataType()),
                        0, storeSymRef),
                    storeSymRef);

                TR::TreeTop *initTree =
                    TR::TreeTop::create(comp(), signExtStore, 0, 0);

                TR::TreeTop *placeHolderTree =
                    loopInvariantBlock->getLastRealTreeTop();
              ...
              TR_ASSERT(signExtStore, "Store for sign extension must be found "
                                      "or created in loop pre-header\n");
              ...
              TR_NodeTriple *nodeTriple = new (comp()->trHeapMemory())
                  TR_NodeTriple(parent, signExtStore,
                                (loadNode ? parent : storeNode), childNum);
              _nodeTriplesToBeChecked.add(nodeTriple);
              _candidatesNeedingSignExtension->set(
                  storeSymRef->getReferenceNumber());
              _candidatesSignExtendedInThisLoop->set(
                  storeSymRef->getReferenceNumber());
            }
          }
        }
    ...
    origNode = node;
    if (0 && *currentArrayAccess && canSkip) {
      TR_UseDefInfo *info = optimizer()->getUseDefInfo();
      if (TR::Compiler->target.is64Bit() && info &&
          (parent->getOpCodeValue() ==
           TR::i2l) /* && node->isNonNegative() */ &&
          (*currentArrayAccess == parent) && enableSignExtGRA) {
        TR_BlockStructure *blockStructure = block->getStructureOf();
        node->setSkipSignExtension(true);
        ...
        if (!defs.IsZero()) {
          ...
          for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            ...
            if (defIndex < info->getFirstRealDefIndex()) // def is
                                                         // unseen--can't mark
                                                         // this node as
                                                         // skippable
            {
            ...
            if (defNode->getOpCode().isStore()) {
              ...
              if (sym->isAuto()) {
                ...
                if (dependentStore &&
                    defNode->getFirstChild()->isNonNegative() && seenLoad) {
    ...
    if (TR::Compiler->target.is64Bit() && parent &&
  ...
  if (!enableNewSignExtGRA &&
      /* !hasCatchBlock && */ node->getOpCode().isLoadVarDirect() &&
      (node->getSymbolReference()->getSymbol()->isAuto())) {
    TR_UseDefInfo *info = optimizer()->getUseDefInfo();
    if (TR::Compiler->target.is64Bit() && info &&
      ...
      if (!defs.IsZero()) {
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
          int32_t defIndex = info->getFirstDefIndex() + (int32_t)cursor;
          if (defIndex < info->getFirstRealDefIndex()) // def is  unseen--can't
                                                       // mark this node as
                                                       // skippable
          {
            node->setSkipSignExtension(false);
  ...
  if (node->getOpCode().isLoadVarDirect() ||
    ...
    if ((symRef->getSymbol()->isAutoOrParm() ||
      ...
      if (block != cfg->getStart()) {
        ...
        // splitting,
        // because this cost is meaningful only when one of operand (source or
        // destination) receives a register.
        // Note: Although we need to re-calculate the cost when one of operand
        // receives a register,
        ...
        if ((node->getOpCode().isStoreDirect() && isSplittingCopy(node)) ||
          ...
          if (TR::Compiler->target.cpu.isZ() &&
              rc->getSymbolReference()->getSymbol()->getDataType() ==
                  TR::Address &&
              parent && (((parent->getOpCode().isStoreIndirect() ||
            ...
            if (trace())
              dumpOptDetails(comp(), "Increased weight of candidate #%d in "
                                     "block_%d to reduce AGI\n",
                             rc->getSymbolReference()->getReferenceNumber(),
                             block->getNumber());
          } else {
          ...
          if (!needStoreAndItExists) {
            ...
            if (comp()->getFlowGraph()->getStructure() && blockStructure) {
              ...
              if (containingLoop) {
                ...
                if ((subNode->getPredecessors().size() == 1)) {
                  ...
                  if (loopInvariantNode->getStructure()->asBlock() &&
                      loopInvariantNode->getStructure()
                          ->asBlock()
                          ->isLoopInvariantBlock())
                    loopInvariantBlock = loopInvariantNode->getStructure()
                                             ->asBlock()
                                             ->getBlock();
                }
                ...
                if (loopInvariantBlock) {
                  if (!_storeSymRef)
                    _storeSymRef = comp()->getSymRefTab()->createTemporary(
                        comp()->getMethodSymbol(), node->getDataType());
                  TR::Node *signExtStore = TR::Node::createWithSymRef(
                      comp()->il.opCodeForDirectStore(node->getDataType()), 1,
                      1, node, _storeSymRef);
  ...
  if (structure->asRegion()) {
    ...
    if (regionStructure->isNaturalLoop() && structureNode) {
      ...
      if (loopInvariantBlock) {
        ...
        /// if (trace())
        traceMsg(comp(), "Trying to split unused locals in loop %d with "
                         "starting sym ref count %d\n",
        ...
        while ((i < symRefCount) && (prevSymRefCount == 0)) {
          ...
          if (candidateIsLiveOnExit) {
            ...
            if (((!isFloat &&
                  (_numberOfGPRs + numRegsForCandidate) <=
              ...
              fixExitsAfterSplit(
                  ...
                  loopInvariantBlock->getEntry()->getNode(), registerCandidates,
                  structureNode, replacedAutosInCurrentLoop, origSymRef);
            }
          }
        ...
        _origSymRefCount = comp()->getSymRefCount(); // _origSymRefCount was
                                                     // always zero becasue
    ...
    if (comp()->cg()->considerTypeForGRA(symRef) && canSplit(symRef, comp()) &&
          ...
          if (!rc) {
            ...
            if (!rc) {
              ...
              if (!dontCreateCandidatesJustForSplitter &&
                  performTransformation(
                      comp(), "%s live range splitter creating candidate for "
                              "#%d in loop %d\n",
                      OPT_DETAILS, origSymRef->getReferenceNumber(),
                      loop->getNumber())) {
                rc = comp()->getGlobalRegisterCandidates()->findOrCreate(
                    origSymRef);
      ...
      if (correspondingSymRef) {
        ...
        if (!node->getOpCode().isStoreDirect() ||
            (!dontReplaceStores ||
             performTransformation(comp(), "%s --- going to replace auto #%d "
                                           "by auto #%d on %s node %p --- \n",
                                   OPT_DETAILS, symRef->getReferenceNumber(),
                                   correspondingSymRef->getReferenceNumber(),
          ...
          traceMsg(comp(), " --- created a store to auto #%d adjacent to "
                           "existing store to auto #%d at %s node %p --- \n",
                   correspondingSymRef->getReferenceNumber(),
...
TR_SymRefCandidatePair *TR_LiveRangeSplitter::splitAndFixPreHeader(
  ...
  //
  dumpOptDetails(comp(), " place initialization of auto #%d by auto #%d in "
                         "loop pre-header block_%d\n",
                 correspondingSymRef->getReferenceNumber(),
...
void TR_LiveRangeSplitter::fixExitsAfterSplit(
    ...
    TR::Node *node, TR_RegisterCandidate **registerCandidates,
        ...
        if (parentOfLoop) {
          ...
          for (nextBlock = blocksIt.getCurrent(); nextBlock;
            ...
            if (!blocksInInnerLoop->get(nextBlock->getNumber())) {
              if (trace())
                traceMsg(comp(), "Adding original candidate #%d in block_%d in "
                                 "outer loop %d (%p)\n",
                         rc->getSymbolReference()->getReferenceNumber(),
      ...
      if (!(exitBlock->getPredecessors().size() == 1)) {
        ...
        for (nextEdge = ei.getFirst(); nextEdge; nextEdge = ei.getNext()) {
          ...
          if (pred->getLastRealTreeTop()->getNode()->getOpCode().isBranch() &&
            ...
            for (nextSplitBlock = splitBlocksIt.getFirst();
              ...
              if (exitBlockIsASplitBlock ||
                ...
                dumpOptDetails(
                    comp(), " place store-back of auto #%d using auto #%d in "
                            "(existing) loop exit block_%d between pred %d and "
                            "exit block_%d\n",
                    orig->getReferenceNumber(),
                    replacement->getReferenceNumber(), splitBlock->getNumber(),
          ...
          if (!splitBlock) {
            ...
            dumpOptDetails(comp(), " place store-back of auto #%d using auto "
                                   "#%d in (new) loop exit block_%d between "
                                   "pred %d and exit block_%d\n",
                           orig->getReferenceNumber(),
                           replacement->getReferenceNumber(),
        ...
        dumpOptDetails(comp(), " place store-back of auto #%d using auto #%d "
                               "in loop exit block_%d \n",
                       orig->getReferenceNumber(),
                       replacement->getReferenceNumber(),
                       exitBlock->getNumber());
