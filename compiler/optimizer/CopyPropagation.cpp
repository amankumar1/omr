...
private:
  ...
  void fixupBCDPrecisionIfRequired() {
    ...
    if (oldNode->getType().isBCD() && newNode->getType().isBCD()) {
      ...
      if (newNode->getNumChildren() == 2) {
        ...
        // symbol sizes match and the other checks
        // below pass then can reduce the newPrecision to 30 so a pdModPrec
        ...
        if (newNode->getType().isAnyPacked()) {
              ...
              oldSize == newSymSize)                            // 16 == 16
          {
            if (trace() || comp()->cg()->traceBCDCodeGen()) {
              traceMsg(comp(), "reduce newPrecision %d->%d for odd to even "
                               "truncation (origNode %s (%p) prec=%d, node %s "
                               "(%p) prec=%d\n",
            ...
            newPrecision = oldPrecision; // 31->30 -- no correction needed for
                                         // same byte odd to even truncation if
                                         // the origNode was only storing out
                                         // the even $ of digits
          }
        }
      }
      ...
      if (needsClean || needsPrecisionCorrection) {
        dumpOptDetails(comp(), "node %p precision %d != propagated symRef #%d "
  ...
  void propagateThroughToCommoningPoints() {
    ...
    if (oldNode->getReferenceCount() > 0) {
      ...
      // further down the block could have commoned the old node.
      // We want to replace those commonings with a commoning to the new RHS of
      // the node because the value is the same.
...
int32_t TR_CopyPropagation::perform() {
...

  {
    ...
    _storeTreeTopsAsArray = (TR::TreeTop **)trMemory()->allocateStackMemory(
    ...
    int32_t *usesToBeFixed = (int32_t *)trMemory()->allocateStackMemory(
        useDefInfo->getTotalNodes() * sizeof(int32_t));
    memset(usesToBeFixed, 0xFF, useDefInfo->getTotalNodes() * sizeof(int32_t));

    int32_t *equivalentDefs = (int32_t *)trMemory()->allocateStackMemory(
        numDefNodes * sizeof(int32_t));
    ...
    for (i = firstRealDefIndex; i <= lastDefIndex; i++) {
      ...
      if (defNode->getOpCode().isStoreDirect() &&
        ...
        for (j = firstRealDefIndex; j < i; j++) {
          ...
          if (prevDefNode == NULL)
            continue;
          if (prevDefNode->getOpCode().isStoreDirect() &&
              prevDefNode->getSymbolReference()->getSymbol()->isAutoOrParm()) {
            ...
            if (rhsOfStoreDefNode == rhsOfPrevStoreDefNode) {
              ...
              int32_t prevDefSlot =
                  prevDefNode->getSymbolReference()->getCPIndex();
                ...
                prevSymRefIsPreferred = true;
                       ...
                       comp()
                           ->getOwningMethodSymbol(
                               prevDefNode->getSymbolReference()
                                   ->getOwningMethodIndex())
                           ->getFirstJitTempIndex()) // autos and parms
                prevSymRefIsPreferred = true;
              ...
              if (prevDefNode->getType().isBCD()) {
                ...
                if (defNode->getType().isBCD() &&
                  ...
                  dumpOptDetails(
                      comp(), "%s   Def nodes %p (precision = %d) and %p "
                              "(precision = %d) are equivalent\n",
                      OPT_DETAILS, defNode, defNode->getDecimalPrecision(),
                      prevDefNode, prevDefNode->getDecimalPrecision());

                  if ((!thisSymRefIsPreferred &&
                       (prevSymRefIsPreferred ||
                        (equivalentDefs[j - firstRealDefIndex] != j)))) {
                    if (trace())
                      traceMsg(comp(), "000setting i %d to j %d\n", i, j);
                    equivalentDefs[i - firstRealDefIndex] =
                        equivalentDefs[j - firstRealDefIndex];
    ...
    for (i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex; i++) {
      ...
      // could use "if (defs.PopulationCount() == 1) continue", but this is more
      // efficient
      ...
      TR::Node *equivalentDefNode =
          useDefInfo->getNode(equivalentDefs[defIndex - firstRealDefIndex]);
      TR::TreeTop *equivalentDefTree =
          useDefInfo->getTreeTop(equivalentDefs[defIndex - firstRealDefIndex]);
      ...
      if (defNode->getOpCode().isStoreDirect() &&
        ...
        if (copySymbolReference->getSymbol()->isAutoOrParm() &&
            performTransformation(
                ...
                equivalentDefNode)) {
          comp()->setOsrStateIsReliable(false); // could check if the
                                                // propagation was done to a
                                                // child of the OSR helper call
                                                // here
          ...
          usesToBeFixed[useNode->getUseDefIndex()] =
              equivalentDefs[defIndex - firstRealDefIndex];
    ...
    for (; fixUseIndex <= lastUseIndex; fixUseIndex++) {
      if (usesToBeFixed[fixUseIndex] > -1) {
        ...
        for (i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex; i++) {
          ...
          if (useDefInfo->getSingleDefiningLoad(useDefInfo->getNode(i)) ==
              useDefInfo->getNode(fixUseIndex)) {
    ...
    for (i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex; i++) {
      ...
      if (!defs.IsZero()) {
        ...
        if (defTree->getEnclosingBlock()->startOfExtendedBlock() ==
          ...
          // trying to do its job here.
          // Our analysis (particularly of RegLoads) assumes it is propagating
          // copies into successor blocks,
        ...
        // Bail out if we're in a NOTEMPS method and we're going to create a
        // temp.  This conditional should match
        ...
        if (!isRegLoad &&
            ...
            comp()->getJittedMethodSymbol() && // avoid NULL pointer on
                                               // non-Wcode builds
        ...
        if (performTransformation(
          ...
          if (!isRegLoad &&
              ...
              (loadNode->getOpCode().hasSymbolReference()) &&
              (loadNode->getSymbolReference() == copySymbolReference) &&
              (rhsOfStoreDefNode != loadNode)) // Modifications to this
                                               // condition should be mirrored
...
void TR_CopyPropagation::rematerializeIndirectLoadsFromAutos() {
  ...
  while (currentTree) {
    ...
    if (node->getOpCode().isStoreIndirect() &&
        ...
        performTransformation(
            comp(), "%s   Rematerializing indirect load from auto in node %p "
                    "(temp #%d)\n",
            OPT_DETAILS, nextNode->getFirstChild(),
            nextNode->getSymbolReference()->getReferenceNumber())) {
      comp()->setOsrStateIsReliable(false); // could check if the propagation
                                            // was done to a child of the OSR
                                            // helper call here
    ...
    if (node->getOpCode().hasSymbolReference()) {
...
#endif
      {
        ...
        if (origNode->getOpCode().isLoadIndirect()) {
          ...
          if (trace() || comp()->cg()->traceBCDCodeGen())
            traceMsg(comp(), "node %s (%p) #%d->#%d, origNode %s (%p) #%d, "
                             "defNode %s (%p) #%d,copySymRef #%d\n",
                     node->getOpCode().getName(), node,
          ...
          if (origNode->getNumChildren() == 2) {
            ...
            // prec=31 then if the symbol sizes match and the other checks
            // below pass then can reduce the newPrecision to 30 so a pdModPrec
            // correction is not needed (as the propagated symbol will not have
            // the top nibble 30->31 set).
            ...
            if (node->getType().isAnyPacked()) {
                  ...
                  nodeSize == origSymSize)               // 16 == 16
              {
                if (trace() || comp()->cg()->traceBCDCodeGen())
                  traceMsg(comp(), "reduce newPrecision %d->%d for odd to even "
                                   "truncation (origNode %s (%p) prec=%d, node "
                                   "%s (%p) prec=%d\n",
                           newPrecision, nodePrecision,
                           origNode->getOpCode().getName(), origNode,
                           origNode->getDecimalPrecision(),
                           node->getOpCode().getName(), node, nodePrecision);
                newPrecision = nodePrecision; // 31->30 -- no correction needed
                                              // for same byte odd to even
                                              // truncation if the origNode was
                                              // only storing out the even $ of
                                              // digits
              }
            }
          }
          ...
          if (trace() || comp()->cg()->traceBCDCodeGen())
            traceMsg(comp(), "needsClean = %s, needsPrecisionCorrection = %s "
                             "(cur %d, new %d)\n",
                     needsClean ? "yes" : "no",
          ...
          if (needsClean || needsPrecisionCorrection) {
            ...
            // is a call).
            // Also if the defNode was going to clean must also insert a pdclean
            // on top of the node with the propagated symRef
            ...
            dumpOptDetails(comp(), "node %p precision %d != propagated symRef "
                                   "#%d precision %d and/or needsClean (%s) so "
                                   "create nodeCopy %p "
                                   "(isTwoChildrenCase=%s)\n",
            ...
            if (needsPrecisionCorrection) {
              ...
              TR_ASSERT(modPrecOp != TR::BadILOp, "no bcd modify precision "
                                                  "opcode found for node %p "
                                                  "with type %s\n",
            ...
            if (needsClean) {
              ...
              TR_ASSERT(cleanOp != TR::BadILOp, "no bcd modify precision "
                                                "cleanOp found for node %p "
                                                "with type %s\n",
                        nodeCopy, nodeCopy->getDataType().toString());
            ...
            dumpOptDetails(comp(), "attach new first %s (%p) and second %s "
                                   "(%p) to node %s (%p) in the "
                                   "twoChildren=true case\n",
...
visitCount idiom --
The visit count for a node is set iff the node has NOT been replaced with a
regLoadNode.
This is done to make sure that we will replace ALL qualified nodes with the
regLoadNode.
...
bool TR_CopyPropagation::isLoadNodeSuitableForPropagation(TR::Node *useNode,
  ...
  if (useNode->getType().isBCD() && loadNode->getType().isBCD()) {
    dumpOptDetails(comp(), "isLoadNodeSuitableForPropagation : useNode %p "
                           "(prec %d), loadNode %p (prec %d) -- isCorrect=%s "
                           "(only correct when useNodePrec <= loadNodePrec)\n",
    ...
    if (useNode->getDecimalPrecision() > loadNode->getDecimalPrecision()) {
      ...
      dumpOptDetails(comp(), "isLoadNodeSuitableForPropagation=false for "
                             "useNode %s (%p) prec %d > max %d and mustClean "
                             "store %p\n",
...
// those nodes has ref count > 1, then add to the
...
void collectNodesForIsSafeChecks(TR::Node *n,
  ...
  // than one,
  // add it to the list of nodes to call isSafeToPropagate on because it could
  // be anchored
  // earlier in the block. Only consider nodes with symbol references, because
  // isSafeToPropagate
...
TR::Node *TR_CopyPropagation::skipTreeTopAndGetNode(TR::TreeTop *tt) {
  TR_ASSERT(tt, "tt should always be set no matter if it comes from "
                "_storeTreeTopsAsArray or an iterator");
...
bool TR_CopyPropagation::isRedefinedBetweenStoreTreeAnd(
  ...
  while (currentTree != NULL) {
    ...
    if (visitPredecessors) {
      ...
      for (auto nextEdge = predList.begin(); nextEdge != predList.end();
           ++nextEdge) {
        TR::Block *next = toBlock((*nextEdge)->getFrom());
...
bool TR_CopyPropagation::isCorrectToReplace(
  ...
  if (useNode->getType().isBCD() && storeNode->getType().isBCD()) {
    dumpOptDetails(comp(), "isCorrectToReplace : useNode %p (prec %d), "
                           "storeNode %p (prec %d) -- isCorrect=%s (only "
                           "correct when useNodePrec <= storeNodePrec)\n",
...
TR_CopyPropagation::isCheapRematerializationCandidate(TR::Node *defNode,
  ...
  //  udi=331 nc=1 addr=4
  //    aiadd (internalPtr )
  //    [0x2B016B20] bci=[-1,37,329] rc=1 vc=9438 vn=- sti=- udi=- nc=2 addr=4
  //    flg=0x8000
  //      loadaddr <auto slot 40> [id=152:"PSA"] [#1.63 Auto] [flags 0x4000008
  //      0x0 ]    [0x2B016AD0] bci=[-1,37,329] rc=1 vc=9438 vn=- sti=- udi=-
  //      nc=0 addr=4
  //      iconst 1212 (X!=0 X>=0 )
  //      [0x2B016B70] bci=[-1,37,329] rc=1 vc=9438 vn=- sti=- udi=- nc=0
  //      flg=0x104
  //
  if (node->getOpCode().isLoadIndirect()) {
