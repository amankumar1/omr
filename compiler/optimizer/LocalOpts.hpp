 ...
 * in the same block that can be used to perform the null check implicitly
 * again.
 */
 ...
 * read of a[i]
 * if (a.length <= N) goto blockA; // test emitted for the bound check on the
 * read of a[i]
 * if (i < 0)         goto blockA; // second test emitted for the bound check on
 * the read of a[i]
 * if (b == null)     goto blockA; // test emitted for the null check on the
 * read of b[i]
 * if (b.length <= N) goto blockA; // test emitted for the bound check on the
 * read of b[i]
 * if (i < 0)         goto blockA; // second test emitted for the bound check on
 * the read of b[i]
 * if (a == null)     goto blockA; // test emitted for the null check on the
 ...
 * those
 * redundant tests. This is where and simplification comes in. The above control
 * flow
...
public:
  ...
  // List<TR::Node> *, List<TR::Node> *, List< List<TR::Node> > *,
  // List<TR::Node> *, List< List<TR::Node> > *, List<TR::Node> *,
  // List<TR::Node> *);
  ...
  // All the following instance variables and methods are for
  // LongRegAllocHeuristic
private:
  bool _onlyLongReg;
...
protected:
  ...
  int32_t getNumLongAtNesting(int8_t i) {
    TR_ASSERT((i > -1 && i < LONGREG_NEST),
              "Illegal nesting level for long register\n");
...
public:
  ...
  // temporarilySetProfilingBcInfoOnNewArrayLengthChild(newArray);
  // (do whatever)
