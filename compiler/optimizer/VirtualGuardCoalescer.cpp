...
int32_t TR_VirtualGuardTailSplitter::perform() {
  ...
  if (globalSplit && !comp()->isProfilingCompilation() && _numGuards >= 15 &&
      numBlocks / 5 < _numGuards) {
    // Discovered method is loaded with virtual guards!
    // Perform a global split
    //
    splitGlobal();
    printf("---$$$--- GlobalSplit %d,%d %s\n", nodeCount,
           comp()->getNodeCount(), comp()->signature());
    return 10;
  }
  ...
  //
  static const char *enableColdGuardElimination =
      feGetEnv("TR_EnableColdVirtualGuardEliminationInVGTS");
...
void TR_VirtualGuardTailSplitter::eliminateColdVirtualGuards(
    TR::TreeTop *treetop) {
  TR::Block *block = NULL;
  while (treetop) {
    ...
    TR::TreeTop *predBBStart =
        block->getPrevBlock() ? block->getPrevBlock()->getEntry() : NULL;
    ...
    if (block->getLastRealTreeTop()
      ...
      virtualGuard =
          comp()->findVirtualGuardInfo(block->getLastRealTreeTop()->getNode());

    if (virtualGuard && block->isCold() &&
        performTransformation(comp(), "%s remove guard from cold block_%d\n",
                              OPT_DETAILS, block->getNumber())) {
      TR::Block *dest = block->getLastRealTreeTop()
                            ->getNode()
                            ->getBranchDestination()
                            ->getNode()
                            ->getBlock(); // info->getCallBlock();
      ...
      if (block->nodeIsRemoved()) {
        if (trace())
          traceMsg(comp(),
                   "An orphaned region starting with block_%d was removed\n",
                   block->getNumber());
        ...
        // will treat it as a "head" of a loop which removes the rest of the
        // loop
        // i.e. we cannot start from the block that is in the middle of the loop
        // as if there were any blocks before it we would have processed those
        // first
        treetop = predBBStart;
        continue;
      }
      ...
      TR::Node *gotoNode =
          TR::Node::create(block->getLastRealTreeTop()->getNode(), TR::Goto);
  ...
  while (cursor->getExit() != NULL) {
    ...
    if (!(succs.size() == 1)) {
      ...
      if ((succs.size() == 2) && !lastTree->getNode()->getOpCode().isSwitch()) {
        ...
        if (/* succBlock1->getSuccessors().size() == 1) && */ (
            succBlock1 == cursor->getNextBlock())) {
          // TR::Block *nextBlock =
        ...
        if (!newCursor) {
          ...
          if (/* succBlock2->getSuccessors().size() == 1) && */ (
              succBlock2 == cursor->getNextBlock())) {
            // TR::Block *nextBlock =
...
TR_VirtualGuardTailSplitter::recognizeVirtualGuard(TR::Block *block,
  ...
  if (call && guardNode->isProfiledGuard()) {
    ...
    TR_AddressInfo *valueInfo = (TR_AddressInfo *)profileManager->getValueInfo(
        node->getByteCodeInfo(), comp(),
        TR_ValueProfileInfoManager::allProfileInfoKinds, NotBigDecimalOrString);
...
void TR_VirtualGuardTailSplitter::splitGlobal() {
  ...
  _cfg->removeNode(
      cloner->getToBlock(comp()->getStartTree()->getNode()->getBlock()));
}
...
int32_t TR_VirtualGuardTailSplitter::rematerializeThis() {
  ...
  for (; block; block = block->getNextBlock()) {
    if (prevBlock) {
      ...
      for (auto edge = prevBlock->getSuccessors().begin();
           edge != prevBlock->getSuccessors().end(); ++edge) {
        if (block == (*edge)->getTo()) {
          isSuccessor = true;
          break;
        }
    ...
    if (node->isNopableInlineGuard()) {
      ...
      if (callNode && callNode->getOpCode().isCallIndirect()) {
        ...
        if (thisChild->getOpCode().isLoadVarDirect() &&
          ...
          while (guardBlock) {
            while (prevTree && prevTree != startOfExtendedBlock->getEntry()) {
              prevNode = prevTree->getNode();

              // dumpOptDetails(comp(), "For guard %p searching currNode %p till
              // %p(block_%d)\n", node, prevNode,
              // startOfExtendedBlock->getEntry(),
              // startOfExtendedBlock->getNumber());
            ...
            if (!parentInfo || (parentInfo == vgInfo))
              break;
            else {
          ...
          if (symRef && storeBlock) {
            ...
            if (thisExpr->getOpCode().isLoadVarDirect() &&
              ...
              while (cursorPrev &&
                ...
                //   {
                //   if
                //   (cursorBlock->getPredecessors().find(cursorBlock->getPrevBlock()))
    ...
    if (thisExpr) {
      ...
      collectSymbolReferencesInNode(thisExpr, symbolReferencesInNode,
                                    &numDeadSubNodes, visitCount, comp());

      TR::TreeTop *treeTop;
      for (treeTop = storeBlock->getEntry(); treeTop != tt;
        ...
        if (cursorNode->getOpCode().isResolveCheck()) {
          // Treat unresolved references like a call, except for field
          // references.
          // (Field references can only escape to user code if the class has to
          // be
          ...
          if (!cursorNode->getOpCode().isLoad()) {
            ...
            if (symbolReferencesInNode.ValueAt(
                    currSymRef->getReferenceNumber())) {
              // dumpOptDetails(comp(), "0Cannot rematerialize for guard %p
              // because of node %p\n", node, cursorNode);
        ...
        //
        if (currSymRef != NULL) {
          if (currSymRef->getUseDefAliases().containsAny(symbolReferencesInNode,
                                                         comp())) {
            // dumpOptDetails(comp(), "1Cannot rematerialize for guard %p
            // because of node %p\n", node, cursorNode);
            canRematerialize = false;
            break;
          }
...
/// dumpOptDetails(comp(), "Success: Can rematerialize for guard %p\n", node);
      ...
      numGuardsChanged++;
      TR_SymNodePair *symNodePair =
          (TR_SymNodePair *)trMemory()->allocateStackMemory(
              sizeof(TR_SymNodePair));
      ...
      TR::Node *newThis = TR::Node::createWithSymRef(
          thisExpr, thisExpr->getOpCodeValue(), thisExpr->getNumChildren(),
          thisExpr->getSymbolReference());
      ...
      TR::Node *newStore = TR::Node::createWithSymRef(
          TR::astore, 1, 1, newThis, storeNode->getSymbolReference());
      TR::TreeTop *newStoreTree =
          TR::TreeTop::create(comp(), newStore, NULL, NULL);
      ...
      if (origThis != callNode->getSecondChild())
         ...
         for (i = 0; i < origThis->getNumChildren(); i++)
            {
              //canonicalizeTree(thisExpr->getChild(i), symNodePairs,
      comp()->getVisitCount());
...
void TR_VirtualGuardTailSplitter::canonicalizeTree(
    TR::Node *node, List<TR_SymNodePair> &symNodePairs, vcount_t visitCount) {
  ...
  if (node->getOpCode().isLoadVarDirect()) {
    ...
    TR_SymNodePair *pair;
    for (pair = symNodeIt.getFirst(); pair != NULL;
         pair = symNodeIt.getNext()) {
      // dumpOptDetails(comp(), "Looking at symRef %d(%p) this symRef %d(%p)\n",
      // pair->_symRef->getReferenceNumber(), pair->_symRef,
      // symRef->getReferenceNumber(), symRef);
      if (symRef == pair->_symRef) {
        ...
        for (i = 0; i < pair->_node->getNumChildren(); i++) {
          ...
          // duplicateChild);
          canonicalizeTree(duplicateChild, symNodePairs,
                           comp()->getVisitCount());
...
bool TR_VirtualGuardTailSplitter::isLegalToClone(TR::Node *node,
                                                 vcount_t visitCount) {
  // dumpOptDetails(comp(), "Reached 0 canonicalized for node %p\n", node);
  ...
  // dumpOptDetails(comp(), "Reached 1 canonicalized for node %p\n", node);
  bool canonicalized = false;
  if (node->getOpCode().isCall() || node->getOpCodeValue() == TR::New ||
...
void TR_InnerPreexistence::transform() {
  ...
  for (i = 0; i < _numInlinedSites; ++i) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (!performTransformation(comp(), "%sDevirtualizing call guarded by "
                                         "block_%d preexisting on guard %d\n",
                                 OPT_DETAILS,
