...
class TR_LiveOnNotAllPaths
    : public TR::Allocatable<TR_LiveOnNotAllPaths, TR::Allocator> {
public:
...
public:
  ...
  TR_MovableStore *_movableStore; // all TR_MovableStore's are also
                                  // TR_UseOrKillInfo's so this is a pointer
...
public:
  ...
  int32_t _symIdx;
  TR_BitVector *_commonedLoadsUnderTree; // commoned used syms under this store
                                         // in the IL tree (syms are not first
                                         // used)
  // - syms that have associated temps are excluded (only when
  // enablePreciseSymbolTracking() == false otherwise all syms are included)
  TR_BitVector *_commonedLoadsAfter; // used syms which are commoned following
                                     // the store (syms are first used and are
                                     // commoned later)
  TR::Compilation *comp() { return _comp; }
...
public:
  ...
  //  it should be copied if the store is placed in a block that does NOT extend
  //  the original source block
  //  it should be moved if it is to be placed in a block that extends the
...
public:
  ...
  bool sinkMethodMetaDataStores() {
    return _storeSinkingFlags.testAny(SinkMethodMetaDataStores);
  }
  void setSinkMethodMetaDataStores(bool b) {
    _storeSinkingFlags.set(SinkMethodMetaDataStores, b);
  }
...
protected:
  ...
  // sinking
  // Right now, there's only one tree (a store) represented in these, but in
  // future the
  ...
  void
  searchAndMarkFirstUses(TR::Node *node, TR::TreeTop *tt,
                         TR_MovableStore *movableStore, TR::Block *currentBlock,
...
public:
  // performs store sinking without using dataflow analysis by pushing stores as
  // far
...
private:
  TR::TreeTop *genSideExitTree(TR::TreeTop *store, TR::Block *exitBlock,
                               bool isFirstGen);
  virtual bool storeIsSinkingCandidate(TR::Block *block, TR::Node *node,
                                       int32_t symIdx, bool sinkIndirectLoads,
                                       uint32_t &indirectLoadCount,
                                       ...
                                       vcount_t &treeVisitCount,
                                       vcount_t &highVisitCount);
  virtual bool storeCanMoveThroughBlock(TR_BitVector *blockKilledSet,
                                        TR_BitVector *blockUsedSet,
                                        int32_t symIdx);
  virtual bool sinkStorePlacement(TR_MovableStore *store,
                                  bool nextStoreWasMoved);
  bool passesAnchoringTest(TR_MovableStore *store, bool storeChildIsAnchored,
                           bool nextStoreWasMoved);
