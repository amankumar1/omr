...
TR::ValuePropagation::ValuePropagation(TR::OptimizationManager *manager)
    : TR::Optimization(manager), _parmValues(NULL), _currentParent(NULL),
      _objectCloneCallTreeTops(trMemory()), _objectCloneCallNodes(trMemory()),
      _objectCloneInstanceClasses(trMemory()), _arraylengthNodes(trMemory()),
      _javaLangClassGetComponentTypeCalls(trMemory()),
      _unknownTypeArrayCopyTrees(trMemory()),
      _scalarizedArrayCopies(trMemory()), _cachedStringBufferVcalls(trMemory()),
      _cachedStringPeepHolesVcalls(trMemory()), _predictedThrows(trMemory()),
      ...
      _bcdSignConstraints(NULL), _parmInfo(NULL), _parmTypeValid(NULL),
    ...
    if (!comp()->getFlowGraph()->getStructure() &&
      ...
      if (doTiming) {
        ...
        if (comp()->getOutFile() != NULL) {
          trfprintf(comp()->getOutFile(), "Time taken for %s = ",
                    myTimer.title());
...
TR::ValuePropagation::copyRelationships(Relationship *first) {
...
TR::ValuePropagation::copyStoreRelationships(StoreRelationship *first) {
...
TR::ValuePropagation::createValueConstraint(
...
TR::VPConstraint *TR::ValuePropagation::getConstraint(TR::Node *node,
                                                      bool &isGlobal,
                                                      TR::Node *relative) {
  /*
...
void TR::ValuePropagation::removeNode(TR::Node *node, bool anchorIt) {
  ...
  if (node->getReferenceCount() > 1) {
    ...
    // children
    // must be evaluated before the parent, so they belong before the parent
    // tree.
    // Also, we have to do this even if the curtree is the last tree in the
    // block, since the node may
...
void TR::ValuePropagation::processTrees(TR::TreeTop *startTree,
                                        TR::TreeTop *endTree) {
  TR::TreeTop *treeTop;
...
bool TR::ValuePropagation::canTransformArrayCopyCallForSmall(
...
bool TR::ValuePropagation::transformUnsafeCopyMemoryCall(
...
void TR::ValuePropagation::transformArrayCopyCall(TR::Node *node) {
        ...
        if (referenceArray2) {
          ...
          if (enableTransformReferenceArray) {
            transformReferenceArray =
                true; // for reference array transformation
          }
  ...
  if (transformReferenceArray && !comp()->generateArraylets()) {
    // If only primitive arraycopy is supported for this target, and the call is
    // known to
    ...
    TR_ResolvedMethod *newMethod =
        findResolvedClassMethod(comp(), "Ljava/lang/System;", "arraycopy",
...
//
    ...
    // as we may affect the reference counts of srcObjNode etc. (the removed
    // arraycopy
    // could be a parent of a node still in the trees, see code below that uses
    // srcObjNode etc.)
    //
    if (node->getReferenceCount() < 1) {
      return;
...
TR::TreeTop *
TR::ValuePropagation::createPrimitiveOrReferenceCompareNode(TR::Node *node) {
#ifdef J9_PROJECT_SPECIFIC
...
TR::TreeTop *TR::ValuePropagation::createArrayStoreCompareNode(TR::Node *src,
                                                               TR::Node *dst) {
...
TR::TreeTop *TR::ValuePropagation::createPrimitiveArrayNodeWithoutFlags(
  ...
  if (isOptimizedReferenceArraycopy) {
    ...
    // check
    // nor write barrier, it can be simplified into a three-child arraycopy, but
    // the
    ...
    // have
    // been inserted, and on this path we are generating a truly primitive
    // arraycopy,
    // but the element type remains unknown. root->getArrayCopyElementType()
    // CANNOT be used, since the root node does not have that information (we
    // would
...
#ifdef J9_PROJECT_SPECIFIC
void TR::ValuePropagation::generateArrayTranslateNode(
...
//-------------------------- realtime support
TR::TreeTop *TR::ValuePropagation::buildSameLeafTest(TR::Node *offset,
                                                     TR::Node *len,
                                                     TR::Node *spineShiftNode) {
...
void TR::ValuePropagation::generateRTArrayNodeWithoutFlags(
...
//
TR::TreeTop *
TR::ValuePropagation::createSpineCheckNode(TR::Node *node,
...
void TR::ValuePropagation::transformArrayCopySpineCheck(
    TR_ArrayCopySpineCheck *checkInfo) {
...
void TR::ValuePropagation::transformRealTimeArrayCopy(
  ...
  // of the real call)
  // TR::Node *dummyNode1 = TR::Node::create(TR::treetop, 1, TR::Node::create(
  // vcallNode, TR::iconst, 0, 0));
  ...
  // reference arraycopy with no store check flag.
  // for realtime the fast path is primitive (if we got there there is no need
  // for wrtbar)
...
#ifdef J9_PROJECT_SPECIFIC
void TR::ValuePropagation::transformRTMultiLeafArrayCopy(
    TR_RealTimeArrayCopy *rtArrayCopyTree) {
  ...
  TR_OpaqueClassBlock *helperClass =
      comp()
          ->getSystemClassPointer(); // fe()->getClassFromSignature("RTArrayCopy",
                                     // 11, method);
...
void TR::ValuePropagation::transformObjectCloneCall(
    TR::TreeTop *callTree, TR::ValuePropagation::ObjCloneInfo *cloneInfo) {
  ...
  if (!performTransformation(comp(), "%sInlining object clone call [%p] as new "
                                     "object and JITHelpers object copy\n",
  ...
  if (method->getRecognizedMethod() ==
    ...
    callTree->insertBefore(TR::TreeTop::create(
        comp(), TR::Node::create(callNode->getFirstChild(), TR::treetop, 1,
  ...
  if (callTree->getNode()->getOpCode().isCheck()) {
    ...
    callTree->insertBefore(TR::TreeTop::create(
        comp(), TR::Node::create(objNode, TR::treetop, 1, objNode)));
  }
  ...
  objNode->recursivelyDecReferenceCount();
  callTree->insertBefore(TR::TreeTop::create(
...
void TR::ValuePropagation::transformArrayCloneCall(
    TR::TreeTop *callTree, TR_OpaqueClassBlock *j9arrayClass) {
  ...
  if (callTree->getNode()->getOpCode().isCheck()) {
    ...
    TR::Node *passthrough =
        TR::Node::create(callNode, TR::PassThrough, 1, callNode->getChild(0));
    callTree->insertBefore(TR::TreeTop::create(
        comp(), TR::Node::createWithSymRef(
                    callNode, callTree->getNode()->getOpCodeValue(), 1,
                    passthrough, callTree->getNode()->getSymbolReference())));
...
void TR::ValuePropagation::transformConverterCall(TR::TreeTop *callTree) {
  ...
  dumpOptDetails(comp(), "Insert runtime tests for converter call "
                         "transformation to arraytranslate (%p)\n",
                 callNode);
  ...
  // the result),
  // so that any references to it pick up the result regardless of execution
  // path
  ...
  if (TR::Compiler->target.cpu.isZ()) {
    ...
    // the Java method is
    // faster than the hardware accelerated arraytranslate. The constants used
    // below are deduced
    // from an empirical study of all 11 converters on Z.
    //
    // It is also noteworthy to say that we use ifacmpeq and ificmpgt trees to
    // perform the nullchecks
    // and the "bound checks" because the actual implementation in Java
    // translates the arrays on an
    // element by element basis. This means if an ArrayIndexOutOfBounds were to
    // occur we would have
    // an intermediate translation result. This cannot be achieved if we have a
    // pre-emptive BNDCHK
    // node before the arraytranslate tree. Thus we must fall back to the Java
    // implementation in
    // such cases.
...
void TR::ValuePropagation::launchNode(TR::Node *node, TR::Node *parent,
                                      int32_t whichChild) {
