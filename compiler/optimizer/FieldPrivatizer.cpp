...
void TR_FieldPrivatizer::placeInitializersInLoopInvariantBlock(
  ...
  while (currentNodeElem) {
    ...
    if (_privatizedFieldSymRefs.locate(symRef->getReferenceNumber(), index)) {
      ...
      TR_ASSERT(false, "\n field #%d should have been privatized but didn't "
                       "find corresponding new auto in _privatizedFieldSymRefs",
                symRef->getReferenceNumber());
...
void TR_FieldPrivatizer::addPrivatizedRegisterCandidates(
  ...
  while (privatizedCandidate) {
    if (performTransformation(comp(), "%s Adding auto %d (created for "
                                      "privatization) as a global register "
                                      "candidate in loop %d\n",
                              optDetailString(), privatizedCandidate->getData()
                                                     ->getSymbolReference()
                                                     ->getReferenceNumber(),
                              structure->getNumber()))
...
void TR_FieldPrivatizer::elementPrivatization() {
  ...
  if (!optimizer()->getValueNumberInfo()) {
    dumpOptDetails(comp(),
                   "Can't do element Privatization - no value numbers for %s\n",
                   comp()->signature());
    return;
  }
...
void TR_FieldPrivatizer::findElementCandidates() {
  for (TR::TreeTop *tt = comp()->getMethodSymbol()->getFirstTreeTop(); tt;
       tt = tt->getNextTreeTop()) {
    TR::Node *loadOrStoreNode = walkTreeForLoadOrStoreNode(tt->getNode());

    if (loadOrStoreNode && isNodeInCorrectForm(loadOrStoreNode)) {
      traceMsg(comp(), "Adding node %p tt %p with vn %d to list of candidates "
                       "to privatize\n",
               loadOrStoreNode, tt, _valueNumberInfo->getValueNumber(
                                        loadOrStoreNode->getFirstChild()));

      EPCandidate newCand(
          loadOrStoreNode, tt,
          _valueNumberInfo->getValueNumber(loadOrStoreNode->getFirstChild()));
      _epCandidates.push_back(newCand);
      _epValueNumbers[_valueNumberInfo->getValueNumber(
          loadOrStoreNode->getFirstChild())] = 1;
    }
  }
}

void TR_FieldPrivatizer::privatizeElementCandidates() {
  CS2::HashTable<int32_t, TR::SymbolReference *, TR::Allocator> tempMap(
      comp()->allocator());
  TR::list<EPCandidate> difficultCandidates(
      getTypedAllocator<EPCandidate>(comp()->allocator()));
  for (auto cursor = _epCandidates.begin(); cursor != _epCandidates.end();
       ++cursor) {
    ...
    if (tempMap.Locate(candidate.valueNum, index)) {
      ...
      TR_ASSERT(candidate.node->getOpCode().hasSymbolReference(),
                "Candidate node %p does not have a symbol reference!\n",
                candidate.node);
      ...
      if (offset < 0) // This means that candidate has a value number and some
                      // other node will have the addr calculation, we just have
                      // to wait till it gets added to the hashtable
      {
        difficultCandidates.push_back(candidate);
        continue;
      }

      tempSymRef = comp()->getSymRefTab()->createCoDependententTemporary(
          comp()->getMethodSymbol(), candidate.node->getDataType(), false,
          candidate.node->getSize(), candidate.node->getSymbol(), 0);
    ...
    TR::TreeTop *anchorTT = TR::TreeTop::create(
        comp(),
        TR::Node::create(TR::treetop, 1, candidate.node->getFirstChild()));
    candidate.rootTT->insertBefore(anchorTT);

    // step 2, convert the indirect store in to a direct store.

    TR_ASSERT(
        candidate.node->getReferenceCount() == 1,
        "Candidate store node %p has refcount > 1... How is this possible?\n",
        candidate.node);

    TR::Node::recreate(candidate.node,
                       comp()->il.opCodeForDirectStore(storedt));
  ...
  for (auto difficult_cursor = difficultCandidates.begin();
       difficult_cursor != difficultCandidates.end(); ++difficult_cursor) {
    TR::SymbolReference *tempSymRef = 0;
...
bool TR_FieldPrivatizer::isNodeInCorrectForm(TR::Node *node) {
  ...
  TR_ASSERT(node->getOpCode().isStoreIndirect() ||
                node->getOpCode().isLoadIndirect(),
            "Node is not an indirect store or load!\n");

  if (node->getFirstChild()->getOpCodeValue() == TR::loadaddr) {
    return true;
  } else if (node->getFirstChild()->getOpCode().isAdd() &&
    ...
    if (offset < 0 || offset > 20) // a basic heuristic to not do this for
                                   // massively huge arrays.  Maybe I don't need
                                   // this.
      return false;
