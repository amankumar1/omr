...
TR_LocalAnalysisInfo::TR_LocalAnalysisInfo(TR::Compilation *c, bool t)
  ...
  if (_numNodes < 0) {
    ...
    while (currentTree) {
      ...
      if ((opCode->isStore() &&
           !firstNodeInTree->getSymbolReference()
                ->getSymbol()
                ->isAutoOrParm()) ||
          opCode->isCheck()) {
...
bool TR_LocalAnalysisInfo::isCallLike(TR::Node *node) {
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    // are being skipped for performance (see earlier code in this file) and
    // we want to avoid strange side effects of a parent being commoned but a
...
// mechanism
// to avoid processing nodes twice. However, the trees are already in a similar
// situation so
// the node visit counts must not be corrupted for those iterations.
// Simply incrementing for each use of containsCall will not work, for large
// programs the visit
// count will overflow.
...
// It is guaranteed that there is room for old visit count + 2, that is checked
// before all
// this starts.
//
...
// case
// is TR::aiadd (or TR::aladd) which may not participate in PRE; expressions
// containing
// the TR::aiadd (or TR::aladd) are matched correctly though by the special case
// code.
//
