...
static void convertToTestUnderMask(TR::Node *node, TR::Block *block,
  ...
  if ((iand = node->getChild(0))->getOpCodeValue() == TR::iand &&
      ...
      performTransformation(cm, "%sTransforming iand/iushr to byte test under "
                                "mask [" POINTER_PRINTF_FORMAT "]\n",
                            s->optDetailString(), node)) {
    // anchor iand subtree
    auto anchor =
...
template <typename T>
static void foldConstant(TR::Node *node, T value, TR::Simplifier *s,
    ...
    if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) &&
      ...
      if (performTransformation(s->comp(),
        ...
        TR::Node *newSecondChild = TR::Node::create(
            secondChild, firstChild->getOpCode().isAdd()
...
static TR::Node *addSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (nodeOp.isAdd() && secondChildOp.isLoadConst() &&
    ...
    if (TR::DataType::isSignedInt32<T>() && // this code has not been ported to
      ...
      if (performTransformation(
              s->comp(), "%sReduced xadd of -1 and an xneg in node [%s] to "
                         "bitwise complement\n",
              s->optDetailString(), node->getName(s->getDebug()))) {
        s->anchorChildren(node, s->_curTree);
    ...
    // Turn add with first child negated into a subtract
                 ...
                 s->comp(), "%sReduced iadd with negated first child in node "
                            "[%s] to isub\n",
                 s->optDetailString(), node->getName(s->getDebug()))) {
      s->anchorChildren(node, s->_curTree);
      TR::Node::recreate(node, TR::ILOpCode::getSubOpCode<T>());
  ...
  // turn add with other child negated in to a subtract
    ...
    if (s->getLastRun() &&
        ...
        performTransformation(
            s->comp(), "%sFactored iadd with distributed imul with a codegen "
                       "decomposible constant in node [%s]\n",
            s->optDetailString(), node->getName(s->getDebug()))) {
    ...
    if (lrChild->getOpCode().isLoadConst()) {
      if (secondChildOp.isLoadConst()) {
        if (!s->reassociate() && // use new rules
            performTransformation(
                s->comp(), "%sFound xadd of xconst with xadd or xsub of x and "
                           "const in node [%s]\n",
                s->optDetailString(), node->getName(s->getDebug()))) {
          if (firstChild->getReferenceCount() > 1) {
          ...
          s->_alteredBlock = true;
                 ...
                 (firstChild->getReferenceCount() == 1) &&
                 performTransformation(
                     s->comp(), "%sFound xadd of non-xconst with xadd or isub "
                                "of x and const in node [%s]\n",
                     s->optDetailString(), node->getName(s->getDebug()))) {
...
TR::Node *subSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (node->getOpCodeValue() != TR::aiadd &&
    ...
    if ((rrChild->getOpCode().isLoadConst()) && (rlChild == firstChild) &&
      ...
      if (performTransformation(
              s->comp(), "%sFolded isub with children related through iconst "
                         "in node [%s] to iconst \n",
              s->optDetailString(), node->getName(s->getDebug()))) {
        node->setChild(0, 0);
        node->setChild(1, 0);
  ...
  // more negative value than positive in a 2's complement signed representation
    ...
    if (performTransformation(s->comp(), "%sNormalized isub of iconst > 0 in "
                                         "node [%s] to iadd of -iconst \n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
  ...
  // quicker, but normalizes expressions and drives inegs up the tree
    ...
    if (performTransformation(s->comp(), "%sReduced xsub with negated first "
                                         "child in node [%s] to ineg of xadd\n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
      TR::Node *newFirstChild = firstChild->getFirstChild();
  ...
  // turn bitwise complement + 1 into a 2's complement negate
    ...
    if (performTransformation(
            s->comp(), "%sReduced isub of bitwise complement and iconst -1 in "
                       "node [%s] to 2s complement negation\n",
            s->optDetailString(), node->getName(s->getDebug()))) {
  ...
  //   that problem, the test should be moved back down there.
    ...
    if (lrChild->getOpCode().isLoadConst()) {
      if (secondChildOp.isLoadConst()) {
        if (performTransformation(
                s->comp(), "%sFound xsub of xconst with xadd or xsub of x and "
                           "const in node [%s]\n",
                s->optDetailString(), node->getName(s->getDebug()))) {
          if (firstChild->getReferenceCount() > 1) {
            // it makes sense to uncommon it in this situation
          ...
          if ((firstChild->getReferenceCount() == 1) &&
              (performTransformation(
                  s->comp(), "%sFound xsub of non-xconst with xadd or xsub of "
...
static TR::Node *foldDemotionConversion(TR::Node *node, TR::ILOpCodes opcode,
  ...
  if (node->getOpCode().isConversionWithFraction() &&
    ...
    //    f2pd frac=0
    //       f
    // to just f2d because the extra digits that should be introduced by the
    // frac=5 in the parent will be lost
    return 0;
  }
  ...
  if (node->getOpCode().isConversionWithFraction() &&
    ...
    //    i2pd
    //       i
    // to just i2f because the extra digits that should be introduced by the
    // frac=5 in the parent will be lost
    return 0;
  }
#endif
...
static bool reduceLongOp(TR::Node *node, TR::Block *block, TR::Simplifier *s,
  ...
  if (!performTransformation(
          s->comp(), "%sReducing long operation in node [" POINTER_PRINTF_FORMAT
                     "] to an int operation\n",
          s->optDetailString(), node))
    return false;
...
static TR::Node *removeIfToFollowingBlock(TR::Node *node, TR::Block *block,
                                          TR::Simplifier *s) {
  if (branchToFollowingBlock(node, block, s->comp())) {
    // Branch to the immediately following block. The branch can be removed
    //
    if (performTransformation(s->comp(), "%sRemoving %s [" POINTER_PRINTF_FORMAT
                                         "] to following block\n",
...
//    const (or a),
//    We move this ifcmp to after that def, hopefully the ifcmp can be resolved
//    at compile time
// 2. TODO: for cmp,
static void partialRedundantCompareElimination(TR::Node *node, TR::Block *block,
  ...
  if (node->getSecondChild()->getOpCode().isLoadConst() &&
    ...
    TR::list<TR::CFGEdge *> &predecessors =
        ((TR::CFGNode *)block)->getPredecessors();
  ...
  if (firstOp == TR::a2i &&
    ...
    if ((secondOp == TR::iconst && secondChild->getInt() == 0) ||
      ...
      if (secondOp == TR::a2i) {
        ...
        dumpOptDetails(s->comp(), "Address Compare Conversion: found child 1 "
                                  "a2i and child 2 iconst in node %p\n",
                       node);
      }
    }
    ...
    if ((secondOp == TR::lconst && secondChild->getLongInt() == 0) ||
      ...
      if (secondOp == TR::a2l) {
        ...
        dumpOptDetails(s->comp(), "Address Compare Conversion: found child 1 "
                                  "a2l and child 2 lconst in node %p\n",
                       node);
      }
    }
  }
    ...
    if ((firstOp == TR::i2l) &&
      ...
      if (secondOp == TR::iconst || secondOp == TR::i2l ||
        ...
        if (secondOp == TR::i2l) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(), "Long Compare Narrower: found child 1 "
                                      "i2l and child 2 lconst in iconst range "
                                      "in method %s\n",
                           s->comp()->signature());
          }
        } else if (reportCompareDemotions) {
          dumpOptDetails(s->comp(), "Long Compare Narrower: found child 1 i2l "
                                    "and child 2 iconst in method %s\n",
                         s->comp()->signature());
        }
      }
      ...
      if (firstOp == TR::su2l) {
        if (secondOp == TR::cconst || secondOp == TR::su2l ||
          ...
          if (secondOp == TR::su2l) {
            ...
            secondChild->recursivelyDecReferenceCount();
            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(), "Long Compare Narrower: found both "
                                        "children c2l in method %s\n",
                             s->comp()->signature());
            }
          } else if (secondOp == TR::lconst) {
            if (secondChild->getReferenceCount() == 1) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(), "Long Compare Narrower: found child 1 "
                                        "c2l and child 2 lconst in cconst "
                                        "range in method %s\n",
                             s->comp()->signature());
            }
          } else if (reportCompareDemotions) {
            dumpOptDetails(s->comp(), "Long Compare Narrower: found child 1 "
                                      "c2l and child 2 cconst in method %s\n",
                           s->comp()->signature());
          }
        }
        ...
        if (secondOp == TR::sconst || secondOp == TR::s2l ||
          ...
          if (secondOp == TR::s2l) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(), "Long Compare Narrower: found child 1 "
                                        "s2l and child 2 lconst in sconst "
                                        "range in method %s\n",
                             s->comp()->signature());
            }
          } else if (reportCompareDemotions) {
            dumpOptDetails(s->comp(), "Long Compare Narrower: found child 1 "
                                      "s2l and child 2 sconst in method %s\n",
                           s->comp()->signature());
          }
        }
        ...
        if (secondOp == TR::bconst || secondOp == TR::b2l ||
          ...
          if (secondOp == TR::b2l) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(), "Long Compare Narrower: found child 1 "
                                        "b2l and child 2 lconst in bconst "
                                        "range in method %s\n",
                             s->comp()->signature());
            }
          } else if (reportCompareDemotions) {
            dumpOptDetails(s->comp(), "Long Compare Narrower: found child 1 "
                                      "b2l and child 2 bconst in method %s\n",
                           s->comp()->signature());
          }
        }
      }
...
// Integer boolean OR
//
static TR::Node *isUnsafeIorByteChild(TR::Node *child, TR::ILOpCodes b2iOpCode,
                                      int32_t mulConst, TR::Simplifier *s) {
#ifdef J9_PROJECT_SPECIFIC
  if (child->getOpCodeValue() == TR::imul &&
      ...
      child->getFirstChild()->getFirstChild()->getSymbolReference() ==
          s->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int8)) {
...
static TR::Node *isLastUnsafeIorByteChild(TR::Node *child, TR::Simplifier *s) {
  ...
  if (child->getOpCodeValue() == TR::bu2i && child->getReferenceCount() == 1 &&
      ...
      child->getFirstChild()->getReferenceCount() == 1 &&
      child->getFirstChild()->getSymbolReference() ==
          s->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int8)) {
...
static bool indirectToDirectConvBeneficial(TR::SymbolReference *indirectSymRef,
                                           TR::SymbolReference *directSymRef,
                                           TR::Simplifier *s) {
  return true;
}
...
static void decomposeMultiply(TR::Node *node, TR::Simplifier *s, bool isLong) {
  ...
  if ((!s->containingStructure() || s->getLastRun()) &&
      comp->cg()->mulDecompositionCostIsJustified(
          ...
          isLong ? secondChild->getLongInt() : secondChild->getInt()) &&
      performTransformation(
          comp, "%sDecomposing mul with a constant, to shift left, "
...
TR::Node *getQuotientUsingMagicNumberMultiply(TR::Node *node, TR::Block *block,
  ...
  if (node->getOpCodeValue() == TR::idiv ||
    ...
    TR_ASSERT(0, "getQuotientUsingMagicNumberMultiply called with incorrect "
 ...
 * change
 * uncommoned nodes.  Only valid if we don't see any ops that could shift the
 * bits
...
static bool symRefPairMatches(TR::SymbolReference *actual1,
                              TR::SymbolReference *actual2,
                              TR::SymbolReference *desired1,
...
static bool isLegalToMerge(TR::Node *node, TR::Block *block,
                           TR::Block *nextBlock,
                           TR::list<TR::CFGEdge *> &nextExcpOut,
                           TR::Node *bbStartNode,
                           TR::list<TR::CFGEdge *> &inEdge, TR::Simplifier *s,
                           bool &blockIsEmpty) {
  ...
  // more than one edge
  // Since there may be a goto/branch which go around entry point, in that case
  // merging will skip the prologue for that empty entry point
  ...
  if (cfg &&
      ...
      ((block->getStructureOf() &&
       ...
       (nextBlock->getStructureOf() &&
        nextBlock->getStructureOf()->isLoopInvariantBlock())))
    return false;
...
static void changeBranchDestinationsForMergeBlocks(
    ...
    TR::list<TR::CFGEdge *> &inEdge,
    std::list<TR::CFGEdge *,
              TR::typed_allocator<TR::CFGEdge *, TR::Allocator>>::iterator
        &inEdgeIter,
    TR::Simplifier *s) {
...
static TR::Node *foldAbs(TR::Node *node, TR::Simplifier *s) {
  ...
  if (childNode &&
      ...
      performTransformation(s->comp(),
                            "%sFolded abs for postive argument on node [%p]\n",
                            s->optDetailString(), node)) {
...
// Consider if the iload expression had the value 33554432. After the i2f, the
// value
// would be 3355430 because the float representation of the value does not as
// much
// precision as the integer representation. If the range of the value was known
// to be
...
// constant
// is within the precision of the data type and therefore lost precision won't
// affect the
// comparison.
//
    ...
    if (firstOp == TR::su2i && firstChild->getReferenceCount() == 1) {
      if (secondOp == TR::su2i ||
        ...
        if (secondOp == TR::su2i) {
          ...
          secondChild->recursivelyDecReferenceCount();
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(), "Integer Compare Narrower: found both "
                                      "children c2i in method %s\n",
                           s->comp()->signature());
          }
        } else if (secondOp == TR::iconst) {
          if (secondChild->getReferenceCount() > 1) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(), "Integer Compare Narrower: found child 1 "
                                      "c2i and child 2 iconst in cconst range "
                                      "in method %s\n",
                           s->comp()->signature());
          }
        } else if (reportCompareDemotions) {
          dumpOptDetails(s->comp(), "Integer Compare Narrower: found child 1 "
                                    "c2i and child 2 cconst in method %s\n",
                         s->comp()->signature());
        }
      }
      ...
      if (secondOp == TR::s2i ||
        ...
        if (secondOp == TR::s2i) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(), "Integer Compare Narrower: found child 1 "
                                      "s2i and child 2 iconst in sconst range "
                                      "in method %s\n",
                           s->comp()->signature());
          }
        } else if (reportCompareDemotions) {
          dumpOptDetails(s->comp(), "Integer Compare Narrower: found child 1 "
                                    "s2i and child 2 sconst in method %s\n",
                         s->comp()->signature());
        }
      }
      ...
      if (secondOp == TR::b2i ||
        ...
        if (secondOp == TR::b2i) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(), "Integer Compare Narrower: found child 1 "
                                      "b2i and child 2 iconst in bconst range "
                                      "in method %s\n",
                           s->comp()->signature());
          }
        } else if (reportCompareDemotions) {
          dumpOptDetails(s->comp(), "Integer Compare Narrower: found child 1 "
                                    "b2i and child 2 bconst in method %s\n",
                         s->comp()->signature());
        }
      }
    }
...
static TR::Node *integerToFloatHelper(uint32_t absValue,
  ...
  if (leadingZeroes(absValue) + trailingZeroes(absValue) >= 8) {
    ...
    if (isUnsignedOperation)
      ...
      foldFloatConstant(node, firstChild->getInt() < 0 ? -(float)absValue
                                                       : (float)absValue,
                        s);
...
// n379n       ior
// n373n         imul
// n371n           iload  <auto slot 8>[id=232:"b"] [#60  Auto] [flags 0x3 0x0 ]
// (Int32)
// n372n           iconst 0x400000
// n378n         iushr
// n374n           iload  <auto slot 8>[id=232:"b"] [#60  Auto] [flags 0x3 0x0 ]
// (Int32)
// n377n           iconst 10
//
...
static bool checkAndReplaceRotation(TR::Node *node, TR::Block *block,
  ...
  if ((1 << correctLeftShiftAmount) != mulConst) // Only a rotate if the const
                                                 // values add up to the size of
                                                 // the node type.  ie for ints,
                                                 // this better add up to 32
    return false;
  ...
  default:
    TR_ASSERT(
        0, "Data Type of node %p not supported in checkAndReplaceRotation!\n",
        mulConstNode);
  }

  TR::Node::recreate(node, TR::ILOpCode::getRotateOpCodeFromDt(
                               mulConstNode->getDataType())); // this line will
                                                              // have to change
                                                              // based on T
  ...
  // The nonconst node will survive as it is attached to the rol node before
...
static TR::Node *longToFloatHelper(uint64_t absValue, bool isUnsignedOperation,
  ...
  if (leadingZeroes(absValue) + trailingZeroes(absValue) >= (64 - 24)) {
    foldFloatConstant(node, isUnsignedOperation
    ...
    if (isUnsignedOperation)
      ...
      foldFloatConstant(node, firstChild->getLongInt() < 0 ? -(float)absValue
                                                           : (float)absValue,
                        s);
    return node;
  }
...
static TR::Node *longToDoubleHelper(uint64_t absValue, bool isUnsignedOperation,
  ...
  if (leadingZeroes(absValue) + trailingZeroes(absValue) >= (64 - 53)) {
    foldDoubleConstant(node, isUnsignedOperation
         ...
         // round it manually
  {
    ...
    if (isUnsignedOperation)
      ...
      foldDoubleConstant(node, firstChild->getLongInt() < 0 ? -(double)absValue
                                                            : (double)absValue,
                         s);
    return node;
  }
 ...
 */
//---------------------------------------------------------------------
static bool convertToSinglePrecisionSQRT(TR::Simplifier *s,
                                         TR::Node *sqrtCall) {
#ifdef J9_PROJECT_SPECIFIC
  if (!s->comp()->cg()->supportsSinglePrecisionSQRT())
    return false;
  ...
  if (f2dChild->getOpCodeValue() != TR::f2d)
    return false; // TODO: could get more aggressive

  if (!performTransformation(s->comp(),
                             "%sTransforming [" POINTER_PRINTF_FORMAT
                             "] (double)sqrt(f2d(x))->(float)sqrt(x)\n",
                             s->optDetailString(), sqrtCall))
    return false;

  TR::SymbolReference *fsqrtSymRef =
      s->comp()->getSymRefTab()->findOrCreateSinglePrecisionSQRTSymbol();

  TR::TreeTop *callTreeTop = findTreeTop(
      sqrtCall, s->_curTree->getEnclosingBlock()->startOfExtendedBlock());
  TR_ASSERT(callTreeTop != NULL,
            "should have been able to find this tree top (call %p) in block %d",
  ...
  // replace child
  TR::Node *newf2dChild =
      s->replaceNode(f2dChild, f2dChild->getFirstChild(), s->_curTree);
 ...
 * if supported on the platform.
 ...
 <(53-24) or <(53-31)
        bits precision ,but result might overflow a double range or denormalised
 precision
     - Children of multiplies are both f2d
     - One child of multiply is f2d or i2d and other is double const of <(53-24)
 or <(53-31) bits precision
       and result cannot overflow a double range or precision.  This is
 currently not handled as it requires
...
static bool isOperationFPCompliant(TR::Node *parent, TR::Node *arithExprNode,
  ...
  // from the precision of current type and if smaller than the precision limit
  // then
  ...
  // then there are 10 zeros and hence can increase precision requirements by
  // 53-10=43 bits.
...
static void bitwiseToLogical(TR::Node *node, TR::Block *block,
  ...
  // The smarter way is to uncommon all the references in the next block
  // referencing the
  // current block. The class TR_HandleInjectedBasicBlocks in the inliner will
  // be useful
  //
  ...
  TR::ILOpCodes newOp =
      ((isZero)
           ? TR::ILOpCode(cmp2->getOpCode().getOpCodeForReverseBranch())
                 .convertCmpToIfCmp()
...
TR::Node *indirectLoadSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (nodeOp == TR::aloadi || nodeOp == TR::iloadi || nodeOp == TR::lloadi) {
    if (childOp == TR::aloadi || childOp == TR::iloadi ||
        childOp == TR::lloadi) {
...
#ifdef J9_PROJECT_SPECIFIC
      if (symRefPairMatches(
              nodeSymref, childSymref,
      ...
      if (fieldsAreComplementary &&
          performTransformation(
              s->comp(), "%sFolded complementary field load [%p]->%s->%s\n",
        ...
        if (replacement->getDataType() != node->getDataType()) {
          TR::ILOpCodes convOpCode = TR::ILOpCode::getProperConversion(
              replacement->getDataType(), node->getDataType(), false);
          TR_ASSERT(
              convOpCode != TR::BadILOp,
              "Conversion between two different data types requires an opcode");
          replacement = TR::Node::create(convOpCode, 1, grandchild);
          // Note: be wary of collected refs.  We don't want to convert those
          // accidentally to primitives.
        }
    ...
    if (nodeSymref == s->getSymRefTab()->findClassAndDepthFlagsSymbolRef() &&
        somePackedOp == TR::loadaddr &&
        !somePackedAddress->getSymbolReference()->isUnresolved()) {
      TR::SymbolReference *childSymref =
          somePackedAddress->getSymbolReference();
      TR_OpaqueClassBlock *clazz =
          (TR_OpaqueClassBlock *)childSymref->getSymbol()
              ->getStaticSymbol()
              ->getStaticAddress();
      uintptrj_t konst = s->comp()->fej9()->getClassDepthAndFlagsValue(clazz);
      TR::Node *konstNode =
          nodeOp == TR::iloadi
              ? TR::Node::iconst(node, konst)
              : nodeOp == TR::lloadi ? TR::Node::lconst(node, konst) : NULL;
  ...
  if (firstChild->getOpCodeValue() == TR::loadaddr &&
      !s->comp()->cg()->getLinkage()->isArgumentListSymbol(
          firstChild->getSymbolReference()->getSymbol(),
          s->comp())) // argList symbol can grow later on
  {
    bool newOType = false;
    ...
    if (isSame &&
        ...
        indirectToDirectConvBeneficial(node->getSymbolReference(),
                                       firstChild->getSymbolReference(), s) &&
        performTransformation(
            s->comp(),
...
TR::Node *indirectStoreSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (node->getSymbolReference()->isPacked() &&
      node->getOpCodeValue() != TR::astorei) {
  ...
  if (isDeletedLabelLoadaddr(secondChild) &&
      performTransformation(
          s->comp(), "%sRemoving indirect store [" POINTER_PRINTF_FORMAT
  ...
  if ((node->getOpCodeValue() == TR::wrtbari) && 0 &&
    ...
    if ((valueChild->getOpCodeValue() == TR::New) ||
      ...
      if (removeWrtBar && !s->comp()->getOptions()->realTimeGC() &&
          performTransformation(
              s->comp(), "%sFolded indirect write barrier to iastore because "
                         "GC could not have occurred enough times to require "
...
TR::Node *astoreSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (isDeletedLabelLoadaddr(node->getFirstChild()) &&
      performTransformation(
          s->comp(), "%sRemoving astore [" POINTER_PRINTF_FORMAT
                     "] to deleted label [" POINTER_PRINTF_FORMAT "]\n",
...
TR::Node *directStoreSimplifier(TR::Node *node, TR::Block *block,
  ...
  // swing this tree down below the node
  // that uses it to avoid ugly register copying (because both the original and
  // incremented value are required)
  ...
  // quality improvement
    ...
    if (firstLoadsSymRef && secondIsLoadConst ||
      ...
      // something stores to <sym> or end of block
      // remember tree where last use of node was on this walk: insert our
      // update tree following that last use tree
      ...
      if (loadTree->getNode()->getOpCodeValue() == TR::treetop) {
        ...
        if (loadNode->getOpCode().isLoadVarDirect() &&
          ...
          // <sym>
          // when we find a tree containing a use of <sym>, update insertion
          // point with tree following the use tree
          ...
          if (insertionTree != NULL &&
              performTransformation(
                  s->comp(), "%smove update tree [" POINTER_PRINTF_FORMAT
            ...
            // eliminateDeadTrees might subsequently try to undo it
            // by moving the load below the update tree (bad pass!).  We'll
            // allow it to do that (because we can come along and re-swing
            // the update below the load) so long as it doesn't move the load
            // under a branch tree.  If the load gets moved into a branch
            // tree, then there's no place we could put the update tree that's
            // below the load but still in the block
            // (executive summary: all our careful work above would be for
            // naught).  So mark the load here so that
...
TR::Node *ifdCallSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (callSymbolIsRecognizedAbsMethod(symbol)) {
    node = foldAbs(node, s);
             ...
             callSymbolIsRecognizedPowMethod(
    ...
    // call can have 2 or 3 args.  In both cases, the last two are the
    // parameters of interest.
    TR::Node *expNode = node->getChild(numChildren - 1);
    TR::Node *valueNode = node->getChild(numChildren - 2);

    // In strictmath.pow(), if both arguments are integers, then the result is
    // exactly equal to the mathematical result
    ...
    if (callSymbolIsRecognizedPowMethod(
            node->getSymbol()->getResolvedMethodSymbol())) {
...
TR::Node *lcallSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (s->comp()->cg()->getSupportsCurrentTimeMaxPrecision()) {
    ...
    if (methodSymbol) {
      if ((methodSymbol->getRecognizedMethod() ==
           TR::java_lang_System_currentTimeMillis) &&
          (methodSymbol->isJNI() || methodSymbol->isVMInternalNative() ||
           methodSymbol->isJITInternalNative()) &&
          performTransformation(s->comp(),
                                "%sConvert currentTimeMillis to "
                                "currentTimeMaxPrecision with divide "
                                "of" INT64_PRINTF_FORMAT " on node [%p]\n",
                                s->optDetailString(), divisor, node)) {
        TR::Node *lcallNode = TR::Node::createWithSymRef(
            node, TR::lcall, 0,
            s->comp()
                ->getSymRefTab()
                ->findOrCreateCurrentTimeMaxPrecisionSymbol());
        TR::Node *tempResult = TR::Node::create(TR::treetop, 1, lcallNode);
        TR::TreeTop *callTreeTop = findTreeTop(node, block);
        TR_ASSERT(callTreeTop != NULL,
                  "should have been able to find this tree top");
        ...
        TR::TreeTop *newTree = TR::TreeTop::create(
            s->comp(), callTreeTop->getPrevTreeTop(), tempResult);
        ...
        if (callTreeNode->getOpCode().isResolveCheck()) {
          if (callTreeNode->getOpCodeValue() == TR::ResolveCHK)
            ...
            TR_ASSERT(0, "Null check not expected in call to static method in "
                         "class System\n");
        }

        s->_alteredBlock = true;
      } else if (methodSymbol->getRecognizedMethod() ==
                     TR::java_lang_System_nanoTime &&
                 (methodSymbol->isJNI() || methodSymbol->isVMInternalNative() ||
                  methodSymbol->isJITInternalNative()) &&
                 performTransformation(
                     s->comp(),
                     "%sConvert nanoTime to currentTimeMaxPrecision with "
                     "multiply of " INT64_PRINTF_FORMAT " on node [%p]\n",
  ...
  } else if (s->comp()->cg()->getSupportsFastCTM() &&
             node->getNumChildren() == 0 && node->getReferenceCount() == 2) {
    TR::ResolvedMethodSymbol *methodSymbol =
        node->getSymbol()->getResolvedMethodSymbol();
    if (methodSymbol && (methodSymbol->getRecognizedMethod() ==
                         TR::java_lang_System_currentTimeMillis) &&
      ...
      for (tt = block->getEntry(); tt != exitTree;
           tt = tt->getNextRealTreeTop()) {
        if (((tt->getNode()->getOpCodeValue() == TR::treetop) ||
             (tt->getNode()->getOpCodeValue() == TR::ResolveCHK)) &&
      ...
      if (tt != exitTree) // found the call tree
      {
        ...
        if ((opCode == TR::lstorei || opCode == TR::lstore) &&
          ...
          if (node == valueChild &&
              performTransformation(s->comp(), "%sFolded long store of "
                                               "currentTimeMillis to use "
                                               "address of destination as "
                                               "argument on node [%p]\n",
                                    s->optDetailString(), node)) {
            node->setNumChildren(1);
            ...
            if (addressChild) {
              if (TR::Compiler->target.is64Bit()) {
                ...
                offsetNode->setLongInt(
                    (int64_t)nextNode->getSymbolReference()->getOffset());
                storeAddressNode =
                    TR::Node::create(TR::aladd, 2, addressChild, offsetNode);
              } else
                storeAddressNode = TR::Node::create(
                    TR::aiadd, 2, addressChild,
                    TR::Node::create(
                        node, TR::iconst, 0,
                        (int32_t)nextNode->getSymbolReference()->getOffset()));
            } else {
              storeAddressNode = TR::Node::create(node, TR::loadaddr);
              storeAddressNode->setSymbolReference(
                  nextNode->getSymbolReference());
            }
...
TR::Node *acallSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if ((node->getOpCode().isCallDirect()) &&
      ...
      performTransformation(s->comp(),
                            "%sRemoved dead BigDecimal/BigInteger call node "
...
TR::Node *iaddSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (nodeOp == TR::iadd && secondChildOp == TR::iconst &&
    ...
    if (secondChildOp == TR::iconst && secondChild->getInt() == -1) {
      if (performTransformation(
              s->comp(), "%sReduced iadd of -1 and an ineg in node [%s] to "
                         "bitwise complement\n",
              s->optDetailString(), node->getName(s->getDebug()))) {
        s->anchorChildren(node, s->_curTree);
        TR::Node::recreate(node, TR::ixor);
  ...
  // reduce imul operations by factoring
    ...
    if (lrChild->getOpCodeValue() == TR::iconst) {
      if (secondChildOp == TR::iconst) {
        if (!s->reassociate() && // use new rules
            performTransformation(
                s->comp(), "%sFound iadd of iconst with iadd or isub of x and "
                           "const in node [%s]\n",
                s->optDetailString(), node->getName(s->getDebug()))) {
          if (firstChild->getReferenceCount() > 1) {
            // it makes sense to uncommon it in this situation
          ...
          s->_alteredBlock = true;
                 ...
                 (firstChild->getReferenceCount() == 1) &&
                 performTransformation(
                     s->comp(), "%sFound iadd of non-iconst with iadd or isub "
                                "of x and const in node [%s]\n",
                     s->optDetailString(), node->getName(s->getDebug()))) {
        // move constants up the tree so they will tend to get merged together
        node->setChild(1, lrChild);
    ...
    if ((lrChild->getOpCodeValue() == TR::iconst) &&
      ...
      if (performTransformation(
        ...
        s->_alteredBlock = true;
             ...
             (node->getSecondChild()->getOpCode().isLoadConst()) && // iadd
                                                                    // second
                                                                    // child
                                                                    // should be
                                                                    // const to
    ...
    //
    // n1.3083n   (  0)      iadd (in *GPR_6512) (profilingCode )
    // [0x2BB38260] loc=[-1,2848,1153] rc=0 vc=631 vn=- sti=- udi=14723 nc=2
    // flg=0x80
    // n1.3081n   (  0)        a2i (in *GPR_6512) (profilingCode unneededConv )
    // [0x2BB381C0] loc=[-1,2847,1153] rc=0 vc=631 vn=- sti=- udi=14723 nc=1
    // addr=4 flg=0x8080
    // n1.26322n  (  0)          aiadd (in *GPR_6512) (profilingCode X>=0
    // internalPtr )                       [0x374931DC] loc=[-1,2844,1153] rc=0
    // vc=1859 vn=- sti=- udi=14723 nc=2 addr=4 flg=0x8180
    // n1.3077n   (  2)            ==>aload (in GPR_6513) (profilingCode
    // isUnsignedLoad )
    // n1.3086n   (  0)            ==>iconst 692 (profilingCode X!=0 X>=0 )
    // n1.3082n   (  0)        iconst -24 (profilingCode X!=0 X<=0 )
    ...
    if ((node_aiadd->getSecondChild()->getOpCodeValue() == TR::iconst) &&
        ...
        performTransformation(s->comp(), "%sPushing down iadd [%p] iconst [%p] "
                                         "into aiadd [%p] iconst [%p] below\n",
                              s->optDetailString(), node,
      ...
      if (passesPrecisionTest &&
          performTransformation(
              s->comp(), "%sPushing down iadd [%p] iconst [%p] into aiadd [%p] "
                         "second child [%p], creating new iadd\n",
  ...
  // reduce imul operations by factoring
    ...
    if (secondChildOp == TR::lconst) {
      ...
      if (lrChild->getOpCodeValue() == TR::lconst) {
        if (performTransformation(s->comp(), "%sFound ladd of lconst with ladd "
                                             "or lsub of x and const in node "
                                             "[" POINTER_PRINTF_FORMAT "]\n",
                                  s->optDetailString(), node)) {
          if (firstChild->getReferenceCount() > 1) {
             ...
             // together
      {
        if ((firstChild->getReferenceCount() == 1) &&
            performTransformation(
                s->comp(), "%sFound ladd of non-lconst with ladd or lsub of x "
    ...
    if ((lrChild->getOpCodeValue() == TR::lconst) &&
      ...
      if (performTransformation(s->comp(), "%sFound aladd of lconst with aladd "
                                           "x and lconst in node "
                                           "[" POINTER_PRINTF_FORMAT "]\n",
                                s->optDetailString(), node)) {
        int64_t value = secondChild->getLongInt();
        value += lrChild->getLongInt();
        ...
        s->_alteredBlock = true;
               ...
               performTransformation(
                   s->comp(), "%sFound aladd of non-lconst with aladd x and "
...
TR::Node *isubSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  // quicker, but normalizes expressions and drives inegs up the tree
    ...
    if (performTransformation(s->comp(), "%sReduced isub with negated first "
                                         "child in node [%s] to ineg of iadd\n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
      TR::Node *newFirstChild = firstChild->getFirstChild();
  ...
  //   that problem, the test should be moved back down there.
    ...
    if (lrChild->getOpCodeValue() == TR::iconst) {
      if (secondChildOp == TR::iconst) {
        if (performTransformation(
                s->comp(), "%sFound isub of iconst with iadd or isub of x and "
                           "const in node [%s]\n",
                s->optDetailString(), node->getName(s->getDebug()))) {
          if (firstChild->getReferenceCount() > 1) {
            // it makes sense to uncommon it in this situation
          ...
          if ((firstChild->getReferenceCount() == 1) &&
              (performTransformation(
                  s->comp(), "%sFound isub of non-iconst with iadd or isub of "
                             "x and const in node [%s]\n",
                  s->optDetailString(), node->getName(s->getDebug())))) {
        // move constants up the tree so they will tend to get merged together
        node->setChild(1, lrChild);
   ...
   */
    ...
    if (LBase && RBase && LBase == RBase &&
        performTransformation(s->comp(), "%sRemove loadaddr in address "
                                         "computation in "
                                         "[" POINTER_PRINTF_FORMAT "]\n",
                              s->optDetailString(), node)) {
      if (!exprA)
...
TR::Node *lsubSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (node->getOpCodeValue() != TR::aladd &&
    ...
    if ((rrChild->getOpCodeValue() == TR::lconst) && (rlChild == firstChild) &&
        node->cannotOverflow() && secondChild->cannotOverflow()) {
      if (performTransformation(
              s->comp(), "%sFolded lsub with children related through lconst "
                         "in node [" POINTER_PRINTF_FORMAT "] to lconst \n",
              s->optDetailString(), node)) {
        node->setChild(0, 0);
        node->setChild(1, 0);
  ...
  {
    if (performTransformation(s->comp(), "%sReduced lsub of bitwise complement "
                                         "and lconst -1 in node "
                                         "[" POINTER_PRINTF_FORMAT
  ...
  // reduce imul operations by factoring
    ...
    if (secondChildOp == TR::lconst) {
      TR::Node *lrChild = firstChild->getSecondChild();
      if (lrChild->getOpCodeValue() == TR::lconst) {
        if (performTransformation(s->comp(), "%sFound lsub of lconst with ladd "
                                             "or lsub of x and lconst in node "
                                             "[" POINTER_PRINTF_FORMAT "]\n",
                                  s->optDetailString(), node)) {
          if (firstChild->getReferenceCount() > 1) {
            // it makes sense to uncommon it in this situation
             ...
             // together
      {
        if ((firstChild->getReferenceCount() == 1) &&
            performTransformation(
                s->comp(), "%sFound lsub of non-lconst with ladd or lsub of x "
                           "and lconst in node [" POINTER_PRINTF_FORMAT "]\n",
                s->optDetailString(), node)) {
          node->setChild(1, lrChild);
          firstChild->setChild(1, secondChild);
  ...
  // 184251
    ...
    if (shftAmnt > 0 && (firstChild->getOpCodeValue() == TR::iadd ||
      ...
      if ((iValue << shftAmnt) == (int32_t)(lValue << shftAmnt) &&
          performTransformation(s->comp(), "%sFound lsub/lmul of +ve powOf2 "
                                           "lconst with i2l of iadd or isub of "
                                           "x and iconst in node "
                                           "[" POINTER_PRINTF_FORMAT "]\n",
                                s->optDetailString(), node)) {
      ...
      if (secondChildOp == TR::lconst) {
        ...
        if (lrChild->getOpCodeValue() == TR::iconst) {
          if (node->isNonNegative() && i2lNode->isNonNegative()) {
            ...
            // be able to make this transformation
            // we can loosen this up in the future if we have more information
            // on children nodes but not the parents
            int64_t value = (int64_t)lrChild->getInt();
            value = -secondChild->getLongInt();
            if (performTransformation(s->comp(),
                                      "%sFound lsub of lconst with i2l of iadd "
                                      "or isub of x and iconst in node "
                                      "[" POINTER_PRINTF_FORMAT "]\n",
                                      s->optDetailString(), node)) {
              // Reset the flags on i2lNode to reflect the new child - llChild
...
TR::Node *imulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    if (node->getOpCode().isUnsigned()) {
      foldUIntConstant(node, firstChild->getUnsignedInt() *
...
TR::Node *lmulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChildOp == TR::lmul && firstChild->getReferenceCount() == 1) {
    ...
    if (lrChild->getOpCodeValue() == TR::lconst) {
      if (secondChildOp == TR::lconst) {
        if (performTransformation(s->comp(), "%sFound lmul of lconst with lmul "
                                             "of x and lconst in node "
                                             "[" POINTER_PRINTF_FORMAT "]\n",
                                  s->optDetailString(), node)) {
          if (secondChild->getReferenceCount() == 1) {
            secondChild->setLongInt(secondChild->getLongInt() *
          ...
          s->_alteredBlock = true;
             ...
             // together
      {
        if (performTransformation(s->comp(), "%sFound lmul of non-lconst with "
                                             "lmul of x and lconst in node "
                                             "[" POINTER_PRINTF_FORMAT "]\n",
                                  s->optDetailString(), node)) {
          node->setChild(1, lrChild);
          firstChild->setChild(1, secondChild);
    ...
    TR::Node *lrChild = firstChild->getSecondChild();
    if (lrChild->getOpCodeValue() == TR::lconst &&
        performTransformation(s->comp(), "%sDistributed lmul with lconst over "
                                         "lsub or ladd with lconst in node "
                                         "[" POINTER_PRINTF_FORMAT "]\n",
                              s->optDetailString(), node)) {
  ...
  // the simplification is then invalid
    ...
    if (((i2lNode->getFirstChild()->getOpCodeValue() == TR::isub) ||
      ...
      if (lrChild->getOpCodeValue() == TR::iconst &&
          performTransformation(
              s->comp(), "%sDistributed lmul with lconst over isub or iadd of "
                         "with iconst in node [" POINTER_PRINTF_FORMAT "]\n",
              s->optDetailString(), node)) {
        int64_t product =
...
TR::Node *dmulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (s->comp()->getOption(TR_IgnoreIEEERestrictions) &&
      !node->isFPStrictCompliant() &&
      !s->comp()->cg()->supportsNegativeFusedMultiplyAdd()) // we can create
                                                            // x*1.0 when negFma
                                                            // is available.
                                                            // Don't undo it
  {
...
TR::Node *ldivSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
    ...
    if (divisor != 0) {
      if (firstChild->getOpCode().isLoadConst()) {
        ...
        if (firstChild->isNonNegative()) {
          ...
          if (divisor < 0) {
            if (performTransformation(s->comp(), "%sReduced ldiv power of 2 - "
                                                 "neg nominator with lshr lneg "
                                                 "in node [%p]\n",
                                      s->optDetailString(), node)) {
  ...
  if (node->getOpCodeValue() == TR::ldiv) {
    ...
    if (secondChild->getOpCode().isLoadConst() &&
        ...
        node->getFirstChild()->getOpCode().isLoadVar() && // transformToLongDiv
                                                          // creates a tonne of
                                                          // nodes.  Not good to
                                                          // do this recursively
                                                          // if you have chained
...
TR::Node *iabsSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (child->isNonNegative() &&
      performTransformation(
          s->comp(),
          "%sSimplify iabs of non-negative child at [" POINTER_PRINTF_FORMAT
          "]\n",
          s->optDetailString(), node)) {
...
TR::Node *labsSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (child->isNonNegative() &&
      performTransformation(
          s->comp(),
          "%sSimplify labs of non-negative child at [" POINTER_PRINTF_FORMAT
          "]\n",
          s->optDetailString(), node)) {
    return s->replaceNodeWithChild(node, child, s->_curTree, block);
  }
...
TR::Node *ishlSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldIntConstant(node, firstChild->getInt()
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    //
    TR::Node::recreate(node, node->getOpCodeValue() == TR::iushl ? TR::iumul
                                                                 : TR::imul);
...
TR::Node *lshlSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node, firstChild->getLongInt()
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    //
    performTransformation(s->comp(), "%sCanonicalize long left shift by "
                                     "constant in node [" POINTER_PRINTF_FORMAT
...
TR::Node *bshlSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldByteConstant(node, firstChild->getByte()
                               << (secondChild->getInt() & INT_SHIFT_MASK),
                     s, false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *sshlSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(node, firstChild->getShortInt()
                                   << (secondChild->getInt() & INT_SHIFT_MASK),
                         s, false /* !anchorChildren */);
    return node;
  }
    ...
    if (constGrandChild->getOpCodeValue() == TR::iconst &&
      ...
      if (subTree->getReferenceCount() == 1) {
        if (opCode == TR::s2i && rightShiftValue == 16) {
          if (performTransformation(s->comp(),
                                    "%sReduced left shift followed by iushr "
                                    "equivalent to zero extend short in node "
          ...
          if (performTransformation(s->comp(),
                                    "%sReduced left shift followed by iushr "
                                    "equivalent to zero extend byte in node "
      ...
      //
      if (performTransformation(
              s->comp(), "%sReduced left shift followed by iushr in node "
                         "[" POINTER_PRINTF_FORMAT "] to iand with mask\n",
              s->optDetailString(), node)) {
        TR::Node::recreate(node, TR::iand);
...
TR::Node *lushrSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (right >= left && (right == 56 || right == 48 || right == 32) &&
        performTransformation(s->comp(), "%sshift left followed by shift right "
                                         "%p of %d can become a shift + "
                                         "conversion\n",
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node, ((uint64_t)firstChild->getLongInt()) >>
                                  (secondChild->getInt() & LONG_SHIFT_MASK),
                        s, false /* !anchorChildren */);
    return node;
  }
    ...
    if (constGrandChild->getOpCodeValue() == TR::lconst &&
      ...
      if (subTree->getReferenceCount() == 1) {
        if (opCode == TR::i2l && rightShiftValue == 32) {
          if (performTransformation(s->comp(),
                                    "%sReduced left shift followed by lushr "
                                    "equivalent to zero extend int in node "
      ...
      //
      if (performTransformation(
              s->comp(), "%sReduced left shift followed by lushr in node "
                         "[" POINTER_PRINTF_FORMAT "] to land with mask\n",
              s->optDetailString(), node)) {
...
TR::Node *bushrSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldByteConstant(node, ((uint8_t)firstChild->getByte()) >>
                               (secondChild->getInt() & INT_SHIFT_MASK),
                     s, false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *sushrSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(node, firstChild->getUnsignedShortInt() >>
                                   (secondChild->getInt() & INT_SHIFT_MASK),
                         s, false /* !anchorChildren */);
    return node;
  }
...
TR::Node *iandSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseIntComplement(firstChild) &&
      ...
      if (performTransformation(
              s->comp(), "%sReduced iand with two complemented children in "
                         "node [%s] to complemented ior\n",
              s->optDetailString(), node->getName(s->getDebug()))) {
        TR::Node *orNode =
    ...
    // look for reassociation to fold constants
      ...
      if (lrChild->getOpCodeValue() == TR::iconst) {
        if (secondChildOp == TR::iconst ||
          ...
          if (performTransformation(
                  s->comp(), "%sFound iand of iconst with iand of x and iconst "
                             "in node [%s]\n",
                  s->optDetailString(), node->getName(s->getDebug()))) {
            if (secondChild->getReferenceCount() == 1) {
              secondChild->setInt(secondChild->getInt() & lrChild->getInt());
        ...
        {
          if (performTransformation(
                  s->comp(), "%sFound iand of non-iconst with iand x and "
                             "iconst in node [%s]\n",
                  s->optDetailString(), node->getName(s->getDebug()))) {
            node->setChild(1, lrChild);
            firstChild->setChild(1, secondChild);
...
TR::Node *landSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseLongComplement(firstChild) &&
      ...
      if (performTransformation(s->comp(), "%sReduced land with two "
                                           "complemented children in node "
                                           "[" POINTER_PRINTF_FORMAT
                                           "] to complemented lor\n",
                                s->optDetailString(), node)) {
        TR::Node *orNode =
    ...
    // look for reassociation to fold constants
      ...
      if (lrChild->getOpCodeValue() == TR::lconst) {
        if (secondChildOp == TR::lconst) {
          if (performTransformation(s->comp(), "%sFound land of lconst with "
                                               "land of x and lconst in node "
                                               "[" POINTER_PRINTF_FORMAT "]\n",
                                    s->optDetailString(), node)) {
            if (secondChild->getReferenceCount() == 1) {
              secondChild->setLongInt(secondChild->getLongInt() &
        ...
        {
          if (performTransformation(
                  s->comp(), "%sFound land of non-lconst with land of x and "
                             "lconst in node [" POINTER_PRINTF_FORMAT "]\n",
                  s->optDetailString(), node)) {
            node->setChild(1, lrChild);
            firstChild->setChild(1, secondChild);
...
TR::Node *removeRedundantIntegralOrPattern1(TR::Node *node, TR::Block *block,
  ...
  if (!performTransformation(
          s->comp(), "%sRemove redundant %s 0x%llx [" POINTER_PRINTF_FORMAT
...
// ior constant value and zero extension
// returns the unchanged or new firstChild
//
// after tree
//
...
TR::Node *iorSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseIntComplement(firstChild) &&
        ...
        isBitwiseIntComplement(secondChild)) {
      if (performTransformation(
              s->comp(), "%sReduced ior with two complemented children in node "
                         "[" POINTER_PRINTF_FORMAT "] to complemented iand\n",
              s->optDetailString(), node)) {
        TR::Node *andNode =
      ...
      if (lrChild->getOpCodeValue() == TR::iconst) {
        if (secondChildOp == TR::iconst) {
          ...
          if ((((~and_const) | or_const) == or_const) &&
              performTransformation(s->comp(), "%sFound ior of iconst with "
                                               "iand of x and iconst in node "
                                               "[" POINTER_PRINTF_FORMAT "]\n",
                                    s->optDetailString(), node)) {
            node->setAndIncChild(0, firstChild->getFirstChild());
            firstChild->recursivelyDecReferenceCount();
    ...
    // look for reassociation to fold constants
      ...
      if (lrChild->getOpCodeValue() == TR::iconst) {
        if (secondChildOp == TR::iconst) {
          if (performTransformation(s->comp(), "%sFound ior of iconst with ior "
                                               "of x and iconst in node "
                                               "[" POINTER_PRINTF_FORMAT "]\n",
                                    s->optDetailString(), node)) {
            if (secondChild->getReferenceCount() == 1) {
              secondChild->setInt(secondChild->getInt() | lrChild->getInt());
            ...
            return node;
               ...
               // together
        {
          if (performTransformation(s->comp(), "%sFound ior of non-iconst with "
                                               "ior x and iconst in node "
                                               "[" POINTER_PRINTF_FORMAT "]\n",
                                    s->optDetailString(), node)) {
            node->setChild(1, lrChild);
            firstChild->setChild(1, secondChild);
        ...
        if (firstChild->getSecondChild()->getOpCodeValue() == TR::ior &&
            firstChild->getReferenceCount() == 1 &&
            (byte1 = isUnsafeIorByteChild(
                 firstChild->getSecondChild()->getFirstChild(), TR::bu2i,
                 16777216, s)) &&
            (byte2 = isUnsafeIorByteChild(
                 firstChild->getSecondChild()->getSecondChild(), TR::bu2i,
                 65536, s)) &&
            (byte3 = isUnsafeIorByteChild(firstChild->getFirstChild(), TR::bu2i,
                                          256, s)) &&
          ...
          if ((addr = getUnsafeBaseAddr(byte2, -1)) && addr == byte1 &&
              ...
              performTransformation(
                  s->comp(),
                  "%sconvert ior to iiload node [" POINTER_PRINTF_FORMAT "]\n",
                  s->optDetailString(), node)) {
            TR::Node::recreate(node, TR::iloadi);
            node->setNumChildren(1);
            node->setSymbolReference(
                s->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int32));
  ...
  if (firstChild->getOpCodeValue() == TR::l2i &&
    ...
    if (secondGrandChild->getFirstChild()->getOpCodeValue() == TR::lneg &&
      ...
      if (secondGrandChild->getFirstChild()->getFirstChild() == loadVal &&
          (loadVal->getOpCode().isLoadVar() ||
           loadVal->getOpCode().isLoadReg()) &&
          performTransformation(
              s->comp(),
              "%sTransform ior to lcmp [" POINTER_PRINTF_FORMAT "]\n",
              s->optDetailString(), node)) {
...
TR::Node *lorSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseLongComplement(firstChild) &&
        ...
        isBitwiseLongComplement(secondChild)) {
      if (performTransformation(
              s->comp(), "%sReduced lor with two complemented children in node "
                         "[" POINTER_PRINTF_FORMAT "] to complemented land\n",
              s->optDetailString(), node)) {
        TR::Node *andNode =
    ...
    // look for reassociation to fold constants
      ...
      if (lrChild->getOpCodeValue() == TR::lconst) {
        if (secondChildOp == TR::lconst) {
          if (performTransformation(s->comp(), "%sFound lor of lconst with lor "
                                               "of x and lconst in node "
                                               "[" POINTER_PRINTF_FORMAT "]\n",
                                    s->optDetailString(), node)) {
            if (secondChild->getReferenceCount() == 1) {
            ...
            return node;
               ...
               // together
        {
          if (performTransformation(s->comp(), "%sFound lor of non-lconst with "
                                               "lor of x and lconst in node "
                                               "[" POINTER_PRINTF_FORMAT "]\n",
                                    s->optDetailString(), node)) {
            node->setChild(1, lrChild);
            firstChild->setChild(1, secondChild);
  ...
  // For 32 bit platforms (p and z), evaluator due to historical reasons,
  // evaluator for rotate does not support 64 bit regs on a 32 bit platform by
  // default
...
TR::Node *borSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isAnd() && firstChild->getReferenceCount() == 1 &&
      ...
      (((secondChild->getByte() | firstChild->getSecondChild()->getByte()) &
        0xFF) == 0xFF) &&
      performTransformation(
          s->comp(), "%sReplacing bor [" POINTER_PRINTF_FORMAT
...
TR::Node *ixorSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChildOp == TR::ixor && firstChild->getReferenceCount() == 1) {
    ...
    if (lrChild->getOpCodeValue() == TR::iconst) {
      if (secondChildOp == TR::iconst) {
        if (performTransformation(s->comp(), "%sFound ixor of iconst with ixor "
                                             "of x and iconst in node [%s]\n",
                                  s->optDetailString(),
                                  node->getName(s->getDebug()))) {
          if (secondChild->getReferenceCount() == 1) {
            secondChild->setInt(secondChild->getInt() ^ lrChild->getInt());
          ...
          return node;
             ...
             // together
      {
        if (performTransformation(s->comp(), "%sFound ixor of non-iconst with "
                                             "ixor x and iconst in node [%s]\n",
                                  s->optDetailString(),
                                  node->getName(s->getDebug()))) {
          node->setChild(1, lrChild);
          firstChild->setChild(1, secondChild);
...
TR::Node *lxorSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChildOp == TR::lxor && firstChild->getReferenceCount() == 1) {
    ...
    if (lrChild->getOpCodeValue() == TR::lconst) {
      if (secondChildOp == TR::lconst) {
        if (performTransformation(s->comp(), "%sFound lxor of lconst with lxor "
                                             "of x and lconst in node "
                                             "[" POINTER_PRINTF_FORMAT "]\n",
                                  s->optDetailString(), node)) {
          if (secondChild->getReferenceCount() == 1) {
          ...
          return node;
             ...
             // together
      {
        if (performTransformation(s->comp(), "%sFound lxor of non-lconst with "
                                             "lxor x and lconst in node "
                                             "[" POINTER_PRINTF_FORMAT "]\n",
                                  s->optDetailString(), node)) {
          node->setChild(1, lrChild);
          firstChild->setChild(1, secondChild);
...
TR::Node *i2bSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldByteConstant(node, node->getOpCode().isUnsigned()
  ...
  // 90762 zLinux perlbench perf degradation when we enable cancelling out of
  // i2b and bu2i. todo: investigate why perf degrades when we enable this
  //   if ((result = s->unaryCancelOutWithChild(node, firstChild, s->_curTree,
  //   TR::bu2i)))
  //      return result;
...
TR::Node *i2sSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCodeValue() == TR::ior &&
      ...
      performTransformation(
          s->comp(),
          "%sconvert ior to isload node [" POINTER_PRINTF_FORMAT "]\n",
          s->optDetailString(), node)) {
    TR::Node::recreate(node, TR::sloadi);
    node->setSymbolReference(
        s->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int16));
...
TR::Node *l2aSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if ((firstChild->getOpCodeValue() == TR::lsub ||
    ...
    if (!(addressAddOp == TR::aladd &&
      ...
      node->setNumChildren(2); // upto 2 children do not require node extension.
                               // this should suffice. Any more requires
                               // Node::addChildren
      node->setAndIncChild(
...
TR::Node *d2fSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getReferenceCount() == 2 &&
      convertToSinglePrecisionSQRT(s, firstChild)) {
...
TR::Node *bu2iSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCodeValue() == TR::i2b &&
    ...
    if (((val & 0xFFUL) == 0UL) &&
        performTransformation(
            s->comp(), "%sReplacing bu2i [" POINTER_PRINTF_FORMAT
      ...
      s->anchorNode(firstChild->getFirstChild()->getFirstChild(),
                    s->_curTree); // only really need to anchor this child,
                                  // everything else is guaranteed to not be a
                                  // load
...
TR::Node *bu2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCodeValue() == TR::i2b &&
    ...
    if (((val & 0xFFUL) == 0UL) &&
        performTransformation(
            s->comp(), "%sReplacing bu2l [" POINTER_PRINTF_FORMAT
                       "] with i2b child [" POINTER_PRINTF_FORMAT
                       "] of iand [" POINTER_PRINTF_FORMAT
                       "] with mask 0, with iconst 0\n",
            s->optDetailString(), node, firstChild, firstChild->getFirstChild(),
...
TR::Node *ificmpeqSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (firstChild->getOpCode().isRightShift() && // First child is a right shift
      ...
      (firstChild->getOpCodeValue() == TR::iushr ||
       firstChild->getFirstChild()->isNonNegative()) && // Either a logical
                                                        // shift or a positive
                                                        // first child to
                                                        // guaruntee zero-extend
  ...
  if (firstChild->getOpCode().isBooleanCompare() &&
      !firstChild->getOpCode().isLongCompare() &&
      (secondChild->getOpCode().isLoadConst()) &&
      ...
      performTransformation(s->comp(), "%sChanging if opcode %p because first "
                                       "child %p is a comparison opcode\n",
                            s->optDetailString(), node, firstChild)) {
  ...
  if (node->getOpCode().isBranch()) {
    ...
    if (firstChild->getOpCode().isDouble() &&
      ...
      if (firstChild->getOpCodeValue() == TR::f2d &&
          ...
          performTransformation(s->comp(), "%sDemoted double compare of "
                                           "TR::f2d to dconst to float compare "
                                           "at node [%p]\n",
                                s->optDetailString(), node)) {
      ...
      if (intValueInFloatRange(node, fValue) &&
          ...
          performTransformation(s->comp(), "%sDemoted float compare of TR::i2f "
                                           "to fconst to int compare at node "
                                           "[%p]\n",
                                s->optDetailString(), node)) {
        ...
        if (op != TR::BadILOp) {
          ...
          secondChild->recursivelyDecReferenceCount();
                 ...
                 performTransformation(s->comp(), "%sDemoted float compare of "
                                                  "TR::l2f to fconst to long "
                                                  "compare at node [%p]\n",
                                       s->optDetailString(), node)) {
...
TR::Node *ifCmpWithEqualitySimplifier(TR::Node *node, TR::Block *block,
  ...
  if (!disableFoldIfSet && node->getOpCodeValue() == TR::ifbcmpeq) {
    if (secondChild->getOpCodeValue() == TR::bconst &&
      ...
      if (op != TR::BadILOp &&
          performTransformation(s->comp(), "%sFolding ifbcmpeq of bconst 0 to "
                                           "boolean compare at node "
                                           "[" POINTER_PRINTF_FORMAT
...
TR::Node *icmpeqSimplifier(TR::Node *node, TR::Block *block,
  ...
  if ((firstChild->getOpCode().isBooleanCompare() &&
      ...
      ((secondChild->getOpCode().isLoadConst()) &&
       secondChild->getInt() == 0) &&
      performTransformation(s->comp(), "%sChanging icmpeq opcode %p because "
                                       "first child %p is an int compare\n",
                            s->optDetailString(), node, firstChild)) {
    TR::Node::recreate(node,
...
TR::Node *lucmpltSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldIntConstant(node, firstChild->getUnsignedLongInt() <
...
TR::Node *sucmpeqSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldIntConstant(node, firstChild->getConst<uint16_t>() ==
...
TR::Node *endBlockSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (cfg) {
    ...
    if (rootStructure != NULL) {
      ...
      // This is inconsistent with what we do for CFG in the following, it's
      // better for mergeBlock to merge nextBlock to block, this is a TODO
  ...
  // how it is done in other block merge operations (in particular, goto
  // elimination) so that
  // frequency information attached to bytecode isn't lost in the process of
  // block merging.
  // But Wcode compiler which generates asm file will need the line number info
  // to be correct
  // for BBstart node of Prolog so no copy.
  //
  // dont copy the bytecodeinfo for catch blocks. liveMonitor analysis relies on
  // accurate
  // callerIndex information to propagate locked objects. copying can result in
  // the wrong
...
TR::Node *switchSimplifier(TR::Node *node, TR::Block *block, bool isTableSwitch,
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    ...
    // parent), we can eliminate the
    // shift by performing the reverse shift on all the case values (eg. instead
    // of left-shifting the first child by two,
    ...
    if (node->getOpCode().getOpCodeValue() == TR::lookup && NULL != shiftNode &&
      ...
      if (NULL != shiftAmtNode && shiftAmtNode->getOpCode().isLoadConst()) {
        ...
        // of either the selector node or the
        // case constants must be 0. For the constants, we can check the bits to
        // be shifted out at compile time. For
        // the shift node, which is non-constant, the only way we know those
        // bits are 0 is if they are explicitly
        // zeroed with an AND node.
        //
        // Example: if the selector is 3 (binary 11) and we shift it right 1,
        // and have case values 0 and 1, we'll match 1 (since 11b >> 1 is 1b)
        // If we shift 0 and 1 left 1 bit, we'll compare 11b to 00b and 10b, and
        // not have a match.
  ...
  if (convertToGoto) {
         ...
         childIndex++) // loop from default to last case
    {
      ...
      if (destInst->getOpCodeValue() == TR::Goto) {
        ...
        if (!performTransformation(
                s->comp(), "%sRedirecting switch [" POINTER_PRINTF_FORMAT
...
static bool boundGELength(TR::Node *boundChild, TR::Node *lengthChild,
                          TR::Simplifier *s) {
  ...
  if (boundOp == TR::iadd) {
    ...
    if (first->getOpCodeValue() == TR::aloadi &&
      ...
      if ((boundSymRef->getSymbol()->getRecognizedField() ==
               TR::Symbol::Java_lang_String_value &&
           lengthSymRef->getSymbol()->getRecognizedField() ==
               TR::Symbol::Java_lang_String_count) ||
          (boundSymRef->getSymbol()->getRecognizedField() ==
               TR::Symbol::Java_lang_StringBuffer_value &&
           lengthSymRef->getSymbol()->getRecognizedField() ==
               TR::Symbol::Java_lang_StringBuffer_count) ||
          (boundSymRef->getSymbol()->getRecognizedField() ==
               TR::Symbol::Java_lang_StringBuilder_value &&
           lengthSymRef->getSymbol()->getRecognizedField() ==
               TR::Symbol::Java_lang_StringBuilder_count)) {
        return true;
      }
    }
  }
...
TR::Node *caddSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldCharConstant(node, firstChild->getConst<uint16_t>() +
                               secondChild->getConst<uint16_t>(),
                     s, false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *imulhSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    int64_t src2 = secondChild->getInt();
    if (src2 == 0 && performTransformation(
                         s->comp(), "%ssecond child [%p] of node [%p] is 0, "
                                    "setting the result of imulh to 0\n",
      ...
      if (firstChild->isNegative() &&
          performTransformation(s->comp(), "%sfirst child [%p] of node [%p] is "
                                           "negative, setting the result of "
                                           "imulh to -1\n",
        ...
        node->setInt(-1);
                 ...
                 performTransformation(
                     s->comp(), "%sfirst child [%p] of node [%p] is "
                                "non-negative, setting the result of imulh to "
                                "0\n",
                     s->optDetailString(), firstChild, node)) {
        s->prepareToReplaceNode(node);
        TR::Node::recreate(node, TR::iconst);
    ...
    //  /    \   =>      /  \
      // src1  src2      src1 32-n
             ...
             performTransformation(s->comp(), "%ssecond child [%p] of node "
                                              "[%p] is 2's power, converting "
                                              "imulh to ishr\n",
                                   s->optDetailString(), secondChild, node)) {
      ...
      TR::Node::recreate(node, TR::ishr);
      TR::Node *newSecondChild =
          TR::Node::create(TR::iconst, 0); // Create a new node because changing
                                           // the value of the node might affect
                                           // trees referencing this node
...
TR::Node *bndchkSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (removableBoundCheck ||
    ...
    if (removableBoundCheck) {
      ...
      if (performTransformation(
              s->comp(), "%sRemoved bndchk with irem with arraylength as "
  ...
  // optimization.
  // In this case we are guaranteed that neither of the multiplications was
  // going to
...
TR::Node *arraycopybndchkSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (rhsChild->getOpCodeValue() == TR::iadd) {
    ...
    if (lengthChild == boundChild ||
        boundGELength(boundChild, lengthChild, s)) {
      ...
      if (indexChild->getOpCodeValue() == TR::isub) {
        ...
        if ((indexChild->getFirstChild() == boundChild ||
          ...
          if (performTransformation(
            ...
            return NULL;
                   ...
                   indexChild->getSecondChild()->getOpCodeValue() ==
                       TR::iconst &&
                   indexChild->getSecondChild()->getInt() == -1) {
          TR::ILOpCodes boundOp = boundChild->getOpCodeValue();
          ...
          if (boundChild->getOpCode().isArrayLength() &&
            ...
            TR::SymbolReference *boundSymRef =
                boundChild->getFirstChild()->getSymbolReference();
#ifdef J9_PROJECT_SPECIFIC
            if (boundSymRef->getSymbol()->getRecognizedField() ==
                TR::Symbol::Java_lang_String_value) {
        ...
        if (boundChild->getOpCodeValue() == TR::iadd &&
          ...
          if (performTransformation(
                  s->comp(), "%sRemoved arraycopy bndchk with arrayLength "
                             "index in node [%s] when bound is sum of length "
                             "and index arraylengths\n",
                  s->optDetailString(), node->getName(s->getDebug()))) {
            s->removeNode(node, s->_curTree);
            return NULL;
          }
  ...
  if (boundChild->getOpCode().isLoadConst() &&
    ...
    if (elementSize > 0 &&
        !TR::Compiler->om.isDiscontiguousArray(boundChild->getInt(),
                                               elementSize) &&
        performTransformation(
            s->comp(), "%sRemoving spine check because constant arraylength is "
                       "contiguous in node [%s]\n",
            s->optDetailString(), node->getName(s->getDebug()))) {
...
TR::Node *arraysetSimplifier(TR::Node *node, TR::Block *block,
  ...
  if (fill->getOpCode().isLoadConst() && fill->getDataType() == TR::Int64) {
    ...
    if ((fillVal & 0x0FFFFFFFFL) == ((fillVal >> 32) & 0x0FFFFFFFFL) &&
        performTransformation(s->comp(), "%sTransform large fill arrayset to "
                                         "4byte fill arrayset "
                                         "[" POINTER_PRINTF_FORMAT "]\n",
                              s->optDetailString(), node)) {
    ...
    if (firstChildOp == TR::newarray || firstChildOp == TR::anewarray) {
      ...
      if (arraySize->getOpCode().isLoadConst() && elementSize > 0 &&
          !TR::Compiler->om.isDiscontiguousArray(arraySize->getInt(),
                                                 elementSize) &&
          performTransformation(
              s->comp(), "%sReducing contiguous arraylength of newarray or "
                         "anewarray in node [%s]\n",
              s->optDetailString(), node->getName(s->getDebug()))) {
