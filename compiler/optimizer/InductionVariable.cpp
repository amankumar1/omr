...
TR_LoopStrider::detectCanonicalizedPredictableLoops(TR_Structure *loopStructure,
  ...
  if (loopInvariantBlock) {
    ...
    _storeTreesList =
        (List<TR_StoreTreeInfo> **)trMemory()->allocateStackMemory(
            symRefCount * sizeof(List<TR_StoreTreeInfo> *));
    ...
    _reassociatedAutos =
        (TR::SymbolReference **)trMemory()->allocateStackMemory(
            symRefCount * sizeof(TR::SymbolReference *));
    memset(_reassociatedAutos, 0, symRefCount * sizeof(TR::SymbolReference *));
    _hoistedAutos = (SymRefPair **)trMemory()->allocateStackMemory(
        symRefCount * sizeof(SymRefPair *));
    ...
    // the loop pre-header.
    // on some platforms (z in particular), a long sequence of stores results in
    // "VS Tag Not ready"
    // stalls (aka store buffer stalls). This variable controls whether any
    // induction variables are
    ...
    if (isPredictableLoop > 0) {
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        //       variable
        //       as we might produce incorrect trees (wanted old value but will
        //       get new inced/deced value) .
        //
        ...
        if (storeInRequiredForm) {
          ...
          if (storeInRequiredForm && _storeTreesList) {
            List<TR_StoreTreeInfo> *storeTreesList =
                _storeTreesList[nextInductionVariableNumber];
            ListIterator<TR_StoreTreeInfo> si(storeTreesList);
            TR_StoreTreeInfo *storeTree;
            for (storeTree = si.getCurrent(); storeTree != NULL;
                 storeTree = si.getNext()) {
          ...
          if (replaceAllInductionVariableComputations(
            ...
            for (; k < _nextExpression; k++) {
              ...
              if (performTransformation(
                      comp(), "%s Adding auto %d (created for striding) as a "
                              "global register "
                              "candidate in loop %d\n",
                      OPT_DETAILS, newInductionCandidate->getSymbolReference()
                                       ->getReferenceNumber(),
                      loopStructure->getNumber()))
              ...
              // immediately
              // after the increment/decrement of the original induction
              // variable
              //
            ...
            if (isMulTermConst(bestCandidate) && isLoopTestInductionVariable &&
                 ...
                 * To determine (1), we need the range constraints on stride to
                 * test
                 * if it only contains non-negatives or non-positives.
                 * Reject if constraints are unknown due to non-const additive
                 * term.
                 *                                                         */
                !(0 && // no-strict IV
            ...
            TR_RegisterCandidate *inductionCandidate =
                comp()->getGlobalRegisterCandidates()->findOrCreate(
                    inductionVarSymRef);
            if (performTransformation(comp(), "%s Adding auto %d as a global "
      ...
      if (reassociatedAuto) {
        ...
        if (usingAladd) {
          ...
          arrayRefNode = TR::Node::create(
              TR::aladd, 2, TR::Node::createWithSymRef(byteCodeInfoNode,
                                                       TR::aload, 0, origAuto),
              constantNode);
        } else
          arrayRefNode = TR::Node::create(
              TR::aiadd, 2, TR::Node::createWithSymRef(byteCodeInfoNode,
                                                       TR::aload, 0, origAuto),
        ...
        while (symRefPair) {
          ...
          if (symRefPair->_isConst) {
            ...
            if (usingAladd) {
              ...
              arrayRefNode = TR::Node::create(
                  TR::aladd, 2, TR::Node::createWithSymRef(
                                    byteCodeInfoNode, TR::aload, 0, origAuto),
                  constantNode);
            } else
              arrayRefNode = TR::Node::create(
                  TR::aiadd, 2, TR::Node::createWithSymRef(
                                    byteCodeInfoNode, TR::aload, 0, origAuto),
          ...
          dumpOptDetails(comp(), "\nO^O INDUCTION VARIABLE ANALYSIS: Induction "
                                 "variable analysis inserted invariant "
                                 "initialization tree : %p for new symRef "
                                 "#%d\n",
          ...
          //
          if (performTransformation(
                  comp(), "%s Adding auto %d (created for reassociation) as a "
                          "global register candidate in loop %d\n",
                  OPT_DETAILS, newInductionCandidate->getSymbolReference()
                                   ->getReferenceNumber(),
                  loopStructure->getNumber()))
...
void TR_LoopStrider::findOrCreateStoreInfo(TR::TreeTop *tree, int32_t i) {
  ...
  _storeTreesList[i]->add(new (trStackMemory()) TR_StoreTreeInfo(
...
void TR_LoopStrider::changeLoopCondition(
  ...
  if (!usingAladd && (_linearEquations[bestCandidate][4] > -1) &&
    ...
    // below
    // and then we would be in a situation where we would have to create a
    // TR::aladd node
  ...
  if (firstOperand->getDataType() == TR::Address) {
    ...
    TR::Node::recreate(loopTestNode,
                       TR::ILOpCode::convertSignedCmpToUnsignedCmp(
                           loopTestNode->getOpCodeValue()));
  }
  ...
  if ((replacingNode->getOpCodeValue() == TR::iload ||
    ...
    if (canHoistAdditiveTerm) {
      ...
      dumpOptDetails(comp(), "\nO^O INDUCTION VARIABLE ANALYSIS: Induction "
                             ...
                             "%p for new loop invariant symRef #%d\n",
                     newStoreTreeTop->getNode(),
...
bool TR_LoopStrider::examineTreeForInductionVariableUse(
  ...
  if ((node->getOpCodeValue() == TR::iadd ||
    ...
    if (isExprLoopInvariant(node->getSecondChild())) {
      ...
      if ((node->getFirstChild()->getOpCodeValue() == TR::imul ||
        ...
        if (!mulTerm || !isExprLoopInvariant(mulTerm) ||
            !mulTerm->getType().isIntegral() || // we were checking for signed
                                                // here -- i made the test a bit
                                                // weaker, but I don't think we
            ...
            *newSymbolReference = comp()->getSymRefTab()->createTemporary(
            ...
            if (isInternalPointer && !pinningArrayPointer &&
              ...
              dumpOptDetails(comp(), "\nO^O INDUCTION VARIABLE ANALYSIS: "
                                     "Induction variable analysis inserted "
                                     "initialization tree : %p for new symRef "
                                     "#%d\n",
    ...
    if (!mulTerm || !isExprLoopInvariant(mulTerm) ||
        !mulTerm->getType().isIntegral() || // I changed isSignedInt->isIntegral
                                            // -- but I don't think unsigned
                                            // types ever reach here
        (!mulTerm->getOpCode().isLoadConst() && shift))
      replacingNode = NULL;
    ...
    if (replacingNode) {
      *newSymbolReference = NULL;
      ...
      if (canCreateNewSymRef) {
        ...
        *newSymbolReference = comp()->getSymRefTab()->createTemporary(
        ...
        if (isInternalPointer && !pinningArrayPointer &&
          ...
          placeStore(newStore, loopInvariantBlock);
          dumpOptDetails(comp(), "\nO^O INDUCTION VARIABLE ANALYSIS: Induction "
                                 "variable analysis inserted initialization "
      ...
      dumpOptDetails(
          comp(), "O^O INDUCTION VARIABLE ANALYSIS: (mul/shl) Replaced node : "
...
TR::Node *TR_LoopStrider::placeNewInductionVariableIncrementTree(
  ...
  dumpOptDetails(
      comp(), "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis "
              "inserted loop incremental step tree : %p for new symRef #%d\n",
      newStoreTreeTop->getNode(), newSymbolReference->getReferenceNumber());
  return newStore;
}
...
bool TR_LoopStrider::identifyExpressionLinearInInductionVariable(
  ...
  if (node->getOpCode().isLoadVar()) {
    ...
    {
      if (usingAladd &&
          node->getSecondChild()->getOpCodeValue() ==
              TR::i2l) // handle both TR::xmul and TR::xshl
...
TR::Node *TR_LoopStrider::setUsesLoadUsedInLoopIncrement(TR::Node *node,
  ...
  if (_storeTreesList) {
    List<TR_StoreTreeInfo> *storeTreesList =
        _storeTreesList[_loopDrivingInductionVar];
    ListIterator<TR_StoreTreeInfo> si(storeTreesList);
    TR_StoreTreeInfo *storeTree;
    for (storeTree = si.getCurrent(); storeTree != NULL;
         storeTree = si.getNext()) {
    ...
    if (node->getSecondChild()->getOpCodeValue() == TR::iconst ||
      ...
      if ((isInternalPointer &&
          ...
          (!comp()->cg()->canBeAffectedByStoreTagStalls() ||
           _numInternalPointerOrPinningArrayTempsInitialized <
               MAX_INTERNAL_POINTER_AUTOS_INITIALIZED) &&
          performTransformation(comp(), "%s Replacing reassociated internal "
                                        "pointer based on symRef #%d\n",
...
bool TR_LoopStrider::isStoreInRequiredForm(TR::Node *storeNode,
  ...
  if (_storeTreesList) {
    ...
    for (storeTree = si.getCurrent(); storeTree != NULL;
      ...
      // storeNode, storeTree->_tt->getNode());
...
static bool isLoopRelativelyFrequent(TR_Structure *loop, TR::Block *preheader) {
  ...
  if (estimatedAverageIters < minIters) {
    dumpOptDetails(TR::comp(), "[Sign-Extn] skip loop %d due to estimated "
                               "average iterations %f < %f\n",
...
void TR_LoopStrider::detectLoopsForIndVarConversion(
  ...
  if (loopInvariantBlock) {
    ...
    if (!stressMode &&
        !isLoopRelativelyFrequent(loopStructure,
...
void TR_LoopStrider::walkTreesAndFixUseDefs(
  ...
  replaceLoadsInStructure(loopStructure, _loopDrivingInductionVar,
...
void TR_LoopStrider::eliminateSignExtensionsInSubtree(
  ...
  for (int i = 0; i < parent->getNumChildren(); i++) {
    ...
    if (!performTransformation(
            comp(), "%s [Sign-Extn] Replacing occurrence of n%un i2l with n%un "
                    "as %dth child of n%un\n",
            optDetailString(), node->getGlobalIndex(),
            extended->getGlobalIndex(), i, parent->getGlobalIndex()))
      continue;
 ...
 * \see transmuteDescendantsIntoTruncations(TR::Node*, TR::Node*)
 */
TR::Node *TR_LoopStrider::signExtend(TR::Node *node, TR::NodeChecklist &exLoads,
                                     NodeMap &memo) {
  ...
  if (ext != NULL) {
    ...
    if (trace())
      traceMsg(comp(), "[Sign-Extn] sign-extended n%un %s into n%un %s\n",
               ...
               ext->getGlobalIndex(), ext->getOpCode().getName());
  }
...
TR::Node *TR_LoopStrider::signExtendBinOp(TR::ILOpCodes op64, TR::Node *node,
                                          TR::NodeChecklist &exLoads,
                                          NodeMap &memo) {
...
void TR_InductionVariableAnalysis::gatherCandidates(TR_Structure *s,
  ...
  if (s->asRegion()) {
    ...
    if (!region->isAcyclic()) {
      ...
      if (trace()) {
        traceMsg(comp(), "All Defs inside cyclic region %d: ",
                 region->getNumber());
...
// method recognizes arithmetic
// progressions of all kinds and Geometric progressions where the multiplicative
// constant (r) is a power of 2 (shift).
// If the storeRef argument has a non-null value, this function will find the
// progression taking place on that
// symref only.
//
// Returns true if it recognizes a progression taking place.  Returns the symbol
// reference of the induction
// variable in ref, the kind of progression in prog, and the progression
// constant in incr.
//
...
// The multiplicative constant for the series (r) = 2^(incr).  Naturally this
// means
  ...
  while (it.hasMoreElements()) {
    ...
    if (info->isUnknownValue()) {
      ...
      if (info->getDelta() == 0) {
        ...
        if (trace())
          traceMsg(comp(), "====> Found basic linear induction variable symRef "
      ...
      if (trace())
        traceMsg(comp(), "====> Found basic geometric induction variable "
                         "symRef #%d[%p] with increment %d\n",
                 refNum, symRef, info->getDelta());
...
static void appendPredecessors(TR::list<TR::CFGEdge *> &dest,
                               TR::Block *block) {
  ...
  while (!workList.empty()) {
    ...
    for (TR::TreeTop *tt = curBlock->getFirstRealTreeTop();
      ...
      if (curNode->getOpCode().isStoreDirect() &&
        ...
        if (trace())
          traceMsg(comp(), "\t\tFound store %p of symRef %p in block %d, which "
                           "is not a loop test block\n",
...
bool TR_InductionVariableAnalysis::analyzeExitEdges(
  ...
  for (edge = eit.getFirst(); edge; edge = eit.getNext()) {
    ...
    if (!node->getFirstChild()->getType().isIntegral() &&
        !node->getOpCode().isJumpWithMultipleTargets()) {
      if (trace())
        traceMsg(comp(), "\tReject - Branch is not integral\n");
      return false;
    }
    ...
    if (!node->getOpCode().isJumpWithMultipleTargets()) {
      ...
      // 'controlling branch to exit' in terms of the temp induction variable,
      // so we can continue
  ...
  if (valueNode->getOpCode().isLoadVarDirect() &&
      ...
      if (curNode->getOpCode().isStoreDirect() &&
        ...
        if (trace())
          traceMsg(comp(), "\tFound store %p of symRef %p\n", curNode,
                   valueNode->getSymbolReference()->getSymbol());
        if (curNode->containsNode(
                valueNode, comp()->getVisitCount())) // valueNode is used in the
                                                     // store subtree
          usesUnchangedValueInLoopTest = true;
        else {
          if (trace())
            traceMsg(comp(),
                     "\tRejected - tested value commoned across a store %p\n",
                     tt->getNode());
          return false;
        }
      }
    ...
    if (usesUnchangedValueInLoopTest) {
      if (!isIVUnchangedInLoop(loop, controllingBranch->getEnclosingBlock(),
                               valueNode)) {
        if (trace())
          traceMsg(comp(), "\tReject - IV node has been changed in one of the "
                           "blocks that is not part of the loop test in the "
                           "loop\n");
        return false;
      }
    }
    ...
    for (tt = controllingBranch;; tt = tt->getPrevTreeTop()) {
      ...
      if (curNode->getOpCode().isStoreDirect() &&
        ...
        if (curNode->getFirstChild() != valueNode) {
          if (trace())
            traceMsg(comp(), "\tRejected - exit value %p maybe not same as "
                             "tested value %p\n",
                     curNode, valueNode);
...
TR::Node *TR_InductionVariableAnalysis::getEntryValue(
  ...
  for (TR::TreeTop *tt = block->getLastRealTreeTop(); tt != entryTree;
    ...
    if (node->getOpCode().isStoreDirect()) {
      ...
      // if you find somewhere a call like  foo(&num);  then this kills the num,
      // and we can no longer reason about its value.
...
TR_PrimaryInductionVariable::TR_PrimaryInductionVariable(
  ...
  if (trace ||
      (comp()->getDebug() &&
       comp()->getOptions()->getAnyOption(TR_TraceAll))) // always dump the info
  {
    comp()->incVisitCount();
...
TR::Node *TR_PrimaryInductionVariable::getExitValue() {
  ...
  // computation,
  // the loops should always be well formed in which case we can return the
  // exitBound (or
...
void TR_IVTypeTransformer::changeIVTypeFromAddrToInt(
  ...
  if (!performTransformation(cm, "%s Initializing base and int index in "
                                 "preheader block_%i of loop %i\n",
