...
bool TR_LocalReordering::insertEarlierIfPossible(TR::TreeTop *storeTree,
  ...
  while (currentTree != treeTop) {
    ...
    if (!stopHere) {
      if (currentNode->getOpCode().isCheckCast()) // Moving a store earlier than
                                                  // a checkcast usually results
                                                  // in suboptimal type info
                                                  // (because of the way store
                                                  // constraints work, they only
                                                  // pick up the type info when
    ...
    if (stopHere) {
      if (performTransformation(
              comp(), "\n%sInserting Definition @ 1 : [%p] between %p and %p "
                      "(earlier between %p and %p)\n",
              OPT_DETAILS, storeTree->getNode(), currentTree->getNode(),
  ...
  if (!foundInsertionSpot &&
      performTransformation(
          comp(), "\n%sInserting Definition @ 2 : [%p] between %p and %p "
                  "(earlier between %p and %p)\n",
          OPT_DETAILS, storeTree->getNode(), currentTree->getNode(),
          currentTree->getNextTreeTop()->getNode(),
          storeTree->getPrevTreeTop()->getNode(),
...
void TR_LocalReordering::insertDefinitionBetween(TR::TreeTop *treeTop,
  ...
  while (!(currentTree == exitTree)) {
    ...
    if (isAnySymInDefinedOrUsedBy(currentNode, visitCount1)) {
      if (performTransformation(
              comp(), "\n%sInserting Definition : [%p] between %p and %p "
                      "(earlier between %p and %p)\n",
              OPT_DETAILS, originalNode,
              currentTree->getPrevTreeTop()->getNode(), currentTree->getNode(),
...
bool TR_LocalReordering::isAnySymInDefinedOrUsedBy(TR::Node *node,
  ...
  if (node->canCauseGC() && _seenUnpinnedInternalPointer) {
    dumpOptDetails(comp(), "\n%sisAnySymInDefinedOrUsedBy : found unpinned "
                           "internal pointer at GC point %p\n",
                   OPT_DETAILS, node);
    return true;
  }
...
bool TR_LocalReordering::isAnySymInDefinedBy(TR::Node *node,
  ...
  if (node->canCauseGC() && _seenUnpinnedInternalPointer) {
    dumpOptDetails(comp(), "\n%sisAnySymInDefinedBy : found unpinned internal "
