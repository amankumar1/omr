...
bool TR_LRAddressTree::checkAiadd(TR::Node *aiaddNode, int32_t elementSize) {
  ...
  if (_multiplyNode.isNull() &&
    ...
    dumpOptDetails(comp(), "checkAiadd: sub-tree does not have induction "
                           "variable change consistent with increment of "
                           "multiplier (%d %d)\n",
...
bool TR_Arraycopy::checkArrayStore(TR::Node *storeNode) {
  if (!storeNode->getOpCode().isStoreIndirect() &&
      !(storeNode->getOpCodeValue() == TR::treetop &&
        storeNode->getFirstChild()->getOpCodeValue() == TR::wrtbari)) {
    dumpOptDetails(
        comp(),
...
bool TR_ByteToCharArraycopy::checkByteLoads(TR::Node *loadNodes) {
  ...
  if (getLowLoadAddress()->getOffset() !=
    ...
    dumpOptDetails(comp(), "checkByteLoads: second offset is not one greater "
                           "than first offset (%d %d)\n",
  ...
  if (hbs || lbs) {
    ...
    if (!hbs || !lbs || hbs != lbs) {
      dumpOptDetails(comp(), "checkByteLoads: at least one tree has a base "
                             "sym, but both trees do not have the same sym (%p "
                             "%p)\n",
                     lbs, hbs);
      return false;
    }
...
bool TR_CharToByteArraycopy::checkArrayStores(TR::Node *origHighStoreNode,
  ...
  if (highStoreNode->getOpCodeValue() != TR::bstorei) {
    dumpOptDetails(comp(), "checkArrayStores: char to byte arraycopy high "
                           "arraystore tree does not have an indirect store as "
                           "root\n");
    return false;
  }
  if (lowStoreNode->getOpCodeValue() != TR::bstorei) {
    dumpOptDetails(comp(), "checkArrayStores: char to byte arraycopy low "
                           "arraystore tree does not have an indirect store as "
                           "root\n");
    return false;
  }
  ...
  if ((int32_t)getLowStoreAddress()->getOffset() !=
    ...
    dumpOptDetails(comp(), "checkArrayStores: second offset is not 1 greater "
                           "than first offset (%d %d)\n",
...
// a
// variable and that the variable being incremented is the induction variable
// for
// the loop.
//
// istore <ind var>
...
bool TR_ArrayLoop::checkLoopCmp(TR::Node *loopCmpNode,
  ...
  if (compareOp == TR::ificmpeq || compareOp == TR::ificmpge ||
      compareOp == TR::ificmple || compareOp == TR::ifiucmpeq ||
...
bool TR_LoopReducer::generateArraycopy(TR_InductionVariable *indVar,
                                       TR::Block *loopHeader) {
  // for aladds
  if (!comp()->cg()->getSupportsArrayCopy() &&
  ...
  if (storeNode->getType().isBCD() &&
    ...
    dumpOptDetails(comp(), "arraycopy storeNode %p is a BCD type (%s) and the "
                           "storeSize (%d) is not 1,2,4 or 8 so do not reduce "
                           "arraycopy\n",
  ...
  // istore node to store the induction variable
  // The parameters will be:
  //   The address to store into (the store aiadd tree - with the iload variable
  //   changed if negative increment)
  //   The address to load from  (the load aiadd tree - with the iload variable
  //   changed if negative increment)
  //   The number of bytes to copy(induction variable initial value - comparison
  //   value) * stride
  //
  // Tree-Top
  // ArrayCopy call
  ...
  //loadAddr->getFirstChild()->getSymbol()->getRegisterMappedSymbol()))
  {
    if (comp()->cg()->getSupportsPrimitiveArrayCopy())
      arraycopy->setBackwardArrayCopy(
          true); /* bit available only to primitive arraycopy */
  } else {
  ...
  if (!comp()
           ->cg()
           ->getSupportsPrimitiveArrayCopy()) /* bits only available for
                                                 non-primitive arraycopy */
  {
...
bool TR_LoopReducer::generateArrayset(TR_InductionVariable *indVar,
  ...
  //   The number of elements to set to the value (induction variable initial
...
bool TR_Arraytranslate::checkStore(TR::Node *storeNode) {
  ...
  if (hasBranch()) {
    ...
    if (shrinkNode->getOpCodeValue() != TR::i2s &&
      ...
      dumpOptDetails(comp(), "...store tree does not have "
                             "i2c/i2b/cconst/bconst - no arraytranslate "
                             "reduction\n");
      return false;
    }
    ...
    if (shrinkNode->getOpCodeValue() == TR::i2b ||
      ...
      if (iloadNode->getSymbolReference() !=
        ...
        dumpOptDetails(comp(), "...store tree reference does not match load "
                               "tree reference - no arraytranslate "
                               "reduction\n");
        return false;
      }
    }
...
bool TR_Arraytranslate::checkBreak(TR::Block *breakBlock, TR::Node *breakNode,
  ...
  if (valNode->getOpCodeValue() != TR::iconst ||
    ...
    //      ranges such that we can
    //      add or subtract 1 from the value and still represent it in 2**16
    //      bits for a default value
...
bool TR_Arraytranslate::checkMatIndVarStore(TR::Node *matIndVarStoreNode,
  ...
  if (!matIndVarStoreNode->getOpCode().isStoreDirect()) {
    dumpOptDetails(comp(), "materialized induction variable tree %p does not "
                           "have a direct store as root\n",
                   matIndVarStoreNode);
    return false;
  }
  ...
  if (storeFirstChild->getOpCodeValue() != TR::iadd &&
    ...
    dumpOptDetails(comp(), "first child %p of materialized induction variable "
                           "store is not TR::iadd/TR::isub\n",
                   storeFirstChild);
    return false;
  }
  ...
  if (indVarLoad->getOpCodeValue() != TR::iload ||
    ...
    dumpOptDetails(comp(), "primary iv in the materialized tree is not "
                           "TR::iload %p or second child is not TR::iconst "
                           "%p\n",
...
bool TR_LoopReducer::generateArraycmp(TR_RegionStructure *whileLoop,
  ...
  if (comp()
    ...
    dumpOptDetails(comp(),
                   "arraycmp not safe to perform when NOTEMPS enabled\n");
    return false;
  }
  ...
  // variable
  // other than for comparison to the 'final' value (which is in the known
  // blocks I've examined)
  // in any successor block to the loop. If the induction variable is written to
  // first, then
  // I can ignore that path.
  // The reason this is important is because the reduced arraycmp will not
  // calculate the
  // induction variable's final value correctly if it terminates early, only if
  // it runs to
  ...
  while (!queue.isEmpty()) {
    ...
    if (block != _cfg->getEnd() && block != branchBlock &&
      ...
      if (blockState == transformerReadFirst) {
        dumpOptDetails(comp(), "induction variable is read before write in "
                               "block_%d after compare loop - no arraycmp "
                               "reduction\n",
                       block->getNumber());
        return false;
      }
      ...
      if (blockState == transformerWrittenFirst) {
        dumpOptDetails(comp(), "induction variable is written before read in "
                               "block_%d after compare loop - pruning "
                               "successor\n",
                       block->getNumber());
        continue;
      }
  ...
  //     be used in the arraycmp)
  //     (the istore must be after the conditional branch since it is used in
  //     exactly one subsequent compare)
  //
  // The parameters to ArrayCmp will be:
  //   The first address to compare to (the aiadd tree - with the iload variable
  //   changed if negative increment)
  //   The second address to compare to (the aiadd tree - this may reference the
  //   first iload in which case it can't be updated)
  //   The number of elements to compare (induction variable initial value -
  //   comparison value) / increment / multiplier
  //
  // Tree-Top
  // ArrayCmp call
...
bool TR_LoopReducer::generateArraytranslate(TR_RegionStructure *whileLoop,
  ...
  // instruction.
  // Note that you will probably need to run with recompilation for all the
  // bounds checks
  // to get eliminated - the profile-directed feedback is required to eliminate
  // the
  ...
  if (comp()
      ...
      comp()->getJittedMethodSymbol()->isNoTemps()) {
    dumpOptDetails(comp(),
                   "arraytranslate not safe to perform when NOTEMPS enabled\n");
    return false;
  }
  ...
  if (((firstNum == 3 || firstNum == 4 || firstNum == 5) && secondNum == 0) ||
    ...
    dumpOptDetails(comp(), "Loop has wrong number of tree tops (%d,%d,%d,%d) - "
                           "no arraytranslate reduction\n",
  ...
  if (arraytranslateLoop.compilerGeneratedTable() && !hasBranch && hasBreak) {
    dumpOptDetails(comp(), "Attempt to reduce loop marked with no branch but "
                           "also comp gen'ed table - no arraytranslate "
                           "reduction\n");
    return false;
  }

  // if hasBreak and hasBranch are both false, the loop is a simple byte->char
  // or char->byte conversion
  // the termChar can be ignored by the instruction if only the hardware
  // supports it. if there is no
  // support for ignoring the termChar, then char->byte conversions cannot be
  // reduced
  // (since the termChar is treated as byte from 0x00 - 0xFF, this effectively
  // means there is no good
  ...
  if (mayHaveMaterializedIV &&
      !arraytranslateLoop.checkMatIndVarStore(matIndVarStoreNode,
                                              indVarStoreNode)) {
  ...
  */
  ...
  if (hasBranch) {
    ...
    if (hasBreak) {
      removeEdge(loadSuccList, loadBlock->getNumber(),
                 breakExitBlock->getNumber());
    } else {
      removeEdge(loadSuccList, loadBlock->getNumber(),
                 otherStoreBlock->getNumber());
      removeEdge(loadSuccList, loadBlock->getNumber(), storeBlock->getNumber());
      removeEdge(storeSuccList, storeBlock->getNumber(),
                 loadBlock->getNumber());
...
bool TR_LoopReducer::generateArraytranslateAndTest(
  ...
  if (firstNum != 1 || (secondNum != 2 && secondNum != 3)) {
    dumpOptDetails(comp(), "Loop has wrong number of tree tops (%d,%d) - no "
                           "arraytranslateAndTest reduction\n",
                   firstNum, secondNum);
    return false;
  }
  ...
  if (secondNum == 2) {
    ...
    if (!asyncFound) {
      dumpOptDetails(comp(), "Loop has wrong number of tree tops for no "
                             "async-check (%d,%d) - no arraytranslateAndTest "
                             "reduction\n",
                     firstNum, secondNum);
      return false;
    }
  ...
  if (cmpTargetBlockNumber != nextBlock->getNumber()) {
    TR::Node *compareNode = TR::Node::createif(
        TR::ificmpne, arraytranslateAndTestLoop.getStoreAddress()
                          ->getIndVarLoad()
                          ->duplicateTree(),
...
bool TR_LoopReducer::generateByteToCharArraycopy(
  ...
  {
    dumpOptDetails(comp(), "Loop does not have an increment of +/-1 and +/-2, "
                           "but instead %d and %d - no byte to char arraycopy "
                           "reduction\n",
                   incrA, incrB);
    return false;
  }
  ...
  if (loopHeader->getNumberOfRealTreeTops() != 4) // array store, induction
                                                  // variable store, induction
                                                  // variable store, loop
                                                  // comparison
  {
    dumpOptDetails(
  ...
  if (!arraycopyLoop.checkByteLoads(storeNode->getSecondChild())) {
    dumpOptDetails(comp(), "second child of store does not look like OR of 2 "
                           "byte loads - no byte to char arraycopy "
                           "performed\n");
    return false;
  }
  ...
  if (!arraycopyLoop.getStoreAddress()->checkIndVarStore(charIndVarStoreNode)) {
    bool swap = arraycopyLoop.getHighLoadAddress()->checkIndVarStore(
        charIndVarStoreNode);
    if (swap &&
        arraycopyLoop.getStoreAddress()->checkIndVarStore(
            byteIndVarStoreNode)) {
      ...
      dumpOptDetails(comp(), "Did not encounter byte array induction variable "
                             "increment - no byte to char arraycopy "
                             "performed\n");
      return false;
    }
  ...
  if (!arraycopyLoop.checkLoopCmp(loopCmpNode, charIndVarStoreNode,
    ...
    // induction variable.
    // We should make this work if it was comparing the byte array induction
    // variable as an enhancement,
    // e.g.
    // for (byteInd=start; byteInd<end; byteInd+=2)
    //   { ... }
    dumpOptDetails(comp(), "Loop comparison does not match byte or char "
                           "induction variable - no byte to char arraycopy "
                           "performed\n");
    return false;
  }
  ...
  // Ignoring the fact that they could have different starting values, the
  // primary induction variable
  // is either twice the value of the secondary induction variable, or vice
  // versa. The final value
  // of the secondary induction variable is thus (primaryEnd - primaryStart)
  // <operator> 2,
  ...
  // doesn't insert
  // a multiplier in to the base address calculation of the src node on the
  // arraycopy.
...
bool TR_LoopReducer::generateCharToByteArraycopy(
  ...
  if (!arraycopyLoop.getLoadAddress()->checkIndVarStore(charIndVarStoreNode)) {
    ...
    if (swap &&
      ...
      dumpOptDetails(comp(), "Did not encounter byte array induction variable "
                             "increment - no char to byte arraycopy "
                             "performed\n");
      return false;
    }
  ...
  if (!arraycopyLoop.checkLoopCmp(loopCmpNode, charIndVarStoreNode,
    ...
    //   { ... }
    dumpOptDetails(comp(), "Loop comparison does not match byte or char "
                           "induction variable - no char to byte arraycopy "
                           "performed\n");
    return false;
  }
  ...
  TR::TreeTop::removeDeadTrees(comp(), treeList); // make these trees into nops,
                                                  // but leave them around to
...
void TR_LoopReducer::removeSelfEdge(TR::list<TR::CFGEdge *> succList,
                                    int32_t selfNumber) {
...
void TR_LoopReducer::removeEdge(TR::list<TR::CFGEdge *> succList,
                                int32_t selfNumber, int32_t destNumber) {
  for (auto edge = succList.begin(); edge != succList.end();) {
    TR::Block *dest = toBlock((*edge)->getTo());
    TR::Block *src = toBlock((*edge)->getFrom());
...
void TR_LoopReducer::reduceNaturalLoop(TR_RegionStructure *whileLoop) {
                     ...
                     considered */
  {
    if (((numBlocks == 1) &&
      ...
      dumpOptDetails(comp(), "Multiple Induction Variable loop %d has %d "
                             "blocks and is not reduced\n",
