...
bool TR::LocalDeadStoreElimination::isNonRemovableStore(
  ...
  // too conservative as the
  // seenIdenticalStore check can legally eliminate even nested symbols
  // i.e.
  // istore "a"  <- even with nested "a" this store can be eliminated due to the
  // redefine of "a" below
  //    x
  // istore "a"
  //    y
  // The usedInNestedProc check is to prevent a auto/parm store to a nested
  // symbol in a nested routine being
  // removed when there is only a return and no uses from the store to the end
  // of the block:
  // istore "b"  // not legal to remove this store as the auto/parm 'escapes' to
  // the parent (owning) routine as they share a stack
  //    z
  // return
...
void TR::LocalDeadStoreElimination::transformBlock(TR::TreeTop *entryTree,
  ...
  while (!(_curTree == entryTree)) {
    ...
    if (comp()->getVisitCount() == MAX_VCOUNT - 2) // Inc visit count will
                                                   // assert at MAX_VCOUNT - 1,
                                                   // so this gives us
    ...
    if (storeNode) {
      ...
      // 2) haven't seen a use and not volatile and one of the following is also
      // true
      //    a) the block contains a return and not nested and sym is an auto or
      //    parm (i.e. val doesn't escape)
...
TR::LocalDeadStoreElimination::removeStoreTree(TR::TreeTop *treeTop) {
  ...
  for (cursor.SetToFirst(); cursor.Valid(); cursor.SetToNext()) {
    if (_pendingIdentityStores->ElementAt(cursor).treeTop == treeTop) {
      _pendingIdentityStores->RemoveEntry(
          cursor); // cursor is no longer valid after this
      break;
    }
  }
...
bool TR::LocalDeadStoreElimination::isIdentityStore(TR::Node *storeNode) {
  ...
  PendingIdentityStoreTable::Cursor cursor(*_pendingIdentityStores);
  for (cursor.SetToFirst(); cursor.Valid(); cursor.SetToNext()) {
    if (_pendingIdentityStores->ElementAt(cursor).storeNode->getSymbol() ==
        storeNode->getSymbol()) {
      _pendingIdentityStores->RemoveEntry(
          cursor); // cursor is no longer valid after this
      break;
...
void TR::LocalDeadStoreElimination::examineNode(
    ...
    SharedBitVector &deadSymbolReferences) {
...
// using externalReferenceCount = (accumulated reference count) - (edge count
// traversing tree from this edge)
...
void TR::LocalDeadStoreElimination::adjustStoresInfo(
    TR::Node *node, SharedBitVector &deadSymbolReferences) {
...
TR::Node *TR::LocalDeadStoreElimination::getAnchorNode(
  ...
  if (node->getReferenceCount() > 1) {
    ...
    // of a tree commoned node (> 1 ref in same tree)
    // as the first overall reference therefore visit children nodes from first
    // (0) to last (node->getNumChildren()-1)
    // so the first reference flag is propagated correctly with
    // setIsFirstReferenceToNode in the code just above.
    // When not using TR_FirstNodeReferenceTracker the future use count is
    // decremented below to handle this case.
         ...
         i++) // visit from 0->numChildren-1 see above comment
    {
      ...
      if (!visited.contains(child)) {
        ...
        if (child->getLocalIndex() > 1) {
          ...
          // use count decremented here
          // so the anchored node previously created in this function will be
          // correctly recognized as a first
    ...
    if (storeNode && storeNode->getOpCode().isStoreIndirect() &&
      ...
      if (storeNode->getFirstChild()->getOpCode().hasSymbolReference() &&
          ...
          (storeNode->getFirstChild()
               ...
               ->getKind() == TR::New) &&
          !usedLocalObjectSymbols.ValueAt(storeNode->getFirstChild()
                                              ->getSymbolReference()
                                              ->getSymbol()
                                              ->getLocalIndex()))
  ...
  if (node->getOpCode().hasSymbolReference() &&
      ...
      !(parent->getOpCode().isStoreIndirect() && childNum == 0 &&
        ((uint32_t)parent->getSymbolReference()->getOffset() <
         fe()->getObjectHeaderSizeInBytes())))
    usedLocalObjectSymbols
        [node->getSymbolReference()->getSymbol()->getLocalIndex()] = true;
...
void TR::LocalDeadStoreElimination::killStoreNodes(TR::Node *node) {
  ...
  node->getSymbolReference()->getUseDefAliases().getAliasesWithClear(
      usedef_vector);
  ...
  for (auto it = _storeNodes->begin(); it != _storeNodes->end(); ++it) {
    ...
    if (storeNode && node->getSymbolReference()->sharesSymbol()) {
      ...
      // if
      // (node->getSymbolReference()->getUseDefAliases().contains(storeSymRef,
      // comp()))
