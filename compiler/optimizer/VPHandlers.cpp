...
static void
checkForNonNegativeAndOverflowProperties(TR::ValuePropagation *vp,
                                         TR::Node *node,
...
static int32_t arrayElementSize(const char *signature, int32_t len,
                                TR::Node *node, TR::ValuePropagation *vp) {
  if (signature[0] != '[')
    return 0;
...
static void constrainBaseObjectOfIndirectAccess(TR::ValuePropagation *vp,
                                                TR::Node *node) {
...
static bool tryFoldCompileTimeLoad(TR::ValuePropagation *vp, TR::Node *node,
                                   bool &isGlobal) {
  isGlobal = true;
  ...
  if (node->getOpCode().isIndirect()) {
    ...
    for (TR::Node *curNode = node->getFirstChild(); !baseExpression;
      ...
      if (!constraint) {
        ...
        if (nodeToRemove) {
          TR_ASSERT(didSomething, "How can there be a node to remove if "
                                  "transformIndirectLoadChain didn't do "
                                  "something??");
        ...
        if (curNode->getOpCodeValue() == TR::loadaddr &&
          ...
          if (vp->trace())
            traceMsg(vp->comp(), "  - FAIL: %s %p is not an indirect load and "
        ...
        if (vp->trace())
          traceMsg(vp->comp(), " - FAIL: %s %p points to a j9class but has "
          ...
          if (node->getOpCode().isLoadIndirect() &&
            ...
            TR::SymbolReference *improvedSymRef =
                vp->comp()->getSymRefTab()->findOrCreateSymRefWithKnownObject(
                    ...
                    knot->getPointerLocation(knownObject->getIndex()));
            if (improvedSymRef->hasKnownObjectIndex()) {
              if (performTransformation(
                      vp->comp(), "%sUsing known-object specific symref #%d "
                                  "for obj%d at [%p]\n",
...
static bool containsUnsafeSymbolReference(TR::ValuePropagation *vp,
                                          TR::Node *node) {
  if (vp->comp()->getSymRefTab()->findDLPStaticSymbolReference(
          node->getSymbolReference()) ||
...
static bool owningMethodDoesNotContainNullChecks(TR::ValuePropagation *vp,
                                                 TR::Node *node) {
...
bool constraintFitsInIntegerRange(TR::ValuePropagation *vp,
...
bool reduceLongOpToIntegerOp(TR::ValuePropagation *vp, TR::Node *node,
  ...
  // LoadExtensions (and other codegen peepholes) should in theory catch uneeded
  // conversions (like widening) as described in the case below on 64 bit
  // platforms.
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
      ...
      if (canMoveLongOpChildDirectly(node, i, arithNode)) {
        ...
        TR::Node *newChild = TR::Node::create(
            node, TR::ILOpCode::getProperConversion(
...
static void constrainAConst(TR::ValuePropagation *vp, TR::Node *node,
                            bool isGlobal) {
  TR::VPConstraint *constraint = NULL;
  if (node->getAddress() == 0) {
    ...
    if (node->isClassPointerConstant()) {
      TR::VPConstraint *typeConstraint = TR::VPClass::create(
          vp, TR::VPFixedClass::create(
                  vp, (TR_OpaqueClassBlock *)node->getAddress()),
          NULL, NULL, NULL, TR::VPObjectLocation::create(
...
static void constrainIntAndFloatConstHelper(TR::ValuePropagation *vp,
...
static void constrainIntConst(TR::ValuePropagation *vp, TR::Node *node,
                              bool isGlobal) {
...
static void constrainLongConst(TR::ValuePropagation *vp, TR::Node *node,
                               bool isGlobal) {
...
// any type of BCD node)
// 2a)   for a setsign operation where the setsign value is on the node itself
// 2b)   for a setsign operation where the setsign value is a const child of the
// node
//
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (dataType == TR::Int16 && node->getOpCode().isIndirect() &&
    ...
    if (index->getOpCode().isLoadConst() && array->getOpCode().isIndirect()) {
      ...
      if (baseVPConstraint && baseVPConstraint->isConstString()) {
        ...
        uintptrj_t string = *(uintptrj_t *)constString->getSymRef()
                                 ->getSymbol()
                                 ->castToStaticSymbol()
                                 ->getStaticAddress();

        uint16_t ch = constString->charAt(
            ((TR::Compiler->target.is64Bit() ? index->getLongIntLow()
                                             : index->getInt()) -
...
bool simplifyJ9ClassFlags(TR::ValuePropagation *vp, TR::Node *node,
                          bool isLong) {
#ifdef J9_PROJECT_SPECIFIC
...
static const char *getFieldSignature(TR::ValuePropagation *vp, TR::Node *node,
                                     int32_t &len) {
  TR::SymbolReference *symRef = node->getSymbolReference();
...
static void addKnownObjectConstraints(TR::ValuePropagation *vp,
                                      TR::Node *node) {
  TR::KnownObjectTable *knot = vp->comp()->getKnownObjectTable();
  if (!knot)
    return;
  ...
  if (objectReferenceLocation) {
...

    {
      ...
      clazz =
          TR::Compiler->cls.objectClass(vp->comp(), *objectReferenceLocation);
      ...
      if (isFixedJavaLangClass) {
        ...
        // happens to be java/lang/Class.
        // Must add constraints pertaining to the class that the java/lang/Class
        // object represents.
        //
        clazz = TR::Compiler->cls.classFromJavaLangClass(
            vp->comp(), *objectReferenceLocation);
      }
                        ...
                        // constraint to add
    {
      ...
      if (isFixedJavaLangClass) {
        if (performTransformation(
                vp->comp(), "%sAdd ClassObject constraint to %p based on known "
                            "java/lang/Class %s =obj%d\n",
        ...
        if (performTransformation(
                vp->comp(), "%sAdd known-object constraint to %p based on "
                            "known object obj%d of class %s\n",
      ...
      if (haveClassLookaheadInfo) {
        ...
        if (sym->isStatic() && sym->isFinal()) {
          ...
          if (classOfStatic != vp->comp()->getSystemClassPointer() &&
              isClassInitialized && !vp->comp()->getOption(TR_AOT) &&
              (type == TR::Address)) {
            ...
            if ((*(void **)p != 0) &&
              ...
              int32_t arrLength =
                  *((int32_t *)(((uintptrj_t) * (void ***)p) +
                                (uintptrj_t)vp->fe()
                                    ->getOffsetOfContiguousArraySizeField()));
              ...
              if (arrayFieldInfo && arrayFieldInfo->isDimensionInfoValid()) {
                ...
                if (firstDimension >= 0) {
                  ...
                  if (sig && (len > 0) && (sig[0] == '[' || sig[0] == 'L')) {
                    ...
                    if (elementSize != 0) {
                      if (vp->trace())
                        traceMsg(vp->comp(), "Using class lookahead info to "
                                             "find out non null, array "
...
static TR::Node *findArrayLengthNode(TR::ValuePropagation *vp, TR::Node *node,
...
static TR::Node *findArrayIndexNode(TR::ValuePropagation *vp, TR::Node *node,
                                    int32_t stride) {
    ...
    if (base && node->getOpCode().hasSymbolReference() &&
      ...
      if (!base->isFixedClass()) {
        vp->addBlockOrGlobalConstraint(
            node,
            TR::VPClass::create(vp, NULL, nonnull, NULL, NULL,
                                TR::VPObjectLocation::create(
                                    vp, TR::VPObjectLocation::J9ClassObject)),
            isGlobal);
        ...
        vp->addBlockOrGlobalConstraint(
            node, TR::VPClass::create(
                      vp, TR::VPFixedClass::create(
                              vp, vp->comp()->fej9()->getClassClassPointer(
                                      base->getClass())),
                      nonnull, NULL, NULL,
                      TR::VPObjectLocation::create(
                          vp, TR::VPObjectLocation::J9ClassObject)),
        ...
        vp->addBlockOrGlobalConstraint(
            node,
            TR::VPClass::create(
                vp, TR::VPFixedClass::create(vp, base->getClass()), nonnull,
                NULL, NULL, TR::VPObjectLocation::create(
                                vp, TR::VPObjectLocation::J9ClassObject)),
            isGlobal);
      }
      node->setIsNonNull(true);
...

        {
          ...
          if (constrainIaloadCriticalSection.hasVMAccess()) {
            ...
            if (TR::Compiler->cls.isClassInitialized(vp->comp(), clazz)) {
              ...
              // compenent class pointer precisely
              // but clearly others make invalid assumptions on seeing this
              // fixed class constraint so be conservative
              // and don't let the world know if we aren't sure the elements
              // match this type
              ...
              node->setIsNonNull(true);
               ...
               TR::Compiler->cls.isClassInitialized(vp->comp(),
                                                    base->getClass())) {
      if (symRef ==
        ...
        // class pointer precisely
        // but clearly others make invalid assumptions on seeing this fixed
        // class constraint so be conservative
        // and don't let the world know if we aren't sure the elements match
        // this type
        if (!TR::Compiler->cls.isInterfaceClass(vp->comp(), componentClass) &&
          ...
          vp->addBlockOrGlobalConstraint(
              node, TR::VPClass::create(
                        vp, TR::VPFixedClass::create(vp, componentClass),
                        TR::VPNonNullObject::create(vp), NULL, NULL,
                        TR::VPObjectLocation::create(
                            vp, TR::VPObjectLocation::J9ClassObject)),
              isGlobal);
          ...
          if (bypassOnThisRun) {
...

            {
              ...
              if (constrainIaloadCriticalSection.hasVMAccess()) {
                ...
                if (vp->trace())
                  traceMsg(vp->comp(), "[%p] Unable to acquire VM access.  Not "
            ...
            if (bypassLocation &&
              ...
              node->setSymbolReference(
                  vp->comp()
                      ->getSymRefTab()
                      ->createKnownStaticRefereneceSymbolRef(bypassLocation,
                                                             targetKOI));
              return node;
            }
          }
  ...
  if (node->getOpCode().hasSymbolReference() &&
    ...
    if (constraint && constraint->getClass()) {
      ...
      if (arraySig && *arraySig == '[') {
        ...
        For example, the following two snippets are both rejected by javac but
        accepted by a bytecode verifier.
        ...
        staticField is Serializable
        but it will be rejected by the bytecode verifier if staticField is of a
        class type.
        */
        if (clazz) {
          TR_OpaqueClassBlock *jlClass =
...
TR::Node *constrainStore(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (globalStore) {
    TR::ValuePropagation::Relationship *syncRel =
...
#ifdef J9_PROJECT_SPECIFIC
  if (valueChild->getType().isBCD()) // sign state constraints are currently
                                     // only propagated to loads so only
...
void canRemoveWrtBar(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (constraint) {
    if (constraint->isNullObject() &&
        !vp->comp()->getOptions()->alwaysCallWriteBarrier() &&
...
TR::Node *constrainWrtBar(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (doOpt && ((gcMode == TR_WrtbarCardMarkAndOldCheck) ||
                (gcMode == TR_WrtbarOldCheck)) &&
      (node->getOpCodeValue() == TR::wrtbari) && !node->skipWrtBar()) {
  ...
  // in the same method.
  // If that's the case then the chances are this wrtbar is going to be on a new
  // object and we should
    ...
    if (constraint) {
      if (constraint->isHeapObject() == TR_yes &&
        ...
        node->setIsHeapObjectWrtBar(true);
                 ...
                 performTransformation(vp->comp(), "%sMarking the wrtbar node "
                                                   "[%p] - destination is a "
                                                   "non-heap object\n",
                                       OPT_DETAILS, node)) {
...
TR::Node *constrainGoto(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (vp->trace())
    traceMsg(vp->comp(), "   unconditional branch on node %s (%p), "
                         "vp->_curBlock block_%d target block_%d\n",
...
TR::Node *constrainCheckcast(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if ((result == 0) || ((node->getOpCodeValue() == TR::checkcastAndNULLCHK) &&
          ...
          if (!defs.IsZero()) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid();
              ...
              if (defNode && (defNode->getOpCodeValue() == TR::astore)) {
                ...
                if (methodSig &&
                    !strncmp(methodSig, "java/util/HashMap.get(Ljava/lang/"
...
static TR::VPConstraint *createIntConstConstraint(TR::ValuePropagation *vp,
                                                  int32_t n) {
  return TR::VPIntConst::create(vp, n);
}
static TR::VPConstraint *createIntRangeConstraint(TR::ValuePropagation *vp,
                                                  int32_t l, int32_t h) {
  return TR::VPIntRange::create(vp, l, h);
}
static TR::VPConstraint *createLongConstConstraint(TR::ValuePropagation *vp,
                                                   int64_t n) {
  return TR::VPLongConst::create(vp, n);
}
static TR::VPConstraint *createLongRangeConstraint(TR::ValuePropagation *vp,
                                                   int64_t l, int64_t h) {
...
static TR::Node *constrainHighestOneBitAndLeadingZerosHelper(
    TR::ValuePropagation *vp, TR::Node *node, A getConst, B getRange,
  ...
  if (childConstraint) {
    if (getConst(childConstraint)) {
      ...
      if (low < 0 && high < 0) {
        ...
        MIN_VALUE = MAX_VALUE;
        if (vp->trace()) {
          traceMsg(vp->comp(), "Constraint %lld .. %lld of %p 's first child "
                               "is negative and folded into %lld \n",
...
TR::Node *constrainIntegerHighestOneBit(TR::ValuePropagation *vp,
                                        TR::Node *node) {
...
TR::Node *constrainIntegerNumberOfLeadingZeros(TR::ValuePropagation *vp,
                                               TR::Node *node) {
  return constrainHighestOneBitAndLeadingZerosHelper(
      vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
...
TR::Node *constrainLongNumberOfLeadingZeros(TR::ValuePropagation *vp,
                                            TR::Node *node) {
  return constrainHighestOneBitAndLeadingZerosHelper(
      vp, node, getLongConst, getLongRange, getLong, getLowHighLongs,
      createIntConstConstraint, createIntRangeConstraint,
...
TR::Node *constrainIntegerLowestOneBit(TR::ValuePropagation *vp,
                                       TR::Node *node) {
...
TR::Node *constrainIntegerNumberOfTrailingZeros(TR::ValuePropagation *vp,
                                                TR::Node *node) {
  return constrainLowestOneBitAndTrailingZerosHelper(
      vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
      createIntConstConstraint, createIntRangeConstraint,
...
TR::Node *constrainLongNumberOfTrailingZeros(TR::ValuePropagation *vp,
                                             TR::Node *node) {
  return constrainLowestOneBitAndTrailingZerosHelper(
      vp, node, getLongConst, getLongRange, getLong, getLowHighLongs,
      createIntConstConstraint, createIntRangeConstraint,
...
static void
refineSymbolReferenceForProfiledToOverridden(TR::ValuePropagation *vp,
                                             TR::Node *callNode,
                                             TR::SymbolReference *newSymRef) {
  for (TR::ValuePropagation::VirtualGuardInfo *cvg =
...
refineMethodSymbolInCall(TR::ValuePropagation *vp, TR::Node *node,
                         TR::SymbolReference *symRef,
...
static void devirtualizeCall(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (!methodSymbol->firstArgumentIsReceiver()) {
    if (vp->trace())
      traceMsg(vp->comp(), "Not attempting to de-virtualize call [%p] without "
                           "first argument receiver\n",
               node);
    return;
  }
  ...
  if ((constraint->asClass() &&
    ...
    if (!thisType) {
      if (vp->trace())
        traceMsg(vp->comp(), "Not attempting to de-virtualize call [%p] with "
  ...
  if (argNode->getSymbol()) {
    ...
    if (argSym->isParm()) {
      if (!vp->isParmInvariant(argSym)) {
        if (vp->trace())
          traceMsg(vp->comp(), "Not attempting to de-virtualize call [%p] "
                               "because receiver [%p] is not invariant\n",
                   node, argNode);
        return;
      }
    }
  ...
  if (methodSymbol->isComputed()) {
      ...
      if (constraint && constraint->getKnownObject() && knot) {
        ...
        resolvedMethod =
            vp->comp()->fej9()->createMethodHandleArchetypeSpecimen(
                vp->trMemory(), knot->getPointerLocation(
        ...
        if (resolvedMethod) {
          ...
          if (vp->optimizer()
                  ...
                  ->requested()) {
            if (vp->trace())
              traceMsg(vp->comp(), "Not inlining call [%p] because the "
                                   "MethodHandle.invoke inlining group will do "
                                   "a better job\n",
                       node);
            return;
          }
          ...
          if (vp->trace())
            traceMsg(vp->comp(), "Not inlining call [%p] because there is no "
                                 "more specific method symbol for obj%d\n",
                     node, constraint->getKnownObject()->getIndex());
          return;
        }
      } else {
        if (vp->trace())
          traceMsg(vp->comp(), "Not inlining call [%p] because unable to "
                               "substitute object-specific method symbol\n",
                   node);
        return;
      }
    }
    ...
    if (TR::Compiler->cls.isInterfaceClass(vp->comp(), thisType)) {
      if (vp->trace())
        traceMsg(vp->comp(), "Not attempting to de-virtualize interface call "
                             "[%p] with interface-class receiver\n",
                 node);
      return;
    }
    ...
    if (!originalMethodClass) {
      if (vp->trace())
        traceMsg(vp->comp(), "Not attempting to de-virtualize interface call "
                             "[%p] with receiver class %.*s not yet loaded\n",
                 node, len, s);
      return;
    }
  ...
  if (methodSymbol->isVirtual() || methodSymbol->isInterface()) {
    ...
    if (methodSymbol->isVirtual() &&
      ...
      // symbol to
      // refer to the original devirtualized call.  Not devirtualizing the call
      // shouldn't be
      ...
      // virtual gets
      // added to the devirtualizedCalls list and the guard is eventually
      // removed. This
      // has the benefit that the call no longer interferes with aliasing (and
      // other opts
  ...
  if ((vp->lastTimeThrough() || !node->getOpCode().isIndirect()) &&
    ...
    vp->_devirtualizedCalls.add(
        new (vp->trStackMemory())
            TR::ValuePropagation::CallInfo(vp, thisType, argInfo));
  vp->invalidateUseDefInfo();
  vp->invalidateValueNumberInfo();
}
...
TR::Node *constrainCall(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (vp->comp()->getSymRefTab()->element(TR_induceOSRAtCurrentPC) ==
      node->getSymbolReference()) {
...

  {
    ...
    if (operand && ((operand->isClassObject() == TR_yes) ||
      ...
      vp->_unsafeCallsToInline.add(new (
          vp->trStackMemory()) TR::ValuePropagation::CallInfo(vp, NULL, NULL));
  ...
  if (symbol) {
    ...
    if (isIntegerAbs || isLongAbs) {
      ...
      // be the minimum
      // value of the range (TR::getMinSigned<TR::Int32>() or
      ...
      if (constraint && constraint->getKnownObject() &&
        ...
        if (j9class &&
            ...
            performTransformation(vp->comp(),
                                  "%s Remove "
                                  "TR::sun_misc_Unsafe_ensureClassInitialized "
                                  "call, class already initialized [%p]\n",
                                  OPT_DETAILS, node)) {
          vp->removeNode(node);
...
#ifdef J9_PROJECT_SPECIFIC
void getHelperSymRefs(TR::ValuePropagation *vp, TR::Node *curCallNode,
                      TR::SymbolReference *&getHelpersSymRef,
                      TR::SymbolReference *&helperSymRef, char *helperSig,
                      int32_t helperSigLen,
                      TR::MethodSymbol::Kinds helperCallKind) {
  // Function to retrieve the JITHelpers.getHelpers and JITHelpers.<helperSig>
    ...
    if (!strncmp(sig, helperSig, helperSigLen)) {
      if (TR::MethodSymbol::Virtual == helperCallKind) {
        ...
        helperSymRef = vp->comp()->getSymRefTab()->findOrCreateMethodSymbol(
            JITTED_METHOD_INDEX, -1, m, TR::MethodSymbol::Virtual);
        helperSymRef->setOffset(TR::Compiler->cls.vTableSlot(
            vp->comp(), m->getPersistentIdentifier(), jitHelpersClass));
      } else {
        helperSymRef = vp->comp()->getSymRefTab()->findOrCreateMethodSymbol(
            curCallNode->getSymbolReference()->getOwningMethodIndex(), -1, m,
            helperCallKind);
      }
...
#ifdef J9_PROJECT_SPECIFIC
void transformToOptimizedCloneCall(TR::ValuePropagation *vp, TR::Node *node,
                                   bool isDirectCall) {
  TR::SymbolReference *getHelpersSymRef = NULL;
  TR::SymbolReference *optimizedCloneSymRef = NULL;

  getHelperSymRefs(vp, node, getHelpersSymRef, optimizedCloneSymRef,
                   "optimizedClone", 14, TR::MethodSymbol::Special);

  // printf("helper sym 0x%x, optsym 0x%x \n", getHelpersSymRef,
  // optimizedCloneSymRef);

  if (optimizedCloneSymRef && getHelpersSymRef &&
      performTransformation(
          vp->comp(), "%sChanging call to new optimizedClone at node [%p]\n",
          OPT_DETAILS, node)) {
    // FIXME: add me to the list of calls to be inlined
    //
    TR_Method *method =
        optimizedCloneSymRef->getSymbol()->castToMethodSymbol()->getMethod();
    TR::Node *helpersCallNode = TR::Node::createWithSymRef(
        node, method->directCallOpCode(), 0, getHelpersSymRef);
    TR::TreeTop *helpersCallTT = TR::TreeTop::create(
        vp->comp(), TR::Node::create(TR::treetop, 1, helpersCallNode));
...
TR::Node *transformCloneCallSetup(TR::ValuePropagation *vp, TR::Node *node,
                                  TR::VPConstraint *constraint,
                                  bool isDirectCall) {
  ...
  if (isDirectCall && constraint && constraint->getClass())
     ...
     //If we do not know the class, or the class is an array, pass NULL as the
  second parameter.
     if (clazz && (((TR::Compiler->cls.classDepthOf(clazz) == 0) &&
  !constraint->isFixedClass()) || (constraint->getClassType()->isArray() !=
  TR_no)))
       clazz = NULL;
...
#ifdef J9_PROJECT_SPECIFIC
TR::Node *setCloneClassInNode(TR::ValuePropagation *vp, TR::Node *node,
                              TR::VPConstraint *constraint, bool isGlobal) {
  ...
  if (constraint && constraint->getClass()) {
    ...
    if (node->getCloneClassInNode() && clazz &&
        (node->getCloneClassInNode() != clazz)) {
      TR_YesNoMaybe answer = vp->fe()->isInstanceOf(
          clazz, node->getCloneClassInNode(), true, true);
      if (answer != TR_yes)
        clazz = node->getCloneClassInNode();
    }
    if (performTransformation(
            vp->comp(),
            "%sSetting type on Object.Clone acall node [%p] to [%p]\n",
            OPT_DETAILS, node, clazz))
...
TR::Node *constrainAcall(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (method) {
    if (!node->getOpCode().isIndirect()) {
      static char *enableDynamicObjectClone =
          feGetEnv("TR_enableDynamicObjectClone");
      if (method->getRecognizedMethod() == TR::java_lang_Object_clone ||
        ...
        if (constraint) {
          if (constraint->isFixedClass()) {
            newTypeConstraint =
                TR::VPFixedClass::create(vp, constraint->getClass());
            ...
            if (!vp->comp()->compileRelocatableCode()) {
              if (constraint->getClassType() &&
                ...
                vp->_objectCloneTypes.add(
                    new (vp->trStackMemory())
                        TR::ValuePropagation::ObjCloneInfo(
                            constraint->getClass(), true));
            ...
            newTypeConstraint =
                TR::VPResolvedClass::create(vp, constraint->getClass());
            if (enableDynamicObjectClone &&
                constraint->getClassType()->isArray() == TR_no &&
                !vp->_objectCloneCalls.find(vp->_curTree)) {
              vp->_objectCloneCalls.add(vp->_curTree);
              vp->_objectCloneTypes.add(new (vp->trStackMemory())
                                            TR::ValuePropagation::ObjCloneInfo(
                                                constraint->getClass(), false));
            }
          }
        }
        ...
        if (!constraint ||
          ...
          TR::DebugCounter::incStaticDebugCounter(
              ...
              TR::DebugCounter::debugCounterName(
                  vp->comp(), "inlineClone/miss/(%s)/%s/block_%d",
                  vp->comp()->signature(),
                  vp->comp()->getHotnessName(vp->comp()->getMethodHotness()),
                  vp->_curTree->getEnclosingBlock()->getNumber()));
        else
          TR::DebugCounter::incStaticDebugCounter(
              vp->comp(),
              TR::DebugCounter::debugCounterName(
                  vp->comp(), "inlineClone/hit/(%s)/%s/block_%d",
                  vp->comp()->signature(),
                  vp->comp()->getHotnessName(vp->comp()->getMethodHotness()),
                  vp->_curTree->getEnclosingBlock()->getNumber()));

        TR::VPClassPresence *cloneResultNonNull =
            TR::VPNonNullObject::create(vp);
        ...
        TR::VPConstraint *newConstraint =
            TR::VPClass::create(vp, newTypeConstraint, cloneResultNonNull, NULL,
        ...
        // OptimizedClone
        if (vp->comp()->getOption(TR_EnableJITHelpersoptimizedClone) &&
            newTypeConstraint)
          transformToOptimizedCloneCall(vp, node, true);
        return node;
      } else if (method->getRecognizedMethod() ==
                 TR::java_math_BigDecimal_valueOf) {
        TR_ResolvedMethod *owningMethod = symRef->getOwningMethod(vp->comp());
        TR_OpaqueClassBlock *classObject = vp->fe()->getClassFromSignature(
            "java/math/BigDecimal", 20, owningMethod);
        if (classObject) {
      ...
      if ((method->getRecognizedMethod() == TR::java_math_BigDecimal_add) ||
        ...
        TR_ResolvedMethod *owningMethod = symRef->getOwningMethod(vp->comp());
        TR_OpaqueClassBlock *bigDecimalClass = vp->fe()->getClassFromSignature(
            "java/math/BigDecimal", 20, owningMethod);
        ...
        if (constraint && bigDecimalClass && constraint->isFixedClass() &&
          ...
          TR::VPConstraint *newConstraint =
              TR::VPFixedClass::create(vp, bigDecimalClass);
  ...
  TR_ASSERT(sig[0] == 'L' || sig[0] == '[',
            "Ref call return type is not a class");
  ...
  if (classBlock) {
    ...
    if (jlClass) {
      if (classBlock != jlClass)
        constraint = TR::VPClassType::create(vp, sig, len, owningMethod, false,
                                             classBlock);
      else
        constraint = TR::VPObjectLocation::create(
            vp, TR::VPObjectLocation::JavaLangClassObject);
    ...
    TR::VPConstraint *constraint =
        TR::VPUnresolvedClass::create(vp, sig, len, owningMethod);
    vp->addGlobalConstraint(node, constraint);
  }
...
TR::Node *constrainIumul(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && rhs) {
    ...
    if (lhs->asIntConst() && rhs->asIntConst()) {
      ...
      uint64_t lowerLowerLimit =
          (uint64_t)lhs->getLowInt() * (uint64_t)rhs->getLowInt();
      uint64_t lowerUpperLimit =
          (uint64_t)lhs->getLowInt() * (uint64_t)rhs->getHighInt();
      uint64_t upperLowerLimit =
          (uint64_t)lhs->getHighInt() * (uint64_t)rhs->getLowInt();
      uint64_t upperUpperLimit =
          (uint64_t)lhs->getHighInt() * (uint64_t)rhs->getHighInt();
      ...
      if (lowest < TR::getMinUnsigned<TR::Int32>() ||
          highest > TR::getMaxUnsigned<TR::Int32>()) {
        constraint = NULL;
      } else {
        constraint = TR::VPIntRange::create(vp, (int32_t)lowest,
                                            (int32_t)highest /*, isUnsigned*/);
...
// TR::getMaxSigned<TR::Int32>(); the latter is used
...
bool can64BitUnsignedMultiplyOverflow(uint64_t a, uint64_t b,
  ...
  // 2^32, which is < 2^64, so no overflow.
  // If the high word of b is non-zero, the final product x will be ((a *
  // HIGHWORD(b)) << 32) + (a * LOWWORD(b))
  // If we compute the highword of the above expression in a 64-bit variable, a
...
bool constrainIntegerDivisionRange(int64_t lhsLow, int64_t lhsHigh,
  ...
  // range so it doesn't include 0, since integer division by 0 is
  // meaningless and should be handled by an exception when executed. In other
  // words, this method determines the range of the result
...
TR::Node *cloneDivForDivideByZeroCheck(TR::ValuePropagation *vp,
                                       TR::Node *divNode) {
...
TR::Node *constrainIdiv(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (constraint) {
    if (constraint->asIntConst()) {
      ...
      // the original
      // dividend contained 0 in its range, create a copy of the original div,
      // to be anchored
      // under the original DIVCHK, so that the constant value can be optimized
      // further,
  ...
  if (lhs && lhs->asLongConst() && rhs && rhs->asLongConst()) {
    ...
    if (vp->lastTimeThrough() && !isUnsigned && lhsConstraint &&
      ...
      if (idivConstraint) {
        // If constrainIntegerDivisionRange reduced the range to a constant, but
        // the original
        // divisor contained 0 in its range, create a copy of the original div,
        // to be anchored
        // under the original DIVCHK, so that the constant value can be
        // optimized further,
        // but division by zero is still caught
      ...
      if (lhsConstraint && rhsConstraint) {
        ...
        if (constrainIntegerDivisionRange(
          ...
          if (constraint) {
            ...
            // but the original
            // divisor contained 0 in its range, create a copy of the original
            // div, to be anchored
            // under the original DIVCHK, so that the constant value can be
            // optimized further,
            // but division by zero is still caught
...
TR::Node *removeRedundantREM(TR::ValuePropagation *vp, TR::Node *node,
                             TR::VPConstraint *nodeConstraint,
  ...
  if (powerOfTen) {
    ...
    // divisor is a power of ten then -1 this value is
    ...
    if (!isUnsigned && fcPrecision <= refinedPrecision &&
        performTransformation(
            vp->comp(), "%sRemove %s [0x%p] as child %s [0x%p] prec %lld <= "
      ...
      // that the irem is not needed (999 < 1000)
      ...
      // as unsigned so, for example, a prec=1 dividend that is negative
      // when interpreted as unsigned will actually have 10 digits of precision
    ...
    if (child->asIntConst()) {
      ...
      if (low < 0 && high <= 0) {
        ...
        low = 0;
                     ...
                     vp->comp(), "%sRemoving %s [0x%p] as child %s [0x%p] is "
...
static TR::Node *distributeShift(TR::ValuePropagation *vp, TR::Node *node,
                                 int32_t shiftAmount,
                                 TR::VPConstraint *&constraint) {
    ...
    if (replaceNode) {
      ...
      if (lhsORrhs) {
        ...
        if ((low <= 0) && (((int64_t)(high + 1) & (pow2Val - 1))) == 0) {
          if (vp->trace())
            traceMsg(vp->comp(), "found opportunity to distribute shift in "
                                 "node [%p] replaceNode [%p]\n",
                     node, replaceNode);
        } else {
          if (vp->trace())
            traceMsg(vp->comp(), "failed additive expr constraint is not "
                                 "within range, node [%p] replaceNode [%p]\n",
                     node, replaceNode);
          replaceNode = NULL;
        }
      } else {
      ...
      if (c) {
        if (node->getOpCodeValue() == TR::iushr) {
          ...
          // this path is probably never taken for unsigned
            ...
            if (shiftAmount > 0)
              constraint = TR::VPIntRange::create(vp, 0, (uint32_t)0xffffffff >>
                                                             shiftAmount);
            else
              constraint =
                  TR::VPIntRange::create(vp, 0, TR::getMaxSigned<TR::Int32>());
...
TR::Node *constrainIshr(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (firstChild->isNonNegative() /*&& firstChild->getType().isSignedInt()*/ &&
      vp->lastTimeThrough() &&
      performTransformation(vp->comp(), "%sChange node [" POINTER_PRINTF_FORMAT
                                        "] ishr->iushr\n",
                            OPT_DETAILS, node)) {
  ...
  if (rhs && rhs->asIntConst()) {
    ...
    if (constraint) {
      ...
      if (longShr) {
        ...
        // Integer shifts are
        // masked by 0x1f in java, and if the first child is integer
        // constrained, a shift by an amount in this range is effectively just a
        // shift by 31.
...
TR::Node *constrainIand(TR::ValuePropagation *vp, TR::Node *node) {
      ...
      if (rhs && rhs->asIntConst()) {
        ...
        if ((firstChild->getOpCodeValue() == TR::iloadi) &&
            (firstChild->getSymbolReference() ==
             vp->comp()->getSymRefTab()->findClassIsArraySymbolRef()) &&
            (rhs->getLowInt() ==
          ...
          if ((firstGrandChild->getOpCodeValue() == TR::aloadi) &&
              (firstGrandChild->getSymbolReference() ==
               vp->comp()->getSymRefTab()->findClassRomPtrSymbolRef())) {
            ...
            if ((vftLoad->getOpCodeValue() == TR::aloadi) &&
              ...
              TR::VPConstraint *objConstraint =
                  vp->getConstraint(objectLoad, objectLoadGlobal);
              if (objConstraint && objConstraint->getClassType() &&
                  (objConstraint->getClassType()->isArray() != TR_maybe)) {
                ...
                if (objConstraint->getClassType()->isArray() == TR_yes)
                  constraint = TR::VPIntConst::create(
                      vp, rhs->asIntConst()->getLowInt() /*, isUnsigned*/);
                else
                  constraint = TR::VPIntConst::create(vp, 0 /*, isUnsigned*/);
              } else {
                // if (vp->_isGlobalPropagation)
                //   {
                //   dumpOptDetails(vp->comp(), "NOT Folding isArray test in
                //   %p\n", objectLoad);
  ...
  // trailng zeros as lrhs.
  // When lhs & rhs, and rhs < 0b1000..(number of trailing zeros in lhs), then
  // the whole iand node evaluates to a constant zero.
  ...
  if (rhs && lrhs && lrhs->asIntConst()) {
    ...
    if (rhs->asIntRange()) {
      ...
      if ((node->getFirstChild()->getOpCodeValue() == TR::imul ||
           node->getFirstChild()->getOpCodeValue() == TR::iumul) &&
        ...
        if (high < 1 << shift)
          constraint = TR::VPIntConst::create(vp, 0);
                  ...
                  node->getFirstChild()->getOpCodeValue() == TR::iushl) &&
                 ((high & 0x80000000) == 0) && ((low & 0x80000000) == 0)) {
        if (high < 1 << mask)
          constraint = TR::VPIntConst::create(vp, 0);
      }
...
static TR::Node *constrainWidenToLong(TR::ValuePropagation *vp, TR::Node *node,
...
void replaceWithSmallerType(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (vp->comp()
    ...
    dumpOptDetails(
        vp->comp(),
        "replaceWithSmallerType not safe to perform when NOTEMPS enabled\n");
    return;
  }
  ...
  if (performTransformation(comp, "%sReplace [" POINTER_PRINTF_FORMAT
...
static TR::Node *constrainNarrowIntValue(TR::ValuePropagation *vp,
  ...
  if (constraint) {
    if (constraint->asIntConstraint() || constraint->asMergedConstraints()) {
      ...
      if (constraint->getLowInt() < 0 &&
        ...
        low = high =
            max +
            constraint->getLowInt() % (TR::getMaxUnsigned<TR::Int16>() + 1);
    } else if (constraint->asLongConstraint() ||
               constraint->asMergedLongConstraints()) {
      if (constraint->getLowInt() > 0 &&
          constraint->getLowInt() < TR::getMaxUnsigned<TR::Int16>() &&
...
/*
static int32_t findConstantValue(TR::ValuePropagation *vp, TR::Node *node,
TR::VPConstraint *constraint)
   {
...
static bool constrainWidenToInt(TR::ValuePropagation *vp, TR::Node *&node,
  ...
  */
  ...
  if (constraint) {
    if (isUnsigned) {
      ...
      // number was
      // defined as PIC S999 (3 digits, signed), the incoming range would be
      // [-999, 999].
      // Widened to an unsigned integer, this gives the disjoint range [0, 999],
      // [64537, 65535],
      // which, when merged into one constraint, would be [0, 65535]. In this
      // case, low and high
      // should already be 0 and 65535, so we don't want to adjust anything
      // based on the child
      // constraint.
...
static void changeConditionalToGoto(TR::ValuePropagation *vp, TR::Node *node,
...
static void removeConditionalBranch(TR::ValuePropagation *vp, TR::Node *node,
                                    TR::CFGEdge *branchEdge) {
#ifdef J9_PROJECT_SPECIFIC
  createGuardSiteForRemovedGuard(vp->comp(), node);
#endif
...
//
static TR::VPConstraint *genTypeResult(TR::ValuePropagation *vp,
                                       TR::VPConstraint *objectRefConstraint,
                                       TR::VPConstraint *classConstraint,
                                       bool &applyType, bool isInstanceOf) {
  ...
  if (objectRefConstraint) {
    ...
    if (instanceClass) {
      ...
      //
      if (!presence && objectRefConstraint->getClassPresence() &&
          instanceClass->getClassPresence())
        newConstraint = NULL;
      else if (!newType && objectRefConstraint->getClassType() &&
               instanceClass->getClassType()) {
        if (applyType)
          newConstraint = NULL;
      }
      ...
      // one of the types could be null
      ...
      else if (isInstanceOf && !objectRefConstraint->getClassType() &&
        ...
        //
        // if the castClass type is not java/lang/Class (or one of the
        // interfaces
        // implemented by it) and VP is analyzing an instanceof, then the types
        // cannot
        ...
        if (castIsClassObject) {
          ...
          if (objectRefConstraint->getClassType())
            newType = TR::VPResolvedClass::create(
                vp, (TR_OpaqueClassBlock *)VP_SPECIALKLASS);
          else
            newType = NULL;
        ...
        if ((objectRefConstraint &&
             (objectRefConstraint->isClassObject() == TR_yes)) ||
            castIsClassObject)
          loc = TR::VPObjectLocation::create(vp,
                                             TR::VPObjectLocation::ClassObject);

        // final construction
        newConstraint =
            TR::VPClass::create(vp, newType, presence, NULL, NULL, loc);
      }
    }
  } else {
    if (vp->trace())
      traceMsg(vp->comp(), "ObjectRef has no constraint, so applying cast "
                           "class properties...\n");
    ...
    if (classConstraint->getClassType()->asResolvedClass()) {
      ...
      if (jlClass) {
        ...
        if (klass == jlClass) {
          ...
          newType =
              TR::VPResolvedClass::create(vp, classConstraint->getClass());
      } else
        applyType = false;
    } else {
    ...
    if (applyType) {
      ...
      newConstraint = TR::VPClass::create(
          vp, newType, classConstraint->getClassPresence(), NULL, NULL, loc);
...
static void addDelayedConvertedGuard(TR::Node *node, TR::Node *callNode,
  ...
  if (!callNode->getFirstChild()->getOpCode().hasSymbolReference() ||
      ...
      !callNode->getSecondChild()->getOpCode().isLoadVarDirect() || // it also
                                                                    // implies,
                                                                    // there
                                                                    // should be
                                                                    // a
  ...
  vp->_convertedGuards.add(
      new (vp->trStackMemory()) TR::ValuePropagation::VirtualGuardInfo(
...
//
static TR::Node *constrainIfcmpeqne(TR::ValuePropagation *vp, TR::Node *node,
                                    bool branchOnEqual) {
  constrainChildren(vp, node);
  ...
  if (((node->getOpCodeValue() == TR::ifacmpne ||
    ...
    if (vGuard && !vGuard->canBeRemoved()) {
      if (vp->trace())
        traceMsg(vp->comp(), "   found virtual guard node %p with inner "
  ...
  //
    ...
    if (lhs && rhs) {
      ...
      if (lhs->mustBeEqual(rhs, vp) && ignoreVirtualGuard) {
        if (vp->trace())
          traceMsg(vp->comp(), "   cmp children must be equal by absolute "
                               "constraints: %p == %p\n",
                   lhsChild, rhsChild);
        if (branchOnEqual)
          cannotFallThrough = true;
        ...
        if (vp->trace())
          traceMsg(vp->comp(), "   cmp children must be not equal by absolute "
                               "constraints: %p != %p\n",
                   lhsChild, rhsChild);
        if (branchOnEqual)
          cannotBranch = true;
  ...
  if (!cannotBranch && !cannotFallThrough) {
    ...
    if (rel && ignoreVirtualGuard) {
      ...
      if (rel->mustBeEqual()) {
        if (vp->trace())
          traceMsg(vp->comp(), "   cmp children must be equal by relative "
                               "constraint: %p == %p\n",
                   lhsChild, rhsChild);
        if (branchOnEqual)
          cannotFallThrough = true;
        ...
        if (vp->trace())
          traceMsg(vp->comp(), "   cmp children must be not equal by relative "
                               "constraint: %p != %p\n",
                   lhsChild, rhsChild);
        if (branchOnEqual)
          cannotBranch = true;
      ...
      if (objectRefConstraint) {
        ...
        if (typeConstraint) {
          ...
          if (resolvedTypeConstraint) {
            ...
            if (vp->trace())
              traceMsg(vp->comp(), "MyDebug: clazz %p classInfo %p "
          ...
          if (clazzToBeInitialized &&
              (((clazzNameLen == 35) &&
               ...
               ((clazzNameLen == 41) &&
                !strncmp(clazzToBeInitialized,
                         ...
                         clazzNameLen))) &&
              performTransformation(vp->comp(), "%sUsing side-effect guard to "
                                                "fold away condition on a load "
                                                "from an uninitializec class  "
                                                "%s [%p]\n",
            ...
            vp->_classesToCheckInit.add(
                new (vp->trStackMemory()) TR::ValuePropagation::ClassInitInfo(
  ...
  TR::ValuePropagation::EdgeConstraints *edgeConstraints =
      ...
      case TR_MethodTest: {
           ...
           vp->comp()->getSymRefTab()->findVftSymbolRef()))*/
        {
          ...
          if (ignoreVirtualGuard && classConstraint &&
            ...
            bool testForEquality = (node->getOpCodeValue() == TR::ifacmpeq);
            traceMsg(vp->comp(), "TR_MethodTest: node=%p, vtableEntryNode=%p, "
  ...
  if (instanceofConstraint && instanceofOnBranch) {
    if (instanceofDetected) {
      ...
      if (objectRefConstraint) {
        ...
        if (instanceClass) {
          ...
          newConstraint = genTypeResult(vp, objectRefConstraint, instanceClass,
                                        applyType, isInstanceOf);

          TR::VPConstraint *newTypeConstraint = newConstraint;
          if (newTypeConstraint && !newTypeConstraint->asClassType() &&
              newTypeConstraint->asClass())
          ...
          if (newTypeConstraint && instanceofDetectedAndFixedType &&
              newTypeConstraint->asResolvedClass() &&
              !newTypeConstraint->asFixedClass()) {
            TR_OpaqueClassBlock *clazz =
                newTypeConstraint->asResolvedClass()->getClass();
            if (clazz) {
              TR::VPFixedClass *newFixedConstraint =
        ...
        bool applyType = true;
        newConstraint =
            genTypeResult(vp, objectRefConstraint, instanceofConstraint,
                          applyType, isInstanceOf);
    ...
    /// if (!vp->addEdgeConstraint(instanceofObjectRef, instanceofConstraint,
    /// edgeConstraints))
    else if (!vp->addEdgeConstraint(instanceofObjectRef, instanceofConstraint,
                                    edgeConstraints))
      cannotBranch = true;
  }
  ...
  // no
  // intrinsic value of the comparison, and instead we end with way too many
  // objects
  // related together because they were all related to null.  This increases the
  // time
  // it takes to propagate constraints, but provides no useful value since we
  // track
  ...
  if (instanceofConstraint && !instanceofOnBranch) {
        ...
        if (instanceClass) {
          ...
          if (!newConstraint && applyType)
            removeBranch = true;
                   ...
                   !vp->addBlockConstraint(instanceofObjectRef, newConstraint,
                                           NULL, false))
            removeBranch = true;
          if (removeBranch) {
            if (!vp->intersectionFailed())
    ...
    } else if (!vp->addBlockConstraint(instanceofObjectRef,
                                       instanceofConstraint, NULL, false))
      cannotFallThrough = true;
  }
  ...
  if (cannotBranch && performTransformation(
    ...
    changeConditionalToGoto(vp, node, edge);
  } else if (constraintFromMethodPointer &&
             lhsChild->getFirstChild()
                 ->getFirstChild()
                 ->getOpCode()
                 .isLoadVarDirect()) {
    ...
    // flg=0x1020
    // n233n       iaload <vtable-entry-symbol> [#552 Shadow +768] [flags
    // 0x10607 0x0 ]              [0x7fffd51109b0] bci=[-1,0,2302] rc=1 vc=56
    // vn=7 sti=- udi=- nc=1
    // n232n         iaload <vft-symbol> [#449 Shadow] [flags 0x18607 0x0 ]
    // (X>=0 )                  [0x7fffd5110968] bci=[-1,0,2302] rc=1 vc=56 vn=6
    // sti=- udi=- nc=1 flg=0x100
    // n3n             ==>aload
    // n234n       aconst 0x7fd070 (methodPointerConstant )
    // [0x7fffd51109f8] bci=[0,0,314] rc=1 vc=56 vn=5 sti=- udi=- nc=0
    // flg=0x2000
    ...
    if (!disableProfiled2Overridden && callNode && vp->lastTimeThrough()) {
      ...
      if (rhs && !objectClass && lhs && lhs->getClassType() &&
        ...
        // the call
        // if we ask for a rammethod in such a class we would trigger an assert
        // ("no ram method in the vft slot")
        // to avoid this we would to insert another isInstanceOf (objectClass,
        // callClass)
      ...
      // can cut some corners and
      // use findSingleImplementer to get a concrete implementation
      // regardless of the type(e.g. virtual vs interface)
      // of a call and the type of a receiver (e.g. abstract, interface, normal
      // classes)
      // if findSingleImplementer returns a method than it must be the
      // implementation Inliner used since
              ...
              TR_yes) /*the object class may be less specific than callClass*/
      {
        ...
        if (methodSymbol &&
          ...
          // we should treat this case conservatively meaning we assume that
          // there are more than two concrete classes in the given hierarchy
          // most likely rvm will also be NULL in this case, but it is better to
          // not rely on this implicit connection between different
          // CH queries.
          ...
          /*
            ...
            instantiable class in the hierarchy
            starting from objectClass. If there is more than one concrete class
            in hierarchy
            overriding methods in objectClass and if any call was devirtualized
            by invariantargumentpreexisence downstream
            we will produce the functionally incorrect code by swapping a
            profiled test with a method test as
            the method test is insufficient for the devirtualizations done by
            invariantargumentpreexisence
          */
...
void getLimits(TR::ValuePropagation *vp, int32_t *zLow, int32_t *zHigh,
...
void getLimits(TR::ValuePropagation *vp, int64_t *zLow, int64_t *zHigh,
               TR::Node *symNode, bool isGlobal) {
  TR::VPConstraint *zConstraint = vp->getConstraint(symNode, isGlobal);
...
TR::Node *makeNewRhsNode(TR::ValuePropagation *vp, TR::Node *node,
...
static TR::Node *simplifyInequality(TR::ValuePropagation *vp, TR::Node *node,
  ...
  if ((lhsChild->getOpCode().isAdd() || lhsChild->getOpCode().isSub()) &&
    ...
    if ( // no overflow in x + a
      ...
      // (e.g. 200,000 for WCode).
      // Values larger than that limit are used for other purposes, so there
      // can't be any overlap. It's not safe
...
//
static TR::Node *constrainIfcmplessthan(TR::ValuePropagation *vp,
  ...
  if (!cannotBranch && !cannotFallThrough && !isUnsigned) {
    ...
    if (rel) {
      ...
      if (rel->asEqual() && !cannotFallThrough && !cannotBranch &&
        ...
        // way the comparison will go.
        // Likewise, since lhs-k == rhs, if we know lhs-k can't overflow, we're
        // also good to go.

        TR::VPEqual *relation = rel->asEqual();
        int32_t increment = relation->increment();

        int64_t maxIncrement = 0; // If absIncrement is more than this, overflow
                                  // may occur, and (for example) a==b+k does
                                  // not imply a>b
  ...
  if (lhsChild->getOpCode().isLong()) {
    ...
    if (longBoundary != TR::getMinSigned<TR::Int64>() && !isUnsigned) {
      if (!vp->addEdgeConstraint(
              rhsChild, TR::VPLongRange::create(vp, longBoundary,
                                                TR::getMaxSigned<TR::Int64>()),
              edgeConstraints)) {
        if (!vp->intersectionFailed())
          cannotBranch = true;
  ...
  if (lhsChild->getOpCode().isLong() && !isUnsigned) {
    ...
    if (longBoundary != TR::getMaxSigned<TR::Int64>()) {
      if (!vp->addBlockConstraint(
              rhsChild, TR::VPLongRange::create(
    ...
    if (longBoundary != TR::getMinSigned<TR::Int64>()) {
      if (!vp->addBlockConstraint(
              lhsChild, TR::VPLongRange::create(vp, longBoundary,
                                                TR::getMaxSigned<TR::Int64>()),
              NULL, false)) {
        if (!vp->intersectionFailed())
          cannotFallThrough = true;
  ...
  if (!lhsChild->getOpCode().isLong() && !rhsChild->getOpCode().isLong()) {
    ...
    // TR::VPGreaterThanOrEqual::create(vp, 1-adjustment);
    // if (newConstraint->increment() != 0)
    //   newConstraint->setHasArtificialIncrement();
...
//
static TR::Node *constrainCmpeqne(TR::ValuePropagation *vp, TR::Node *node,
                                  bool testEqual) {
  constrainChildren(vp, node);
  ...
  if (result >= 0) {
    ...
    if (secondChild->getOpCode().isLoadConst() && lhsIsBool &&
      ...
      if (((constValue == 1) && ((node->getOpCodeValue() == TR::icmpeq) ||
        ...
        if (performTransformation(vp->comp(), "%sReduced identity operation on "
                                              "bool in node "
...
//
static TR::Node *constrainCmplessthan(TR::ValuePropagation *vp, TR::Node *node,
                                      TR::Node *lhsChild, TR::Node *rhsChild,
                                      bool orEqual) {
  ...
  if (node->getOpCodeValue() != TR::table) {
    ...
    if (!isInt64 && constraint && constraint->asIntConstraint()) {
      ...
      if (low >= minCase && high <= maxCase)
        node->setCannotOverflow(true); // Let evaluator know value range of
                                       // selector is covered by min and max
                                       // case
    }                                  // if IntConstraint
...
static void constrainClassObjectLoadaddr(TR::ValuePropagation *vp,
...
void constrainNewlyFoldedConst(TR::ValuePropagation *vp, TR::Node *node,
                               bool isGlobal) {
  switch (node->getOpCodeValue()) {
  case TR::iconst:
...
//
static int32_t handleNullCheck(TR::ValuePropagation *vp, TR::Node *node,
                               bool resolveChkToo) {
...
//
static bool handleResolveCheck(TR::ValuePropagation *vp, TR::Node *node,
                               bool nullChkToo) {
  ...
  // this one
  // can only be removed if this constraint (int const 1) reaches them. This is
  // because seeing
  // a resolvechk for the load is not sufficient (in every case) to remove the
  // resolvechk on the dominated store (if the unresolved field happened to be
  // final, then the
...
TR::Node *constrainBndChkWithSpineChk(TR::ValuePropagation *vp,
                                      TR::Node *node) {
  constrainChildren(vp, node);
  ...
  if (sizeConstraint && !vp->comp()->generateArraylets()) {
    ...
    if (elementSize > 0 &&
        ...
        performTransformation(vp->comp(), "%sRemoving unnecessary spine check "
                                          "from composite node [%p] : min "
                                          "arraylength = %d, max "
  ...
  if ((sizeNode == indexNode) &&
      !((sizeNode->getOpCodeValue() == TR::iconst) &&
        (sizeNode->getInt() == 0))) {
    vp->mustTakeException();
    return node;
  }
    ...
    //
      ...
      if (sig && sig[0] == '[') {
        ...
        //
          ...
          // arrayComponentClass, object->isFixedClass(),
          // array->isFixedClass());
          ...
          if (isInstance == TR_yes) {
            ...
            if (vp->trace())
              traceMsg(vp->comp(), "Setting arrayStoreClass on ArrayStoreChk "
                                   "node [%p] to [%p]\n",
                       node, objectClass);
            node->setArrayStoreClassInNode(objectClass);
                   ...
                   !(TR::Options::getCmdLineOptions()->getOption(
                       TR_DisableArrayStoreCheckOpts)) &&
                   arrayComponentClass && objectClass &&
                   vp->fe()->isInstanceOf(objectClass, arrayComponentClass,
                                          true, true)) {
            if (vp->trace())
              traceMsg(vp->comp(), "Setting arrayComponentClass on "
                                   "ArrayStoreChk node [%p] to [%p]\n",
...
TR::Node *constrainArraycopy(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (node->getNumChildren() == 3) {
    ListIterator<TR::ValuePropagation::TR_TreeTopNodePair> treesIt1(
    ...
    if (!alreadyAdded)
      vp->_scalarizedArrayCopies.add(
          new (vp->comp()->trStackMemory())
              TR::ValuePropagation::TR_TreeTopNodePair(vp->_curTree, node));
  } else {
    vp->createExceptionEdgeConstraints(TR::Block::CanCatchArrayStoreCheck, NULL,
                                       node);
  }
...
TR::Node *constrainBCDToIntegral(TR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (node->getType().isInt64()) {
    ...
    // the isNonNegative (x >= 0) flag set,
    // we know that it can't possibly be represented as a negative number, so
    // the min value for the constraint must be at least 0
