...
public:
  ...
  TR_Dominators(TR::Compilation *c, TR::ResolvedMethodSymbol *methSym,
                bool post = false);
...
private:
  ...
  public:
    BBInfo(TR::Allocator allocator)
        : _bucket(allocator), _block(NULL), _parent(-1), _idom(-1),
          _ancestor(-1), _label(-1), _child(-1), _sdno(-1), _size(0) {}
...
public:
  ...
  TR_PostDominators(TR::Compilation *comp)
      : TR_Dominators(comp, true),
        _directControlDependents(comp->getFlowGraph()->getNumberOfNodes() + 1,
