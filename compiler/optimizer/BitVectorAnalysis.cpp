...
bool TR_BasicDFSetAnalysis<Container *>::performAnalysis(
  ...
  // this->getKind(), comp()->getFlowGraph()->getNextNodeNumber(),
  // getNumberOfBits());
  // comp()->printMemStatsBefore("DJS - Before DFA");
  // Table of bit vectors to be used during the analysis.
  CS2::TableOf<TR::BitVector, TR::Allocator> bvTable(64, comp()->allocator());
...
template <class Container>
TR::BitVector *TR_BasicDFSetAnalysis<Container *>::allocateBitVector() {
    ...
    if (!_hasImproperRegion) {
      ...
      if (traceBVA())
        dumpOptDetails(comp(), "\n ************** Completed initialization of "
                               "gen and kill sets for all structures "
                               "************* \n");
    }
  } else {
...
void TR_ForwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfoForRegion(
...

  {
    analysisInfo->_regularGenSetInfo =
        new (this->trStackMemory()) TR_LinkHead<typename TR_BasicDFSetAnalysis<
...
void TR_ForwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfo(
  ...
  while (this->_analysisQueue.getListHead() &&
    ...
    if (node != regionStructure->getEntry()) {
      ...
      predList.insert(predList.end(), node->getExceptionPredecessors().begin(),
                      node->getExceptionPredecessors().end());
      bool firstPred = true;
    ...
    *this->_temp2 = *_currentRegularKillSetInfo;
    ...
    succList.insert(succList.end(), node->getExceptionSuccessors().begin(),
                    node->getExceptionSuccessors().end());
    int count = 0;
    ...
    if (this->supportsGenAndKillSets() &&
      ...
      for (auto succ = succList.begin(); succ != succList.end(); ++succ) {
        ...
        if (bitVector) {
          *_info -= *bitVector;
          if (this->traceBVA()) {
            traceMsg(this->comp(), "\n%sKill Set Info for Region or Block : %p "
                                   "numbered %d and exit %d is : \n",
                     (normalSucc ? "" : "E"), nodeStructure->getStructure(),
                     nodeStructure->getStructure()->getNumber(),
                     succNode->getNumber());
            bitVector->print(this->comp());
            traceMsg(this->comp(), "\n%s Info for Region or Block : %p "
                                   "numbered %d and exit %d is : \n",
        ...
        if (bitVector) {
          *_info |= *bitVector;
          if (this->traceBVA()) {
            traceMsg(this->comp(), "\n%sGen Set Info for Region or Block : %p "
                                   "numbered %d and exit %d is : \n",
                     (normalSucc ? "" : "E"), nodeStructure->getStructure(),
                     nodeStructure->getStructure()->getNumber(),
                     succNode->getNumber());
            bitVector->print(this->comp());
            traceMsg(this->comp(), "\n1%s Info for Region or Block : %p "
                                   "numbered %d and exit %d is : \n",
                     (normalSucc ? "Normal" : "Exception"),
                     nodeStructure->getStructure(),
                     nodeStructure->getStructure()->getNumber(),
