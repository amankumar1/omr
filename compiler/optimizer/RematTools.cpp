...
TR_YesNoMaybe RematTools::gatherNodesToCheck(
  ...
  if (opCode.hasSymbolReference() && !opCode.isLoad()) {
    if (trace)
      traceMsg(comp, "  priv arg remat: Can't fully remat [%p] due to [%p] - "
    ...
    if (comp->requiresSpineChecks() &&
      ...
      if (trace)
        traceMsg(comp, "  priv arg remat: Can't fully remat [%p] due to [%p] - "
                       "array access needs spine check",
                 privArg, currentNode);
    ...
    if (trace)
      traceMsg(comp, "  guarded call remat: Can't fully remat [%p] due to [%p] "
                     "- unhandled case",
               privArg, currentNode);
    return TR_no;
  }
 ...
 * scanning. Any symrefs killed after they have been added to the set of
 * interest
 ...
 * from
 * if we can't full rematerialize a privatized inliner arg. If we find a store
 * of
 * one of the failed arguments, we record the store node and add the symref to
 * the
 * dependencies for the arg. If the symref is not killed before the end of the
 * block
...
void RematTools::walkTreeTopsCalculatingRematFailureAlternatives(
  ...
  while (start != end) {
    if (trace)
      traceMsg(comp, "  priv arg remat: visiting [%p]: isStore %d privArg %d "
                     "failedTargetMatch %d",
    ...
    if (start->getNode() && start->getNode()->getOpCode().isStoreDirect() &&
      ...
      for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
        if ((*iter) && (*iter)->getNode()->getOpCode().hasSymbolReference() &&
          ...
          if (trace)
            traceMsg(comp, "  priv arg remat: attempting partial remat with "
                           "node [%p] for [%p]",
