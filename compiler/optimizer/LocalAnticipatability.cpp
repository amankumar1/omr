...
void TR_LocalAnticipatability::analyzeBlock(TR::Block *block,
  ...
  // to avoid
  // running into problems where there are two loads of the same symbol (one
  // before a def
  // point (say a call) and one after the def point in a given basic block). In
  // this case we MUST say that
  // the symbol load is not anticipatable as we do not want to common the two
  // different uses
        ...
        if ((child->getLocalIndex() != MAX_SCOUNT) &&
          ...
          if (killedExpressions->get(child->getLocalIndex()) &&
              ...
              !_localTransparency->getAnalysisInfo(block->getNumber())
                   ->get(node->getLocalIndex())) {
            if (trace())
              traceMsg(comp(), "\n55Definition #%d is NOT locally "
                               "anticipatable in block_%d\n",
                       node->getLocalIndex(), block->getNumber());
            killedExpressions->set(node->getLocalIndex());
            _info[block->getNumber()]._analysisInfo->reset(
                node->getLocalIndex());
      ...
      if (childOpCode.hasSymbolReference() &&
        ...
        if (seenDefinedSymbolReferences->get(
          ...
          if (trace())
            (TR::Compiler->target.is64Bit())
                ? traceMsg(comp(), "\n330Definition #%d (aladd) is NOT locally "
                                   "anticipatable in block_%d because of "
                                   "child\n",
                           node->getLocalIndex(), block->getNumber())
                : traceMsg(comp(), "\n330Definition #%d (aiadd) is NOT locally "
                                   "anticipatable in block_%d because of "
...
void TR_LocalAnticipatability::updateUsesAndDefs(
  ...
  if (opCode.hasSymbolReference() &&
    ...
    if (opCode.isResolveCheck()) {
      ...
      TR_NodeKillAliasSetInterface aliasSet =
    ...
    if ((opCode.isLoadVar() ||
      ...
      *seenUsedSymbolReferences = *temp;
      ...
      if (opCode.isStore()) {
        ...
        //
        if (_registersScarce || (node->getSymbolReference() ==
