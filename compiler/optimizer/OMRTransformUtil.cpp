...
TR::Node *OMR::TransformUtil::scalarizeArrayCopy(
  ...
  if (targetRef == NULL || sourceRef == NULL) {
    if (trace)
      traceMsg(comp, "do not scalarizeArrayCopy node %p : targetRef is NULL "
                     "(%s) or sourceRef is NULL (%s)\n",
...
TR::Node *OMR::TransformUtil::scalarizeAddressParameter(
  ...
  TR_ASSERT(ref, "symRef should not be NULL in scalarizeAddressParameter for "
                 "address node %p\n",
            address);
