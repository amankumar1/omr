    ...
    if (_useDefInfo == NULL && !_optimizer->cantBuildLocalsUseDefInfo()) {
      if (!(requiresGlobals && _optimizer->cantBuildGlobalsUseDefInfo())) {
        ...
        _useDefInfo = new (comp->allocator())
            TR_UseDefInfo(comp, comp->getFlowGraph(), optimizer,
                          requiresGlobals, prefersGlobals);
...
bool TR_ValueNumberInfo::congruentNodes(TR::Node *node, TR::Node *entryNode) {
  ...
  if (node->getType().isBCD()) {
    if (!node->isDecimalSizeAndShapeEquivalent(entryNode)) {
      if (trace())
        traceMsg(comp(), "BCD node %s (%p) and BCD entryNode %s (%p) have "
                         "size/shape mismatch -- do not consider as matching\n",
    ...
    if (!node->isSignStateEquivalent(entryNode)) {
      if (trace() || comp()->cg()->traceBCDCodeGen())
        traceMsg(comp(), "x^x : BCD node %s (%p) and BCD entryNode %s (%p) "
                         "have sign state mismatch -- do not consider as "
                         "matching\n",
                 node->getOpCode().getName(), node,
                 entryNode->getOpCode().getName(), entryNode);
      return false;
    ...
    if (trace())
      traceMsg(comp(), "fracConv node %s (%p) and fracConv entryNode %s (%p) "
                       "have fraction mismatch -- do not consider as "
                       "matching\n",
               node->getOpCode().getName(), node,
               entryNode->getOpCode().getName(), entryNode);
    return false;
    ...
    if (trace())
      traceMsg(comp(), "castedToBCD mismatch : node %s (%p) castedToBCD %d and "
                       "entryNode %s (%p) castedToBCD %d -- do not consider as "
                       "matching\n",
...
void TR_ValueNumberInfo::initializeNode(TR::Node *node,
  ...
  for (entry = _hashTable._buckets[hashValue]; entry; entry = entry->_next) {
    ...
    if (node->getOpCode().isSetSignOnNode() &&
        node->getSetSign() != entryNode->getSetSign()) {
      if (trace())
        traceMsg(comp(), "Nodes %s (%p) and entryNode %s (%p) with "
                         "setSignOnNode have sign mismatch -- do not consider "
                         "as matching\n",
                 node->getOpCode().getName(), node,
                 entryNode->getOpCode().getName(), entryNode);
      continue;
    ...
    if (node->getType().isBCD()) {
      if (!node->isDecimalSizeAndShapeEquivalent(entryNode)) {
        if (trace())
          traceMsg(comp(), "BCD node %s (%p) and BCD entryNode %s (%p) have "
                           "size/shape mismatch -- do not consider as "
                           "matching\n",
                   node->getOpCode().getName(), node,
                   entryNode->getOpCode().getName(), entryNode);
        continue;
      ...
      continue;
               ...
               node->castedToBCD() != entryNode->castedToBCD()) {
      if (trace())
        traceMsg(comp(), "castedToBCD mismatch : node %s (%p) castedToBCD %d "
                         "and entryNode %s (%p) castedToBCD %d -- do not "
      ...
      continue;
               ...
               node->getDFPPrecision() != entryNode->getDFPPrecision()) {
      if (trace())
        traceMsg(comp(), "DFP node %s (%p) and entryNode %s (%p) have "
                         "different precisions -- do not consider as "
                         "matching\n",
                 node->getOpCode().getName(), node,
                 entryNode->getOpCode().getName(), entryNode);
      continue;
  ...
  if (defNode) {
    ...
    if (baseVN == -1 || (defNode->getOpCode().isIndirect() &&
      ...
      if (value >= 0) {
        ...
        if (trace()) {
          traceMsg(comp(), "  Change value number for load %d at [%p] to value "
                           "number %d of dominating load %d at [%p]\n",
      ...
      if (trace()) {
        traceMsg(comp(), "  Use value number %d of dominating load %d at [%p] "
                         "for load %d at [%p]\n",
  ...
  if (!definingLoads.IsZero() && node->getOpCode().isLoadVarDirect()) {
    ...
    if (defValue >= 0) {
      if (value >= 0) {
        ...
        if (trace()) {
          traceMsg(comp(), "  2Use value number %d of dominating load %d at "
                           "[%p] for load %d at [%p]\n",
                   getVN(defNode), defNode->getGlobalIndex(), defNode,
                   node->getGlobalIndex(), node);
        }
      ...
      if (!usesFromDefs.IsZero()) {
        ...
        for (cursor2.SetToFirstOne(); cursor2.Valid(); cursor2.SetToNextOne()) {
          ...
          if (useNode && (useNode != node) &&
            ...
            if (!otherUseDefiningLoads.IsZero() &&
              ...
              if (baseVN == -1 || (useNode->getOpCode().isIndirect() &&
                ...
                if (value >= 0) {
                  ...
                  if (trace()) {
                    traceMsg(comp(), "  Change value number for load %d at "
                                     "[%p] to value number %d of load %d at "
                                     "[%p] reached by same dominating loads "
                                     "(defs)\n",
                ...
                if (trace()) {
                  traceMsg(comp(), "  Use value number %d of dominating load "
                                   "%d at [%p] for load %d at [%p]\n",
