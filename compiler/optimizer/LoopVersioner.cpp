...
int32_t TR_LoopVersioner::performWithoutDominators() {
  ...
  for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL;
    ...
    bool nullChecksMayBeEliminated = detectChecksToBeEliminated(
        ...
        &divCheckTrees, &iwrtbarTrees, &checkCastTrees, &arrayStoreCheckTrees,
    ...
    if (!shouldOnlySpecializeLoops() && !refineAliases())
      checkCastTreesWillBeEliminated =
          detectInvariantTrees(&checkCastTrees, false, &containsNonInlineGuard,
    ...
    if (!shouldOnlySpecializeLoops() && !refineAliases()) {
      if (((debug("nullCheckVersion") ||
          ...
          iwrtBarsWillBeEliminated || checkCastTreesWillBeEliminated ||
          arrayStoreCheckTreesWillBeEliminated ||
        ...
        conditionalsWillBeEliminated =
            detectInvariantTrees(&conditionalTrees, true,
                                 &containsNonInlineGuard, reverseBranchInLoops);
      } else {
        conditionalsWillBeEliminated =
            detectInvariantTrees(&conditionalTrees, false,
                                 &containsNonInlineGuard, reverseBranchInLoops);
    ...
    if ((versionToRefineAliases || !refineAliases()) &&
      ...
      versionNaturalLoop(naturalLoop, &nullCheckedReferences, &nullCheckTrees,
                         ...
                         &divCheckTrees, &iwrtbarTrees, &checkCastTrees,
      ...
      if (hotGuard->getOpCode().isIf() &&
        ...
        if (!useOldLoopTransfer) {
          ...
          if (doIt) {
            ...
            if (comp()->getOptions()->dynamicDebugCounterIsEnabled(
                    debugCounter)) {
          ...
          if (doIt && performTransformation(
                          comp(), "%sLoop transfer for guard %p in loop %d\n",
                          OPT_DETAILS_LOOP_VERSIONER, hotGuard,
                          vgInfo->_loopEntry->getNumber())) {
...
void TR_LoopVersioner::fixupVirtualGuardTargets(VirtualGuardInfo *vgInfo) {
  ...
  VGLoopInfo **distinctTargets = (VGLoopInfo **)trMemory()->allocateStackMemory(
      numNodes * sizeof(VGLoopInfo *));
    ...
    if (nextVG->_isGuarded) {
      if (nextVG->_isInsideInnerLoop) {
        ...
        if (!distinctTargets[num]) {
          distinctTargets[num] =
              (VGLoopInfo *)trMemory()->allocateStackMemory(sizeof(VGLoopInfo));
  ...
  //
  TR::SymbolReference *vgTempSymRef = comp()->getSymRefTab()->createTemporary(
  ...
  for (i = 0; i < numNodes; i++)
    if (distinctTargets[i] && distinctTargets[i]->_count != 0) {
      ...
      TR::Block *invariantBlock =
          distinctTargets[i]->_coldGuardLoopInvariantBlock;
      if (!invariantBlock) {
        _cfg->setStructure(rootStructure);
        for (auto e = coldGuardLoopEntry->getPredecessors().begin();
             e != coldGuardLoopEntry->getPredecessors().end(); ++e) {
        ...
        if (!invariantBlock) {
          ...
          for (auto e = coldGuardLoopEntry->getPredecessors().begin();
            ...
            TR_RegionStructure *region =
                coldGuardLoopEntry->getStructureOf()->getParent()->asRegion();
          ...
          _cfg->addEdge(TR::CFGEdge::createEdge(
              invariantBlock, coldGuardLoopEntry, trMemory()));
            ...
            if (from->getNextBlock() == coldGuardLoopEntry) {
              if (branchNode->getOpCode().isBranch()) {
                ...
                if (branchNode->getBranchDestination()->getNode()->getBlock() ==
                  ...
                  _cfg->addEdge(TR::CFGEdge::createEdge(from, invariantBlock,
                                                        trMemory()));
                } else {
                  TR::Block *gotoBlock =
                      createEmptyGoto(NULL, invariantBlock, NULL);
                  ...
                  _cfg->addEdge(TR::CFGEdge::createEdge(
                      gotoBlock, invariantBlock, trMemory()));
                  _cfg->addEdge(
                      TR::CFGEdge::createEdge(from, gotoBlock, trMemory()));
                  dumpOptDetails(comp(),
                                 "created goto block_%d to invariant block\n",
                                 gotoBlock->getNumber());
                }
              } else {
                TR::Node *gotoNode = TR::Node::create(
                    branchNode, TR::Goto, 0, invariantBlock->getEntry());
                TR::TreeTop *gotoTT = TR::TreeTop::create(comp(), gotoNode);
                from->append(gotoTT);
                _cfg->addEdge(
                    TR::CFGEdge::createEdge(from, invariantBlock, trMemory()));
              }
            } else {
              // last tree ends in branch, fix destination
              //
      ...
      if (!loopTempSymRef) {
        loopTempSymRef = comp()->getSymRefTab()->createTemporary(
            comp()->getMethodSymbol(), TR::Int32);
        distinctTargets[i]->_loopTempSymRef = loopTempSymRef;
        // reset the selector
        //
        clonedHeader->prepend(TR::TreeTop::create(
            comp(),
            TR::Node::createWithSymRef(
                TR::istore, 1, 1, TR::Node::create(node, TR::iconst, 0,
                                                   distinctTargets[i]->_count),
                loopTempSymRef)));
      ...
      TR::Node *loadTemp =
          TR::Node::createWithSymRef(node, TR::iload, 0, loopTempSymRef);
      TR::Node *subNode = TR::Node::create(
          TR::isub, 2,
          TR::Node::create(node, TR::iconst, 0, distinctTargets[i]->_count),
          loadTemp);
      TR::Node *storeTemp =
          TR::Node::createWithSymRef(TR::istore, 1, 1, subNode, loopTempSymRef);
      ...
      TR::Node *innerSwitchNode = createSwitchNode(cloneBlock, loopTempSymRef,
                                                   distinctTargets[i]->_count);
      ...
      //
      cloneBlock->prepend(TR::TreeTop::create(
          comp(), TR::Node::createWithSymRef(
                      TR::istore, 1, 1,
                      TR::Node::create(innerSwitchNode, TR::iconst, 0, 0),
                      loopTempSymRef)));
      for (nextVG = vgIt.getFirst(); nextVG; nextVG = vgIt.getNext()) {
        if (nextVG->_isGuarded) {
  ...
  TR::Node *switchNode =
      createSwitchNode(clonedHeader, vgTempSymRef, caseKonst + numDistinct);
  switchBlock->prepend(TR::TreeTop::create(comp(), switchNode));

  // store the default value of the
  // temp in the loop invariant block
  //
  TR::Node *defaultStore = TR::Node::createWithSymRef(
      TR::istore, 1, 1, TR::Node::create(switchNode, TR::iconst, 0, 0),
      vgTempSymRef);
  vgInfo->_coldLoopInvariantBlock->prepend(
      TR::TreeTop::create(comp(), defaultStore));

  // reset the temp on entry to the loop
  //
  clonedHeader->prepend(TR::TreeTop::create(
      comp(),
      TR::Node::createWithSymRef(TR::istore, 1, 1,
                                 TR::Node::create(switchNode, TR::iconst, 0, 0),
                                 vgTempSymRef)));

  vgIt.reset();
  for (nextVG = vgIt.getFirst(), caseKonst = 0; nextVG;
       nextVG = vgIt.getNext()) {
    /// getDebug()->printIRTrees(comp()->getOutFile(), "after processing IR
    /// trees:", comp()->getMethodSymbol());
    ...
    if (nextVG->_isGuarded) {
      ...
      TR::Block *oldDestBlock =
          hotGuardNode->getBranchDestination()->getNode()->getBlock();
      TR::CFGEdge *oldDestEdge = hotGuardBlock->getEdge(oldDestBlock);

      // create the gotoblock and store value into the temp
      //
      TR::Block *gotoBlock =
          createEmptyGoto(hotGuardBlock, switchBlock, endTree);
      endTree = gotoBlock->getExit();
      _cfg->addNode(gotoBlock);
      _cfg->addEdge(
          TR::CFGEdge::createEdge(hotGuardBlock, gotoBlock, trMemory()));
      _cfg->addEdge(
          TR::CFGEdge::createEdge(gotoBlock, switchBlock, trMemory()));
      ...
      if (nextVG->_isInsideInnerLoop) {
        ...
        storeTemp = TR::Node::createWithSymRef(
            TR::istore, 1, 1,
            TR::Node::create(hotGuardNode, TR::iconst, 0,
                             distinctTargets[loopNum]->_targetNum),
            vgTempSymRef);
        int32_t vgNum = --(distinctTargets[loopNum]->_count);
        TR::Node *vgNumTemp = TR::Node::createWithSymRef(
            TR::istore, 1, 1,
            TR::Node::create(hotGuardNode, TR::iconst, 0, vgNum),
            distinctTargets[loopNum]->_loopTempSymRef);
        ...
        for (LoopTemps *l = selectorTemps.getFirst(); l; l = l->getNext()) {
          if (l->_symRef == distinctTargets[loopNum]->_loopTempSymRef)
            continue;
          TR::Node *resetNode = TR::Node::createWithSymRef(
              TR::istore, 1, 1,
        ...
        TR::Node *innerSwitchNode =
            nextVG->_coldGuardLoopEntryBlock->getFirstRealTreeTop()->getNode();
        ...
        //
        TR::TreeTop *defaultTT =
            innerSwitchNode->getSecondChild()->getBranchDestination();
        if (innerCaseNode->getBranchDestination() == defaultTT)
          innerCaseNode->setBranchDestination(
              coldGuardNode->getBranchDestination());
        TR::Block *callBlock =
            coldGuardNode->getBranchDestination()->getNode()->getBlock();

        bool addIt = true;
        for (auto e = nextVG->_coldGuardLoopEntryBlock->getSuccessors().begin();
             e != nextVG->_coldGuardLoopEntryBlock->getSuccessors().end();
             ++e) {
        ...
        if (addIt)
          _cfg->addEdge(TR::CFGEdge::createEdge(
              nextVG->_coldGuardLoopEntryBlock, callBlock, trMemory()));

        branchDest =
            distinctTargets[loopNum]->_coldGuardLoopInvariantBlock->getEntry();
        ...
        storeTemp = TR::Node::createWithSymRef(
            TR::istore, 1, 1,
            TR::Node::create(hotGuardNode, TR::iconst, 0, ++caseKonst),
            vgTempSymRef);
      ...
      for (auto e = switchBlock->getSuccessors().begin();
           e != switchBlock->getSuccessors().end(); ++e) {
        if ((*e)->getTo() == tgtBlock) {
          addIt = false;
          break;
        }
      ...
      if (addIt)
        _cfg->addEdge(
            TR::CFGEdge::createEdge(switchBlock, tgtBlock, trMemory()));
    }
  }
}
...
bool TR_LoopVersioner::detectInvariantNodes(
  ...
  for (; nextNode;) {
    ...
    // their children intact and so they can't be marked as invariant
...
bool TR_LoopVersioner::detectInvariantTrees(
    List<TR::TreeTop> *trees, bool onlyDetectHighlyBiasedBranches,
    bool *containsNonInlineGuard, SharedSparseBitVector &reverseBranchInLoops) {
    ...
    if (includeComparisonTree) {
      ...
      if (onlyDetectHighlyBiasedBranches && (thisChild || nextRealNode) &&
        ...
        if (guardInfo->getKind() == TR_DummyGuard ||
            !guardInfo->getInnerAssumptions().isEmpty()) // inner assumption may
                                                         // have receivers other
                                                         // than 'thisChild'
...
TR::Node *TR_LoopVersioner::isDependentOnInvariant(TR::Node *useNode) {
  ...
  if (trace())
    traceMsg(comp(), "Definition Counts for node [%p] is %d inside "
...
bool TR_LoopVersioner::detectInvariantIwrtbars(
    List<TR::TreeTop> *iwrtbarTrees) {
      ...
      if (baseChild->getOpCode().hasSymbolReference() &&
        ...
        if (isBaseInvariant) {
          if (_checksInDupHeader.find(nextTree->getData())) {
            ...
            // used inside
            // isDependentOnAllocation may not give the right answer if the expr
            // is outside the loop
            // isExprInvariant is relevant mostly in the context of a loop is my
            // worry
            //
...
bool opCodeIsHoistable(TR::Node *node, TR::Compilation *comp) {
  ...
  // If we move effectful stuff, things
...
bool TR_LoopVersioner::isExprInvariantRecursive(TR::Node *node,
  ...
  if (opCode.hasSymbolReference()) {
    TR::SymbolReference *symReference = node->getSymbolReference();
    if ((_seenDefinedSymbolReferences->get(
             symReference->getReferenceNumber()) &&
         (!ignoreHeapificationStore ||
          _writtenAndNotJustForHeapification->get(
              symReference->getReferenceNumber()))) ||
...
bool TR_LoopVersioner::hasWrtbarBeenSeen(List<TR::TreeTop> *iwrtbarTrees,
                                         TR::Node *iwrtbarNode) {
  ListElement<TR::TreeTop> *nextTree = iwrtbarTrees->getListHead();
  for (; nextTree;) {
    TR::Node *node = nextTree->getData()->getNode();
...
int32_t *computeCallsiteCounts(TR_ScratchList<TR::Block> *loopBlocks,
  ...
  for (TR::Block *block = loopItr.getCurrent(); block;
    ...
    for (OMR::TreeTop *tt = block->getFirstRealTreeTop();
      ...
      // because
      // it throws the treetop count relative to non-weak memory platforms where
      // we may
  ...
  if (comp->getMethodHotness() >= hot && callSymRef) {
    if (callSymRef->getSymbol() &&
      ...
      if (disableLoopCodeRatioCheck || loopCodeRatio < 0.25) {
        traceMsg(comp, "Skipping versioning of profiled guard %p because we "
                       "found more than 2 JIT'd implementors at warm or above "
                       "and the loop code ratio is too low\n",
                 guardNode);
        risky = true;
        TR::DebugCounter::incStaticDebugCounter(
            comp, TR::DebugCounter::debugCounterName(
                      comp, "profiledVersioning/unsuitableForVersioning/"
      ...
      if (trace()) {
        traceMsg(comp, "Found profiled guard %p is for a method subject to "
                       "phase change - skipping versioning\n",
                 guardNode);
      }
      risky = true;
    }
...
bool TR_LoopVersioner::isBranchSuitableToVersion(
  ...
  if (node->isProfiledGuard()) {
    ...
    if (isGetFieldCacheCounts || disableProfiledGuardVersioning) {
      ...
      TR_ValueProfileInfoManager *profileManager =
          TR_ValueProfileInfoManager::get(comp);
      TR_AddressInfo *valueInfo =
          (TR_AddressInfo *)profileManager->getValueInfo(
              ics._byteCodeInfo, comp,
      ...
      if (valueInfo) {
        ...
        if (valueInfo->getTopProbability() >=
          ...
          suitableForVersioning = checkProfiledGuardSuitability(
              loopBlocks, node, comp->getInlinedCallerSymRef(
                                    node->getByteCodeInfo().getCallerIndex()),
              comp);
        } else {
          suitableForVersioning = false;
...
bool TR_LoopVersioner::detectChecksToBeEliminated(
    ...
    List<TR::TreeTop> *divCheckTrees, List<TR::TreeTop> *iwrtbarTrees,
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
    ...
    if (nextBlock->isCold()) {
      ...
      if (blockFrequency >= 0 // frequency must be valid
          ...
          performTransformation(comp(), "%sDisregard unimportant block_%d "
                                        "frequency %d < loop %d frequency %d\n",
                                OPT_DETAILS_LOOP_VERSIONER,
                                nextBlock->getNumber(), blockFrequency,
                                whileLoop->getNumber(), loopFrequency)) {
        isUnimportant = true;
        if (trace() ||
            comp()->getOptions()->getOption(TR_CountOptTransformations)) {
      ...
      if ((currentNode->getNumChildren() > 0) &&
        ...
        if (!currentNode->getFirstChild()
          ...
          if (!guard) {
            ...
            // a guarded inlined call without a
        ...
        if (currentOpCode.isNullCheck()) {
          ...
          if (dupOfThisBlockAlreadyExecutedBeforeLoop)
            _checksInDupHeader.add(currentTree);
                       ...
                       .isCheckCast()) // (currentOpCode.getOpCodeValue() ==
                                       // TR::checkcast)
        {
          TR_ASSERT(currentNode->getSecondChild()->getOpCodeValue() ==
                        TR::loadaddr,
                    "second child should be LOADADDR");
          ...
          if (!comp()->fe()->isUnloadAssumptionRequired(
            ...
            if (trace()) // if we move the checkcast before the inlined body
              ...
              traceMsg(comp(), "Class for Checkcast %p is not loaded by the "
                               "same classloader as the compiled method\n",
                       currentTree->getNode());
          }
    ...
    if (node->getType().isInt32() ||
      ...
      if (/* comp()->getRecompilationInfo() && */ collectProfiledExprs &&
        ...
        TR_ValueInfo *valueInfo =
            (TR_ValueInfo *)TR_ValueProfiler::getProfiledValueInfo(node,
                                                                   comp());
        if (valueInfo) {
          if ((valueInfo->getTopProbability() > MIN_PROFILED_FREQUENCY) &&
              (valueInfo->_totalFrequency > 0) && !_containsCall &&
            ...
            if ((!node->getType().isInt64()) ||
              ...
              if (comp()->requiresSpineChecks() &&
                ...
                dumpOptDetails(comp(), "From value profiling, NOT USING "
                                       "inconclusive contigarraylength node %p "
                                       "with value 0 freq %d total freq %d\n",
                               node,
                               ((int32_t)(valueInfo->getTopProbability() *
                                          ((float)valueInfo->_totalFrequency))),
                ...
                //
                /////if (debug("traceLVE"))
                dumpOptDetails(comp(), "From value profiling, node %p has "
                                       "value %d freq %d total freq %d\n",
...
void TR_LoopVersioner::versionNaturalLoop(
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
    ...
    if (lastTree->getOpCode().isIf() &&
      ...
      traceMsg(comp(), "virtualGuardPair at guard node %p hotGuardBlock %d "
                       "coldGuardBlock %d\n",
      ...
      if (clonedSucc) {
        ...
        if (!fallsThrough)
          ...
          TR_BlockStructure *newGotoBlockStructure = new (trHeapMemory())
              TR_BlockStructure(comp(), newGotoBlock->getNumber(),
                                newGotoBlock);
  ...
  if (_requiresAdditionalCheckForIncrement &&
      performTransformation(comp(), "%s Creating test outside loop for "
                                    "checking if loop driving induction "
                                    "variable is incremented by a loop "
  ...
  if (_loopConditionInvariant && _asyncCheckTree) {
...
#ifdef J9_PROJECT_SPECIFIC
      TR_ValueInfo *valueInfo =
          (TR_ValueInfo *)TR_ValueProfiler::getProfiledValueInfo(loopLimit,
                                                                 comp());
      if (valueInfo) {
        if (valueInfo->getTotalFrequency()) {
          dumpOptDetails(
              comp(), "From value profiling, loop test node %p has value %d "
          ...
          // if (shouldOnlySpecializeLoops())
          {
            ...
            TR_ScratchList<TR_ExtraAbstractInfo> valuesSortedByFrequency(
                trMemory());
            valueInfo->getSortedList(
                comp(), (List<TR_ExtraAbstractInfo> *)&valuesSortedByFrequency);
            ListIterator<TR_ExtraAbstractInfo> sortedValuesIt(
                (List<TR_ExtraAbstractInfo> *)&valuesSortedByFrequency);

            float totalFreq = (float)valueInfo->getTotalFrequency();
            float valuedFreq = 0;

            for (TR_ExtraAbstractInfo *profiledInfo = sortedValuesIt.getFirst();
                 profiledInfo != NULL;
            ...
            if (valueInfo->getMaxValue() > profiledLoopLimit &&
              ...
              dumpOptDetails(comp(), "Maximum profiled value of %d exceeds "
                                     "profiled loop limit of %d, and value "
                                     "profiled iterations below limit account "
                                     "for only %5.3f of iterations.  Delaying "
                                     "asyncCheck removal for node %p\n",
    ...
    if (!refineAliases() && _canPredictIters &&
        !comp()->isProfilingCompilation() &&
        performTransformation(comp(), "%s Creating test outside loop for "
                                      "deciding if async check is required\n",
                              OPT_DETAILS_LOOP_VERSIONER)) {
      TR::Node *duplicateLoopLimit =
          _loopTestTree->getNode()->getSecondChild()->duplicateTree();
      if (isIncreasing)
        duplicateLoopLimit = TR::Node::create(
            TR::isub, 2, duplicateLoopLimit,
            _loopTestTree->getNode()->getFirstChild()->duplicateTree());
      else {
      ...
      collectAllExpressionsToBeChecked(
          nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
          duplicateLoopLimit, &comparisonTrees, clonedLoopInvariantBlock,
          comp()->incVisitCount());
      TR::Node *nextComparisonNode =
          TR::Node::createif(comparisonOpCode, duplicateLoopLimit,
                             TR::Node::create(duplicateLoopLimit, TR::iconst, 0,
                                              profiledLoopLimit),
                             clonedLoopInvariantBlock->getEntry());
      ...
      comparisonTrees.add(nextComparisonNode);
      dumpOptDetails(comp(), "The node %p has been created for testing if "
  ...
  if (comp()->cg()->performsChecksExplicitly() ||
    ...
    if (!nullCheckTrees->isEmpty() && !shouldOnlySpecializeLoops()) {
      buildNullCheckComparisonsTree(nullCheckedReferences, nullCheckTrees,
                                    boundCheckTrees, divCheckTrees,
                                    checkCastTrees, arrayStoreCheckTrees,
  ...
  if (!boundCheckTrees->isEmpty()) {
    ...
    buildBoundCheckComparisonsTree(
        nullCheckTrees, boundCheckTrees, divCheckTrees, checkCastTrees,
        arrayStoreCheckTrees, spineCheckTrees, &comparisonTrees,
        clonedLoopInvariantBlock, reverseBranch);
  }
  ...
  if (shouldOnlySpecializeLoops() && !spineCheckTrees->isEmpty()) {
    buildSpineCheckComparisonsTree(
        nullCheckTrees, spineCheckTrees, divCheckTrees, checkCastTrees,
        arrayStoreCheckTrees, &comparisonTrees, clonedLoopInvariantBlock);
  }
  ...
  if (!divCheckTrees->isEmpty() && !shouldOnlySpecializeLoops()) {
    buildDivCheckComparisonsTree(nullCheckTrees, divCheckTrees, checkCastTrees,
                                 arrayStoreCheckTrees, &comparisonTrees,
  ...
  // write barriers.
  //
  if (!iwrtbarTrees->isEmpty() && !shouldOnlySpecializeLoops()) {
    buildIwrtbarComparisonsTree(iwrtbarTrees, nullCheckTrees, divCheckTrees,
                                checkCastTrees, arrayStoreCheckTrees,
                                &comparisonTrees, clonedLoopInvariantBlock);
  }
  ...
  if (!specializedNodes->isEmpty()) {
    ...
    bool specializedLongs = buildSpecializationTree(
        nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        &comparisonTrees, specializedNodes, clonedLoopInvariantBlock,
        invariantBlock, symRefs);
  ...
  if (!conditionalTrees->isEmpty())
    buildConditionalTree(nullCheckTrees, divCheckTrees, checkCastTrees,
                         arrayStoreCheckTrees, conditionalTrees,
                         &comparisonTrees, clonedLoopInvariantBlock,
                         reverseBranchInLoops);
  ...
  if (!checkCastTrees->isEmpty()) {
    buildCheckCastComparisonsTree(nullCheckTrees, divCheckTrees, checkCastTrees,
                                  arrayStoreCheckTrees, &comparisonTrees,
                                  clonedLoopInvariantBlock);
  ...
  if (!arrayStoreCheckTrees->isEmpty()) {
    buildArrayStoreCheckComparisonsTree(
        nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        &comparisonTrees, clonedLoopInvariantBlock);
    ...
    while (nextTree) {
      ...
      TR::TreeTop *firstNewTree = TR::TreeTop::create(
          comp(), TR::Node::create(TR::treetop, 1,
                                   arrayStoreCheckNode->getFirstChild()),
          NULL, NULL);
      TR::Node *child = arrayStoreCheckNode->getFirstChild();
      if (child->getOpCodeValue() == TR::wrtbari &&
          comp()->getOptions()->getGcMode() == TR_WrtbarNone &&
          performTransformation(comp(),
                                "%sChanging iwrtbar node [%p] to an iastore\n",
      ...
      if (arrayStoreCheckNode->getNumChildren() > 1) {
        ...
        secondNewTree = TR::TreeTop::create(
            comp(), TR::Node::create(TR::treetop, 1,
                                     arrayStoreCheckNode->getSecondChild()),
            NULL, NULL);
        child = arrayStoreCheckNode->getSecondChild();
        if (child->getOpCodeValue() == TR::wrtbari &&
            comp()->getOptions()->getGcMode() == TR_WrtbarNone &&
            performTransformation(
                comp(), "%sChanging iwrtbar node [%p] to an iastore\n",
                OPT_DETAILS_LOOP_VERSIONER, child)) {
          TR::Node::recreate(child, TR::astorei);
          child->getChild(2)->recursivelyDecReferenceCount();
          child->setNumChildren(2);
  ...
  if (invariantNodes && !invariantNodes->isEmpty()) {
    // printf("Reached here in %s\n", comp()->signature());
    buildLoopInvariantTree(nullCheckTrees, divCheckTrees, checkCastTrees,
                           arrayStoreCheckTrees, &comparisonTrees,
                           invariantNodes,
                           /*invariantTranslationNodesList*/ NULL,
                           clonedLoopInvariantBlock, invariantBlock);
    invariantNodes->deleteAll();
  }

  if (comparisonTrees.isEmpty() && _containsGuard &&
      performTransformation(comp(), "%s Creating test outside loop for "
                                    "checking if virtual guard is required\n",
                            OPT_DETAILS_LOOP_VERSIONER)) {
  ...
  if (comparisonTrees.isEmpty()) {
    ...
    TR::Node *nextComparisonNode =
        TR::Node::createif(TR::ificmpne, constNode, constNode,
                           clonedLoopInvariantBlock->getEntry());
    comparisonTrees.add(nextComparisonNode);
    dumpOptDetails(comp(), "The comparison tree is empty.  The node %p has "
                           "been created because a test is required when there "
                           "are two versions of the loop.\n",
                   nextComparisonNode);
  }
  ...
  //
  for (auto nextPred = invariantBlock->getPredecessors().begin();
       nextPred != invariantBlock->getPredecessors().end();) {
  ...
  TR_BlockStructure *clonedInvariantBlockStructure = new (trHeapMemory())
  ...
  TR_RegionStructure *properRegion = new (trHeapMemory())
  ...
  TR_StructureSubGraphNode *clonedWhileNode =
      new (trHeapMemory()) TR_StructureSubGraphNode(clonedWhileLoop);
  TR_StructureSubGraphNode *whileNode =
      new (trHeapMemory()) TR_StructureSubGraphNode(whileLoop);
  TR_StructureSubGraphNode *invariantNode =
      new (trHeapMemory()) TR_StructureSubGraphNode(invariantBlockStructure);
  TR_StructureSubGraphNode *clonedInvariantNode = new (trHeapMemory())
  ...
  while (currComparisonBlock) {
    ...
    TR_BlockStructure *comparisonBlockStructure = new (trHeapMemory())
    ...
    TR_StructureSubGraphNode *comparisonNode =
        new (trHeapMemory()) TR_StructureSubGraphNode(comparisonBlockStructure);
    ...
    if (currComparisonBlock) {
      TR_StructureSubGraphNode *criticalEdgeNode =
          new (trHeapMemory()) TR_StructureSubGraphNode(
  ...
  // invariant block strcuture (the natural loop originally) must now  be
  // removed
  ...
  for (newGotoBlockStructure = newGotoBlockStructuresIt.getCurrent();
    ...
    TR_StructureSubGraphNode *newGotoBlockNode =
        new (trHeapMemory()) TR_StructureSubGraphNode(newGotoBlockStructure);
...
void TR_LoopVersioner::buildNullCheckComparisonsTree(
    List<TR::Node> *nullCheckedReferences, List<TR::TreeTop> *nullCheckTrees,
    List<TR::TreeTop> *boundCheckTrees, List<TR::TreeTop> *divCheckTrees,
    List<TR::TreeTop> *checkCastTrees, List<TR::TreeTop> *arrayStoreCheckTrees,
    List<TR::Node> *comparisonTrees, TR::Block *exitGotoBlock) {
  ...
  for (; nextNode;) {
        ...
        if (invariantNullCheckReference) {
          ...
          if (nullCheckReferenceisField) {
            ...
            dupInvariantNullCheckReference->setAndIncChild(
                0, invariantNullCheckReference->duplicateTree());
            nodeToBeNullChkd = dupInvariantNullCheckReference;
          } else {
            dupInvariantNullCheckReference =
                invariantNullCheckReference->duplicateTree();
            TR::Node *oldInvariantNullCheckReference = nextNode->getData();
            nextTree->getData()->getNode()->setNullCheckReference(
                dupInvariantNullCheckReference);
    ...
    /// collectAllExpressionsToBeChecked(nullCheckTrees, divCheckTrees,
    /// checkCastTrees, arrayStoreCheckTrees, nextNode->getData(),
    /// comparisonTrees, exitGotoBlock, visitCount);
    collectAllExpressionsToBeChecked(
        nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
    ...
    if (performTransformation(
            comp(),
            "%s Creating test outside loop for checking if %p is null\n",
      ...
      TR::Node *nextComparisonNode =
          TR::Node::createif(TR::ifacmpeq, duplicateNullCheckReference,
                             TR::Node::aconst(duplicateNullCheckReference, 0),
                             exitGotoBlock->getEntry());
      comparisonTrees->add(nextComparisonNode);

      dumpOptDetails(comp(), "The node %p has been created for testing if null "
                             "check is required\n",
                     nextComparisonNode);

      if (nextTree->getData()->getNode()->getOpCodeValue() == TR::NULLCHK)
        TR::Node::recreate(nextTree->getData()->getNode(), TR::treetop);
      else if (nextTree->getData()->getNode()->getOpCodeValue() ==
...
void TR_LoopVersioner::buildIwrtbarComparisonsTree(
    List<TR::TreeTop> *iwrtbarTrees, List<TR::TreeTop> *nullCheckTrees,
    List<TR::TreeTop> *divCheckTrees, List<TR::TreeTop> *checkCastTrees,
    List<TR::TreeTop> *arrayStoreCheckTrees, List<TR::Node> *comparisonTrees,
  ...
  while (nextTree) {
    ...
    // collectAllExpressionsToBeChecked(nullCheckTrees, divCheckTrees,
    // checkCastTrees, arrayStoreCheckTrees,
    // divCheckNode->getFirstChild()->getSecondChild(), comparisonTrees,
    // exitGotoBlock, visitCount);

    if (performTransformation(comp(), "%s Creating test outside loop for "
                                      "checking if %p is iwrtbar is required\n",
      ...
      if (!isX86 && (isVariableHeapBase || isVariableHeapSize)) {
        if (TR::Compiler->target.is64Bit())
          ifNode1 = TR::Node::create(
              TR::lucmpge, 2, TR::Node::create(TR::a2l, 1, duplicateBase),
              TR::Node::createWithSymRef(
                  TR::lload, 0,
                  comp()
                      ->getSymRefTab()
          ...
          ifNode1 = TR::Node::create(
              TR::iucmpge, 2, TR::Node::create(TR::a2i, 1, duplicateBase),
              TR::Node::createWithSymRef(
                  TR::iload, 0,
                  comp()
                      ->getSymRefTab()
        ...
        if (TR::Compiler->target.is64Bit()) {
          ifNode1 = TR::Node::create(
              TR::lucmpge, 2, TR::Node::create(TR::a2l, 1, duplicateBase),
              TR::Node::create(duplicateBase, TR::lconst, 0, 0));
          ifNode1->getSecondChild()->setLongInt(fej9->getLowTenureAddress());
        } else
          ifNode1 = TR::Node::create(
              TR::iucmpge, 2, TR::Node::create(TR::a2i, 1, duplicateBase),
              TR::Node::create(duplicateBase, TR::iconst, 0,
                               fej9->getLowTenureAddress()));
      }
      ...
      dumpOptDetails(
          comp(),
          "1 The node %p has been created for testing if iwrtbar is required\n",
          ifNode1);
      ...
      if (!isX86 && (isVariableHeapBase || isVariableHeapSize)) {
        if (TR::Compiler->target.is64Bit())
          ifNode2 = TR::Node::create(
              TR::lucmplt, 2, TR::Node::create(TR::a2l, 1, duplicateBase),
              TR::Node::createWithSymRef(
                  TR::lload, 0,
                  comp()
                      ->getSymRefTab()
          ...
          ifNode2 = TR::Node::create(
              TR::iucmplt, 2, TR::Node::create(TR::a2i, 1, duplicateBase),
              TR::Node::createWithSymRef(
                  TR::iload, 0,
                  comp()
                      ->getSymRefTab()
      ...
      dumpOptDetails(
          comp(),
          "2 The node %p has been created for testing if iwrtbar is required\n",
          ifNode2);

      // printf("Found opportunity for skipping wrtbar %p in %s at freq %d\n",
      // iwrtbarNode, comp()->signature(),
      // iwrtbarTree->getEnclosingBlock()->getFrequency()); fflush(stdout);
...
void TR_LoopVersioner::buildDivCheckComparisonsTree(
  ...
  while (nextTree) {
    ...
    if (performTransformation(comp(), "%s Creating test outside loop for "
                                      "checking if %p is divide by zero\n",
                              OPT_DETAILS_LOOP_VERSIONER, divCheckNode)) {
      TR::Node *duplicateDivisor =
          divCheckNode->getFirstChild()->getSecondChild()->duplicateTree();
      TR::Node *ifNode;
      ...
      dumpOptDetails(
          comp(),
          "The node %p has been created for testing if div check is required\n",
          ifNode);
    }
...

{
  ...
  while (nextTree) {
    ...
    collectAllExpressionsToBeChecked(
        nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        checkCastNode, comparisonTrees, exitGotoBlock, visitCount);
    nextTree = nextTree->getNextElement();
  }
}
...
void TR_LoopVersioner::buildArrayStoreCheckComparisonsTree(
  ...
  while (nextTree) {
    if (!performTransformation(
            comp(),
            "%s Creating test outside loop for checking if %p is casted\n",
    ...
    collectAllExpressionsToBeChecked(
        nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        childOfAddressNode, comparisonTrees, exitGotoBlock, visitCount);
    visitCount = comp()->incVisitCount();
    // collectAllExpressionsToBeChecked(nullCheckTrees, divCheckTrees,
    // checkCastTrees, arrayStoreCheckTrees, arrayNode, comparisonTrees,
    // exitGotoBlock, visitCount);
    ...
    collectAllExpressionsToBeChecked(
        nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        duplicateClassPtr, comparisonTrees, exitGotoBlock, visitCount);
...
bool TR_LoopVersioner::buildLoopInvariantTree(
    List<TR::TreeTop> *nullCheckTrees, List<TR::TreeTop> *divCheckTrees,
    List<TR::TreeTop> *checkCastTrees, List<TR::TreeTop> *arrayStoreCheckTrees,
    List<TR::Node> *comparisonTrees, List<TR_NodeParentSymRef> *invariantNodes,
    List<TR_NodeParentSymRefWeightTuple> *invariantTranslationNodesList,
    TR::Block *exitGotoBlock, TR::Block *loopInvariantBlock) {

  if (comp()
          ->getJittedMethodSymbol() && // avoid NULL pointer on non-Wcode builds
      comp()->getJittedMethodSymbol()->isNoTemps()) {
    dumpOptDetails(
        comp(),
        "buildLoopInvariantTree not safe to perform when NOTEMPS enabled\n");
    return false;
  }
  ...
  while (nextInvariantNode) {
    ...
    // ICM will assert while decrementing their ref counts which might already
    // be 0s
    if (nextInvariantNode->getData()->_node->getReferenceCount() < 1 ||
        nextInvariantNode->getData()->_parent->getReferenceCount() < 1) {
      if (trace())
        traceMsg(comp(), "skipping node %p or its parent %p which were removed "
                         "as a part of a versioned check (e.g. count < 1)\n",
                 nextInvariantNode->getData()->_node,
                 nextInvariantNode->getData()->_parent);
      nextInvariantNode = nextInvariantNode->getNextElement();
      continue;
    }
    ...
    collectAllExpressionsToBeChecked(
        nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        invariantNode, comparisonTrees, exitGotoBlock, visitCount);

    // printf("Creating test for loop invariant expression %p in in block_%d
    // %s\n", invariantNode, loopInvariantBlock->getNumber(),
    // comp()->signature());

    if (performTransformation(comp(), "%s Creating store outside the loop for "
                                      "loop invariant expression %p\n",
      ...
      if (invariantNode->getOpCode().hasSymbolReference() &&
          invariantNode->getSymbolReference()->getSymbol()->isNotCollected())
      ...
      if (comp()->useCompressedPointers()) {
        if (duplicateNode->getOpCode().isLoadIndirect() &&
          ...
          TR::TreeTop *translateTT = TR::TreeTop::create(
              comp(), TR::Node::createCompressedRefsAnchor(duplicateNode), NULL,
              NULL);
      ...
      TR::Node *storeForInvariantNode = TR::Node::createWithSymRef(
          comp()->il.opCodeForDirectStore(duplicateNode->getDataType()), 1, 1,
          duplicateNode, newSymbolReference);
      TR::TreeTop *storeForInvariantTree =
          TR::TreeTop::create(comp(), storeForInvariantNode, 0, 0);
      ...
      // printf("Creating store %p outside the loop for loop invariant
      // expression %p in in block_%d %s\n", storeForInvariantNode,
      // invariantNode, loopInvariantBlock->getNumber(), comp()->signature());
      // fflush(stdout);

      ListElement<TR_NodeParentSymRef> *otherInvariantNodeElem =
          nextInvariantNode->getNextElement();
      ListElement<TR_NodeParentSymRef> *prevOtherInvariantNodeElem =
          nextInvariantNode;
      for (; otherInvariantNodeElem;) {
        ...
        if (!otherInvariantNodeElem->getData()->_symRef &&
            optimizer()->areNodesEquivalent(otherInvariantNode,
                                            invariantNode)) {
          visitCount = comp()->incVisitCount();
          if (optimizer()->areSyntacticallyEquivalent(
                  otherInvariantNode, invariantNode, visitCount)) {
            otherInvariantNodeElem->getData()->_symRef = newSymbolReference;

            /*
            int32_t childNum;
            for (childNum=0;childNum<otherInvariantNode->getNumChildren();
            childNum++)
               otherInvariantNode->getChild(childNum)->recursivelyDecReferenceCount();
            otherInvariantNode->setNumChildren(0);
            TR::Node::recreate(otherInvariantNode,
            comp()->il.opCodeForDirectLoad(invariantNode->getDataType()));
            ...
            if (otherInvariantNodeElem->getData()->_parent &&
               ...
               TR::Node::recreate(otherInvariantNodeElem->getData()->_parent,
            TR::treetop);
      ...
      TR::Node::recreate(invariantNode,
      comp()->il.opCodeForDirectLoad(invariantNode->getDataType()));
      ...
      if (nextInvariantNode->getData()->_parent &&
          ...
          TR::Node::recreate(nextInvariantNode->getData()->_parent,
      TR::treetop);
      */
  ...
  while (nextInvariantNode) {
    if (nextInvariantNode->getData()->_symRef) {
      ...
      // ICM will assert while decrementing their ref counts which might already
      // be 0s
      if (nextInvariantNode->getData()->_node->getReferenceCount() > 0 &&
          nextInvariantNode->getData()->_parent->getReferenceCount() > 0) {
        TR::Node *invariantNode = nextInvariantNode->getData()->_node;
        int32_t childNum;
        ...
        for (childNum = 0; childNum < invariantNode->getNumChildren();
             childNum++)
          invariantNode->getChild(childNum)->recursivelyDecReferenceCount();
        invariantNode->setNumChildren(0);
        TR::Node::recreate(invariantNode, comp()->il.opCodeForDirectLoad(
                                              invariantNode->getDataType()));
        invariantNode->setSymbolReference(
            nextInvariantNode->getData()->_symRef);
      } else {
        if (trace())
          traceMsg(comp(), "skipping node %p or its parent %p which were "
                           "removed as a part of a versioned check (e.g. count "
                           "< 1)\n",
                   nextInvariantNode->getData()->_node,
                   nextInvariantNode->getData()->_parent);
      }
...
bool TR_LoopVersioner::buildSpecializationTree(
  ...
  if (comp()
    ...
    dumpOptDetails(
        comp(),
        "buildSpecializationTree not safe to perform when NOTEMPS enabled\n");
    return false;
  }
  ...
  while (nextSpecializedNode) {
    ...
    collectAllExpressionsToBeChecked(
        nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        nodeToBeChecked, comparisonTrees, exitGotoBlock, visitCount);

#ifdef J9_PROJECT_SPECIFIC
    if (performTransformation(comp(), "%s Creating test outside loop for "
                                      "checking if %p is value profiled\n",
      ...
      TR_ValueInfo *valueInfo =
          (TR_ValueInfo *)TR_ValueProfiler::getProfiledValueInfo(
              specializedNode, comp());
      ...
      dumpOptDetails(comp(), "The node %p has been created for testing if "
                             "value profiling check is required\n",
                     comparisonNode);
...
void TR_LoopVersioner::buildConditionalTree(
    List<TR::TreeTop> *nullCheckTrees, List<TR::TreeTop> *divCheckTrees,
    List<TR::TreeTop> *checkCastTrees, List<TR::TreeTop> *arrayStoreCheckTrees,
    List<TR::TreeTop> *conditionalTrees, List<TR::Node> *comparisonTrees,
    TR::Block *exitGotoBlock, SharedSparseBitVector &reverseBranchInLoops) {
  ...
  while (nextTree) {
    ...
    // if (includeComparisonTree)
    {
      ...
      collectAllExpressionsToBeChecked(
          nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
          conditionalNode, comparisonTrees, exitGotoBlock, visitCount);

      if (performTransformation(comp(), "%s Creating test outside loop for "
                                        "checking if %p is conditional\n",
        ...
        // conditionals we only pull from fast path
        // This exposed a bug where slow path had an unconditional that removed
        // a valid path.
        // Fix is only allowing unconditioning in both if there is single if
        // conditional tree.
        //
        if ((_conditionalTree == conditionalNode) &&
        ...
        if (trace())
          traceMsg(comp(), "Frequency Test for conditional node [%p], "
                           "destination Frequency %d, fallThrough frequency "
                           "%d\n",
        ...
        if (reverseBranchInLoops[conditionalNode->getGlobalIndex()]) {
          if (trace())
            traceMsg(comp(), "Branch reversed for conditional node [%p], "
                             "destination Frequency %d, fallThrough frequency "
                             "%d\n",
                     conditionalNode, destBlock->getFrequency(),
                     fallThroughBlock->getFrequency());
        ...
        if (conditionalNode->isVersionableIfWithMaxExpr() ||
          ...
          for (int32_t childNum = 0;
               childNum < conditionalNode->getNumChildren(); childNum++) {
          ...
          if (isInductionVar &&
            ...
            TR::Node *storeNode = _storeTrees[indexNode->getSymbolReference()
                                                  ->getReferenceNumber()]
                                      ->getNode();
            ...
            duplicateIndexNode = conditionalNode->getChild(indexChildIndex)
                                     ->duplicateTree(comp());
            ...
            if (duplicateIndexNode->getOpCode().isLoad() &&
                duplicateIndexNode->getSymbolReference()
                        ->getReferenceNumber() == indexSymRefNum)
              duplicateIndexNode = maxValue;
            else
              replaceInductionVariable(conditionalNode, duplicateIndexNode,
                                       indexChildIndex, indexSymRefNum,
                                       maxValue, visitCount);
          }
        }
          ...
          if (indexNodeOccursAsSecondChild) {
            if (!reverseBranch)
              duplicateComparisonNode = TR::Node::createif(
                  ...
                  duplicateIndexNode, exitGotoBlock->getEntry());
            else
              duplicateComparisonNode = TR::Node::createif(
        ...
        if (duplicateComparisonNode->getFirstChild()->getOpCodeValue() == TR::
          ...
          duplicateComparisonNode->getFirstChild()->getFirstChild()->setIsNull(
              false);
          duplicateComparisonNode->getFirstChild()
              ->getFirstChild()
              ->setIsNonNull(false);
        } else {
        ...
        TR::Node *constNode =
            TR::Node::create(conditionalNode, TR::iconst, 0, 0);
        ...
        if (changeConditionalToUnconditionalInBothVersions) {
          if (_duplicateConditionalTree
                  ->isTheVirtualGuardForAGuardedInlinedCall()) {
            TR::Node *callNode =
                _duplicateConditionalTree->getVirtualCallNodeForGuard();
            if (callNode)
              callNode->resetIsTheVirtualCallNodeForAGuardedInlinedCall();
          }
          ...
          _duplicateConditionalTree->getFirstChild()
              ->recursivelyDecReferenceCount();
          _duplicateConditionalTree->setChild(0, constNode);
          constNode->incReferenceCount();

          _duplicateConditionalTree->getSecondChild()
              ->recursivelyDecReferenceCount();
          ...
          _duplicateConditionalTree
              ->resetIsTheVirtualGuardForAGuardedInlinedCall();
        }
      }
    }
...
void TR_LoopVersioner::buildSpineCheckComparisonsTree(
    List<TR::TreeTop> *nullCheckTrees, List<TR::TreeTop> *spineCheckTrees,
    List<TR::TreeTop> *divCheckTrees, List<TR::TreeTop> *checkCastTrees,
    List<TR::TreeTop> *arrayStoreCheckTrees, List<TR::Node> *comparisonTrees,
    TR::Block *exitGotoBlock) {
  ListElement<TR::TreeTop> *nextTree = spineCheckTrees->getListHead();
  ...
  while (nextTree) {
    ...
    if (performTransformation(
            comp(),
            "%s Creating test outside loop for checking if %p has spine\n",
            OPT_DETAILS_LOOP_VERSIONER, spineCheckNode)) {
      performSpineCheck = true;
      TR::Node *contigArrayLength = TR::Node::create(
          TR::contigarraylength, 1, arrayBase->duplicateTree());
      TR::Node *nextComparisonNode =
          TR::Node::createif(TR::ificmpne, contigArrayLength,
    ...
    if (performSpineCheck) {
      ...
      TR::Node *shiftNode =
          is64BitTarget ? TR::Node::lconst(spineCheckNode, (int64_t)shift)
      ...
      TR::Node *spineShiftNode =
          is64BitTarget
              ? TR::Node::lconst(spineCheckNode, (int64_t)arraySpineShift)
      ...
      node = TR::Node::create(is64BitTarget ? TR::lshr : TR::ishr, 2,
                              spineIndex, spineShiftNode);
      node = TR::Node::create(is64BitTarget ? TR::lshl : TR::ishl, 2, node,
                              shiftNode);
      node = TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, node,
                              hdrSize);
      node = TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2,
                              arrayBase, node);
      ...
      if (is64BitTarget) {
        ...
        leafOffset =
            TR::Node::create(spineCheckNode, TR::iconst, 0,
      ...
      if (strideShiftNode) {
        nodeLeafOffset = TR::Node::create(is64BitTarget ? TR::lshl : TR::ishl,
                                          2, nodeLeafOffset, strideShiftNode);
      }
      ...
      TR::TreeTop *firstNewTree =
          TR::TreeTop::create(comp(), arrayTT, NULL, NULL);
...
void TR_LoopVersioner::buildBoundCheckComparisonsTree(
    List<TR::TreeTop> *nullCheckTrees, List<TR::TreeTop> *boundCheckTrees,
    List<TR::TreeTop> *divCheckTrees, List<TR::TreeTop> *checkCastTrees,
    List<TR::TreeTop> *arrayStoreCheckTrees, List<TR::TreeTop> *spineCheckTrees,
    List<TR::Node> *comparisonTrees, TR::Block *exitGotoBlock,
    bool reverseBranch) {
  ListElement<TR::TreeTop> *nextTree = boundCheckTrees->getListHead();
  bool isAddition;
  ...
  while (nextTree) {
      ...
      if (arrayLengthNode->getOpCode().isArrayLength()) {
        ...
        if (arrayLengthNode->getOpCode().hasSymbolReference() &&
          ...
          if (invariantArrayLen) {
            ...
            boundCheckNode->setAndIncChild(boundChildIndex,
                                           dupInvariantArrayLen);
    ...
    collectAllExpressionsToBeChecked(
        nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        boundCheckNode, comparisonTrees, exitGotoBlock, visitCount);
    TR::Node *nextComparisonNode;
    ...
    if (isIndexInvariant) {
      ...
      if (performTransformation(comp(), "%s Creating test outside loop for "
        ...
        dumpOptDetails(
            comp(),
            "The node %p has been created for testing if exceed bounds\n",
            nextComparisonNode);
      }
      ...
      if (performTransformation(comp(), "%s Creating test outside loop for "
        ...
        if (boundCheckNode->getOpCodeValue() == TR::BNDCHK ||
          ...
          nextComparisonNode = TR::Node::createif(
              TR::ificmpge, indexNode->duplicateTree(),
              arrayLengthNode->duplicateTree(), exitGotoBlock->getEntry());
        else
          nextComparisonNode = TR::Node::createif(
              TR::ificmpgt, indexNode->duplicateTree(),
              arrayLengthNode->duplicateTree(), exitGotoBlock->getEntry());
      ...
      if (performFirstBoundCheck && performSecondBoundCheck) {
        ...
        if (boundCheckNode->getOpCodeValue() == TR::BNDCHKwithSpineCHK) {
          ...
          // printf("adding spiencheck from versioning
          // bndchk\n");fflush(stdout);
          spineCheckTrees->add(nextTree->getData());
        } else {
          TR::TreeTop *firstNewTree = TR::TreeTop::create(
              comp(), TR::Node::create(TR::treetop, 1, boundCheckNode->getChild(
                                                           boundChildIndex)),
              NULL, NULL);
          TR::TreeTop *secondNewTree = TR::TreeTop::create(
              comp(), TR::Node::create(TR::treetop, 1, boundCheckNode->getChild(
      ...
      bool performFirstBoundCheck = performTransformation(
          comp(),
          "%s Creating test outside loop for checking if %p exceeds bounds\n",
          OPT_DETAILS_LOOP_VERSIONER, boundCheckNode);
      bool performSecondBoundCheck = performTransformation(
          comp(),
          "%s Creating test outside loop for checking if %p exceeds bounds\n",
          OPT_DETAILS_LOOP_VERSIONER, boundCheckNode);
      ...
      if (performFirstBoundCheck) {
        if (isSpecialInductionVariable) {
          // printf("Found an opportunity for special versioning in method
          // %s\n", comp()->signature());
          nextComparisonNode = TR::Node::createif(
              TR::ificmplt,
          ...
          if ((isAddition && !indVarOccursAsSecondChildOfSub) ||
            ...
            nextComparisonNode = TR::Node::createif(
                ...
                boundCheckNode->getChild(boundChildIndex)->duplicateTree(),
                exitGotoBlock->getEntry());
        ...
        dumpOptDetails(
            comp(),
            "1: The node %p has been created for testing if exceed bounds\n",
            nextComparisonNode);
      }
      ...
      if (performSecondBoundCheck) {
        if (isLoopDrivingInductionVariable || isDerivedInductionVariable)
          ...
          loopLimit =
              TR::Node::create(boundCheckNode, TR::iconst, 0, exitValue);
        ...
        //
        // maxValuej = [Ej +/- ceiling((N-Ei)/incr(i))*incr(j)] -/+ incr(j) +/-
        // 1
        //
        //
        //    isub
        ...
        int32_t boundChildIndex =
            (boundCheckNode->getOpCodeValue() == TR::BNDCHKwithSpineCHK) ? 2
                                                                         : 0;
        ...
        if (isSpecialInductionVariable) {
          nextComparisonNode = TR::Node::createif(
              TR::ifiucmpgt,
              boundCheckNode->getChild(indexChildIndex)->duplicateTree(),
              correctCheckNode->duplicateTree(), exitGotoBlock->getEntry());
          if (trace())
            traceMsg(comp(), "Special Induction variable added in each iter -> "
                             "Creating %p (%s)\n",
                     nextComparisonNode,
                     nextComparisonNode->getOpCode().getName());
              ...
              if (isLoopDrivingInductionVariable) {
                if ((_loopTestTree->getNode()->getOpCodeValue() ==
                  ...
                  nextComparisonNode = TR::Node::createif(
                      ...
                      boundCheckNode->getChild(boundChildIndex)
                          ->duplicateTree(),
                      exitGotoBlock->getEntry());
              }
        ...
        dumpOptDetails(
            comp(),
            "2: The node %p has been created for testing if exceed bounds\n",
            nextComparisonNode);

        if (0 && complexButPredictableForm) {
          TR::Node *loadNode = TR::Node::createWithSymRef(
              boundCheckNode, TR::iload, 0, indexSymRef);
          TR::Node *storeConstNode = TR::Node::create(
              boundCheckNode, TR::iconst, 0, additiveConstantInStore);
          TR::Node *iremNode =
              TR::Node::create(TR::irem, 2, loadNode, storeConstNode);
          TR::Node *comparedConstNode = TR::Node::create(
              boundCheckNode, TR::iconst, 0,
              additiveConstantInStore - additiveConstantInIndex);
          nextComparisonNode =
              TR::Node::createif(TR::ificmpge, iremNode, comparedConstNode,
                                 exitGotoBlock->getEntry());
              ...
              if (!firstChild->getOpCode().isLoad()) {
                if (!storeRhs->getOpCode().isLoad()) {
                  ...
                  replaceInductionVariable(
                      NULL, storeRhs, -1, origIndexSymRef->getReferenceNumber(),
                      replacementLoad, visitCount);
                } else
                  storeRhs = replacementLoad;
          ...
          if (!isIndexChildMultiplied) {
            ...
            if (isAddition) {
              if ((_loopTestTree->getNode()->getOpCodeValue() ==
                  ...
                  (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpgt))
                nextComparisonNode =
                    TR::Node::createif(TR::ificmpgt, firstChild, secondChild,
                                       exitGotoBlock->getEntry());
              else
                nextComparisonNode =
                    TR::Node::createif(TR::ificmpge, firstChild, secondChild,
                                       exitGotoBlock->getEntry());
              ...
              if ((_loopTestTree->getNode()->getOpCodeValue() ==
                  ...
                  (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt))
                nextComparisonNode =
                    TR::Node::createif(TR::ificmplt, firstChild, secondChild,
                                       exitGotoBlock->getEntry());
              else
                nextComparisonNode =
                    TR::Node::createif(TR::ificmple, firstChild, secondChild,
                                       exitGotoBlock->getEntry());
            ...
            dumpOptDetails(comp(), "3: The node %p has been created for "
                                   "testing if exceed bounds\n",
                           nextComparisonNode);
...
void TR_LoopVersioner::collectAllExpressionsToBeChecked(
  ...
  int32_t i;
  for (i = 0; i < node->getNumChildren(); i++) {
    // Do not process the load or store child of a BNDCHKwithSpineCHK node
    // because we are
    // already generating check trees for it as part of the topmost bound check
    // node.
    //
    ...
    collectAllExpressionsToBeChecked(
        nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        node->getChild(i), comparisonTrees, exitGotoBlock, visitCount);
  }
  ...
  if (node->isInternalPointer() ||
    ...
    if (!node->getFirstChild()->isThisPointer() &&
      ...
      while (nullCheckTree) {
        ...
        if (nullCheckNode->getOpCode().isNullCheck()) {
          if (nullCheckNode->getNullCheckReference() == node->getFirstChild()) {
            if (nullCheckTree->getData()->getNode()->getOpCodeValue() ==
                TR::NULLCHK)
              TR::Node::recreate(nullCheckTree->getData()->getNode(),
                                 TR::treetop);
            else if (nullCheckTree->getData()->getNode()->getOpCodeValue() ==
                     TR::ResolveAndNULLCHK)
              TR::Node::recreate(nullCheckTree->getData()->getNode(),
                                 TR::ResolveCHK);
      ...
      if (!eliminatedNullCheck) {
        ...
        while (nullCheckTree) {
          TR::Node *nullCheckNode = nullCheckTree->getData()->getNode();
          if (nullCheckNode->getOpCode().isNullCheck()) {
            if (nullCheckNode->getNullCheckReference() ==
                node->getFirstChild()) {
              if (nullCheckTree->getData()->getNode()->getOpCodeValue() ==
                  TR::NULLCHK)
                TR::Node::recreate(nullCheckTree->getData()->getNode(),
      ...
      TR::Node *ifacmpeqNode = TR::Node::createif(
          TR::ifacmpeq, duplicateNullCheckReference, TR::Node::aconst(node, 0),
          exitGotoBlock->getEntry());
    ...
    if (node->isInternalPointer() &&
      ...
      if (node->isInternalPointer() || node->getOpCode().isArrayLength()) {
        ...
        if (otherSig)
          otherClassObject = otherSymRef->getOwningMethod(comp())
                                 ->getDeclaringClassFromFieldOrStatic(
                                     comp(), otherSymRef->getCPIndex());

        // the call to findOrCreateClassSymbol is safe even though we pass CPI
        // of -1 since the call above getClassFromSignature is not vetted for
        // AOT
        if (cl && otherClassObject &&
          ...
          duplicateClassPtr = TR::Node::createWithSymRef(
              ...
              comp()->getSymRefTab()->findOrCreateClassSymbol(
                  otherSymRef->getOwningMethodSymbol(comp()), -1,
                  otherClassObject, false));
      }
    }
    ...
    if (instanceOfReqd) {
      if (otherClassObject) {
        if (performTransformation(
                comp(), "%s Creating test outside loop for checking if %p is "
                        "of the correct type\n",
                OPT_DETAILS_LOOP_VERSIONER, node->getFirstChild())) {
          ...
          TR::Node *instanceofNode = TR::Node::createWithSymRef(
              TR:: instanceof
              , 2, 2, duplicateCheckedValue, duplicateClassPtr,
              comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(
                  comp()->getMethodSymbol()));
          ...
          dumpOptDetails(comp(), "The node %p has been created for testing if "
                         ...
                         ificmpeqNode);
          // printf("The node %p has been created for testing if object is of
...
#ifdef J9_PROJECT_SPECIFIC
        if (performTransformation(comp(), "%s Creating test outside loop for "
                                          "checking if %p is of array type\n",
                                  OPT_DETAILS_LOOP_VERSIONER,
                                  node->getFirstChild())) {
          TR::Node *duplicateCheckedValue =
              node->getFirstChild()->duplicateTree();
          ...
          TR::Node *vftLoad = TR::Node::createWithSymRef(
              TR::aloadi, 1, 1, duplicateCheckedValue,
              comp()->getSymRefTab()->findOrCreateVftSymbolRef());
          ...
          TR::Node *andNode =
              TR::Node::create(TR::iand, 2, isArrayField, andConstNode);
          TR::Node *cmp = TR::Node::createif(
              TR::ificmpne, andNode, andConstNode, exitGotoBlock->getEntry());
    ...
    if (childInRequiredForm) {
      if (childInRequiredForm->getOpCodeValue() == TR::ishl ||
        ...
        if (childInRequiredForm->getSecondChild()->getOpCode().isLoadConst() &&
          ...
          duplicateIndex =
              childInRequiredForm->getFirstChild()->duplicateTree();
    ...
    if (!duplicateIndex) {
      if (!childInRequiredForm) {
        ...
        duplicateIndex =
            TR::Node::create(type.isInt32() ? TR::isub : TR::lsub, 2,
                             offset->duplicateTree(), constNode);
        if (type.isInt64())
      ...
      duplicateIndex = TR::Node::create(
          type.isInt32() ? TR::iushr : TR::lushr, 2, duplicateIndex,
          TR::Node::create(node, TR::iconst, 0, shiftWidth));
    }

    TR::Node *duplicateBase =
        node->getFirstChild()->getFirstChild()->duplicateTree();
    TR::Node *arrayLengthNode =
        TR::Node::create(TR::arraylength, 1, duplicateBase);
    ...
    TR::Node *ificmpgeNode = TR::Node::createif(
        type.isInt32() ? TR::ificmpge : TR::iflcmpge, duplicateIndex,
        arrayLengthNode, exitGotoBlock->getEntry());
    ...
    TR::Node *ificmpltNode = TR::Node::createif(
        type.isInt32() ? TR::ificmplt : TR::iflcmplt,
        duplicateIndex->duplicateTree(), constNode, exitGotoBlock->getEntry());
    if (type.isInt64())
      ificmpltNode->getSecondChild()->setLongInt(0);
    comparisonTrees->add(ificmpltNode);
    dumpOptDetails(comp(), "The node %p has been created for testing if bounds "
                           "check is required\n",
                   ificmpgeNode);
    dumpOptDetails(comp(), "The node %p has been created for testing if bounds "
                           "check is required\n",
        ...
        if ((checkCastNode == node) &&
          ...
          if (node->getOpCodeValue() == TR::checkcastAndNULLCHK) {
            ...
            TR::Node *ifacmpeqNode = TR::Node::createif(
                ...
                TR::Node::aconst(node, 0), exitGotoBlock->getEntry());
            comparisonTrees->add(ifacmpeqNode);
            dumpOptDetails(comp(), "The node %p has been created for testing "
                                   "if null check is required\n",
                           ifacmpeqNode);
          }
