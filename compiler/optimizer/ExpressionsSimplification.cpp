...
void TR_ExpressionsSimplification::setSummationReductionCandidates(
  ...
  if (node->getOpCodeValue() != TR::istore
    ...
    if (trace())
      traceMsg(comp(), "Node %p: The opcode is not istore so not a summation "
                       "reduction candidate\n",
               node);
    return;
  }
...
void TR_ExpressionsSimplification::setStoreMotionCandidates(TR::Node *node,
  ...
  if (node->getOpCode().isStore() && !node->getSymbol()->isStatic() &&
    ...
    if (trace()) {
      traceMsg(comp(), "Node %p: The store's operands are all loop-invariant, "
                       "adding candidate\n",
               node);
      traceMsg(
          comp(),
...
bool TR_ExpressionsSimplification::tranformSummationReductionCandidate(
  ...
  if (expNode) {
    ...
    if (loopInfo->getNumIterations() > 0 || // make sure that the loop is going
        ...
        _currentRegion->isCanonicalizedLoop()) // or that the loop is
                                               // canonicalized, in which case
                                               // the preheader is
...
void TR_ExpressionsSimplification::tranformStoreMotionCandidate(
  ...
  // once, or is canonicalized.
  // but to be safe we still perform on canonicalized loops only.
  if (_currentRegion->isCanonicalizedLoop()) // make sure that the loop is
                                             // canonicalized, in which case the
                                             // preheader is
  { // executed in its first iteration and is protected.
    if (performTransformation(
...
void TR_ExpressionsSimplification::removeUnsupportedCandidates() {
  ...
  for (TR::TreeTop *candidateTT = candidateTTs.getFirst(); candidateTT;
    ...
    if (!_supportedExpressions->get(candidate->getGlobalIndex())) {
      if (trace())
        traceMsg(comp(), "Removing candidate %p which is unsupported or has "
...
void TR_ExpressionsSimplification::removeUncertainBlocks(
  ...
  for (TR::Block *block = blocks.getFirst(); block; block = blocks.getNext()) {
    ...
    if (!(cfgNode->getExceptionSuccessors().empty()) ||
      ...
      if (trace())
        traceMsg(comp(), "An exception can be thrown from block_%d. Removing "
                         "all the blocks, since we cannot know the number of "
                         "iterations.\n",
                 block->getNumber());
  ...
  if (postDominators.isValid()) {
    ...
    for (TR::Block *block = blocks.getFirst(); block;
      ...
      if (postDominators.dominates(block, entryBlock) == 0) {
        ...
        if (trace())
          traceMsg(comp(), "Block_%d is not guaranteed to be executed at least "
                           "once. Removing it from the list.\n",
                   block->getNumber());
      }
    }
  } else {
...
TR_ExpressionsSimplification::findLoopInfo(TR_RegionStructure *region) {
  ...
  TR_StructureSubGraphNode *exitNode =
      toStructureSubGraphNode(exitEdges.getFirst()->getFrom());
  ...
  if (!firstChildOfLastTree->getOpCode().hasSymbolReference()) {
    if (trace())
      traceMsg(comp(), "The branch node's first child node %p - its opcode "
                       "does not have a symbol reference\n",
...
TR_ExpressionsSimplification::iaddisubSimplifier(TR::Node *invariantNode,
  ...
  if (loopInfo->getBoundaryNode()) {
    ...
    if (loopInfo->getNumIterations() > 0) {
      newNode =
          TR::Node::create(TR::imul, 2, invariantNode->duplicateTree(),
                           TR::Node::create(invariantNode, TR::iconst, 0,
                                            loopInfo->getNumIterations()));
    }
    return newNode;
  }
