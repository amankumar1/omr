...
#define TR_CALLSITE_INHERIT_CONSTRUCTOR_AND_TR_ALLOC(EXTENDED, BASE)           \
  ...
  EXTENDED(TR_ResolvedMethod *callerResolvedMethod,                            \
           ...
           TR_Method *interfaceMethod, TR_OpaqueClassBlock *receiverClass,     \
...
struct TR_CallSite : public TR_Link<TR_CallSite> {
  ...
  TR_Method *_interfaceMethod; // If we have an interface, we'll only have a
                               // TR_Method until we determine others
  TR_OpaqueClassBlock
  ...
  TR::ResolvedMethodSymbol *_initialCalleeSymbol; // and in those cases, we
  ...
  vcount_t _visitCount; // a woefully bad choice of name.  The number of times
                        // we have visited this callsite during inlining.  In
                        // theory, when you have multiple targets for a
                        // callsite, you will have multiple visits to a
                        // callsite.
...
protected:
  TR_CALLSITE_INHERIT_CONSTRUCTOR_AND_TR_ALLOC(TR_FunctionPointerCallSite,
                                               TR_IndirectCallSite)
};
