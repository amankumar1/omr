...
TR_RegisterAnticipatability *
TR_RegisterAnticipatability::asRegisterAnticipatability() {
  return this;
}
...
void TR_RegisterAnticipatability::analyzeNode(TR::Node *, vcount_t,
                                              TR_BlockStructure *,
                                              TR_BitVector *) {}
...
void TR_RegisterAnticipatability::initializeRegisterUsageInfo() {
  ...
  _outSetInfo = (TR_BitVector **)trMemory()->allocateStackMemory(
      _numberOfNodes * sizeof(TR_BitVector *));

  for (int32_t i = 0; i < _numberOfNodes; i++) {
    _registerUsageInfo[i] = new (trStackMemory())
        TR_BitVector(_numberOfBits, trMemory(), stackAlloc);
    copyFromInto(originalRegUsageInfo[i], _registerUsageInfo[i]);
    _outSetInfo[i] = new (trStackMemory())
        TR_BitVector(_numberOfBits, trMemory(), stackAlloc);
    _outSetInfo[i]->empty();
  }
}

TR_RegisterAnticipatability::TR_RegisterAnticipatability(
    TR::Compilation *comp, TR::Optimizer *optimizer,
    TR_Structure *rootStructure, TR_BitVector **regUsageInfo, bool trace)
  ...
  _registerUsageInfo =
      regUsageInfo; // Will be copied in initializeRegisterUsageInfo
  ...
  //
  int32_t entryNum = comp->getStartTree()
                         ->getEnclosingBlock()
                         ->getNumber(); // dummy entry block
...
void TR_RegisterAnticipatability::analyzeTreeTopsInBlockStructure(
  ...
  *_exceptionInfo |=
      *_registerUsageInfo[blockNum]; // FIXME: is this too conservative?
