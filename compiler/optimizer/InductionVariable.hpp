...
public:
  ...
  typedef TR::typed_allocator<std::pair<ncount_t, TR::Node *>, TR::Allocator>
      NodeMapAllocator;
  typedef std::map<ncount_t, TR::Node *, std::less<ncount_t>, NodeMapAllocator>
      NodeMap;
  ...
  void walkTreesAndFixUseDefs(TR_Structure *, TR::SymbolReference *,
                              TR::NodeChecklist &);
  void replaceLoadsInStructure(TR_Structure *, int32_t, TR::SymbolReference *,
                               TR::NodeChecklist &, TR::NodeChecklist &);
  void replaceLoadsInSubtree(TR::Node *, int32_t, TR::SymbolReference *,
                             TR::NodeChecklist &, TR::NodeChecklist &);
  void eliminateSignExtensions(TR::NodeChecklist &);
  void eliminateSignExtensionsInSubtree(TR::Node *, TR::NodeChecklist &,
                                        TR::NodeChecklist &, NodeMap &);
  TR::Node *signExtend(TR::Node *, TR::NodeChecklist &, NodeMap &);
  TR::Node *signExtendBinOp(TR::ILOpCodes, TR::Node *, TR::NodeChecklist &,
                            NodeMap &);
  ...
  int32_t maxInternalPointers() {
    // was int32_t
    // reservedForLoops=comp()->getOption(TR_ProcessHugeMethods)?6:2;
    // increased to 8 since we did see the limit being reached and believe 2
    // would
    // be too little as a buffer for the internal pointer spills. 8 is a
    // hypothetical
 ...
 * nodes into
 * ones with integer node tests. The main motivation is that IVA does not
 * recognize address type node.
 * Since a lot of loop-based optimizations and analysis are written for loops
 * with integral IV,
 * this is a simple solution to catch many address incrementing loops. The
 * other, full-fledged solution is
 * to recognize address IV in IVA and everywhere else, but that may entail much
 * more work.
 *
 * Address IV is also a fairly common coding pattern in C (pointer manip) and
 * C++ with iterators
 * of contiguous memory.
 * This pass is analogous to loop strider in that it undo's the pointer incr
 * which strides by size of underlying
 * type.
 */
