...
//
  ...
  private:
    AuxiliaryData(TR::Compilation *c)
        : _onceReadSymbols(c->allocator("UseDefAux"),
                           BitVector(c->allocator("UseDefAux"))),
          _onceWrittenSymbols(c->allocator("UseDefAux"),
                              BitVector(c->allocator("UseDefAux"))),
          _defsForSymbol(c->allocator("UseDefAux"),
                         BitVector(c->allocator("UseDefAux"))),
          _symsKilledByMustKills(
              c->allocator("UseDefAux"),
              TR::SparseBitVector(c->allocator("UseDefAux"))),
          ...
          _defsForOSR(c->allocator("UseDefAux"),
                      TR_UseDefInfo::BitVector(c->allocator("UseDefAux"))) {}
    ...
    CS2::ArrayOf<TR::SparseBitVector, TR::Allocator>
        _symsKilledByMustKills; // symbol localIndex killed by function call due
                                // to mustDef
    ...
    CS2::ArrayOf<CS2::Pair<TR::Node *, TR::TreeTop *>, TR::Allocator>
        _expandedAtoms; // TR::Node            **_expandedNodes;
...
private:
  ...
  // taken when setting up use-def
  // The conservative answer is to not index autos that have volatile aliases.

  void setVolatileSybolsIndexAndRecurse(TR::BitVector &volatileSymbols,
                                        int32_t symRefNum);
...
private:
  ...
  void buildUseDefs(TR::Node *node, void *vanalysisInfo,
                    TR::BitVector &nodesToBeDereferenced, TR::Node *parent,
                    AuxiliaryData &aux);
...
private:
  ...
  CS2::ArrayOf<const BitVector *, TR::Allocator>
