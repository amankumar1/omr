...
public:
  ...
  static bool supportedType(TR::Node *node) {
    // The z codegen doesn't generate the correct instructions (yet) to allow
    // this
    // TR::comp()->getOption(TR_DisableDirectStaticAccessOnZ) &&

    if (!TR::comp()->cg()->getAccessStaticsIndirectly() &&
        node->getOpCode().hasSymbolReference() &&
        node->getSymbol()->isStatic() && node->getOpCode().isLoadDirect() &&
...
private:
  inline bool countIsSigned(TR::Node *load) {
    if (load->getType().isAddress() || load->getType().isAggregate())
      ...
      return _counts[load->getGlobalIndex()] / totalFlags >=
             0; // bias towards signed load
  }

  inline void setOverrideOpt(TR::Node *node, overrideFlags overrideType) {
    if (trace())
      traceMsg(comp(), "Setting override on node %p at %d\n", node,
               node->getGlobalIndex());
  ...
  inline bool isOverriden(TR::Node *node, overrideFlags overrideType) {
    if (trace())
      traceMsg(comp(), "Checking override on node %p at %d\n", node,
               node->getGlobalIndex());
  ...
  bool detectUnneededConversionPattern(TR::Node *conversion, TR::Node *child,
                                       bool &mustForce64);
  bool detectReverseNeededConversionPattern(TR::Node *parent,
                                            TR::Node *conversion);
  void setPreferredExtension(TR::Node *node, vcount_t visitCount);
  ncount_t indexNodesForCodegen(TR::Node *parent, ncount_t nodeCount,
                                vcount_t visitCount);
};
