...
bool TR_LocalLiveRangeReduction::isAnySymInDefinedOrUsedBy(
  ...
  if (TR::Compiler->target.cpu.isPower() &&
    ...
    // the flush is needed at least as a scheduling barrier
  ...
  for (int32_t i = 0; i < currentNode->getNumChildren(); i++) {
    ...
    if (movingTreeRefInfo->getFirstRefNodesList()->find(child)) {
      ...
      if (child->exceptionsRaised() ||
        ...
        if (trace())
          traceMsg(comp(), "cannot move %p beyond %p - cannot change "
                           "evaluation point of %p\n ",
        ...
        return true;
               ...
               !child->isNonNegative()) {
        if (trace())
          traceMsg(comp(), "cannot move %p beyond %p - changing the eval point "
                           "of %p will casue extra cg instruction ",
                   movingNode, currentTreeRefInfo->getTreeTop()->getNode(),
                   currentNode);
        return true;
      }
...
bool TR_LocalLiveRangeReduction::verifyRefInfo(List<TR::Node> *verifier,
  ...
  for (node = listIt.getFirst(); node != NULL; node = listIt.getNext()) {
    if (verifier->find(node))
      ...
      if (trace())
        traceMsg(comp(), "LocalLiveRangeReduction:node %p should not have beed "
                         "in the List\n",
                 node);
      return false;
    }
