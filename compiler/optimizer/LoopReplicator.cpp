  ...
  for (TR_RegionStructure *loop = it.getFirst(); loop; loop = it.getNext()) {
    ...
    if (isInnerLoopHot) {
      ...
      if (!searchList(innerLoopHeader, common, lInfo)) {
        traceMsg(comp(), "not going to replicate loop because hot inner loop "
                         "%d is not on the trace\n",
                 loop->getNumber());
        return false;
      }
    }
  ...
  for (;;) {
    ...
    for (auto e = tracePrefixCursor->getPredecessors().begin();
      ...
      if (pred->isCold() && !searchList(pred, common, lInfo)) {
        if (trace())
          traceMsg(comp(), "Found a cold side-entry into block_%d from "
                           "block_%d. Will replicate.\n",
...
bool TR_LoopReplicator::computeWeight(TR::CFGEdge *edge) {
  ...
  if (trace()) {
    ...
    traceMsg(comp(), "      w(Y): %d w(X): %d w(seed): %d w(Y)/w(X): %.4f "
                     "w(Y)/w(seed): %.4f\n",
