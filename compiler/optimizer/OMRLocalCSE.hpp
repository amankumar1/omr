...
public:
  ...
  PreviousNodeConversion(TR_Memory *m, TR::Node *convertedStoreNode) {
    ...
    _conversions = new (m->trHeapMemory())
        TR_Array<struct previousConversion *>(m, 8, true, heapAlloc);
    next = 0;
  }

  TR::Node *getConvertedStoreNode() { return _convertedStoreNode; }
  TR_Array<struct previousConversion *> *getConversions() {
    return _conversions;
  }

  void addConvertedNode(TR::Node *convertedNode, TR::SymbolReference *symRef) {
    struct previousConversion *pc =
        (struct previousConversion *)_trMemory->allocateHeapMemory(
            sizeof(struct previousConversion));
    pc->symRef = symRef;
...
protected:
  ...
  void examineNode(TR::Node *, SharedSparseBitVector &, TR::Node *, int32_t,
  ...
  void makeNodeAvailableForCommoning(TR::Node *, TR::Node *,
                                     SharedSparseBitVector &, bool *);
  ...
  void setPreviousConversion(TR::Node *storeNode, TR::Node *convertedNode,
                             TR::SymbolReference *symRef);
