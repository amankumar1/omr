...
public:
  ...
  void initialize(uint32_t numSymRefs, int32_t numBlocks) {
    _maxEntries = numSymRefs * numBlocks;
  }
...
private:
  ...
  enum { MB50 = 0x19000000, MB100 = 0x32000000 };

  void collectReferencedAutoSymRefs(TR::Node *node, Set *referencedAutos,
                                    vcount_t visitCount);
...
public:
  ...
  public:
    BlockInfo(TR_Memory *m, TR::Allocator &a)
        : _block_set(), _block_map(_block_map.DefaultSize, TR::Allocator(a)) {
    ...
    bool find(uint32_t block) { return (bool)_block_set.get(block); }
    ...
    Cursor all() { return Cursor(_block_set); }
  };
  ...
  void addBlock(TR::Block *b, int32_t numberOfLoadsAndStores, TR_Memory *,
                bool ifNotFound = false);
  ...
  TR_BitVector &getBlocksLiveWithinGenSetsOnly() {
    return _liveWithinGenSetsOnly;
  }
  TR::Symbol *getSymbol();
  ...
  void setDontAssignVMThreadRegister(bool b) {
    _dontAssignVMThreadRegister = b;
  }
  ...
  void setInitialBlocksWeightComputed(bool b) {
    _initialBlocksWeightComputed = b;
  }
...
public:
  ...
  TR::GlobalSet &getReferencedAutoSymRefs() {
...
public:
  ...
  typedef CS2::HashTable<int32_t, struct coordinates, TR::Allocator>
      Coordinates;
