...
TR_CompactLocals::TR_CompactLocals(TR::OptimizationManager *manager)
    : _liveVars(NULL), _prevLiveVars(NULL), _temp(NULL),
      _localIndexToSymbolMap(NULL), _localsIG(NULL), TR::Optimization(manager) {
}
...
int32_t TR_CompactLocals::perform() {
  ...
  // Create a local index to symbol mapping table and seed the interference
  // graph.
  //
  _localIndexToSymbolMap = new (trStackMemory())
      TR_Array<TR::AutomaticSymbol *>(trMemory(), numLocals, false, stackAlloc);
  ...
  *_prevLiveVars = *bv;
  ...
  while (bvi.hasMoreElements()) {
    ...
    while (wbvi.hasMoreElements()) {
      ...
      if (sym1 && sym2 && eligibleLocal(sym1) && eligibleLocal(sym2) &&
...
bool TR_CompactLocals::eligibleLocal(TR::AutomaticSymbol *localSym) {
  ...
  if (_localsIG->getNumNodes() > MAX_NUMBER_OF_LOCALS &&
      _localsIG->getIGNodeForEntity(localSym) == NULL)
    return false;
  return true;
}
