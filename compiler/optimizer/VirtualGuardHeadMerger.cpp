...
// cold1 is
// a predecessor of cold2 (a situation commonly greated by virtual guard tail
// splitter) we
...
// BBSTART
// guard2 -> cold1
// This is safe because there are no trees between the guards and calling the
// caller will
// result in the call to the callee if we need to patch guard2. cold2 and its
// mergebacks
// can then be eliminated
//
// In addition this opt will try to move guard2 up from the end of a block to
// the
// start of the block. We can do this if guard2 is an HCR guard and there is no
// GC point
...
// guard if we
// detect that the taken side of the guard merges back in the next block - this
// happens
// for some empty methods and is common for Object.<init> at the top of
// constructors.
    ...
    if (isMergeableGuard(guard1)) {
            ...
            ((block->getNextBlock()->getSuccessors().size() == 2) &&
             block->getNextBlock()
                ...
                performTransformation(
                    comp(), "%sCloning block_%d and placing clone after "
                            "block_%d to reduce HCR guard nops\n",
                    OPT_DETAILS, block->getNextBlock()->getNumber(),
                    cold1->getNumber()))
          tailSplitBlock(block, cold1);
      }

      // guard motion is fairly complex but what we want to achieve around
      // guard1 is a sequence
      // of relocated privarg blocks, followed by a sequence of runtime
      // patchable guards going to
      // guard1's cold block, followed by a sequence of stop-the-world guards
      // going to guard1's
      // cold block
      //
      ...
      for (TR::Block *nextBlock = block->getNextBlock(); nextBlock;
        ...
        if (guard1->getInlinedSiteIndex() == guard2->getInlinedSiteIndex()) {
          if (trace())
            traceMsg(comp(), "  Guard1 [%p] is guarding the same call as "
                             "Guard2 [%p] - proceeding with guard merging\n",
                     guard1, guard2);
          ...
          if (trace())
            traceMsg(comp(), "  Guard1 [%p] is the caller of Guard2 [%p] - "
                             "proceeding with guard merging\n",
                     guard1, guard2);
        } else if ((cold1->getSuccessors().size() == 1) &&
                   cold1->hasSuccessor(cold2)) {
          if (trace())
            traceMsg(comp(), "  Guard1 cold destination block_%d has guard2 "
                             "cold destination block_%d as its only successor "
                             "- proceeding with guard merging\n",
                     cold1->getNumber(), cold2->getNumber());
        } else {
          if (trace())
            traceMsg(comp(), "  Cold1 block_%d and cold2 block_%d of guard2 "
                             "[%p] in unknown relationship - abandon the merge "
                             "attempt\n",
        ...
        if (!performTransformation(
                comp(), "%sRedirecting guard [%p] in block_%d to parent guard "
                        "cold block_%d\n",
                OPT_DETAILS, guard2, guard2Block->getNumber(),
                cold1->getNumber()))
          continue;
        ...
        if (guard2Tree != guard2Block->getFirstRealTreeTop()) {
          ...
          // TODO We can safely leave code ahead of an HCR guard in place if no
          // runtime guards are going to be moved or if
          //     there is no data dependence between the code moved from ahead
          //     of the runtime guards and the code ahead
          //     of the HCR guard. This opt will increase the chances of a patch
          //     point not requiring nops (perf opportunity).
        ...
        if (insertPoint != guard2Block->getPrevBlock()) {
          TR::DebugCounter::incStaticDebugCounter(
              comp(), TR::DebugCounter::debugCounterName(
                          ...
                          guard1->isHCRGuard() ? "hcr" : "runtime",
                          guard2->isHCRGuard() ? "hcr" : "runtime",
                          comp()->signature()));
          cfg->setStructure(NULL);
