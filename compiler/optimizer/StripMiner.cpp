...
void TR_StripMiner::collectLoops(TR_Structure *str) {
  ...
  if (region->isNaturalLoop()) {
    ...
    if (checkIfIncrementalIncreasesOfPIV(li)) {
      if (trace())
        traceMsg(comp(), "\tReject loop %d ==> multiple store of induction "
                         "variable were found\n",
                 region->getNumber());
      return;
    }
    ...
    if (comp()->generateArraylets() && li->_arrayDataSize <= 0) {
      if (trace()) {
        if (li->_arrayDataSize < 0)
          traceMsg(comp(), "\tReject loop %d ==> array accesses of more than "
                           "one data size\n",
                   region->getNumber());
        else
    ...
    if ((li->_offset == -1) ||
      ...
      if (trace())
        traceMsg(comp(), "\tReject loop %d ==> needs a offsetLoop - cannot "
                         "deal with this now\n",
                 region->getNumber());
    ...
    if (iterCount != -1) {
      if (iterCount < li->_stripLen) {
        if (trace())
          traceMsg(comp(), "\tReject loop %d ==> iteration count is less than "
                           "the strip length\n",
                   region->getNumber());
        return;
      }
    ...
    if (li->_preOffset + li->_postOffset >= li->_stripLen) {
      if (trace())
        traceMsg(comp(), "\tReject loop %d ==> pre offset = %d, post offsets = "
                         "%d, strip length = %d\n",
                 region->getNumber(), li->_preOffset, li->_postOffset,
                 li->_stripLen);
      return;
    ...
    if (trace()) {
      traceMsg(comp(), "\tSuccess => adding candidate loop %d to the queue\n",
               region->getNumber());
      traceMsg(comp(), "\t\tpre-header = %d, loop test = %d, primary induction "
                       "variable = %d\n",
               preHeader->getNumber(), loopTest->getNumber(),
               piv->getSymRef()->getReferenceNumber());
      traceMsg(comp(), "\t\tpre-offset = %d, post-offset = %d, offset = %d, "
                       "strip length = %d\n",
...
void TR_StripMiner::transformLoop(LoopInfo *li) {
  ...
  bool isInt32 =
      ltTree->getNode()->getOpCode().isBooleanCompare() && // isCompare only
                                                           // returns true for
                                                           // signed compare --
                                                           // a better name
                                                           // would be
  ...
  if (testOverflow) {
    ...
    //
    if (trace())
      traceMsg(comp(), "\t   adding edge: overflow test block [%d] => original "
                       "pre-header [%d]\n",
  ...
  if (li->_needOffsetLoop) {
    ...
    if (testOverflow) {
      ...
      //
      if (trace())
        traceMsg(comp(), "\t   adding edge: overflow test block [%d] => outer "
                         "pre-header [%d]\n",
  ...
  if (li->_preOffset > 0) {
    ...
    gotoBlock->setFrequency((int32_t)((float)gotoBlock->getFrequency() / 10));
    if (trace()) {
      traceMsg(comp(), "\t   adding edges: outer header [%d] => goto [%d]; "
...
TR::Block *TR_StripMiner::stripMineLoop(LoopInfo *li, TR::Block *outerHeader) {
  ...
  if (trace()) {
    traceMsg(comp(), "\t   adding edges: branch [%d] => goto [%d]; "
...
void TR_StripMiner::redirect(TR::Block *source, TR::Block *oldDest,
  ...
  if (found) {
    ...
    if (trace()) {
      traceMsg(comp(), "\t      adding edges: source [%d] => goto [%d]; goto "
                       "[%d] => new dest [%d]\n",
...
void TR_StripMiner::examineNode(LoopInfo *li, TR::Node *parent, TR::Node *node,
  ...
  if (checkClone && node->getSymbolReference() == oldSymRef) {
    if (node->getOpCode().isLoad()) {
      ...
      if (foundLoad) {
        if (trace())
          traceMsg(comp(), "      adding node [%p] to load list parent: [%p], "
                           "childNum: %d\n",
...
void TR_StripMiner::replaceLoopPivs(LoopInfo *li, TR::ILOpCodes newOpCode,
  ...
  for (parent = it.getFirst(); parent != NULL; parent = it.getNext()) {
    ...
    if ((parentNode->getOpCode().isRightShift() ||
        ...
        skipNode) {
      if (trace())
        traceMsg(comp(), "      replacing load [%p] with new load [%p]: parent "
                         "[%p], childNum %d\n",
      ...
      if (node->getOpCode().isStore() &&
        ...
        if (((node->getFirstChild()->getOpCode().isSub()) ||
          ...
          if (pivIncInStore != li->_piv->getDeltaOnBackEdge()) {
            if (trace())
              traceMsg(comp(), "\t loop %d ==> Found a store to induction "
                               "variable with increment different than "
                               "deltaObBackEdge\n",
