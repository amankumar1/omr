...
TR::TreeTop *OMR::Simplifier::simplifyExtendedBlock(TR::TreeTop *treeTop) {
  ...
  for (; treeTop; treeTop = treeTop->getNextTreeTop()) {
    ...
    if (_performLowerTreeSimplifier) {
      _performLowerTreeNode = postWalkLowerTreeSimplifier(
          _performLowerTreeSimplifier, _performLowerTreeNode, block,
          (TR::Simplifier *)this);
...
TR::Node *OMR::Simplifier::unaryCancelOutWithChild(TR::Node *node,
  ...
  if (node->getOpCode().isConversionWithFraction() &&
    ...
    //    f2pd frac=0
    //       f
    // to just 'f' because the extra digits that should be introduced by the
    // frac=5 in the parent will be lost
    if (trace())
      traceMsg(comp(), "disallow unaryCancel of node %p and firstChild %p due "
                       "to mismatch of decimal fractions (%d != %d)\n",
               node, firstChild, node->getDecimalFraction(),
               firstChild->getDecimalFraction());
    return 0;
  }
  if (firstChild->getOpCodeValue() == opcode && node->getType().isBCD() &&
    ...
    //    pd2zd p=2        34
    //       pdx p=4     1234
    // if folding is performed to remove zd2pd/pd2zd then the result will be
    // 1234 instead of 0034
    if (trace())
      traceMsg(comp(), "disallow unaryCancel of node %p and firstChild %p due "
                       "to intermediate truncation of node\n",
               node, firstChild);
    return 0;
  } else if (firstChild->getOpCodeValue() == opcode &&
             node->getType().isBCD() && !firstChild->getType().isBCD()) {
    // illegal to fold an intermediate truncation:
    // dd2zd p=20 srcP=13
    //   zd2dd (no p specifed, but by data type, must be <= 16 and by srcP must
    //   be <= 13)
    //     zdX p=20
    // Folding gives an incorrect result if either srcP or the implied p of the
    // zd2dd is less than p on the dd2zd
    ...
    if (childP < nodeP && childP < grandChildP) {
      if (trace())
        traceMsg(comp(), "disallow unaryCancel of node %p and firstChild %p "
                         "due to intermediate truncation of node\n",
                 node, firstChild);
      return 0;
    }
  } else if (firstChild->getOpCodeValue() == opcode &&
    ...
    //   l2dd
    //     lX
    // Folding could give an incorrect result because the max precision of a dd
    // is 16 and the max precision of an l is 19
    if (node->getDataType().canGetMaxPrecisionFromType() &&
        firstChild->getDataType().canGetMaxPrecisionFromType() &&
        node->getDataType().getMaxPrecisionFromType() >
            firstChild->getDataType().getMaxPrecisionFromType()) {
      if (trace())
        traceMsg(comp(), "disallow unaryCancel of node %p and firstChild %p "
                         "due to intermediate truncation of node\n",
                 node, firstChild);
  ...
  if (firstChild->getOpCodeValue() == opcode &&
    ...
    // truncations (see BCD case above) but then would require a node
    // op that would just correct for size (e.g. addrSizeMod size=3 to replace
    // the o2a/a2o pair)
    //
    ...
    // as the 'truncated' bytes are known to be zero
    // (i.e. there really isn't an intermediate truncation of 4->3 even though
    // it appears that way from looking at the sizes alone)
    // o2i
    //   i2o size=3
    //     iushr
    //       x
    ...
    if (node->getType().isIntegral() && nodeSize == grandChild->getSize() &&
      ...
      if (grandChild->getOpCode().isRightShift() &&
        ...
        disallow = false;
        if (trace())
          traceMsg(comp(), "do allow unaryCancel of node %s (%p) and "
                           "firstChild %s (%p) as grandChild %s (%p) zeros the "
                           "%d truncated bytes\n",
    ...
    if (disallow) {
      if (trace())
        traceMsg(comp(), "disallow unaryCancel of node %s (%p) and firstChild "
                         "%s (%p) due to unequal sizes (nodeSize %d, "
  ...
  if (firstChild->getOpCodeValue() == opcode &&
      performTransformation(
          comp(), "%sRemoving node [" POINTER_PRINTF_FORMAT
...
#ifdef J9_PROJECT_SPECIFIC
    TR_RawBCDSignCode alwaysGeneratedSign =
        comp()->cg()->alwaysGeneratedSign(node);
    if (node->getType().isBCD() && grandChild->getType().isBCD() &&
      ...
      // must maintain the top level node's precision when replacing with the
      // grandchild
      // (otherwise if the parent of the node is call it will pass a too small
      // or too big value)
      ...
      if (node->getDecimalPrecision() != grandChild->getDecimalPrecision()) {
        ...
        grandChild = TR::Node::create(
            TR::ILOpCode::modifyPrecisionOpCode(grandChild->getDataType()), 1,
            origGrandChild);
        origGrandChild->decReferenceCount(); // inc'd an extra time when
                                             // creating modPrecision node above
        grandChild->incReferenceCount();
        grandChild->setDecimalPrecision(node->getDecimalPrecision());
        dumpOptDetails(comp(), "%sCreate %s [" POINTER_PRINTF_FORMAT
                               "] to reconcile precision mismatch between node "
                               "%s [" POINTER_PRINTF_FORMAT
                               "] grandChild %s [" POINTER_PRINTF_FORMAT
                               "] (%d != %d)\n",
                       optDetailString(), grandChild->getOpCode().getName(),
                       grandChild, node->getOpCode().getName(), node,
                       origOrigGrandChild->getOpCode().getName(),
                       origOrigGrandChild, node->getDecimalPrecision(),
                       origOrigGrandChild->getDecimalPrecision());
      }
      // if the top level was always setting a particular sign code (e.g. ud2pd)
      // then must maintain this side-effect here when cancelling
      if (alwaysGeneratedSign != raw_bcd_sign_unknown) {
        TR::Node *origGrandChild = grandChild;
        TR::ILOpCodes setSignOp =
            TR::ILOpCode::setSignOpCode(grandChild->getDataType());
        TR_ASSERT(setSignOp != TR::BadILOp,
                  "could not find setSignOp for type %s on %s (%p)\n",
                  grandChild->getDataType().toString(),
                  grandChild->getOpCode().getName(), grandChild);
        grandChild = TR::Node::create(
            setSignOp, 2, origGrandChild,
            TR::Node::iconst(origGrandChild,
                             TR::DataType::getValue(alwaysGeneratedSign)));
        origGrandChild->decReferenceCount(); // inc'd an extra time when
                                             // creating setSign node above
        grandChild->incReferenceCount();
        grandChild->setDecimalPrecision(origGrandChild->getDecimalPrecision());
        dumpOptDetails(
            comp(), "%sCreate %s [" POINTER_PRINTF_FORMAT
                    "] to preserve setsign side-effect between node %s "
                    "[" POINTER_PRINTF_FORMAT
                    "] grandChild %s [" POINTER_PRINTF_FORMAT "] (sign=0x%x)\n",
            optDetailString(), grandChild->getOpCode().getName(), grandChild,
            node->getOpCode().getName(), node,
      ...
      // Folding gives an incorrect result if the truncation on the dd2zd isn't
      // preserved
      ...
      if (childP < nodeP && childP < grandChildP) {
        ...
        dumpOptDetails(comp(), "%sCreate %s [" POINTER_PRINTF_FORMAT
                       ...
                       grandChild, node->getOpCode().getName(), node,
                       origOrigGrandChild->getOpCode().getName(),
                       origOrigGrandChild, nodeP, childP);
      }
    } else
#endif
        if (bytesLeftAfterTruncation > 0) {
      TR_ASSERT(bytesLeftAfterTruncation < 8,
                "bytesLeftAfterTruncation %d should be < 8 for node %p\n",
                bytesLeftAfterTruncation, node);
      TR_ASSERT(grandChild->getType().isInt64(),
                "node %s (%p) should an Int64 type\n",
                grandChild->getOpCode().getName(), grandChild);
      ...
      grandChild =
          TR::Node::create(TR::land, 2, grandChild,
                           TR::Node::lconst(grandChild, constantForAnd));
      grandChild->getFirstChild()->decReferenceCount(); // inc'd an extra time
                                                        // when creating
                                                        // modPrecision node
                                                        // above
      grandChild->incReferenceCount();
      dumpOptDetails(comp(), "%sCreate %s [" POINTER_PRINTF_FORMAT
                             "] 0x%llx to account for %d truncated bytes "
                             "between node %s [" POINTER_PRINTF_FORMAT
                             "] grandChild %s [" POINTER_PRINTF_FORMAT "]\n",
                     optDetailString(), grandChild->getOpCode().getName(),
                     grandChild, (uint64_t)constantForAnd,
                     bytesLeftAfterTruncation, node->getOpCode().getName(),
                     node, grandChild->getFirstChild()->getOpCode().getName(),
