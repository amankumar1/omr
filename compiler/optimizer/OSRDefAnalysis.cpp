...
TR_OSRDefInfo::TR_OSRDefInfo(TR::OptimizationManager *manager)
  ...
  // base constructor, base class
  // instances of virtual functions are called, because the derived object would
  // not yet have been set up.
  // ie prepareUseDefInfo will call base function
  // TR_UseDefInfo::prepareAnalysisback NOT
...
void TR_OSRDefInfo::performFurtherAnalysis(AuxiliaryData &aux) {
  ...
  for (intptrj_t i = 0; i < methodSymbol->getOSRPoints().size(); ++i) {
    ...
    // i-th osr point has
    // been deleted (e.g., because it was unreachable), and therefore, we have
    // never set _defsForOSR[i] to
    // a non-NULL value
    if (!info.IsNull()) {
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (methodSymbol->sharesStackSlot(defSymRef)) {
          ...
          comp()->getOSRCompilationData()->addSlotSharingInfo(
              point->getNodeByteCodeInfo(), slot, symRefNum, symRefOrder,
              defSymRef->getSymbol()->getSize(), takesTwoSlots);
          if (trace()) {
            TR_ByteCodeInfo &bcInfo = point->getNodeByteCodeInfo();
            traceMsg(comp(), "added (callerIndex=%d, bcIndex=%d)->(slot=%d, "
                             "ref#=%d) at OSR point %d side %d def %d\n",
...
void TR_OSRDefInfo::addSharingInfo(AuxiliaryData &aux) {
  ...
  // mapped to that slot.
  // we compute the union of the def bit-vectors of the symbols in S and assign
  // it to the def bit-vector
  ...
  for (int i = 0; ppsListArray && i < ppsListArray->size(); ++i) {
    ...
    if ((ppsList.getSize() > 1) || isTwoSlotSymRefAtThisSlot ||
      ...
      // shallow swap bit vectors, causing the contents of prevUnionTwoSlotDef
      // prior to the swap
      // to be discarded, and its new contents from twoSlotUnionDef to be
      // preserved
...
void TR_OSRDefInfo::buildOSRDefs(void *vblockInfo, AuxiliaryData &aux) {
  ...
  /*
     ...
     reachingDefinitions._blockAnalysisInfo[block->getNumber()];
           if (trace())
              {
  ...
  for (int i = 0; i < numOSRPoints; ++i) {
    ...
    if (trace()) {
      ...
      TR_ByteCodeInfo &bcinfo =
          _methodSymbol->getOSRPoints()[i]->getNodeByteCodeInfo();
      traceMsg(
          comp(),
          "OSR defs at index %d bcIndex %d callerIndex %d induceAfter %d\n", i,
          bcinfo.getByteCodeIndex(), bcinfo.getCallerIndex(),
          _methodSymbol->getOSRPoints()[i]->induceAfter());
      *comp() << info;
      traceMsg(comp(), "\n");
    }
  }
...
void TR_OSRDefInfo::buildOSRDefs(TR::Node *node, void *vanalysisInfo,
  ...
  if (expandedNodeIndex != NULL_USEDEF_SYMBOL_INDEX && expandedNodeIndex != 0) {
    ...
    TR_UseDefInfo::BitVector const &defsForSymbol =
        aux._defsForSymbol[symIndex];
    if (!defsForSymbol.IsZero() && isExpandedDefIndex(expandedNodeIndex) &&
        !sym->isRegularShadow() && !sym->isMethod()) {
      /*
                  if (trace())
                     {
                     traceMsg(comp(), "defs for symbol %d with symref index
...
int32_t TR_OSRDefAnalysis::perform() {
  ...
  if (!requiresAnalysis()) {
    if (trace()) {
      traceMsg(comp(), "%s OSR reaching definitions analysis is not required "
    ...
    if (trace()) {
      traceMsg(comp(), "%s OSR reaching definition analysis is not required "
...
TR_OSRLiveRangeAnalysis::TR_OSRLiveRangeAnalysis(
    ...
    : TR::Optimization(manager), _deadVars(NULL), _liveVars(NULL) {}

int32_t TR_OSRLiveRangeAnalysis::perform() {
  if (comp()->getOption(TR_EnableOSR)) {
    static const char *disableOSRLiveRangeAnalysis =
        feGetEnv("TR_DisableOSRLiveRangeAnalysis");
    // if (comp()->getOption(TR_DisableOSRLiveRangeAnalysis)) // add to Options
    // later
    if (disableOSRLiveRangeAnalysis) {
      if (comp()->getOption(TR_TraceOSR))
        traceMsg(comp(),
                 "OSR is enabled but OSR live range analysis is not.\n");
      return 0;
    }
  } else {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(), "OSR Option not enabled -- returning from OSR live "
                       "range analysis.\n");
    return 0;
  }
  ...
  if (!canAffordAnalysis()) {
    if (comp()->getOption(TR_TraceOSR)) {
      traceMsg(comp(), "%s OSR live range analysis is not affordable for this "
                       "method because of the expected cost\n",
               optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
      traceMsg(comp(), "Returning...\n");
    }
    ...
    if (comp()->getOption(TR_TraceOSR)) {
      traceMsg(
          comp(),
          "%s OSR live range analysis is not required because we are peeking\n",
          optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
      traceMsg(comp(), "Returning...\n");
    }
    return 0;
  ...
  if (comp()->getOption(TR_TraceOSR)) {
    ...
    comp()->dumpMethodTrees("Before OSR live range analysis",
                            optimizer()->getMethodSymbol());
  }

  {
    ...
    _pendingSlotValueParents =
        (NodeParentInfo **)trMemory()->allocateStackMemory(
            numBits * sizeof(NodeParentInfo *));
    ...
    while (block) {
      ...
      for (tt = block->getExit(); tt != firstTT; tt = tt->getPrevTreeTop()) {
        if (tt->getNode()->getOpCodeValue() == TR::BBEnd) {
          ...
          if (comp()->getOption(TR_TraceOSR)) {
            traceMsg(comp(), "BB_End for block_%d: live vars = ",
                     block->getNumber());
        ...
        if (comp()->isPotentialOSRPoint(tt)) {
          ...
          TR_ASSERT(osrPoint != NULL, "Cannot find an OSR point for node %p",
                    tt->getNode());
        } else
          osrPoint = NULL;

        if (osrPoint && !osrPoint->induceAfter())
          buildOSRLiveRangeInfo(tt->getNode(), _liveVars, osrPoint,
                                liveLocalIndexToSymRefNumberMap,
                                maxSymRefNumber, numBits, osrMethodData);

        maintainLiveness(tt->getNode(), NULL, -1, visitCount, &liveLocals,
                         _liveVars, block);
  ...
  if (node->getOpCode().isStoreDirect()) {
        ...
        if (_pendingPushVars->get(localIndex)) {
          if (_pendingSlotValueParents[localIndex]) {
            NodeParentInfo *nodeParentInfo =
                _pendingSlotValueParents[localIndex];
            ...
            if (node->getFirstChild()->getVisitCount() != visitCount) {
              ...
              node->getFirstChild()->setLocalIndex(
              ...
              if (node->getFirstChild()->getOpCode().hasSymbolReference() &&
                  node->getFirstChild()
                      ...
                      ->getSymbol()
                      ->isAuto()) {
                TR::AutomaticSymbol *rhsLocal = node->getFirstChild()
                                                    ->getSymbolReference()
                                                    ->getSymbol()
                                                    ->getAutoSymbol();
                if (rhsLocal && !rhsLocal->isLiveLocalIndexUninitialized()) {
                  ...
                  TR_ASSERT(rhsLocalIndex >= 0, "bad local index: %d\n",
                            rhsLocalIndex);
                  rhsLocal->setLocalIndex(
                      rhsLocal->getLocalIndex() +
                      node->getFirstChild()->getReferenceCount());
                }
              }
            }
    ...
    if (local && !local->isLiveLocalIndexUninitialized()) {
      ...
      if (0 && _pendingPushVars->get(localIndex)) {
        ...
        if (!nodeParentInfo) {
          nodeParentInfo =
              (NodeParentInfo *)comp()->trMemory()->allocateStackMemory(
                  sizeof(NodeParentInfo));
        ...
        ParentInfo *parentInfo =
            (ParentInfo *)comp()->trMemory()->allocateStackMemory(
                sizeof(ParentInfo));
...
void TR_OSRLiveRangeAnalysis::buildOSRLiveRangeInfo(
    ...
    int32_t *liveLocalIndexToSymRefNumberMap, int32_t maxSymRefNumber,
  ...
  *_deadVars -= *liveVars;
  ...
  if (_deadVars->isEmpty())
    ...
    deadSymRefs = new (trHeapMemory())
        TR_BitVector(maxSymRefNumber + 1, trMemory(), heapAlloc);
  ...
  osrMethodData->addLiveRangeInfo(node->getByteCodeIndex(), deadSymRefs);

  if (comp()->getOption(TR_TraceOSR)) {
    traceMsg(comp(), "Dead variables at OSR point %p \n", node);
    if (deadSymRefs)
  ...
  for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
    ...
    for (auto edge = block->getExceptionSuccessors().begin();
      ...
      if (catchBlock->isOSRCatchBlock()) {
        if (!blockCanOSR &&
            performTransformation(comp(), "%s: Remove redundant exception edge "
                                          "from block_%d at [%p] to OSR catch "
                                          "block_%d at [%p]\n",
                                  OPT_DETAILS, block->getNumber(), block,
          ...
          TR::DebugCounter::incStaticDebugCounter(
              ...
              TR::DebugCounter::debugCounterName(
                  comp(), "exceptionEdgeRemoval/(%s)", comp()->signature()));
        }
      }
    }
  }
...
int32_t TR_OSRExceptionEdgeRemoval::perform() {
  ...
  if (comp()->getOption(TR_FullSpeedDebug)) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(), "FSD enabled -- returning from OSR exception edge "
                       "removal analysis since we implement FSD with exception "
                       "edges.\n");
    return 0;
  }
  ...
  if (comp()->isPeekingMethod()) {
    if (comp()->getOption(TR_TraceOSR)) {
      traceMsg(comp(), "%s OSR exception edge removal is not required because "
                       "we are peeking\n",
               optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
      traceMsg(comp(), "Returning...\n");
    }
    ...
    for (treeTop = block->getEntry(); treeTop != block->getExit();
      ...
      if ((treeTop->getNode()->getNumChildren() > 0) &&
          ...
          (comp()->getSymRefTab()->element(TR_induceOSRAtCurrentPC) ==
           treeTop->getNode()->getFirstChild()->getSymbolReference())) {
