...
// Utility function to count the number of bits set in an unsigned 32-bit
// integer.
//
...
TR::Node *generatePoisonNode(TR::Compilation *comp, TR::Block *currentBlock,
  ...
  if (liveAutoSymRef->getSymbol()->getType().isAddress())
    storeNode = TR::Node::createStore(
        liveAutoSymRef,
        TR::Node::aconst(currentBlock->getEntry()->getNode(), 0x0));
  else if (liveAutoSymRef->getSymbol()->getType().isInt64())
    storeNode = TR::Node::createStore(
        liveAutoSymRef,
        TR::Node::lconst(currentBlock->getEntry()->getNode(), 0xc1aed1e5));
  else if (liveAutoSymRef->getSymbol()->getType().isInt32())
    storeNode = TR::Node::createStore(
        liveAutoSymRef,
        TR::Node::iconst(currentBlock->getEntry()->getNode(), 0xc1aed1e5));
  else
    poisoned = false;

  if (comp->getOption(TR_TraceCG) && comp->getOption(TR_PoisonDeadSlots)) {
    if (poisoned)
      traceMsg(comp, "POISON DEAD SLOTS --- Live local %d  from parent block "
                     "%d going dead .... poisoning slot with node 0x%x .\n",
               liveAutoSymRef->getReferenceNumber(), currentBlock->getNumber(),
               storeNode);
    else
      traceMsg(comp, "POISON DEAD SLOTS --- Live local %d of unsupported type "
                     "from parent block %d going dead .... poisoning "
                     "skipped.\n",
               liveAutoSymRef->getReferenceNumber(), currentBlock->getNumber());
  }
...
OMR::CodeGenerator::CodeGenerator()
    : _compilation(TR::comp()), _trMemory(TR::comp()->trMemory()),
      ...
      _vmThreadLiveCount(0), _vmThreadSpillInstr(NULL), _stackAtlas(NULL),
      _methodStackMap(NULL), _binaryBufferStart(NULL),
      _binaryBufferCursor(NULL), _largestOutgoingArgSize(0), _warmCodeEnd(NULL),
      _coldCodeStart(NULL), _estimatedWarmLength(0), _estimatedColdLength(0),
      _estimatedSnippetStart(0), _accumulatedInstructionLengthError(0),
      _registerSaveDescription(0),
      _extendedToInt64GlobalRegisters(self()->comp()->allocator()),
      _liveButMaybeUnreferencedLocals(NULL), _liveMonitors(NULL),
      _assignedGlobalRegisters(NULL), _aheadOfTimeCompile(NULL),
      _globalRegisterTable(NULL), _currentGRABlockLiveOutSet(NULL),
      _localsIG(NULL), _lastGlobalGPR(0), _firstGlobalFPR(0), _lastGlobalFPR(0),
      _firstOverlappedGlobalFPR(0), _lastOverlappedGlobalFPR(0),
      _firstGlobalAR(0), _lastGlobalAR(0), _last8BitGlobalGPR(0),
      _globalGPRPartitionLimit(0), _globalFPRPartitionLimit(0),
      _firstGlobalVRF(-1), _lastGlobalVRF(-1), _firstOverlappedGlobalVRF(-1),
      ...
      _codeCache(0), _committedToCodeCache(false),
      _dummyTempStorageRefNode(NULL), _blockRegisterPressureCache(NULL),
      ...
      _internalPointerSpillFreeList(
          getTypedAllocator<TR_BackingStore *>(TR::comp()->allocator())),
      _spilledRegisterList(NULL), _firstTimeLiveOOLRegisterList(NULL),
      ...
      _nodesUnderComputeCCList(
          getTypedAllocator<TR::Node *>(TR::comp()->allocator())),
      _nodesToUncommonList(
          getTypedAllocator<TR::Node *>(TR::comp()->allocator())),
      _nodesSpineCheckedList(
          getTypedAllocator<TR::Node *>(TR::comp()->allocator())),
      _collectedSpillList(
          getTypedAllocator<TR_BackingStore *>(TR::comp()->allocator())),
      _allSpillList(
          getTypedAllocator<TR_BackingStore *>(TR::comp()->allocator())),
      _relocationList(
          getTypedAllocator<TR_Relocation *>(TR::comp()->allocator())),
      _aotRelocationList(
          getTypedAllocator<TR_Relocation *>(TR::comp()->allocator())),
      _breakPointList(getTypedAllocator<uint8_t *>(TR::comp()->allocator())),
      _jniCallSites(
          getTypedAllocator<TR_Pair<TR_ResolvedMethod, TR::Instruction> *>(
              TR::comp()->allocator())),
      _lowestSavedReg(0), _preJitMethodEntrySize(0),
      _jitMethodEntryPaddingSize(0), _signExtensionFlags(NULL),
      _lastInstructionBeforeCurrentEvaluationTreeTop(NULL),
      _unlatchedRegisterList(NULL), _indentation(2),
      _preservedRegsInPrologue(NULL), _currentBlock(NULL),
      ...
      _stackOfMemoryReferencesCreatedDuringEvaluation(
          self()->comp() ? self()->comp()->trMemory() : 0, 16),
      _afterRA(false), randomizer(self()->comp()),
      _outOfLineColdPathNestedDepth(0), _codeGenPhase(self()),
      _symbolDataTypeMap(self()->comp()->allocator()), _lmmdFailed(false) {
...
TR::CodeGenerator *OMR::CodeGenerator::self() {
...
void OMR::CodeGenerator::setUpForInstructionSelection() {
  ...
  // treetops because
  // the _register and _label fields are unioned members of a node.
  // prepareNodeForInstructionSelection
  // zeros the _register field while the second for loop sets label fields on
  // destination nodes.
  //
...

  {
    ...
    if (!comp->getOption(TR_DisableOptAttributesMemoryFree)) {
      if (!TR::Compiler->target.cpu.isX86()) {
        // x86 code uses getRegister post instruction selection, prevents
        // FreeOptAttributes() from being called
...
bool OMR::CodeGenerator::use64BitRegsOn32Bit() {
  ...
  if (!(TR::Compiler->target.isZOS() || TR::Compiler->target.isLinux()))
    ...
    bool longReg =
        self()->comp()->getOptions()->getOption(TR_Enable64BitRegsOn32Bit);
    bool longRegHeur = self()->comp()->getOptions()->getOption(
        TR_Enable64BitRegsOn32BitHeuristic);
    bool use64BitRegs =
        (longReg && !longRegHeur &&
         self()->comp()->getJittedMethodSymbol()->mayHaveLongOps()) ||
...
void OMR::CodeGenerator::setVMThreadRequired(bool v) {
  if (self()->comp()->getOption(TR_DisableLateEdgeSplitting) &&
      _liveRegisters[TR_GPR]) {
    if (v) {
      ...
      TR_ASSERT(
          _vmThreadRegister->isLive(),
          "Tried to remove the vmThread from live list, but wasn't live\n");
      TR_ASSERT(_vmThreadLiveCount != 0,
                "Tried to underflow the VMThread register live count\n");
...
bool OMR::CodeGenerator::getIsInWarmCodeCache() {
...
void OMR::CodeGenerator::initializeLinkage() {
  ...
  linkage =
      linkage ? linkage : self()->createLinkage(self()
                                                    ->comp()
...
uint32_t OMR::CodeGenerator::getWarmCodeLength() // cast explicitly
{
  return (uint32_t)(self()->getWarmCodeEnd() - self()->getCodeStart());
}

uint32_t OMR::CodeGenerator::getColdCodeLength() // cast explicitly
{
  return (uint32_t)(
      _coldCodeStart ? self()->getCodeEnd() - self()->getColdCodeStart() : 0);
}
...
bool OMR::CodeGenerator::supportsNativeLongOperations() {
  return (TR::Compiler->target.is64Bit() || self()->use64BitRegsOn32Bit());
}

void OMR::CodeGenerator::setVMThreadSpillInstruction(TR::Instruction *i) {
  if (_vmThreadSpillInstr == NULL &&
      self()->comp()->getOption(TR_DisableLateEdgeSplitting)) {
    _vmThreadSpillInstr = i;
  } else {
    _vmThreadSpillInstr =
        (TR::Instruction *)0xffffffff; // set to sentinel to indicate that
...
uint16_t OMR::CodeGenerator::getNumberOfGlobalGPRs() {
  if (self()->comp()->cg()->supportsHighWordFacility() &&
...
int32_t OMR::CodeGenerator::generateCode() {
...
// J9
//
TR::Node *OMR::CodeGenerator::createOrFindClonedNode(TR::Node *node,
                                                     int32_t numChildren) {
  TR_HashId index;
...
int32_t TR_SetMonitorStateOnBlockEntry::addSuccessors(
    TR::CFGNode *cfgNode, TR_Stack<TR::SymbolReference *> *monitorStack,
    bool traceIt, bool dontPropagateMonitor, MonitorInBlock monitorType,
    int32_t callerIndex, bool walkOnlyExceptionSuccs) {
  if (traceIt)
    traceMsg(comp(), "\tIn SMSOBE::addSuccessors for cfgNode %d, monitorStack "
                     "%p dontPropagateMonitor %d monitorType = %d callerIndex "
                     "%d walkOlyExceptionSuccs %d\n",
             cfgNode->getNumber(), monitorStack, dontPropagateMonitor,
             monitorType, callerIndex, walkOnlyExceptionSuccs);
  ...
  //
  // 2. MonitorExit (at this stage the monitor has been temporarily pushed on
  // the monitorStack)
  // Normal Edge       -> pop before propagate
  // Exception Edge    ->
  //   check if the exception successor of the current block has the same caller
  //   index or not
  //   if same caller index, then:
  //   a) in a synchronized method, the catch block will eventually unlock the
  //   monitor. so we assume
  //   that if the blocks are in the same method, then the monitor will be
  //   unlocked by the catch block
  //   b) for a synchronized block, all blocks within the region (including the
  //   block with the monexit)
  //   will always branch to the "catch-all" block inserted by the JIT and not
  //   to a user catch block.
  //   the catch-all block will unlock the monitor and rethrow the exception.
  //   so in both these cases, we want to push the monitor along the exception
  //   successor because the monitor
  //   will be unlocked eventually.
  //
  //   if not the same caller index, then:
  //   a) the monexit block and the exception successor are in different methods
  //   (ie. the monexit - regardless of
  //   sync method or sync block, came from an inlined method and the successor
  //   is in the caller).
  //   in this case, we dont want to push the monitor along the exception
  //   successor because the exception successor
  //   has no idea that it needs to unlock the monitor.
  //
  //                   -> pop before propagate / push before propagate in the
  //                   case described above
  //   else            -> propagate the monitorStack (with the monitor pushed on
  //   it temporarily) ie. do not pop
  //
  ...
  for (TR::CFGEdge *edge = succs.getFirst(); edge; edge = succs.getNext()) {
    ...
    if (succBlock->getEntry() && succBlock->getVisitCount() != _visitCount) {
      ...
      if (monitorType == MonitorEnter) {
        if (traceIt)
          traceMsg(comp(), "\tIn TR_SetMonitorStateOnBlockEntry::addSuccessors "
                           "monitorType = MonitorEnter  block %d\n",
                   succBlock->getNumber());
      ...
      if (monitorType == MonitorExit) {
        ...
        } else if (succBlock->isCatchBlock()) // already processed during
                                              // exception successors walk
          continue;
      }

      if (traceIt)
        traceMsg(comp(), "process succBlock %d propagate (t/f: %d) "
                         "isCatchBlock=%d monitorType=%d callerIndex=%d "
                         "entryCallerIndex=%d\n",
                 succBlock->getNumber(), addInfo, succBlock->isCatchBlock(),
                 monitorType, callerIndex, succBlock->getEntry()
                                               ->getNode()
                                               ->getByteCodeInfo()
                                               .getCallerIndex());
      ...
      if (monitorStack) {
        ...
        if (traceIt)
          traceMsg(comp(), "\tIn SMSOnBE::addSuccesors  created "
                           "newMonitorStack %p and monitorStack %p\n",
                   newMonitorStack, monitorStack);

        // pop the last element of the stack if dontPropagateMonitor is true
        //
        if (!addInfo && !newMonitorStack->isEmpty()) {
          if (traceIt)
        ...
        _liveMonitorStacks->AddEntryAtPosition(succBlock->getNumber(),
                                               newMonitorStack);
        if (traceIt)
          traceMsg(comp(), "adding monitorstack to successor %d (%p size %d)\n",
                   succBlock->getNumber(), newMonitorStack,
                   newMonitorStack->size());
      }

      if (traceIt)
        traceMsg(
            comp(),
            "\tIn SMSOnBE::addSuccessors adding block %d to blocksToVisit\n",
            succBlock->getNumber());
...
//      i) if synchronized method then the syncObjectTemp would have been used
//      to
//      initialize the hidden slot in the DLT entry (monitorStack size is 1) and
//      then control could branch into the middle of a synchronized region
//      (without
//      ever seeing any monents). At the monexit, the analysis would then try to
//      pop
//      the lone monitor on the stack so prevent this by checking if the stack
//      size is 1.
//      This is done because the special slot needs to be live across the entire
//      method
//
//      ii) an improvement to i) is done at blocks that exit the method. in
//      these cases,
//      the analysis would have encountered the monexits corresponding to the
//      synchronized
//      'this' so it needs to empty the stack
//
//      iii) an exception to i) is when the method is a static synchronized
//      method. in this
//      case, the syncObjectTemp is not used to initialize the monitor slot in
//      the DLT entry
//      (so the monitorStack size is 0). so the analysis should avoid any
//      special checks
//
//      in case i) or iii) fails, this means that the DLT control landed
//      straight into the sync
//      region (with no monitorStack), in this case the analysis needs to ensure
//      that an empty
//      monitorStack is not popped
//
//
// b) if the monexit is not at callerIndex=-1, then this means that the monexit
// was
// part of a monent-monexit region that would have normally appeared in the
// method. an imbalance
// in the monitorStack indicates an error, but return the default answer as
// "yes", the stack
// can be popped.
//
...
static bool canPopMonitorStack(TR::Compilation *comp,
                               ...
                               TR::Node *node, bool blockExitsMethod,
                               bool traceIt) {
  ...
  if (comp->isDLT()) {
    if (callerIndex == -1) {
      if (comp->getJittedMethodSymbol()->isSynchronised()) {
        ...
        // the bit for this special slot so that we do not have any wrong GC
        // maps
        // TODO : if we need to empty the monitor stack at the end of the method
        // then pop it off the stack only when we reach a block whose successor
        // is
        // the dummy exit block
        //
        if (monitorStack->size() == 1 &&
            !comp->getJittedMethodSymbol()->isStatic() && !blockExitsMethod) {
          if (traceIt)
            traceMsg(comp, "monitorStack is empty (except for special DLT sync "
                           "object slot) for DLT compile at monexit %p\n",
                     node);
          return false;
        } else if (monitorStack->isEmpty()) {
          if (traceIt)
            traceMsg(comp,
                     "monitorStack is empty for DLT compile at monexit %p\n",
                     node);
          return false;
        }
      } else {
        if (monitorStack->isEmpty()) {
          if (traceIt)
            traceMsg(comp, "monitorStack is empty for non-synchronized DLT "
                           "compile at monexit %p\n",
                     node);
          return false;
        }
      }
    } else {
      // TODO : could add an assert error here : check if the elem to be popped
      // off
      ...
      if (!monitorStack->isEmpty() &&
         ...
         if (monitorStack->top()->getOwningMethodIndex() != callerIndex)
            traceMsg(comp(), "unbalanced monitorStack, trying to pop %d but top
      is %d symRef: %d\n", callerIndex,
      monitorStack->top()->getOwningMethodIndex(),
      monitorStack->top()->getReferenceNumber());
         TR_ASSERT(monitorStack->top()->getOwningMethodIndex() == callerIndex,
      "unbalanced monitorStack, trying to pop %d but top is %d\n", callerIndex,
      monitorStack->top()->getOwningMethodIndex());
         }
      */
      return true;
...
static bool needToPushMonitor(TR::Compilation *comp, TR::Block *block,
                              bool traceIt) {
  ...
  // BBEnd
  //
  // older javac includes the return in the exception range (for the catch-all
  // block)
  // when MethodEnter/MethodExit hooks are enabled, we split the blocks as
  // follows:
  // synchronized {
  // ...
  // }
  ...
  // BBEnd
  // ...
  // since the return is included in the exception range, the new blocks (after
  // the split)
  // also have exception successors to the catch-all block. this results in
  // unbalanced
  // monitors because we would have popped the monitorStack at the block
  // containing the monexit
  // new javac seem to correctly exclude the return from the exception range
  // To support this case, walk the successors of the current block and check if
  // the successor
  // has identical exception successors.
  //
  // A new case.  When a transaction exists, the monexitfence is in a separate
  // block from the
  // monexit and the tfinish.  So the two blocks will generally have an
  // exception edge with the same destination block
  ...
  for (auto e = block->getSuccessors().begin();
    ...
    if (comp->getFlowGraph()->compareExceptionSuccessors(block, succ) == 0) {
      if (traceIt)
        traceMsg(
            comp,
            "found identical exception successors for block %d and succ %d\n",
            block->getNumber(), succ->getNumber());

      retval = true;
      for (TR::TreeTop *tt = succ->getEntry(); tt != succ->getExit();
           tt = tt->getNextTreeTop()) {
        TR::Node *aNode = tt->getNode();

        if (aNode &&
            (aNode->getOpCodeValue() == TR::tfinish ||
             aNode->getOpCodeValue() == TR::monexit ||
             ((aNode->getOpCodeValue() == TR::treetop ||
               aNode->getOpCodeValue() == TR::NULLCHK) &&
              aNode->getFirstChild()->getOpCodeValue() == TR::monexit))) {
          if (traceIt)
            traceMsg(comp, "overriding identical exception decision because "
                           "node %p in block %d is either monexit or tfinish",
...
void TR_SetMonitorStateOnBlockEntry::set(bool &lmmdFailed, bool traceIt) {
  ...
  while (!_blocksToVisit.isEmpty()) {
    ...
    for (TR::TreeTop *tt = block->getEntry();; tt = tt->getNextTreeTop()) {
      ...
      if ((node->getOpCode()
               .isStore() && // only monents are represented by this store now
        ...
        if (monitorStack) {
          monitorStack = new (trHeapMemory())
              TR_Stack<TR::SymbolReference *>(*monitorStack);
          if (traceIt)
            traceMsg(comp(), "adding monitor to stack symbol=%p symRef=%d "
                             "(size=%d) (node %p)\n",
                     node->getSymbol(),
                     node->getSymbolReference()->getReferenceNumber(),
                     monitorStack->size() + 1, node);
        } else {
          monitorStack =
              new (trHeapMemory()) TR_Stack<TR::SymbolReference *>(trMemory());
          if (traceIt)
            traceMsg(comp(), "adding monitor to fresh stack symbol=%p "
                             "symRef=%d (size=%d) (node %p)\n",
                     node->getSymbol(),
                     node->getSymbolReference()->getReferenceNumber(),
                     monitorStack->size() + 1, node);
        }
        ...
        // don't push the monitor info along exception successors for this
        // block.
        // the typical pattern should be:
        // ...
        // astore <holdsMonitoredObject>
        // monent
        // BBEnd   // exception successor of this block will not unlock the
        // object (it
        //         // actually belongs to the caller
        // BBStart //start of inlined method, these blocks will have the
        // catchall block
        ...
        if (node->getSymbolReference()->holdsMonitoredObjectForSyncMethod() &&
          ...
          isSyncMethodMonent = true;
                 ...
                 canPopMonitorStack(comp(), monitorStack, node,
                                    blockExitsMethod, traceIt)) {
        if (!block->getExceptionSuccessors().empty())
          monitorExitFence++;

        // The check for this assume was moved in the if statement above.
        // JCK has tests for unbalanced monitor exits and we would crash during
        // compilation
        // if we tried to pop a non-existent monitor off the stack.
        // TR_ASSERT(!monitorStack->isEmpty(), "monitor stack is empty at block
        // %d node %p\n",
        //         block->getNumber(), node);
        //
        monitorStackTop = monitorStack->top();
        if (monitorStackTop &&
            monitorStackTop->holdsMonitoredObjectForSyncMethod())
        ...
        //
        monitorPoppedForExceptionSucc = addSuccessors(
            block, monitorStack, traceIt, false /*not used*/, MonitorExit,
            callerIndex, true /*walkOnlyExceptionSuccs*/);
        // monexit
        if (monitorStack->topIndex() == 0) {
          ...
          if (traceIt)
            traceMsg(
                comp(),
                "popping monitor off stack symRef=%d, BEFORE pop size=%d, ",
                monitorStack->top()->getReferenceNumber(),
                monitorStack->size());
          monitorStack->pop();
          if (traceIt)
      ...
      } else if (node->getOpCode().getOpCodeValue() != TR::monexit &&
          ...
          for (; edge != block->getExceptionSuccessors().end(); ++edge) {
            ...
            if (node->getByteCodeInfo().getCallerIndex() ==
              ...
              if (traceInitMonitorsForExceptionAfterMonexit)
                traceMsg(comp(), "block_%d has exceptions after monexit with "
                                 "catch block in the same method %s\n",
      ...
      if (tt == block->getExit()) {
        ...
        if ((monitorExitFence + monitorEnterStore) >= 2) {
          if (traceIt)
            traceMsg(comp(),
                     "block_%d has monitorEnterStore=%d monitorExitFence=%d\n",
        ...
        if (blockHasMonent) {
          ...
          // the monitor to the exception successors (this is because if there
          // is
          // an exception successor of a block containing the monent and control
          // to
          // the exception is reached, this means that the monitor is not
          // locked, ie
        ...
        if (blockHasMonexit) {
          ...
          // b) if the successor is an exception edge, then pop the stack *only*
          // if the
          // monexit came from a synchronized method (check the callerIndex on
          // the catch block
          // and the monexit)
          //
        ...
        if (traceIt)
          traceMsg(comp(), "blockHasMonent=%d blockHasMonexit=%d "
                           "dontPropagateMonitor=%d callerIndex=%d "
                           "monitorPoppedForExceptionSucc=%d\n",
                   blockHasMonent, blockHasMonexit, dontPropagateMonitor,
                   callerIndex, monitorPoppedForExceptionSucc);

        addSuccessors(block, monitorStack, traceIt, dontPropagateMonitor,
                      monitorType, callerIndex);
        break;
      }
    }
  }

  static bool disableCountingMonitors =
      feGetEnv("TR_disableCountingMonitors") ? true : false;
  if (initializeMonitorAutos && !disableCountingMonitors) {
    lmmdFailed = true;
    TR_Array<List<TR::RegisterMappedSymbol> *> &monitorAutos =
        comp()->getMonitorAutos();
    for (int32_t i = 0; i < monitorAutos.size(); i++) {
      ...
      if (autos) {
        ...
        for (TR::RegisterMappedSymbol *a = iterator.getFirst(); a;
             a = iterator.getNext()) {
...
// nodes are definitely disjoint (i.e. guaranteed not to overlap)
// any combination of stores,loads and address nodes are allowed as node1 and
// node2
...
TR_StorageOverlapKind OMR::CodeGenerator::storageMayOverlap(TR::Node *node1,
  ...
  if ((node2->getOpCode().isLoadVarOrStore() ||
    ...
    if (self()->traceBCDCodeGen())
      traceMsg(self()->comp(), "overlap=true : node1 %s (%p) and/or node2 %s "
                               "(%p) are not valid load/store/address nodes\n",
...
bool OMR::CodeGenerator::uniqueAddressOccurrence(
  ...
  if (duringEvaluation) {
    ...
    if (addr1->getReferenceCount() <= 1 && // refCounts may be zero when called
                                           // during ilgen and the nodes are
                                           // freshly popped
...
bool OMR::CodeGenerator::nodeMatches(TR::Node *addr1, TR::Node *addr2,
  ...
  if (addr1 == addr2) {
    ...
    foundMatch = true;
  } else if (addr1->getOpCodeValue() == TR::aload &&
             addr2->getOpCodeValue() == TR::aload &&
             self()->getLinkage()->isAddressOfStaticSymRef(
                 addr1->getSymbolReference()) &&
             self()->getLinkage()->isAddressOfStaticSymRef(
                 addr2->getSymbolReference()) &&
             addr1->getSymbolReference() == addr2->getSymbolReference()) {
    foundMatch = true;
  } else if (addr1->getOpCodeValue() == TR::aload &&
             addr2->getOpCodeValue() == TR::aload &&
             self()->getLinkage()->isAddressOfPrivateStaticSymRef(
                 addr1->getSymbolReference()) &&
             self()->getLinkage()->isAddressOfPrivateStaticSymRef(
                 addr2->getSymbolReference())) {
    foundMatch = true;
    ...
    if (addr1->getOpCodeValue() == TR::aload &&
        addr2->getOpCodeValue() == TR::aload &&
        addr1->getSymbolReference() == addr2->getSymbolReference()) {
      foundMatch = true;
    } else if (addr1->getOpCodeValue() == TR::aloadi &&
...
bool OMR::CodeGenerator::addressesMatch(TR::Node *addr1, TR::Node *addr2,
  ...
  if (!foundMatch && addressesUnderSameTreeTop) {
    ...
    if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2) &&
      ...
      if (traceInspect())
        traceMsg(comp(), "\t\tfound possibly matching additions : update addr1 "
                         "to %s (%p), addr2 to %s (%p) and continue matching\n",
...
void OMR::CodeGenerator::zeroOutAutoOnEdge(
    TR::SymbolReference *liveAutoSymRef, TR::Block *block, TR::Block *succBlock,
  ...
  if ((succBlock->getPredecessors().size() == 1))
    ...
    for (auto blocksIt = newBlocks->begin(); blocksIt != newBlocks->end();
         ++blocksIt) {
      if ((*blocksIt)->getSuccessors().front()->getTo()->asBlock() ==
          succBlock) {
        storeBlock = *blocksIt;
        break;
      }
  ...
  if (!storeBlock) {
    ...
    TR::Block *newBlock =
        block->splitEdge(block, succBlock, self()->comp(), NULL, false);
    ...
    if (glRegDeps) {
      ...
      newBlock->getEntry()->getNode()->setAndIncChild(0,
                                                      origDuplicateGlRegDeps);
      for (int32_t i = origDuplicateGlRegDeps->getNumChildren() - 1; i >= 0;
           --i) {
        TR::Node *dep = origDuplicateGlRegDeps->getChild(i);
        if (self()->comp()->getOption(TR_MimicInterpreterFrameShape) ||
            self()->comp()->getOption(TR_PoisonDeadSlots))
          dep->setRegister(
              NULL); // basically need to do prepareNodeForInstructionSelection
      ...
      if (self()->comp()->getOption(TR_MimicInterpreterFrameShape) ||
        ...
        if ((newBlock->getSuccessors().size() == 1) &&
          ...
          TR_ASSERT(
              glRegDepsParent->getOpCodeValue() == TR::Goto,
              "Expected block to fall through or end in goto; it ends with %s "
              "%s\n",
    ...
    newBlock->setLiveLocals(new (self()->trHeapMemory())
                                TR_BitVector(*succBlock->getLiveLocals()));
  ...
  if (self()->comp()->getOption(TR_PoisonDeadSlots))
    ...
    storeNode = TR::Node::createStore(
        liveAutoSymRef, TR::Node::aconst(block->getEntry()->getNode(), 0));
...
uint8_t *OMR::CodeGenerator::allocateCodeMemory(uint32_t warmSize,
                                                uint32_t coldSize,
                                                uint8_t **coldCode,
                                                bool isMethodHeaderNeeded) {
  uint8_t *warmCode;
  warmCode = self()->fe()->allocateCodeMemory(
      self()->comp(), warmSize, coldSize, coldCode, isMethodHeaderNeeded);
  if (self()->getCodeGeneratorPhase() ==
      TR::CodeGenPhase::BinaryEncodingPhase) {
    self()->commitToCodeCache();
  }
  TR_ASSERT(!((warmSize && !warmCode) || (coldSize && !coldCode)),
            "Allocation failed but didn't throw an exception");
  return warmCode;
}

uint8_t *OMR::CodeGenerator::allocateCodeMemory(uint32_t size, bool isCold,
                                                bool isMethodHeaderNeeded) {
  uint8_t *coldCode;
  if (isCold) {
...
void OMR::CodeGenerator::resizeCodeMemory() {
  int32_t warmCodeLength =
      self()->getWarmCodeEnd() - self()->getBinaryBufferStart();
  self()->fe()->resizeCodeMemory(self()->comp(), self()->getBinaryBufferStart(),
                                 warmCodeLength);
}
...
bool OMR::CodeGenerator::isMemoryUpdate(TR::Node *node) {
  ...
  // side
  // effect of swapping children of commutative operations in order to expose
  // the
  // opportunity.
  //
...
// array MUST be in numerically increasing order for the binary search to work.
//
// The table is composed of 32-bit values because the compiler seems to have a
// problem
...
uint8_t *OMR::CodeGenerator::alignBinaryBufferCursor() {
  uintptr_t boundary =
      self()->comp()->getOptions()->getJitMethodEntryAlignmentBoundary(self());
  ...
  if (boundary && (boundary & boundary - 1) == 0) {
    ...
    _binaryBufferCursor =
        (uint8_t *)(((uintptr_t)_binaryBufferCursor + round) & ~round);
    _binaryBufferCursor -= offset;
...
uint8_t *OMR::CodeGenerator::emitSnippets(bool isWarm) {
  // *this    swipeable for debugging purposes
  uint8_t *codeOffset;
  uint8_t *retVal;
...
static bool mergeableGuard(TR::Instruction *guard) {
  ...
  return mergeOnlyHCRGuards ? isStopTheWorldGuard(guard->getNode())
                            : guard->getNode()->isNopableInlineGuard();
}

static bool mergeableGuards(TR::Instruction *earlierGuard,
                            TR::Instruction *laterGuard) {
  return mergeableGuard(earlierGuard) && mergeableGuard(laterGuard) &&
         earlierGuard->getNode()->getBranchDestination() ==
             laterGuard->getNode()->getBranchDestination() &&
         (!isStopTheWorldGuard(earlierGuard->getNode()) ||
          isStopTheWorldGuard(laterGuard->getNode()));
}
...
OMR::CodeGenerator::getVirtualGuardForPatching(TR::Instruction *vgdnop) {
  ...
  // order
  // only separated by BBStart and BBEnd trees Skip back to the BBStart since we
  // need
  ...
  if (toReturn != vgdnop) {
    TR::DebugCounter::incStaticDebugCounter(
        ...
        TR::DebugCounter::debugCounterName(self()->comp(), "guardMerge/(%s)",
                                           self()->comp()->signature()));
    traceMsg(self()->comp(), "vgdnop instruction [%p] begins scanning for "
                             "patch instructions for mergeable guard [%p]\n",
             vgdnop, toReturn);
  }
  return toReturn;
}
...
bool OMR::CodeGenerator::requiresAtomicPatching(TR::Instruction *vgdnop) {
...
TR::Instruction *OMR::CodeGenerator::saveOrRestoreRegisters(
    TR_BitVector *regs, TR::Instruction *cursor, bool doSaves) {
    ...
    if (prevIdx != -1) {
      if (curIdx == prevIdx + 1) {
        ...
        if (i > 1) {
          ...
          //
          traceMsg(self()->comp(),
                   "found pattern for start %d end %d at cursor %p\n", startIdx,
                   endIdx, cursor);
          cursor = self()->getLinkage()->composeSavesRestores(
              cursor, startIdx, endIdx, -1 /*_mapRegsToStack[startIdx]*/,
              numRegs, doSaves);
          savedRegs += i;
        } else {
          ...
          //
          traceMsg(self()->comp(),
                   "pattern broken idx %d at cursor %p doSaves %d\n", prevIdx,
                   cursor, doSaves);
          if (doSaves)
            cursor = self()->getLinkage()->savePreservedRegister(cursor,
                                                                 prevIdx, -1);
          else
            cursor = self()->getLinkage()->restorePreservedRegister(
                cursor, prevIdx, -1);
          savedRegs++;
        }
        startIdx = -1;
  ...
  // do the remaining
  if ((numRegs > 1) && (startIdx != -1)) {
    // compose save restores
    cursor = self()->getLinkage()->composeSavesRestores(
        cursor, startIdx, endIdx, -1 /*_mapRegsToStack[startIdx]*/, numRegs,
        doSaves);
  ...
  if (savedRegs != numRegs) {
    ...
    for (; savedRegs; --savedRegs)
      resIt.getNextElement();
    while (resIt.hasMoreElements()) {
      int32_t curIdx = resIt.getNextElement();
      ...
      if (doSaves)
        cursor =
            self()->getLinkage()->savePreservedRegister(cursor, curIdx, -1);
      else
        cursor =
            self()->getLinkage()->restorePreservedRegister(cursor, curIdx, -1);
    }
  }
  return cursor;
}
...
void OMR::CodeGenerator::dumpSpillStats(TR_FrontEnd *fe) {
  if (debug("spillStats")) {
    TR_VerboseLog::writeLine(TR_Vlog_INFO,
                             "Register Spilling/Rematerialization:");
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "%8d registers spilled",
                             _totalNumSpilledRegisters);
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "%8d constants rematerialized",
                             _totalNumRematerializedConstants);
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "%8d locals rematerialized",
                             _totalNumRematerializedLocals);
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "%8d statics rematerialized",
                             _totalNumRematerializedStatics);
    TR_VerboseLog::writeLine(TR_Vlog_INFO,
                             "%8d indirect accesses rematerialized",
                             _totalNumRematerializedIndirects);
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "%8d addresses rematerialized",
                             _totalNumRematerializedAddresses);
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "%8d XMMRs rematerialized",
                             _totalNumRematerializedXMMRs);
  }
}
...
// machine provides
bool OMR::CodeGenerator::isMaterialized(int64_t value) {
  if (self()->materializesLargeConstants()) // check the int case
  {
    ...
    if ((value >= smallestPos) || (value <= largestNeg))
      return true;
  }
  return false;
}
...
void OMR::CodeGenerator::startRecordingRegisterUsage() {
  self()->setReferencedRegisterList(
      new (self()->trHeapMemory()) TR::list<OMR::RegisterUsage *>(
          getTypedAllocator<OMR::RegisterUsage *>(TR::comp()->allocator())));
...
void OMR::CodeGenerator::addRelocation(TR_Relocation *r) {
  if (r->isAOTRelocation()) {
    TR_ASSERT(false, "Cannot use addRelocation to add an AOT relocation. "
                     "Please use addAOTRelocation here");
  } else {
    _relocationList.push_front(r);
  }
}

void OMR::CodeGenerator::addAOTRelocation(TR_Relocation *r,
                                          char *generatingFileName,
                                          uintptr_t generatingLineNumber,
                                          TR::Node *node) {
  TR_ASSERT(generatingFileName,
            "AOT relocation location has improper NULL filename specified");
  if (self()->comp()->compileRelocatableCode()) {
    TR_RelocationDebugInfo *genData =
        new (self()->trHeapMemory()) TR_RelocationDebugInfo;
...
void OMR::CodeGenerator::addAOTRelocation(TR_Relocation *r,
                                          TR_RelocationDebugInfo *info) {
  if (self()->comp()->compileRelocatableCode()) {
    TR_ASSERT(
        info,
        "AOT relocation location does not have associated debug information");
    r->setDebugInfo(info);
    _aotRelocationList.push_back(r);
  }
}
...
intptrj_t OMR::CodeGenerator::hiValue(intptrj_t address) {
  if (self()->comp()->compileRelocatableCode()) // We don't want to store values
                                                // using HI_VALUE at compile
                                                // time, otherwise, we do this a
                                                // 2nd time when we relocate
                                                // (and new value is based on
                                                // old one)
...
TR::RegisterPair *
OMR::CodeGenerator::allocate64bitRegisterPair(TR::Register *lo,
                                              TR::Register *ho) {
  TR::RegisterPair *temp =
      new (self()->trHeapMemory()) TR::RegisterPair(TR_GPR64);
