  ...
  if (castClassSymRef->isUnresolved()) {
    ...
    if (castClassSym &&
        ...
        (!TR::Compiler->cls.isAbstractClass(cg->comp(), clazz)
         ...
         // optimizer
         // but in some cases they can persist to codegen which without the
         // following
         // case causes assertions because we opt out of calling the helper and
         // doing
         // all inline tests. Really we could just jmp to the failed side, but
         // to reduce
         // service risk we are going to do an equality test that we know will
         // fail
         // NOTE final abstract is not enough - all array classes are final
         // abstract
...
bool OMR::TreeEvaluator::instanceOfOrCheckCastNeedSuperTest(
  ...
  if (!isStaticClassSymRef(castClassSymRef)) {
    ...
    // changes to platform code. The benefit is little at this point (shows up
    // from reference arraycopy reductions)
...
TR::Register *OMR::TreeEvaluator::computeCCEvaluator(TR::Node *node,
  ...
  if (child->getRegister()) {
    ...
    // we need to decrease reference counts on children of virtual clone which
    // were raised in
    // tree lowering to allow children's registers to be kept alive to this
    // point.
...
bool OMR::TreeEvaluator::nodeIsIAddOverflowCheck(
  ...
  static TR_Pattern *pattern = pb.choice(
      ...
      pb.children(
          pb(TR::iand,
             pb(TR::ixor, pb(TR::iadd, u, u->operationNode, pb(u, u->leftChild),
                             pb(u, u->rightChild)),
                pb(u, u->leftChild)),
...
bool OMR::TreeEvaluator::nodeIsISubOverflowCheck(
  ...
  static TR_Pattern *pattern = pb.choice(
      ...
      pb.children(pb(TR::iand,
                     pb(TR::ixor, pb(TR::isub, u, u->operationNode,
...
bool OMR::TreeEvaluator::nodeIsLAddOverflowCheck(
  ...
  static TR_Pattern *pattern = pb.choice(
      ...
      pb.children(
          pb(TR::land,
             pb(TR::lxor, pb(TR::ladd, u, u->operationNode, pb(u, u->leftChild),
                             pb(u, u->rightChild)),
                pb(u, u->leftChild)),
...
bool OMR::TreeEvaluator::nodeIsLSubOverflowCheck(
  ...
  static TR_Pattern *pattern = pb.choice(
      ...
      pb.children(pb(TR::land,
                     pb(TR::lxor, pb(TR::lsub, u, u->operationNode,
                                     pb(u, u->leftChild), pb(u, u->rightChild)),
                        pb(u, u->leftChild)),
...
// OutOfLineCodeSection::evaluateNodesWithFutureUses
// however, it seems to something extra for the spinechk nodes. These two
// routines should
// be consolidated
//
  ...
  if (node->getFutureUseCount() > 0) {
    ...
    if (actualLoadOrStoreChild->getOpCode().isStore() ||
      ...
      if (comp->getOption(TR_TraceCG)) {
        traceMsg(comp, "avoiding escaping commoned subtree %p [RealLoad/Store: "
                       "%p], but processing its children: node is ",
...
// Note that this routine only make sense during tree evaluation time. It
// assumes
// that all previous (strictly historical) uses have already been discounted
// from
