...
TR_RelocationDebugInfo *TR_Relocation::getDebugInfo() { return this->_genData; }
void TR_8BitLabelRelativeRelocation::apply(TR::CodeGenerator *codeGen) {
  // *this    swipeable for debugging purposes
  AOTcgDiag2(codeGen->comp(),
             "TR_8BitLabelRelativeRelocation::apply cursor=%x label=%x\n",
...
void TR_12BitLabelRelativeRelocation::apply(TR::CodeGenerator *codeGen) {
  // *this    swipeable for debugging purposes
  AOTcgDiag2(codeGen->comp(),
             "TR_12BitLabelRelativeRelocation::apply cursor=%x label=%x\n",
             getUpdateLocation(), getLabel());
...
void TR_16BitLabelRelativeRelocation::apply(TR::CodeGenerator *codeGen) {
  // *this    swipeable for debugging purposes
  AOTcgDiag2(codeGen->comp(),
             "TR_16BitLabelRelativeRelocation::apply cursor=%x label=%x\n",
             getUpdateLocation(), getLabel());
...
void TR_24BitLabelRelativeRelocation::apply(TR::CodeGenerator *codeGen) {
  // *this    swipeable for debugging purposes
  AOTcgDiag2(codeGen->comp(),
             "TR_24BitLabelRelativeRelocation::apply cursor=%x label=%x\n",
             getUpdateLocation(), getLabel());
...
void TR_32BitLabelRelativeRelocation::apply(TR::CodeGenerator *codeGen) {
  // *this    swipeable for debugging purposes
  AOTcgDiag2(codeGen->comp(),
             "TR_32BitLabelRelativeRelocation::apply cursor=%x label=%x\n",
             getUpdateLocation(), getLabel());
...
void TR_LabelAbsoluteRelocation::apply(TR::CodeGenerator *codeGen) {
  ...
  AOTcgDiag2(codeGen->comp(),
             "TR_LabelAbsoluteRelocation::apply cursor=%x label=%x\n", cursor,
             getLabel());
...
void TR_InstructionAbsoluteRelocation::apply(TR::CodeGenerator *codeGen) {
  ...
  AOTcgDiag2(
      codeGen->comp(),
      "TR_InstructionAbsoluteRelocation::apply cursor=%x instruction=%x\n",
      cursor, address);
  *cursor = address;
}

void TR_16BitLoadLabelRelativeRelocation::apply(TR::CodeGenerator *codeGen) {
  AOTcgDiag3(codeGen->comp(), "TR_16BitLoadLabelRelativeRelocation::apply "
                              "lastInstruction=%x startLabel=%x endLabel=%x\n",
...
void TR_32BitLoadLabelRelativeRelocation::apply(TR::CodeGenerator *codeGen) {
  AOTcgDiag3(codeGen->comp(), "TR_32BitLoadLabelRelativeRelocation::apply "
                              "lastInstruction=%x startLabel=%x endLabel=%x\n",
             getLastInstruction(), getStartLabel(), getEndLabel());
...
void TR_64BitLoadLabelRelativeRelocation::apply(TR::CodeGenerator *codeGen) {
  AOTcgDiag2(codeGen->comp(), "TR_64BitLoadLabelRelativeRelocation::apply "
                              "lastInstruction=%x label=%x\n",
...
uint8_t TR_ExternalRelocation::collectModifier() {
  ...
  if (TR::Compiler->target.cpu.isPower() &&
      (_kind == TR_ArrayCopyHelper || _kind == TR_ArrayCopyToc ||
       _kind == TR_RamMethod || _kind == TR_GlobalValue ||
       _kind == TR_BodyInfoAddressLoad || _kind == TR_DataAddress ||
       _kind == TR_JNISpecialTargetAddress ||
       _kind == TR_JNIStaticTargetAddress ||
       _kind == TR_JNIVirtualTargetAddress ||
       _kind == TR_StaticRamMethodConst || _kind == TR_VirtualRamMethodConst ||
       _kind == TR_SpecialRamMethodConst)) {
...
void TR_ExternalRelocation::addAOTRelocation(TR::CodeGenerator *codeGen) {
  ...
  if (comp->getOption(TR_AOT)) {
    TR_LinkHead<TR_IteratedExternalRelocation> &aot =
    ...
    for (r = aot.getFirst(); r != 0; r = r->getNext()) {
      if (r->getTargetAddress2())
        AOTcgDiag6(comp,
                   "r=%x full=%x target=%x target2=%x, kind=%x modifier=%x\n",
        ...
        AOTcgDiag5(comp, "r=%x full=%x target=%x kind=%x modifier=%x\n", r,
                   r->full(), r->getTargetAddress(), r->getTargetKind(),
                   r->getModifierValue());
      ...
      if (r->full() == false && r->getTargetAddress() == _targetAddress &&
        ...
        if (r->getTargetAddress2())
          AOTcgDiag6(
              comp, "r=%x full=%x target=%x target2=%x kind=%x modifier=%x\n",
              r, r->full(), r->getTargetAddress(), r->getTargetAddress2(),
              r->getTargetKind(), r->getModifierValue());
        else
          AOTcgDiag5(comp, "r=%x full=%x target=%x kind=%x modifier=%x\n", r,
    ...
    TR_IteratedExternalRelocation *temp =
        _targetAddress2
            ? new (codeGen->trHeapMemory()) TR_IteratedExternalRelocation(
    ...
    if (_targetAddress2)
      AOTcgDiag6(comp,
                 "temp=%x full=%x target=%x target2=%x kind=%x modifier=%x\n",
                 temp, temp->full(), temp->getTargetAddress(),
                 temp->getTargetAddress2(), temp->getTargetKind(),
                 temp->getModifierValue());
    else
      AOTcgDiag5(comp, "temp=%x full=%x target=%x kind=%x modifier=%x\n", temp,
                 temp->full(), temp->getTargetAddress(), temp->getTargetKind(),
                 temp->getModifierValue());
...
void TR_ExternalRelocation::apply(TR::CodeGenerator *codeGen) {
  ...
  if (comp->getOption(TR_AOT)) {
    ...
    getRelocationRecord()->addRelocationEntry(
        (uint32_t)(getUpdateLocation() - aotMethodCodeStart));
  }
}
...
TR_32BitExternalOrderedPairRelocation::TR_32BitExternalOrderedPairRelocation(
  ...
  AOTcgDiag0(codeGen->comp(), "TR_32BitExternalOrderedPairRelocation::TR_"
                              "32BitExternalOrderedPairRelocation\n");
...
void TR_32BitExternalOrderedPairRelocation::apply(TR::CodeGenerator *codeGen) {
  ...
  if (comp->getOption(TR_AOT)) {
    ...
    TR_ExternalRelocationTargetKind kind =
        getRelocationRecord()->getTargetKind();
    if (TR::Compiler->target.cpu.isPower() &&
        (kind == TR_ArrayCopyHelper || kind == TR_ArrayCopyToc ||
      ...
      rec->addRelocationEntry(
          (uint32_t)(instr->getBinaryEncoding() - codeStart));
      rec->addRelocationEntry(
...
char *TR_ExternalRelocation::_externalRelocationTargetKindNames
    [TR_NumExternalRelocationKinds] = {
        "TR_ConstantPool (0)", "TR_HelperAddress (1)",
        "TR_RelativeMethodAddress (2)", "TR_AbsoluteMethodAddress (3)",
        "TR_DataAddress (4)", "TR_ClassObject (5)", "TR_MethodObject (6)",
        "TR_InterfaceObject (7)", "TR_AbsoluteHelperAddress (8)",
        "TR_FixedSequenceAddress (9)", "TR_FixedSequenceAddress2 (10)",
        "TR_JNIVirtualTargetAddress (11)", "TR_JNIStaticTargetAddress  (12)",
        "TR_ArrayCopyHelper (13)", "TR_ArrayCopyToc (14)",
        "TR_BodyInfoAddress (15)", "TR_Thunks (16)",
        "TR_StaticRamMethodConst (17)", "TR_Trampolines (18)",
        "TR_PicTrampolines (19)", "TR_CheckMethodEnter(20)",
        "TR_RamMethod (21)", "TR_RamMethodSequence (22)",
        "TR_RamMethodSequenceReg (23)", "TR_VerifyClassObjectForAlloc (24)",
        "TR_ConstantPoolOrderedPair (25)",
        "TR_AbsoluteMethodAddressOrderedPair (36)",
        "TR_VerifyRefArrayForAlloc (27)", "TR_J2IThunks (28)",
        "TR_GlobalValue (29)", "TR_BodyInfoAddressLoad (30)",
        ...
        "TR_SpecialRamMethodConst (36)", "TR_InlinedHCRMethod (37)",
        "TR_ValidateStaticField (38)", "TR_ValidateClass (39)",
        "TR_ClassAddress (40)", "TR_HCR (41)",
        "TR_ProfiledMethodGuardRelocation (42)",
        "TR_ProfiledClassGuardRelocation (43)",
        "TR_HierarchyGuardRelocation (44)", "TR_AbstractGuardRelocation (45)",
        "TR_ProfiledInlinedMethodRelocation (46)", "TR_MethodPointer (47)",
        "TR_ClassPointer (48)", "TR_CheckMethodExit (49)",
        "TR_ValidateArbitraryClass (50)", "TR_EmitClass (51)",
        "TR_JNISpecialTargetAddress (52)", "TR_VirtualRamMethodConst (53)"
...
TR_IteratedExternalRelocation::TR_IteratedExternalRelocation(
    uint8_t *target, TR_ExternalRelocationTargetKind k, flags8_t modifier,
    TR::CodeGenerator *codeGen)
    : TR_Link<TR_IteratedExternalRelocation>(), _numberOfRelocationSites(0),
...
void TR_IteratedExternalRelocation::initialiseRelocation(
    TR::CodeGenerator *codeGen) {
  // *this    swipeable for debugging purposes
  AOTcgDiag0(TR::comp(),
             "TR_IteratedExternalRelocation::initialiseRelocation\n");
  _relocationDataCursor =
      codeGen->getAheadOfTimeCompile()->initialiseAOTRelocationHeader(this);
}

void TR_IteratedExternalRelocation::addRelocationEntry(
    uint32_t locationOffset) {
  // *this    swipeable for debugging purposes
  TR::Compilation *comp = TR::comp();
  AOTcgDiag1(comp, "TR_IteratedExternalRelocation::addRelocationEntry "
                   "_relocationDataCursor=%x\n",
             _relocationDataCursor);
  if (!needsWideOffsets()) {
