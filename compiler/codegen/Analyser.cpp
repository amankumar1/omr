...
void TR_Analyser::setInputs(TR::Node *firstChild, TR::Register *firstRegister,
  ...
  if (firstChild->getOpCode().isMemoryReference() &&
      ...
      !firstChild->getSymbol()->isRegisterSymbol() && !lockedIntoRegister1) {
    setMem1();
  }
  ...
  if (secondChild->getOpCode().isMemoryReference() &&
      ...
      !secondChild->getSymbol()->isRegisterSymbol() && !lockedIntoRegister2) {
    setMem2();
  }
  ...
  if (!dontClobberAnything) {
    ...
    if (!nonClobberingDestination) {
      ...
      // doesn't
      // actually clobber destination operand so want to make either one a
      // possible
