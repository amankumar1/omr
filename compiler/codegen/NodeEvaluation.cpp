...
TR::Register *OMR::CodeGenerator::evaluate(TR::Node *node) {
  ...
  if (opcode != TR::BBStart && node->getRegister()) {
    ...
    //    not been
    //    dereferenced yet, and the base (and optionally index) nodes may have
    //    registers
    //    assigned to them.
    //
    ...
    // evaluate
    // on node_C and then call populateMemoryReference on node_B. This fixes
    // this scenario, but
    // the capability of another tree and evaluation pattern to create this bug
    // still exists.
    //
    ...
    // evaluator, then
    // the second sub-evaluate call could be destructive to the first
    // populateMemoryReference.
    //
    ...
    // non-restricted nodes that
    // have a reference count > 1. If any of those node's reference counts reach
    // 1, then artificially
    // inflate those reference counts by 1 for the lifetime of the parent
    // evaluation.
    //
    ...
    // escape their evaluator.
    // Explicitly note memory references that are not loaded into registers and
    // automatically call
  ...
  if (reg && reg->getOpaquePseudoRegister()) {
    ...
    if (node->getReferenceCount() == 1) {
      ...
      if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(), "\tdecrement owningRegisterCount %d->%d on "
                                 "ref #%d (%s) for reg %s as %s (%p) refCount "
                                 "== 1 (going to 0)\n",
    ...
    TR_ASSERT(false, "tempMemSlots should only be attached to pseudoRegisters "
                     "and not node %p\n",
              node);
  }
#endif
