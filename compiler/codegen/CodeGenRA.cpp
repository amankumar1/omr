  ...
  if (liveRegisters && liveRegisters->getFirstLiveRegister()) {
    ...
    for (TR_LiveRegisterInfo *p = liveRegisters->getFirstLiveRegister(); p;
      ...
      if (p->getNode() && p->getNode()->hasOptAttributes())
        traceMsg(TR::comp(), "Virtual register %s (for node [%s], ref "
                             "count=%d) is live at end of method\n",
                 debug->getName(p->getRegister()), debug->getName(p->getNode()),
                 p->getNode()->getReferenceCount());
      else if (p->getNode())
        traceMsg(
            TR::comp(),
            "Virtual register %s (for node [%s]) is live at end of method\n",
            debug->getName(p->getRegister()), debug->getName(p->getNode()));
      else
        traceMsg(TR::comp(), "Virtual register %s %p is live at end of method "
                             "with node count %d\n",
...
int32_t OMR::CodeGenerator::estimateRegisterPressure(
  ...
  while (treeTop) {
    ...
    self()->estimateRegisterPressure(
        ...
        self()->comp()->cg()->getMaximumNumbersOfAssignableGPRs(),
        valuesInGlobalRegs, isCold, visitCount, symRef, symRefIsLive,
...
TR_BackingStore *OMR::CodeGenerator::allocateVMThreadSpill() {
  // *this    swipeable for debugging purposes
  int32_t slot;
  TR::AutomaticSymbol *spillSymbol = TR::AutomaticSymbol::create(
      self()->trHeapMemory(), self()->IntJ(),
      self()->comp()->getOption(TR_ForceLargeRAMoves)
          ? 8
          : TR::Compiler->om.sizeofReferenceAddress());
  spillSymbol->setSpillTempAuto();
  self()->comp()->getMethodSymbol()->addAutomatic(spillSymbol);
  TR_BackingStore *spill = new (self()->trHeapMemory())
...
OMR::CodeGenerator::allocateSpill(int32_t dataSize,
  ...
  TR_ASSERT(!containsCollectedReference ||
  ...
  if (spill) {
    ...
    //
    int spillSize;
    const int MIN_SPILL_SIZE = (self()->comp()->getOption(TR_ForceLargeRAMoves))
                                   ? 8
                                   : TR::Compiler->om.sizeofReferenceAddress();
    int32_t slot;
...
void OMR::CodeGenerator::freeSpill(TR_BackingStore *spill, int32_t dataSize,
  ...
  if (spill->getSymbolReference()
    ...
    if (spill->isEmpty()) {
      ...
      if (self()->getTraceRAOption(TR_TraceRASpillTemps))
        traceMsg(self()->comp(), "\n -> first half is still occupied; "
                                 "conservatively keeping out of "
                                 "spill4FreeList");
    } else {
...
TR::Register *OMR::CodeGenerator::allocateRegister(TR_RegisterKinds rk) {
  ...
  if (rk == TR_GPR &&
      (!self()->getDebug() || !self()->getTraceRAOption(TR_TraceRASpillTemps) ||
       performTransformation(
           self()->comp(), "O^O SPILL TEMPS: Set UpperHalfIsDead on %s\n",
           self()->getDebug()->getName(temp)))) // allocateRegister is called
                                                // for the vmthread during
                                                // initialization when getDebug
...
static bool blockIsIgnorablyCold(TR::Block *block, TR::CodeGenerator *cg) {
  ...
  return !cannotSpillInBlock &&
         (block->isCold() || blockIsMuchColderThanContainingLoop(block, cg));
}
...
void OMR::CodeGenerator::TR_RegisterPressureState::updateRegisterPressure(
  ...
  _gprPressure +=
      TR::comp()->cg()->gprCount(TR::DataType(dt), symbol->getSize());
...
TR_GlobalRegisterNumber OMR::CodeGenerator::pickRegister(
  ...
  bool enableHighWordGRA = self()->comp()->cg()->supportsHighWordFacility() &&
  ...
  if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
    ...
    if (!self()->comp()->cg()->getSupportsVMThreadGRA() ||
    ...
    while (blockIterator.hasMoreElements()) {
      ...
      // do not assign HPR to candidate if we skipped any block simulation for
      // correctness
      ...
      if (!block->isExtensionOfPreviousBlock() &&
        ...
        // do
        // any for now.  In reality, we'll need to consider exactly when we want
        // to consider
        // the cache unusable.  Presumably we could attempt a cacheable
        // simulation of each
        // block, and if the block is "uncacheable" we could just not use the
        // cache for that one.
        //
        ...
        if (cacheIsInconclusive) {
          if (canAffordFullSimulation) {
            ...
            if (enableHighWordGRA) {
              ...
              if (dtype == TR::Int8 || dtype == TR::Int16 ||
                  dtype == TR::Int32) {
    ...
    while (blockIterator.hasMoreElements()) {
      ...
      if (block != self()->comp()->getFlowGraph()->getEnd() &&
        ...
        // copy
        //
        int32_t blockWeight = 1; // Note that we use a static loop-nesting-depth
                                 // heuristic here for consistency with the rest
                                 // of GRA
        ...
        if (blockWeight > hottestCopiedRegisterWeight) {
          ...
          for (TR::TreeTop *tt = block->getEntry(); tt != stopTree;
            ...
            if (candidate) {
              if (!isUnpreferred) {
                ...
                if (remainingRegisters.isSet(preferredRegister)) {
                  ...
                  if (self()->traceSimulateTreeEvaluation()) {
                    traceMsg(
                        self()->comp(), "            found copy to/from #%d, "
              ...
              } else // is Unpreferred
              {
                if (self()->traceSimulateTreeEvaluation()) {
                  traceMsg(
                      self()->comp(), "            unprefer copy to/from #%d, "
                                      "register %d (%s), weight %d, at %s\n",
                      candidate->getSymbolReference()->getReferenceNumber(),
            ...
            if (callNode && (self()->getLinkage()->isRecognizedBuiltin(
                                 self()->comp(), callNode) ||
                ...
                if (candidateRegister != -1) {
                  if (!isUnpreferred) {
                    if (remainingRegisters.isSet(candidateRegister)) {
                      ...
                      hottestCopiedRegisterWeight = blockWeight;
                      if (self()->traceSimulateTreeEvaluation()) {
                        traceMsg(self()->comp(), "            found call "
                                                 "parameter index %i, register "
                                                 "%d (%s), weight %d, at %s\n",
                  ...
                  {
                    if (self()->traceSimulateTreeEvaluation()) {
                      traceMsg(self()->comp(), "            unprefer call "
                                               "parameter index %i, register "
                                               "%d (%s), weight %d, at %s\n",
                               i, candidateRegister,
                               self()->getDebug()->getGlobalRegisterName(
            ...
            if (!candidate) {
              ...
              if (candidate) {
                if (self()->traceSimulateTreeEvaluation()) {
                  traceMsg(
                      self()->comp(), "            unprefer used candidate "
                                      "#%d, register %d (%s), weight %d, at "
                                      "%s\n",
                      candidate->getSymbolReference()->getReferenceNumber(),
                      candidate->getGlobalRegisterNumber(),
                      self()->getDebug()->getGlobalRegisterName(
    ...
    if (enableHighWordGRA) {
      TR_BitVector HPRMasks = *self()->getGlobalRegisters(
          TR_hprSpill,
          self()->comp()->getMethodSymbol()->getLinkageConvention());
      // We cannot assign an HPR if the corresponding GPR is alive.
      // There is no safe way to anticipate whether instruction selection will
      // clobber the High word
      // e.g. generating 64-bit instructions for long arithmetics on 32-bit
      // platform
      // todo: maybe try checking candidate data type while assigning GPR? only
      // clobber HPR candidate
      // if the GPR candidate is long

      if (remainingRegisters.intersects(HPRMasks)) {
        for (int i = self()->getFirstGlobalGPR();
             i < self()->getFirstGlobalHPR(); ++i) {
          if (!remainingRegisters.isSet(i)) {
            TR_GlobalRegisterNumber highWordReg =
                self()->getGlobalHPRFromGPR(i);
            if (self()->traceSimulateTreeEvaluation()) {
              traceMsg(self()->comp(),
                       "            %s is not available, rejecting %s\n",
                       self()->getDebug()->getGlobalRegisterName(i),
                       self()->getDebug()->getGlobalRegisterName(highWordReg));
            }
      ...
      if (remainingRegisters.intersects(HPRMasks)) {
        ...
        if (hprSimulated && !hprColdBlockSkipped) {
          if (self()->traceSimulateTreeEvaluation()) {
            traceMsg(self()->comp(), "            HPR candidates are "
                                     "available, rejecting all GPR\n");
    ...
    if (remainingRegisters.isEmpty()) {
      if (self()->terseSimulateTreeEvaluation())
        traceMsg(self()->comp(), "         } No good registers for candidate "
                                 "#%d after reg pressure simulation\n",
                 rc->getSymbolReference()->getReferenceNumber());
      else
        dumpOptDetails(self()->comp(), "No good registers for candidate #%d "
                                       "after reg pressure simulation\n",
                       rc->getSymbolReference()->getReferenceNumber());
...
TR_GlobalRegisterNumber OMR::CodeGenerator::findCoalescenceRegisterForParameter(
  ...
  if (paramNode->getOpCode().isLoadVarDirect()) {
    ...
    *isUnpreferred =
    ...
    TR_GlobalRegisterNumber candidateRegister =
        self()->getLinkage()->getInRegisterNumberForParameter(callNode,
                                                              childIndex);
...
OMR::CodeGenerator::findUsedCandidate(TR::Node *node, TR_RegisterCandidate *rc,
  ...
  if (node->getOpCode().isLoadVarDirect() || node->getOpCode().isStoreDirect())
    gprToCoalesce = TR::comp()->getGlobalRegisterCandidates()->find(
        node->getSymbolReference());
...
#define LINKAGE_THAT_IS_ALWAYS_PRESENT                                         \
  (TR::comp()->getMethodSymbol()->getLinkageConvention())
...
void OMR::CodeGenerator::computeSimulatedSpilledRegs(
  ...
  if (symRefNum == _blockAndCandidateRegisterPressureCacheTags[blockNum]) {
    if (traceSimulateTreeEvaluation())
      traceMsg(comp(), "Using candidate-specific regsiter pressure simulation "
                       "cache for block_%d, sym #%d\n",
               blockNum, symRefNum);
...
bool OMR::CodeGenerator::isCandidateLoad(TR::Node *node,
  ...
  return state->_candidate
             ? self()->isCandidateLoad(node,
                                       state->_candidate->getSymbolReference())
             : false;
}
...
inline void standardNodeSimulationAnnotations(
    ...
    TR::Compilation *comp) {
  if (state->_candidate)
    traceMsg(comp, " %c%c", state->_pressureRiskFromStart
                                ? '+'
...
rememberMostRecentValue(TR::SymbolReference *symRef, TR::Node *valueNode,
  ...
  if (state->_alreadyAssignedOnExit.isSet(symRef->getReferenceNumber()) ||
    ...
    TR_ASSERT(candidate, "rememberMostRecentValue: there should be a matching "
                         "candidate for #%d %s",
...
void OMR::CodeGenerator::simulateTreeEvaluation(
  ...
  if (nodeState.hasRegister()) {
    ...
    if (isCall) {
      isCall = self()->comp()->cg()->willBeEvaluatedAsCallByCodeGen(
          node, self()->comp());
    }
      ...
      if (isCall) {
        ...
        if ((numGPRTemporaries >= 1 || numFPRTemporaries >= 1 ||
            ...
            (self()->comp()->getOption(TR_TraceRegisterPressureDetails) &&
             !performTransformation(self()->comp(), "O^O GLOBAL REGISTER "
                                                    "ALLOCATION: Call %p has "
                                                    "%d GPR and %d FPR and VRF "
                                                    "% dummy registers\n",
      ...
      if (isCall) {
        ...
        if (killedRegisters) {
          ...
          while (bvi.hasMoreElements()) {
            int32_t realRegNum =
                bvi.getNextElement(); // GPR0..GPR15, realRegNum 0..15
        ...
        if (!rcSymbol->isParm() ||
            !linkage->isSpecialNonVolatileArgumentRegister(
                rcSymbol->getParmSymbol()->getLinkageRegisterIndex()))
...
void OMR::CodeGenerator::simulateNodeEvaluation(
  ...
  // pressure,
  // so those children can be decremented immediately.  childrenInRegisters
  // indicates which
  // children have NOT been decremented.
  //
  flags32_t childrenInRegisters(0); // (Don't bother with a bitvector on the
                                    // slight chance that some child beyond the
  ...
  //   traceMsg(TR::comp(), "state->_gprPressure = %d summary->_gprPressure = %d
...
static int64_t powerOfTwoScaleFactor(TR::Node *node) // INT_MAX means the node
                                                     // doesn't represent a
                                                     // fixed-point mul by a
                                                     // power of 2
{
  TR::ILOpCode &op = node->getOpCode();
...
void OMR::CodeGenerator::getNumberOfTemporaryRegistersUsedByCall(
...

  {
    ...
    bv = *self()->getGlobalRegisters(
        ...
        self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
    bv |= *self()->getGlobalRegisters(
        TR_vmThreadSpill, self()
                              ->comp()
                              ->getJittedMethodSymbol()
                              ->getLinkageConvention()); // vmthread, when
                                                         // present, is also a
                                                         // virtual reg not
  ...
  if (node->decFutureUseCount() == 0) {
    ...
    if (nodeState._childRefcountsHaveBeenDecremented) {
                                    ...
                                    // state)._willBeRematerialized)
      {
        ...
        // cause it
        // to be counted both as a "liveCandidateLoad" as well as pressure risk,
        // which
        // makes the logs look confusing because you get both a "|" and a "+",
        // making it
        // look like there are two values of the candidate live at the same
        // time.  This
        // should be counted ONLY as pressure risk, not as a live candidate
        // load, because
        // the latter should include only virtual regs that would be live even
        // in the
        // absence of GRA.  I'm thinking I should be calling
        // simulateNodeGoingDead here,
        ...
        if (state->_candidate &&
          ...
          traceMsg(self()->comp(), " keeping:%s",
                   self()->getDebug()->getName(node)); // Some node other than
                                                       // the candidate's most
                                                       // recent value is being
                                                       // kept alive
      } else {
        if (nodeState._isCausingPressureRiskUntilEnd) {
          state->_pressureRiskUntilEnd--;
