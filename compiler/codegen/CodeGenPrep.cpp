...
void OMR::CodeGenerator::lowerTreesPreChildrenVisit(TR::Node *parent,
  ...
  if (!disableILMulPwr2Opt && ((parent->getOpCodeValue() == TR::imul) ||
      ...
      if (!storeLocal->holdsMonitoredObject() &&
        ...
        if ((storeIGNode != NULL) && (loadIGNode != NULL)) {
          ...
          if (colourLoad == colourStore && colourLoad != UNCOLOURED &&
              performTransformation(
                  self()->comp(), "%sCoalescing locals by removing store tree "
                                  "%p, load = %d, store = %d \n",
 ...
 * is a [i]bstore).  If the
 * operations between the store and the convert cannot trigger an overflow or
 * underflow of the byte value
 * ie consist only of bitwise and, or or negate, then we can consider the upper
 * bits to be garbage as they
 * will be thrownaway as part of the store.  Thus, we can mark the convert an
 * unneeded.  Codegen can then
...
void OMR::CodeGenerator::cleanupFlags(TR::Node *node) {
  ...
  if (node->getOpCodeValue() == TR::computeCC) {
    ...
    // children
    // as though their parent had been uncommoned. This has the effect of
    // keeping their registers alive
  ...
  //
  if (node->isDualHigh()) {
    TR_ASSERT(!node->getChild(2)->isAdjunct(), "Code Limitation: Cannot cope "
                                               "with common adjunct between "
                                               "dual highs. node = %p.",
              node);
...
void OMR::CodeGenerator::addCountersToEdges(TR::Block *b) {
  ...
  if (lastNode->getOpCode().isBranch() && !found) {
    ...
    if (c && self()->comp()->getOptions()->dynamicDebugCounterIsEnabled(c) &&
      ...
      traceMsg(self()->comp(), "\nSplitting edge, create new intermediate "
                               "block_%d to add edge counters",
               newBlock->getNumber());
      if (glRegDeps)
    ...
    if (node->getOpCode().isNew()) {
      ...
      if (node->getOpCodeValue() == TR::New ||
        ...
        if (classChild->getOpCodeValue() != TR::loadaddr) {
          TR::DebugCounter::prependDebugCounter(
              self()->comp(), TR::DebugCounter::debugCounterName(
                                  self()->comp(), "allocations/%s/child-%s",
                                  opName, classChild->getOpCode().getName()),
              tt);
        } else if (classChild->getSymbolReference()->isUnresolved()) {
          TR::DebugCounter::prependDebugCounter(
              self()->comp(), TR::DebugCounter::debugCounterName(
                                  self()->comp(), "allocations/%s/unresolved",
                                  opName, classChild->getOpCode().getName()),
              tt);
        } else {
          int32_t length;
          char *className = TR::Compiler->cls.classNameChars(
              self()->comp(), (TR_OpaqueClassBlock *)classChild->getSymbol()
                                  ->castToStaticSymbol()
                                  ->getStaticAddress(),
              length);
          TR::DebugCounter::prependDebugCounter(
              self()->comp(), TR::DebugCounter::debugCounterName(
                                  self()->comp(), "allocations/%s/(%.*s)",
