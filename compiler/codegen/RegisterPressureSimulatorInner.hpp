...
struct TR_RegisterPressureState {
  ...
  uint32_t _numLiveCandidateLoads; // Number of isCandidateLoad nodes that have
                                   // been evaluated, but whose refcount is not
                                   // yet zero
  ...
  TR_RegisterPressureState(
      ...
      : _currentBlock(NULL), _currentTreeTop(NULL), _candidate(candidate),
        ...
        _rematNestDepth(0), _currentMemref(0), _candidateIsHPREligible(0) {}
  TR_RegisterPressureState(const TR_RegisterPressureState &other,
      ...
      : _currentBlock(other._currentBlock),
        ...
        _currentMemref(other._currentMemref), _candidateIsHPREligible(0) {}
...
private:
  void accumulate(uint32_t gprPressure, uint32_t fprPressure = 0,
    ...
    /*
       ...
       "gprPressure is getting too high!");
             TR_ASSERT(_fprPressure <= INT_MAX/2 && fprPressure <= INT_MAX/2,
       "fprPressure is getting too high!");
