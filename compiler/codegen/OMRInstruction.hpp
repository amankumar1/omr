...
public:
   ...
   */
  bool isLastWarmInstruction() {
    return (_index & TO_MASK(LastWarmInstruction)) != 0;
  }
  void setLastWarmInstruction(bool v) {
    v ? _index |= TO_MASK(LastWarmInstruction) : _index &=
                                                 ~TO_MASK(LastWarmInstruction);
  }
...
#include "codegen/InstructionKindEnum.hpp"
  ...
  void setNeedsAOTRelocation(bool v = true) {
    v ? _index |= TO_MASK(NeedsAOTRelocation) : _index &=
                                                ~TO_MASK(NeedsAOTRelocation);
  }
  ...
  int32_t setRegisterSaveDescription(int32_t v) {
    return (_registerSaveDescription = v);
  }
