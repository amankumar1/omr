  ...
  for (auto location = self()->getCollectedSpillList().begin();
    ...
    if ((*location)->containsCollectedReference() &&
      ...
      if ((TR::Compiler->target.cpu.isPower() ||
        ...
        if (self()->getDebug())
          traceMsg(
              comp, "\nSkipping GC map [%p] index %d (%s) for instruction [%p] "
                    "in OOL hot path because it has already been reverse "
                    "spilled.\n",
              map, s->getGCMapIndex(),
...
void OMR::CodeGenerator::remapGCIndicesInInternalPtrFormat() {
  ...
  // the
  // internal pointer itself as scalar temps; so GC ignores them in its normal
  // course.
  //
  TR::Compilation *comp = self()->comp();
