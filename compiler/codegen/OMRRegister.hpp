...
public:
  ...
  TR::Register *setColouredRegister(TR::Register *r) {
    return (_colouredRegister = r);
  }
...
protected:
  ...
  { PlaceholderReg = 0x0001,
    ...
    UpperHalfIsDead = 0x0400, // GPRS ONLY -- no need to save/restore the
                              // high-order bits when spilled; aliased with
                              // IsSinglePrecision
  };
...
private:
  ...
  TR::Register *
      _colouredRegister; // register to which this register has been coloured to
  ...
  // register so that they tend
  // to end up being in the right register for instructions that care (like div
  // or call)
  ...
  // Holds the condition codes produced by the operation that produced the
  // container register.
  TR::Register *_cc;
};
}
