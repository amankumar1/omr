...
public:
  TR_SetMonitorStateOnBlockEntry(TR::Compilation *c,
                                 CS2::TableOf<TR_Stack<TR::SymbolReference *> *,
                                              TR::Allocator> *liveMonitorStacks)
...
private:
  int32_t addSuccessors(TR::CFGNode *cfgNode, TR_Stack<TR::SymbolReference *> *,
                        bool traceIt, bool dontPropagateMonitor = false,
  ...
  CS2::TableOf<TR_Stack<TR::SymbolReference *> *, TR::Allocator>
      *_liveMonitorStacks;
};
...
protected:
  ...
  TR_HashTabInt
      _uncommmonedNodes; // uncommoned nodes keyed by the original nodes
...
public:
  ...
  // called to assign/map TOC slots for constant items. If any platform chooses
  // to use TOC mechanism,
  // its codeGen should override this method. Also, any maintenance/details are
  ...
  int32_t whichChildToEvaluate(TR::Node *node); // Decide which child of the
                                                // given node should be
                                                // evaluated first.
  ...
  void incRefCountForOpaquePseudoRegister(TR::Node *node, TR::CodeGenerator *cg,
                                          TR::Compilation *comp) {}
  ...
  bool supportsMarshallingUnmarshallingIntrinsics() {
    return true;
  }                                                   // no virt, default
  ...
  bool isInternalControlFlowReg(TR::Register *reg) {
    return false;
  } // no virt, default
  void startInternalControlFlow(TR::Instruction *instr) {
  } // no virt, default, cast
  ...
  // NOT USED?
  bool supportsDebugCounters(TR::DebugCounterInjectionPoint injectionPoint) {
    return injectionPoint == TR::TR_BeforeCodegen;
  } // no virt, default

  void incrementEventCounter(TR::Node *node, TR::SymbolReference *symRef,
                             TR::CodeGenerator *cg) {
  ...
  TR::Linkage *
  createLinkage(TR_LinkageConventions lc); // no virt, default, cast
  ...
  // The number of nodes we want between a monexit and the next monent before
  // transforming a monitored region with
  // transactional lock elision.
  int32_t getMinimumNumberOfNodesBetweenMonitorsForTLE() {
    return 15;
  } // no virt, default
  ...
  // that codegen sequences are used
  ...
  // high precision timer can be converted into millisecond timer or nanosecond
  // timer through appropriate
  // division or multiplication (respectively). For example, 390 high precision
  // timer provides accuracy to
  // 1/8 of a microsecond so converters are 8000 and 125 respectively
  // (multiplying the two should give you 1000000)
  ...
  // Shrink wrapping
  //
  TR_BitVector *getPreservedRegsInPrologue() {
    return _preservedRegsInPrologue;
  }
  TR_BitVector *setPreservedRegsInPrologue(TR_BitVector *v) {
    return (_preservedRegsInPrologue = v);
  }

  int32_t getLowestSavedRegister() { return _lowestSavedReg; }
  void setLowestSavedRegister(int32_t v) { _lowestSavedReg = v; }

  bool processInstruction(TR::Instruction *instr,
  ...
  TR::Instruction *splitEdge(TR::Instruction *cursor, bool isFallThrough,
  ...
  int32_t computeRegisterSaveDescription(TR_BitVector *regs,
                                         bool populateInfo = false) {
    return 0;
  } // no virt
  void processIncomingParameterUsage(TR_BitVector **registerUsageInfo,
                                     int32_t blockNum) {
    return;
  } // no virt
  ...
  bool isTargetSnippetOrOutOfLine(TR::Instruction *instr,
  ...
  uint32_t setEstimatedWarmLength(uint32_t l) {
    return (_estimatedWarmLength = l);
  }
  uint32_t getEstimatedColdLength() { return _estimatedColdLength; }
  uint32_t setEstimatedColdLength(uint32_t l) {
    return (_estimatedColdLength = l);
  }
  uint32_t getEstimatedMethodLength() {
    return _estimatedWarmLength + _estimatedColdLength;
  }
  ...
  uint8_t *getWarmCodeEnd() {
    return _coldCodeStart ? _warmCodeEnd : _binaryBufferCursor;
  }
  ...
  uint8_t *allocateCodeMemory(uint32_t size, bool isCold,
                              bool isMethodHeaderNeeded = true);
  uint8_t *allocateCodeMemory(uint32_t warmSize, uint32_t coldSize,
                              uint8_t **coldCode,
                              bool isMethodHeaderNeeded = true);
  ...
  TR_BitVector *setSignExtensionFlags(TR_BitVector *flag) {
    return _signExtensionFlags = flag;
  }
  ...
  TR_GlobalRegisterNumber setFirstGlobalHPR(TR_GlobalRegisterNumber n) {
    return (_firstGlobalHPR = n);
  }
  TR_GlobalRegisterNumber getLastGlobalHPR() { return _lastGlobalHPR; }
  TR_GlobalRegisterNumber setLastGlobalHPR(TR_GlobalRegisterNumber n) {
    return (_lastGlobalHPR = n);
  }
  ...
  TR_GlobalRegisterNumber setFirstGlobalAR(TR_GlobalRegisterNumber n) {
    return (_firstGlobalAR = n);
  }
  TR_GlobalRegisterNumber getLastGlobalAR() { return _lastGlobalAR; }
  TR_GlobalRegisterNumber setLastGlobalAR(TR_GlobalRegisterNumber n) {
    return (_lastGlobalAR = n);
  }
  ...
  bool isGlobalHPR(TR_GlobalRegisterNumber n) {
    return (n >= _firstGlobalHPR && n <= _lastGlobalHPR);
  }
  ...
  void setOverlapOffsetBetweenAliasedGRNs(TR_GlobalRegisterNumber n) {
    TR_ASSERT(n >= 0, "Offset for aliased global register numbers must be "
                      "positive. Currently: %d",
              n);
  ...
  pickRegister(TR_RegisterCandidate *, TR::Block **,
               ...
               TR_LinkHead<TR_RegisterCandidate> *candidates); // no virt
  ...
  static void incNumRematerializedConstants() {
    _totalNumRematerializedConstants++;
  }
  static void incNumRematerializedLocals() { _totalNumRematerializedLocals++; }
  static void incNumRematerializedStatics() {
    _totalNumRematerializedStatics++;
  }
  static void incNumRematerializedIndirects() {
    _totalNumRematerializedIndirects++;
  }
  static void incNumRematerializedAddresses() {
  ...
  TR::RegisterPair *allocate64bitRegisterPair(TR::Register *lo = 0,
                                              TR::Register *ho = 0);
  ...
  TR::list<TR_Relocation *> &getAOTRelocationList() {
    return _aotRelocationList;
  }

  void addRelocation(TR_Relocation *r);
  void addAOTRelocation(TR_Relocation *r, char *generatingFileName,
  ...
  void apply32BitLabelRelativeRelocation(int32_t *cursor,
                                         TR::LabelSymbol *); // no virt
  void apply32BitLabelTableRelocation(int32_t *cursor,
                                      TR::LabelSymbol *); // no virt
  ...
  // platform
  // specific code generators should override isSnippetMatched if they choose to
  // use it.
  ...
  // called to emit any target address snippets.  The platform specific code
  // generators
  ...
  int32_t setEstimatedLocationsForTargetAddressSnippetLabels(
      int32_t estimatedSnippetStart, bool isWarm = 0) {
    return 0;
  } // no virt, cast
...
#include "codegen/RegisterPressureSimulatorInner.hpp"
  ...
  TR::Instruction *getInstructionAtEndInternalControlFlow() {
    return _instructionAtEndInternalControlFlow;
  }
  ...
  TR::Instruction *
  generateNop(TR::Node *node, TR::Instruction *instruction = 0,
              TR_NOPKind nopKind = TR_NOPStandard); // no virt, cast
  ...
  // otherwise 0
  // For example, if a page is 4096 bytes and array translation required page
  // alignment, 4095 would
  // be returned.
  // arrayTranslateMinimumNumberOfElements returns the minimum number of
  // elements that need
  // to be processed for it to be worth it to execute the 'translate' built-in
  // function
  ...
  // undefined behaviour.
  // We appear to have gotten relatively benign wrapping behaviour, but the
  // wrapped result is
  // -freq2 when freq2 is even, and INT_MAX - freq2 + 1 when freq2 is odd, which
  // means that we
  ...
  // tests, we can
  // make sure here that the default may be safely multiplied by a block
  // frequency. The new
  ...
  // transformations
  // regardless of the expected number of iterations, because the transformed IL
  // provides
  // strictly more information to codegen, which can then generate the best
  // sequence at its
  // discretion, fabricating a loop if necessary. But at least in the case of
  // idiom
  ...
  bool doInlineAllocate(TR::Node *node) {
    TR_ASSERT(0, "unexpected call to OMR::CodeGenerator::doInlineAllocate");
    return false;
  } // no virt, default
  ...
  bool alwaysUseTrampolines() {
    return _enabledFlags.testAny(AlwaysUseTrampolines);
  }
  ...
  void zeroOutAutoOnEdge(TR::SymbolReference *liveAutoSym, TR::Block *block,
                         TR::Block *succBlock, TR::list<TR::Block *> *newBlocks,
  ...
  TR::DataType IntJ() {
    return TR::Compiler->target.is64Bit() ? TR::Int64 : TR::Int32;
  }
  ...
  TR::Instruction *saveOrRestoreRegisters(TR_BitVector *regs,
  ...
  bool supportsIntDFPConversions() {
    return _enabledFlags.testAny(SupportsIntDFPConversions);
  }
  void setSupportsIntDFPConversions() {
    _enabledFlags.set(SupportsIntDFPConversions);
  }
  ...
  bool getSupportsArraySetToZero() {
    return _flags3.testAny(SupportsArraySetToZero);
  }
  ...
  void setSupportsBigDecimalLongLookasideVersioning() {
    _flags3.set(SupportsBigDecimalLongLookasideVersioning);
  }
  ...
  bool getSupportsTMDoubleWordCASORSet() {
    return _flags3.testAny(SupportsTMDoubleWordCASORSet);
  }
  void setSupportsTMDoubleWordCASORSet() {
    _flags3.set(SupportsTMDoubleWordCASORSet);
  }

  bool getSupportsTMHashMapAndLinkedQueue() {
    return _flags4.testAny(SupportsTMHashMapAndLinkedQueue);
  }
  void setSupportsTMHashMapAndLinkedQueue() {
    _flags4.set(SupportsTMHashMapAndLinkedQueue);
  }
  ...
  bool getSupportsBCDToDFPReduction() {
    return _flags4.testAny(SupportsBCDToDFPReduction);
  }
  void setSupportsBCDToDFPReduction() {
    _flags4.set(SupportsBCDToDFPReduction);
  }
  ...
  bool getSupportsShrinkWrapping() {
    return _flags3.testAny(SupportsShrinkWrapping);
  }
  ...
  bool getUsesLoadStoreMultiple() {
    return _flags3.testAny(UsesLoadStoreMultiple);
  }
  ...
  bool trackingInMemoryKilledLoads() {
    return _flags4.testAny(TrackingInMemoryKilledLoads);
  }
  void setTrackingInMemoryKilledLoads() {
    _flags4.set(TrackingInMemoryKilledLoads);
  }
  void resetTrackingInMemoryKilledLoads() {
    _flags4.reset(TrackingInMemoryKilledLoads);
  }
...
protected:
  ...
  // isTemporaryBased storageReferences just have a symRef but some other
  // routines expect a node so use the below to fill in this symRef on this node
