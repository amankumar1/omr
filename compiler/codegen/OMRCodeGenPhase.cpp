...
void OMR::CodeGenPhase::performProcessRelocationsPhase(
  ...
  if (comp->getOption(TR_AOT) &&
    ...
    if (comp->getOption(TR_TraceRelocatableDataDetailsCG)) // verbose output
    {
      ...
      traceMsg(comp, "Code start = %8x, Method start pc = %x, Method start pc "
                     "offset = 0x%x\n",
               aotMethodCodeStart, cg->getCodeStart(),
               cg->getCodeStart() - aotMethodCodeStart);
    }
  ...
  if (regex && TR::SimpleRegex::match(regex, comp->getCurrentMethod())) {
    if (TR::Compiler->target.is64Bit()) {
      setDllSlip((char *)cg->getCodeStart(),
                 (char *)cg->getCodeStart() + cg->getWarmCodeLength(),
                 "SLIPDLL64", comp);
      if (cg->getColdCodeStart())
        setDllSlip((char *)cg->getColdCodeStart(),
                   (char *)cg->getColdCodeStart() + cg->getColdCodeLength(),
                   "SLIPDLL64", comp);
    } else {
      setDllSlip((char *)cg->getCodeStart(),
                 (char *)cg->getCodeStart() + cg->getWarmCodeLength(),
...
void OMR::CodeGenPhase::performEmitSnippetsPhase(TR::CodeGenerator *cg,
  ...
  if (comp->getOption(TR_TraceCG) ||
    ...
    comp->getDebug()->print(comp->getOutFile(), cg->getSnippetList(),
                            true); // print Warm Snippets
    comp->getDebug()->print(comp->getOutFile(), cg->getSnippetList(),
                            false); // print the rest
    traceMsg(comp, "</snippets>\n");
    ...
    diagnostic(
        ...
        cg->getWarmCodeLength() + cg->getColdCodeLength(), snippetLength);
  }
}
...
// check reference counts
  ...
  for (int r = 0; r < NumRegisterKinds; r++) {
    if (TO_KIND_MASK(r) & cg->getSupportedLiveRegisterKinds()) {
      TR::CodeGenerator::checkForLiveRegisters(
          cg->getLiveRegisters((TR_RegisterKinds)r));
    }
  }
#endif
...
void OMR::CodeGenPhase::performShrinkWrappingPhase(TR::CodeGenerator *cg,
                                                   TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
