...
uintptr_t findCorrectInlinedSiteIndex(void *constantPool, TR::Compilation *comp,
                                      uintptr_t currentInlinedSiteIndex) {
  uintptr_t cp2 = 0;
  ...
  if ((uintptr_t)constantPool == cp2) {
    ...
    if ((uintptr_t)constantPool ==
        (uintptr_t)comp->getCurrentMethod()->constantPool()) {
      matchFound = true;
      ...
      for (uintptr_t i = 0; i < comp->getNumInlinedCallSites(); i++) {
        ...
        if ((uintptr_t)constantPool ==
            (uintptr_t)aotMethodInfo->resolvedMethod->constantPool()) {
          matchFound = true;
          inlinedSiteIndex = i;
          break;
        }
  ...
  if (comp->cg()->needGuardSitesEvenWhenGuardRemoved() &&
    ...
    if (virtualGuard->getKind() == TR_HCRGuard) {
      if (comp->getOption(TR_TraceReloCG))
        traceMsg(comp, "createGuardSiteForRemovedGuard: removing HCRGuard, no "
    ...
    if (comp->getOption(TR_TraceAll))
      traceMsg(comp, "createGuardSiteForRemovedGuard: removedGuardKind %d, "
                     "removedGurad %p, _callNode %p, _guardNode %p, _thisClass "
                     "%p, _calleeIndex %d, _byteCodeIndex %d, addedAOTNopSite "
                     "%p\n",
