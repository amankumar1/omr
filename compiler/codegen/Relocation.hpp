...
public:
  ...
  TR_LabelRelocation(uint8_t *p, TR::LabelSymbol *l)
      : TR_Relocation(p), _label(l) {}
...
public:
  ...
  TR_12BitLabelRelativeRelocation(uint8_t *p, TR::LabelSymbol *l,
...
class TR_16BitLabelRelativeRelocation : public TR_LabelRelocation {
  ...
  // relocation is at 32-47 bits, TR_16BitLabelRelativeRelocation(*p,*l, 4,
  // true)
  //
  // The regular TR_16BitLabelRelativeRelocation(uint8_t *p, TR::LabelSymbol *l)
  // only supports instructions with 16-bit relocations,
...
public:
  ...
  TR_16BitLabelRelativeRelocation(uint8_t *p, TR::LabelSymbol *l,
                                  int8_t divisor, bool isInstOffset = false)
      : TR_LabelRelocation(p, l), _addressDifferenceDivisor(divisor),
...
public:
  TR_InstructionAbsoluteRelocation(
      uint8_t *updateLocation, TR::Instruction *i,
      bool useEndAddr) /* specified if the start or
                          the end address of the instruction
                          is required */
      : TR_Relocation(updateLocation),
        _instruction(i),
...
public:
  ...
  TR_16BitLoadLabelRelativeRelocation(TR::Instruction *i,
                                      TR::LabelSymbol *start,
                                      TR::LabelSymbol *end, int32_t delta)
      : TR_Relocation(NULL), _lastInstruction(i), _startLabel(start),
...
public:
  ...
  TR_32BitLoadLabelRelativeRelocation(TR::Instruction *i,
                                      TR::LabelSymbol *start,
                                      TR::LabelSymbol *end, int32_t delta)
      : TR_Relocation(NULL), _lastInstruction(i), _startLabel(start),
        _endLabel(end), _deltaToStartLabel(delta) {}
...
class TR_IteratedExternalRelocation
    : public TR_Link<TR_IteratedExternalRelocation> {
public:
  TR_IteratedExternalRelocation()
  ...
  TR_IteratedExternalRelocation(uint8_t *target,
                                TR_ExternalRelocationTargetKind k,
                                flags8_t modifier, TR::CodeGenerator *codeGen);
  TR_IteratedExternalRelocation(uint8_t *target, uint8_t *target2,
                                TR_ExternalRelocationTargetKind k,
                                flags8_t modifier, TR::CodeGenerator *codeGen);
...
public:
  TR_ExternalRelocation()
      : TR_Relocation(), _targetAddress(NULL), _targetAddress2(NULL),
        _kind(TR_ConstantPool), _relocationRecord(NULL) {}

  TR_ExternalRelocation(uint8_t *p, uint8_t *target,
                        TR_ExternalRelocationTargetKind kind,
                        TR::CodeGenerator *codeGen)
      : TR_Relocation(p), _targetAddress(target), _targetAddress2(NULL),
        _kind(kind), _relocationRecord(NULL) {}

  TR_ExternalRelocation(uint8_t *p, uint8_t *target, uint8_t *target2,
                        TR_ExternalRelocationTargetKind kind,
                        TR::CodeGenerator *codeGen)
      : TR_Relocation(p), _targetAddress(target), _targetAddress2(target2),
        _kind(kind), _relocationRecord(NULL) {}
  ...
  static char *getName(TR_ExternalRelocationTargetKind k) {
...
public:
  TR_32BitExternalOrderedPairRelocation(uint8_t *location1, uint8_t *location2,
                                        uint8_t *target,
                                        TR_ExternalRelocationTargetKind k,
                                        TR::CodeGenerator *codeGen);
...
public:
  TR_BeforeBinaryEncodingExternalRelocation() : TR_ExternalRelocation() {}

  TR_BeforeBinaryEncodingExternalRelocation(
      TR::Instruction *instr, uint8_t *target,
      TR_ExternalRelocationTargetKind kind, TR::CodeGenerator *codeGen)
      : TR_ExternalRelocation(NULL, target, kind, codeGen),
        _instruction(instr) {}

  TR_BeforeBinaryEncodingExternalRelocation(
      TR::Instruction *instr, uint8_t *target, uint8_t *target2,
      TR_ExternalRelocationTargetKind kind, TR::CodeGenerator *codeGen)
