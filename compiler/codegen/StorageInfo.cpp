...
void TR_StorageInfo::populateAddress(TR::Node *address) {
  ...
  if (_address->getOpCode().hasSymbolReference() &&
    ...
    } else if (_address->getOpCodeValue() == TR::aload &&
               _symRef->getOffset() == 0) {
      if (comp()->cg()->getLinkage()->isAddressOfStaticSymRef(_symRef)) {
        // the base address of the static area has been cached in _symRef and is
        // being loaded here with an aload
        _class = TR_StaticBaseAddress;
      } else if (comp()->cg()->getLinkage()->isAddressOfPrivateStaticSymRef(
                     _symRef)) {
        // the base address of the private static area has been cached in
        // _symRef and is being loaded here with an aload
...
TR_StorageOverlapKind TR_StorageInfo::mayOverlapWith(TR_StorageInfo *info) {
  ...
  if (symbolMismatch && _class == TR_DirectMappedAuto &&
    ...
    if (comp()->cg()->traceBCDCodeGen())
      traceMsg(comp(), "\t\toverlap=false : autoDirectMapped and diff symbols "
                       "(#%d (%p) and #%d (%p))\n",
    ...
    if (comp()->cg()->traceBCDCodeGen())
      traceMsg(comp(), "\t\toverlap=false : staticDirectMapped and diff "
                       "symbols (#%d (%p) and #%d (%p))\n",
               _symRef->getReferenceNumber(), _symRef->getSymbol(),
               info->_symRef->getReferenceNumber(), info->_symRef->getSymbol());
    ...
    return TR_NoOverlap; // directly mapped autos from loadaddrs or direct
                         // stores cannot interfere with static addresses
    // note that aload from autos and static addresses certainly can as the
    // aload may contain a static address
  }

  // TR_StaticBaseAddress and TR_DirectMappedStatic are unlikely to be found in
  // the same compile unit (different fundamental ways
  // of accessing statics) but if they are both present then they may overlap as
  // the TR_StaticBaseAddress points to the start of the
  // the static area and TR_DirectMappedStatic points to a particular static
  // symbol
  ...
  if (compareRanges) {
    ...
    if (comp()->cg()->traceBCDCodeGen())
      traceMsg(comp(), "\t\t\tcompareRanges : range1 %d->%d vs range2 %d->%d "
                       "--> overlap range %d->%d\n",
               startOne, endOne, startTwo, endTwo, overlapStart, overlapEnd);

    TR_StorageOverlapKind overlap = TR_NoOverlap;

    if (overlapStart >= overlapEnd) // i.e. is this an impossible (>) or trivial
                                    // range (==) with start >= end? If so then
                                    // there is no overlap
    {
