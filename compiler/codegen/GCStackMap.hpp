...
public:
  ...
  TR_InternalPointerMap(TR_Memory *m)
      : _numInternalPtrs(0), _numDistinctPinningArrays(0), _internalPtrPairs(m),
        _size(0) {}
...
public:
  ...
  void setHighWordRegisterBits(uint32_t bits) {
    _registerMap.setHighWordRegisterBits(bits);
  }
  void resetHighWordRegistersBits(uint32_t bits) {
    _registerMap.resetHighWordRegisterBits(bits);
  }
