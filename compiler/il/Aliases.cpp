...
OMR::SymbolReference::getUseDefAliasesBV(bool isDirectCall,
  ...
  case TR::Symbol::IsResolvedMethod: {
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_EnableHCR)) {
      ...
      case TR::java_lang_System_arraycopy: {
        TR_BitVector *aliases = new (comp->trHeapMemory()) TR_BitVector(
  ...
  case TR::Symbol::IsShadow: {
    ...
    if (_symbol == symRefTab->findGenericIntShadowSymbol()) {
      aliases = new (comp->trHeapMemory()) TR_BitVector(
          symRefTab->getNumSymRefs(), comp->trMemory(), heapAlloc, growable);
      *aliases |= symRefTab->aliasBuilder.arrayElementSymRefs();
      if (comp->generateArraylets())
        *aliases |= symRefTab->aliasBuilder.arrayletElementSymRefs();
...
const char *OMR::SymbolReference::getTypeSignature(int32_t &len,
  ...
  case TR::Symbol::IsShadow:
    return (
        _cpIndex > 0
            ? self()->getOwningMethod(comp)->fieldSignatureChars(_cpIndex, len)
            : 0);
...
TR_BitVector *OMR::SymbolReference::getSymRefUseDefAliasesBV(TR::Compilation *c,
                                                             bool isDirectCall,
                                                             bool gcSafe) {
