...
OMR::Block::Block(TR_Memory *m)
    : TR::CFGNode(m), _pEntry(NULL), _pExit(NULL), _pStructureOf(NULL),
      _liveLocals(NULL), _globalRegisters(0), _registerSaveDescriptionBits(0),
      _catchBlockExtension(NULL), _firstInstruction(NULL),
      _lastInstruction(NULL), _blockSize(-1), _debugCounters(NULL), _flags(0),
      _moreflags(0) {
  self()->setFrequency(-1);
...
OMR::Block::Block(TR::TreeTop *entry, TR::TreeTop *exit, TR_Memory *m)
    : TR::CFGNode(m), _pEntry(entry), _pExit(exit), _pStructureOf(NULL),
      _liveLocals(NULL), _globalRegisters(0), _registerSaveDescriptionBits(0),
      _catchBlockExtension(NULL), _firstInstruction(NULL),
      _lastInstruction(NULL), _blockSize(-1), _debugCounters(NULL), _flags(0),
      _moreflags(0) {
...
OMR::Block::Block(TR::Block &other, TR::TreeTop *entry, TR::TreeTop *exit)
    : TR::CFGNode(other.trMemory()), _pEntry(entry), _pExit(exit),
  ...
  if (other._globalRegisters)
    _globalRegisters = new (other.trHeapMemory())
 ...
 * never be used.
 * If the only predecessor to the block belonging to the BBStart is the previous
 * block that means that
 * we will fallthrough to this block without a branch, and therefore we don't
 * need a label. There are
 * some exceptions:
...
TR::Block *OMR::Block::breakFallThrough(TR::Compilation *comp,
  ...
  if (lastNode->getOpCode().isBranch()) {
    ...
    comp->getFlowGraph()->addEdge(
        TR::CFGEdge::createEdge(faller, gotoBlock, self()->trMemory()));
    comp->getFlowGraph()->addEdge(
        TR::CFGEdge::createEdge(gotoBlock, fallee, self()->trMemory()));
 ...
 * newFallThroughBlock.  It does _NOT_
 *       remove any fall-through edge that might have previously existed from
 * block.  That responsibility
 *       is left to the caller since removing the edge at the wrong time could
 * leave the original fallthrough
 *       successor with no predecessors which could leave it in an unexpectedly
 * deleted state.
 */
 ...
 * to "newTo" as well as remove the original edge from
 *        "from" to origTo.  BE CAREFUL when calling this function that there is
 * another predecessor edge for "origTo" or else when
 *        redirectFlowToNewDestination removes the predecessor "from", "origTo"
 * might be deleted.
 */
      ...
      if (lastNode->getOpCode().hasBranchChildren()) {
        ...
        for (int32_t i = 0; i < lastNode->getNumChildren() - 1; i++) {
          ...
          traceMsg(comp, "considering node %p with branch destination %p \n",
                   child, child->getBranchDestination()
...
void OMR::Block::uncommonNodesBetweenBlocks(TR::Compilation *comp,
                                            TR::Block *newBlock) {
  TR_ScratchList<TR::SymbolReference> symbolReferenceTempsA(self()->trMemory());
  TR_ScratchList<TR::SymbolReference> injectedBasicBlockTemps(
      self()->trMemory());
  TR_ScratchList<TR::SymbolReference> symbolReferenceTempsC(self()->trMemory());

  TR_HandleInjectedBasicBlock ibb(
      comp, NULL, comp->getMethodSymbol(), symbolReferenceTempsA,
      injectedBasicBlockTemps, symbolReferenceTempsC);
...
TR::Block *OMR::Block::split(TR::TreeTop *startOfNewBlock, TR::CFG *cfg,
  ...
  TR::Block *block2 = new (self()->trHeapMemory()) TR::Block(
      TR::TreeTop::create(comp, TR::Node::create(startNode, TR::BBStart)),
      self()->getExit(), self()->trMemory());
  ...
  if (structure) {
    ...
    if (thisBlockStructure) {
      TR_BlockStructure *blockStructure2 = new (self()->trHeapMemory())
        ...
        if (excSuccWasAdded) {
          if (toStructureSubGraphNode((*nextSucc)->getTo())->getStructure())
            TR::CFGEdge::createExceptionEdge(
                blockStructureNode2, (*nextSucc)->getTo(), self()->trMemory());
          else
 ...
 * removed from its block, compareTree will
 *                     now end this block.
...

{
  ...
  cfg->addEdge(
      TR::CFGEdge::createEdge(exitBlock, afterExitBlock, self()->trMemory()));
 ...
 * this block.
 *   \param compareTree   the compare tree to use in constructing the
 * conditional blocks and cfg
 *   \param ifTree        the tree to use on the 'if' branch when constructing
 * the blocks and cfg
 *   \param elseTree      the tree to use on the 'else' branch when constructing
 * the blocks and cfg
 *   \param cfg           the cfg to update with the new information
 */
TR::Block *OMR::Block::createConditionalBlocksBeforeTree(
  ...
  cfg->addEdge(
      TR::CFGEdge::createEdge(ifBlock, remainderBlock, self()->trMemory()));
  ...
  if (elseTree) {
    ...
    cfg->addEdge(
        TR::CFGEdge::createEdge(self(), elseBlock, self()->trMemory()));
    cfg->addEdge(
        TR::CFGEdge::createEdge(elseBlock, remainderBlock, self()->trMemory()));
...
TR::Block *OMR::Block::splitEdge(TR::Block *from, TR::Block *to,
  ...
  // throw
  // splitting the block at the point of the return or throw is the safest way
  // to do this
  else {
    TR::TreeTop *lastTree = from->getLastRealTreeTop();
    TR::ILOpCodes opCode = lastTree->getNode()->getOpCodeValue();
    TR_ASSERT(opCode == TR::athrow ||
                  lastTree->getNode()->getOpCode().isReturn(),
...
void OMR::Block::addExceptionRangeForSnippet(uint32_t startPC, uint32_t endPC) {
  _snippetBoundaries.add(new (self()->trHeapMemory())
...
TR::Block *TR_BlockCloner::cloneBlocks(TR::Block *from, TR::Block *lastBlock) {
  ...
  for (; from; from = from->getNextBlock()) {
    ...
    TR::Block *to = new (from->trHeapMemory())
...
TR::Block *TR_BlockCloner::cloneBlocks(TR_LinkHeadAndTail<BlockMapper> *bMap) {
  ...
  // target (the old fall through node)
  // we don't mess with the last node as the connection is handled by whoever
  // called us
  //
  for (BlockMapper *itr = bMap->getFirst(); itr->getNext();
    ...
    // an if
    // if branch was taken reverse the branch condition so we fall through to
    // the next block as expected
    //
...
TR::Block *TR_BlockCloner::doBlockClone(TR_LinkHeadAndTail<BlockMapper> *bMap) {
  ...
  // really handle the case when you are not cloning along a
  // fall through path, and you are trying to preserve branches exactly (ie
  // _cloneBranchesExactly is set to true
  // Luckily, so far no code has ever not done that.  Most code that
  // clonesBranchesExactly clones a single block, and the few cases that
  // don't appear to be cloning along a fall through path
  //
  for (m = _blockMappings.getFirst(); m; m = m->getNext()) {
    _cfg->addNode(m->_to);
    if (comp->ilGenTrace())
      dumpOptDetails(comp, "BLOCK CLONER: Newly created block_%d is a clone of "
                           "original block_%d\n",
