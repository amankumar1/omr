...
OMR::Node::Node()
    : _opCode(TR::BadILOp), _numChildren(0), _globalIndex(0), _flags(0),
      _byteCodeInfo(), _optAttributes(NULL), _unionBase(), _unionPropertyA() {}
...
OMR::Node::Node(TR::Node *originatingByteCodeNode, TR::ILOpCodes op,
                uint16_t numChildren, OptAttributes *optAttributes)
    : _opCode(op), _numChildren(numChildren), _globalIndex(0), _flags(0),
      _byteCodeInfo(), _optAttributes(NULL), _unionBase(), _unionPropertyA() {
  ...
  if (optAttributes != NULL) {
    ...
    self()->setOptAttributes(new (comp->getNodePool())
    ...
    memset(&(self()->getOptAttributes()->_unionA), 0,
           sizeof(self()->getOptAttributes()->_unionA));
  ...
  if (ilGen) {
    ...
    TR_ASSERT(_byteCodeInfo.getCallerIndex() < (INT_MAX / 4),
 ...
 * memory occupied by originalNode and returning the
 * same address as it.
 *
 * It will have the same globalIndex, poolIndex, byteCodeInfo, and optAttributes
 * as the originalNode, but properties from
 * the originalNode that are valid for the new node only will be copied. All
 * other properties will be reset to their default values.
 */
TR::Node *OMR::Node::recreateAndCopyValidPropertiesImpl(
  ...
  // (and other properties), but we do need to deallocate
  // any objects it has created. This is one of them. It will be recreated if
  // necessary on createInternal.
  // other properties such as nameSymRef & callNodeData, are associated with the
  // _globalIndex
  ...
  // replacement node,
  // but it does not matter if copyValidProperties is complete, though it must
  // be accurate
...
TR::Node *OMR::Node::createInternal(TR::Node *originatingByteCodeNode,
  ...
  if (!originalNode)
    ...
    TR::Node *node = new (TR::comp()->getNodePool(), poolIndex)
        TR::Node(0, op, numChildren, optAttributes);
...
TR::Node *OMR::Node::createWithRoomForOneMore(TR::ILOpCodes op,
  ...
  if (symbolRefOrBranchTarget != NULL || node->hasSymbolReference() ||
    ...
    if (node->hasSymbolReference())
      ...
      TR_ASSERT(false, "attempting to access _branchDestinationNode or "
                       "_symbolReference field for node %s %p that does not "
                       "have one of them",
...
TR::Node *OMR::Node::createWithRoomForThree(TR::ILOpCodes op, TR::Node *first,
  ...
  // variableNewArray, arraystorechk or a node with a padding address
  ...
  TR_ASSERT(node->getOpCode().isIf() ||
            ...
            "arraystorechk, variableNewArray node or a node with a padding "
            "address");
#else
  TR_ASSERT(node->getOpCode().isIf() ||
                (node->getOpCodeValue() == TR::variableNewArray),
...
TR::Node *OMR::Node::duplicateTree(bool duplicateChildren) {
  ...
  // Privatization) seem to rely on the old behaviour of duplicateTree
 ...
 */
TR::Node *OMR::Node::duplicateTreeWithCommoning() {
  CS2::HashTable<TR::Node *, TR::Node *, CS2::allocator> nodeMapping;
...
bool OMR::Node::isUnsafeToDuplicateAndExecuteAgain(int32_t *nodeVisitBudget) {
  ...
  if (self()->getOpCode().hasSymbolReference()) {
    if (self()->getSymbolReference()->isUnresolved()) {
      ...
      // is particularly
      // problematic if the variable is an address to be dereferenced by the
      // containing tree.
      // We'll aggressively call this "safe" here, but the caller must ensure
      // that loads within
 ...
 * look
 *        into the subtree.
 *     2) We must maintain the visit counts of the nodes visited; to do this we
 * use
...
bool OMR::Node::containsAnyNode(List<TR::Node *> &nodeList,
                                vcount_t visitCount) {
  for (ListElement<TR::Node *> *el = nodeList.getListHead(); el;
...
/**
 * Calls containsNode on each node from bitvector. The bitvector is a Set of
 * nodes which holds
 * NodePool indices which is accessible via TR::Node::getGlobalIndex.
 * The node is retrieved per index through the NodePool.
 */
bool OMR::Node::containsAnyNode(TR::SparseBitVector &nodeSet,
 ...
 * this
 * is a composite operator, made from a high order part and its adjunct operator
 * which is the first child of its third child. It returns true if the node has
 * the form:
 *
 *    highOp
 *      firstChild
 ...
 * The node and its pair have each other as its third child, completing the
 * cycle.
 ...
 * evaluation (or anchoring)
 * of an address tree that turns out not to be required at tree evaluation (or
 * tree simplifiation) time
 *
...
bool OMR::Node::isNotCollected() {
  TR::Compilation *comp = TR::comp();
  if (self()->getOpCode().hasSymbolReference() &&
      self()->getSymbolReference()->getSymbol()->isNotCollected())
    return true;
  else if (comp->generateArraylets() &&
           (self()->getOpCode().isArrayRef() &&
            (self()->getFirstChild()->isNotCollected() ||
             !self()->getFirstChild()->isInternalPointer())))
    return true;
  return false;
}
 ...
 * that
 * have the current visit count must retain it and all nodes that do not must
 * not
 * get it.
 */
bool OMR::Node::isSafeToReplaceNode(TR::TreeTop *curTreeTop) {
...

  {
    ...
    //
    if (definingSymRef &&
        definingSymRef->getUseDefAliases().containsAny(symbolReferencesInNode,
                                                       c))
      return false;
  }
...
bool OMR::Node::mayModifyValue(TR::SymbolReference *symRef) {
  ...
  // isSafeToReplace
  // so that if the aliasing information is available it's used otherwise, it
  // uses the existing
  // algorithm.
  //
...
bool OMR::Node::anchorConstChildren() {
  ...
  {
    TR_ASSERT(self()->getNumChildren() == 1, "expecting BCD/Aggr const node %p "
                                             "to have 1 child and not %d "
                                             "children\n",
...
void OMR::Node::setNullCheckReference(TR::Node *refNode) {
  ...
  if (firstChild->getNumChildren() == 0) {
    TR_ASSERT(0, "Null check node " POINTER_PRINTF_FORMAT
...
TR::Node *OMR::Node::getReturnReason() { return self()->getReturnCode(true); }

TR::Node *OMR::Node::getReturnCode(bool isReason) {
  TR::Compilation *c = TR::comp();
 ...
 * children are
 * evaluated in a register, this function will return true if the evaluation of
 * this node
...
bool OMR::Node::nodeMightKillCondCode() {
  ...
  // at this point, we will assume that the CC is killed if we at not on Trex or
  // higher
  ...
  if (((opcode.isLoad() || opcode.isStore() || opcode.isLoadReg() ||
       ...
       (self()->getSize() == 4 || self()->getSize() == 8 ||
        self()->getSize() == 2 ||
        (self()->getSize() == 1 &&
         TR::Compiler->target.cpu.getS390SupportsZ9()) ||
        self()->getSize() == 0)))
    return false;
...
#define SignExtensionFlagsIndex(x)                                             \
  reinterpret_cast<long>(                                                      \
      self()->hasNodeExtension()                                               \
          ? _unionBase._extension.getExtensionPtr()->getElem<TR::Node *>(1)    \
          : _unionBase._children[1]) *                                         \
          numSignExtensionFlags +                                              \
      x
#define SupportedType(node)                                                    \
  (node->getOpCode().isLoadVar() && TR_LoadExtensions::supportedType(node) ||  \
   (node->getOpCode().isLoadConst() &&                                         \
    TR_LoadExtensions::supportedConstLoad(node, c)))

bool OMR::Node::couldIgnoreExtend() {
  TR::Compilation *c = TR::comp();
  TR_BitVector *flags = c->cg()->signExtensionFlags();
  // traceMsg(c, "\t\tChecking couldIgnore on %d+%d == %c on %p\n",
  // SignExtensionFlagsIndex(0), (int)couldSkipExtension, flags &&
  // SupportedType(getOpCode()) &&
  // flags->isSet(SignExtensionFlagsIndex(couldSkipExtension))?'1':'0', this);
  return flags && SupportedType(self()) &&
         flags->isSet(SignExtensionFlagsIndex(couldSkipExtension));
}

void OMR::Node::setCouldIgnoreExtend(bool b) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(SupportedType(self()),
            "couldIgnoreExtend() should only be called for loads");
  TR_BitVector *flags = c->cg()->signExtensionFlags();
...
const char *OMR::Node::printCouldIgnoreExtend() {
  return self()->couldIgnoreExtend() ? "couldSkipExt " : "";
}

bool OMR::Node::force64BitLoad() {
  TR::Compilation *c = TR::comp();
  TR_BitVector *flags = c->cg()->signExtensionFlags();
  // traceMsg(c, "\t\tChecking force64BitReg on %d+%d == %c on %p\n",
  // SignExtensionFlagsIndex(0), (int)force64BitReg, flags &&
  // SupportedType(getOpCode()) &&
  // flags->isSet(SignExtensionFlagsIndex(force64BitReg))?'1':'0', this);
  return flags && SupportedType(self()) &&
         flags->isSet(SignExtensionFlagsIndex(force64BitReg));
}

void OMR::Node::setForce64BitLoad(bool b) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(SupportedType(self()),
            "setForce64BitLoad() should only be called for loads");
  TR_BitVector *flags = c->cg()->signExtensionFlags();
  TR_ASSERT(flags, "Flags for signExtension have not been allocated");
...
const char *OMR::Node::printForce64BitLoad() {
  return self()->force64BitLoad() ? "force64Reg " : "";
}

bool OMR::Node::isUnsignedLoad() {
  TR::Compilation *c = TR::comp();
  TR_BitVector *flags = c->cg()->signExtensionFlags();
  // traceMsg(c, "\t\tChecking unsignedLoad on %d+%d == %c on %p\n",
  // SignExtensionFlagsIndex(0), (int)unsignedLoad, flags &&
  // SupportedType(getOpCode()) &&
  // flags->isSet(SignExtensionFlagsIndex(unsignedLoad))?'1':'0', this);
  return flags && SupportedType(self()) &&
         flags->isSet(SignExtensionFlagsIndex(unsignedLoad));
}

void OMR::Node::setIsUnsignedLoad(bool b) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(SupportedType(self()),
            "setIsUnsignedLoad() should only be called for loads");
  TR_BitVector *flags = c->cg()->signExtensionFlags();
  TR_ASSERT(flags, "Flags for signExtension have not been allocated");
  //   traceMsg(c->cg(), "Node %p was marked as %d\n", this,
  //   (long)(_unionBase._children[1]));
...
const char *OMR::Node::printIsUnsignedLoad() {
...
OMR::Node::OptAttributes::OptAttributes()
    : _visitCount(0), _localIndex(0), _referenceCount(0) {}

void *OMR::Node::OptAttributes::operator new(size_t s, TR::NodePool &nodes) {
...
void OMR::Node::setIsNotRematerializeable() {
  ...
  if (performNodeTransformation1(
    ...
    self()->getOptAttributes()->_localIndex =
        (self()->getOptAttributes()->_localIndex | SCOUNT_HIGH_BIT);
}
...
TR::Register *OMR::Node::getRegister() {
  ...
  if ((uintptr_t)self()->getOptAttributes()->_unionA._register & 1)
...
int32_t OMR::Node::getEvaluationPriority(TR::CodeGenerator *codeGen) {
  if (self()->getOptAttributes()->_unionA._register ==
      0) // not evaluated into register & priority unknown
  {
  ...
  if ((uintptr_t)(self()->getOptAttributes()->_unionA._register) &
      1) // evaluation priority
...
int32_t OMR::Node::setEvaluationPriority(int32_t p) {
  if (self()->getOptAttributes()->_unionA._register ==
      ...
      ((uintptr_t)(self()->getOptAttributes()->_unionA._register) &
       1)) // not evaluated, priority known
  {
    self()->getOptAttributes()->_unionA._register =
        (TR::Register *)(uintptr_t)((p << 1) | 1);
...
TR::DataType OMR::Node::getArrayCopyElementType() {
  ...
  if (_numChildren == 3 || _numChildren == 4 || _numChildren == 6) {
    return (TR::DataTypes)_unionBase._extension.getExtensionPtr()
        ->getElem<uint32_t>(_numChildren);
  }
...
void OMR::Node::setArrayCopyElementType(TR::DataType type) {
  ...
  if (_numChildren == 3 || _numChildren == 4 || _numChildren == 6) {
    ...
    TR_ASSERT(numExtensionElems > _numChildren,
              "need an extra slot to set arrayCopyElementType\n");
...
TR::ILOpCodes OMR::Node::setOverflowCHKOperation(TR::ILOpCodes op) {
  TR_ASSERT(self()->getOpCodeValue() == TR::OverflowCHK,
...
TR::ILOpCodes OMR::Node::getOverflowCHKOperation() {
  TR_ASSERT(self()->getOpCodeValue() == TR::OverflowCHK,
...
TR::SymbolReference *OMR::Node::getSymbolReference() {
  ...
  TR_ASSERT(self()->hasSymbolReference(), "attempting to access "
                                          "_symbolReference field for node %s "
                                          "%p that does not have it",
...
TR::SymbolReference *OMR::Node::setSymbolReference(TR::SymbolReference *p) {
  ...
  {
    TR_ASSERT(self()->hasSymbolReference(), "attempting to access "
                                            "_symbolReference field for node "
...
TR::Symbol *OMR::Node::getSymbol() {
  ...
  if (self()->hasRegLoadStoreSymbolReference())
    return self()->getRegLoadStoreSymbolReference()
               ...
               : NULL;
#else
  TR_ASSERT(hasSymbolReference(), "attempting to access _symbolReference field "
                                  "for node %s %p that does not have it",
            self()->getOpCode().getName(), this);
#endif
...
int32_t OMR::Node::getArrayStride() {
  TR_ASSERT(self()->hasArrayStride(), "attempting to access _arrayStride field "
                                      "for node %s %p that does not have it",
            self()->getOpCode().getName(), this);
...
TR::DataType OMR::Node::computeDataType() {
  ...
  TR_ASSERT(self()->hasDataType(), "attempting to access _dataType field for "
                                   "node %s %p that does not have it",
            self()->getOpCode().getName(), self());
...
bool OMR::Node::isNull() {
  return (self()->getOpCodeValue() == TR::loadaddr ? false : _flags.testAny(
                                                                 nodeIsNull));
}
...
void OMR::Node::setIsInvalid8BitGlobalRegister(bool v) {
  ...
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "invalid8BitGlobalRegister flag on node %p "
                                    "to %d\n",
                                 self(), v))
...
const char *OMR::Node::printIsHPREligible() {
 ...
 */
bool OMR::Node::isEligibleForHighWordOpcode() {
...
bool OMR::Node::isDebug() { return _flags.testAny(nodeIsDebug); }

void OMR::Node::setIsDebug(bool v) { _flags.set(nodeIsDebug, v); }
...
void OMR::Node::setIsSafeToSkipTableBoundCheck(bool b) {
  ...
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "canSkipTableBoundCheck flag on node %p to "
                                    "%d\n",
                                 self(), b))
...
void OMR::Node::setStoredValueIsIrrelevant(bool v) {
  ...
  TR_ASSERT(c->getOption(TR_EnableOSR),
            "Currently this OMR::Node flag means different things on different "
            "front ends. If we wanted to reuse this query across front ends "
            "other than Java (which uses these kinds of stores for OSR) then "
            "we probably need to map it to a different bit in the OMR::Node "
            "flags");
  TR_ASSERT((self()->getOpCode().isStore() &&
             self()->getSymbolReference()->getSymbol()->isAutoOrParm()),
            "Opcode must be store to a stack slot");
  if (self()->getOpCode().isStore() &&
      self()->getSymbolReference()->getSymbol()->isAutoOrParm()) {
    if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                      "StoredValueIsIrrelevant flag on node %p "
                                      "to %d\n",
                                   self(), v))
...
void OMR::Node::setLiveMonitorInitStore(bool v) {
  ...
  if (self()->getOpCode().hasSymbolReference() &&
      self()->getSymbol()->holdsMonitoredObject() &&
      performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "liveMonitorInitStore flag on node %p to "
                                    "%d\n",
                                 self(), v))
...
void OMR::Node::setUseSignExtensionMode(bool b) {
  ...
  if (self()->getOpCode().isLoadVar() && self()->getType().isInt32() &&
      performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "useSignExtensionMode flag on node %p to "
                                    "%d\n",
                                 self(), b)) {
...
void OMR::Node::setHasFoldedImplicitNULLCHK(bool v) {
  ...
  TR_ASSERT(self()->getOpCode().isBndCheck() ||
            ...
            "assertion failure");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "foldedImplicitNULLCHK flag on node %p to "
                                    "%d\n",
                                 self(), v))
...
void OMR::Node::setThrowInsertedByOSR(bool v) {
  ...
  if (c->getOption(TR_EnableOSR) && (self()->getOpCodeValue() == TR::athrow)) {
    if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                      "ThrowInsertedByOSR flag on node %p to "
                                      "%d\n",
                                   self(), v))
...
void OMR::Node::setIsTheVirtualCallNodeForAGuardedInlinedCall() {
  TR::Compilation *c = TR::comp();
  if (performNodeTransformation1(c, "O^O NODE FLAGS: Setting "
...
void OMR::Node::resetIsTheVirtualCallNodeForAGuardedInlinedCall() {
  TR::Compilation *c = TR::comp();
  if (performNodeTransformation1(c, "ReO^O NODE FLAGS: Setting "
                                    "virtualCallNodeForAGuardedInlinedCall "
                                    "flag on node %p\n",
                                 self()))
...
void OMR::Node::setDontTransformArrayCopyCall() {
  ...
  if (self()->isArrayCopyCall()) {
    if (performNodeTransformation1(c, "O^O NODE FLAGS: Setting "
                                      "dontTransformArrayCopyCall flag on node "
                                      "%p\n",
                                   self()))
...
void OMR::Node::setNodeIsRecognizedArrayCopyCall(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::call,
            "Opcode must be vcall to arraycopy");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "nodeIsRecognizedArrayCopyCall flag on "
                                    "node %p to %d\n",
                                 self(), v))
...
void OMR::Node::setContainsCompressionSequence(bool v) {
  ...
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "isCompressionSequence flag on node %p to "
                                    "%d\n",
                                 self(), v))
...
void OMR::Node::setSourceIsByteArrayTranslate(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslate,
            "Opcode must be arraytranslate");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "sourceIsByteArrayTranslate flag on node "
                                    "%p to %d\n",
                                 self(), v))
...
void OMR::Node::setTargetIsByteArrayTranslate(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslate,
            "Opcode must be arraytranslate");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "targetIsByteArrayTranslate flag on node "
                                    "%p to %d\n",
                                 self(), v))
...
void OMR::Node::setSourceCellIsTermChar(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslate,
            "Opcode must be arraytranslate");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "sourceCellIsTermChar flag on node %p to "
                                    "%d\n",
                                 self(), v))
...
void OMR::Node::setTableBackedByRawStorage(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslate,
            "Opcode must be arraytranslate");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "tableBackedByRawStorage flag on node %p "
                                    "to %d\n",
                                 self(), v))
...
void OMR::Node::setHalfWordElementArrayCopy(bool v) {
  ...
  if (v) {
    if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                      "halfWordElementArrayCopy flag on node "
                                      "%p to %d\n",
                                   self(), v))
...
void OMR::Node::setWordElementArrayCopy(bool v) {
  ...
  if (v) {
    if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                      "wordElementArrayCopy flag on node %p to "
                                      "%d\n",
                                   self(), v))
...
void OMR::Node::setRarePathForwardArrayCopy(bool v) {
  ...
  if (v != self()->isRarePathForwardArrayCopy() &&
      performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "rarePathForwardArrayCopy flag on node %p "
                                    "to %d\n",
                                 self(), v))
...
void OMR::Node::setNoArrayStoreCheckArrayCopy(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::arraycopy,
            "Opcode must be arraycopy");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "noArrayStoreCheckArrayCopy flag on node "
                                    "%p to %d\n",
                                 self(), v))
...
void OMR::Node::setArrayChkPrimitiveArray1(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::ArrayCHK,
            "Opcode must be ArrayCHK");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "arrayChkPrimitiveArray1 flag on node %p "
                                    "to %d\n",
                                 self(), v))
...
void OMR::Node::setArrayChkReferenceArray1(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::ArrayCHK,
            "Opcode must be ArrayCHK");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "arrayChkReferenceArray1 flag on node %p "
                                    "to %d\n",
                                 self(), v))
...
void OMR::Node::setArrayChkPrimitiveArray2(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::ArrayCHK,
            "Opcode must be ArrayCHK");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "arrayChkPrimitiveArray2 flag on node %p "
                                    "to %d\n",
                                 self(), v))
...
void OMR::Node::setArrayChkReferenceArray2(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::ArrayCHK,
            "Opcode must be ArrayCHK");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "arrayChkReferenceArray2 flag on node %p "
                                    "to %d\n",
                                 self(), v))
...
void OMR::Node::setNeedsPrecisionAdjustment(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::fRegLoad ||
            ...
            "Opcode must be FP global reg load");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "needsPrecisionAdjustment flag on node %p "
                                    "to %d\n",
                                 self(), v))
...
void OMR::Node::setIsMaxLoopIterationGuard(bool v) {
  ...
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "maxLoopIterationGuard flag on node %p to "
                                    "%d\n",
                                 self(), v))
...
void OMR::Node::resetIsTheVirtualGuardForAGuardedInlinedCall() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(c, "O^O NODE FLAGS: Resetting "
...
void OMR::Node::setIsMutableCallSiteTargetGuard() {
  ...
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(c, "O^O NODE FLAGS: Setting "
                                    "mutableCallSiteTargetGuard flag on node "
                                    "%p\n",
                                 self()))
...
void OMR::Node::setIsMethodEnterExitGuard(bool b) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (b && performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                         "methodEnterExitGuard flag to %d on "
                                         "node %p\n",
                                      b, self()))
...
void OMR::Node::setStoreAlreadyEvaluated(bool b) {
  ...
  TR_ASSERT(self()->getOpCode().isStore(), "assertion failure");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "storeAlreadyEvaluated flag on node %p to "
                                    "%d\n",
                                 self(), b))
...
void OMR::Node::setNormalizeNanValues(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::fbits2i ||
            ...
            "Opcode must be fbits2i or dbits2l");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "mustNormalizeNanValues flag on node %p to "
                                    "%d\n",
                                 self(), v))
...
void OMR::Node::setIsClassPointerConstant(bool b) {
  ...
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "classPointerConstant flag on node %p to "
                                    "%d\n",
                                 self(), b))
...
void OMR::Node::setIsMethodPointerConstant(bool b) {
  ...
  TR_ASSERT((self()->getOpCodeValue() == TR::aconst),
            "Can only call this for aconst\n");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "methodPointerConstant flag on node %p to "
                                    "%d\n",
                                 self(), b))
...
void OMR::Node::setAllocationCanBeRemoved(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::New ||
            ...
            "Opcode must be newarray or anewarray");
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "allocationCanBeRemoved flag on node %p to "
                                    "%d\n",
                                 self(), v))
...
void OMR::Node::setIsAdjunct(bool v) {
  TR_ASSERT(self()->getOpCodeValue() == TR::lmul ||
                self()->getOpCodeValue() == TR::luadd ||
                self()->getOpCodeValue() == TR::lusub,
            "Opcode must be lmul, lusub, or luadd");
...
void OMR::Node::setIsFPStrictCompliant(bool v) {
  ...
  if (performNodeTransformation2(c, "O^O NODE FLAGS: Setting "
                                    "resultFPStrictCompliant flag on node %p "
                                    "to %d\n",
                                 self(), v))
