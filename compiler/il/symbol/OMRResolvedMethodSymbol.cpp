...
OMR::ResolvedMethodSymbol::ResolvedMethodSymbol(TR_ResolvedMethod *method,
    ...
    : TR::MethodSymbol(TR_Private, method->convertToMethod()),
      ...
      _trivialDeadTreeBlocksList(comp->trMemory()), _comp(comp),
      _firstJitTempIndex(-1), _cannotAttemptOSR(NULL),
      _shouldNotAttemptOSR(NULL), _pythonConstsSymRef(NULL),
      _pythonNumLocalVars(0), _pythonLocalVarSymRefs(NULL), _properties(0) {
...
OMR::ResolvedMethodSymbol::getLogicalParameterList(TR::Compilation *comp) {
  if (comp->getMethodSymbol() == self()) {
    List<TR::ParameterSymbol> *l =
        comp->cg()->getLinkage()->getMainBodyLogicalParameterList();
    if (l == NULL) {
...
void bcIndexForFakeInduce(TR::Compilation *comp,
  ...
  while (p) {
    ...
    *q = '\0';
    ...
    traceMsg(comp, "signature: %s, callSiteInsertionPoint: %d, "
...
TR::TreeTop *OMR::ResolvedMethodSymbol::genInduceOSRCallNode(
  ...
  // in induceOSR's block
  // and that method gets inlined
  // there is no guarantee that the future block containing the induceOSR call
  // still will have an exception
  // edge to the OSR catch block.
  TR::SymbolReferenceTable *symRefTab = self()->comp()->getSymRefTab();
  TR::SymbolReference *induceOSRSymRef = symRefTab->findOrCreateRuntimeHelper(
      TR_induceOSRAtCurrentPC, true, true, true);
  // treat jitInduceOSR like an interpreted call so that each platform's codegen
  // generate a snippet for it
  ...
  induceOSRCallNode->getByteCodeInfo().setByteCodeIndex(
      point->getInduceByteCodeInfo().getByteCodeIndex());
  ...
  if (self()->supportsInduceOSR(induceBCI, block, NULL, self()->comp())) {
    ...
    if (remainderTree->getNode()->getOpCodeValue() != TR::BBEnd) {
      if (extendRemainder) {
        ...
        if (self()->comp()->getOption(TR_TraceOSR))
          traceMsg(self()->comp(), "  Split of block_%d at n%dn produced "
                                   "block_%d which is an extension\n",
                   block->getNumber(),
    ...
    self()->genInduceOSRCallAndCleanUpFollowingTreesImmediately(
        osrBlock->getExit(), induceBCI, false, false, self()->comp());
    return true;
  }
  return false;
}
...
TR::TreeTop *OMR::ResolvedMethodSymbol::induceImmediateOSRWithoutChecksBefore(
  ...
  if (self()->supportsInduceOSR(insertionPoint->getNode()->getByteCodeInfo(),
                                insertionPoint->getEnclosingBlock(), NULL,
                                self()->comp()))
    return self()->genInduceOSRCallAndCleanUpFollowingTreesImmediately(
        ...
        false, self()->comp());
  if (self()->comp()->getOption(TR_TraceOSR))
    traceMsg(self()->comp(), "induceImmediateOSRWithoutChecksBefore n%dn "
                             "failed - supportsInduceOSR returned false\n",
...
OMR::ResolvedMethodSymbol::genInduceOSRCallAndCleanUpFollowingTreesImmediately(
    TR::TreeTop *insertionPoint, TR_ByteCodeInfo induceBCI, bool copyChildren,
  ...
  TR::TreeTop *induceOSRCallTree = self()->genInduceOSRCall(
      insertionPoint, induceBCI.getCallerIndex(), numChildrenOfInduceCall,
      copyChildren, shouldSplitBlock);
  ...
  if (induceOSRCallTree) {
    ...
    if (refCallNode) {
      TR::Node *induceOSRCallNode =
          induceOSRCallTree->getNode()->getFirstChild();
...
TR::TreeTop *OMR::ResolvedMethodSymbol::genInduceOSRCallForGuardedCallee(
    TR::TreeTop *insertionPoint, TR::ResolvedMethodSymbol *calleeSymbol,
    int32_t numChildren, bool copyChildren, bool shouldSplitBlock) {
  if (self()->comp()->getOption(TR_TraceOSR)) {
    const char *mSignature =
        calleeSymbol->signature(self()->comp()->trMemory());
    traceMsg(self()->comp(), "induce %p generated for %s index %d\n",
             insertionPoint->getNode(), mSignature,
             insertionPoint->getNode()->getByteCodeInfo().getCallerIndex());
  }
  ...
  TR_OSRMethodData *osrMethodData =
      self()->comp()->getOSRCompilationData()->findCallerOSRMethodData(
          self()->comp()->getOSRCompilationData()->findOrCreateOSRMethodData(
              self()->comp()->getCurrentInlinedSiteIndex(), calleeSymbol));
  return self()->genInduceOSRCall(
      insertionPoint, self()->comp()->getCurrentInlinedSiteIndex(),
      osrMethodData, calleeSymbol, numChildren, copyChildren, shouldSplitBlock);
}
...
TR::TreeTop *OMR::ResolvedMethodSymbol::genInduceOSRCall(
  ...
  return self()->genInduceOSRCall(insertionPoint, inlinedSiteIndex,
                                  osrMethodData, NULL, numChildren,
                                  copyChildren, shouldSplitBlock);
}
...
TR::TreeTop *OMR::ResolvedMethodSymbol::genInduceOSRCall(
    ...
    TR_OSRMethodData *osrMethodData,
    TR::ResolvedMethodSymbol *callSymbolForDeadSlots, int32_t numChildren,
    bool copyChildren, bool shouldSplitBlock) {
  ...
  TR::TreeTop *induceOSRCallTree = self()->genInduceOSRCallNode(
      insertionPoint, numChildren, copyChildren, shouldSplitBlock);
...
void OMR::ResolvedMethodSymbol::genAndAttachOSRCodeBlocks(
  ...
  for (TR::TreeTop *tt = self()->getFirstTreeTop(); tt;
    ...
    // successor
    // which we are responsible for creating if we decide to continue with this
    // OSR point
    if (self()->comp()->isPotentialOSRPoint(NULL, tt->getNode())) {
      ...
      TR_OSRPoint *osrPoint = new (self()->comp()->trHeapMemory()) TR_OSRPoint(
          ttnode, osrOffset, osrMethodData, self()->comp()->trMemory());
      osrPoint->setOSRIndex(self()->addOSRPoint(osrPoint));
      if (self()->comp()->getOption(TR_TraceOSR))
        traceMsg(self()->comp(),
                 "osr point added for [%p] at %d:%d with induction offset %d\n",
  ...
  // if an OSR code block has been created, attach its treetops and osr catch
  // block's
...
void OMR::ResolvedMethodSymbol::genOSRHelperCall(
  ...
  if (self()->comp()->getCurrentInlinedSiteIndex() == -1) {
    ...
    if (!voidReturn) {
      TR::Node *retValNode = TR::Node::create(
          firstNode, self()->comp()->il.opCodeForConst(
  ...
  self()->comp()->getFlowGraph()->addEdge(
      OSRCodeBlock, self()->comp()->getFlowGraph()->getEnd());
}
...
bool OMR::ResolvedMethodSymbol::genIL(TR_FrontEnd *fe, TR::Compilation *comp,
  ...
  if (!_firstTreeTop || !comp->isPeekingMethod()) {
    ...
    if (comp->getOption(TR_EnableOSR) && !comp->isPeekingMethod()) {
      ...
      _shouldNotAttemptOSR = new (comp->trHeapMemory())
          TR_BitVector(1, comp->trMemory(), heapAlloc, growable);
    }
    ...
    //the sym refs anyway by using methodSymbol's auto sym refs and pending push
    //sym refs list; this only confuses analyses into thinking these autos
    //cannot be used when they actually can be
    ...
    if (_methodFlags.testAny(IlGenSuccess)) {
      ...
      // pending pushes,
      // parms, and locals alive. Only do this in non HCR mode since under HCR
      // we will be more selective about where to
      // add OSR points.
      //
      bool doOSR = comp->getOption(TR_EnableOSR) && !comp->isPeekingMethod() &&
                   (comp->getOption(TR_EnableNextGenHCR) ||
                    !comp->getOption(TR_EnableHCR));

      TR::Optimizer *optimizer = NULL;
      TR::Optimizer *previousOptimizer = NULL;
      if (doOSR || comp->getMethodHotness() >= cold) {
      ...
      if (doOSR) {
        TR_ASSERT(comp->getOSRCompilationData(),
                  "OSR compilation data is NULL\n");
        if (comp->canAffordOSRControlFlow()) {
          self()->genAndAttachOSRCodeBlocks(comp->getCurrentInlinedSiteIndex());
          if (!self()->getOSRPoints().isEmpty()) {
            self()->genOSRHelperCall(comp->getCurrentInlinedSiteIndex(),
                                     symRefTab);
          ...
          if (!comp->isOutermostMethod())
            self()->cleanupUnreachableOSRBlocks(
...
bool OMR::ResolvedMethodSymbol::supportsInduceOSR(
    TR_ByteCodeInfo bci, TR::Block *blockToOSRAt,
    TR::ResolvedMethodSymbol *calleeSymbolIfCallNode, TR::Compilation *comp,
    bool runCleanup) {
...
void OMR::ResolvedMethodSymbol::setShouldNotAttemptOSR(int32_t n) {
  _shouldNotAttemptOSR->set(n);
}

bool OMR::ResolvedMethodSymbol::cannotAttemptOSR(
    TR_ByteCodeInfo bci, TR::Block *blockToOSRAt,
    TR::ResolvedMethodSymbol *calleeSymbolIfCallNode, TR::Compilation *comp,
    bool runCleanup) {
  if (comp->getOption(TR_TraceOSR))
  ...
  if (bci.doNotProfile()) {
    ...
    return true; // if this is a call node created by the optimizer (string
                 // peepholes) it may not exist in the bytecodes
  }
  ...
  // safe - note that we only check
  // cannotOSR in this case as we only abort the OSR if correctness is in
  // jeopardy
  while (osrMethodData) {
    if (comp->getOption(TR_TraceOSR))
      traceMsg(comp, "Checking if OSR can be attempted at caller index %d and "
                     "bytecode index %d\n",
  ...
  if (comp->getMethodSymbol() != symToCheckOSRBlocksReachability &&
      !symToCheckOSRBlocksReachability->allCallerOSRBlocksArePresent(callSite,
                                                                     comp)) {
    if (runCleanup)
      symToCheckOSRBlocksReachability->cleanupUnreachableOSRBlocks(callSite,
                                                                   comp);
    if (comp->getOption(TR_TraceOSR))
  ...
  if (blockToOSRAt &&
    ...
    if (comp->getOption(TR_TraceOSR))
      traceMsg(comp, "Missing OSR exception successor block_%d for block_%d - "
                     "cannot OSR\n",
...
bool OMR::ResolvedMethodSymbol::allCallerOSRBlocksArePresent(
...
void OMR::ResolvedMethodSymbol::insertStoresForDeadStackSlotsBeforeInducingOSR(
  ...
  if (!comp->osrStateIsReliable()) {
    traceMsg(comp, "OSR state may not be reliable enough to trust liveness "
                   "info computed at IL gen time; so avoiding dead stack slot "
                   "store insertion\n");
    return;
  }
  ...
  if (callSymbolForDeadSlots)
    osrMethodData = comp->getOSRCompilationData()->findCallerOSRMethodData(
        comp->getOSRCompilationData()->findOrCreateOSRMethodData(
            inlinedSiteIndex, callSymbolForDeadSlots));
  else
    osrMethodData = comp->getOSRCompilationData()->findOrCreateOSRMethodData(
        callSite, self());
  ...
  while (osrMethodData) {
    if (comp->getOption(TR_TraceOSR))
      traceMsg(comp, "Inserting stores for dead stack slots in method at "
                     "caller index %d and bytecode index %d for induceOSR call "
                     "%p\n",
    ...
    if (deadSymRefs) {
      ...
      while (bvi.hasMoreElements()) {
        ...
        if (performTransformation(comp, "OSR LIVE RANGE ANALYSIS : Local %d is "
                                        "reset before induceOSR call tree %p "
                                        "(caller index %d bytecode index %d)\n",
                                  nextSymRef->getReferenceNumber(),
                                  induceOSRTree->getNode(), callSite,
                                  byteCodeIndex)) {
          TR::Node *storeNode = TR::Node::createWithSymRef(
              comp->il.opCodeForDirectStore(
                  nextSymRef->getSymbol()->getDataType()),
              1, 1,
              TR::Node::createConstDead(induceOSRTree->getNode(),
                                        nextSymRef->getSymbol()->getDataType()),
              nextSymRef);
...
TR_OSRPoint *OMR::ResolvedMethodSymbol::findOSRPoint(TR::Node *node) {
...
void OMR::ResolvedMethodSymbol::addMethodMetaDataSymbol(
    TR::RegisterMappedSymbol *s) {
  TR_ASSERT(!s || s->isMethodMetaData(), "should be method metadata");
  if (!_methodMetaDataList.find(s))
    _methodMetaDataList.add(s);
}

void OMR::ResolvedMethodSymbol::addTrivialDeadTreeBlock(TR::Block *b) {
    ...
    if (self()->comp()->getMethodSymbol() == self())
      ...
      _autoSymRefs =
          new (m->trHeapMemory()) TR_Array<List<TR::SymbolReference>>(
              m, _resolvedMethod->numberOfParameterSlots() +
...
void OMR::ResolvedMethodSymbol::removeTree(TR::TreeTop *tt) {
  ...
  if (prev != NULL)
    ...
    TR_ASSERT(_firstTreeTop == tt,
              "treetops are not forming a single doubly linked list");
    _firstTreeTop = next;
  }
  if (next != NULL)
        ...
        if (!excepSucc.empty()) {
          for (auto e = excepSucc.begin(); e != excepSucc.end(); ++e) {
            ...
            if (dest == node) {
              ...
              dumpOptDetails(comp, "Detected catch block with exception "
                                   "successor as itself %d\n",
                             node->getNumber());
              ...
              TR::Node *asyncNode = TR::Node::createWithSymRef(
                  ...
                  0, comp->getSymRefTab()->findOrCreateAsyncCheckSymbolRef(
...
void OMR::ResolvedMethodSymbol::removeUnusedLocals() {
  ...
  while (cursor) {
    ...
    // references were
    // removed as a result of coalescing (i.e. a store of a load of coalesced
    // autos that
