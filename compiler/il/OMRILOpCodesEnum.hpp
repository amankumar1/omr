...
FirstOMROp,
    ...
    wrtbar,      // direct write barrier store checks for new space in old space
                 // reference store
    ...
    astorei, // store indirect address                (child1 a dest, child2 a
             // value)
    wrtbari, // indirect write barrier store checks for new space in old space
             // reference store
    ...
    // reference.
    // But for array references, children 1 and 3 will be quite different
    // although
    ...
    iuternary,  // second or the third child.  Analogous to the "condition ? a :
                // b" operations in C/Java.
    luternary,  //
    buternary,  //
    suternary,  //
    ...
    trtLookup, // special lookupswitch (child1 must be trt, child2 the default
               // destination, subsequent children are case nodes)
    // The internal control flow is similar to lookup, but each CASE represents
    // a special semantics associated with a flag on it
    ...
    table, // tableswitch (child1 is the selector, child2 the default
           // destination, subsequent children are the branch targets
           // (the last child may be a branch table address, use
           // getCaseIndexUpperBound() when iterating over branch targets)
    exceptionRangeFence, // (J9) SymbolReference is the aliasing effect,
                         // initialiser is where the code address gets put when
                         // binary is generated
    // used for delimiting function, try blocks, catch clauses, finally clauses,
    // etc.
    ...
    OverflowCHK, // Overflow check. child 1 is the operation node(add, mul,
                 // sub). Child 2 and child 3 are the operands. Symbolref
                 // indicates failure action/destination
