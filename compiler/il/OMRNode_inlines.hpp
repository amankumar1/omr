...
TR_NodeUseAliasSetInterface OMR::Node::mayUse() {
  TR_NodeUseAliasSetInterface aliasSetInterface(self());
  return aliasSetInterface;
}

TR_NodeKillAliasSetInterface OMR::Node::mayKill(bool gcSafe) {
  TR_NodeKillAliasSetInterface aliasSetInterface(
      self(), self()->getOpCode().isCallDirect(), gcSafe);
  return aliasSetInterface;
}
...
inline TR::Node *OMR::Node::getFirstChild() {
  TR_ASSERT(self()->getNumChildren() >= 1,
            "getFirstChild() called on node %p with no children", this);
  return self()->getChild(0);
}

inline TR::Node *OMR::Node::getLastChild() {
  TR_ASSERT(self()->getNumChildren() >= 1,
            "getLastChild() called on node with no children");
  return self()->getChild(_numChildren - 1);
}

inline TR::Node *OMR::Node::getSecondChild() {
  TR_ASSERT(self()->getNumChildren() >= 2,
            "getSecondChild() called on node with less than 2 children()");
  return self()->getChild(1);
}

inline TR::Node *OMR::Node::getThirdChild() { return self()->getChild(2); }
...
vcount_t OMR::Node::incVisitCount() {
  ...
  TR_ASSERT(self()->getOptAttributes()->_visitCount > 0,
...
rcount_t OMR::Node::incReferenceCount() {
  ...
  TR_ASSERT(self()->getOptAttributes()->_referenceCount > 0,
            "Assertion failure : %s (0x%p)", self()->getOpCode().getName(),
            this);
  return self()->getOptAttributes()->_referenceCount;
}
...
scount_t OMR::Node::incLocalIndex() {
  ...
  TR_ASSERT(self()->getOptAttributes()->_localIndex > 0, "assertion failure");
  return self()->getOptAttributes()->_localIndex;
}
...
ncount_t OMR::Node::getNodePoolIndex() {
  return self()->getOptAttributes()->_poolIndex;
}

void OMR::Node::setNodePoolIndex(ncount_t i) {
