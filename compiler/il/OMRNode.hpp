...
protected:
  Node(TR::Node *originatingByteCodeNode, TR::ILOpCodes op,
       uint16_t numChildren, OptAttributes *optAttributes = NULL);
...
private:
  ...
  static bool isLegalCallToCreate(TR::ILOpCodes opvalue) {
    ...
    TR_ASSERT(opvalue != TR::tstart,
              "use createWithRoomForFive to create node: %s", opcode.getName());
...
public:
  ...
  // these call containsNode on each node found in the list or bitvector set of
  // Nodes
  bool containsAnyNode(List<TR::Node *> &nodeList, vcount_t visitCount);
  bool containsAnyNode(TR::SparseBitVector &nodeSet, vcount_t visitCount,
                       TR::Compilation *comp);
  ...
  /// is a composite operator, made from a high order part and its adjunct
  /// operator
  /// which is the first child of its third child. It returns true if the node
  /// has the form:
  ///
  ///     highOp
  ///       firstChild
...
protected:
  ...
  OptAttributes *getOptAttributes() {
    TR_ASSERT(_optAttributes != NULL, "_optAttributes is NULL!");
    return _optAttributes;
  }
...
protected:
   ...
   *
   * The OptAttribute class is meant contain fields of TR::Node which are
   * relevant
