...
void TR::NodePool::removeNode(NodeIndex poolIdx) {
  TR_ASSERT(_pool.Exists(poolIdx),
            "Node with Pool Index %d does not exist in the table", poolIdx);
...
TR::Node *TR::NodePool::getNodeAtPoolIndex(ncount_t poolIdx) {
  TR_ASSERT(_pool.Exists(poolIdx),
            "Node with Pool Index %d does not exist in the table", poolIdx);
  return &_pool.ElementAt(poolIdx);
}

TR::Node *TR::NodePool::allocate(ncount_t poolIndex) {
  if (!poolIndex) {
    poolIndex = _poolIndex = _pool.AddEntry();
    _globalIndex++;
    TR_ASSERT(_globalIndex < MAX_NODE_COUNT,
              "Reached TR::Node allocation limit");
  }
...
OMR::Node::OptAttributes *TR::NodePool::allocateOptAttributes() {
  ncount_t optAttribPoolIndex = _optAttribPool.AddEntryAtPosition(_poolIndex);
  TR_ASSERT(optAttribPoolIndex == _poolIndex,
            "Cannot eliminate variable _optAttribPoolIndex and use _poolIndex "
            "instead. Was hoping this was possible");
  _optAttribGlobalIndex++;
  OMR::Node::OptAttributes &oa = _optAttribPool.ElementAt(_poolIndex);
  return &oa;
}

void TR::NodePool::FreeOptAttributes() {
...
void TR::NodePool::removeNodeAndReduceGlobalIndex(TR::Node *node) {
  removeNode(node->getNodePoolIndex());
  _globalIndex--;
}
