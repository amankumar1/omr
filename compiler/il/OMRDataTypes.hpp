...
// smaller limits when deciding what operations to inline
// If these sizes are changed then the sizes in the Node::DecimalInfo bit
// container below may have to be changed
...
enum TR_RawBCDSignCode {
  ...
  raw_bcd_sign_0xd = 2,
  raw_bcd_sign_0xf = 3,
  last_raw_bcd_sign = raw_bcd_sign_0xf, // NOTE: the TR_RawBCDSignCode fields in
                                        // Node::DecimalInfo are 3 bits so the
                                        // max value for a raw_bcd_sign_* is 7
                                        // (enum range is -8->7)
...
// inherently
// platform-specific and non-portable.  The recommended way to coerce pointers
// is by
// chaining static casts and converting through void*.  For better readability
// and for
...
template <TR::DataTypes T> inline const int64_t getMinSigned() {
  static_assert(T == -1, "getMinSigned is not valid for this type");
  return 0;
}
template <TR::DataTypes T> inline const uint64_t getMinUnsigned() {
  static_assert(T == -1, "getMinUnsigned is not valid for this type");
  return 0;
}
template <TR::DataTypes T> inline const int64_t getMaxSigned() {
  static_assert(T == -1, "getMaxSigned is not valid for this type");
  return 0;
}
template <TR::DataTypes T> inline const uint64_t getMaxUnsigned() {
  static_assert(T == -1, "getMaxUnsigned is not valid for this type");
  return 0;
}
template <TR::DataTypes T> inline const int32_t getMaxSignedPrecision() {
  static_assert(T == -1, "getMaxSignedPrecision is not valid for this type");
  return 0;
}
template <TR::DataTypes T> inline const int32_t getMaxUnsignedPrecision() {
...
template <> inline const int64_t getMinSigned<TR::Int32>() {
  return ((int32_t)0X80000000);
}
template <> inline const int64_t getMinSigned<TR::Int64>() {
...
template <> inline const uint64_t getMinUnsigned<TR::Int32>() {
  return ((uint32_t)0);
}
template <> inline const uint64_t getMinUnsigned<TR::Int64>() {
  return ((uint64_t)0);
}
...
template <> inline const int64_t getMaxSigned<TR::Int32>() {
  return ((int32_t)0X7FFFFFFF);
}
template <> inline const int64_t getMaxSigned<TR::Int64>() {
  return (
...
template <> inline const uint64_t getMaxUnsigned<TR::Int32>() {
  return ((uint32_t)0xFFFFFFFF);
}
template <> inline const uint64_t getMaxUnsigned<TR::Int64>() {
  return ((uint64_t)-1);
}

template <> inline const int32_t getMaxSignedPrecision<TR::Int8>() { return 3; }
template <> inline const int32_t getMaxSignedPrecision<TR::Int16>() {
  return 5;
}
template <> inline const int32_t getMaxSignedPrecision<TR::Int32>() {
  return 10;
}
template <> inline const int32_t getMaxSignedPrecision<TR::Int64>() {
  return 19;
}

template <> inline const int32_t getMaxUnsignedPrecision<TR::Int8>() {
  return 3;
}
template <> inline const int32_t getMaxUnsignedPrecision<TR::Int16>() {
  return 5;
}
template <> inline const int32_t getMaxUnsignedPrecision<TR::Int32>() {
  return 10;
}
template <> inline const int32_t getMaxUnsignedPrecision<TR::Int64>() {
  return 20;
}
...
public:
  ...
  bool isVector() {
    return getDataType() == TR::VectorInt8 ||
           getDataType() == TR::VectorInt16 ||
           getDataType() == TR::VectorInt32 ||
           getDataType() == TR::VectorInt64 ||
           getDataType() == TR::VectorFloat ||
           getDataType() == TR::VectorDouble;
  }
  bool isBFPorHFP() {
    return getDataType() == TR::Float || getDataType() == TR::Double;
  }
