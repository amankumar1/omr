...
bool OMR::Compilation::nodeNeeds2Regs(TR::Node *node) {
  return (node->getType().isInt64() && TR::Compiler->target.is32Bit() &&
...
uint8_t *OMR::Compilation::getNOPTranslateTable() { return _NOPTranslateTable; }
...
OMR::Compilation::Compilation(int32_t id, OMR_VMThread *omrVMThread,
                              ...
                              TR::Options &options,
                              const TR::Region &dispatchRegion, TR_Memory *m,
                              TR_OptimizationPlan *optimizationPlan)
    : _trMemory(m), _knownObjectTable(NULL),
      _fe((firstCompileStarted = true, fe)), _ilGenRequest(ilGenRequest),
      _options(&options), _omrVMThread(omrVMThread), _currentOptIndex(0),
      _lastBegunOptIndex(0), _lastPerformedOptIndex(0),
      _currentOptSubIndex(0), // The transformation index within the current opt
      _lastPerformedOptSubIndex(0), _debug(0), _region(dispatchRegion),
      _compThreadID(id), _allocator(TRCS2MemoryAllocator(m)),
      _arenaAllocator(TR::Allocator(self()->allocator("Arena"))),
      _allocatorName(NULL), _method(compilee), _firstInstruction(NULL),
      _appendInstruction(NULL), _firstColdInstruction(NULL),
      _returnInfo(TR_VoidReturn), _methodSymbols(m, 10), _visitCount(0),
      _nodeCount(0), _accurateNodeCount(0), _lastValidNodeCount(0),
      _maxInlineDepth(0), _numNestingLevels(0), _numLivePendingPushSlots(0),
      _ilGenerator(0), _optimizer(0),
      _usesPreexistence(options.getOption(TR_ForceUsePreexistence)),
      _loopVersionedWrtAsyncChecks(false), _nextOptLevel(unknownHotness),
      _errorCode(COMPILATION_SUCCEEDED), _codeCacheSwitched(false),
      _commitedCallSiteInfo(false), _currentSymRefTab(NULL),
      _peekingSymRefTab(NULL), _recompilationInfo(0), _primaryRandom(NULL),
      _adhocRandom(NULL), _toNumberMap(self()->allocator("toNumberMap")),
      _toStringMap(self()->allocator("toStringMap")),
      _toCommentMap(self()->allocator("toCommentMap")),
      _signature(compilee->signature(m)), _containsBigDecimalLoad(false),
      _useLongRegAllocation(false), _resolvedMethodSymbolReferences(m),
      _inlinedCallSites(m), _monitorAutos(m), _peekingArgInfo(m),
      _inlinedCallStack(m), _inlinedCallArgInfoStack(m), _inlinedCalls(0),
      ...
      _snippetsToBePatchedOnClassRedefinition(
          getTypedAllocator<TR::Snippet *>(self()->allocator())),
      _snippetsToBePatchedOnRegisterNative(
          getTypedAllocator<TR_Pair<TR::Snippet, TR_ResolvedMethod> *>(
              self()->allocator())),
      ...
      _noEarlyInline(true), _optimizationPlan(optimizationPlan),
      _verboseOptTransformationCount(0), _currentBlock(NULL),
      _unreferencedAutoOrParmOrExternals(
          self()->allocator("unreferenced external or auto or Parm")),
      _externalSparseBitVector(self()->allocator("external")),
      _monitorAutoSymRefsInCompiledMethod(
          getTypedAllocator<TR::SymbolReference *>(self()->allocator())),
      _aotMethodCodeStart(NULL), _failCHtableCommitFlag(false),
      _numReservedIPICTrampolines(0), _osrStateIsReliable(true),
      ...
      _prevSymRefTabSize(0), _compilationNodes(NULL), _nodeOpCodeLength(0),
      _gpuPtxList(m), _gpuKernelLineNumberList(m), _gpuPtxCount(0),
      _scratchSpaceLimit(TR::Options::_scratchSpaceLimit),
      _cpuTimeAtStartOfCompilation(-1), _bitVectorPool(self()),
      _tlsManager(*self()) {
  _aotClassInfo = new (m->trHeapMemory()) TR::list<TR::AOTClassInfo *>(
      getTypedAllocator<TR::AOTClassInfo *>(self()->allocator()));

  if (TR::isJ9()) {
    _ObjectClassPointer =
        fe->getClassFromSignature("Ljava/lang/Object;", 18, _method);
    _RunnableClassPointer =
        fe->getClassFromSignature("Ljava/lang/Runnable;", 20, _method);
    _StringClassPointer =
        fe->getClassFromSignature("Ljava/lang/String;", 18, _method);
    _SystemClassPointer =
        fe->getClassFromSignature("Ljava/lang/System;", 18, _method);
    _ReferenceClassPointer =
        fe->getClassFromSignature("Ljava/lang/ref/Reference;", 25, _method);
    _JITHelpersClassPointer =
        fe->getClassFromSignature("Lcom/ibm/jit/JITHelpers;", 24, _method);
  } else {
    _ObjectClassPointer = _RunnableClassPointer = _StringClassPointer =
        _SystemClassPointer = _ReferenceClassPointer = _JITHelpersClassPointer =
            NULL;
  }
  ...
  if (!TR::Options::getCmdLineOptions()->getOption(
          TR_DisableFastAssumptionReclamation))
    _metadataAssumptionList =
        new (m->trPersistentMemory()) TR_SentinelRuntimeAssumption();
#endif
  ...
  if (_options->getOption(TR_EnableOSR)) {
    ...
    if (((self()->getMethodHotness() < warm) ||
        ...
        !_options->getOption(TR_FullSpeedDebug)) // Off for two reasons : 1) not
                                                 // sure if we can afford the
                                                 // increase in compile time due
                                                 // to the extra OSR control
                                                 // flow at cold and 2) not sure
                                                 // at this stage in 727 whether
                                                 // OSR can work with AOT (will
                                                 // try to find out soon) but
...
TR_Hotness OMR::Compilation::getDeFactoHotness() {
...
bool OMR::Compilation::canAffordOSRControlFlow() {
  if (self()->getOption(TR_DisableOSR) || !self()->getOption(TR_EnableOSR)) {
    if (self()->getOption(TR_TraceOSR)) {
      traceMsg(self(), "canAffordOSRControlFlow returning false due to OSR "
                       "options: disableOSR %d, enableOSR %d\n",
               self()->getOption(TR_DisableOSR),
               self()->getOption(TR_EnableOSR));
    }
    return false;
  }
...
bool OMR::Compilation::supportsInduceOSR() {
  ...
  if (self()->getOption(
          TR_MimicInterpreterFrameShape) /* && areSlotsSharedByRefAndNonRef() */) {
    if (self()->getOption(TR_TraceOSR))
  ...
  if (_osrCompilationData &&
    ...
    if (self()->getOption(TR_TraceOSR))
      traceMsg(self(), "Cannot guarantee OSR transfer of control to the "
                       "interpreter will work for calls preventing induced OSR "
                       "(e.g. Quad) because of differences in JIT vs "
...
bool OMR::Compilation::isPotentialOSRPoint(TR::TreeTop *tt, TR::Node *ttNode) {
  ...
  if (potentialOSRPoint && !self()->getOption(TR_FullSpeedDebug)) {
    ...
    potentialOSRPoint = method->supportsInduceOSR(
        bci, tt ? tt->getEnclosingBlock() : NULL, NULL, self(), false);
  }
...
TR_OpaqueClassBlock *OMR::Compilation::getClassClassPointer() {
  return _ObjectClassPointer
             ? self()->fe()->getClassClassPointer(_ObjectClassPointer)
             : 0;
}
...
int32_t OMR::Compilation::compile() {
    ...
    if ((self()->getMethodHotness() <= warm)) {
      ...
      if (TR::Compiler->target.cpu.isX86())
        TR::Options::getCmdLineOptions()->setOption(
            TR_DisableLateEdgeSplitting); // TR_DisableLateEdgeSplitting only
                                          // used on X86
    }
  ...
  if (self()->getOptions()->getDelayCompile()) {
    ...
    fprintf(stderr, "\nDelayCompile: Starting a delay of length %d for method "
                    "%s at time %d.",
  ...
  TR_PersistentMethodInfo *methodInfo =
      TR_PersistentMethodInfo::get(self()->getCurrentMethod());
  if (methodInfo && self()->isProfilingCompilation())
      ...
      if (self()->getOptions()->getVerboseOption(TR_VerboseInlining)) {
        ...
        for (int32_t i = 0; i < self()->getNumInlinedCallSites(); i++) {
          ...
          TR_VerboseLog::writeLine(
              ...
              self()->fej9()->getMethodSize(site._methodInfo), calleeBuf);
        }
            ...
            if (node->getOpCode().isCall()) {
              ...
              if (method) {
                ...
                if (node->getSymbol()->getResolvedMethodSymbol())
                  calleeSize = self()->fej9()->getMethodSize(
                      node->getSymbol()
...
int32_t OMR::Compilation::performOptimizations() {
  ...
  // This opt is needed if certain ilgen input is seen but there is no optimizer
  // created at this point.
  // So the block are tracked during ilgen and the opt is turned on here.
  //
  ListIterator<TR::Block> listIt(
      &_methodSymbol->getTrivialDeadTreeBlocksList());
  for (TR::Block *block = listIt.getFirst(); block; block = listIt.getNext()) {
    ((TR::Optimizer *)(_optimizer))
        ->setRequestOptimization(OMR::trivialDeadTreeRemoval, true, block);
  }

  if (_methodSymbol->hasUnkilledTemps()) {
    ((TR::Optimizer *)(_optimizer))
        ->setRequestOptimization(OMR::globalDeadStoreElimination);
  }
...
bool OMR::Compilation::incInlineDepth(TR_OpaqueMethodBlock *methodInfo,
  ...
  // callerIndex in TR_ByteCodeInfo
  // For example, in Java TR_ByteCodeInfo::maxCallerIndex is set to 4095 (12
  // bits and one used for signness)
  ...
  // callstack
  // for all the callsites with the caller indices != -1 to make sure each catch
  // block
  // can be uniquely identified by a (depthIndex, handlerIndex) pair.
  // otherwise JVM might print out an incorrect stacktrace if an exception is
  // thrown
...
OMR::Compilation::restoreInlineDepthUntil(int32_t stopIndex,
  ...
  _inlinedCallArgInfoStack.push(NULL); // We have no argInfo for
                                       // previously-inlined frames, though
...
void OMR::Compilation::addMonitorAuto(TR::RegisterMappedSymbol *a,
                                      int32_t callerIndex) {
  List<TR::RegisterMappedSymbol> *autos = _monitorAutos[callerIndex + 1];
  if (!autos)
    _monitorAutos[callerIndex + 1] = autos = new (self()->trHeapMemory())
        List<TR::RegisterMappedSymbol>(self()->trMemory());

  autos->add(a);
}

void OMR::Compilation::addAsMonitorAuto(TR::SymbolReference *symRef,
                                        bool dontAddIfDLT) {
  symRef->getSymbol()->setHoldsMonitoredObject();
  int32_t siteIndex = self()->getCurrentInlinedSiteIndex();
  if (!self()->isPeekingMethod()) {
    self()->addMonitorAuto(symRef->getSymbol()->castToRegisterMappedSymbol(),
                           siteIndex);
    if (!dontAddIfDLT) {
      if (siteIndex == -1)
        self()->getMonitorAutoSymRefsInCompiledMethod()->push_front(symRef);
    } else {
      // only add the symref into the list for initialization when not in DLT
      // and not peeking.
      // in DLT, we already use the corresp. slot to store the locked object
      // from the interpreter
      // so initializing the symRef later in the block can overwrite the first
      // store.
...
TR_VirtualGuard *OMR::Compilation::findVirtualGuardInfo(TR::Node *guardNode) {
  ...
  if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
    traceMsg(self(), "Looking for a guard for node %p with kind %d bcindex %d "
                     "calleeindex %d\n",
             guardNode, guardKind,
...
int32_t OMR::Compilation::getOptMessageIndex() {
  return (self()->getOptimizer() ? self()->getOptimizer()->getOptMessageIndex()
                                 : 0);
}
...
TR::CodeCache *OMR::Compilation::getCurrentCodeCache() {
  return _codeGenerator ? _codeGenerator->getCodeCache() : 0;
}

void OMR::Compilation::setCurrentCodeCache(TR::CodeCache *codeCache) {
  if (_codeGenerator)
    _codeGenerator->setCodeCache(codeCache);
}

void OMR::Compilation::switchCodeCache(TR::CodeCache *newCodeCache) {
  TR_ASSERT(self()->getCurrentCodeCache() != newCodeCache,
            "Attempting to switch to the currently held code cache");
  self()->setCurrentCodeCache(newCodeCache); // Even if we signal, we need to
                                             // update the reserved code cache
                                             // for recompilations.
...
void OMR::Compilation::verifyTrees(TR::ResolvedMethodSymbol *methodSymbol) {
  if (self()->getDebug() &&
      !self()->getOptions()->getOption(TR_DisableVerification) &&
...
bool OMR::Compilation::notYetRunMeansCold() {
  ...
  TR_ResolvedMethod *currentMethod =
      self()->getJittedMethodSymbol()->getResolvedMethod();
  ...
  if (currentMethod->containingClass() == self()->getStringClassPointer()) {
    if (currentMethod->isConstructor()) {
      ...
      if (!strncmp(sig, "([CIIII)", 8) || !strncmp(sig, "([CIICII)", 9) ||
          !strncmp(sig, "(II[C)", 6))
        initialCount = 0;
    } else {
  ...
  if (self()->isDLT() || (initialCount < TR_UNRESOLVED_IMPLIES_COLD_COUNT) ||
      ((self()->getOption(TR_UnresolvedAreNotColdAtCold) &&
...
void OMR::Compilation::setReturnInfo(TR_ReturnInfo i) {
  ...
  if (_method->isConstructor() &&
      TR::Compiler->cls.hasFinalFieldsInClass(self(),
                                              _method->containingClass())) {
...
TR::HCRMode OMR::Compilation::getHCRMode() {
  ...
  return self()->getOption(TR_EnableOSR) &&
                 self()->getOption(TR_EnableNextGenHCR)
             ? TR::osr
             : TR::traditional;
}
...
#if defined(DEBUG)
#if LINUX
  // glibc allocates something at dl_init; check if a method is being compiled
  // to avoid
