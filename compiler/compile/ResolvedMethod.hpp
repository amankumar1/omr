...
public:
  ...
  virtual TR::DataType parmType(uint32_t parmNumber); // returns the type of the
                                                      // parmNumber'th parameter
                                                      // (0-based)
  ...
  virtual int32_t packedArrayFieldLength(int32_t cpIndex,
                                         TR::Compilation *comp);
  TR_OpaqueClassBlock *
  ...
  // C++ pattern.
  // Namely, we would like to have the same default arg for every override of
  // genMethodILForPeeking.
  // Unfortunately, Consistently setting the default arg for every override is
  // error-prone,
  // so what we do instead is to create a non-virtual genMethodILForPeeking with
  // a default arg
  // that calls a virtual counterpart.
  // This approach allows us to mantain the def arg invariant across all the
  // overrides.
