 ...
 * symbol is
 *    live at that offset. This is needed so that we can popluate the OSR
 * buffer.
 *
 ...
 * inlined
 * methods.  It's a basic block that initially (i.e., during ILGen) contains a
 * call
 * to `prepareForOSR` which has all the autos and pending push temps of its
 * corresponding method as its children. Eventually, i.e., in `CodeGenPrep.cpp`,
 * the
 ...
 *    symbol that is live at that offset. The symbol is read from the OSR
 * scratch
 *    buffer and written to the OSR buffer.
 *
 * During inlining, we link OSR code blocks in the sense that, each OSR code
 * block
 * will have a goto to the OSR code block of its caller. The OSR code block of
 * the
 ...
 * symref, the sequence contains only one element, otherwise, the sequence
 * contains
...
public:
   ...
   * information that
   * that enables to copy values of symbols that share slot(s) from OSR scratch
   * buffer to OSR buffer.
   * This information is deduced from live symbols at that instruction pc.
   * More formally, it represents a pair (i, s) where i is an instruction PC and
   * s is an array
...
private:
  typedef TR_Array<TR_Instruction2SharedSlotMapEntry>
...
private:
  ...
  TR::ResolvedMethodSymbol *methodSymbol; // NOTE: This field must appear first,
                                          // because its comp() is used to
                                          // initialize other fields

  /// a mapping from shared slot indices to an array of offsets in the scratch
  /// buffer.
  /// For each shared slot, there is a set of two or more syms that share that
  /// slot. The elements
  /// of the array above is the same as this set. The elements of the array are
  /// indexed by their symRefOrder.
  ...
  /// falls through to the osr code block.
  /// All incoming exception edges (from normal blocks) should go to the osr
  /// catch block and
  /// all incoming normal edges (from callees code blocks) should go to the osr
  /// code block.
...
public:
  ...
  TR_OSRPoint(TR::Node *node, int32_t induceOffset,
  ...
  bool induceAfter() {
    return _induceBCInfo.getByteCodeIndex() > _nodeBCInfo.getByteCodeIndex();
  }
