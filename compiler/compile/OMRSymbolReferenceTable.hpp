...
public:
  ...
  enum CommonNonhelperSymbol {
    ...
    pythonTypeIteratorMethodSymbol, // used for the iterator method slot from a
                                    // python type
  ...
  int32_t getArrayletShadowIndex(TR::DataType t) {
    return _numHelperSymbols + firstArrayShadowSymbol + TR::NumTypes + t;
  }
  ...
  TR::SymbolReference *createRuntimeHelper(TR_RuntimeHelper, bool = false,
                                           bool = false,
                                           bool preservesAllRegisters = false);
  TR::SymbolReference *findOrCreateRuntimeHelper(TR_RuntimeHelper, bool, bool,
                                                 bool preservesAllRegisters);

  TR::ParameterSymbol *
  createParameterSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol,
                        int32_t slot, TR::DataType, bool isUnsigned);
  TR::SymbolReference *
  ...
  TR::SymbolReference *
  createCoDependententTemporary(TR::ResolvedMethodSymbol *owningMethodSymbol,
                                TR::DataType, bool isInternalPointer,
                                size_t size, TR::Symbol *coDependent,
                                int32_t offset);
  ...
  TR::SymbolReference *createKnownStaticRefereneceSymbolRef(
  ...
  TR::SymbolReference *findOrCreateMonitorExitSymbolRef(
      TR::ResolvedMethodSymbol *owningMethodSymbol);
  TR::SymbolReference *findOrCreateNewStructHeaderSymbolRef(
      TR::ResolvedMethodSymbol *owningMethodSymbol = NULL);

  // Z
  TR::SymbolReference *
  findDLPStaticSymbolReference(TR::SymbolReference *staticSymbolReference);
  TR::SymbolReference *findOrCreateDLPStaticSymbolReference(
  ...
  TR::SymbolReference *createRefinedArrayShadowSymbolRef(
      TR::DataType, TR::Symbol *); // TODO: to be changed to a special sym ref
  ...
  // register or a value we would like the register allocator to map to a
  // register.
  // Such a RegisterSymbol belongs to a method as its conceptual life span is
  // the
  // life of a procedure. A set of hardware registers is pre-created for each
  // method.
  //
  void initRegisterSymbols(TR::ResolvedMethodSymbol *);
  TR::SymbolReference *createRegisterSymbol(TR::ResolvedMethodSymbol *,
                                            TR_RegisterKinds, TR::DataType,
...
protected:
  TR::SymbolReference *findOrCreateCPSymbol(TR::ResolvedMethodSymbol *, int32_t,
                                            TR::DataType, bool, void *);
  ...
  CS2::HashTable<uint32_t, TR::Symbol *, TR::Allocator>
      _aggregateShadowSymbolMap;
  CS2::HashTable<TR::SparseBitVector *, TR::SymbolReference *, TR::Allocator>
