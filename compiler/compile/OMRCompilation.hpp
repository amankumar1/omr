...
// argument)
// to the dll.  The use of these macros should ensure that the string is not
// added to
// the dll.
//
#if defined(_MSC_VER)
...
#define dumpOptDetails(comp, msg, ...)                                         \
  (TRACE_OPT_DETAILS(comp)                                                     \
       ? (comp)->getDebug()->performTransformationImpl(false, msg,             \
                                                       __VA_ARGS__)            \
       : true)
#define performTransformation(comp, msg, ...)                                  \
  (TRACE_OPT_DETAILS(comp)                                                     \
...
public:
  ...
  Compilation(int32_t compThreadId, OMR_VMThread *omrVMThread, TR_FrontEnd *,
              ...
              const TR::Region &dispatchRegion, TR_Memory *,
  ...
  TR::Instruction *setFirstColdInstruction(TR::Instruction *fi) {
    return (_firstColdInstruction = fi);
  }
  ...
  TR::list<TR_Pair<TR::Snippet, TR_ResolvedMethod> *> *
  getSnippetsToBePatchedOnRegisterNative() {
  ...
  TR_OpaqueClassBlock *getReferenceClassPointer() {
    return _ReferenceClassPointer;
  }
  TR_OpaqueClassBlock *getJITHelpersClassPointer() {
    return _JITHelpersClassPointer;
  }
  TR_OpaqueClassBlock *getClassClassPointer();

  TR_Array<List<TR::RegisterMappedSymbol> *> &getMonitorAutos() {
    return _monitorAutos;
  }
   ...
   * be distilled from that. Supplying a node directly is a very special
   * operation
  ...
  TR::SparseBitVector &getUnreferencedAutoOrParmOrExternals() {
    return _unreferencedAutoOrParmOrExternals;
  }
  TR::SparseBitVector &getExternalSparseBitVector() {
    return _externalSparseBitVector;
  }
...
public:
  ...
  TR_RuntimeAssumption **getMetadataAssumptionList() {
    return &_metadataAssumptionList;
  }
  void setMetadataAssumptionList(TR_RuntimeAssumption *a) {
...
protected:
  ...
  TR_RuntimeAssumption *_metadataAssumptionList; // A special
                                                 // TR_RuntimeAssumption to play
                                                 // the role of a sentinel for a
                                                 // linked list
#endif
...
private:
  ...
  // optimization trees
  // it gets set to 0 in TR_Debug::printBasicPreNodeInfoAndIndent() which
  // preceeds node opCode printouts. Any methods that print parts of node
  // opCodes should
  // always increment this as there may be information following opCodes that
  // need to be aligned as in the case of trees or in future development.
