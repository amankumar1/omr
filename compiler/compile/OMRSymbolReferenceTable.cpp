...
OMR::SymbolReferenceTable::SymbolReferenceTable(size_t sizeHint,
  ...
  _genericCallHash =
      new (trHeapMemory()) TR_HashTab(comp->trMemory(), heapAlloc, 60);
}
...
TR::SymbolReference *
OMR::SymbolReferenceTable::findOrCreateNewStructHeaderSymbolRef(
...
OMR::SymbolReferenceTable::findOrCreateOSRReturnAddressSymbolRef() {
  if (!element(osrReturnAddressSymbol)) {
    ...
    // We can't let the load/store of the exception symbol swing down
    aliasBuilder.addressStaticSymRefs().set(
        getNonhelperIndex(osrReturnAddressSymbol)); // add the symRef to the
                                                    // statics list to get
...
OMR::SymbolReferenceTable::createRefinedArrayShadowSymbolRef(
  ...
  return createRefinedArrayShadowSymbolRef(type,
                                           (TR::Symbol *)symRef->getSymbol());
}
...
OMR::SymbolReferenceTable::createRefinedArrayShadowSymbolRef(TR::DataType type,
  ...
  newRef->setCPIndex(-1);
  aliasBuilder.arrayElementSymRefs().set(index); // this ensures the newly
                                                 // created shadow can still be
                                                 // aliased to all other array
                                                 // shadows
...
TR::SymbolReference *
OMR::SymbolReferenceTable::createKnownStaticRefereneceSymbolRef(
...
TR::SymbolReference *OMR::SymbolReferenceTable::createRuntimeHelper(
  ...
  TR::MethodSymbol *methodSymbol =
      TR::MethodSymbol::create(trHeapMemory(), TR_Helper);
 ...
 * OMR::SymbolReference::getUseonlyAliasesBV.
 * we want to make sure defs are not moved across the overflowCHK in case an
...
TR::SymbolReference *OMR::SymbolReferenceTable::methodSymRefFromName(
  ...
  TR_OpaqueMethodBlock *method = fe()->getMethodFromName(
      className, methodName, methodSignature,
      comp()->getCurrentMethod()->getNonPersistentIdentifier());
  TR_ASSERT(method, "methodSymRefFromName: method must exist: %s.%s%s",
            className, methodName, methodSignature);
  TR_ASSERT(kind != TR::MethodSymbol::Virtual,
            "methodSymRefFromName doesn't support virtual methods"); // Until
                                                                     // we're
                                                                     // able to
                                                                     // look up
                                                                     // vtable
...
OMR::SymbolReferenceTable::findOrCreateSymRefWithKnownObject(
  ...
  TR::SymbolReference *result = new (trHeapMemory())
      TR::SymbolReference(self(), *original, 0, objectIndex);
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateAsyncCheckSymbolRef(
  ...
  return self()->findOrCreateRubyHelperSymbolRef(
      RubyHelper_rb_threadptr_execute_interrupts, true, /*canGCandReturn*/
      true,                                             /*canGCandExcept*/
...
TR::SymbolReference *OMR::SymbolReferenceTable::findDLPStaticSymbolReference(
    TR::SymbolReference *staticSymbolReference) {
...
TR::SymbolReference *
OMR::SymbolReferenceTable::findOrCreateDLPStaticSymbolReference(
    TR::SymbolReference *staticSymbolReference) {
  ...
  if (!staticSymbolReference->isUnresolved()) {
    for (symRef = i.getFirst(); symRef; symRef = i.getNext())
      if (symRef->getSymbol()->getStaticSymbol()->getStaticAddress() ==
          staticSymbolReference->getSymbol()
              ->getStaticSymbol()
              ->getStaticAddress())
        return symRef;
  }
  ...
  TR::StaticSymbol *sym = TR::StaticSymbol::create(trHeapMemory(), TR::Address);
  sym->setStaticAddress(staticSymbolReference->getSymbol()
                            ->getStaticSymbol()
                            ->getStaticAddress());
  sym->setNotCollected();
  symRef = new (trHeapMemory())
      TR::SymbolReference(self(), *staticSymbolReference, 0);
  symRef->setSymbol(sym);
  int32_t flags =
      staticSymbolReference->getSymbol()->getStaticSymbol()->getFlags();
...
void OMR::SymbolReferenceTable::initRegisterSymbols(
    TR::ResolvedMethodSymbol *owningMethodSymbol) {
  int32_t i, nr;
  ...
  for (i = 0; i < nr; i++) {
    TR::SymbolReference *symRef = createRegisterSymbol(
        owningMethodSymbol, machine->getRealRegister(i)->getKind(), TR::NoType,
        i);
    TR::AutomaticSymbol *sym = symRef->getSymbol()->castToRegisterSymbol();
    sym->setRealRegister();
    _registerSymbolRefs->operator[](i) =
        symRef; // FIXME: Why doesn't this just use the operator directly?
  }

#if defined(TR_TARGET_S390)
  // Let machine/OS specific setup happen on all these real register symbols we
  // just created
  comp()->cg()->registerSymbolSetup();
#endif
}

TR::SymbolReference *OMR::SymbolReferenceTable::createRegisterSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, TR_RegisterKinds regKind,
    TR::DataType dataType, TR_GlobalRegisterNumber grn) {
  ...
  mcount_t owningMethodIndex =
      owningMethodSymbol != NULL ? owningMethodSymbol->getResolvedMethodIndex()
                                 : noMethod;
  if (dataType != TR::NoType)
    dt = dataType;
  TR::Symbol *sym = TR::AutomaticSymbol::createRegisterSymbol(
      trHeapMemory(), regKind, grn, dt, size, fe());
  TR::SymbolReference *symRef = new (trHeapMemory())
      TR::SymbolReference(self(), sym, owningMethodIndex, 0 /* slot */);

  return symRef;
}

TR::SymbolReference *
OMR::SymbolReferenceTable::getRegisterSymbol(TR_GlobalRegisterNumber grn) {
  if (!_registerSymbolRefs)
    initRegisterSymbols(comp()->getMethodSymbol());
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateClassSymbol(
  ...
  // created for stores to statics.
  // In the latter case a load is created for the class the static is defined in
  // to be used by the write barrier code.
  // For explicit references to classes the cpIndex passed into this routine
  // refers to a class.  For the implicit
  // reference resulting from a store to a static the cpIndex passed into this
  // routine is for the static.
  //
  // If the cpindex stored in the symref correspond to a static then we set the
  // 'addressIsCPIndexOfStatic' bit.
  // This bit is used for unresolved references to call a specific resolve
  // helper that understand that the cpIndex
  ...
  // previously created symbol references contains
  // the same resolved address.  As a result we can end up sharing symbol
  // references between an explicit reference to the class
  // and an implicit reference used on a write barrier node.   In such a case we
  // make sure that the shared symref uses the
  // cpindex for the explicit reference and has the 'addressIsCPIndexOfStatic'
  // set to false.
  //
  ...
  if (fej9->getSupportsRecognizedMethods()) {
    TR_ASSERT(cpIndex != -1 || !comp()->compileRelocatableCode(),
              "we missed one of the cases where enabling recognized methods "
              "triggers a creation of a classSymbol with CPI of -1");
  }
#endif
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateMethodSymbol(
  ...
  if (comp()->getHasMethodHandleInvoke() && resolvedMethod &&
      resolvedMethod->owningMethodDoesntMatter()) // Any symbol with the same
                                                  // TR_OpaqueMethodBlock will
                                                  // do
  {
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateAutoSymbol(
  ...
  for (; symRef; symRef = i.getNext())
    ...
    // coerced
    {
      ...
      if (symRef->getSymbol()->getParmSymbol() ||
        ...
        traceMsg(comp(), "stack mapping can't handle a parameter slot that is "
                         "shared between refs and nonrefs 0");
    ...
    for (; symRef2; symRef2 = i2.getNext()) {
      if (comp()->getOption(TR_MimicInterpreterFrameShape) &&
        ...
        if (symRef2->getSymbol()->getParmSymbol() ||
            comp()->getOption(TR_DontJitIfSlotsSharedByRefAndNonRef)) {
          traceMsg(comp(), "stack mapping can't handle a parameter slot that "
                           "is shared between refs and nonrefs 1");
          throw TR::CompilationException();
        }
        slotSharedbyRefAndNonRef = true;
    ...
    for (; symRef2; symRef2 = i2.getNext()) {
      if (comp()->getOption(TR_MimicInterpreterFrameShape) &&
        ...
        if (symRef2->getSymbol()->getParmSymbol() ||
            comp()->getOption(TR_DontJitIfSlotsSharedByRefAndNonRef)) {
          traceMsg(comp(), "stack mapping can't handle a parameter slot that "
                           "is shared between refs and nonrefs 3");
          throw TR::CompilationException();
        }
        slotSharedbyRefAndNonRef = true;
        symRef2->getSymbol()->setSlotSharedByRefAndNonRef(
  ...
  // the bytecodes (Walker code)
  // This routine does not consider slot number and so we are disabling it to
  // avoid issues when we need to
...
static bool parmSlotCameFromExpandingAnArchetypeArgPlaceholder(
    int32_t slot, TR::ResolvedMethodSymbol *sym, TR_Memory *mem) {
...
TR::ParameterSymbol *OMR::SymbolReferenceTable::createParameterSymbol(
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t slot,
    TR::DataType type, bool isUnsigned) {
  TR::ParameterSymbol *sym =
      TR::ParameterSymbol::create(trHeapMemory(), type, isUnsigned, slot);
  ...
  if (!parmSlotCameFromExpandingAnArchetypeArgPlaceholder(
          slot, owningMethodSymbol, trMemory()))
...
TR::SymbolReference *OMR::SymbolReferenceTable::createCoDependententTemporary(
    TR::ResolvedMethodSymbol *owningMethodSymbol, TR::DataType type,
    bool isInternalPointer, size_t size, TR::Symbol *coDependent,
    int32_t offset) {
  TR::SymbolReference *tempSymRef =
      findOrCreateAutoSymbol(owningMethodSymbol, offset, type, true,
...
OMR::SymbolReferenceTable::findOrCreatePendingPushTemporary(
  ...
  TR_ASSERT(!type.isBCD() || size,
            "binary coded decimal types must provide a size\n");
#endif
  return findOrCreateAutoSymbol(owningMethodSymbol, -(slot + 1), type, true,
