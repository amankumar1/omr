...
public:
  ...
  trace_allocator(const base_allocator &a = base_allocator())
      : base_allocator(a) {}
};
...
private:
  struct AllocationRecord {
    ...
    template <class ostr>
    friend ostr &operator<<(ostr &o, const struct AllocationRecord &a) {
...
public:
  ...
  void deallocate(void *pointer, size_t size, const char *name = NULL) {
    ...
    if (accounting.Locate((const char *)name, hi)) {
      ...
      if (accounting[hi]._count == 0)
        accounting.Remove(hi);
    }
  ...
  struct stat_sort {
    stat_sort(size_t s = 0, size_t c = 0, const char *n = NULL)
        : size(s), count(c), name(n) {}

    bool operator<(const struct stat_sort s2) { return size > s2.size; }
    size_t size, count;
    const char *name;
    ...
    template <class ostr>
    friend ostr &operator<<(ostr &o, const struct stat_sort &a) {
      return o << a.size << " (" << a.count << ")"
               << "\t" << (a.name ? a.name : "");
    }
  };
  ...
  template <class ostr, class allocator> ostr &stats(ostr &o, allocator &a) {
    ...
    uint32_t count = 0;
    typename HashTable<const char *, AllocationRecord, base_allocator>::Cursor
        ac(accounting);
    ...
    count = 0;
    for (ac.SetToFirst(); ac.Valid(); ac.SetToNext()) {
      sstats[count++] = stat_sort(accounting[ac]._size, accounting[ac]._count,
                                  accounting.KeyAt(ac));
    }
    sstats.Sort();
  ...
  account_allocator(const base_allocator &a = base_allocator())
      : base_allocator(a), accounting(a) {}
  account_allocator(const account_allocator &a)
      : base_allocator(a), accounting(a) {}
};
...
public:
  check_allocator(const base_allocator &a = base_allocator())
      : base_allocator(a), record_table(a) {}
  check_allocator(const check_allocator &c)
      : base_allocator(base_allocator(c)), record_table(c) {}

  ~check_allocator() {
    typename HashTable<void *, allocation_record, base_allocator>::Cursor rc(
        record_table);
    for (rc.SetToFirst(); rc.Valid(); rc.SetToNext()) {
      allocation_record &r = record_table[rc];
      printf("Leftover pointer: %p[%ld] %s\n", record_table.KeyAt(rc), r.size,
             r.name ? r.name : "");
    }
  ...
  void *allocate(size_t size, const char *name = NULL) {
    ...
    if (record_table.Locate(ret, hi)) {
      ...
      printf("duplicate return: %p\n*alloc1=%s[%ld]\n*alloc2=%s[%ld]\n", ret,
             x.name ? x.name : "", x.size, name ? name : "", size);
    }
  ...
  void deallocate(void *pointer, size_t size, const char *name = NULL) {
    ...
    if (size != x.size) {
      printf("mismatched size: %ld!=%ld\n*alloc=%s\n*dealloc=%s\n", size,
             x.size, name ? name : "", x.name ? x.name : "");
    ...
    if (memcmp(pointer, &pointer, sizeof(void *)))
      printf("buffer underflow: %p!=%p\n*alloc=%s[%ld]\n*dealloc=%s[%ld]\n",
             pointer, *(void **)pointer, x.name ? x.name : "", x.size,
             name ? name : "", size);
    if (memcmp((char *)pointer + size + sizeof(void *), &pointer,
               sizeof(void *))) {
      printf("buffer overflow: %p!=%p\n*alloc=%s[%ld]\n*dealloc=%s[%ld]\n",
             pointer, *(void **)((char *)pointer + size + 4),
             x.name ? x.name : "", x.size, name ? name : "", size);
      CS2Assert(false, ("Overflow: %p", pointer));
    }
  ...
  void *reallocate(size_t newsize, void *pointer, size_t size,
    ...
    void *ret = base_allocator::reallocate(
        newsize + 2 * sizeof(void *), pointer, size + 2 * sizeof(void *), name);
    memcpy(ret, &ret, sizeof(void *));
...
template <class base_allocator> class named_allocator : private base_allocator {
  const char *allocator_name;

public:
  named_allocator(base_allocator b = base_allocator(), const char *name = NULL)
      : base_allocator(b), allocator_name(name) {}
