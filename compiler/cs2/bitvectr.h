...
inline void ABitVector<Allocator>::GrowTo(BitIndex newBitSize, bool geometric,
  ...
  if (oldBitSize == 0) {
    ...
    fBitWords = (BitWord *)Allocator::reallocate(
        SizeInBytes(fNumBits), fBitWords, SizeInBytes(oldBitSize));
    memset(((char *)fBitWords) + SizeInBytes(oldBitSize), 0,
           SizeInBytes(fNumBits) - SizeInBytes(oldBitSize));
  }
}
...
inline uint32_t ABitVector<Allocator>::PopulationCount(uint32_t numBits) const {
  ...
  // popcntb results in 8 bits sections
  // WORDS_IN_16_BIT_ACCUM - worst-case maximum number of words to accumulate
  // popcntb results in 16 bits sections
    ...
    for (wordIndex = 0; wordIndex < streamWordSize;
      ...
      for (BitIndex i2 = wordIndex; i2 < wordIndex + WORDS_IN_16_BIT_ACCUM;
        ...
        for (BitIndex i1 = i2; i1 < i2 + WORDS_IN_8_BIT_ACCUM; i1++) {
          ...
          c1a += __popcntb(
              fBitWords[streamWordSize + i1]); // count the number of 1's in
                                               // each byte of the word at
                                               // wordIndex+streamWordSize index
          c1b += __popcntb(
              fBitWords[streamWordSize * 2 + i1]); // count the number of 1's in
                                                   // each byte of the word at
                                                   // wordIndex+2*streamWordSize
                                                   // index
          c1c += __popcntb(
              fBitWords[streamWordSize * 3 + i1]); // count the number of 1's in
                                                   // each byte of the word at
                                                   // wordIndex+3*streamWordSize
                                                   // index
        }
#ifdef BITVECTOR_64BIT
  ...
  if (lastWord - wordIndex >= WORDS_IN_8_BIT_ACCUM * 2) {
    ...
    for (; wordIndex < wordIndex + streamWordSize;
      ...
      for (BitIndex i1 = wordIndex; i1 < wordIndex + WORDS_IN_8_BIT_ACCUM;
        ...
        c1a += __popcntb(fBitWords[streamWordSize + i1]); // count the number of
                                                          // 1's in each byte of
                                                          // the word at
                                                          // wordIndex index
      }
