...
template <class ABitVector, class Allocator> int foo(int seed, Allocator &a) {
  ...
  for (; (i != v1.end()) && c.Valid(); ++i, c.SetToNextOne()) {
    if (c != i) {
      printf("%d: seed = %d ERROR: iterator index %d does not match cursor "
             "index %d.\n",
             __LINE__, seed, (uint32_t)i, (uint32_t)c);
      return 1;
    }
    if (c != *i) {
      printf("%d: seed = %d ERROR: deref iterator index %d does not match "
             "cursor index %d.\n",
             __LINE__, seed, (uint32_t)*i, (uint32_t)c);
      return 1;
    }
  ...
  if ((i != v1.end()) != c.Valid()) {
    printf("%d: seed = %d ERROR: iterator has %sterminated, whereas cursor has "
           "%sterminated.\n",
           __LINE__, seed, (i == v1.end()) ? "" : "not ",
           !c.Valid() ? "" : "not ");
    return 1;
  }
  return 0;
