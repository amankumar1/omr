...
template <class ABitVector, class Allocator> int foo(int seed, Allocator &a) {
...

  {
    ABitVector v4(v3);
    if (v4.IsNull()) {
      printf("%d: ERROR v4 initial value should not be NULL after a copy "
             "constructor from non NULL\n",
             __LINE__);
      return 0;
    }
  }
...

  {
    ABitVector v4(v3);
    if (!v4.IsNull()) {
      printf("%d: ERROR v4 initial value should be NULL after a copy "
  ...
  for (int i = 0; i < count; i++) {
    ...
    if ((int)v1[iy])
      wasSet = true;
    if (v2.IsNull() && wasSet) {
      printf("%d: ERROR v2 should not be NULL after v2[%d]=v1[%d], v1[%d]=%d  "
             "(iteration %d)\n",
  ...
  for (int i = 0; i < count; i++) {
    ...
    if ((int)v2[iy])
      wasSet = true;
    if (v3.IsNull() && wasSet) {
      printf("%d: ERROR v3 should not be NULL after v3[%d]=v2[%d], v2[%d]=%d  "
             "(iteration %d)\n",
...
int main() {
  ...
  for (int seed = 0; seed < 1000; seed++)
    if (foo<sparse, allocator>(seed, a) != foo<dense, allocator>(seed, a)) {
      printf("%d: ERROR %d!=%d\n", __LINE__, foo<sparse, allocator>(seed, a),
             foo<dense, allocator>(seed, a));
      return 1;
    }
  return 0;
}
