...
TR::Linkage *OMR::ARM::CodeGenerator::createLinkage(TR_LinkageConventions lc) {
  ...
  //       linkage = new (self()->trHeapMemory())
  //       TR_ARMInterpretedStaticLinkage(this);
  //       break;
  case TR_Private:
...
// With -msoft-float any floating point return value is placed in gr0 for single
// precision and
// gr0 and gr1 for double precision.  This is where the jit expects the result
// following a directToJNI call.
// The floating point return value on non-hardhat linux-arm builds is through
// the coprocessor register f0.
// Therefore the HasHardFloatReturn flag must be set to force generation of
// instructions to move the result
// from f0 to gr0/gr1 following a directToJNI dispatch.
  ...
  if (!debug("hasFramePointer"))
    self()->setFrameRegister(self()->machine()->getARMRealRegister(
        _linkageProperties->getStackPointerRegister()));
  else
    self()->setFrameRegister(self()->machine()->getARMRealRegister(
        _linkageProperties->getFramePointerRegister()));

  self()->setMethodMetaDataRegister(self()->machine()->getARMRealRegister(
...
OMR::ARM::CodeGenerator::generateSwitchToInterpreterPrePrologue(
  ...
  TR::Register *gr4 =
      self()->machine()->getARMRealRegister(TR::RealRegister::gr4);
  TR::Register *lr = self()->machine()->getARMRealRegister(
...
static void removeGhostRegistersFromGCMaps(TR::CodeGenerator *cg,
  ...
  // it will not be killed immediatey after it's first use in the hot path
  // if it's also used on the cold path, since RA still needs to be done on the
  ...
  // top of the hot path.
  // If there are any GC points between the top of the hot path and the first
  // use the real reg holding the virtual will be included,
  // so we need to fix this.
  TR::Instruction *instr = branchOOL->getNext();
  while (!instr->isLabel() ||
         !((TR::ARMLabelInstruction *)instr)
              ->getLabelSymbol()
    ...
    if (instr->needsGCMap()) {
      ...
      for (uint32_t regNum = TR::RealRegister::FirstGPR;
        ...
        if (map->getRegisterMap() & regMask) {
          TR::RealRegister *regInRegMap = cg->machine()->getARMRealRegister(
          ...
          if (regInRegMap->getState() == TR::RealRegister::Free) {
            ...
            // and it's first use on the hot path was after the GC point
...
void OMR::ARM::CodeGenerator::endInstructionSelection() {
  ...
  if (_returnTypeInfoInstruction != NULL) {
    _returnTypeInfoInstruction->setSourceImmediate(
        static_cast<uint32_t>(TR::comp()->getReturnInfo()));
  }
}
...
void OMR::ARM::CodeGenerator::doBinaryEncoding() {
  ...
  if (isPrivateLinkage) {
    ...
    // TODO: Probably bogus; what does loadUpArguments do when cursorInstruction
    // == NULL?
  ...
  while (cursorInstruction &&
         cursorInstruction->getOpCodeValue() != ARMOp_proc) {
  ...
  while (cursorInstruction) {
    ...
    if (isPrivateLinkage && cursorInstruction == j2jEntryInstruction) {
      uint32_t magicWord =
          ...
          static_cast<uint32_t>(comp->getReturnInfo());
      TR_ASSERT(_returnTypeInfoInstruction &&
                    _returnTypeInfoInstruction->getOpCodeValue() == ARMOp_dd,
                "assertion failure");
  ...
  if (!comp->getOption(TR_DisableOOL)) {
    ...
    while (oiCursor) {
      uint32_t startOffset =
          oiCursor->getFirstInstruction()->getBinaryEncoding() -
          self()->getCodeStart();
      uint32_t endOffset =
          oiCursor->getAppendInstruction()->getBinaryEncoding() -
          self()->getCodeStart();

      TR::Block *block = oiCursor->getBlock();
      bool needsETE =
          oiCursor->getFirstInstruction()
              ->getNode()
              ->getOpCode()
              .hasSymbolReference() &&
...
void OMR::ARM::CodeGenerator::buildRegisterMapForInstruction(
  ...
  for (int i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR;
    ...
    TR::RealRegister *realReg =
        self()->machine()->getARMRealRegister((TR::RealRegister::RegNum)i);
...
int32_t OMR::ARM::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(
  ...
  if (node->getOpCode().isSwitch()) {
    ...
    if (node->getOpCodeValue() == TR::table) {
      ...
      // ControlFlowEvaluator.cpp#tableEvaluator(),
