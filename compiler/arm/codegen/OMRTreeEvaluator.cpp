...
loadAddressConstantFixed(TR::CodeGenerator *cg, TR::Node *node, intptrj_t value,
  ...
  if (value != 0x0) {
    ...
    if (typeAddress == -1) {
      if (doAOTRelocation)
        cg->addAOTRelocation(
            new (cg->trHeapMemory()) TR_BeforeBinaryEncodingExternalRelocation(
...
TR::Register *OMR::ARM::TreeEvaluator::lloadEvaluator(TR::Node *node,
  ...
  if (needSync && TR::Compiler->target.cpu.id() != TR_DefaultARMProcessor) {
    ...
    addDependency(deps, lowReg,
                  bigEndian ? TR::RealRegister::gr1 : TR::RealRegister::gr0,
                  TR_GPR, cg);
    addDependency(deps, highReg,
...
TR::Register *OMR::ARM::TreeEvaluator::wrtbarEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::MemoryReference *tempMR =
      new (cg->trHeapMemory()) TR::MemoryReference(node, 4, cg);
...
TR::Register *OMR::ARM::TreeEvaluator::iwrtbarEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  TR::MemoryReference *tempMR =
      new (cg->trHeapMemory()) TR::MemoryReference(node, 4, cg);
...
TR::Register *OMR::ARM::TreeEvaluator::lstoreEvaluator(TR::Node *node,
  ...
  if (needSync && TR::Compiler->target.cpu.id() != TR_DefaultARMProcessor) {
    ...
    addDependency(deps, valueReg->getLowOrder(), TR::RealRegister::gr1, TR_GPR,
                  cg);
    addDependency(deps, valueReg->getHighOrder(), TR::RealRegister::gr2, TR_GPR,
                  cg);
...
TR::Register *
OMR::ARM::TreeEvaluator::monexitfenceEvaluator(TR::Node *node,
                                               TR::CodeGenerator *cg) {
...
OMR::ARM::TreeEvaluator::asynccheckEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    negated = tmpChild < 0 && tmpChild != 0x80000000; // the negative of the
...
static void signExtend(TR::Node *node, TR::Register *dst, TR::Register *src,
  ...
  op = new (cg->trHeapMemory())
      TR_ARMOperand2(ARMOp2RegASRImmed, dst, 32 - bitsInDst);
  generateTrg1Src1Instruction(cg, ARMOp_mov, node, dst, op);
}

static void zeroExtend(TR::Node *node, TR::Register *dst, TR::Register *src,
                       int32_t bitsInDst, TR::CodeGenerator *cg) {
  TR_ARMOperand2 *op;
  ...
  generateTrg1Src1Instruction(cg, ARMOp_mov, node, dst, op);
  op = new (cg->trHeapMemory())
      TR_ARMOperand2(ARMOp2RegLSRImmed, dst, 32 - bitsInDst);
  generateTrg1Src1Instruction(cg, ARMOp_mov, node, dst, op);
}
