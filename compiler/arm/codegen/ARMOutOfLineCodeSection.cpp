...
void TR_ARMOutOfLineCodeSection::assignRegisters(
  ...
  // captures all current real register associations.
  // This is necessary to get the register assigner back into its original state
  // before the helper stream was processed.
  ...
  // beginning, but unused on the cold path
  // it won't show up in the liveRealRegDeps. If a dead virtual occupies that
  // real reg we need to clean it up.
  for (int32_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastFPR;
       ++i) {
    TR::RealRegister *r =
        _cg->machine()->getARMRealRegister((TR::RealRegister::RegNum)i);
