...
uint32_t encodeHelperBranch(bool isBranchAndLink, TR::SymbolReference *symRef,
  ...
  if (distance > BRANCH_FORWARD_LIMIT || distance < BRANCH_BACKWARD_LIMIT) {
    target = cg->fe()->indexedTrampolineLookup(symRef->getReferenceNumber(),
                                               (void *)cursor);
  ...
  return (isBranchAndLink ? 0x0B000000 : 0x0A000000) |
         encodeBranchDistance((uintptr_t)cursor, target) |
         (((uint32_t)cc) << 28);
}
...
uint8_t *TR::ARMImmSymInstruction::generateBinaryEncoding() {
  ...
  if (getOpCodeValue() == ARMOp_bl) {
    ...
    TR_ResolvedMethod *resolvedMethod =
        (sym == NULL) ? NULL : sym->getResolvedMethod();
        ...
        *(int32_t *)cursor =
        ...
        if (distance <= 0x01fffffc && distance >= 0xfe000000) {
          *(int32_t *)cursor |=
          ...
          if (sym->isJNI() && node && node->isPreparedForDirectJNI() &&
            ...
            *(int32_t *)cursor = imm;
            ...
            trmpln = (void *)cg()->fe()->methodTrampolineLookup(
                comp, getSymbolReference(), (void *)cursor);
            distance = (intptr_t)trmpln - (intptr_t)cursor;
            TR_ASSERT(distance <= BRANCH_FORWARD_LIMIT &&
                          distance >= BRANCH_BACKWARD_LIMIT,
                      "CodeCache is more than 32MB.\n");
            *(int32_t *)cursor |=
                encodeBranchDistance((uintptr_t)cursor, (uintptr_t)trmpln);
          }
...
TR::ARMImmSymInstruction::estimateBinaryLength(int32_t currentEstimate) {
  ...
  if (sym && sym->isJNI() && node && node->isPreparedForDirectJNI() &&
      getConditionCode() == ARMConditionCodeAL)
