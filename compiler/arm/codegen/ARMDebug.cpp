  ...
  for (i = 0; i < numConditions; ++i) {
    ...
    if (r == TR::RealRegister::NoReg)
      ...
      trfprintf(pOutFile, "%s]",
                getName(_cg->machine()->getARMRealRegister(r)));
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARMImmSymInstruction *instr) {
  ...
  bool longJump = imm && (distance > BRANCH_FORWARD_LIMIT ||
...
void TR_Debug::printARMGCRegisterMap(TR::FILE *pOutFile,
  ...
  for (int i = 15; i >= 0; i--) {
    if (map->getMap() & (1 << i))
      trfprintf(pOutFile, "%s ",
                getName(machine->getARMRealRegister((TR::RealRegister::RegNum)(
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARMCallSnippet *snippet) {
  ...
  TR::Machine *machine = _cg->machine();
  const TR_ARMLinkageProperties &linkage =
  ...
  for (int i = callNode->getFirstArgumentIndex();
    ...
    case TR::Address:
      ...
      if (numIntArgs < linkage.getNumIntArgRegs()) {
        ...
        print(pOutFile, machine->getARMRealRegister(
...
void TR_Debug::print(TR::FILE *pOutFile,
  ...
  if (constantIsImmed8r(frameSize, &base, &rotate)) {
    ...
    bufferPos += 4;
    printPrefix(pOutFile, NULL, bufferPos, 4);
    trfprintf(pOutFile, "add\tgr11, gr11, #0x%08x", (frameSize >> 8 & 0xFF)
                                                        << 8);
    bufferPos += 4;
    printPrefix(pOutFile, NULL, bufferPos, 4);
    trfprintf(pOutFile, "add\tgr11, gr11, #0x%08x", (frameSize >> 16 & 0xFF)
                                                        << 16);
    bufferPos += 4;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARMRecompilationSnippet *snippet) {
  ...
  if (name)
    trfprintf(pOutFile, "bl\t%s\t;%s (" POINTER_PRINTF_FORMAT ")", name, info,
              target);
  else
...
void TR_Debug::printARM(TR::FILE *pOutFile, uint8_t *instrStart,
                        uint8_t *instrEnd) {
  ...
  for (; instrStart < instrEnd; instrStart += 4) {
    ...
    trfprintf(pOutFile, "0x%08x %08x        %-11s%s\n", instrStart,
              *(uint32_t *)instrStart, opcodeBuf, opBuf);
  }
}
...
static const char *opCodeToNameMap[] = {
    ...
    "strh",    "stm",      "stmdb",   "swp",     "fence",  "ret",     "wrtbar",
    "proc",    "dd",       "dmb_v6",  "dmb",     "dmb_st", "ldrex",   "strex",
    "iflong",  "setblong", "setbool", "setbflt", "lcmp",   "flcmpl",  "flcmpg",
