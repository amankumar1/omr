...
TR::Register *OMR::ARM::TreeEvaluator::ireturnEvaluator(TR::Node *node,
  ...
  addDependency(deps, returnRegister,
...
TR::Register *OMR::ARM::TreeEvaluator::lreturnEvaluator(TR::Node *node,
  ...
  addDependency(deps, lowReg, cg->getProperties().getLongLowReturnRegister(),
                TR_GPR, cg);
  addDependency(deps, highReg, cg->getProperties().getLongHighReturnRegister(),
                TR_GPR, cg);
...
static TR::Instruction *compareIntsForOrder(TR_ARMConditionCode branchType,
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    negated = cmpValue < 0 && cmpValue != 0x80000000; // the negative of the
                                                      // maximum negative int is
                                                      // itself so can not use
                                                      // cmn here
...
static TR::Register *compareLongsForOrder(TR_ARMConditionCode branchOp,
  ...
  TR::Register *src2Reg = cg->evaluate(secondChild);
  if (/* isSmall() || */ TR::comp()->getOption(TR_DisableOOL) ||
      disableOOLForLongCompares) {
    TR::ARMControlFlowInstruction *cfop =
    ...
    // dependency to hold that, and hold
    // until the compare for them are done.
    TR::RegisterDependencyConditions *newDeps = new (cg->trHeapMemory())
        TR::RegisterDependencyConditions(2, 2, cg->trMemory());
    addDependency(newDeps, src1Reg->getLowOrder(), TR::RealRegister::NoReg,
                  TR_GPR, cg);
    addDependency(newDeps, src2Reg->getLowOrder(), TR::RealRegister::NoReg,
                  TR_GPR, cg);
...
// regards to number of used registers (or dependency),
// the matching GlDepRegs free checks in
...
static void lookupScheme4(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  /*  TODO implement lookups
     ...
     sizeof(int), cg->getCurrentEvaluationBlock()->isCold());
     ...
     cg);
     addDependency(conditions, addrRegister, TR::RealRegister::NoReg, TR_GPR,
     cg);
     addDependency(conditions, selector, TR::RealRegister::NoReg, TR_GPR, cg);
     addDependency(conditions, lowRegister, TR::RealRegister::NoReg, TR_GPR,
     cg);
     addDependency(conditions, highRegister, TR::RealRegister::NoReg, TR_GPR,
     cg);
     addDependency(conditions, pivotRegister, TR::RealRegister::NoReg, TR_GPR,
     cg);
     addDependency(conditions, cndRegister, TR::RealRegister::NoReg, TR_CCR,
     cg);
     ...
     TR::LabelSymbol::create(cg->trHeapMemory(),cg);
     TR::LabelSymbol *searchLabel =
     TR::LabelSymbol::create(cg->trHeapMemory(),cg);
     TR::LabelSymbol *biggerLabel =
     TR::LabelSymbol::create(cg->trHeapMemory(),cg);
     TR::LabelSymbol *linkLabel =
     TR::LabelSymbol::create(cg->trHeapMemory(),cg);
     ...
     new (cg->trHeapMemory()) TR::ARMTrg1Src1Imm2Instruction(0xfffffffc, 31,
     pivotRegister, pivotRegister, ARMOp_rlwinm, cg);
     generateTrg1MemInstruction(cg, ARMOp_lwzx, dataRegister, new
     (cg->trHeapMemory()) TR_ARMMemoryReference(addrRegister, pivotRegister,
     cg));
     generateTrg1Src2Instruction(node, cg, ARMOp_cmp4, cndRegister,
     dataRegister, selector);
     generateConditionalBranchInstruction(node, cg, ARMOp_bne, searchLabel,
     cndRegister);
     generateLabelInstruction(node, cg, ARMOp_bl, linkLabel);
     generateLabelInstruction(node, cg, ARMOp_label, linkLabel);
     new (cg->trHeapMemory()) TR::ARMTrg1Instruction(ARMOp_mflr, dataRegister,
     cg);
     generateTrg1Src1ImmInstruction(node, cg, ARMOp_addi, pivotRegister,
     pivotRegister, 20);
     generateTrg1Src2Instruction(node, cg, ARMOp_add, dataRegister,
     pivotRegister, dataRegister);
     new (cg->trHeapMemory()) TR::ARMSrc1Instruction(ARMOp_mtctr, dataRegister,
     cg);
     ...
     cndRegister);
     generateTrg1Src2Instruction(node, cg, ARMOp_cmp4, cndRegister,
     pivotRegister, highRegister);
     generateConditionalBranchInstruction(node, cg, ARMOp_beq,
     node->getSecondChild()->getBranchDestination()->getNode()->getLabel(),
     cndRegister);
     generateTrg1Src1ImmInstruction(node, cg, ARMOp_addi, lowRegister,
     pivotRegister, 4);
     generateLabelInstruction(node, cg, ARMOp_b, backLabel);

     generateLabelInstruction(node, cg, ARMOp_label, biggerLabel);
     generateTrg1Src2Instruction(node, cg, ARMOp_cmp4, cndRegister,
     pivotRegister, lowRegister);
     generateConditionalBranchInstruction(node, cg, ARMOp_beq,
     node->getSecondChild()->getBranchDestination()->getNode()->getLabel(),
     cndRegister);
     generateTrg1Src1ImmInstruction(node, cg, ARMOp_addi, highRegister,
     pivotRegister, -4);
     new (cg->trHeapMemory()) TR::ARMDepLabelInstruction(ARMOp_b, backLabel,
     conditions, cg);
     */
}
...
TR::Register *OMR::ARM::TreeEvaluator::tableEvaluator(TR::Node *node,
  ...
  if (numBranchTableEntries < 5) {
    ...
    TR::RealRegister *machineIP =
        cg->machine()->getARMRealRegister(TR::RealRegister::gr15);
    uint32_t base, rotate;
...
TR::Register *
OMR::ARM::TreeEvaluator::resolveCHKEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg) {
