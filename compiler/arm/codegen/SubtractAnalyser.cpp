...
void TR_ARMSubtractAnalyser::integerSubtractAnalyser(
  ...
  } else // assert getSubReg1Mem2() == true
  {
    //      TR_ARMMemoryReference *tempMR = new (cg()->trHeapMemory())
    //      TR_ARMMemoryReference(secondChild);
    //      new (cg()->trHeapMemory()) TR::ARMRegMemInstruction(memToRegOpCode,
    //      firstRegister, tempMR, cg());
    //      tempMR->decNodeReferenceCounts();
    //      root->setRegister(firstRegister);
  }
...
void TR_ARMSubtractAnalyser::longSubtractAnalyser(TR::Node *root) {
  ...
  if (getCopyReg1()) {
    ...
    } else // assert getSubReg3Mem2() == true
    {
      ...
      //         TR_ARMMemoryReference(secondChild);
      //         new (cg()->trHeapMemory()) TR::ARMRegMemInstruction(SUB4RegMem,
      //         lowThird, lowMR, cg());
      //         TR_ARMMemoryReference *highMR = new (cg()->trHeapMemory())
      //         TR_ARMMemoryReference(*lowMR, 4);
      //         new (cg()->trHeapMemory()) TR::ARMRegMemInstruction(SBB4RegMem,
      //         highThird, highMR, cg());
    ...
    //      new (cg()->trHeapMemory()) TR::ARMRegRegInstruction(SBB4RegReg,
  ...
  } else // assert getSubReg1Mem2() == true
  {
    ...
    //      TR_ARMMemoryReference(secondChild);
    //      new (cg()->trHeapMemory()) TR::ARMRegMemInstruction(SUB4RegMem,
    //      firstRegister->getLowOrder(), lowMR, cg());
    //      TR_ARMMemoryReference *highMR = new (cg()->trHeapMemory())
    //      TR_ARMMemoryReference(*lowMR, 4);
    //      new (cg()->trHeapMemory()) TR::ARMRegMemInstruction(SBB4RegMem,
