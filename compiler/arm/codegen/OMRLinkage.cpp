...
TR::Instruction *OMR::ARM::Linkage::saveArguments(TR::Instruction *cursor) {
  ...
  TR::RealRegister *stackPtr = machine->getARMRealRegister(
      self()->getProperties().getStackPointerRegister());
  TR::ResolvedMethodSymbol *bodySymbol =
      codeGen->comp()->getJittedMethodSymbol();
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
    ...
    case TR::Address:
      if (hasToBeOnStack) {
        argRegister = machine->getARMRealRegister(
  ...
  if (child->getOpCodeValue() == TR::loadaddr) {
    ...
    } else // must be loadaddr of parm or local
    {
      if (child->pointsToNonNull()) {
        ...
        addDependency(deps, pushReg, TR::RealRegister::NoReg, TR_GPR,
                      self()->cg());
        addDependency(deps, objReg, TR::RealRegister::NoReg, TR_GPR,
                      self()->cg());
...
TR::Register *OMR::ARM::Linkage::pushAddressArg(TR::Node *child) {
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadConst()) {
    ...
    pushRegister = codeGen->allocateRegister();
    if (isClass &&
        self()->cg()->wantToPatchClassPointer(
...
int32_t OMR::ARM::Linkage::buildARMLinkageArgs(
  ...
  if (isVirtual) {
    ...
    addDependency(dependencies, vftReg, TR::RealRegister::NoReg, TR_GPR,
                  self()->cg()); // dep 0

#ifndef LOCK_R14
    addDependency(dependencies, NULL, TR::RealRegister::gr14, TR_GPR,
                  cg()); // dep 2
#endif
  }
  ...
  if (isVirtual && (specialArgReg != TR::RealRegister::gr11)) {
    addDependency(dependencies, NULL, TR::RealRegister::gr11, TR_GPR,
                  self()->cg()); // dep 1
  }
    ...
    if (comp->getOption(TR_TraceCG)) {
      traceMsg(comp, "Special arg %s in %s\n",
               ...
               comp->getDebug()->getName(
                   self()->cg()->machine()->getARMRealRegister(specialArgReg)));
    }
  ...
  if (numMemArgs > 0) {
    pushToMemory =
        new (self()->trStackMemory()) TR_ARMMemoryArgument[numMemArgs];
  }
      ...
      if (isSpecialArg) {
        ...
        if (numIntegerArgs < numIntArgRegs) {
          ...
          if (numIntegerArgs == 0 /* &&
            ...
            addDependency(dependencies, reg,
    ...
    case TR::Double:
      ...
      if (isSpecialArg) {
        ...
        if (specialArgReg == properties.getIntegerReturnRegister(0)) {
          ...
          addDependency(dependencies, specialArgRegister, specialArgReg, TR_GPR,
                        self()->cg());
        }
      } else {
        if (numIntegerArgs < numIntArgRegs) {
  ...
  */
  ...
  for (i = 0; i < 8 /*properties.getNumFloatArgRegs()*/; i++) {
    addDependency(dependencies, NULL, (TR::RealRegister::RegNum)(
                                          (uint32_t)TR::RealRegister::fp0 + i),
                  TR_FPR, self()->cg());
  }
#endif
  ...
  case TR_Helper:
    addDependency(dependencies, NULL, TR::RealRegister::gr4, TR_GPR,
                  self()->cg());
    addDependency(dependencies, NULL, TR::RealRegister::gr5, TR_GPR,
                  self()->cg());
  // deliberate fall-through
  case TR_System:
#ifndef LOCK_R14
...
TR_HeapMemory OMR::ARM::Linkage::trHeapMemory() { return self()->trMemory(); }

TR_StackMemory OMR::ARM::Linkage::trStackMemory() { return self()->trMemory(); }
...
OMR::ARM::Linkage::buildARMLinkageDirectDispatch(TR::Node *callNode,
  ...
  isMyself = (vmm != NULL) &&
             vmm->isSameMethod(codeGen->comp()->getCurrentMethod()) &&
             !codeGen->comp()->isDLT();

  if (callSymRef->getReferenceNumber() >= TR_ARMnumRuntimeHelpers)
    codeGen->comp()->fe()->reserveTrampolineIfNecessary(codeGen->comp(),
                                                        callSymRef, false);
  ...
  if ((callSymbol->isJITInternalNative() ||
       (!callSymRef->isUnresolved() && !callSymbol->isInterpreted() &&
        ((codeGen->comp()->compileRelocatableCode() &&
          callSymbol->isHelper()) ||
         !codeGen->comp()->compileRelocatableCode())))) {
    ...
    gcPoint = generateImmSymInstruction(
        ...
        new (self()->trHeapMemory())
            TR::SymbolReference(codeGen->comp()->getSymRefTab(), label),
        snippet);
  }
