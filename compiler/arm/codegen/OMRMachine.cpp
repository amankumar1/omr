...
OMR::ARM::Machine::Machine(TR::CodeGenerator *cg)
    : OMR::Machine(NUM_ARM_GPR, NUM_ARM_FPR), _cg(cg) {
  ...
  if (forced != NULL) {
    ...
    for (int i = first; i <= last; i++) {
      TR::RealRegister *realReg =
          self()->getARMRealRegister((TR::RealRegister::RegNum)i);
  ...
  case TR_GPR:
    if (candidates[0]->getBackingStorage()) {
      ...
      if (!location->isOccupied()) {
        ...
        // the list (see TODO in ARMInstruction.cpp). Therefore we need to set
...
OMR::ARM::Machine::createCondForLiveAndSpilledGPRs(
  ...
  if (c) {
    ...
    for (int32_t j = TR::RealRegister::FirstGPR; j <= endReg; j++) {
      TR::RealRegister *realReg =
          self()->getARMRealRegister((TR::RealRegister::RegNum)j);
      if (realReg->getState() == TR::RealRegister::Assigned) {
