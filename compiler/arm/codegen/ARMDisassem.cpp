...
#ifdef __VFP_FP__
const char *doubleRegStr(uint32_t regNum,
                         bool W) { // If a register value is to be updated by
                                   // the instruction, it is printed in
                                   // capitals.
  switch (regNum >> 1) {
  case 0:
    return (W ? "D0" : "d0");
...
const char *singleRegStr(uint32_t regNum,
                         bool W) { // If a register value is to be updated by
                                   // the instruction, it is printed in
                                   // capitals.
  switch (regNum) {
...
void dataProcessing(int32_t *instrPtr, char *mBuf, char *iBuf) {
  ...
  static char *opcodeName[] = {"and", "eor", "sub", "rsb", "add", "adc",
                               ...
                               "orr", "mov", "bic", "mvn"};
  bool test_cmp = instrBits->bits23_24 == 2; // instr->opcode: 0b1000 || 0b1001
                                             // || 0b1010 || 0b1011 (tst, teq,
                                             // cmp, cmn)
  bool mov =
      instrBits->bit21 &&
...
void vfpSingleRegTransfer(int32_t *instrPtr, char *mBuf, char *iBuf) {
  ...
  bool isDouble = (instr->cp == 11); // Coprocessor 11 is double, 10 is float
  if (opCode == 0) {
    sprintf(mBuf, "%s%s", isDouble ? (instr->r ? "fmrs" : "fmsr")
                                   : (instr->r ? "fmrdl" : "fmdlr"),
            condName[instr->cond]);
    if (isDouble) {
      if (instr->r) // L-bit
