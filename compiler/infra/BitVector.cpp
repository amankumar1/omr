...
void TR_BitVector::setChunkSize(int32_t chunkSize) {
  ...
  chunk_t *newChunks = (chunk_t *)_trMemory->allocateMemory(
      chunkSize * sizeof(chunk_t), _allocationKind, TR_Memory::BitVector);
...
const char *TR_BitVector::getHexString() {
  ...
  char *buf = (char *)_trMemory->allocateMemory(
      _numChunks * chunk_hexlen + 1, _allocationKind, TR_Memory::BitVector);
  char *pos = buf;
#ifdef TRACK_TRBITVECTOR_MEMORY
