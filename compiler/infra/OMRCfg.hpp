...
public:
  ...
  CFG(TR::Compilation *c, TR::ResolvedMethodSymbol *m)
      : _compilation(c), _method(m), _rootStructure(NULL), _pStart(NULL),
        _pEnd(NULL), _nextNodeNumber(0), _numEdges(0),
        _mightHaveUnreachableBlocks(false), _doesHaveUnreachableBlocks(false),
        _removingUnreachableBlocks(false), _ignoreUnreachableBlocks(false),
        _structureGraphNodes(c->allocator()), _removeEdgeNestingDepth(0),
        _forwardTraversalOrder(NULL), _backwardTraversalOrder(NULL),
        _forwardTraversalLength(0), _backwardTraversalLength(0),
        _maxFrequency(-1), _maxEdgeFrequency(-1), _oldMaxFrequency(-1),
        _oldMaxEdgeFrequency(-1), _frequencySet(NULL), _calledFrequency(0),
        _initialBlockFrequency(-1), _edgeProbabilities(NULL) {}
  ...
  TR_StructureSubGraphNode *getStructureSubGraphNode(uint32_t index) {
    return _structureGraphNodes[index];
  }
  ...
  void removeEdge(TR::list<TR::CFGEdge *> succList, int32_t selfNumber,
                  int32_t destNumber);
