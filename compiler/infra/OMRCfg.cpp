...
TR::CFGNode *OMR::CFG::addNode(TR::CFGNode *n, TR_RegionStructure *parent,
  ...
  if (parent && getStructure()) {
    ...
    if (block) {
      ...
      if (!blockStructure)
        blockStructure = new (trHeapMemory())
      ...
      if (!blockNode) {
        blockNode =
            new (trHeapMemory()) TR_StructureSubGraphNode(blockStructure);
        if (!isEntryInParent) {
...
uint32_t OMR::CFG::addStructureSubGraphNodes(TR_StructureSubGraphNode *node) {
  uint32_t index = _structureGraphNodes.AddEntry(node);
  node->setUniqueSubGraphIndex(index);
  return index;
}

void OMR::CFG::removeStructureSubGraphNodes(TR_StructureSubGraphNode *node) {
...
void OMR::CFG::addExceptionEdge(TR::CFGNode *f, TR::CFGNode *t,
  ...
  TR::CFGEdge *e =
      TR::CFGEdge::createExceptionEdge(f, t, trMemory(), allocKind);
  _numEdges++;
...
TR_Structure *OMR::CFG::invalidateStructure() { return setStructure(0); }
...
TR_OrderedExceptionHandlerIterator::TR_OrderedExceptionHandlerIterator(
  ...
  if (tryBlock->getExceptionSuccessors().empty())
    ...
    _handlers = (TR::Block **)tryBlock->trMemory()->allocateStackMemory(
        _dim * sizeof(TR::Block *));
...
TR::CFGEdge *TR::CFGEdge::createEdge(TR::CFGNode *pF, TR::CFGNode *pT,
                                     ...
                                     TR_AllocationKind allocKind) {
  TR::CFGEdge *newEdge =
      new (trMemory, allocKind) TR::CFGEdge(pF, pT, allocKind);
...
TR::CFGNode::CFGNode(TR_Memory *m)
    : _nodeNumber(-1), _visitCount(0), _frequency(-1),
      _forwardTraversalIndex(-1), _backwardTraversalIndex(-1), _m(m),
      _successors(getTypedAllocator<TR::CFGEdge *>(TR::comp()->allocator())),
      _predecessors(getTypedAllocator<TR::CFGEdge *>(TR::comp()->allocator())),
      _exceptionSuccessors(
          getTypedAllocator<TR::CFGEdge *>(TR::comp()->allocator())),
      _exceptionPredecessors(
          getTypedAllocator<TR::CFGEdge *>(TR::comp()->allocator())) {}
...
TR::CFGEdge *TR::CFGNode::getEdgeMatchingNodeInAList(
    TR::CFGNode *n, TR::list<TR::CFGEdge *> &list, FUNC blockGetter) {
...
void OMR::CFG::removeEdge(TR::list<TR::CFGEdge *> succList, int32_t selfNumber,
                          int32_t destNumber) {
            ...
            if (loops[cursor].isInLoop.ValueAt(n)) {
              ...
              if (trace)
                dumpOptDetails(comp(), "Extend loop at %d up to %d\n",
                               stack[loops[cursor].bottom]->getNumber(),
                               number);
            }
          }
        } else {
...
// blocks in the CFG
// The traversal order array is always allocated anew since we don't know if the
// number of blocks
// in the CFG has changed since the last time we allocated the array.
// This function uses two parallel stacks to accomplish a post-order traversal:
// blockStack and iteratorStack.
// blockStack holds blocks whose out edges (either successors or predecessors
// depending on the traversal direction)
// haven't yet all been visited.  Blocks are pushed on the stack to be visited.
// The iteratorStack holds the iterators for the out edges for the
...
int32_t OMR::CFG::createTraversalOrder(bool forward,
  ...
  if (traversalInsertPosition > 0) {
    ...
    // beginning of the traversalOrder array
...
TR_BlockCloner *OMR::CFG::clone() {
  ...
  TR_BlockCloner *cloner =
      new (trHeapMemory()) TR_BlockCloner(self(), false, true);
...
Lists as Argument 1
static void
   summarizeEdgeFrequencies(TR::list<TR::CFGEdge*> list, int32_t *sumEdgeFreq,
int32_t *edgeCount)
   {
          ...
          if (branchToBlock) {
            if (!branchToBlock->isCold()) {
              if ((branchToBlock->getFrequency() + blockFreq) >
                  INITIAL_BLOCK_FREQUENCY_FACTOR) {
                if (_compilation->getOption(TR_TraceBFGeneration))
                  dumpOptDetails(comp(), "Setting frequency of %d on block_%d "
                                         "(GOTO block) - MAX block frequency\n",
                                 INITIAL_BLOCK_FREQUENCY_FACTOR,
                ...
                branchToBlock->setFrequency(branchToBlock->getFrequency() +
                                            blockFreq);
                if (_compilation->getOption(TR_TraceBFGeneration))
                  dumpOptDetails(comp(), "Adding frequency of %d on block_%d "
                                         "(GOTO block), prev frequency was "
                                         "%d\n",
...
void OMR::CFG::setBlockAndEdgeFrequenciesBasedOnStructure() {
  ...
  if (setBlockFrequencies) {
    if (_compilation->getOption(TR_TraceBFGeneration))
      dumpOptDetails(comp(), "\nsetBlockAndEdgeFrequenciesBasedOnStructure: "
                             "Propagating block and edge frequencies within "
                             "regions...\n");
...

    {
      ...
      if (_compilation->getOption(TR_TraceBFGeneration))
        dumpOptDetails(comp(), "\nsetBlockAndEdgeFrequenciesBasedOnStructure: "
                               "Computing region weight factors based on CFG "
      ...
      if (_compilation->getOption(TR_TraceBFGeneration))
        dumpOptDetails(comp(), "\nsetBlockAndEdgeFrequenciesBasedOnStructure: "
                               "Propagating weight factors based on CFG "
                               "structure...\n");
...
void OMR::CFG::getBranchCounters(TR::Node *node, TR::Block *block,
  ...
  if (((branchToBlock->getFrequency() >= 0) &&
    ...
    *taken =
        branchToBlock
            ->getFrequency(); // (branchToBlock->getFrequency()*blockFreq)/(branchToBlock->getFrequency()
