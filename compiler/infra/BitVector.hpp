...
public:
  ...
  TR_BitVector()
      : _numChunks(0), _chunks(NULL), _firstChunkWithNonZero(0),
        _lastChunkWithNonZero(-1), _allocationKind(heapAlloc),
        _growable(growable), _trMemory(0) {}
...
#endif
    if (_numChunks) {
      _chunks = (chunk_t *)m->allocateMemory(_numChunks * sizeof(chunk_t),
                                             _allocationKind, ot);
  ...
  void init(int64_t initBits, TR_Memory *m,
    ...
    _allocationKind = allocKind; // THIS IS DANGEROUS! Should we assert that
                                 // allocKinds should match?
    _growable = growable;
    _chunks = NULL;
    _numChunks = 0;
