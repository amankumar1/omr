...
public:
  ...
  ListElement<T> *add(T *p) {
    TR_ASSERT(this->_allocKind != persistentAlloc,
              "Should not be using heap allocation to add to a persistent "
              "list. The 'add' routine in TR_PersistentList should be "
              "called\n");
    // return this->_pHead = new (trMemory, this->_allocKind) ListElement<T>(p,
    // this->_pHead);
    return this->_pHead = new (this->trMemory(), this->_allocKind)
               ListElement<T>(p, this->_pHead);
  }

  ListElement<T> *add(T *p, TR_AllocationKind allocKind) {
    // TR_ASSERT(_allocKind == allocKind,
    //       "Should not be using two different kinds of allocation to add to a
    //       particular list.\n");
    // return this->_pHead = new (trMemory, allocKind) ListElement<T>(p,
    // this->_pHead);
    return this->_pHead = new (this->trMemory(), allocKind)
               ListElement<T>(p, this->_pHead);
  }
  ...
  ListElement<T> *addAfter(T *elem, ListElement<T> *previous) {
    ...
    if (previous == NULL)
      ...
      TR_ASSERT(this->_allocKind != persistentAlloc,
                "Should not be using heap allocation to add to a persistent "
                "list. The 'addAfter' routine in TR_PersistentList should be "
                "called\n");
      p = new (this->trMemory(), this->_allocKind)
...
public:
  ...
  ListElement<T> *add(T *p) {
    TR_ASSERT(_pList->_allocKind != persistentAlloc,
              "Should not be using heap allocation to append to a persistent "
              "list. Should use 'addPersistent' instead\n");
    return addListElement(new (_pList->trMemory(), _pList->_allocKind)
...
public:
  ...
  ListElement<T> *add(T *p) {
    return List<T>::_pHead = new (List<T>::trStackMemory())
               ListElement<T>(p, List<T>::_pHead);
  }
...
public:
  TR_PersistentList() : List<T>((TR_Memory *)0) {
    List<T>::_pHead = 0;
    List<T>::_allocKind = persistentAlloc;
  }
  TR_PersistentList(T *p) : List<T>((TR_Memory *)0) {
...
public:
    ...
    if (_pTail == NULL) {
      List<T>::_pHead = _pTail =
          _useFreeList ? new (_freeList) ListElement<T>(elem, List<T>::_pHead)
                       : new (List<T>::trStackMemory())
                             ListElement<T>(elem, List<T>::_pHead);
    } else {
      ListElement<T> *p = _useFreeList ? new (_freeList)
                                             ListElement<T>(elem, NULL)
                                       : new (List<T>::trStackMemory())
                                             ListElement<T>(elem, NULL);
  ...
  void releaseFreeList() {
    ...
    // itself survives
    // beyond the stack scope of allocation of items and thus need to reset the
    // free
...
public:
  ...
  ListHeadAndTail(TR_Memory *m, T *p) : List<T>(m) {
    init();
    add(p);
  }
  ...
  ListElement<T> *add(T *p) {
    this->_pHead = new (this->trMemory(), this->_allocKind)
        ListElement<T>(p, this->_pHead);
    if (!_pTail)
      _pTail = this->_pHead;
    return this->_pHead;
  ...
  ListElement<T> *append(T *p) {
    ListElement<T> *ret =
        new (this->trMemory(), this->_allocKind) ListElement<T>(p, 0);
    if (_pTail)
  ...
  T *remove(T *elem) {
    TR_ASSERT(0, "remove is not implemented for ListHeadAndTail");
  }
};
