...
// headed up with an aiadd
// but can also be an iadd or ladd tree in the case of unsafe get/put being
// used.
//
...
bool TR_AddressTree::process(TR::Node *aiaddNode,
  ...
  // l2i's that can be strewn about for
      ...
      if (opCodeIsubSecondChild != TR::iconst &&
        ...
        if (opCodeIsubFirstChild == TR::imul ||
          ...
          if (indSymFromSubTree->getOpCodeValue() == TR::iload ||
            ...
            // induction
            // variable active for loops we consider, but it does however
            // require that
            // the induction variable be the first child, so we will catch a
            // case of:
            //   a[indVar + j]
            // but not:
            //   a[j + indVar]
