...
protected:
  ...
  TreeTopIteratorImpl(TR::TreeTop *start, TR::Optimization *opt = NULL,
...
public:
  TreeTopIterator(TR::TreeTop *start, TR::Optimization *opt = NULL,
                  const char *name = NULL)
...
protected:
  ...
  NodeIterator(TR::TreeTop *start, TR::Optimization *opt = NULL,
               const char *name = NULL);
...
public:
  PreorderNodeIterator(TR::TreeTop *start, TR::Optimization *opt = NULL,
                       const char *name = NULL);
...
protected:
  NodeOccurrenceIterator(TR::TreeTop *start, TR::Optimization *opt = NULL,
                         const char *name = NULL)
...
public:
  AllBlockIterator(TR::CFG *cfg, TR::Optimization *opt = NULL,
                   const char *name = NULL);
...
public:
  ...
  // "Valid" comprises any kind of validity rules anyone might want to impose on
  // the trees.
  //
...
protected: // Soundness
  ...
  void soundnessRule(TR::TreeTop *location, bool condition,
                     const char *formatStr,
                     ...); // returns isSoundSoFar for convenience
    ...
    public:
      ...
      TR::Node *currentNode() {
        if (isPastEnd())
          ...
          return _window._sideTable.getAt(_window._basis + _currentOffset)
              ._node;
      }
