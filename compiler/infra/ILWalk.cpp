...
void TR::TreeTopIteratorImpl::logCurrentLocation() {
  ...
  if (_name && _opt && _opt->trace() &&
      _opt->comp()->getOption(TR_TraceILWalks)) {
    if (currentTree()) {
...
TR::NodeIterator::NodeIterator(TR::TreeTop *start, TR::Optimization *opt,
                               const char *name)
    : TreeTopIteratorImpl(start, opt, name), _checklist(opt->comp()),
...
TR::Node *TR::PreorderNodeIterator::currentNode() { return _stack.top()._node; }
...
TR::PreorderNodeOccurrenceIterator::PreorderNodeOccurrenceIterator(
    TR::TreeTop *start, TR::Optimization *opt, const char *name)
...
TR::ReversePostorderSnapshotBlockIterator::
    ReversePostorderSnapshotBlockIterator(TR::CFG *cfg, TR::Optimization *opt,
                                          const char *name)
    : BlockIterator(opt, name),
      _postorder(opt->comp()->trMemory(), cfg->getNumberOfNodes(), false,
                 stackAlloc) {
...
void TR::ReversePostorderSnapshotBlockIterator::stepForward() {
  TR_ASSERT(currentBlock(), "cannot stepForward a "
                            "ReversePostorderSnapshotBlockIterator that has "
                            "already finished its walk");
  do
    --_currentIndex;
...
TR::AllBlockIterator::AllBlockIterator(TR::CFG *cfg, TR::Optimization *opt,
                                       const char *name)
    : BlockIterator(opt, name), _cfg(cfg),
  ...
  if (REMOVED_BLOCKS_CAN_BE_REINSERTED && !next) {
    ...
    for (next = _cfg->getFirstNode(); next; next = next->getNext()) {
      if (!_alreadyVisited.contains(next->asBlock())) {
        ...
        //
        if (isLoggingEnabled())
          traceMsg(_opt->comp(), "BLOCK  %s REMOVED_BLOCKS_CAN_BE_REINSERTED: "
                                 "block_%d found via extra scan\n",
...
TR::ILValidator::ILValidator(TR::Optimization *opt)
    : _opt(opt), _nodeStates(opt->comp()->trMemory()),
      _liveNodes(_nodeStates, opt->comp()->trMemory()) {}

TR::Compilation *TR::ILValidator::comp() { return _opt->comp(); }
...
void TR::ILValidator::checkSoundness(TR::TreeTop *start, TR::TreeTop *stop) {
  ...
  // Can't use iterators here, because those presuppose the IL is sound.  Walk
  // trees the old-fashioned way.
  //
  for (TR::TreeTop *currentTree = start; currentTree != stop;
       currentTree = currentTree->getNextTreeTop()) {
    soundnessRule(currentTree, currentTree->getNode() != NULL,
                  "Tree must have a node");
    soundnessRule(currentTree, !treetopNodes.contains(currentTree->getNode()),
    ...
    if (next) {
      soundnessRule(currentTree, next->getNode() != NULL,
...
void TR::ILValidator::checkNodeSoundness(TR::TreeTop *location, TR::Node *node,
                                         NodeChecklist &ancestorNodes,
                                         NodeChecklist &visitedNodes) {
  TR_ASSERT(node != NULL, "checkNodeSoundness requires that node is not NULL");
  if (visitedNodes.contains(node))
    return;
  visitedNodes.add(node);

  soundnessRule(location, !ancestorNodes.contains(node),
                "n%dn must not be its own ancestor", node->getGlobalIndex());
  ancestorNodes.add(node);

  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    TR::Node *child = node->getChild(i);
    soundnessRule(location, child != NULL, "n%dn child %d must not be NULL",
...
void TR::ILValidator::soundnessRule(TR::TreeTop *location, bool condition,
  ...
  if (!condition) {
    ...
    va_end(args);
    fprintf(stderr, "\n");
    static char *continueAfterValidationError =
        feGetEnv("TR_continueAfterValidationError");
...
void TR::ILValidator::validityRule(Location &location, bool condition,
                                   const char *formatStr, ...) {
  if (!condition) {
    // TODO: Beef this up to do smart things in debug and prod
    _isValidSoFar = false;
...
void TR::ILValidator::updateNodeState(Location &newLocation) {
  ...
  if (node->getReferenceCount() == state._futureReferenceCount) {
    ...
    if (node->getReferenceCount() == 0) {
      validityRule(newLocation, node->getOpCode().isTreeTop(),
    ...
    //
    for (auto childIter = node->childIterator(); TR::Node *child = *childIter;
         ++childIter) {
      NodeState &childState = _nodeStates[child];
      if (_liveNodes.contains(child)) {
        validityRule(newLocation, childState._futureReferenceCount >= 1,
                     "Child %s n%dn already has reference count 0",
        ...
        validityRule(newLocation, node->getOpCode().isTreeTop(),
                     "Child %s n%dn has already gone dead",
                     child->getOpCode().getName(), child->getGlobalIndex());
      }
    }
  }
  ...
  if (isLoggingEnabled()) {
    ...
    if (traceLiveNodesDuringValidation && !_liveNodes.isEmpty()) {
      ...
      char *separator = "";
      for (LiveNodeWindow::Iterator lnwi(_liveNodes); lnwi.currentNode();
           ++lnwi) {
...
bool TR::ILValidator::treesAreValid(TR::TreeTop *start, TR::TreeTop *stop) {
  ...
  for (PostorderNodeOccurrenceIterator iter(start, _opt, "VALIDATOR");
       iter != stop; ++iter) {
    ...
    if (node->getOpCodeValue() == TR::BBEnd) {
      ...
      if (nextTree) {
        validityRule(iter, nextTree->getNode()->getOpCodeValue() == TR::BBStart,
...
TR::ILValidator::LiveNodeWindow::LiveNodeWindow(
    NodeSideTable<NodeState> &sideTable, TR_Memory *memory)
  ...
  for (LiveNodeWindow::Iterator lnwi(_liveNodes); lnwi.currentNode(); ++lnwi) {
    ...
    if (liveNode->isInternalPointer())
      validityRule(location, liveNode->getPinningArrayPointer() != NULL,
                   "Internal pointer node n%dn without pinning array pointer "
                   "cannot live across asynccheck",
