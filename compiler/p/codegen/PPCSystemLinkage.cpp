...
TR_PPCSystemLinkage::TR_PPCSystemLinkage(TR::CodeGenerator *cg)
  ...
  // you must
  // assume the additional 64 bits are volatile because the callee will only
  // preserve
  // the FPR portion and the additional 64 bits will be undefined after the
  // callee returns.
  if (TR::Compiler->target.is64Bit()) {
...
const TR_PPCLinkageProperties &TR_PPCSystemLinkage::getProperties() {
  return _properties;
}

void TR_PPCSystemLinkage::initPPCRealRegisterLinkage() {
  ...
  machine->getPPCRealRegister((TR::RealRegister::RegNum)icount)
...
uint32_t TR_PPCSystemLinkage::getRightToLeft() {
  // *this    swipeable for debugging purposes
  return getProperties().getRightToLeft();
}

bool TR_PPCSystemLinkage::hasToBeOnStack(TR::ParameterSymbol *parm) {
  return (parm->getAllocatedIndex() >= 0 && parm->isParmHasToBeOnStack());
}

uintptr_t
TR_PPCSystemLinkage::calculateActualParameterOffset(uintptr_t o,
...
uintptr_t
TR_PPCSystemLinkage::calculateParameterRegisterOffset(uintptr_t o,
                                                      TR::ParameterSymbol &p) {
  TR::ResolvedMethodSymbol *bodySymbol = comp()->getJittedMethodSymbol();
  if (1 || (p.getDataType() == TR::Aggregate) ||
      (p.getSize() >= sizeof(uint64_t))) {
    return o;
  } else {
...
void TR_PPCSystemLinkage::mapParameters(TR::ResolvedMethodSymbol *method,
  ...
  if (linkage.getRightToLeft()) {
    while (parmCursor != NULL) {
      if (saveParmsInLocalArea)
        parmCursor->setParameterOffset(calculateActualParameterOffset(
            offset_from_top + stackIndex, *parmCursor));
      else
        parmCursor->setParameterOffset(calculateActualParameterOffset(
            offset_from_top + offsetToFirstParm + stackIndex, *parmCursor));
      offset_from_top +=
...
void TR_PPCSystemLinkage::mapStack(TR::ResolvedMethodSymbol *method) {
  // *this    swipeable for debugging purposes
  ...
  if (!method->isEHAware()) {
    while (regIndex <= TR::RealRegister::LastGPR &&
           !machine->getPPCRealRegister(regIndex)->getHasBeenAssignedInMethod())
...
void TR_PPCSystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol *p,
                                             uint32_t &stackIndex) {
  // *this    swipeable for debugging purposes
  size_t align = 1;
...
void TR_PPCSystemLinkage::createPrologue(TR::Instruction *cursor) {
...
void TR_PPCSystemLinkage::createPrologue(TR::Instruction *cursor,
  ...
  cg()->setStackPointerRegister(
      machine->getPPCRealRegister(properties.getNormalStackPointerRegister()));
  ...
  if (!bodySymbol->isEHAware()) {
    while (
        savedFirst <= TR::RealRegister::LastFPR &&
        !machine->getPPCRealRegister(savedFirst)->getHasBeenAssignedInMethod())
  ...
  for (regIndex = TR::RealRegister::LastFPR; regIndex >= savedFirst;
    ...
    cursor = generateMemSrc1Instruction(
        ...
        machine->getPPCRealRegister(regIndex), cursor);
  }

  savedFirst = TR::RealRegister::gr13;
  if (!bodySymbol->isEHAware()) {
    while (
        savedFirst <= TR::RealRegister::LastGPR &&
        !machine->getPPCRealRegister(savedFirst)->getHasBeenAssignedInMethod())
      savedFirst = (TR::RealRegister::RegNum)((uint32_t)savedFirst + 1);
  }
    ...
    if (TR::Compiler->target.cpu.id() == TR_PPCgp ||
      ...
      cursor = generateMemSrc1Instruction(
          ...
          new (trHeapMemory()) TR::MemoryReference(
              ...
              cg()),
          machine->getPPCRealRegister(savedFirst), cursor);
    }
  }

  if (bodySymbol->isEHAware() ||
      machine->getPPCRealRegister(TR::RealRegister::cr2)
          ->getHasBeenAssignedInMethod() ||
      machine->getPPCRealRegister(TR::RealRegister::cr3)
          ->getHasBeenAssignedInMethod() ||
      machine->getPPCRealRegister(TR::RealRegister::cr4)
          ->getHasBeenAssignedInMethod()) {
...
void TR_PPCSystemLinkage::createEpilogue(TR::Instruction *cursor) {
  // *this    swipeable for debugging purposes
  ...
  for (regIndex = TR::RealRegister::LastFPR; regIndex >= savedFirst;
    ...
    if (cg()->restoreRegister(regIndex, blockNumber))
      cursor = generateTrg1MemInstruction(
          ...
          machine->getPPCRealRegister(regIndex),
  ...
  if (savedFirst <= TR::RealRegister::LastGPR) {
    if (TR::Compiler->target.cpu.id() == TR_PPCgp ||
      ...
      for (regIndex = TR::RealRegister::LastGPR; regIndex >= savedFirst;
        ...
        if (cg()->restoreRegister(regIndex, blockNumber))
          cursor = generateTrg1MemInstruction(
              ...
              machine->getPPCRealRegister(regIndex),
              new (trHeapMemory()) TR::MemoryReference(
                  sp, saveSize, TR::Compiler->om.sizeofReferenceAddress(),
                  cg()),
              cursor);
      }
    else {
      ...
      cursor = generateTrg1MemInstruction(
          cg(), (savedFirst == TR::RealRegister::LastGPR) ? TR::InstOpCode::lwz
                                                          : TR::InstOpCode::lmw,
          currentNode, machine->getPPCRealRegister(savedFirst),
          new (trHeapMemory()) TR::MemoryReference(
...
int32_t
TR_PPCSystemLinkage::buildArgs(TR::Node *callNode,
...

{
  ...
  for (i = firstArgumentChild; i < callNode->getNumChildren(); i++) {
    ...
    case TR::Address: // have to do something for GC maps here
      ...
      if (numIntegerArgs < properties.getNumIntArgRegs()) {
        ...
        if (numIntegerArgs == 0 &&
          ...
          addDependency(dependencies, argRegister,
    ...
    case TR::Int64:
      ...
      if (!aix_style_linkage) {
        if (numIntegerArgs & 1) {
          if (numIntegerArgs < properties.getNumIntArgRegs())
            addDependency(dependencies, NULL,
                          properties.getIntegerArgumentRegister(numIntegerArgs),
                          TR_GPR, cg());
          numIntegerArgs++;
        }
      ...
      for (int r = 0; r < ((childType == TR::Float) ? 1 : 2); r++) {
        ...
        if (numFloatArgs < properties.getNumFloatArgRegs()) {
          ...
          if ((numFloatArgs == 0 && resType.isFloatingPoint())) {
            ...
            addDependency(dependencies, argReg,
  ...
  if (liveVMX || liveVSXScalar || liveVSXVector) {
    for (i = (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::LastFPR +
      ...
      if (!properties.getPreserved((TR::RealRegister::RegNum)i) || !isHelper) {
        addDependency(dependencies, NULL, (TR::RealRegister::RegNum)i,
                      TR_VSX_SCALAR, cg());
      }
    }
  }
  ...
  for (i = (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::fp0 +
    ...
    if (!properties.getPreserved((TR::RealRegister::RegNum)i) ||
        liveVSXVector) {
      addDependency(dependencies, NULL, (TR::RealRegister::RegNum)i, TR_FPR,
                    cg());
    }
  }
...
void TR_PPCSystemLinkage::buildDirectCall(
    TR::Node *callNode, TR::SymbolReference *callSymRef,
    TR::RegisterDependencyConditions *dependencies,
    const TR_PPCLinkageProperties &pp, int32_t argSize) {
...
void TR_PPCSystemLinkage::buildVirtualDispatch(
...
// Here we use aix_style_linkage only.
  ...
  TR_ASSERT((gr3 != NULL), "TR_PPCSystemLinkage::buildVirtualDispatch no "
  ...
  TR::RealRegister *grSysStackReg = cg()->machine()->getPPCRealRegister(
      properties.getNormalStackPointerRegister());

  TR_ASSERT((gr0 != NULL), "TR_PPCSystemLinkage::buildVirtualDispatch no "
                           "dependence set on scratch register gr0.");
  TR_ASSERT((grTOCReg != NULL), "TR_PPCSystemLinkage::buildVirtualDispatch no "
                                "dependence set on Library TOC register gr2.");
  TR_ASSERT((grSysStackReg != NULL), "TR_PPCSystemLinkage::"
                                     "buildVirtualDispatch no dependence set "
                                     "on Native C Stack register gr1.");
...
void TR_PPCSystemLinkage::setParameterLinkageRegisterIndex(
...
void TR_PPCSystemLinkage::setParameterLinkageRegisterIndex(
  ...
  while ((paramCursor != NULL) &&
    ...
    case TR::Double:
      ...
      numFloatArgs++;
      numIntArgs++;
      if (paramCursor->getDataType() == TR::Double)
        numIntArgs++;
      break;
    case TR::Aggregate: {
