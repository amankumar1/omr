...
OMR::Power::MemoryReference::MemoryReference(TR::Node *rootLoadOrStore,
  ...
  if (rootLoadOrStore->getOpCode().isIndirect()) {
    ...
    if (!ref->isUnresolved() && isLocalObject) {
      ...
      if (ref->isUnresolved()) {
        ...
        // register
        // rather than getting the stack pointer as the base which is not
        // correct.
        if (isLocalObject)
          cg->evaluate(base);
  ...
  if (symbol->isParm()) {
    //        setOffset(getOffset() +
    //        cg->getLinkage()->calculateActualParameterOffset(0,
    //        *symbol->castToParmSymbol()));
  }
  // TODO: aliasing sets?
}
...
void OMR::Power::MemoryReference::populateMemoryReference(
  ...
  if (subTree->getOpCodeValue() == TR::i2l && subTree->isNonNegative() &&
      ...
      subTree->getReferenceCount() == 1 && subTree->getRegister() == NULL) {
    if (performTransformation(cg->comp(), "O^O PPC Evaluator: Skip unecessary "
                                          "sign extension on index [%p].\n",
                              subTree)) {
...
void OMR::Power::MemoryReference::consolidateRegisters(TR::Register *srcReg,
  ...
  TR::Node *tempNode =
      (srcTree == NULL) ? comp->getAppendInstruction()->getNode() : srcTree;
...
uint8_t *OMR::Power::MemoryReference::generateBinaryEncoding(
  ...
  base = (self()->getBaseRegister() == NULL)
             ? cg->machine()->getPPCRealRegister(TR::RealRegister::gr0)
  ...
  if (TR::Compiler->target.is64Bit() && self()->isTOCAccess()) {
    ...
    if (self()->getUnresolvedSnippet() != NULL) {
      ...
      cg->addRelocation(
          new (cg->trHeapMemory()) TR_24BitLabelRelativeRelocation(
  ...
  if (self()->getUnresolvedSnippet() != NULL) {
    ...
    getUnresolvedSnippet()->setAddressOfDataReference(cursor);
    getUnresolvedSnippet()->setMemoryReference(self());
    cg->addRelocation(new (cg->trHeapMemory()) TR_24BitLabelRelativeRelocation(
        cursor, getUnresolvedSnippet()->getSnippetLabel()));
    *wcursor = 0x48000000; // b SnippetLabel;
    wcursor++;
    cursor += PPC_INSTRUCTION_LENGTH;
      ...
      *wcursor = preserve;
      ...
      if (self()->isBaseModifiable()) {
        ...
        *wcursor = preserve | lower;
        ...
        TR::RealRegister *rX = cg->machine()->getPPCRealRegister(
            choose_rX(currentInstruction, base));
        *wcursor = 0x9000fffc;
...
void OMR::Power::MemoryReference::accessStaticItem(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (cg->comp()->compileRelocatableCode()) {
      /* don't want to trash node prematurely by the code for handling guarded
       * counting recompilations symbols */
      ...
      if (symbol->isCountForRecompile()) {
        ...
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        loadAddressConstantInSnippet(cg, GCRnode, 0, reg, NULL,
    ...
    if (ref->isUnresolved() || comp->compileRelocatableCode()) {
      ...
      if (symbol->isCountForRecompile()) {
        ...
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(cg, GCRnode, 0, reg, NULL, false,
                            TR_BodyInfoAddressLoad);
      } else if (symbol->isCompiledMethod()) {
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(cg, GCRnode, 0, reg, NULL, false,
      ...
      // Unless we get a new type of relocation, we have to totally load the
      // address for
      // AOT purpose for long data type since its low half can reuse the memory
      // reference
      // without appropriate relocation to fix it.
      specialCase = symbol->isStatic() && (symbol->getType().isInt64()) &&
      ...
      if (symbol->isStartPC()) {
        loadAddressConstantInSnippet(cg, node, 0, reg, NULL,
                                     TR::InstOpCode::addi, false, NULL);
        return;
      }
      ...
      if (symbol->isConst()) {
        relocationTarget =
            (uint8_t *)ref->getOwningMethod(comp)->constantPool();
      ...
      if (comp->getOption(TR_AOT)) {
        ...
        staticRelocation = new (cg->trHeapMemory()) TR_PPCPairedRelocation(
            rel1, rel2, (uint8_t *)recordInfo, relocationKind, node);
