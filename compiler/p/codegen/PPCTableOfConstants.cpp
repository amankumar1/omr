...
void *TR_PPCTableOfConstants::initTOC(TR_FrontEnd *fe,
  ...
  // tocBase[TR_PPCnumRuntimeHelpers-1] is free.
  ...
  while (chain != CHAIN_END) {
    ...
    case TR_FLAG_tocFloatKey:
      if (hash[idx]._fKey == tmplate->_fKey) {
        if (hash[idx]._flag & TR_FLAG_tocFloatHigh)
          *offsetInSlot = 4;
        if (debug("traceTOCHash"))
          ; // diagnostic(comp, "   Hash Hit float:%x    return:%d
    ...
    case TR_FLAG_tocStatic2ClassKey:
      if (hash[idx]._keyTag == tmplate->_keyTag &&
        ...
        if (debug("traceTOCHash"))
          ; // diagnostic(comp, "   Hash Hit static2class:%p  cpIndex:%d
            // return:%d\n", (void *)hash[idx]._cpKey, hash[idx]._staticCPIndex,
            // rval);
        tocManagement->getTOCMonitor()->exit();
        return rval;
      }
  ...
  case TR_FLAG_tocDoubleKey:
    ...
    if (debug("traceTOCHash"))
      ; // diagnostic(comp, "    New hash double:%p set:%p  return:%d\n", (void
        // *)hash[idx]._dKey, (void *)getTOCSlot(rval*sizeof(intptrj_t)), rval);
    break;
  case TR_FLAG_tocFloatKey:
    ...
    if (debug("traceTOCHash"))
      ; // diagnostic(comp, "    New hash float:%x set:%p return:%d
        // offset:%d\n", hash[idx]._fKey, (void
        // *)getTOCSlot(rval*sizeof(intptrj_t)), rval, *offsetInSlot);
    break;
  case TR_FLAG_tocStatic2ClassKey:
    hash[idx]._keyTag = tmplate->_keyTag;
    hash[idx]._cpKey = tmplate->_cpKey;
    hash[idx]._staticCPIndex = tmplate->_staticCPIndex;
    if (debug("traceTOCHash"))
      ; // diagnostic(comp, "    New hash static2class:%p cpIndex:%d
        // return:%d\n", (void *)hash[idx]._cpKey, hash[idx]._staticCPIndex,
        // rval);
    break;
  }
...
bool TR_PPCTableOfConstants::isPermanentEntriesAddtionComplete() // only
                                                                 // zEmulator
                                                                 // uses this
{
  bool retValue;
