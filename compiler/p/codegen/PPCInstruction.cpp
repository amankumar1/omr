...
void TR::PPCLabelInstruction::assignRegisters(
  ...
  if (((isBranchOp() && !usesCountRegister()) ||
    ...
    TR_ASSERT(oi, "Could not find PPCOutOfLineCodeSection stream from label.  "
                  "instr=%p, label=%p\n",
              this, getLabelSymbol());
...
void TR::PPCTrg1Instruction::assignRegisters(TR_RegisterKinds kindToBeAssigned,
  ...
  if (excludeGPR0 && (assignedRegister != NULL) &&
      (toRealRegister(assignedRegister) ==
       machine->getPPCRealRegister(TR::RealRegister::gr0))) {
...
void TR::PPCControlFlowInstruction::assignRegisters(
  ...
  case TR::InstOpCode::setbool:
    if (TR::Compiler->target.cpu.id() >= TR_PPCp9 &&
      ...
      cg()->traceRAInstruction(cursor = generateTrg1Src2ImmInstruction(
                                   ...
                                   getTargetRegister(0), getSourceImmediate(2),
                                   cursor));
  ...
  case TR::InstOpCode::cfnan:
    if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
      ...
      cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                                   cg(), TR::InstOpCode::beq,
                                   PPCOpProp_BranchLikely, currentNode, label2,
                                   getSourceRegister(1), cursor));
    else
      cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                                   cg(), TR::InstOpCode::beq, currentNode,
                                   label2, getSourceRegister(1), cursor));
    cg()->traceRAInstruction(cursor = generateTrg1ImmInstruction(
                                 cg(), TR::InstOpCode::lis, currentNode,
                                 getTargetRegister(0), 0x7fc0, cursor));
    cg()->traceRAInstruction(
        cursor = generateLabelInstruction(cg(), TR::InstOpCode::label,
                                          currentNode, label2, cursor));
    break;
  case TR::InstOpCode::cdnan:
    if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
      ...
      cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                                   cg(), TR::InstOpCode::beq, currentNode,
                                   label2, getSourceRegister(0), cursor));
    if (TR::Compiler->target.is64Bit()) {
      cg()->traceRAInstruction(
          cursor = loadConstant(cg(), currentNode,
                                (int64_t)CONSTANT64(0x7ff8000000000000),
                                getTargetRegister(0), cursor));
    } else {
      cg()->traceRAInstruction(cursor = generateTrg1ImmInstruction(
                                   cg(), TR::InstOpCode::lis, currentNode,
                                   getTargetRegister(1), 0x7ff8, cursor));
      cg()->traceRAInstruction(cursor = generateTrg1ImmInstruction(
                                   cg(), TR::InstOpCode::li, currentNode,
                                   getTargetRegister(0), 0, cursor));
    }
    cg()->traceRAInstruction(
        cursor = generateLabelInstruction(cg(), TR::InstOpCode::label,
                                          currentNode, label2, cursor));
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCp8)
      cg()->traceRAInstruction(
          cursor = generateTrg1Src1Instruction(
              cg(), TR::InstOpCode::mfvsrwz, currentNode, getTargetRegister(2),
              getTargetRegister(1),
              cursor)); // generateTrg1Src1Instruction(cg(),
                        // TR::InstOpCode::mftgpr, currentNode,
  ...
  case TR::InstOpCode::iternary: {
    // cursor = generateTrg1Src2Instruction   (cg(), TR::InstOpCode::cmp8,
    ...
    cg()->traceRAInstruction(
        cursor = generateTrg1Src1Instruction(cg(), getOpCode2Value(),
                                             currentNode, getTargetRegister(1),
                                             getSourceRegister(1), cursor));
    cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
