...
void TR_Debug::print(TR::FILE *pOutFile, TR::PPCDepInstruction *instr) {
  printPrefix(pOutFile, instr);
  trfprintf(pOutFile, "%s", getOpCodeName(&instr->getOpCode()));
  if (_comp->getOption(TR_DisableShrinkWrapping) &&
      instr->getDependencyConditions())
...
bool TR_Debug::isBranchToTrampoline(TR::SymbolReference *symRef,
  ...
  if (distance < BRANCH_BACKWARD_LIMIT || distance > BRANCH_FORWARD_LIMIT) {
    distance = _comp->fe()->indexedTrampolineLookup(
...
void TR_Debug::print(TR::FILE *pOutFile, TR::PPCDepImmSymInstruction *instr) {
  ...
  if (cursor) {
    ...
    } else if (distance < BRANCH_BACKWARD_LIMIT ||
               distance > BRANCH_FORWARD_LIMIT) {
      int32_t refNum = instr->getSymbolReference()->getReferenceNumber();
      if (refNum < TR_PPCnumRuntimeHelpers) {
        distance =
            _comp->fe()->indexedTrampolineLookup(refNum, (void *)cursor) -
            (intptrj_t)cursor;
      } else {
        distance = _comp->fe()->methodTrampolineLookup(
...
void TR_Debug::printPPCGCRegisterMap(TR::FILE *pOutFile,
  ...
  for (int i = 31; i >= 0; i--) {
    if (map->getMap() & (1 << i))
      trfprintf(pOutFile, "%s ",
                getName(machine->getPPCRealRegister((TR::RealRegister::RegNum)(
