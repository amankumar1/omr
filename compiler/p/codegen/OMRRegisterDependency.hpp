  ...
  void setExcludeGPR0(TR::Register *r, uint32_t numberOfRegisters) {
    for (uint32_t i = 0; i < numberOfRegisters; ++i) {
      if (_dependencies[i].getRegister() == r) {
        ...
        // sufficient to exclude gr0 on the first of such dependencies
        // because !gr0 NoReg dependencies are handled before NoReg
        // dependencies, so we'll assign it to !gr0 first and then any remaining
...
public:
  ...
  TR::Register *searchPreConditionRegister(TR::RealRegister::RegNum rr) {
    return (_preConditions == NULL ? NULL : _preConditions->searchForRegister(
                                                rr, _addCursorForPre));
  }

  TR::Register *searchPostConditionRegister(TR::RealRegister::RegNum rr) {
    return (_postConditions == NULL ? NULL : _postConditions->searchForRegister(
                                                 rr, _addCursorForPost));
  }
...
// that an evaluator can use
// and therefore frees you from trying to determine ahead of time how many
// dependencies you need.
// It can create a TR::RegisterDependencyConditions object with exactly the
// number of dependencies currently defined.
// Therefore, objects of this class should be stack allocated so that they'll
// automatically be freed when no longer
// needed.
//
