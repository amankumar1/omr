...
OMR::Power::TreeEvaluator::dloadHelper(TR::Node *node, TR::CodeGenerator *cg,
  ...
  if (TR::Compiler->target.is32Bit() && needSync && !cg->is64BitProcessor()) {
    ...
    if (node->getSymbolReference()->isUnresolved()) {
      ...
      // layout
      // needs to be fixed for patching if it turns out to be not a volatile
      // after all.
...
static TR::Register *generateFusedMultiplyAdd(TR::Node *addNode,
  ...
  if (!isFPStrictMul(mulNode, cg->comp())) {
    ...
    // TR_ASSERT(!addChild->getOpCode().isSub(),"Subtract not commutative %p
    // %p\n", addNode, addChild);
  }
...
TR::Register *OMR::Power::TreeEvaluator::fremEvaluator(TR::Node *node,
  ...
  generateHelperBranchAndLinkInstruction(TR_PPCdoubleRemainder, node,
                                         dependencies, cg);
...
TR::Register *OMR::Power::TreeEvaluator::int2dbl(TR::Node *node,
  ...
  if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() &&
    ...
    generateHelperBranchAndLinkInstruction(TR_PPCinteger2Double, node,
                                           dependencies, cg);
...
TR::Register *OMR::Power::TreeEvaluator::i2fEvaluator(TR::Node *node,
  ...
  if (((TR::Compiler->target.cpu.id() >= TR_PPCp7 &&
    ...
    trgReg = int2dbl(node, cg->evaluate(child),
...
TR::Register *OMR::Power::TreeEvaluator::long2dbl(TR::Node *node,
  ...
  if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() &&
    ...
    generateHelperBranchAndLinkInstruction(TR_PPClong2Double, node,
                                           dependencies, cg);
...
TR::Register *OMR::Power::TreeEvaluator::long2float(TR::Node *node,
  ...
  if (TR::Compiler->target.cpu.id() >= TR_PPCp7 &&
    ...
    generateHelperBranchAndLinkInstruction(TR_PPClong2Float, node, dependencies,
                                           cg);
...
TR::Register *OMR::Power::TreeEvaluator::d2lEvaluator(TR::Node *node,
  ...
  if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() &&
    ...
    addDependency(dependencies, trgReg->getHighOrder(), TR::RealRegister::gr3,
                  TR_GPR, cg);
    addDependency(dependencies, trgReg->getLowOrder(), TR::RealRegister::gr4,
                  TR_GPR, cg);
    addDependency(dependencies, NULL, TR::RealRegister::gr0, TR_GPR, cg);
    addDependency(dependencies, NULL, TR::RealRegister::gr5, TR_GPR, cg);
    addDependency(dependencies, NULL, TR::RealRegister::gr6, TR_GPR, cg);
    ...
    generateHelperBranchAndLinkInstruction(TR_PPCdouble2Long, node,
                                           dependencies, cg);
...
OMR::Power::TreeEvaluator::dcmpltEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.cpu.id() >= TR_PPCp9) {
    imm = (TR::RealRegister::FPCC_FL << 21 | TR::RealRegister::FPCC_FL << 16 |
           TR::RealRegister::FPCC_FU << 11);
  }
...
OMR::Power::TreeEvaluator::dcmpgtEvaluator(TR::Node *node,
  ...
  int64_t imm = 0;
  if (TR::Compiler->target.cpu.id() >= TR_PPCp9) {
    imm = (TR::RealRegister::FPCC_FG << 21 | TR::RealRegister::FPCC_FG << 16 |
           TR::RealRegister::FPCC_FU << 11);
  }
  return compareFloatAndSetOrderedBoolean(TR::InstOpCode::bgt,
                                          TR::InstOpCode::bad, imm, node, cg);
...
OMR::Power::TreeEvaluator::getstackEvaluator(TR::Node *node,
  ...
  TR::Register *spReg = cg->machine()->getPPCRealRegister(
