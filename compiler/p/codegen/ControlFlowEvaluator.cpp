...
extern TR::Register *addConstantToInteger(TR::Node *node, TR::Register *trgReg,
...
extern TR::Register *addConstantToLong(TR::Node *node, TR::Register *srcReg,
                                       ...
                                       TR::CodeGenerator *cg);
extern TR::Register *addConstantToLong(TR::Node *node, TR::Register *srcHigh,
...
extern void generateZeroExtendInstruction(TR::Node *node, TR::Register *trgReg,
                                          ...
                                          TR::CodeGenerator *cg);
extern void generateSignExtendInstruction(TR::Node *node, TR::Register *trgReg,
                                          TR::Register *srcReg,
                                          TR::CodeGenerator *cg);
...
extern TR::Register *generateZeroExtendedTempRegister(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  int32_t operandSize = node->getOpCode().getSize();
  TR_ASSERT(operandSize == 4 || operandSize == 2 || operandSize == 1,
            "operand size %d should not need zero extension", operandSize);
  TR::InstOpCode::Mnemonic loadOp =
      (operandSize == 4) ? TR::InstOpCode::lwz : (operandSize == 2)
                                                     ? TR::InstOpCode::lhz
                                                     : TR::InstOpCode::lbz;

  TR::Register *srcReg = NULL;
  if (node->getReferenceCount() == 1 && node->getOpCode().isMemoryReference() &&
      node->getRegister() == NULL) {
    srcReg = cg->allocateRegister();
    TR::MemoryReference *tempMR =
        new (cg->trHeapMemory()) TR::MemoryReference(node, operandSize, cg);
    ...
    if (cg->canClobberNodesRegister(node))
      ...
      srcReg = cg->allocateRegister();
    generateZeroExtendInstruction(node, srcReg, evaluatedSrcReg,
                                  operandSize * 8, cg);
  }
...
static TR::Register *generateSignExtendedTempRegister(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  int32_t operandSize = node->getOpCode().getSize();
  TR_ASSERT(operandSize == 4 || operandSize == 2 || operandSize == 1,
            "operand size %d should not need zero extension", operandSize);
  TR::InstOpCode::Mnemonic loadOp =
      (operandSize == 4) ? TR::InstOpCode::lwa : (operandSize == 2)
                                                     ? TR::InstOpCode::lha
                                                     : TR::InstOpCode::lbz;
  ...
  if (node->getReferenceCount() == 1 && node->getOpCode().isMemoryReference() &&
    ...
    if (loadOp == TR::InstOpCode::lbz)
      generateSignExtendInstruction(node, srcReg, srcReg,
                                    cg); // no lba instruction
    tempMR->decNodeReferenceCounts(cg);
  } else {
    TR::Register *evaluatedSrcReg = cg->evaluate(node);
    if (cg->canClobberNodesRegister(node))
...
static void computeCC_xcmpStrengthReducedCC(
    TR::Node *node, TR::Register *trgReg, TR::Register *src1Reg,
    TR::Register *src2Reg, uint8_t ccMask, bool isSignedCompare,
    bool needsZeroExtension, // not all unsigned compares need zero extension so
                             // cannot use !isSignedCompare
    TR::CodeGenerator *cg) {
  TR_ASSERT(ccMask == 0xc || ccMask == 0xa || ccMask == 0x6,
            "ccMask of 0x%x not supported\n", ccMask);
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (isSignedCompare && value <= UPPER_IMMED) {
      // when ccMask == 0xc then the value is encoded in the addi below in 16
      // bits as -value.  When value = 0x8000 = LOWER_IMMED
      // then -value also equals (in 16 bits) 0x8000 so an immediate form cannot
      // be used.
      if (ccMask == 0xc && (value > LOWER_IMMED))
        useImmedForm = true;
      else if (ccMask != 0xc && (value >= LOWER_IMMED))
        useImmedForm = true;
    } else if (!isSignedCompare &&
               (uint64_t)value <= 0x7fff) // in unsigned case cannot let addi2
                                          // and subfic sign extend
      useImmedForm = true;
  }

  if (needsZeroExtension) {
    TR_ASSERT(src1Reg == NULL && src2Reg == NULL,
              "clobber evaluate must be done when zero extending");
    TR_ASSERT(!isSignedCompare,
              " a signed comparison should not need zero extension\n");
  ...
  TR_ASSERT(operandSize == secondChild->getOpCode().getSize(),
            "operand sizes should match\n");
  if (isSignedCompare) {
    TR_ASSERT(src1Reg == NULL && src2Reg == NULL,
              "clobber evaluate must be done when sign extending");
    // it is not safe to rely on the upper bits so perform sign extension to a
    // temp register here
  ...
  } else if (ccMask == 0xa || ccMask == 0x6) // 0,2 or 1,2 possible
  {
    if (useImmedForm)
      ...
      generateTrg1Src2Instruction(cg, TR::InstOpCode::subf, node, trgReg,
                                  src1Reg, src2Reg); // op2-op1
    generateShiftRightLogicalImmediateLong(cg, node, trgReg, trgReg, 63);
    if (ccMask == 0xa)
      generateShiftLeftImmediate(cg, node, trgReg, trgReg,
                                 1); // sign_bit*2 : 0->0, 1->2
    else
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, trgReg,
                                     trgReg, 1); // sign_bit+1 : 0->1, 1->2
  }
...
TR::Register *computeCC_compareUnsigned(TR::Node *node, TR::Register *trgReg,
                                        TR::Register *src1Reg,
                                        TR::Register *src2Reg,
                                        bool is64BitCompare,
...
TR::Register *OMR::Power::TreeEvaluator::compareIntsForOrder(
  ...
  if (skipCompare(node)) {
    TR_ASSERT(secondChild->getOpCode().isLoadConst(),
              "_clc should be compared with 0");
    if (loopInversed)
...
OMR::Power::TreeEvaluator::iternaryEvaluator(TR::Node *node,
  ...
  if (firstChild->getOpCode().isBooleanCompare() &&
    ...
    if (compare_type.isIntegral() || compare_type.isAddress()) {
      ...
      if (!skip_compare) {
        if (useImmediateCompare) {
          generateTrg1Src1ImmInstruction(
              cg, cmp2cmpi(firstChild->getOpCodeValue(), cg), node, ccr,
              cmp1Reg, value);
        } else {
      ...
      cmp2Reg = cg->evaluate(firstChild->getSecondChild());
      if (!skip_compare)
        generateTrg1Src2Instruction(cg, TR::InstOpCode::fcmpu, node, ccr,
                                    cmp1Reg, cmp2Reg);
      generateTrg1Src1Instruction(cg, move_opcode, node, resultReg, trueReg);
      generateConditionalBranchInstruction(cg, branch_opcode, node, doneLabel,
                                           ccr);
    ...
    TR::PPCControlFlowInstruction *i =
        (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(
            cg, TR::InstOpCode::iternary, node);
    i->addTargetRegister(ccr);
    i->addTargetRegister(resultReg);
    i->addSourceRegister(condReg->getRegisterPair() ? condReg->getLowOrder()
                                                    : condReg);
...
TR::Register *OMR::Power::TreeEvaluator::compareIntsForEquality(
  ...
  if (firstChild->getOpCodeValue() == TR::b2i &&
     ...
     * not affected by sign extension
     * Skips evaluating b2i if it is unused (i.e. refcount == 1, which is
     * usually the case)
     */
    ...
    TR::InstOpCode::Mnemonic opCode =
        (node->getOpCodeValue() == TR::ificmpeq ||
         ...
         node->getOpCodeValue() == TR::ifsucmpeq ||
    ...
    // decremented, so we have to decrement it instead
    // If a child is in a register, regardless of whether or not we evaluated
    // it, we don't have to decrement the grandchild
      ...
      if (secondChild->getOpCode().isLoadConst() &&
        ...
        if (node->getOpCodeValue() == TR::ifbucmpne ||
          ...
          generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node,
                                          tReg, src1Reg, 0, 0xff);
          value &= 0xff;
        } else if (node->getOpCodeValue() == TR::ifsucmpne ||
                   node->getOpCodeValue() == TR::ifsucmpeq) {
          tReg = cg->allocateRegister();
          newReg = true;
          generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node,
...
OMR::Power::TreeEvaluator::compareLongsForEquality(TR::Node *node,
  ...
  TR::InstOpCode::Mnemonic branchOp = node->getOpCode().isCompareTrueIfEqual()
                                          ? TR::InstOpCode::beq
                                          : TR::InstOpCode::bne;
  return compareLongsForEquality(
      branchOp, node->getBranchDestination()->getNode()->getLabel(), node, cg,
      false, false);
}
  ...
  if (!disableCompareToOpt && TR::Compiler->target.is32Bit() &&
    ...
    if (firstShiftAmountNode->getOpCode().isLoadConst() &&
      ...
      if (firstShiftValue > 32 && secondShiftValue > 32) {
        ...
        return compareIntsForEquality(branchOp, dstLabel, node, cg, isHint,
                                      likeliness);
      }
    }
  }
...
OMR::Power::TreeEvaluator::iflcmpltEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  compareLongsForOrder(TR::InstOpCode::blt, TR::InstOpCode::bgt, node, cg,
                       true);
  return NULL;
}
...
OMR::Power::TreeEvaluator::iflcmpgeEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  compareLongsForOrder(TR::InstOpCode::bge, TR::InstOpCode::ble, node, cg,
                       true);
  return NULL;
}
...
OMR::Power::TreeEvaluator::iflcmpgtEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  compareLongsForOrder(TR::InstOpCode::bgt, TR::InstOpCode::blt, node, cg,
                       true);
  return NULL;
}
...
OMR::Power::TreeEvaluator::iflcmpleEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  compareLongsForOrder(TR::InstOpCode::ble, TR::InstOpCode::bge, node, cg,
                       true);
  return NULL;
}
...
TR::Register *handleSkipCompare(TR::Node *node, TR::InstOpCode::Mnemonic opcode,
                                TR::CodeGenerator *cg) {
  TR::Register *trgReg = cg->allocateRegister();
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
...
OMR::Power::TreeEvaluator::icmpltEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    TR::Register *src2Reg = cg->evaluate(secondChild);
    genBranchSequence(node, src1Reg, src2Reg, trgReg, TR::InstOpCode::blt,
...
OMR::Power::TreeEvaluator::icmpleEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    TR::Register *src2Reg = cg->evaluate(secondChild);
    genBranchSequence(node, src1Reg, src2Reg, trgReg, TR::InstOpCode::ble,
                      TR::InstOpCode::cmp4, cg);
  }
  node->setRegister(trgReg);
  cg->decReferenceCount(firstChild);
...
OMR::Power::TreeEvaluator::icmpgeEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    TR::Register *src2Reg = cg->evaluate(secondChild);
    genBranchSequence(node, src1Reg, src2Reg, trgReg, TR::InstOpCode::bge,
                      TR::InstOpCode::cmp4, cg);
  }
  node->setRegister(trgReg);
  cg->decReferenceCount(firstChild);
...
OMR::Power::TreeEvaluator::icmpgtEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    TR::Register *src2Reg = cg->evaluate(secondChild);
    genBranchSequence(node, src1Reg, src2Reg, trgReg, TR::InstOpCode::bgt,
                      TR::InstOpCode::cmp4, cg);
  }
  node->setRegister(trgReg);
  cg->decReferenceCount(firstChild);
...
static void lookupScheme1(TR::Node *node, bool unbalanced, bool fromTableEval,
  ...
  if (two_reg) {
    ...
    addDependency(conditions, selector->getHighOrder(), TR::RealRegister::NoReg,
                  TR_GPR, cg);
    addDependency(conditions, selector->getLowOrder(), TR::RealRegister::NoReg,
                  TR_GPR, cg);
  } else
...
static void lookupScheme2(TR::Node *node, bool unbalanced, bool fromTableEval,
  ...
  if (two_reg) {
    addDependency(conditions, valRegister->getHighOrder(),
                  TR::RealRegister::NoReg, TR_GPR, cg);
    addDependency(conditions, valRegister->getLowOrder(),
                  TR::RealRegister::NoReg, TR_GPR, cg);
    addDependency(conditions, selector->getHighOrder(), TR::RealRegister::NoReg,
                  TR_GPR, cg);
    addDependency(conditions, selector->getLowOrder(), TR::RealRegister::NoReg,
...
static void lookupScheme3(TR::Node *node, bool unbalanced,
  ...
  if (isInt64 && TR::Compiler->target.is64Bit()) {
    addDependency(conditions, dataRegister->getHighOrder(),
                  TR::RealRegister::NoReg, TR_GPR, cg);
    addDependency(conditions, dataRegister->getLowOrder(),
                  TR::RealRegister::NoReg, TR_GPR, cg);
    addDependency(conditions, selector->getHighOrder(), TR::RealRegister::NoReg,
                  TR_GPR, cg);
    addDependency(conditions, selector->getLowOrder(), TR::RealRegister::NoReg,
    ...
    addDependency(conditions, dataRegister, TR::RealRegister::NoReg, TR_GPR,
                  cg);
    addDependency(conditions, selector, TR::RealRegister::NoReg, TR_GPR, cg);
  }
  ...
  else // if 32bit mode
  {
    ...
    cg->getAheadOfTimeCompile()->getRelocationList().push_front(
        new (cg->trHeapMemory())
            TR_PPCPairedRelocation(rel1, rel2, (uint8_t *)recordInfo,
  ...
  if (valueToCheck->getOpCode().isBooleanCompare() &&
    ...
    if (valueToCheck->getOpCode().isCompareForOrder()) {
      if (valueToCheck->getChild(0)->getOpCode().is8Byte() ||
        ...
        compareLongsForOrder(branchOp, reverseBranchOp, slowPathLabel,
                             valueToCheck, cg,
...
static TR::Register *generateMaxMin(TR::Node *node, TR::CodeGenerator *cg,
  ...
  for (int i = 1; i < n; i++) {
    ...
    if (two_reg) {
      ...
      generateConditionalBranchInstruction(cg, max ? TR::InstOpCode::bge
                                                   : TR::InstOpCode::ble,
                                           node, label, condReg);
      generateTrg1Src1Instruction(cg, move_op, node, trgReg, reg);
...
OMR::Power::TreeEvaluator::ifxcmpoEvaluator(TR::Node *node,
  ...
  generateTrg1Src1ImmInstruction(cg, nodeIs64Bit ? TR::InstOpCode::cmpi8
                                                 : TR::InstOpCode::cmpi4,
                                 node, cr, res, 0);
