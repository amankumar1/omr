...
public:
  ...
  TR_PPCOpCodes getRecordFormOpCodeValue() {
    return (TR_PPCOpCodes)(_opCode + 1);
  }

  bool isRecordForm() {
    return (properties[_opCode] & PPCOpProp_IsRecordForm) != 0;
  }

  bool hasRecordForm() {
    return (properties[_opCode] & PPCOpProp_HasRecordForm) != 0;
  }
  ...
  bool gprOp() {
    return (properties[_opCode] & (PPCOpProp_DoubleFP | PPCOpProp_SingleFP)) ==
           0;
  }
  ...
  bool useAlternateFormat() {
    return (properties[_opCode] & PPCOpProp_AltFormat) != 0;
  }

  bool useAlternateFormatx() {
    return (properties[_opCode] & PPCOpProp_AltFormatx) != 0;
  }

  bool readsCarryFlag() {
    return (properties[_opCode] & PPCOpProp_ReadsCarryFlag) != 0;
  }

  bool setsCarryFlag() {
    return (properties[_opCode] & PPCOpProp_SetsCarryFlag) != 0;
  }

  bool setsOverflowFlag() {
    return (properties[_opCode] & PPCOpProp_SetsOverflowFlag) != 0;
  }

  bool usesCountRegister() {
    return (properties[_opCode] & PPCOpProp_UsesCtr) != 0;
  }

  bool setsCountRegister() {
    return (properties[_opCode] & PPCOpProp_SetsCtr) != 0;
  }
  ...
  bool isFloat() {
    return (properties[_opCode] & (PPCOpProp_SingleFP | PPCOpProp_DoubleFP)) !=
           0;
  }
  ...
  bool usesTarget() {
    return (properties[_opCode] & PPCOpProp_UsesTarget) != 0;
  }

  bool useMaskEnd() {
    return (properties[_opCode] & PPCOpProp_UseMaskEnd) != 0;
  }

  bool isRotateOrShift() {
    return (properties[_opCode] & PPCOpProp_IsRotateOrShift) != 0;
  }

  bool isCompare() { return (properties[_opCode] & PPCOpProp_CompareOp) != 0; }

  bool readsFPSCR() {
    return (properties[_opCode] & PPCOpProp_ReadsFPSCR) != 0;
  }

  bool setsFPSCR() { return (properties[_opCode] & PPCOpProp_SetsFPSCR) != 0; }

  bool isSyncSideEffectFree() {
    return (properties[_opCode] & PPCOpProp_SyncSideEffectFree) != 0;
  }

  bool offsetRequiresWordAlignment() {
    return (properties[_opCode] & PPCOpProp_OffsetRequiresWordAlignment) != 0;
  }
  ...
  bool isLongRunningFPOp() {
    return _opCode == PPCOp_fdiv || _opCode == PPCOp_fdivs ||
           _opCode == PPCOp_fsqrt || _opCode == PPCOp_fsqrts;
  }

  bool isFXMult() {
    return _opCode == PPCOp_mullw || _opCode == PPCOp_mulli ||
           _opCode == PPCOp_mulhw || _opCode == PPCOp_mulhd ||
           _opCode == PPCOp_mulhwu || _opCode == PPCOp_mulhdu;
  }

  bool isAdmin() {
    return _opCode == PPCOp_ret || _opCode == PPCOp_fence ||
           _opCode == PPCOp_depend || _opCode == PPCOp_proc ||
           _opCode == PPCOp_assocreg || _opCode == PPCOp_dd;
  }

  static const TR_PPCOpCodeBinaryEntry
  getOpCodeBinaryEncoding(TR_PPCOpCodes opCode) {
