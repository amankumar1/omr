...
static int32_t identifyFarConditionalBranches(int32_t estimate,
                                              TR::CodeGenerator *cg, int32_t *,
                                              int32_t);
extern TR::Register *computeCC_bitwise(TR::CodeGenerator *cg, TR::Node *node,
                                       TR::Register *targetReg,
                                       bool needsZeroExtension = true);

OMR::Power::CodeGenerator::CodeGenerator()
    : OMR::CodeGenerator(), _stackPtrRegister(NULL), _constantData(NULL),
      _blockCallInfo(NULL), _assignmentDirection(Backward),
      _transientLongRegisters(self()->trMemory()), conversionBuffer(NULL),
      _outOfLineCodeSectionList(getTypedAllocator<TR_PPCOutOfLineCodeSection *>(
          self()->comp()->allocator())),
      _globalLabelsEncountered(
          getTypedAllocator<TR::LabelSymbol *>(self()->comp()->allocator())) {
  ...
  _specializedEpilogues =
      self()->comp()->getOption(TR_EnableSpecializedEpilogues) &&
      self()->comp()->getOptLevel() >= hot;
  ...
  if (_specializedEpilogues) {
    _blocksThatModifyRegister =
        (TR_BitVector **)self()->trMemory()->allocateHeapMemory(
            TR::RealRegister::NumRegisters * sizeof(TR_BitVector *),
            TR_Memory::CodeGenerator);
  ...
  self()->setStackPointerRegister(self()->machine()->getPPCRealRegister(
      _linkageProperties->getNormalStackPointerRegister()));
  self()->setMethodMetaDataRegister(self()->machine()->getPPCRealRegister(
      _linkageProperties->getMethodMetaDataRegister()));
  self()->setTOCBaseRegister(self()->machine()->getPPCRealRegister(
  ...
  if (TR::Options::getCmdLineOptions()->getOption(TR_AggressiveOpts) &&
      !disableArraySet)
  ...
  if (self()->getSupportsTM() && !disableTMDCAS &&
      TR::Compiler->target.is64Bit() && !TR::Options::useCompressedPointers()) {
...
OMR::Power::CodeGenerator::generateSwitchToInterpreterPrePrologue(
  ...
  TR::Register *gr0 =
      self()->machine()->getPPCRealRegister(TR::RealRegister::gr0);
...
void OMR::Power::CodeGenerator::mapTOCEntries() {}
...
static void mrPeepholes(TR::CodeGenerator *cg, TR::Instruction *mrInstruction) {
  ...
  while (isWAWOrmrPeepholeCandidateInstr(current) &&
    ...
    // register move
    // Note: Don't care about GC points in snippets, since we won't remove a reg
    // move if inEBB, i.e. if we've encountered a branch (to a snippet or
    // elsewhere)
    if (current->getGCMap() &&
    ...
    if (memRef != NULL) {
      ...
      case OMR::Instruction::IsTrg1Src1Imm: {
        ...
        if (inst->getSource1Register() == mrTargetReg) {
          if (toRealRegister(mrSourceReg)->getRegisterNumber() ==
            ...
            all_mr_source_uses_rewritten = false;
          else if (performTransformation(comp, "O^O PPC PEEPHOLE: Rewrite "
                                               "Trg1Src1Imm source register "
                                               "%p.\n",
                                         current)) {
            inst->setSource1Register(mrSourceReg);
            extendWindow = true;
      ...
      case OMR::Instruction::IsTrg1Src1Imm2: {
        ...
        if (inst->getSource1Register() == mrTargetReg) {
          if (performTransformation(comp, "O^O PPC PEEPHOLE: Rewrite "
                                          "Trg1Src1Imm2 source register %p.\n",
                                    current)) {
            inst->setSource1Register(mrSourceReg);
            extendWindow = true;
          } else
      ...
      case OMR::Instruction::IsTrg1Src2: {
        ...
        if (inst->getSource1Register() == mrTargetReg) {
          if (performTransformation(comp, "O^O PPC PEEPHOLE: Rewrite Trg1Src2 "
      ...
      case OMR::Instruction::IsTrg1Src2Imm: {
        ...
        if (inst->getSource1Register() == mrTargetReg) {
          if (performTransformation(comp, "O^O PPC PEEPHOLE: Rewrite "
                                          "Trg1Src2Imm 1st source register "
                                          "%p.\n",
                                    current)) {
            inst->setSource1Register(mrSourceReg);
            extendWindow = true;
        ...
        if (inst->getSource2Register() == mrTargetReg) {
          if (performTransformation(comp, "O^O PPC PEEPHOLE: Rewrite "
                                          "Trg1Src2Imm 2nd source register "
                                          "%p.\n",
                                    current)) {
            inst->setSource2Register(mrSourceReg);
            extendWindow = true;
      ...
      if (!inEBB && all_mr_source_uses_rewritten &&
        ...
        if (!stackMaps.empty()) {
          for (auto stackMapIter = stackMaps.begin();
            ...
            if (cg->getDebug())
              traceMsg(comp, "Adjusting register map %p; removing %s, adding "
                             "%s due to removal of mr %p\n",
  ...
  while (current != NULL && !current->isLabel() &&
    ...
    // if target reg is same as source of compare
    {
      ...
      if (current->getOpCode().hasRecordForm()) {
        ...
        if (TR::Compiler->target.cpu.id() == TR_PPCgp ||
          ...
          if (opCode == TR::InstOpCode::rlwinm) {
            ...
            if (inst->getSourceImmediate() == 0 &&
              ...
              if (performTransformation(comp, "O^O PPC PEEPHOLE: Change %p to "
                                              "andi_r, remove compare "
                                              "immediate %p.\n",
        ...
        if (performTransformation(comp, "O^O PPC PEEPHOLE: Change %p to record "
                                        "form, remove compare immediate %p.\n",
                                  current, cmpiInstruction)) {
...
static void lhsPeephole(TR::CodeGenerator *cg,
  ...
  if (storeMemRef->hasDelayedOffset() != loadMemRef->hasDelayedOffset()) {
    ...
    // offset
    // at this point, so we can't compare their offsets to the offsets used by
    // regular references,
  ...
  if (srcReg == trgReg) {
    ...
    if (performTransformation(
            comp,
            "O^O PPC PEEPHOLE: Remove redundant load %p after store %p.\n",
  ...
  if (performTransformation(comp, "O^O PPC PEEPHOLE: Replace redundant load %p "
                                  "after store %p with mr.\n",
                            loadInstruction, storeInstruction)) {
...
static void syncPeephole(TR::CodeGenerator *cg,
  ...
  while (visited < window) {
    ...
    if (removeFirst) {
      if (performTransformation(comp, "O^O PPC PEEPHOLE: Remove redundant "
  ...
  if (currTargetReg && currTargetReg->getKind() != TR_CCR &&
    ...
    while (isWAWOrmrPeepholeCandidateInstr(current) && !current->isBranchOp() &&
      ...
      if (current->getTargetRegister(0) == currTargetReg) {
        ...
        if (isConstData) {
          if (performTransformation(comp, "O^O PPC PEEPHOLE: Remove dead "
                                          "instrcution group from WAW %p -> "
                                          "%p.\n",
                                    instr, current)) {
...
//
bool OMR::Power::CodeGenerator::processInstruction(
    TR::Instruction *instr, TR_BitVector **registerUsageInfo, int32_t &blockNum,
    int32_t &blockMarker, bool traceIt) {
  TR::Instruction *ppcInstr = instr;
  ...
  case OMR::Instruction::IsAdmin: {
    ...
    if (node) {
      ...
      if (node->getOpCodeValue() == TR::BBStart) {
        ...
        if (traceIt)
          traceMsg(self()->comp(),
                   "Done generating register use information for block_%d\n",
                   blockNum);
      }
    }
    return false;
  ...
  case OMR::Instruction::IsTrg1Mem: {
    int32_t tgtRegNum =
        ((TR::RealRegister *)((TR::PPCTrg1Instruction *)ppcInstr)
             ->getTargetRegister())
            ->getRegisterNumber();
    if (traceIt)
  ...
  case OMR::Instruction::IsControlFlow: {
    int32_t numTargets =
        ((TR::PPCControlFlowInstruction *)ppcInstr)->getNumTargets();
    for (int32_t i = 0; i < numTargets; i++) {
      int32_t tgtRegNum =
          ((TR::RealRegister *)((TR::PPCControlFlowInstruction *)ppcInstr)
               ->getTargetRegister(i))
              ->getRegisterNumber();
      if (traceIt)
...
uint32_t OMR::Power::CodeGenerator::isPreservedRegister(int32_t regIndex) {
  ...
  for (int32_t pindex = TR::RealRegister::gr15;
       pindex <= TR::RealRegister::LastGPR; pindex++) {
...
bool OMR::Power::CodeGenerator::isBranchInstruction(TR::Instruction *instr) {
  return (instr->isBranchOp() ||
          instr->getOpCodeValue() == TR::InstOpCode::vgdnop);
}
...
TR::Instruction *OMR::Power::CodeGenerator::splitEdge(
  ...
  if (!cursor)
    cursor = generateLabelInstruction(self(), TR::InstOpCode::label,
                                      location->getNode(), newLabel, location);

  if (!isFallThrough && needsJump) {
    TR::Instruction *jmpLocation = cursor->getPrev();
    TR::Instruction *i = generateLabelInstruction(self(), TR::InstOpCode::b,
                                                  jmpLocation->getNode(),
                                                  targetLabel, jmpLocation);
...
OMR::Power::CodeGenerator::splitBlockEntry(TR::Instruction *instr) {
  ...
  return generateLabelInstruction(self(), TR::InstOpCode::label,
                                  location->getNode(), newLabel, location);
}

int32_t
OMR::Power::CodeGenerator::computeRegisterSaveDescription(TR_BitVector *regs,
                                                          bool populateInfo) {
  uint32_t rsd = 0;
...
void OMR::Power::CodeGenerator::processIncomingParameterUsage(
...
void OMR::Power::CodeGenerator::updateSnippetMapWithRSD(TR::Instruction *instr,
                                                        int32_t rsd) {
  ...
  TR_PPCOutOfLineCodeSection *oiCursor =
...
bool OMR::Power::CodeGenerator::isTargetSnippetOrOutOfLine(
...
bool OMR::Power::CodeGenerator::enableAESInHardwareTransformations() {
  if ((TR::Compiler->target.cpu.getPPCSupportsAES() ||
       (TR::Compiler->target.cpu.getPPCSupportsVMX() &&
        TR::Compiler->target.cpu.getPPCSupportsVSX())) &&
...
void OMR::Power::CodeGenerator::generateBinaryEncodingPrologue(
  ...
  int32_t boundary =
      comp->getOptions()->getJitMethodEntryAlignmentBoundary(self());
...
void OMR::Power::CodeGenerator::doBinaryEncoding() {
  ...
  if (_specializedEpilogues) {
    TR_ASSERT((TR::Optimizer *)self()->comp()->getOptimizer(),
              "No optimizer\n");
    ...
    while (instr) {
      ...
      if (reg) {
        TR_ASSERT(instr->getBlockIndex(),
                  "Instruction %p without block index\n", instr);
        self()
            ->getBlocksThatModifyRegister(reg->getRegisterNumber())
            ->set(instr->getBlockIndex());
      }
      instr = instr->getNext();
    }
  }
  ...
  while (data.cursorInstruction) {
    ...
    if (data.cursorInstruction->isLastWarmInstruction()) {
      ...
      data.estimate =
          data.warmEstimate + MIN_DISTANCE_BETWEEN_WARM_AND_COLD_CODE;
    }
  ...
  if (data.estimate > 32768) {
    ...
    data.estimate = identifyFarConditionalBranches(
        data.estimate, self(), &incWarmSize, data.warmEstimate);
  ...
  if (data.warmEstimate) {
    ...
    self()->setEstimatedColdLength((data.estimate - data.warmEstimate -
    ...
    if (self()->getGlobalLabelsEncountered().empty()) {
      ...
      self()->setEstimatedColdLength(
          ...
          (i * sizeof(uint32_t) * 6)); // 2 instructions + traceback table
  ...
  TR::Instruction *nop;
  TR::Register *gr1 =
      self()->machine()->getPPCRealRegister(TR::RealRegister::gr1);
  ...
  while (data.cursorInstruction) {
    if (data.cursorInstruction->isLabel()) {
      if ((data.cursorInstruction)->isNopCandidate()) {
        ...
        if (uselessFetched >=
            8 - data.cursorInstruction->MAX_LOOP_ALIGN_NOPS()) {
          if (TR::Compiler->target.cpu.id() >= TR_PPCp6) {
            nop = self()->generateNop(data.cursorInstruction->getNode(),
                                      data.cursorInstruction->getPrev(),
                                      TR_NOPEndGroup); // handles P6, P7
  ...
  if (!self()->getGlobalLabelsEncountered().empty()) {
    // ListIterator<TR::LabelSymbol>
    // glblLbls(&self()->getGlobalLabelsEncountered());
  ...
  // labels.
  // If needed, we should move this step to common code around relocation
  // processing.
  if (TR::Compiler->target.is64Bit()) {
    int32_t idx;
...
TR::Register *OMR::Power::CodeGenerator::gprClobberEvaluate(TR::Node *node) {
  ...
  if (!self()->canClobberNodesRegister(node)) {
    if (TR::Compiler->target.is32Bit() && node->getType().isInt64()) {
      ...
      generateTrg1Src1Instruction(self(), resultReg->getKind() == TR_GPR
                                              ? TR::InstOpCode::mr
                                              : TR::InstOpCode::fmr,
...
void OMR::Power::CodeGenerator::buildRegisterMapForInstruction(
  ...
  for (int i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR;
    ...
    TR::RealRegister *realReg =
        self()->machine()->getPPCRealRegister((TR::RealRegister::RegNum)i);
...
void OMR::Power::CodeGenerator::emitDataSnippets(bool isWarm) {
  if (!isWarm ||
      self()->comp()->getOption(
          TR_EnableHCR)) // PPC constant data should not be warm.
...
int32_t OMR::Power::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(
  ...
  if (isWarm && !self()->comp()->getOption(TR_EnableHCR)) // PPC currently
                                                          // should not have any
                                                          // constant data
                                                          // snippets as warm.
...
void OMR::Power::CodeGenerator::apply16BitLoadLabelRelativeRelocation(
  ...
  // can get the return address off of the stack
  // and then use this return address and the offset loaded here to find an exit
  // label in the generated code.
  // If the register deps are not done properly then the register assigner may
  // insert instructions after the call but before
  // the startLabel. If this bad register motion occurs then the loaded offset
  // would be wrong (when used in this way)
...
void OMR::Power::CodeGenerator::apply64BitLoadLabelRelativeRelocation(
  ...
  // load with update from this base register
...
uintptrj_t j2Prof_startInterval(int32_t *preambLen, int32_t *ipAssistLen,
                                int32_t *prologueLen, TR::Compilation *comp) {
  ...
  *ipAssistLen = ((*sizePtr) >> 16) & 0x0000ffff;
  ...
  nextIntervalBufferPtr = (currentIntervalStart == NULL)
                              ? cg->getBinaryBufferCursor()
...
uint32_t j2Prof_nextInterval(TR::Compilation *comp) {
  TR::CodeGenerator *cg = comp->cg();
  TR::Instruction *currentIntervalStart;
  uint8_t *intervalBinaryStart;
...
void j2Prof_methodReport(TR_Method *method, TR::Compilation *comp) {
  ...
  buffer[len] = '\0';
  fprintf(j2Profile, "SIGNATURE\t%s\tSTART=0x%p\tEND=0x%p\tHOTNESS\t", buffer,
...
void j2Prof_trampolineReport(uint8_t *startP, uint8_t *endP,
                             int32_t num_trampoline) {
  int32_t tindex, len;
...
bool OMR::Power::CodeGenerator::suppressInliningOfRecognizedMethod(
  ...
  if (tempReg == NULL) {
      ...
      if (typeAddress == -1) {
        ...
        if (typeAddress == TR_DataAddress) {
          if (doAOTRelocation) {
            ...
            // printf("TreeEvaluator.cpp:137, inlinedSiteIndex: %d,
            // tempSR->getCPIndex(): %x, tempSR->getOffset(): %x\n",
            // node->getInlinedSiteIndex(),
            // node->getSymbolReference()->getCPIndex(),
            // node->getSymbolReference()->getOffset());fflush(stdout);
            self()->addAOTRelocation(
                new (self()->trHeapMemory())
                    TR_BeforeBinaryEncodingExternalRelocation(
                        firstInstruction, (uint8_t *)recordInfo, TR_DataAddress,
                        self()),
                __FILE__, __LINE__, node);
          }
        } else {
    ...
    if (value != 0x0) {
      ...
      if (typeAddress == -1) {
        ...
        if (typeAddress == TR_RamMethodSequence) // NOTE: 32 bit changed to use
                                                 // TR_RamMethodSequence for
                                                 // ordered pair, hence, we
...
void OMR::Power::CodeGenerator::addMetaDataForLoadIntConstantFixed(
  ...
  if (typeAddress == TR_DataAddress) {
    ...
    self()->addAOTRelocation(
        new (self()->trHeapMemory()) TR_32BitExternalOrderedPairRelocation(
...
void OMR::Power::CodeGenerator::addMetaDataFor32BitFixedLoadLabelAddressIntoReg(
  ...
  self()->getAheadOfTimeCompile()->getRelocationList().push_front(
      new (self()->trHeapMemory()) TR_PPCPairedRelocation(
          firstInstruction, secondInstruction, (uint8_t *)recordInfo,
          TR_AbsoluteMethodAddressOrderedPair, node));

  self()->addRelocation(
      new (self()->trHeapMemory()) TR_PPCPairedLabelAbsoluteRelocation(
...
void OMR::Power::CodeGenerator::addMetaDataFor64BitFixedLoadLabelAddressIntoReg(
  ...
  if (!self()->comp()->compileRelocatableCode()) {
    self()->addRelocation(
        new (self()->trHeapMemory())
            TR_PPCPairedLabelAbsoluteRelocation(q[0], q[1], q[2], q[3], label));
  }

  self()->addAOTRelocation(
      new (self()->trHeapMemory()) TR_BeforeBinaryEncodingExternalRelocation(
          q[0], (uint8_t *)(label),
