...
public:
  ...
  void addDependency(TR::RegisterDependency &dep, uint32_t index) {
    ...
    if (rr != TR::RealRegister::NoReg && rr != TR::RealRegister::SpilledReg) {
      TR_ASSERT(rr >= 0 && rr < TR::RealRegister::NumRegisters,
                "Register number %d used as index but out of range", rr);
      TR_ASSERT(
          targetTable[rr] == MAP_NIL || // TODO: Figure out where the same dep
    ...
    if (assignedReg) {
      ...
      TR_ASSERT(rr >= 0 && rr < TR::RealRegister::NumRegisters,
                "Register number %d used as index but out of range", rr);
      TR_ASSERT(
          assignedTable[rr] == MAP_NIL || // TODO: Figure out where the same dep
                                          // is being added more than once!
              deps[assignedTable[rr]].getRegister() ==
  ...
  getDependencyWithAssigned(TR::RealRegister::RegNum rr) {
    TR_ASSERT(rr >= 0 && rr < TR::RealRegister::NumRegisters,
              "Register number used as index but out of range");
...
void OMR::Power::RegisterDependencyConditions::bookKeepingRegisterUses(
  ...
  // properly, however no such mechanism exists for instruction selection
  // so we don't want these associations to clobber the associations that were
  // set in main line code, which are more important
...
findDependencyChainHead(TR::RegisterDependency *dep,
  ...
  TR::RegisterDependency *cursor =
      map.getDependencyWithAssigned(dep->getRealRegister());
  ...
  while (cursor != dep) {
    TR::RegisterDependency *nextDep =
        map.getDependencyWithAssigned(cursor->getRealRegister());
    if (!nextDep)
      break;
    cursor = nextDep;
  }
...
static void assignFreeRegisters(TR::Instruction *currentInstruction,
  ...
  // free reg
  while (dep) {
    TR_ASSERT(machine->getPPCRealRegister(dep->getRealRegister())->getState() ==
...
static void assignContendedRegisters(TR::Instruction *currentInstruction,
  ...
  if (map.getDependencyWithTarget(assignedReg->getRegisterNumber()) ==
      map.getDependencyWithAssigned(targetRegNum)) {
  ...
  TR::RealRegister *spareReg = machine->findBestFreeRegister(
      ...
      false, targetRegNum == TR::RealRegister::NoReg
                 ? virtReg
...
void TR_PPCRegisterDependencyGroup::assignRegisters(
  ...
  if (!comp->getOption(TR_DisableOOL)) {
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      if (dependentRegNum == TR::RealRegister::SpilledReg) {
        ...
        if (virtReg->getAssignedRealRegister()) {
          ...
          TR::MemoryReference *tempMR = new (cg->trHeapMemory())
              TR::MemoryReference(currentNode, (TR::SymbolReference *)virtReg
                                                   ->getBackingStorage()
                                                   ->getSymbolReference(),
                                  sizeof(uintptr_t), cg);
          TR::InstOpCode::Mnemonic opCode;
      ...
      // the OOL during backwards RA assignment
               ...
               virtReg->getAssignedRealRegister()) {
        TR::PPCLabelInstruction *labelInstr =
            (TR::PPCLabelInstruction *)currentInstruction;
  ...
  TR_ASSERT(locked_gprs == machine->getNumberOfLockedRegisters(TR_GPR),
            "Inconsistent number of locked GPRs");
  TR_ASSERT(locked_fprs == machine->getNumberOfLockedRegisters(TR_FPR),
            "Inconsistent number of locked FPRs");
  TR_ASSERT(locked_vrfs == machine->getNumberOfLockedRegisters(TR_VRF),
            "Inconsistent number of locked VRFs");
#endif

  // To handle circular dependencies, we block a real register if (1) it is
  // already assigned to a correct
  // virtual register and (2) if it is assigned to one register in the list but
  // is required by another.
  // However, if all available registers are requested, we do not block in case
  // (2) to avoid all registers
  // being blocked.
  ...
  TR_ASSERT(num_gprs <=
                (TR::RealRegister::LastGPR - TR::RealRegister::FirstGPR + 1 -
  ...
  TR_ASSERT(num_vrfs <=
                (TR::RealRegister::LastVRF - TR::RealRegister::FirstVRF + 1 -
                 machine->getNumberOfLockedRegisters(TR_VRF)),
            "Too many VRF dependencies, unable to assign");

  if (num_gprs == (TR::RealRegister::LastGPR - TR::RealRegister::FirstGPR + 1 -
                   machine->getNumberOfLockedRegisters(TR_GPR)))
    block_gprs = false;
  if (num_fprs == (TR::RealRegister::LastFPR - TR::RealRegister::FirstFPR + 1 -
                   machine->getNumberOfLockedRegisters(TR_FPR)))
    block_fprs = false;
  if (num_vrfs == (TR::RealRegister::LastVRF - TR::RealRegister::FirstVRF + 1 -
                   machine->getNumberOfLockedRegisters(TR_VRF)))
    block_vrfs = false;
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    if (virtReg->getAssignedRealRegister() != NULL) {
      if (_dependencies[i].getRealRegister() == TR::RealRegister::NoReg) {
        ...
        assignedRegNum = toRealRegister(virtReg->getAssignedRealRegister())
                             ->getRegisterNumber();

        // always block if required register and assigned register match;
        // block if assigned register is required by other dependency but only
        // if
