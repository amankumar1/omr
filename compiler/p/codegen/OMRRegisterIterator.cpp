...
OMR::Power::RegisterIterator::RegisterIterator(TR::Machine *machine,
...
TR::Register *OMR::Power::RegisterIterator::getFirst() {
  return _machine->getPPCRealRegister(
      (TR::RealRegister::RegNum)(_cursor = _firstRegIndex));
}

TR::Register *OMR::Power::RegisterIterator::getCurrent() {
  return _machine->getPPCRealRegister((TR::RealRegister::RegNum)_cursor);
}

TR::Register *OMR::Power::RegisterIterator::getNext() {
  return _cursor == _lastRegIndex ? NULL
                                  : _machine->getPPCRealRegister(
                                        (TR::RealRegister::RegNum)(++_cursor));
}
