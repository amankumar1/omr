...
extern TR::Register *
computeCC_compareUnsigned(TR::Node *node, TR::Register *trgReg,
                          TR::Register *src1Reg, TR::Register *src2Reg,
                          bool is64BitCompare, bool needsZeroExtension,
                          TR::CodeGenerator *cg);
extern void generateZeroExtendInstruction(TR::Node *node, TR::Register *trgReg,
                                          TR::Register *srcReg,
...
TR::Register *OMR::Power::TreeEvaluator::iloadEvaluator(TR::Node *node,
  ...
  // instruction
  // layout in order to patch if it turns out that the reference isn't really
  // volatile.
  ...
  if (needSync) {
    postSyncConditions(node, cg, tempReg, tempMR,
...
TR::Register *OMR::Power::TreeEvaluator::lloadEvaluator(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (needSync) {
      postSyncConditions(node, cg, trgReg, tempMR,
                         TR::Compiler->target.cpu.id() >= TR_PPCp7
                             ? TR::InstOpCode::lwsync
                             : TR::InstOpCode::isync);
    }
  ...
  } else // 32 bit target
  {
    ...
    if (reverseLoad) // 32-bit reverse load. Handles sync
    {
      ...
      if (needSync) {
        ...
        generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node, tempMRStore1,
                                   doubleReg);
        generateInstruction(cg, TR::Compiler->target.cpu.id() >= TR_PPCp7
                                    ? TR::InstOpCode::lwsync
                                    : TR::InstOpCode::isync,
                            node);
    ...
    if (needSync) {
          ...
          cg->is64BitProcessor()) // resolved as volatile
      {
        ...
        // layout needs to
        // be fixed and the dependency needs to be enhanced to guarantee the
        // layout.
        //
        ...
        if (node->getSymbolReference()->isUnresolved()) {
          if (tempMR->getBaseRegister() != NULL) {
            addDependency(dependencies, tempMR->getBaseRegister(),
          ...
          if (tempMR->getIndexRegister() != NULL)
            addDependency(dependencies, tempMR->getIndexRegister(),
                          TR::RealRegister::NoReg, TR_GPR, cg);
        }
...
OMR::Power::TreeEvaluator::istoreEvaluator(TR::Node *node,
  ...
  if (needSync) {
    ...
    postSyncConditions(node, cg, srcReg, tempMR, TR::InstOpCode::sync,
                       lazyVolatile);
  }
...
OMR::Power::TreeEvaluator::astoreEvaluator(TR::Node *node,
  ...
  if (needSync) {
    ...
    // and will not mark unresolved snippets with in sync sequence to avoid
    // PicBuilder overwriting the instruction that normally would have been sync
    postSyncConditions(node, cg, valueChild, tempMR, TR::InstOpCode::sync,
                       lazyVolatile);
  }
  tempMR->decNodeReferenceCounts(cg);
  ...
  } else // 32 bit target
  {
    ...
    if (needSync) {
          ...
          cg->is64BitProcessor()) // resolved as volatile
      {
        ...
        addDependency(dependencies, valueReg->getHighOrder(),
                      TR::RealRegister::gr4, TR_GPR, cg);
        addDependency(dependencies, valueReg->getLowOrder(),
...
TR::Register *OMR::Power::TreeEvaluator::vaddEvaluator(TR::Node *node,
  ...
  switch (node->getDataType()) {
  case TR::VectorInt32:
    return inlineVectorBinaryOp(node, cg, TR::InstOpCode::vadduwm);
  case TR::VectorFloat:
    return inlineVectorBinaryOp(node, cg, TR::InstOpCode::xvaddsp);
  case TR::VectorDouble:
    return inlineVectorBinaryOp(node, cg, TR::InstOpCode::xvadddp);
  default:
    TR_ASSERT(false, "unrecognized vector type %s\n",
              node->getDataType().toString());
    return NULL;
...
TR::Register *OMR::Power::TreeEvaluator::vsubEvaluator(TR::Node *node,
  ...
  switch (node->getDataType()) {
  case TR::VectorInt32:
    return inlineVectorBinaryOp(node, cg, TR::InstOpCode::vsubuwm);
  case TR::VectorFloat:
    return inlineVectorBinaryOp(node, cg, TR::InstOpCode::xvsubsp);
  case TR::VectorDouble:
    return inlineVectorBinaryOp(node, cg, TR::InstOpCode::xvsubdp);
  default:
    TR_ASSERT(false, "unrecognized vector type %s\n",
              node->getDataType().toString());
    return NULL;
...
OMR::Power::TreeEvaluator::vnegInt32Helper(TR::Node *node,
  ...
  generateTrg1Src2Instruction(cg, TR::InstOpCode::xxlxor, node, resReg, srcReg,
                              srcReg);
  generateTrg1Src2Instruction(cg, TR::InstOpCode::vsubsws, node, resReg, resReg,
                              srcReg);
  node->setRegister(resReg);
...
static void inlineVSXArrayCopy(
  ...
  if (!node->isForwardArrayCopy()) {
    ...
    generateTrg1Src2Instruction(cg, TR::Compiler->target.is64Bit()
                                        ? TR::InstOpCode::cmpl8
                                        : TR::InstOpCode::cmpl4,
...
OMR::Power::TreeEvaluator::arraytranslateEvaluator(TR::Node *node,
  ...
  if (verbosePPCTRTOOT)
    fprintf(stderr, "%s @ %s [isSourceByte: %d] [isOT: %d] [%d && %d && %d]: "
                    "arraytranslate\n",
...
static TR::Register *inlineArrayCmp(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  generateTrg1Src1ImmInstruction(
      ...
      condRegToBeUsed, byteLenRemainingRegister, byteLen);
  generateConditionalBranchInstruction(cg, TR::InstOpCode::blt, node,
                                       /* residueLoopStartLabel */ mid2Label,
                                       condRegToBeUsed);
  ...
  if (byteLenRemainingRegister) {
    if (byteLen == 4) {
      generateTrg1MemInstruction(
          cg, TR::InstOpCode::lwzu, node, src1Reg,
          new (cg->trHeapMemory()) TR::MemoryReference(src1AddrReg, 4, 4, cg));
      generateTrg1MemInstruction(
          cg, TR::InstOpCode::lwzu, node, src2Reg,
          new (cg->trHeapMemory()) TR::MemoryReference(src2AddrReg, 4, 4, cg));
    } else {
      generateTrg1MemInstruction(
          cg, TR::InstOpCode::ldu, node, src1Reg,
          new (cg->trHeapMemory()) TR::MemoryReference(src1AddrReg, 8, 8, cg));
      generateTrg1MemInstruction(
          cg, TR::InstOpCode::ldu, node, src2Reg,
          new (cg->trHeapMemory()) TR::MemoryReference(src2AddrReg, 8, 8, cg));
    }
  } else {
    switch (byteLen) {
    case 1:
      loadArrayCmpSources(node, TR::InstOpCode::lbz, 1, byteLen, src1Reg,
                          src2Reg, src1AddrReg, src2AddrReg, cg);
      break;
    case 2:
      loadArrayCmpSources(node, TR::InstOpCode::lhz, 2, byteLen, src1Reg,
                          src2Reg, src1AddrReg, src2AddrReg, cg);
      break;
    case 3:
    case 4:
      loadArrayCmpSources(node, TR::InstOpCode::lwz, 4, byteLen, src1Reg,
                          src2Reg, src1AddrReg, src2AddrReg, cg);
      break;
    case 5:
    case 6:
    case 7:
    case 8:
      loadArrayCmpSources(node, TR::InstOpCode::ld, 8, byteLen, src1Reg,
                          src2Reg, src1AddrReg, src2AddrReg, cg);
      break;
    default:
  ...
  if (!byteLenRemainingRegister)
    ccReg = computeCC_compareUnsigned(node, ccReg, src1Reg, src2Reg,
                                      (byteLen > 4) ? true : false,
                                      false /* needsZeroExtenstion */, cg);
  else {
    generateTrg1Src2Instruction(cg, (byteLen == 8) ? TR::InstOpCode::cmp8
                                                   : TR::InstOpCode::cmp4,
                                node, condReg, src1Reg, src2Reg);
    ...
    generateTrg1Src1ImmInstruction(
        ...
        node, condRegToBeUsed, byteLenRemainingRegister, 0);
    ...
    addDependency(dependencies, src1AddrReg, TR::RealRegister::NoReg, TR_GPR,
                  cg);
    addDependency(dependencies, src2AddrReg, TR::RealRegister::NoReg, TR_GPR,
                  cg);
    addDependency(dependencies, byteLenRegister, TR::RealRegister::NoReg,
                  TR_GPR, cg);
    addDependency(dependencies, byteLenRemainingRegister,
                  TR::RealRegister::NoReg, TR_GPR, cg);
    ...
    if (condReg2)
      addDependency(dependencies, condReg2, TR::RealRegister::NoReg, TR_CCR,
                    cg);

    // generateLabelInstruction(cg, TR::InstOpCode::label, node,
    // residueEndLabel);
    generateDepLabelInstruction(cg, TR::InstOpCode::label, node,
                                residueEndLabel, dependencies);
...
// call the "C" helper, handle the exception case
  ...
  static bool disableVSXArrayCopyInlining =
      (feGetEnv("TR_disableVSXArrayCopyInlining") != NULL);
  ...
  if (verboseArrayCopy)
    fprintf(stderr, "arraycopy [0x%p] isReferenceArrayCopy:[%d] "
                    "isForwardArrayCopy:[%d] isHalfWordElementArrayCopy:[%d] "
                    "isWordElementArrayCopy:[%d] %s @ %s\n",
            node, 0, node->isForwardArrayCopy(),
...
TR::Register *inlineIntegerRotateLeft(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (secondChild->getOpCodeValue() == TR::iconst ||
    ...
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node,
                                    targetRegister, srcRegister, value,
                                    0xffffffff);
  } else {
...
TR::Register *inlineLongRotateLeft(TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(node->getNumChildren() == 2,
            "Wrong number of children in inlineLongRotateLeft");
...
void addPrefetch(TR::CodeGenerator *cg, TR::Node *node,
                 TR::Register *targetRegister) {
  static bool disableHotFieldPrefetch =
      (feGetEnv("TR_DisableHotFieldPrefetch") != NULL);
  static bool disableHotFieldNextElementPrefetch =
      (feGetEnv("TR_DisableHotFieldNextElementPrefetch") != NULL);
  static bool disableTreeMapPrefetch =
      (feGetEnv("TR_DisableTreeMapPrefetch") != NULL);
  static bool disableStringObjPrefetch =
      (feGetEnv("TR_DisableStringObjPrefetch") != NULL);
  ...
  if (node->getOpCodeValue() == TR::aloadi ||
    ...
    if (!disableHotFieldPrefetch &&
        prefetchOffset >= 0) // Prefetch based on hot field information
    {
      bool canSkipNullChk = false;
      static bool disableDelayPrefetch =
          (feGetEnv("TR_DisableDelayPrefetch") != NULL);
      ...
      if (!canSkipNullChk) {
        ...
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpli4, node,
                                       condReg, targetRegister, NULLVALUE);
        generateConditionalBranchInstruction(cg, TR::InstOpCode::beql, node,
                                             endCtrlFlowLabel, condReg);

        TR::Register *tempReg = cg->allocateRegister();
        TR::RegisterDependencyConditions *deps = new (cg->trHeapMemory())
            TR::RegisterDependencyConditions(1, 2, cg->trMemory());
        ...
        if (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) {
          TR::MemoryReference *tempMR = new (cg->trHeapMemory())
              TR::MemoryReference(targetRegister, prefetchOffset, 8, cg);
          generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, tempReg,
                                     tempMR);
        } else {
          TR::MemoryReference *tempMR = new (cg->trHeapMemory())
              TR::MemoryReference(targetRegister, prefetchOffset, 4, cg);
          generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, tempReg,
                                     tempMR);
        }
        ...
        // Delay the dcbt to after the null check and fall through to the next
        // block's treetop.
        ...
        {
          TR_PrefetchInfo *pf = new (cg->trHeapMemory())
              TR_PrefetchInfo(cg->getCurrentEvaluationTreeTop(), useTree, node,
                              useNode, prefetchOffset, false);
      ...
      // if the pointer came from an array.  Seems to give no gain at all,
      ...
      if (!disableHotFieldNextElementPrefetch) {
              ...
              if (indexObject->getOpCode().isLoadConst()) {
                ...
                if (len >= LOWER_IMMED && len <= UPPER_IMMED)
                  generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node,
                                                 pointerReg, baseReg, len);
                else {
                  indexReg = cg->allocateRegister();
                  loadConstant(cg, node, len, indexReg);
                  generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node,
                                              pointerReg, baseReg, indexReg);
                  cg->stopUsingRegister(indexReg);
                }
              } else
                generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node,
        ...
        // 64bit CR
          ...
          if (!(firstChild->getOpCodeValue() == TR::iu2l &&
            ...
            fromRegLoad = true;
            pointerReg =
                firstChild->getFirstChild()->getFirstChild()->getRegister();
          }
        }
        // 64bit - TODO
        else
      ...
      if (!optDisabled) {
        ...
        // 32 bit only.... For -Xgc:noconcurrentmark, heapBase will be 0 and
        // heapTop will be ok
        // Otherwise, for a 2.25Gb or bigger heap, heapTop will be 0.  Relying
        // on correct JIT initialization
        uintptr_t heapTop = comp->getOptions()->getHeapTop()
                                ? comp->getOptions()->getHeapTop()
                                : 0xFFFFFFFF;

        if (pointerReg &&
            (heapTop > comp->getOptions()->getHeapBase())) // Check for gencon
        {
          ...
          if (TR::Compiler->target.is64Bit() &&
            ...
            if (!prefetch2Ahead)
              generateTrg1MemInstruction(
                  cg, TR::InstOpCode::ld, node, temp3Reg,
                  new (cg->trHeapMemory()) TR::MemoryReference(
                      pointerReg,
                      (int32_t)TR::Compiler->om.sizeofReferenceField(), 8, cg));
            else
              generateTrg1MemInstruction(
                  cg, TR::InstOpCode::ld, node, temp3Reg,
                  new (cg->trHeapMemory()) TR::MemoryReference(
            ...
            if (!prefetch2Ahead)
              generateTrg1MemInstruction(
                  cg, TR::InstOpCode::lwz, node, temp3Reg,
                  new (cg->trHeapMemory()) TR::MemoryReference(
                      pointerReg,
                      (int32_t)TR::Compiler->om.sizeofReferenceField(), 4, cg));
            else
              generateTrg1MemInstruction(
                  cg, TR::InstOpCode::lwz, node, temp3Reg,
                  new (cg->trHeapMemory()) TR::MemoryReference(
          ...
          if (comp->getOptions()->getHeapBase() != NULL) {
            loadAddressConstant(cg, node,
                                (intptrj_t)(comp->getOptions()->getHeapBase()),
                                tempReg);
            generateTrg1Src2Instruction(cg, TR::InstOpCode::cmpl4, node,
                                        condReg, temp3Reg, tempReg);
            generateConditionalBranchInstruction(cg, TR::InstOpCode::blt, node,
                                                 endCtrlFlowLabel, condReg);
          }
          if (heapTop != 0xFFFFFFFF) {
            loadAddressConstant(cg, node, (intptrj_t)(heapTop - prefetchOffset),
                                tempReg);
            generateTrg1Src2Instruction(cg, TR::InstOpCode::cmpl4, node,
                                        condReg, temp3Reg, tempReg);
            generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, node,
          ...
          TR::MemoryReference *targetMR = new (cg->trHeapMemory())
              TR::MemoryReference(temp3Reg, (int32_t)0, 4, cg);
          targetMR->forceIndexedForm(node, cg);
          generateMemInstruction(
              cg, TR::InstOpCode::dcbt, node,
    ...
    if (!disableStringObjPrefetch && 0 && node->getSymbolReference() &&
        !node->getSymbolReference()->isUnresolved() &&
        (node->getSymbolReference()->getSymbol()->getKind() ==
         TR::Symbol::IsShadow) &&
      ...
      if (fieldName && strstr(fieldName, "Ljava/lang/String;")) {
        TR::MemoryReference *targetMR = new (cg->trHeapMemory())
            TR::MemoryReference(targetRegister, (int32_t)0, 4, cg);
        targetMR->forceIndexedForm(node, cg);
        generateMemInstruction(cg, TR::InstOpCode::dcbt, node, targetMR);
      }
    }
  ...
  if (node->getOpCodeValue() == TR::aloadi ||
    ...
    if (!disableTreeMapPrefetch) {
      // 32bit
      if (TR::Compiler->target.is32Bit()) {
        if (!(firstChild && firstChild->getOpCodeValue() == TR::aiadd &&
              firstChild->isInternalPointer() &&
              // comp->fe()->sampleSignature(node->getOwningMethod(comp)) !=
              // NULL &&
              strstr(comp->fe()->sampleSignature(node->getOwningMethod(), 0, 0,
                                                 cg->trMemory()),
                     "java/util/TreeMap$UnboundedValueIterator.next()"))) {
          optDisabled = true;
        }
      }
      // 64bit cr
               ...
               comp->useCompressedPointers()) {
        if (!(firstChild && firstChild->getOpCodeValue() == TR::aladd &&
              firstChild->isInternalPointer() &&
              // comp->fe()->sampleSignature(node->getOwningMethod(comp)) !=
              // NULL &&
              strstr(comp->fe()->sampleSignature(node->getOwningMethod(), 0, 0,
    ...
    if (!optDisabled) {
      ...
      if (baseReg && indexReg && loopSize > 0) {
        TR::Register *tempReg = cg->allocateRegister();
        generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::addi, node, tempReg, indexReg,
            (int32_t)(prefetchElementStride *
                      TR::Compiler->om.sizeofReferenceField()));
        if (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) {
          TR::MemoryReference *targetMR = new (cg->trHeapMemory())
              TR::MemoryReference(baseReg, tempReg, 8, cg);
          generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, tempReg,
                                     targetMR);
        } else {
          TR::MemoryReference *targetMR = new (cg->trHeapMemory())
              TR::MemoryReference(baseReg, tempReg, 4, cg);
          generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, tempReg,
                                     targetMR);
        }
        TR::MemoryReference *target2MR =
            new (cg->trHeapMemory()) TR::MemoryReference(tempReg, 0, 4, cg);
    ...
    // Take the source register of the store and apply on the prefetchOffset
    // right away
    int32_t prefetchOffset = comp->findPrefetchInfo(node);
    if (prefetchOffset >= 0) {
      TR::MemoryReference *targetMR = new (cg->trHeapMemory())
          TR::MemoryReference(targetRegister, prefetchOffset,
...
void OMR::Power::TreeEvaluator::postSyncConditions(
  ...
  if (symRef->isUnresolved() && memRef->getUnresolvedSnippet() != NULL) {
    ...
    addDependency(conditions, dataReg, TR::RealRegister::NoReg,
                  dataReg->getKind(), cg);
    if (memRef->getIndexRegister() != NULL)
      addDependency(conditions, memRef->getIndexRegister(),
                    TR::RealRegister::NoReg, TR_GPR, cg);
...
void OMR::Power::TreeEvaluator::preserveTOCRegister(
  ...
  TR::Register *grTOCReg =
      cg->machine()->getPPCRealRegister(TR::RealRegister::gr2);
  TR::Register *grSysStackReg =
      cg->machine()->getPPCRealRegister(TR::RealRegister::gr1);
