...
bool OMR::Power::Instruction::isNopCandidate() {
...
int OMR::Power::Instruction::MAX_LOOP_ALIGN_NOPS() {
  ...
  if (LOCAL_MAX_LOOP_ALIGN_NOPS == -1) {
    ...
    LOCAL_MAX_LOOP_ALIGN_NOPS =
        (TR_LoopAlignNops == NULL ? 7 : atoi(TR_LoopAlignNops));
  }
...
uint8_t *TR::PPCDepImmSymInstruction::generateBinaryEncoding() {
  ...
  if (getOpCodeValue() == TR::InstOpCode::bl ||
    ...
    if (resolvedMethod != NULL &&
        resolvedMethod->isSameMethod(cg()->comp()->getCurrentMethod())) {
      ...
      *(int32_t *)cursor |= (jitTojitStart - cursor) & 0x03fffffc;
      ...
      if (distance > BRANCH_FORWARD_LIMIT || distance < BRANCH_BACKWARD_LIMIT) {
        ...
        if (refNum < TR_PPCnumRuntimeHelpers) {
          distance =
              cg()->fe()->indexedTrampolineLookup(refNum, (void *)cursor) -
              (intptrj_t)cursor;
        }
      }
