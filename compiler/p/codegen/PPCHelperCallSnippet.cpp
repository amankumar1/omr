...
uint8_t *TR::PPCHelperCallSnippet::genHelperCall(uint8_t *buffer) {
  intptrj_t distance = (intptrj_t)getDestination()
                           ->getSymbol()
                           ->castToMethodSymbol()
                           ->getMethodAddress() -
                       (intptrj_t)buffer;

  if (!(distance >= BRANCH_BACKWARD_LIMIT &&
        distance <= BRANCH_FORWARD_LIMIT)) {
    distance = TR::comp()->fe()->indexedTrampolineLookup(
                   getDestination()->getReferenceNumber(), (void *)buffer) -
               (intptrj_t)buffer;
    TR_ASSERT(distance >= BRANCH_BACKWARD_LIMIT &&
                  distance <= BRANCH_FORWARD_LIMIT,
...
uint8_t *TR::PPCArrayCopyCallSnippet::emitSnippetBody() {
  ...
  TR::RealRegister *lengthReg =
      cg()->machine()->getPPCRealRegister(_lengthRegNum);
...
uint8_t *TR_Debug::print(TR::FILE *pOutFile,
  ...
  TR::RealRegister *lengthReg =
      _cg->machine()->getPPCRealRegister(snippet->getLengthRegNum());
