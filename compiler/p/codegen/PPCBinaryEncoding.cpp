    ...
    if (label->getCodeLocation() != NULL) {
      *(int32_t *)cursor |= ((label->getCodeLocation() - cursor) & 0x03fffffc);
      ...
      cg()->addRelocation(new (cg()->trHeapMemory())
                              TR_24BitLabelRelativeRelocation(cursor, label));
    }
    cursor += 4;
  } else // regular LABEL
  {
...
static bool reversedConditionalBranchOpCode(TR::InstOpCode::Mnemonic op,
  ...
  case TR::InstOpCode::bun:
    *rop = TR::InstOpCode::bnun;
    return (false);
  default:
    TR_ASSERT(0, "New PPC conditional branch opcodes have to have "
                 "corresponding reversed opcode: %d\n",
              (int32_t)op);
...
uint8_t *TR::PPCAlignedLabelInstruction::generateBinaryEncoding() {
  ...
  if ((alignment - 1) & (intptr_t)cursor) {
    paddingNOPsNeeded = (alignment - ((alignment - 1) & (intptr_t)cursor)) /
  ...
  label->setCodeLocation(
      cursor); // point past any NOPs to the following instruction
  setBinaryLength(cursor - instructionStart);
  cg()->addAccumulatedInstructionLengthError(getEstimatedBinaryLength() -
                                             getBinaryLength());
  setBinaryEncoding(instructionStart);
...
int32_t
TR::PPCAlignedLabelInstruction::estimateBinaryLength(int32_t currentEstimate) {
  TR_ASSERT((getAlignment() % PPC_INSTRUCTION_LENGTH) == 0,
            "label alignment must be a multiple of the instruction length, "
            "alignment = %d",
            getAlignment());
  TR_ASSERT(getOpCodeValue() == TR::InstOpCode::label,
            "AlignedLabel only supported for TR::InstOpCode::Label opcodes");
...
uint8_t *TR::PPCVirtualGuardNOPInstruction::generateBinaryEncoding() {
  ...
  if (cg()->sizeOfInstructionToBePatched(this) == 0 ||
      ...
      // this site because
      // 1) Those instructions might have AOT data relocations (and therefore
      // will be incorrectly patched again)
      // 2) We might want to re-enable the code path and unpatch, in which case
      // we would have to know what the old instruction was
