...
TR::MemoryReference *OMR::Power::Linkage::getOutgoingArgumentMemRef(
    ...
    TR_PPCMemoryArgument &memArg, uint32_t length) {
  ...
  TR::MemoryReference *result = new (self()->trHeapMemory())
      TR::MemoryReference(machine->getPPCRealRegister(
                              properties.getNormalStackPointerRegister()),
                          argSize + properties.getOffsetToFirstParm(), length,
                          self()->cg());
...
#define REGNUM(ri) ((TR::RealRegister::RegNum)(ri))
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
    ...
    int32_t offset = self()->calculateParameterRegisterOffset(
        paramCursor->getParameterOffset(), *paramCursor);
    TR::DataType type = paramCursor->getType();
    int32_t dtype = type.getDataType();

    // TODO: Is there an accurate assume to insert here ?
    // TR_ASSERT(!fsd || (fsd && lri >=0 && (ai<0 ||
    // hasToBeOnStack(paramCursor))), "Registers not saved on Full Speed
    // Debug");
    if (lri >= 0) {
      if (!paramCursor->isReferencedParameter() &&
      ...
      if (((ai < 0 || self()->hasToBeOnStack(paramCursor)) && !fsd) ||
        ...
        case TR::Float:
          cursor = generateMemSrc1Instruction(
              self()->cg(), TR::InstOpCode::stfs, firstNode,
              new (self()->trHeapMemory())
                  TR::MemoryReference(stackPtr, offset, 4, self()->cg()),
              REAL_REGISTER(regNum), cursor);
          break;
        case TR::Double:
          cursor = generateMemSrc1Instruction(
              self()->cg(), TR::InstOpCode::stfd, firstNode,
              new (self()->trHeapMemory())
        ...
        if (regNum != ai) // Equal assignment: do nothing
        {
          if (freeScratchable.isSet(ai)) {
            cursor = generateTrg1Src1Instruction(
                self()->cg(), (type.isFloatingPoint()) ? TR::InstOpCode::fmr
                                                       : TR::InstOpCode::mr,
        ...
        if (!(target < 0) && freeScratchable.isSet(target)) {
          ...
          case 1:
            cursor = generateTrg1MemInstruction(
                ...
                new (self()->trHeapMemory())
                    TR::MemoryReference(stackPtr, source, 4, self()->cg()),
                cursor);
            break;
          case 2:
            cursor = generateTrg1MemInstruction(
                ...
                REAL_REGISTER(REGNUM(target)),
                new (self()->trHeapMemory())
                    TR::MemoryReference(stackPtr, source, 8, self()->cg()),
                cursor);
            break;
          case 3:
            cursor = generateTrg1MemInstruction(
  ...
  while ((paramCursor != NULL) &&
    ...
    case TR::Int32:
      if (hasToLoadFromStack && numIntArgs < properties.getNumIntArgRegs()) {
        argRegister = machine->getPPCRealRegister(
    ...
    case TR::Float:
      if (hasToLoadFromStack &&
        ...
        cursor = generateTrg1MemInstruction(
            self()->cg(), TR::InstOpCode::lfs, firstNode, argRegister,
            new (self()->trHeapMemory())
                TR::MemoryReference(stackPtr, offset, 4, self()->cg()),
            cursor);
      }
    ...
    case TR::Double:
      if (hasToLoadFromStack &&
        ...
        argRegister = machine->getPPCRealRegister(
            properties.getFloatArgumentRegister(numFloatArgs));
        cursor = generateTrg1MemInstruction(
            self()->cg(), TR::InstOpCode::lfd, firstNode, argRegister,
            new (self()->trHeapMemory())
                TR::MemoryReference(stackPtr, offset, 8, self()->cg()),
            cursor);
      }
...
TR_HeapMemory OMR::Power::Linkage::trHeapMemory() { return self()->trMemory(); }

TR_StackMemory OMR::Power::Linkage::trStackMemory() {
