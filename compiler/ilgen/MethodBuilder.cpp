...
#define MB_REPLAY(...)                                                         \
  {                                                                            \
    REPLAY({                                                                   \
      ...
      (*_rpCpp) << "\t" << _rpLine << std::endl;                               \
    })                                                                         \
  }
#define MB_REPLAY_NONL(...)                                                    \
  {                                                                            \
    REPLAY({                                                                   \
      sprintf(_rpLine, ##__VA_ARGS__);                                         \
      (*_rpCpp) << "\t" << _rpLine;                                            \
...
MethodBuilder::MethodBuilder(TR::TypeDictionary *types)
    : TR::IlBuilder(asMethodBuilder(), types), _methodName(""),
      _returnType(NoType), _numParameters(0), _cachedParameterTypes(0),
      _cachedSignature(0), _definingFile(0), _definingLine(0), _symbols(0),
      _newSymbolsAreTemps(false), _useBytecodeBuilders(false),
      _countBlocksWorklist(0), _connectTreesWorklist(0),
  ...
  REPLAY({
    ...
    (*_rpCpp) << "#include \"ilgen/TypeDictionary.hpp\"" << std::endl
              << std::endl;
    (*_rpCpp) << "#include \"ReplayMethod.hpp\"" << std::endl << std::endl;
    (*_rpCpp) << "ReplayMethod::ReplayMethod(TR::TypeDictionary *types)"
              << std::endl;
    (*_rpCpp) << "\t: TR::MethodBuilder(types) {" << std::endl;

    _rpILCpp = new std::fstream("ReplayMethodBuildIL.cpp", std::fstream::out);
    (*_rpILCpp) << "#include \"ilgen/TypeDictionary.hpp\"" << std::endl
                << std::endl;
...
TR::MethodBuilder *MethodBuilder::asMethodBuilder() {
  return static_cast<TR::MethodBuilder *>(this);
}

void MethodBuilder::initMaps() {
  _parameterSlot =
      new (PERSISTENT_NEW) TR_HashTabString(typeDictionary()->trMemory());
  _symbolTypes =
      new (PERSISTENT_NEW) TR_HashTabString(typeDictionary()->trMemory());
  _symbolNameFromSlot =
      new (PERSISTENT_NEW) TR_HashTabInt(typeDictionary()->trMemory());
  _symbolIsArray =
      new (PERSISTENT_NEW) TR_HashTabString(typeDictionary()->trMemory());
  _memoryLocations =
      new (PERSISTENT_NEW) TR_HashTabString(typeDictionary()->trMemory());
...
void MethodBuilder::setupForBuildIL() {
  initSequence();
...
uint32_t MethodBuilder::countBlocks() {
  if (_count > -1)
    return _count;
  ...
  while (!_countBlocksWorklist->isEmpty()) {
    ...
    for (TR::BytecodeBuilder *builder = iter.getFirst(); !iter.atEnd();
      ...
      // unreachable
      // but unreachable code analysis will assert if their trees aren't in the
      // method
      // we could iterate through the trees to remove this builder's blocks from
      // the CFG
      // but it's probably easier to just connect the trees and let them be
      // ripped out later
...
bool MethodBuilder::connectTrees() {
...
bool MethodBuilder::symbolDefined(const char *name) {
...
TR::IlValue *MethodBuilder::lookupSymbol(const char *name) {
  ...
  TR::DataType type =
      ((TR::IlType *)(_symbolTypes->getData(typesID)))->getPrimitiveType();

  TR_HashId slotID;
  if (_parameterSlot->locate(name, slotID)) {
    symRef = symRefTab()->findOrCreateAutoSymbol(
        _methodSymbol, (int32_t)(uintptr_t)_parameterSlot->getData(slotID),
    ...
    _symbolNameFromSlot->add(symRef->getCPIndex(), nameFromSlotID,
                             (void *)name);
  }
...
TR::ResolvedMethod *MethodBuilder::lookupFunction(const char *name) {
...
bool MethodBuilder::isSymbolAnArray(const char *name) {
  TR_HashId isArrayID;
  return _symbolIsArray->locate(name, isArrayID);
}

TR::BytecodeBuilder *MethodBuilder::OrphanBytecodeBuilder(int32_t bcIndex,
                                                          char *name) {
...
void MethodBuilder::DefineLocal(const char *name, TR::IlType *dt) {
  MB_REPLAY("DefineLocal(\"%s\", %s);", name, REPLAY_TYPE(dt));
  TR_HashId typesID;
  _symbolTypes->add(name, typesID, (void *)dt);
}

void MethodBuilder::DefineMemory(const char *name, TR::IlType *dt,
                                 void *location) {
  MB_REPLAY("DefineMemory(\"%s\", %s, " REPLAY_POINTER_FMT ");", name,
            REPLAY_TYPE(dt), REPLAY_POINTER(location, name));
  TR_HashId typesID;
  _symbolTypes->add(name, typesID, (void *)dt);
...
void MethodBuilder::DefineParameter(const char *name, TR::IlType *dt) {
...
void MethodBuilder::DefineArrayParameter(const char *name,
                                         TR::IlType *elementType) {
  MB_REPLAY("DefineArrayParameter(\"%s\", %s);", name,
...
void MethodBuilder::DefineReturnType(TR::IlType *dt) {
  MB_REPLAY("DefineReturnType(%s);", REPLAY_TYPE(dt));
  _returnType = dt;
}

void MethodBuilder::DefineFunction(const char *const name,
                                   const char *const fileName,
                                   const char *const lineNumber,
                                   void *entryPoint, TR::IlType *returnType,
                                   int32_t numParms, ...) {
  TR::IlType **parmTypes =
      (TR::IlType **)malloc(numParms * sizeof(TR::IlType *));
  va_list parms;
...
void MethodBuilder::DefineFunction(const char *const name,
  ...
  MB_REPLAY("               " REPLAY_POINTER_FMT ",",
            REPLAY_POINTER(entryPoint, name));
  ...
  TR::ResolvedMethod *method = new (PERSISTENT_NEW)
      TR::ResolvedMethod((char *)fileName, (char *)lineNumber, (char *)name,
                         numParms, parmTypes, returnType, entryPoint, 0);

  TR_HashId functionsID;
  _functions->add(name, functionsID, (void *)method);
}

const char *MethodBuilder::getSymbolName(int32_t slot) {
  TR_HashId nameFromSlotID;
...
TR::IlType **MethodBuilder::getParameterTypes() {
  if (_cachedParameterTypes)
    return _cachedParameterTypes;

  TR_ASSERT(_numParameters < 10,
  ...
  for (int32_t p = 0; p < _numParameters; p++) {
    ...
    const char *name =
        (const char *)_symbolNameFromSlot->getData(nameFromSlotID);
...
void MethodBuilder::addToBlockCountingWorklist(TR::BytecodeBuilder *builder) {
  TraceIL("[ %p ] TR::MethodBuilder::addToBlockCountingWorklist %p\n", this,
          builder);
  _countBlocksWorklist->add(builder);
}

void MethodBuilder::addToTreeConnectingWorklist(TR::BytecodeBuilder *builder) {
  if (!builder->_connectedTrees) {
...
void MethodBuilder::addBytecodeBuilderToList(TR::BytecodeBuilder *bcBuilder) {
