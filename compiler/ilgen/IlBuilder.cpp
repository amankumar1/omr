...
#define ILB_REPLAY(...)                                                        \
  {                                                                            \
    REPLAY({                                                                   \
      sprintf(_rpLine, ##__VA_ARGS__);                                         \
      (*_rpILCpp) << "\t" << _rpLine << std::endl;                             \
    })                                                                         \
  }
...
IlBuilder::IlBuilder(TR::IlBuilder *source)
    : TR::IlInjector(source), _methodBuilder(source->_methodBuilder),
      _sequence(0), _sequenceAppender(0), _entryBlock(0), _exitBlock(0),
      _count(-1), _partOfSequence(false), _connectedTrees(false),
      _comesBack(true), _haveReplayName(false), _rpILCpp(0) {}

bool IlBuilder::TraceEnabled_log() {
  bool traceEnabled = _comp->getOption(TR_TraceILGen);
  return traceEnabled;
}

void IlBuilder::TraceIL_log(const char *s, ...) {
  va_list argp;
  va_start(argp, s);
...
void IlBuilder::setupForBuildIL() {
  initSequence();
...
void IlBuilder::print(const char *title, bool recurse) {
  if (!TraceEnabled)
    return;
...
void IlBuilder::printBlock(TR::Block *block) {
  if (!TraceEnabled)
    return;
...
TR::IlValue *IlBuilder::lookupSymbol(const char *name) {
  TR_ASSERT(_methodBuilder,
            "cannot look up symbols in an IlBuilder that has no MethodBuilder");
  return _methodBuilder->lookupSymbol(name);
}

void IlBuilder::defineSymbol(const char *name, TR::IlValue *v) {
  TR_ASSERT(_methodBuilder,
            "cannot define symbols in an IlBuilder that has no MethodBuilder");
  _methodBuilder->defineSymbol(name, v);
}

void IlBuilder::defineValue(const char *name, TR::IlType *dt) {
  TR::SymbolReference *newSymRef =
      symRefTab()->createTemporary(methodSymbol(), dt->getPrimitiveType());
  defineSymbol(name, newSymRef);
}

TR::IlValue *IlBuilder::newValue(TR::DataType dt) {
  TR::SymbolReference *newSymRef =
...
void IlBuilder::pullInBuilderTrees(TR::IlBuilder *builder,
...
TR::Node *IlBuilder::zero(TR::DataType dt) {
  switch (dt) {
  case TR::Int8:
...
TR::Node *IlBuilder::zero(TR::IlType *dt) {
  return zero(dt->getPrimitiveType());
}

TR::Node *IlBuilder::zeroNodeForValue(TR::IlValue *v) {
  return zero(v->getSymbol()->getDataType());
}

TR::IlValue *IlBuilder::zeroForValue(TR::IlValue *v) {
  TR::IlValue *returnValue = newValue(v->getSymbol()->getDataType());
  storeNode(returnValue, zeroNodeForValue(v));
  return returnValue;
}

TR::IlBuilder *IlBuilder::OrphanBuilder() {
  TR::IlBuilder *orphan =
  ...
  ILB_REPLAY("%s = %s->OrphanBuilder();", REPLAY_BUILDER(orphan),
             REPLAY_BUILDER(this));
  return orphan;
}

TR::Block *IlBuilder::emptyBlock() {
  TR::Block *empty = TR::Block::createEmptyBlock(NULL, _comp);
  cfg()->addNode(empty);
  return empty;
}

TR::IlBuilder *IlBuilder::createBuilderIfNeeded(TR::IlBuilder *builder) {
  if (builder == NULL)
    builder = OrphanBuilder();
  return builder;
}

IlBuilder::SequenceEntry *IlBuilder::blockEntry(TR::Block *block) {
  return new (_comp->trMemory()->trHeapMemory())
      IlBuilder::SequenceEntry(block);
}

IlBuilder::SequenceEntry *IlBuilder::builderEntry(TR::IlBuilder *builder) {
  return new (_comp->trMemory()->trHeapMemory())
      IlBuilder::SequenceEntry(builder);
}

void IlBuilder::appendBlock(TR::Block *newBlock, bool addEdge) {
  if (newBlock == NULL) {
    newBlock = emptyBlock();
  }
...
void IlBuilder::AppendBuilder(TR::IlBuilder *builder) {
  ILB_REPLAY("%s->AppendBuilder(%s);", REPLAY_BUILDER(this),
             REPLAY_BUILDER(builder));
  TR_ASSERT(builder->_partOfSequence == false,
  ...
  // in builder, so it can fall through to this block we're about to create
...
TR::Node *IlBuilder::loadValue(TR::IlValue *v) {
  return TR::Node::createLoad(v);
}

void IlBuilder::storeNode(TR::IlValue *dest, TR::Node *v) {
  appendBlock();
  genTreeTop(TR::Node::createStore(dest, v));
}

void IlBuilder::indirectStoreNode(TR::Node *addr, TR::Node *v) {
...
TR::IlValue *IlBuilder::indirectLoadNode(TR::IlType *dt, TR::Node *addr,
                                         bool isVectorLoad) {
...
void IlBuilder::Store(const char *varName, TR::IlValue *value) {
  ILB_REPLAY("%s->Store(\"%s\", %s);", REPLAY_BUILDER(this), varName,
             REPLAY_VALUE(value));

  if (!_methodBuilder->symbolDefined(varName))
    _methodBuilder->defineValue(
        varName, _types->PrimitiveType(value->getSymbol()->getDataType()));
  TR::IlValue *sym = lookupSymbol(varName);

  TraceIL("IlBuilder[ %p ]::Store %s %d gets %d\n", this, varName,
          sym->getCPIndex(), value->getCPIndex());
  storeNode(sym, loadValue(value));
}

void IlBuilder::VectorStore(const char *varName, TR::IlValue *value) {
  ILB_REPLAY("%s->Store(\"%s\", %s);", REPLAY_BUILDER(this), varName,
             REPLAY_VALUE(value));
...
void IlBuilder::StoreAt(TR::IlValue *address, TR::IlValue *value) {
  ILB_REPLAY("%s->StoreAt(%s, %s);", REPLAY_BUILDER(this),
             REPLAY_VALUE(address), REPLAY_VALUE(value));

  TR_ASSERT(address->getSymbol()->getDataType() == TR::Address,
            "StoreAt needs an address operand");

  TraceIL("IlBuilder[ %p ]::StoreAt address %d gets %d\n", this,
          address->getCPIndex(), value->getCPIndex());
  indirectStoreNode(loadValue(address), loadValue(value));
}

void IlBuilder::VectorStoreAt(TR::IlValue *address, TR::IlValue *value) {
  ILB_REPLAY("%s->VectorStoreAt(%s, %s);", REPLAY_BUILDER(this),
             REPLAY_VALUE(address), REPLAY_VALUE(value));
...
TR::IlValue *IlBuilder::CreateLocalArray(int32_t numElements,
                                         TR::IlType *elementType) {
  uint32_t size =
      numElements * TR::DataType::getSize(elementType->getPrimitiveType());
  ...
  ILB_REPLAY("%s = %s->CreateLocalArray(%d, %s);",
             REPLAY_VALUE(arrayAddressValue), REPLAY_BUILDER(this), numElements,
             REPLAY_TYPE(elementType));
...
TR::IlValue *IlBuilder::CreateLocalStruct(TR::IlType *structType) {
  ...
  ILB_REPLAY("%s = %s->CreateLocalStruct(%s);",
             REPLAY_VALUE(structAddressValue), REPLAY_BUILDER(this),
             REPLAY_TYPE(newStructType));
...
void IlBuilder::StoreIndirect(const char *type, const char *field,
                              TR::IlValue *object, TR::IlValue *value) {
  ILB_REPLAY("%s->StoreIndirect(\"%s\", \"%s\", %s, %s);", REPLAY_BUILDER(this),
             type, field, REPLAY_VALUE(object), REPLAY_VALUE(value));

  TR::SymbolReference *symRef =
      (TR::SymbolReference *)_types->FieldReference(type, field);
  TR::DataType fieldType = symRef->getSymbol()->getDataType();
  TraceIL("IlBuilder[ %p ]::StoreIndirect %s.%s (%d) into (%d)\n", this, type,
          field, value->getCPIndex(), object->getCPIndex());
...
TR::IlValue *IlBuilder::Load(const char *name) {
  TR::IlValue *nameSym = lookupSymbol(name);
  // looks awful much of the time, but we need to anchor the load in case
  // symbol's value is changed later
  ...
  TraceIL("IlBuilder[ %p ]::%d is Load %s (%d)\n", this,
          returnValue->getCPIndex(), name, nameSym->getCPIndex());
  storeNode(returnValue, TR::Node::createLoad(nameSym));

  ILB_REPLAY("%s = %s->Load(\"%s\");", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), name)
  return returnValue;
}

TR::IlValue *IlBuilder::VectorLoad(const char *name) {
  TR::IlValue *nameSymRef = lookupSymbol(name);
  appendBlock();
  TR::DataType returnType = nameSymRef->getSymbol()->getDataType();
  TR_ASSERT(returnType.isVector(),
  ...
  TraceIL("IlBuilder[ %p ]::%d is VectorLoad %s (%d)\n", this,
          returnValue->getCPIndex(), name, nameSymRef->getCPIndex());
...
TR::IlValue *IlBuilder::LoadIndirect(const char *type, const char *field,
                                     TR::IlValue *object) {
  TR::SymbolReference *symRef =
      (TR::SymbolReference *)_types->FieldReference(type, field);
  TR::DataType fieldType = symRef->getSymbol()->getDataType();
  ...
  TraceIL("IlBuilder[ %p ]::%d is LoadIndirect %s.%s from (%d)\n", this,
          returnValue->getCPIndex(), type, field, object->getCPIndex());
  storeNode(returnValue, TR::Node::createWithSymRef(
                             comp()->il.opCodeForIndirectLoad(fieldType), 1,
                             loadValue(object), 0, symRef));
  ILB_REPLAY("%s = %s->LoadIndirect(%s, %s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), type, field, REPLAY_VALUE(object));
  return returnValue;
}

TR::IlValue *IlBuilder::LoadAt(TR::IlType *dt, TR::IlValue *address) {
  TR_ASSERT(address->getSymbol()->getDataType() == TR::Address,
            "LoadAt needs an address operand");
  TraceIL("IlBuilder[ %p ]::LoadAt type %d address %d\n", this,
          dt->getPrimitiveType(), address->getCPIndex());
  TR::IlValue *returnValue = indirectLoadNode(dt, loadValue(address));
  ILB_REPLAY("%s = %s->LoadAt(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_TYPE(dt), REPLAY_VALUE(address));
  return returnValue;
}

TR::IlValue *IlBuilder::VectorLoadAt(TR::IlType *dt, TR::IlValue *address) {
  TR_ASSERT(address->getSymbol()->getDataType() == TR::Address,
            "LoadAt needs an address operand");
  TraceIL("IlBuilder[ %p ]::LoadAt type %d address %d\n", this,
          dt->getPrimitiveType(), address->getCPIndex());
...
TR::IlValue *IlBuilder::IndexAt(TR::IlType *dt, TR::IlValue *base,
                                TR::IlValue *index) {
  ...
  ILB_REPLAY("%s = %s->IndexAt(%s, %s, %s);", REPLAY_VALUE(address),
             REPLAY_BUILDER(this), REPLAY_TYPE(dt), REPLAY_VALUE(base),
             REPLAY_VALUE(index));

  return address;
}

TR::IlValue *IlBuilder::NullAddress() {
  appendBlock();
  TR::IlValue *returnValue = newValue(Address);
  storeNode(returnValue, TR::Node::aconst(0));
  ILB_REPLAY("%s = %s->NullAddress();", REPLAY_VALUE(address),
             REPLAY_BUILDER(this));
  TraceIL("IlBuilder[ %p ]::%d is NullAddress\n", this,
          returnValue->getCPIndex());
  return returnValue;
}

TR::IlValue *IlBuilder::ConstInt8(int8_t value) {
  appendBlock();
  TR::IlValue *returnValue = newValue(Int8);
  storeNode(returnValue, TR::Node::bconst(value));
  TraceIL("IlBuilder[ %p ]::%d is ConstInt8 %d\n", this,
          returnValue->getCPIndex(), value);
  ILB_REPLAY("%s = %s->ConstInt8(%d);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), value);
  return returnValue;
}

TR::IlValue *IlBuilder::ConstInt16(int16_t value) {
  appendBlock();
  TR::IlValue *returnValue = newValue(Int16);
  storeNode(returnValue, TR::Node::sconst(value));
  TraceIL("IlBuilder[ %p ]::%d is ConstInt16 %d\n", this,
          returnValue->getCPIndex(), value);
  ILB_REPLAY("%s = %s->ConstInt16(%d);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), value);
  return returnValue;
}

TR::IlValue *IlBuilder::ConstInt32(int32_t value) {
  appendBlock();
  TR::IlValue *returnValue = newValue(Int32);
  storeNode(returnValue, TR::Node::iconst(value));
  TraceIL("IlBuilder[ %p ]::%d is ConstInt32 %d\n", this,
          returnValue->getCPIndex(), value);
  ILB_REPLAY("%s = %s->ConstInt32(%d);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), value);
  return returnValue;
}

TR::IlValue *IlBuilder::ConstInt64(int64_t value) {
  appendBlock();
  TR::IlValue *returnValue = newValue(Int64);
  storeNode(returnValue, TR::Node::lconst(value));
  TraceIL("IlBuilder[ %p ]::%d is ConstInt64 %d\n", this,
          returnValue->getCPIndex(), value);
  ILB_REPLAY("%s = %s->ConstInt64(%ld);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), value);
  return returnValue;
}

TR::IlValue *IlBuilder::ConstFloat(float value) {
  appendBlock();
  ...
  TraceIL("IlBuilder[ %p ]::%d is ConstFloat %f\n", this,
          returnValue->getCPIndex(), value);
  ILB_REPLAY("%s = %s->ConstFloat(%f);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), value);
  return returnValue;
}

TR::IlValue *IlBuilder::ConstDouble(double value) {
  appendBlock();
  ...
  TraceIL("IlBuilder[ %p ]::%d is ConstDouble %lf\n", this,
          returnValue->getCPIndex(), value);
  ILB_REPLAY("%s = %s->ConstDouble(%lf);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), value);
  return returnValue;
}

TR::IlValue *IlBuilder::ConstString(const char *value) {
  appendBlock();
  TR::IlValue *returnValue = newValue(Address);
  storeNode(returnValue, TR::Node::aconst((uintptrj_t)value));
  TraceIL("IlBuilder[ %p ]::%d is ConstString %p\n", this,
          returnValue->getCPIndex(), value);
  ILB_REPLAY("%s = %s->ConstString(%p);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), value);
  return returnValue;
}

TR::IlValue *IlBuilder::ConstAddress(void *value) {
  appendBlock();
  TR::IlValue *returnValue = newValue(Address);
  storeNode(returnValue, TR::Node::aconst((uintptrj_t)value));
  TraceIL("IlBuilder[ %p ]::%d is ConstAddress %p\n", this,
          returnValue->getCPIndex(), value);
  ILB_REPLAY("%s = %s->ConstAddress(%p);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), value);
  return returnValue;
}

TR::IlValue *IlBuilder::ConvertTo(TR::IlType *t, TR::IlValue *v) {
  appendBlock();
  TR::IlValue *convertedValue = newValue(t);
  TR::DataType t1 = v->getSymbol()->getDataType();
  TR::DataType t2 = t->getPrimitiveType();
  TR::ILOpCodes convertOp = TR::DataType::getDataTypeConversion(
      v->getSymbol()->getDataType(), t->getPrimitiveType());
  TR::Node *result = TR::Node::create(convertOp, 1, loadValue(v));
  storeNode(convertedValue, result);
  ILB_REPLAY("%s = %s->ConvertTo(%s, %s);", REPLAY_VALUE(convertedValue),
             REPLAY_BUILDER(this), REPLAY_TYPE(t), REPLAY_VALUE(value));
  return convertedValue;
}

TR::IlValue *IlBuilder::unaryOp(TR::ILOpCodes op, TR::IlValue *v) {
  appendBlock();
  TR::IlValue *returnValue = newValue(v->getSymbol()->getDataType());
...
void IlBuilder::doVectorConversions(TR::Node **leftPtr, TR::Node **rightPtr) {
...
TR::Node *IlBuilder::binaryOpNodeFromNodes(TR::ILOpCodes op, TR::Node *leftNode,
                                           TR::Node *rightNode) {
  ...
  TR_ASSERT(leftType == rightType || isAddressBump || isRevAddressBump,
            "binaryOp requires both left and right operands to have same type "
            "or one is address and other is Int32/64");
...
TR::IlValue *IlBuilder::binaryOpFromNodes(TR::ILOpCodes op, TR::Node *leftNode,
                                          TR::Node *rightNode) {
...
TR::IlValue *IlBuilder::binaryOpFromOpMap(OpCodeMapper mapOp, TR::IlValue *left,
                                          TR::IlValue *right) {
  appendBlock();
...
TR::IlValue *IlBuilder::binaryOpFromOpCode(TR::ILOpCodes op, TR::IlValue *left,
                                           TR::IlValue *right) {
  appendBlock();
  TR::Node *leftNode = loadValue(left);
  TR::Node *rightNode = loadValue(right);
...
TR::IlValue *IlBuilder::compareOp(TR_ComparisonTypes ct, bool needUnsigned,
...
TR::IlValue *IlBuilder::NotEqualTo(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue = compareOp(TR_cmpNE, false, left, right);
  TraceIL("IlBuilder[ %p ]::%d is NotEqualTo %d != %d?\n", this,
          returnValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  ILB_REPLAY("%s = %s->NotEqualTo(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(left), REPLAY_VALUE(right));
  return returnValue;
}

void IlBuilder::Goto(TR::IlBuilder **dest) {
...
void IlBuilder::Return(TR::IlValue *value) {
  ...
  TR::Node *returnNode = TR::Node::create(
      TR::ILOpCode::returnOpCode(value->getSymbol()->getDataType()), 1,
      loadValue(value));
  genTreeTop(returnNode);
  cfg()->addEdge(_currentBlock, cfg()->getEnd());
  setDoesNotComeBack();
...
TR::IlValue *IlBuilder::Sub(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::subtractOpCode, left, right);
  TraceIL("IlBuilder[ %p ]::%d is Sub %d - %d\n", this,
          returnValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  ILB_REPLAY("%s = %s->Sub(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(left), REPLAY_VALUE(right));
  return returnValue;
}
 ...
 */
void IlBuilder::appendExceptionHandler(TR::Block *blockThrowsException,
...
void IlBuilder::setHandlerInfo(uint32_t catchType) {
...
TR::Node *IlBuilder::genOverflowCHKTreeTop(TR::Node *operationNode) {
...
TR::IlValue *IlBuilder::operationWithOverflow(TR::ILOpCodes op,
                                              TR::Node *leftNode,
                                              TR::Node *rightNode,
                                              TR::IlBuilder **handler) {
  /*
...
TR::IlValue *IlBuilder::AddWithOverflow(TR::IlBuilder **handler,
                                        TR::IlValue *left, TR::IlValue *right) {
  appendBlock();
  ...
  if (left->getSymbol()->getDataType() == TR::Address) {
    if (right->getSymbol()->getDataType() == TR::Int32)
      ...
      TR_ASSERT(0, "the right child type must be either TR::Int32 or TR::Int64 "
                   "when the left child of Add is TR::Address\n");
  } else
    op = addOpCode(leftNode->getDataType());

  TR::IlValue *addValue =
      operationWithOverflow(op, leftNode, rightNode, handler);
  TraceIL("IlBuilder[ %p ]::%d is AddWithOverflow %d + %d\n", this,
          addValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  // ILB_REPLAY("%s = %s->AddWithOverflow(%s, %s);", REPLAY_VALUE(addValue),
  // REPLAY_BUILDER(this), REPLAY_BUILDER(*handler), REPLAY_VALUE(left),
  // REPLAY_VALUE(right));
  return addValue;
}

TR::IlValue *IlBuilder::SubWithOverflow(TR::IlBuilder **handler,
                                        TR::IlValue *left, TR::IlValue *right) {
  appendBlock();

  TR::Node *leftNode = loadValue(left);
  TR::Node *rightNode = loadValue(right);
  TR::IlValue *subValue = operationWithOverflow(
      TR::ILOpCode::subtractOpCode(leftNode->getDataType()), leftNode,
      rightNode, handler);
  TraceIL("IlBuilder[ %p ]::%d is SubWithOverflow %d + %d\n", this,
          subValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  return subValue;
}

TR::IlValue *IlBuilder::MulWithOverflow(TR::IlBuilder **handler,
                                        TR::IlValue *left, TR::IlValue *right) {
  appendBlock();

  TR::Node *leftNode = loadValue(left);
  TR::Node *rightNode = loadValue(right);
  TR::IlValue *mulValue = operationWithOverflow(
      TR::ILOpCode::multiplyOpCode(leftNode->getDataType()), leftNode,
      rightNode, handler);
  TraceIL("IlBuilder[ %p ]::%d is MulWithOverflow %d + %d\n", this,
          mulValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  return mulValue;
}

TR::IlValue *IlBuilder::Mul(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::multiplyOpCode, left, right);
  TraceIL("IlBuilder[ %p ]::%d is Mul %d * %d\n", this,
          returnValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  ILB_REPLAY("%s = %s->Mul(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(left), REPLAY_VALUE(right));
  return returnValue;
}

TR::IlValue *IlBuilder::Div(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::divideOpCode, left, right);
  TraceIL("IlBuilder[ %p ]::%d is Div %d / %d\n", this,
          returnValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  ILB_REPLAY("%s = %s->Div(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(left), REPLAY_VALUE(right));
  return returnValue;
}

TR::IlValue *IlBuilder::And(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::andOpCode, left, right);
  TraceIL("IlBuilder[ %p ]::%d is And %d & %d\n", this,
          returnValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  ILB_REPLAY("%s = %s->And(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(left), REPLAY_VALUE(right));
  return returnValue;
}

TR::IlValue *IlBuilder::Or(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::orOpCode, left, right);
  TraceIL("IlBuilder[ %p ]::%d is Or %d | %d\n", this,
          returnValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  ILB_REPLAY("%s = %s->Or(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(left), REPLAY_VALUE(right));
  return returnValue;
}

TR::IlValue *IlBuilder::Xor(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::xorOpCode, left, right);
  TraceIL("IlBuilder[ %p ]::%d is Xor %d ^ %d\n", this,
          returnValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  ILB_REPLAY("%s = %s->Xor(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(left), REPLAY_VALUE(right));
  return returnValue;
}

TR::IlValue *IlBuilder::ShiftL(TR::IlValue *v, TR::IlValue *amount) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::shiftLeftOpCode, v, amount);
  TraceIL("IlBuilder[ %p ]::%d is shr %d << %d\n", this,
          returnValue->getCPIndex(), v->getCPIndex(), amount->getCPIndex());
  ILB_REPLAY("%s = %s->ShiftL(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(v), REPLAY_VALUE(amount));
  return returnValue;
}

TR::IlValue *IlBuilder::ShiftR(TR::IlValue *v, TR::IlValue *amount) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::shiftRightOpCode, v, amount);
  TraceIL("IlBuilder[ %p ]::%d is shr %d << %d\n", this,
          returnValue->getCPIndex(), v->getCPIndex(), amount->getCPIndex());
  ILB_REPLAY("%s = %s->ShiftR(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(v), REPLAY_VALUE(amount));
  return returnValue;
}

TR::IlValue *IlBuilder::UnsignedShiftR(TR::IlValue *v, TR::IlValue *amount) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::unsignedShiftRightOpCode, v, amount);
  TraceIL("IlBuilder[ %p ]::%d is shr %d << %d\n", this,
          returnValue->getCPIndex(), v->getCPIndex(), amount->getCPIndex());
  ILB_REPLAY("%s = %s->UnsignedShiftR(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(v), REPLAY_VALUE(amount));
  return returnValue;
}

TR::IlValue *IlBuilder::EqualTo(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue = compareOp(TR_cmpEQ, false, left, right);
  TraceIL("IlBuilder[ %p ]::%d is EqualTo %d == %d?\n", this,
          returnValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  ILB_REPLAY("%s = %s->EqualTo(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(left), REPLAY_VALUE(right));
  return returnValue;
}

TR::IlValue *IlBuilder::LessThan(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue = compareOp(TR_cmpLT, false, left, right);
  TraceIL("IlBuilder[ %p ]::%d is LessThan %d < %d?\n", this,
          returnValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  ILB_REPLAY("%s = %s->LessThan(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(left), REPLAY_VALUE(right));
  return returnValue;
}

TR::IlValue *IlBuilder::GreaterThan(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue = compareOp(TR_cmpGT, false, left, right);
  TraceIL("IlBuilder[ %p ]::%d is GreaterThan %d < %d?\n", this,
          returnValue->getCPIndex(), left->getCPIndex(), right->getCPIndex());
  ILB_REPLAY("%s = %s->GreaterThan(%s, %s);", REPLAY_VALUE(returnValue),
             REPLAY_BUILDER(this), REPLAY_VALUE(left), REPLAY_VALUE(right));
  return returnValue;
}

TR::IlValue *IlBuilder::Call(const char *functionName, int32_t numArgs, ...) {
  TR::IlValue **argValues =
      (TR::IlValue **)_comp->trMemory()->allocateHeapMemory(
...
TR::IlValue *IlBuilder::Call(const char *functionName, int32_t numArgs,
  ...
  TR::SymbolReference *methodSymRef = symRefTab()->findOrCreateMethodSymbol(
      JITTED_METHOD_INDEX, -1, resolvedMethod, TR::MethodSymbol::Static);
...
void IlBuilder::IfCmpNotEqualZero(TR::IlBuilder **target,
                                  TR::IlValue *condition) {
  *target = createBuilderIfNeeded(*target);
  ILB_REPLAY("%s->IfCmpNotEqualZero(%s, %s, %s);", REPLAY_BUILDER(this),
             REPLAY_BUILDER(target), REPLAY_VALUE(condition));
  TraceIL("IlBuilder[ %p ]::IfCmpNotEqualZero %d? -> B%d\n", this,
          condition->getCPIndex(), (*target)->getEntry()->getNumber());
  ifCmpNotEqualZero(condition, (*target)->getEntry());
}

void IlBuilder::IfCmpNotEqual(TR::IlBuilder **target, TR::IlValue *left,
                              TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
  ILB_REPLAY("%s->IfCmpNotEqual(%s, %s, %s);", REPLAY_BUILDER(this),
             REPLAY_BUILDER(target), REPLAY_VALUE(left), REPLAY_VALUE(right));
  TraceIL("IlBuilder[ %p ]::IfCmpNotEqual %d == %d? -> B%d\n", this,
          left->getCPIndex(), right->getCPIndex(),
          (*target)->getEntry()->getNumber());
  ifCmpNotEqual(left, right, (*target)->getEntry());
}

void IlBuilder::IfCmpEqualZero(TR::IlBuilder **target, TR::IlValue *condition) {
  *target = createBuilderIfNeeded(*target);
  ILB_REPLAY("%s->IfCmpEqualZero(%s, %s, %s);", REPLAY_BUILDER(this),
             REPLAY_BUILDER(target), REPLAY_VALUE(condition));
...
void IlBuilder::IfCmpEqual(TR::IlBuilder **target, TR::IlValue *left,
                           TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
  ILB_REPLAY("%s->IfCmpEqual(%s, %s, %s);", REPLAY_BUILDER(this),
             REPLAY_BUILDER(target), REPLAY_VALUE(left), REPLAY_VALUE(right));
...
void IlBuilder::IfCmpLessThan(TR::IlBuilder **target, TR::IlValue *left,
                              TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
  ILB_REPLAY("%s->IfCmpLessThan(%s, %s, %s);", REPLAY_BUILDER(this),
             REPLAY_BUILDER(target), REPLAY_VALUE(left), REPLAY_VALUE(right));
...
void IlBuilder::IfCmpGreaterThan(TR::IlBuilder **target, TR::IlValue *left,
                                 TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
  ILB_REPLAY("%s->IfCmpGreaterThan(%s, %s, %s);", REPLAY_BUILDER(this),
             REPLAY_BUILDER(target), REPLAY_VALUE(left), REPLAY_VALUE(right));
...
void IlBuilder::ifCmpNotEqualZero(TR::IlValue *condition, TR::Block *target) {
  ifCmpNotEqual(condition, zeroForValue(condition), target);
}

void IlBuilder::ifCmpNotEqual(TR::IlValue *left, TR::IlValue *right,
                              TR::Block *target) {
  appendBlock();
  TR::Node *leftNode = loadValue(left);
  ifjump(comp()->il.opCodeForIfCompareNotEquals(leftNode->getDataType()),
         leftNode, loadValue(right), target);
  appendBlock();
}

void IlBuilder::ifCmpEqualZero(TR::IlValue *condition, TR::Block *target) {
  ifCmpEqual(condition, zeroForValue(condition), target);
}

void IlBuilder::ifCmpEqual(TR::IlValue *left, TR::IlValue *right,
                           TR::Block *target) {
  appendBlock();
  TR::Node *leftNode = loadValue(left);
  ifjump(comp()->il.opCodeForIfCompareEquals(leftNode->getDataType()), leftNode,
         loadValue(right), target);
  appendBlock();
}

void IlBuilder::ifCmpLessThan(TR::IlValue *left, TR::IlValue *right,
                              TR::Block *target) {
  appendBlock();
  TR::Node *leftNode = loadValue(left);
...
void IlBuilder::ifCmpGreaterThan(TR::IlValue *left, TR::IlValue *right,
                                 TR::Block *target) {
  appendBlock();
  TR::Node *leftNode = loadValue(left);
...
void IlBuilder::appendGoto(TR::Block *destBlock) {
  appendBlock();
  gotoBlock(destBlock);
}
 ...
 * causes branch around to the merge point
 * #1: then path exists "out of line", else path falls through from If and then
 * jumps to merge, followed by then path which
 *     falls through to the merge point
 */
void IlBuilder::IfThenElse(TR::IlBuilder **thenPath, TR::IlBuilder **elsePath,
                           TR::IlValue *condition) {
  // ILB_REPLAY_BEGIN();
  TR_ASSERT(thenPath != NULL || elsePath != NULL,
  ...
  // ILB_REPLAY_END();
  // ILB_REPLAY("IfThenElse(%s, %s, %s);", REPLAY_PTRTOBUILDER(thenPath),
  // REPLAY_PTRTOBUILDER(elsePath), REPLAY_VALUE(condition));
}

void IlBuilder::Switch(const char *selectionVar, TR::IlBuilder **defaultBuilder,
                       uint32_t numCases, int32_t *caseValues,
                       TR::IlBuilder **caseBuilders, bool *caseFallsThrough) {
  // ILB_REPLAY_BEGIN();
  TR::IlValue *selectorValue = Load(selectionVar);
  TR_ASSERT(selectorValue->getSymbol()->getDataType() == TR::Int32,
  ...
  *defaultBuilder = createBuilderIfNeeded(*defaultBuilder);
  ...
  // passed in via caseBuilder (or will be passed
  //   back via caseBuilders, and second a builder that branches to the
  //   breakBuilder (unless this case falls through)
...
void IlBuilder::Switch(const char *selectionVar, TR::IlBuilder **defaultBuilder,
  ...
  // ILB_REPLAY("%s->IfCmpGreaterThan(%s, %s);", REPLAY_BUILDER(this),
  // REPLAY_BUILDER(target), REPLAY_VALUE(left), REPLAY_VALUE(right));
  int32_t *caseValues = (int32_t *)_comp->trMemory()->allocateHeapMemory(
      numCases * sizeof(int32_t));
  TR_ASSERT(0 != caseValues, "out of memory");

  TR::IlBuilder **caseBuilders =
      (TR::IlBuilder **)_comp->trMemory()->allocateHeapMemory(
          numCases * sizeof(TR::IlBuilder *));
  TR_ASSERT(0 != caseBuilders, "out of memory");

  bool *caseFallsThrough =
      (bool *)_comp->trMemory()->allocateHeapMemory(numCases * sizeof(bool));
  ...
  Switch(selectionVar, defaultBuilder, numCases, caseValues, caseBuilders,
         caseFallsThrough);

  // if Switch created any new builders, we need to put those back into the
  // arguments passed into this Switch call
  va_start(cases, numCases);
  for (int32_t c = 0; c < numCases; c++) {
...
void IlBuilder::ForLoop(bool countsUp, const char *indVar,
                        TR::IlBuilder **loopCode, TR::IlBuilder **breakBuilder,
                        TR::IlBuilder **continueBuilder, TR::IlValue *initial,
                        TR::IlValue *end, TR::IlValue *increment) {
  // ILB_REPLAY_BEGIN();
  ...
  *loopCode = createBuilderIfNeeded(*loopCode);
  ...
  TraceIL("IlBuilder[ %p ]::ForLoop ind %s initial %d end %d increment %d "
          ...
          this, indVar, initial->getCPIndex(), end->getCPIndex(),
          increment->getCPIndex(), *loopCode, countsUp);
  ...
  if (continueBuilder) {
    TR_ASSERT(*continueBuilder == NULL, "ForLoop returns continueBuilder, "
                                        "cannot provide continueBuilder as "
                                        "input");
...
void IlBuilder::DoWhileLoop(const char *whileCondition, TR::IlBuilder **body,
  ...
  *body = createBuilderIfNeeded(*body);
  ...
  if (continueBuilder) {
    TR_ASSERT(*continueBuilder == NULL, "DoWhileLoop returns continueBuilder, "
                                        "cannot provide continueBuilder as "
                                        "input");
  ...
  if (breakBuilder) {
    TR_ASSERT(*breakBuilder == NULL, "DoWhileLoop returns breakBuilder, cannot "
                                     "provide breakBuilder as input");
    *breakBuilder = OrphanBuilder();
    AppendBuilder(*breakBuilder);
  }
...
void IlBuilder::WhileDoLoop(const char *whileCondition, TR::IlBuilder **body,
                            TR::IlBuilder **breakBuilder,
                            TR::IlBuilder **continueBuilder) {
  // ILB_REPLAY_BEGIN();
  ...
  if (breakBuilder) {
    TR_ASSERT(*breakBuilder == NULL, "WhileDoLoop returns breakBuilder, cannot "
                                     "provide breakBuilder as input");
    *breakBuilder = done;
  }
  ...
  TR::IlBuilder *loopContinue = OrphanBuilder();
  if (continueBuilder) {
    TR_ASSERT(*continueBuilder == NULL, "WhileDoLoop returns continueBuilder, "
                                        "cannot provide continueBuilder as "
                                        "input");
    *continueBuilder = loopContinue;
  }
