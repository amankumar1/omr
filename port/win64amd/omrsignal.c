...
static uint32_t infoForGPR(struct OMRPortLibrary *portLibrary,
                           struct J9Win32SignalInfo *info, int32_t index,
...
int32_t omrsig_protect(struct OMRPortLibrary *portLibrary,
   ...
   * masterVectoredExceptionHandler.
   * Because we are checking without acquiring the monitor first, we'll have to
   * check again.
   * Note that that we don't uninstall the masterHandlerMonitor, so we only have
   * to account for it going from not installed to installed.
  */
  ...
  if (J9_ARE_ALL_BITS_SET(flags, OMRPORT_SIG_FLAG_MAY_RETURN)) {
     ...
     * clause/block below,
     * this tells the VectoredExceptionHandler to defer to this _except
     * clause/block */
    thisRecord.deferToTryExcept = TRUE;

    /*
     * Jazz 55704 - functions are statically marked with UNW_FLAG_EHANDLER if
     * they contain a _try/__except block.
     * This confuses tryExceptHandlerExistsOnStack() into seeing omrsig_protect
     * as a _try/__except function,
...
uint32_t omrsig_set_async_signal_handler(struct OMRPortLibrary *portLibrary,
  ...
  if (cursor == NULL) {
    ...
    if (flags != 0) {
      J9WinAMD64AsyncHandlerRecord *record = portLibrary->mem_allocate_memory(
...
int32_t omrsig_startup(struct OMRPortLibrary *portLibrary) {
  ...
  j9SigProtectFunction = RtlLookupFunctionEntry(
 ...
 *
 * In windows, a new thread is created to run the handlers each time a signal is
 * received,
...
static uint32_t infoForOther(struct OMRPortLibrary *portLibrary,
  ...
  *name = "";
   ...
   * exposing it through OMRPORT_SIG_WINDOWS_DEFER_TRY_EXCEPT_HANDLER
  ...
  if (OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH == result) {
    ...
    while (thisRecord) {
      if (thisRecord->flags & type) {
        ...
        result = thisRecord->handler(thisRecord->portLibrary, type, &j9info,
...
addMasterVectoredExceptionHandler(struct OMRPortLibrary *portLibrary) {
   ...
   * current
   * point of execution. This only works for code in a module (e.g. a DLL).
   * Since JIT
...
static int32_t initializeSignalTools(OMRPortLibrary *portLibrary) {
  ...
  if (omrthread_monitor_init_with_name(&masterExceptionMonitor, 0,
                                       "portLibrary_omrsig_async_monitor")) {
    omrthread_monitor_destroy(asyncMonitor);
    return -1;
  }
...
static void fillInWinAMD64SignalInfo(struct OMRPortLibrary *portLibrary,
  ...
  j9info->portLibType = mapWin32ExceptionToPortlibType(
      exceptionInfo->ExceptionRecord->ExceptionCode);
  ...
  if (0 == omrthread_attach_ex(NULL, J9THREAD_ATTR_DEFAULT)) {
    ...
    while (cursor) {
      ...
      if (cursor->flags & flags) {
        ...
        /* Returning TRUE stops control from being passed to the next handler
         * routine. The OS default handler may be the next handler routine.
         * The default action of the OS default handler routine is to shut down
         * the process
         */
    ...
    /* TODO: possible timing hole. The thread library could be unloaded by the
     * time we reach this line. We can't use omrthread_exit(), as that kills the
