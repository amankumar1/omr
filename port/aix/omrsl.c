...
uintptr_t omrsl_open_shared_library(struct OMRPortLibrary *portLibrary,
  ...
  uintptr_t result;
  int lazyOrNow =
      J9_ARE_ALL_BITS_SET(flags, OMRPORT_SLOPEN_LAZY) ? RTLD_LAZY : RTLD_NOW;
   ...
   * defined LIBPATH) allows
   * us to load the library with the current libpath instead of the one at
   * process creation
   * time. We can then call dlopen() as per normal and the just loaded library
   * will be found.
   */
            ...
            if (NULL == handle) {
              ...
              if (NULL == handle) {
                ...
                if ((NULL != actualFileName) &&
                   ...
                   * specifying an archive member.
                   * eg archive(member).
                   * This translates into a file of name 'archive', to which
                   * dlopen will look to load 'member' since RTLD_MEMBER is
                   * specified.
                   */
