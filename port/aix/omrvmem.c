...
int32_t omrvmem_startup(struct OMRPortLibrary *portLibrary) {
  ...
  if (num_psizes == -1) {
     ...
     * anything dangerous if this happens as it may, some day.
     * We will use the earliest values in the set since they are the ones we
     * should understand (if the OS adds new sizes at the beginning of the list
...
void *omrvmem_commit_memory(struct OMRPortLibrary *portLibrary, void *address,
  ...
  if (rangeIsValid(identifier, address, byteAmount)) {
    if (0 == (identifier->mode & OMRPORT_VMEM_MEMORY_MODE_EXECUTE)) {
       ...
       * are not required for executable pages
       * because when using default page size they are allocated using malloc()
       * which may not return page aligned address and any attempt to align it
       * to page boundary would waste one page of memory.
       */
   ...
   * required.
   * However, if the memory cannot actually be faulted in when needed, the VM
   * will crash
   * during execution in a much less obvious way.  Adding a VM flag to force the
   * init is
   * a RAS TODO.
   */
}
...
int32_t omrvmem_free_memory(struct OMRPortLibrary *portLibrary,
  ...
  if (OMRPORT_VMEM_RESERVE_USED_J9MEM_ALLOCATE_MEMORY ==
    ...
    omrmem_categories_decrement_counters(identifier->category,
                                         identifier->size);
...
void *omrvmem_reserve_memory_ex(struct OMRPortLibrary *portLibrary,
   ...
   * call (essentially all of the available memory), and
   * a 64-bit process can request at least 1 TB.  We will consider 1 TB to be an
   * acceptable restriction, and not resort to the additional
  ...
  printf("\n\tomrvmem_reserve_memory_ex byteAmount: %p, startAddress: %p, "
         ...
         params->pageSize, (OMRPORT_VMEM_STRICT_PAGE_SIZE & params->options)
  ...
  if (0 == params->pageSize) {
     ...
     *were called with one of our understood sizes.
 ...
 * Do it only if known page size allocation was requested - for compatibility
 * with old code
 */
#if defined(J9VMEM_DEBUG)
 ...
 * detectAndRecordPageSize() (below) will determine if the page size we get is
 * acceptable */
#if defined(J9VMEM_DEBUG)
...
static BOOLEAN detectAndRecordPageSize(struct OMRPortLibrary *portLibrary,
   ...
   * memory */
  update_vmemIdentifier(identifier, memoryPointer, (void *)addressKey,
 ...
 * as
 * the error handling code uses per thread buffers to store the last error.  If
 * memory
...
static uintptr_t getDataSegmentPageSize(struct OMRPortLibrary *portLibrary) {
  ...
  if (0 == rc) {
     ...
     * invalid,
     * which should never happen here as portLibrary->portGlobals is valid until
     * VM shutdown.
     */
...
void omrvmem_default_large_page_size_ex(struct OMRPortLibrary *portLibrary,
  ...
  if (NULL != pageSize) {
    if (OMRPORT_VMEM_MEMORY_MODE_EXECUTE !=
       ...
       * page sizes (in that order).
       * To be compatible with how the -Xlp option has historically worked, 16
       * MiB is the default page size.
       * If, for some reason, that page size can't be found, we search for 64KB
       * page size.
       * If that is also not found, we will return the page size at [1] since
       * failing is
       * probably not a reasonable answer as it would break our support for that
       * legacy option (and that
...
static void *getMemoryInRangeForDefaultPages(
   ...
   * can be used for thread stacks.  Thus, explicitly use the mmap
   * allocator for default-sized pages (large pages still must use shmat).
   * The only reasons to use shmat, other than unification of the code paths,
   * are to support large pages or NUMA.  NUMA is only supported on
   * the Balanced policy which does not yet run on 32-bit and large page
   * allocations come through a different path.
   *
   ...
   * returned by omrvmem_reserve_memory(). This is
   * not supported for memory returned by default_pageSize_reserve_memory(), as
   * it uses the mmap() API for historical reasons.
   *
   * To support omrvmem_numa_set_affinity() under balanced, we redefine
   * getMemoryInRangeForDefaultPages() to simply call
   * reserveLargePages(), which uses the shmat()/shmget() APIs which do not have
   * the same problem.
   *
   * Note that there is a limitation in older versions of AIX (pre-6.1) where
   * shmem may be unable to allocate as much memory as
   * mmap (up to one segment is lost - 256M).  However, the NUMA API is not
   * supported prior to AIX 6.1 so these requirements
   * never appear together.  Thus, if NUMA is supported on the system, use
   * shmem.  Otherwise, use mmap (as we will not be trying
   * to use the NUMA API).
   ...
   *
   */
  if (__ENHANCED_AFFINITY() && (OMRPORT_VMEM_MEMORY_MODE_EXECUTE !=
                                (OMRPORT_VMEM_MEMORY_MODE_EXECUTE & mode))) {
 ...
 * Returns pointer to newly allocated memory upon success and if default page
 * size
 ...
 * using shmem for vmem, which NUMA requires,
...
int32_t omrvmem_get_process_memory_size(struct OMRPortLibrary *portLibrary,
  ...
  case OMRPORT_VMEM_PROCESS_VIRTUAL:
     ...
     * such an
     * API existed, we could return getProcessPrivateMemorySize() + sharedSize
     * here.
     *
