...
extern uint32_t compareAndSwapU32(uint32_t *location, uint32_t oldValue,
...
void omrmem_categories_increment_counters(OMRMemCategory *category,
  ...
  do {
    oldValue = category->liveAllocations;
                               ...
                               oldValue + 1,
                               &category->liveAllocationsLockWord) != oldValue);
...
void omrmem_categories_increment_bytes(OMRMemCategory *category,
  ...
  } while (compareAndSwapUDATA(&category->liveBytes, oldValue, oldValue + size,
                               &category->liveBytesLockWord) != oldValue);
}
...
OMRMemCategory *omrmem_get_category(struct OMRPortLibrary *portLibrary,
   ...
   * PORT_LIBRARY and ALLOCATE32 are inserted in the category tables but the
   * port library
...
void omrmem_walk_categories(struct OMRPortLibrary *portLibrary,
  ...
  if (portLibrary->portGlobals->control.language_memory_categories.categories !=
    ...
    _recursive_category_walk_root(
        portLibrary, state, portLibrary->portGlobals->control
 ...
 * the memory operations may be created here.  All resources created here should
 * be destroyed
