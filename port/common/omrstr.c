 ...
 *encoding or Unicode (wide character),
 * and modified UTF-8.
 ...
 *operating system)
 *                              J9STR_CODE_PLATFORM_OMR_INTERNAL (encoding used
 *by certain operating system calls)
 *                              J9STR_CODE_WINTHREADACP,
 *J9STR_CODE_WINDEFAULTACP (thread and default ANSI code page, Windows only)
 ...
 *  	- OMRPORT_ERROR_STRING_MEM_ALLOCATE_FAILED if the port library could
 *not allocate a working buffer
 ...
 *  	[ISO Latin-1 (8859-1), UTF-8, Windows ANSI default and current code
 *pages] to modified UTF-8
...
int32_t omrstr_convert(struct OMRPortLibrary *portLibrary, int32_t fromCode,
  ...
  case J9STR_CODE_MUTF8: {
    switch (toCode) {
    case J9STR_CODE_PLATFORM_RAW:
      result = convertMutf8ToPlatform(portLibrary, inBuffer, inBufferSize,
  ...
  case J9STR_CODE_LATIN1: {
    ...
    case J9STR_CODE_MUTF8: {
      const uint8_t *utf8Cursor = inBuffer;
      uintptr_t utf8Remaining = inBufferSize;
      result = convertLatin1ToMutf8(portLibrary, &utf8Cursor, &utf8Remaining,
                                    outBuffer, outBufferSize);
      /*
 ...
 * and the maximum
 * size willing to create.  For example for no restrictions result=NULL,
 * length=(uint32_t)(-1), to restrict
 ...
 * that were created by @ref omrstr_startup
 ...
 * the string operations may be created here.  All resources created here should
 * be destroyed
...
#define ASID_BUF_LEN 16
  ...
  if (0 ==
      portLibrary->sysinfo_get_username(portLibrary, username,
                                        USERNAME_BUF_LEN)) {
...
intptr_t omrstr_set_time_tokens(struct OMRPortLibrary *portLibrary,
  ...
  char buf
      [TICK_BUF_SIZE +
       20]; /* TICK_BUF_SIZE + 4 + 2 + 2 + 2 + 2 + 2 + 2 + 3 + an extra byte */
...
intptr_t omrstr_set_token(struct OMRPortLibrary *portLibrary,
  ...
  tokenBufLen = portLibrary->str_vprintf(portLibrary, NULL, (unsigned int)-1,
                                         format, args);
  va_end(args);
 ...
 * holding data. This
 * is useful is RAS where memory must be pre-allocated, allowing strings up to
 * 511 chars + null
 ...
 * characters with the
 * token provided. The longest key which matches the characters in format will
 * be selected.
 *
...
uintptr_t omrstr_subst_tokens(struct OMRPortLibrary *portLibrary, char *buf,
  ...
  /* Second case: we're actually writing to buf. */
     ...
     * terminate
     * it then set cnt to the minimum buf size required for a successful subst
     * call */
    else {
      buf[bufLen - 1] = '\0';
 ...
 * string are copied.
 * to buf without conversion.  Conversion specifiers are introduced by a '%'
 * character, and are replaced in buf as follows:.
 * <ul>
 ...
 * milliseconds since January 1st 1970.  Note that
 * the implementation will not do any timezone correction and assumes that this
 * value is in terms of the required timezone.
 *
...
* wide characters to modified UTF-8.
*  iconv() is resumable if the output buffer is full, but Windows
* MultiByteToWideChar() is not.
...
/* set up buffers and convertors */
  ...
  requiredBufferSize =
      WIDE_CHAR_SIZE * MultiByteToWideChar(codePage, OS_ENCODING_MB_FLAGS,
                                           inBuffer, (int)inBufferSize, NULL,
...
* characters to platform encoding .
* iconv() both is resumable if the output buffer is full, but Windows
* WideCharToMultiByte() is not.
 * @param[in]  inBuffer        input string  to be converted.
 * @param[in]  inBufferSize  input string size in bytes.
 * @param[in] outBuffer    user-allocated output buffer that stores converted
* characters, ignored if inBufferSize is 0.
...
/* set up convertor state.  Note: no convertor state is required on Windows */
  ...
  while (mutf8Remaining > 0) { /* translated section by section as dictated by
    ...
    platformPartCount =
        convertWideToPlatform(portLibrary, encodingState, &wideBufferCursor,
     ...
     * after the converted characters to 0,
...
static int32_t convertWideToMutf8(const uint8_t **inBuffer,
  ...
  if (0 == outBufferSize) { /* we just want the length */
    while (wideRemaining > 0) {
      ...
      encodeResult = encodeUTF8CharN(wideChar, NULL, 3); /* convert one wide
                                                            character, get the
                                                            length, ignore the
                                                            converted result */
      if (0 == encodeResult) {
...
static int32_t convertLatin1ToMutf8(struct OMRPortLibrary *portLibrary,
  ...
  while ((latinRemaining > 0) &&
         ((mutf8Remaining > 0) || (0 == outBufferSize)) /* outBufferSize == 0
                                                           indicates we just
                                                           want the length */
         && (resultSize >= 0)) {
    ...
    uint16_t localWideBuffer[CONVERSION_BUFFER_SIZE]; /* handle short strings
                                                         without allocating
                                                         memory */
...
* pointer and size are updated (except on Windows) to allow
* resumption if the translation failed due to output buffer too small.  The
* output buffer may not be null.
* The output buffer may contain additional data such as a byte order mark (0xff,
* 0xfe or vice versa)
* Resumable, so it updates its inputs
* @param [in] portLibrary port library
* @param [in] encodingState iconv_t on on Linux, z/OS and AIX, void * on Windows
* (unused)
...
* @param [inout] inBufferSize size in characters of the platform-encoded string.
* Updated to indicate number of remaining untranslated characters.
...
convertPlatformToWide(struct OMRPortLibrary *portLibrary,
  ...
  int32_t mbChars = (int32_t)MultiByteToWideChar(
      codePage, OS_ENCODING_MB_FLAGS, *inBuffer, (int)*inBufferSize,
...
* buffer pointers and sizes are updated (except on Windows) to allow
* resumption if the translation failed due to output buffer too small.
* Resumable, so it updates its inputs
* The output buffer may contain additional data such as a byte order mark (0xff,
* 0xfe or vice versa)
* If the buffer is large enough, the byte after the last output character is set
* to 0.
* @param [in] portLibrary port library
...
static int32_t convertWideToPlatform(struct OMRPortLibrary *portLibrary,
  ...
  if (0 == outBufferSize) { /* get the required buffer size */
    ...
    iconvResult = iconv(
        encodingState, (char **)inBuffer, (size_t *)inBufferSize,
        (char **)&platformCursor,
        (size_t *)&platformLimit); /* input and output pointers and limits are
                                      updated */
  ...
  resultSize = WideCharToMultiByte(
      OS_ENCODING_CODE_PAGE, OS_ENCODING_MB_FLAGS, wideCursor,
