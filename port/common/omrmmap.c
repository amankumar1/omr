 ...
 *                              so that it can be shared between processes.  If
...
* may wait for the file write
 * to complete or this may be scheduled for a later time and the function return
...
* Synchronous update required, function will not
 *                                                                     return
* until file updated.  Note that to achieve this on Windows requires the
 *                                                                     file to
* be opened with the FILE_FLAG_WRITE_THROUGH flag
 * @args                                           OMRPORT_MMAP_SYNC_ASYNC
* Asynchronous update required, function returns
 *                                                                    immediately,
* file will be updated later
 * @args                                           OMRPORT_MMAP_SYNC_INVALIDATE
* Requests that other mappings of the same
 *                                                                    file be
* invalidated, so that they can be updated with the values just written
 *
 ...
 * the memory mapping operations may be created here.  All resources created
 ...
 * that were created by @ref omrmmap_startup
 ...
 * or part of the interval address->(address+len).
 * The size of the memory page for a specified memory region can be requested
 * via @ref omrmmap_get_region_granularity
 *
 ...
 * means that if the page had previously been marked
 * OMRPORT_PAGE_PROTECT_READ and omrmmap_protect is called with
 * OMRPORT_PAGE_PROTECT_WRITE, it will no longer be readable.
 *
 * @param[in] portLibrary the Port Library.
 ...
 * specified interval
 * \arg OMRPORT_PAGE_PROTECT_NONE accessing the memory will cause a segmentation
 * violation
