 ...
 * should be at least larger than this.
 * We account for the header size plus 2 padding slots for the initial block in
 * the heap.
 */
...
* failure.
*
* @note this function can also be used to re-initialize a heap -- useful when we
* want to erase its contents and start from scratch.
*
...
*
* @note the algorithm used in this suballocator is based on the first-fit method
* in KNUTH, D. E. The Art of Computer Programming. Vol. 1: Fundamental
...
* initialized heap.
* Grow is assumed to happen at the end of the memory range originally used for
* the heap, i.e.
