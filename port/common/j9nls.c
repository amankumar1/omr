...
static void writeSyslog(struct OMRPortLibrary *portLibrary, uintptr_t flags,
  ...
  COPY_VA_LIST(copyOfArgs, args);
  length = portLibrary->str_vprintf(portLibrary, NULL, (uint32_t)(-1), format,
                                    copyOfArgs);
  length += 1; /* For the \0 */
...
void j9nls_vprintf(struct OMRPortLibrary *portLibrary, uintptr_t flags,
  ...
  if (TrcEnabled_Trc_PRT_j9nls_vprintf) {
     ...
     * Uses simple buffer for message
     * text, limit of 1024 characters. NLS messages are typically less than 80
     * characters.
     */
    COPY_VA_LIST(argsForTrace, args);
    portLibrary->str_vprintf(portLibrary, buffer, sizeof(buffer), message,
                             argsForTrace);
...
static void open_catalog(struct OMRPortLibrary *portLibrary) {
   ...
   *locale descriptors to the file name as possible to find a file.
   * e.g.
 ...
 * the NLS library operations may be created here.  All resources created here
 * should be destroyed
 ...
 * was called).  This should be
 * called whenever the port library memory allocation routines are changed, to
 * ensure that allocation and
 * deallocation routines are correctly paired ( @ref omrmem_allocate_memory and
 * @ref omrmem_free_memory ).
 *
 * @param[in] portLibrary The port library
 *
 * @deprecated NLS API is deprecated.
