...
int32_t initializeTable(OMRPortLibrary *portLibrary, uintptr_t dllHandle,
  ...
  for (uintptr_t index = 0; index < entryCount; ++index) {
    ...
    if (0 !=
        portLibrary->sl_lookup_name(portLibrary, dllHandle, (char *)entry->name,
...
uint32_t openDriver(OMRPortLibrary *portLibrary) {
  ...
  if (0 !=
      portLibrary->sl_open_shared_library(portLibrary, (char *)driverLibrary,
  ...
  /* initialize the driver */
  {
    ...
    if (0 !=
        portLibrary->sl_lookup_name(portLibrary, globals->driverHandle,
                                    (char *)"cuInit", (uintptr_t *)&driverInit,
                                    "iI")) {
...
uint32_t openRuntimeAndGetVersion(OMRPortLibrary *portLibrary,
  ...
  if (0 !=
      portLibrary->sl_open_shared_library(portLibrary, (char *)library,
                                          &dllHandle, OMRPORT_SLOPEN_LAZY)) {
  ...
  /* query the runtime version */
  {
    ...
    if (0 !=
        portLibrary->sl_lookup_name(portLibrary, dllHandle,
...
#define OMRCUDA_LIBRARY_ENTRY(major, minor)                                    \
  { ((major)*1000) + ((minor)*10), OMRCUDA_LIBRARY_NAME(major, minor) }
    ...
    OMRCUDA_LIBRARY_ENTRY(7, 5), OMRCUDA_LIBRARY_ENTRY(7, 0),
    OMRCUDA_LIBRARY_ENTRY(6, 5), OMRCUDA_LIBRARY_ENTRY(6, 0),
...
VMINLINE cudaError_t validateDeviceId(OMRPortLibrary *portLibrary,
  ...
  return (0 == deviceCount) ? cudaErrorNoDevice : (deviceId < deviceCount)
                                                      ? cudaSuccess
 ...
 * for
 * a kernel function. The set of flags is defined by the enum
 ...
 * maximal
 * occupancy of a kernel function in conjunction with use of the template
 * function
 * 'withDevice'.
 *
 * @param[in] function the function pointer
 ...
 * @param[in] dynamicSharedMemoryFunction a function which gives the dynamic
 * shared
 * memory required for a particular block size; NULL may be used if the
 * requirements
 * don't depend upon the block size, in which case userData will be used to
 * define
 ...
 * memory.
 * The value of the flags parameter must be produced by ORing together one or
 * more
 * of the literals defined by the enum J9CudaHostAllocFlags, declared in
 * omrcuda.h.
 *
 * @param[in] portLibrary the port library pointer
 * @param[in] size the number of bytes requested
