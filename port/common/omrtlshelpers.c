 ...
 * the threads.
 * For example when an OS system call fails the error code associated with that
 * error is
 * relevant to the thread that called the OS function; it has no meaning to any
 * other thread.
 *
 * This file contains the functions supported by the port library for creating,
 * accessing and
 * destroying per thread buffers.@see omrportptb.h for details on the per thread
 * buffer structure.
 *
 * Only the function @omrport_tls_free is available via the port library
 * function table.  The rest of
 ...
 * the portl library thread local storage operations may be created here.  All
 * resources created here should be destroyed
 ...
 * do not allocate a new
 * one, the function @ref omrport_tls_get is used for that purpose. This
 * function is not exported outside
 * the port library as most applications will want a per thread buffer created
 * to store their data.  This
