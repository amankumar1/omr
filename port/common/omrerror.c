 ...
 * The OS error code as well as the corresponding portable error code are stored
 * in the per thread
 * buffers @ref omrportptb.h via a call the @ref omrerror_set_last_error or @ref
 * omrerror_set_last_error_with_message.
 * These functions are meant for private use by the port library, to set the
 * error related to a system call failure.
 * They are in the port library table as people overriding port library
 * functions may need to set the error
 * message accordingly.
 *
 * The majority of applications are not interested in the human readable error
 * message corresponding to
 * the error.  As a result the error message is not stored at time of the
 * reported error, but can be looked
 * up at a later time.
 */
 ...
 * the error handling operations may be created here.  All resources created
 ...
 * that were created by @ref omrerror_startup
 ...
 * reference.  Once stored an
 * application may obtain the error message describing the last stored error by
 * calling omrerror_last_error_message.
 ...
 * code is not stored.
 * This event would only occur if the per thread buffers could not be allocated,
 * which is highly unlikely.  In this
 * case an application will receive a generic message/errorCode when querying
 * for the last stored values.
 */
