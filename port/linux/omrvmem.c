 ...
 * being available on all systems.
 * We define our own helper to make the syscall so that we can use it without
 ...
 * that being available on all systems.
 * We define our own helper to make the syscall so that we can use it without
 * relying on external libraries.
 *
 ...
 * indices of
 * all NUMA nodes on this system. Store them into a bitmask in the port platform
 * globals.
 */
static intptr_t initializeNumaGlobals(struct OMRPortLibrary *portLibrary) {
  ...
  if (0 == mempolicyReturnCode) {
     ...
     * are specified in either the system default or process default
     * policies then the mask will be all 0's which (if used later) would
     * indicate no nodes are available for virtual memory allocation. */
  ...
  /* only proceed if we could successfully look up the memory policy and default
   * affinity */
...
static void *reserveLargePages(struct OMRPortLibrary *portLibrary,
   ...
   * They have to be attached to become allocated for use
...
static uintptr_t get_hugepages_info(struct OMRPortLibrary *portLibrary,
   ...
   * mystery. Meanwhile
   *  we have to parse /proc/meminfo to figure out how many pages are
   * available/free as well as
   *  their size
   */
  line_ptr = read_buf;
...
void port_numa_interleave_memory(struct OMRPortLibrary *portLibrary,
  ...
  if (1 == PPG_numa_platform_supports_numa) {
     ...
     * treat the maxnode argument.
     * Depending on the system we're running on, it may fail if we specify a
     * mask that is greater than the number of nodes.
     *
     * To support both configs, we first try to pass in the count of all bits in
     * the mask, regardless of
     * how many we're using (the unused ones are zero and will simply be
     * ignored). If that fails, we try
...
/* There are cases where mbind() can fail that we are currently not handling.
 * Instead of
 * asserting, and thereby bringing down the VM in such cases, simply record the
 * above
 * tracepoint and continue execution, since this isn't a fatal error.
 *
 * In one instance on RHEL6, it was observed that the /sys/devices/system/node/
 * filesystem
 * had entries for node0 and node4, but setting node0 in the mask would cause
 * the mbind()
 * call to fail. On this sytem, there was a file
 * /sys/devices/system/node/has_normal_memory
 * which only listed node 4. It might make sense to parse that file, if it
 * exists, and to
 * only set the bits for nodes that are listed in that file. Alternatively we
 * could test
 * each node using mbind() with only the node's bit set, to determine if we
 * should set
...
intptr_t omrvmem_find_valid_page_size(struct OMRPortLibrary *portLibrary,
  ...
  if (0 != validPageSize) {
#if defined(LINUXPPC)
/* On Linux PPC, for executable pages, search through the list of supported page
 * sizes only if
    ...
    if ((numaNode > 0) && (numaNode <= PPG_numa_max_node_bits) &&
      ...
      if (bit == (PPG_numa_available_node_mask.mask[wordIndex] & bit)) {
        ...
        if (0 != do_mbind(address, byteAmount, MPOL_PREFERRED, nodeMask.mask,
           ...
           * running just like in port_numa_interleave_memory.
           * This assert is useful to determine any problems calling mbind and
