...
uintptr_t renameDump(struct OMRPortLibrary *portLibrary, char *filename,
  ...
  if (0 != waitCoreRC) {
    ...
    portLibrary->str_printf(portLibrary, filename, EsMaxPath,
                            "The core file created by child process with pid = "
                            "%i was not found. Expected to find core file with "
                            "name \"%s\"",
...
/* at this point we know the file in derivedCorePath exists */
...
#ifdef DUMP_DBG
  {
     ...
     * this system. If it is,
     * then the child process will have set it to force the automatic inclusion
     * of the shared classes
     * cache. In that case we need do nothing. If the file is not present then
     * go forth and copy the
   ...
   * complete core
   * file image. The insertion process should only add memory segments that have
   * a program
  ...
  if (filename[0] != '\0') {
    ...
    if ((0 != renameRC) &&
      ...
      portLibrary->tty_printf(portLibrary, "Warning: unable to move dump to "
                                           "\"%s\" across file systems (check "
                                           "kernel core_pattern). Using "
 ...
 * pages, replace the tokens and store the
 ...
 * If we find a %t token, and %p is also specified, we use a wildcard to
 * 		  obtain a filename match. The child process PID provides
 * reasonable confidence that we will find the correct dump. Otherwise return
 * failure.
 *  Note: From the man pages: "A single % at the end of the template is dropped
 * from the core filename, as is the combination of a % followed by any
  ...
  *outCursor = '\0';
  ...
  if (corePatternSpecifiesPercentT && !corePatternSpecifiesPercentP) {
    ...
    portLibrary->str_printf(
        ...
        "\"%t\" specifier is not supported without \"%p\".");
    return -1;
  }
...
static intptr_t waitCore(char *path) {
  ...
  starttime = time(NULL);
  char *wildcardPos =
      strchr(path, '*'); /* position of '*' wildcard, if any, in the expected
                            core dump filename */

  if (NULL == wildcardPos) {
    /* Normal case, no wildcard in the dump filename. Wait for the expected dump
     * file to appear, using
     ...
     * file to appear, listing
     * files in the core dump directory to look for a file name that matches the
     * expected core dump name.
     */
    char *pathSeparator =
 ...
 * process.
 * Modify each page to be writable. This will cause it to be included in core
 * dumps.
 *
 ...
 * unlocked
 * by this one. Hence the use of native file IO in markAllPagesWritable to avoid
 * the
