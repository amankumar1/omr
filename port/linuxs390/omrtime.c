...
int64_t omrtime_current_time_millis(struct OMRPortLibrary *portLibrary) {
   ...
   * unfortunately
   * that particular clock does not necessarily match the user adjusted OS
   * clock.
   * This leads to a discrepency between the time the user has set and wants to
   * see
   * and what we provide by retrieving the hw clock that the user is not allowed
   * to
