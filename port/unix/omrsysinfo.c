...
void omrsysinfo_set_number_entitled_CPUs(struct OMRPortLibrary *portLibrary,
                                         uintptr_t number) {
...
#if defined(J9OS_I5)
      len = strlen(sysinfo.version) + strlen(sysinfo.release) +
            5; /* 'V', 'R', 'M', '0', & null */
#else
      len = strlen(sysinfo.version) + strlen(sysinfo.release) +
            2; /* "." and terminating null character */
#endif /* J9OS_I5 */
      buffer = portLibrary->mem_allocate_memory(
          portLibrary, len, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
      if (NULL == buffer) {
...
#if defined(J9OS_I5)
      portLibrary->str_printf(portLibrary, buffer, len, "V%sR%sM0",
                              sysinfo.version, sysinfo.release);
#else
      portLibrary->str_printf(portLibrary, buffer, len, "%s.%s",
                              sysinfo.version, sysinfo.release);
#endif /* J9OS_I5 */
#else
...
static intptr_t find_executable_name(struct OMRPortLibrary *portLibrary,
  ...
  if (NULL != *result) {
     ...
     * /proc/self/exe is read after a
     * call to fstat on a NFSv4 share, then the string ' (deleted)' will be
     * appended to
     * the file name.
     *
  ...
  *result = portLibrary->mem_allocate_memory(
  ...
  do {
    ...
    if (-1 == readBytes) {
       ...
       * short)!  No error number
       * is expected to be set here, so use OMRPORT_ERROR_FILE_IO to indicate
       * I/O error.
       */
 ...
 * symbolic links (or where we can't tell) or if an unexpected error occurs,
 * just answer FALSE.
 */
static BOOLEAN isSymbolicLink(struct OMRPortLibrary *portLibrary,
                              char *filename) {
 ...
 * are the relative pathname of
 * the thing linked to).  A buffer large enough to hold the result (and the
 * terminating NUL) is
 ...
 * which matched the file.  A buffer large enough to hold the proper path entry
 * (without a
 * trailing slash, but with the terminating NUL) is allocated with
 * portLibrary->mem_allocate_memory.
 * The caller should free this buffer with portLibrary->mem_free_memory when it
 * is no longer
...
uintptr_t omrsysinfo_get_number_CPUs_by_type(struct OMRPortLibrary *portLibrary,
  ...
  case OMRPORT_CPU_PHYSICAL:
    ...
    toReturn =
        omrsysinfo_get_number_CPUs_by_type(portLibrary, OMRPORT_CPU_ONLINE);
  ...
  case OMRPORT_CPU_BOUND: {
     ...
     * thread
     * placement--explicit thread to processor binding is not supported. Thus,
     * this
  ...
  case OMRPORT_CPU_TARGET: {
    ...
    uintptr_t bound =
        omrsysinfo_get_number_CPUs_by_type(portLibrary, OMRPORT_CPU_BOUND);
...
static int32_t retrieveLinuxMemoryStats(struct OMRPortLibrary *portLibrary,
  ...
  if (NULL == memStatFs) {
    ...
    Trc_PRT_retrieveLinuxMemoryStats_Exit(
  ...
  /* Reset return code to 0, since we are successful in retrieving memory usage.
   * Failure
   * conditions would have jumped over to _cleanup section and shall return an
   * appropriately
   * set error code.
   */
  rc = 0;
...
int32_t omrsysinfo_startup(struct OMRPortLibrary *portLibrary) {
   ...
   * NULL, but
   * shouldn't cause failure to startup port library.  Failure will be noticed
   * only
...
#define USERID_BUF_LEN 16
   ...
   * becomes dead code path
   * if we ever decide to retire atoe library. This macro is defined in atoe.h,
   * which is only included by macro
   * J9ZOS390. Hence, there are no danger of undefined variable.
   * j9 atoe library used malloc. Need to free any structure obtained from
...
intptr_t omrsysinfo_get_CPU_utilization(struct OMRPortLibrary *portLibrary,
  ...
  Trc_PRT_sysinfo_get_CPU_utilization_perfstat(stats.user, stats.sys,
                                               stats.ncpus);
  cpuTime->numberOfCpus =
      stats.ncpus; /* get the actual number of CPUs against which the time is
                      reported */
...
/**
 * Initialises the supplied buffer such that it can be used by the @ref
 ...
 * the name=value entries of the environment,
 * 	copyEnvToBuffer will store as many entries as possible, until the buffer
 * is used up.
 *
...
static int32_t retrieveLinuxProcessorStats(struct OMRPortLibrary *portLibrary,
   ...
   * this according
   * to its requirements. As such, this cannot be allocated from port-library
   * heap allocators.
   */
  ...
  while ((-1 != getline(&linePtr, &lineSz, procStatFs)) &&
    ...
    if (0 == cpuCntr) {
       ...
       * identified by integers not
       * in range 1 ... (N-1), [where N is the number of CPUs], i.e., exceed
     ...
     * /proc/stat file or did the
     * platform change the file format (unlikely, but even so, must check
     * against inconsistencies in the
...
static int32_t retrieveAIXProcessorStats(struct OMRPortLibrary *portLibrary,
   ...
   * but counts-in
   * stale CPU records too. At present, we can't tell exactly which CPUs are
   * online at this
   * very moment. Note: AIX perfstat documentations incorrectly states this as
   * highest_CPUID,
   * but it actually is highest_CPUID + 1. This is likely to corrected in
   * subsequent release
   * notes.
   */
...
int32_t omrsysinfo_get_processor_info(struct OMRPortLibrary *portLibrary,
   ...
   * actually try reading processor
   * usage statistics) until the time we hit the situation where the allocated
  ...
  do {
     ...
     * But first check whether we are
     * here to re-allocate, since we fell short with the allocated space or this
     * is the first allocation.
     */
