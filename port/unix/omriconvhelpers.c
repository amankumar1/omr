 ...
 * freed in iconv_free()
 * Populate the converter cache within portLibGlobal struct and their respective
 * mutexes.
 *
...
int32_t iconv_global_init(struct OMRPortLibrary *portLibrary) {
   ...
   * string
   * by defining J9_USE_ORIG_EBCDIC_LANGINFO which is checked in
 ...
 *fromCode character
 * encoding to the toCode character encoding.  The iconv descriptor may be
 *stored in a per-thread
 * cache.  Caching is determined by available memory and the value of the
...
iconv_t iconv_get(struct OMRPortLibrary *portLibrary, J9IconvName converterName,
   ...
   * per-thread buffer could not be allocated.
   * We attempt to allocate an uncached iconv descriptor in those cases, and use
   * ptBuffer == NULL as a check for caching.
   */
  ...
  if (PPG_global_converter_enabled) {
     ...
     * hack for CICS
     * the logic is based on the toCode and fromCode. If it correspond to one of
     * the four converters
...
void iconv_free(struct OMRPortLibrary *portLibrary, J9IconvName converterName,
  ...
  if (PPG_global_converter_enabled) {
    ...
    /* this part is only pertinent if -Xipt argument is not specified. It is a
     * hack for CICS
     * the logic is based on comparing the converter to be freed to the existing
     * converter,
     * if a match is found within the cached converters, only the corresponding
     * mutex is exited
     */
