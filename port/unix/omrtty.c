...
intptr_t omrtty_available(struct OMRPortLibrary *portLibrary) {
  ...
  curr = lseek(STDIN_FILENO, 0L, SEEK_CUR); /* don't use tell(), it doesn't
                                               exist on all platforms, i.e.
                                               linux */
  if (curr != -1) {
  ...
  /*[PR 102639] 64 bit platforms use a 32 bit value, using intptr_t fails on big
   * endian */
  /* Pass in intptr_t because ioctl() is device dependent, some devices may
   * write 64 bits */
  if (rc != -1) {
