 ...
 * must be not be collapsable and
 ...
 * unreliable so we add a
 * timeout to poll calls whether or not we have a deadline so we have a sample
 * based scheduler visible
 * spinlock.
 */
#if defined(LINUXPPC)
 ...
 * to user
 * space so we need to spin reasonably quickly or we'll not resume until the
 * timeout in
 * error cases.
 */
 ...
 * It's a combination o
 * spinlock techniques modified with blocking read/write/poll scheduler visible
 * calls to avoid
 * thread contention.
 *
...
static int barrier_init_r(barrier_r *barrier, int value) {
  ...
  do {
    old_value = barrier->initial_value;
                               ...
                               value, &barrier->spinlock) != old_value);
  do {
 ...
 * the barrier via barrier_enter_r. If a maximum amount of time to block for is
 * specified then this will
...
static int barrier_release_r(barrier_r *barrier, uintptr_t seconds) {
  ...
  if ((compareAndSwapUDATA((uintptr_t *)&barrier->released, 0, 1,
                           &barrier->spinlock)) != 0) {
    /* barrier->released should have been 0, write something into the pipe so
     * that poll doesn't block,
  ...
  while ((compareAndSwapUDATA((uintptr_t *)&barrier->in_count, -1, -1,
                              &barrier->spinlock)) > 0) {
 ...
 * expected entrants have called this
...
static int barrier_enter_r(barrier_r *barrier, uintptr_t deadline) {
  ...
  do {
    old_value = barrier->in_count;
                               ...
                               old_value - 1, &barrier->spinlock) != old_value);

  if (old_value == 1 && (compareAndSwapUDATA((uintptr_t *)&barrier->released, 0,
                                             0, &barrier->spinlock))) {
 ...
 * for. If all clients have already entered the barrier
 ...
 * any waiters then waits for them
 ...
 * combination of
 * spinlock techniques to acquire the lock and blocking read/write/poll calls to
 * prevent massive
  ...
  while (1) {
    ...
    while (old_value > 0) {
      if (compareAndSwapUDATA((uintptr_t *)&sem->sem_value, old_value,
                              old_value - 1, &sem->spinlock) == old_value) {
...
static int sem_trywait_r(sem_t_r *sem) {
  ...
  /* try to get the lock */
  old_value =
      compareAndSwapUDATA((uintptr_t *)&sem->sem_value, -1, -1, &sem->spinlock);
  while (old_value > 0) {
    int value = compareAndSwapUDATA((uintptr_t *)&sem->sem_value, old_value,
                                    old_value - 1, &sem->spinlock);
    if (value == old_value) {
      /* successfully acquired lock */
      return 0;
    }
 ...
 * any. Otherwise it increments the
 ...
 * the semaphores capacity to 0 so that
 * no more clients can acquire the lock. If something holds the lock then we
 * undo the block and return an error,
...
static int sem_destroy_r(sem_t_r *sem) {
  ...
  do {
    old_value = sem->sem_value;
                               ...
                               old_value - sem->initial_value,
                               &sem->spinlock) != old_value);
 ...
 * first check that
 * the signal we're handling is one that's expected to give a degree of
 * confidence that the
 * value we're passed is a legitimate platform data pointer. After that the
 * procedure is simple:
 * 1. wait until posted
 ...
 * and an error is noted
 * then we skip stack production and wait for release. There's no timeout on the
 * barrier enter as
 ...
 * is necessary so we can queue the
 * correct number of signals to suspend all the threads (we might be able to
 * simply call sigqueue until we get a
 * queue full error, but then we can't queue additional signals to prompt thread
 * scheduling. Also it may not be
 * capped on all systems).
 * Linux kernels >= 2.6 we use a count of the entries in /proc/self/task
 * (omitting . and ..)
 * Linux kernels == 2.4 we check the status file for /proc/.* entries and count
 * those where the Tgid matches our pid
...
static int count_threads(struct PlatformWalkData *data) {
  ...
  if (tids == NULL) {
     ...
     * tell if they belong to
     * us by checking the thread group id from the 3rd line of
     * /proc/.<pid>/status
     */
    while ((file = readdir(proc)) != NULL) {
      ...
      if (file->d_type == DT_DIR && file->d_name[0] == '.' &&
        ...
        /* "/proc/.<pid>/status" 13 for /proc/ and /status, 11 for the .pid, 1
         * for the null */
...
static int count_threads(struct PlatformWalkData *data) {
  ...
  while (getthrds64(getpid(), &thread, sizeof(struct thrdentry64), &cursor,
     ...
     * for reuse.
     * Experimentation shows that threads can receive signals before they have a
     * stack or complete construction
 ...
 * threads in the process bar the calling
 * thread are suspended. The signals have to be real-time signals (ie. >=
 * SIGRTMIN) or it's not possible to pass
 * a data via sigqueue. Also, non real-time signals of the same value can be
 * collapsed and we require any threads
 * scheduled during this suspension to immediately receive a signal, so we need
 * at least one signal per unsuspended
 * thread on the queue.
 *
...
static int suspend_all_preemptive(struct PlatformWalkData *data) {
  ...
  if (data->oldHandler.sa_sigaction == upcall_handler) {
     ...
     * while cleaning as the thread that installed the initial handler will do
     * so
     */
   ...
   * suspended threads matches the number of
  ...
  do {
    ...
    for (i = data->threadCount; i < thread_count - 1; i++) {
       ...
       * queue. 10 is an arbitrary
       * number to see if we can avoid the added complexity of coordination
       * between the signal handler and
       * this suspend logic. Testing shows 48 threads suspended on AIX5.3 at the
 ...
 * general behaviour
 * is to swallow any spurious signals from the queue so they are not delivered
 * after we remove our
 * custom handler, the custom handler is removed, then we wake any threads
 * blocking on the relase
 * barrier. On AIX we set the signal handler to SIG_IGN before attempting to
 * swallow signals off
 * the queue because it seems to be possible for signals to be allocated to
 * threads without yet
 * being delivered, preventing that from working. Setting SIG_IGN seems to
 * remove not yet delivered
 * signals.
 *
 * @param data - platform specific control data used during suspend/resume
 */
...
/* now there are no outstanding signals on the queue we can drop the handler */
  ...
  if (data->cleanupRequired) {
     ...
     * but not yet have been delivered
     * to a thread. As a result we can't be sure that SIG_SUSPEND won't be
     * delivered after we uninstall the handler
  ...
  if (data->cleanupRequired) {
    ...
    while (sigpending(&set) == 0 && sigismember(&set, SUSPEND_SIG)) {
       ...
       * that sigwait isn't in the initial
       * set of posix signal safe functions, however the fact that it bypasses
       * the installed signal handler is
       * too useful it ignore, so we risk it. It could be replaced with
       * sigsuspend and some handler juggling if
       * need be but the added complexity isn't worth it until it's shown to be
       * necessary
       */
      sigemptyset(&set);
      sigaddset(&set, SUSPEND_SIG);
       ...
       * made the sigpending() call above,
       * so use a non-blocking sigtimedwait() call to clear it if it's still
       * there, don't actually wait
       */
      time_out.tv_sec = 0;
      time_out.tv_nsec = 0;
    ...
    if ((data->oldHandler.sa_flags & SA_SIGINFO) == 0 &&
       ...
       * shouldn't be any suspend signals
 ...
 * specified it is used instead of grabbing
...
static int setup_native_thread(J9ThreadWalkState *state,
  ...
  if (heapAllocate || sigContext) {
    ...
    if (sigContext) {
      ...
      memcpy(state->current_thread->context,
             ((J9UnixSignalInfo *)sigContext)->platformSignalInfo.context,
             size);
  ...
  if (state->current_thread->callstack == NULL) {
     ...
     * already. It confuses heap/not heap allocations if we
     * pass it here.
     */
    ...
    if (state->current_thread->callstack == NULL &&
       ...
       * differentiate constant strings and alloc'd strings
...
static uintptr_t sigqueue_is_reliable(void) {
  ...
  /* sigqueue() is sufficiently reliable on newer Linux kernels (version 3.* and
   * later). */
  return release >= 3;
 ...
 *all native threads
 * or a subset depending on platform (see platform specific documentation for
 *detail).
 * This function will suspend all of the native threads that will be presented
 *through the iterator
 * and they will remain suspended until this function or nextDo return NULL. The
 *context for the
 ...
 *			are:
 * 			deadline - the system time in seconds at which to abort
 *introspection and resume
 ...
 *live context for the
 * 			calling thread.
 *
 * @return NULL if there is a problem suspending threads, gathering thread
 *contexts or if the heap
 * is too small to contain even the context without stack trace. Errors are
 *reported in the error,
 * and error_detail fields of the state structure. A brief textual description
 *is in error_string.
 */
...
#ifdef AIXPPC
  /* On AIX, initialise semaphore pipes to be sync-on-write (O_DSYNC flag).
   * Previously we used the fdatasync()
   * call after writing to the pipe, but on AIX it is not legal to call
   * fdatasync() inside a signal handler.
   */
 ...
 * iteration. The only way to
 * resume suspended threads is to continue calling this function until it
 * returns NULL.
 *
 * @param state state structure initialised by a call to one of the startDo
 * functions.
 *
...
J9PlatformThread *omrintrospect_threads_nextDo(J9ThreadWalkState *state) {
   ...
   * siglongjmp if there was a segfault or
