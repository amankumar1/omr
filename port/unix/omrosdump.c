...
exist on AIX 5.3. The code that uses these definitions does a runtime lookup to
see if the
...
uintptr_t omrdump_create(struct OMRPortLibrary *portLibrary, char *filename,
  ...
  if ((sys_parmRC == 0) && (myvar.v.v_fullcore.value == 1)) {
    ...
    portLibrary->tty_err_printf(
        portLibrary, "Note: \"Enable full CORE dump\" in smit is set to FALSE "
                     "and as a result there will be limited threading "
...
/*
 * CMVC 95748: don't use abort() after fork() on Linux as this seems to upset
 * certain levels of glibc
 */
...
int32_t omrdump_startup(struct OMRPortLibrary *portLibrary) {
  ...
  if (0 == portLibrary->sl_open_shared_library(portLibrary, NULL, &handle, 0)) {
    ...
    if (0 ==
        portLibrary->sl_lookup_name(portLibrary, handle, "proc_setattr",
                                    (uintptr_t *)&setattr, "IPLj")) {
      procattr_t attr;
      int rc;
      ...
      if (rc != 0) {
        if (ENOSYS == errno) {
           ...
           * 6100-04 to 6100-05 without a subsequent reboot,
           * in which case, behave the same as if we're running on an AIX
           * version which does not support proc_setattr() */
        } else {
          int32_t error =
