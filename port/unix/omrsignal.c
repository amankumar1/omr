...
int32_t omrsig_can_protect(struct OMRPortLibrary *portLibrary, uint32_t flags) {
  ...
  if (J9_ARE_NO_BITS_SET(signalOptionsGlobal,
  ...
  if (J9_ARE_NO_BITS_SET(signalOptionsGlobal,
    ...
    supportedFlags |= OMRPORT_SIG_FLAG_SIGQUIT | OMRPORT_SIG_FLAG_SIGABRT |
                      OMRPORT_SIG_FLAG_SIGTERM;
  }
...
int32_t omrsig_protect(struct OMRPortLibrary *portLibrary,
  ...
  if (J9_ARE_ANY_BITS_SET(signalOptionsGlobal,
                          OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS)) {
   ...
   * signal(s).
   * We will need to aquire the masterHandlerMonitor if
  ...
  if (0 != flagsWithoutMasterHandlers) {
    ...
    /* we have not installed handlers for all the signals, aquire the
     * masterHandlerMonitor, check again and install handlers as required all
  ...
  if (J9_ARE_ANY_BITS_SET(flags, OMRPORT_SIG_FLAG_MAY_RETURN)) {
     ...
     * we jump back into this function
     * because any signals that may have occurred within the scope of this layer
     * of protection would have been handled
     * by that point.
     *
     * The only scenario where this is of real concern, is if more than one
     * signal was handled per call to omrsig_protect. In
     * this case, the current signal in tls will be pointing at a stale stack
     * frame and signal: CMVC 126838
     */
...
uint32_t omrsig_set_async_signal_handler(struct OMRPortLibrary *portLibrary,
  ...
  if (NULL == cursor) {
    ...
    if (0 != flags) {
      J9UnixAsyncHandlerRecord *record = portLibrary->mem_allocate_memory(
...
#endif
{
   ...
   * transaction.
   * If there is a signal during a transaction, two contexts are provided: one
   * for the start of the transaction
   ...
   * the context from the start of the transaction.
   * The masterSynchSignalHandler determines if the signal occurred within a
   * transaction,
   * and if so, returns such that the transaction is executed in the failed
   * state,
   * causing the non-transactional failure path to be taken (referred to as the
   * "abort handler" in
   ...
   * failure path, are lost.
   * Please see Jazz design 63039: Handle signals in PowerPC transactional
   * memory operations for further discussion.
   */

  /*
   * This contains two contexts: the context at the start of the transaction,
   * plus a link to the
   * context within the transaction.  We use the former, other than to determine
...
#define MSR_TS_MASK 0x6U
  ...
  if ((NULL != platformContext->uc_link) &&
      J9_ARE_ANY_BITS_SET(platformContext->uc_link->uc_mcontext.regs->msr,
                          MSR_TS_MASK))
#endif
  {
    /*
  ...
  if ((OMRPORT_SIG_EXCEPTION_COOPERATIVE_SHUTDOWN == result) ||
      (J9_ARE_ALL_BITS_SET(signalOptionsGlobal,
                           OMRPORT_SIG_OPTIONS_COOPERATIVE_SHUTDOWN))) {

    /* Determine if returning from the signal handler will result in Language
     * Environment (LE) initiating Resource Recovery Services (RRS)
     *  and terminate the enclave, or if we need to explicitly trigger an abend
     * to force RRS and termination.
     *
    ...
    if (TRUE == triggerAbend) {
      sigrelse(
          SIGABND); /* CMVC 191934: need to unblock sigabnd before issuing the
                       abend call */
#if defined(OMR_ENV_DATA64)
 ...
 * library or (2) the thread hadn't registered
...
/* if we got this far there weren't any handlers on the stack that knew what to
 * with this signal
 ...
 * time to act.
 * Each expected aynch signal type has an associated semaphore which is used to
 * count the number of "pending" signals.
 *
 */
#if defined(S390) && defined(LINUX)
static void masterASynchSignalHandler(int signal, siginfo_t *sigInfo,
 ...
 * master signal handlers
 * Not to be confused with omrsig_protect, which registers the user's handler
 * with the port library.
 *
...
static uint32_t registerSignalHandlerWithOS(OMRPortLibrary *portLibrary,
   ...
   * handler.
   * However, it can still be masked if one explicitly requests so in sa_mask
   * field, as we do on z/OS
   */
  newAction.sa_flags |= SA_NODEFER;

#if defined(J9ZOS390)
  /* z/OS doesn't have posix semaphores.
   * A side effect of that is that we don't want to re-enter the
   * masterASyncHandler
   * Therefore, mask all async signals for the masterASyncHandler. The signal(s)
   * will be queued and delivered
   * to the masterASyncHandler once the handler returns. No signals will be
   * lost.
   */
 ...
 * "uintptr_t breakingEventAddr"
 * has been introduced in masterSynchSignalHandler() to obtain BEA on zLinux but
 * not for other platforms.
 * As a result, the total number of parameters in masterSynchSignalHandler() on
 * zLinux
 * is 4 while the number is 3 on other platforms, coz there is no need to change
 * the signature of
...
/* mask out subtypes */
  ...
  portLibSignal &= OMRPORT_SIG_FLAG_SIGALLSYNC | OMRPORT_SIG_FLAG_SIGQUIT |
                   ...
                   OMRPORT_SIG_FLAG_SIGXFSZ | OMRPORT_SIG_FLAG_SIGRECONFIG;
#else
  portLibSignal &= OMRPORT_SIG_FLAG_SIGALLSYNC | OMRPORT_SIG_FLAG_SIGQUIT |
                   OMRPORT_SIG_FLAG_SIGABRT | OMRPORT_SIG_FLAG_SIGTERM |
...
static intptr_t addAsyncSignalsToSet(sigset_t *ss) {
  ...
  for (i = 0; i < sizeof(signalMap) / sizeof(signalMap[0]); i++) {
    ...
    if (J9_ARE_ANY_BITS_SET(signalMap[i].portLibSignalNo,
                            OMRPORT_SIG_FLAG_SIGALLASYNC)) {
...
static int32_t registerMasterHandlers(OMRPortLibrary *portLibrary,
  ...
  if (0 != flagsWithoutHandlers) {
     ...
     * synchronous signals (eg. "4") so we are not at risk of registering a
     * handler
...
static int32_t initializeSignalTools(OMRPortLibrary *portLibrary) {
  ...
  portLibrary->str_printf(portLibrary, semNames[1], 128,
                          "/omr/sigQuitPending-%d", getpid());
  portLibrary->str_printf(portLibrary, semNames[2], 128,
                          "/omr/sigAbrtPending-%d", getpid());
  portLibrary->str_printf(portLibrary, semNames[3], 128,
                          "/omr/sigTermPending-%d", getpid());
  portLibrary->str_printf(portLibrary, semNames[4], 128,
                          "/omr/sigReconfigPending-%d", getpid());
  portLibrary->str_printf(portLibrary, semNames[5], 128,
                          "/omr/sigXfszPending-%d", getpid());
...
int32_t omrsig_set_options(struct OMRPortLibrary *portLibrary,
  ...
  if (0 != (options & OMRPORT_SIG_OPTIONS_ZOS_USE_CEEHDLR)) {
     ...
     * Switch over to LE condition handling, unless we have already installed
     *POSIX handlers for any sync signals
     **/
    ...
    if (0 == (signalsWithMasterHandlers & OMRPORT_SIG_FLAG_SIGALLSYNC)) {
      /* we haven't installed any synchronous handlers, so it's OK to switch to
