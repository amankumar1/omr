...
const j9csrsi_t *j9csrsi_init(int32_t *ret) {
  ...
  if (NULL != sess) {
     ...
     */
    sess->request = (CSRSI_REQUEST_V1CPC_MACHINE | CSRSI_REQUEST_V2CPC_LPAR |
...
j9csrsi_is_lpar(const j9csrsi_t *session) {
  ...
  if (NULL != session) {
    const siv1v2v3 *const ia = (const siv1v2v3 *)&(session->infoarea);
    const si00 *const hdr = (const si00 *)&(ia->siv1v2v3si00);

    ret = (BOOLEAN)(SI00CPCVARIETY_V2CPC_LPAR == hdr->si00cpcvariety) &&
          hdr->si00validsi22v2;
  }
...
j9csrsi_is_vmh(const j9csrsi_t *session) {
  ...
  if (NULL != session) {
    ...
    ret = (BOOLEAN)(SI00CPCVARIETY_V3CPC_VM == hdr->si00cpcvariety) &&
          hdr->si00validsi22v3;
  }
...
int32_t j9csrsi_get_vmhpidentifier(const j9csrsi_t *session, uint32_t position,
  ...
  if ((NULL != session) && (NULL != buf) && (0U < len) &&
      j9csrsi_is_vmh(session)) {
    const siv1v2v3 *const ia = (const siv1v2v3 *)&(session->infoarea);
    const si22v3 *const info = (const si22v3 *)&(ia->siv1v2v3si22v3);

    if (info->si22v3dbcountfield._si22v3dbcount > position) {
      const si22v3db *const db = &(info->si22v3dbe[position]);
      const char *const str = &(db->si22v3dbvmhpidentifier[0U]);
      const uint32_t str_sz = sizeof(db->si22v3dbvmhpidentifier);
...
int32_t j9csrsi_get_cpctype(const j9csrsi_t *session, char *buf, uint32_t len) {
  ...
  if ((NULL != session) && (NULL != buf) && (0U < len)) {
    ...
    if (hdr->si00validsi11v1 &&
      ...
      const si11v1 *const info = (const si11v1 *)&(ia->siv1v2v3si11v1);
