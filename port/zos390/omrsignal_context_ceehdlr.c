 ...
 */
void fillInLEJumpInfo(struct OMRPortLibrary *portLibrary, j9_31bit_mch *regs,
                      void *jumpInfo) {
  int i = 0;
...
uint32_t infoForFPR_ceehdlr(struct OMRPortLibrary *portLibrary,
                            J9LEConditionInfo *info, int32_t index,
 ...
 * port/zos390/omrsignal_context.c
 * Some case statements for an immediate value are preceded by a fall-through
 * case statement on a constant. This is done so that
 * it can be used both as an iterator (see documentation for omrsig_info() in
 * port/common/omrsignal.c and so that values
 ...
 * thread info dangles
 * 	off the caa), this routine must run on the thread that triggered the
 * condition corresponding to @ref info
 *
 */
uint32_t infoForModule_ceehdlr(struct OMRPortLibrary *portLibrary,
  ...
  *name = "";
   ...
   * condition,
   *  but the DSA (stack pointer) is stored in cib_sv1 for both the XPLINK and
   * OSLINK formats */
