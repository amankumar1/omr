...
int32_t omrvmem_free_memory(struct OMRPortLibrary *portLibrary, void *address,
  ...
  printf("omrvmem_free_memory freeing %p, pageSize 0x%x pageFlags %u using "
         "allocate %zu\n",
         address, identifier->pageSize, identifier->pageFlags,
  ...
  if (OMRPORT_VMEM_RESERVE_USED_J9MEM_ALLOCATE_MEMORY ==
    ...
    omrmem_categories_decrement_counters(identifier->category,
                                         identifier->size);
  } else if ((OMRPORT_VMEM_RESERVE_USED_J9ALLOCATE_LARGE_PAGES_BELOW_BAR ==
              identifier->allocator) ||
             (OMRPORT_VMEM_RESERVE_USED_J9ALLOCATE_4K_PAGES_BELOW_BAR ==
              identifier->allocator)) {
    Trc_PRT_vmem_omrvmem_free_memory_using_free_memory_below_bar(address,
                                                                 byteAmount);
    rc = omrfree_memory_below_bar(address, identifier->size,
                                  OMRPORT_VMEM_SUBPOOL);
    omrmem_categories_decrement_counters(identifier->category,
                                         identifier->size);
#if defined(OMR_ENV_DATA64)
  } else if (
      (OMRPORT_VMEM_RESERVE_USED_J9ALLOCATE_4K_PAGES_IN_2TO32G_AREA ==
       identifier->allocator) ||
    ...
    Trc_PRT_vmem_omrvmem_free_memory_using_free_memory_above_bar(
        address, byteAmount, identifier->allocator);
    rc = omrfree_memory_above_bar(address, ttkn);
    omrmem_categories_decrement_counters(identifier->category,
                                         identifier->size);
  } else if (OMRPORT_VMEM_RESERVE_USED_J9ALLOCATE_4K_PAGES_ABOVE_BAR ==
             identifier->allocator) {
    Trc_PRT_vmem_omrvmem_free_memory_using_free_memory_above_bar(
        address, byteAmount, identifier->allocator);
...
default_pageSize_reserve_low_memory(struct OMRPortLibrary *portLibrary,
  ...
  if (J9_ARE_ANY_BITS_SET(OMRPORT_VMEM_MEMORY_MODE_VIRTUAL, mode)) {
     ...
     * extra uintptr_t to store the original
 ...
 *above 2GB bar.
 * For large page sizes (> 4KB), it attempts to use page sizes in following
 *order (provided OMRPORT_VMEM_STRICT_PAGE_SIZE
 * is not set) to satisfy the allocation request, starting with the requested
 *page size :
 * 	- 2GB fixed pages
 *	- 1MB fixed pages
 *	- 1MB pageable pages
 ...
 *modified to maintain above mentioned order
...
#define MAX_2TO64G_MEMORY_IN_MB                                                \
  ...
  if ((FOUR_K == pageSize) &&
    ...
    LP_DEBUG_PRINTF1("\t reservePagesAboveBar calling "
    ...
    LP_DEBUG_PRINTF2("\t "
                     "omrallocate_4K_pages_in_userExtendedPrivateArea(0x%zx) "
                     "returned 0x%zx\n",
                     numSegments, ptr);
    ...
    if (0 != PPG_vmem_pageSize[index]) {
      ...
      if ((NULL == ptr) && (index > 0) && (ONE_M == PPG_vmem_pageSize[index]) &&
        ...
        LP_DEBUG_PRINTF2("\t reservePagesAboveBar calling "
                         "omrallocate_1M_pageable_pages_above_bar(0x%zx, "
                         "0x%x)\n",
                         numSegments, userExtendedPrivateAreaMemoryType);
  ...
  if (J9_ARE_ANY_BITS_SET(OMRPORT_VMEM_MEMORY_MODE_VIRTUAL, mode)) {
...
#else  /* if 0 */
    {
      ...
      if (0 != rc) {
         ...
         * EINVAL or EMVSERR in case of failure.
         * EINVAL indicates invalid argument. More detailed information is
         * returned by __errno2().
     ...
     * uintptr_t to store the original
     * address, which can be used by free
     */
    allocSize = GET_4K_ALIGNED_ALLOCATION_SIZE(byteAmount);
...
void *omrvmem_reserve_memory_ex(struct OMRPortLibrary *portLibrary,
   ...
   * applicaable for 64-bit z/OS systems only.
   *  Routines using STORAGE allocate memory below 2GB bar. Such routines can be
   * used on both 31-bit and 64-bit z/OS system.s
   */
   ...
   * also, until native alignment is implemented, adjusts the parameters
   * to ensure that the requested alignment can be honoured within the allocated
   * block.  Note that we still want to create a copy of the structure
   * (because this function modifies the struct) if we implement native
   * alignment and remove this helper.
   */
  ...
  if (use2To32GArea &&
     ...
     * the request
     * as the caller needs to know with absolute certainty whether or not they
     * can rely on
     * the 2to32G support being there
     *
     * Having done the check here, we only need to check for the request for
     * OMRPORT_VMEM_ZOS_USE2TO32G_AREA later on
     * knowing that if they had asked for it and made it that far that the
     * support is there
     */

    LP_DEBUG_PRINTF("\n omrvmem_reserve_memory_ex: "
                    "OMRPORT_VMEM_ZOS_USE2TO32G_AREA requested but not "
                    "supported\n");
 ...
 *pages.
 * Before making any further calls, it performs some basic validation to ensure
 ...
 *OMRPORT_VMEM_ZOS_USE2TO32G_AREA is set, it tries to allocate
 * 	  memory above the bar, failing which it falls back to default page
 *size.
 ...
 *below the bar,
 *		  else, if OMRPORT_VMEM_STRICT_ADDRESS is set, fall back to
 *default page size,
...
static void *reservePages(struct OMRPortLibrary *portLibrary,
  ...
  if (FALSE == isLargePageSizeSupported(portLibrary, params->pageSize,
     ...
     * pages above the bar.
     * Else, if page size requested is 1MB pageable, allocate large pages below
     * the bar.
     * Else fail.
     */
    if (TRUE == use2To32GArea) {
      ...
      if ((NULL == baseAddress) && (FALSE == useStrictAddress)) {
         ...
         * return memory in low memory range.
 ...
 * On 64-bit system, default page size when mode is
 * OMRPORT_VMEM_MEMORY_MODE_EXECUTE is 1M pageable pages (provided it is
 * supported),
 ...
 * size supported by underlying system.
 * Note that it does not check 4K page size as it is not considered large page
 * size.
 *
 * @param [in] portLibrary the portLibrary
