...
#define csrsi_byaddr(Request, Flen, Fptr, Rcptr)                                                 \
  ?                                                                                              \
  ? < ((struct CSRSI_PSA *)0)->CSRSI_cvt->CSRSI_cvtcsrt->CSRSI_addr(Request, Flen, Fptr, Rcptr); \
  ? ? > ;
#endif
...
struct CSRSI_CVT ? ? < unsigned char CSRSI_cvt_filler1 ? ? (116 ? ?);
struct ? ? < int32_t CSRSI_cvtdcb_rsvd1 : 4; /* Not needed                   */
...
struct ? ? < int32_t CSRSI_cvtcsrsi : 1; /* If on, indicates that the
...
struct CSRSI_PSA ? ? < char CSRSI_psa_filler ? ? (16 ? ?);
...
typedef struct ?
               ? < uint32_t si22v1format
               : 8;                  /* A 1-byte value. When the
...
struct ? ? < unsigned char _si22v1mpcpucapaf ? ? (2 ? ?); /* Each individual
...
typedef struct
    ? ? < uint32_t si22v1altcpucapability; /* A 32-bit unsigned binary
                                       ...
                                       alternate capability.
                                                                      @L1A*/
struct ? ? < unsigned char _si22v1altmpcpucapaf ?
                                                ? (2 ? ?); /* Each individual
...
typedef struct ? ? < unsigned char _filler1 ? ? (28 ? ?); /* Reserved @H1A*/
unsigned char _filler2 ? ? (3 ? ?);    /* Reserved                  @H1A*/
struct ? ? < uint32_t _filler3 : 4;    /* Reserved
                                                          @H1A*/
...
typedef struct ? ? < char si00cpcvariety;  /* SI00CPCVariety_V1CPC_MACHINE,
                                              SI00CPCVariety_V2CPC_LPAR, or
                                              SI00CPCVariety_V3CPC_VM   @H1A*/
struct ? ? < int32_t _si00validsi11v1 : 1; /* si11v1 was requested and
...
typedef struct ? ? < si00 siv1si00; /* Area mapped by
                       struct si00               @H1A*/
...
typedef struct ? ? < si00 siv1v2si00; /* Area mapped by
                         by struct si00            @H1A*/
...
typedef struct ? ? < si00 siv1v2v3si00; /* Area
...
typedef struct ? ? < si00 siv1v3si00; /* Area mapped
                       by struct si00            @H1A*/
...
typedef struct ? ? < si00 siv2si00; /* Area mapped by
                                       struct si00               @H1A*/
...
typedef struct ? ? < si00 siv2v3si00; /* Area mapped
                                         by struct si00            @H1A*/
...
typedef struct ? ? < si00 siv3si00; /* Area mapped by
                                       struct si00               @H1A*/
