...
/*
 *
 * Much of the following j9vm_le_condition_handler parameter documentation is
 *from:
 *
 ...
 *here http://publibz.boulder.ibm.com/epubs/pdf/ceea91a0.pdf,
 * 	but the POSIX signal mappings are not included for hardware raised
 *signals.
 *
 ...
 * The messages for software raised POSIX signals are in Table 76, pg. 423 here
 *http://publibz.boulder.ibm.com/epubs/pdf/cbcpg1a0.pdf
 *
*/
void j9vm_le_condition_handler(_FEEDBACK *fc, _INT4 *token, _INT4 *leResult,
                               _FEEDBACK *newfc) {
  struct J9ZOSLEConditionHandlerRecord *thisRecord =
      (struct J9ZOSLEConditionHandlerRecord *)*token;
...
#pragma convlit(resume)
  ...
  if (1 == thisRecord->recursiveCheck) {
     ...
     * current signal.
     * Any handler that was already handling a condition will also percolate,
     * until we reach
   ...
   *
   * If at this point portlibSignalNo == OMRPORT_SIG_FLAG_DOES_NOT_MAP_TO_POSIX,
   * we have received a condition that cannot be mapped to a harware-raised
   * POSIX signal.
   * 	- in this case, only handlers that were registered against
   * OMRPORT_SIG_FLAG_SIGALLSYNC should get called,
   *
   * If instead the portlibSignalNo has been set, then simply call handlers that
   * had registered explicitly for that
   * 	signal type
   */
  if ((0 != (thisRecord->flags & portlibSignalNo)) ||
    ...
    handlerResult =
        thisRecord->handler(thisRecord->portLibrary, portlibSignalNo, &j9Info,
                            thisRecord->handler_arg);
    thisRecord->recursiveCheck = 0;

    /* control leaves j9vm_le_condition_handler in all cases following this
...
int32_t omrsig_protect_ceehdlr(struct OMRPortLibrary *portLibrary,
  ...
  if (0 ==
    ...
    /* j9vm_le_condition_handler will jump here in the case of
