...
#if defined(                                                                   \
    J9ZOS39064) /* Remove 'TDUMP' to alleviate the 44 chars constraint on      \
                   dataset names */
...
static intptr_t tdump(struct OMRPortLibrary *portLibrary, char *asciiLabel,
   ...
   * in j9generate_ieat_dumps.s. The
   * message below needs to be kept consistent with the options set on the macro
   * call.
   */
  omrtty_err_printf(portLibrary, "IEATDUMP in progress with options "
                                 "SDATA=(LPA,GRSQ,LSQA,NUC,PSA,RGN,SQA,SUM,SWA,"
                                 "TRT)\n");
...
static intptr_t ceedump(struct OMRPortLibrary *portLibrary, char *asciiLabel,
   ...
   *  	If the data set is not defined, or is not usable for any reason,
   * cdump() returns a failure code of 1.
   *  	This occurs even if the call to CEE3DMP is successful.
   *  	For more information see "Debugging C/C++ Routines" in "z/OS Language
