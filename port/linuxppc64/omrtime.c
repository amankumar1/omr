 ...
 * This structure has to be large enough to include the fields used by the
 * assembler routines
 * Creating the structure the same size as sizeof(vdso_data) when using
 * asm/vdso_datapage.h
 */
struct vdso_data {
 ...
 */
static BOOLEAN getLinuxKernelVersion(uint32_t *kernelVersion,
  ...
  if (0 == uname(&name)) {
    if (3 == sscanf(name.release, "%u.%u.%u", kernelVersion, majorRevision,
                    minorRevision)) {
      success = TRUE;
    }
  }
...
int32_t omrtime_startup(struct OMRPortLibrary *portLibrary) {
   ...
   * breaks our
   * implementation of __getNanos(). The issue is that the tb_to_xs field in the
   * vdso struct
   * can change at runtime, which causes __getNanos() to sometimes report
   * inaccurate time -
   * including time that can go backwards. SLES10 and RHEL5 do not have this
   * issue.
   *
   * We work around this by falling back to the gettimeofday() implementation,
   * which was
   * measured to be around 3x slower compared to __getNanos(), on newer Kernel
   * versions.
  ...
  if (NULL != systemcfgP_millis) {
    ...
    if (getLinuxKernelVersion(&kernelVersion, &majorRevision, &minorRevision)) {
      if ((2 == kernelVersion) && (6 == majorRevision) &&
          (18 >= minorRevision)) {
