 ...
 * suspend_all_preemtive.
 * In actuality it decrements the suspend count of every thread in the process
 * by 1 even if,
 * somehow, we've not incremented it in suspend_all_preemptive (externally
 * spawned threads by
 ...
 * and suspends all
 * of our threads barring the one executing the function. It then takes a new
 * snapshot to determine
 * if new threads were created during the suspension. This is repeated until the
...
/*
 * NOTE: According to MSDN this call to GetThreadContext() should not work for
 * the
 * current thread. It seems to work, however, if we populate the EIP and EBP
 * context
 ...
 */
    ...
    __asm {
      ...
      /* Define a routine called 'getCtx' */
			getCtx:
          /* Retrieve the previous instruction pointer from the stack and copy
             into register eax */
			pop eax;
