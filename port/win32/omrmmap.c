...
void omrmmap_dont_need(struct OMRPortLibrary *portLibrary,
  ...
  if (pageSize > 0 && length >= pageSize) {
    uintptr_t const *endAddress =
        (uintptr_t const *)((U_8 *)startAddress + length);
    uintptr_t const *roundedStart = (uintptr_t const *)ROUND_UP_TO_POWEROF2(
        (uintptr_t)startAddress, pageSize);
