 ...
 * longer than the Windows defined MAX_PATH,
 * the path is converted to an absolute path and prepended with \\?\ or \\?\UNC
 * if it is a networked path (eg. \\server\share\..)
 *
...
static wchar_t *file_get_unicode_path(OMRPortLibrary *portLibrary,
  ...
  if (pathLen * sizeof(wchar_t) <= MAX_PATH) {
    ...
    if (('\\' == utf8Path[0]) && ('\\' == utf8Path[1])) {
       ...
       * of the initial "\" in the file path
     ...
     * The MSDN docs for GetFullPathNameW state
     * that it should not be used in multi-threaded applications, but the docs
     * are fairly clear that this is because the conversion from a
     * relative path to a fully qualified path depends on the current working
     * directory (CWD), which is a process global that may change.
     *
     * However, we can't simply limit the use of GetFullPathNameW to
     * non-absolute paths, because all occurrences of "." and "..",
     * need to be removed. This includes the occurrences that don't simply
     * prepend the path, in which case the CWD is not relevant and
...
int32_t omrfile_attr(struct OMRPortLibrary *portLibrary, const char *path) {
  ...
  unicodePath = file_get_unicode_path(portLibrary, path, unicodeBuffer,
                                      UNICODE_BUFFER_SIZE);
  if (NULL == unicodePath) {
    return -1;
  }
...
uintptr_t omrfile_findfirst(struct OMRPortLibrary *portLibrary,
  ...
  /* Convert the filename from UTF8 to Unicode */
  unicodePath = file_get_unicode_path(portLibrary, newPath, unicodeBuffer,
                                      UNICODE_BUFFER_SIZE);
  if (NULL == unicodePath) {
...
int32_t omrfile_move(struct OMRPortLibrary *portLibrary, const char *pathExist,
  ...
  unicodePathExist = file_get_unicode_path(
      portLibrary, pathExist, unicodeBufferExist, UNICODE_BUFFER_SIZE);
  if (NULL == unicodePathExist) {
    return -1;
  }
  unicodePathNew = file_get_unicode_path(portLibrary, pathNew, unicodeBufferNew,
                                         UNICODE_BUFFER_SIZE);
...
intptr_t omrfile_read(struct OMRPortLibrary *portLibrary, intptr_t fd,
   ...
   * read,
   * 	so the (DWORD)nbytes cast is functionally correct, though less than
   * ideal.
   * If the caller wants to read more than 2^32 bytes they will have to make
   * more than one call,
...
int32_t omrfile_sync(struct OMRPortLibrary *portLibrary, intptr_t fd) {
   ...
   * freeze while waiting for process to terminate while child
   ...
   * we never actually flushed STDIN and STDOUT handles but
...
void omrfile_vprintf(struct OMRPortLibrary *portLibrary, intptr_t fd,
  ...
  numberWritten = portLibrary->str_vprintf(
      portLibrary, outputBuffer, sizeof(outputBuffer), format, copyOfArgs);

  /* str_vprintf always null terminates, returns number characters written
  ...
  /* Either the buffer was too small, or it was the exact size.  Unfortunately
   * can't tell the difference,
   * need to determine the size of the buffer (another call to str_vprintf) then
   * print to the buffer,
  ...
  numberWritten = portLibrary->str_vprintf(portLibrary, NULL, (uint32_t)(-1),
                                           format, copyOfArgs);
  numberWritten += 1;

  allocatedBuffer = portLibrary->mem_allocate_memory(
      portLibrary, numberWritten, OMR_GET_CALLSITE(),
      OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  numberWritten = portLibrary->str_vprintf(portLibrary, allocatedBuffer,
                                           numberWritten, format, args);
