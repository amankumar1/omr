...
intptr_t omrsysinfo_get_username(struct OMRPortLibrary *portLibrary,
  ...
  stored into
  the buffer, including the terminating null character. If the buffer is not
  large enough,
...
static int32_t copyEnvToBuffer(struct OMRPortLibrary *portLibrary, void *args) {
  ...
  while ((0 == envVars[0] && 0 == envVars[1]) == FALSE) {
    ...
    if (spaceLeft >= (uintptr_t)spaceForThisEntry) {
      WideCharToMultiByte(OS_ENCODING_CODE_PAGE, 0, envVars, -1, cursor,
...
int32_t omrsysinfo_get_processor_info(struct OMRPortLibrary *portLibrary,
   ...
   * changes between our
   * allocating space and actually obtaining the processor data? We shall need
   * to rehash things here.
   */
  do {
    ...
    if (((int32_t)portLibrary->sysinfo_get_number_CPUs_by_type(
       ...
       * to forge ahead with collecting data. Collect current raw data value for
       * all counters in the
       * usage stats query.
       */
      status = PdhCollectQueryData(statsHandle);
     ...
     * collecting data. Must
     * free up the memory since this is insufficient for us to continue with,
  ...
  for (cpuCntr = 0; cpuCntr < procInfo->totalProcessorCount + 1; cpuCntr++) {
     ...
     * we need add the
     * Processor Times counter for retrieving the processor timestamp, obtained
     * below.
     */
    procInfo->procInfoArray[cpuCntr].busyTime =
        procInfo->procInfoArray[cpuCntr].systemTime +
   ...
   * the aggregates,
   * but averages over all processors. So we need to multiply with processor
   * counts as port
   * library clients are interested in aggregates and also, to be consistent
...
intptr_t omrsysinfo_get_cwd(struct OMRPortLibrary *portLibrary, char *buf,
  ...
  wchar_t *unicodeBuffer = NULL; /* unicode version of CWD, size:
                                    pathLengthInWChars*sizeof(wchar_t) plus 1
                                    for terminator and 1 for terminating slash
                                    if required */
...
intptr_t omrsysinfo_get_tmp(struct OMRPortLibrary *portLibrary, char *buf,
  ...
  mutf8Size = portLibrary->str_convert(
      portLibrary, J9STR_CODE_WIDE, J9STR_CODE_MUTF8, (uint8_t *)unicodeBuffer,
