 ...
 * (If you put more than 999 chars into the buffer before hitting enter, you
 * lock up standard input.)
 */
      ...
      if (PeekConsoleInput(PPG_tty_consoleInputHd,
        ...
        for (x = 0; x < (int)available; x++) {
          if ((KEY_EVENT == inputRecordPointer[x].EventType) &&
            ...
            bytesCanBeRead = TRUE; /* report true if we find a return char since
                                      we know that it is safe to read at least
                                      that one byte */
            break;
          }
        }
