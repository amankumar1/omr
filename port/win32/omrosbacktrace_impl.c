 ...
 * already loaded. Once loaded,
 * we keep the DLL and function table for the life of the JVM.
...
int32_t load_dbg_functions(struct OMRPortLibrary *portLibrary) {
  ...
  if (NULL == PPG_dbgHlpLibraryFunctions->hDbgHelpLib) {
    ...
    if (NULL == PPG_dbgHlpLibraryFunctions->hDbgHelpLib) {
      PPG_dbgHlpLibraryFunctions->hDbgHelpLib =
          (HINSTANCE)j9getdbghelp_loadDLL();
    }
 ...
 * symbol handler, if not already
 * initialised. We reset the symbol options and free loaded symbols on
 * completion of the native stack walk
 * for javacores. PPG_pdbData.numberOfModules indicates whether we have
 * currently initialized symbols.
 *
 * Note - this is called single threaded, e.g. under PPG_osBacktrace_Mutex
 *
 * @param portLibrary - a pointer to an initialized port library
 ...
 * PPG_pdbData.numberOfModules
 * set to 0 indicates that we have unloaded the debug symbols and reset the
 * options.
 *
 ...
 * only one or two
 * values in the context that are actually used to construct the stack but these
 * vary by platform
 * so arn't detailed here. If no heap is specified then this function will use
 * malloc to allocate
 * the memory necessary for the stack frame structures which must be freed by
 * the caller.
 *
...
uintptr_t omrintrospect_backtrace_thread_raw(struct OMRPortLibrary *portLibrary,
  ...
  if (0 == load_dbg_functions(portLibrary) &&
    ...
    for (frameNumber = 0; rc != FALSE; frameNumber++) {
       ...
       * win32 is exclusively little endian so this
       * works as expected
       */
 ...
 * and looks up the symbols for the frames. The format of the string generated
 * is:
 * 		symbol_name (statement_id instruction_pointer [module+offset])
 * If it isn't possible to determine any of the items in the string then they
 * are omitted. If no heap is specified
 * then this function will use malloc to allocate the memory necessary for the
