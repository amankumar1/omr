...
int32_t omrvmem_free_memory(struct OMRPortLibrary *portLibrary, void *address,
  ...
  if (ret != 0) {
    ...
    if (NULL != portLibrary->portGlobals->platformGlobals.bindingPool) {
       ...
       * failure in allocating the first node in the range.
...
void *omrvmem_reserve_memory_ex(struct OMRPortLibrary *portLibrary,
  ...
  if (NULL != memoryPointer) {
     ...
     * committing something which might only include part of a
     * bound range so create the "unbound range" entry so that we can
     * immediately find the boundaries of the extent
     */
 ...
 * that were created by @ref omrvmem_startup
 * should be destroyed here. This function replaces the stub version during
 * omrvmem_startup. This is done so that omrvmem
 ...
 * the virtual memory operations may be created here.  All resources created
...
int32_t omrvmem_startup(struct OMRPortLibrary *portLibrary) {
   ...
   * calls
   * are made to omrvmem operations. Specifically, to provide synchronization
   * when modifying the
  ...
  if (0 ==
      portLibrary->sl_open_shared_library(portLibrary, "Kernel32", &handle,
                                          TRUE)) {
    if (0 ==
        portLibrary->sl_lookup_name(portLibrary, handle, "GetLargePageMinimum",
 ...
 * default page size, other entries
...
void omrvmem_default_large_page_size_ex(struct OMRPortLibrary *portLibrary,
   ...
   * supported
   * (platforms with more complicated logic will use their own special process
...
static void *getMemoryInRange(struct OMRPortLibrary *portLibrary,
   ...
   * Large allocations with large pages can be slow on certain Windows versions,
   * see CMVC 194435.
   * Test using the caller's setting for MEM_LARGE_PAGES, because the allocation
   * may be feasible with small pages
  ...
  while ((startAddress <= currentAddress) && (endAddress >= currentAddress)) {
    ...
    if (NULL != memoryPointer) {
      if ((startAddress <= memoryPointer) && (endAddress >= memoryPointer)) {
        ...
        VirtualFree((LPVOID)memoryPointer, (size_t)0, MEM_RELEASE);
        memoryPointer = NULL; /* This is necessary, cause below
                                 OMRPORT_VMEM_STRICT_ADDRESS if statement might
...
intptr_t omrvmem_numa_set_affinity(struct OMRPortLibrary *portLibrary,
  ...
  if (0 == returnValue) {
    /* now, walk all the extents in the range [address, address+byteAmount) and
     * delete them */
...
intptr_t omrvmem_numa_get_node_details(struct OMRPortLibrary *portLibrary,
  ...
  if (NULL != kernel32Module) {
     ...
     * enabled" so that all systems
     * have NUMA properties but some have only 1 node.  This way, the special
     * case is removed and the GC can decide how to handle that environment.
     */
...
static void *commitAndTouch(struct OMRPortLibrary *portLibrary,
  ...
  while ((current != 0) && (current < top)) {
     ...
     * important to only commit pages which
     * haven't yet been committed. If we were to recommit a page
     * touchOnNumaNode() would overwrite its first
 ...
 * such a way that the pages should be allocated on the
 * given j9NumaNode.
 * @param portLibrary[in] The Port Library instance
...
static int32_t getProcessPrivateMemorySize(struct OMRPortLibrary *portLibrary,
  ...
  if (MANAGEMENT_COUNTER_PATH_UNINITIALIZED == isCounterPathInitialized) {
     ...
     * This is done
     * for performance reason as normal API invocation needs two passes, first
     * to acquire the space needed, then
     * second pass to actually fetch the data. Per MSDN documentation,
     * pathElement contains pointers to each
     * individual string path elements of the structure. The actual strings are
     * appended to the end of structure.
     * Hence, the total memory required cannot exceed the size of the structure
     * plus the size of maximum counter
     * path allowed.
     */
     ...
     * cannot be pre-allocated as there
    ...
    while ('\0' != *str) {
      ...
      if (value.longValue == pid) {
         ...
         * obtain the necessary buffer size.
         * However, to avoid overhead of invoking the function twice, we pass a
         * pre-allocated max size buffer.
         */
        if (ERROR_SUCCESS !=
