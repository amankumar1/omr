...
uintptr_t omrsl_open_shared_library(struct OMRPortLibrary *portLibrary,
  ...
  if (!openExec) {
    ...
    if (NULL == dllHandle) {
      BOOLEAN useNlsMessage = J9_ARE_ALL_BITS_SET(
       ...
       * retrieving system based
       * error messages can be costly.  If not, use the NLS message instead.
       * Also, if system
   ...
   * generated by loadLibrary() on Windows
   * if an invalid quoted path is detected in the library path list
   * (-Djava.library.path=). In such case, VM fails
   * to process the remaining paths and exits immediately. To fix up the issue,
   * ERROR_BAD_PATHNAME should be covered
  ...
  if (notFound) {
     ...
     * at all. */
    dllHandle =
        LoadLibraryExW(unicodeName, NULL, DONT_RESOLVE_DLL_REFERENCES |
