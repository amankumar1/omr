 ...
 * and some
 * distros incorrectly assume that errno is an int, in their header.  Including
 * it here will
...
uintptr_t omrthread_get_ras_tid(void) {
   ...
   * scalar value, into a uintptr_t
   * by getting its address, casting that to a uintptr_t pointer and then
