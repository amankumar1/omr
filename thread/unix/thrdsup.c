 ...
 */
clockid_t timeoutClock =
    CLOCK_REALTIME; /**< the clock used to derive absolute time from relative
                       wait time */
pthread_condattr_t *defaultCondAttr = NULL;  /**< attribute passed to
                                                pthread_cond_init(). NULL means
                                                the system default. */
 ...
 */
static intptr_t zos_init_yielding(void) {
/* Suspend conversion of strings to ascii, and ensure we use the OS's setenv and
 * getenv functions */
