...
intptr_t omrthread_attr_init(omrthread_attr_t *attr) {
   ...
   * its multiplexed threads which causes
   * a running thread to never give up the CPU once it has started executing. By
   * setting the PTHREAD_SCOPE_SYSTEM flag,
   * we are telling the system to use the kernel's time-slicing scheduling
   * algorithm to choose how this thread will be
   * scheduled. Effectively the kernel, not the threads library will decide when
   * this thread runs by scheduling it
 ...
 * policy.
 * It doesn't set omrthread_attr_t.schedpolicy, but the schedpolicy value is
 * ignored.
 *
...
static intptr_t setSchedpolicy(pthread_attr_t *pattr,
  ...
  if (J9THREAD_SCHEDPOLICY_INHERIT == policy) {
     ...
     * is enabled, but it appears that leaving one of the privileged
     * schedpolicies
...
static intptr_t setPriority(pthread_attr_t *pattr, omrthread_prio_t priority) {
   ...
   * PTHREAD_INHERIT_SCHED in setSchedpolicy,
   * however there is no harm in setting the priority here and checking the
   * value is valid.
   */
