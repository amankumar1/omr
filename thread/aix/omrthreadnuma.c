 ...
 * preferentially) on
 * processors associated with the specified NUMA nodes contained in the
 * numaNodes array (of nodeCount length).
 ...
 * yet started,
 * setting the affinity will be deferred until the thread is started. This
 * function will still
 * return 0, indicating success, however there is no way to tell if the affinity
 * was successfully
 ...
 * this function will return 0 even if
 * NUMA is not available. Use omrthread_numa_get_max_node() to test for the
 * availability of NUMA.
 */
...
static intptr_t bindThreadToNode(omrthread_t thread, uintptr_t nodeNumber) {
  ...
  if ((NULL != PTR_ra_attach) && (NULL != PTR_rs_get_homesrad)) {
     ...
     * a non-strict call to PTR_ra_attach
