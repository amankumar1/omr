...
#define NOTIFY_WRAPPER(thread)                                                 \
  do {                                                                         \
    if (J9THREAD_LIB_NOTIFY_POLICY_SIGNAL ==                                   \
        (thread)->library->notifyPolicy) {                                     \
...
uintptr_t omrthread_lib_use_realtime_scheduling(void) {
#if defined(LINUX) || defined(OSX)
  omrthread_library_t lib = GLOBAL_DATA(default_library);
...
static intptr_t init_spinParameters(omrthread_library_t lib) {
  ...
  if (init_threadParam("secondarySpinForObjectMonitors",
                       &lib->secondarySpinForObjectMonitors)) {
    return -1;
  }
 ...
 * survivor thread,
 * fix the tid and handle and recreate the mutex and cond. Otherwise, free
 * J9Thread.tracing,
 * flush the destroyed monitor lists back into the pool, and tls_finalize() the
 * tls.
 *
 * Note that the thread mutex and cond are reallocated and initialized without
 * freeing the
 * old ones, since if it allocates the same piece of memory, it fails with mutex
 * already
 * initialized.
 *
 * @pre There is a pre-condition that the monitors have been reset first with
 * postForkResetMonitors
 * in order for omrthread_tls_finalize to use mutexes. A user-provided
 * tls_finalize callback
 * may fail post-fork if it uses mutexes/cond vars the thread library does not
 * know about.
 ...
 * allocated,
 * clear the waiting, blocking, and notifyAll thread list (with an assert if its
 * the
 * fork survivor), set the owner to null if it is not the fork survivor, and
 * reset
 ...
 * specified name.<br>
 * A new named global is created if a named global with the specified name can't
 * be found.
 *
...
intptr_t omrthread_attach_ex(omrthread_t *handle, omrthread_attr_t *attr) {
   ...
   * passed in, then set to
   * J9THREAD_CATEGORY_SYSTEM_THREAD. Since a number of OMR APIs call
   * omrthread_attach, having
   * J9THREAD_CATEGORY_SYSTEM_THREAD as the default, means that these APIs can
   * call with a NULL attribute
...
static void storeExitCpuUsage(omrthread_t thread) {
  ...
  if (J9_ARE_ALL_BITS_SET(lib->flags, J9THREAD_LIB_FLAG_ENABLE_CPU_MONITOR)) {
    ...
    if (threadCpuTime > 0) {
      ...
      if (J9_ARE_ALL_BITS_SET(thread->effective_category,
                              J9THREAD_CATEGORY_RESOURCE_MONITOR_THREAD)) {
        cumulativeInfo->resourceMonitorCpuTime += threadCpuTime;
      } else if (J9_ARE_ALL_BITS_SET(thread->effective_category,
...
/* Set the numa affinity if it is pending on this thread */
#if defined(OMR_PORT_NUMA_SUPPORT)
  {
    ...
    /* If the affinityCount is zero, this call will have the effect of clearing
     * any affinity that the thread may have inherited
     * and it assuming the initial affinity that the process had (if reverting
     * to that affinity isn't possible on the platform,
 ...
 * terminates.
 * This is useful if the thread wishes to signal its termination to a watcher,
 * since
 ...
 * @param[in] globalAlreadyLocked indicated whether the thread library global
 * mutex is already locked
 * @return 0 on success or negative value on failure.
 *
 * @see threadCreate
...
static intptr_t threadDestroy(omrthread_t thread, int globalAlreadyLocked) {
   ...
   * threadFree(), since this leads to
   * seg faults in the JVM. See PR 82332: [zOS S390] 80
   * VM_Extended.TestJvmCpuMonitorMXBeanLocal : crash
   */
...
intptr_t omrthread_sleep_interruptable(int64_t millis, intptr_t nanos) {
  ...
  J9OSCOND_WAIT_IF_TIMEDOUT(self->condition, self->mutex, boundedMillis,
                            nanos) {
    break;
  }
  else {
...
intptr_t omrthread_monitor_destroy(omrthread_monitor_t monitor) {
  ...
  if (monitor->owner || monitor_maximum_wait_number(monitor)) {
     ...
     * cancelled.
     * There's actually a very small timing hole here -- if the thread had just
     * locked the
     * mutex and not yet set the owner field when it was cancelled, we have no
     * way of
     * knowing that the mutex may be in an invalid state. The same thing can
     * happen
     * if the thread has just cleared the field and is about to unlock the
     * mutex.
 ...
 * monitor onto a thread
 * local list.  omrthread_monitor_flush_destroyed_monitor_list() MUST be called
 * to flush the thread
 * local list to the global list.  The JLM data for this monitor is not
 * destroyed.
 *
 ...
 * the GLOBAL_LOCK.
 * Monitors can only be added to the thread local list via
...
static intptr_t monitor_wait_original(omrthread_t self,
  ...
  if (millis || nanos) {
    ...
    J9OSCOND_WAIT_IF_TIMEDOUT(MONITOR_WAIT_CONDITION(self, monitor),
...
/* DONE WAITING AT THIS POINT */
  ...
  if (monitor_enter_three_tier(
          self, monitor, (BOOLEAN)((interruptible & J9THREAD_FLAG_ABORTABLE)
                                       ? SET_ABORTABLE
                                       : DONT_SET_ABORTABLE)) ==
      J9THREAD_INTERRUPTED_MONITOR_ENTER) {
...
static intptr_t monitor_wait_three_tier(omrthread_t self,
  ...
  if (millis || nanos) {
    ...
    J9OSCOND_WAIT_IF_TIMEDOUT(self->condition, monitor->mutex, boundedMillis,
                              nanos) {
      THREAD_LOCK(self, CALLER_MONITOR_WAIT2);
      intrFlags = self->flags & intrMask;
      interrupted = J9THR_WAIT_INTERRUPTED(intrFlags);
...
intptr_t omrthread_set_category(omrthread_t thread, uintptr_t category,
   ...
   * done.
   * Check if we are in the middle of a GC cycle, if so, no accounting updates
   * will be done */
  if (J9_ARE_ALL_BITS_SET(lib->flags, J9THREAD_LIB_FLAG_ENABLE_CPU_MONITOR) &&
