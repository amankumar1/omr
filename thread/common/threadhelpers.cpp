...
/**
 * Spin on a monitor's lockingWord field until we can atomically swap out a
 * value of SPINLOCK_UNOWNED
...
intptr_t omrthread_spinlock_acquire(omrthread_t self,
  ...
  J9ThreadMonitorTracing *tracing =
      (self->library->flags & J9THREAD_LIB_FLAG_JLM_ENABLED) ? monitor->tracing
                                                             : NULL;
#endif /* OMR_THR_JLM */
...
#if defined(OMR_THR_JLM)
        if (NULL != tracing) {
          /* Update JLM spin counts after partial set of spins - add JLM counts
           * atomically.
          ...
          VM_AtomicSupport::add(&tracing->spin2_count,
                                ((m - j) * n) + n - i + 1);
        }
#endif /* OMR_THR_JLM */
...
#if defined(OMR_THR_JLM)
  if (NULL != tracing) {
    /*update JLM spin counts after complete set of spins - add JLM counts
     * atomically
     * let m=monitor _spinCount3, n=monitor _spinCount2
