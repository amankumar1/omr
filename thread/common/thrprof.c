 ...
 *J9THREAD_ERR if not supported
 *		   on this platform or some other platform specific error
 *occurred.
 ...
 * Windows
 * from the port library as we cannot call the port functions from the thread
 * library.
 *
...
#define J9TIME_NANOSECONDS_PER_SECOND J9CONST_U64(1000000000)
  ...
  return hiresTime;
#elif defined(                                                                 \
    WIN32)         /* defined(LINUX) && (defined(J9HAMMER) || defined(J9X86)) */
  LARGE_INTEGER i;
 ...
 * Also add the usage details of threads that have already exited into the
 * appropriate
 * categories.
...
intptr_t omrthread_get_jvm_cpu_usage_info(J9ThreadsCpuUsage *cpuUsage) {
  ...
  for (; NULL != walkThread; walkThread = pool_nextDo(&state)) {
     ...
     * initialized.
     * So we check if J9THREAD_FLAG_CPU_SAMPLING_ENABLED is set without holding
     * the lock first.
     ...
     * implying the thread isn't available to
     * the OS; skip accounting for such threads, as storeExitCpuUsage() must
     * have already done this for it. Return
    ...
    if (J9_ARE_ALL_BITS_SET(walkThread->effective_category,
                            J9THREAD_CATEGORY_RESOURCE_MONITOR_THREAD)) {
      resourceMonitorCpuTime += threadCpuTime;
    } else if (J9_ARE_ALL_BITS_SET(walkThread->effective_category,
  ...
  for (i = 0; i < J9THREAD_MAX_USER_DEFINED_THREAD_CATEGORIES; i++) {
     ...
     * copy and ends up crashing for
     * some unknown reason. The if check introduces variability in the loop
     * causing gcc not to use xmm.
     */
