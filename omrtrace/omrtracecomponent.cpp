...
omr_error_t initialiseComponentData(UtComponentData **componentDataPtr,
   ...
   * counters the modules may have been
   * freed already. */
...
omr_error_t initialiseComponentList(UtComponentList **componentListPtr) {
  ...
  *componentListPtr = componentList;
  UT_DBGOUT(2,
            ("<UT> initialiseComponentList: %p completed\n", componentListPtr));
  return OMR_ERROR_NONE;
}
  ...
  if (componentList->deferredConfigInfoHead != NULL) {
    ...
    while (configInfo != NULL) {
      ...
      if (configAppliesToAll ||
         ...
         * groups or other inappropriate settings.
...
omr_error_t removeModuleFromList(UtModuleInfo *module,
  ...
  /* for flight controller functinality, any unloaded components should be moved
     onto an unloaded list, so their
  ...
  if (compData == NULL) {
    ...
    while (compData != NULL) { /* configuring live and registered module */
      ...
      if (MODULE_IS_AUXILIARY(modInfo)) {
        reportCommandLineError(suppressMessages, "Component %s is marked "
                                                 "auxiliary and cannot be "
                                                 "configured directly.",
                               componentName);
        return OMR_ERROR_INTERNAL;
      }
    ...
    if (*temp == '-') {
      ...
      if (!isdigit(*temp)) {
        reportCommandLineError(suppressMessages,
                               ...
                               "tpnid{componentName.offset1-offset2} e.g. "
                               "tpnid{j9trc.2-6}");
        return OMR_ERROR_ILLEGAL_ARGUMENT;
      }
      while (isdigit(*temp)) {
