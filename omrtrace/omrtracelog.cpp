      ...
      if (*trcBuf != NULL) {
        ...
        *p = (char *)&(*trcBuf)->record + (*trcBuf)->record.nextEntry;
        ...
        if ((*trcBuf)->record.nextEntry != (*trcBuf)->record.firstEntry) {
           ...
           * but not with the data we are currently copying so we need to set
           * this here as a precaution.
           * nextEntry is always reconstructed at the end of the trace point
           * from the cursor so this is
...
static void traceV(OMR_TraceThread *thr, UtModuleInfo *modInfo,
  ...
  if (J9_ARE_ANY_BITS_SET(thr->currentOutputMask, UT_MAXIMAL | UT_EXCEPTION) &&
      (str != NULL)) {
    int i;
...
void doTracePoint(OMR_TraceThread *thr, UtModuleInfo *modInfo, uint32_t traceId,
  ...
  if (isRegular) {
     ...
     * went. Regular tracepoints
     * need to record where they are about to go in case they generate auxiliary
     * tracepoints.
     */
     ...
     * jstacktrace data to
     * minimal (i.e. throwing away all the stack data) makes no sense - so it is
     * converted to
