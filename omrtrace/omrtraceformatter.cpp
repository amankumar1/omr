...
omr_trc_getTraceFileIterator(OMRPortLibrary *portLib, char *fileName,
  ...
  if (dummyHeader.endianSignature != UT_ENDIAN_SIGNATURE) {
     ...
     * header
     * and change the formatter to cope with files from other platforms OR
     * document
 ...
 * global trace lock until
 * trcFreeTracePointIterator is called as we assume we are working on data from
 * a file that trace is
...
omr_error_t omr_trc_getTracePointIteratorForNextBuffer(
  ...
  iterator->endPlatform = omrtime_hires_clock(); /* TODO - Is there a better
...
static const char *parseTracePoint(OMRPortLibrary *portLib,
  ...
  if (traceId == 0x0010) {
    /* 	too much danger of looping to recurse here - return a warning string
     and let
     omr_trc_formatNextTracePoint(thr, iter) make the call on whether it can
     extract
     any further data */
  ...
  if (!strncmp(modName, "INTERNALTRACECOMPONENT", 22)) {
     ...
     * getFormatString
     * to be able to do look up on it - save the overwritten data and reinstate
     * once finished. Doing
     * this avoids allocing a temp buffer. We also need to check to see if this
     * is a composite name,
     * e.g. pool(j9mm). If it is then we need to take the string before the
     * opening brace as the
     * component name. We don't do this in getFormatString because that's in the
     * fastpath for
     * -Xtrace:print
     */
...
const char *omr_trc_formatNextTracePoint(UtTracePointIterator *iter,
   ...
   * originally
   * started in this buffer, then we have found the partially overwritten
   * tracepoint
   ...
   * exists.
   * 'offset - tpLength + 1' points to the start of the current tracepoint's
   * data, which
   * must be at least 'record->firstEntry' or 'iter->start' if the tracepoint
   * fits in this buffer.
   */
