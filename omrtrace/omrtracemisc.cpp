  ...
  for (; originalCursor < originalLen; originalCursor++) {
    if (original[originalCursor] == '%') {
      ...
      if (original[originalCursor] == 'p') {
        ...
        reportCommandLineError(atRuntime, "Invalid special character '%%%c' in "
                                          "a trace filename. Only %%p, %%d and "
                                          "%%t are allowed.",
...
#define TEMPBUFLEN 150
  ...
  if ((f = omrfile_open("utTrcCounters", EsOpenText | EsOpenWrite |
                                             EsOpenTruncate | EsOpenCreateNoTag,
                        0)) < 0) {
    if ((f = omrfile_open("utTrcCounters", EsOpenText | EsOpenWrite |
                                               EsOpenCreate | EsOpenCreateNoTag,
                          0666)) < 0) {
...
omr_error_t addTraceConfig(OMR_TraceThread *thr, const char *cmd) {
  ...
  if (NULL != cfg) {
     ...
     * Unfortunately it may be in use by a snap dump or we may have returned a
     * pointer to
     * it via trcGetTraceMetadata so freeing it or nulling it is unsafe.
     * For now we are leaving it unchanged.
     * If nothing has requested the trace header yet and it is NULL then it will
     * be created
